...
// 2. schema
const sp = ROOT + '/ai-platform/server/lib/hsm-adapter/crypto-policy-schema.json';
...
} else { console.log('2. schema: already has pqSeabedGating'); }
...
if (!b.includes('emitSeabedGatingPoolInitialized')) {
  ...
  if (bidx === -1) { console.log('3. base-adapter: marker not found'); } else {
    const insertB = [
...
} else { console.log('3. base-adapter: already has emitSeabedGatingPoolInitialized'); }
...
if (!m.includes('hsm_seabed_gating_pool_initialized_total')) {
  ...
  if (mIdx === -1) { console.log('4. hsm-metrics: marker not found'); } else {
...
} else { console.log('4. hsm-metrics: already has seabed counters'); }

// 5. run-all-tracks
const rp = ROOT + '/ai-platform/server/lib/hsm-adapter/__tests__/run-all-tracks.cjs';
...
if (!r.includes('pq-deep-sea-mineral-rights-gating')) {
  ...
  if (rIdx === -1) { console.log('5. run-all-tracks: marker not found'); } else {
...
} else { console.log('5. run-all-tracks: already has pq-deep-sea'); }
