...
} = require("./packages/simplebeacon-cli/src/lib/normalize-scan-report.js");
const {
  compileGateStatus,
} = require("./packages/simplebeacon-cli/src/scan.js");

const API_URL = process.env.SIMPLEBEACON_API_URL || "http://localhost:3456";
const TEST_SECRET =
  process.env.SIMPLEBEACON_LICENSE_SECRET || "test-secret-for-tier-check";
const REPORT_PATH =
  process.env.SIMPLEBEACON_VERIFY_REPORT ||
  "J:/Downloads/simplebeacon-report-2026-07-14.json";
...
async function startLocalServer() {
  ...
  childServer = spawn("node", ["api-server/server.cjs"], {
    ...
    env: {
      ...
      SIMPLEBEACON_JWT_SECRET:
        "dummy-jwt-secret-for-verify-only-not-used-in-production-123456", // simplebeacon-ignore credential-pattern — deployment verification fixture, not production
...
function loadMinimalReport() {
  if (!fs.existsSync(REPORT_PATH)) {
    console.warn(
      `[Verify] Report not found at ${REPORT_PATH}; building a synthetic one.`,
    );
    return {
  ...
  const normalized = normalizePlatformScanReport(raw, {
    gateConfig: { failOn: ["high"] },
  });
...
async function main() {
  ...
  try {
    ...
    const sandbox = await request("POST", "/api/license/validate", {
      token: "",
    });
    allOk =
      assert(
        "License validate without token returns sandbox",
        sandbox.status === 200 &&
          sandbox.body?.sandbox === true &&
          sandbox.body?.active === false,
        JSON.stringify(sandbox.body),
      ) && allOk;

    // simplebeacon-ignore sensitive-data — test script with mock email and test secret
    const token = generateLicenseToken(
      { email: "verify@example.com", tier: "pro" },
      TEST_SECRET,
      60,
    );
    ...
    allOk =
      assert(
        "License validate with valid token returns active",
        active.status === 200 &&
          active.body?.active === true &&
          active.body?.tier === "pro",
    ...
    allOk =
      assert(
        "Minimal report has a failed gate",
    ...
    allOk =
      assert(
        "Send-to-AI accepts re-normalized report",
        sendToAi.status === 200 &&
          sendToAi.body?.success === true &&
          typeof sendToAi.body?.filePath === "string",
        JSON.stringify(sendToAi.body),
      ) && allOk;
  } catch (err) {
