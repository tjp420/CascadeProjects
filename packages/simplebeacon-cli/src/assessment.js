...
function bucketIssues(rawIssues = []) {
  ...
  for (const issue of rawIssues) {
    ...
    else if (/fiction|consistency|kpi|jest count/i.test(type))
      buckets.fictionKpis.push(entry);
...
function buildHeadline(report, gateResult, buckets, headlineOptions = {}) {
  ...
  const medium =
    report.gate?.warningCount ?? report.severityCounts?.medium ?? 0;
  const prodLeaks =
    report.productionLeakFindings ?? buckets.productionLeaks.length;
  const euAiCount = report.euAiActFindings ?? buckets.euAiAct.length;
  const parts = [];
  if (euAiCount) parts.push(`${euAiCount} EU AI Act signal(s)`);
  if (buckets.fictionKpis.length)
    parts.push(`${buckets.fictionKpis.length} fiction/KPI issue(s)`);
  if (prodLeaks) parts.push(`${prodLeaks} production-path leak(s)`);
  if (buckets.credentials.length)
    parts.push(`${buckets.credentials.length} credential pattern(s)`);
  if (buckets.schemaDrift.length)
    parts.push(`${buckets.schemaDrift.length} schema issue(s)`);
  ...
  const assessment = {
    ...
    findings: {
      credentials: {
        ...
        summary: summarizeBucket(
          buckets.credentials,
          "No credential patterns detected in scanned paths.",
        ),
      },
      fictionKpis: {
        ...
        summary: summarizeBucket(
          buckets.fictionKpis,
          "No fiction KPI or consistency drift detected.",
        ),
      },
      productionLeaks: {
      ...
      schemaDrift: {
        ...
        summary: summarizeBucket(
          buckets.schemaDrift,
          "All registered page samples match schema specs.",
        ),
      },
      ...(isEuAiAct
        ? {
    ...
    recommendedActions: {
      immediate: [
        gateResult?.pass
          ? null
          : "Fix high-severity findings before enabling --gate on main",
        isEuAiAct && buckets.euAiAct.length
          ? "Address EU AI Act transparency and documentation gaps before August 2026"
          : null,
        buckets.productionLeaks.length
          ? "Remove hardcoded -sample.json paths from production code"
          : null,
        buckets.fictionKpis.length
          ? "Replace fiction KPIs with repository-audit baseline values"
          : null,
        isEuAiAct
          ? "Add docs/model-card.md and docs/risk-assessment.md for AI integrations"
          : null,
    ...
    complementaryStack: {
      keepUsing: isEuAiAct
        ? [
            "Legal counsel for formal conformity assessment",
            "Snyk or GitHub Advanced Security for CVEs",
          ]
        : [
            "Snyk or GitHub Advanced Security for CVEs",
      ...
      addSimplebeaconFor: isEuAiAct
        ...
        : [
            "Mock/fiction drift in sample JSON",
            "Hardcoded sample paths in production directories",
          ],
    },
    pilotProposal: {
      offer: isEuAiAct
    ...
    ...(isEuAiAct
      ? {
          disclaimer:
            "Static technical pattern review — not legal advice or formal conformity assessment under Regulation (EU) 2024/1689.",
        }
      : {}),
    ...
    sourceReport: {
      ...
      ...(isStale
        ? {
            staleReportWarning: `Source scan report is older than 7 days (generated ${generatedAt}). Re-run scan for current results.`,
          }
        : {}),
    },
  };
