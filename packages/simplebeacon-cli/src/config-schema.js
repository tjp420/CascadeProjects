...
function validateConfig(config) {
  ...
  if (config.profile && !VALID_PROFILES.has(config.profile)) {
    warnings.push(
      `Unknown profile "${config.profile}" — use minimal, standard, or cascade`,
    );
  }
  ...
  if (config.scanPaths != null && !Array.isArray(config.scanPaths)) {
    ...
    for (const scanPath of config.scanPaths) {
      ...
      if (/^(?:[A-Za-z]:[\\/]|\/)/.test(scanPath) || scanPath.includes("..")) {
        errors.push(
          `scanPaths entry must be repository-relative and non-traversing: "${scanPath}"`,
        );
      }
    }
  }
  ...
  if (
    config.productionPaths != null &&
    !Array.isArray(config.productionPaths)
  ) {
    ...
    for (const productionPath of config.productionPaths) {
      ...
      if (
        /^(?:[A-Za-z]:[\\/]|\/)/.test(productionPath) ||
        productionPath.includes("..")
      ) {
        errors.push(
          `productionPaths entry must be repository-relative and non-traversing: "${productionPath}"`,
        );
      }
    }
  }
    ...
    for (const [name, scanner] of Object.entries(config.scanners)) {
      if (scanner && typeof scanner === "object") {
        if (scanner.action && !VALID_SCANNER_ACTIONS.has(scanner.action)) {
          warnings.push(
            `Scanner "${name}" has invalid action "${scanner.action}" — use BLOCK, WARN, or SKIP`,
          );
        }
        if (scanner.severity && !VALID_SEVERITIES.has(scanner.severity)) {
          warnings.push(
            `Scanner "${name}" has invalid severity "${scanner.severity}"`,
          );
        }
      }
    }
