...
function createReportHandlers({
  ...
  return {
    ...
    suggest_fixes: withGuard(async ({ projectRoot, reportPath, maxFixes }) => {
      ...
      const issues = (report.detectedIssues || report.rawIssues || []).filter(
        (i) => i.severity === "critical" || i.severity === "high",
      );
      const suggestions = issues
        .slice(0, maxFixes ? Number(maxFixes) : 5)
        .map((issue, idx) => ({
          priority: idx + 1,
    ...
    export_report: withGuard(async (args) => {
      ...
      const outPath = args.outPath
        ? path.resolve(root, args.outPath)
        : path.join(root, ".simplebeacon", "exported-report.json");
      try {
