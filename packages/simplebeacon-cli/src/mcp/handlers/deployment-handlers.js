...
function createDeploymentHandlers({ withGuard, resolveProjectRoot, formatToolResult }) {
    return {
        scan_deployment_readiness: withGuard(async (args) => {
            ...
            try {
                ...
                const blocking = result.issues.filter(i => i.severity === 'high' || i.severity === 'critical');
