  ...
  return {
    ...
    run_analyzer_suite: withGuard(async ({ projectRoot, selectedIssueIds }) => {
      ...
      const {
        getCachedAnalysis,
        setCachedAnalysis,
      } = require("../../lib/ai-problem-analyzer-cache");
      const {
        sanitizeAiProblemAnalyzerExport,
      } = require("../../lib/ai-problem-analyzer-export-sanitize");
      ...
      try {
        ...
        return formatToolResult({
          error:
            "No scan report found. Run `npx simplebeacon scan --format json --output .simplebeacon/report.json` first.",
        });
      }
      ...
      if (cached) {
        ...
        const ids =
          Array.isArray(selectedIssueIds) && selectedIssueIds.length
            ? selectedIssueIds
            : Array.from(
                { length: 48 },
                (_, i) => `A-${String(i + 1).padStart(2, "0")}`,
              );
        analysisResult = buildAiSystemsIssueAnalysis(ids, {
          context: { scanReport: report },
        });
        setCachedAnalysis(root, report, analysisResult);
      }
      const payload = sanitizeAiProblemAnalyzerExport(analysisResult, {
        projectPath: root,
      ...
      return formatToolResult({
        ...
        insufficientData:
          analysisResult.riskSummary.executionStatus.insufficientData,
        stub: analysisResult.riskSummary.executionStatus.stub,
        overallRiskLevel: analysisResult.riskSummary.overallRiskLevel,
        peakSeverity:
          analysisResult.riskSummary.severityCounts.critical > 0
            ? "critical"
            : analysisResult.riskSummary.severityCounts.high > 0
              ? "high"
              : "low",
        topPriorityIssues:
          payload.topPriorityIssues?.map((i) => ({
            id: i.id,
            title: i.title,
            severity: i.severity,
        ...
        coverageGaps:
          payload.coverageGaps?.map((g) => ({
            id: g.id,
            title: g.title,
            missingInputPointer: g.missingInputPointer,
          })) || [],
        methodology:
          "Deterministic local analyzer suite — no code uploaded to remote servers",
      });
    }),

    generate_marketing: withGuard(async (args) => {
      const root = resolveProjectRoot(args.projectRoot);
      const rp = args.reportPath
        ? path.resolve(root, args.reportPath)
      ...
      try {
        ...
        return formatToolResult({
          error: "No scan report found. Run scan_project first.",
        });
      }
      const {
        generateMarketingContent,
      } = require("../../lib/marketing/marketing-content-generator");
      ...
      if (!validChannels.includes(channel)) {
        return formatToolResult({
          error: `Invalid channel: ${channel}. Valid: ${validChannels.join(", ")}`,
        });
      }
      try {
        ...
        return formatToolResult({
          error: err.message || "Marketing generation failed",
        });
      }
    }),
    ...
    list_rulesets: withGuard(() => {
      const rulesets = {
        ...
        categories: [
          ...
          {
            id: "syntax",
            label: "Syntax / Structural",
            severityDefault: "high",
          },
          {
            id: "security",
            label: "Credential & Production Leak",
            severityDefault: "critical",
          },
          {
            id: "ai-quality",
            label: "AI-Generated Slop & Fiction KPI",
            severityDefault: "medium",
          },
          {
            id: "compliance",
            label: "Regulatory (EU AI Act)",
            severityDefault: "medium",
          },
        ...
        mcpInstructions: {
          purpose:
            "When writing code, avoid all banned patterns. Use runtime config or sample data fixtures instead of hardcoded metrics, credentials, or mock paths.",
          enforcement:
            "Local deterministic scan — no code uploaded to remote servers",
          privacyNote:
            "Rule queries are stateless; no source code leaves the developer machine",
        },
      };
