...
function createPdaHandlers({ withGuard, resolveProjectRoot, formatToolResult, formatMarkdownResult }) {
    ...
    return {
        ...
        agent_register: withGuard((args) => {
            ...
            const agent = pda.registerAgent(root, args.name || 'unknown', args.type || 'custom');
        ...
        agent_remember: withGuard((args) => {
            ...
            const memory = pda.remember(root, agentId, args.key, args.value, args.category || 'context', { ttlSeconds: args.ttlSeconds });
        ...
        agent_recall: withGuard((args) => {
            ...
            return formatToolResult({ success: true, memories, count: memories.length });
        }),
        ...
        task_update: withGuard((args) => {
            ...
            if (!task) return formatToolResult({ success: false, error: 'Task not found' });
            return formatToolResult({ success: true, task });
        }),
        ...
        task_complete: withGuard((args) => {
            ...
            if (!result) return formatToolResult({ success: false, error: 'Task not found' });
            if (result.error === 'parent_incomplete') {
                return formatToolResult({ success: false, error: result.error, message: result.message, parentTask: result.parentTask });
            }
        ...
        policy_list: withGuard((args) => {
            ...
            return formatToolResult({ success: true, policies, source, count: policies.length });
        }),
        ...
        handoff_read: withGuard((args) => {
            ...
            for (const agent of agents) {
                ...
                if (mem && (!latestHandoff || mem.updatedAt > latestHandoff.updatedAt)) {
                    latestHandoff = mem;
                }
            }

            if (!latestHandoff) {
                return formatToolResult({ success: true, handoff: null, message: 'No handoff brief found' });
            }

            try {
                const brief = JSON.parse(latestHandoff.value);
                return formatToolResult({ success: true, handoff: brief, memoryId: latestHandoff.id });
            } catch {
                return formatToolResult({ success: true, handoff: { raw: latestHandoff.value }, memoryId: latestHandoff.id });
            }
        }),
        ...
        cross_project_learn: withGuard((args) => {
            ...
            const searchRoots = (args.searchRoots && args.searchRoots.length > 0)
                ? args.searchRoots
                : defaultRoots;
            const maxDepth = args.maxDepth || 5;

            const projects = pda.collectProjectData(searchRoots, { maxDepth, includeWorktrees: false });
