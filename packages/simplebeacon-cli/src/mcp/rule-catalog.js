...
function explainFinding(patternId, options = {}) {
  ...
  if (leakPattern) {
    return {
      ...
      severity: "critical",
      summary:
        PRODUCTION_LEAK_SUMMARIES[id] || `Production leak pattern (${id})`,
      tuning:
        id === "sample-json" || id === "web-data-sample"
  ...
  if (id.startsWith(FICTION_RULE_PREFIX) || id.startsWith("source-fiction-")) {
    return {
      ...
      severity: "medium",
      summary:
        "Hardcoded metric matches a rejected fiction value in baseline.json",
      tuning:
        "Use measured KPIs from audit baseline or move metrics to sample JSON only",
      deterministic: true,
      usesLlm: false,
    };
  }
  ...
  if (options.type) {
    return {
      ...
      tuning:
        "See packages/simplebeacon-cli/src/rules/ and .simplebeacon/config.json allowlists",
      deterministic: true,
      usesLlm: false,
    };
  }
