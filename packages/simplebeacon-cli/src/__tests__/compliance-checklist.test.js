...
describe('compliance-checklist', () => {
    ...
    describe('evaluateRule', () => {
        ...
        it('evaluates zero-credential-findings as pass with zero findings', () => {
            const result = evaluateRule(
                { id: 'c1', title: 'Creds', category: 'security', severity: 'critical', check: 'zero-credential-findings' },
        ...
        it('evaluates npm-no-critical-high as skip when no npmAudit', () => {
            const result = evaluateRule(
                { id: 'n1', title: 'NPM', category: 'dependencies', severity: 'high', check: 'npm-no-critical-high' },
        ...
        it('evaluates npm-no-critical-high as pass with zero critical/high', () => {
            const result = evaluateRule(
                { id: 'n1', title: 'NPM', category: 'dependencies', severity: 'high', check: 'npm-no-critical-high' },
                { report: {}, npmAudit: { summary: { critical: 0, high: 0, moderate: 2, low: 1, total: 3 }, source: 'test' } }
            );
            expect(result.status).toBe('pass');
        });
        ...
        it('evaluates npm-no-critical-high as fail with critical issues', () => {
            const result = evaluateRule(
                { id: 'n1', title: 'NPM', category: 'dependencies', severity: 'high', check: 'npm-no-critical-high' },
                { report: {}, npmAudit: { summary: { critical: 1, high: 0, moderate: 0, low: 0, total: 1 } } }
            );
            expect(result.status).toBe('fail');
        });
        ...
        it('evaluates npm-moderate-limit with default limit of 0', () => {
            const result = evaluateRule(
                { id: 'n2', title: 'NPM Moderate', category: 'dependencies', severity: 'medium', check: 'npm-moderate-limit' },
        ...
        it('evaluates schema-compliance as skip when no samples', () => {
            const result = evaluateRule(
                { id: 's1', title: 'Schema', category: 'quality', severity: 'medium', check: 'schema-compliance' },
        ...
        it('evaluates consistency-pass as skip when not checked', () => {
            const result = evaluateRule(
                { id: 'co1', title: 'Consistency', category: 'quality', severity: 'medium', check: 'consistency-pass' },
        ...
        it('evaluates eu-ai-act-high-risk-reviewed as skip when not scanned', () => {
            const result = evaluateRule(
                { id: 'eu1', title: 'High Risk', category: 'compliance', severity: 'high', check: 'eu-ai-act-high-risk-reviewed' },
                { report: {} }
            );
            expect(result.status).toBe('skip');
        });
        ...
        it('evaluates unknown check as skip', () => {
            const result = evaluateRule(
                { id: 'u1', title: 'Unknown', category: 'test', severity: 'low', check: 'nonexistent-check' },
                { report: {} }
            );
            expect(result.status).toBe('skip');
        ...
        it('evaluates cleanup-bloat-reviewed as pass with no bloat', () => {
            const result = evaluateRule(
                { id: 'b1', title: 'Bloat', category: 'hygiene', severity: 'low', check: 'cleanup-bloat-reviewed' },
