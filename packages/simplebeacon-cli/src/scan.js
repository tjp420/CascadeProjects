...
} = require("./rules/agency-handoff-patterns");
...
} = require("./rules/hallucinated-import-scanner");
...
const {
  ...
  isPipelineScan,
} = require("./lib/scan-usage-tracker");
...
const {
  ...
  reconcileScanReport,
} = require("./lib/normalize-scan-report");
...
function readFileCached(filePath) {
  if (typeof filePath !== "string")
    throw new TypeError("readFileCached expects a string path");
...
async function readFileCachedAsync(filePath) {
  if (typeof filePath !== "string")
    throw new TypeError("readFileCachedAsync expects a string path");
  const cached = fileContentCache.get(filePath);
  if (cached !== undefined) return cached;
...
const SECURITY_NAMES = Object.freeze([
  "SECURITY.md",
  "SECURITY.txt",
  "security.md",
  "security.txt",
...
function normalizeScannerOutput(
  issues,
  scanResult,
  type,
  defaultId,
  ...
  if (!Array.isArray(issues) || !scanResult || typeof scanResult !== "object")
    return;
...
const FP_PATH_PATTERNS = Object.freeze([
  {
    type: "Duplicate Data",
    pathIncludes: [
      "simplebeacon-dashboard/js-es2018/",
      "simplebeacon-dashboard/js/",
      "simplebeacon-dashboard/data/",
      "tsconfig.json",
      "simplebeacon-vscode-merged/dashboard-web/data/",
      "simplebeacon-vscode-merged/dashboard-web/js-es2018/utils-lib/",
      "simplebeacon-dashboard/js/demo/",
      "simplebeacon-vscode-merged/dashboard-web/js/demo/",
      "coming-soon/public/dashboard/js/demo/",
      "web/dashboard/js/utils-lib/",
      "coming-soon/public/dashboard/trust-verification.json",
      "simplebeacon-vscode-merged/dashboard-web/trust-verification.json",
    ],
  },
  {
    type: "Credential Pattern",
    pathIncludes: ["ai-agent-report-for-dashboard.json"],
  },
  ...
  { type: "unvalidated-redirect", pathIncludes: ["server/index.cjs"] },
  {
    ...
    pathIncludes: [
      "server/index.cjs",
      "server/bootstrap/phase2-integration.cjs",
      "server/routes/ai-math-audit-route.cjs",
      "server/routes/compliance-schema-api.cjs",
      "server/routes/demo-simplebeacon-api.cjs",
      "server/routes/eu-ai-act-audit-route.cjs",
      "server/routes/flexible-analyze-api.cjs",
      "server/routes/local-models-api.cjs",
      "server/routes/mock-data-api.cjs",
      "server/routes/realtime-analysis-api.cjs",
      "server/routes/repository-scanner-api.cjs",
      "server/lib/audit-booking-route.cjs",
      "server/lib/data-cleanup-scan.cjs",
      "server/lib/eu-ai-act-sprint-route.cjs",
      "server/lib/operator-deliverable-route.cjs",
      "server/lib/outreach-resend-webhook.cjs",
      "server/lib/legacy-page-redirects.cjs",
      "server/lib/outreach-route.cjs",
      "server/lib/register-operator-routes.cjs",
      "src/api/optimization-api.cjs",
      "src/api/build-from-path-route.cjs",
      "src/api/roadmap-analysis-history.cjs",
      "src/api/simplebeacon-api.cjs",
      "src/api/simplebeacon-billing-api.cjs",
      "src/api/trust-api.cjs",
    ],
  },
  {
    type: "eval-danger",
    pathIncludes: ["server/lib/codebase-analyzer-patterns.cjs"],
  },
  {
    type: "insecure-random",
    pathIncludes: [
      "server/lib/codebase-analyzer-patterns.cjs",
      "server/routes/sso-routes.cjs",
      "package-lock.json",
      "**/package-lock.json",
    ],
  },
  {
    ...
    pathIncludes: ["server/lib/codebase-analyzer-helpers.cjs"],
  },
  {
  ...
  },
  {
    ...
    pathIncludes: [
      "server/api/assessment/index.cjs",
      "server/lib/simplebeacon-subscription-store.cjs",
    ],
  },
...
const SCAN_RESULT_DEFAULTS = Object.freeze({
  ...
  "fiction-kpi-patterns": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
    patterns: [],
  }),
  "llm-slop-patterns": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
    patterns: [],
  }),
  "agency-handoff-patterns": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
    patterns: [],
  }),
  "eu-ai-act-patterns": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
    summary: null,
  ...
  "jest-baseline": Object.freeze({
    checked: false,
    passed: true,
    issues: [],
    summary: null,
  }),
  "token-bleed-patterns": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
  }),
  "architecture-drift-patterns": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
  }),
  "security-patterns": Object.freeze({ scanned: 0, findings: 0, issues: [] }),
  "file-reduction": Object.freeze({
    allFindings: [],
    findings: {},
    summary: {},
  }),
  "dependency-graph": Object.freeze({
    scanned: 0,
    findings: 0,
    issues: [],
    results: [],
...
const SCAN_RESULT_SIMPLE_DEFAULTS = Object.freeze({
  scanned: 0,
  findings: 0,
  issues: [],
  results: [],
...
function _getScanResult(resultMap, key) {
  ...
  return (
    resultMap.get(key) ||
    SCAN_RESULT_DEFAULTS[key] ||
    SCAN_RESULT_SIMPLE_DEFAULTS
  );
}
...
function applyTierLimits(report, options = {}) {
  ...
  if (
    ...
    maxFiles >= 0 &&
    typeof report.totalFiles === "number" &&
    report.totalFiles > maxFiles
  ) {
    report.filesAnalyzed = Math.min(report.filesAnalyzed || 0, maxFiles);
    report.ruleScopedFilesAnalyzed = Math.min(
      report.ruleScopedFilesAnalyzed || 0,
      maxFiles,
    );
    report.totalFiles = maxFiles;
  ...
  if (
    ...
    maxFindings >= 0 &&
    Array.isArray(report.rawIssues)
  ) {
  ...
  report.sandbox = {
    ...
    upgradeUrl,
    message: sandboxMode
      ? "Running in Sandbox Mode. Upgrade to unlock team tier features."
      : undefined,
  };
...
function getReportFindings(report) {
  if (!report || typeof report !== "object") return [];
  if (Array.isArray(report.rawIssues) && report.rawIssues.length > 0)
    return report.rawIssues;
  if (Array.isArray(report.findings) && report.findings.length > 0)
    return report.findings;
  if (Array.isArray(report.detectedIssues) && report.detectedIssues.length > 0)
...
function compileGateStatus(report, gateConfig = {}) {
  ...
  const failOn = Array.isArray(gateConfig.failOn)
    ...
    : gateConfig.failOn
      ? [gateConfig.failOn]
      : ["high"];
  const warnOn = Array.isArray(gateConfig.warnOn)
    ? gateConfig.warnOn
    : gateConfig.warnOn
      ? [gateConfig.warnOn]
      : ["medium", "low"];
  const severityWeights = { critical: 4, high: 3, medium: 2, low: 1 };
  const failWeights = failOn
    .map((s) => severityWeights[s.toLowerCase()] || 3)
    .filter(Boolean);
  ...
  const blockingCount = blockingIssues.reduce(
    (sum, i) => sum + (i.count || 1),
    0,
  );
  const warningCount = warningIssues.reduce(
    (sum, i) => sum + (i.count || 1),
    0,
  );
 ...
 */
function resolveEffectiveScanPaths(
  scanRoot,
  platformRoot,
  config,
  extraPaths = [],
) {
  if (typeof scanRoot !== "string" || typeof platformRoot !== "string")
    return [];
...
const WALK_PROGRESS_EVERY =
  Number(process.env.SIMPLEBEACON_WALK_PROGRESS_EVERY) || 250;
...
async function walkAndCollectFiles(opts) {
  ...
  const reportProgress =
    ...
    results.length > 0 &&
    results.length % 1000 === 0;
  if (reportProgress) {
    process.stderr.write(
      `\rScanning: ${results.length.toLocaleString()} files discovered... `,
    );
  }
  let entries;
...
async function readJsonFile(filePath) {
  ...
  try {
    ...
    if (code === "EACCES" || code === "EPERM") {
      console.warn(
        `[simplebeacon] Permission denied reading ${filePath}: ${message}`,
      );
      return {
        valid: false,
        issue: `permission denied (${code})`,
        raw: null,
        skipped: true,
      };
    }
...
function applyPageSampleValidation({
  ...
  issues.push({
    ...
    recommendedAction:
      "Update mock data to conform to dashboard page schema requirements",
...
async function runSecurityPatternScan(files, secOpts, config) {
  const secPaths = secOpts.sourcePaths ||
  ...
  for (let i = 0; i < files.length; i += BATCH) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (file) => {
        ...
        const isEnvFile =
          basename === ".env" || /^\.env\.[a-z]+$/i.test(basename);
        const ext = file.ext || "";
...
function buildScanReport(opts) {
  ...
  const estimatedCost =
    totalRisks > 0 ? `$${(totalRisks * 5000).toLocaleString()}` : "$0";
  ...
  const {
    ...
    deploymentReadinessScan,
    customHeuristicScan,
    _gzdoomIntegrityScan,
  } = resolved;
  ...
  const scanScope = {
    ...
    pageSpecsFromScanPaths:
      schemaStats.pageSampleSchemaChecked - pageSpecsFromAlias,
    ...
    fictionJsonFilesScanned:
      consistency.jsonFilesScanned ?? consistency.checked ?? 0,
  ...
  const draftReport = {
    ...
    mockSampleFiles: uniqueFiles.filter((f) => {
      ...
      return (
        !isStructuralUtility &&
        (/(?:web\/data|data\/mock|data-central|fixtures?|sample)/i.test(
          f.relativePath,
        ) ||
    ...
    productionLeakSuppressedIntent:
      productionLeakScan.suppressedIntentCount || 0,
    sourceCodeFilesScanned: sourceFictionScan.scanned,
    sourceFictionPatternHits: sourceFictionScan.findings,
    llmSlopFilesScanned: llmSlopScan.scanned,
    llmSlopPatternHits: llmSlopScan.findings,
    ...
    mockDataCategories: [...opts.categories.values()].map((cat) => ({
      ...
      qualityScore: Math.max(
        ...
        Math.min(
          100,
          Math.round(100 - (cat.issues / Math.max(cat.fileCount, 1)) * 40),
        ),
      ),
      issues: cat.issues,
      confidence: null,
    ...
    totalScanTimeMs: (ruleTimings || []).reduce(
  ...
  draftReport.gate = {
    ...
    blockingCount: (gateSummary.blockingIssues || []).reduce(
      (sum, i) => sum + (i.count || 1),
      0,
    ),
    warningCount: (gateSummary.warningIssues || []).reduce(
      (sum, i) => sum + (i.count || 1),
      0,
    ),
...
async function scanMockDataDirectories(baseDir, extraPaths = [], options = {}) {
  if (typeof baseDir !== "string") {
    throw new TypeError(
      "scanMockDataDirectories expects baseDir to be a string",
    );
  }
  ...
  try {
    const analyzerCachePath = path.join(
      scanRoot,
      ".simplebeacon",
      "analyzer-cache.json",
    );
  ...
  const root = scanRoot;
  const rawConfig =
    options.config || loadSimplebeaconConfig(root, options.configPath);
  ...
  if (simplebeaconIgnorePatterns.length > 0) {
    config.ignore = Array.from(
      new Set([...(config.ignore || []), ...simplebeaconIgnorePatterns]),
    );
  }

  // --- Scan quota enforcement ---
  const tierInfo =
    options.tierLimits && options.tier
      ? {
          tier: options.tier,
          paid: Boolean(options.paidLicense),
          limits: options.tierLimits,
        }
      : detectTier();
  ...
  const sanitizedExtraPaths = (extraPaths || [])
    ...
    .filter(Boolean);
  const scanPaths = resolveEffectiveScanPaths(
    scanRoot,
    root,
    config,
    sanitizedExtraPaths,
  ...
  const FULL_TREE_MINIMAL_SKIP_DIRS = [
    ".git",
    "github-cache",
    ".simplebeacon",
    ".vscode-test",
  ...
  const inventoryRoot =
    detectedPlatformRoot !== scanRoot ? scanRoot : detectedPlatformRoot;
  const inventoryPromise = countRepositoryInventory(inventoryRoot, {
    profile: config.fullDirectoryScan
      ? "all"
      : options.inventoryProfile || "universal",
    skipDirs: config.fullDirectoryScan
      ? config.fullDirectoryScanSkipDirs
        ? [...config.fullDirectoryScanSkipDirs]
        : FULL_TREE_MINIMAL_SKIP_DIRS
      : [...MOCK_WALK_SKIP_DIRS],
  });
  ...
  const files = [];
  const scanMaxDepth = config.fullDirectoryScan
    ? 100
    : config.scanMaxDepth || 12;
  let skipDirs = MOCK_WALK_SKIP_DIRS;
  if (config.fullDirectoryScan) {
    skipDirs = config.fullDirectoryScanSkipDirs
      ? new Set(config.fullDirectoryScanSkipDirs)
      : new Set(FULL_TREE_MINIMAL_SKIP_DIRS);
  }
  ...
  try {
    ...
    for (const scanPath of scanPaths) {
      if (fs.existsSync(scanPath)) {
        // simplebeacon-ignore sync-io — path validation before async walk
        await walkAndCollectFiles({
          dir: scanPath,
          results: files,
          depth: 0,
          ...
          onProgress: ({ processed }) => {
            progressWriter.update({
              phase: "walk",
              label: config.fullDirectoryScan
                ? "Full-tree gate walk"
                : "Gate walk",
              processed,
    ...
    if (Array.isArray(config.ignore) && config.ignore.length > 0) {
      uniqueFiles = uniqueFiles.filter(
        (f) => !isIgnoredPath(f.relativePath || "", config.ignore),
      );
    }
    ...
    if (Array.isArray(options.diffFiles) && options.diffFiles.length > 0) {
      const diffSet = new Set(
        options.diffFiles.map((entry) => String(entry).replace(/\\/g, "/")),
      );
      uniqueFiles = uniqueFiles.filter((f) =>
        diffSet.has((f.relativePath || "").replace(/\\/g, "/")),
      );
    }
    ...
    for (let i = 0; i < jsonFilesToProcess.length; i += JSON_BATCH_SIZE) {
      ...
      const results = await Promise.all(
        batch.map((item) => readJsonFile(item.file.path)),
      );
        ...
        if (!parsed.valid) {
          if (!isNodeModulesFile && bucket) {
            ...
            issues.push({
              ...
              type:
                parsed.issue === "empty file" ? "Empty File" : "Invalid JSON",
              filePath: file.path,
              count: 1,
              description: `${file.name}: ${parsed.issue}`,
              recommendedAction:
                parsed.issue === "empty file"
          ...
          if (
            schemaEnabled &&
            file.name.endsWith("-sample.json") &&
            PAGE_SAMPLE_SPECS[file.name]
          ) {
    ...
    if (schemaEnabled) {
      ...
      pageSpecsFromAlias =
        schemaStats.pageSampleSchemaChecked - pageSpecsBeforeAlias;
    }
    ...
    const duplicateGroups = findDuplicateContentGroups(hashEntries).filter(
      (group) => {
        if (group.every((entry) => nodeModulesRe.test(entry.path)))
          return false;
        if (
          group.some(
            (entry) =>
              newFolderRe.test(entry.path) || simplebeaconRe.test(entry.path),
          )
        )
          return false;
        if (
          group.every((entry) =>
            isIgnoredPath(
              displayRelativePath(root, entry.path),
              ignorePatterns,
            ),
          )
        )
    ...
    for (const group of duplicateGroups) {
      const relativePaths = group.map((entry) =>
        displayRelativePath(root, entry.path),
      );
      issues.push({
    ...
    const scanPromises = [];
    const scanKeys = [];
    const _simpleScanOpts = {
      maxDepth: 20,
      skipDirs: config.skipDirs || MOCK_WALK_SKIP_DIRS,
    };
    ...
    function scannerEntry(key, varName, scannerFn, buildOpts, overrides = {}) {
      return {
        key,
        varName,
        enabled: overrides.alwaysRun
          ? undefined
          : (cfg) => isRuleEnabled(cfg, key),
    ...
    const SCANNER_REGISTRY = [
      {
        key: "roadmap",
        varName: "roadmapValidation",
        enabled: (cfg) => isRuleEnabled(cfg, "roadmap"),
        run: () =>
          validateRoadmapFiles(root, {
      ...
      {
        key: "llm-slop-patterns",
        varName: "llmSlopScan",
        enabled: (cfg) =>
          options.slopCop === true || isRuleEnabled(cfg, "llm-slop-patterns"),
        run: () => {
      ...
      },
      {
        ...
        varName: "securityPatternScan",
        enabled: (cfg) => isRuleEnabled(cfg, "security-patterns"),
        run: () =>
          runSecurityPatternScan(
            uniqueFiles,
            getRuleOptions(config, "security-patterns"),
            config,
          ),
      },
      ...
      ),
      {
        ...
        run: () => {
          ...
          return scanGzdoomIntegrity(root, {
            ...
            cvarAllowlist:
              opts.cvarAllowlist || config.gzdoom?.cvarAllowlist || [],
            cvarAllowlistPrefixes:
              opts.cvarAllowlistPrefixes ||
              config.gzdoom?.cvarAllowlistPrefixes ||
              [],
            companionMod:
              opts.companionMod || config.gzdoom?.companionMod || null,
            companionModPaths:
              opts.companionModPaths ||
              config.gzdoom?.companionModPaths ||
              null,
          });
        },
      },
    ...
    const onlyRules =
      Array.isArray(options.onlyRules) && options.onlyRules.length > 0
        ? new Set(options.onlyRules)
        : null;
    const skipRules =
      Array.isArray(options.skipRules) && options.skipRules.length > 0
        ? new Set(options.skipRules)
        : null;
    ...
    function tickProgress(name) {
      ...
      if (!quiet && process.stderr.isTTY && totalRules > 0) {
        process.stderr.write(
          `\rScanning: ${completedRules}/${totalRules} rules complete (${name})... `,
        );
      }
      progressWriter.update({
        phase: "rules",
    ...
    const trackedPromises = scanPromises.map((p, i) => {
      ...
      return p
        ...
        .catch((err) => {
          ...
          console.error(
            `[simplebeacon] Rule scanner '${scanKeys[i]}' failed: ${errMsg}`,
          );
          scanErrors.push({
            rule: scanKeys[i],
            error: errMsg,
          ...
          return {
            ...
            issues: [],
            error: errMsg,
            _timing: { rule: scanKeys[i], elapsedMs },
          };
        });
    });
    ...
    if (!quiet && process.stderr.isTTY && totalRules > 0) {
      process.stderr.write(
        `\rScanning: ${totalRules}/${totalRules} rules complete.          \n`,
      );
    }
    ...
    normalizeScannerOutput(
      ...
      null,
      null,
      (getRuleOptions(config, "fiction-kpi-patterns") || {}).severity ||
        "medium",
    );
    normalizeScannerOutput(
      issues,
      ...
      null,
      null,
      (getRuleOptions(config, "llm-slop-patterns") || {}).severity || "medium",
    );
    ...
    normalizeScannerOutput(
      ...
      "insecure-random",
      "SB-SEC-006",
      "Weak crypto or insecure random",
    );
    normalizeScannerOutput(
      issues,
      secretInCommentsScan,
    ...
    normalizeScannerOutput(
      ...
      "config-drift",
      "SB-SEC-008",
      "Secret file tracked by git or not gitignored",
    );
    normalizeScannerOutput(
      issues,
      redosScan,
    ...
    normalizeScannerOutput(
      ...
      "cleanup",
      "SB-QUAL-001",
      "Unused import or unreachable code",
      "low",
    );
    normalizeScannerOutput(
      issues,
    ...
    normalizeScannerOutput(
      ...
      "cve-vulnerability",
      "SB-CVE-001",
      "Known CVE vulnerability in dependency",
    );
    normalizeScannerOutput(
      issues,
      sbomScan,
    ...
    normalizeScannerOutput(
      ...
      "gzdoom-integrity",
      "GZ-XREF",
      "GZDoom mod reference integrity issue",
    );
    if (fileReduction.allFindings?.length) {
      for (const finding of fileReduction.allFindings) {
        issues.push({
          id:
            finding.id ||
            `file-reduction-${finding.type}-${crypto.randomBytes(3).toString("hex")}`,
          severity: finding.severity || "low",
          type: finding.type || "File Reduction",
          filePath: finding.path || finding.filePath || null,
          count: finding.count || 1,
          description:
            finding.description || finding.message || `${finding.type} finding`,
          recommendedAction:
            finding.recommendedAction ||
            finding.fixSuggestion ||
            "Review file-reduction report",
          affectedFiles:
    ...
    const filteredIssues = issues.filter(
      (issue) => !isKnownFalsePositive(issue),
    );
    ...
    for (let i = 0; i < uniqueFiles.length; i += LINE_COUNT_BATCH) {
      ...
      const counts = await Promise.all(
        batch.map((f) => countFileLines(f.path, f.ext)),
      );
    ...
    const rulesEnabled = Object.keys(config.rules || {}).filter((name) =>
      isRuleEnabled(config, name),
    );
    ...
    const totalElapsedMs = Math.round(
      Number(process.hrtime.bigint() - scanStart) / 1_000_000,
    );
...
async function runScan(baseDir, options = {}) {
  ...
  try {
    return await scanMockDataDirectories(
      baseDir,
      safeOptions.extraPaths || [],
      safeOptions,
    );
  } finally {
    if (offlineGuard) {
