...
function resolveComplianceCounts(root) {
  if (typeof root !== 'string')
    return { licenseCount: 0, securityCount: 0, summary: null, remediation: null };
  return {
...
const FP_PATH_PATTERNS = Object.freeze([
  ...
  { type: 'eval-danger', pathIncludes: ['server/lib/codebase-analyzer-patterns.cjs'] },
  {
    ...
    pathIncludes: ['server/lib/codebase-analyzer-patterns.cjs', 'server/routes/sso-routes.cjs'],
  },
  { type: 'prototype-pollution', pathIncludes: ['server/lib/codebase-analyzer-helpers.cjs'] },
  {
    type: 'unvalidated-redirect',
    pathIncludes: ['server/routes/sso-routes.cjs', 'server/lib/legacy-page-redirects.cjs'],
  },
  {
...
function isKnownFalsePositive(issue) {
  ...
  return FP_PATH_PATTERNS.some(
    (rule) => rule.type === type && rule.pathIncludes.some((sub) => fp.includes(sub))
  );
}
...
async function walkAndCollectFiles(opts) {
  ...
  const reportProgress =
    !quiet && process.stderr.isTTY && results.length > 0 && results.length % 1000 === 0;
  if (reportProgress) {
  ...
  const draftReport = {
    ...
    mockDataCategories: [...opts.categories.values()].map((cat) => ({
      ...
      qualityScore: Math.max(
        60,
        Math.min(100, Math.round(100 - (cat.issues / Math.max(cat.fileCount, 1)) * 40))
      ),
      issues: cat.issues,
      confidence: null,
  ...
  try {
    ...
    for (const file of uniqueFiles) {
      ...
      const isStdoutCapture =
        /-stdout\.json$/i.test(file.name) || /report-stdout\.json$/i.test(file.name);
    ...
    const duplicateGroups = findDuplicateContentGroups(hashEntries).filter((group) => {
      ...
      if (group.some((entry) => newFolderRe.test(entry.path) || simplebeaconRe.test(entry.path)))
        return false;
      if (
        group.every((entry) => isIgnoredPath(displayRelativePath(root, entry.path), ignorePatterns))
      )
        return false;
      return true;
    });
    ...
    const SCANNER_REGISTRY = [
      {
        ...
        run: () =>
          validateRoadmapFiles(root, { baseline: config.baseline, ignoreGlobs: config.ignore }),
      },
      {
        key: 'consistency',
        varName: 'consistency',
      ...
      ),
      {
        ...
        run: () => {
          ...
          return scanLlmSlopPatterns(root, {
            ...
            ignoreGlobs: opts.ignoreGlobs || config.ignore,
            registryCheck:
              opts.registryCheck === true || process.env.SIMPLEBEACON_REGISTRY_CHECK === 'true',
      ...
      },
      {
        ...
        run: () =>
          runSecurityPatternScan(uniqueFiles, getRuleOptions(config, 'security-patterns'), config),
      },
    ...
    function tickProgress(name) {
      ...
      if (onProgress) {
        try {
          onProgress({ completed: completedRules, total: totalRules, currentRule: name });
        } catch {}
      }
    }
    ...
    const timeoutMs =
      typeof options.timeoutMs === 'number' && options.timeoutMs > 0 ? options.timeoutMs : 600000; // 10 minutes default
    let timeout;
    let scanResults;
    try {
    ...
    normalizeScannerOutput(
      ...
      null,
      null,
      (getRuleOptions(config, 'fiction-kpi-patterns') || {}).severity || 'medium'
    );
    normalizeScannerOutput(
      issues,
      llmSlopScan,
    ...
    if (fileReduction.allFindings?.length) {
      for (const finding of fileReduction.allFindings) {
        issues.push({
          id:
            finding.id || `file-reduction-${finding.type}-${crypto.randomBytes(3).toString('hex')}`,
          ...
          recommendedAction:
            finding.recommendedAction || finding.fixSuggestion || 'Review file-reduction report',
          affectedFiles:
            finding.affectedFiles || finding.paths || (finding.path ? [finding.path] : []),
          metadata: {
