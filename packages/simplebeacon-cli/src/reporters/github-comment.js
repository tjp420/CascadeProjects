...
function isAnonymizedReport(report) {
  return (
    report.schemaVersion === "anonymized-v1" || report.repoFingerprint != null
  );
}
...
function normalizeIssues(report) {
  ...
  return raw.map((issue) => ({
    severity: String(
      issue.severity || issue.severityBand || "medium",
    ).toLowerCase(),
...
function fixHint(issue) {
  ...
  if (kind === "credentials")
    return "Rotate the secret and move it to an environment variable.";
  if (kind === "production-leak")
    return "Replace mock/sample JSON imports with API-backed config.";
  if (kind === "fiction-kpi")
    return "Replace placeholder KPI literals with measured baseline values.";
...
function formatGithubComment(report, gateResult = null, options = {}) {
  ...
  const dashboardUrl =
    options.dashboardUrl || "https://simplebeacon.ai/dashboard/dashboard";
  const pricingUrl =
    report.sandbox?.upgradeUrl ||
    options.pricingUrl ||
  ...
  const failOn = gate?.failOn ||
    report.scanScope?.gatePolicy?.failOn || ["high"];
  lines.push(`<details${pass ? "" : " open"}>`);
  lines.push(
    `<summary><strong>${pass ? "Scan summary" : `Blocking findings (${gate?.blockingCount || 0})`}</strong></summary>`,
  );
  lines.push("");
  lines.push(
    `_Fails on: ${Array.isArray(failOn) ? failOn.join(", ") : failOn}_`,
  );
  lines.push("");
  ...
  if (!issues.length) {
    ...
    if (issues.length > top.length) {
      lines.push(
        `<details><summary>+ ${issues.length - top.length} more finding(s)</summary>`,
      );
      lines.push("");
      issues.slice(top.length, 20).forEach((issue, index) => {
        lines.push(
          `**${index + top.length + 1}.** ${formatIssueRow(issue, options)}`,
        );
        lines.push("");
      });
      lines.push("</details>");
  ...
  lines.push("---");
  lines.push(
    `[Team dashboard](${dashboardUrl}) · [Upgrade for org-wide CI history](${pricingUrl})`,
  );
  lines.push("");
  lines.push(
    "_48 analyzers + 25 scan engines catch AI code debt that traditional linting misses — no upload, no LLM, no false positives._",
  );
...
async function postGithubComment(reportPath, options = {}) {
  ...
  if (
    ...
    throw new Error(
      "GitHub comment posting blocked in offline mode — --github-comment requires network access. Remove --offline to post PR comments.",
    );
  }
  ...
  if (!repo)
    throw new Error("GITHUB_REPOSITORY is required to post PR comments");
  if (!issueNumber)
    throw new Error(
      "Pull request number is required (GITHUB_EVENT_PULL_REQUEST_NUMBER)",
    );
