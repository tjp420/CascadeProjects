...
function formatGithubComment(report, gateResult = null, options = {}) {
  ...
  const pricingUrl =
    report.sandbox?.upgradeUrl || options.pricingUrl || 'https://simplebeacon.ai/pricing';
  ...
  const lines = ['## 🔦 SimpleBeacon — AI Circuit Breaker', '', headline, ''];
  ...
  lines.push('');
  lines.push(
    '_SimpleBeacon gates AI slop, credential leaks, and mock-path drift — complementary to Snyk/GHAS._'
  );
...
async function postGithubComment(reportPath, options = {}) {
  ...
  if (!issueNumber)
    throw new Error('Pull request number is required (GITHUB_EVENT_PULL_REQUEST_NUMBER)');
