...
const { GUIDE_PLAYBOOKS, issueKind, collectActiveGuideIds } = require('./remediation-guides');
...
function formatTextReport(report, gateResult = null) {
  ...
  const prioritized = sortedIssues.filter((issue) => remediationText(issue).length > 0).slice(0, 8);
...
function formatActionPlanReport(report, gateResult = null) {
  ...
  if (guideIds.length === 0) {
    lines.push(
      paint('No prioritized action items — scan is clean under configured paths.', 'green')
    );
    return lines.join('\n');
  }
  ...
  for (const id of orderedIds) {
    ...
    totalMinutes += est;
    const diffColor =
      guide.difficulty === 'Easy' ? 'green' : guide.difficulty === 'Moderate' ? 'yellow' : 'red';
    lines.push(
