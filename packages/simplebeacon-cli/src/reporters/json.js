...
function formatJsonReport(report, gateResult = null) {
  ...
  const hasBlockingFindings =
    (rawGate.blockingCount || 0) > 0 ||
    (rawGate.blockingIssues || []).length > 0;
  ...
  const enrichedGate = {
    ...
    blockingCount: (rawGate.blockingIssues || []).reduce(
      (sum, i) => sum + (i.count || 1),
      0,
    ),
    warningCount: (rawGate.warningIssues || []).reduce(
      (sum, i) => sum + (i.count || 1),
      0,
    ),
    ...
    summary:
      derivedPass === false &&
      rawGate.summary &&
      /passed|clear|green/i.test(rawGate.summary)
        ? "Gate blocked — review findings before release."
        : rawGate.summary ||
          (derivedPass === true
            ? "Gate passed — no blocking credentials found."
            : (() => {
                const bc = (rawGate.blockingIssues || []).reduce(
                  (sum, i) => sum + (i.count || 1),
                  0,
                );
                return `Gate blocked — ${bc} blocking issue${bc === 1 ? "" : "s"} detected. Review before release.`;
              })()),
    ...
    remediation: [
      ...new Set(
        (rawGate.blockingIssues || []).filter((i) => i.fix).map((i) => i.fix),
      ),
    ].slice(0, 3),
  };
  ...
  const dataQuality = report.dataQuality || {
    ...
    severityColor:
      report.emptyFiles || report.invalidJson ? "#EF4444" : "#34D399",
    summary:
      report.emptyFiles || report.invalidJson
  ...
  const debugIssue = (report.detectedIssues || []).find(
    (i) => i.type === "Debug Artifact",
  );
  const debugArtifactCount = debugIssue
    ? debugIssue.count || 0
    : (report.llmSlopPatternHits || 0) + (report.sourceFictionPatternHits || 0);
  const cleanup = report.cleanup || {
    debugArtifactCount: debugArtifactCount,
    severity:
      debugArtifactCount > 20
        ? "high"
  ...
  const govIssue = (report.detectedIssues || []).find(
    (i) => i.type === "License/Governance Marker",
  );
  ...
  const hasStaleCompliance =
    report.compliance &&
    report.compliance.licenseCount === 0 &&
    report.compliance.securityCount === 0 &&
    govCount > 0;
  const licenseCount = hasStaleCompliance
    ? govCount
    : (report.compliance?.licenseCount ?? govCount);
  const securityCount = report.compliance?.securityCount ?? 0;
  const complianceSummary =
    licenseCount || securityCount
  ...
  const recs = [];
  if (licenseCount === 0)
    recs.push("Add a LICENSE file to clarify distribution terms.");
  if (securityCount === 0)
  ...
  const euHighRisk =
    report.euAiActHighRisk || report.euAiAct?.highRiskIndicators || 0;
  const euTransparency =
    report.euAiActTransparency || report.euAiAct?.transparencyGaps || 0;
  const euDocs =
    report.euAiActDocumentation || report.euAiAct?.documentationArtifacts || 0;
  const euIndicators =
    report.euAiActFindings || report.euAiAct?.aiSystemIndicators || 0;
  const euDocFound =
    report.euAiActDocFound || report.euAiAct?.documentationFound || [];
  const euNote =
    report.euAiActDeadlineNote || report.euAiAct?.deadlineNote || "";
  ...
  const allFiles =
    report.fileList ||
    report.sampleFiles ||
    report.repositoryInventory?.files ||
    [];
  const filePaths = Array.isArray(allFiles) ? allFiles : [];
  const lowerPaths = filePaths.map((f) =>
      ...
      try {
        const walk = (dir) => {
          ...
          for (const ent of entries) {
            if (
              ent.isDirectory() &&
              !/^(node_modules|\.git|\.simplebeacon)$/.test(ent.name)
            ) {
  ...
  const readinessChecks = [
    ...
    {
      name: "CHANGELOG",
      found: hasFile(/changelog|changes|history/),
      critical: false,
    },
    {
      name: "Tests",
      found: hasDeep(
        /test|spec|\.test\.|\.spec\.|__tests__|jest\.config|vitest\.config|cypress/,
      ),
      critical: true,
    },
    {
      name: "CI/CD",
      found: hasDeep(
        /\.github|\.gitlab-ci|jenkins|\.circleci|\.travis|azure-pipelines|ci\.(yml|yaml)|build\.(yml|yaml)|deploy\.(yml|yaml)/,
      ),
      critical: true,
    },
    {
      name: "Docker",
      found: hasFile(/dockerfile|docker-compose|\.dockerignore/),
      critical: false,
    },
    {
      name: "Linting/Formatting",
      found: hasFile(/eslint|prettier|\.editorconfig|lint-staged|husky/),
      critical: false,
    },
    {
      name: "TypeScript Config",
      found: hasFile(/tsconfig|\.ts$/),
      critical: false,
    },
    {
      name: "Build Tool Config",
      found: hasFile(
        /webpack|rollup|vite|esbuild|parcel|babel|gulpfile|gruntfile/,
      ),
      critical: false,
    },
    {
      name: ".env.example",
      found: hasFile(/\.env\.example|\.env\.sample|\.env\.template/),
      critical: true,
    },
  ...
  const readinessScore = Math.round(
    (readinessChecks.filter((c) => c.found).length / readinessChecks.length) *
      100,
  );
    ...
    recommendations:
      missingCritical.length > 0
        ? [
            ...
            "Start with package.json, README, .gitignore, and .env.example.",
          ]
        : missingNice.length > 0
          ? [
              "Add recommended files to improve maintainability.",
              "Consider Docker, linting config, and CHANGELOG.",
            ]
          : [
  ...
  const invalidJson =
    report.invalidJson ?? report.dataQuality?.invalidJsonCount ?? null;
  const emptyFiles =
    report.emptyFiles ?? report.dataQuality?.emptyJsonCount ?? null;
  const dupes =
    report.duplicateGroups ?? report.consolidation?.duplicateGroups ?? null;
  const credFindings =
    report.credentialFindings ?? enrichedGate?.blockingCount ?? null;
  const todoMarkers =
    report.todoMarkerCount ?? report.roadmap?.todoCount ?? null;
  const phases = [];
  ...
  if (credFindings != null && credFindings > 0) {
    integrityTasks.push({
      description: `Rotate ${credFindings} exposed credential${credFindings === 1 ? "" : "s"}`,
      type: "fix",
      done: false,
      isStructured: true,
    });
  ...
  if (invalidJson > 0)
    integrityTasks.push({
      description: `Fix ${invalidJson} invalid JSON file${invalidJson === 1 ? "" : "s"}`,
      type: "fix",
      done: false,
      isStructured: true,
    });
  if (emptyFiles > 0)
    integrityTasks.push({
      description: `Remove ${emptyFiles} empty file${emptyFiles === 1 ? "" : "s"}`,
      type: "fix",
      done: false,
      isStructured: true,
    });
  ...
  phases.push({
    ...
    effort: "2–4 days",
    description: integrityClean
      ? "Data integrity verified — no structural issues detected."
      : `Resolve structural issues${invalidJson > 0 ? ": " + invalidJson + " invalid JSON" : ""}${emptyFiles > 0 ? ": " + emptyFiles + " empty file" + (emptyFiles === 1 ? "" : "s") : ""}.`,
  ...
  if (dupes > 0)
    consistencyTasks.push({
      description: `Consolidate ${dupes} duplicate group${dupes === 1 ? "" : "s"}`,
      type: "fix",
      done: false,
      isStructured: true,
    });
  ...
  const consistencyProgress = consistencyClean
    ? 100
    : dupes <= 5
      ? 75
      : dupes <= 20
  ...
  phases.push({
    ...
    effort: "3–5 days",
    description: consistencyClean
      ? "Consistency verified — no duplicates or naming drift."
      : `Eliminate redundancy: ${dupes} duplicate group${dupes === 1 ? "" : "s"}.`,
    tasks: consistencyTasks,
    progress: consistencyProgress,
    status: consistencyClean
      ? "completed"
  ...
  if (licenseCount > 0)
    govTasks.push({
      description: `Audit ${licenseCount} open-source license file${licenseCount === 1 ? "" : "s"}`,
      type: "review",
      done: true,
      isStructured: true,
    });
  if (securityCount > 0)
    govTasks.push({
      description: `Review ${securityCount} security/governance file${securityCount === 1 ? "" : "s"}`,
      type: "review",
      done: true,
      isStructured: true,
    });
  ...
  govTasks.push({
    description: "Add LICENSE and SECURITY.md if missing",
    type: "fix",
    done: govScore >= 2,
    isStructured: true,
  });
  ...
  if (ai > 0)
    euTasks.push({
      description: `Review ${ai} AI system indicator${ai === 1 ? "" : "s"} (Art. 6)`,
      type: "review",
      done: false,
      isStructured: true,
    });
  ...
  const optTasks = [];
  if (todoMarkers != null && todoMarkers > 0)
    optTasks.push({
      description: `Address ${todoMarkers} pending-task marker${todoMarkers === 1 ? "" : "s"} in source code`,
      type: "fix",
      done: false,
      isStructured: true,
    });
  ...
  const diagnosticReport = report.diagnosticReport || {
    ...
    unaccounted: Math.max(
      0,
      totalFiles -
        (report.filesAnalyzed || report.ruleScopedFilesAnalyzed || 0) -
        (report.largeFileSkips || report.binarySkipped || 0) -
        (report.readErrors || 0),
    ),
  };
