...
function formatJsonReport(report, gateResult = null) {
  ...
  const hasBlockingFindings =
    (rawGate.blockingCount || 0) > 0 || (rawGate.blockingIssues || []).length > 0;
  ...
  const enrichedGate = {
    ...
    summary:
      derivedPass === false && rawGate.summary && /passed|clear|green/i.test(rawGate.summary)
    ...
    remediation: [
      ...new Set((rawGate.blockingIssues || []).filter((i) => i.fix).map((i) => i.fix)),
    ].slice(0, 3),
  };
  ...
  const dataQuality = report.dataQuality || {
    ...
    remediation:
      report.emptyFiles || report.invalidJson ? 'Review empty or invalid JSON files.' : null,
  };
  ...
  const cleanup = report.cleanup || {
    ...
    severityColor:
      debugArtifactCount > 20 ? '#EF4444' : debugArtifactCount > 0 ? '#F59E0B' : '#34D399',
  ...
  const health =
    govScore >= 10 ? 'excellent' : govScore >= 5 ? 'good' : govScore >= 2 ? 'fair' : 'poor';
  const recs = [];
  ...
  const fileReduction = report.fileReduction || {
    ...
    remediation: report.duplicateGroups ? 'Consolidate duplicate files into shared modules.' : null,
  };
  ...
  const readinessChecks = [
    ...
    {
      name: 'Tests',
      found: hasDeep(/test|spec|\.test\.|\.spec\.|__tests__|jest\.config|vitest\.config|cypress/),
      critical: true,
    },
    {
      name: 'CI/CD',
    ...
    { name: 'TypeScript Config', found: hasFile(/tsconfig|\.ts$/), critical: false },
    {
      ...
      found: hasFile(/webpack|rollup|vite|esbuild|parcel|babel|gulpfile|gruntfile/),
      critical: false,
    },
    {
      name: '.env.example',
  ...
  const readinessScore = Math.round(
    (readinessChecks.filter((c) => c.found).length / readinessChecks.length) * 100
  );
  ...
  const integrityClean =
    (invalidJson === 0 || invalidJson == null) && (emptyFiles === 0 || emptyFiles == null);
