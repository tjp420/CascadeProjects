...
const ALERT_TEMPLATES = {
  ...
  "SB-SEC-014": {
    ...
    impact:
      "Service account keys grant access to GCP resources (Cloud Storage, BigQuery, Compute Engine). An exposed key allows attackers to impersonate the service account, exfiltrate data, spin up compute resources for cryptomining, or pivot to other GCP services via IAM escalation.",
    immediateAction:
      "Rotate the exposed key immediately in the GCP Console (IAM & Admin > Service Accounts > Keys). Revoke the compromised key after creating a replacement. Audit GCP audit logs for unauthorized API calls made with the key.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Never commit service account JSON keys to version control. Use GCP Secret Manager, Workload Identity, or environment variables. Add pre-commit hooks with gitleaks or simplebeacon to scan for key patterns.",
    references: [
  ...
  "SB-SEC-015": {
    ...
    impact:
      "Azure storage keys grant full access to blob, table, queue, and file services in the storage account. An exposed key allows attackers to read, modify, or delete stored data, upload malicious content, and escalate to other Azure services via the associated subscription.",
    immediateAction:
      "Rotate the storage key in the Azure Portal (Storage Account > Access keys > Rotate). Rotate the client secret in Azure Active Directory > App Registrations. Audit Azure Activity Logs for unauthorized access.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Use Azure Key Vault for all secrets. Never hardcode AccountKey or AZURE_CLIENT_SECRET. Use Managed Identities where possible to eliminate the need for secrets entirely.",
    references: [
  ...
  "SB-SEC-016": {
    ...
    impact:
      "OAuth tokens grant authenticated access to the issuing platform. A Google ya29.* token can access Gmail, Drive, and Calendar. A GitHub token can modify repositories and access private data. A Slack token can read messages and exfiltrate company communications. A Stripe key can issue refunds and access customer data.",
    immediateAction:
      "Revoke the exposed token immediately on the issuing platform. For Google: Security > Third-party apps. For GitHub: Settings > Developer settings > Tokens. For Slack: Workspace > API tokens. For Stripe: Dashboard > API keys.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Store OAuth tokens in environment variables or secret managers. Use OAuth flows that exchange authorization codes for tokens at runtime rather than hardcoding tokens. Implement token refresh logic instead of persisting long-lived tokens.",
    references: [
  ...
  "SB-SEC-017": {
    ...
    impact:
      "Privileged mode grants the container full access to host devices, kernel capabilities, and the host filesystem. An attacker who compromises the container can escape to the host, access other containers, and gain root-level control over the entire host system.",
    immediateAction:
      "Remove the privileged flag from the container configuration. Identify the specific capability the container needs and grant only that capability via --cap-add.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Never use privileged mode in production. Use the principle of least capability — grant only the specific capabilities needed. Use Docker security profiles (AppArmor, seccomp) to further restrict container behavior.",
    references: [
  ...
  "SB-SEC-018": {
    ...
    impact:
      "Running as root inside a container increases the blast radius of container escape vulnerabilities. If an attacker compromises the container, they already have root privileges, making it easier to access host resources and escalate attacks.",
    immediateAction:
      "Change the USER directive to a non-root user. Create a dedicated user in the Dockerfile with minimal permissions.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Always create a dedicated non-root user in Dockerfiles. Use multi-stage builds to minimize the final image. Set read-only filesystem where possible with `--read-only` flag.",
    references: [
  ...
  "SB-SEC-019": {
    ...
    severity: "critical",
    summary:
      "A secret (password, API key, token) is hardcoded in a Docker ENV directive",
    impact:
      "Docker ENV values are baked into the image layers and are visible to anyone who can pull the image. Secrets in ENV directives cannot be rotated without rebuilding the image. They appear in `docker inspect`, `docker history`, and any registry that stores the image.",
    immediateAction:
      "Rotate the exposed secret. Remove it from the Dockerfile immediately. Use Docker secrets or environment variable files instead of ENV directives.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Never put secrets in ENV directives. Use Docker secrets, Kubernetes secrets, or external secret managers (Vault, AWS Secrets Manager). Use multi-stage builds and .dockerignore to prevent accidental secret inclusion.",
    references: [
  ...
  "SB-SEC-020": {
    ...
    impact:
      "Without a health check, the container orchestrator (Docker Swarm, Kubernetes, ECS) cannot detect when the application inside the container is unhealthy. The container continues running and receiving traffic even if the application has hung or crashed without exiting.",
    immediateAction:
      "Add a HEALTHCHECK instruction to the Dockerfile that tests the application endpoint.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Always include a HEALTHCHECK in production Dockerfiles. Use a dedicated /health or /ready endpoint that verifies database connectivity and critical dependencies, not just HTTP 200.",
    references: [
  ...
  "SB-SEC-021": {
    ...
    impact:
      "Typosquat packages are malicious packages published with names similar to popular packages (e.g., `nodjs` instead of `nodejs`). They often contain malware that steals credentials, installs backdoors, or enrolls the machine in a botnet upon installation.",
    immediateAction:
      "Do not install the flagged package. Verify the package name against the official npm registry. If already installed, remove it and audit the system for compromise.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Always verify package names against the official registry. Use npm audit and Snyk to scan for known malicious packages. Enable npm 2FA. Consider using a private npm registry proxy that blocks known typosquats.",
    references: [
  ...
  "SB-SEC-022": {
    ...
    impact:
      "postinstall scripts run automatically when a package is installed. Malicious scripts can download and execute arbitrary code, steal environment variables and secrets, install backdoors, or modify system files — all without user interaction.",
    immediateAction:
      "Review the postinstall script. If it makes network calls or executes dynamic code, remove it. Audit the system for compromise if the script was already executed.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Avoid postinstall scripts that make network calls. Use `npm install --ignore-scripts` in CI/CD. Audit package.json before adding dependencies. Use npm 7+ lockfile to detect unexpected script changes.",
    references: [
  ...
  "SB-SEC-023": {
    ...
    impact:
      "Unpinned dependencies can resolve to different versions across installs, leading to non-reproducible builds. A malicious or buggy update can break the application or introduce vulnerabilities without any code change. Supply chain attacks often target floating version specifiers.",
    immediateAction:
      "Pin the dependency to an exact version. Run npm install to verify the lockfile is consistent.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Pin all dependencies to exact versions. Use `npm ci` instead of `npm install` in CI. Use Dependabot or Renovate for automated, reviewed dependency updates. Regularly run `npm audit` to check for vulnerabilities.",
    references: [
  ...
  "SB-SEC-009": {
    ...
    impact:
      "Environment files typically contain database credentials, API keys, encryption keys, and other secrets. Once committed, these secrets are in git history permanently and accessible to anyone with repository access. Even if deleted, the secrets remain in historical commits.",
    immediateAction:
      "Rotate ALL secrets that were in the .env file. Remove the file from git tracking. Add .env to .gitignore. Use git filter-branch or BFG to purge the file from history.",
    remediationSteps: [
    ...
    preventionGuidance:
      "Never commit .env files. Add .env to .gitignore on day one. Use .env.example with placeholder values. Install pre-commit hooks that block .env commits. Use secret managers for production secrets.",
    references: [
  ...
  "SB-SEC-013": {
    ...
    severity: "critical",
    summary:
      "A CI/CD secret (GITHUB_TOKEN, AWS keys, NPM_TOKEN) is hardcoded in a workflow or config file",
    impact:
      "CI/CD secrets in workflow files are visible to anyone with read access to the repository. Attackers can use these secrets to publish packages, modify infrastructure, or access production environments. GitHub Actions secrets in particular can be exfiltrated via pull requests from forks.",
    immediateAction:
      "Rotate the exposed secret. Move it to the CI/CD platform secret store (GitHub Actions secrets, GitLab CI variables). Audit CI/CD logs for unauthorized usage.",
    remediationSteps: [
      "Rotate the exposed secret at the provider",
    ...
    preventionGuidance:
      "Never hardcode secrets in workflow files. Use the CI/CD platform secret store. Restrict secret access to specific environments. Use OIDC federation where possible to eliminate long-lived secrets entirely.",
    references: [
