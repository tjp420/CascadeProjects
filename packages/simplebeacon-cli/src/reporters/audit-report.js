...
const AUDIT_CHECKLIST_ROWS = [
  {
    ...
    title: "Zero hardcoded credential patterns",
  },
  { check: "zero-production-leaks", title: "Production path separation" },
  {
    check: "schema-compliance",
    title: "Schema conformity (configured samples)",
  },
...
function issueSeverityBand(issue) {
  const band = String(
    issue.severityBand || issue.severity || "low",
  ).toLowerCase();
  if (
    band === "critical" ||
    band === "high" ||
...
function findingTitle(issue) {
  ...
  if (/production leak/i.test(type))
    return "Production code references mock sample JSON";
  if (/fiction|kpi|consistency/i.test(type))
    return "AI-generated fiction KPI patterns";
...
function findingRisk(issue) {
  ...
  return (
    issue.description || "Review this finding before production deployment."
  );
}
...
function formatFindingTable(issue) {
  ...
  if (issue.snippet) {
    ...
    rows.push([
      "Detected values",
      issue.description || "See scan report for matched values",
    ]);
  }
  ...
  rows.push([
    "Remediation",
    issue.recommendedAction || defaultRemediation(issue),
  ]);
  ...
  if (issue.alertTemplate) {
    ...
    if (at.remediationSteps && at.remediationSteps.length > 0) {
      rows.push([
        "Remediation steps",
        at.remediationSteps.map((s, i) => `${i + 1}. ${s}`).join("  "),
      ]);
    }
    ...
    if (at.rotationRequired) {
      rows.push([
        "Rotation required",
        "Yes — rotate any credentials that may have been exposed",
      ]);
    }
  }
...
function buildRemainingSummary(mediumRemainder, lowIssues) {
  ...
  if (mediumRemainder.length) {
    const count = mediumRemainder.reduce(
      (sum, issue) => sum + (issue.count || 1),
      0,
    );
    const samplePath =
      mediumRemainder[0]?.filePath || "configured sample paths";
    const folder = samplePath.includes("/")
      ? samplePath.split("/").slice(0, -1).join("/")
      : samplePath;
    lines.push(
      `- **${count} medium finding${count === 1 ? "" : "s"}** across sample JSON under \`${folder}\` — missing required page-spec keys and cross-file KPI drift vs baseline.`,
    );
  }
  ...
  if (lowIssues.length) {
    ...
    lines.push(
      `- **${count} low finding${count === 1 ? "" : "s"}** — informational roadmap template pattern (no gate block by default).`,
    );
  }
...
function findRuleForRow(rules, row) {
  return (
    rules.find((rule) => rule.check === row.check) ||
...
function buildComplianceTable(report, assessment, projectRoot) {
  ...
  const rows = AUDIT_CHECKLIST_ROWS.map((row) => {
    ...
    if (row.check === "zero-credential-findings") {
      ...
      if (
        report.consistencyChecked == null ||
...
function buildHeadline(report, assessment, severityCounts, _issues) {
  ...
  if (severityCounts.critical) {
    parts.push(
      `${severityCounts.critical} critical issue${severityCounts.critical === 1 ? "" : "s"}`,
    );
  }
  if (severityCounts.high) {
    parts.push(
      `${severityCounts.high} high-severity issue${severityCounts.high === 1 ? "" : "s"}`,
    );
  }
  if (severityCounts.medium) {
    parts.push(
      `${severityCounts.medium} medium issue${severityCounts.medium === 1 ? "" : "s"}`,
    );
  }
...
function _buildCommandsRun(options) {
  if (options.assessment?.commandsRun?.length) {
    ...
    if (!commands.some((cmd) => /report/.test(cmd))) {
      commands.push(
        'npx simplebeacon report --company "[Company]" --client "[Project]" --output AUDIT_REPORT.md',
      );
    }
    return commands;
  }
...
function compileAuditReportMarkdown(report, options = {}) {
  ...
  const generatedAt =
    report.generatedAt || options.assessment?.generatedAt || null;
  const assessment = options.assessment || null;
  const projectRoot = options.projectRoot || report.projectRoot || "";
  const executiveSummary = buildExecutiveSummaryBody(report, {
    ...options,
    assessment,
  });
  ...
  const commandsRun = _buildCommandsRun({
    ...options,
    assessment,
    company,
    client,
...
function compileReport(
  report,
  clientName,
  assessorName = "Simplebeacon Security Audit Service",
) {
