...
function generateBuildArtifactSection(findings = []) {
  ...
  for (const finding of findings.slice(0, 100)) {
    lines.push(
      `| \`${finding.path}\` | ${finding.reason} | ${finding.fileCount || 1} | ${formatBytes(finding.sizeBytes || 0)} | ${finding.action} |`,
    );
  }
...
function generateAssetConsolidationSection(findings = []) {
  ...
  for (const group of findings.slice(0, 50)) {
    ...
    lines.push(
      `- Duplicates: ${group.duplicates.map((p) => `\`${p}\``).join(", ")}`,
    );
    lines.push(
...
function generateUnusedFileSection(findings = [], entryPoints = []) {
  ...
  if (entryPoints.length) {
    lines.push(
      `Entry points preserved: ${entryPoints.map((p) => `\`${p}\``).join(", ")}`,
      "",
    );
  }
  if (!findings.length) {
  ...
  for (const finding of findings.slice(0, 100)) {
    lines.push(
      `| \`${finding.path}\` | ${finding.reason} | ${finding.confidence} | ${finding.action} |`,
    );
  }
  if (findings.length > 100) {
    lines.push("", `_…and ${findings.length - 100} more._`);
...
function generateGenericFindingSection(
  title,
  findings = [],
  columns = ["Path", "Reason", "Severity", "Action"],
) {
  const lines = [`## ${title}`, ""];
  if (!findings.length) {
  ...
  for (const finding of findings.slice(0, 100)) {
    lines.push(
      `| \`${finding.path}\` | ${finding.reason} | ${finding.severity || "low"} | ${finding.action} |`,
    );
  }
  if (findings.length > 100) {
    lines.push("", `_…and ${findings.length - 100} more._`);
...
function generateFileReductionReport(report, options = {}) {
  ...
  const sections = [
    ...
    generateUnusedFileSection(
      ...
      report.metadata?.entryPoints || [],
    ),
    generateGenericFindingSection(
      "Configuration Management",
      findings.configManagement,
    ),
    generateGenericFindingSection(
      "Dependency Health",
      findings.dependencyHealth,
    ),
    generateGenericFindingSection(
