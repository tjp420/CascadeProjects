...
function buildEnhanceContext(report, assessment, options = {}) {
  ...
  return {
    ...
    severityCounts,
    totalFiles:
      summary.filesScanned ?? report.totalFiles ?? report.filesAnalyzed ?? 0,
    rawIssues: rawIssues.slice(0, 24).map(normalizeRawIssue),
    failedRules,
    recommendedActions: (assessment?.recommendedActions?.immediate || []).slice(
      0,
      6,
    ),
  };
...
function parseEnhanceResponse(raw) {
  ...
  return {
    ...
    businessImpact: parsed.businessImpact
      ? String(parsed.businessImpact).trim()
      : "",
...
function assembleEnhancedExecutiveSummary(parts, report, options = {}) {
  ...
  const impactBlock = parts.businessImpact ? `${parts.businessImpact}\n\n` : "";
...
function callOpenAIChatCompletion(prompt, options = {}) {
  ...
  if (
    ...
    throw new Error(
      "OpenAI enhancement blocked in offline mode — --enhance requires network access. Remove --offline to use --enhance.",
    );
  }
    ...
    messages: [
      {
        role: "system",
        content:
          "You rewrite audit executive summaries for agency owners. Output JSON only. Never invent security findings.",
      },
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            reject(
              new Error(`OpenAI API ${res.statusCode}: ${raw.slice(0, 300)}`),
            );
            return;
          }
...
async function enhanceExecutiveSummary(
  markdown,
  report,
  assessment,
  options = {},
