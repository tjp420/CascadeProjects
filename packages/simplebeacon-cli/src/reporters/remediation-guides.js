...
const GUIDE_PLAYBOOKS = {
  credentials: {
    ...
    difficulty: "Moderate",
    whyItMatters:
      "Secrets in git history, logs, or error output can expose infrastructure before go-live. Rotation is required if a key was ever real.",
    steps: [
  ...
  "production-leak": {
    ...
    difficulty: "Moderate",
    whyItMatters:
      "Production modules that import `-sample.json` or `/mock/` paths ship demo metrics to real users at launch.",
    steps: [
  ...
  "fiction-kpi": {
    ...
    difficulty: "Easy",
    whyItMatters:
      "Placeholder completion rates and confidence scores inflate dashboards with numbers that were never measured in this repo.",
    steps: [
  ...
  schema: {
    ...
    difficulty: "Easy",
    whyItMatters:
      "Samples that miss required page-spec keys break dashboard rendering or fail validation in CI.",
    steps: [
  ...
  "npm-audit": {
    ...
    difficulty: "Easy",
    whyItMatters:
      "Moderate CVEs can escalate as new exploits are published; critical/high issues block safe production deploys.",
    steps: [
  ...
  "ci-integration": {
    ...
    difficulty: "Moderate",
    whyItMatters:
      "A PR gate stops mock leaks and credential patterns from reaching main after this audit is closed.",
    steps: [
  ...
  roadmap: {
    ...
    difficulty: "Easy",
    whyItMatters:
      "Low-severity template markers are informational only but can confuse stakeholders if mistaken for live status.",
    steps: [
...
function collectActiveGuideIds(issues, assessment) {
  ...
  if (
    failedChecks.includes("npm-no-critical-high") ||
    failedChecks.includes("npm-moderate-limit")
  ) {
    ids.add("npm-audit");
  }

  const gateFails =
    assessment?.executiveSummary?.gateResult === "FAIL" ||
...
function buildPersonalizedActionPlan(issues, assessment) {
  const guideIds = collectActiveGuideIds(issues, assessment).filter(
  ...
  if (guideIds.includes("credentials")) {
    week1.push({
      ...
      impact:
        "Clears critical security blockers and credential checklist failures",
    });
  }
  ...
  if (guideIds.includes("fiction-kpi") || guideIds.includes("schema")) {
    week2.push({
      ...
      minutes:
        estimateMinutes("fiction-kpi") +
        (guideIds.includes("schema") ? estimateMinutes("schema") : 0),
  ...
  const totalMinutes = [...week1, ...week2, ...week3].reduce(
