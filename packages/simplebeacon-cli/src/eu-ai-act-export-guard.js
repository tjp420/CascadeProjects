...
} = require("./eu-ai-act-legal-attestation.js");
/**
...
function evaluateEuExportEligibility(bundle = {}) {
  ...
  if (!classification?.riskTier) {
    errors.push({
      code: "EUAI-000",
      message: "Legal classification record missing (EUAI-000)",
    });
  }
  if (!isLegalReviewAttestation(attestation)) {
    errors.push({
      code: "EUAI-ATT",
      message:
        "legal_review_complete attestation required for client-facing EU export",
    });
  }
