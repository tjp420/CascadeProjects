...
class SupplyChainSecurityScanner {
  ...
  async scan(projectRoot, options = {}) {
    ...
    const packageJsonFiles = inventory.files.filter(
    ...
    for (const lockFile of lockFiles.slice(0, this.maxFilesToScan)) {
      ...
      for (const pkg of packages) {
        ...
        const typosquatTarget = isTyposquat(
          pkg.name,
          this.popularPackages,
          this.maxTyposquatDistance,
        );
        if (typosquatTarget) {
          findings.push({
    ...
    return {
      ...
      summary: {
        ...
        typosquatFindings: findings.filter(
          (f) => f.type === "supply-chain-typosquat",
        ).length,
        compromisedFindings: findings.filter(
          (f) => f.type === "supply-chain-compromised",
        ).length,
        suspiciousScriptFindings: findings.filter(
          (f) => f.type === "supply-chain-suspicious-script",
        ).length,
      },
    };
  }
