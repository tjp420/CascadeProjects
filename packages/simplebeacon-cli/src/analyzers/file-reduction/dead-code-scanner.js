...
const EXPORT_PATTERNS = [
  ...
  },
  {
    kind: "named-list",
    regex:
      /export\s*\{\s*([A-Za-z_$][A-Za-z0-9_$]*(?:\s*,\s*[A-Za-z_$][A-Za-z0-9_$]*)*)\s*\}/g,
  },
  {
    kind: "default",
    regex:
      /export\s+default\s+(?:function\s+|class\s+)?([A-Za-z_$][A-Za-z0-9_$]*)/g,
  },
  {
    kind: "default-anon",
    regex: /export\s+default\s+(?:function\s*\(|class\s*\{)/g,
  },
];
...
const IMPORT_PATTERNS_FOR_NAMES = [
  ...
  {
    kind: "star",
    regex: /import\s*\*\s+as\s+([A-Za-z_$][A-Za-z0-9_$]*)\s+from/g,
  },
...
class DeadCodeScanner {
  ...
  async scan(projectRoot, options = {}) {
    ...
    const filteredFindings = findings.filter((f) => {
      return (
        this.confidenceRank[f.confidence] >=
        this.confidenceRank[this.minConfidence]
      );
    });
    ...
    return {
      ...
      summary: {
        ...
        orphanedExports: findings.filter((f) => f.type === "orphaned-export")
          .length,
      },
    };
  }
