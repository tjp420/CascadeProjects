...
function isPublicPemPath(normalized) {
  return (
    ...
    /(?:public|live-public|-public)/i.test(normalized)
  );
}
...
function getRecentCommits(projectRoot, count = MAX_COMMITS_TO_SCAN) {
  try {
    const output = execSync(
      `git log --pretty=format:"%H%n%s%n%b%n---COMMIT_END---" -n ${count}`,
...
function getLargeBlobs(
  projectRoot,
  thresholdBytes = LARGE_BLOB_THRESHOLD_BYTES,
) {
  try {
    const output = execSync(
...
class GitHygieneScanner {
  constructor(config = {}) {
    this.largeFileThreshold =
      config.largeFileThreshold ?? LARGE_FILE_THRESHOLD_BYTES;
    this.largeBlobThreshold =
      config.largeBlobThreshold ?? LARGE_BLOB_THRESHOLD_BYTES;
    this.maxCommits = config.maxCommits ?? MAX_COMMITS_TO_SCAN;
    this.enabled = config.enabled !== false;
    this.excludedPaths = new Set(
      (config.excludedPaths || []).map((p) => p.replace(/\\/g, "/")),
    );
  ...
  async scan(projectRoot, options = {}) {
          ...
          if (!this.excludedCommitHashes.has(commit.hash.toLowerCase())) {
            findings.push({
              ...
              metadata: {
                ...
                subject: commit.subject.slice(0, 60),
              },
            });
          }
          break;
    ...
    return {
      ...
      summary: {
        isGitRepo: true,
        sensitiveFiles: findings.filter((f) => f.type === "git-sensitive-file")
          .length,
        largeFiles: findings.filter((f) => f.type === "git-large-file").length,
        largeBlobs: findings.filter((f) => f.type === "git-large-blob").length,
        suspiciousCommits: findings.filter(
          (f) => f.type === "git-sensitive-commit",
        ).length,
        commitsScanned,
        filesAnalyzed,
      },
