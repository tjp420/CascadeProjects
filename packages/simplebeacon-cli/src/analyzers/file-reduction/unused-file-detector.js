...
} = require('./utils/import-parser');
const {
  parseNonCodeReferences,
  addReference,
  parseWorkerScriptReferences,
} = require('./utils/file-reference-tracker');
...
class UnusedFileDetector {
  ...
  async scan(projectRoot, options = {}) {
    ...
    const orphanFiles = sourceFiles
      ...
      .filter(
        (file) => !entryPoints.some((entry) => path.resolve(entry) === path.resolve(file.path))
      )
    ...
    return {
      ...
      metadata: {
        entryPoints: entryPoints.map((entry) =>
          path.relative(inventory.root, entry).split(path.sep).join('/')
        ),
      },
    };
  }
  ...
  collectEntryPoints(inventory, graph) {
    ...
    for (const file of inventory.files) {
      ...
      if (/^(next|postcss|tailwind)\.config\.(js|mjs|ts|cjs)$/i.test(file.name)) {
        entries.add(file.path);
      }
      ...
      if (
        ...
        /(?:^|\/)website-51543\.html$|(?:^|\/)development-roadmap-51543\.html$/i.test(rel)
      ) {
        entries.add(file.path);
      }
      if (
        /(?:^|\/)src\/app\/(layout|page|loading|error|not-found|global-error|route)\.[jt]sx?$/.test(
          rel
        )
      ) {
        entries.add(file.path);
      ...
      if (
        /(?:^|\/)packages\/simplebeacon-cli\/(?:bin|src\/reporters|src\/rules)\//.test(rel) &&
    ...
    for (const node of graph.values()) {
      ...
      if (
        ...
        this.entryBasenames.has(path.basename(node.path).toLowerCase())
      ) {
  ...
  isCandidate(relativePath) {
    ...
    if (
      /^(credential-pattern-scanner|roadmap-json-specs|sample-consistency-checker)\.js$/.test(
        basename
      ) &&
