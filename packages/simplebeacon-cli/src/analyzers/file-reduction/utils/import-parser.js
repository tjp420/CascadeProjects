...
const PY_PATTERNS = [
  ...
  { kind: "py-from", regex: /^\s*from\s+([a-zA-Z0-9_.]+)\s+import/gm },
];
...
function parseJSImports(content, filePath, projectRoot) {
  ...
  for (const pattern of JS_PATTERNS) {
    for (const specifier of extractMatches(content, pattern.regex)) {
      const normalized = normalizeSpecifier(specifier);
      if (!isRelativeSpecifier(normalized) && !normalized.startsWith("@/"))
        continue;
...
const RUNTIME_REFERENCE_PATTERNS = [
  ...
  {
    kind: "fs-readFile",
    regex: /fs\.(?:promises\.)?readFile(?:Sync)?\s*\(\s*['"`]([^'"`]+)['"`]/g,
  },
  { kind: "readFileSync", regex: /readFileSync\s*\(\s*['"`]([^'"`]+)['"`]/g },
  {
    kind: "createReadStream",
    regex: /createReadStream\s*\(\s*['"`]([^'"`]+)['"`]/g,
  },
];
