...
const WORKER_MULTI_PATTERN =
  /path\.join\s*\(\s*__dirname\s*((?:\s*,\s*['"][^'"]+['"])+\s*)\)/g;
const PACKAGE_REL_PATTERN =
  /path\.join\s*\(\s*[^,)]+,\s*((?:['"][^'"]+['"]\s*,?\s*)+)\)/g;
const SPAWN_PATTERN =
  /(?:spawn|spawnSync|execFile(?:Sync)?)\s*\(\s*['"]([^'"]+\.py)['"]/g;
...
function resolveStaticSiteAbsolutePath(fromFile, specifier, projectRoot) {
  ...
  for (const rootName of siteRoots) {
    ...
    if (
      normalizePathKey(candidate).startsWith(rootNorm) &&
      fs.existsSync(candidate)
    ) {
      return candidate;
    }
...
function parseWorkerScriptReferences(filePath, content, projectRoot) {
  ...
  forEachMatch(content, WORKER_MULTI_PATTERN, (multiMatch) => {
    const segments = [...multiMatch[1].matchAll(/['"]([^'"]+)['"]/g)].map(
      (part) => part[1],
    );
  ...
  forEachMatch(content, PACKAGE_REL_PATTERN, (packageMatch) => {
    const segments = [...packageMatch[1].matchAll(/['"]([^'"]+)['"]/g)].map(
      (part) => part[1],
    );
    if (!segments.length) return;
    const resolvedPath = path.resolve(fromDir, ...segments);
