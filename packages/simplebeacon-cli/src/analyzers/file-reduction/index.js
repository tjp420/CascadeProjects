...
} = require("../../lib/cross-analyzer-intelligence");
...
async function runFileReductionAnalysis(projectRoot, options = {}) {
  ...
  const enabledScanners = DEFAULT_SCANNERS.filter((entry) => {
  ...
  for (const entry of enabledScanners) {
    ...
    results[entry.id] = await scanner.scan(projectRoot, {
      ...options,
      inventory,
    });
  }
  ...
  const rawFindings = Object.values(results).flatMap(
  ...
  const totalProjectBytes = inventory.files.reduce(
    (sum, f) => sum + (f.size || 0),
    0,
  );
  ...
  const findings = Object.fromEntries(
    DEFAULT_SCANNERS.map((s) => [
      kebabToCamel(s.id),
      results[s.id]?.findings || [],
    ]),
  );
  const summaryCounts = Object.fromEntries(
    DEFAULT_SCANNERS.map((s) => [
      `${kebabToCamel(s.id)}Findings`,
      (results[s.id]?.findings || []).length,
    ]),
  );
...
module.exports = Object.freeze({
  ...
  DEFAULT_SCANNERS,
  fileReductionRules,
  BuildArtifactScanner: fileReductionRules.scanners.find(
    (s) => s.id === "build-artifacts",
  )?.class,
  AssetConsolidationScanner: fileReductionRules.scanners.find(
    (s) => s.id === "asset-consolidation",
  )?.class,
  UnusedFileDetector: fileReductionRules.scanners.find(
    (s) => s.id === "unused-files",
  )?.class,
  walkProjectFiles,
});
