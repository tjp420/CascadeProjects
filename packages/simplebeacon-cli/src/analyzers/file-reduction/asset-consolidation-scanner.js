...
const PACKAGED_FAVICON_PATTERN =
  /(?:^|\/)cloudflare-deploy\/favicon\.(svg|ico)$/i;

class AssetConsolidationScanner {
  constructor(config = {}) {
    this.assetExtensions = new Set(
      (config.assetExtensions || assetConfig.assetExtensions).map((ext) =>
        ext.toLowerCase(),
      ),
    );
    this.maxFileSizeForHash =
      config.maxFileSizeForHash || assetConfig.maxFileSizeForHash;
  ...
  async scan(projectRoot, options = {}) {
    ...
    const hashed = await hashFiles(assetFiles, {
      maxBytes: this.maxFileSizeForHash,
    });
    const duplicateGroups = groupByHash(hashed).filter(
      (group) => group.length >= this.minGroupSize,
    );
