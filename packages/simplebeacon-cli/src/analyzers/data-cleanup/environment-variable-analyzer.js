  ...
  async scan(projectRoot, options = {}) {
    ...
    for (const file of envFiles) {
      ...
      for (const hit of scanTextContent(file.relativePath, content)) {
        if (isLocalEnvFile(file.relativePath)) continue;
        if (isExampleEnvFile(file.relativePath)) {
          const lineValue = (lines[(hit.line || 1) - 1] || "")
            .split("=")
            .slice(1)
            .join("=")
            .trim();
          if (isPlaceholderEnvValue(lineValue)) continue;
        }
        findings.push({
          type: "env-secret",
    ...
    for (const [key, refs] of referencedKeys.entries()) {
      ...
      const productionRefs = [...refs].filter(
        (ref) => !isNonProductionSourcePath(ref),
      );
      if (!productionRefs.length) continue;
      const defined = [...envKeys.keys()].some((bucketKey) =>
        bucketKey.endsWith(`::${key}`),
      );
      if (defined) continue;
      findings.push({
    ...
    for (const key of definedKeys) {
      ...
      if (isPlannedEnvKey(key)) continue;
      const bucketKey = [...envKeys.keys()].find((candidate) =>
      ...
      if (
        sample.every(
          (entry) =>
            isPhase2ExampleEnvFile(entry.file) || isExampleEnvFile(entry.file),
        )
      )
        continue;
      findings.push({
    ...
    return {
      ...
      summary: {
        ...
        missingKeys: findings.filter((f) => f.type === "missing-env-key")
          .length,
