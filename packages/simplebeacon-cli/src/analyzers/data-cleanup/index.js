...
function defineScanner(id, modulePath, options = {}) {
    ...
    if (typeof id !== 'string' || id.length === 0) {
        throw new TypeError(`Scanner id must be a non-empty string, got: ${typeof id}`);
    }
    if (typeof priority !== 'number' || !Number.isFinite(priority)) {
        throw new TypeError(`Scanner priority must be a finite number, got: ${priority}`);
    }
...
function loadScanner(entry) {
    ...
    try {
        ...
        throw new Error(`Failed to load scanner "${entry.id}" from "${entry.modulePath}": ${err.message}`);
    }
}
...
[
    { id: 'config-management', path: './config-management-analyzer', priority: 4 },
    { id: 'dependency-health', path: './dependency-health-analyzer', priority: 5 },
    { id: 'environment-variables', path: './environment-variable-analyzer', priority: 6 },
    { id: 'data-freshness', path: './data-freshness-analyzer', priority: 7 },
    { id: 'data-access-patterns', path: './data-access-pattern-analyzer', priority: 8 },
...
].forEach(def => defineScanner(def.id, def.path, { priority: def.priority, enabled: true, lazy: true }));
