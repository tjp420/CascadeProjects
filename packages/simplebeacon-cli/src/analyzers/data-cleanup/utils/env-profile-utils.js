...
function isExampleEnvFile(relativePath) {
  const base = path.posix
    .basename(normalizeEnvRelativePath(relativePath))
    .toLowerCase();
...
function isPlaceholderEnvValue(value) {
  ...
  return (
    /^(replace|your_|sk_test_|sk_live_|rk_live_|whsec_|price_|\.\.\.|changeme|dummy|placeholder|test-token)/i.test(
      trimmed,
    ) ||
...
function shouldSkipEnvInconsistency(key, values) {
  ...
  const allExample = files.every(
    (file) => isExampleEnvFile(file) || isTemplateEnvFile(file),
  );
  ...
  if (hasExample && hasLive) {
    if (
      /^(JWT_|.*_SECRET|.*_PASSWORD|STRIPE_|.*_KEY|DB_PASSWORD|DATABASE_URL)/i.test(
        key,
      )
    ) {
      return true;
...
function isPhase2ExampleEnvFile(relativePath) {
  const base = path.posix
    .basename(normalizeEnvRelativePath(relativePath))
    .toLowerCase();
  return base.includes(".env.example.phase2") || base.includes(".env.phase2");
}
...
function isRuntimeInjectedEnvKey(key) {
  return (
    /^(CI|NODE_ENV|FORCE_COLOR|NO_COLOR|DOTENV_CONFIG_PATH|npm_lifecycle_event|npm_node_execpath)$/i.test(
      key,
    ) ||
    /^GITHUB_/i.test(key) ||
    /^npm_config_/i.test(key) ||
    /^(USERPROFILE|HOME|HOMEDRIVE|HOMEPATH|APPDATA|LOCALAPPDATA|TEMP|TMP|PATH|PATHEXT|OS|COMPUTERNAME|USERNAME)$/i.test(
      key,
    )
  );
}
...
function isNonProductionSourcePath(relativePath) {
  ...
  return (
    /(^|\/)(tests?|__tests__|fixtures?|docs?|examples?)(\/|$)/.test(rel) ||
...
module.exports = {
  ...
  isPlannedEnvKey,
  isOptionalEnvKeyWithCodeDefault: (key) =>
    OPTIONAL_ENV_KEYS_WITH_CODE_DEFAULTS.has(key),
