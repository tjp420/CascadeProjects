...
const CONFIG_PATTERNS = [
  ...
  },
  {
    id: "bundler-config",
    match: (name) =>
      /^(webpack|vite|rollup|esbuild|parcel)\.config\./i.test(name),
    label: "Bundler config",
  },
  {
    id: "test-config",
    match: (name) =>
      /^(jest|vitest|playwright|cypress)\.config\./i.test(name) ||
      name === "jest.config.js",
    label: "Test runner config",
  },
  {
    id: "ts-config",
    match: (name) => /^tsconfig.*\.json$/i.test(name),
...
class ConfigManagementAnalyzer {
  async scan(projectRoot, options = {}) {
    ...
    const operationalEnvFiles = envFiles.filter(
      (file) =>
        !/\.example|\.template|\.sample|env-sample|env_example/i.test(
          file.name,
        ),
    );
    const envDirs = new Set(
      operationalEnvFiles.map((f) =>
        f.relativePath.replace(/\\/g, "/").split("/").slice(0, -1).join("/"),
      ),
    );
    if (operationalEnvFiles.length > 3 && envDirs.size <= 1) {
      findings.push({
        ...
        metadata: {
          files: operationalEnvFiles.map((file) => file.relativePath),
        },
      });
    }
      ...
      if (category === "bundler-config") {
        ...
        for (const file of files) {
          try {
            ...
            else if (/target\s*:\s*['"]web['"]/.test(content))
              targets.add("web");
            else if (/server\s*\{/.test(content) || /proxy\s*\{/.test(content))
              targets.add("web");
    ...
    return {
      ...
      summary: {
        ...
        sprawlFindings: findings.filter((f) => f.type === "config-sprawl")
          .length,
        duplicateConfigTypes: findings.filter(
          (f) => f.type === "duplicate-config-type",
        ).length,
        inconsistentEnvKeys: findings.filter(
          (f) => f.type === "env-inconsistency",
        ).length,
        unusedConfigs: findings.filter((f) => f.type === "unused-config")
          .length,
      },
    };
  }
    ...
    for (const config of configFiles) {
      ...
      if (!referenced) {
        findings.push({
          ...
          reason:
            "Config file is not referenced by any scanned source or config file",
          severity: "low",
          confidence: "medium",
          action: "remove-or-archive",
          metadata: {
            configType:
              CONFIG_PATTERNS.find((entry) => entry.match(config.name))?.id ||
              "unknown",
          },
        });
      }
