...
const CONFIG_PATTERNS = [
  ...
  {
    id: 'bundler-config',
    match: (name) => /^(webpack|vite|rollup|esbuild|parcel)\.config\./i.test(name),
    label: 'Bundler config',
  },
  {
    id: 'test-config',
    match: (name) =>
      /^(jest|vitest|playwright|cypress)\.config\./i.test(name) || name === 'jest.config.js',
...
class ConfigManagementAnalyzer {
  async scan(projectRoot, options = {}) {
    ...
    const configFiles = filterWorkspaceFiles(
      inventory.files.filter((file) => CONFIG_PATTERNS.some((pattern) => pattern.match(file.name)))
    );
    ...
    for (const [category, files] of byCategory.entries()) {
      ...
      if (category === 'bundler-config') {
        const dirs = new Set(
          files.map((f) => f.relativePath.replace(/\\/g, '/').split('/').slice(0, -1).join('/'))
        );
    ...
    const pkgDirs = new Set(
      packageJsons.map((f) => f.relativePath.replace(/\\/g, '/').split('/').slice(0, -1).join('/'))
    );
