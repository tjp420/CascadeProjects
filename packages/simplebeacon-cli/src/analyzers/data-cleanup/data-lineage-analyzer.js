...
} = require("../file-reduction/utils/file-reference-tracker");
const {
  parseImports,
  parseRuntimeReferences,
} = require("../file-reduction/utils/import-parser");
...
function prioritizeSourceFiles(files, maxFiles) {
  ...
  for (const file of files) {
    ...
    if (
      SOURCE_SCAN_PRIORITY_PREFIXES.some((prefix) => rel.startsWith(prefix))
    ) {
...
class DataLineageAnalyzer {
  ...
  async scan(projectRoot, options = {}) {
    ...
    const dataFiles = inventory.files.filter(
      (file) => isDataFile(file) && !isVendorPath(file.relativePath),
    );
    ...
    for (const sourceFile of sourceFiles) {
      ...
      for (const ref of parseNonCodeReferences(
        sourceFile.path,
        content,
        inventory.root,
      )) {
        ...
        const rel = path
          .relative(inventory.root, ref.resolvedPath)
          .split(path.sep)
          .join("/");
      ...
      for (const imp of parseImports(
        ...
        const rel = path
          .relative(inventory.root, imp.resolvedPath)
          .split(path.sep)
          .join("/");
        if (consumers.has(rel)) {
          consumers.get(rel).add(sourceFile.relativePath);
      ...
      for (const ref of parseRuntimeReferences(
        sourceFile.path,
        content,
        inventory.root,
      )) {
        ...
        for (const dataFile of dataFiles) {
          ...
          if (
            ...
            (ref.resolvedPath &&
              path.normalize(ref.resolvedPath) ===
                path.normalize(dataFile.path)) ||
    ...
    for (const dataFile of dataFiles) {
      ...
      if (
        refs.length === 0 &&
        !isLineageAllowlisted(rel, this.allowlistPatterns)
      ) {
        findings.push({
          type: "orphaned-data",
          path: rel,
