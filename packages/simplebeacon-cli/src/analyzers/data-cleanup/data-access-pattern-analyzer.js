...
function isAsyncReadPattern(content, matchIndex) {
  ...
  return (
    /\bawait\s+(?:fs\.promises\.)?readFile\b/.test(snippet) || /\bawait\s+readFile\b/.test(snippet)
  );
}
    ...
    for (const file of sourceFiles) {
      ...
      for (const pattern of ACCESS_PATTERNS) {
        ...
        if (
          ...
          (isLazyCachedSyncRead(content, match.index) || isAsyncReadPattern(content, match.index))
        )
          continue;
        findings.push({
          type: 'data-access-pattern',
