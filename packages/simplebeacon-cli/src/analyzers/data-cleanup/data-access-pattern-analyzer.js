...
function isLazyCachedSyncRead(content, matchIndex) {
  ...
  return (
    /\bif\s*\(\s*!cached[A-Za-z0-9_]*\s*\)/.test(snippet) ||
...
function isAsyncReadPattern(content, matchIndex) {
  ...
  return (
    /\bawait\s+(?:fs\.promises\.)?readFile\b/.test(snippet) ||
...
const SQL_KEYWORDS =
  /\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|GRANT|REVOKE|FROM|WHERE|JOIN|TABLE)\b/i;
const DB_CALLEE_NAMES =
  /\b(query|execute|exec|run|all|prepare|raw|queryRow|queryRows|statement|sql|get|each)\b/i;
    ...
    if (node.type === "BinaryExpression" && node.operator === "+") {
      ...
      if (
        (left.type === "StringLiteral" &&
          isSqlString(left.value) &&
          right.type !== "StringLiteral") ||
        (right.type === "StringLiteral" &&
          isSqlString(right.value) &&
          left.type !== "StringLiteral")
      ) {
        return true;
      }
    ...
    CallExpression(pathNode) {
      ...
      if (isDynamicSql(firstArg)) {
        push(
          ...
          "Dynamic SQL passed to database query method — use parameterized queries",
        );
      }
    },
  });
...
const ACCESS_PATTERNS = [
  {
    ...
    regex:
      /\.(?:map|forEach|filter|reduce)\s*\([\s\S]{0,500}?readFile(?:Sync)?\s*\(/g,
  ...
  },
  {
    ...
    regex:
      /(?:router|app)\.(?:get|post|put|delete|patch)\s*\([\s\S]{0,700}?readFileSync\s*\(/g,
...
class DataAccessPatternAnalyzer {
  constructor(config = {}) {
    this.sourcePrefixes = config.sourcePrefixes || [
      "server/",
      "src/",
      "packages/",
      "lib/",
  ...
  async scan(projectRoot, options = {}) {
    ...
    const sourceFiles = inventory.files
      ...
      .filter((file) =>
        this.sourcePrefixes.some((prefix) =>
    ...
    for (const file of sourceFiles) {
      ...
      for (const pattern of ACCESS_PATTERNS) {
        ...
        if (
          pattern.id === "parse-sync-read" &&
          (isLazyCachedSyncRead(content, match.index) ||
            isAsyncReadPattern(content, match.index))
        )
          continue;
        findings.push({
          type: "data-access-pattern",
