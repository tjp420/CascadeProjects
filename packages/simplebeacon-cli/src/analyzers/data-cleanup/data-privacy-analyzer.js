...
function isDocumentationPath(relativePath) {
  ...
  return (
    /^docs\//i.test(normalized) || /MOCK_DATA_GUIDE\.md$/i.test(normalized)
  );
}
    ...
    for (const pattern of PII_PATTERNS) {
      ...
      while (match) {
        const snippet = line
          ...
          .toLowerCase();
        if (
          pattern.allowSubstrings.some((allowed) =>
...
function isDockerComposeFile(relativePath) {
  const base = path.posix
    .basename(String(relativePath || "").replace(/\\/g, "/"))
    .toLowerCase();
  return base.startsWith("docker-compose");
}

function isIntentionalTestFixturePath(relativePath) {
...
function isProtectedSampleDataPath(relativePath) {
  ...
  return (
    /(?:^|\/)web\/data\//.test(rel) ||
    /(?:^|\/)data\/mock\//.test(rel) ||
    /(?:^|\/)data-central\//.test(rel) ||
    /-sample\.json$/i.test(rel)
  );
}

function isIntentionalMockPath(relativePath) {
  const rel = String(relativePath || "")
    .replace(/\\/g, "/")
    .toLowerCase();
  return (
    /(?:^|\/)web\/api\/mock-backend\.js$/.test(rel) ||
    /(?:^|\/)tools\/generate-sample-certificate\.js$/.test(rel)
  );
}
...
class DataPrivacyAnalyzer {
  async scan(projectRoot, options = {}) {
    ...
    const targets = inventory.files.filter(
      (file) =>
        SCANNABLE_EXTENSIONS.has(file.ext) &&
        (isDataFile(file) || isDataPath(file.relativePath)),
    );

    const findings = [];
    for (const file of targets) {
      if (file.size > 512000) continue;
      if (isDockerComposeFile(file.relativePath)) continue;
      if (isIntentionalTestFixturePath(file.relativePath)) continue;
      if (isProtectedSampleDataPath(file.relativePath)) continue;
      if (isIntentionalMockPath(file.relativePath)) continue;
      let content = "";
      try {
      ...
      for (const hit of scanTextContent(
        file.relativePath,
        contentForCredentialScan(file.relativePath, content),
      )) {
        findings.push({
          type: "data-privacy",
    ...
    return {
      ...
      summary: {
        ...
        credentialHits: deduped.filter((f) => f.reason.includes("Credential"))
          .length,
