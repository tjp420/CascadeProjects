...
const PII_PATTERNS = [
    {
        ...
        allowSubstrings: ['example.com', 'example.org', 'test.com', 'simplebeacon.ai', 'cascade.ai', 'localhost', 'user@domain']
    },
    {
        id: 'ssn-pattern',
...
function isDockerComposeFile(relativePath) {
    const base = path.posix.basename(String(relativePath || '').replace(/\\/g, '/')).toLowerCase();
...
function isIntentionalMockPath(relativePath) {
    const rel = String(relativePath || '').replace(/\\/g, '/').toLowerCase();
    return /(?:^|\/)web\/api\/mock-backend\.js$/.test(rel)
...
class DataPrivacyAnalyzer {
    async scan(projectRoot, options = {}) {
        ...
        for (const file of targets) {
            ...
            for (const hit of scanTextContent(file.relativePath, contentForCredentialScan(file.relativePath, content))) {
                findings.push({
                    type: 'data-privacy',
