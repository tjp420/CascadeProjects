...
const {
  ...
  isWorkspacePath,
} = require("./utils/workspace-path-utils");
...
function prioritizeDependencyScanFiles(files, maxFiles = 1500) {
  ...
  for (const file of files) {
    ...
    if (
      /(?:^|\/)(tools|scripts|bin|packages\/simplebeacon-cli\/bin)\//.test(rel)
    ) {
      priority.push(file);
    } else {
      rest.push(file);
...
class DependencyHealthAnalyzer {
  async scan(projectRoot, options = {}) {
    ...
    for (const file of packageFiles) {
      if (SKIP_PACKAGE_JSON_PATHS.some((re) => re.test(file.relativePath)))
        continue;
      let pkg;
      try {
      ...
      const usedPackages = await this.collectUsedPackagesForPackage(
        file.path,
        inventory,
      );
      const isNextApp = declared.has("next");
    ...
    for (const [name, entries] of dependencyIndex.entries()) {
      ...
      if (allRanges) {
        const majorVersions = new Set(
          versions.map((v) => v.replace(/^[\^~]/, "").split(".")[0]),
        );
    ...
    return {
      ...
      summary: {
        ...
        unusedDependencies: findings.filter(
          (f) => f.type === "unused-dependency",
        ).length,
        duplicateDependencies: findings.filter(
          (f) => f.type === "duplicate-dependency",
        ).length,
  ...
  isLikelyToolingDependency(name, section) {
    if (section === "devDependencies") {
      return /^(eslint|jest|vitest|supertest|typescript|prettier|husky|nodemon|webpack|vite|rollup|postcss|tailwindcss|autoprefixer|@types\/)/.test(
        name,
      );
    }
    return false;
  ...
  isNextFrameworkDependency(name) {
    return [
      "react",
      "react-dom",
      "postcss",
