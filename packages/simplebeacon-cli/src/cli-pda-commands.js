...
function runAgentCommand(options) {
  ...
  switch (sub) {
    ...
    default:
      console.error(
        "Usage: simplebeacon agent [register|list|detect] [--name <name>] [--type <type>]",
      );
      return 2;
  }
}
...
function runTaskCommand(options) {
  ...
  switch (sub) {
    ...
    case "create": {
      ...
      const task = pda.createTask(
        root,
        agentId,
        options.title || options._args?.[1] || "Untitled",
        {
    ...
    default:
      console.error(
        "Usage: simplebeacon task [add|list|update|complete|block|approvals]",
      );
      return 2;
  }
}
...
function runPolicyCommand(options) {
  ...
  switch (sub) {
    ...
    case "add": {
      ...
      if (!type || !action) {
        console.error("Type and action required");
        return 2;
      }
    ...
    case "remove": {
      ...
      if (!policyId) {
        console.error("Policy ID required");
        return 2;
      }
...
function runGateFinalizeCommand(options) {
  ...
  if (result.canFinalize) {
    ...
    if (result.gateResult && !result.gateResult.pass) {
      console.error(
        `  Gate scan: ${result.gateResult.blockingCount} blocking findings`,
      );
    }
    return 1;
  }
...
function runLearnCommand(options) {
  ...
  const projects = pda.collectProjectData(roots, {
    maxDepth,
