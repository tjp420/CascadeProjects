...
function getGateStatus(projectRoot) {
  ...
  try {
    ...
    const pass = gateObj
      ? gateObj.pass === true
      : report.gate === "pass" || report.gatePass === true;
    const blockingCount = gateObj
      ? gateObj.blockingCount || 0
      : report.blockingCount || report.criticalCount || 0;
    return {
      pass,
      blockingCount,
      qualityScore: report.qualityScore || 0,
...
function canFinalize(projectRoot, agentId, opts = {}) {
  ...
  const hasMustScan =
    policyResult.violations.some((v) => v.checkCommand) ||
  ...
  if (hasMustScan) {
    ...
    const shouldCheckGate =
      opts.runScan !== false || opts.useExistingReport === true;
    ...
    if (shouldCheckGate) {
      ...
      if (gateResult.pass) {
        ...
        const blockingScanPolicy = policyResult.violations.find(
          (v) => v.checkCommand,
        );
        if (blockingScanPolicy) {
          result.canFinalize = false;
          result.blockingCount +=
            gateResult.blockingCount > 0 ? gateResult.blockingCount : 1;
        }
      }
    }
  }
