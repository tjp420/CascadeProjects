...
const POLICY_TYPES = ['forbidden_action', 'required_check', 'approval_required'];
...
function getPoliciesPath(projectRoot) {
    return path.join(projectRoot || process.cwd(), '.simplebeacon', 'policies.json');
}
...
function checkAction(projectRoot, action, context = {}) {
    ...
    return { allowed, violations, warnings, approvalsNeeded, blocked, needsApproval };
}
...
function matchesAction(policy, action, context) {
    ...
    if (context.branch && policy.branch) {
        if (Array.isArray(policy.branch) && policy.branch.includes(context.branch)) return true;
        if (typeof policy.branch === 'string' && policy.branch === context.branch) return true;
    }
    return false;
}
...
function addPolicy(projectRoot, policy) {
    ...
    if (!POLICY_TYPES.includes(policy.type)) {
        throw new Error(`Invalid policy type: ${policy.type}. Must be one of: ${POLICY_TYPES.join(', ')}`);
    }
