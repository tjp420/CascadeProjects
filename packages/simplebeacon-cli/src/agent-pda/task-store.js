...
function load(dbPath) {
    try {
        ...
        if (!data.tasks || !Array.isArray(data.tasks)) return { tasks: [], version: 1 };
        return data;
    } catch {
...
function blockTask(projectRoot, taskId, reason) {
    return updateTask(projectRoot, taskId, { status: 'blocked', blockReason: reason });
}
...
function listTasks(projectRoot, agentId, status) {
    ...
    return data.tasks.filter(t => {
        ...
        const statusOrder = { in_progress: 0, blocked: 1, pending: 2, completed: 3, cancelled: 4 };
...
function getPendingApprovals(projectRoot) {
    const dbPath = getDbPath(projectRoot);
    const data = load(dbPath);
    return data.tasks.filter(t => t.approvalRequired && !t.approvedBy && t.status !== 'cancelled');
}
