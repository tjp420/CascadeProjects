...
function _readProjectData(projectRoot, sbDir) {
  ...
  if (fs.existsSync(pdaDir)) {
    ...
    if (fs.existsSync(agentsFile)) {
      try {
        ...
        data.agentCount = Array.isArray(agents)
          ? agents.length
    ...
    if (fs.existsSync(tasksFile)) {
      try {
        ...
        data.taskCount = Array.isArray(tasks)
          ? tasks.length
    ...
    if (fs.existsSync(memFile)) {
      try {
        ...
        data.memoryCount = Array.isArray(mems)
          ? mems.length
  ...
  if (fs.existsSync(policyFile)) {
    try {
      ...
      data.policyCount = Array.isArray(policies)
        ? policies.length
...
function extractPatterns(projects) {
  ...
  for (const p of projects) {
    ...
    if (p.profile) {
      metrics.projectsByProfile[p.profile] =
        (metrics.projectsByProfile[p.profile] || 0) + 1;
    }

    for (const [type, count] of Object.entries(p.findingTypes)) {
      metrics.topFindingTypes[type] =
        (metrics.topFindingTypes[type] || 0) + count;
    }
    for (const [rule, count] of Object.entries(p.rulesTriggered)) {
      metrics.topRulesTriggered[rule] =
        (metrics.topRulesTriggered[rule] || 0) + count;
    }
  }

  // Pattern 1: Archived/disabled files still blocking the gate
  const blockedByArchive = projects.filter(
    (p) => p.gatePass === false && p.blockingCount > 0 && p.findingCount === 0, // blockers come from gate-only rules, not findings
  );
  if (blockedByArchive.length > 0) {
    patterns.push({
      ...
      description:
        "Files that have been moved to archive folders or renamed with DISABLED_/.bak prefixes still get scanned and block the gate. The files are not active code — they are leftovers from previous fixes.",
      projectsAffected: blockedByArchive.map((p) => p.projectRoot),
      occurrences: blockedByArchive.length,
      universalRule:
        "Gate scanner should exclude files in archive/, disabled/, .bak, and similar paths by default. Add a .simplebeaconignore pattern for common archive conventions.",
  ...
  const stubProjects = projects.filter(
    (p) => p.findingTypes["empty-stub-function"] > 0,
  );
  if (stubProjects.length > 0) {
    const totalStubs = stubProjects.reduce(
      (sum, p) => sum + p.findingTypes["empty-stub-function"],
      0,
    );
    patterns.push({
      ...
      description:
        "AI coding agents generate placeholder functions that return nothing or return hardcoded values. These pass syntax checks but fail at runtime. The pattern appears across web apps, CLI tools, and game mods.",
      projectsAffected: stubProjects.map((p) => p.projectRoot),
      occurrences: totalStubs,
      universalRule:
        "Gate should flag any function with an empty body or that returns a hardcoded value without processing input. This is language-agnostic — applies to JS, TS, ZScript, Python, Go, etc.",
      severityImpact:
        "medium — not a crash, but indicates incomplete implementation",
    });
  }

  // Pattern 3: Duplicate basenames across project structure
  const dupProjects = projects.filter(
    (p) => p.findingTypes["duplicate-basename"] > 0,
  );
  if (dupProjects.length > 0) {
    patterns.push({
      ...
      description:
        "Multiple files with the same basename exist in different directories. This causes import confusion, build conflicts, and makes it hard for AI agents to know which file to edit.",
      projectsAffected: dupProjects.map((p) => p.projectRoot),
      occurrences: dupProjects.reduce(
        (sum, p) => sum + p.findingTypes["duplicate-basename"],
        0,
      ),
      universalRule:
        "Gate should warn when two files in the same project have the same basename but different paths. AI agents should be warned before editing to confirm they have the right file.",
      severityImpact: "low — warning, but prevents agent confusion",
    });
  }

  // Pattern 4: Backup/fixture files in scan path
  const backupProjects = projects.filter(
    (p) => p.findingTypes["backup-or-fixture"] > 0,
  );
  if (backupProjects.length > 0) {
    patterns.push({
      ...
      description:
        "Files with .bak, .old, .backup, or fixture/ in the path are being scanned as if they are production code. These inflate finding counts and can block the gate with issues that do not affect runtime.",
      projectsAffected: backupProjects.map((p) => p.projectRoot),
      occurrences: backupProjects.reduce(
        (sum, p) => sum + p.findingTypes["backup-or-fixture"],
        0,
      ),
      universalRule:
        "Add .bak, .old, .backup, fixture/, fixtures/, test-fixtures/ to default .simplebeaconignore. Gate should skip these by default.",
      severityImpact: "low — noise reduction, prevents false blockers",
    });
  }

  // Pattern 5: License header issues
  const licenseProjects = projects.filter(
    (p) => p.findingTypes["licensed-under"] > 0,
  );
  if (licenseProjects.length > 0) {
    patterns.push({
      ...
      description:
        'The "licensed-under" rule fires on standard license text in source files. This is the highest-volume finding type across projects, suggesting the rule is too broad.',
      projectsAffected: licenseProjects.map((p) => p.projectRoot),
      occurrences: licenseProjects.reduce(
        (sum, p) => sum + p.findingTypes["licensed-under"],
        0,
      ),
      universalRule:
        "Narrow the licensed-under rule to only flag files that claim a different license than the project LICENSE file. Standard MIT/Apache/GPL headers should be allowlisted when they match the project license.",
      severityImpact:
        "low — noise reduction, 85+ false positives in one project",
    });
  }
  ...
  if (Object.keys(profilePassRates).length > 0) {
    patterns.push({
      ...
      description:
        "Different project profiles (gamedev, eu-ai-act, complete, minimal) have different gate pass rates. This indicates that some profiles have rules that are too strict or too loose for their target project type.",
      data: profilePassRates,
      universalRule:
        "Review profiles with low pass rates — rules may need narrowing. Review profiles with 100% pass rates — rules may be too loose.",
  ...
  if (pdaProjects.length > 0 || projects.length > 0) {
    patterns.push({
      ...
      description:
        "How many projects have actually initialized PDA state (agents, tasks, memories). Low adoption means agents are not using the PDA even when the MCP server is available.",
      data: {
        totalProjects: projects.length,
        pdaEnabled: pdaProjects.length,
        adoptionRate:
          projects.length > 0
            ? ((pdaProjects.length / projects.length) * 100).toFixed(1) + "%"
            : "0%",
      ...
      universalRule:
        "If adoption is low, the MCP server may not be wired to all agents. If adoption is high but task/memory counts are low, agents have the tools but are not using them.",
...
function _generateRecommendations(patterns, _metrics) {
  ...
  const profilePattern = patterns.find(
    (p) => p.id === "gate-pass-rate-by-profile",
  );
  if (profilePattern) {
  ...
  if (pdaPattern && pdaPattern.data.adoptionRate !== "100.0%") {
    recs.push({
      ...
      action:
        "Wire MCP config to more agents and run init --starter in projects without PDA state",
...
function generateReport(analysis) {
  ...
  lines.push("");
  const sortedTypes = Object.entries(metrics.topFindingTypes).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  for (const r of recommendations) {
    lines.push(
      `- **[${r.priority.toUpperCase()}]** [${r.category}] ${r.action}`,
    );
  ...
  lines.push("");
  lines.push(
    "1. **Gate rules** — patterns with high severity impact become new rules or tightened rules",
  );
  lines.push(
    "2. **Noise reduction** — patterns with high volume but low severity get narrowed or excluded",
  );
  lines.push(
    "3. **Profile tuning** — profiles with low pass rates get their rules reviewed",
  );
  lines.push(
  ...
  lines.push("");
  lines.push(
    "Each round of analysis makes the gate smarter for ALL projects, not just one.",
  );
