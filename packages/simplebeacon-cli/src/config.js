...
function resolveScanPaths(baseDir, config, extraPaths = []) {
  ...
  const resolved = relativePaths.map((rel) => resolvePathFromBase(baseDir, rel)).filter(Boolean);
...
function loadSimplebeaconConfig(baseDir, configPath = null) {
  ...
  const profile = fileConfig.profile || (isCascadeMonorepo(root) ? 'cascade' : 'standard');
