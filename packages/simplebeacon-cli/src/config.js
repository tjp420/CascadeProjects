    ...
    if (config.rules) {
      for (const [ruleName, userRule] of Object.entries(config.rules)) {
        ...
        if (
          ...
          FREE_RULE_ENGINES.has(ruleName) &&
          !profileRules[ruleName]
        ) {
        ...
        if (userRule && profileRules[ruleName]) {
          if (
            Array.isArray(userRule.ignoreGlobs) &&
            userRule.ignoreGlobs.length
          ) {
            profileRules[ruleName] = {
              ...profileRules[ruleName],
              ignoreGlobs: userRule.ignoreGlobs,
            };
          }
          if (
            Array.isArray(userRule.allowlistFiles) &&
            userRule.allowlistFiles.length
          ) {
            profileRules[ruleName] = {
              ...profileRules[ruleName],
              allowlistFiles: userRule.allowlistFiles,
            };
          }
        }
      }
