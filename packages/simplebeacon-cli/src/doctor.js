...
function runDoctor() {
  ...
  if (nodeVersion < 22) {
    criticalErrors.push(
      `Node version engine mismatch: running v${nodeVersion}, requires >=22.`,
    );
  }
  ...
  try {
    ...
    if (cacheStats.totalEntries === 0) {
      airGapReady = false;
      airGapWarnings.push(
        'Registry cache is empty — run "simplebeacon cache prewarm" before air-gapping',
      );
    } else if (cacheStats.fresh === 0) {
      airGapReady = false;
      airGapWarnings.push(
        `Registry cache has ${cacheStats.stale} stale entries — run "simplebeacon cache prewarm" to refresh`,
      );
    } else {
      console.log(
        `  ✅ Registry cache: ${cacheStats.fresh} fresh entries (${cacheStats.stale} stale)`,
      );
    }
  } catch {
    airGapReady = false;
    airGapWarnings.push(
      "Unable to read registry cache — offline-resolver module not available",
    );
  }

  if (airGapReady) {
    console.log(
      "  ✅ Air-gap readiness: cache populated and ready for offline scans",
    );
  } else {
...
function generateSupportPackage(diagnostics) {
  ...
  const payload = JSON.stringify({
    ...
    system: diagnostics,
    anonymizedPathHash: crypto
      .createHash("sha256")
      .update(diagnostics.cwd)
      .digest("hex"),
  });
