...
const Simplebeacon = deepFreeze({
  ...
  mcp: _createMultiNamespace('mcp', ['./mcp/tools', './mcp/stdio-server', './lib/snippet-scanner']),
  ...
  const lazyOut = new Proxy(
    {},
    {
      ...
      getOwnPropertyDescriptor(target, prop) {
        if (typeof prop !== 'string') return undefined;
        if (prop === 'version')
          return { value: version, writable: false, enumerable: true, configurable: false };
        ...
        if (prop === 'Simplebeacon')
          return { value: Simplebeacon, writable: false, enumerable: true, configurable: false };
        if (prop === 'getExportNames')
          return { value: getExportNames, writable: false, enumerable: true, configurable: false };
        ...
        if (prop === '__barrel__')
          return { value: __barrel__, writable: false, enumerable: true, configurable: false };
        for (const { name, path: p } of specs) {
          const mod = _loadModule(name, p);
          if (prop in mod)
            return { value: mod[prop], writable: false, enumerable: true, configurable: false };
        }
        return undefined;
      },
    }
