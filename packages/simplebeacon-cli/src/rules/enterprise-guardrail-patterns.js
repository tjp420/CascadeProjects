...
const RULE_CATALOG = [
    ...
    },
    {
        ...
        severity: 'medium',
        description: 'LLM API call inside unbounded loop (for/while/forEach/map) without explicit iteration cap'
    },
    {
        id: 'SB-ENT-004',
        category: 'resilience',
...
const LOOP_BOUNDED_RE = /\b(for\s*\(|while\s*\(|\.forEach\s*\(|\.map\s*\()[^)]*\blength\b|\blimit\b|\bcount\b|\bslice\b|\btake\b/i;
const LOOP_BLOCK_MAX_LINES = 80;

const LLM_CLIENT_NEW_RE = /\bnew\s+(OpenAI|Anthropic|Bedrock|VertexAI|GenerativeAI|Ollama|AI21|Cohere)\b/i;
...
function scanUnboundedLoops(relativePath, content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        findings.push({
            ...
            recommendedAction: 'Add an explicit iteration limit (e.g., slice, take, count) or move LLM call outside the loop',
            affectedFiles: [relativePath],
            metadata: {
                ruleId: 'SB-ENT-003',
                category: 'loop-budget'
...
function scanMissingRetryConfig(relativePath, content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        findings.push({
            ...
            recommendedAction: 'Set maxRetries and timeout on LLM client constructors to prevent runaway retry storms',
            affectedFiles: [relativePath],
            metadata: {
                ruleId: 'SB-ENT-004',
                category: 'resilience'
...
function scanUnsafeStreamCalls(relativePath, content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        findings.push({
            ...
            recommendedAction: 'Attach an AbortController signal or set a stream timeout to prevent indefinite stream hangs',
            affectedFiles: [relativePath],
            metadata: {
                ruleId: 'SB-ENT-005',
                category: 'stream-safety'
