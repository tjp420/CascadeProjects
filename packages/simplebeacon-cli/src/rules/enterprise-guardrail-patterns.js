...
const RULE_CATALOG = [
  ...
  },
  {
    ...
    severity: "medium",
    description:
      "max_tokens value exceeds safe threshold (>16,384) — risk of oversized billing",
  },
  {
    id: "SB-ENT-003",
  ...
  },
  {
    ...
    severity: "medium",
    description:
      "LLM SDK client instantiated without explicit maxRetries or timeout configuration",
  },
  {
    id: "SB-ENT-005",
    ...
    severity: "medium",
    description:
      "Streaming LLM call without AbortController or explicit stream timeout",
  },
];
...
const HIGH_MAX_TOKENS_RE =
  /\bmax_(?:completion_)?tokens\b\s*:\s*(\d{5,}|[2-9]\d{4}|1[7-9]\d{3})/i;

const LOOP_START_RE =
  /\b(for\s*\(|while\s*\(|\.forEach\s*\(|\.map\s*\(|\.flatMap\s*\()/i;
...
function buildLeakPattern(extraTokens = []) {
  ...
  return new RegExp(
    `\\b(?:${tokens.map(escapeRegexToken).join("|")})\\b`,
    "gi",
  );
}
...
function callBlockHasTokenCap(lines, startLineIndex) {
  const block = lines
    .slice(startLineIndex, startLineIndex + CALL_BLOCK_MAX_LINES)
    .join("\n");
...
function scanUnsafeStreamCalls(relativePath, content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!lineHasLlmInvocation(line)) continue;
    const block = lines.slice(i, i + CALL_BLOCK_MAX_LINES).join("\n");
    if (ABORT_CONTROLLER_RE.test(block) || STREAM_TIMEOUT_RE.test(block))
      continue;
    findings.push({
