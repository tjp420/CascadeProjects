...
function parseJestSummary(output) {
  ...
  const suitesMatch = text.match(
    /Test Suites:\s+(?:(\d+) failed,\s*)?(?:(\d+) skipped,\s*)?(\d+) passed,\s*(\d+) total/,
  );
...
function buildJestBaselineIssues(
  summary,
  baseline,
  expectedPassing,
  expectedLabel,
  ...
  if (summary.testsFailed > 0) {
    ...
    summary.testsPassed !== expectedPassing
  ) {
    issues.push({
      ...
      recommendedAction:
        "Run npm test and sync .simplebeacon/baseline.json jestTestsPassing",
...
async function checkJestBaseline(baseDir, options = {}) {
  ...
  const testCommand =
    options.testCommand ||
    "npx jest --config jest.config.js --no-coverage --passWithNoTests";
        ...
        : [
            {
              ...
              recommendedAction:
                "Run npm test and refresh .simplebeacon/jest-result.json",
      ...
      issues: [
        {
          ...
          recommendedAction:
            "Fix test runner or update testCommand in .simplebeacon/config.json",
          affectedFiles: ["package.json"],
        },
      ],
      summary: null,
  ...
  const issues = buildJestBaselineIssues(
    summary,
    baseline,
    expectedPassing,
    expectedLabel,
