...
function isAllowlistedMatch(line, matchText) {
  const strippedLine = String(line || "")
    .replace(/\/\/.*$/, "")
    .replace(/\/\*[\s\S]*?\*\//, "");
...
function isFenceDetectorMetaLine(line, relativePath, ruleId) {
  ...
  if (/\/[`]{3}[a-z]*/i.test(trimmed) && /\/[gimsuy]*['"]?\)/.test(trimmed))
    return true;
  if (
    /[`]{3}(?:json|javascript|typescript|python)/i.test(trimmed) &&
...
async function walkFiles(dir, results = [], options = {}, depth = 0) {
  ...
  for (const entry of entries) {
    ...
    if (!SCANNABLE_EXTENSIONS.has(ext) && !MANIFEST_NAMES.has(baseName))
      continue;
...
function scanTextPatterns(relativePath, content, ext, options = {}) {
  ...
  if (
    ...
    /simplebeacon-ignore/i.test(content.substring(0, 500))
  ) {
    return [];
  }
  const findings = [];
  ...
  for (const rule of RULE_CATALOG) {
    ...
    if (exclusions && exclusions.ext && exclusions.ext.length > 0) {
      ...
      if (
        exclusions.ext.some((e) => relativePath.endsWith(e) || fileExt === e)
      ) {
        continue;
      }
    }
    ...
    // SB-FICTION-001: skip YAML/OpenAPI specs — they contain documentation
    // examples like "Authorization: Bearer con_your_api_key_here" which are
    // API spec descriptions, not AI slop placeholders.
    if (rule.id === "SB-FICTION-001" && (ext === ".yaml" || ext === ".yml")) continue;
    ...
    while ((match = rule.regex.exec(content)) !== null) {
      ...
      if (
        exclusions &&
        exclusions.linePrefixes &&
        exclusions.linePrefixes.length > 0
      ) {
        const trimmed = line.trim();
        if (
          exclusions.linePrefixes.some((prefix) => trimmed.startsWith(prefix))
        ) {
          continue;
        }
      }

      if (isAllowlistedMatch(line, match[0]) && rule.id !== "SB-FICTION-007")
        continue;
      if (isFenceDetectorMetaLine(line, relativePath, rule.id)) continue;
      if (rule.id === "SB-FICTION-002" && isJSDocLine(line)) continue;
      // SB-FICTION-001 (AI placeholder) should skip JSDoc documentation blocks
      // — patterns like YOUR_API_KEY_HERE in JSDoc are documentation examples,
      // not AI slop.
      if (rule.id === "SB-FICTION-001" && isJSDocLine(line)) continue;
      // SB-FICTION-001: also skip inline // comments when the match is a
      // "Generated by Gemini/Claude/GPT" pattern in a technical context.
      // Legitimate comments like "// for histories that were not generated by
      // Gemini" are documentation, not AI slop. The real slop pattern is
      // "Generated by Claude/GPT" as a code comment stamp, which typically
      // appears as the first line or a standalone comment, not mid-sentence.
      if (
        rule.id === "SB-FICTION-001" &&
        isCommentLine(line, ext) &&
        /generated by (?:claude|gpt|chatgpt|copilot|gemini)/i.test(match[0]) &&
        /\b(not|never|were not|was not|aren't|isn't|without)\b/i.test(line)
      )
        continue;
      if (
        isCommentLine(line, ext) &&
        ...
        rule.id !== "SB-FICTION-001" &&
        rule.id !== "SB-FICTION-005" &&
        rule.id !== "SB-FICTION-006" &&
        rule.id !== "SB-FICTION-008"
      )
        continue;
      ...
      findings.push({
        ...
        recommendedAction:
          "Replace placeholder copy with production-ready values before client handoff",
  ...
  for (const section of sections) {
    ...
    for (const name of Object.keys(block)) {
      ...
      findings.push({
        ...
        recommendedAction:
          "Verify package exists on npm/PyPI or remove fabricated dependency",
...
async function scanUnknownNpmDependencies(relativePath, content, options = {}) {
  ...
  for (const name of names) {
    ...
    checked += 1;
    const exists = await npmRegistryExists(
      name,
      options.registryTimeoutMs || 4000,
    );
    if (exists === false) {
      flagged.push({
        ...
        recommendedAction:
          "Remove or replace dependency; confirm package name on registry.npmjs.org",
        affectedFiles: [relativePath],
        metadata: {
...
async function scanLlmSlopPatterns(baseDir, options = {}) {
  ...
  for (const file of files) {
    ...
    issues.push(
      ...scanTextPatterns(file.relativePath, content, file.ext, {
    ...
    if (options.registryCheck === true) {
      issues.push(
        ...(await scanUnknownNpmDependencies(
          file.relativePath,
          content,
          options,
        )),
