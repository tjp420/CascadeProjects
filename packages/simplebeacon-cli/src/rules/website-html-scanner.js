...
function fetchPage(url, timeoutMs) {
  return new Promise((resolve) => {
    ...
    const req = client.get(url, { timeout: timeoutMs }, (res) => {
      ...
      res.on('end', () => resolve({ url, status: res.statusCode, headers: res.headers, body, size: body.length }));
    });
...
function analyzeHtml(html, url) {
  ...
  if (inlineScriptMatches.length > 0) {
    findings.push({
      ...
      ruleId: 'SB-WEB-001',
      metadata: { url, count: inlineScriptMatches.length, category: 'inline-scripts' }
    });
  }
  ...
  if (inlineStyleMatches.length > 3) {
    findings.push({
      ...
      ruleId: 'SB-WEB-001',
      metadata: { url, count: inlineStyleMatches.length, category: 'inline-styles' }
    });
  }
  ...
  const placeholderMatches = html.match(/\b(lorem ipsum|coming soon|placeholder text|tbd|TBD)\b/gi) || [];
  if (placeholderMatches.length > 0) {
    findings.push({
      ...
      ruleId: 'SB-WEB-003',
      metadata: { url, count: placeholderMatches.length, category: 'placeholder-text' }
    });
  }
  ...
  if (!html.includes('name="description"') && !html.includes("name='description'")) {
    findings.push({
      ...
      severity: 'medium',
      url,
      description: 'Missing meta description tag — impacts SEO and social sharing',
      ruleId: 'SB-WEB-004',
  ...
  if (!html.includes('rel="canonical"') && !html.includes("rel='canonical'")) {
    findings.push({
      ...
      severity: 'low',
      url,
      description: 'Missing canonical link tag — may cause duplicate content SEO issues',
      ruleId: 'SB-WEB-005',
  ...
  if (!html.includes('application/ld+json')) {
    findings.push({
      ...
      severity: 'low',
      url,
      description: 'No JSON-LD structured data found — impacts search engine understanding and rich results',
      ruleId: 'SB-WEB-006',
  ...
  if (!html.includes('<main') && !html.includes('role="main"') && !html.includes("role='main'")) {
    findings.push({
      type: 'website-html',
      severity: 'low',
      url,
      description: 'Missing <main> element or role="main" — impacts accessibility landmark navigation',
      ruleId: 'SB-WEB-007',
  ...
  if (consoleLogMatches.length > 0) {
    findings.push({
      ...
      ruleId: 'SB-WEB-009',
      metadata: { url, count: consoleLogMatches.length, category: 'console-log' }
    });
  }
  ...
  if (brokenLinkMatches.length > 0) {
    findings.push({
      ...
      ruleId: 'SB-WEB-010',
      metadata: { url, count: brokenLinkMatches.length, category: 'broken-links' }
    });
  }
  ...
  const localhostUrlMatches = html.match(/https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?\//gi) || [];
  if (localhostUrlMatches.length > 0) {
    findings.push({
      ...
      ruleId: 'SB-WEB-011',
      metadata: { url, count: localhostUrlMatches.length, category: 'localhost-leak', matches: localhostUrlMatches.slice(0, 5) }
    });
  }
