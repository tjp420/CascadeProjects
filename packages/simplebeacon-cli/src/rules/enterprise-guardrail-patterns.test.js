...
const res = await openai.chat.completions.create({ model: 'gpt-4', messages: [] });
    ...
    assert.ok(
      patterns.includes("SB-ENT-001"),
      "expected SB-ENT-001 (data leak)",
    );
    assert.ok(
      patterns.includes("SB-ENT-002"),
      "expected SB-ENT-002 (token budget)",
    );
  });
  ...
  it("respects excluded paths", () => {
    const content = `const res = await openai.chat.completions.create({ model: 'gpt-4', messages: [] });`;
    const findings = scanEnterpriseGuardrailContent(
      "simplebeacon-rule-tests/test.js",
      content,
    );
    assert.strictEqual(findings.length, 0);
  });
