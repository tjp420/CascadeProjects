...
const STRING_CHECKS = [
  ...
  {
    id: 'workspace-health',
    terms: ["require('../../../", 'require("../../../', "from '../../../", 'from "../../../'],
    ext: ['.js', '.ts', '.jsx', '.tsx', '.cjs', '.mjs'],
  },
  {
    id: 'api-contract',
    terms: ['openapi:', 'swagger:', 'openapi.json', 'swagger.json', 'openapi.yaml', 'swagger.yaml'],
  ...
  {
    id: 'i18n',
    terms: ["textContent = '", 'textContent = "', "innerText = '", 'innerText = "'],
    ext: ['.js', '.ts', '.jsx', '.tsx'],
  },
...
function shouldSkipCheckForPath(checkId, relativePath) {
  ...
  if (checkId === 'ai-indicators') {
    if (/\b(auto-processor\.js|audit_viz\.py|orchestrator\.test\.js|test-gateway\.js)\b/.test(rel))
      return true;
  ...
  if (checkId === 'workspace-health') {
    ...
    if (
      /\/(shared-utils|server\/lib|server\/routes\/lib|src\/api\/billing)\//.test(rel) &&
  ...
  for (const check of STRING_CHECKS) {
    ...
    for (const term of check.terms) {
      ...
      if (check.id === 'governance-marker') {
        ...
        if (/\bdebug\s*\|\s*info\s*\|\s*notice\s*\|\s*warning\s*\|\s*critical\b/.test(matchLine))
          continue;
      }
...
async function scanComprehensive(uniqueFiles, options = {}) {
  ...
  for (const file of uniqueFiles) {
    ...
    const isSkippedName =
      /\b(index|config|types|constants|utils|helpers|setup|init|main|cli|bin)\b/i.test(baseName);
    if (
