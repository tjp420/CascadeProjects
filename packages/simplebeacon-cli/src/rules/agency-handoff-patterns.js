...
function isExcludedPath(relativePath) {
  ...
  if (
    /(?:^|\/)web\/simplebeacon-dashboard\/js(?:-es2018)?\/views\/analyzeview\.js$/.test(normalized)
  )
    return true;
...
function scanWebhookFileRisk(relativePath, content) {
  ...
  if (/constructEvent|verifyHeader|createHmac|stripe\.webhooks|Webhook\.sign/i.test(content))
    return [];
...
function scanTextPatterns(relativePath, content, ext) {
  ...
  for (const rule of RULE_CATALOG) {
    ...
    while ((match = rule.regex.exec(content)) !== null) {
      ...
      if (
        rule.id === 'SB-DEPLOY-001' &&
        /connect-src|connectSrc|Content-Security-Policy|buildConnectSrc/.test(line)
      )
        continue;
      if (
        ...
        continue;
      if (rule.id === 'SB-DEPLOY-005' && /connectSrc|buildConnectSrc|CSP|helmet\s*\(/.test(line))
        continue;
...
function scanEnvCommitRisk(baseDir) {
  ...
  const isGitignoredEnv = (name) =>
    gitignoreLines.some((line) => line === name || (line === '.env' && name.startsWith('.env')));
