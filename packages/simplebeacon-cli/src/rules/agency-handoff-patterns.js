...
function isScannerImplementationPath(relativePath) {
  ...
  if (
    ...
    return true;
  if (
    /(?:^|\/)packages\/simplebeacon-cli\/src\/(?:rules|reporters|analyzers|proxy|lib)\//.test(
      normalized,
    )
  )
    return true;
  if (
    ...
    return true;
  if (
    /(?:^|\/)server\/lib\/(?:codebase-analyzer|production-leak|fiction-kpi|mock-data-scanner|simplebeacon-report|scan-orchestr|secret-config|audit-remediation-recipes|sample-path-resolver|code-roadmap-generator)/.test(
      normalized,
    )
  )
    return true;
  if (/(?:^|\/)server\/routes\/flexible-analyze-api/.test(normalized))
    return true;
  return false;
}
...
const RULE_CATALOG = [
  {
    ...
    severity: "high",
    description:
      "Localhost or loopback URL with port in production-eligible code",
  },
  {
    id: "SB-DEPLOY-002",
    category: "deploy-leak",
  ...
  },
  {
    ...
    type: "Auth Misconfiguration",
    regex:
      /(?:BYPASS_AUTH|SKIP_AUTH|ALLOW_DEV_EPHEMERAL|AUTH_BYPASS)\s*[=:]\s*['"]?true\b/gi,
    severity: "critical",
  ...
  },
  {
    ...
    type: "Auth Misconfiguration",
    regex:
      /(?:password|passwd)\s*[:=]\s*['"](?:demo123|admin|password|changeme)['"]/gi,
    severity: "high",
  ...
  },
  {
    ...
    regex:
      /console\.(?:log|debug|info)\([^)]*(?:prompt|completion|messages|openai|anthropic|llm)/gi,
    severity: "medium",
    description:
      "Console logging of AI prompts or completions in production path",
  },
  {
    id: "SB-AI-002",
    category: "ai-telemetry-leak",
    type: "AI Telemetry Leak",
    regex:
      /logger\.(?:debug|info|log)\([^)]*(?:prompt|completion|chat\.completions)/gi,
    severity: "medium",
  ...
  },
  {
    ...
    type: "Web Security Risk",
    regex:
      /(?:redirect|res\.redirect)\([^)]*(?:req\.query|req\.params|req\.body)/gi,
    severity: "medium",
...
function isExcludedPath(relativePath) {
  ...
  if (
    /(?:^|\/)web\/simplebeacon-dashboard\/js(?:-es2018)?\/views\/analyzeview\.js$/.test(
      normalized,
    )
  )
    return true;
...
function scanWebhookFileRisk(relativePath, content) {
  if (
    ...
    !/webhook/i.test(content.slice(0, 8000))
  )
    return [];
  if (
    /constructEvent|verifyHeader|createHmac|stripe\.webhooks|Webhook\.sign/i.test(
      content,
    )
  )
    return [];
  if (!/(?:app|router)\.(?:post|use)\(\s*['"][^'"]*webhook/i.test(content))
    return [];
  return [
    {
      ...
      recommendation:
        "Verify webhook signatures (e.g. Stripe constructEvent) before processing body.",
      recommendedAction:
        "Verify webhook signatures (e.g. Stripe constructEvent) before processing body.",
...
function scanTextPatterns(relativePath, content, ext) {
  ...
  for (const rule of RULE_CATALOG) {
    ...
    while ((match = rule.regex.exec(content)) !== null) {
      ...
      if (isCommentLine(line, ext) && !rule.id.startsWith("SB-HANDOFF"))
        continue;
      if (isAllowlistedMatch(line, match[0])) continue;
      if (/simplebeacon-ignore/.test(line)) continue;
      if (
        rule.id === "SB-DEPLOY-001" &&
        /(?:localhost|127\.0\.0\.1):(?:54355|11434)\b/.test(match[0])
      )
        continue;
      if (
        rule.id === "SB-DEPLOY-001" &&
        /connect-src|connectSrc|Content-Security-Policy|buildConnectSrc/.test(
          line,
        )
      )
        continue;
      if (
        (rule.id === "SB-DEPLOY-001" || rule.id === "SB-DEPLOY-005") &&
        /console\.log\s*\(/.test(line)
      )
        continue;
      if (
        rule.id === "SB-DEPLOY-005" &&
        /connectSrc|buildConnectSrc|CSP|helmet\s*\(/.test(line)
      )
        continue;
      // Skip dynamic CSP connectSrc builder lines (base.push with localhost template literals)
      if (
        (rule.id === "SB-DEPLOY-001" || rule.id === "SB-DEPLOY-005") &&
        /base\.push\s*\(/.test(line) &&
        /localhost|127\.0\.0\.1/.test(line)
      )
        continue;
      if (
        rule.id === "SB-AI-001" &&
        /completionRate|roadmap|percent|progress|taskCount/i.test(line)
      )
        continue;
...
async function walkDir(dir, baseDir, files, depth) {
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      !ENV_COMMIT_NAMES.has(entry.name.toLowerCase())
    )
      continue;
...
function scanEnvCommitRisk(baseDir) {
  ...
  const isGitignoredEnv = (name) =>
    gitignoreLines.some(
      (line) => line === name || (line === ".env" && name.startsWith(".env")),
    );
  ...
  for (const name of ENV_COMMIT_NAMES) {
    ...
    issues.push({
      ...
      recommendation:
        "Move secrets to host env or secret manager; keep only .env.example in git.",
      recommendedAction:
        "Move secrets to host env or secret manager; keep only .env.example in git.",
      affectedFiles: [name],
...
function scanHandoffIntegrity(baseDir) {
  ...
  if (fs.existsSync(pkgPath) && !fs.existsSync(lockPath)) {
    issues.push({
      ...
      count: 1,
      description:
        "package.json present but package-lock.json missing — non-reproducible installs",
      recommendation:
        "Run npm install and commit package-lock.json before handoff.",
      recommendedAction:
        "Run npm install and commit package-lock.json before handoff.",
