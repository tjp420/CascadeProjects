...
const EXPORT_NAMED_REGEX =
  /export\s+(?:const|let|var|function|class|type|interface)\s+(\w+)/gm;
const EXPORT_DEFAULT_REGEX =
...
function extractImportNames(content) {
  ...
  // const foo = require('...')
  const requireRegex =
    /(?:const|let|var)\s*\{?\s*([^}=]+)\s*\}?\s*=\s*require\s*\(/gm;
...
async function scanDependencyGraph(baseDir, options = {}) {
  ...
  for (const [importingFile, depPaths] of fileGraph.entries()) {
    for (const depPath of depPaths) {
      ...
      const names =
        allImports.get(
          path.relative(baseDir, importingFile).replace(/\\/g, "/"),
        ) || new Set();
  ...
  return {
    ...
    results: [],
    dependencyGraph: {
      nodes: Array.from(fileGraph.keys()).map((p) =>
