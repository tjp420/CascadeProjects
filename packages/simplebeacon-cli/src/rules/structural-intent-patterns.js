...
const RECOMMENDED_ACTIONS = {
  ...
  "SB-INTENT-002":
    "Load secrets from vault/env; never commit placeholder credentials",
  "SB-INTENT-003":
  ...
  return (findings || []).map((finding, index) => {
    ...
    return {
      ...
      description:
        finding.description ||
        finding.summary ||
        `Structural intent rule ${pattern}`,
      recommendedAction:
...
async function scanStructuralIntentPatterns(baseDir, options = {}) {
  ...
  const slmEnabled =
    intelligence.slm?.enabled === true ||
    process.env.SIMPLEBEACON_SLM_ENABLED === "1";
  ...
  for (const file of files) {
    ...
    if (needsSlm) {
      const slm = runLocalSlmReview(content, {
        filePath: file.relativePath,
        intelligence,
      });
      if (slm.reviewed) {
