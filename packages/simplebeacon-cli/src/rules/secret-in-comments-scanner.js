...
const SECRET_PATTERNS = [
  {
    ...
    severity: "high",
    description:
      "Password value exposed in a comment — remove or rotate immediately",
  },
  {
    id: "SB-SEC-007b",
    name: "Secret in Comment — API Key",
    regex:
      /(?:api[_-]?key|apikey|api_token)\s*[:=]\s*['"`]?([a-zA-Z0-9_\-]{16,})/i,
    severity: "high",
  ...
  },
  {
    ...
    regex:
      /(?:token|auth_token|access_token|bearer)\s*[:=]\s*['"`]?([a-zA-Z0-9_\-]{16,})/i,
    severity: "high",
    description:
      "Authentication token exposed in a comment — remove and rotate",
  },
  {
    id: "SB-SEC-007d",
    name: "Secret in Comment — Secret Key",
    regex:
      /(?:secret[_-]?key|secretkey|client_secret)\s*[:=]\s*['"`]?([a-zA-Z0-9_\-]{16,})/i,
    severity: "high",
  ...
  },
  {
    ...
    severity: "high",
    description:
      "Database connection string exposed in a comment — rotate credentials",
  },
];
...
function isScannable(filePath) {
  ...
  if (
    basename === ".env.v1-internal" ||
    /\.env\.(backup|old|bak|save|~)$/.test(basename)
  )
    return false;
  ...
  for (const comment of comments) {
    for (const rule of SECRET_PATTERNS) {
      ...
      if (match) {
        ...
        const line =
          lineIndex >= 0
            ? content.substring(0, lineIndex).split("\n").length
            : 0;
        findings.push({
          ruleId: rule.id,
          ruleName: rule.name,
