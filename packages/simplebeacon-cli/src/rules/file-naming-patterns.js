...
const SCANNABLE_EXTENSIONS = new Set([
    ...
    '.vue', '.svelte', '.json', '.yaml', '.yml', '.md', '.css', '.scss'
]);

const SKIP_DIRS = new Set([
    'node_modules', '.git', 'coverage', 'dist', 'build', 'archive',
    '.simplebeacon', 'tests', 'test', '__tests__', 'fixtures', 'docs',
    'coming-soon', 'reports', 'templates', 'deployments', 'public',
    'temp', 'downloads', 'findings',
...
const RULE_CATALOG = [
    {
        ...
        severity: 'medium',
        description: 'File name contains copy/duplicate debris (e.g., "(2)", " - Copy", "backup_final_v3")'
    },
    {
        id: 'SB-FNAME-002',
    ...
    },
    {
        ...
        severity: 'medium',
        description: 'Suspicious AI-generated file name with redundant or vague terms (e.g., "optimized_final_actual", "helper_utility")'
    },
    {
        id: 'SB-FNAME-004',
...
const COPY_DEBRIS_RE = /\s*\(\d+\)|\s+-\s*Copy\b|\s*-\s*copy\b|backup_\w*\d+|_bak\d*|_old\d*|_orig\d*/i;
const PLACEHOLDER_NAME_RE = /^(?:Untitled|untitled|script|Script|new_file|newFile|temp|tmp|foo|bar|baz|qux|something|thing|stuff|my_|old_|misc_|random_)/i;
const LLM_NAMING_SLOP_RE = /(?:optimized|final|actual|real|updated|correct|fixed|working|proper|corrected|helper|utility|utils|common|shared|base|core|generic|standard|default|misc|utils2?|helpers?|tools?)(?:_|\b)(?:optimized|final|actual|real|updated|correct|fixed|working|proper|corrected|helper|utility|utils|common|shared|base|core|generic|standard|default|misc)/i;
const VERSION_CHAIN_RE = /(?:v\d+|_v\d+|final|FINAL|real|REAL|actual|ACTUAL)(?:_|\b)(?:v\d+|final|FINAL|real|REAL|actual|ACTUAL)/i;

const ALLOWLIST = new Set([
    'package.json', 'package-lock.json', 'yarn.lock', 'pnpm-lock.yaml',
    'tsconfig.json', 'jsconfig.json', 'README.md', 'LICENSE', 'CHANGELOG.md',
    '.gitignore', '.env.example', '.eslintrc.json', '.prettierrc',
...
async function scanFileNamingPatterns(baseDir, options = {}) {
    const sourcePaths = options.sourcePaths || ['src', 'server', 'lib', 'packages', 'app', 'web', 'api', 'config'];
