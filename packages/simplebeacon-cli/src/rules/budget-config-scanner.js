...
const RULE_CATALOG = [
  {
    ...
    severity: "medium",
    description:
      "Missing simplebeacon.budget.json or inline budget config for LLM usage",
  },
  {
    id: "SB-ENT-006a",
    category: "budget-as-code",
    severity: "medium",
    description:
      "LLM call max_tokens exceeds budget config maxTokensPerRequest",
  },
  {
    id: "SB-ENT-006b",
    category: "budget-as-code",
...
const BUDGET_FILE_NAMES = [
  "simplebeacon.budget.json",
  ".simplebeacon.budget.json",
];
...
function scanBudgetCompliance(relativePath, content, budgetConfig) {
  ...
  if (!budgetConfig) {
    findings.push({
      ...
      recommendedAction:
        "Create simplebeacon.budget.json with maxMonthlySpend, maxTokensPerRequest, and maxRequestsPerMinute",
  ...
  if (!budgetConfig.maxMonthlySpend || !budgetConfig.maxTokensPerRequest) {
    findings.push({
      ...
      recommendedAction:
        "Add maxMonthlySpend (USD) and maxTokensPerRequest to simplebeacon.budget.json",
      affectedFiles: [relativePath],
      metadata: {
        ruleId: "SB-ENT-006b",
        category: "budget-as-code",
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      budgetConfig.maxTokensPerRequest &&
      value > budgetConfig.maxTokensPerRequest
    ) {
      findings.push({
