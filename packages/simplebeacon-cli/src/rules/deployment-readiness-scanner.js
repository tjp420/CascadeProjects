...
const RULE_CATALOG = [
  {
    ...
    severity: "medium",
    description:
      "Service directory exists but is not declared in root package.json workspaces",
  },
  {
    id: "SB-DEP-002",
    category: "deployment-readiness",
    ...
    severity: "high",
    description:
      "Environment variable required by code is missing from render.yaml",
  },
  {
    id: "SB-DEP-003",
    category: "deployment-readiness",
    ...
    severity: "high",
    description:
      "Multiple services create the same database table with incompatible schemas",
  },
  {
    id: "SB-DEP-004",
    category: "deployment-readiness",
    ...
    severity: "medium",
    description:
      "Services use different CORS configuration approaches or origin lists",
  },
  {
    id: "SB-DEP-005",
    category: "deployment-readiness",
    ...
    severity: "low",
    description:
      "Service directory has a package.json with a start script but no render.yaml",
  },
  {
    id: "SB-DEP-006",
    category: "deployment-readiness",
    ...
    severity: "high",
    description:
      "Multiple services target the same database without schema isolation",
  },
];
  ...
  function walk(dir, depth) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        entry.isFile() &&
        (entry.name === "render.yaml" || entry.name === "render.yml")
      ) {
...
function parseRenderYaml(filePath) {
  ...
  for (const line of lines) {
    ...
    if (
      currentSection === "services" &&
      currentService &&
      /^healthCheckPath:\s*(.+)/.test(trimmed)
    ) {
    ...
    if (inEnvVars && currentService) {
      ...
      if (envMatch) {
        ...
        envVars.push({
          key: envMatch[1],
          service: currentService.name || currentService.dir,
        });
        continue;
      }
      ...
      if (currentService.envVars.length > 0) {
        const lastEnv =
          currentService.envVars[currentService.envVars.length - 1];
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (pkg.scripts && pkg.scripts.start) {
          const relDir =
            path.relative(baseDir, dir).split(path.sep).join("/") || ".";
          const hasRenderYaml =
            fs.existsSync(path.join(dir, "render.yaml")) ||
            fs.existsSync(path.join(dir, "render.yml"));
          services.push({
            dir: relDir,
...
function checkCorsApproach(baseDir, serviceDir) {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      if (
        content.includes("cors-config.cjs") ||
        content.includes("resolveCorsOptions")
      ) {
        return {
          approach: "shared-cors-config",
          file: path.relative(baseDir, candidate).split(path.sep).join("/"),
        };
      }
      if (
        content.includes("Access-Control-Allow-Origin") ||
        content.includes("app.use(cors(")
      ) {
        return {
          approach: "inline-cors",
          file: path.relative(baseDir, candidate).split(path.sep).join("/"),
        };
      }
    } catch {
...
async function scanDeploymentReadiness(baseDir, _options = {}) {
  ...
  for (const svc of serviceDirs) {
    ...
    const renderFile = renderFiles.find(
      (rf) =>
        path.dirname(rf) === path.join(baseDir, svc.dir) ||
        (svc.dir === "." && path.dirname(rf) === baseDir),
    );
  ...
  for (const [tableName, entries] of Object.entries(tablesByName)) {
    ...
    if (services.size > 1) {
      ...
      issues.push({
        ...
        recommendedAction:
          "Use separate databases per service or consolidate shared tables",
        affectedFiles: entries.map((e) => e.file),
        metadata: {
          patternId: "SB-DEP-003",
          table: tableName,
          services: [...services],
          exception: isResolvedBySeparateDbs
            ? documentedException.reason
            : undefined,
        },
      });
    }
  ...
  for (const renderFile of renderFiles) {
    ...
    const dir =
      path
        .relative(baseDir, path.dirname(renderFile))
        .split(path.sep)
        .join("/") || ".";
  ...
  if (Object.keys(approachesByType).length > 1) {
    ...
    issues.push({
      ...
      recommendation:
        "Consolidate all services to use the shared cors-config.cjs module (resolveCorsOptions + isAllowedOrigin).",
      recommendedAction: "Use shared cors-config.cjs across all services",
      affectedFiles: corsApproaches.map((c) => c.file),
      metadata: {
        patternId: "SB-DEP-004",
        approaches: Object.keys(approachesByType),
      },
    });
  }
  ...
  for (const svc of serviceDirs) {
    ...
    if (
      !svc.hasRenderYaml &&
      svc.startScript &&
      !svc.startScript.includes("check-port")
    ) {
      ...
      if (/node\s/.test(svc.startScript) && !svc.dir.startsWith("packages/")) {
        ...
        const coveredByParent = renderStartCommands.some(
          (cmd) => cmd.includes(svc.dir + "/") || cmd.includes(svc.dir + "\\"),
        );
        ...
        try {
          ...
          const allScripts = Object.values(scripts).filter(
            (s) => typeof s === "string",
          );
          isLocalTool = allScripts.some((s) => s.includes("pkg "));
          isCloudflarePages = allScripts.some(
            (s) =>
              s.includes("wrangler pages deploy") ||
              s.includes("wrangler pages publish"),
          );
        } catch {
          /* ignore */
        }
