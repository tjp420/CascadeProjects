    ...
    babelTraverse(ast, {
        FunctionDeclaration(path) {
            ...
            if (node.id && node.id.name) {
                ...
                exports.set(name, { isExported, node, line: node.loc ? node.loc.start.line : 0 });
            }
        },
        Identifier(path) {
            const node = path.node;
...
function analyzePromiseChains(ast, relativePath) {
    ...
    babelTraverse(ast, {
        MemberExpression(path) {
            const node = path.node;
            if (node.property && node.property.name === 'then' && path.parent && path.parent.type === 'CallExpression') {
                ...
                if (current.type === 'CallExpression' && current.callee === node) {
                    ...
                    if (parentExpr && parentExpr.type === 'MemberExpression' && parentExpr.property && parentExpr.property.name === 'catch') {
                        chainCount++;
                    }
                }
            }
    ...
    for (const file of files) {
        ...
        for (const f of fileFindings) {
            issues.push({
                ...
                recommendedAction: f.ruleId === 'SB-QUAL-003'
