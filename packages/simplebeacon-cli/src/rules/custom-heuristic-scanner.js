...
function loadAstParser() {
    ...
    try { babelParser = require('@babel/parser'); } catch { return null; }
    return babelParser;
}

function loadAstTraverse() {
    if (babelTraverse) return babelTraverse;
    try { babelTraverse = require('@babel/traverse').default; } catch { return null; }
...
const VALID_EXTENSIONS = new Set([
    ...
    '.sh', '.bash', '.ps1',
    '.sql', '.graphql', '.gql',
    '.md', '.txt', '.xml', '.toml', '.ini', '.cfg',
]);

const SKIP_DIRS = new Set([
    'node_modules', '.git', 'coverage', 'dist', 'build', 'archive',
    '.simplebeacon', 'tests', 'test', '__tests__', 'fixtures', 'docs',
    'deliverables', 'coming-soon', 'reports', 'security-reports',
    'templates', 'data-central', 'deployments', 'functions',
    'cloudflare-deploy', 'temp', 'tests-legacy', '.github-sync',
    '.cursor', '.vscode', 'downloads', 'findings',
    'vendor', 'bin', 'tmp', 'out',
]);
...
function validateRule(rule, index) {
    ...
    if (engine === 'ast' || (rule.astCriteria && typeof rule.astCriteria === 'object')) {
        ...
        if (!rule.pattern || typeof rule.pattern !== 'string') {
            errors.push(`${prefix}: missing or invalid "pattern" field (must be a regex string)`);
        } else if (rule.pattern.length > MAX_PATTERN_LENGTH) {
            errors.push(`${prefix}: pattern exceeds max length of ${MAX_PATTERN_LENGTH} characters`);
        } else {
            try {
    ...
    if (rule.severity && !VALID_SEVERITIES.has(rule.severity)) {
        warnings.push(`${prefix}: invalid severity "${rule.severity}" — defaulting to "medium"`);
    }

    if (rule.fileExtensions && Array.isArray(rule.fileExtensions)) {
        for (const ext of rule.fileExtensions) {
            if (typeof ext !== 'string' || !ext.startsWith('.')) {
                warnings.push(`${prefix}: invalid file extension "${ext}" — must start with a dot`);
            }
        }
    }

    if (rule.maxFileSize && (typeof rule.maxFileSize !== 'number' || rule.maxFileSize < 0)) {
...
function loadCustomRules(projectRoot, options = {}) {
    ...
    for (let i = 0; i < Math.min(ruleList.length, MAX_RULES); i++) {
        ...
        const normalized = {
            ...
            severity: VALID_SEVERITIES.has(rule.severity) ? rule.severity : 'medium',
            fileExtensions,
            description: rule.description || `${rule.name || rule.id}: ${isAst ? 'custom AST rule match' : 'custom pattern match'}`,
            recommendation: rule.recommendation || 'Review and address the flagged pattern',
            excludePaths: Array.isArray(rule.excludePaths) ? rule.excludePaths : [],
            includePaths: Array.isArray(rule.includePaths) ? rule.includePaths : null,
            maxFileSize: typeof rule.maxFileSize === 'number' ? rule.maxFileSize : DEFAULT_MAX_FILE_SIZE,
...
function calleeLabel(node) {
    ...
    if (node.type === 'MemberExpression') {
        const prop = node.property.type === 'Identifier' ? node.property.name : (node.property.value || '');
        if (node.object.type === 'MemberExpression') return `${calleeLabel(node.object)}.${prop}`;
...
function scanFileAgainstAstRule(content, relPath, rule) {
    ...
    traverse(ast, {
        [criteria.nodeType](pathNode) {
            ...
            const line = pathNode.node.loc && pathNode.node.loc.start ? pathNode.node.loc.start.line : 1;
...
async function scanCustomHeuristicRules(projectRoot, options = {}) {
    ...
    for (const file of files) {
        ...
        for (const rule of enabledRules) {
            ...
            const findings = rule.engine === 'ast'
