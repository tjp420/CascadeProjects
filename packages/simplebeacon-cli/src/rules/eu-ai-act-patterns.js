...
const DOCUMENTATION_MARKERS = [
  { id: "model-card", pattern: /model[-_\s]?card/i, label: "Model card" },
  {
    ...
    pattern:
      /technical[-_\s]?documentation|ai[-_\s]?system[-_\s]?documentation/i,
  ...
  },
  {
    ...
    pattern:
      /conformity[-_\s]?declaration|eu[-_\s]?declaration[-_\s]?of[-_\s]?conformity/i,
...
const HIGH_RISK_CATALOG = [
  {
    ...
    severity: "high",
    description:
      "Employment or recruitment AI decision pattern (Annex III area)",
    fixTemplate:
  ...
  },
  {
    ...
    severity: "high",
    description:
      "Biometric identification or categorisation pattern (Annex III area)",
    fixTemplate:
  ...
  },
  {
    ...
    severity: "high",
    description:
      "Education or vocational training AI assessment pattern (Annex III area)",
    fixTemplate:
  ...
  },
  {
    ...
    severity: "high",
    description:
      "Migration, asylum, or border control AI pattern (Annex III area)",
    fixTemplate:
...
const COMPLIANCE_OBLIGATIONS = [
  ...
  },
  {
    ...
    regex:
      /\b(?:instructions\s*for\s*use|deployer\s*guide|user\s*manual|operator\s*guide)\b/gi,
    severity: "medium",
    description:
  ...
  },
  {
    ...
    severity: "medium",
    description:
      "Missing robustness, accuracy, and cybersecurity testing artifacts",
    fixTemplate:
  ...
  },
  {
    ...
    severity: "medium",
    description:
      "High-risk AI system deployed without deployer obligation compliance",
    fixTemplate:
  ...
  },
  {
    ...
    severity: "high",
    description:
      "Missing Fundamental Rights Impact Assessment (FRIA) for high-risk AI deployment",
    fixTemplate:
...
const COMPLIANCE_ARTIFACT_MARKERS = [
  ...
  {
    id: "fria",
    pattern: /fundamental\s*rights\s*impact\s*assessment|fria/i,
    label: "FRIA",
  },
  {
    id: "robustness-test",
  ...
  {
    id: "audit-log",
    pattern: /audit\s*log|decision\s*log/i,
    label: "Audit log",
  },
];
...
function buildFix(rule, relativePath, lineNumber, _lineText) {
  const fileName =
    typeof relativePath === "string" ? path.basename(relativePath) : "file";
...
function collapsePatternIssuesByFile(issues, relativePath) {
  ...
  const matches = [
    ...new Set(issues.map((i) => i.metadata?.match).filter(Boolean)),
  ];
...
function hasTransparencyDisclosure(content) {
  return TRANSPARENCY_DISCLOSURE_PATTERNS.some((pattern) =>
...
function scanTransparencyGaps(relativePath, content, severityDefault) {
  ...
  if (/(?:^|\/)server\/(?:routes|services|lib)\//.test(normalized))
    return issues;
  if (/(?:^|\/)packages\/simplebeacon-cli\/docs\//.test(normalized))
    return issues;
...
function scanHumanOversightGaps(
  relativePath,
  content,
  hasHighRiskInFile,
  severityDefault,
  ...
  if (HUMAN_OVERSIGHT_PATTERNS.some((pattern) => pattern.test(content)))
    return [];
...
function scanLoggingGaps(relativePath, content, hasAiInFile, _severityDefault) {
  ...
  const isDecisionPath = /(?:route|controller|service|handler|api)/i.test(
    relativePath,
  );
...
function hasDocumentedAiInventory(documentation) {
  ...
  const hasSystemDoc = paths.some((p) =>
    /(?:^|\/)ai-system-documentation\.md$/i.test(p),
  );
...
async function scanEuAiActPatterns(baseDir, options = {}) {
  ...
  for (const file of uniqueFiles) {
    ...
    const highRiskIssues = scanCatalogPatterns(
      relativePath,
      content,
      HIGH_RISK_CATALOG,
      "high",
    ...
    const transparencyIssues = scanTransparencyGaps(
      relativePath,
      content,
      severityDefault,
    );
    ...
    issues.push(
      ...scanLoggingGaps(relativePath, content, hasAi, severityDefault),
    );
  ...
  const filteredIssues = filterDocumentedAiInventoryIssues(
    issues,
    documentation,
    {
  ...
  const summary = {
    ...
    missingArtifacts: absenceIssues
      .map((i) => i.metadata?.patternId)
      .filter(Boolean),
...
function scanComplianceObligations(
  relativePath,
  content,
  hasHighRisk,
  hasAi,
  ...
  for (const rule of COMPLIANCE_OBLIGATIONS) {
    ...
    while ((m = regex.exec(content)) !== null) {
      ...
      if (rule.requiresLogging && LOGGING_PATTERNS.some((p) => p.test(content)))
        continue;
...
function scanAbsenceComplianceGaps(
  baseDir,
  complianceDocs,
  hasHighRisk,
  severityDefault,
