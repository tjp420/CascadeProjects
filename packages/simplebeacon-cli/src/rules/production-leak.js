...
} = require("../lib/production-leak-intent");

const DEFAULT_PRODUCTION_PATHS = Object.freeze([
  "server/",
  "src/",
  "app/",
  "lib/",
...
function getActiveLeakPatterns(options = {}) {
  ...
  const base = !opts.plainSampleJson
    ? LEAK_PATTERNS
    : [...LEAK_PATTERNS, PLAIN_SAMPLE_JSON_PATTERN];
  return base.map((p) => ({
    id: p.id,
...
function lineNumberAt(content, index) {
  ...
  const safeIndex = Number.isFinite(index)
    ? Math.max(0, Math.min(index, content.length))
    : 0;
  return content.slice(0, safeIndex).split("\n").length;
}

function globMatch(relativePath, pattern) {
  if (typeof relativePath !== "string" || typeof pattern !== "string")
    return false;
...
function shouldSuppressMatch({ relativePath, line, matchText }) {
  ...
  if (typeof relativePath === "string") {
    ...
    if (
      /\.simplebeacon|truthcheck|repository-audit|page-sample-specs/i.test(
        matchStr,
      )
    )
      return true;
  ...
  if (
    /mock\/sample(?:\s+(?:json|files|data|paths?|only)|\s*—)/i.test(matchStr) ||
    /Mock\/sample\s+files/i.test(matchStr)
  )
    return true;

  if (
    /instead of\s+["'`]template(?:\s|-)sample["'`]/i.test(matchStr) ||
    /use the phrase\s+["'`]sample-suffix subset["'`]/i.test(matchStr)
  )
    return true;

  if (/\bexpress\.static\s*\(/.test(lineStr) && /web[/\\]data/.test(lineStr))
    return true;

  if (/\bfilePath\s*[:=]\s*[`'][^`']*mock_data_/.test(lineStr)) return true;

  if (
    /\b(mockDataAnalysis|mockData|sampleData|fixtureData|demoData|testData)\s*:\s*Joi\./.test(
      lineStr,
    )
  )
    return true;
  ...
  if (
    /\bexpress\.static\s*\(\s*[^)]+web[/\\]data/.test(lineStr) ||
    /\bjoin\s*\(\s*__dirname\s*,\s*['"]\.\.\/web\/data['"]/.test(lineStr)
  )
    return true;
...
function scanFileContent(relativePath, content, options = {}) {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.regex.exec(content)) !== null) {
      ...
      if (
        shouldSuppressMatch({ relativePath, line, matchText: match[0] }) ||
        shouldSuppressMatch({ relativePath, line, matchText: snippet })
      )
        continue;
      ...
      const cardType =
        pattern.id === "sample-json-ref"
          ? "sample-json-ref"
          : pattern.id === "c-sample-data-ref" ||
              pattern.id === "c-mock-path-ref"
            ? "mock-path-leak"
            : pattern.id.startsWith("mock") || pattern.id.startsWith("c-")
              ? "mock-path-leak"
              : "production-leak";
      findings.push({
...
async function scanProductionLeaks(baseDir, options = {}) {
  ...
  const ignoreGlobs = Array.isArray(opts.ignoreGlobs)
    ? opts.ignoreGlobs
    : DEFAULT_IGNORE_GLOBS;
  const allowlistFiles = Array.isArray(opts.allowlistFiles)
    ? opts.allowlistFiles.map((p) => String(p).split("\\").join("/"))
    : [];
  const scannerMetaFiles = Array.isArray(opts.scannerMetaFiles)
