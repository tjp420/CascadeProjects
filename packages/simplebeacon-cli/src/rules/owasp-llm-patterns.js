...
const RULE_CATALOG = [
    {
        ...
        regex: /(?:prompt|messages|content|userMessage|inputText)\s*(?:\+?=|\.\s*push\s*\()\s*.*(?:req\.(?:body|query|params)|userInput|user_input|input_text|request\.(?:body|query))/gi,
        severity: 'high',
        description: 'User input concatenated into LLM prompt without sanitization',
        fixTemplate: 'Sanitize and validate user input before incorporating it into LLM prompts. Use a delimiter-based prompt structure (e.g., XML tags) to separate instructions from user content. Implement input validation and reject prompts containing override instructions.'
    },
    {
        id: 'OWASP-LLM02-001',
        owaspId: 'LLM02:2025',
        ...
        regex: /(?:openai|anthropic|claude|gpt|llm|chat|completion|embeddings?)\s*(?:\.\s*\w+)*\s*\.\s*(?:create|complete|generate|chat)\s*\(.*(?:email|ssn|password|secret|token|api_?key|credit_?card|phone|address|passport)/gi,
        severity: 'high',
        description: 'PII or secrets passed to LLM API calls',
        fixTemplate: 'Never pass PII or secrets to LLM APIs. Strip sensitive fields before sending data to external AI services. Use a data classification layer to redact PII. Log what data is sent (not the data itself) for audit purposes.'
    },
    {
        id: 'OWASP-LLM03-001',
        owaspId: 'LLM03:2025',
        ...
        regex: /(?:require|import)\s*\(?\s*['"](?:openai|anthropic|@anthropic-ai\/sdk|langchain|@langchain\/core|llamaindex|huggingface|transformers|@huggingface\/inference)['"]\s*\)?/gi,
        severity: 'medium',
        description: 'LLM/AI package imported without version pinning or integrity verification',
        fixTemplate: 'Pin LLM package versions in package.json with exact versions (not ^ or ~). Verify package integrity using npm config set integrity true. Review the package lockfile for unexpected transitive dependencies. Consider using npm audit signatures.'
    },
    {
        id: 'OWASP-LLM04-001',
        owaspId: 'LLM04:2025',
        ...
        regex: /(?:fetch|axios|http\.get|requests\.get)\s*\(\s*['"](?:https?:)?\/\/[^'"]*(?:scrape|crawl|dataset|training|corpus|raw\.githubusercontent)/gi,
        severity: 'medium',
        description: 'Training or fine-tuning data sourced from unvalidated URLs',
        fixTemplate: 'Validate all training data sources. Use checksums or hashes to verify data integrity. Implement data provenance tracking. Avoid scraping untrusted URLs for training data. Run data poisoning detection (e.g., spectral signature analysis) before training.'
    },
    {
        id: 'OWASP-LLM05-001',
        owaspId: 'LLM05:2025',
        ...
        regex: /(?:innerHTML|outerHTML|document\.write|dangerouslySetInnerHTML|eval\s*\(|new\s+Function\s*\()\s*(?:=|\()\s*(?:aiResponse|llmOutput|completion|response\.text|result\.content|model\.output|chatResponse)/gi,
        severity: 'high',
        description: 'LLM output rendered into DOM or executed without sanitization',
        fixTemplate: 'Treat all LLM output as untrusted. Sanitize with DOMPurify before rendering. Never pass LLM output to eval() or new Function(). Use textContent instead of innerHTML where possible. Validate LLM output against a schema before use.'
    },
    {
        id: 'OWASP-LLM06-001',
        owaspId: 'LLM06:2025',
        ...
        regex: /(?:exec|spawn|execSync|child_process|fetch|axios\.(?:get|post|put|delete)|db\.(?:query|execute|run)|database\.(?:query|execute)|sql\.)\s*\(\s*(?:aiCommand|llmResult|aiGenerated|model\.output|completion\.text|chatResponse\.content|llmOutput)/gi,
        severity: 'high',
        description: 'LLM output directly driving code execution, HTTP requests, or database queries',
        fixTemplate: 'Never allow LLM output to directly execute commands or queries. Implement an allowlist of permitted actions. Require human approval before executing LLM-suggested operations. Validate all LLM-generated SQL/commands against a strict schema before execution.'
    },
    {
        id: 'OWASP-LLM07-001',
        owaspId: 'LLM07:2025',
        ...
        regex: /(?:role|type)\s*:\s*['"]system['"]\s*,?\s*(?:content|text|prompt)\s*:\s*.*(?:api[_-]?key|secret|password|token|credential|private[_-]?key|BEGIN\s+(?:RSA|EC|OPENSSH))/gi,
        severity: 'medium',
        description: 'Secrets or credentials embedded in system prompts',
        fixTemplate: 'Never include secrets in system prompts — they can be extracted via prompt injection. Load secrets from environment variables at runtime, not in prompt text. Treat system prompts as potentially visible to end users.'
    },
    {
        id: 'OWASP-LLM08-001',
        owaspId: 'LLM08:2025',
        ...
        regex: /(?:embeddings?|vector|pinecone|weaviate|chroma|qdrant|pgvector)\s*\.\s*(?:create|upsert|insert|query|search)\s*\(.*(?:req\.(?:body|query)|userInput|untrusted)/gi,
        severity: 'medium',
        description: 'Embedding or vector search using unvalidated user input without access control',
        fixTemplate: 'Implement tenant isolation in vector databases. Validate and sanitize all inputs before generating embeddings. Enforce per-user or per-tenant access control on vector search results. Rate-limit embedding API calls to prevent vector database poisoning.'
    },
    {
        id: 'OWASP-LLM09-001',
        owaspId: 'LLM09:2025',
        ...
        regex: /(?:return|res\.(?:json|send|render)|response\.(?:json|send|render))\s*\(?\s*(?:aiResponse|llmOutput|completion|model\.output|chatResponse)/gi,
        severity: 'low',
        description: 'LLM output returned to users without fact-checking or hallucination disclaimer',
        fixTemplate: 'Add a visible disclaimer that output is AI-generated and may contain errors. Implement fact-checking or grounding against a verified knowledge base. Log AI-generated responses for audit. Consider confidence scoring before presenting output as factual.'
    },
    {
        id: 'OWASP-LLM10-001',
        owaspId: 'LLM10:2025',
        ...
        regex: /max_?tokens\s*:\s*(?:\d{6,}|999999|Infinity|undefined|null|0x[fF]+)|while\s*\(\s*(?:true|!done|running)\s*\)\s*\{[^}]*(?:openai|anthropic|llm|chat|completion)\s*\./gi,
        severity: 'high',
        description: 'LLM API calls without token caps, rate limits, or timeouts',
        fixTemplate: 'Set reasonable max_tokens limits (e.g., 4096). Implement rate limiting per user and per API key. Add timeouts to all LLM API calls. Monitor and alert on token consumption. Implement circuit breakers for cascading LLM failures.'
    }
];
