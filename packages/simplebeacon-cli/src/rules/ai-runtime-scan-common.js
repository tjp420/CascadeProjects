...
function isScannerImplementationPath(relativePath) {
  ...
  if (
    ...
    return true;
  if (
    /(?:^|\/)server\/lib\/(?:codebase-analyzer|production-leak|fiction-kpi|mock-data-scanner|simplebeacon-report|scan-orchestr|secret-config|audit-remediation-recipes|sample-path-resolver|code-roadmap-generator|dashboard-vault-auth|eu-ai-act-sprint-route)/.test(
      normalized,
    )
  )
    return true;
...
function isPlatformScannerMetaPath(relativePath) {
  const normalized = String(relativePath || "")
    ...
    .toLowerCase();
  if (
    /(?:^|\/)src\/api\/(?:dashboard-stub-api|simplebeacon-api|simplebeacon-billing-api)\.(?:js|cjs)$/.test(
      normalized,
    )
  ) {
    return true;
  ...
  if (
    /(?:^|\/)server\/services\/model-inference-service\.(?:js|cjs)$/.test(
      normalized,
    )
  ) {
    return true;
...
function isDashboardAnalyzerCatalogPath(relativePath) {
  const normalized = String(relativePath || "")
    ...
    .toLowerCase();
  if (
    /(?:^|\/)web\/simplebeacon-dashboard\/js\/views\/analyzeview\.js$/.test(
      normalized,
    )
  )
    return true;
...
function isUnderProductionPaths(
  relativePath,
  productionPaths = DEFAULT_PRODUCTION_PATHS,
) {
