...
function buildPatternsFromBaseline(baseline = {}) {
  ...
  for (const score of fiction.aiConfidenceScores || []) {
    ...
    patterns.push({
      ...
      regex: new RegExp(
        `(?:aiConfidence|confidence)\\s*[:=]\\s*["']?${value.replace(".", "\\.")}\\b`,
        "gi",
      ),
      severity: "medium",
...
function isDocumentationContext(line) {
  ...
  if (EXCLUSION_SUBSTRINGS.some((token) => lower.includes(token.toLowerCase())))
    return true;
...
function scanFileContent(relativePath, content, patterns, ext) {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.regex.exec(content)) !== null) {
      ...
      const cardType = pattern.id.startsWith("ai-confidence-")
        ? "hardcoded-confidence"
        : pattern.id.startsWith("completion-rate-")
      ...
      findings.push({
        ...
        recommendedAction:
          "Replace with measured repository-audit values or move to sample JSON only",
...
async function scanSourceFictionPatterns(baseDir, options = {}) {
  ...
  for (const rel of sourcePaths) {
    ...
    if (fs.existsSync(abs)) {
      await walkSourceFiles(abs, files, {
        baseDir,
        ignoreGlobs,
        pathExclusions,
      });
  ...
  for (const file of files) {
    ...
    issues.push(
      ...scanFileContent(file.relativePath, content, patterns, file.ext),
    );
  }
