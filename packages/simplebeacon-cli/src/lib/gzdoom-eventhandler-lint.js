...
function parseEventHandlerRegistrations(content, filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m) names.push({ name: m[1], filePath, line: i + 1, source: 'eventhandlers' });
    }
    return names;
}
...
function parseMapInfoHandlers(content, filePath) {
    ...
    while ((match = re.exec(content)) !== null) {
        ...
        while ((nameMatch = QUOTED_NAME_RE.exec(block)) !== null) {
            names.push({ name: nameMatch[1], filePath, line: content.slice(0, match.index).split('\n').length, source: 'mapinfo' });
        }
    }
    return names;
}
    ...
    function walk(dir, depth) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (/node_modules|\.git|tools|Docs|build_temp|Backup$/i.test(entry.name)) continue;
                walk(full, depth + 1);
                continue;
            }
...
async function lintGzdoomEventHandlers(modPath, options = {}) {
    ...
    const reachable = opts.respectIncludes !== false
    ...
    for (const file of collectLintFiles(root, ignoreGlobs)) {
        ...
        if (lower === 'eventhandlers') registrations.push(...parseEventHandlerRegistrations(content, file.relativePath));
        if (lower === 'mapinfo') registrations.push(...parseMapInfoHandlers(content, file.relativePath));
    }
    ...
    for (const reg of registrations) {
        ...
        issues.push({
            ...
            recommendedAction: 'Include the handler ZScript in the manifest or remove the registration entry',
            affectedFiles: [reg.filePath],
            metadata: { engine: 'gzdoom-eventhandler-lint', handler: reg.name, source: reg.source }
        });
    }
    ...
    for (const [name, regList] of groupBy(registrations, (r) => r.name)) {
        ...
        if (regList.length > 1 && sources.length > 1) {
            issues.push({
                ...
                recommendedAction: 'Register each handler in only one place (EVENTHANDLERS or MAPINFO AddEventHandlers)',
                affectedFiles: regList.map((r) => r.filePath),
                metadata: { engine: 'gzdoom-eventhandler-lint', handler: name, sources }
            });
        }
    }
    ...
    for (const [name, cls] of handlerClasses) {
        ...
        issues.push({
            ...
            recommendedAction: 'Add to EVENTHANDLERS or MAPINFO AddEventHandlers if it should run at runtime',
