...
function buildAnonymizedAggregate(issues) {
  ...
  for (const issue of issues) {
    ...
    byCategory[typeMeta.category] =
      (byCategory[typeMeta.category] || 0) + count;
  }
...
function buildRulesFingerprint(scanScope = {}) {
  const rules = Array.isArray(scanScope.rulesEnabled)
...
function buildAnonymizedExport(report) {
  ...
  const anonymizedIssues = rawIssues.map((issue, idx) =>
    anonymizeIssue(issue, idx),
  );
  ...
  const payload = {
    ...
    gate: {
      ...
      blockingCount:
        typeof gate.blockingCount === "number" ? gate.blockingCount : 0,
      warningCount:
        typeof gate.warningCount === "number" ? gate.warningCount : 0,
    },
    metrics: {
...
function attachAnalyzerSuiteToReport(report, analyzerSuiteExport) {
  ...
  if (!analyzerSuiteExport || typeof analyzerSuiteExport !== "object")
    return report;
...
function signAnonymizedExport(payload, secret) {
  const hmacKey =
    secret ||
    process.env.SIMPLEBEACON_ANON_HMAC ||
    "simplebeacon-anon-insecure";
