...
const MENU_CVAR_RE =
  /(?:Option|Slider|Toggle)\s+"[^"]*"\s*,\s*"([a-zA-Z_][a-zA-Z0-9_]*)"/g;
const ZSCRIPT_CVAR_RE =
  /(?:CVar\.)?FindCVar\s*\(\s*"([a-zA-Z_][a-zA-Z0-9_]*)"\s*\)/g;
...
async function lintGzdoomCvars(modPath, options = {}) {
  ...
  for (const ref of references) {
    ...
    const issueSeverity = isMenu
      ? severity
      : opts.zscriptUndefinedSeverity || "medium";
    const bucket = undefinedByName.get(ref.name);
    if (bucket) {
      bucket.refs.push(ref);
      if (issueSeverity === "high" && bucket.severity !== "high")
  ...
  for (const entry of undefinedByName.values()) {
    ...
    issues.push({
      ...
      affectedFiles: [...new Set(entry.refs.map((r) => r.filePath))].slice(
        0,
        20,
      ),
