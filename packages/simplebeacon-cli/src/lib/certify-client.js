...
const DEFAULT_PUBLIC_KEY_URL = 'https://simplebeacon.ai/api/v1/certify/public-key';
...
async function certifyReport(reportPath, options = {}) {
    ...
    const certPath = options.outputPath || reportPath.replace(/\.json$/i, '.sbcert');
...
async function verifyCertificate(certPath, reportPath, publicKeyUrl) {
    ...
    if (certFile.payload.hash !== reportHash) {
        return { valid: false, reason: 'Report hash mismatch — report has been modified since certification' };
    }
    ...
    if (typeof process !== 'undefined' && process.versions && process.versions.node) {
        ...
        const response = await fetch(url);
        if (!response.ok) {
            return { valid: false, reason: `Failed to fetch public key: HTTP ${response.status}` };
        }
