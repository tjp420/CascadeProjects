...
const PII_CATEGORIES = [
  ...
  },
  {
    ...
    test: (path) =>
      /web\/data\//.test(path) || /mock|sample|fixture|seed|snapshot|-sample\.json/i.test(path),
  },
  {
    id: 'test-fixture',
...
const CREDENTIAL_CATEGORIES = [
  {
    ...
    label: 'Intentional test fixture',
    test: (path) =>
      /simplebeacon-toxic-fixtures|toxic-fixtures|simplebeacon-rule-tests/i.test(path),
  },
  {
...
function triagePrivacyFindings(findings = []) {
  ...
  return {
    ...
    credentialsNeedingReview: triaged.filter(
      (item) => item.kind === 'credential' && item.category === 'review-required'
    ).length,
  };
}
