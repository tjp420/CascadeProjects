...
const PII_CATEGORIES = [
  {
    ...
    test: (path) =>
      /docs\/|GUIDELINES|SAMPLE_REPORT|MARKETING\.md|MockData/i.test(path),
  },
  {
    id: "generated-report",
  ...
  },
  {
    ...
    test: (path) =>
      /web\/data\//.test(path) ||
      /mock|sample|fixture|seed|snapshot|-sample\.json/i.test(path),
  },
  {
    id: "test-fixture",
    label: "Intentional test fixture",
    test: (path) =>
      /simplebeacon-toxic-fixtures|toxic-fixtures|tests\/fixtures|simplebeacon-rule-tests/i.test(
        path,
      ),
  },
];
...
const CREDENTIAL_CATEGORIES = [
  {
    ...
    label: "Intentional test fixture",
    test: (path) =>
      /simplebeacon-toxic-fixtures|toxic-fixtures|simplebeacon-rule-tests/i.test(
        path,
      ),
  },
  {
...
function privacyCategoryLabel(categoryId) {
  ...
  return (
    all.find((entry) => entry.id === categoryId)?.label || "Review required"
  );
}
...
function triagePrivacyFindings(findings = []) {
  const triaged = findings.map((finding) => {
    ...
    return {
      ...
      kind: String(finding.reason || "").includes("Credential")
        ? "credential"
        : "pii",
      category,
  ...
  return {
    ...
    piiNeedingReview: triaged.filter(
      (item) => item.kind === "pii" && item.category === "review-required",
    ).length,
    credentialsNeedingReview: triaged.filter(
      (item) =>
        item.kind === "credential" && item.category === "review-required",
    ).length,
  };
}
