...
function normalizeTier(tier) {
  ...
  const reverse = Object.fromEntries(Object.entries(TIER_ALIASES).map(([k, v]) => [v, k]));
  return reverse[t] || t;
}
...
function generateLicenseToken(claims, secret, ttlMinutes = 60) {
  ...
  const scanQuota =
    Number.isFinite(claims.scanQuota) && claims.scanQuota >= 0
      ? claims.scanQuota
...
function isTokenExpiringSoon(token, thresholdMinutes = 10) {
  ...
  const threshold =
    Number.isFinite(thresholdMinutes) && thresholdMinutes > 0 ? thresholdMinutes : 10;
  const parts = token.split('.');
