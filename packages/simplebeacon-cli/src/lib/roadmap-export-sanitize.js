...
function stripFictionCoverageFromText(text) {
    ...
    let out = text
        ...
        .replace(/current test coverage of \d+(?:\.\d+)?%/gi, 'coverage requires live Jest run')
        .replace(/minimum test coverage of \d+(?:\.\d+)?%/gi, 'minimum test coverage requires live Jest baseline')
...
function sanitizeRoadmapExport(roadmap, options = {}) {
    ...
    if (benchmarkScan) {
        ...
        if (misscopedPlatformWalk && next.aiIntegration) {
            next.aiIntegration = sanitizeAiIntegrationForBenchmark(next.aiIntegration, true);
        } else if (next.aiIntegration) {
            next.aiIntegration = sanitizeAiIntegrationForBenchmark(next.aiIntegration, false);
        }
