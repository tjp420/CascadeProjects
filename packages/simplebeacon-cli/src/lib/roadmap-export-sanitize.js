/**
 * Sanitize dynamic-project-roadmap-analysis exports — benchmark clones, template sprint fiction.
 */
...
/** Generic Simplebeacon product-sprint template lines — not valid on OSS benchmark clones */
...
function inferScanTargetRootFromHints(roadmap, options = {}) {
  ...
  const sourceRoot = normalizeExportPath(
    roadmap?.sourceProjectPath || roadmap?.projectRoot || "",
  );
      ...
      ? Object.fromEntries(
          Object.entries(next.projectStructure.mainCategories).map(
            ([key, category]) => [
              ...
              {
                ...category,
                path: normalizeExportPath(category.path || ""),
              },
            ],
          ),
        )
...
function stripFictionCoverageFromText(text) {
  ...
  let out = text
    ...
    .replace(
      /current test coverage of \d+(?:\.\d+)?%/gi,
      "coverage requires live Jest run",
    )
    .replace(
      /minimum test coverage of \d+(?:\.\d+)?%/gi,
      "minimum test coverage requires live Jest baseline",
    )
...
function sanitizeProductStrategicInsightsForExport(insights) {
  ...
  if (Array.isArray(next.recommendations)) {
    next.recommendations = next.recommendations.map((rec) => {
      ...
      return {
        ...rec,
        action: stripFictionCoverageFromText(rec.action),
      };
    });
  }
...
function buildProductRoadmapHygieneSummary(roadmap, gateContext = {}) {
  ...
  return {
    ...
    coverageEvidenceSource:
      roadmap.coverageEvidenceSource ??
      (roadmap.progressMetrics?.metrics?.testCoverage != null
...
function sanitizeProductDevelopmentPhases(phases, codeAnalysis) {
  ...
  return phases.map((phase) => {
    ...
    return features.some(
      (feature, index) => feature !== phase.features?.[index],
    )
      ? { ...phase, features }
      : phase;
  });
...
function sanitizeCodebaseMetricsForBenchmark(metrics, structure) {
  ...
  return {
    ...
    documentation: {
      readmeFiles:
        structure.languages?.[".md"] ??
        metrics?.documentation?.readmeFiles ??
        0,
...
function isBenchmarkRoadmap(roadmap, options = {}) {
  return resolveRoadmapExportContext(roadmap, options).benchmarkScan;
}
...
function recommendationsNeedBenchmarkReplace(rec) {
  ...
  return matchesRoadmapTemplate(blob);
}
...
function benchmarkRecommendations() {
  return {
    ...
    shortTerm: [
      ...
      "Do not use Simplebeacon v1-internal or gate deploy runbooks for this clone",
    ],
  ...
  if (Array.isArray(next.features)) {
    if (misscopedPlatformWalk) {
      ...
      next.features = next.features.filter((feature) => {
        ...
        if (/1000\/1000/.test(label) || Number(feature?.count) === 1000)
          return false;
 ...
 * @returns {object}
 */
function sanitizeRoadmapExport(roadmap, options = {}) {
  ...
  if (benchmarkScan) {
    ...
    if (misscopedPlatformWalk && next.aiIntegration) {
      next.aiIntegration = sanitizeAiIntegrationForBenchmark(
        next.aiIntegration,
        true,
      );
    } else if (next.aiIntegration) {
      next.aiIntegration = sanitizeAiIntegrationForBenchmark(
        next.aiIntegration,
        false,
      );
      ...
      next.projectStructure = alignProjectStructureInventory(
        {
          ...
          platformRoot:
            productPlatformRoot || next.projectStructure.platformRoot,
    ...
    next = sanitizeProductRoadmapExport(next, options);
  }
...
module.exports = {
  SIMPLEBEACON_ROADMAP_MARKERS,
  isBenchmarkRoadmap,
  resolveRoadmapExportContext,
  resolveRoadmapGateContext,
  inferScanTargetRootFromHints,
  sanitizeRoadmapExport,
  sanitizeProductRoadmapExport,
  isStaleRoadmapCoverageMetrics,
  filterTemplateLines,
  benchmarkRecommendations,
  dedupeRoadmapExportNotes,
  collapseDuplicateBenchmarkPhases,
};
