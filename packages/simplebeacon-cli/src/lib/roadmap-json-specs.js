...
function validateRoadmapJson(fileName, payload, baseline) {
  ...
  if (
    spec.requireRepositoryAudit &&
    baseline?.dataSource &&
    payload.dataSource !== baseline.dataSource
  ) {
    violations.push({
      kind: "data-source",
  ...
  if (spec.legacyFiction && !spec.canonical) {
    violations.push({
      kind: "legacy-fiction",
      message:
        "Archived legacy export — use data/roadmap/ai-roadmap-report.json (repository-audit)",
    });
  }
  ...
  for (const [fileName, spec] of Object.entries(ROADMAP_JSON_SPECS)) {
    ...
    if (spec.maxBytes && stat.size > spec.maxBytes) {
      issues.push({
        ...
        recommendedAction:
          "Replace with measured baseline or archive outside scan paths",
        affectedFiles: [fileName],
    ...
    if (result.valid) {
      ...
      const severity = result.violations.some(
        (v) => v.kind === "legacy-fiction",
      )
        ? "medium"
        : "high";
      issues.push({
