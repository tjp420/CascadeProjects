...
function resolveGateInventoryTotals(
  gateReport,
  currentGate = null,
  hygiene = null,
) {
  const repositoryFilesTotal =
    currentGate?.repositoryFilesTotal ??
    gateReport?.repositoryFilesTotal ??
    gateReport?.repositoryInventory?.totalFiles ??
  ...
  const ruleScopedFilesAnalyzed =
    currentGate?.ruleScopedFilesAnalyzed ??
    gateReport?.ruleScopedFilesAnalyzed ??
    gateReport?.scanScope?.ruleScopedFilesAnalyzed ??
...
function resolveReAttestationGateContext(payload, options = {}) {
  ...
  const currentGate =
    resolveCurrentGateSnapshot(gateReport) ??
    (payload?.currentGate && typeof payload.currentGate === "object"
      ? payload.currentGate
      : null);
  const { repositoryFilesTotal, ruleScopedFilesAnalyzed, credentialScanned } =
    resolveGateInventoryTotals(gateReport, currentGate, hygiene);
  ...
  const fictionSampleFilesScanned =
    ...
    null;
  const gatePass =
    currentGate?.pass ?? hygiene.gatePass ?? gateReport.gate?.pass ?? null;
  const blockingCount =
    currentGate?.blockingCount ??
    hygiene.blockingCount ??
    gateReport.gate?.blockingCount ??
    gateReport.issueCount ??
    null;
  const qualityScore =
    currentGate?.qualityScore ??
    hygiene.qualityScore ??
    gateReport.qualityScore ??
    null;
  const jestBaselineChecked =
    currentGate?.jestBaselineChecked === false ||
    hygiene.jestBaselineChecked === false ||
    gateReport.jestBaselineChecked === false ||
    gateReport.scanScope?.jestExecutedDuringScan === false
      ? false
      : (currentGate?.jestBaselineChecked ??
        hygiene.jestBaselineChecked ??
        true);
  const generatedAt =
    currentGate?.generatedAt ??
    hygiene.gateGeneratedAt ??
...
function buildReAttestationHygieneSummary(tierId, payload, options = {}) {
  ...
  return {
    workflowStatus:
      tierId === "operator" ? "reference-only" : "awaiting-operator-comparison",
    ...
    jestBaselineChecked:
      currentGate?.jestBaselineChecked ?? jestBaselineChecked,
...
function buildReAttestationExportNotes(tierId, gateContext = {}) {
  ...
  const gateProfileResolved =
    gateProfile ?? gateReport?.scanScope?.profile ?? null;
  if (gate) {
    notes.push(
    ...
    if (
      gate.repositoryFilesTotal != null &&
      gate.ruleScopedFilesAnalyzed != null &&
    ...
    if (gateProfileResolved) {
      notes.push(
        `Gate rule bundle profile: ${gateProfileResolved} — compare bundled gate to original clearance499 scan profile before re-attestation sign-off.`,
      );
    }
    if (gate.jestBaselineChecked === false) {
      notes.push(
        "Bundled gate did not run Jest — compare test attestation separately before warranty re-attestation sign-off.",
      );
    }
    if (tierId === "operator") {
      notes.push(
        "For full gate metrics use json/simplebeacon-gate.json bundled in this operator ZIP.",
      );
    }
  } else {
    notes.push(
      "No gate report was bundled — attach json/simplebeacon-gate.json before operator comparison.",
    );
  }
...
function sanitizeReAttestationNoteArtifactExport(payload, options = {}) {
  if (!payload || payload.type !== "simplebeacon-re-attestation-note")
    return payload;

  const tierId = options.tierId || payload.tier || "operator";
  const rawPath =
    options.projectPath || payload.projectPath || payload.originalProject || "";
  ...
  return {
    ...
    scanScope: buildReAttestationScanScope(
      payload.scanScope,
      tierId,
      payload,
      options,
