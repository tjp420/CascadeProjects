...
function resolveCurrentGateSnapshot(gateReport) {
  ...
  return {
    ...
    ruleScopedFilesAnalyzed:
      gateReport.ruleScopedFilesAnalyzed ?? gateReport.scanScope?.ruleScopedFilesAnalyzed ?? null,
    repositoryFilesTotal:
      gateReport.repositoryFilesTotal ?? gateReport.repositoryInventory?.totalFiles ?? null,
...
function resolveReAttestationGateContext(payload, options = {}) {
  ...
  const blockingCount =
    ...
    null;
  const qualityScore =
    currentGate?.qualityScore ?? hygiene.qualityScore ?? gateReport.qualityScore ?? null;
  ...
  const generatedAt =
    currentGate?.generatedAt ?? hygiene.gateGeneratedAt ?? gateReport.generatedAt ?? null;
...
function buildReAttestationNoteArtifact(options = {}) {
  ...
  return sanitizeReAttestationNoteArtifactExport(
    ...
    },
    {
      ...
      repositoryFilesTotal:
        options.repositoryFilesTotal ?? options.gateReport?.repositoryFilesTotal ?? null,
    }
  );
}
