...
const GLDEFS_TEXTURE_RE = /\b(?:brightmap|glow|material)\s+(?:texture\s+)?"([^"]+)"/gi;
...
function collectGldefsLumpFiles(root, ignoreGlobs = []) {
    const files = [];
    const skipDirs = new Set(['node_modules', '.git', 'coverage', 'dist', 'build', '.simplebeacon', 'github-cache', 'archive', 'tools', 'Docs', 'build_temp', 'Backup']);
    function walk(dir, depth) {
        ...
        for (const entry of entries) {
            ...
            const isSprite = /\.(png|jpg|jpeg|webp)$/i.test(lower) && (rel.includes('/sprites/') || rel.startsWith('sprites/'));
            if (isGl || isTex || isVox) files.push({ path: full, relativePath: rel, name: entry.name, kind: 'lump' });
            else if (isSprite) files.push({ path: full, relativePath: rel, name: entry.name, kind: 'sprite' });
        }
    }
...
function isGldefsFile(name) {
    ...
    return lower === 'gldefs' || lower.startsWith('gldefs_') || lower.endsWith('gldefs.txt');
}
        ...
        if (lower === 'textures.txt' || lower.endsWith('/textures.txt')) {
            ...
            while ((match = TEXTURE_PATCH_RE.exec(content)) !== null) {
                ...
                issues.push({
                    ...
                    recommendedAction: 'Add the PNG patch file or fix the path in TEXTURES.txt',
            ...
            for (const re of [GLDEFS_MAP_RE, GLDEFS_TEXTURE_RE]) {
                ...
                while ((match = re.exec(content)) !== null) {
                    ...
                    issues.push({
                        ...
                        recommendedAction: 'Add the texture/brightmap file or remove the GLDEFS entry',
                        affectedFiles: [file.relativePath],
            ...
            while ((match = VOXELDEF_ENTRY_RE.exec(content)) !== null) {
                ...
                if (!fileExistsAt(root, kvxPath)) {
                    ...
                    issues.push({
                        ...
                        recommendedAction: 'Add the .kvx file or comment out the VOXELDEF entry',
                        affectedFiles: [file.relativePath],
                ...
                if (spriteNames.size && !spriteNames.has(sprite.trim())) {
                    ...
                    if (!reported.has(skey)) {
                        ...
                        issues.push({
                            ...
                            recommendedAction: 'Add sprite lump or verify the 4-char sprite name',
                            affectedFiles: [file.relativePath],
