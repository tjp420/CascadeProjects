...
const DEFAULT_VALIDATE_URL = process.env.SIMPLEBEACON_LICENSE_VALIDATE_URL
    || 'https://simplebeacon.ai/api/license/validate';
...
const DEFAULT_UPGRADE_URL = process.env.SIMPLEBEACON_UPGRADE_URL
...
async function tryRemoteValidation(token) {
    ...
    try {
        ...
        if (data && (data.active === true || data.registered === true || data.valid === true)) {
            ...
            const active = data.active === true || (data.hasActiveSubscription === true && data.valid === true) || (data.active === undefined && data.registered === true);
...
async function resolveCiLicense(options = {}) {
    ...
    if (inPipeline || options.allowRemote) {
        ...
        return {
            ok: false,
            error: 'invalid_token',
            message: 'SIMPLEBEACON_LICENSE_TOKEN is present but invalid or expired. Fix the secret or remove it for community mode.'
        };
    }
    ...
    return {
        ...
        warning: 'License token could not be verified locally (set SIMPLEBEACON_LICENSE_SECRET for offline validation).'
    };
}
