...
const DEFAULT_VALIDATE_URL =
  process.env.SIMPLEBEACON_LICENSE_VALIDATE_URL ||
  "https://simplebeacon.ai/api/license/validate";
const VALIDATE_TIMEOUT_MS =
  Number(process.env.SIMPLEBEACON_LICENSE_VALIDATE_TIMEOUT_MS) || 4500;
...
function tierFromClaims(claims) {
  const raw = String(
    claims?.tier || claims?.product || "developer",
  ).toLowerCase();
...
async function tryRemoteValidation(token) {
  ...
  try {
    ...
    if (
      data &&
      (data.active === true || data.registered === true || data.valid === true)
    ) {
  ...
  if (inPipeline || options.allowRemote) {
    ...
    if (remote.networkError && failOpenOnNetwork) {
      return {
        ...
        warning:
          "License server unreachable — running community gate (scan not blocked).",
      };
    }
    if (failOpenOnExpired) {
      return {
        ...
        warning:
          "SIMPLEBEACON_LICENSE_TOKEN is expired or invalid. Running community gate (scan not blocked). Renew at " +
          DEFAULT_UPGRADE_URL +
          " or set --strict-license to enforce.",
      };
    }
    return {
      ok: false,
