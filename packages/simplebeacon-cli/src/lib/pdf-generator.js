...
const PILLARS = {
  slop: {
    ...
    businessPitch:
      "Prevents shipping fake AI-generated variables to live clients",
  ...
  leak: {
    ...
    businessPitch:
      "Neutralises exposed backend access tokens before hackers find them",
  ...
  shadowAi: {
    ...
    businessPitch:
      "Maps entire software structure to catch unapproved AI dependencies",
    regulatoryFrameworks: ["EU AI Act Annex III", "NIST AI RMF 1.0"],
    avgFinePerIncident: 350000,
    findingTypes: [
      "EU AI Act — AI System Indicator",
      "Shadow AI",
      "Unapproved Model",
    ],
  },
...
function buildRiskProfile(report) {
  ...
  for (const issue of detectedIssues) {
    ...
    profile[pillarKey][sev] =
      (profile[pillarKey][sev] || 0) + (issue.count || 1);
  ...
  for (const [key, data] of Object.entries(profile)) {
    ...
    for (const [sev, count] of Object.entries(data)) {
      if (["critical", "high", "medium", "low"].includes(sev) && count > 0) {
        pillarTotal +=
          count * pillar.avgFinePerIncident * (SEVERITY_MULTIPLIERS[sev] || 1);
      }
    }
    total += pillarTotal;
    breakdown.push({
...
function buildExecutiveHtml(report, licenseClaims) {
  ...
  const pillarCards = Object.entries(PILLARS)
    .map(([key, pillar]) => {
      ...
      const badgeColor =
        data.critical > 0 ? "#f8d7da" : data.high > 0 ? "#fff3cd" : "#d1e7dd";
      const badgeText =
        data.critical > 0 ? "#842029" : data.high > 0 ? "#664d03" : "#0f5132";
      return `
...
async function generateExecutivePdf(reportPath, outputPath) {
  ...
  const resolvedReport = path.resolve(
    reportPath || ".simplebeacon/report.json",
  );
  ...
  const resolvedOutput = path.resolve(
    outputPath || "simplebeacon-executive-risk-certificate.html",
  );
  try {
