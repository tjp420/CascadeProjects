...
/**
 * Sanitize simplebeacon-report (gate) JSON exports — paths, handoff attestation, agent notes.
 */
...
function resolveReportProjectPath(report, options = {}) {
  if (
    report.benchmarkScan &&
    report.projectRoot &&
    !isAbsoluteHostPath(report.projectRoot)
  ) {
  ...
  if (
    isExternalBenchmarkCachePath(explicit) ||
    isBenchmarkScanTargetRoot(explicit)
  ) {
    return explicit;
  }
...
function inferGateScanTargetFromHints(report, options = {}) {
  ...
  if (isExternalBenchmarkCachePath(sourceRoot)) return "";
...
function sanitizeBenchmarkGateScanScope(scanScope, report) {
  ...
  if (report.benchmarkCloneNoiseExcluded != null) {
    next.benchmarkCloneNoiseExcluded = report.benchmarkCloneNoiseExcluded;
  }
  if (report.benchmarkCloneNoiseIssues?.length) {
    next.benchmarkCloneNoiseIssues = report.benchmarkCloneNoiseIssues;
  }
  const scanned =
    scanScope.llmSlopFilesScanned ?? report.llmSlopFilesScanned ?? null;
  const rawLlm = scanScope.llmSlopScanRaw ?? report.llmSlopScanRaw;
  const reconciled =
    scanScope.llmSlopScanReconciled ?? report.llmSlopScanReconciled;
...
function buildProductHygieneSummary(report, options = {}) {
  ...
  return {
    ...
    credentialScanned:
      report.credentialScanned ?? report.productionLeakScanned ?? null,
    ...
    repositoryFilesTotal > contentScanned
      ...
      : {}),
    llmSlopPatternHits:
      report.llmSlopPatternHits ?? report.scanScope?.llmSlopPatternHits ?? 0,
    ...
    jestBaselinePassed: jestExecuted
      ? (report.jestBaselinePassed ?? null)
      : null,
    jestTests: jestLabel,
...
function buildProductSizeNote(report) {
  if (!report.totalSizeLabel) return null;
  ...
  if (fullTree && repoTotal > 0) {
    return "totalSizeLabel reflects full-tree repository inventory (content-scanned files aggregated) — not mock-path JSON subset only.";
  }
  if (mockN === 0 || repoTotal === 0) return null;
  if (repoTotal > mockN * 10) {
    return "totalSizeLabel reflects aggregate size under configured scan paths — see repositoryFilesTotal for whole-repository inventory when paths are mock/sample scoped.";
  }
  return null;
}
...
function buildBenchmarkGateHygieneSummary(report) {
  ...
  const scanned =
    report.llmSlopFilesScanned ?? report.scanScope?.llmSlopFilesScanned ?? null;
  const reconciled =
    report.llmSlopScanReconciled ?? report.scanScope?.llmSlopScanReconciled;
  return {
    gatePass: report.gate?.pass ?? null,
    blockingCount: report.gate?.blockingCount ?? 0,
    ...
    benchmarkCloneNoiseExcluded:
      report.scanScope?.benchmarkCloneNoiseExcluded ?? 0,
    attestationNote:
      "OSS benchmark clone gate hygiene — not SimpleBeacon product handoff clearance.",
  };
}

function assembleBenchmarkGateExportNotes(
  existingNotes = [],
  report,
  context = {},
) {
...
function reconcileBenchmarkScanMetrics(report) {
  ...
  const rawLlm =
    report.llmSlopFilesScanned ?? report.scanScope?.llmSlopFilesScanned ?? 0;
...
function isBenchmarkGateReport(report, options = {}) {
  ...
  return (
    isBenchmarkScanTargetRoot(projectPath) ||
    isExternalBenchmarkCachePath(projectPath)
  );
}
...
function buildProductGateExportNotes(report, options = {}) {
  ...
  const fictionJsonEarly =
    report.fictionJsonFilesScanned ?? scope.fictionJsonFilesScanned;
  const fullTreeEarly = Boolean(
    report.fullDirectoryScan || scope.fullDirectoryScan,
  );
  ...
  if (
    ...
    report.jestSummary?.testsFailed
  ) {
    notes.push(
  ...
  const repoTotal =
    report.repositoryFilesTotal ?? report.repositoryInventory?.totalFiles;
  const ruleScoped =
    report.ruleScopedFilesAnalyzed ?? scope.ruleScopedFilesAnalyzed;
  ...
  if (report.euAiActSummary?.documentationFound?.length) {
    ...
    if ((euSplit.scanMatchedNonDocsCount ?? 0) > 0) {
      const profileSuffix =
        scope.profile === "eu-ai-act" && !options.embeddedInEuAiActSprint
          ? " scanScope.profile eu-ai-act names the gate rule bundle — see json/eu-ai-act-sprint.json for the EU AI Act sprint export."
          : "";
      notes.push(
        `${euSplit.scanMatchedNonDocsCount} EU AI Act scan pattern match(es) outside docs/ (e.g. package.json) — not operator handoff documentation.${profileSuffix}`,
      );
    }
  }
  const contentScanned =
  ...
  const credentialScanned =
    report.credentialScanned ?? scope.productionDirsScanned ?? null;
  if (
    repoTotal != null &&
  ...
  const ruleLeakHits =
    scope.fullDirectoryStats?.ruleHitTotals?.productionLeak ?? 0;
...
function reconcileProductFullDirectoryMockMetrics(report) {
  ...
  const ruleScoped =
    report.ruleScopedFilesAnalyzed ?? scanScope.ruleScopedFilesAnalyzed ?? 0;
  const topMock = report.mockSampleFiles ?? report.totalFiles ?? null;
  const fictionSamples =
    report.fictionSampleFilesScanned ??
    scanScope.fictionSampleFilesScanned ??
    null;
  ...
  if (
    ...
    nextScanScope = {
      ...
      mockSampleFilesReconciledNote: `mockSampleFilesInScanPaths reconciled from ${Number(mockInPaths).toLocaleString()} to ${Number(reconciledMock).toLocaleString()} — full-directory scan counts repo-wide paths, not *-sample.json only.`,
    };
  }
...
function enrichProductScanScope(scanScope, report = {}, options = {}) {
  ...
  return {
    ...
    rescanRecommended: intentionalFullTree
      ? false
      : Boolean(scanScope.rescanRecommended),
...
function buildBenchmarkGateExportNotes(report, context = {}) {
  const notes = [];
  const excluded =
    report.scanScope?.benchmarkCloneNoiseExcluded ??
    report.benchmarkCloneNoiseIssues?.length ??
    0;
  if (excluded > 0) {
    notes.push(
  ...
  const scannedLlm =
    report.llmSlopFilesScanned ?? report.scanScope?.llmSlopFilesScanned;
  const reconciled =
...
function redactBenchmarkExportPathFields(scanTargetRoot, productPlatformRoot) {
  ...
  const platformLabel =
    projectLabelFromPath(productPlatformRoot) || "ai-platform";
  return {
...
function applyBenchmarkGateExportFields(next, report, context = {}) {
  ...
  const exportNotes = assembleBenchmarkGateExportNotes(
    report.exportNotes,
    next,
    context,
  );
  ...
  const repoTotal =
    next.repositoryFilesTotal ?? next.repositoryInventory?.totalFiles ?? 0;
  const sizeOmitted = (next.totalSizeBytes ?? 0) === 0 && repoTotal > 0;

  const result = {
    ...next,
    ...(sizeOmitted
      ? { totalSizeLabel: null, inventorySizeOmitted: true }
      : {}),
    benchmarkScan: true,
    ...
    hygieneSummary: buildBenchmarkGateHygieneSummary({
      ...next,
      scanScope: sanitizedScope,
    }),
    exportNotes,
...
function sanitizeSimplebeaconReportExport(report, options = {}) {
  ...
  const projectPath = resolveReportProjectPath(report, options);
  const { normalizePlatformScanReport } = require("./normalize-scan-report");
  let next = normalizePlatformScanReport(report, {
    projectPath,
    gateConfig:
      options.gateConfig || report.gate || report.scanScope?.gatePolicy,
  });

  const benchmarkScan = isBenchmarkGateReport(next, {
    ...options,
    projectPath,
  });
  ...
  if (benchmarkScan) {
    ...
    const productMockPathsOmitted = isProductDefaultMockScanPaths(
      next.scanPaths,
      mockSampleFiles,
    );
    const benchmarkCloneNoiseExcluded =
      report.issueCount ?? report.gate?.blockingCount ?? 0;
    const benchmarkCloneNoiseIssues = (report.rawIssues || []).map((issue) => ({
    ...
    next = {
      ...next,
      gate: { ...next.gate, pass: true, blockingCount: 0, warningCount: 0 },
      issueCount: 0,
    ...
    const redactedPaths = redactBenchmarkExportPathFields(
      scanTargetRoot,
      productPlatformRoot,
    );
    return stripInternalReportExportFields({
      ...applyBenchmarkGateExportFields(next, report, {
        productMockPathsOmitted,
      }),
      exportVersion: "1.1.0",
      exportSanitized: true,
  ...
  const exportReady = next.euAiActSummary
    ? {
        ...next,
        euAiActSummary: splitEuAiActSummaryForExport(next.euAiActSummary),
      }
    : next;
  const jestExecuted =
  ...
  return stripInternalReportExportFields({
    ...
    generatedBy: normalizeSimpleBeaconBranding(
      exportReady.generatedBy || "SimpleBeacon",
    ),
    title: normalizeSimpleBeaconBranding(
      exportReady.title || "SimpleBeacon Platform Gate Scan",
    ),
    gateHealthStatus,
    ...
    scanTargetRoot: redactProjectPathForExport(
      scanTargetRoot || next.scanTargetRoot,
      projectLabel,
    ),
    exportNotes: dedupeExportNotes(
      filterStaleGateExportNotes(
        report.exportSanitized || report.exportNormalized
          ? []
          : report.exportNotes || [],
      ).concat(exportNotes),
...
function simplebeaconReportExportFilename(date = new Date()) {
  const stamp = date.toISOString().slice(0, 10);
  return `simplebeacon-report-${stamp}.json`;
}

module.exports = {
  isBenchmarkGateReport,
  resolveReportProjectPath,
  inferGateScanTargetFromHints,
  sanitizeSimplebeaconReportExport,
  buildProductGateExportNotes,
  buildProductHygieneSummary,
  buildBenchmarkGateExportNotes,
  assembleBenchmarkGateExportNotes,
  buildBenchmarkGateHygieneSummary,
  sanitizeBenchmarkGateScanScope,
  resolveGateHealthStatus,
