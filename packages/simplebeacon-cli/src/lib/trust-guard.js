...
function createNetworkGuard(options = {}) {
  ...
  if (originals.fetch) {
    globalThis.fetch = async function patchedFetch(input, init) {
      const target =
        typeof input === "string" ? input : input?.url || "fetch:unknown";
      record("fetch", target);
  ...
  return {
    ...
    assertOfflineClean() {
      if (offline && events.length > 0) {
        ...
        throw new Error(
          `Offline mode detected network activity (${sample.kind} → ${sample.target})`,
        );
      }
    },
    dispose() {
...
function printTrustCompletion(options = {}, paint = (text) => text) {
  ...
  if (options.networkEventCount === 0) {
    ...
    console.error(
      paint(
        `Network activity: ${options.networkEventCount} request(s) (use --offline to enforce zero)`,
        "yellow",
      ),
    );
  }
