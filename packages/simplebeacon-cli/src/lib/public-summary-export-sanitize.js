...
function flattenPublicSummaryBlock(block) {
  ...
  return {
    summary: looksLikePublicMetrics(summary)
      ? summary
      : summary?.summary || summary || {},
    severityCounts,
  };
...
function resolvePublicSummaryGateContext(payload, options = {}) {
  ...
  const gatePass =
    gateReport.gate?.pass ?? summary.gatePass ?? hygiene.gatePass ?? null;
  const blockingCount =
...
function buildPublicSummaryHygieneSummary(summary, payload, options = {}) {
  ...
  return {
    ...
    totalIssuesFound:
      gateReport?.issueCount ?? summary?.totalIssuesFound ?? null,
...
function buildPublicSummaryExportNotes(summary, context = {}) {
  ...
  const gateTotal =
    ...
    context.gateRepositoryFilesTotal ??
    filesScanned;
  ...
  const jestExecuted =
    context.jestBaselineChecked !== false &&
  ...
  if (
    credentialScanned != null &&
    filesScanned != null &&
    credentialScanned < filesScanned
  ) {
    notes.push(
...
function sanitizePublicSummaryArtifactExport(payload, options = {}) {
  if (!payload || payload.type !== "simplebeacon-public-summary")
    return payload;
  ...
  return {
    ...
    note:
      payload.note ||
      "Detailed file paths and remediation steps require a paid deliverable tier.",
