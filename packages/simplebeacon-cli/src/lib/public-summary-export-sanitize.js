...
function flattenPublicSummaryBlock(block) {
    ...
    if (summary && typeof summary === 'object' && summary.summary && typeof summary.summary === 'object') {
        const inner = summary.summary;
        if (inner.filesScanned != null || inner.gatePass != null || inner.status != null || inner.totalIssuesFound != null) {
    ...
    if (summary && typeof summary === 'object' && !Array.isArray(summary)) {
        const looksLikeMetrics = summary.filesScanned != null
            || summary.status != null
...
function looksLikePublicMetrics(value) {
    ...
    return value.filesScanned != null
        || value.status != null
        || value.gatePass != null
...
function resolvePublicSummaryGateContext(payload, options = {}) {
    ...
    const gateProfile = gateReport.scanScope?.profile
        ...
        ?? null;
    const ruleScopedFilesAnalyzed = gateReport.ruleScopedFilesAnalyzed
...
function buildPublicSummaryHygieneSummary(summary, payload, options = {}) {
    ...
    const { repositoryFilesTotal: gateTotal, credentialScanned, contentScanned, gateProfile, gateReport,
        ruleScopedFilesAnalyzed, fictionSampleFilesScanned, fictionJsonFilesScanned, fullDirectoryScan,
        gatePass, blockingCount, jestBaselineChecked } = gateContext;
    return {
        ...
        attestationNote: 'Public-tier gate summary — paths and remediation withheld; not vendor handoff certification.'
    };
}

function enrichPublicSummaryScanScope(scanScope, payload, options = {}) {
    const gateContext = resolvePublicSummaryGateContext(payload, options);
    const { repositoryFilesTotal: gateTotal, gateProfile, fullDirectoryScan, gateReport } = gateContext;
    return {
        ...(scanScope || {}),
        resultsViewScope: 'public-tier-gate-summary',
        reportHealth: gateReport?.scanScope?.reportHealth || scanScope?.reportHealth
            || (fullDirectoryScan ? 'platform-scoped-full-tree' : 'platform-scoped'),
        securityHandoffEligible: false,
        ...(gateTotal != null ? { gateRepositoryFilesTotal: gateTotal } : {}),
        ...(gateProfile ? { gateRuleBundleProfile: gateProfile } : {}),
        ...(fullDirectoryScan ? { fullDirectoryScan: true } : {}),
        publicExportNote: scanScope?.publicExportNote
...
function buildPublicSummaryExportNotes(summary, context = {}) {
    ...
    const gateTotal = context.repositoryFilesTotal ?? context.gateRepositoryFilesTotal ?? filesScanned;
    ...
    const jestExecuted = context.jestBaselineChecked !== false
        && context.jestExecutedDuringScan !== false;

    if (fullTree && filesScanned != null && fictionSamples != null && fictionSamples > 0
    ...
    if (!jestExecuted) {
        notes.push('Public gate summary — Jest was not run during this scan; use gate/complete scan for test attestation.');
    }
    if (summary?.gatePass === true && (summary?.totalIssuesFound ?? 0) === 0) {
        notes.push('Gate pass on configured severities — hygiene attestation only, not SimpleBeacon vendor handoff or Complete scan clearance.');
    }
...
function sanitizePublicSummaryArtifactExport(payload, options = {}) {
    ...
    const reconciledSummary = {
        ...
        ...(gateContext.gatePass != null
            ? { status: gateContext.gatePass ? 'PASS' : 'FAIL' }
            : {}),
        ...(gateContext.blockingCount != null || gateReport?.issueCount != null
            ? { totalIssuesFound: gateReport?.issueCount ?? gateContext.blockingCount ?? summary.totalIssuesFound }
            : {})
    };
    ...
    const enrichedPayload = {
        ...
        scanScope: enrichPublicSummaryScanScope(payload.scanScope, { ...payload, summary: reconciledSummary }, options),
        hygieneSummary: buildPublicSummaryHygieneSummary(reconciledSummary, { ...payload, summary: reconciledSummary }, options)
    };
    ...
    return {
        ...
        note: payload.note || 'Detailed file paths and remediation steps require a paid deliverable tier.',
