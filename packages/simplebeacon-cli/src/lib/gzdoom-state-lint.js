...
const AMBIENT_LABELS = new Set(['spawn', 'see', 'heal', 'crash', 'idle', 'wound']);
...
function is3DVariantClass(className) {
    ...
    return /(?:^|_)(?:Pure3D|PureVoxel|Model|Voxel)(?:_|$|[A-Z])/i.test(name)
 ...
 */
function lintStatesBlock(className, relativePath, statesBody, startLine, severity) {
    const issues = [];
    ...
    for (const [label, frames] of byLabel.entries()) {
        ...
        for (const { sprite, frame, line } of frames) {
            ...
            for (const other of others) {
                ...
                const issueSeverity = COMBAT_LABELS.has(otherNorm) ? severity : 'medium';
                ...
                issues.push({
                    ...
                    recommendedAction:
                        `Use disjoint sprite frames for ${other.label} vs ${label} in class ${className}`,
...
async function lintGzdoomDeathFrameReuse(baseDir, options = {}) {
    ...
    for (const file of zsFiles) {
        ...
        issues.push(...lintZScriptDeathFrameReuse(content, file.relativePath, { severity }));
    }
