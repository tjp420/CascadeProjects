...
function validateAgainstSpec(spec, payload) {
  ...
  if (spec.overviewKeys?.length) {
    if (!payload.overview) {
      violations.push({
        ...
        message: "Missing overview object",
      });
    } else {
      for (const key of spec.overviewKeys) {
        if (!(key in payload.overview)) {
          violations.push({
            kind: "missing-overview-key",
            message: `Missing overview.${key}`,
          });
        }
      }
    }
  ...
  for (const key of spec.arrayKeys || []) {
    if (!Array.isArray(payload[key])) {
      violations.push({
        kind: "invalid-array",
        message: `Expected array: ${key}`,
      });
    } else if (
      payload[key].length === 0 &&
      !(spec.allowEmptyArrays || []).includes(key)
    ) {
...
function strategyForIssueType(type) {
  ...
  if (normalized.includes("json") || normalized.includes("invalid json"))
    return "json-syntax";
