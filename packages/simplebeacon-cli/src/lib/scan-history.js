...
function repairHistoryEntry(entry = {}) {
  ...
  if (repaired.warningCount != null || repaired.blockingCount != null) {
    repaired.issueCount =
      (repaired.blockingCount ?? 0) + (repaired.warningCount ?? 0);
  } else if (
    gatePass &&
    critical === 0 &&
    high === 0 &&
    (repaired.issueCount ?? 0) > 20
  ) {
  ...
  if (
    ...
    critical === 0 &&
    high === 0 &&
    (sev.medium ?? 0) > (repaired.issueCount ?? 0)
  ) {
    ...
    if (!Number.isNaN(reportMs)) {
      const closeMatches = entries.filter((entry) => {
        ...
        return (
          ...
          Math.abs(entryMs - reportMs) <= constants.TIMEOUT_5S
        );
      });
...
function appendScanHistory(platformRoot, report, options = {}) {
  ...
  if (
    ...
    last.qualityScore === entry.qualityScore
  ) {
