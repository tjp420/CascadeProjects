...
function _checkBudget(budget, tokenCount, costEstimateCents) {
    ...
    if (budget.maxTokensPerRequest && tokenCount > budget.maxTokensPerRequest) {
        violations.push(_makeViolation('TOKEN_CAP_EXCEEDED',
    ...
    if (budget.maxRequestsPerMinute && rpm >= budget.maxRequestsPerMinute) {
        violations.push(_makeViolation('RPM_CAP_EXCEEDED',
    ...
    if (costEstimateCents && budget.maxMonthlySpend) {
        ...
        const hourlyCost = _windowedCount(_counters.hour, hourlySpendKey, budget.hourlyWindowMs || DEFAULT_HOURLY_WINDOW_MS);
        const projectedMonthly = hourlyCost * 24 * 30;
        if (projectedMonthly >= budget.maxMonthlySpend * 100) {
            violations.push(_makeViolation('SPEND_PROJECTION_EXCEEDED',
...
function createSentinelMiddleware(options = {}) {
    ...
    return function sentinelMiddleware(req, res, next) {
        ...
        const isLlmRoute = /\/v1\/(chat\/completions|embeddings|completions|messages)/.test(req.path || req.url || '');
        if (!isLlmRoute) {
            return next ? next() : undefined;
        }

        const tokenCount = req.body?.max_tokens || req.body?.max_completion_tokens || req.body?.maxOutputTokens || 0;
    ...
    const proxy = new Proxy(client, {
        get(target, prop) {
            ...
            return function wrapped(...args) {
                const params = args[0] || {};
                const tokenCount = params.max_tokens || params.max_completion_tokens || params.maxOutputTokens || 0;
                const violations = _checkBudget(budget, tokenCount, 0);
