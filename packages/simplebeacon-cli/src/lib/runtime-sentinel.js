...
function _checkBudget(budget, tokenCount, costEstimateCents) {
  ...
  const rpm = _windowedCount(
    _counters.minute,
    rpmKey,
    budget.minuteWindowMs || DEFAULT_WINDOW_MS,
  );
    ...
    if (projectedMonthly >= budget.maxMonthlySpend * 100) {
      violations.push(
        _makeViolation(
          ...
          {
            projectedMonthly: projectedMonthly / 100,
            cap: budget.maxMonthlySpend,
          },
        ),
      );
    }
...
async function createSentinelMiddleware(options = {}) {
  ...
  return function sentinelMiddleware(req, res, next) {
    ...
    const isLlmRoute =
      /\/v1\/(chat\/completions|embeddings|completions|messages)/.test(
        req.path || req.url || "",
      );
    if (!isLlmRoute) {
      ...
      return function wrapped(...args) {
        ...
        if (violations.length) {
          ...
          return Promise.reject(
            new Error(`[RuntimeSentinel] ${violations[0].message}`),
          );
        }
