...
function reconcileLegacyConsolidationCounts(summary = {}) {
  let benchmarkCacheCandidatesExcluded =
    summary.benchmarkCacheCandidatesExcluded ?? 0;
...
function buildBenchmarkConsolidationConclusion(scan) {
  ...
  const parts = [
    ...
    repoFiles != null
      ...
      : null,
    jsonScanned != null
      ? `${Number(jsonScanned).toLocaleString()} JSON hashed in clone`
      : null,
    s.potentialSavingsLabel
      ? `Potential savings if acted on: ${s.potentialSavingsLabel}`
      : null,
...
function consolidationCandidateTouchesExcludedExport(candidate) {
  ...
  return (candidate?.files || []).some((file) =>
    consolidationPathTouchesExcluded(
...
function filterAdvancedAnalysis(analysis, benchmarkScan) {
  ...
  const fuzzyPairs = filterFuzzyPairs(
    analysis.fuzzyNearDuplicates?.pairs || [],
  ).filter(
    (pair) =>
      benchmarkScan ||
  ...
  const patternGroups = (
    analysis.patternConsolidation?.recommendations || []
  ).filter(
    (group) =>
      benchmarkScan ||
...
function normalizeConsolidationPaths(scan) {
  const normFile = (file) => {
    ...
    const path =
      file.path != null ? String(file.path).replace(/\\/g, "/") : file.path;
    const relativePath =
  ...
  return {
    ...
    projectRoot: scan.projectRoot
      ? String(scan.projectRoot).replace(/\\/g, "/")
      : scan.projectRoot,
    platformRoot: scan.platformRoot
      ? String(scan.platformRoot).replace(/\\/g, "/")
      : scan.platformRoot,
    scanTargetRoot: scan.scanTargetRoot
      ? String(scan.scanTargetRoot).replace(/\\/g, "/")
      : scan.scanTargetRoot,
...
function buildProductConsolidationHygieneSummary(
  ...
  const repoTotal =
    summaryBase.repositoryFilesTotal ??
    scan.repositoryInventory?.totalFiles ??
    null;
  const repoAudited =
    summaryBase.repositoryFilesAudited ??
    scan.scanScope?.repositoryFilesAudited ??
    null;
  return {
    ...
    jsonFilesAnalyzed:
      summaryBase.jsonFilesAnalyzed ??
      scan.scanScope?.jsonFilesAnalyzed ??
      null,
    sampleDataFilesAnalyzed:
      summaryBase.sampleDataFilesAnalyzed ??
      scan.scanScope?.sampleDataFilesAnalyzed ??
      null,
    ...
    intentionalPairsExcluded:
      summaryBase.intentionalPairsExcluded ??
...
function enrichProductConsolidationScanScope(scanScope, scan, options = {}) {
  ...
  return {
    ...
    ...(gateProfile ? { gateRuleBundleProfile: gateProfile } : {}),
    mergeWalkFiles:
      summary.repositoryFilesAudited ??
      scanScope?.repositoryFilesAudited ??
      null,
    jsonFilesHashed:
      summary.jsonFilesAnalyzed ?? scanScope?.jsonFilesAnalyzed ?? null,
    sampleDataFilesAnalyzed:
      summary.sampleDataFilesAnalyzed ??
      scanScope?.sampleDataFilesAnalyzed ??
      null,
...
function buildProductConsolidationExportNotes(
  ...
  const repoTotal =
    scan.summary?.repositoryFilesTotal ?? scan.repositoryInventory?.totalFiles;
  const repoAudited =
    scan.summary?.repositoryFilesAudited ?? scope.repositoryFilesAudited;
  ...
  const profile =
    scan.repositoryInventory?.profile ||
  ...
  const jsonN =
    scan.summary?.jsonFilesAnalyzed ?? scan.scanScope?.jsonFilesAnalyzed;
  const sampleN =
    scan.summary?.sampleDataFilesAnalyzed ??
    scan.scanScope?.sampleDataFilesAnalyzed;
...
function buildProductConsolidationAiSummary(scan) {
  const s = scan.summary || {};
  const repoTotal =
    s.repositoryFilesTotal ?? scan.repositoryInventory?.totalFiles;
  const repoAudited =
    s.repositoryFilesAudited ?? scan.scanScope?.repositoryFilesAudited;
  const parts = [
...
function sanitizeScanScope(scanScope, scan, benchmarkScan, projectPath) {
  if (!scanScope) return scanScope;
  const repoFiles =
    scan.summary?.repositoryFilesTotal ?? scanScope.repositoryFilesTotal;
  const productPlatformRoot = resolveProductPlatformRoot(projectPath);

  if (!benchmarkScan) {
    return {
      ...scanScope,
      limitations: (scanScope.limitations || []).map((line) =>
        rewriteProductScopedText(line, false),
      ),
    };
  }

  const samplePaths = scan.scanPaths || scanScope.sampleDataPaths || [];
  const productPathsMissing = samplePaths.some((p) =>
...
function sanitizeConsolidationExport(scan, options = {}) {
  ...
  const benchmarkScan = isBenchmarkConsolidation(working, {
    ...options,
    projectPath,
  });
  ...
  const repoRaw =
    working.summary.repositoryFilesTotal ??
    working.repositoryInventory?.totalFiles ??
    null;
  ...
  mergeCandidates = mergeCandidates.map((c) =>
    benchmarkScan
      ? {
          ...c,
          recommendation: rewriteProductScopedText(c.recommendation, true),
        }
      : c,
  );
  let reductionOpportunities = (working.reductionOpportunities || []).filter(
  ...
  const exactDuplicateGroups = reductionOpportunities.filter(
    (o) => o.type === "duplicate-removal",
  ).length;
  ...
  const summaryBase = {
    ...
    fuzzyNearDuplicatePairs:
      advancedAnalysis?.fuzzyNearDuplicates?.pairsFound ??
  ...
  const next = {
      ...
      ...(benchmarkScan
        ...
        : staleProductInventory
          ? {
              ...
              repositoryFilesTotal:
                working.scanScope?.platformRepositoryFilesTotal ?? null,
    ...
    aiSummary: benchmarkScan
      ...
      : mergeCandidates.length === 0 &&
          (summaryBase.potentialSavingsBytes ?? 0) === 0
        ? buildProductConsolidationAiSummary({
            ...working,
            summary: summaryBase,
          })
        : rewriteProductScopedText(working.aiSummary, false),
    aiSummaryProvider: working.aiSummaryProvider
      ? String(working.aiSummaryProvider).replace(
          /\bSimplebeacon\b/g,
          "SimpleBeacon",
        )
    ...
    ...(benchmarkScan
      ? {
          ...
          hygieneSummary: {
            ...
            intentionalPairsExcluded,
            repositoryFilesTotal:
              repoRaw ?? summaryBase.repositoryFilesTotal ?? null,
    ...
    exportNotes: benchmarkScan
      ...
      : filterExportNotes(
          [
            ...(working.exportSanitized || working.exportNormalized
              ? []
              : working.exportNotes || []),
    ...
    if (next.scanScope) {
      next.scanScope = {
        ...
        sampleDataPathsOmitted:
          working.scanPaths || working.scanScope?.sampleDataPaths || [],
      };
    }
  }
  ...
  if (!benchmarkScan) {
    ...
    if (staleProductInventory) {
      next.scanScope.inventoryProfile =
        next.scanScope?.inventoryProfile || "explorer";
    }
  }
