...
function normalizeConsolidationPaths(scan) {
  const normFile = (file) => {
    ...
    const relativePath =
      file.relativePath != null ? String(file.relativePath).replace(/\\/g, '/') : file.relativePath;
    return {
      ...file,
      ...(path != null ? { path } : {}),
    ...
    ...(scan.repositoryInventory
      ? {
          repositoryInventory: {
            ...scan.repositoryInventory,
            projectRoot:
              String(scan.repositoryInventory.projectRoot || scan.projectRoot || '').replace(
                /\\/g,
                '/'
              ) || scan.projectRoot,
          },
...
function sanitizeConsolidationExport(scan, options = {}) {
  ...
  const repoRaw =
    working.summary.repositoryFilesTotal ?? working.repositoryInventory?.totalFiles ?? null;
  ...
  mergeCandidates = mergeCandidates.map((c) =>
    benchmarkScan ? { ...c, recommendation: rewriteProductScopedText(c.recommendation, true) } : c
  );
  ...
  recommendations = recommendations.map((r) => sanitizeRecommendation(r, benchmarkScan));
  ...
  const summaryBase = {
    ...
    fuzzyNearDuplicatePairs:
      advancedAnalysis?.fuzzyNearDuplicates?.pairsFound ?? working.summary.fuzzyNearDuplicatePairs,
    benchmarkCacheCandidatesExcluded,
    fuzzyPairsExcluded,
    ...(intentionalPairsExcluded > 0
      ? {
  ...
  const next = {
    ...
    exportNotes: benchmarkScan
      ? assembleBenchmarkConsolidationExportNotes(working.exportNotes, summaryBase, pairExclusions)
      : filterExportNotes(
          [
