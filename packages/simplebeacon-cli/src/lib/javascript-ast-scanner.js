...
} = require("../rules/ai-runtime-scan-common");
...
const SLOP_PATTERN =
  /(?:mock_data|sample\.json|test-api|placeholder|todo_truncate_this|dummy-key|(?:\/|\\)mock(?:\/|\\)|-sample\.json)/i;
const HIGH_RISK_PATTERN =
  /(?:biometric|facial_recognition|credit_score|loan_approval|resume_screen|hiring_filter|polygraph|recidivism)/i;
...
const JAVASCRIPT_AST_RULE_CATALOG = [
  ...
  },
  {
    ...
    severity: "medium",
    description:
      "Function returns only null/undefined — likely AI stub (JavaScript AST)",
  },
  {
    id: "SB-JS-TB-001",
    ...
    severity: "medium",
    description:
      "LLM call without max_tokens / max_completion_tokens (JavaScript AST)",
  },
  {
    id: "SB-JS-EU-001",
    category: "eu-ai-act",
    ...
    severity: "high",
    description:
      "Identifier or string matches Annex III high-risk term (JavaScript AST)",
  },
  {
    id: "SB-JS-SQL-001",
    ...
    severity: "critical",
    description:
      "SQL string built via template literal or concat inside query executor (JavaScript AST)",
  },
  {
    id: "SB-JS-SQL-002",
    category: "security-patterns",
    ...
    severity: "critical",
    description:
      "Non-literal SQL argument passed to query executor without placeholder array (JavaScript AST)",
  },
];

const RECOMMENDATIONS = {
  "SB-JS-FICTION-001":
    "Replace mock/sample strings with runtime config or test-scoped fixtures.",
  "SB-JS-FICTION-002":
    "Implement the function or remove the AI-generated stub before merge.",
  "SB-JS-TB-001":
    "Pass max_tokens, max_completion_tokens, or maxOutputTokens on LLM client calls.",
  "SB-JS-EU-001":
    "Document Annex III classification, transparency, and human oversight for this flow.",
  "SB-JS-SQL-001":
    "Use parameterized queries with placeholder arrays. Never concatenate variables into SQL strings.",
  "SB-JS-SQL-002":
    "Pass query parameters as a separate array to the driver. Do not interpolate user input into SQL.",
};
...
function isLlmCall(node) {
  ...
  if (
    /openai\.chat\.completions\.create|anthropic\.messages\.create|\.chat\.completions\.create|\.responses\.create/i.test(
      label,
    )
  )
    return true;
  if (
    !/(?:openai|anthropic|claude|gpt|llm|chat\.completion|completion|embedding|streamtext|generatetext|ai\.)/i.test(
      label,
    )
  )
    return false;
...
const SQL_KEYWORDS =
  /\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|EXEC|EXECUTE|UNION|WHERE|FROM|JOIN|TABLE)\b/i;
const QUERY_EXECUTOR_NAMES = new Set([
  "query",
  "execute",
  "run",
  "all",
...
function isQueryExecutor(node) {
  if (node.type === "Identifier" && QUERY_EXECUTOR_NAMES.has(node.name))
    return true;
  if (node.type === "MemberExpression") {
    const prop = node.property.type === "Identifier" ? node.property.name : "";
...
function hasSqlKeywords(node) {
  ...
  if (node.type === "StringLiteral" && SQL_KEYWORDS.test(node.value))
    return true;
  ...
  getTraverse()(ast, {
    StringLiteral(pathNode) {
      ...
      if (SLOP_PATTERN.test(val)) {
        push(
          JAVASCRIPT_AST_RULE_CATALOG[0],
          pathNode.node.loc?.start?.line || 1,
          `Hardcoded placeholder string detected: '${val.slice(0, 80)}'`,
        );
      }
      if (HIGH_RISK_PATTERN.test(val)) {
        push(
          JAVASCRIPT_AST_RULE_CATALOG[3],
          pathNode.node.loc?.start?.line || 1,
...
function scanTextPatterns(relativePath, content, ext, options = {}) {
  ...
  if (
    options.productionPathsOnly !== false &&
    !isUnderProductionPaths(
      relativePath,
      options.productionPaths || ["server/", "src/", "app/", "lib/"],
    )
  ) {
    return [];
  }
...
function scanJavascriptAstSnippet(
  projectRoot,
  relativePath,
  content,
  options = {},
