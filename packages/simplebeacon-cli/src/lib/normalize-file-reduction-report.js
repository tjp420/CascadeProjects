...
const {
  DEFAULT_SKIP_GLOBS,
} = require("../analyzers/file-reduction/unused-file-detector");
...
function isSkippedUnusedFinding(finding) {
  ...
  const normalized = String(finding.path || "")
    .split("\\")
    .join("/");
...
function filterBenchmarkFindings(findings = []) {
  return findings.filter(
    (finding) =>
      !isBenchmarkFinding(finding) && !isSkippedUnusedFinding(finding),
  );
}
...
function isStaleFileReductionScan(report) {
  ...
  return allFindings.some((finding) =>
    isExternalBenchmarkCachePath(finding.path),
  );
}
...
function recomputeSummary(filteredFindings, inventoryFiles) {
  ...
  const reclaimableBytes = allFindings.reduce((sum, finding) => {
    ...
    if (finding.type === "build-artifact")
      return sum + (finding.sizeBytes || 0);
    return sum;
  }, 0);
...
function normalizeFileReductionReport(report) {
  ...
  const projectIsBenchmark = isExternalBenchmarkCachePath(
    report.projectRoot || "",
  );
  const staleFullTreeScan =
    isStaleFileReductionScan(report) && !projectIsBenchmark;
  ...
  const filteredFindings = Object.fromEntries(
    Object.entries(findings).map(([key, bucket]) => [
      key,
      filterBenchmarkFindings(bucket),
    ]),
  );
  ...
  const { allFindings, summary } = recomputeSummary(
    filteredFindings,
    inventoryFiles || 1,
  );
  ...
  const staleLimitation =
    "This report used a full-repo walk (69k+ files) — re-run file reduction after updating Simplebeacon to exclude github-cache/.";
  const benchmarkNote =
    "github-cache/ OSS benchmark clones are excluded from platform file-reduction scoring (not your product code).";
  ...
  const scanScope = {
    ...
    inventoryMetricsStale:
      staleFullTreeScan || Boolean(report.scanScope?.inventoryMetricsStale),
  };
  ...
  if (projectIsBenchmark || staleFullTreeScan || benchmarkExcluded > 0) {
    scanScope.limitations = [
      ...new Set([
        ...
        ...(benchmarkExcluded > 0 && !staleFullTreeScan && !projectIsBenchmark
          ? [benchmarkNote]
          : []),
      ]),
    ];
  ...
  const normalized = {
    ...
    metadata: {
      ...(report.metadata || {}),
      entryPoints: (report.metadata?.entryPoints || []).filter(
        (entry) => !isExternalBenchmarkCachePath(entry),
      ),
    },
    scanScope,
  };
  ...
  if (normalized.scanners?.["unused-files"]) {
    normalized.scanners["unused-files"] = {
      ...
      entryPoints:
        normalized.metadata.entryPoints?.length ??
        normalized.scanners["unused-files"].entryPoints,
    };
  }
