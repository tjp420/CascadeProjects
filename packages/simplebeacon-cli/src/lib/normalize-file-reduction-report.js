...
function filterBenchmarkFindings(findings = []) {
  return findings.filter(
    (finding) => !isBenchmarkFinding(finding) && !isSkippedUnusedFinding(finding)
  );
}
...
function normalizeFileReductionReport(report) {
  ...
  const inventoryFiles = staleFullTreeScan ? null : (report.inventory?.totalFiles ?? null);
  ...
  const priorLimitations = (report.scanScope?.limitations || []).filter(
    (line) =>
      line && !/github-cache/.test(line) && line !== staleLimitation && line !== benchmarkNote
  );
  ...
  const scanScope = {
    ...
    rescanRecommended:
      staleFullTreeScan || benchmarkExcluded > 0 || Boolean(report.scanScope?.rescanRecommended),
  ...
  if (normalized.scanners?.['unused-files']) {
    normalized.scanners['unused-files'] = {
      ...
      entryPoints:
        normalized.metadata.entryPoints?.length ?? normalized.scanners['unused-files'].entryPoints,
    };
  }
