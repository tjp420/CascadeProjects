...
function resolveBaselineStatus(report = {}, baseline = {}) {
  ...
  return {
    compliance: `${current}%`,
    variance:
      variance === 0 ? "0% from baseline" : `-${variance}% from baseline`,
    status,
  ...
  const pageSamplesLabel =
    report.pageSampleSchemaChecked != null
      ...
      : (baseline.pageSamplesLabel ??
        baselineComparison.overview?.pageSamplesLabel ??
        null);
  ...
  return {
    ...
    qualityMetrics: {
      ...
      overview: {
        ...
        schemaCompliance:
          report.schemaCompliance ??
          qualityMetrics.overview?.schemaCompliance ??
          null,
        consistencyScore:
          report.consistencyScore ??
          qualityMetrics.overview?.consistencyScore ??
          null,
      },
        ...
        if (metric.name === "quality_score") {
          return {
            ...
            context: {
              ...(metric.context || {}),
              issueCount:
                (report.gate?.blockingCount ?? 0) +
                (report.gate?.warningCount ?? 0),
        ...
        if (metric.name === "consistency_score") {
          return {
            ...
            context: {
              ...(metric.context || {}),
              samplesScanned:
                report.consistencyChecked ?? metric.context?.samplesScanned,
    ...
    baselineComparison: {
      ...
      currentValue:
        report.consistencyScore ??
        report.schemaCompliance ??
        baselineComparison.currentValue,
      overview: {
        ...(baselineComparison.overview || {}),
        compliance: `${report.consistencyScore ?? report.schemaCompliance ?? 100}%`,
        jestTestsLabel:
          baseline.jestTestsLabel ??
          baselineComparison.overview?.jestTestsLabel ??
          null,
        pageSamplesLabel,
        gatePass:
          report.gate?.pass ?? baselineComparison.overview?.gatePass ?? false,
      },
      comparisons: (baselineComparison.comparisons || []).map((row) => {
          ...
          return {
            ...
            variance: Math.max(
              ...
              (row.baselineValue ?? 100) -
                (report.schemaCompliance ?? row.currentValue ?? 0),
            ),
          };
        }
        if (row.metric === "consistency_score") {
          return {
            ...
            variance: Math.max(
              0,
              (row.baselineValue ?? 100) -
                (report.consistencyScore ?? row.currentValue ?? 0),
            ),
          };
        }
        if (row.metric === "page_samples") {
          ...
          return {
            ...
            variance: Math.max(
              0,
              (row.baselineValue ?? current) - (current ?? 0),
            ),
          };
        }
        return row;
...
function buildDashboardPayload({ report, baseline, history, fictionCatalog }) {
  ...
  return {
    ...
    scanStatus: {
      ...
      lastScanRelative: formatRelativeTime(
        report.generatedAt || historyEntry.date,
      ),
      ...
      consistencyScore:
        report.consistencyScore ?? report.schemaCompliance ?? null,
      ...
      blockingCount:
        report.gate?.blockingCount ?? historyEntry.blockingCount ?? 0,
      totalFilesScanned:
        resolveFilesAnalyzed(report) || historyEntry.totalFilesScanned || 0,
    },
    trends: {
      fictionalPatternsTrend: buildTrendSeries(
        entries,
        "fictionPatternsFound",
        5,
      ),
...
function buildScanResults(report, historyEntry = {}, baseline = {}) {
  const fictionFound =
    historyEntry.fictionPatternsFound ?? countFictionIssues(report);
  ...
  return {
    ...
    results: {
      ...
      baselineCompliance:
        complianceScore >= 95 &&
        (report.gate?.pass ?? historyEntry.gatePass ?? false),
      totalFilesScanned:
        historyEntry.totalFilesScanned ?? resolveFilesAnalyzed(report),
      issueCount:
        historyEntry.issueCount ??
        (report.gate?.blockingCount ?? 0) + (report.gate?.warningCount ?? 0),
      warningCount: historyEntry.warningCount ?? report.gate?.warningCount ?? 0,
      blockingCount:
        historyEntry.blockingCount ?? report.gate?.blockingCount ?? 0,
      gatePass: historyEntry.gatePass ?? report.gate?.pass ?? false,
      severityCounts:
        historyEntry.severityCounts || report.severityCounts || {},
...
function resolveJestBaselineStatus(report = {}, baseline = {}) {
  ...
  const status =
    summary.testsFailed > 0 || !matches || !suitesMatch ? "fail" : "pass";
...
function buildAuditLayers(report = {}, baseline = {}) {
  ...
  return {
    ...
    schema: {
      ...
      severity: "high",
      status:
        (report.schemaPassed ?? 0) >= (report.schemaChecked ?? 0)
          ? "pass"
          : "fail",
    },
    productionLeaks: {
      ...
      findings:
        report.productionLeakFindings ?? countByType(/production leak/i),
      severity: "medium",
    ...
    jestBaseline: {
      enabled: Boolean(
        baseline.jestTestsLabel || baseline.jestTestsPassing != null,
      ),
...
function buildAuditPayload(context, extras = {}) {
  ...
  return {
    ...
    pageSamples: overlayAuditPageSamples(
      extras.pageSamples || {},
      report,
      baseline,
    ),
