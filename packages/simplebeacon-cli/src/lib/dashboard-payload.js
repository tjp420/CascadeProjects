...
function overlayAuditPageSamples(samples = {}, report = {}, baseline = {}) {
  ...
  const qualityScore =
    report.gate?.pass && report.schemaCompliance != null
      ? report.schemaCompliance
      : (report.qualityScore ??
        qualityMetrics.currentScore ??
        qualityMetrics.overview?.qualityScore ??
        0);
  ...
  return {
    ...
    qualityMetrics: {
      ...
      overview: {
        ...
        qualityScore,
        schemaCompliance:
          report.schemaCompliance ?? qualityMetrics.overview?.schemaCompliance ?? null,
        consistencyScore:
          report.consistencyScore ?? qualityMetrics.overview?.consistencyScore ?? null,
      },
      metrics: (qualityMetrics.metrics || []).map((metric) => {
        ...
        if (metric.name === 'consistency_score') {
          return {
            ...
            context: {
              ...
              fictionHits: Math.max(
                0,
                (report.consistencyChecked ?? 0) - (report.consistencyPassed ?? 0)
              ),
            },
          };
        }
        if (metric.name === 'credential_findings') {
          return {
            ...
            context: {
              ...(metric.context || {}),
              scanned: report.credentialScanned ?? metric.context?.scanned,
            },
          };
        }
        if (metric.name === 'production_leak_findings') {
          return {
            ...
            context: {
              ...(metric.context || {}),
              scanned: report.productionLeakScanned ?? metric.context?.scanned,
            },
          };
        }
        return metric;
    ...
    baselineComparison: {
      ...
      currentValue:
        report.consistencyScore ?? report.schemaCompliance ?? baselineComparison.currentValue,
      overview: {
        ...(baselineComparison.overview || {}),
        compliance: `${report.consistencyScore ?? report.schemaCompliance ?? 100}%`,
        jestTestsLabel:
          baseline.jestTestsLabel ?? baselineComparison.overview?.jestTestsLabel ?? null,
        pageSamplesLabel,
      ...
      comparisons: (baselineComparison.comparisons || []).map((row) => {
        if (row.metric === 'schema_compliance') {
          return {
            ...
            variance: Math.max(
              0,
              (row.baselineValue ?? 100) - (report.schemaCompliance ?? row.currentValue ?? 0)
            ),
          };
        }
        if (row.metric === 'consistency_score') {
          return {
            ...
            variance: Math.max(
              0,
              (row.baselineValue ?? 100) - (report.consistencyScore ?? row.currentValue ?? 0)
            ),
          };
        }
...
function buildDashboardPayload({ report, baseline, history, fictionCatalog }) {
  ...
  const adoptionTrend = entries
    .slice(-5)
    .map((entry) => entry.totalFilesScanned ?? resolveFilesAnalyzed(report));
...
function buildScanResults(report, historyEntry = {}, baseline = {}) {
  ...
  return {
    ...
    results: {
      ...
      baselineCompliance:
        complianceScore >= 95 && (report.gate?.pass ?? historyEntry.gatePass ?? false),
      totalFilesScanned: historyEntry.totalFilesScanned ?? resolveFilesAnalyzed(report),
      issueCount:
        historyEntry.issueCount ??
        (report.gate?.blockingCount ?? 0) + (report.gate?.warningCount ?? 0),
...
function resolveJestBaselineStatus(report = {}, baseline = {}) {
  ...
  const suitesMatch =
    baseline.jestSuites == null ||
    summary.suitesPassed == null ||
    summary.suitesPassed === baseline.jestSuites;
  const status = summary.testsFailed > 0 || !matches || !suitesMatch ? 'fail' : 'pass';
...
function buildAuditLayers(report = {}, baseline = {}) {
  ...
  const countByType = (pattern) =>
    raw
      .filter((issue) => pattern.test(String(issue.type)))
  ...
  return {
    ...
    roadmap: {
      ...
      severity: 'medium',
      status:
        (report.roadmapSchemaPassed ?? 0) >= (report.roadmapSchemaChecked ?? 0) &&
