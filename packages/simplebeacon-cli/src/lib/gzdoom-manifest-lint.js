...
const TRACKING_NOTE_RE = /\b(?:TODO|FIXME|TRACK|ISSUE|DISABLED|HACK|TEMP|WIP|BUG-\d+|#\d+)\b/i;
...
function findMissingIncludes(baseDir, allFiles) {
    ...
    for (const file of sourceFiles) {
        if (!/\.(zs|zscript)$/i.test(file.relativePath) && file.name.toLowerCase() !== 'zscript') continue;
        let content;
...
function lineOfInclude(content, includeRef) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(`"${includeRef}"`) || lines[i].includes(`'${includeRef}'`)) return i + 1;
    }
    return 1;
}
...
function findDisabledIncludesWithoutNotes(baseDir, ignoreGlobs = []) {
    const hits = [];
    const scanRoots = [
        path.join(baseDir, 'zscript'),
        baseDir
    ];
    function walkFile(full, rel) {
        if (!/\.(zs|zscript)$/i.test(path.basename(full)) && path.basename(full).toLowerCase() !== 'zscript') return;
...
async function lintGzdoomManifest(modPath, options = {}) {
    ...
    for (const m of missing) {
        issues.push({
            ...
            recommendedAction: 'Fix the include path, add the missing .zs file, or remove the include',
    ...
    for (const d of disabled) {
        issues.push({
            ...
            recommendedAction: 'Add a TODO comment explaining why the include is disabled and when to re-enable',
