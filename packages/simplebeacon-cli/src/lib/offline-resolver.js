...
function detectProxyEnvironment() {
  ...
  for (const varName of proxyVars) {
    ...
    if (value) {
      ...
      if (
        varName.toLowerCase().includes("proxy") &&
        !varName.toLowerCase().startsWith("no_")
      ) {
        hasProxy = true;
      }
    }
...
async function prewarmCache(packageNames = [], options = {}) {
  ...
  const batchSize = Math.ceil(
    packages.length / Math.ceil(packages.length / concurrency),
  );
  for (let i = 0; i < packages.length; i += batchSize) {
    const batch = packages.slice(i, i + batchSize);
    const promises = batch.map(async (name) => {
      const exists = await resolvePackageExists(name, {
        ...options,
        useCache: false,
      });
      if (exists === true) {
