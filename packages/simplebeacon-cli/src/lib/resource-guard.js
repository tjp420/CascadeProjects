...
function preflightOrThrow() {
  const minFreeBytes = parseEnvNumber(
    "SIMPLEBEACON_MIN_FREE_BYTES",
    DEFAULT_MIN_FREE_BYTES,
  );
  const minFreePercent = parseEnvNumber(
    "SIMPLEBEACON_MIN_FREE_PERCENT",
    DEFAULT_MIN_FREE_PERCENT,
  );
  const { total, free, freePercent } = getSystemMemory();
  if (free < minFreeBytes || freePercent < minFreePercent) {
    const msg =
      `ResourceGuard preflight failed: free=${Math.round(free / 1024 / 1024)}MB (${freePercent.toFixed(1)}%) < required ${Math.round(minFreeBytes / 1024 / 1024)}MB or ${minFreePercent}%\n` +
      buildAdvice();
...
function sampleAndThrow(context = {}) {
  ...
  if (free < minFreeBytes || freePercent < minFreePercent) {
    const details = [];
    if (context && context.filesFound != null)
      details.push(`filesFound=${context.filesFound}`);
    if (context && context.phase) details.push(`phase=${context.phase}`);
    const msg =
      `ResourceGuard triggered during scan ${details.length ? "(" + details.join(",") + ")" : ""}: free=${Math.round(free / 1024 / 1024)}MB (${freePercent.toFixed(1)}%) < required ${Math.round(minFreeBytes / 1024 / 1024)}MB or ${minFreePercent}%\n` +
      buildAdvice();
    const err = new Error(msg);
