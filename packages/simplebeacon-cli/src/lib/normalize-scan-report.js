...
const INFORMATIONAL_ISSUE_TYPES = new Set(['Legacy Fiction Roadmap', 'Oversized Roadmap File']);
...
function reconcileScanReport(report) {
  ...
  if (hasBlockers) {
    ...
    report.gate = {
      ...
      pass: false,
      blockingCount:
        blockingCount || blockingIssues.reduce((sum, issue) => sum + (issue.count || 1), 0),
      blockingIssues,
    };
...
function normalizePlatformScanReport(report, options = {}) {
  ...
  let sourceIssues = report.rawIssues && report.rawIssues.length ? report.rawIssues : [];
