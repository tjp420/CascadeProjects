...
const {
  scanTextPatterns: scanAgencyHandoffText,
...
const {
  scanTextPatterns: scanTokenBleedText,
} = require("../rules/token-bleed-patterns");
const {
...
function runTextRulePasses(relativePath, content, ext, options = {}) {
  ...
  if (options.fictionPatterns?.length) {
    const fiction = scanFictionFileContent(
      relativePath,
      content,
      options.fictionPatterns,
      ext,
    );
  ...
  if (options.euAiAct !== false) {
    const eu = scanEuAiActFileContent(
      relativePath,
      content,
      options.euAiActSeverity || "medium",
    );
  ...
  const prodPaths = options.productionPaths || [
    "server/",
    "src/",
    "app/",
    "lib/",
