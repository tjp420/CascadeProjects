...
const { scanTextPatterns: scanLlmSlopText, scanSuspiciousDependencies } = require('../rules/llm-slop-patterns');
...
function runTextRulePasses(relativePath, content, ext, options = {}) {
    ...
    if (options.productionLeak !== false) {
        ...
        if (!prodOnly || isUnderProductionPaths(relativePath, prodPaths)) {
            const leak = scanProductionLeakContent(relativePath, content, options.productionLeakOptions || {});
