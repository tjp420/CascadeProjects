...
function hasHollowGateAttestation(gateReport) {
  ...
  return (
    Boolean(gateReport.gate?.pass) &&
    ruleScopedFilesFromSimplebeacon(gateReport) === 0
  );
}
...
function checklistHasStalePassRows(checklist, gateReport) {
  ...
  return checklist.rules.some((rule) => {
    ...
    if (rule.id === "SUPPLY-001" || rule.id === "SUPPLY-002") {
      return /Scanned 0 path|0 critical, 0 high \(scan\)/i.test(
...
function refreshComplianceChecklist(
  checklist,
  gateReport,
  projectPath,
  npmAudit,
    ...
    if (rule.id === "SUPPLY-001") {
      if (npmAudit.skipped) {
        return {
          ...
          status: "skip",
          evidence:
            npmAudit.scopeNote || "npm audit skipped for this scan path",
        };
      }
...
function buildComplianceHygieneSummary(
  ...
  const fullDirectoryScan =
    ...
    (gateReport?.ruleScopedFilesAnalyzed > 0 &&
    gateReport?.repositoryFilesTotal > 0 &&
    gateReport.ruleScopedFilesAnalyzed === gateReport.repositoryFilesTotal
      ? true
      : null);
  return {
    ...
    ...(checklist?.hygieneSummary?.gatePass != null
      ? { gatePass: checklist.hygieneSummary.gatePass }
      : {}),
    ...(gateReport?.gate?.pass != null
      ? { gatePass: gateReport.gate.pass }
      : {}),
...
function buildComplianceExportNotes(checklist, gateReport, npmAudit, context) {
  ...
  if (context.benchmarkScan) {
    notes.push(
      "Benchmark clone — compliance checklist is not valid for Simplebeacon product handoff.",
    );
  }
  if (context.hollowGate) {
    notes.push(
      "Gate passed with zero rule-scoped files — credential and production-leak rules did not run on product paths.",
    );
  }
  if (gateReport && gateReport.jestBaselineChecked === false) {
    notes.push(
      "Jest was not executed during the gate scan — run npm test or simplebeacon:full before vendor handoff sign-off.",
    );
  }
  if (npmAudit?.supplyChainStatus === "pass") {
    ...
    notes.push(
      npmAudit.scopeNote || "npm audit was skipped for this scan path.",
    );
  }
  if (
  ...
  if (gateReport?.gate?.pass === true) {
    notes.push(
      "Paired gate scan PASS — checklist aligned with simplebeacon-gate.json attestation.",
    );
  }
  if (gateReport?.gate?.pass === true) {
    notes.push(
...
function sanitizeGateReportForComplianceExport(gateReport, projectPath) {
  ...
  const enriched = sanitizeSimplebeaconReportExport(gateReport, {
    projectPath: projectPath || gateReport.projectRoot,
  });
  const benchmarkScan = isBenchmarkCacheProjectPath(
    projectPath || enriched.projectRoot || "",
  );
  ...
  const priorLimitations = (enriched.scanScope?.limitations || []).filter(
    (line) =>
      line &&
      !/exclude.*github-cache/i.test(String(line)) &&
  ...
  return {
    ...
    scanScope: {
      ...
      reportHealth: benchmarkScan
        ...
        : enriched.scanScope?.reportHealth,
      resultsViewScope: benchmarkScan
        ? "benchmark-clone"
        : enriched.scanScope?.resultsViewScope,
...
function sanitizeComplianceForExport(compliance, context = {}) {
  ...
  const hollowGate =
    context.hollowGate ?? hasHollowGateAttestation(context.gateReport);
  const stale =
    checklistHasStalePassRows(compliance, context.gateReport) ||
        ...
        rules: next.rules.map((rule) => {
          ...
          if (["GATE-001", "CRED-001", "LEAK-001"].includes(rule.id)) {
            return {
              ...
              status: "skip",
              evidence:
                "Benchmark clone — gate rule scope is 0 files; not evaluated on product paths.",
            };
          }
          return rule;
        }),
...
function sanitizeComplianceChecklistArtifactExport(checklist, options = {}) {
  ...
  const exportNotes = buildComplianceExportNotes(
    sanitized,
    gateReport,
    npmAuditSanitized,
    context,
...
function sanitizeComplianceBundleExport(payload = {}) {
  ...
  const productPlatformRoot = benchmarkScan
    ? resolveProductPlatformRoot(projectPath)
    : null;
  const gateReport = sanitizeGateReportForComplianceExport(
    payload.gateReport,
    projectPath,
  );
  const hollowGate = hasHollowGateAttestation(gateReport);
  ...
  return {
    ...
    exportNotes: buildComplianceExportNotes(
      checklist,
      gateReport,
      npmAuditSanitized,
      context,
