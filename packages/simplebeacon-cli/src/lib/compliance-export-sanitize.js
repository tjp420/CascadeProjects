...
function sanitizeGateReportForComplianceExport(gateReport, projectPath) {
  ...
  const priorLimitations = (enriched.scanScope?.limitations || []).filter(
    (line) =>
      line && !/exclude.*github-cache/i.test(String(line)) && !/69k\+ files/i.test(String(line))
  );
  ...
  return {
    ...enriched,
    ...(hollowGate
      ? { gateAttestation: benchmarkScan ? 'limited-benchmark' : 'limited-scope' }
      : {}),
    scanScope: {
      ...(enriched.scanScope || {}),
...
function sanitizeComplianceForExport(compliance, context = {}) {
  ...
  const hollowGate = context.hollowGate ?? hasHollowGateAttestation(context.gateReport);
  const stale =
