...
function generateLinkedInPost(report, _options = {}) {
  ...
  if (topFindings.length > 0) {
    lines.push(
      `Top priority finding: ${topFindings[0].type} — ${topFindings[0].description}`,
    );
    lines.push("");
  }
...
function generateNewsletter(report, _options = {}) {
  ...
  if (findings.credentialFindings > 0) {
    lines.push(
      `**Credential findings:** ${findings.credentialFindings} — review recommended`,
    );
  }
...
function generatePressKit(report, _options = {}) {
  ...
  lines.push(
    `2. **AI governance** — detect EU AI Act gaps in AI-integrated products`,
  );
  lines.push(
    `3. **Mock data hygiene** — prevent sample JSON from leaking to production`,
  );
  lines.push(
    `4. **Credential scanning** — catch API keys and tokens in code review`,
  );
  lines.push("");
  lines.push(`## Quotes`);
  lines.push("");
  ...
  lines.push(
    `> — Engineering Lead, ${report.projectName || "AI Platform Company"}`,
  );
  lines.push("");
  lines.push(`## Assets`);
  lines.push("");
