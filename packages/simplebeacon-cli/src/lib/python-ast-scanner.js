...
const PYTHON_AST_RULE_CATALOG = [
  {
    ...
    severity: "medium",
    description:
      "Hardcoded placeholder or mock-path string in Python source (AST)",
  },
  {
    ...
    severity: "medium",
    description:
      "Python function returns None immediately — likely AI stub (AST)",
  },
  {
    id: "SB-PY-TB-001",
    ...
    severity: "medium",
    description:
      "Python LLM call without max_tokens / max_completion_tokens (AST)",
  },
  {
    id: "SB-PY-EU-001",
    category: "eu-ai-act",
    ...
    severity: "high",
    description:
      "Python identifier or string matches Annex III high-risk term (AST)",
  },
];
...
const RECOMMENDATIONS = {
  ...
  "SB-PY-TB-001":
    "Pass max_tokens or max_completion_tokens on LLM client calls.",
  "SB-PY-EU-001":
...
function normalizePythonFinding(raw, projectRoot, severityDefault = "medium") {
  ...
  return {
    ...
    recommendation:
      RECOMMENDATIONS[pattern] || "Review Python AST finding before merge.",
    recommendedAction:
      RECOMMENDATIONS[pattern] || "Review Python AST finding before merge.",
...
function runPythonAstScan(projectRoot, options = {}) {
  ...
  const spawnArgs = pythonBin === "py" ? ["-3", ...args] : args;
  ...
  if (result.error || result.status !== 0) {
    ...
    return {
      ...
      error:
        ...
        `Python AST scan exited ${result.status}`,
    };
  }
  ...
  const issues = (payload.findings || []).map((raw) =>
    normalizePythonFinding(raw, projectRoot, severityDefault),
  );
