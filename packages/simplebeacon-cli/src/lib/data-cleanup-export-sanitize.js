...
} = require("../analyzers/file-reduction/build-artifact-scanner");
...
function stripRuntimeLogFindings(report) {
  ...
  for (const [key, list] of Object.entries(report.findings)) {
    nextFindings[key] = Array.isArray(list)
      ? list.filter((finding) => !isRuntimeLogFinding(finding))
      : list;
  }
  ...
  if (
    nextAllFindings === report.allFindings &&
    nextFindings.buildArtifacts === report.findings.buildArtifacts
  ) {
    return report;
  }
  ...
  if (next.summary) {
    next.summary = {
      ...
      buildArtifactFindings:
        nextFindings.buildArtifacts?.length ??
        next.summary.buildArtifactFindings,
    };
  }
  if (next.scanners?.["build-artifacts"]) {
    ...
    const reviewArtifacts = artifacts.filter(
      (f) => f.action === "review-before-delete",
    );
    const reviewBytes = reviewArtifacts.reduce(
    ...
    next.scanners = {
      ...
      "build-artifacts": {
        ...
        reclaimableBytes: artifacts.reduce(
          (sum, f) => sum + (f.sizeBytes || 0),
          0,
        ),
      },
...
function replaceMisleadingDataQualityNotes(notes = [], benchmarkScan) {
  const filtered = notes.filter((note) => {
    ...
    return (
      !/exclude(s)?\s+github-cache/i.test(String(note)) &&
...
function sanitizeExecutiveSummaryForExport(executiveSummary, context) {
  ...
  const next = {
    ...executiveSummary,
    notes: replaceMisleadingDataQualityNotes(
      ...
      next.priorityActions = [
        {
          ...
          detail:
            "This OSS clone has no root package.json — dependency and env scanners did not run. Compare against ai-platform for product workspace health.",
        },
      ];
    } else if (
      (ws.missingEnvKeys || 0) > 0 ||
      (next.data?.syncIoPatterns || 0) > 0
    ) {
...
function sanitizeScannerStatisticsForExport(stats, context) {
  ...
  const scopeNote =
    context.profile === "file-reduction"
...
function sanitizeBenchmarkExecutivePriorityActions(executiveSummary = {}) {
  ...
  const actions = (executiveSummary.priorityActions || [])
    .map((action) => {
      ...
      if (
        /missing environment keys|optional CLI environment keys/i.test(title)
      ) {
...
function rebuildBenchmarkDataQualityCounts(report, filteredFindings) {
  ...
  const envFindings = filteredFindings.filter(
    (f) => f.type === "missing-env-key",
  );
  const accessFindings = filteredFindings.filter(
    (f) => f.type === "data-access-pattern",
  );
  ...
  return {
    ...
    aggregation: {
      ...
      topFiles: aggregated.topFiles.filter(
        (entry) => entry.filePath !== "unknown",
      ),
    },
    executiveSummary,
...
function sanitizeBenchmarkDataQualityExport(report) {
  ...
  const next = {
    ...
    findings: Object.fromEntries(
      Object.keys(report.findings || {}).map((k) => [k, []]),
    ),
    allFindings: [],
    summary: {
      ...rebuilt.summary,
    ...
    scannerStatistics: patchScannerStatisticsEnv(report.scannerStatistics, 0),
    exportNormalized: true,
...
function patchFileReductionPlanUnusedCounts(plan, unusedCount) {
  ...
  if (
    unusedCount === 0 &&
    (plan.totals?.safeToDeleteBytes ?? 0) === 0 &&
    (plan.totals?.duplicateAssetBytes ?? 0) === 0
  ) {
    next.hygieneSummary = {
      ...(plan.hygieneSummary || {}),
      safeToDeleteBytes: 0,
...
function sanitizeBenchmarkFileReductionExport(report) {
  ...
  const unusedCount =
    normalized.summary?.unusedFileCandidates ??
    normalized.findings?.unusedFiles?.length ??
    0;
  const fr = normalized.executiveSummary?.fileReduction || {};

  let executiveSummary = normalized.executiveSummary;
  if (executiveSummary) {
    const actions = (executiveSummary.priorityActions || []).filter(
      (action) => {
        if (
          /unused-file candidates/i.test(String(action.title || "")) &&
          unusedCount === 0
        ) {
          return false;
        }
    ...
    executiveSummary = {
      ...
      notes: [
        ...
        ...(executiveSummary.notes || []).filter(
          (n) =>
            !/production paths first/i.test(String(n)) &&
      ...
      remediationHint:
        unusedCount > 0 || (fr.duplicateAssetBytes || 0) > 0
  ...
  const patchedPlan = patchFileReductionPlanUnusedCounts(
...
function resolveFileReductionRemediationHint(fr = {}) {
  ...
  if (safeBytes > 0) {
    return (
      "Phase 1 safe-delete: ~" +
      Number(safeBytes).toLocaleString() +
      " B in regenerable artifact directories — see fileReductionPlan.safeToDelete before deleting."
    );
  }
...
function resolveFileReductionStatus(report) {
  ...
  const safeBytes =
    report.fileReductionPlan?.totals?.safeToDeleteBytes ??
...
function enrichProductInventoryForExport(
  ...
  const auditFiles =
    options.repositoryFilesTotal ?? options.auditRepositoryFiles ?? null;
...
function buildDataQualityHygieneSummary(report, options = {}) {
  ...
  return {
    dataQualityStatus:
      report.dataQualityStatus ?? resolveDataQualityStatus(report),
    ...
    envKeysReferenced:
      report.scanners?.["environment-variables"]?.referencedKeys ?? null,
    dataLineageSampleFiles:
      report.scanners?.["data-lineage"]?.dataFilesTracked ??
      report.metadata?.dataLineage?.length ??
      null,
    dataAccessSourceScanned:
      report.scanners?.["data-access-patterns"]?.sourceFilesScanned ?? null,
    ...
    report.hygieneSummary?.jestBaselineChecked === false
...
function buildFileReductionHygieneSummary(report, options = {}) {
  ...
  const workspaceFiles = report.inventory?.totalFiles ?? null;
  const unusedScanned =
    report.scanners?.["unused-files"]?.sourceFilesScanned ??
    report.fileReductionPlan?.unusedFiles?.sourceFilesScanned ??
    null;
  const entryPoints =
    report.metadata?.entryPoints?.length ??
    report.scanners?.["unused-files"]?.entryPoints ??
    null;
  return {
    fileReductionStatus:
      report.fileReductionStatus ?? resolveFileReductionStatus(report),
    workspaceFilesScanned: workspaceFiles,
    ...(gateTotal != null ? { gateRepositoryFilesTotal: gateTotal } : {}),
    ...(gateTotal != null &&
    workspaceFiles != null &&
    ...
    safeToDeleteBytes:
      ...
      0,
    duplicateAssetBytes:
      report.fileReductionPlan?.totals?.duplicateAssetBytes ??
    ...
    assetFilesScanned:
      report.scanners?.["asset-consolidation"]?.assetFilesScanned ?? null,
...
function buildProductFileReductionExportNotes(report, options = {}) {
  ...
  const assetScanned =
    report.scanners?.["asset-consolidation"]?.assetFilesScanned;
  ...
  const zeroByteShells = shells.filter(
    (d) =>
      (d.bytes ?? 0) === 0 && /^(node_modules|coverage)$/.test(String(d.path)),
  );
  ...
  const reviewBytes =
    report.fileReductionPlan?.totals?.reviewBeforeDeleteBytes ?? 0;
  if (reviewBytes > 0) {
    notes.push(
  ...
  if (
    (report.summary?.totalFindings ?? 0) === 0 &&
...
function buildProductDataQualityExportNotes(report, options = {}) {
  ...
  const sourceScanned =
    report.scanners?.["data-access-patterns"]?.sourceFilesScanned;
  ...
  const missing =
    report.scanners?.["environment-variables"]?.missingKeys ??
    report.findings?.environmentVariables?.filter(
      (f) => f.type === "missing-env-key",
    ).length ??
    0;
  const syncIo =
    report.scanners?.["data-access-patterns"]?.patternFindings ??
  ...
  if (mirrorExcluded > 0) {
    ...
    const mirrorConsumers = (report.metadata?.dataLineage || []).flatMap(
      (row) => (row.consumers || []).filter((c) => isMirrorCliConsumerPath(c)),
    );
  ...
  const referencedKeys =
    report.scanners?.["environment-variables"]?.referencedKeys ?? 0;
...
function fixEstimatedReductionPct(summary, inventory) {
  ...
  return {
    ...summary,
    estimatedReductionPct: Math.min(
      100,
      Math.round((findings / totalFiles) * 1000) / 10,
    ),
  };
}
...
function repairCompactAllFindings(report) {
  ...
  if (!topGroups.length || !Array.isArray(report.allFindings))
  ...
  const repaired = report.allFindings.map((finding) => {
    ...
    const normalized = normalizeDuplicateGroupForBrief({
      ...finding,
      ...group,
    });
    return normalized
      ? {
          ...finding,
          path: normalized.keeper,
          keeper: normalized.keeper,
...
function repairCompactAssetFindings(report) {
  ...
  const repaired = (report.findings?.assetConsolidation || []).map(
    (finding, index) => {
      const group =
        topGroups[index] ||
        topGroups.find((g) => g.reclaimableBytes === finding.reclaimableBytes);
...
function sanitizeFileReductionPlanForProduct(plan) {
  ...
  if (plan.reviewBeforeDelete?.logs?.length) {
    const keptLogs = plan.reviewBeforeDelete.logs.filter(
      (entry) => !shouldSkipRuntimeLogFile(entry.path),
    );
...
function sanitizeExecutiveSummaryForFileReduction(executiveSummary, context) {
  ...
  if (context.benchmarkScan) {
    ...
    next.notes = [
      "File-reduction on OSS clone — not Simplebeacon platform inventory reduction.",
      ...(next.notes || []).filter(
        (n) => !/workspace scanners.*package\.json/i.test(String(n)),
      ),
    ]
      .filter((note, index, all) => all.indexOf(note) === index)
      .slice(0, 8);
  ...
  if ((fr.reviewBeforeDeleteBytes || 0) === 0) {
    next.priorityActions = actions.filter(
      (action) => !/review build artifact/i.test(String(action.title || "")),
    );
  } else {
...
function sanitizeDataCleanupReportExport(report, options = {}) {
  ...
  if (next.executiveSummary) {
    next.executiveSummary =
      profile === "file-reduction"
        ? sanitizeExecutiveSummaryForFileReduction(next.executiveSummary, {
            ...context,
  ...
  if (next.fileReductionPlan && profile === "file-reduction") {
    ...
    if (
      next.findings?.assetConsolidation?.length &&
      next.fileReductionPlan.duplicateAssets?.topGroups?.length
    ) {
      next.findings = {
        ...next.findings,
        assetConsolidation:
  ...
  if (next.scannerStatistics) {
    next.scannerStatistics = sanitizeScannerStatisticsForExport(
      next.scannerStatistics,
      context,
    );
  }
  ...
  if (benchmarkScan) {
    ...
    if (profile === "data-quality" && next.executiveSummary) {
      next.executiveSummary = {
        ...
        exportProfile: "data-quality",
        remediationHint:
          (next.summary?.totalFindings ?? 0) > 0
