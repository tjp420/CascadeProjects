  ...
  if (next.summary) {
    next.summary = {
      ...
      buildArtifactFindings:
        nextFindings.buildArtifacts?.length ?? next.summary.buildArtifactFindings,
    };
  }
...
function patchFileReductionPlanUnusedCounts(plan, unusedCount) {
  ...
  if (Array.isArray(plan.summaryTable)) {
    next.summaryTable = plan.summaryTable.map((row) =>
      /unused source files/i.test(String(row.category || '')) ? { ...row, files: unusedCount } : row
    );
  }
  if (
    unusedCount === 0 &&
...
function sanitizeBenchmarkFileReductionExport(report) {
  ...
  const unusedCount =
    normalized.summary?.unusedFileCandidates ?? normalized.findings?.unusedFiles?.length ?? 0;
  const fr = normalized.executiveSummary?.fileReduction || {};

  let executiveSummary = normalized.executiveSummary;
  if (executiveSummary) {
    const actions = (executiveSummary.priorityActions || []).filter((action) => {
...
function sanitizeBenchmarkExportByProfile(report) {
  ...
  return {
    exportNormalized: true,
    fileReductionStatus:
      report.scanProfile === 'file-reduction' ? resolveFileReductionStatus(report) : undefined,
    dataQualityStatus: resolveDataQualityStatus(report),
    securityHandoffEligible: false,
  };
}
...
function buildFileReductionHygieneSummary(report, options = {}) {
  ...
  const unusedScanned =
    ...
    null;
  const entryPoints =
    report.metadata?.entryPoints?.length ?? report.scanners?.['unused-files']?.entryPoints ?? null;
  return {
...
function buildProductFileReductionExportNotes(report, options = {}) {
  ...
  const zeroByteShells = shells.filter(
    (d) => (d.bytes ?? 0) === 0 && /^(node_modules|coverage)$/.test(String(d.path))
  );
...
function buildProductDataQualityExportNotes(report, options = {}) {
  ...
  const lineageSamples =
    report.scanners?.['data-lineage']?.dataFilesTracked ?? report.metadata?.dataLineage?.length;
  if (
    lineageSamples != null &&
  ...
  if (mirrorExcluded > 0) {
    ...
    const mirrorConsumers = (report.metadata?.dataLineage || []).flatMap((row) =>
...
function sanitizeExecutiveSummaryForFileReduction(executiveSummary, context) {
  ...
  if (context.benchmarkScan) {
    ...
    const safeArtifacts = (context.findings?.buildArtifacts || []).filter(
      (f) =>
        f.action === 'safe-to-delete' && !String(f.reason || '').includes('contents not walked')
    );
    if (
