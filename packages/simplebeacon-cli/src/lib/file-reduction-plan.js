...
function dedupeTopLevelDirectories(findings = []) {
  ...
  for (const finding of normalized) {
    if (
      roots.some(
        (root) => finding.path === root || finding.path.startsWith(`${root}/`),
      )
    ) {
      continue;
    }
...
function groupDirectoriesByCategory(findings = []) {
  ...
  for (const finding of findings) {
    ...
    const key =
      finding.category || finding.reason?.replace(" directory", "") || "other";
...
function buildFileReductionPlan(report) {
  ...
  const safeDirectories = dedupeTopLevelDirectories([
    ...buildArtifacts,
    ...directoryBloat,
  ]).sort((a, b) => (b.sizeBytes || 0) - (a.sizeBytes || 0));
  const safeRoots = safeDirectories.map((finding) =>
    String(finding.path || "").replace(/\\/g, "/"),
  );

  const reviewFiles = [
    ...buildArtifacts.filter(
      (finding) => finding.action === "review-before-delete",
    ),
    ...directoryBloat.filter(
      (finding) => finding.action === "review-before-delete",
    ),
  ]
    .filter(
  ...
  const reviewLogs = reviewFiles.filter(
    (finding) => finding.category === "logs",
  );
  const reviewBinaries = reviewFiles.filter(
    (finding) => finding.category === "binaries",
  );
  const reviewSourceMaps = reviewFiles.filter(
    (finding) => finding.category === "source-maps",
  );

  const safeBytes = safeDirectories.reduce(
  ...
  return {
    scopeNote:
      "Directory totals exclude nested artifact paths to avoid double-counting reclaimable space.",
    totals: {
      reclaimableBytes:
        report.summary?.reclaimableBytes || buildSummary.reclaimableBytes || 0,
    ...
    safeToDelete: {
      ...
      files: safeDirectories.reduce(
        (sum, finding) => sum + (finding.fileCount || 0),
        0,
      ),
    ...
    unusedFiles: {
      ...
      entryPoints:
        unusedSummary.entryPoints || report.metadata?.entryPoints?.length || 0,
    ...
    summaryTable: [
      ...
      },
      {
        ...
        files: duplicateAssets.reduce(
          (sum, group) => sum + (group.duplicates?.length || 0),
          0,
        ),
