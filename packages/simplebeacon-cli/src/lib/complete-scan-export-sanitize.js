...
const {
  sanitizeNpmAuditExport: sanitizeNpmAuditPayload,
} = require("./npm-audit-export-sanitize");
const {
...
function sanitizeRoadmapForBenchmark(
  roadmap,
  productPlatformRoot,
  scanTargetRoot,
...
function sanitizeEmbeddedEuAiActSprint(sprint, nestedOptions = {}) {
  if (!sprint || typeof sprint !== "object" || sprint.ok === false)
    return sprint;
  try {
    const {
      sanitizeEuAiActSprintArtifactExport,
    } = require("../../../../server/lib/eu-ai-act-export.js");
    return sanitizeEuAiActSprintArtifactExport(sprint, nestedOptions);
  } catch {
    const label = projectLabelFromPath(
      nestedOptions.projectPath || sprint.projectPath || "",
    );
    return {
      ...sprint,
        ...
        ? {
            artifacts: {
              ...
              report: (() => {
                ...
                const projectNormalized = String(
                  nestedOptions.projectPath || sprint.projectPath || "",
                ).replace(/\\/g, "/");
                if (
                  projectNormalized &&
                  normalized.startsWith(projectNormalized + "/")
                ) {
      ...
      ...(sprint.complianceChecklist
        ? {
            complianceChecklist: {
              ...sprint.complianceChecklist,
              projectRoot: redactProjectPathForExport(
                sprint.complianceChecklist.projectRoot,
                label,
              ),
            },
          }
    ...
    ...(bundle.completeScanAnalysis
      ? {
          completeScanAnalysis: {
            ...bundle.completeScanAnalysis,
            projectPath: redactProjectPathForExport(
              bundle.completeScanAnalysis.projectPath || projectPath,
              label,
            ),
          },
        }
...
function resolveBenchmarkGateAttestation(sb, hollowGate) {
  ...
  if (sb?.gate?.pass) {
    const ruleScoped =
      sb.ruleScopedFilesAnalyzed ?? sb.scanScope?.ruleScopedFilesAnalyzed ?? 0;
...
function buildBenchmarkCompleteScanHygieneSummary(bundle, auditFiles) {
  ...
  const scannedLlm =
    sb?.llmSlopFilesScanned ?? sb?.scanScope?.llmSlopFilesScanned ?? null;
  const reconciledLlm =
    sb?.llmSlopScanReconciled ?? sb?.scanScope?.llmSlopScanReconciled;
  return {
    simplebeaconGatePass: bundle.summary?.simplebeaconGatePass ?? null,
    simplebeaconGateAttestation:
      bundle.summary?.simplebeaconGateAttestation ?? null,
    simplebeaconIssues: bundle.summary?.simplebeaconIssues ?? 0,
    complianceFailed: bundle.summary?.complianceFailed ?? 0,
    repositoryFilesTotal:
      bundle.summary?.platformScope?.repositoryFilesTotal ?? auditFiles ?? null,
    ...
    ...(bundle.summary?.cleanupProjectedFilesRaw != null
      ...
      : {}),
    roadmapMisscoped:
      bundle.results?.roadmap?.misscopedPlatformCodeWalk === true,
    codebaseMisscoped:
      bundle.results?.codebase?.misscopedPlatformCodeWalk === true,
    fictionJsonFilesScanned:
      sb?.fictionJsonFilesScanned ??
...
function resolveCompleteScanGateContext(bundle, options = {}) {
  ...
  const contentScanned =
    sb.scanScope?.fullDirectoryStats?.contentScanned ??
...
function buildProductCompleteScanHygieneSummary(bundle, gateContext = {}) {
  ...
  const hygiene = bundle.hygieneSummary || {};
  const completeScanHealthStatus =
    hygiene.completeScanHealthStatus ??
    (gatePass &&
    (complianceFailed ?? bundle.summary?.complianceFailed ?? 1) === 0
  ...
  return {
    ...
    repositoryFilesTotal > credentialScanned
    ...
    ...(bundle.summary?.euAiActIncluded != null
      ? { euAiActIncluded: bundle.summary.euAiActIncluded }
      : {}),
...
function buildProductCompleteScanScanScope(bundle, gateContext = {}) {
  ...
  return {
    ...
    ...(gateProfile ? { gateRuleBundleProfile: gateProfile } : {}),
    completeScanNote:
      bundle.scanScope?.completeScanNote ||
...
function rebuildCompleteScanAnalysis(bundle, benchmarkScan) {
  ...
  const analysis = {
    ...
    dataQuality: dqExec
      ? {
          ...
          credentialsNeedingReview:
            dqExec.security?.credentialsNeedingReview ?? null,
    ...
    priorityActions: priorityActions.length
      ? priorityActions
      : bundle.completeScanAnalysis?.priorityActions || [],
    notes: replaceMisleadingAnalysisNotes(
      bundle.completeScanAnalysis?.notes || [],
      benchmarkScan,
    ),
  };
...
function buildProductCompleteScanExportNotes(bundle, context = {}) {
  ...
  const sb = bundle.results?.simplebeacon;
  const auditFiles =
    context.repositoryFilesTotal ??
    sb?.repositoryFilesTotal ??
    sb?.repositoryInventory?.totalFiles;
  const cleanupInv =
    bundle.results?.cleanupAssistant?.inventory?.totalFiles ??
  ...
  if (context.complianceHandoffEligible === false) {
    notes.push(
      "Compliance checklist export attests rule rows only — handoffEligible remains false until operator sign-off.",
    );
  }
...
function recomputeCompleteScanSummary(bundle, context = {}) {
  ...
  if (sb) {
    summary.simplebeaconGatePass =
      sb.gate?.pass ?? summary.simplebeaconGatePass;
    summary.simplebeaconIssues =
      sb.issueCount ??
      sb.gate?.blockingCount ??
      sb.rawIssues?.length ??
      summary.simplebeaconIssues;
    if (context.benchmarkScan) {
      summary.simplebeaconGateAttestation = resolveBenchmarkGateAttestation(
        sb,
        context.hollowGate,
      );
      ...
      summary.simplebeaconGateAttestation =
        sb.gateAttestation ||
        (sb.gate?.pass
          ? "platform-gate-pass"
          : sb.gate?.pass === false
  ...
  const complianceSummary =
    compliance?.checklist?.summary ?? compliance?.summary;
  if (complianceSummary) {
    summary.compliancePassed =
      complianceSummary.passed ?? summary.compliancePassed;
    summary.complianceFailed =
      complianceSummary.failed ?? summary.complianceFailed;
  }

  if (consolidation?.summary) {
    summary.consolidationDuplicateGroups =
      consolidation.summary.exactDuplicateGroups ??
  ...
  if (mockScan) {
    ...
    summary.fictionDigestTrust =
      mockScan.digestTrust ?? summary.fictionDigestTrust;
  }

  if (
    roadmap?.codeAnalysis?.structure?.totalFiles != null ||
    summary.roadmapFiles != null
  ) {
    const auditFiles =
      sb?.repositoryFilesTotal ?? sb?.repositoryInventory?.totalFiles ?? null;
  ...
  if (fileReduction?.summary?.unusedCandidates != null) {
    summary.fileReductionUnusedCandidates =
      fileReduction.summary.unusedCandidates;
  }
  ...
  const cleanupExplorer =
    cleanup?.inventory?.explorerInventoryRaw ??
  ...
  if (
    ...
    summary.cleanupProjectedFilesNote =
      cleanup?.inventory?.inventoryNote ||
...
function applySummaryAttestation(bundle, context) {
  ...
  if (context.benchmarkScan) {
    ...
    summary.simplebeaconGateAttestation = resolveBenchmarkGateAttestation(
      sb,
      context.hollowGate || hasHollowGateAttestation(sb),
    );
  ...
  summary.complianceHandoffEligible =
    context.complianceHandoffEligible ?? false;
  if (context.complianceExportNotes?.length) {
    summary.complianceExportNotes = dedupeCompleteScanExportNotes(
      context.complianceExportNotes,
    );
  }
  ...
  summary.platformScope = {
    ...
    mockSampleFiles:
      sb?.mockSampleFiles ?? summary.platformScope?.mockSampleFiles ?? null,
    ...
    simplebeaconGatePass:
      summary.simplebeaconGatePass ?? sb?.gate?.pass ?? null,
  ...
  return {
    ...bundle,
    projectPath: redactProjectPathForExport(
      normalizedPath || bundle.projectPath,
      projectLabelFromPath(normalizedPath),
    ),
    summary,
  };
}
...
function sanitizeCompleteScanExport(bundle, options = {}) {
  ...
  const benchmarkScan =
    options.benchmarkScan ?? isBenchmarkCacheProjectPath(projectPath);
  ...
  const repositoryFilesTotal =
    next.results.simplebeacon?.repositoryFilesTotal ??
  ...
  if (next.results.consolidation) {
    next.results.consolidation = sanitizeConsolidationExport(
      next.results.consolidation,
      nestedOptions,
    );
  }
  if (next.results.mockScan) {
    next.results.mockScan = sanitizeFictionDigestExport(
  ...
  if (next.results.compliance) {
    if (sbAfterGate) {
      ...
      next.results.compliance = sanitizeComplianceChecklistArtifactExport(
        bundled.checklist,
        {
          projectPath,
          gateReport: sbAfterGate,
      ...
      next.results.compliance = sanitizeComplianceForExport(
  ...
  if (next.completeScanAnalysis) {
    next.completeScanAnalysis = {
      ...next.completeScanAnalysis,
      notes: replaceMisleadingAnalysisNotes(
        next.completeScanAnalysis.notes,
        benchmarkScan,
      ),
  ...
  if (!benchmarkScan) {
    ...
    next = {
      ...
      completeScanHealthStatus:
        ...
        (next.summary?.complianceFailed ?? 1) === 0
          ? "hygiene-pass-not-handoff"
          : "review-required",
      ...
      exportNotes: buildProductCompleteScanExportNotes(next, {
        ...context,
        ...gateContext,
      }),
