    ...
    return {
      ...
      ...(sprint.report
        ? {
            report: {
              ...sprint.report,
              projectRoot: redactProjectPathForExport(sprint.report.projectRoot, label),
            },
          }
        : {}),
      ...
      ...(sprint.assessment
        ? {
            assessment: {
              ...sprint.assessment,
              projectRoot: redactProjectPathForExport(sprint.assessment.projectRoot, label),
            },
          }
        : {}),
    };
...
function buildBenchmarkCompleteScanHygieneSummary(bundle, auditFiles) {
  ...
  return {
    ...
    fictionJsonFilesScanned:
      sb?.fictionJsonFilesScanned ?? sb?.scanScope?.fictionJsonFilesScanned ?? null,
...
function resolveCompleteScanGateContext(bundle, options = {}) {
  ...
  const fictionSampleFilesScanned =
    ...
    null;
  const gatePass =
    bundle.summary?.simplebeaconGatePass ?? sb.gate?.pass ?? hygiene.gatePass ?? null;
  const blockingCount =
...
function buildProductCompleteScanExportNotes(bundle, context = {}) {
  ...
  const sb = bundle.results?.simplebeacon;
  const auditFiles =
    context.repositoryFilesTotal ?? sb?.repositoryFilesTotal ?? sb?.repositoryInventory?.totalFiles;
  const cleanupInv =
...
function recomputeCompleteScanSummary(bundle, context = {}) {
  ...
  if (sb) {
    ...
    summary.simplebeaconIssues =
      sb.issueCount ?? sb.gate?.blockingCount ?? sb.rawIssues?.length ?? summary.simplebeaconIssues;
...
function applySummaryAttestation(bundle, context) {
  ...
  if (context.benchmarkScan) {
    ...
    summary.simplebeaconGateAttestation =
      sb?.gateAttestation || (sb?.gate?.pass ? 'platform-gate-pass' : 'not-evaluated');
  }
