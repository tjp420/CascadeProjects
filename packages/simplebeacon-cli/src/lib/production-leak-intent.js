...
function isPlainSampleJsonMatch(matchText) {
  return (
    /sample\.json/i.test(String(matchText || '')) && !/-sample\.json/i.test(String(matchText || ''))
  );
}
 ...
 */
function classifyProductionLeakMatch({ relativePath, content, lineIndex, matchText, patternId }) {
