...
function extractSnippet(filePath, lineHint, contextLines = 6) {
  ...
  const ctx =
    Number.isFinite(contextLines) && contextLines > 0 ? contextLines : 6;
...
function fixGoStub(snippet, _finding) {
  ...
  const stubRe =
    /func\s+(\w+)\s*\([^)]*\)[^{]*\{\s*(?:return\s+(?:nil|""|0|false)\s*)?(?:panic\("[^"]*"\)\s*)?\}/;
  const m = snippet.match(stubRe);
  if (!m) return null;
  const funcName = m[1];
  const search = m[0];
...
function fixJsStub(snippet, _finding) {
  ...
  const stubRe =
    /((?:async\s+)?function\s+(\w+)\s*\([^)]*\)\s*\{)\s*(?:return\s+(?:undefined|null|void\s+0)\s*;?)?\s*\}/;
  const m = snippet.match(stubRe);
  if (!m) return null;
  const funcName = m[2];
  const search = m[0];
...
function fixTokenBleed(snippet, finding) {
  ...
  if (ext === "py") {
    ...
    const callRe =
      /((?:openai|anthropic|client|llm)\.\w+\.\w+\.(?:create|generate|invoke|stream|batch)\s*\()([\s\S]*?)(\))/;
  ...
  const callRe =
    /((?:openai|anthropic|client|llm)\.\w+\.\w+\.(?:create|generate|invoke|stream|batch)\s*\(\s*\{)([\s\S]*?)(\}\s*\))/;
  const m = snippet.match(callRe);
  if (!m) return null;
  if (/max_tokens|max_completion_tokens|maxOutputTokens/i.test(m[2]))
    return null;
  const search = m[0];
...
function fixSqlInjection(snippet, _finding) {
  ...
  const sqlRe =
    /(\w+(?:\.\w+)*)\s*\(\s*`((?:SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|EXEC)\s[^`]*)`\s*\)/;
...
function fixGoTokenBleed(snippet, _finding) {
  ...
  const callRe =
    /((?:openai|anthropic|client|llm)\.(?:Chat|Complete|Generate|Create|Invoke|Stream|Batch)\w*(?:\.\w+)?)\s*\(\s*(\w+)\s*,\s*&(\w+\.\w+\{)([^}]*)(\}\s*\))/;
  const m = snippet.match(callRe);
  if (!m) return null;
...
function remediateFinding(finding, options = {}) {
  ...
  if (!fixFn) {
    return {
      applied: false,
      reason: `No deterministic fix for pattern ${patternId}`,
      engine: "deterministic",
    };
  }
  ...
  const filePath =
    finding.filePath ||
    finding.file ||
    (finding.affectedFiles && finding.affectedFiles[0]);
  if (!filePath) {
    return {
      applied: false,
      reason: "No filePath in finding",
      engine: "deterministic",
    };
  }
  ...
  try {
    ...
    return {
      applied: false,
      reason: `Fix generator error: ${err.message}`,
      engine: "deterministic",
    };
  }
  ...
  if (!fix || !fix.search) {
    return {
      applied: false,
      reason: "Pattern did not match for deterministic fix",
      engine: "deterministic",
    };
  }
  ...
  try {
    ...
    if (!hasMatch) {
      return {
        applied: false,
        diff,
        reason: "Search text not found in file (whitespace mismatch?)",
        engine: "deterministic",
      };
    }
...
function runDeterministicRemediation(findings, options = {}) {
  ...
  const results = [];
  const maxFixes =
    Number.isFinite(options.maxFixes) && options.maxFixes > 0
      ? options.maxFixes
      : 20;
...
function suggestSensitiveDataPatch(finding) {
  ...
  const secretRe =
    /(["'])(sk_live_[A-Za-z0-9-_]+|re_[A-Za-z0-9-_]+|AIza[0-9A-Za-z\-_]{35}|-----BEGIN PRIVATE KEY-----[\s\S]+?-----END PRIVATE KEY-----|AKIA[0-9A-Z]{16}|[A-Za-z0-9-_]{32,})\1/;
  ...
  const suggestedEnv = keyMatch
    ? `${keyMatch[2].toUpperCase()}_SECRET`
    : "REDACTED_SECRET";
...
function suggestHardcodedConfidencePatch(finding) {
  ...
  return {
    ...
    suggestion: {
      instructions: [
        "Replace hardcoded confidence with a configurable threshold (env var). Review behavior after change.",
      ],
    },
  };
}
...
function suggestFictionKpiPatch(finding) {
  ...
  return {
    ...
    suggestion: {
      instructions: [
        "Replace fictional KPI placeholders with real metrics or configuration.",
      ],
    },
  };
}
