...
function ingestDecorate(content, relativePath, graph) {
    ...
    while ((match = actorRe.exec(stripped)) !== null) {
        registerActor(graph, match[1], relativePath, lineNumberAt(stripped, match.index), 'decorate');
    }
    ...
    while ((match = replacesRe.exec(stripped)) !== null) {
        registerActor(graph, match[1], relativePath, lineNumberAt(stripped, match.index), 'decorate');
        registerReplaces(graph, match[1], match[2], relativePath, lineNumberAt(stripped, match.index));
    }
...
async function expandIncludes(baseDir, files, ignoreGlobs = []) {
    const { extractIncludes, resolveIncludePath } = require('./gzdoom-include-resolver');
