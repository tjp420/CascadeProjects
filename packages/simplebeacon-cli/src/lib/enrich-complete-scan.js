...
function dedupePriorityActions(actions = []) {
  ...
  return actions.filter((action) => {
    const key = `${action.priority}:${action.title}:${action.detail}`;
...
function buildCompleteScanAnalysis(completeScan) {
  ...
  const priorityActions = dedupePriorityActions([
...
function enrichCompleteScan(completeScan) {
  ...
  if (enriched.results?.fileReduction) {
    enriched.results.fileReduction = enrichCleanupReport(
      enriched.results.fileReduction,
      {
  ...
  if (enriched.results?.dataQuality) {
    enriched.results.dataQuality = enrichCleanupReport(
      enriched.results.dataQuality,
      {
  ...
  enriched.summary = {
    ...
    fileReductionSafeToDeleteBytes:
      frPlan?.totals?.safeToDeleteBytes ??
      fr?.scanners?.["build-artifacts"]?.safeToDeleteBytes ??
      null,
    fileReductionReviewBytes:
      frPlan?.totals?.reviewBeforeDeleteBytes ??
      fr?.scanners?.["build-artifacts"]?.reviewBeforeDeleteBytes ??
      null,
    fileReductionImmediateSavingsBytes:
      frPlan?.totals?.estimatedImmediateSavingsBytes ?? null,
    fileReductionReclaimableBytes:
      fr?.summary?.reclaimableBytes ??
      enriched.summary?.fileReductionReclaimableBytes ??
      null,
    fileReductionUnusedCandidates:
      frPlan?.unusedFiles?.candidates ??
      fr?.summary?.unusedFileCandidates ??
      null,
    dataQualityFindings:
      dq?.summary?.totalFindings ??
      enriched.summary?.dataQualityFindings ??
      null,
    dataQualityWorkspacePackages:
      dqWorkspace?.packageJsonFiles ??
      dq?.scanners?.["dependency-health"]?.packageJsonFiles ??
      null,
    dataQualityUnusedDependencies:
      dqWorkspace?.unusedDependencies ??
      dq?.scanners?.["dependency-health"]?.unusedDependencies ??
      null,
    dataQualityCredentialsNeedingReview:
      dqSecurity?.credentialsNeedingReview ?? null,
