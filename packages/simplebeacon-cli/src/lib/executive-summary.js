...
function buildPriorityActions(report, scanners, privacyTriage) {
  ...
  for (const item of privacyTriage.items) {
    ...
    actions.push({
      ...
      title:
        item.kind === 'credential' ? 'Investigate credential exposure' : 'Review PII in data file',
  ...
  return {
    ...
    security: {
      ...
      piiByCategory: Object.entries(privacyTriage.byCategory)
        .filter(
          ([category]) => category !== 'review-required' || privacyTriage.byCategory[category]
        )
        .map(([category, count]) => ({
          category,
      ...
      piiSamples: piiFindings.filter((item) => item.category === 'review-required').slice(0, 5),
    },
    workspace: {
    ...
    fileReduction: {
      ...
      safeToDeleteBytes:
        build.safeToDeleteBytes || report.fileReductionPlan?.totals?.safeToDeleteBytes || 0,
