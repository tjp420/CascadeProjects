...
function buildExecutiveSummary(report) {
  ...
  const privacyTriage = triagePrivacyFindings(
    report.findings?.dataPrivacy || [],
  );
  const credentialFindings = privacyTriage.items.filter(
  ...
  if (
    ...
    privacyTriage.credentialsNeedingReview === 0
  ) {
    notes.unshift(
      "All credential hits are documented examples or intentional test fixtures.",
    );
  }

  if (piiFindings.length && privacyTriage.piiNeedingReview === 0) {
    notes.unshift(
      "All PII hits are in documentation, reports, or mock/sample data.",
    );
  }
  ...
  return {
      ...
      piiByCategory: Object.entries(privacyTriage.byCategory)
        .filter(
          ([category]) =>
            ...
            privacyTriage.byCategory[category],
        )
        .map(([category, count]) => ({
          category,
    ...
    fileReduction: {
      reclaimableBytes:
        report.summary?.reclaimableBytes || build.reclaimableBytes || 0,
      safeToDeleteBytes:
        build.safeToDeleteBytes ||
        report.fileReductionPlan?.totals?.safeToDeleteBytes ||
        0,
      reviewBeforeDeleteBytes:
        build.reviewBeforeDeleteBytes ||
        report.fileReductionPlan?.totals?.reviewBeforeDeleteBytes ||
        0,
      duplicateAssetBytes:
        report.scanners?.["asset-consolidation"]?.reclaimableBytes || 0,
      estimatedImmediateSavingsBytes:
        report.fileReductionPlan?.totals?.estimatedImmediateSavingsBytes || 0,
