...
function buildScannerStatistics(report) {
  ...
  return {
    ...
    scanners: {
      ...
      "dependency-health": {
        ...
        stats: {
          ...
          duplicateDependencies:
            dep.duplicateDependencies || depCounts["duplicate-dependency"] || 0,
        },
        findings: {
      ...
      "data-freshness": {
        ...
        findings: {
          ...
          staleData:
            countFindingsByType(findings.dataFreshness)["stale-data"] || 0,
        },
      },
      ...
      "data-privacy": {
        ...
        findings: {
          ...
          credentialHits: privacyCounts["data-privacy"]
            ? (findings.dataPrivacy || []).filter((f) =>
                String(f.reason).includes("Credential"),
              ).length
            : 0,
          piiHits: (findings.dataPrivacy || []).filter(
            (f) => !String(f.reason).includes("Credential"),
          ).length,
        },
      },
      "data-lineage": {
    ...
    findingsBreakdown: {
      ...
      dataPrivacy: {
        total: (findings.dataPrivacy || []).length,
        credentialHits: (findings.dataPrivacy || []).filter((f) =>
          String(f.reason).includes("Credential"),
        ).length,
        piiHits: (findings.dataPrivacy || []).filter(
          (f) => !String(f.reason).includes("Credential"),
