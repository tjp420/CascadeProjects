...
const {
  isWorkspacePath,
} = require("../analyzers/data-cleanup/utils/workspace-path-utils");
...
function rebuildWorkspaceScopedSummaries(report) {
  ...
  const environmentFindings = filterWorkspaceFindings(
    findings.environmentVariables,
  );
  ...
  report.scanners = {
    ...
    "config-management": {
      ...
      packageJsonFiles:
        new Set(
          configFindings
            .filter((finding) =>
              String(finding.path || "").endsWith("package.json"),
            )
    ...
    "dependency-health": {
      ...
      packageJsonFiles:
        new Set(dependencyFindings.map((finding) => finding.path)).size ||
      ...
      uniqueDependencies:
        ...
        report.scanners?.["dependency-health"]?.uniqueDependencies ||
        0,
...
function compactDataCleanupReportForClient(report, options = {}) {
  ...
  const compact = {
    ...
    compact: true,
    findings: {
      buildArtifacts: sliceFindings(
        report.findings?.buildArtifacts,
        bucketLimit,
      ),
      ...
      configManagement: sliceFindings(
        report.findings?.configManagement,
        bucketLimit,
      ),
      dependencyHealth: sliceFindings(
        report.findings?.dependencyHealth,
        bucketLimit,
      ),
      environmentVariables: sliceFindings(
        report.findings?.environmentVariables,
        bucketLimit,
      ),
      dataFreshness: sliceFindings(report.findings?.dataFreshness, 8),
      dataAccessPatterns: sliceFindings(
        report.findings?.dataAccessPatterns,
        bucketLimit,
      ),
    ...
    allFindings: (report.allFindings || [])
      ...
      .map(slimFindingForClient),
  };
  if (options.note !== false) {
    compact.compactNote =
      "Top findings only — counts and fileReductionPlan retain full scan totals.";
  }
  return compact;
}
