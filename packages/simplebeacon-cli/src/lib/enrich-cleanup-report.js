...
function rebuildWorkspaceScopedSummaries(report) {
  ...
  report.scanners = {
    ...
    'dependency-health': {
      ...
      packageJsonFiles:
        ...
        0,
      uniqueDependencies:
        new Set(dependencyFindings.map((finding) => finding.metadata?.dependency).filter(Boolean))
          .size ||
  ...
  const unchangedTotal = Object.entries(findings)
    .filter(
      ([key]) => !['configManagement', 'dependencyHealth', 'environmentVariables'].includes(key)
    )
