...
const CREDENTIAL_PATTERNS = [
  ...
  },
  {
    id: "database-url",
    regex: new RegExp(
      "(?:postgres|postgresql|mysql|mongodb(?:\\+srv)?):\\/\\/[^\\s'\"]+:[^\\s'\"]+@[^\\s'\"]+",
      "gi",
    ),
    severity: "high",
  }, // simplebeacon-ignore redos — scanner rule definition
  {
    id: "sendgrid-key",
    regex: /\bSG\.[A-Za-z0-9_-]{20,}\b/g,
    severity: "high",
  },
  ...
  },
  {
    id: "generic-api-key",
    regex:
      /\b(api[_-]?key|secret[_-]?key|access[_-]?token)\s*[:=]\s*['"][^'"\s]{12,}['"]/gi,
    severity: "medium",
  },
  {
    id: "bearer-token",
...
function isAllowlisted(match, content, fileName = "") {
  ...
  if (
    /auth-strategies\.js$/i.test(fileName) &&
  ...
  if (
    /\*\*\*REDACTED\*\*\*/.test(match[0]) ||
    /process\.env\./i.test(snippet)
  ) {
    return true;
  }
...
function scanTextContent(fileName, content, filePath = fileName) {
  ...
  for (const pattern of CREDENTIAL_PATTERNS) {
    if (!pattern || !pattern.regex || typeof pattern.regex.exec !== "function")
      continue;
...
function isCredentialScanExcludedPath(file) {
  ...
  return (
    ...
    /packages\/simplebeacon-cli\/src\/(?:lib|rules|reporters|analyzers|proxy)\//.test(
      rel,
    ) ||
...
async function scanCredentialPatterns(files, options = {}) {
  ...
  for (const file of files) {
    ...
    if (
      ignoreGlobs.length &&
      file.relativePath &&
      ignoreGlobs.some((pattern) => globMatch(file.relativePath, pattern))
    )
      continue;
  ...
  if (
    options.scanProduction &&
    options.baseDir &&
    options.productionPaths?.length
  ) {
    const prodFiles = [];
    ...
    for (const file of prodFiles) {
      const relativePath = path
        .relative(options.baseDir, file.path)
        .split(path.sep)
        .join("/");
      if (isCredentialScanExcludedPath({ relativePath })) continue;
      if (ignoreGlobs.some((pattern) => globMatch(relativePath, pattern)))
        continue;
      if (!SCANNABLE_EXTENSIONS.has(file.ext)) continue;
      if (file.size > MAX_SCAN_BYTES) continue;
