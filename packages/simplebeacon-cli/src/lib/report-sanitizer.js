...
const REDACTION_RULES = [
  ...
  {
    pattern: /\b(sk|pk)_(test|live)_[A-Za-z0-9]{16,}\b/g,
    replacement: "$1_$2_████████████████",
  },
  {
  ...
  {
    pattern:
      /\b(api[_-]?key|secret[_-]?key|access[_-]?token|password)\s*[:=]\s*['"][^'"\s]{8,}['"]/gi,
...
function sanitizeAssessment(assessment, options = {}) {
  ...
  if (sanitized.findings && typeof sanitized.findings === "object") {
    for (const bucket of Object.values(sanitized.findings)) {
      ...
      if (bucket?.summary)
        bucket.summary = redactSecretsInString(bucket.summary);
    }
  }

  if (sanitized.complianceChecklist?.rules) {
    sanitized.complianceChecklist.rules =
      sanitized.complianceChecklist.rules.map((rule) =>
        sanitizePlainObject(rule),
      );
  }
...
function collectIssuesFromScan(rawScanJson) {
  ...
  if (Array.isArray(rawScanJson.detectedIssues))
    return rawScanJson.detectedIssues;
  if (Array.isArray(rawScanJson.findings)) return rawScanJson.findings;

  const collected = [];
  const simplebeacon =
    rawScanJson.results?.simplebeacon || rawScanJson.simplebeacon;
  if (simplebeacon) {
    collected.push(
...
function resolveFilesScanned(rawScanJson) {
  return (
    rawScanJson.summary?.codeFilesAnalyzed ??
...
function sanitizePublicOutput(rawScanJson) {
  ...
  const totalIssuesFound = issues.reduce(
  ...
  return {
    summary: {
      ...
      gatePass:
        ...
        null,
      qualityScore:
        rawScanJson.qualityScore ??
        rawScanJson.results?.simplebeacon?.qualityScore ??
        null,
      codeHealth:
        rawScanJson.summary?.codebaseHealthScore ??
...
function stripSensitiveScanFields(report) {
  ...
  if (Array.isArray(next.findings)) {
    next.findings = next.findings.filter(
      (f) => f && f.category === "workspace-health",
    );
  }
...
function applyPublicGateToAnalyzeResponse(payload) {
  ...
  const scanRoot =
    payload.report || payload.data || payload.completeScan || payload;
  ...
  if (next.completeScan)
    next.completeScan = stripSensitiveScanFields(next.completeScan);
  return next;
}
