...
function sanitizeAssessment(assessment, options = {}) {
  ...
  if (sanitized.complianceChecklist?.rules) {
    sanitized.complianceChecklist.rules = sanitized.complianceChecklist.rules.map((rule) =>
  ...
  return {
    summary: {
      ...
      gatePass:
        ...
        null,
      qualityScore:
        rawScanJson.qualityScore ?? rawScanJson.results?.simplebeacon?.qualityScore ?? null,
      codeHealth:
...
function applyPublicGateToAnalyzeResponse(payload) {
  ...
  const scanRoot = payload.report || payload.data || payload.completeScan || payload;
