...
function collectDevelopmentPhases(payload) {
  const phases = [];
  if (Array.isArray(payload.developmentPhases))
    phases.push(...payload.developmentPhases);
  if (Array.isArray(payload.roadmap?.developmentPhases))
    phases.push(...payload.roadmap.developmentPhases);
  return phases;
}
...
function detectStaleRoadmapTemplate(payload) {
  ...
  const sprint3 = phases.find((phase) =>
...
function deepIncludesFiction(value, baseline, depth = 0, keyPath = "") {
  ...
  if (SKIP_FICTION_PATHS.has(leafKey) || LEGACY_SKIP_PATH_ALIASES.has(leafKey))
    return [];
  ...
  if (typeof value === "string") {
    ...
    if (/\b1247\b/.test(value) && /mock/i.test(value))
      hits.push("1247 mock files claim");
    if (/\b1559\b/.test(value) && /files?\/?s/i.test(value))
      hits.push("1559 files/s throughput claim");
    return hits;
  }

  if (typeof value === "number") {
    if (NON_FICTION_METRIC_KEYS.has(leafKey)) return hits;
    if ((fiction.completionRates || []).includes(value))
      hits.push(`${value}% completion claim`);
    if ((fiction.openIssueCounts || []).includes(value))
      hits.push(`${value} open issues claim`);
    if ((fiction.aiConfidenceScores || []).includes(value))
      hits.push(`${value}% AI confidence claim`);
    return hits;
  }
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      ...
      hits.push(
        ...deepIncludesFiction(value[index], baseline, depth + 1, itemPath),
      );
    }
    return hits;
  }
  ...
  if (typeof value === "object") {
    for (const [key, nested] of Object.entries(value)) {
      ...
      if (SKIP_FICTION_PATHS.has(key) || LEGACY_SKIP_PATH_ALIASES.has(key))
        continue;
      if (
        key === "totalFeatures" &&
        (fiction.featureCounts || []).includes(nested)
      ) {
        hits.push(`totalFeatures=${nested}`);
      }
      if (
        (key === "completionRate" || key === "currentCompletion") &&
      ...
      if (
        key === "issuesDetected" &&
        (fiction.openIssueCounts || []).includes(nested)
      ) {
        hits.push(`issuesDetected=${nested}`);
      }
      if (
        (key === "aiConfidence" ||
          key === "previousConfidence" ||
          key === "confidence") &&
      ...
      hits.push(
        ...deepIncludesFiction(nested, baseline, depth + 1, nestedPath),
      );
    }
  }
...
function extractKpis(payload, fileName, baseline) {
  ...
  if (payload.dataSource) kpis.dataSource = payload.dataSource;
  if (payload.overview?.currentRelease)
    kpis.currentRelease = payload.overview.currentRelease;
  if (payload.releaseOverview?.currentRelease)
    kpis.currentRelease = payload.releaseOverview.currentRelease;
  if (payload.executiveSummary?.currentRelease)
    kpis.currentRelease = payload.executiveSummary.currentRelease;

  if (payload.overview?.jestTests) kpis.jestTests = payload.overview.jestTests;
  if (payload.repositoryMetrics?.jestTests != null)
    kpis.jestTests = payload.repositoryMetrics.jestTests;
  if (payload.repositorySnapshot?.jestTests != null)
    kpis.jestTests = payload.repositorySnapshot.jestTests;
...
function fictionIssuesFromExtractions(extractions) {
  ...
  for (const entry of extractions) {
    const hits = (entry.fictionHits || []).filter(
      (h) => !String(h).startsWith("parse error"),
    );
...
function driftIssuesFromExtractions(extractions, baseline) {
  ...
  for (const entry of extractions) {
    if (
      baseline.dataSource &&
      entry.kpis.dataSource &&
      entry.kpis.dataSource !== baseline.dataSource
    ) {
      issues.push({
        ...
        recommendedAction:
          "Set dataSource to repository-audit or import via stale guard",
        affectedFiles: [entry.fileName],
      });
    }

    if (
      baseline.currentRelease &&
      entry.kpis.currentRelease &&
      entry.kpis.currentRelease !== baseline.currentRelease
    ) {
      issues.push({
        ...
        recommendedAction:
          "Align release milestone with release-timeline-sample.json",
        affectedFiles: [entry.fileName],
      });
    }
    ...
    if (baseline.jestTestsPassing != null && jest != null) {
      const numeric =
        typeof jest === "number"
          ? jest
          : parseInt(String(jest).split("/")[0], 10);
      if (!Number.isNaN(numeric) && numeric !== baseline.jestTestsPassing) {
        issues.push({
          ...
          recommendedAction:
            "Run npm test and sync sample KPIs to measured baseline",
          affectedFiles: [entry.fileName],
        });
      }
    }
    ...
    if (
      ...
      entry.kpis.model &&
      !String(entry.kpis.model).includes(baseline.activeModel.split(".")[0])
    ) {
      ...
      if (!/platform-checklist|agi-chatbot-test/i.test(model)) {
        issues.push({
          ...
          recommendedAction:
            "Use measured activeModel or platform-checklist in sample modelInfo",
          affectedFiles: [entry.fileName],
        });
      }
    }
  ...
  if (uniqueReleases.length > 1) {
    issues.push({
      ...
      recommendedAction:
        "Sync release-timeline, master-roadmap, and engineering-baseline samples",
      affectedFiles: extractions
        .filter((e) => e.kpis.currentRelease)
        .map((e) => e.fileName),
    });
  }
  ...
  async function walk(dir, depth = 0) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (SKIP_PATH_SUBSTRINGS.some((sub) => relativePath.includes(sub)))
          continue;
...
async function checkSampleConsistency(baseDir, options = {}) {
  const scope =
    options.fictionScope === "sample-paths-only"
      ? "sample-paths-only"
      : "repository-json";
    ...
    const extras = options.scanPathFiles
      ...
      .filter((file) => {
        ...
        const ignoreGlobs = Array.isArray(options.ignoreGlobs)
          ? options.ignoreGlobs
          : [];
        if (ignoreGlobs.some((pattern) => globMatch(rel, pattern)))
          return false;
        if (SKIP_PATH_SUBSTRINGS.some((sub) => rel.includes(sub))) return false;
        if (SKIP_FILE_NAMES.has(path.basename(file.path).toLowerCase()))
          return false;
        return true;
      });
  ...
  const sampleCount = fictionFileRefs.filter((f) =>
  ...
  return {
    ...
    score: checked
      ? Math.round(((checked - issues.length) / checked) * 100)
      : null,
    issues,
