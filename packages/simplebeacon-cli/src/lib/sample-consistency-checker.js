...
function detectStaleRoadmapTemplate(payload) {
  ...
  const overview =
    payload.projectOverview || payload.executiveSummary || payload.roadmap?.executiveSummary;
...
function deepIncludesFiction(value, baseline, depth = 0, keyPath = '') {
  ...
  if (typeof value === 'object') {
    for (const [key, nested] of Object.entries(value)) {
      ...
      if (
        (key === 'aiConfidence' || key === 'previousConfidence' || key === 'confidence') &&
...
function driftIssuesFromExtractions(extractions, baseline) {
  ...
  const releaseValues = extractions.map((e) => e.kpis.currentRelease).filter(Boolean);
...
async function checkSampleConsistency(baseDir, options = {}) {
  const scope =
    options.fictionScope === 'sample-paths-only' ? 'sample-paths-only' : 'repository-json';
  ...
  if (Array.isArray(options.scanPathFiles)) {
    const extras = options.scanPathFiles
      ...
      .map((file) => ({
        path: file.path,
        name: file.name,
        relativePath: file.relativePath || path.relative(baseDir, file.path).replace(/\\/g, '/'),
        size: file.size,
      }))
      .filter((file) => {
