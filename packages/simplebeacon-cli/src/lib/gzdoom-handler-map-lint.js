...
const MODEL_CLASS_RE = /\bclass\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?::\s*([A-Za-z_][A-Za-z0-9_.]*))?/g;
const HANDLER_CLASS_LITERAL_RE = /GetClassName\(\)\s*==\s*"([A-Za-z_][A-Za-z0-9_]*)"|"([A-Za-z_][A-Za-z0-9_]*)"\s*==\s*\w+\.GetClassName\(\)/g;
...
function monsterTokenFromClass(className) {
    ...
    const m = name.match(/(Revenant|Imp|Arachnotron|Baron|Cacodemon|Cyberdemon|Mancubus|PainElemental|Archvile|Zombieman|ShotgunGuy|ChaingunGuy|Demon|Spectre|Lost|Soul|HellKnight)/i);
    return m ? m[1] : name;
}
...
function collectHandlerReferences(handlersRoot, ignoreGlobs = []) {
    ...
    function walk(dir, depth) {
        ...
        for (const entry of entries) {
            ...
            const tokenMatches = content.match(/\b(Revenant|Imp|Arachnotron|Baron|Cacodemon|Cyberdemon|Mancubus|PainElemental|Archvile|Zombieman|ShotgunGuy|ChaingunGuy|Demon|Spectre|LostSoul|HellKnight)\b/gi);
            if (tokenMatches) {
...
function hasHandlerCoverage(entry, handlerRefs) {
    ...
    const token = (entry.token || monsterTokenFromClass(entry.className || entry.actor)).toLowerCase();
...
async function lintGzdoomHandlerMap(modPath, options = {}) {
    ...
    const companionRoots = roots.filter((r) => path.resolve(r) !== path.resolve(modPath));
    ...
    for (const root of companionRoots.length ? companionRoots : roots) {
        ...
        for (const [k, v] of collectModelActors(root, ignoreGlobs)) modelActors.set(k, v);
        for (const [k, v] of collectMonsterClasses(monsterRoot, ignoreGlobs, 250)) monsterClasses.set(k, v);
    }
    ...
    for (const entry of monsterClasses.values()) {
        ...
        issues.push({
            ...
            recommendedAction: 'Add a death/light handler for this monster family or confirm generic R3DR00MDeathHandler covers it',
    ...
    for (const entry of modelActors.values()) {
        ...
        if (hasHandlerCoverage({ className: entry.actor, token }, handlerRefs)) continue;
        if (!MONSTER_MODEL_CLASS_RE.test(entry.actor) && !/(Revenant|Imp|Arach|Baron|Caco|Cyber|Manc|Pain|Arch|Zomb|Shotg|Chaing|Demon|Spectre|Lost|Hell)/i.test(entry.actor)) {
            continue;
        }
        issues.push({
            ...
            recommendedAction: 'Verify death lighting covers this MODELDEF actor or add a handler mapping',
            affectedFiles: [entry.filePath],
            metadata: { engine: 'gzdoom-handler-map-lint', actor: entry.actor, monsterToken: token }
        });
    }
