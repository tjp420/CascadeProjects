    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((paramMatch = PARAM_DEFAULT_RE.exec(params)) !== null) {
            issues.push({
                ...
                recommendedAction: 'Remove the default value and assign inside the function body, or use overloads',
                affectedFiles: [filePath],
                metadata: { engine: 'gzdoom-zscript-lint', function: head[1], parameter: paramMatch[1] }
            });
        }
    }
    return issues;
...
async function lintGzdoomZscript(modPath, options = {}) {
    ...
    let scanFiles = allFiles.filter((f) => f.kind === 'source' && /\.(zs|zscript)$/i.test(f.relativePath));
