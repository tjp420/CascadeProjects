...
function classifyDirectory(entry, policy) {
  ...
  if (pathMatchesProtected(entryPath, policy.protectedPaths))
    return "protected";
  if (isSimplebeaconCachePath(entryPath) && !policy.allowSimplebeaconCache)
    return "review";
...
function buildAgentPrompt({
  ...
  return [
    ...
    "",
    "Deletion policy:",
    "- Safe to delete now: regenerable artifacts only (" +
      formatCount(estimatedReduction.files) +
      " files, " +
...
function buildAgentInstructions(context) {
  return [
    ...
    "Never delete paths under protected list: " +
      context.policy.protectedPaths.join(", ") +
      ".",
