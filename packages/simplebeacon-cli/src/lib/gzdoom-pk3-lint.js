...
const STALE_SUFFIX_RE =
  /(?:_(?:DUPLICATE|OLD|backup)|\.(?:corrupt|clean|bak|old|tmp))(?:\.(?:zs|zscript))?$/i;
...
async function lintGzdoomPk3(modPath, options = {}) {
  ...
  const issues = [];
  const requiredLumps =
    Array.isArray(opts.requiredLumps) && opts.requiredLumps.length
      ? opts.requiredLumps
  ...
  for (const rel of [...new Set(staleHits)].slice(0, 50)) {
    const inBuild =
      rel.startsWith("build_temp/") || rel.startsWith("build_temp\\");
    issues.push({
      ...
      recommendedAction:
        "Delete or exclude *_DUPLICATE, *_OLD, .corrupt, .clean files from build_repack.bat",
      affectedFiles: [rel],
  ...
  const reachableZs = [...reachable].filter((r) =>
    r.toLowerCase().endsWith(".zs"),
  );
  const missingFromRoot = reachableZs.filter(
    (r) => !fs.existsSync(path.join(root, r.replace(/\//g, path.sep))),
  );
  ...
  for (const rel of missingFromRoot.slice(0, 20)) {
    issues.push({
      ...
      recommendedAction:
        "Restore the file or remove it from the ZSCRIPT manifest chain",
      affectedFiles: [rel],
