...
function sanitizeBenchmarkPriorityActions(actions = [], dataQuality = {}) {
  ...
  return actions.map((action) => {
    if (!/missing environment keys/i.test(String(action.title || ''))) return action;
    return {
      priority: 'low',
...
function sanitizeCleanupBriefExport(brief, options = {}) {
  ...
  next = {
    ...
    policy: {
      ...
      productProtectedPathsAdvisory: priorPolicy.protectedPaths || PRODUCT_PROTECTED_PATHS,
      policyScopeNote:
    ...
    agentPrompt: buildBenchmarkAgentPrompt({
      ...
      inventory: {
        ...(next.inventory || {}),
        ...(auditFiles != null && (next.inventory?.totalFiles ?? 0) > auditFiles
          ? { auditRepositoryFiles: auditFiles, explorerInventoryRaw: next.inventory.totalFiles }
          : {}),
      },
    }),
