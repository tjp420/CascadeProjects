...
/**
 * Sanitize cleanup-brief JSON exports — benchmark clones, compact duplicate groups.
 */
...
function resolveProjectedInventoryNote(next, auditFiles) {
  const safeFiles =
    next.estimatedReduction?.files ?? next.tiers?.safeNow?.files ?? 0;
  const projectedTotal = next.projectedInventory?.totalFiles;
  if (
    auditFiles != null &&
    (next.inventory?.totalFiles ?? 0) > auditFiles * 2
  ) {
...
/**
 * Normalize asset-duplicate group from full or compact file-reduction findings.
 * @param {object} group
 * @returns {object|null} null when group has no actionable detail
 */
function normalizeDuplicateGroupForBrief(group) {
...
function pruneDuplicateAssetsForExport(groups = [], brief) {
  ...
  const withPaths = normalized.filter(
    (g) => g.duplicates?.length > 0 && g.keeper,
  );
  if (withPaths.length) {
    return {
...
function normalizeBriefNoteKey(note) {
  const text = String(note || "").trim();
  if (/OSS clone under github-cache/i.test(text)) return "oss-clone-disclaimer";
  return text;
}
...
function replaceMisleadingBriefNotes(notes = [], benchmarkScan) {
  const filtered = notes.filter((note) => {
    ...
    return (
      ...
      !/OSS clone under github-cache/i.test(String(note))
    );
  });
  if (benchmarkScan) {
    filtered.unshift(
  ...
  return filtered
    .filter((note) => {
      const key = normalizeBriefNoteKey(note);
...
function sanitizeBenchmarkPriorityActions(actions = [], dataQuality = {}) {
  const missing = dataQuality.missingEnvKeys ?? 0;
  if (!missing) return actions;
  return actions.map((action) => {
    if (!/missing environment keys/i.test(String(action.title || "")))
      return action;
    return {
      priority: "low",
      title: "Optional CLI environment keys",
      detail: `${missing} optional runtime key(s) (SIMPLEBEACON_*, OPENAI_*) — document in .env.example; not required for offline MCP scan.`,
    };
  });
}

function sanitizeBenchmarkScanAnalysis(scanAnalysis = {}) {
  if (!scanAnalysis || typeof scanAnalysis !== "object") return scanAnalysis;
  const dataQuality = { ...(scanAnalysis.dataQuality || {}) };
  const priorityActions = sanitizeBenchmarkPriorityActions(
    scanAnalysis.priorityActions || [],
    dataQuality,
  );
  return {
    ...scanAnalysis,
    ...
    dataQualityNote:
      dataQuality.missingEnvKeys > 0
        ? "Missing env keys on OSS clone are optional CLI/MCP runtime vars — not platform handoff blockers."
        : undefined,
  };
}
...
function buildBenchmarkAgentPrompt(brief) {
  ...
  const displayFiles = inventory.auditRepositoryFiles ?? inventory.totalFiles;
  const inventoryLine =
    inventory.explorerInventoryRaw && inventory.auditRepositoryFiles
      ? `Inventory: ${formatCount(displayFiles)} audit-scoped files (${formatCount(inventory.explorerInventoryRaw)} explorer-style count includes un-walked shells)`
      : `Inventory: ${formatCount(displayFiles)} files / ${formatCount(inventory.totalFolders)} folders`;
  return [
...
function resolveArtifactProfileForExport(
  ...
  const hasFollowUp =
    investigate > 0 || dupBytes > 0 || (tiers.reviewFirst?.files ?? 0) > 0;
...
function resolveCleanupStatus(brief) {
  const safeFiles =
    brief.estimatedReduction?.files ?? brief.tiers?.safeNow?.files ?? 0;
...
function resolveRedactedBriefProjectPath(brief, options = {}) {
  const rawPath =
    options.projectPath ||
    brief.projectPath ||
    brief.scanAnalysis?.projectPath ||
    "";
...
function buildCleanupBriefHygieneSummary(brief, options = {}) {
  ...
  return {
    ...
    phase1SafeDeleteBytes:
      brief.estimatedReduction?.bytes ?? brief.tiers?.safeNow?.bytes ?? 0,
...
function buildProductExportNotes(brief, options = {}) {
  ...
  const dqOpen = countDataQualityOpenFindings(
    brief.scanAnalysis?.dataQuality || {},
  );
  ...
  if (cleanupStatus === "safe-delete-available" && safeFiles > 0) {
    notes.push(
      "Phase 1 safe-to-delete: " +
        formatCount(safeFiles) +
        " files (" +
        formatBytes(safeBytes) +
        ") in regenerable artifact directories — restore with npm install / rebuild after delete.",
    );
  } else if (
  ...
  if (
    brief.estimatedReduction?.phase2DuplicateBytes > 0 &&
    brief.duplicateAssets?.length
  ) {
    notes.push(
...
function enrichProductAgentPrompt(brief) {
  ...
  const lines = [
    ...
    "",
    "Deletion policy:",
    "- Safe to delete now (phase 1): " +
      formatCount(safeFiles) +
      " files, " +
      formatBytes(estimatedReduction.bytes) +
      ".",
  ...
  if (investigate > 0) {
    lines.push(
      "- Investigate only (not auto-delete): " +
        formatCount(investigate) +
        " unused-file candidates — static analysis",
    );
  }
  if (dupBytes > 0) {
    lines.push(
      `- Phase 2 optional: consolidate ~${formatCount(dupFiles)} duplicate file(s), ${formatBytes(dupBytes)} — paths listed in duplicateAssets`,
    );
  }
  lines.push(
    "",
...
function enrichProductAgentInstructions(brief) {
  ...
  const lines = [
    ...
    "Never delete paths under protected list: " +
      protectedPaths.join(", ") +
      ".",
  ...
  if (investigate > 0) {
    lines.push(
      investigate +
        " unused-file candidates are investigate-only — verify imports before any deletion.",
    );
  }
  if (dupBytes > 0) {
    lines.push(
 ...
 * @returns {object}
 */
function sanitizeCleanupBriefExport(brief, options = {}) {
  ...
  if (next.scanAnalysis) {
    ...
    if (benchmarkScan) {
      scanAnalysis = sanitizeBenchmarkScanAnalysis(scanAnalysis);
    }
    next.scanAnalysis = scanAnalysis;
  }
  ...
  if (!benchmarkScan) {
    ...
    return {
      ...
      agentBriefReady: Boolean(
        next.sourceScans?.fileReductionPresent ||
        next.sourceScans?.dataQualityPresent,
      ),
      securityHandoffEligible: false,
  ...
  const benchmarkDataQualityActions = sanitizeBenchmarkPriorityActions(
    brief.dataQualityActions || next.scanAnalysis?.priorityActions || [],
    next.scanAnalysis?.dataQuality || {},
  );
  next = {
    ...next,
    ...
    exportNormalized: true,
    exportSanitized: true,
    cleanupStatus: resolveCleanupStatus({ ...next, benchmarkScan: true }),
    agentBriefReady: Boolean(
      next.sourceScans?.fileReductionPresent ||
      next.sourceScans?.dataQualityPresent,
    ),
    securityHandoffEligible: false,
    dataQualityActions: benchmarkDataQualityActions,
    productPlatformRoot: redactedPlatformRoot || undefined,
    policy: {
      ...priorPolicy,
      protectedPaths: BENCHMARK_PROTECTED_PATHS,
      productProtectedPathsAdvisory:
        priorPolicy.protectedPaths || PRODUCT_PROTECTED_PATHS,
      policyScopeNote:
    ...
    inventory: {
      ...
      ...(auditFiles != null && (next.inventory?.totalFiles ?? 0) > auditFiles
        ? {
            auditRepositoryFiles: auditFiles,
            explorerInventoryRaw: next.inventory.totalFiles,
            inventoryNote: `Explorer-style inventory (${Number(next.inventory.totalFiles).toLocaleString()} files) — gate audit profile counted ${Number(auditFiles).toLocaleString()} files on this clone.`,
          }
        : {}),
    },
    scanAnalysis: next.scanAnalysis
