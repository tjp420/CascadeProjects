...
function sanitizeBenchmarkScanAnalysis(scanAnalysis = {}) {
    ...
    return {
        ...
        dataQuality,
        priorityActions,
        dataQualityNote: dataQuality.missingEnvKeys > 0
...
function buildBenchmarkAgentPrompt(brief) {
    ...
    const inventoryLine = inventory.explorerInventoryRaw && inventory.auditRepositoryFiles
...
function sanitizeCleanupBriefExport(brief, options = {}) {
    ...
    const auditFiles = options.repositoryFilesTotal
        ?? options.auditRepositoryFiles
        ?? null;
