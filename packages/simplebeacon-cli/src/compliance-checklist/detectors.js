...
function detectNpmAuditSummary(projectRoot) {
  ...
  try {
    const auditRunnerPath = path.join(
      root,
      "server",
      "lib",
      "npm-audit-runner.js",
  ...
  const naturalVer =
    ...
    String(
      pkg?.dependencies?.natural || pkg?.devDependencies?.natural || "",
    ).replace(/^[\^~>=<]+/, "");
...
function isPlaceholderSecret(value) {
  ...
  return /^(REPLACE_ON_HOST|changeme|your-secret|replace-me|example|placeholder)/i.test(
    normalized,
  );
}
...
function detectProductionAuthProfile(projectRoot) {
  ...
  const env = fromFile
    ? parseEnvMap(fs.readFileSync(envPath, "utf8"))
    : process.env;
  ...
  const hasRefreshSecret = Object.prototype.hasOwnProperty.call(
    env,
    "JWT_REFRESH_SECRET",
  );
