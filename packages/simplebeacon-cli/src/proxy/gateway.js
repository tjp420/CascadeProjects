...
const MAX_BODY_BYTES = Number(
  process.env.SIMPLEBEACON_PROXY_MAX_BODY || 8 * constants.BYTES_PER_KB * 1024,
);
const BLOCK_MESSAGE = {
  error:
    "Security Block: Sensitive corporate pattern or token leak detected locally.",
};
...
function resolveUpstream(req) {
  ...
  const headerHost =
    req.headers["x-simplebeacon-upstream"] ||
...
function forwardRequest(req, res, upstream, bodyBuffer, projectRoot) {
  ...
  const proxyReq = transport.request(options, (proxyRes) => {
    ...
    readResponseBody(proxyRes)
      .then((responseBuffer) => {
        ...
        if (
          ...
          if (enforced.modified) {
            ...
            logFindings(projectRoot, "inbound", enforced.violations, {
              blocked: false,
              replaced: true,
            });
          }
        ...
        responseHeaders["X-Simplebeacon-Proxy"] = modified
          ? "inbound-replaced"
          : "pass";
      ...
      .catch((error) => {
        sendJson(res, 502, {
          error: `Simplebeacon Proxy upstream read failed: ${error.message}`,
        });
      });
  });
  ...
  proxyReq.on("error", (error) => {
    sendJson(res, 502, {
      error: `Simplebeacon Proxy upstream error: ${error.message}`,
    });
  });
...
async function handleRequest(req, res, options = {}) {
  ...
  if (!upstream) {
    sendJson(res, 502, {
      error:
        "No upstream configured. Set SIMPLEBEACON_PROXY_UPSTREAM or send x-simplebeacon-upstream header.",
    });
    return;
  }
...
function createGateway(options = {}) {
  const port = Number(
    options.port || process.env.SIMPLEBEACON_PROXY_PORT || DEFAULT_PORT,
  );
  const projectRoot = options.projectRoot || process.cwd();
  ...
  const server = http.createServer((req, res) => {
    handleRequest(req, res, { port, projectRoot }).catch((error) => {
      sendJson(res, 500, {
        error: `Simplebeacon Proxy error: ${error.message}`,
      });
    });
  });
  ...
  startGateway()
    .then(({ url, projectRoot }) => {
      ...
      process.stdout.write(
        `Violations log: ${require("./violation-log").resolveLogPath(projectRoot)}\n`,
      );
    })
    .catch((error) => {
