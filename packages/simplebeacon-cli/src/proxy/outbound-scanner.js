...
function scanOutboundText(text, options = {}) {
  ...
  const credentialFindings = scanTextContent(
    "outbound-prompt.txt",
    text,
  ...
  const blocked =
    findings.some((issue) => {
      ...
      return (
        band === "critical" || band === "high" || issue.severity === "high"
      );
    }) ||
    (options.blockMedium === true &&
      findings.some((i) => (i.severityBand || i.severity) === "medium"));
