...
function hasRejectedFictionEntries(rejectedFiction = {}) {
  return Object.values(rejectedFiction).some(
...
function extractAssistantText(responseBody) {
  ...
  try {
    ...
    console.warn(
      "[inbound-enforcer] Response is not valid JSON, returning raw body",
    );
  }
...
function buildSafeFallback(originalBody, violations) {
  const summary = violations
    ...
    .map((v) => {
      if (v.kind === "schema")
        return `Schema drift (${v.fileName}): ${v.messages.join("; ")}`;
  ...
  const safeContent = [
    ...
    summary
      ? `Findings: ${summary}`
      : "Findings: fiction KPI or schema consistency mismatch.",
  ...
  try {
    ...
    console.warn(
      "[inbound-enforcer] Failed to parse originalBody for fallback",
    );
  }
