module.exports = function evaluateEuAiActHumanOversight(rule, { report }) {
    ...
    if (highRisk === 0) {
        return { id: rule.id, title: rule.title, category: rule.category, severity: rule.severity, remediation: rule.remediation || null, status: 'pass', evidence: 'No high-risk AI patterns — human oversight rule not applicable' };
    }
    const ok = gaps === 0;
    return {
        id: rule.id,
