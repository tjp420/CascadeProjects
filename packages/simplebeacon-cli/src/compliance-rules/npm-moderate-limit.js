module.exports = function evaluateNpmModerateLimit(rule, { npmAudit }) {
    if (!npmAudit?.summary) {
        return { id: rule.id, title: rule.title, category: rule.category, severity: rule.severity, remediation: rule.remediation || null, status: 'skip', evidence: 'No package.json — npm audit not applicable' };
    }
