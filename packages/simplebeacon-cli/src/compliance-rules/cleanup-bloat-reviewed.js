module.exports = function evaluateCleanupBloatReviewed(rule, { dataCleanup }) {
    ...
    const reclaimableBytes = dataCleanup?.summary?.reclaimableBytes
        ...
        ?? 0;
    if (bloatCount === 0) {
        return { id: rule.id, title: rule.title, category: rule.category, severity: rule.severity, remediation: rule.remediation || null, status: 'pass', evidence: 'No directory bloat detected — codebase is clean' };
    }
