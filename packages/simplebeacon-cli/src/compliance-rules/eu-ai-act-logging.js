module.exports = function evaluateEuAiActLogging(rule, { report }) {
    ...
    if (aiHits === 0) {
        return { id: rule.id, title: rule.title, category: rule.category, severity: rule.severity, remediation: rule.remediation || null, status: 'pass', evidence: 'No AI decision paths detected' };
    }
    const ok = gaps === 0;
    return {
        id: rule.id,
