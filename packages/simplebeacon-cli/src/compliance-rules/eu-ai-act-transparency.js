module.exports = function evaluateEuAiActTransparency(rule, { report }) {
    ...
    if (aiHits === 0 && gaps === 0) {
        return { id: rule.id, title: rule.title, category: rule.category, severity: rule.severity, remediation: rule.remediation || null, status: 'pass', evidence: 'No generative AI integrations detected in user-facing paths' };
    }
    const ok = gaps === 0;
    return {
        id: rule.id,
