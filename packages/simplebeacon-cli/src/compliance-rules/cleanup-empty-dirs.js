module.exports = function evaluateCleanupEmptyDirs(rule, { dataCleanup }) {
    ...
    if (emptyDirs.length === 0 && bloatCount === 0) {
        return { id: rule.id, title: rule.title, category: rule.category, severity: rule.severity, remediation: rule.remediation || null, status: 'pass', evidence: 'No empty directories detected' };
    }
