module.exports = function evaluateConsistencyPass(rule, { report }) {
    ...
    const passed = report.consistencyPassed === true ? checked : (report.consistencyPassed ?? 0);
    const score = report.consistencyScore ?? (checked ? Math.round((passed / checked) * 100) : 0);
    const ok = score >= 95;
    return {
        id: rule.id,
        title: rule.title,
