...
} = require("../../../server/lib/assessment-retention");

test("redactSecretsInString masks common secret patterns", () => {
  const input =
    "found AKIAIOSFODNN7EXAMPLE and sk-abcdefghijklmnopqrstuvwxyz1234567890";
...
test("purgeExpiredAssessments removes old assessment directories", async () => {
  ...
  await fsp.writeFile(
    path.join(oldDir, "assessment.json"),
    `${JSON.stringify({
      metadata: {
  ...
  const result = await purgeExpiredAssessments(root, {
