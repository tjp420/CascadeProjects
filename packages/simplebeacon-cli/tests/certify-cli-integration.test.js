...
test("--certify produces a .sbcert file alongside the JSON report", async () => {
  ...
  try {
    ...
    assert.equal(
      result.exitCode,
      0,
      `CLI should exit 0, got ${result.exitCode}. stderr: ${result.stderr}`,
    );
...
test("--certify is skipped in offline mode with a warning", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      ".sbcert file should NOT exist in offline mode",
    );
...
test("--certify is skipped for non-JSON output with a warning", async () => {
  ...
  try {
    ...
    assert.ok(
      !fs.existsSync(certPath),
      ".sbcert file should NOT exist for text output",
    );
...
test("--certify failure is non-blocking (scan still succeeds)", async () => {
  ...
  try {
    ...
    assert.equal(
      result.exitCode,
      0,
      "scan should succeed even if certify fails",
    );
    ...
    assert.ok(
      !fs.existsSync(certPath),
      ".sbcert should not exist when certify fails",
    );
