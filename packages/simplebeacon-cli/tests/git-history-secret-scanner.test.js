...
const {
  ...
  isAllowlisted,
  secretPreview,
} = require("../src/rules/git-history-secret-scanner");
...
test("parseGitLogOutput deduplicates same secret in same commit", () => {
  ...
  const awsFindings = result.issues.filter(
    (i) => i.metadata.patternId === "aws-access-key",
  );
  assert.equal(
    awsFindings.length,
    1,
    "should deduplicate same secret in same commit",
  );
});
...
test("scanGitHistorySecrets detects secret in real git repo", async () => {
  ...
  try {
    ...
    execFileSync("git", ["commit", "-m", "add config"], {
      cwd: dir,
      windowsHide: true,
      stdio: "pipe",
    });
    ...
    execFileSync("git", ["commit", "-m", "remove secret"], {
      cwd: dir,
      windowsHide: true,
      stdio: "pipe",
    });
