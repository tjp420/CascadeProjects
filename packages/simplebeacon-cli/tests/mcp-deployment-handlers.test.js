...
function makeTempProject() {
    ...
    fs.writeFileSync(path.join(root, 'api-server', 'package.json'), JSON.stringify({
        name: 'test-api',
        version: '1.0.0',
        scripts: { start: 'node server.cjs' }
    }, null, 2));
    fs.writeFileSync(path.join(root, 'api-server', 'server.cjs'), "const port = process.env.PORT || 3000;\n");
    fs.writeFileSync(path.join(root, 'api-server', 'render.yaml'), [
        'services:',
        '  - type: web',
        '    name: test-api',
...
test('scan_deployment_readiness is registered in TOOL_DEFINITIONS', () => {
    const tool = TOOL_DEFINITIONS.find(t => t.name === 'scan_deployment_readiness');
...
test('scan_deployment_readiness returns ready=true for clean project', async () => {
    ...
    try {
        ...
        const result = await handlers.scan_deployment_readiness({ projectRoot: root });
...
test('scan_deployment_readiness returns ready=false when env var missing', async () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(root, 'api-server', 'server.cjs'),
...
test('scan_deployment_readiness returns structured payload', async () => {
    ...
    try {
        ...
        assert.equal(payload.methodology, 'Deterministic topology scan — no code uploaded');
        handlers.dispose();
    } finally {
        fs.rmSync(root, { recursive: true, force: true });
    }
