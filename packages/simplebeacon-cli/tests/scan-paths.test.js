...
test("resolveEffectiveScanPaths honors local config for isolated honey-pot repos", () => {
  const paths = resolveEffectiveScanPaths(
    "/tmp/client-repo",
    "/tmp/client-repo",
    {
      scanPaths: ["web/data", "data/mock"],
...
test("resolveEffectiveScanPaths uses configured paths for platform root", () => {
  ...
  assert.ok(
    paths.some((p) => p.endsWith("web\\data") || p.endsWith("web/data")),
  );
  assert.ok(
    paths.some((p) => p.endsWith("data\\mock") || p.endsWith("data/mock")),
  );
});
...
test("computeFilesAnalyzed uses the broadest rule coverage", () => {
  ...
  assert.equal(
    computeFilesAnalyzed(10, { scanned: 8 }, { scanned: 0 }, { scanned: 0 }),
    10,
  );
});
