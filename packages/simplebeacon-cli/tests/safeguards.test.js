...
test("hook install dry-run does not create files", () => {
  ...
  const result = installSimplebeaconHook(testDir, {
    type: "pre-commit",
    dryRun: true,
  });
  ...
  assert.equal(result.dryRun, true);
  assert.equal(
    fs.existsSync(path.join(testDir, ".simplebeacon", "hooks", "pre-commit")),
    false,
  );
...
test("init force overwrite creates backup before replacing config", () => {
  ...
  const backups = fs
    .readdirSync(simplebeaconDir)
    .filter((name) => name.includes(".simplebeacon-backup."));
