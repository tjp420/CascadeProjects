...
function makeTempProject() {
  ...
  fs.writeFileSync(
    path.join(root, "src", "sample-data.json"),
    '{"mock": true}\n',
    "utf8",
  );
  return root;
...
test("runScan detects mock data patterns in sample files", async () => {
  ...
  fs.writeFileSync(
    path.join(tmpDir, "src", "sample-data.json"),
    '{"mock": true}',
    "utf8",
  );
  fs.writeFileSync(
    path.join(tmpDir, "src", "fixture.js"),
    "module.exports = {};",
    "utf8",
  );
...
test("runScan produces deterministic qualityScore range", async () => {
  ...
  try {
    ...
    assert.ok(report.qualityScore >= 0 && report.qualityScore <= 100);
    assert.ok(
      Array.isArray(report.rawIssues) || Array.isArray(report.detectedIssues),
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
