...
test("buildEnhanceContext uses rawIssues and assessment executive summary only", () => {
  ...
  assert.equal(
    context.deterministicHeadline,
    assessment.executiveSummary.headline,
  );
...
test("parseEnhanceResponse extracts intro, businessImpact, and headline", () => {
  const parsed = parseEnhanceResponse(
    JSON.stringify({
      intro:
        "The repository has blocking security issues before client handoff.",
  ...
  const enhanced = await enhanceExecutiveSummary(
    ...
    assessment,
    {
      ...
      callAI: async () =>
        JSON.stringify({
          intro:
            "Before go-live, this codebase exposes credentials and ships mock analytics data in production paths.",
          businessImpact:
            "Agency reputation and client trust are at risk if these issues reach production.",
          headline:
            "Remove hardcoded AWS credentials and mock JSON imports before client handoff.",
        }),
    },
  );
...
test("replaceExecutiveSummaryBody swaps narrative while keeping section boundaries", () => {
  ...
  const updated = replaceExecutiveSummaryBody(
    markdown,
