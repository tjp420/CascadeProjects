...
describe('gzdoom cvar lint', () => {
    ...
    it('does NOT flag MENUDEF Command entries as undefined CVARs', () => {
        ...
        const refs = parseMenuReferences('Command "Day Sky", "sky_day"\nCommand "Fix Music", "music_fix"', 'MENUDEF_R3D');
        assert.equal(refs.length, 0, 'Command entries should not produce CVAR references');
    });
    ...
    it('accepts GZDoom built-in engine CVARs without CVARINFO definitions', async () => {
        const tmp = require('fs').mkdtempSync(require('path').join(require('os').tmpdir(), 'sb-cvar3-'));
        const fs = require('fs');
        const path = require('path');
        // MENUDEF references r_models and r_drawvoxels (engine built-ins)
        // but CVARINFO doesn't define them — they should NOT be flagged.
        fs.writeFileSync(path.join(tmp, 'CVARINFO'), 'user int r3d_custom = 0;\n', 'utf8');
        fs.writeFileSync(path.join(tmp, 'MENUDEF_R3D'),
            'Option "Models", "r_models", "OnOff"\n' +
            'Option "Voxels", "r_drawvoxels", "OnOff"\n' +
            'Option "Dynamic Lights", "r_dynamiclights", "OnOff"\n',
            'utf8');
        const result = await lintGzdoomCvars(tmp, { cvarPrefix: 'r3d_' });
        const undefinedIssues = result.issues.filter((i) => i.type === 'gzdoom-cvar-undefined');
        assert.ok(!undefinedIssues.some((i) => /r_models/.test(i.description)),
            'r_models is a built-in engine CVAR and should not be flagged');
        assert.ok(!undefinedIssues.some((i) => /r_drawvoxels/.test(i.description)),
            'r_drawvoxels is a built-in engine CVAR and should not be flagged');
        assert.ok(!undefinedIssues.some((i) => /r_dynamiclights/.test(i.description)),
