...
test("scanTextPatterns ignores fence-detector regex definitions", () => {
  ...
  assert.equal(
    scanTextPatterns(
      "packages/simplebeacon-cli/src/rules/llm-slop-patterns.js",
      ruleLine,
      ".js",
    ).length,
    0,
  ...
  assert.equal(
    scanTextPatterns(
      "packages/simplebeacon-cli/src/proxy/inbound-enforcer.js",
      parserLine,
      ".js",
    ).length,
    0,
...
test("scanLlmSlopPatterns walks repo and finds placeholder in source", async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { name: "demo", dependencies: { "mock-api-client": "0.0.1" } },
      null,
      2,
    ),
  );
...
test('scanTextPatterns flags "per your request" in FIXME', () => {
  const content =
    "// FIXME: per your request, this timeout should be configurable\n";
  const hits = scanTextPatterns("src/config.js", content, ".js");
  assert.ok(hits.some((h) => h.pattern === "SB-FICTION-006"));
});
...
test('scanTextPatterns flags boilerplate "This component renders" comment', () => {
  const content =
    "// This component renders the user profile card\nconst Profile = () => {};\n";
