...
test('scanTextPatterns ignores fence-detector regex definitions', () => {
  ...
  assert.equal(
    scanTextPatterns('packages/simplebeacon-cli/src/rules/llm-slop-patterns.js', ruleLine, '.js')
      .length,
    0
  );
  assert.equal(
    scanTextPatterns('packages/simplebeacon-cli/src/proxy/inbound-enforcer.js', parserLine, '.js')
      .length,
    0
  );
});
