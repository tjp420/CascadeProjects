...
test("walkProjectFiles skips github-cache and node_modules", async () => {
  ...
  try {
    ...
    await fs.promises.mkdir(path.join(root, "github-cache", "clone", "pkg"), {
      recursive: true,
    });
    await fs.promises.writeFile(
      path.join(root, "github-cache", "clone", "pkg", "noise.js"),
      "x\n",
      "utf8",
    );
    await fs.promises.mkdir(path.join(root, "node_modules", "dep"), {
      recursive: true,
    });
    await fs.promises.writeFile(
    ...
    assert.equal(
      rels.some((r) => r.includes("github-cache")),
      false,
    );
    assert.equal(
    ...
    assert.equal(
      directories.some((d) => d.name === "node_modules" && d.skipped),
      true,
    );
  } finally {
