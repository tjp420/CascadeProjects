...
for (let i = 0; i < args.length; i += 1) {
    if (args[i] === '--files' && args[i + 1]) { targetFiles = parseInt(args[i + 1], 10); }
    ...
    function populate(dir, currentDepth) {
        ...
        if (currentDepth >= depth) {
            for (let i = 0; i < filesPerDir && created < totalFiles; i += 1) {
                fs.writeFileSync(path.join(dir, `file_${created}.js`), `// file ${created}\nconst x = ${created};\n`);
                created += 1;
            }
            return;
        }
        ...
        for (let d = 0; d < subDirs; d += 1) {
            ...
            for (let i = 0; i < filesPerDir / subDirs && created < totalFiles; i += 1) {
                fs.writeFileSync(path.join(subDir, `file_${created}.js`), `// file ${created}\nconst x = ${created};\n`);
                created += 1;
            }
...
async function main() {
    ...
    console.log(`Generating synthetic tree: ${targetFiles} files, depth ${maxDepth}...`);
    ...
    const result1 = await walkAllFiles(tmpDir, { skipDirs: new Set(['node_modules', '.git']) });
    ...
    console.log(`walkProjectFiles:      ${walkProjMs}ms  (${result2.files.length} files, ${result2.directories.length} dirs)`);
    ...
    console.log(`countRepositoryInv:    ${invMs}ms  (${result3.totalFiles} files, ${result3.totalFolders} folders)`);
