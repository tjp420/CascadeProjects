...
} = require("../src/lib/production-leak-intent");
const {
  parseJestSummary,
  readJestResultCache,
  checkJestBaseline,
} = require("../src/rules/jest-baseline");
...
test("production-leak ignores dev-tools page-spec metadata descriptions", () => {
  const content =
    "description: `Validates ${label} registered dashboard page-spec JSON files`";
...
test("production-leak ignores mock/sample prose in template literals", () => {
  const content =
    "const msg = `Scope: mock/sample JSON files only — not a path`;";
  const findings = leakFindings(
    "server/services/cloud-inference-service.js",
    content,
  );
  assert.equal(findings.length, 0);
});
...
test("production-leak ignores instructional template-sample wording", () => {
  const content =
    'const msg = `Use the phrase "sample-suffix subset" instead of "template-sample".`;';
  const findings = leakFindings(
    "server/services/cloud-inference-service.js",
    content,
  );
...
test("production-leak suppresses demo tool paths for -sample.json imports", () => {
  ...
  const result = scanFileContent(
    "src/applets/tools/jsonata-tool/jsonata-tool.ts",
    content,
  );
  assert.equal(result.findings.length, 0);
...
test("globMatch supports tsx test file patterns", () => {
  assert.equal(
    globMatch("src/components/voice/index.test.tsx", "**/*.test.tsx"),
    true,
  );
  assert.equal(
    globMatch("src/components/voice/index.tsx", "**/*.test.tsx"),
    false,
  );
});
...
test("credential scanner detects AWS key and allows demo placeholder", () => {
  ...
  const demo = scanTextContent(
    "auth.json",
    '{"password":"demo123","email":"dev@simplebeacon.ai"}',
  );
...
test("formatTextReport renders without color when NO_COLOR set", () => {
  ...
  const text = formatTextReport(
    { projectRoot: "/tmp", totalFiles: 0, qualityScore: 100, rawIssues: [] },
    { pass: true },
  );
...
test("checkJestBaseline validates cached jest-result.json when runTests is false", async () => {
  ...
  const result = await checkJestBaseline(platformRoot, {
    baseline: {
      jestTestsPassing: 991,
      jestTestsLabel: "991/991",
      jestSuites: 72,
    },
    runTests: false,
  });
