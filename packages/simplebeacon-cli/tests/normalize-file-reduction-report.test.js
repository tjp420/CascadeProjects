...
test("isStaleFileReductionScan detects github-cache pollution", () => {
  const report = {
    ...
    allFindings: [
      { path: "github-cache/aws-aws-cli/bin/aws", type: "unused-file" },
    ],
  };
...
test("normalizeFileReductionReport strips benchmark-cache findings from stale export", () => {
  ...
  assert.equal(
    normalized.allFindings.some((finding) =>
      String(finding.path || "").includes("github-cache"),
    ),
    false,
  );
      ...
      unusedFiles: [
        {
          type: "unused-file",
          severity: "medium",
          path: "batch_consolidator.py",
        },
        {
          type: "unused-file",
          severity: "medium",
          path: "standardized_schema.json",
        },
      ],
      assetConsolidation: [],
      configManagement: [],
...
test("globMatch supports **/exact-filename patterns", () => {
  assert.equal(
    globMatch("standardized_schema.json", "**/standardized_schema.json"),
    true,
  );
  assert.equal(
    globMatch(
      "schemas/standardized_schema.json",
      "**/standardized_schema.json",
    ),
    true,
  );
  assert.equal(
    globMatch("batch_consolidator.py", "**/batch_consolidator.py"),
    true,
  );
...
test("parseHtmlReferences resolves dashboard css when html lives under web/", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(root, htmlRel),
      '<link rel="stylesheet" href="/simplebeacon-dashboard/css/theme.css?v=1">',
      "utf8",
    );
    ...
    const refs = parseHtmlReferences(
      fs.readFileSync(path.join(root, htmlRel), "utf8"),
      path.join(root, htmlRel),
      root,
    );
