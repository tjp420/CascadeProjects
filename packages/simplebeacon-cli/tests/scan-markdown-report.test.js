...
const SAMPLE_REPORT = {
    ...
    gate: {
        ...
        blockingIssues: [],
        warningIssues: [
            { severity: 'medium', type: 'LLM Slop', filePath: 'src/placeholder.js', count: 2, fix: 'Replace placeholder' },
            { severity: 'low', type: 'Empty File', filePath: 'src/empty.js', count: 1, fix: 'Remove or populate' }
        ]
    },
    stats: {
        filesAnalyzed: 1200,
    ...
    rawIssues: [
        { pattern: 'SB-FICTION-001', count: 1, severity: 'high', type: 'llm-placeholder', filePath: 'src/config.js' },
        { pattern: 'SB-FICTION-007', count: 1, severity: 'high', type: 'mock-return-value', filePath: 'src/auth.js' }
    ],
    qualityScorecard: {
        accuracy: 100,
        completeness: 100,
    ...
    remediationPhases: [
        { id: 'integrity', title: 'Phase 1: Data Integrity', severity: 'medium', progress: 100, effort: '2-4 days', status: 'completed' },
        { id: 'consistency', title: 'Phase 2: Consistency', severity: 'low', progress: 75, effort: '3-5 days', status: 'in-progress' },
        { id: 'compliance', title: 'Phase 3: Governance', severity: 'medium', progress: 50, effort: '2-3 days', status: 'in-progress' }
    ],
    euAiAct: {
...
test('generateHeader shows BLOCKED when gate fails', () => {
    const report = { ...SAMPLE_REPORT, gate: { pass: false, blockingCount: 2, blockingIssues: [], warningIssues: [] } };
...
test('generateFictionRuleBreakdown shows hit counts for flagged rules', () => {
    const breakdown = generateFictionRuleBreakdown(SAMPLE_REPORT);
    assert.ok(breakdown.includes('| 1 |'), 'Should show count 1 for SB-FICTION-001');
...
test('generateEuAiActSection shows indicators when present', () => {
    const report = {
        ...SAMPLE_REPORT,
        euAiAct: { aiSystemIndicators: 3, highRiskIndicators: 1, transparencyGaps: 2, documentationArtifacts: 1 }
    };
...
test('generateMarkdownReport handles minimal report without crashing', () => {
    const minimal = {
        ...
        gate: { pass: false, blockingCount: 1, blockingIssues: [{ severity: 'high', type: 'Secret', filePath: 'src/secret.js', count: 1, fix: 'Rotate key' }], warningIssues: [] }
    };
...
test('generateMarkdownReport throws on null input', () => {
    assert.throws(() => generateMarkdownReport(null), /Report object is required/);
});
...
test('selectPayload returns markdown for markdown format', () => {
    ...
    const fakeReport = { summary: { qualityScore: 90, totalFiles: 5, totalLines: 50 }, gate: { pass: true, blockingIssues: [], warningIssues: [] } };
    const fakeGateResult = { pass: true, blockingIssues: [], warningIssues: [] };
    const fakeJsonReport = { ...fakeReport, gate: { pass: true, blockingCount: 0, warningCount: 0, blockingIssues: [], warningIssues: [] }, summary: { qualityScore: 90, totalFiles: 5, totalLines: 50 } };
    const payload = selectPayload(fakeReport, fakeGateResult, fakeJsonReport, 'markdown');
