...
const {
  ...
  RULE_CATALOG,
  OWASP_CATEGORIES,
} = require("../src/rules/owasp-llm-patterns");
...
test("RULE_CATALOG has 10 rules covering all OWASP LLM categories", () => {
  ...
  for (let i = 1; i <= 10; i++) {
    assert.ok(
      owaspIds.includes(`LLM${String(i).padStart(2, "0")}:2025`),
      `Missing LLM${String(i).padStart(2, "0")}`,
    );
  }
});
...
test("LLM01: flags prompt injection — user input into prompt", async () => {
  ...
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM01-001"),
  );
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === "OWASP-LLM01-001")
      .severity,
    "high",
  );
...
test("LLM02: flags sensitive info in LLM API calls", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM02-001"),
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test("LLM03: flags unpinned LLM package import", async () => {
  ...
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM03-001"),
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test("LLM04: flags training data from unvalidated URLs", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["src"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM04-001"),
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test("LLM05: flags LLM output into innerHTML", async () => {
  ...
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM05-001"),
  );
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === "OWASP-LLM05-001")
      .severity,
    "high",
  );
  fs.rmSync(dir, { recursive: true, force: true });
...
test("LLM06: flags LLM output driving exec", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM06-001"),
  );
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === "OWASP-LLM06-001")
      .severity,
    "high",
  );
  fs.rmSync(dir, { recursive: true, force: true });
...
test("LLM07: flags secrets in system prompts", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM07-001"),
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test("LLM08: flags embedding from unvalidated user input", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM08-001"),
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test("LLM09: flags LLM output returned without disclaimer", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM09-001"),
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test("LLM10: flags unbounded max_tokens", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "OWASP-LLM10-001"),
  );
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === "OWASP-LLM10-001")
      .severity,
    "high",
  );
  fs.rmSync(dir, { recursive: true, force: true });
...
test("suppression comment prevents finding", async () => {
  ...
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ["server"] });
  const llm05 = result.issues.filter(
    (i) => i.metadata?.patternId === "OWASP-LLM05-001",
  );
