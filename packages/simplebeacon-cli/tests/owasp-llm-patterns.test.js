...
test('LLM01: flags prompt injection — user input into prompt', async () => {
  ...
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === 'OWASP-LLM01-001').severity,
    'high'
  );
...
test('LLM03: flags unpinned LLM package import', async () => {
  ...
  fs.writeFileSync(path.join(dir, 'src', 'index.js'), "const OpenAI = require('openai');");
...
test('LLM05: flags LLM output into innerHTML', async () => {
  ...
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === 'OWASP-LLM05-001').severity,
    'high'
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test('LLM06: flags LLM output driving exec', async () => {
  ...
  fs.mkdirSync(path.join(dir, 'server'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'server', 'agent.js'), 'const result = exec(aiCommand);');
  const result = await scanOwaspLlmPatterns(dir, { sourcePaths: ['server'] });
  assert.ok(result.issues.some((i) => i.metadata?.patternId === 'OWASP-LLM06-001'));
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === 'OWASP-LLM06-001').severity,
    'high'
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
...
test('LLM10: flags unbounded max_tokens', async () => {
  ...
  assert.equal(
    result.issues.find((i) => i.metadata?.patternId === 'OWASP-LLM10-001').severity,
    'high'
  );
  fs.rmSync(dir, { recursive: true, force: true });
});
