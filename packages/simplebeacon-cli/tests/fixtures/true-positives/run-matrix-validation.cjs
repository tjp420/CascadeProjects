...
const { scanTokenBleedPatterns } = require('../../../src/rules/token-bleed-patterns');
...
function makeFixtureProject(fixtureDir, fixtureRelPath) {
    ...
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ name: 'matrix-fixture', version: '1.0.0' }), 'utf8');
...
function findingMatches(actual, expected) {
    ...
    const typeMatch = actualType.includes(expectedType) || expectedType.includes(actualType);
    ...
    const expectedRank = SEVERITY_RANK[String(expected.minSeverity).toLowerCase()] || 0;
...
async function main() {
    ...
    console.log('  Fixtures: ' + fixtures.length + '  |  Engines tested: ' + new Set(fixtures.map(f => f.engineId)).size);
    console.log('========================================================');
    console.log('');
    ...
    for (const fixture of fixtures) {
        ...
        if (!fs.existsSync(fixtureFile)) {
            ...
            results.push({ fixture, status: 'skip', reason: 'file missing', metrics: null });
            allPassed = false;
            continue;
        }

        process.stdout.write('  [' + fixture.engineId + '] ' + fixture.id + ' ... ');
        ...
        try {
            ...
            if (metrics.recall < 1) {
                ...
                console.log('    Actual:   ' + JSON.stringify(actualFindings.map(f => ({ type: f.type, severity: f.severity }))));
                results.push({ fixture, status: 'fail', metrics, actualFindings, unmatched });
                allPassed = false;
            } else {
                console.log('PASS (precision=' + metrics.precision + ', recall=' + metrics.recall + ', F1=' + metrics.f1 + ')');
                if (metrics.falsePositives > 0) {
                    console.log('    ' + metrics.falsePositives + ' additional finding(s) — review for false positives');
                }
    ...
    // ── Suite-level metrics ──────────────────────────────────────────────
    const suitePrecision = (totalTruePositives + totalFalsePositives) > 0
        ? totalTruePositives / (totalTruePositives + totalFalsePositives)
        : 1;
    const suiteRecall = (totalTruePositives + totalFalseNegatives) > 0
        ? totalTruePositives / (totalTruePositives + totalFalseNegatives)
        : 1;
    const suiteF1 = (suitePrecision + suiteRecall) > 0
    ...
    for (const [engine, counts] of Object.entries(engineMap)) {
        ...
        console.log('    ' + engine.padEnd(25) + ' ' + status + '  (' + counts.passes + ' pass, ' + counts.fails + ' fail, ' + counts.errors + ' error)');
    }
    console.log('');
