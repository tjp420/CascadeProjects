...
test("DEFAULT_RULES is the union of STRUCTURAL_RULES and LEGACY_RULES", () => {
  assert.equal(
    ...
    STRUCTURAL_RULES.length + LEGACY_RULES.length,
  );
});
...
test("every rule has a RegExp pattern", () => {
  for (const rule of DEFAULT_RULES) {
    assert.ok(
      ...
      `Rule ${rule.id} missing RegExp pattern`,
    );
  }
});
...
test("token rules have keyType, non-token rules do not", () => {
  for (const rule of STRUCTURAL_RULES) {
    if (rule.keyType) {
      assert.ok(
        rule.id.startsWith("SB-FIX-TOKEN-"),
        `Token rule ${rule.id} should have TOKEN prefix`,
      );
    }
  }
});
...
test("preserves legitimate TODO comments", () => {
  const engine = new RemediationEngine(STRUCTURAL_RULES);
  const input =
    "// TODO: refactor for Q4 performance initiative\nconst x = 1;\n";
  const result = engine.processBuffer(input, "test.js");
...
test("quarantines Stripe live secret key", () => {
  const engine = new RemediationEngine(STRUCTURAL_RULES);
  const input =
    'const stripe = require("stripe")("sk_live_abcdefghijklmnopqrstuvwxyz0123456789");\n';
...
test("quarantines AWS access key ID", () => {
  ...
  assert.ok(
    result.content.includes("process.env.SIMPLEBEACON_QUARANTINE_AWS_KEY_0"),
  );
  assert.equal(result.quarantine.length, 1);
});
...
test("processFile atomic write produces correct content (no corruption)", () => {
  ...
  withTempFile(input, (filePath) => {
    ...
    const after = fs.readFileSync(filePath, "utf8");
    assert.ok(
      after.includes("process.env.SIMPLEBEACON_QUARANTINE_STRIPE_KEY_0"),
    );
...
test("processFiles processes multiple files and returns per-file results", () => {
  ...
  try {
    const results = engine.processFiles([file1, file2, file3], {
      dryRun: true,
    });
...
test("verify throws when file content does not match expected hash", () => {
  ...
  try {
    ...
    assert.throws(
      () => engine.verify(filePath, wrongHash),
      /Verification failed/,
    );
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
...
test("custom token rule with keyType produces quarantine entries", () => {
  ...
  assert.ok(
    result.content.includes("process.env.SIMPLEBEACON_QUARANTINE_CUSTOM_0"),
  );
  assert.equal(result.quarantine.length, 1);
});
...
test("re-processing already remediated content is a no-op", () => {
  ...
  assert.ok(
    !second.changed,
    "Re-processing remediated content should not change",
  );
...
test("processFile on non-existent file throws ENOENT", () => {
  ...
  assert.throws(
    () => engine.processFile(fakePath, { dryRun: true }),
...
test("processBuffer handles a large input without errors", () => {
  ...
  assert.ok(
    outputLines.length > 9000,
    "Output should have roughly the same number of lines",
  );
});
...
test("preserves inline code that is not a fence", () => {
  ...
  assert.ok(
    !result.changed,
    "Inline backticks should not be treated as fences",
  );
