...
test('isExternalBenchmarkCachePath detects github-cache clones', () => {
  assert.equal(isExternalBenchmarkCachePath('github-cache/facebook-react/tsconfig.json'), true);
  assert.equal(isExternalBenchmarkCachePath('java-ai-vulnerable/cycode-sca.html'), true);
...
test('partitionBenchmarkIssues keeps jest baseline issues on platform scope', () => {
  ...
  const { platformIssues, excludedScanNoiseIssues } = partitionBenchmarkIssues([jestIssue]);
  assert.equal(platformIssues.length, 1);
