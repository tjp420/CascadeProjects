...
test("isExternalBenchmarkCachePath detects github-cache clones", () => {
  ...
  assert.equal(isExternalBenchmarkCachePath("server/routes/api.js"), false);
});
...
test("partitionBenchmarkIssues splits platform vs cache noise", () => {
  const { platformIssues, benchmarkCacheIssues, excludedScanNoiseIssues } =
    partitionBenchmarkIssues([
      ...
      {
        filePath: "github-cache/aws-aws-cli/package.json",
        type: "Invalid JSON",
      },
      {
        filePath: ".simplebeacon/credential-incident-triage.json",
        type: "Credential Pattern",
      },
      {
        filePath: "packages/simplebeacon-cli/tests/rules.test.js",
        type: "Credential Pattern",
      },
    ]);
...
test("partitionBenchmarkIssues keeps jest baseline issues on platform scope", () => {
  const jestIssue = {
  ...
  const { platformIssues, excludedScanNoiseIssues } = partitionBenchmarkIssues([
    jestIssue,
  ]);
  assert.equal(platformIssues.length, 1);
  assert.equal(excludedScanNoiseIssues.length, 0);
});
...
test("resolveEffectiveScanPaths uses web/data for ai-platform, not full tree", () => {
  ...
  for (const p of paths) {
    ...
    assert.notEqual(
      key,
      platformKey,
      `should not walk full platform root: ${p}`,
    );
  }
