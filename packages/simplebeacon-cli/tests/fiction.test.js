...
test("deepIncludesFiction detects rejected feature and confidence values", () => {
  ...
  const hits = deepIncludesFiction(
    {
      overview: {
        totalFeatures: REJECTED_FEATURE_COUNT,
...
test("deepIncludesFiction flags totalFeatures 8 in measured samples", () => {
  ...
  assert.ok(
    hits.some((h) =>
      h.includes(`totalFeatures=${REJECTED_SMALL_FEATURE_COUNT}`),
    ),
  );
});
  ...
  const hits = deepIncludesFiction(
    {
      dataSource: "repository-audit",
      overview: {
        tierCount: REJECTED_SMALL_FEATURE_COUNT,
        stubRoutes: REJECTED_SMALL_FEATURE_COUNT,
      },
    },
    baseline,
  );
      ...
      visualComparison: {
        charts: {
          completionRateComparison: {
            data: [
              { label: "GGUF Assessment", value: REJECTED_COMPLETION_RATE },
            ],
          },
        },
      },
...
test("checkSampleConsistency scans all sample files for fiction", async () => {
  ...
  fs.writeFileSync(
    path.join(dataDir, "anchor-sample.json"),
    `${JSON.stringify(
      {
        type: "test-model",
        dataSource: "repository-audit",
  ...
  fs.writeFileSync(
    path.join(dataDir, "other-sample.json"),
    `${JSON.stringify(
      {
        type: "test-model",
  ...
  assert.ok(
    result.issues.some(
      (i) =>
        i.type === "Fictional KPI" && i.filePath === "data/other-sample.json",
    ),
  );
});
...
test("checkSampleConsistency honors ignore globs for repository-json scope", async () => {
  ...
  assert.equal(
    result.issues.some((i) => i.filePath.includes("docs/stale.json")),
    false,
  );
});
