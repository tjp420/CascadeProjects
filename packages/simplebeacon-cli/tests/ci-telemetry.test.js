...
describe('ci-telemetry', () => {
  test('buildCiTelemetryPayload extracts team aggregation metadata', () => {
    const report = {
      ...
      totalFiles: 12,
      qualityScore: 88,
      scanScope: { diffOnly: true, diffFileCount: 4, rulesEnabled: ['gate', 'fiction'] },
      rawIssues: [
  ...
  test('legacy CI fields are omitted unless env flag is set', () => {
    const report = { gate: { pass: true, blockingCount: 0 }, severityCounts: {} };
    const payload = buildCiTelemetryPayload(report, { tier: 'team' }, {
  ...
  test('postTeamTelemetry skips when air-gapped', async () => {
    await withMockFetch(() => { throw new Error('fetch should not run'); }, async () => {
      const result = await postTeamTelemetry(PAID_REPORT, PAID_LICENSE, { airGapped: true });
  ...
  test('postTeamTelemetry skips when offline', async () => {
    await withMockFetch(() => { throw new Error('fetch should not run'); }, async () => {
      const result = await postTeamTelemetry(PAID_REPORT, PAID_LICENSE, { offline: true });
      assert.equal(result.skipped, true);
  ...
  test('postTeamTelemetry skips community tier', async () => {
    await withMockFetch(() => { throw new Error('fetch should not run'); }, async () => {
      const result = await postTeamTelemetry(PAID_REPORT, { paid: false, tier: 'developer' });
      assert.equal(result.skipped, true);
  ...
  test('postTeamTelemetry POSTs team_scan payload for paid tier', async () => {
    await withEnv({ SIMPLEBEACON_LICENSE_TOKEN: 'test-token' }, async () => {
      await withMockFetch(async (url, opts) => {
        ...
        const result = await postTeamTelemetry(PAID_REPORT, PAID_LICENSE, { scanSource: 'ci' });
...
describe('ci-telemetry CLI hook', () => {
  test('air-gapped scan skips telemetry POST', async () => {
    ...
    const token = generateLicenseToken({ email: 'team@test.com', tier: 'team' }, secret, 3600);

    const result = spawnSync(process.execPath, [
      BIN, 'scan', '--path', root, '--format', 'json', '--air-gapped', '--no-trust-banner', '--quiet'
    ], {
      cwd: root,
      encoding: 'utf8',
      env: {
    ...
    try {
      ...
      assert.doesNotMatch(`${result.stdout}\n${result.stderr}`, /Team telemetry recorded/);
    } finally {
  ...
  test('unreachable telemetry endpoint does not fail scan exit code', async () => {
    ...
    const result = spawnSync(process.execPath, [
      BIN, 'scan', '--path', root, '--format', 'json', '--gate', '--no-trust-banner'
    ], {
      cwd: root,
      encoding: 'utf8',
      timeout: 60000,
    ...
    try {
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(`${result.stdout}\n${result.stderr}`, /Warning: Team telemetry endpoint unreachable/);
    } finally {
      await fsp.rm(root, { recursive: true, force: true });
    }
  });
