...
const {
  ...
  extractPatterns,
  generateReport,
} = require("../src/agent-pda/cross-project-learner");
...
function makeFakeProject(root, opts = {}) {
  ...
  if (opts.hasPda) {
    ...
    if (opts.agents) {
      fs.writeFileSync(
        path.join(pdaDir, "agents.json"),
        JSON.stringify(opts.agents),
      );
    }
    if (opts.tasks) {
      fs.writeFileSync(
        path.join(pdaDir, "tasks.json"),
        JSON.stringify(opts.tasks),
      );
    }
    if (opts.memories) {
      fs.writeFileSync(
        path.join(pdaDir, "memories.json"),
        JSON.stringify(opts.memories),
      );
    }
  }
}
...
test("Cross-project learner: detects archived-files-block-gate pattern", () => {
  ...
  try {
    ...
    const pattern = analysis.patterns.find(
      (p) => p.id === "archived-files-block-gate",
    );
...
test("Cross-project learner: detects ai-generated-stub-functions pattern", () => {
  ...
  try {
    ...
    const analysis = extractPatterns(projects);
    const pattern = analysis.patterns.find(
      (p) => p.id === "ai-generated-stub-functions",
    );
...
test("Cross-project learner: recommendations are sorted by priority", () => {
  ...
  try {
    ...
    const highIdx = analysis.recommendations.findIndex(
      (r) => r.priority === "high",
    );
    const medIdx = analysis.recommendations.findIndex(
      (r) => r.priority === "medium",
    );
