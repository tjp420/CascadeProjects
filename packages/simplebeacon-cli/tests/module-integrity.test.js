...
const { runDoctor } = require(
  path.join(ROOT, "packages/simplebeacon-cli/src/doctor.js"),
);
const { signLicense } = require(path.join(ROOT, "sales/license/generator.js"));
const { checkExpiringLicenses } = require(
  path.join(ROOT, "sales/license/renewal-tracker.js"),
);
const { decryptSupportToken } = require(
  path.join(ROOT, "sales/support/decrypt-token.js"),
);
const { evaluateFunnelMetrics } = require(
  path.join(
    ROOT,
    "ai-platform/web/simplebeacon-dashboard/js/utils/funnelTrigger.js",
  ),
);
const { rotateLicenseToken } = require(
  path.join(ROOT, "sales/license/rotate-keys.js"),
);

// License validation is authored in TypeScript; use the compiled JS output if available.
const compiledLicenseManager = path.join(
  ROOT,
  "simplebeacon-vscode-merged/out/licenseManager.js",
);
...
describe("Module Integrity Suite", () => {
  ...
  describe("decrypt-token.js", () => {
    test("should round-trip encrypt and decrypt a support token", () => {
      const cipherKey = crypto.scryptSync(
        "simplebeacon-public-triage-salt",
        "salt",
        32,
      );
      ...
      let encrypted = cipher.update(
        JSON.stringify({ nodeVersion: "24.9.0" }),
        "utf8",
        "hex",
      );
  ...
  describe("rotate-keys.js", () => {
    test("should rotate a valid license token to a new key pair", () => {
      ...
      const oldPriv = oldKeys.privateKey.export({
        type: "pkcs8",
        format: "pem",
      });

      const newKeys = crypto.generateKeyPairSync("rsa", {
        modulusLength: 2048,
      });
      const newPriv = newKeys.privateKey.export({
        type: "pkcs8",
        format: "pem",
      });

      const originalToken = signLicense(
        "rotate-corp",
        "enterprise",
        "2028-12-31",
        oldPriv,
