...
test('findRenderYamlFiles discovers render.yaml files', async () => {
    const repo = await createTempRepo({
        ...
        'packages/cli/package.json': '{"name":"cli","scripts":{"start":"node cli.js"}}',
    });
    try {
...
test('parseWorkspaces expands glob patterns', async () => {
    ...
    try {
        const workspaces = parseWorkspaces(repo.dir).map(w => w.replace(/\\/g, '/'));
...
test('findServiceDirectories discovers dirs with start scripts', async () => {
    const repo = await createTempRepo({
        ...
        'api-server/package.json': JSON.stringify({ name: 'api', scripts: { start: 'node server.cjs' } }),
        'api-server/render.yaml': SAMPLE_RENDER_YAML,
        'lib/package.json': JSON.stringify({ name: 'lib', scripts: { build: 'tsc' } }), // no start script
    });
    try {
...
test('findCreateTableStatements finds CREATE TABLE in SQL files', async () => {
    const repo = await createTempRepo({
        'api-server/migrations/001-init.sql': 'CREATE TABLE users (id UUID PRIMARY KEY);',
        'ai-platform/server/db/schema.sql': 'CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY);',
    });
    try {
...
test('extractEnvVarsFromSource finds process.env references', async () => {
    const repo = await createTempRepo({
        'server.cjs': 'const port = process.env.PORT || 3000; const key = process.env.SECRET_KEY;',
    });
    try {
...
test('checkCorsApproach detects shared-cors-config', async () => {
    const repo = await createTempRepo({
        'api-server/server.cjs': 'const { isAllowedOrigin } = require("./lib/cors-config.cjs");',
...
test('checkCorsApproach detects inline-cors', async () => {
    const repo = await createTempRepo({
        'coming-soon/server.cjs': 'app.use((req,res,next) => { res.setHeader("Access-Control-Allow-Origin", "*"); next(); });',
    });
    try {
...
test('scanDeploymentReadiness detects SB-DEP-003 schema conflict', async () => {
    const repo = await createTempRepo({
        ...
        'api-server/migrations/001.sql': 'CREATE TABLE users (id UUID PRIMARY KEY DEFAULT gen_random_uuid());',
        'ai-platform/package.json': JSON.stringify({ name: 'platform', scripts: { start: 'node server.cjs' } }),
        'ai-platform/server/db/schema.sql': 'CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY);',
    });
    try {
        const result = await scanDeploymentReadiness(repo.dir);
        const schemaConflict = result.issues.find(i => i.pattern === 'SB-DEP-003');
...
test('scanDeploymentReadiness detects SB-DEP-001 workspace membership', async () => {
    ...
    try {
        const result = await scanDeploymentReadiness(repo.dir);
        const workspaceIssue = result.issues.find(i => i.pattern === 'SB-DEP-001' && i.metadata.service === 'ai-platform');
        assert.ok(workspaceIssue, 'Should detect SB-DEP-001 for ai-platform not in workspaces');
    } finally {
        repo.cleanup();
    }
});
  ...
  - type: web
    ...
    envVars:
      - key: NODE_ENV
        value: production
`,
        'api-server/server.cjs': 'const port = process.env.PORT || 3000; const key = process.env.MISSING_SECRET;',
    });
    try {
        const result = await scanDeploymentReadiness(repo.dir);
        const envIssue = result.issues.find(i => i.pattern === 'SB-DEP-002' && i.metadata.envVar === 'MISSING_SECRET');
...
test('scanDeploymentReadiness detects SB-DEP-004 CORS divergence', async () => {
    const repo = await createTempRepo({
        ...
        'api-server/server.cjs': 'const { isAllowedOrigin } = require("./lib/cors-config.cjs");',
        'api-server/lib/cors-config.cjs': 'function resolveCorsOptions() {}',
        'coming-soon/package.json': JSON.stringify({ name: 'web', scripts: { start: 'node server.cjs' } }),
        'coming-soon/server.cjs': 'res.setHeader("Access-Control-Allow-Origin", "*");',
    });
    try {
        const result = await scanDeploymentReadiness(repo.dir);
