...
test("resolveAutoAnalysisMode prefers simplebeacon for mock data indicators", () => {
  assert.equal(
    resolveAutoAnalysisMode("/tmp/project/data/mock"),
    "simplebeacon",
  );
  assert.equal(
    resolveAutoAnalysisMode("/tmp/project/mock/data"),
    "simplebeacon",
  );
  assert.equal(
    resolveAutoAnalysisMode("/tmp/project/fixtures"),
    "simplebeacon",
  );
...
test("resolveAutoAnalysisMode respects custom indicators", () => {
  assert.equal(
    resolveAutoAnalysisMode("/tmp/project/web/data", ["web/data"]),
    "simplebeacon",
  );
  assert.equal(
    resolveAutoAnalysisMode("/tmp/project/custom-dir", ["custom-dir"]),
    "simplebeacon",
  );
  assert.equal(
    resolveAutoAnalysisMode("/tmp/project/other", ["custom-dir"]),
    "roadmap",
  );
});
