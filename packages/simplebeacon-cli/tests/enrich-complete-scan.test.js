...
test("compactDataCleanupReportForClient keeps plans but drops bulk finding arrays", () => {
  ...
  } = require("../src/lib/enrich-cleanup-report");
  const bulky = {
    summary: { totalFindings: 15000, unusedFileCandidates: 14000 },
    fileReductionPlan: { totals: { safeToDeleteBytes: 1000 } },
    executiveSummary: {
      priorityActions: [{ title: "Reclaim build artifact space" }],
    },
    findings: {
      unusedFiles: Array.from({ length: 14000 }, (_, i) => ({
        path: `unused-${i}.js`,
        type: "unused-file",
      })),
    },
    allFindings: Array.from({ length: 15000 }, (_, i) => ({
      path: `f-${i}`,
      type: "unused-file",
    })),
  };
...
test("enrichCompleteScan adds analysis and corrected summary fields", () => {
  ...
  assert.ok(
    enriched.completeScanAnalysis.fileReduction.immediateSavingsBytes >= 900,
  );
