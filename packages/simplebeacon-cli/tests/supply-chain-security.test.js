...
const { SupplyChainSecurityScanner } = require('../src/analyzers/file-reduction/supply-chain-security-scanner');
...
test('SupplyChainSecurityScanner flags typosquatting in package.json', async () => {
    ...
    assert.ok(result.findings.some((f) => f.type === 'supply-chain-typosquat' && f.metadata.package === 'lodashd'));
    assert.ok(result.findings.some((f) => f.type === 'supply-chain-typosquat' && f.metadata.package === 'expresss'));
...
test('SupplyChainSecurityScanner detects suspicious install scripts', async () => {
    ...
    assert.ok(result.findings.some((f) => f.type === 'supply-chain-suspicious-script'));
