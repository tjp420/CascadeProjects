...
} = require("../src/analyzers/file-reduction/asset-consolidation-scanner");
const {
  UnusedFileDetector,
} = require("../src/analyzers/file-reduction/unused-file-detector");
const { runFileReductionAnalysis } = require("../src/analyzers/file-reduction");
const {
  parseJSImports,
} = require("../src/analyzers/file-reduction/utils/import-parser");
const {
...
test("BuildArtifactScanner detects node_modules and standalone .map files", async () => {
  ...
  assert.ok(
    !result.findings.some((f) => f.path === "node_modules/nested/node_modules"),
  );
  assert.ok(result.findings.some((f) => f.path === "src/app.js.map"));
  assert.ok(!result.findings.some((f) => f.path === "dist/app.min.js.map"));
  assert.ok(
    result.summary.safeToDeleteBytes <= result.summary.reclaimableBytes,
  );
});
...
test("UnusedFileDetector does not treat node_modules index.js as entry points", async () => {
  ...
  assert.equal(
    entryPoints.some((p) => p.includes("node_modules")),
    false,
  );
...
test("UnusedFileDetector skips tests-legacy and .github-sync mirror trees", async () => {
  ...
  assert.equal(
    paths.includes(".github-sync/simplebeacon/bin/orphan.js"),
    false,
  );
});
...
test("UnusedFileDetector flags unreferenced modules but keeps entry points", async () => {
  ...
  assert.equal(
    result.findings.some((f) => f.path === "lib/helper.js"),
    false,
  );
});

test("parseJSImports resolves tsconfig @/ path aliases", () => {
  const root = makeTempProject({
    "web/app/tsconfig.json": JSON.stringify({
      compilerOptions: { paths: { "@/*": ["./src/*"] } },
    }),
    "web/app/src/components/Panel.tsx": "export const Panel = 1;\n",
    "web/app/src/app/page.tsx": "import { Panel } from '@/components/Panel';\n",
  });
  const pagePath = path.join(root, "web", "app", "src", "app", "page.tsx");
  ...
  assert.ok(imports[0].resolvedPath.endsWith(`${path.sep}Panel.tsx`));
});
...
test("parseJSImports resolves cache-busted relative imports", () => {
  ...
  assert.equal(imports.length, 1);
  assert.ok(
    imports[0].resolvedPath.endsWith(
      `${path.sep}src${path.sep}views${path.sep}Panel.js`,
    ),
  );
});
...
test("UnusedFileDetector skips coverage artifacts", async () => {
  ...
  const paths = result.findings.map((f) => f.path);
  assert.equal(
    paths.some((p) => p.startsWith("coverage/")),
    false,
  );
});
    ...
    "package.json": JSON.stringify({
      ...
      scripts: {
        "test:coverage:critical-path":
          "jest --config jest.critical-path.config.js --coverage",
      },
    }),
    "index.js": "require('./lib/core');\n",
    "lib/core.js": "module.exports = () => 1;\n",
...
test("UnusedFileDetector skips monorepo-prefixed tests tree", async () => {
  ...
  const paths = result.findings.map((f) => f.path);
  assert.equal(
    paths.some((p) => p.startsWith("ai-platform/tests/")),
    false,
  );
});
...
test("UnusedFileDetector skips monorepo-prefixed coverage artifacts", async () => {
  ...
  const paths = result.findings.map((f) => f.path);
  assert.equal(
    paths.some((p) => p.includes("coverage/")),
    false,
  );
});
...
test("UnusedFileDetector treats monorepo-prefixed jest.critical-path.config.js as entry point", async () => {
  const root = makeTempProject({
    ...
    "ai-platform/jest.critical-path.config.js":
      "module.exports = require('./jest.config');\n",
  ...
  const paths = result.findings.map((f) => f.path);
  assert.equal(
    paths.includes("ai-platform/jest.critical-path.config.js"),
    false,
  );
});
...
test("parseHtmlReferences resolves web-root absolute asset paths", () => {
  const root = makeTempProject({
    "ai-platform/web/simplebeacon-dashboard/index.html":
      '<link rel="stylesheet" href="/simplebeacon-dashboard/css/theme.css">',
  ...
  const {
    parseHtmlReferences,
  } = require("../src/analyzers/file-reduction/utils/file-reference-tracker");
