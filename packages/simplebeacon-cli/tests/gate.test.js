...
test("ai-platform simplebeacon scan runs with cascade config", () => {
  ...
  const result = runSimplebeacon(
    ["scan", "--path", AI_PLATFORM, "--format", "text"],
    AI_PLATFORM,
  );
  assert.equal(
    result.status,
...
test("ai-platform gate report has no critical issues and no fiction KPIs", () => {
  ...
  const result = runSimplebeacon(
    ["scan", "--path", AI_PLATFORM, "--format", "json", "--output", outFile],
    AI_PLATFORM,
  );
...
test("parent workspace scan resolves ai-platform mock data paths", () => {
  ...
  const outFile = path.join(
    parent,
    ".simplebeacon/parent-scan-test-report.json",
  );
  ...
  const result = runSimplebeacon(
    ["scan", "--path", parent, "--format", "json", "--output", outFile],
    AI_PLATFORM,
    120000,
  );
  ...
  if (result.status === null) {
    ...
    console.log(
      "  Parent workspace scan timed out (120s) — skipping (monorepo too large for CI)",
    );
    return;
  }
  ...
  assert.ok(
    report.totalFiles > 0,
    `expected mock files, got ${report.totalFiles}`,
  );
  assert.equal(
