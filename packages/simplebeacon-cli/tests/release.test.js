...
test('parseArgs extracts --publish and --otp', () => {
    const args = parseArgs(['node', 'release.cjs', 'minor', '--publish', '--otp', '123456']);
...
test('parseArgs extracts --dry-run and --skip-tests', () => {
    const args = parseArgs(['node', 'release.cjs', 'major', '--dry-run', '--skip-tests']);
...
test('categorizeCommits separates features, fixes, chores, others', () => {
    ...
    assert.strictEqual(result.features[0].subject, 'feat: add markdown report generator');
    assert.strictEqual(result.fixes.length, 1);
    assert.strictEqual(result.fixes[0].subject, 'fix: resolve webhook signature bug');
...
test('generateChangelogEntry produces formatted markdown', () => {
    const commits = {
        features: [{ hash: 'abc1234', subject: 'feat: add markdown report', author: 'Alice', date: '2026-08-07' }],
        fixes: [{ hash: 'def5678', subject: 'fix: webhook bug', author: 'Bob', date: '2026-08-07' }],
        chores: [{ hash: 'ghi9012', subject: 'chore: deps', author: 'Carol', date: '2026-08-06' }],
        others: []
    };
