...
test('ConfigManagementAnalyzer flags env sprawl and profile-local inconsistencies', async () => {
  ...
  assert.ok(
    result.findings.some((f) => f.type === 'env-inconsistency' && f.metadata.key === 'DB_HOST')
  );
  assert.ok(
...
test('DependencyHealthAnalyzer detects duplicate sections and version drift', async () => {
  ...
  assert.ok(
    result.findings.some(
      (f) => f.type === 'duplicate-dependency' && f.metadata.dependency === 'express'
    ),
    'should detect duplicate express dependency'
  );
  assert.ok(
    result.findings.some((f) => f.type === 'version-drift' && f.metadata.dependency === 'lodash'),
...
test('EnvironmentVariableAnalyzer detects missing and unused env keys', async () => {
  ...
  const result = await scanner.scan(root, { inventory });
  assert.ok(
    result.findings.some((f) => f.type === 'missing-env-key' && f.metadata.key === 'API_BASE'),
    'should flag API_BASE as missing'
  );
  assert.ok(
    result.findings.some((f) => f.type === 'unused-env-key' && f.metadata.key === 'LEGACY_FLAG'),
...
test('EnvironmentVariableAnalyzer treats OS-injected env keys as runtime-provided', async () => {
  ...
  const result = await scanner.scan(root, { inventory });
  assert.ok(
    !result.findings.some((f) => f.type === 'missing-env-key' && f.metadata.key === 'USERPROFILE'),
...
test('EnvironmentVariableAnalyzer skips phase-2 SSO example keys', async () => {
  ...
  const result = await scanner.scan(root, { inventory });
  assert.ok(
    !result.findings.some((f) => f.type === 'unused-env-key' && f.metadata.key === 'SAML_ENABLED'),
    'phase-2 SSO example keys should be skipped'
  );
  assert.ok(
    !result.findings.some((f) => f.type === 'unused-env-key' && f.metadata.key === 'LDAP_URL'),
    'phase-2 SSO example keys should be skipped'
  );
});
...
test('EnvironmentVariableAnalyzer detects get() and resolveCredential env references', async () => {
  ...
  assert.ok(
    !result.findings.some(
      (f) => f.type === 'unused-env-key' && f.metadata.key === 'STRIPE_PUBLISHABLE_KEY'
    ),
...
test('DependencyHealthAnalyzer detects tools/ requires before file cap', async () => {
  ...
  assert.ok(
    !result.findings.some(
      (f) => f.type === 'unused-dependency' && f.metadata.dependency === 'archiver'
    ),
...
test('buildExecutiveSummary categorizes credential findings', () => {
  ...
  assert.ok(
    summary.notes.some((note) => note.includes('test fixtures') || note.includes('mock/sample')),
...
test('DataAccessPatternAnalyzer flags raw SQL via AST', async () => {
  ...
  assert.ok(
    result.findings.some(
      (f) => f.type === 'data-access-pattern' && f.metadata?.patternId === 'raw-sql-ast'
    ),
...
test('scanPiiContent skips comment and documentation example contexts', () => {
  const docFindings = scanPiiContent(
    ...
    ['// contact admin@example.com for help', 'See admin@company.com in production only'].join('\n')
  );
...
test('DataLineageAnalyzer detects runtime fetch references to data files', async () => {
  ...
  assert.ok(
    result.metadata.lineage.some(
      (entry) => entry.path.includes('web/data/users.json') && entry.consumerCount >= 1
    ),
...
test('ConfigManagementAnalyzer flags unreferenced non-root configs', async () => {
  ...
  assert.ok(
    result.findings.some(
      (f) => f.type === 'unused-config' && f.path.includes('tools/vite.config.js')
    ),
