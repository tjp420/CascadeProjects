...
const {
  GitHygieneScanner,
} = require("../src/analyzers/file-reduction/git-hygiene-scanner");
...
test("GitHygieneScanner flags sensitive tracked files", async () => {
  ...
  assert.ok(
    result.findings.some(
      (f) => f.type === "git-sensitive-file" && f.path === ".env",
    ),
  );
  assert.ok(
    result.findings.some(
      (f) => f.type === "git-sensitive-file" && f.path === "id_rsa",
    ),
  );
...
test("GitHygieneScanner flags large tracked files", async () => {
  ...
  assert.ok(
    result.findings.some(
      (f) => f.type === "git-large-file" && f.path === "large.bin",
    ),
  );
});
...
test("GitHygieneScanner ignores auth token source modules", async () => {
  ...
  assert.ok(
    !result.findings.some((f) => f.path.includes("tokenStockpileService")),
  );
  assert.ok(
    result.findings.some(
      (f) => f.type === "git-sensitive-file" && f.path === "token.txt",
    ),
  );
});
...
test("GitHygieneScanner detects suspicious commit messages", async () => {
  ...
  execSync("git add .", { cwd: root, stdio: "ignore" });
  execSync(
    'git commit -m "add token ghp_abcdefghijklmnopqrstuvwxyz1234567890"',
