...
test("shouldExcludePath blocks global defaults", () => {
  ...
  assert.equal(
    shouldExcludePath("github-cache/microsoft-pyrit/package.json"),
    true,
  );
  assert.equal(
    shouldExcludePath("deliverables/vendor-handoff/manifest.json"),
    true,
  );
});
...
test("shouldExcludePath respects user exclusions", () => {
  const userExclusions = [
    "dashboard-inline-core",
    "gguf-data-service",
    "temp_dashboard",
  ];
  assert.equal(
    shouldExcludePath(
      "packages/dashboard-inline-core/index.js",
      userExclusions,
    ),
    true,
  );
  assert.equal(
    shouldExcludePath("services/gguf-data-service/api.js", userExclusions),
    true,
  );
  assert.equal(
    shouldExcludePath("scripts/temp_dashboard/data.js", userExclusions),
    true,
  );
  assert.equal(
    shouldExcludePath("server/routes/users.js", userExclusions),
    false,
  );
});
...
test("shouldExcludePath combines defaults with user exclusions", () => {
  ...
  assert.equal(
    shouldExcludePath("node_modules/foo/index.js", userExclusions),
    true,
  ); // default
  assert.equal(
    shouldExcludePath("packages/custom-module/index.js", userExclusions),
    true,
  ); // user
  assert.equal(
    shouldExcludePath("server/routes/users.js", userExclusions),
...
test("shouldExcludePath is case-insensitive for path matching", () => {
  ...
  assert.equal(
    shouldExcludePath("packages/dashboard-core/index.js", userExclusions),
    true,
  );
  assert.equal(
    shouldExcludePath("packages/Dashboard-Core/index.js", userExclusions),
