...
const {
  sanitizeFilePath,
  sanitizeString,
} = require("../../src/lib/input-sanitizer");
const {
  resolvePathFromBase,
...
test("normalizePathKey lowercases and uses forward slashes", () => {
  ...
  assert.ok(
    normalized.includes("project/web/data"),
    "should lowercase and use forward slashes",
  );
});
...
test("resolvePathFromBase rejects traversal outside base", () => {
  ...
  assert.throws(() => resolvePathFromBase(base, "../escape"), ConfigError);
});
...
test("loadSimplebeaconConfig throws ConfigError for explicit invalid config", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "simplebeacon-config-invalid-"),
  );
