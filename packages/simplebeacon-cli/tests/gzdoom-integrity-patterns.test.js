...
describe("gzdoom log parser", () => {
  it("parses unknown sprite, unknown class, and duplicate class lines", () => {
    const log = require("fs").readFileSync(
      path.join(FIXTURE_ROOT, "fog_pbr.log"),
      "utf8",
    );
    ...
    assert.ok(
      entries.some(
        (e) => e.kind === "unknown-sprite" && e.details.sprite === "RGVX",
      ),
    );
    assert.ok(
      entries.some(
        (e) =>
          e.kind === "unknown-class" && e.details.className === "DitchedPistol",
      ),
    );
    assert.ok(
      entries.some(
        (e) => e.kind === "duplicate-class" && e.details.className === "Rocket",
      ),
    );
  });
  ...
  it("parses Script error and missing string lines", () => {
    ...
    const entries = parseGzdoomLog(log);
    assert.ok(
      entries.some((e) => e.kind === "script-error" && e.details.line === 120),
    );
    assert.ok(
      entries.some(
        (e) => e.details.filePath && /sndinfo/i.test(e.details.filePath),
      ),
    );
...
describe("gzdoom symbol graph", () => {
  it("flags unresolved sprite, actor, projectile, weaponslot, duplicate, and replaces conflicts", async () => {
    ...
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-unresolved-sprite" && /RGVX/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-unresolved-actor" &&
          /VoxelRiotgun/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-unresolved-projectile" &&
          /MissingBullet/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-unresolved-weaponslot" &&
          /UndefinedMinigun/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-duplicate-class" && /Rocket/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-replaces-conflict" &&
          /Shotgun/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-double-ammo-consumption" &&
          /PistolMag/.test(i.description),
      ),
    );
  });
  ...
  it("correlates runtime log entries to source files", async () => {
    ...
    assert.ok(
      correlated.some(
        (i) =>
          i.type === "gzdoom-runtime-unknown-sprite" &&
          i.filePath === "MODELDEF",
      ),
    );
    assert.ok(
      correlated.some((i) => i.type === "gzdoom-runtime-unknown-class"),
    );
    assert.ok(
      correlated.some((i) => i.type === "gzdoom-runtime-duplicate-class"),
    );
  });
});
  ...
  it("suppresses unresolved MODELDEF refs to engine actors like BackPack", async () => {
    ...
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-unresolved-actor" &&
          /CustomMissingActor/.test(i.description),
      ),
    );
  });
});
...
describe("gzdoom include reachability", () => {
  it("skips orphan .zs files not reachable from ZSCRIPT", async () => {
    const files =
      await require("../src/lib/gzdoom-symbol-graph").collectGzdoomFiles(
        INCLUDE_FIXTURE,
      );
    const { reachable, orphans: _orphans } = resolveReachableGzdoomFiles(
      INCLUDE_FIXTURE,
      files,
    );
    assert.ok(reachable.has("active.zs"));
    assert.ok(_orphans.includes("orphan-only.zs"));
    const graph = await buildGzdoomSymbolGraph(INCLUDE_FIXTURE, {
  ...
  it("does not flag duplicate ActiveWeapon when orphan file is excluded", async () => {
    const issues = validateGzdoomCrossReferences(
      await buildGzdoomSymbolGraph(INCLUDE_FIXTURE),
    );
    assert.ok(
      !issues.some(
        (i) =>
          i.type === "gzdoom-duplicate-class" &&
          /ActiveWeapon/.test(i.description),
      ),
    );
  });
});
...
describe("gzdoom inventory cleanup suppression", () => {
  it("suppresses asymmetry inside Strip/Remove helper functions", async () => {
    ...
    assert.ok(
      !issues.some(
        (i) =>
          i.type === "gzdoom-inventory-asymmetry" &&
          /RenderAliasToken/.test(i.description),
      ),
    );
    assert.ok(
      issues.some(
        (i) =>
          i.type === "gzdoom-inventory-asymmetry" &&
          /NeverGivenItem/.test(i.description),
      ),
    );
  });
});
...
describe("stripComments", () => {
  ...
  it("handles nested-looking block comment closes correctly", () => {
    const input =
      "/* outer /* inner */ class NotReal : Actor {} */\nclass Real : Actor {}\n";
    const stripped = stripComments(input);
...
describe("gzdoom comment-aware class parsing", () => {
  it("does not flag duplicate class when name appears in comment", async () => {
    ...
    const dupIssues = issues.filter(
      (i) =>
        i.type === "gzdoom-duplicate-class" &&
        /R3DPainElementalSprite2/.test(i.description),
    );
    assert.equal(
      dupIssues.length,
      0,
...
describe("gzdoom DECORATE include expansion", () => {
  it("follows DECORATE #include to discover actors in .txt files", async () => {
    const tmp = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sb-inc-"),
    );
    const fs = require("fs");
    const path = require("path");
    // DECORATE entry lump includes a .txt file
    fs.writeFileSync(
      path.join(tmp, "DECORATE"),
    ...
    fs.writeFileSync(
      path.join(tmp, "actors", "weapons", "knuckle.txt"),
      "Actor Knuckle : Weapon\n{\n  States\n  {\n  Ready:\n    KNCK A 1;\n    Loop;\n  }\n}\n",
      "utf8",
    );
    // MODELDEF references the Knuckle actor
    fs.writeFileSync(
      path.join(tmp, "MODELDEF"),
...
describe("gzdoom case-insensitive path dedup", () => {
  it("does not flag duplicate classes when includes use case-variant paths", async () => {
    const tmp = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sb-case-"),
    );
    const fs = require("fs");
    const path = require("path");
    ...
    fs.writeFileSync(
      path.join(tmp, "monsters", "DemonClass.zs"),
    ...
    const dupIssues = issues.filter(
      (i) =>
        i.type === "gzdoom-duplicate-class" &&
        /UniqueDemon/.test(i.description),
    );
    assert.equal(
      dupIssues.length,
      0,
...
describe("gzdoom ZSCRIPT.zs entry point detection", () => {
  it("accepts ZSCRIPT.zs (with .zs extension) as entry point", async () => {
    const tmp = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sb-zs-"),
    );
    const fs = require("fs");
    const path = require("path");
    ...
    fs.writeFileSync(
      path.join(tmp, "ZSCRIPT.zs"),
      '#include "active.zs"\n',
      "utf8",
    );
    fs.writeFileSync(
      path.join(tmp, "active.zs"),
      "class EntryActive : Actor {}\n",
      "utf8",
    );
    // Orphan file not reachable from ZSCRIPT.zs
    fs.writeFileSync(
      path.join(tmp, "orphan.zs"),
      "class OrphanClass : Actor {}\n",
      "utf8",
    );

    const files =
      await require("../src/lib/gzdoom-symbol-graph").collectGzdoomFiles(tmp);
    const { reachable, orphans: _orphans } = resolveReachableGzdoomFiles(
      tmp,
      files,
    );
    assert.ok(
      reachable.has("ZSCRIPT.zs"),
      "ZSCRIPT.zs should be detected as entry point",
    );
    assert.ok(
      reachable.has("active.zs"),
      "active.zs should be reachable from ZSCRIPT.zs",
    );
    assert.ok(
      _orphans.includes("orphan.zs"),
      "orphan.zs should be flagged as unreachable",
    );

    const graph = await buildGzdoomSymbolGraph(tmp, { respectIncludes: true });
    assert.ok(
      graph.actors.has("EntryActive"),
      "EntryActive should be discovered via ZSCRIPT.zs",
    );
    assert.ok(
      !graph.actors.has("OrphanClass"),
      "OrphanClass should not be discovered (unreachable file)",
    );
  });
});
...
describe("gzdoom expanded vanilla actors", () => {
  it("suppresses unresolved MODELDEF refs to vanilla decorations and props", async () => {
    const tmp = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sb-vanilla-"),
    );
    const fs = require("fs");
    const path = require("path");
    ...
    assert.ok(
      !issues.some(
        (i) =>
          i.type === "gzdoom-unresolved-actor" &&
          /ExplosiveBarrel/.test(i.description),
      ),
...
describe("gzdoom extend class handling", () => {
  it('does not flag duplicate class for ZScript "extend class" blocks', async () => {
    const tmp = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sb-ext-"),
    );
    const fs = require("fs");
    const path = require("path");
    ...
    fs.writeFileSync(path.join(tmp, "ZSCRIPT"), '#include "real.zs"\n', "utf8");
    fs.writeFileSync(
      path.join(tmp, "real.zs"),
      "class MyActor : Actor {\n" +
...
describe("gzdoom commented-out #include handling", () => {
  it("does not follow #include directives inside comments", async () => {
    const tmp = require("fs").mkdtempSync(
      require("path").join(require("os").tmpdir(), "sb-cinc-"),
    );
    const fs = require("fs");
    const path = require("path");
    ...
    fs.writeFileSync(
      path.join(tmp, "archived_dup.zs"),
      "class ActiveClass : Actor {}\n",
      "utf8",
    );
    fs.writeFileSync(
      path.join(tmp, "blocked_dup.zs"),
      "class ActiveClass : Actor {}\n",
      "utf8",
    );
    ...
    const dupIssues = issues.filter(
      (i) =>
        i.type === "gzdoom-duplicate-class" &&
        /ActiveClass/.test(i.description),
    );
    assert.equal(
      dupIssues.length,
      0,
