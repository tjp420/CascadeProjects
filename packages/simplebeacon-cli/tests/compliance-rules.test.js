...
describe('compliance-rules/index.js', () => {
    ...
    it('auto-discovered rules match filesystem', () => {
        ...
        assert.strictEqual(regKeys.length, ruleFiles.length, 'registry size should match number of rule files');
    ...
    it('getRegistry snapshot is not mutated by later registrations', () => {
        ...
        assert.ok(!Object.keys(snapshot).includes('another-dynamic'), 'snapshot should remain unchanged');
    });
