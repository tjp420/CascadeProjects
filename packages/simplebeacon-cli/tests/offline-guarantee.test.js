...
test("offline guard blocks non-local https.request", () => {
  ...
  try {
    assert.throws(() => {
      ...
      https.request("https://api.openai.com/v1/chat");
...
test("offline guard records but does not block when offline=false", () => {
  ...
  try {
    ...
    assert.ok(
      ...
      "should record at least 1 violation",
    );
    assert.ok(
...
test("runScan with offline:true completes without network violations", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      "report should have rawIssues array",
    );
  } finally {
...
test("report-enhance callOpenAIChatCompletion blocks in offline mode", () => {
  ...
  assert.throws(
    () =>
      callOpenAIChatCompletion("test prompt", {
        offline: true,
        apiKey: "sk-test",
      }),
...
test("github-comment postGithubComment blocks in offline mode", async () => {
  ...
  await assert.rejects(
    () =>
      postGithubComment("/tmp/fake-report.json", {
        offline: true,
        token: "fake",
        repo: "test/repo",
        issueNumber: "1",
