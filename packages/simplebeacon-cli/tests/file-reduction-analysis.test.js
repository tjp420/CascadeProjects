...
describe("file-reduction/index.js", () => {
  ...
  it("findings keys match scanner IDs", async () => {
    ...
    for (const key of expectedKeys) {
      assert.ok(
        Array.isArray(report.findings[key]),
        `findings.${key} should be an array`,
      );
    }
  });
  ...
  it("summary keys match scanner IDs with Findings suffix", async () => {
    ...
    const expectedKeys = moduleUnderTest.DEFAULT_SCANNERS.map(
      (s) =>
        s.id.replace(/-([a-z])/g, (_, ch) => ch.toUpperCase()) + "Findings",
    );
    for (const key of expectedKeys) {
      assert.strictEqual(
        typeof report.summary[key],
        "number",
        `summary.${key} should be a number`,
      );
    }
  });
  ...
  it("enables only default scanners when no explicit config", async () => {
    ...
    const enabledByDefault = moduleUnderTest.DEFAULT_SCANNERS.filter(
      (s) => s.enabled,
    );
  ...
  it("handles empty inventory gracefully", async () => {
    ...
    const report = await moduleUnderTest.runFileReductionAnalysis(emptyDir, {
      dryRun: true,
    });
