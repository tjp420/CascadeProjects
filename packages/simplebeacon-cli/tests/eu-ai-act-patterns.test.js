...
test("hasTransparencyDisclosure detects Article 50 markers", () => {
  assert.equal(
    hasTransparencyDisclosure("This content is AI-generated."),
    true,
  );
  assert.equal(
    hasTransparencyDisclosure("You are interacting with an AI assistant."),
    true,
  );
  assert.equal(
    hasTransparencyDisclosure("const x = openai.chat.completions.create();"),
    false,
  );
});
...
test("scanEuAiActPatterns flags transparency gap in user-facing AI code", async () => {
  ...
  assert.ok(
    result.issues.some((i) => i.metadata?.patternId === "EUAI-T50-001"),
  );
...
test("scanEuAiActPatterns collapses multiple AI indicator matches per file", async () => {
  ...
  const aiIssues = result.issues.filter(
    (i) => i.metadata?.patternId === "EUAI-AI-001",
  );
...
test("detectDocumentationArtifacts finds model-card.md", () => {
  ...
  fs.writeFileSync(
    path.join(dir, "docs", "model-card.md"),
...
test("scanEuAiActPatterns suppresses documented AI inventory indicators when compliance docs exist", async () => {
  ...
  assert.equal(
    result.issues.filter((i) => i.metadata?.patternId === "EUAI-AI-001").length,
    0,
  );
