...
describe("gzdoom death-frame reuse lint", () => {
  it("parses labeled state frames from a States block", () => {
    ...
    const missileM = byLabel
      .get("Missile")
      .filter((f) => f.frame === "M" && f.sprite === "RSKN");
    assert.equal(missileM.length, 1);
  });

  it("flags RSKN K/L/M shared between Missile/Melee and Death in buggy Revenant fixture", () => {
    const issues = lintZScriptDeathFrameReuse(
      BUGGY,
      "zscript/monsters/Demons/Revenant/revenant_model_variants.zs",
    );
    ...
    assert.ok(
      combat.length >= 6,
      `expected >=6 high/combat issues, got ${combat.length}`,
    );
    assert.ok(
      combat.some(
        (i) =>
    ...
    assert.ok(
      combat.some(
        (i) => i.metadata.frame === "K" && i.metadata.otherState === "Melee",
      ),
    );
  });
  ...
  it("scans fixture directory via lintGzdoomDeathFrameReuse", async () => {
    const result = await lintGzdoomDeathFrameReuse(
      path.join(__dirname, "fixtures", "gzdoom-death-frame"),
      {
...
`;
    ...
    assert.ok(
      issues.length > 0,
      "Should flag death-frame reuse in 3D model class",
    );
  });
...
`;
    ...
    assert.equal(
      issues.length,
      0,
      "Should NOT flag sprite-only variant (vanilla behavior)",
    );
  });
