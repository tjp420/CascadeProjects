...
function makeMockReport(overrides = {}) {
  return {
    ...
    scanPaths: [
      "/home/acme/super-secret-project/src",
      "/home/acme/super-secret-project/web/data",
    ],
    ...
    rawIssues: [
      {
        ...
        filePath:
          "/home/acme/super-secret-project/web/data/dashboard-sample.json",
        count: 1,
        ...
        metadata: {
          ...
          specFile: "dashboard-sample.json",
          violations: [{ message: "Missing revenue" }],
        },
      },
      {
      ...
      },
      {
        ...
        type: "Fiction KPI",
        filePath:
          "/home/acme/super-secret-project/web/data/metrics-sample.json",
        count: 1,
  ...
  describe("buildAnonymizedExport", () => {
    ...
    it("contains only abstract issue tokens (no paths, no descriptions)", () => {
      ...
      for (const issue of payload.issues) {
        ...
        assert.ok(
          SEVERITY_BANDS.includes(issue.s),
          "severity must be a valid band",
        );
        ...
        assert.strictEqual(
          issue.affectedFiles,
          undefined,
          "affectedFiles must be stripped",
        );
        assert.strictEqual(
          issue.recommendedAction,
          undefined,
          "recommendedAction must be stripped",
        );
      }
    });
    ...
    it("produces different repoFingerprints for different roots", () => {
      const a = buildAnonymizedExport(
        makeMockReport({ projectRoot: "/a" }),
      ).repoFingerprint;
      const b = buildAnonymizedExport(
        makeMockReport({ projectRoot: "/b" }),
      ).repoFingerprint;
