...
const { resolveCliProjectRoot, sanitizeCliPathOptions } = require('../src/lib/path-utils');
...
function resolvePolicyGateConfig() {
  const fingerprint = String(process.env.SIMPLEBEACON_POLICY_TRUST_FINGERPRINT || '')
    .trim()
    .toLowerCase();
  ...
  return {
    trustStore:
      fingerprint && publicKeyPem ? new TrustStore({ [fingerprint]: publicKeyPem }) : null,
    orgPolicyPath:
...
function parseArgs(argv) {
  ...
  for (let i = flagStart; i < args.length; i += 1) {
    ...
    if (arg.startsWith('-') && !knownFlags.has(arg)) {
      throw new ConfigError(`Unknown argument: ${args[i]}. Run with --help for usage.`, {
        arg: args[i],
      });
    }
...
async function executeOneScan(options, networkGuard) {
  ...
  if (options.complete || options.fullDirectoryScan) {
    ...
    if (options.verbose)
      console.error('[scan] --complete enabled: full directory scan + all analyzers');
  }
  if (options.failOn) {
  ...
  try {
    ...
    if (options.output) {
      writeManagedFileSync(path.resolve(options.output), `${payload}\n`, {
        force: true,
        validators:
          options.format === 'json' ? [validateJSON, validateNotEmpty] : [validateNotEmpty],
      });
    ...
    if (options.paidLicense && !airGapped) {
      try {
        ...
        const telemetryResult = await postCiTelemetry(
          jsonReport,
          {
            paid: true,
            tier: options.tier || 'developer',
    ...
    if (options.gate && !gateResult.pass) {
      console.error(
        paint(`Gate failed: ${gateResult.blockingIssues.length} blocking issue(s)`, 'red')
      );
      return 1;
    }
    ...
    if (options.fixDryRun && remediation && remediation.total > 0) {
      console.error(
        paint(
          remediation.total + ' fixable issue(s) found in dry-run; run without --dry-run to apply.',
          'red'
        )
      );
      return 1;
...
async function runUploadCommand(options) {
  ...
  if (!options.report && !options.path) {
    throw new ConfigError('Provide --path to scan or --report to upload an existing report', {
      command: 'upload',
    });
  }
  ...
  try {
    const response = await fetch(`${apiUrl}/api/simplebeacon/upload-report`, {
      ...
      body: JSON.stringify({
        ...
        scannedPath:
          report.projectPath || report.projectRoot || options.path || options.report || '',
      }),
      signal: abortController.signal,
    });
...
function runSecretsGateCommand(options) {
  ...
  if (asJson) {
    ...
    writeStdoutLine(
      result.message || (result.pass ? 'Staged secrets gate passed' : 'Staged secrets gate failed')
    );
...
async function runBuyClearanceCommand(options) {
  ...
  const pollSeconds =
    Number.isFinite(options.pollSeconds) && options.pollSeconds > 0 ? options.pollSeconds : 5;
  const maxPolls =
    Number.isFinite(options.maxPolls) && options.maxPolls > 0 ? options.maxPolls : 60;
    ...
    try {
      ...
      if (pollData.paymentStatus === 'paid') {
        if (pollData.licenseToken) {
          ...
          writeStdoutLine(
            JSON.stringify({ success: true, licensePath, email: pollData.email }, null, 2)
          );
          return;
        }
        console.error(
...
function runFixCommand(options) {
  ...
  if (envVariablesToExport.length > 0 && !dryRun) {
    ...
    console.error(
      '[OK] Appended ' + envVariablesToExport.length + ' quarantine definitions to .env'
    );
  } else if (envVariablesToExport.length > 0 && dryRun) {
    console.error(
      '[!] ' + envVariablesToExport.length + ' tokens would be extracted to .env on write:'
    );
