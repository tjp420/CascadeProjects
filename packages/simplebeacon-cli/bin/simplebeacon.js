...
} = require("../src/lib/ast-remediator");
...
const { SimplebeaconError, ConfigError } = require("../src/lib/errors");
const {
...
} = require("../src/lib/file-reduction-orchestrator");
const {
  generateFileReductionReport,
} = require("../src/reporters/file-reduction-report");
...
function resolvePolicyGateConfig() {
  const fingerprint = String(
    process.env.SIMPLEBEACON_POLICY_TRUST_FINGERPRINT || "",
  )
    .trim()
    .toLowerCase();
  let publicKeyPem = String(
    process.env.SIMPLEBEACON_POLICY_PUBLIC_KEY || "",
  ).trim();

  const publicKeyPath = String(
    process.env.SIMPLEBEACON_POLICY_PUBLIC_KEY_PATH || "",
  ).trim();
  ...
  return {
    trustStore:
      fingerprint && publicKeyPem
        ? new TrustStore({ [fingerprint]: publicKeyPem })
        : null,
    orgPolicyPath:
      String(process.env.SIMPLEBEACON_ORG_POLICY_PATH || "").trim() ||
      path.join(
    ...
    repoPolicyPath:
      String(process.env.SIMPLEBEACON_REPO_POLICY_PATH || "").trim() ||
      path.join(process.cwd(), "simplebeacon.policy.json"),
  };
}

function runPolicyGate() {
  const bypassRequested = ["1", "true", "yes", "on"].includes(
    String(process.env.SIMPLEBEACON_DISABLE_POLICY_GATE || "")
      .trim()
      .toLowerCase(),
  );
  const isLocalDevMode =
    process.env.NODE_ENV === "development" && bypassRequested;
  ...
  const { trustStore, orgPolicyPath, repoPolicyPath } =
    resolvePolicyGateConfig();

  if (!trustStore) {
    console.error(
      "[AUDIT FAILURE] Policy gate requires SIMPLEBEACON_POLICY_TRUST_FINGERPRINT and SIMPLEBEACON_POLICY_PUBLIC_KEY or SIMPLEBEACON_POLICY_PUBLIC_KEY_PATH",
    );
    process.exit(78);
  }

  const repoPath =
    repoPolicyPath && fs.existsSync(repoPolicyPath) ? repoPolicyPath : null;
...
const FLAG_MAP = [
  ...
  {
    aliases: ["--with-analyzer-suite"],
    key: "withAnalyzerSuite",
    type: "boolean",
  },
  {
    aliases: ["--fullDirectoryScan", "--full"],
    key: "fullDirectoryScan",
    type: "boolean",
  },
...
function parseArgs(argv) {
  ...
  for (let i = flagStart; i < args.length; i += 1) {
    ...
    const requireNext = (flagName) => {
      const v = next();
      if (v === null)
        throw new ConfigError(`${flagName} requires a value`, {
          flag: flagName,
        });
      return v;
    };
    ...
    if (flagDef.type === "boolean") {
      ...
      const n = Number(requireNext(arg));
      options[flagDef.key] =
        Number.isFinite(n) && n > 0 ? n : (flagDef.fallback ?? 1);
    }
  }
...
async function uploadReportToCloud(uploadUrl, apiToken, report) {
  if (typeof uploadUrl !== "string" || !uploadUrl) {
    throw new ConfigError("uploadUrl must be a non-empty string", {
      uploadUrl,
    });
  }
  if (typeof apiToken !== "string" || !apiToken) {
    throw new ConfigError("--api-token is required when using --upload", {
      uploadUrl,
    });
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        data.message ||
          data.error ||
          `Cloud upload failed (${response.status})`,
      );
    }
...
async function executeOneScan(options, networkGuard) {
  if (!options || typeof options !== "object")
    throw new TypeError("executeOneScan requires an options object");
  const scanRoot = options.path;
  const { platformRoot } = resolvePlatformRoot(scanRoot);
  const configPath = options.config
    ? path.resolve(scanRoot, options.config)
    : options.config;
  ...
  if (options.complete || options.fullDirectoryScan) {
    ...
    if (options.verbose)
      console.error(
        "[scan] --complete enabled: full directory scan + all 48 analyzers + 25 scan engines",
      );
  }
  if (options.failOn) {
  ...
  if (options.verbose) {
    ...
    console.error(
      `Production paths: ${config.productionPaths?.join(", ") || "(none)"}`,
    );
  ...
  try {
    ...
    if (options.diff) {
      ...
      if (diffFiles && diffFiles.length) {
        ...
        if (options.verbose) {
          console.error(
            `[scan] Diff-only mode: ${diffFiles.length} changed file(s)`,
          );
        }
      } else if (!options.quiet) {
        console.error(
          "[scan] --diff enabled but no changed files detected; scanning full configured scope.",
        );
      }
    }
    ...
    const report = await runScan(sanitizedScanRoot, {
      ...
      ci:
        options.withCi || Boolean(process.env.CI || process.env.GITHUB_ACTIONS),
    ...
    const gateResult =
      ...
      typeof report.gate.pass === "boolean"
        ? report.gate
    ...
    if (options.upload) {
      const uploadResult = await uploadReportToCloud(
        options.upload,
        options.apiToken,
        jsonReport,
      );
    ...
    if (options.output) {
      ...
      if (
        ...
        try {
          ...
          console.error(
            `${paint("✓", "green")} Compliance certificate issued: ${certResult.certPath}`,
          );
          console.error(
            `  Signature: ${certResult.signature.slice(0, 16)}...${certResult.signature.slice(-8)}`,
          );
          console.error(`  Issued at: ${certResult.issuedAt}`);
        } catch (certErr) {
          console.error(
            `${paint("⚠", "yellow")} Certification failed: ${certErr.message}`,
          );
          // Non-blocking — the scan itself succeeded
        }
      } else if (options.certify && (options.offline || airGapped)) {
        console.error(
          `${paint("⚠", "yellow")} --certify requires network access — skipped in offline/air-gapped mode`,
        );
      } else if (options.certify && options.format !== "json") {
        console.error(
          `${paint("⚠", "yellow")} --certify requires --format json — skipped for ${options.format} output`,
        );
      }
    } else {
    ...
    if (options.paidLicense && !airGapped && !options.offline) {
      try {
        ...
        const telemetryResult = await postTeamTelemetry(
          jsonReport,
          {
            paid: true,
            tier: options.tier || "developer",
      ...
      if (fixableIssues.length > 0) {
        ...
        if (fixEngine === "deterministic" || fixEngine === "auto") {
          ...
          const deterministicIssues = remainingIssues.filter(
            (i) =>
              supportedPatterns.has(i.pattern) ||
              supportedPatterns.has(i.metadata?.patternId),
          );
          if (deterministicIssues.length > 0) {
            ...
            console.error(
              `   Applied: ${detResult.applied} | Failed: ${detResult.failed} | Total: ${detResult.total}`,
            );
            ...
            remainingIssues = remainingIssues.filter(
              (i) =>
                !fixedPatterns.has(i.pattern) &&
                !fixedPatterns.has(i.metadata?.patternId),
            );
          }
        }
        ...
        if (
          ...
          const llmResult = await runLocalRemediation(remainingIssues, {
            ...
            maxFixes: options.maxFixes,
            model:
              options.fixProvider === "ollama" || !options.fixProvider
                ? process.env.SIMPLEBEACON_FIX_MODEL || "llama3.2:latest"
                : null,
          });
          console.error(
            `   Applied: ${llmResult.applied} | Failed: ${llmResult.failed} | Total: ${llmResult.total}`,
          );
    ...
    if (options.gate && !gateResult.pass) {
      console.error(
        paint(
          `Gate failed: ${gateResult.blockingIssues.length} blocking issue(s)`,
          "red",
        ),
      );
      return 1;
    ...
    if (options.fixDryRun && remediation && remediation.total > 0) {
      console.error(
        paint(
          remediation.total +
            " fixable issue(s) found in dry-run; run without --dry-run to apply.",
          "red",
        ),
      );
      return 1;
...
async function runScanCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runScanCommand requires an options object");
  ...
  if (airGapped) {
    ...
    if (options.upload) {
      console.error(
        "[simplebeacon] --air-gapped cannot be used with --upload (contradictory)",
      );
      return 1;
    }
  }
  ...
  if (options.offline && options.upload) {
    console.error(
      "Offline mode blocked: --offline cannot be used with --upload",
    );
    return 1;
  }
  const networkGuard = createNetworkGuard({ offline: options.offline });
  printTrustBanner(
    { quiet: options.noTrustBanner, offline: options.offline },
    paint,
  );
  if (airGapped && !options.noTrustBanner) {
    console.error(
      `${paint("✓", "green")} Air-gapped mode — remote license validation and telemetry disabled`,
    );
  }

  const license = await resolveCiLicense({
    failOpenOnNetwork: true,
    failOpenOnExpired: !options.strictLicense,
    allowRemote: !airGapped,
    airGapped,
  });
  if (!license.ok) {
    console.error(
      `[simplebeacon] ${license.message || "Invalid SIMPLEBEACON_LICENSE_TOKEN"}`,
    );
    return 1;
  }
  ...
  try {
    if (options.watch) {
      ...
      console.error(
        paint(
          "[watch] Monitoring project for changes. Press Ctrl+C to stop.",
          "cyan",
        ),
      );
        ...
        .map((p) => {
          try {
            return fs.watch(p, { recursive: true }, (eventType, filename) => {
              if (
                !filename ||
                /node_modules|\.git|\.simplebeacon\/report/.test(filename)
              )
                return;
              if (debounceTimer) clearTimeout(debounceTimer);
              debounceTimer = setTimeout(() => {
                console.error(
                  paint(`[watch] Change detected: ${filename}`, "yellow"),
                );
                run();
              }, 500);
            });
            ...
            console.error(
              paint(`[watch] Failed to watch ${p}: ${err.message}`, "yellow"),
            );
            return null;
          }
        })
...
async function runUploadCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runUploadCommand requires an options object");
  if (!options.apiToken) {
    throw new ConfigError("--api-token is required for upload", {
      command: "upload",
    });
  }
  if (!options.report && !options.path) {
    throw new ConfigError(
      "Provide --path to scan or --report to upload an existing report",
      { command: "upload" },
    );
  }
  ...
  if (options.report) {
    ...
    try {
      ...
      const gateResult =
        rawReport.gate &&
        typeof rawReport.gate === "object" &&
        typeof rawReport.gate.pass === "boolean"
          ? rawReport.gate
...
async function uploadReportToDashboard(report, options) {
  const apiUrl = (
    options.apiUrl ||
    process.env.SIMPLEBEACON_API_URL ||
  ...
  try {
    ...
    writeStdoutLine(
      `👉 View dashboard: ${apiUrl}/dashboard/analyze?cliReport=${data.reportId}`,
    );
    return data;
  } finally {
    clearTimeout(timer);
...
async function runBaselineSyncCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runBaselineSyncCommand requires an options object");
  const root = sanitizePath(options.path);
  if (options.dryRun) {
    writeStdoutLine(
      "DRY RUN — baseline sync requires a test run; use without --dry-run to execute.",
    );
    return;
  }
  const { summary, baselinePath, baseline } = await syncJestBaseline(root, {
    config: options.config,
  });

  writeStdoutLine(`Baseline synced: ${baselinePath}`);
  writeStdoutLine(
    `  Jest: ${baseline.jestTestsLabel} (${summary.suitesPassed} suites)`,
  );
}
...
async function runCommentCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runCommentCommand requires an options object");
...
async function loadOrRunReport(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("loadOrRunReport requires an options object");
  const reportPath = path.resolve(
    options.report || ".simplebeacon/report.json",
  );
...
async function runAssessCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runAssessCommand requires an options object");
  const root = sanitizePath(options.path);
  ...
  const outputPath = path.resolve(
    options.output || ".simplebeacon/assessment.json",
  );
  ...
  writeStdoutLine(
    `Compliance: ${complianceSummary.passed ?? 0}/${(complianceSummary.passed ?? 0) + (complianceSummary.failed ?? 0)} rules pass (score ${execSummary.complianceScore ?? "—"})`,
  );
  ...
  if (options.withAnalyzerSuite) {
    ...
    } = require("../src/lib/ai-problem-analyzer-suite");
    const {
      getCachedAnalysis,
      setCachedAnalysis,
    } = require("../src/lib/ai-problem-analyzer-cache");
    const {
    ...
    if (cached) {
      ...
      writeStdoutLine(
        "[Analyzer Suite] Using cached analysis (report unchanged).",
      );
    } else {
      ...
      analysisResult = buildAiSystemsIssueAnalysis(allIssueIds, {
        context: { scanReport: report },
      });
      setCachedAnalysis(root, report, analysisResult);
    }

    const exportPayload = sanitizeAiProblemAnalyzerExport(analysisResult, {
      projectPath: root,
      context: { healthScore: report.qualityScore },
    });
    const suitePath = path.resolve(
      options.output
        ? options.output.replace(/\.json$/, "-analyzer-suite.json")
        : ".simplebeacon/analyzer-suite.json",
    );
    writeManagedFileSync(
      suitePath,
    ...
    writeStdoutLine(
      `Measured: ${analysisResult.riskSummary.executionStatus.measured} | Insufficient Data: ${analysisResult.riskSummary.executionStatus.insufficientData} | Stubs: ${analysisResult.riskSummary.executionStatus.stub}`,
    );
    writeStdoutLine(
      `Overall Risk Level: ${analysisResult.riskSummary.overallRiskLevel}`,
    );
  }
}
...
async function runReportCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runReportCommand requires an options object");
  const root = sanitizePath(options.path);
  const reportPath = path.resolve(
    options.report || ".simplebeacon/report.json",
  );
  if (!fs.existsSync(reportPath)) {
    throw new Error(
      `Report not found: ${reportPath}. Run: npx simplebeacon scan --format json --output .simplebeacon/report.json --gate`,
    );
  }
  ...
  let assessment = null;
  const assessmentPath = path.resolve(
    options.assessment || ".simplebeacon/assessment.json",
  );
  if (options.enhance) {
    if (!fs.existsSync(assessmentPath)) {
      throw new Error(
        `Assessment required for --enhance: ${assessmentPath}. Run: npx simplebeacon assess`,
      );
    }
  ...
  if (options.enhance) {
    ...
    try {
      ...
      console.error(
        paint(
          `Warning: AI enhancement failed — using deterministic executive summary (${error.message})`,
          "yellow",
        ),
      );
    }
...
async function runComplianceCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runComplianceCommand requires an options object");
  const root = sanitizePath(options.path);
  const report = await loadOrRunReport(options);
  let npmAudit = null;
  try {
    const { runNpmAudit } = require(
      path.join(root, "server/lib/npm-audit-runner"),
    );
  ...
  const outputPath = path.resolve(
    options.output || ".simplebeacon/compliance-result.json",
  );
  ...
  for (const rule of checklist?.rules ?? []) {
    ...
    writeStdoutLine(
      `  ${icon} ${rule.id ?? "—"} ${rule.title ?? "—"} — ${rule.evidence ?? ""}`,
    );
  }
...
function runInitCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runInitCommand requires an options object");
  const root = sanitizePath(options.path);
  ...
  if (created.dryRun) {
    ...
    if (stack) {
      ...
      if (stack.ciWorkflow?.dryRun) {
        writeStdoutLine(
          `Would create: ${stack.ciWorkflow.path} (${stack.ciWorkflow.platformLabel})`,
        );
  ...
  writeStdoutLine(
    "  npx simplebeacon baseline sync   # after a green test run",
  );
  ...
  if (stack) {
    ...
    if (stack.mcp?.created) {
      writeStdoutLine(
        `Created ${stack.mcp.configPath} (MCP mode: ${stack.mcp.mode})`,
      );
    } else if (stack.mcp?.skipped) {
    ...
    if (stack.ciWorkflow?.created) {
      writeStdoutLine(
        `Created ${stack.ciWorkflow.path} (${stack.ciWorkflow.platformLabel})`,
      );
    } else if (stack.ciWorkflow?.skipped) {
      writeStdoutLine(
...
function runHookInstallCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runHookInstallCommand requires an options object");
  ...
  writeStdoutLine(
    `Installed ${result.type} hook (${result.kind}): ${result.hookPath}`,
  );
  if (result.manual) {
    writeStdoutLine("");
    writeStdoutLine(
      "Not a Git repo — copy the script into .husky/ or .git/hooks/ and chmod +x.",
    );
  } else if (result.kind === "husky") {
    writeStdoutLine(
      "Ensure Husky is enabled: npm install -D husky && npx husky init",
    );
  }
}
...
async function runReduceCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runReduceCommand requires an options object");
  const root = sanitizePath(options.path);
  ...
  const rendered = generateFileReductionReport(report, {
    format: options.format,
  });

  writeManagedFileSync(
    outputPath,
    options.format === "json" ? `${rendered}\n` : rendered,
    {
      force: true,
      validators:
        options.format === "json"
  ...
  writeStdoutLine(
    `Findings: ${report.summary.totalFindings} | Reclaimable: ${report.summary.reclaimableBytes} bytes`,
  );
  if (options.verbose) {
...
function runSecretsGateCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runSecretsGateCommand requires an options object");
  ...
  for (const file of stagedFiles) {
    ...
    if (file.split("/").some((p) => SKIP_DIRS.has(p))) {
      skippedFiles++;
      continue;
    }
    try {
  ...
  const result = {
    ...
    message:
      allFindings.length === 0
        ...
        : `${allFindings.length} secret(s) detected`,
  };
  ...
  if (asJson) {
    ...
    writeStdoutLine(
      result.message ||
        (result.pass
          ? "Staged secrets gate passed"
          : "Staged secrets gate failed"),
    );
    if (result.findings?.length) {
      ...
      for (const finding of result.findings) {
        ...
        console.error(
          `  ${file}:${finding.line}  ${finding.pattern}  (${preview})`,
        );
...
function runGateStatusCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runGateStatusCommand requires an options object");
  const root = resolveCliProjectRoot(options.path);
  const status = readGateStatus(root, {
    reportPath: options.report
      ? path.relative(root, path.resolve(root, options.report))
      : undefined,
  });
  ...
  writeStdoutLine(
    `Report: ${status.reportPath} (${status.generatedAt || "unknown time"})`,
  );
  writeStdoutLine(
    `Blocking: ${status.blockingCount} · Warnings: ${status.warningCount} · Fail on: ${status.failOn.join(", ")}`,
  );
...
async function runPdfCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runPdfCommand requires an options object");
  const reportPath = path.resolve(
    options.report || ".simplebeacon/report.json",
  );
  const outputPath =
    options.output || "simplebeacon-executive-risk-certificate.html";
...
async function runBuyClearanceCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runBuyClearanceCommand requires an options object");
  ...
  if (!response.ok) {
    throw new Error(
      data.message || data.error || `Checkout failed (${response.status})`,
    );
  }
  if (!data.url) {
  ...
  execFile(cmd, cmdArgs, (err) => {
    if (err)
      console.error(
        "[buy-clearance] Could not open browser automatically. Please open the URL manually.",
      );
  });

  const sessionUrl = `${server}/api/simplebeacon/billing/session?session_id=${data.sessionId}`;
  const pollSeconds =
    Number.isFinite(options.pollSeconds) && options.pollSeconds > 0
      ? options.pollSeconds
      : 5;
  const maxPolls =
    Number.isFinite(options.maxPolls) && options.maxPolls > 0
      ? options.maxPolls
      : 60;

  console.error(
    `[buy-clearance] Polling for payment completion (max ${maxPolls} attempts, ${pollSeconds}s interval)...`,
  );
  const os = require("os");
      ...
      if (pollData.paymentStatus === "paid") {
        if (pollData.licenseToken) {
          ...
          try {
            ...
            throw new Error(
              `Cannot create license directory ${licenseDir}: ${mkdirErr.message}`,
            );
          }
          ...
          try {
            ...
            throw new Error(
              `Cannot write license token to ${licensePath}: ${writeErr.message}`,
            );
          }
          console.error("[buy-clearance] License token saved");
          writeStdoutLine(
            JSON.stringify(
              { success: true, licensePath, email: pollData.email },
              null,
              2,
            ),
          );
        ...
        console.error(
          "[buy-clearance] Payment confirmed but license token not yet available. Retrying...",
        );
      }
    } catch (err) {
  ...
  throw new Error(
    "Payment confirmation timed out. Run `npx simplebeacon buy-clearance --email <addr>` again to retry, or check your email for the license token.",
  );
}

async function runReferCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runReferCommand requires an options object");
...
function validateCommandOptions(options) {
  ...
  if (cmd === "buy-clearance" && !options.email) {
    throw new ConfigError("--email is required for buy-clearance", {
      command: cmd,
    });
  }
  if (
    cmd === "refer" &&
    !options.from &&
    !process.env.SIMPLEBEACON_REFERRER_EMAIL &&
    !process.env.SIMPLEBEACON_EMAIL &&
    !process.env.SIMPLEBEACON_LICENSE_TOKEN
  ) {
    ...
    if (!fs.existsSync(licensePath)) {
      throw new ConfigError(
        "--from is required for refer (or set SIMPLEBEACON_REFERRER_EMAIL / save ~/.simplebeacon/license.jwt)",
        { command: cmd },
      );
    }
  }
...
async function runCacheCommand(options) {
  ...
  switch (subcommand) {
    ...
    case "prewarm": {
      console.error(
        "[simplebeacon] Pre-warming registry cache (this requires network access)...",
      );
      const results = await prewarmCache([], {
        onProgress: (name, exists) => {
    ...
    case "export": {
      ...
      if (!outputPath) {
        console.error(
          "[simplebeacon] Usage: simplebeacon cache export <output-file>",
        );
        return 1;
      }
      const data = exportCache(outputPath);
      console.error(
        `Exported ${data.entryCount} cache entries to ${outputPath}`,
      );
      console.error(`  Known-good packages: ${data.knownGoodPackages.length}`);
      console.error(
        `  Known-hallucinated packages: ${data.knownHallucinatedPackages.length}`,
      );
      return 0;
    }
    case "import": {
      ...
      if (!inputPath) {
        console.error(
          "[simplebeacon] Usage: simplebeacon cache import <input-file>",
        );
        return 1;
      }
      try {
        ...
        console.error(
          `Imported ${result.imported} cache entries from ${inputPath}`,
        );
        return 0;
      } catch (err) {
...
async function runTeamMetricsCommand(options) {
  ...
  switch (subcommand) {
    case "report": {
      ...
      console.error(
        `Total issues:   ${stats.totalIssues} (${stats.totalBlocking} blocking, ${stats.totalWarnings} warnings)`,
      );
      ...
      for (const proj of report.projectBreakdown) {
        ...
        console.error(
          `  ${proj.friendlyName}: score=${proj.latestScore} avg=${proj.averageScore} gate=${gate} scans=${proj.scanCount}`,
        );
      }
      return 0;
    }
    case "ingest": {
      ...
      const result = ingestScanHistory(root, history, {
        salt: options.salt || "",
      });
      console.error(
        `[simplebeacon] Ingested ${result.ingested} scan(s) into team metrics (${result.total} total snapshots)`,
      );
      if (result.duplicate) {
        console.error(
          "  (latest scan was already ingested — use --force to override)",
        );
      }
      return 0;
    }
    case "export": {
      ...
      if (!outputPath) {
        console.error(
          "[simplebeacon] Usage: simplebeacon team-metrics export <output-file>",
        );
        return 1;
      }
      const data = exportAnonymizedReport(outputPath);
      console.error(
        `[simplebeacon] Exported anonymized team metrics to ${outputPath}`,
      );
      console.error(`  Projects: ${data.projectBreakdown.length}`);
      console.error(`  Snapshots: ${data.snapshotCount}`);
      console.error(
        `  Avg score: ${data.aggregatedStats.averageQualityScore}/100`,
      );
      return 0;
    }
    case "register": {
      const root = path.resolve(options.projectRoot || process.cwd());
      const name =
        options._positional?.[1] || options.name || path.basename(root);
      const result = registerProject(root, name, options.salt || "");
      console.error(
        `[simplebeacon] Registered project '${name}' (ID: ${result.id})`,
      );
      return 0;
    }
    case "set-team": {
      ...
      if (!teamId) {
        console.error(
          "[simplebeacon] Usage: simplebeacon team-metrics set-team <team-id>",
        );
        return 1;
      }
      setTeamId(teamId);
    ...
    default:
      console.error(
        `[simplebeacon] Unknown team-metrics subcommand: ${subcommand}`,
      );
      console.error(
        "Available: report, ingest, export, register, set-team, clear",
      );
      return 1;
  }
}
...
function runFixCommand(options) {
  ...
  const TEXT_EXTENSIONS =
    /\.(js|jsx|ts|tsx|mjs|cjs|json|md|py|rb|go|rs|java|c|cpp|h|hpp|css|scss|html|vue|svelte)$/;
  const files = [];
  ...
  for (const filePath of files) {
    ...
    for (const ruleId of result.rulesApplied) {
      writeStdoutLine(
        "  " + ruleId + ": " + (result.matchCounts[ruleId] || 0) + " match(es)",
      );
    }
  ...
  if (envVariablesToExport.length > 0 && !dryRun) {
    ...
    console.error(
      "[OK] Appended " +
        envVariablesToExport.length +
        " quarantine definitions to .env",
    );
  } else if (envVariablesToExport.length > 0 && dryRun) {
    console.error(
      "[!] " +
        envVariablesToExport.length +
        " tokens would be extracted to .env on write:",
    );
  ...
  if (dryRun && stats.filesChanged > 0) {
    console.error(
      "No modifications written to disk. Re-run without --dry-run to apply.",
    );
    return 1;
  }
...
async function main() {
  ...
  const skipPolicyGate =
    argvCommand === "secrets-gate" || argvCommand === "fix";
...
async function runAiPlanCommand(options) {
  if (!options || typeof options !== "object")
    throw new TypeError("runAiPlanCommand requires an options object");
  const root = sanitizePath(options.path);
...
function generateAIIssueList(report) {
  ...
  const sortedCategories = Object.entries(groupedIssues).sort((a, b) => {
    ...
    const aTotalCount = Object.values(a[1]).reduce(
      (sum, arr) => sum + arr.length,
      0,
    );
    const bTotalCount = Object.values(b[1]).reduce(
      (sum, arr) => sum + arr.length,
      0,
    );
  ...
  for (const [category, severityGroups] of sortedCategories) {
    ...
    for (const issue of allIssues) {
      ...
      const description =
        issue.description || issue.message || issue.type || "No description";
  ...
  plan +=
    "- Use the `simplebeacon scan --complete` flag to run all 48 analyzers + 25 scan engines\n";
