...
function runSmokeTest() {
  ...
  process.stderr.write(
    `scan_snippet: ${snippet.findingCount} finding(s), ${snippet.blockingCount} blocking\n`,
  );
  if (snippet.findings[0]) {
    process.stderr.write(
      `  → [${snippet.findings[0].severity}] ${snippet.findings[0].description.slice(0, 72)}\n`,
    );
  }

  const gate = JSON.parse(
    handlers.gate_status({ projectRoot, limit: 3 }).content[0].text,
  );
  if (gate.ok) {
    process.stderr.write(
      `gate_status: ${gate.gatePass ? "PASS" : "REVIEW"} (${gate.blockingCount} blocking)\n`,
    );
  } else {
