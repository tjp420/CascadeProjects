...
function parseArgs(argv) {
  ...
  if (!bump) {
    ...
    console.error(
      "Usage: node scripts/release.cjs <patch|minor|major> [--publish] [--otp 123456] [--dry-run]",
    );
    process.exit(1);
  }
  return {
...
function categorizeCommits(log) {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      /^chore|^refactor|^style|^docs|^test|^ci|^build/i.test(subject)
    ) {
...
function generateChangelogEntry(version, bumpType, commits, previousVersion) {
  ...
  lines.push("");
  lines.push(
    `### ${bumpType.charAt(0).toUpperCase() + bumpType.slice(1)} release${previousVersion ? ` from ${previousVersion}` : ""}`,
  );
  lines.push("");
...
function main() {
  ...
  console.log(
    `   ${commits.features.length} features, ${commits.fixes.length} fixes, ${commits.chores.length} chores, ${commits.others.length} others`,
  );
  ...
  if (opts.publish) {
    ...
    if (opts.dryRun) {
      ...
      try {
        ...
        console.error(
          "   npm publish failed. You may need to publish manually.",
        );
        process.exit(1);
      }
    }
    ...
    console.log(
      "8. Skipping npm publish (use --publish --otp <code> to publish).",
    );
  }
