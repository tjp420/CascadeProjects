  ...
  try {
    ...
    if (external.length > 0) {
      ...
      if (process.env.SB_FAIL_ON_EXTERNAL === "1") {
        ...
        console.warn(
          "verify-isolation: external endpoints observed, continuing (SB_FAIL_ON_EXTERNAL!=1)",
        );
      }
    }

    console.log(
      "verify-isolation: no external endpoints detected or non-fatal mode",
    );
    process.exit(0);
  } catch (err) {
    console.error(
      "verify-isolation: error",
