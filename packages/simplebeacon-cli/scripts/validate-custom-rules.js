...
function validateRuleMeta(rule, file) {
  ...
  if (!["low", "medium", "high"].includes(rule.impact))
    throw new Error(`${file}: invalid impact`);
  ...
  for (const f of files) {
    try {
      ...
      for (const rule of rules) {
        if (!isObject(rule))
          throw new Error(
            `${f}: exported value must be an object or array of objects`,
          );
        ...
        if (rule.matcher.type === "regex") {
          ...
          if (!p)
            throw new Error(`${f}: matcher.pattern required for regex rules`);
          ...
          if (!rule.matcher.visitor && !rule.matcher.visitorFile)
            throw new Error(
              `${f}: ast matcher requires visitor or visitorFile`,
            );
        }

        if (!rule.id || typeof rule.id !== "string")
          throw new Error(`${f}: id must be a string`);
