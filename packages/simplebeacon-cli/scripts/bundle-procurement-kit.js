...
function escapeHtmlExceptHtml(str) {
  ...
  return str
    ...
    .replace(/>/g, "&gt;")
    .replace(
      /&lt;(\/)?(h[1-6]|p|a|code|pre|ul|li|strong|em)([^&]*)&gt;/g,
      "<$1$2$3>",
    );
}
...
async function main() {
  ...
  const verifySrc = path.resolve(
    repoRoot,
    ".simplebeacon/artifacts/verify-isolation.json",
  );
  if (fs.existsSync(verifySrc)) {
    ...
    if (fs.existsSync(alt))
      fs.copyFileSync(alt, path.join(tmpDir, "verify-isolation.json"));
  }

  // include dependency security report if produced by CI audit
  const secReport = path.resolve(
    repoRoot,
    ".simplebeacon/dependency-security-report.json",
  );
  if (fs.existsSync(secReport)) {
    fs.copyFileSync(
      secReport,
      path.join(tmpDir, "dependency-security-report.json"),
    );
  }
  ...
  if (fs.existsSync(rulesReadme)) {
    writeHtmlFromMd(
      rulesReadme,
      path.join(tmpDir, "rules-readme.html"),
  ...
  const zipName = path.resolve(
    repoRoot,
    "generated",
    "simplebeacon-procurement-kit.zip",
  );
