...
function importPrivateKey(gpgHome) {
  ...
  if (!keyData.includes("-----BEGIN PGP PRIVATE KEY BLOCK-----")) {
    throw new Error(
      "GPG_PRIVATE_KEY does not contain a valid PGP private key block",
    );
  }
  ...
  if (importResult.status !== 0) {
    throw new Error(
      `GPG key import failed: ${importResult.stderr || importResult.stdout}`,
    );
  }
...
function signFile(gpgHome, keyId, passphrase, filePath) {
  ...
  if (result.status !== 0) {
    throw new Error(
      `GPG signing failed for ${filePath}: ${result.stderr || result.stdout}`,
    );
  }
  ...
  const sigHash = crypto
    .createHash("sha256")
    .update(fs.readFileSync(sigPath))
    .digest("hex");
