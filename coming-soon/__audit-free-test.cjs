...
(async () => {
    ...
    const lockedCount = await page.evaluate(
        () => document.querySelectorAll('#analyzerCardGrid .analyzer-card.locked').length
    );

    // Run a tiny synthetic scan with a file that should trigger a credential hit
    const scanReport = await page.evaluate(async () => {
        const file = new File(['const apiKey = "sk_live_abcdefghijklmnopqrstuvwxyz";'], 'app.js', {
    ...
    if (profileValue !== 'gate') {
        console.error('FAIL: profile select should be gate, got', profileValue);
        failed = true;
    }
    if (firstSelected !== 'gate') {
        console.error('FAIL: first selected module should be gate, got', firstSelected);
        failed = true;
    }
    if (lockedCount === 0) {
        console.error('FAIL: no modules locked for free tier');
        failed = true;
    }
    if (scanReport?.gate?.blockingCount === 0) {
        console.error('FAIL: expected at least one blocking credential finding');
        failed = true;
    }
    if (unexpected.length) {
        console.error('FAIL: unexpected report sections:', unexpected.join(', '));
        failed = true;
    }
