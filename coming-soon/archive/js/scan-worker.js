...
function shouldSkipFile(path) {
    ...
    return /(^|[\/])(node_modules|\.git|\.github|\.husky|dist|build|\.next|out|coverage|frontend-build|\.github-sync|github-cache|\.simplebeacon|\.cursor|\.windsurf|deployments|backups)([\/]|$)/i.test(normalized);
}
...
async function scanFiles(files) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const fileLang = detectDominantLanguage([file.path]);
    ...
    if (textErrors > 0) {
        self.postMessage({ type: 'warn', message: `${textErrors} files could not be read in worker (file.text() unavailable on path-only objects). Main thread handles actual scanning.` });
    }
