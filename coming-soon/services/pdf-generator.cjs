...
function buildExpensePayload(sessionData) {
    ...
    return {
        ...
        complianceDeclaration:
            'Zero Data Custody Verified. No corporate source code or sensitive data was transferred or stored off-site during this assessment. All scans executed 100% locally in the browser sandbox or CLI. Compliant with standard internal proxy/security frameworks. No NDA or DPA required.',
        subtotal: amountUsd,
        total: amountUsd,
        currency: 'USD'
    };
...
function generateReceiptPdf(sessionData) {
    return new Promise((resolve, reject) => {
        ...
        // --- Header ---
        doc.fontSize(20).font('Helvetica-Bold').fillColor('#0a0e18').text(COMPANY.name, 50, 50);
        doc.fontSize(9)
            .font('Helvetica')
            .fillColor('#666')
        ...
        // --- Bill To ---
        const billY = 220;
        doc.fontSize(8).font('Helvetica-Bold').fillColor('#999').text('BILLED TO', 50, billY);
        doc.fontSize(10)
            .font('Helvetica')
            .fillColor('#333')
        ...
        doc.fontSize(7)
            ...
            .fillColor('#999')
            .text(
                'This receipt is system-generated and valid for corporate expense reporting. No signature required.',
                50,
                740,
                { align: 'center', width: 512 }
            )
            .text(COMPANY.legalName + ' · ' + COMPANY.taxId + ' · ' + COMPANY.website, 50, 752, {
                align: 'center',
                width: 512
            });
