...
function renderPreview(data) {
    ...
    if (data.type === 'simplebeacon-npm-audit' && !data.gate) {
        ...
        const pjc1 = data.packageJsonCount ?? 0;
        const dc1 = data.dependencyCount ?? 0;
        data.npmAudit = {
    ...
    const gatePass = hasBlockingFindings ? false : (gate.pass ?? null);
    ...
    const quality =
        ...
        (rawIssues.length ? Math.max(0, 100 - rawIssues.length * 2) : 0);
    const files =
        data.totalFiles ??
        data.repositoryFilesTotal ??
        data.filesAnalyzed ??
        data.summary?.files ??
        data.summary?.repositoryFiles ??
        0;
    ...
    const aiHits = detectedIssues
        ...
        .map(i => i.filePath || '')
        .filter(Boolean);
    const licenseFiles = data.governance?.licenseFiles || data.compliance?.licenseFiles || [];
    const securityFiles = data.governance?.securityFiles || data.compliance?.securityFiles || [];
    const riskAssessmentFiles = data.compliance?.riskAssessmentFiles || [];
    ...
    const analysisModulesHtml = (() => {
        ...
        const totalFiles = data.codebase?.totalFiles || files || 1;
        const totalLines = data.codebase?.totalLines || 0;
        ...
        const derivedPass = hasBlockingFindings ? false : (g.pass ?? null);
        ...
        pushModule(
            ...
            [
                ...
                { value: g.blockingCount ?? 0, label: 'Blocking' }
            ],
            g.summary ||
        ...
        const mockTotal = data.mockSampleFiles ?? mock.reduce((a, c) => a + (c.fileCount || 0), 0);
        ...
        if (br.checklist && br.checklist.length > 0 && !brLooksStale) {
            ...
            readinessScore =
                br.readinessScore ?? Math.round((checks.filter(c => c.found).length / checks.length) * 100);
            readinessStatus =
                br.readinessStatus ||
        ...
        const csScore = data.consistencyScore ?? null;
    ...
    scanPreview.innerHTML = `
        <div class="cert-preview ${hasToken ? '' : 'watermarked'}">
            ...
            <div class="scan-meta" style="margin-top:0;">
                ...
                <div class="meta-item">
                    <div class="value">${data.consistencyScore ?? '--'}</div>
                    <div class="label">Consistency</div>
                </div>
                <div class="meta-item">
                    <div class="value">${data.fictionKpiHits ?? '--'}</div>
                    ...
                    <div style="flex:1;min-width:220px;background:rgba(15,23,42,0.6);border:1px solid rgba(96,165,250,0.15);border-radius:8px;padding:12px;margin-bottom:8px;border-left:3px solid ${severityColor(p.severity)}">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                            <div style="flex:1;height:6px;background:rgba(30,41,59,0.8);border-radius:3px;overflow:hidden;">
                                <div style="width:${p.progress ?? 0}%;height:100%;background:${severityColor(p.severity)};border-radius:3px;transition:width 0.3s;"></div>
                            </div>
                            <span style="font-size:0.65rem;color:#94A3B8;min-width:32px;text-align:right;">${p.progress ?? 0}%</span>
                        </div>
