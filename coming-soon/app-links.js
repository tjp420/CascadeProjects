...
(function () {
    ...
    function applyCheckoutLinks() {
        ...
        document
            .querySelectorAll('[data-agency-checkout="agency_project_pack"], a.checkout-agency-project')
            .forEach(function (el) {
        ...
        document
            .querySelectorAll('[data-agency-checkout="agency_growth_pack"], a.checkout-agency-growth')
            .forEach(function (el) {
        ...
        document.querySelectorAll('.checkout-btn[data-tier]').forEach(function (el) {
            ...
            el.addEventListener('click', function (e) {
                ...
                try {
                    if (
                        el.hasAttribute &&
                        (el.hasAttribute('data-price-id') || el.hasAttribute('data-annual-price-id'))
                    ) {
                        // Record that the modal was opened for debugging
                        try {
                            localStorage.setItem('sb_last_checkout_path', 'modal:' + tierKey);
                        } catch (_) {}
                        try {
                            console.info('[pricing] open modal for tier', tierKey);
                        } catch (_) {}
                ...
                try {
                    localStorage.setItem('sb_last_checkout_path', 'redirect:' + (url || tierKey));
                } catch (_) {}
                try {
                    console.info('[pricing] redirect to', url || tierKey);
                } catch (_) {}
    ...
    function applySampleReportLinks() {
        // simplebeacon-ignore production-leak — links to demo/sample report page
        ...
        if (!sampleUrl.startsWith('/') && window.location.protocol !== 'file:') {
            // simplebeacon-ignore production-leak — URL path normalization
        ...
        document.querySelectorAll('[data-sample-report]').forEach(function (el) {
            // simplebeacon-ignore production-leak — demo report link selector
            el.href = sampleUrl;
            clearLinkHandlers(el);
        });
    ...
    function applyEnvironmentBanner() {
        ...
        banner.innerHTML =
            '<strong>Staging / test environment</strong> — Stripe test checkout only (card ' +
            STRIPE_TEST_CARD_PREFIX +
            '…). ' +
