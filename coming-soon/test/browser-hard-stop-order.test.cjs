...
describe('L2-01 incomplete-drop report contract (qualityScore: 0)', () => {
  for (const rel of [
    ...
    it(`${rel}: gateScore requires MIN_FILES_FOR_PASS (no quality 100 on 1-file drop)`, () => {
      ...
      assert.doesNotMatch(
        src,
        /gateScore\s*=\s*blockingCount\s*===\s*0\s*&&\s*totalFiles\s*>\s*0\s*\?\s*100/
      );
    });
  }
...
describe('D-03 AnalyzeView twin incomplete-drop call-site parity', () => {
  ...
  it('both twins refuse protected-dir inventory ≤2 after local scan', () => {
    for (const rel of [
      ...
      const src = read(rel);
      assert.ok(
        /inventoryCount\s*<=\s*2/.test(src),
        `${rel} missing inventoryCount <= 2 protected-dir guard`
      );
      assert.ok(
  ...
  it('all 5 js AnalyzeView incomplete-drop call sites reject (return or throw)', () => {
    ...
    while ((m = re.exec(src))) {
      ...
      const reject = /\breturn\s*;/.test(body) || /throw\s+new\s+Error\(\s*incompleteFolderDropMessage/.test(body);
