...
function generateToken(email, tier, expiresInMinutes) {
  return jwt.sign(
    { email, tier, jti: 'test-jti-' + Date.now() },
    TEST_SECRET,
...
test('attachTokenToChain creates an attached paid node under the free token', () => {
  ...
  const attachResult = tokenChain.attachTokenToChain(freeHash, paidToken, { email: TEST_EMAIL, tier: 'team' }, 30 * 24 * 60);
...
test('attachTokenToChain fails if free token not in chain', () => {
  const paidToken = generateToken(TEST_EMAIL, 'team', 30 * 24 * 60);
  const result = tokenChain.attachTokenToChain('nonexistent-hash', paidToken, { email: TEST_EMAIL, tier: 'team' }, 30 * 24 * 60);
...
test('paid token validation passes with active subscription and registered token', () => {
  ...
  d.prepare('INSERT INTO customers (email, subscription_status, tier) VALUES (?, ?, ?)')
    .run(TEST_EMAIL, 'active', 'team');

  // Create paid subscription record
  const now = new Date();
  const periodEnd = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString();
  d.prepare('INSERT INTO paid_subscriptions (customer_email, stripe_subscription_id, status, current_period_end) VALUES (?, ?, ?, ?)')
  ...
  const activeSub = d.prepare('SELECT * FROM paid_subscriptions WHERE customer_email = ? AND status = ? ORDER BY current_period_end DESC LIMIT 1')
...
test('paid token validation fails without active subscription', () => {
  ...
  d.prepare('INSERT INTO customers (email, subscription_status, tier) VALUES (?, ?, ?)')
    .run(TEST_EMAIL, 'inactive', 'community');
...
test('expireStaleTokens marks expired tokens', () => {
  ...
  const d = db.getDb();
  d.prepare("UPDATE token_nodes SET expires_at = ? WHERE token_hash = ?")
