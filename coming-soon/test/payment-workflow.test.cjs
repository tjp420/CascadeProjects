...
async function testTokenVerification() {
    ...
    try {
        ...
        fail('Missing token returns 400', e);
    }
    ...
    try {
        ...
        fail('Invalid token returns valid:false', e);
    }
    ...
    try {
        ...
        fail('Valid token verification', e);
    }
...
async function testCheckoutLicenseGeneration() {
    ...
    try {
        ...
        fail('Missing projectName validation', e);
    }
    ...
    try {
        ...
        fail('Token structure validation', e);
    }
...
async function testDashboardAccessControl() {
    ...
    try {
        ...
        assert.strictEqual(r.status, 200);
        assert.ok(
            r.body.includes('Token Authentication') || r.body.includes('tokenInput'),
            'Should contain token form'
        );
        ok('Unlock page loads without authentication');
    } catch (e) {
        fail('Unlock page loads', e);
    }
    ...
    try {
        ...
        fail('Certificate upload page loads', e);
    }
    ...
    try {
        ...
        fail('Dashboard API auth required', e);
    }
    ...
    try {
        ...
        fail('Admin page access control', e);
    }
}
...
async function testEndToEndWorkflow() {
    ...
    try {
        ...
        assert.strictEqual(r.status, 200);
        assert.ok(
            r.body.includes('Developer') || r.body.includes('Startup') || r.body.includes('Growth'),
    ...
    try {
        ...
        fail('Token verification step', e);
    }
    ...
    try {
        ...
        assert.strictEqual(r.status, 200);
        assert.ok(
            r.body.includes('SimpleBeacon') || r.body.includes('dashboard') || r.body.includes('scan'),
            'Should be dashboard content'
        );
        ok('Step 5: Dashboard accessible after token verification');
    } catch (e) {
        fail('Dashboard access step', e);
    }
