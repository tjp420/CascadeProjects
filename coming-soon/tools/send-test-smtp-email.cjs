...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--to' && argv[i + 1]) {
      ...
      console.log('Usage: node tools/send-test-smtp-email.cjs --to you@example.com [--from from@domain] [--verify]');
...
async function main() {
  ...
  const text = 'This is a test email from SimpleBeacon sent via Zoho Mail SMTP.\n\nIf you received this, SMTP is configured correctly.\n\nConfiguration:\n  Host: ' + host + '\n  Port: ' + port + '\n  User: ' + user + '\n  From: ' + fromAddr;
  const html = '<p>This is a test email from SimpleBeacon sent via <strong>Zoho Mail SMTP</strong>.</p><p>If you received this, SMTP is configured correctly.</p><hr><p><small>Host: ' + host + ':' + port + ' | User: ' + user + ' | From: ' + fromAddr + '</small></p>';
