...
function buildFallbackEmailHtml(clientName, email, projectName, config, token, certUrl) {
    ...
    return (
        '<p>Hi ' +
        escapeHtml(clientName || email) +
        ',</p>' +
        '<p>Your license token for <strong>' +
        ...
        s.ctaRadius +
        ';">Upload Report &amp; Download Certificate</a></p>' +
        '<p>This token expires in ' +
        config.days +
        ' days.</p>' +
        '<p style="color:' +
        s.mutedColor +
        ...
        ';margin-top:' +
        s.finePrintMargin +
        ";\"><strong>Didn't see this email?</strong> Check your spam or junk folder — license tokens sometimes end up there. If it's missing, contact support.</p>"
    );
}

const DEFAULT_PORT = 3001;
const PUBLIC_URL = process.env.PUBLIC_URL || 'http://' + 'localhost' + ':' + (process.env.PORT || DEFAULT_PORT);
const CHECKOUT_WEBHOOK_SOURCE = 'checkout-webhook';
...
function getTestCheckoutRateKey(req) {
    const fallback = req.ip || req.socket.remoteAddress || 'unknown';
    if (process.env.NODE_ENV === 'production') return fallback;
    const override = req.get('x-test-checkout-client');
    if (
        typeof override === 'string' &&
        override.length > 0 &&
        override.length <= 64 &&
        /^[A-Za-z0-9._:-]+$/.test(override)
    ) {
...
router.post('/api/test-checkout', async (req, res) => {
    try {
        ...
        if (isPaidTier && process.env.SIMPLEBEACON_DEMO_MODE !== 'true') {
            return res
                .status(403)
                .json({ error: 'Paid tiers require Stripe checkout. Use /api/create-checkout-session instead.' });
        }
        ...
        const rateKey = getTestCheckoutRateKey(req);
        const now = Date.now();
        const tcEntry = testCheckoutRateLog.get(rateKey);
        if (tcEntry && now < tcEntry.resetAt) {
            ...
            testCheckoutRateLog.set(rateKey, { count: 1, resetAt: now + TEST_CHECKOUT_RATE_LIMIT_MS });
        }
        ...
        try {
            ...
            const stepsMap = {
                ...
                executive_clearance:
                    '<li>Click the button above to open your dashboard</li><li>Paste your license token (already filled if you use the link)</li><li>Upload your source code or select a local directory</li><li>Run the Complete Scan with all 15 analyzers</li><li>Download your Executive Risk Certificate + Audit Report ZIP</li>',
                eu_ai_act_sprint:
                    '<li>Click the button above to open your dashboard</li><li>Paste your license token (already filled if you use the link)</li><li>Upload your source code zip or select a local directory</li><li>Run the EU AI Act compliance scan</li><li>Download your EU AI Act Readiness PDF instantly</li>',
                runtime_shield:
                    '<li>Click the button above to open your dashboard</li><li>Paste your license token (already filled if you use the link)</li><li>Configure runtime sentinel for your stack</li><li>Set per-request and per-minute AI API spend caps</li><li>Monitor real-time spend dashboard with alerts</li>'
            };
            const featuresMap = {
                instant_report:
                    '<li>Code Hygiene Gate scan</li><li>Production leak detection</li><li>Mock data / fixture detection</li><li>Instant PDF certificate</li>',
                executive_clearance:
                    '<li>Complete Scan (15 analyzers)</li><li>Codebase analysis + npm audit</li><li>Compliance checklist + EU AI Act</li><li>Executive audit report + certificate ZIP</li><li>Re-attestation support</li>',
                eu_ai_act_sprint:
                    '<li>EU AI Act Article 52-55 assessment</li><li>Risk classification (minimal / limited / high / unacceptable)</li><li>Conformity gap analysis</li><li>Remediation roadmap</li><li>Ready-to-submit documentation</li>',
                runtime_shield:
                    '<li>Runtime sentinel library + middleware</li><li>Per-request, per-minute, per-hour spend caps</li><li>Real-time dashboard + Slack/PagerDuty alerts</li><li>Custom budget-policy rules</li><li>Dedicated cost-governance onboarding</li>'
            };
            const deliveryMap = {
                ...
                eu_ai_act_sprint: {
                    visible: 'visible',
                    headline: 'Manual review included',
                    detail: 'A compliance analyst will review your scan within 24 hours and email a finalized attestation package.'
                },
                runtime_shield: {
                    visible: 'visible',
                    headline: 'Onboarding scheduled',
                    detail: 'A Solutions Engineer will contact you within 1 business day to begin runtime integration.'
                }
            };
            ...
            template = template
                ...
                .replace(
                    /\{\{INVOICE_LINE\}\}/g,
                    safe(
                        isFree ? 'Community tier — no invoice' : 'Invoice #INV-' + Date.now().toString(36).toUpperCase()
                    )
                )
                ...
                .replace(
                    /\{\{PRIMARY_CTA\}\}/g,
                    safe(isFree ? 'Get Started →' : 'Upload Report & Download Certificate')
                )
                ...
                .replace(
                    /\{\{DELIVERY_STYLE\}\}/g,
                    safe(d.visible === 'visible' ? 'style="display:block;"' : 'style="display:none;"')
                )
...
function setupCheckoutWebhook(app) {
    app.post('/api/checkout/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
        ...
        if (event && event.id) {
            const inserted = db.recordWebhookEvent(event.id, CHECKOUT_WEBHOOK_SOURCE, event.type, payloadHash);
            if (!inserted) {
        ...
        try {
            ...
            template = template
                ...
                .replace(
                    /\{\{STEPS_LIST\}\}/g,
                    safe(
                        '<li>Click the button above to open the certificate upload page</li><li>Paste your license token (already filled if you use the link)</li><li>Upload your source code zip or select a local directory</li><li>The scan runs locally — no code leaves your machine</li><li>Download your certificate instantly</li>'
                    )
                )
                .replace(/\{\{FEATURES_STYLE\}\}/g, safe('style="display:block;"'))
                .replace(
                    ...
                    safe(
                        scans
                            .map(s => {
                                const opt = SCAN_OPTION_MAP[s];
                                return '<li>' + (opt ? opt.name : s) + '</li>';
                            })
                            .join('') || '<li>Custom Audit Plan</li>'
                    )
                )
        ...
        try {
            ...
            if (referralResult.converted) {
                logger.info(
                    '[CheckoutWebhook] Referral conversion:',
                    referralResult.attributionId,
                    referralResult.rewardId
                );
            }
        } catch (referralErr) {
            logger.error('[CheckoutWebhook] Referral attribution failed:', referralErr.message);
        }
