...
(function () {
    ...
    function applyTopicFromQuery() {
        ...
        var topic = String(params.get('topic') || params.get('subject') || '')
            .trim()
            .toLowerCase();
        ...
        fetch('/api/contact', {
            ...
            .then(function (res) {
                if (res.ok) {
                    ...
                    toggleInvoiceFields();
                    setStatus(
                        'Message sent — we reply within one business day. Check your inbox for our response.',
                        'success'
                    );
                    return;
                }
            ...
            .catch(function (error) {
                setStatus(
                    'Could not send your message. ' + (error.message || 'Please try again or email us directly.'),
                    'error'
                );
            })
            .finally(function () {
    ...
    if (handoffRoot) {
        var devHandoff =
            /localhost|127\.0\.0\.1/.test(window.location.hostname) || window.location.search.indexOf('dev=1') !== -1;
    ...
    if (!handoffRoot || handoffRoot.hidden) {
        ...
        function loadHandoffFields() {
            ...
            if (data.files) {
                var el = document.getElementById('handoffFiles');
                if (el) el.value = data.files;
            }
            if (data.decisions) {
                var el = document.getElementById('handoffDecisions');
                if (el) el.value = data.decisions;
            }
            if (data.pending) {
                var el = document.getElementById('handoffPending');
                if (el) el.value = data.pending;
            }
            if (data.notes) {
                var el = document.getElementById('handoffNotes');
        ...
        function smartFillHandoff() {
            ...
            if (pendingEl && !pendingEl.value) {
                pendingEl.value =
                    '- Review and verify all changes before next session\n- Run node --test if tests exist';
            }
            updateCounts();
        }
        ...
        function generateHandoffReport() {
            var files = String(document.getElementById('handoffFiles')?.value || '')
                .split('\n')
                .map(function (s) {
                    return s.trim();
                })
                .filter(Boolean);
            var decisions = String(document.getElementById('handoffDecisions')?.value || '')
                .split('\n')
                .map(function (s) {
                    return s.trim();
                })
                .filter(function (s) {
                    return s.length > 0 && !s.match(/^\s*[-*]\s*$/);
                });
            var pending = String(document.getElementById('handoffPending')?.value || '')
                .split('\n')
                .map(function (s) {
                    return s.trim();
                })
            ...
            var notes = String(document.getElementById('handoffNotes')?.value || '')
                .split('\n')
                .map(function (s) {
                    return s.trim();
                })
            ...
            var report = {
                ...
                estimatedContextTokens: Math.round(
                    JSON.stringify({ files: files, decisions: decisions, pending: pending, notes: notes }).length / 4
                )
            };
            ...
            if (handoffStats) {
                handoffStats.innerHTML =
                    '<span><strong style="color:var(--text);">' +
                    files.length +
                    '</strong> files</span><span><strong style="color:var(--text);">' +
                    decisions.length +
                    '</strong> decisions</span><span><strong style="color:var(--text);">' +
                    pending.length +
                    '</strong> tasks</span><span><strong style="color:var(--text);">' +
                    report.estimatedContextTokens +
                    '</strong> est. tokens</span>';
        ...
        function copyMarkdownToClipboard() {
            ...
            navigator.clipboard
                .writeText(currentHandoffMarkdown)
                .then(function () {
