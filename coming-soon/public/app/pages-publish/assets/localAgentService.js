...
function $e(e, t = 1, n = {}) {
  ...
  const a =
      n.scheme ||
      (typeof navigator < "u" && /Cursor/i.test(navigator.userAgent || "")
        ? "cursor"
        : "vscode"),
    o = Ee(r),
...
function Ve() {
  ...
  if (e)
    try {
      Y(new URL(String(e).replace(/\/api\/?$/, "")).origin) &&
        (P({ updateUrl: !0 }), (t = !0));
    } catch {}
  if (
    typeof window < "u" &&
    window.__SB_BRIDGE_HOST__ &&
    Y(window.__SB_BRIDGE_HOST__)
  ) {
    try {
...
function G() {
  ...
  if (typeof sessionStorage < "u")
    try {
      return (
        sessionStorage.getItem(ce) || sessionStorage.getItem("sb_notify_base")
      );
    } catch {}
  return null;
}
function b() {
  ...
  try {
    ...
    return (r !== "127.0.0.1" && r !== "localhost") || ($() && Y(n.origin))
      ? null
      : n.origin;
  } catch {
    return null;
...
function T() {
  ...
  return (
    /^(localhost|127\.0\.0\.1|\[::1\])$/i.test(e) &&
    window.location.protocol === "http:"
  );
}
function ge(e) {
...
async function Ke(e = y) {
  ...
  const t = String(e || y).replace(/\/$/, ""),
    n =
      typeof window < "u" ? window.location.origin : "https://simplebeacon.ai",
    r = C(),
    a = M(),
    o = a ? await V() : { ok: !1 },
    s = Ne(t);
      ...
      : o.ok && c.bridgeReachable && c.error
        ...
        : o.ok
          ? {
              ...
              error:
                c.error ||
                "Extension bridge could not reach Ollama. Run ollama serve locally.",
            }
          : {
              ok: !1,
...
async function Qe(e) {
  ...
  try {
    ...
    if (
      o.workspace &&
      (
        String(o.workspace).replace(/\\/g, "/").split("/").pop() || ""
      ).toLowerCase() === String(e).toLowerCase()
    )
...
async function Ye() {
  ...
  try {
    ...
    return !r.ok || a.success !== !0
      ? null
      : String(a.path || "").trim() || null;
  } catch {
    return null;
  }
}
...
async function tt(e, t = {}, n = z) {
  const r = await _(`${n}/inventory`, {
      ...
      body: JSON.stringify({
        projectPath: e,
        fullDirectoryScan: t.fullDirectoryScan,
      }),
    }),
    a = await r.json().catch(() => ({}));
  if (!r.ok || !a.success)
    throw new Error(a.error || `Agent inventory failed (${r.status})`);
  return a.inventory;
}
async function nt(e, t = {}, n = z) {
  ...
  const o = await _(
      `${r}/scan`,
      {
        ...
        body: JSON.stringify({
          projectPath: e,
          fullDirectoryScan: a.fullDirectoryScan === !0,
        }),
      },
      6e5,
    ),
  ...
  if (!o.ok || !s.success)
    throw new Error(s.error || `Agent scan failed (${o.status})`);
  return s.report;
}
      ...
      if (
        !(
          (
            await (
              await n(
                `${e}/api/simplebeacon/scan/progress?projectPath=${encodeURIComponent(t)}`,
              )
            )
              .json()
              .catch(() => ({}))
...
async function st(e, t = ee()) {
  if (me(t)) {
    ...
    if (!o.ok || !s.success)
      throw new Error(
        s.error || s.warning || `Extension scan failed (${o.status})`,
      );
    return s.scanning
      ? {
          success: !0,
  ...
  const n = await _(`${t}/api/analyze`, {
    ...
    r = await n.json().catch(() => ({}));
  if (!n.ok || !r.success)
    throw new Error(r.error || `Agent scan failed (${n.status})`);
  return r;
}
function it(e, t) {
  if (!t) return;
  ...
  ((W.className = "sb-compliance-title"),
    (W.textContent = "Compliance summary"));
  ...
  if (
    ...
    ((d.className = "sb-compliance-path"),
      (d.textContent = l),
      i.appendChild(d));
  }
  ...
  if (
    ...
    ((d.className = "sb-compliance-files"),
      (d.open = a.length > 0 && a.length <= 12));
    const m = document.createElement("summary");
    ((m.textContent = `File inventory (${a.length} flagged · ${o} clean)`),
      d.appendChild(m));
    const h = document.createElement("div");
    ...
    const oe = () => {
      ...
      v === "issues"
        ? (S = a)
        : v === "clean" &&
          (S = r.filter((u) => !u.status || u.status === "Clean"));
      ...
      if (
        (S.slice(0, w).forEach((u) => {
          ...
          ((Z.className = "sb-compliance-file-badge"),
            (Z.textContent = se ? "Clean" : String(u.status)));
...
function ct(e, t) {
  return !(t != null && t.available) || !(t != null && t.scannerAvailable)
    ? !1
    : Re(e);
}
function k() {
