...
const ALERT_TEMPLATES = {
    ...
    'SB-SEC-015': {
        ...
        references: ['https://learn.microsoft.com/en-us/azure/key-vault/general/best-practices'],
        rotationRequired: true,
        cwe: 'CWE-798: Use of Hard-coded Credentials'
    },
    'SB-SEC-016': {
...
export function enrichFindingWithAlert(finding) {
    ...
    const ruleId =
        finding.pattern || finding.patternId || (finding.id ? finding.id.split('-').slice(0, 3).join('-') : null);
...
export function renderAlertCard(finding) {
    ...
    const refs = t.references
        .map(r => `<a href="${r}" target="_blank" rel="noopener" style="color:#3B82F6;">${r}</a>`)
        .join('<br>');
