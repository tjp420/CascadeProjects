...
test.describe("Storage Quota Fallback Loop", () => {
  ...
  test("should gracefully fall back to IndexedDB when localStorage hits quota limits", async ({
    page,
  }) => {
    ...
    await page.evaluate(async () => {
      ...
      // @ts-ignore
      if (
        window.SimpleBeaconStorage &&
        typeof window.SimpleBeaconStorage.saveReport === "function"
      ) {
        // @ts-ignore
        await window.SimpleBeaconStorage.saveReport(
          "test-70k-payload",
          mockReport,
        );
      } else {
