...
async function injectAuth(page: Page, opts: InjectOptions) {
  await page.addInitScript((args: InjectOptions) => {
    ...
    const payload = btoa(
      JSON.stringify({ exp: 99999999999, email: "test@simplebeacon.ai" }),
    );
    ...
    if (args.batchHistory) {
      localStorage.setItem(
        "sb_telemetry_batches",
        JSON.stringify(args.batchHistory),
      );
    }
    if (args.complianceHistory) {
      localStorage.setItem(
        "sb_telemetry_compliance",
        JSON.stringify(args.complianceHistory),
      );
    }
    if (args.resolutionHistory) {
      localStorage.setItem(
        "sb_telemetry_resolution",
        JSON.stringify(args.resolutionHistory),
      );
    }
  }, opts);
}
...
test.describe("Advanced Telemetry — Free-tier paywall", () => {
  test("1. Free-tier user sees Team Pro paywall on telemetry page", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("EU AI Act Mapping is a Team Pro Feature"),
    ).toBeVisible();
  });

  test("2. Free-tier user does NOT see batch execution stats", async ({
    page,
  }) => {
    await injectAuth(page, { plan: "free" });
    await page.goto(TELEMETRY_URL);
    ...
    await expect(
      page.getByText("Multi-File Batch Execution"),
    ).not.toBeVisible();
  });
});

test.describe("Advanced Telemetry — Team Pro full access", () => {
  test("3. Team Pro user sees the full telemetry dashboard", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Advanced Telemetry" }),
    ).toBeVisible();
  ...
  test("4. Batch execution stat cards show correct values", async ({
    page,
  }) => {
    await injectAuth(page, {
      plan: "team_pro",
  ...
  test("5. Compliance trend section renders with score cards", async ({
    page,
  }) => {
    await injectAuth(page, {
      plan: "team_pro",
  ...
  test("6. Issue resolution tracking renders with resolution rate", async ({
    page,
  }) => {
    await injectAuth(page, {
      plan: "team_pro",
  ...
  test("8. Team Pro user with no telemetry data sees empty state", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Run First Scan" }),
    ).toBeVisible();
  });
});
