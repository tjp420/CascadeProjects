...
async function injectAuth(page: Page, opts: InjectOptions) {
  await page.addInitScript((args: InjectOptions) => {
    ...
    if (args.scanResult) {
      localStorage.setItem(
        "sb_last_scan_full",
        JSON.stringify(args.scanResult),
      );
    }
  }, opts);
}
...
test.describe("Free-tier scan paywall", () => {
  test("1. Free-tier user with 0 scans sees the Start Scan button (not paywalled)", async ({
    page,
  }) => {
  ...
  test('2. Free-tier user with 2 scans sees "1 of 3 free scans remaining this month"', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("1 of 3 free scans remaining this month"),
    ).toBeVisible();
  });
  ...
  test('3. Free-tier user with 3 scans (limit reached) sees the "Free Scan Limit Reached" paywall overlay', async ({
    ...
    await expect(
      page.getByRole("button", { name: "Start Scan" }),
    ).not.toBeVisible();
  ...
  test('4. The paywall overlay contains "Upgrade to Developer — $49/mo" CTA', async ({
    page,
  }) => {
    await injectAuth(page, { plan: "free", scanCount: 3 });
    await mockApiEndpoints(page, 3);
...
test.describe("Free-tier feature paywalls", () => {
  test("5. Export tab shows CI gate paywall for free-tier users", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("CI/CD Integration is a Developer Feature"),
    ).toBeVisible();
  });
...
test.describe("Developer-tier access", () => {
  test("7. Developer-tier user does NOT see the scan limit counter (unlimited)", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/free scans remaining this month/),
    ).not.toBeVisible();
  });

  test("8. Developer-tier user CAN see the Export tab content (no CI gate paywall)", async ({
    page,
  }) => {
    await injectAuth(page, { plan: "developer" });
    await mockApiEndpoints(page, 0);
    ...
    await expect(
      page.getByRole("button", { name: "JSON Report" }),
    ).toBeVisible();
  });
});
