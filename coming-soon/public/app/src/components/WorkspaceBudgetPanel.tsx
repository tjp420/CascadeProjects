                                ...
                                {display.alerts && display.alerts.length > 0 && (
                                    <div className="mb-4 flex flex-wrap gap-1">
                                        {display.alerts.slice(-3).map((a, i) => (
                                            <Badge
                                                key={i}
                                                variant={a.type === 'hard_stop' ? 'destructive' : 'secondary'}
                                                className="text-xs"
                                            >
                                ...
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                    <BudgetField label="Limit (USD)">
                                        <Input
                                            ...
                                            disabled={!isAdmin}
                                            onChange={e =>
                                                updateDraft(b.scope, { limitUSD: parseFloat(e.target.value) })
                                            }
                                        />
                                    </BudgetField>
                                    ...
                                    <div className="flex items-end gap-4">
                                        <label className="flex cursor-pointer items-center gap-2 text-sm">
                                            <input
                                                ...
                                                disabled={!isAdmin}
                                                onChange={e =>
                                                    updateDraftConfig(b.scope, { autoResetEnabled: e.target.checked })
                                                }
                                            />
                                            Auto reset
                                        </label>
