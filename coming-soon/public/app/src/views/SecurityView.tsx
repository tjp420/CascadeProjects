...
function categoryIcon(cat: string) {
    ...
    if (c.includes('prototype') || c.includes('pollution')) return <ShieldAlert className="h-4 w-4" />;
...
export function SecurityView() {
    ...
    const vulnBySev = npmAudit?.vulnerabilities?.bySeverity ?? npmAudit?.metadata?.vulnerabilities ?? {};
                                    ...
                                    <div key={adv.id || i} className="rounded-lg border p-3">
                                        ...
                                        <div className="mt-1 flex items-center gap-4 text-xs text-foreground-muted">
                                            ...
                                            {adv.vulnerableVersions && (
                                                <span>
                                                    Affected:{' '}
                                                    <code className="font-mono">{adv.vulnerableVersions}</code>
                                                </span>
                                            )}
                                            {adv.patchedVersions && (
                                                <span>
                ...
                <Card>
                    <CardContent className="flex flex-col items-center gap-3 py-12">
                        ...
                        <p className="text-sm text-foreground-muted text-center max-w-md">
                            Provider failover, identity federation, semantic cache, webhook signing, agentic
                            orchestration, and tool schema validation are available on the Enterprise plan.
                        </p>
