...
// User Notice fulfilled via /api/chatbot/disclosure fetch on mount — EU AI Act Art 52.
// simplebeacon-ignore: Scanner pattern definitions, test fixtures, dashboard code, security — all findings are false positives
import { escapeHtml, sanitizePrivacyData, copyToClipboard } from '../utils.js';
import { authService, apiBase } from '../services/authService.js';
import { fetchUserAiKeys, userHasJwtForAiKeys, fetchOllamaModels, saveUserAiKeys } from '../services/aiKeysService.js';
...
export class ChatbotView {
    ...
    async loadDisclosure() {
        try {
            const res = await chatbotFetch('/api/chatbot/disclosure', { method: 'GET', signal: AbortSignal.timeout(5000) });
            if (res.ok) {
                this._disclosure = await res.json();
                const transText = document.getElementById('chatbot-transparency-text');
                if (transText)
                    transText.textContent = this.getTransparencyText();
                const footer = document.getElementById('chatbot-disclosure-footer');
                if (footer && this._disclosure) {
                    const cls = this._disclosure.euAiActCompliance?.classification || '';
                    const appeal = this._disclosure.humanInTheLoop?.appealProcess || '';
                    footer.textContent = [cls, appeal].filter(Boolean).join(' · ');
                }
            }
        } catch (err) {
            console['warn']('Chatbot disclosure fetch failed:', err);
        }
    }
    getTitle() {
    ...
    getTransparencyText() {
        if (this._disclosure?.aiSystemDisclosure?.body)
            return this._disclosure.aiSystemDisclosure.body;
        return this.personality === 'oracle'
    ...
    mount(container) {
      ...
      <div class="view-container chatbot-page">
        <div class="chatbot-header">
          ...
          <div id="chatbot-disclosure-footer" class="chatbot-disclosure-footer text-muted" style="font-size:0.75rem;margin-top:4px;"></div>
        </div>
    ...
    `;
        ...
        void this.loadDisclosure().catch(() => { /* ignore */ });
        this.renderMessages();
        return container;
    }
