...
function getTokenRegistry() {
    try {
        var raw = localStorage.getItem('sb-token-registry');
        return raw ? JSON.parse(raw) : {};
    } catch {
        return {};
    }
...
export class ProfileView {
    ...
    mount(container) {
        ...
        var stockpiledRows = listStockpiled(token)
            .map(function (ref) {
                var entry = ref.entry,
                    index = ref.index;
                var meta = entry.meta || decodeTokenMeta(entry.token);
                return (
                    '<div class="profile-stockpile-row" data-stockpile-index="' +
                    index +
                    '">' +
                    '<div class="profile-stockpile-meta"><code>' +
                    escapeHtml(tokenHint(entry.token)) +
                    '</code>' +
                    '<span class="profile-stockpile-tier">' +
                    escapeHtml(String(meta.tier)) +
                    '</span>' +
                    '<span class="profile-stockpile-expiry">expires ' +
                    escapeHtml(meta.expiresLabel) +
                    '</span></div>' +
                    '<button type="button" class="btn btn-secondary btn-sm profile-stockpile-load" data-stockpile-load="' +
                    index +
                    '">Load</button></div>'
                );
            })
        ...
        var avatarHtml =
            user && (user.avatarUrl || user.picture)
                ? '<img class="profile-avatar-img" src="' +
                  escapeHtml(user.avatarUrl || user.picture || '') +
                  '" alt="Avatar" />'
                : email
        ...
        var fragment = document
            ...
            .createContextualFragment(
                '<div class="profile-page profile-page-v2">' +
                    ...
                    '<div class="profile-stat-item"><span class="stat-label">Token</span><span class="stat-value">' +
                    escapeHtml(tokenType) +
                    '</span></div>' +
                    '<div class="profile-stat-item"><span class="stat-label">Project</span><span class="stat-value">' +
                    escapeHtml(project) +
                    '</span></div>' +
                    '<div class="profile-stat-item"><span class="stat-label">Age</span><span class="stat-value">' +
                    escapeHtml(accountAge) +
                    '</span></div>' +
                    '<div class="profile-stat-item"><span class="stat-label">Expires</span><span class="stat-value" style="color:' +
                    expiryInfo.color +
                    ';">' +
                    escapeHtml(expiryInfo.label) +
                    '</span></div>' +
                    '</div>' +
                    ...
                    '<label class="login-method-card ' +
                    (loginMethod === 'token' ? 'active' : '') +
                    '">' +
                    '<input type="radio" name="loginMethod" value="token" ' +
                    (loginMethod === 'token' ? 'checked' : '') +
                    '>' +
                    ...
                    '<label class="login-method-card ' +
                    (loginMethod === 'both' ? 'active' : '') +
                    '">' +
                    '<input type="radio" name="loginMethod" value="both" ' +
                    (loginMethod === 'both' ? 'checked' : '') +
                    '>' +
                    ...
                    '<input type="password" id="profile-email-password" value="' +
                    escapeHtml(profile.emailPassword || '') +
                    '" placeholder="Set a password for email login…" autocomplete="new-password"></div>' +
                    '<div class="profile-divider"></div>' +
                    '<div class="profile-field"><label for="profile-token">License Token</label>' +
                    '<div class="profile-input-group">' +
                    '<input type="password" id="profile-token" value="' +
                    escapeHtml(token) +
                    '" placeholder="Paste your license token…" autocomplete="off">' +
                    ...
                    '<input type="password" id="profile-token-password" value="' +
                    escapeHtml(profile.tokenPassword || '') +
                    '" placeholder="Set a password for token login…" autocomplete="new-password"></div>' +
                    '</div></div>' +
                    '<div class="profile-card" id="profile-stockpile-card">' +
                    '<div class="profile-card-header"><i data-lucide="layers" class="icon-18 profile-card-icon"></i><h2>Token Loader (' +
                    reservedCount +
                    ' reserved)</h2></div>' +
                    '<div class="profile-card-body">' +
                    '<div class="profile-input-group">' +
                    ...
                    (reservedCount > 0
                        ? '<div class="profile-stockpile-list">' + stockpiledRows + '</div>'
                        : '<p class="profile-help">No reserved tokens yet.</p>') +
        ...
        ['#profile-email', '#profile-email-password', '#profile-token', '#profile-token-password'].forEach(
            function (sel) {
        ...
        if (saveBtn)
            saveBtn.addEventListener('click', function () {
                ...
                if (tokenInput && tokenInput.value.trim())
                    localStorage.setItem('cascadeAuthToken', tokenInput.value.trim());
            ...
            copyBtn.addEventListener(
                ...
                async function () {
                    ...
                    if (!input || !input.value) {
                        if (this.app.showToast) this.app.showToast('No token to copy', 'error');
                        return;
                    }
                    var copied = false;
                    ...
                    if (copied) {
                        ...
                        setTimeout(function () {
                            if (typeof window.setSafeHTML === 'function')
                                window.setSafeHTML(copyBtn, '<i data-lucide="copy" class="icon-16"></i>');
                            if (typeof window.lucide !== 'undefined') window.lucide.createIcons();
                        }, 1500);
            ...
            clearBtn.addEventListener(
                ...
                function () {
                    Object.keys(localStorage)
                        .filter(function (k) {
                            return k.startsWith('sb_') || k.includes('simplebeacon');
                        })
                        .forEach(function (k) {
                            localStorage.removeItem(k);
                        });
        ...
        if (stockpileAddBtn)
            stockpileAddBtn.addEventListener(
                'click',
                function () {
                    ...
                    if (result.ok) {
                        showToast(
                            result.duplicate ? 'Token already stockpiled' : 'Time token added to loader',
                            'success'
                        );
                        if (input) input.value = '';
        ...
        // Stockpile load
        container.querySelectorAll('[data-stockpile-load]').forEach(
            function (btn) {
