...
export class PlatformService {
  ...
  async refreshNpmAudit(options = {}) {
    ...
    if (options.force) {
      ...
      const npmAuditPayload = await readJsonResponseBody(
        npmAuditHttpResponse,
        {},
      );
      if (!npmAuditHttpResponse.ok) {
        throw new Error(
          npmAuditPayload.message ||
            npmAuditPayload.error ||
            "npm audit failed",
        );
      }
  ...
  async runBaselineSync() {
    const syncHttpResponse = await fetch(
    ...
    if (!syncHttpResponse.ok)
      throw new Error(
        syncResponseBody.message ||
          syncResponseBody.error ||
  ...
  async fetchMergerReductionScan(projectPath) {
    ...
    const data = await fetchJson(
      `/api/merger-tool/reduction-scan?${params}`,
      180000,
    );
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description:
          "Deterministic gate on .simplebeacon/config.json scanPaths and all rules",
      },
      {
        id: "mock-scan",
        ...
        description:
          "Fiction/KPI digest scoped to sample paths — not a full-repo semantic walk",
      },
      {
        id: "consolidation",
        ...
        description:
          "Duplicate JSON groups, similar schemas, and oversized files in sample paths",
      },
      {
        id: "codebase",
      ...
      },
      {
        ...
        analyzeMode: "auto",
        description:
          "Picks Simplebeacon for platform or mock paths; roadmap for other directories",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        route: "results",
        description:
          "Filter by severity and category — empty with gate PASS means a clean scan",
      },
      {
        id: "audit",
      ...
      },
      {
        ...
        route: "security",
        description:
          "Credential patterns and production-leak findings from live Simplebeacon scan",
      },
      {
        id: "quality",
    ...
    items: [
      {
        ...
        route: "tools",
        description:
          "Run scans, sync baseline, npm audit, dev tools catalog, and CI workflows",
      },
      {
        id: "settings",
        label: "Settings",
      ...
      },
      {
        ...
        route: "help",
        description:
          "Scan number glossary, dashboard pages, CLI commands, and FAQ",
      },
    ],
  },
];
