...
function collectReasoningTasks(input = {}) {
    if (Array.isArray(input.reasoningTasks) && input.reasoningTasks.length) {
        return input.reasoningTasks.map(row => {
            ...
            const fallacyHits =
                REASONING_FALLACY_MARKERS.filter(pattern => pattern.test(text)).length + Number(row.fallacyCount ?? 0);
...
function runResponseLatencyAnalyzer(definition, issueId, input = {}) {
    ...
    if (isScanReportContext(input)) {
        ...
        const scanDurationMs = Number(
            metrics.scanDurationMs ?? metrics.dataQualityScanDurationMs ?? input.fileReduction?.durationMs ?? 0
        );
...
function deriveTracesFromContext(context = {}) {
    ...
    for (const item of insights.slice(0, 8)) {
        const reason = item.understanding?.summary || item.understanding?.layers?.semantic?.purpose || '';
        if (!reason) continue;
        traces.push({
            decision: 'analyze',
            reason,
...
export function enrichAnalyzerContextFromCodeInsights(context = {}) {
    ...
    if (understanding?.fileInsights?.length) {
        const lines = understanding.fileInsights
            .map(item => {
                const summary = item.understanding?.summary || item.understanding?.layers?.semantic?.purpose || '';
