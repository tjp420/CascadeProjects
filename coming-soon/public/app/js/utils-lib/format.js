...
const AI_SKIP_PATTERNS = [
  {
    test: /openai is not configured/i,
    msg: "add your OpenAI key in Settings → AI providers",
  },
  {
    test: /anthropic is not configured/i,
    msg: "add your Anthropic key in Settings → AI providers",
  },
  {
...
export function sanitizePrivacyData(text) {
  ...
  cleaned = cleaned.replace(
    /\b(sk-[a-zA-Z0-9]{20,}|sk-proj-[a-zA-Z0-9_\-]{20,})/gi,
    "[REDACTED_OPENAI_KEY]",
  );
...
const REDACTION_PATTERNS = [
  {
    regex: /^(?:…|\.{3})\/[^/]+(\/.+)?$/,
    replace: (m, rest) => (rest ? `…${rest}` : "…"),
  },
  {
    regex: /^[a-zA-Z]:\/Users\/[^/]+(\/.+)?$/i,
    replace: (m, rest) => (rest ? `…${rest}` : "…"),
  },
  {
    regex: /^\/Users\/[^/]+(\/.+)?$/,
...
export function formatScanPathForDisplay(scanPath, projectRoot) {
  ...
  const rawRoot =
    typeof projectRoot === "string" ? normalizeSlashes(projectRoot) : "";
