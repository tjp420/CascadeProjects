...
function buildProductHygieneSummary(report, options = {}) {
    ...
    const jestExecuted = report.jestBaselineChecked !== false && report.scanScope?.jestExecutedDuringScan !== false;
    const jestLabel =
        jestExecuted && jestSummary?.testsTotal != null ? `${jestSummary.testsPassed}/${jestSummary.testsTotal}` : null;
    ...
    const gateRepositoryFilesTotal =
        options.gateRepositoryFilesTotal ?? options.repositoryFilesTotal ?? repositoryFilesTotal ?? null;
...
function buildBenchmarkGateExportNotes(report, context = {}) {
    ...
    const excluded = report.scanScope?.benchmarkCloneNoiseExcluded ?? report.benchmarkCloneNoiseIssues?.length ?? 0;
    ...
    const rawLlm =
        report.llmSlopScanRaw ?? report.hygieneSummary?.llmSlopScanReconciledFrom ?? report.scanScope?.llmSlopScanRaw;
...
export function sanitizeSimplebeaconReportExport(report, options = {}) {
    ...
    const exportReady = next.euAiActSummary
        ...
        : next;
    const jestExecuted =
        exportReady.jestBaselineChecked !== false && exportReady.scanScope?.jestExecutedDuringScan !== false;
