...
function buildPhaseCardHtml(phase, phaseIdx, projectKey, roadmap) {
    ...
    const depBlock = phase.dependsOn
        ? `<div class="dep-line"><span class="dep-arrow">↳</span> Depends on <strong>${escapeHtml(roadmap.phases.find(x => x.id === phase.dependsOn)?.title || phase.dependsOn)}</strong></div>`
        : '';
...
function generateRoadmap(report) {
    ...
    const emptyFiles =
        src.emptyFiles != null
            ...
            : src.dataQuality?.emptyJsonCount != null
    ...
    const dupes =
        src.duplicateGroups != null
            ...
            : src.consolidation?.duplicateGroups != null
    ...
    const scanIsClean =
        qualityScore === 100 && (src.gate?.pass === true || src.gate?.blockingCount === 0) && issueCount === 0;
    ...
    // Phase 1: Security — always show
    {
        ...
        const impactHtml = credIssue?.impact
    ...
    const bloatCount = report.cleanup?.bloatArtifactCount || 0;
    {
        ...
        const impactHtml = debugIssue?.impact
    ...
    const foundGovCount = (licenseCount || 0) + (securityCount || 0);
    {
        ...
        const impactHtml = govIssue?.impact
...
function exportRoadmapJson() {
    ...
    const projectName =
        ...
        currentReport.metadata?.project ||
        currentReport.summary?.project ||
...
document.getElementById('exportGitHubBtn').addEventListener('click', () => {
    ...
    currentRoadmap.phases.forEach(p => {
        ...
        p.tasks.forEach((t, taskIdx) => {
            ...
            if (p.dependsOn) {
                const depTitle = currentRoadmap.phases.find(x => x.id === p.dependsOn)?.title || p.dependsOn;
