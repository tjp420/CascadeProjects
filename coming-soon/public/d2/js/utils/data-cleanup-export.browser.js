...
function isBenchmarkReport(report) {
  const root = String(report?.projectRoot || report?.projectPath || "")
    .replace(/\\/g, "/")
    .toLowerCase();
...
function repairCompactAssetFindings(report) {
  ...
  const repaired = (report.findings?.assetConsolidation || []).map(
    (finding, index) => {
      ...
      return (
        normalizeDuplicateGroupForBrief({ ...finding, ...group }) || finding
      );
    },
  );
          ...
          ? enrichProductDataQualityScanScope(
              {
                ...(next.scanScope || {}),
                resultsViewScope:
                  next.scanScope?.resultsViewScope || "platform-only",
            ...
            ? enrichProductFileReductionScanScope(
                {
                  ...
                  resultsViewScope:
                    next.scanScope?.resultsViewScope || "platform-only",
                  reportHealth:
                    next.scanScope?.reportHealth || "platform-scoped",
                  securityHandoffEligible: false,
                  fileReductionNote:
                    "File-reduction export — reclaim tiers are guidance only, not vendor handoff clearance.",
                },
