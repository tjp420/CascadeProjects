...
async function test() {
  // Node.js project
  const node = identifyProgram(
    "my-app",
    makePaths(["package.json", "node_modules/foo/index.js", "src/main.js"]),
  );
  assert.strictEqual(node.profile, "nodejs");
  assert.ok(
    node.confidence >= 80,
    `Node confidence too low: ${node.confidence}`,
  );
  ...
  const unity = identifyProgram(
    ...
    makePaths([
      ...
      "ProjectSettings/UnityProjectSettings.asset",
  ...
  assert.ok(
    unity.confidence >= 80,
    `Unity confidence too low: ${unity.confidence}`,
  );

  // Python project
  const python = identifyProgram(
    "py-app",
    makePaths(["requirements.txt", ".venv/bin/python", "main.py"]),
  );
  assert.strictEqual(python.profile, "python");
  assert.ok(
    python.confidence >= 75,
    `Python confidence too low: ${python.confidence}`,
  );
  ...
  assert.ok(
    unreal.confidence >= 80,
    `Unreal confidence too low: ${unreal.confidence}`,
  );

  // VS Code: workspace
  const vscode = identifyProgram(
    "repo",
    makePaths([".vscode/settings.json", ".vscode/launch.json", "src/index.js"]),
  );
  assert.strictEqual(vscode.profile, "vscode");
  assert.ok(
    vscode.confidence >= 70,
    `VS Code: confidence too low: ${vscode.confidence}`,
  );

  // Unknown / mixed project
  const unknown = identifyProgram(
    "custom",
    makePaths(["readme.md", "build.sh", "assets/logo.png"]),
  );
