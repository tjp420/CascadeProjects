...
export function ToolSchemaValidationDashboard() {
  ...
  const [schemaDirection, setSchemaDirection] = useState<
    "request" | "response"
  >("response");
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [statsResp, schemasResp, violationsResp, configResp] =
        await Promise.all([
          fetch(apiUrl("/tool-schemas/stats"), { headers: authHeaders() }),
          fetch(apiUrl("/tool-schemas"), { headers: authHeaders() }),
          fetch(apiUrl("/tool-schemas/violations/list?limit=50"), {
            headers: authHeaders(),
          }),
      ...
      if (configData.success)
        setGlobalStrict(configData.config?.strictMode || false);
    } catch {
      // silent
    } finally {
      setLoading(false);
  ...
  const selectToolSchema = (toolId: string) => {
    ...
    if (schema) {
      ...
      if ("strictMode" in schema)
        setStrictMode((schema as ToolSchema).strictMode || false);
    }
  };
  ...
  const deleteSchema = async (toolId: string) => {
    try {
      const resp = await fetch(apiUrl(`/tool-schemas/${toolId}`), {
        method: "DELETE",
        headers: authHeaders(),
      });
      const data = await resp.json();
  ...
  const runValidation = async () => {
    ...
    try {
      ...
      const resp = await fetch(
        apiUrl(`/tool-schemas/${selectedTool}/validate`),
        {
          method: "POST",
          headers: { ...authHeaders(), "Content-Type": "application/json" },
  ...
  const toggleGlobalStrict = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.success(
          "Global strict mode " + (!globalStrict ? "enabled" : "disabled"),
        );
      }
    } catch {
  ...
  const clearViolations = async () => {
    try {
      const resp = await fetch(apiUrl("/tool-schemas/violations/clear"), {
        method: "POST",
        headers: authHeaders(),
      });
      const data = await resp.json();
      ...
      <Card>
        <CardContent className="flex items-center justify-center py-12 gap-3">
          ...
          <span className="text-sm text-foreground-muted">
            Loading tool schema validation data...
          </span>
        </CardContent>
      </Card>
    );
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                JSON schema enforcement for agent tool execution responses —
                validates tool outputs before passing to inference pipeline
              </CardDescription>
            </div>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (schema)
                        setSchemaEditorText(
                          schema[dir]
                            ? JSON.stringify(schema[dir], null, 2)
                            : "",
                        );
                    }}
                  ...
                  {!BUILTIN_TOOLS_HAS(selectedTool) && (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => deleteSchema(selectedTool)}
                    >
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2">
              <select
                ...
                onChange={(e) =>
                  setValidateDirection(e.target.value as "request" | "response")
                }
                className="text-xs border border-border rounded-md p-1 bg-background"
              >
            ...
            {validateResult && (
              <div className="rounded-md border border-border bg-muted/10 p-3 space-y-2 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant={validateResult.valid ? "success" : "destructive"}
                    className="text-[10px]"
                  >
                ...
                {violations.map((v) => (
                  ...
                  >
                    <div className="flex items-center gap-2 flex-wrap">
                      <Badge
                        variant={
                          v.direction === "request" ? "warning" : "destructive"
                        }
                        className="text-[10px]"
                      >
                        {v.direction}
