...
function computeComplianceStats(history: ComplianceTrendEntry[]) {
  ...
  const avgEuAiAct = Math.round(
    history.reduce((s, h) => s + h.euAiActScore, 0) / totalScans,
  );
  const avgSoc2 = Math.round(
    history.reduce((s, h) => s + h.soc2Score, 0) / totalScans,
  );
  const avgGate = Math.round(
    history.reduce((s, h) => s + h.gateScore, 0) / totalScans,
  );
  const avgOverall = Math.round(
    history.reduce((s, h) => s + h.overallScore, 0) / totalScans,
  );
  const latest = history[history.length - 1];
...
function computeResolutionStats(history: IssueResolutionEntry[]) {
  ...
  const resolutionRate =
    totalNew > 0 ? Math.round((totalResolved / totalNew) * 100) : 0;
...
export function TelemetryView() {
            ...
            if (Array.isArray(scanHistory)) {
              ...
              if (batchHistory.length === 0) {
                batchHistory = scanHistory.map((h: any) => ({
                  ...
                  totalChunks: Math.max(
                    1,
                    Math.ceil((h.totalFilesScanned || 0) / 10),
                  ),
                  totalBatches: Math.max(
                    1,
                    Math.ceil((h.totalFilesScanned || 0) / 50),
                  ),
                  totalTokensEstimated: (h.totalFilesScanned || 0) * 350,
                  durationMs:
                    h.processingTimeMs ||
                    Math.round((h.totalFilesScanned || 0) * 12),
                  avgChunkTokens: 350,
                  errors: 0,
                }));
              }
              ...
              if (complianceHistory.length === 0) {
                complianceHistory = scanHistory.map((h: any) => {
                  const gateScore = h.gatePass
                    ? 100
                    : Math.max(0, 100 - (h.issueCount || 0) * 5);
                  const euAiActScore = Math.max(
                    0,
                    Math.min(
                      100,
                      gateScore - (h.fictionPatternsFound || 0) * 3,
                    ),
                  );
                  const soc2Score = Math.max(
                    0,
                    Math.min(
                      100,
                      gateScore - (h.severityCounts?.critical || 0) * 10,
                    ),
                  );
                  const overallScore = Math.round(
                    (euAiActScore + soc2Score + gateScore) / 3,
                  );
                  return {
                    scanId: h.scanId || `scan-${h.date}`,
                    date: h.date,
              ...
              if (resolutionHistory.length === 0 && scanHistory.length >= 2) {
                resolutionHistory = scanHistory
                  .slice(1)
                  .map((h: any, i: number) => {
                    const prev = scanHistory[i];
                    const newIssues = Math.max(
                      0,
                      (h.issueCount || 0) - (prev.issueCount || 0),
                    );
                    const resolvedIssues = Math.max(
                      0,
                      (prev.issueCount || 0) - (h.issueCount || 0),
                    );
                    return {
                      scanId: h.scanId || `scan-${h.date}`,
                      date: h.date,
  ...
  const { batchHistory, complianceHistory, resolutionHistory, loading, error } =
    state;
      ...
      <div className="mx-auto max-w-5xl p-6 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-foreground-muted">
              Multi-file batch analytics, compliance trends, and issue
              resolution tracking
            </p>
          </div>
        </div>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Batch execution times, EU AI Act compliance trends, and issue
              resolution tracking
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-foreground-muted">
                Run multi-file scans to start tracking batch execution times,
                compliance trends, and issue resolution rates.
              </p>
            </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            trend={
              batchStats.durationTrend === "down"
                ? "up"
                : batchStats.durationTrend === "up"
                  ? "down"
                  : "flat"
            }
            color={
          ...
          <StatCard
            ...
            subtitle={
              batchStats.totalErrors === 0 ? "No errors" : "Some chunks failed"
            }
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Execution time per multi-file sweep
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Estimated tokens processed per sweep
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={260}>
        ...
        {batchHistory.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Detailed history of multi-file sweep performance
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              ...
              color={
                complianceStats.avgOverall >= 80
                  ? "success"
                  : complianceStats.avgOverall >= 60
                    ? "warning"
                    : "danger"
              }
            />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                EU AI Act, SOC 2, and Gate scores over time
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              ...
              subtitle={
                resolutionStats.netChange > 0
                  ? `+${resolutionStats.netChange} net`
                  : `${resolutionStats.netChange} net`
              }
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Issue inflow vs. resolution over time
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
