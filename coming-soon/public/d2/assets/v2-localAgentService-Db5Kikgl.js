import { t as Ae, E as Se, v as P, w as Ee, x as F, V as _e } from './main.js';
import './v2-vendor-radix-CNRt243h.js';
import './v2-vendor-charts-B5D8dQNA.js';
function Ce(e) {
...
function fe(e, t) {
    ...
    const o = r.includes('\\') ? '\\' : '/',
        a = n.replace(/^[/\\]+/, '').replace(/\//g, o);
    return `${r}${o}${a}`;
}
function $e(e, t = 1, n = {}) {
    const r = fe(e, n.projectRoot);
    if (!r || !pe(r)) return null;
    const o = n.scheme || (typeof navigator < 'u' && /Cursor/i.test(navigator.userAgent || '') ? 'cursor' : 'vscode'),
        a = Ce(r),
        s = Math.max(1, Number(t) || 1);
    return /^\/\//.test(a) || /^[A-Za-z]:\//.test(a) ? `${o}://file/${a}:${s}` : `${o}://file/${a}:${s}`;
}
function le(e, t = 1, n = {}) {
    const r = fe(e, n.projectRoot);
    if (!r) return !1;
    const o = Math.max(1, Number(t) || 1),
        a = { command: 'openFile', file: r, path: r, line: o },
        s = Ae();
    if (s)
        try {
...
const M = 'http://127.0.0.1:55432',
    ee = 'http://127.0.0.1:4000',
    ue = 'sb_api_base',
    ae = 3e3,
    de = { windows: F, linux: F, macos: F, unknown: F };
let S = null,
    j = 0;
const Be = 3e4,
    Le = 12e4;
function he(e, t) {
    const n = t && t.available ? Be : Le;
    return e + n > Date.now();
}
let O = null,
    E = null,
...
function Xe() {
    ...
    if (e)
        try {
            te(new URL(String(e).replace(/\/api\/?$/, '')).origin) && (P({ updateUrl: !0 }), (t = !0));
        } catch {}
    if (typeof window < 'u' && window.__SB_BRIDGE_HOST__ && te(window.__SB_BRIDGE_HOST__)) {
        try {
...
function Ne(e, t = {}, n = 4e3) {
    return new Promise((r, o) => {
        if (!me()) {
            o(new Error('Parent bridge unavailable'));
            return;
        }
...
function V() {
    ...
    try {
        const e = new URLSearchParams(window.location.search),
            t = e.get(ue) || e.get('sb_notify_base');
        if (t) return t;
    } catch {}
    if (typeof sessionStorage < 'u')
        try {
            return sessionStorage.getItem(ue) || sessionStorage.getItem('sb_notify_base');
        } catch {}
    return null;
}
function b() {
    ...
    try {
        ...
        return (r !== '127.0.0.1' && r !== 'localhost') || (_() && te(n.origin)) ? null : n.origin;
    } catch {
        return null;
    }
}
...
const A = 'http://127.0.0.1:11434',
    we = [54358, 54697, 58681],
    Oe = new Set([61455, 54358, 54697, 58681, 58e3, 53900, 3e3, 8080]);
function I(e) {
    const t = String(e || '')
        .trim()
        .replace(/\/+$/, '');
    ...
    try {
        const n = new URL(t),
            r = parseInt(n.port, 10);
        if ((Number.isFinite(r) && Oe.has(r)) || (typeof window < 'u' && t === window.location.origin)) return A;
    } catch {
        return A;
    }
    return t;
...
function Ke(e = 'chatbot') {
    ...
    return `${t}://${Se}/connect?route=${encodeURIComponent(n)}`;
}
function T() {
    if (typeof window > 'u') return !1;
...
async function re(e = we, t = {}) {
    if (typeof window > 'u' || !T()) return null;
    const n = C();
    for (const r of e) {
        const o = `http://127.0.0.1:${r}`;
        if (!(U(o) && !g()))
            try {
                const a = await n(`${o}/api/health`, { signal: AbortSignal.timeout(2200) });
                if (a.ok) {
                    const l = await a.json().catch(() => ({}));
                    if (l && (l.service === 'simplebeacon-bridge' || l.platform === 'Simplebeacon')) return `${o}/api`;
                }
                const s = await n(`${o}/api/ping`, { signal: AbortSignal.timeout(2200) });
                if (!s.ok) continue;
                const c = await s.json().catch(() => ({}));
                if (c && (c.online === !0 || c.status === 'ok')) return `${o}/api`;
            } catch {}
    }
    return null;
}
async function Qe(e = {}) {
    ...
    if (!T()) return { ok: !1, source: w() ? 'hosted-https' : 'none', needsDeepLink: w() };
    const r = await re();
    return r
        ? Ee(r, { websiteMode: !0, updateUrl: !0 })
...
function Re(e, t = A) {
    ...
    if (o) {
        ...
        return r === '/api/tags'
            ? `${o}/api/simplebeacon/ollama/models?${a}`
            : r === '/api/chat'
              ? `${o}/api/simplebeacon/ollama/chat?${a}`
              : `${o}/api/simplebeacon/ollama/proxy?${a}&path=${encodeURIComponent(r)}`;
    }
    return `${n}${r}`;
}
async function oe(e) {
...
async function tt(e = A) {
    ...
    if (w() && U(r) && !g()) return !1;
    try {
        const o = await n(r, { signal: AbortSignal.timeout(2500) });
        return (await oe(o)).ok;
    } catch {
        return !1;
    }
}
    ...
    for (const l of s)
        try {
            const i = await r(l, { signal: AbortSignal.timeout(4e3) }),
                p = await oe(i);
            if (((c = { ...p, status: p.status || i.status }), p.ok))
                return { ok: !0, corsBlocked: !1, status: p.status };
...
async function rt(e) {
    ...
    try {
        const a = await (
            await r(`${t}/api/status`, { headers: { Accept: 'application/json' } })
        )
            .json()
            .catch(() => ({}));
        if (
...
async function ot() {
    ...
    try {
        ...
        return !r.ok || o.success !== !0 ? null : String(o.path || '').trim() || null;
    } catch {
        return null;
    }
}
async function $(e, t = {}, n = 3e5) {
    const r = be(),
        o = new Promise((s, c) => {
            setTimeout(() => c(new Error('Local agent request timed out')), n);
        });
    return await Promise.race([r(e, t), o]);
}
function Te(e) {
        ...
        ? S && he(j, S)
            ...
            : O ||
              ((O = (async () => {
                  ...
                  try {
                      const t = await $(`${e}/health`, { method: 'GET', headers: { Accept: 'application/json' } }, ae),
                      ...
                      return ((S = r), (j = Date.now()), r);
                  } catch (t) {
                      return (
                          (S = { available: !1, scannerAvailable: !1, likelyBlocked: !g() && Ie(e, t) }),
                          (j = Date.now()),
                          S
                      );
                  } finally {
...
async function st(e, t = {}, n = M) {
    const r = await $(`${n}/inventory`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
            body: JSON.stringify({ projectPath: e, fullDirectoryScan: t.fullDirectoryScan })
        }),
        o = await r.json().catch(() => ({}));
    if (!r.ok || !o.success) throw new Error(o.error || `Agent inventory failed (${r.status})`);
    return o.inventory;
}
async function it(e, t = {}, n = M) {
    let r = n,
        o = t;
    typeof t == 'string' && ((r = t), (o = {}));
    const a = await $(
            `${r}/scan`,
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
                body: JSON.stringify({ projectPath: e, fullDirectoryScan: o.fullDirectoryScan === !0 })
            },
            6e5
        ),
...
async function ct(e, t = M) {
    const n = new URLSearchParams({ projectPath: String(e || '') }),
        r = await $(`${t}/progress?${n}`, { headers: { Accept: 'application/json' } }, 15e3),
        o = await r.json().catch(() => ({}));
    return !r.ok || !o.success ? { active: !1 } : o.progress || { active: !1 };
}
async function lt(e) {
    const t = b();
    ...
    try {
        const r = await $(
                `${t}/api/simplebeacon/scan/progress?${n}`,
...
async function ut(e = ne()) {
    if (!je())
        return { available: !1, likelyBlocked: !1, extensionBridge: !1, origin: ne() || ee, integratedSkipped: _() };
    if (!e) return { available: !1, likelyBlocked: !1, extensionBridge: !1, origin: ee, integratedSkipped: !0 };
    const t = ge(e);
    return w() && !t && !g()
          ...
          : R ||
            ((R = (async () => {
                if (!t && !g() && U(e)) {
                    ...
                    return ((E = n), (z = Date.now()), n);
                }
                try {
                    const n = await $(`${e}/api/ping`, { method: 'GET', headers: { Accept: 'application/json' } }, ae),
                        r = await n.json().catch(() => ({})),
                        o = { available: n.ok && r.online === !0, extensionBridge: t, origin: e };
                    return ((E = o), (z = Date.now()), o);
                } catch {
                    const r = { available: !1, extensionBridge: t, origin: e };
                    return ((E = r), (z = Date.now()), r);
                } finally {
                    R = null;
                }
            })()),
...
async function Fe(e, t, n) {
    ...
    let o = 0;
    for (; Date.now() - r < De;) {
        await new Promise(a => setTimeout(a, Ue));
        try {
            if (
                !(
                    (
...
async function dt(e, t = ne()) {
    if (ge(t)) {
        const o = C(),
            a = await o(`${t}/api/scan`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
...
function pt(e, t) {
    ...
    if (!n) return;
    const r = e.files || [],
        o = r.filter(d => d.status && d.status !== 'Clean'),
        a = r.length - o.length,
    ...
    ((X.className = 'sb-compliance-status'),
        (X.textContent = n.complianceStatus || 'Review required'),
        f.appendChild(q),
        f.appendChild(J),
        f.appendChild(X));
    const D = document.createElement('div');
    ((D.className = 'sb-compliance-grade'),
        D.setAttribute('aria-label', `Grade ${n.letterGrade || '?'}`),
        (D.textContent = n.letterGrade || '?'),
        p.appendChild(f),
        p.appendChild(D),
        i.appendChild(p));
    const Z = document.createElement('div');
    if (
        ((Z.className = 'sb-compliance-metrics'),
        [
            { label: 'Files scanned', value: `${r.length} / ${s}` },
            { label: 'Heuristic score', value: `${n.score || 0}/100` },
            { label: 'Issues flagged', value: String(o.length) },
            ...
            ((k.className = d.danger ? 'sb-compliance-metric-danger' : ''),
                (k.textContent = d.value),
                m.appendChild(h),
                m.appendChild(k),
                Z.appendChild(m));
        }),
    ...
    if (
        ((K.className = 'sb-compliance-actions'),
        [
            ...
            ((m.type = 'button'),
                ...
                m.addEventListener('click', () => {
                    ...
                    if (d.route === 'export') {
                        h &&
                            h.scanService &&
                            typeof h.scanService.exportReport == 'function' &&
                            h.scanService.exportReport();
                        return;
                    }
        ...
        ((d.className = 'sb-compliance-files'), (d.open = o.length > 0 && o.length <= 12));
        const m = document.createElement('summary');
        ((m.textContent = `File inventory (${o.length} flagged · ${a} clean)`), d.appendChild(m));
        const h = document.createElement('div');
        h.className = 'sb-compliance-files-toolbar';
        let k = 'issues';
        const ve = [
                { id: 'issues', label: `Issues (${o.length})` },
                { id: 'all', label: `All (${r.length})` },
                { id: 'clean', label: `Clean (${a})` }
            ],
            x = document.createElement('div');
        x.className = 'sb-compliance-files-list';
        const ie = () => {
            x.replaceChildren();
            let v = r;
            k === 'issues' ? (v = o) : k === 'clean' && (v = r.filter(u => !u.status || u.status === 'Clean'));
            const y = k === 'all' ? 80 : 120;
            if (
                (v.slice(0, y).forEach(u => {
                    const B = document.createElement('div'),
                        ce = !u.status || u.status === 'Clean';
                    B.className = `sb-compliance-file-row${ce ? ' is-clean' : ' is-issue'}`;
                    const Q = document.createElement('span');
                    ((Q.className = 'sb-compliance-file-badge'), (Q.textContent = ce ? 'Clean' : String(u.status)));
                    ...
                    ((L.className = 'sb-compliance-file-path'),
                        ...
                        (u.absolutePath || u.name) &&
                            ...
                            L.addEventListener('click', () => {
                                le(u.absolutePath || u.name, u.line || 1, { projectRoot: l });
                            })));
                    ...
                    ((Y.className = 'sb-compliance-file-size'),
                        (Y.textContent = `${u.size || 0} B`),
                        B.appendChild(Q),
                        B.appendChild(L),
                        B.appendChild(N),
                        B.appendChild(Y),
                ...
                ((u.className = 'sb-compliance-files-more text-muted'),
                    (u.textContent = `+ ${v.length - y} more files — run a full export for the complete inventory.`),
                    x.appendChild(u));
            }
        };
        (ve.forEach(v => {
            ...
            ((y.type = 'button'),
                (y.className = `btn btn-ghost btn-xs sb-compliance-filter${v.id === k ? ' is-active' : ''}`),
                (y.textContent = v.label),
    ...
    t.appendChild(i);
    const se = t.closest('#analyze-path-dropzone');
    se && se.classList.add('has-compliance-report');
}
function ft(e, t) {
    return !(t != null && t.available) || !(t != null && t.scannerAvailable) ? !1 : Te(e);
}
function w() {
...
function je() {
    return g() || b() ? !0 : w() || _() ? !1 : T();
}
function ze() {
    return w() ? g() || H() : !(_() && !b() && !g());
}
const Ge = { available: !1, scannerAvailable: !1, hostedSkipped: !0 },
    Me = { available: !1, scannerAvailable: !1, integratedSkipped: !0 };
function ht(e) {
    return !e || (w() && (!e.available || e.hostedSkipped))
        ? ''
        : e.available
          ? e.scannerAvailable
...
function mt(e) {
    const t = e || ke();
    return de[t] || de.unknown;
}
function bt(e) {
    var t, n, r;
    if (w() && (!(e != null && e.available) || (e != null && e.hostedSkipped)))
    ...
    if (!(!((r = e) === null || r === void 0) && r.scannerAvailable)) {
        var o;
        return `Local Scan Agent is running but the scanner is not loaded.${!((o = e) === null || o === void 0) && o.scannerLoadError ? ` (${e.scannerLoadError})` : ''} Restart the agent or reinstall the portable package.`;
    }
...
export {
    Ne as bridgeFetchViaParent,
    et as buildBridgeOllamaChatUrls,
    Pe as buildBridgeOllamaProbeUrls,
    Ke as buildExtensionConnectDeepLink,
    me as canUseParentBridgeFetch,
    Xe as clearStaleIntegratedBridgeParams,
    ke as detectPlatform,
    Qe as discoverAndApplyExtensionBridge,
    st as fetchInventoryViaAgent,
    ct as fetchScanProgressViaAgent,
    lt as fetchScanProgressViaExtensionBridge,
    rt as findFolderViaBridge,
    ht as formatAgentStatus,
    mt as getAgentDownloadUrl,
    bt as getAgentFallbackMessage,
    be as getBridgeFetch,
    b as getExtensionBridgeOrigin,
    wt as getInstallInstructions,
    C as getLocalBridgeFetch,
    gt as getPlatformLabel,
    Ze as getVsixDownloadUrl,
    G as hasExplicitBridgeParam,
    H as hasExtensionBridgeConfigured,
    w as isHostedHttpsDashboard,
    _ as isIntegratedLocalDashboard,
    Te as isLocalPath,
    ot as pickFolderViaExtensionBridge,
    at as probeAgent,
    ut as probeAgent4000,
    W as probeExtensionBridgeHealth,
    re as probeExtensionDataServer,
    tt as probeLocalOllama,
    nt as probeUserInitiatedOllama,
    pt as renderAgentCertificate,
    Re as resolveOllamaProxyUrl,
    it as scanViaAgent,
    dt as scanViaAgent4000,
    je as shouldProbeAgent4000,
    ze as shouldProbeLocalAgent,
    ft as shouldUseAgent,
    Ye as validateExtensionBridgeOnLoad
};
//# sourceMappingURL=v2-localAgentService-Db5Kikgl.js.map
