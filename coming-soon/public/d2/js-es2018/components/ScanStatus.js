...
} from "../utils-lib/dom.js?v=20260725dropfix2";
import {
  resolveDisplayScore,
...
} from "../services/localAgentService.js?v=20260722scanfix2";
import {
...
} from "../services/browserSandboxScanService.js?v=20260725dropfix2";
/**
...
export async function runDashboardScanFromInput(input, options = {}) {
    ...
    try {
      ...
      if (status.available && !status.extensionBridge) {
        ...
        if (onLocalScanResult) {
          onLocalScanResult({
            projectPath: result.verifiedAddress || result.path,
            summary: result.certificate,
            source: "agent4000",
          });
        }
      ...
      /* fall through to main agent fallback */
    }
    ...
    try {
      ...
      if (status.available && status.scannerAvailable) {
        ...
        if (onLocalScanResult) {
          onLocalScanResult({
            projectPath:
              result.projectPath || result.verifiedAddress || result.path,
            summary: result.certificate,
            source: "agent",
          });
        }
        setLastProjectPath(
          result.projectPath || result.verifiedAddress || result.path || path,
        );
        return;
      }
    } catch (_b) {
      /* fall through to existing server flow */
    }
  }
  ...
  if (isAbsoluteLocal && isRemoteDeployment) {
    showToast(
      "This hosted dashboard cannot read typed PC paths. Opening folder picker to scan privately in your browser…",
      "info",
      { duration: 8000 },
    );
    if (typeof options.onBrowserSandboxScan === "function") {
      ...
      if (scopeCard) {
        ...
        setTimeout(
          () => scopeCard.classList.remove("sandbox-scanner-highlight"),
          3000,
        );
      }
    }
...
async function runSandboxedScanForDashboard(onLocalScanResult, container) {
  ...
  const terminal =
    root.querySelector("#sandbox-scan-terminal") ||
    document.getElementById("sandbox-scan-terminal");
  const resultsEl =
    root.querySelector("#agent-4000-results") ||
    document.getElementById("agent-4000-results");
  if (dropzone) {
  ...
  try {
    ...
    if (onLocalScanResult) {
      onLocalScanResult({
        projectPath: report.verifiedAddress || report.path,
        summary: report.certificate,
        source: "sandbox",
      });
    }
    try {
      if (typeof sessionStorage !== "undefined") {
        ...
        sessionStorage.setItem(
          ...
          report.verifiedAddress || report.path || "",
        );
      }
    } catch (_a) {}
    ...
    if (!window.location.pathname.includes("/analyze")) {
      ...
      setTimeout(() => {
        if (
          ...
          window.simplebeaconApp &&
          typeof window.simplebeaconApp.navigate === "function"
        ) {
    ...
    if (isFilePickerBlockedError(err)) {
      ...
      const browseInput =
        root.querySelector("#scan-browse-input") ||
...
export function renderScanStatus(report, options = {}) {
  ...
  const gate =
    (report === null || report === void 0 ? void 0 : report.gate) || {};
...
export function updateScanStatusDom(root, report) {
  ...
  if (redesign) {
    ...
    const freshnessEl = redesign.querySelector(
...
export function bindScanStatus(container, options = {}) {
   ...
   */
  const runScan = () =>
    runDashboardScanFromInput(input, {
      ...options,
      onBrowserSandboxScan: () =>
        runSandboxedScanForDashboard(onLocalScanResult, container),
    });
  /**
  ...
  function extractAbsoluteDroppedFolderPath(event, folderName) {
    ...
    if (plainTrimmed) {
      ...
      if (/^[a-zA-Z]:|^\\|^\//.test(normalizedPlain)) {
        if (
          folderName &&
          (normalizedPlain.endsWith(`/${folderName}`) ||
            normalizedPlain.split("/").pop() === folderName)
        ) {
    ...
    for (const line of filesLines) {
      ...
      if (/^[a-zA-Z]:|^\\|^\//.test(normalized)) {
        if (
          folderName &&
          (normalized.endsWith(`/${folderName}`) ||
            normalized.split("/").pop() === folderName)
        ) {
          return normalized;
        }
      }
    ...
    const files =
      (_b = (_a = dt) === null || _a === void 0 ? void 0 : _a.files) !== null &&
      _b !== void 0
        ? _b
        : null;
    if (files?.length) {
  ...
  function deriveUserHomeBase() {
    ...
    if (defaultPath) {
      ...
      if (isWindowsClient && isUnixAbsolute) {
        const driveMatch = String(getLastProjectPath() || "").match(
          /^([a-zA-Z]:)/,
        );
  ...
  function deriveFallbackBase(input, folderName) {
    ...
    const normName = String(folderName || "")
      .replace(/\\/g, "/")
      .replace(/\/+$/, "");
    if (!normName) return "";
    ...
    if (current && isAbs(current.replace(/\\/g, "/"))) {
      ...
      if (isWindowsClient && /^\//.test(norm) && !/^[a-zA-Z]:/.test(norm)); // fall through
      else if (norm.endsWith(`/${normName}`) || norm === normName) return norm;
      else return `${norm}/${normName}`;
    }
    for (const candidate of [getLastProjectPath(), getDefaultProjectPath()]) {
      const c = String(candidate || "").trim();
      if (!c) continue;
      const cNorm = c.replace(/\\/g, "/").replace(/\/+$/, "");
      if (!isAbs(cNorm)) continue;
      ...
      if (isWindowsClient && /^\//.test(cNorm) && !/^[a-zA-Z]:/.test(cNorm))
        continue;
      if (cNorm.endsWith(`/${normName}`) || cNorm === normName) return cNorm;
      return `${cNorm}/${normName}`;
    }
    return normName;
  }
  scanBtn === null || scanBtn === void 0
    ? void 0
    ...
    : browseInput.addEventListener("change", async (e) => {
        ...
        if (filePath && relPath) {
          ...
          if (normalizedFull.endsWith(normalizedRel)) {
            ...
            if (isRemoteDashboardHost() && isAbsoluteLocalPath(resolvedPath)) {
              showToast(
                "Hosted dashboard cannot scan typed PC paths. Scanning the selected folder in your browser…",
                "info",
              );
        ...
        if (isRemoteDashboardHost()) {
          ...
          try {
            ...
            showToast(
              `Browser scan complete — ${report.summary?.codeFilesAnalyzed || fileArray.length} files analyzed`,
              "success",
            );
            if (onLocalScanResult) {
              onLocalScanResult({
          ...
          try {
            ...
            if (isRemoteDashboardHost()) {
              ...
              try {
                ...
                showToast(
                  `Browser scan complete — ${report.summary?.codeFilesAnalyzed || 0} files analyzed`,
                  "success",
                );
                if (onLocalScanResult) {
                  onLocalScanResult({
  ...
  setDefaultBtn === null || setDefaultBtn === void 0
    ? void 0
  ...
  sandboxPickerBtn === null || sandboxPickerBtn === void 0
    ? void 0
    : sandboxPickerBtn.addEventListener("click", () => {
        if (!canUseDirectoryPicker()) {
          const browseInput =
            container.querySelector("#scan-browse-input") ||
  ...
  const progressDetail = container.querySelector(
    "#scan-dropzone-progress-detail",
  );
  ...
  if (dropzone && input) {
    ...
    dropzone.addEventListener("drop", async (event) => {
      ...
      setDropzoneState("scanning");
      if (terminal) terminal.textContent = "Reading dropped items…";
      try {
        const droppedFolder =
          (webkitEntry && webkitEntry.isDirectory) ||
          (await isDroppedFolder(itemArray));
        const firstFile =
        ...
        if (droppedFolder) {
          ...
          if (onLocalScanResult) onLocalScanResult(report);
        } else if (fileArray.length) {
          const files = fileArray;
          const resolvedPath =
            files[0].path || files[0].name || "selected-files";
          ...
          if (
            ...
            try {
              ...
              console["warn"](
                "[ScanStatus] findFolderViaBridge failed for non-folder drop:",
                bridgeErr,
              );
            }
          }
        ...
        if (
          ...
          const folderName =
            ...
            "selected";
          const fallbackPath =
            snapshotPath || extractAbsoluteDroppedFolderPath(event, folderName);
          if (fallbackPath) {
            if (input) {
              input.value = fallbackPath;
              input.dataset.userModified = "true";
        ...
        if (errorMessage) errorMessage.textContent = msg || "Scan failed.";
        setDropzoneState("error");
      }
    });
    // Retry button
    const retryBtn = container.querySelector("#scan-dropzone-retry");
    retryBtn === null || retryBtn === void 0
      ? void 0
      : retryBtn.addEventListener("click", resetDropzone);
    // View Results button navigates to the full results view
    const viewResultsBtn = container.querySelector(
      "#scan-dropzone-view-results",
    );
    viewResultsBtn === null || viewResultsBtn === void 0
      ? void 0
