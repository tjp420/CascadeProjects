...
async function requestChallenge(purpose, extra = {}) {
  ...
  if (res.status === 404) {
    throw new Error(
      "Security keys are not enabled on this server yet. Use email/password sign-in for now.",
    );
  }
  if (!res.ok || !data.success) {
    throw new Error(
      data.error || data.message || `Challenge failed (${res.status})`,
    );
  }
  return data;
}
...
export async function registerSecurityKey({
  ...
  const userIdSource =
    authService.getUser()?.id || email || "simplebeacon-user";
    ...
    publicKey: {
      ...
      user: {
        id: base64urlToBuffer(
          bufferToBase64url(new TextEncoder().encode(userIdSource)),
        ),
        name: email || authService.getUser()?.email || "SimpleBeacon user",
        displayName:
          email || authService.getUser()?.email || "SimpleBeacon user",
      },
      pubKeyCredParams: [
...
export async function authenticateWithSecurityKey() {
  ...
  if (!res.ok || !data.success || !data.token) {
    throw new Error(
      data.error ||
        data.message ||
        `Security key sign-in failed (${res.status})`,
    );
  }
  return data;
}
...
export async function listSecurityKeys() {
  ...
  if (!res.ok || !data.success) {
    throw new Error(
      data.error ||
        data.message ||
        `Failed to load security keys (${res.status})`,
    );
  }
...
export async function removeSecurityKey(credentialId) {
  ...
  if (!res.ok || !data.success) {
    throw new Error(
      data.error ||
        data.message ||
        `Failed to remove security key (${res.status})`,
    );
  }
  return true;
}
