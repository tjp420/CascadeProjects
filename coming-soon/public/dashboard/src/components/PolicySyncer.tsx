...
export function PolicySyncer() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const polResp = await fetch(apiUrl(`audit/pii/policies/${encodeURIComponent(orgId)}`), {
                headers: authHeaders(),
                credentials: 'include'
            });
    ...
    const fetchBlockedCount = useCallback(async () => {
        try {
            const resp = await fetch(apiUrl('audit/log?action=compliance_policy_violation&limit=1&offset=0'), {
                headers: authHeaders(),
                credentials: 'include'
            });
            if (!resp.ok) return;
