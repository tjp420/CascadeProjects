...
export function AnalyzeView() {
    ...
    useEffect(() => {
        ...
        return () => {
            try {
                if (typeof window !== 'undefined') delete (window as any).SimpleBeaconStorage;
            } catch (_e) {}
        };
    }, []);
        ...
        async (options: {
            ...
            try {
                ...
                if (
                    ...
                    (scanResult.totalFiles > 0 && scanResult.totalFiles < 3 && scanResult.gate.pass === false)
                ) {
                    toast.warning(
    ...
    const handleScan = useCallback(async () => {
        ...
        try {
            const maybe =
                (window as any).__SB_USER || localStorage.getItem('sb_user') || localStorage.getItem('sb_user_id');
            if (maybe) {
                try {
        ...
        try {
            ...
            if (!isUrl && !isGithubUrl(scanPath) && hosted && !isServerDefaultPath) {
                ...
                if (!isAbsoluteLocalPath(scanPath) && !bridgeBase && !hasFsaEarly) {
                    ...
                    toast.info(
                        'Your browser cannot resolve "' + scanPath + '" automatically. Click Select Folder below.'
                    );
                    return;
                }
                    ...
                    while (pollAttempts < maxPollAttempts) {
                        ...
                        try {
                            const pollResp = await fetch(
                                apiUrl(`/analyze/progress?scanId=${encodeURIComponent(scanId)}`),
                                {
                                    headers: authHeaders()
                                }
                            );
        ...
        <div className="mx-auto max-w-7xl p-6 space-y-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-foreground-muted">
                                The hosted dashboard needs permission to reach your local SimpleBeacon bridge. Grant
                                permission in your browser and retry.
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-foreground-muted">
                                    Start the Local Agent on your machine to enable direct filesystem scans from the
                                    hosted dashboard. The agent listens on loopback and proxies requests to your
                                    filesystem.
                                </p>
                            </div>
                            <div className="flex items-center gap-2">
                        ...
                        <Tabs defaultValue="instructions" className="mt-3">
                            ...
                            <TabsContent value="instructions">
                                <ol className="mt-3 ml-4 text-sm list-decimal">
                                    <li>
                                        Download/start the agent: <code>node local-agent/agent.js</code> or use the
                                        platform installer.
                                    </li>
                                                ...
                                                return (
                                                    <div key={i} className="mb-2">
                                                        <div className="text-foreground-muted text-[11px]">
                                                            {be.timestamp || be.time || ''} — {be.traceId || ''}
                                                        </div>
                    ...
                    <Tabs value={mode} onValueChange={v => setMode(v as ScanMode)}>
                        ...
                        <TabsContent value="local" className="space-y-3">
                            ...
                            >
                                ...
                                {typeof (window as any).showDirectoryPicker !== 'function' && (
                                    <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">
                                        For large folders (3,000+ files), drag-and-drop is recommended — the file picker
                                        has a browser-imposed limit.
                                    </p>
                                )}
                            ...
                            <label className="flex items-center gap-2 text-xs text-foreground-muted">
                                ...
                                />
                                <span>
                                    <strong>Deep scan</strong> — bypass vendor/docs/build filters to scan all files
                                </span>
                            </label>
                        </TabsContent>
                            ...
                            <div className="flex items-center justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-xs text-foreground-muted mt-1">
                                        Your browser requires a manual folder selection. Click the button to choose a
                                        directory — your files are scanned locally in this browser and never uploaded.
                                    </p>
                                </div>
                                <Button
                                    size="sm"
            ...
            {scanState === 'complete' && fileErrorsCount ? (
                <div className="rounded-md border p-3 bg-muted/5 text-sm">
                    <div className="flex items-center justify-between">
                        <div>
                            <strong>File access issues:</strong> {fileErrorsCount} file(s) could not be read during the
                            scan.
                            {fileErrorExamples && fileErrorExamples.length > 0 && (
                                <div className="mt-2 text-xs text-foreground-muted">
                                    ...
                                    {fileErrorExamples.length > 3 ? ` (+${fileErrorExamples.length - 3} more)` : null}
                                </div>
                            )}
                        </div>
                        <div>
                        ...
                        <div className="flex items-start gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                ...
                                <div className="text-xs text-foreground-muted truncate">
                                    {lastErrorMsg || 'Sign in to run analysis on the hosted dashboard.'}
                                </div>
                            </div>
                        </div>
                        <Button size="sm" className="shrink-0" onClick={() => clearAuthAndRedirect()}>
                            Sign in again
                        </Button>
                    </CardContent>
                </Card>
                        ...
                        <CardContent className="space-y-3 p-4">
                            {fullReport?.diagnosticReport ? (
                                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                                    ...
                                    <div className="rounded-md border p-3">
                                        <div className="text-xs text-foreground-muted">Filtered Files</div>
                                        <div className="text-xl font-bold">
                                            {fullReport.diagnosticReport.filteredFiles}
                                        </div>
                                    </div>
                                    <div className="rounded-md border p-3">
                                        <div className="text-xs text-foreground-muted">Scanned Files</div>
                                        <div className="text-xl font-bold">
                                    ...
                                    <div className="rounded-md border p-3">
                                        <div className="text-xs text-foreground-muted">Large File Skips</div>
                                    ...
                                    <div className="rounded-md border p-3">
                                        <div className="text-xs text-foreground-muted">Ignored by Pattern</div>
                                        <div className="text-xl font-bold">
                ...
                <TabsContent value="scope">
                    <Card>
                        <CardContent className="p-4">
                            <p className="text-sm text-foreground-muted">
                                Deterministic gate scan (AI narrative hidden for compliance integrity). Source files are
                                not semantically reviewed. Gate passes on configured severities. Scoped to configured
                                scanPaths and production directories — pattern matching only.
                            </p>
                        </CardContent>
                    </Card>
                </TabsContent>
