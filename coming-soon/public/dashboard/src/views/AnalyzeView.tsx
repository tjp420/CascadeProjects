...
async function pickFolderViaExtensionBridge(
  ...
  for (const route of ["/api/analyze/pick-folder", "/api/pick-folder"]) {
    try {
      const res = await fetch(`${bridgeBase}${route}`, {
        method: "POST",
        headers,
      });
      if (res.ok) {
        ...
        if (activeBridge) {
          ...
          if (bridgeReachable) {
            try {
              ...
              appendLog(
                `[SimpleBeacon] Extension bridge scan failed: ${bridgeErr?.message || bridgeErr}. Falling back to browser-local scan...`,
              );
            }
          } else {
            appendLog(
...
function ScanResults({
  ...
  const blocking =
    (result.severityCounts?.critical || 0) + (result.severityCounts?.high || 0);
  const lockedFixes = remainingLockedRows(
    Math.min(Math.max(result.issueCount || 0, 0), 50),
    3,
  );
  return (
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              <FileCode className="h-4 w-4" /> Open remediation preview
            </Button>
            {paidRoadmap ? (
              <Button
                variant="outline"
          ...
          {!paidRoadmap && !isZeroResult && lockedFixes > 0 ? (
            <div className="relative mt-4 min-h-[200px] overflow-hidden rounded-lg border">
              <div className="pointer-events-none select-none blur-sm opacity-40 p-4 space-y-2">
                <p className="text-sm">
                  {blocking} critical/high detector labels in this scan.
                </p>
                <p className="text-xs text-foreground-muted">
                  File paths and line numbers stay on this device until checkout.
                </p>
