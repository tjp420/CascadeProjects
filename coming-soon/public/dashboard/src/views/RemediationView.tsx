...
function priorityColor(priority?: string): string {
    const p = typeof priority === 'string' && priority ? priority.toLowerCase() : '';
    if (p === 'critical' || p === 'high') return 'bg-red-500/15 text-red-500 border-red-500/30';
...
export function RemediationView() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const roadmap = json.roadmap || json.report?._roadmapAnalysis || json.report || json;
            setData(roadmap);
        } catch (e: any) {
    ...
    const triggerRescan = async () => {
        ...
        try {
            ...
            await axios.post(
                apiUrl('/scan/trigger'),
                { patternId: undefined, projectPath },
                { headers: authHeaders() }
            );
    ...
    const exportRoadmap = () => {
        ...
        const root = (data.sourceProjectPath || data.projectName || 'roadmap').replace(/[\/:\\\s]+/g, '-').slice(0, 60);
        a.href = url;
            ...
            <div className="mx-auto max-w-5xl p-6 space-y-6">
                ...
                <Card>
                    <CardContent className="flex flex-col items-center gap-3 py-12">
                        ...
                        <p className="text-sm text-foreground-muted">Generating remediation roadmap…</p>
                    </CardContent>
                </Card>
            </div>
        );
    ...
    const completionRate = summary.completionRate ?? data.progressMetrics?.completionRate ?? 0;
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <Card>
                    <CardContent className="flex flex-col items-center gap-1 py-4">
                        ...
                        <span className="text-2xl font-bold">{summary.completedFeatures ?? 0}</span>
                ...
                <Card>
                    <CardContent className="flex flex-col items-center gap-1 py-4">
                        ...
                        <span className="text-2xl font-bold">{summary.inProgressFeatures ?? 0}</span>
                ...
                <Card>
                    <CardContent className="flex flex-col items-center gap-1 py-4">
                        ...
                        <span className="text-2xl font-bold">{summary.plannedFeatures ?? 0}</span>
                        ...
                        {actionPlan.map((item, i) => (
                            <div key={i} className="flex items-start gap-3 rounded-lg border p-3">
                                <div className="flex flex-col gap-1 flex-1">
                                    ...
                                    {item.category && (
                                        <span className="text-xs text-foreground-muted">Category: {item.category}</span>
                                    )}
                            ...
                            <div key={i} className="flex items-start gap-3 rounded-lg border p-3">
                                ...
                                <div className="flex flex-col gap-1 flex-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {risk.category && (
                                            <span className="text-xs text-foreground-muted">{risk.category}</span>
                                        )}
                                    </div>
                                        ...
                                        {phase.features.map((feat, j) => {
                                            ...
                                            return (
                                                <div key={j} className="flex items-center gap-2">
                                                    ...
                                                    <Badge className={statusColor(featStatus)} variant="outline">
                                                        {featStatus}
                                                    </Badge>
                                                </div>
                                            );
                        ...
                        {recommendations.slice(0, 15).map((rec, i) => (
                            <div key={i} className="flex items-start gap-3 rounded-lg border p-3">
                                ...
                                <div className="flex flex-col gap-1 flex-1">
                                    ...
                                    {rec.description && (
                                        <span className="text-xs text-foreground-muted">{rec.description}</span>
                                    )}
