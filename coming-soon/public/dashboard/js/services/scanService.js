...
export class ScanService {
    ...
    async fetchAudit(includeNpmAudit = false) {
        ...
        try {
            ...
            const demoReportFlag =
                params.get('sb_demo_report') === '1' || localStorage.getItem('sb_demo_report') === '1';
            if (demoReportFlag) {
                try {
    ...
    exportIssuesCsv(issues) {
        ...
        const rows = issues.map(i =>
            [i.severity, i.type, i.description, i.filePath, i.count ?? 1, i.recommendedAction].map(escape).join(',')
        );
        downloadText(
