...
export function sanitizeAiProblemAnalyzerExport(analysisResult, options = {}) {
    ...
    const projectLabel = projectLabelFromPath(
        options.projectPath || options.context?.report?.projectRoot || analysisResult.projectPath || 'ai-platform'
    );
    ...
    const healthScore =
        options.context?.healthScore ?? options.context?.report?.qualityScore ?? sanitized.healthScore ?? null;
