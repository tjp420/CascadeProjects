...
export function sanitizeCompleteScanBundle(bundle, options = {}) {
  ...
  nestedOptions.repositoryFilesTotal =
    sb?.repositoryFilesTotal ?? sb?.repositoryInventory?.totalFiles;
  ...
  next.summary.platformScope = {
    ...
    simplebeaconGatePass:
      benchmarkScan && hollowGate
        ? null
        : (sb?.gate?.pass ?? next.summary.simplebeaconGatePass ?? null),
    simplebeaconGateAttestation:
      benchmarkScan && hollowGate
        ? "limited-benchmark"
        : benchmarkScan
  ...
  next.summary = {
    ...
    ...(sbFinal
      ? {
          simplebeaconGatePass:
            sbFinal.gate?.pass ?? next.summary.simplebeaconGatePass,
          simplebeaconIssues:
            sbFinal.issueCount ??
    ...
    ...(complianceResult?.summary || complianceResult?.checklist?.summary
      ? {
          compliancePassed:
            (complianceResult.checklist?.summary ?? complianceResult.summary)
              .passed ?? next.summary.compliancePassed,
          complianceFailed:
            (complianceResult.checklist?.summary ?? complianceResult.summary)
              .failed ?? next.summary.complianceFailed,
        }
      : {}),
    ...
    ...(mockScanResult
      ? {
          ...
          fictionDigestTrust:
            mockScanResult.digestTrust ?? next.summary.fictionDigestTrust,
        }
      : {}),
  };
  ...
  if (next.summary.platformScope) {
    next.summary.platformScope = {
      ...
      simplebeaconGatePass:
        next.summary.simplebeaconGatePass ?? sbFinal?.gate?.pass ?? null,
      simplebeaconGateAttestation:
        benchmarkScan && hollowGate
          ? "limited-benchmark"
          : benchmarkScan
  ...
  const auditFiles =
    sbFinal?.repositoryFilesTotal ?? sbFinal?.repositoryInventory?.totalFiles;
  ...
  if (!benchmarkScan) {
    ...
    const credentialScanned =
      sbFinal?.credentialScanned ?? sbFinal?.productionLeakScanned ?? null;
    ...
    const fictionJson =
      sbFinal?.fictionJsonFilesScanned ??
      sbFinal?.scanScope?.fictionJsonFilesScanned;
    const fictionSamples =
      sbFinal?.fictionSampleFilesScanned ?? sbFinal?.mockSampleFiles;
    ...
    const blockingCount =
      sbFinal?.gate?.blockingCount ?? sbFinal?.issueCount ?? null;
    ...
    if (
      sbFinal?.scanScope?.jestExecutedDuringScan === false ||
    ...
    const hygieneSummary = {
      ...
      jestBaselineChecked:
        sbFinal?.jestBaselineChecked === false ? false : undefined,
      attestationNote:
...
export function sanitizeRoadmapExport(roadmap, options = {}) {
  ...
  const benchmarkScan =
    ...
    isBenchmarkCachePath(
      roadmap.sourceProjectPath || roadmap.projectStructure?.projectRoot || "",
    );
  ...
  for (const [name, category] of Object.entries(
    roadmap.projectStructure?.mainCategories || {},
  )) {
  ...
  if (
    ...
    staleWalk &&
    next.projectStructure?.totalFiles != null
  ) {
