...
function isIntentionalConsolidationCandidate(item) {
   ...
   * @returns {any}
   */
  const paths = ((item === null || item === void 0 ? void 0 : item.files) || [])
    .map((file) =>
      normalizeRelPath(
        typeof file === "string"
          ? file
  ...
  if (
    (browserRe.test(a) || browserRe.test(b)) &&
    (toSource(a) === b || toSource(b) === a)
  )
    return true;
  /**
   * Is mcp config.
...
function upsertRow(map, path, row) {
  ...
  map.set(key, {
    ...existing,
    ...row,
    detail: [existing.detail, row.detail].filter(Boolean).join(" · "),
  });
}
/**
 * Rows from issues.
...
function rowsFromGateReport(report) {
  ...
  const docs =
    ((_a = report.euAiActSummary) === null || _a === void 0
      ? void 0
...
function rowsFromGateReportFixed(report) {
  ...
  for (const path of ((_a = report.euAiActSummary) === null || _a === void 0
    ? void 0
    : _a.documentationFound) || []) {
    upsertRow(map, path, {
      path: normalizeRelPath(path),
  ...
  const scoped =
    (_b = report.ruleScopedFilesAnalyzed) !== null && _b !== void 0
      ? _b
      : (_c = report.scanScope) === null || _c === void 0
        ? void 0
        : _c.ruleScopedFilesAnalyzed;
  const passImplicit =
    scoped != null ? Math.max(0, scoped - failN - warnN) : null;
  return {
    rows: [...map.values()],
    summary: {
      scoped,
...
function rowsFromConsolidation(scan) {
  const map = new Map();
  for (const item of (scan === null || scan === void 0
    ? void 0
  ...
  for (const group of (scan === null || scan === void 0
    ? void 0
    : scan.exactDuplicateGroups) || []) {
...
function rowsFromNpmAudit(npmAudit) {
  ...
  for (const vuln of list) {
    ...
    upsertRow(map, name, {
      path: name,
      status: ["critical", "high"].includes(String(vuln.severity).toLowerCase())
        ? "fail"
        : "warn",
      rule: "npm-audit",
      detail:
        vuln.title || vuln.recommendation || vuln.severity || "Vulnerability",
    });
  }
  if (
    !map.size &&
...
function rowsFromCleanupBrief(brief) {
  ...
  const map = new Map();
  const tiers =
    (brief === null || brief === void 0 ? void 0 : brief.tiers) || {};
  for (const entry of ((_a = tiers.safeNow) === null || _a === void 0
    ? void 0
    : _a.directories) || []) {
  ...
  for (const entry of ((_b = tiers.reviewFirst) === null || _b === void 0
    ? void 0
    : _b.items) || []) {
    upsertRow(map, entry.path, {
      path: normalizeRelPath(entry.path),
  ...
  for (const entry of ((_c = tiers.protected) === null || _c === void 0
    ? void 0
    : _c.directories) || []) {
    upsertRow(map, entry.path, {
      path: normalizeRelPath(entry.path),
...
function stepPayload(lastResult, stepId) {
  ...
  if (
    ...
    return (_b =
      (_a = lastResult.steps) === null || _a === void 0
        ? void 0
        : _a.find((step) => step.id === stepId)) !== null && _b !== void 0
      ? _b
      : null;
  }
  return null;
  ...
  switch (mode) {
    case "simplebeacon":
      if (
        (lastResult === null || lastResult === void 0
          ? void 0
          : lastResult.kind) === "simplebeacon-report"
      )
      ...
      if (
        ...
        return {
          kind: "gate",
          report:
            (_a = stepPayload(lastResult, "simplebeacon")) === null ||
            _a === void 0
              ? void 0
              : _a.report,
        };
    ...
    case "eu-ai-act":
      if (
        (lastResult === null || lastResult === void 0
          ? void 0
          : lastResult.kind) === "eu-ai-act"
      )
        return {
          kind: "gate",
          report:
            (_b = lastResult.sprint) === null || _b === void 0
              ? void 0
              : _b.report,
        };
      if (
        ...
        return {
          kind: "gate",
          report:
            (_c = stepPayload(lastResult, "simplebeacon")) === null ||
            _c === void 0
              ? void 0
              : _c.report,
        };
    ...
    case "mock-scan":
      if (
        ...
        return {
          kind: "fiction",
          report: step === null || step === void 0 ? void 0 : step.report,
          issues:
            step === null || step === void 0 ? void 0 : step.fictionIssues,
        };
      }
      if (
        lastResult === null || lastResult === void 0
        ...
        return {
          ...
          issues: lastResult.fictionIssues,
        };
    ...
    case "consolidation":
      ...
      if (
        ...
        return {
          kind: "consolidation",
          scan:
            (_d = stepPayload(lastResult, "consolidation")) === null ||
            _d === void 0
              ? void 0
              : _d.scan,
        };
    ...
    case "codebase":
      ...
      if (
        ...
        return {
          kind: "codebase",
          scan:
            (_e = stepPayload(lastResult, "codebase")) === null || _e === void 0
              ? void 0
              : _e.scan,
        };
      return null;
    ...
    case "data-quality":
      ...
      if (
        ...
        return {
          kind: "cleanup",
          scan:
            (_f = stepPayload(lastResult, mode)) === null || _f === void 0
              ? void 0
              : _f.scan,
        };
      return null;
    case "cleanup-assistant":
      if (
        (lastResult === null || lastResult === void 0
          ? void 0
          : lastResult.kind) === "cleanup-assistant"
      )
      ...
      if (
        ...
        return {
          kind: "cleanup-brief",
          brief:
            (_g = stepPayload(lastResult, "cleanup-assistant")) === null ||
            _g === void 0
              ? void 0
              : _g.brief,
        };
    ...
    case "compliance":
      ...
      if (
        ...
        return {
          kind: "compliance",
          checklist:
            (_h = stepPayload(lastResult, "compliance")) === null ||
            _h === void 0
              ? void 0
              : _h.checklist,
        };
    ...
    case "npm-audit":
      ...
      if (
        ...
        return {
          kind: "npm",
          npmAudit:
            (_j = stepPayload(lastResult, "npm-audit")) === null ||
            _j === void 0
              ? void 0
              : _j.npmAudit,
        };
    ...
    case "roadmap":
      ...
      if (
        ...
        return {
          kind: "roadmap",
          data:
            (_k = stepPayload(lastResult, "roadmap")) === null || _k === void 0
              ? void 0
              : _k.data,
        };
      return null;
...
function buildRows(mode, payload) {
  ...
  if (payload.kind === "fiction") {
    const issues =
      payload.issues ||
      (payload.report
        ? payload.report.rawIssues || payload.report.detectedIssues || []
        : []);
    const fictionOnly = issues.filter((i) =>
      /fiction|fictional|consistency|kpi/i.test(String(i.type || "")),
    );
    return {
      rows: rowsFromIssues(
        fictionOnly.length ? fictionOnly : issues,
        "fiction",
      ),
      summary: {
        scoped:
          (_b =
            (_a = payload.report) === null || _a === void 0
              ? void 0
              : _a.fictionJsonFilesScanned) !== null && _b !== void 0
            ? _b
            : (_d =
                  (_c = payload.report) === null || _c === void 0
                    ? void 0
                    : _c.scanScope) === null || _d === void 0
              ? void 0
    ...
    return {
      ...
      summary: {
        scoped:
          (_g =
            (_f =
              (_e = payload.scan) === null || _e === void 0
                ? void 0
                : _e.summary) === null || _f === void 0
              ? void 0
              : _f.jsonFilesAnalyzed) !== null && _g !== void 0
            ? _g
            : (_j =
                  (_h = payload.scan) === null || _h === void 0
                    ? void 0
                    : _h.summary) === null || _j === void 0
              ? void 0
              : _j.sampleDataFilesAnalyzed,
      },
      note: null,
    };
  }
  if (payload.kind === "codebase") {
    return {
      rows: rowsFromFindings(
        ((_k = payload.scan) === null || _k === void 0
          ? void 0
          : _k.findings) || [],
        "codebase",
      ),
      summary: {
        scoped:
          (_o =
            (_m =
              (_l = payload.scan) === null || _l === void 0
                ? void 0
                : _l.summary) === null || _m === void 0
              ? void 0
              : _m.filesScanned) !== null && _o !== void 0
            ? _o
            : (_q =
                  (_p = payload.scan) === null || _p === void 0
                    ? void 0
                    : _p.summary) === null || _q === void 0
              ? void 0
              : _q.codeFilesAnalyzed,
      },
      note: null,
    };
  }
  if (payload.kind === "cleanup") {
    ...
    const top =
      (scan === null || scan === void 0 ? void 0 : scan.topFindings) || [];
    const scannerFindings = [];
    for (const [scannerId, block] of Object.entries(
      (scan === null || scan === void 0 ? void 0 : scan.scanners) || {},
    )) {
      const items =
        ((_r = block === null || block === void 0 ? void 0 : block.findings) ===
          null || _r === void 0
          ? void 0
          : _r.items) ||
        (block === null || block === void 0 ? void 0 : block.findings) ||
        [];
    ...
    return {
      rows: rowsFromFindings(
        [...top, ...scannerFindings],
        (scan === null || scan === void 0 ? void 0 : scan.scanProfile) ||
          "cleanup",
      ),
      summary: {
        scoped:
          (_t =
            (_s =
              scan === null || scan === void 0 ? void 0 : scan.inventory) ===
              null || _s === void 0
              ? void 0
  ...
  if (payload.kind === "cleanup-brief") {
    return {
      rows: rowsFromCleanupBrief(payload.brief),
      summary: null,
      note: null,
    };
  }
  ...
  if (payload.kind === "roadmap") {
    const phases =
      ((_w =
        (_v = payload.data) === null || _v === void 0 ? void 0 : _v.roadmap) ===
        null || _w === void 0
        ? void 0
        : _w.phases) ||
      ((_x = payload.data) === null || _x === void 0 ? void 0 : _x.phases) ||
      [];
    return {
  ...
  if (payload.kind === "complete" && payload.steps) {
    ...
    for (const step of payload.steps) {
      ...
      for (const row of built.rows) {
        ...
        const isStaleMerge =
          subMode === "consolidation" && path.includes("roadmap.html");
        if (isDotfileNoise || isStaleMerge) continue;
        upsertRow(map, `${step.id}:${row.path}`, {
          ...row,
          path: row.path,
          rule: `${step.id} · ${row.rule}`,
        });
      }
    }
    return {
      rows: [...map.values()],
      summary: null,
      note: "Complete bundle — combined rows from all ten steps.",
    };
  }
...
function sortRows(rows) {
  ...
  return [...rows].sort((a, b) => {
    var _a, _b;
    return (
      ((_a = order[a.status]) !== null && _a !== void 0 ? _a : 9) -
        ((_b = order[b.status]) !== null && _b !== void 0 ? _b : 9) ||
...
export function renderModeFileResultsPanel(modeValue, context = {}) {
  ...
  const nodeModulesRows = rows.filter((r) =>
    String(r.path).includes("node_modules"),
  );
  const visibleRows = rows.filter(
    (r) => !String(r.path).includes("node_modules"),
  );
  ...
  const implicitNote =
    (summary === null || summary === void 0 ? void 0 : summary.passImplicit) > 0
      ? `${formatNumber(summary.passImplicit)} additional gate-scoped file(s) passed with no listed findings.`
      : "";
  const scopedNote =
    (summary === null || summary === void 0 ? void 0 : summary.scoped) != null
