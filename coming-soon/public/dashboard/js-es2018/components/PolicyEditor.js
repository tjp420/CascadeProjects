...
function readGateSelection(root, group) {
  return SEVERITIES.filter((sev) => {
    const input = root.querySelector(
      `[data-policy-gate="${group}"][value="${sev}"]`,
    );
...
function renderPreview(config) {
  ...
  const enabledRules = Object.entries(rules).filter(
    ([, r]) => r && r.enabled !== false,
  );
  const disabledRules = Object.entries(rules).filter(
    ([, r]) => r && r.enabled === false,
  );
            ...
            if (preset && preset.rules) {
              ...
              for (const [ruleName, ruleCfg] of Object.entries(preset.rules)) {
                if (draftConfig.rules[ruleName]) {
                  draftConfig.rules[ruleName].enabled =
                    ruleCfg.enabled !== false;
                } else {
                  draftConfig.rules[ruleName] = {
                    enabled: ruleCfg.enabled !== false,
                  };
                }
              }
            }
  ...
  function bindRuleToggles() {
    root.querySelectorAll("[data-policy-rule]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        btn.setAttribute(
          "aria-label",
          `${newEnabled ? "Disable" : "Enable"} ${formatRuleName(name)}`,
        );
        syncFromDom();
      });
    });
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      ...
      if (newRoot) {
        ...
        const newProfileSelect = newRoot.querySelector(
        ...
        if (newSaveBtn) {
          newSaveBtn.addEventListener("click", async () => {
            ...
            try {
              if (
                !scanService ||
                typeof scanService.saveConfig !== "function"
              ) {
                throw new Error("Scan service not available");
              }
