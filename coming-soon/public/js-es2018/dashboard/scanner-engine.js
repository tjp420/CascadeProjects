    ...
    for (let li = 0; li < lines.length && matches.length < maxMatches; li++) {
        ...
        if (regex.test(line)) {
            ...
            matches.push({
                ...
                snippet: snippet,
                context: [
                    ((_a = lines[Math.max(0, li - 1)]) === null || _a === void 0 ? void 0 : _a.trim().slice(0, 100)) ||
                        null,
                    snippet,
                    ((_b = lines[Math.min(lines.length - 1, li + 1)]) === null || _b === void 0
                        ? void 0
                        : _b.trim().slice(0, 100)) || null
                ].filter(Boolean)
            });
        }
    }
...
function buildSuggestedFixes(collections) {
    ...
    const isBuildArtifact = path =>
        /(^|\/)(node_modules|\.git|dist|build|\.next|out|coverage|frontend-build)\//i.test(path) ||
        /(^|\/)vscode-extension\/out\//i.test(path) ||
        /\.map$/i.test(path);
    ...
    const isFalsePositive = (file, type, snippet) => {
        ...
        if (
            type === 'Debug Artifact' &&
            /server\.cjs$/.test(fp) &&
            /console\.(warn|error)\s*\(\s*['"`]\[[A-Z][a-zA-Z]*\]/.test(s)
        )
            return true;
        // analyze-directory.js — CLI progress and summary output
        if (
            type === 'Debug Artifact' &&
            /analyze-directory\.js$/.test(fp) &&
            /console\.log\s*\(.*(?:TOTAL|FOLDERS|SIZE|LINES|DURATION|SKIPPED|Analysis complete|Report saved|Target:|Started:|files indexed|Top \d|Largest Files)/.test(
                s
            )
        )
            return true;
        ...
        if (
            type === 'Debug Artifact' &&
            /js\/dashboard\/main\.js$/.test(fp) &&
            /console\.(log|warn|error|info|debug)\s*\(.*\[[A-Za-z][a-zA-Z]*\]/.test(s)
        )
            return true;
        // token-manager.js — intentional or commented-out config logging
        if (
            type === 'Debug Artifact' &&
            /js\/dashboard\/token-manager\.js$/.test(fp) &&
            /console\.(log|warn|error|info|debug)\s*\(/.test(s)
        )
            return true;
        // Internal stub API imports (not leaks)
        if (type === 'Production Leak' && /setupDashboardStubAPIs|require\(['"]\.\/.*stub-api['"]\)|stub APIs/.test(s))
            return true;
        // Legitimate sample variables (not path leaks)
        if (
            type === 'Production Leak' &&
            /\b(?:sampleFiles|sampleUrl|sampleData|recentSamples|sampleCount|recentFiles)\b/.test(s)
        )
            return true;
        // UI config for sample reports
        if (type === 'Production Leak' && /sampleReportUrl|data-sample-report|applySampleReportLinks/.test(s))
            return true;
        ...
        if (
            ...
            /Article\s*50|EU AI Act|Transparency|high-risk/.test(s) &&
            /server\/|tools\//.test(fp)
        )
            return true;
        ...
        if (
            ...
            /build-extension\.js|build-public\.js|replace-dashboard\.js/.test(fp)
        )
            return true;
        ...
        if (type === 'Hardcoded Fiction KPI' && /uploadPanel\.ts$/.test(fp) && /return\s*\{\s*type:\s*['"]/.test(s))
            return true;
        // Type safety: VS Code API callbacks, generic utilities, dynamic tree structures
        if (type === 'Type Safety Gap' && /resolve:\s*\(value:\s*any\)|reject:\s*\(reason\?:\s*any\)/.test(s))
            return true;
        if (type === 'Type Safety Gap' && /postJson\(.*payload:\s*any|_uploadReport\(.*data:\s*any/.test(s))
            return true;
        ...
        if (
            ...
            /\/views\/(Audit|Quality|Security|Trust|Profile|Chatbot|SignIn)View\.js$/.test(fp) &&
            /container\.innerHTML|recoveryForm\.innerHTML/.test(s)
        )
            return true;
        // Config Drift: comment text and contextFilter documentation, not actual config
        if (
            type === 'Configuration Drift' &&
            /\/\/.*hardcoded|contextFilter|scanner-engine\.js|CONFIG_DRIFT_PATTERN/.test(s)
        )
            return true;
        if (type === 'Configuration Drift' && /browser-extension\//.test(fp)) return true;
        if (
            type === 'Configuration Drift' &&
            /Literal URL values and secrets in config files bypass environment controls/.test(s)
        )
            return true; // simplebeacon:production-leak-intent: scanner-fp - regex pattern description text, not actual secret
        ...
        if (
            ...
            /\/\*\*.*Build a Content-Security-Policy|Build a Content-Security-Policy meta tag/.test(s)
        )
            return true;
        if (
            type === 'Missing Security Header' &&
            /missing security header|security header|csp-source/.test(s) &&
            /return true|return false/.test(s)
        )
            return true;
        ...
        if (type === 'AI Residue' && /catch\s*\([^)]*\)\s*\{[^}]*console\.warn.*vscode\.postMessage/.test(s))
            return true;
        ...
        if (
            ...
            /id=["']?(layoutSelect|categoryFilter|autoScan|fullTreeCheck|chatbot-remove-filters)/.test(s)
        )
            return true;
        return false;
    };
    Object.entries(collections).forEach(([collectionName, findings]) => {
        ...
        findings.forEach(f => {
            ...
            if (type === 'Debug Artifact') {
                ...
                // Standardized auto-fix for AI-generated stubs: insert a clearly
                // identifiable TODO marker and an explicit fail-fast fallback.
                // The marker includes `[Auto-Generated Issue]` and an `AUTO_FIX`
                // tag so the scanner can group synthetic markers separately.
                replacement = `// TODO: [Auto-Generated Issue] Replace AI-generated stub with deterministic placeholder.\n// AUTO_FIX: ai_residue\n// Fallback: explicit error to fail fast during tests or CI\nthrow new Error('Auto-generated stub: replace implementation');`;
                suggestedPatch = `--- a/${file}\n+++ b/${file}\n@@ -${line},1 +${line},3 @@\n-${snippet}\n+// TODO: [Auto-Generated Issue] Replace AI-generated stub with deterministic placeholder.\n+// AUTO_FIX: ai_residue\n+throw new Error('Auto-generated stub: replace implementation');`;
...
const REPORT_SECTION_SCHEMA = [
    ...
    },
    {
        ...
        detail: 'any types, missing PropTypes, excessive params'
    },
    {
        section: 'testCoverage',
        hitsVar: 'testHits',
        findingsVar: 'testFindings',
        label: 'test coverage gap',
        detail: 'skipped/empty tests'
    },
    {
        section: 'accessibility',
        hitsVar: 'a11yHits',
        findingsVar: 'a11yFindings',
        label: 'accessibility gap',
        detail: 'missing alt, unlabeled inputs'
    },
    {
        section: 'i18n',
        hitsVar: 'i18nHits',
        findingsVar: 'i18nFindings',
        label: 'i18n issue',
        detail: 'hardcoded strings, locale-naive formatting'
    },
    {
    ...
    },
    {
        ...
        detail: 'literal URL values, .env in repo'
    },
    {
        ...
        detail: 'Review CSP, X-Frame-Options, HSTS coverage'
    },
    {
        ...
        detail: 'raw SQL concat, unbounded queries'
    },
    {
        ...
        detail: 'hook misuse, missing cleanup'
    },
    {
        ...
        detail: 'circular imports, mismatched deps'
    },
    {
        section: 'unusedDeps',
        ...
        detail: 'unused deps in package.json'
    },
    {
        section: 'apiContract',
        ...
        detail: 'unconsumed endpoints, stale OpenAPI'
    },
    {
        section: 'complexity',
    ...
    },
    {
        section: 'llmSlop',
        hitsVar: 'llmSlopHits',
        findingsVar: 'llmSlopFindings',
        label: 'LLM slop pattern',
        detail: 'placeholder debris, markdown fences, hardcoded metrics'
    },
    {
        section: 'tokenBleed',
        ...
        detail: 'long string literals in prompt content'
    },
    {
        ...
        detail: 'mock/fixture/sample path references'
    },
    {
        section: 'fictionKpi',
        ...
        detail: 'unverified metrics and completion rates'
    },
    {
        section: 'security',
        hitsVar: 'securityHits',
        findingsVar: 'securityFindings',
        label: 'security vulnerability',
        detail: 'eval, XSS, prototype pollution, insecure random, missing rate limits'
    },
    {
        section: 'quality',
        hitsVar: 'qualityHits',
        findingsVar: 'qualityFindings',
        label: 'code quality issue',
        detail: 'unhandled promises, missing strict mode, uninitialized reads'
    },
    {
        section: 'maintainability',
        hitsVar: 'maintainabilityHits',
        findingsVar: 'maintainabilityFindings',
        label: 'maintainability issue',
        detail: 'magic numbers, hardcoded literals'
    },
    {
        section: 'evalDanger',
        ...
        detail: 'eval(), new Function(), unsafe dynamic code execution'
    },
    {
        ...
        detail: 'unsanitized innerHTML assignments'
    },
    {
        ...
        detail: 'Object.prototype or __proto__ modification'
    },
    {
        ...
        detail: 'promise chains missing .catch() handlers'
    },
    {
        section: 'magicNumber',
        ...
        label: 'magic number',
        detail: 'hardcoded numeric literals without named constants'
    },
    {
        ...
        detail: 'files without use strict directive'
    },
    {
        ...
        detail: 'variables read before initialization'
    },
    {
        ...
        detail: 'redirects with user-controlled destinations'
    },
    {
        ...
        detail: 'API endpoints without rate limit protection'
    },
    {
        ...
        detail: 'Math.random() used for security-sensitive operations'
    },
    {
        ...
        detail: 'passwords, tokens, or secrets logged to console'
    },
    {
        ...
        detail: 'static confidence scores instead of computed values'
    },
    {
        ...
        detail: 'static completion rates instead of real metrics'
    },
    {
        ...
        detail: 'mock or fixture path references in production code'
    },
    {
        ...
        detail: 'sample JSON file references in production code'
    },
    {
        ...
        detail: 'license or copyright markers requiring review'
    },
    {
        ...
        detail: 'placeholder comments generated by AI'
    },
    {
        ...
        detail: 'placeholder block comments from AI'
    },
    {
        ...
        detail: 'markdown code fences leaked into source'
    },
    {
        ...
        detail: 'empty function bodies likely from AI generation'
    },
    {
        section: 'arrowStub',
        ...
        detail: 'arrow functions returning empty objects'
    },
    {
        ...
        detail: 'unresolved HACK, XXX, or WORKAROUND markers'
    }
];
/**
...
const CATEGORY_COLLECTORS = {
    aiIndicators: (path, reg, matches, ctx) => {
        ...
        appendTerminalLine(
            `AI indicator detected: <span style="color:#94A3B8;">${path}</span> (${matches.length} match${matches.length !== 1 ? 'es' : ''})`,
            'warn'
        );
    },
    ...
    aiResidue: (path, reg, matches, ctx) => {
        ...
        trackIndividualCollector(path, reg, enriched, ctx);
        appendTerminalLine(
            `AI residue detected (${reg.name}): <span style="color:#94A3B8;">${path}</span> (${matches.length} match${matches.length !== 1 ? 'es' : ''})`,
            'warn'
        );
    },
    ...
    sensitiveData: (path, reg, matches, ctx) => {
        ...
        const triaged = matches.map(m => {
            ...
            if (/node_modules\//.test(lowerPath)) {
                triageConfidence = 'low';
                category = 'third-party-metadata';
            } else if (/\.git\//.test(lowerPath)) {
                triageConfidence = 'low';
                category = 'git-metadata';
            } else if (
                /\.(md|txt|markdown)$/.test(lowerPath) &&
                /support|contact|hello|press|security|docs/.test(snippet)
            ) {
                triageConfidence = 'low';
                category = 'public-contact-info';
            } else if (/package\.json$/.test(lowerPath) && /author|contributor|license/.test(snippet)) {
                triageConfidence = 'low';
                category = 'package-metadata';
            } else if (
                /tools\/(generate|send|scan|test)/.test(lowerPath) ||
                /demo|fixture|mock|sample|test/.test(lowerPath)
            ) {
                triageConfidence = 'medium';
                category = 'tooling-or-test';
            } else if (/(console\.log|console\.warn).*token|password|secret/.test(snippet)) {
                triageConfidence = 'high';
                category = 'secret-in-log';
            } else if (/\.env/.test(lowerPath)) {
                triageConfidence = 'high';
                category = 'env-secret';
            } else if (/localstorage\.setitem.*token|auth|session|password/.test(snippet)) {
                triageConfidence = 'high';
                category = 'client-storage-secret';
            }
            ...
            return {
                ...m,
                triage: { confidence: triageConfidence, category },
                dynamicSeverity: dynamicSev,
        ...
        ctx.sensitiveDataFindings.push({
            file: path,
            type: reg.name,
            matches: triaged,
...
const CUSTOM_HANDLER_REGISTRY = {
    gate: {
        ...
        collect: function (collector, ctx) {
            if (collector) ctx.credentialFindings = (ctx.credentialFindings || []).concat(collector);
        }
    },
    consolidation: {
        category: 'governance',
        collect: function (collector, ctx) {
            if (collector) ctx.consolidation = collector;
        }
    },
    'mock-data': {
        category: 'aiResidue',
        collect: function (collector, ctx) {
            if (collector) ctx.mockDataCategories = collector;
        }
    },
    roadmap: {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.roadmap = collector;
        }
    },
    codebase: {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.codebase = collector;
        }
    },
    'file-reduction': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.fileReduction = collector;
        }
    },
    'data-quality': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.dataQuality = collector;
        }
    },
    cleanup: {
        category: 'debug',
        collect: function (collector, ctx) {
            if (collector) ctx.cleanup = collector;
        }
    },
    'npm-audit': {
        category: 'unusedDeps',
        collect: function (collector, ctx) {
            if (collector) ctx.npmAudit = collector;
        }
    },
    compliance: {
        category: 'governance',
        collect: function (collector, ctx) {
            if (collector) ctx.compliance = collector;
        }
    },
    'eu-ai-act': {
        category: 'aiIndicators',
        collect: function (collector, ctx) {
            if (collector) ctx.euAiActSummary = collector;
        }
    },
    'dependency-vulns': {
        category: 'unusedDeps',
        collect: function (collector, ctx) {
            if (collector) ctx.dependencyAudit = collector;
        }
    },
    'build-readiness': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.buildReadiness = collector;
        }
    },
    'junk-files': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.junkFiles = collector;
        }
    },
    'ai-residue': {
        category: 'aiResidue',
        collect: function (collector, ctx) {
            if (collector) ctx.aiResidueFindings = (ctx.aiResidueFindings || []).concat(collector);
        }
    },
    performance: {
        category: 'performance',
        collect: function (collector, ctx) {
            if (collector) ctx.performanceFindings = (ctx.performanceFindings || []).concat(collector);
        }
    },
    'type-safety': {
        category: 'typeSafety',
        collect: function (collector, ctx) {
            if (collector) ctx.typeSafetyFindings = (ctx.typeSafetyFindings || []).concat(collector);
        }
    },
    documentation: {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.documentationFindings = (ctx.documentationFindings || []).concat(collector);
        }
    },
    'test-coverage': {
        category: 'testCoverage',
        collect: function (collector, ctx) {
            if (collector) ctx.testCoverageFindings = (ctx.testCoverageFindings || []).concat(collector);
        }
    },
    accessibility: {
        category: 'accessibility',
        collect: function (collector, ctx) {
            if (collector) ctx.accessibilityFindings = (ctx.accessibilityFindings || []).concat(collector);
        }
    },
    i18n: {
        category: 'i18n',
        collect: function (collector, ctx) {
            if (collector) ctx.i18nFindings = (ctx.i18nFindings || []).concat(collector);
        }
    },
    'database-patterns': {
        category: 'databasePatterns',
        collect: function (collector, ctx) {
            if (collector) ctx.databasePatternsFindings = (ctx.databasePatternsFindings || []).concat(collector);
        }
    },
    'framework-practices': {
        category: 'frameworkPractices',
        collect: function (collector, ctx) {
            if (collector) ctx.frameworkPracticesFindings = (ctx.frameworkPracticesFindings || []).concat(collector);
        }
    },
    'workspace-health': {
        category: 'workspaceHealth',
        collect: function (collector, ctx) {
            if (collector) ctx.workspaceHealthFindings = (ctx.workspaceHealthFindings || []).concat(collector);
        }
    },
    'unused-deps': {
        category: 'unusedDeps',
        collect: function (collector, ctx) {
            if (collector) ctx.unusedDepsFindings = (ctx.unusedDepsFindings || []).concat(collector);
        }
    },
    'api-contract': {
        category: 'apiContract',
        collect: function (collector, ctx) {
            if (collector) ctx.apiContractFindings = (ctx.apiContractFindings || []).concat(collector);
        }
    },
    complexity: {
        category: 'complexity',
        collect: function (collector, ctx) {
            if (collector) ctx.complexityFindings = (ctx.complexityFindings || []).concat(collector);
        }
    },
    'fix-preview': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.fixPreview = collector;
        }
    },
    'file-naming': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.fileNamingFindings = (ctx.fileNamingFindings || []).concat(collector);
        }
    },
    'removable-files': {
        category: 'maintainability',
        collect: function (collector, ctx) {
            if (collector) ctx.removableFilesFindings = (ctx.removableFilesFindings || []).concat(collector);
        }
    }
};
/**
...
function getAnalyzersForLanguage(langKey, profile) {
    ...
    for (const [id, entry] of Object.entries(PATTERN_REGISTRY)) {
        ...
        const cat = ((_a = ANALYZER_SCHEMA[id]) === null || _a === void 0 ? void 0 : _a.category) || '';
...
function buildEuAiActControls(aiHits, licenseFiles, securityFiles, aiFindings = []) {
    const controls = [];
    const docCount =
        ((licenseFiles === null || licenseFiles === void 0 ? void 0 : licenseFiles.length) || 0) +
        ((securityFiles === null || securityFiles === void 0 ? void 0 : securityFiles.length) || 0);
    ...
    controls.push({
        ...
        evidence:
            prohibitedCount > 0
                ...
                : aiCount > 0
                  ? `${aiCount} generic AI SDK import(s) — no prohibited-practice indicators found.`
                  : 'None detected',
        action:
            prohibitedCount > 0
    ...
    const annexIIIPatterns = [
        {
            ...
            pattern:
                /\b(police|law enforcement|criminal justice|arrest|warrant|patrol|surveillance cam|detention|prison|probation|parole|investigat(e|ion))\b/i
        },
        {
            domain: 'migration / border control',
            pattern: /\b(border|immigration|asylum|visa|refugee|migrant|deportation|customs|passport|entry|exit)\b/i
        },
        {
            domain: 'democratic processes',
            pattern: /\b(election|vote|ballot|campaign|polling|voter|referendum|democratic|candidate)\b/i
        },
        {
            domain: 'employment / recruitment',
            pattern:
                /\b(recruit|hire|employment|worker|employee|resume|interview|job|candidate|performance review|workplace|HR|human resources|talent)\b/i
        },
        {
            domain: 'education',
            pattern:
                /\b(student|school|education|university|college|grade|exam|assessment|academic|course|enrollment|admission|curriculum)\b/i
        },
        {
            domain: 'healthcare / medical',
            pattern:
                /\b(hospital|patient|diagnosis|medical|healthcare|treatment|clinical|prescription|symptom|disease|therapy|surgery)\b/i
        },
        {
            domain: 'credit / insurance / finance',
            pattern:
                /\b(credit score|credit rating|loan|insurance|underwriting|premium|claim|mortgage|banking|finance|investment)\b/i
        },
        {
            domain: 'critical infrastructure / transport',
            pattern:
                /\b(transport|traffic|vehicle|aircraft|railway|subway|energy|power grid|utility|water supply|telecommunication|nuclear|dam)\b/i
        },
        {
            domain: 'administration of justice',
            pattern: /\b(judge|court|judicial|trial|legal|litigation|verdict|sentence|evidence|witness)\b/i
        }
    ];
    ...
    controls.push({
        ...
        evidence: isHighRiskDomain
            ...
            : aiCount > 0
              ? `${aiCount} generic AI indicator(s); ${hasDocs ? docCount + ' governance doc(s) present — verify Annex III classification is explicitly documented' : '0 governance docs — add risk-assessment.md'}`
              : 'None detected',
    ...
    controls.push({
        ...
        evidence:
            aiCount > 0
                ? `${aiCount} AI indicator(s); ${hasDocs ? docCount + ' governance doc(s) present — verify data governance coverage' : 'No data governance documentation detected'}`
                : 'None detected',
        action:
            aiCount > 0
    ...
    controls.push({
        ...
        evidence:
            aiCount > 0
                ? `${aiCount} AI indicator(s); ${hasDocs ? docCount + ' governance doc(s) present — verify oversight coverage' : 'No oversight documentation detected'}`
                : 'None detected',
        action:
            aiCount > 0
    ...
    controls.push({
        ...
        evidence:
            aiCount > 0
                ? `${hasDocs ? docCount + ' doc(s) present — verify risk management coverage' : 'No risk management documentation detected'}`
                : 'None detected',
        action:
            aiCount > 0
...
function analyzeFileNaming(paths) {
    ...
    for (const rawPath of paths) {
        ...
        if (/\s/.test(fileName)) {
            findings.push({
                file: path,
                type: 'Space in Filename',
                severity: 'medium',
                detail: 'Spaces in filenames break CLI pipelines and cause quoting issues. Use kebab-case or snake_case.',
                suggestion: fileName.replace(/\s+/g, '-')
            });
        }
        ...
        if (/[^a-zA-Z0-9._\-]/.test(baseName) && !/\s/.test(fileName)) {
            findings.push({
                file: path,
                type: 'Special Characters in Filename',
                severity: 'medium',
                detail: 'Special characters in filenames cause cross-platform compatibility issues and shell escaping problems.',
                suggestion: baseName.replace(/[^a-zA-Z0-9._\-]/g, '_') + (ext ? '.' + ext : '')
            });
        }
        ...
        if (baseName.length > 60) {
            findings.push({
                file: path,
                type: 'Overly Long Filename',
                severity: 'low',
                detail: `Filename is ${baseName.length} chars — long names are hard to read and can exceed OS limits.`,
                suggestion: 'Shorten to core descriptor words.'
            });
        }
        ...
        if (/v?\d+\.\d+|_\d+_\d+_\d+|-\d{4}-\d{2}-\d{2}/.test(baseName)) {
            findings.push({
                file: path,
                type: 'Version/Date in Filename',
                severity: 'low',
                detail: 'Version numbers or dates in filenames suggest manual versioning instead of Git. Rename to canonical name.',
                suggestion:
                    baseName
                        .replace(/v?\d+\.\d+.*$/, '')
                        .replace(/_\d+_\d+_\d+.*$/, '')
                        .replace(/-\d{4}-\d{2}-\d{2}.*$/, '') + (ext ? '.' + ext : '')
            });
        }
        // Data handling: files that contain data but have generic names
        if (
            /\.(json|csv|xml|yaml|yml|sql|db|sqlite|parquet|xlsx)$/.test(ext) &&
            /^(data|file|output|result|temp|tmp|export|import|backup|copy|new|old|test|sample)\d*$/i.test(baseName)
        ) {
            findings.push({
                file: path,
                type: 'Generic Data Filename',
                severity: 'medium',
                detail: 'Data files with generic names (data.json, file.csv) make it impossible to understand content without opening them.',
                suggestion: 'Use descriptive names: users-export-2024.json, monthly-sales.csv'
            });
        }
        ...
        if (ext && /[A-Z]/.test(fileName.slice(fileName.lastIndexOf('.') + 1))) {
            findings.push({
                file: path,
                type: 'Mixed-Case Extension',
                severity: 'low',
                detail: 'File extensions should be lowercase for cross-platform consistency.',
                suggestion: fileName.slice(0, fileName.lastIndexOf('.')) + '.' + ext.toLowerCase()
            });
        }
    ...
    if (totalNamed > 0) {
        const dominant = Object.entries(styleStats)
            .filter(([k]) => k !== 'other')
            .sort((a, b) => b[1] - a[1])[0];
        ...
        if (nonDominant.length > 0 && dominant[1] > 5) {
            ...
            findings.unshift({
                file: 'project-root',
                type: 'Mixed Naming Conventions',
                severity: 'low',
                detail: `Dominant style is ${dominant[0]} (${dominant[1]} files), but also found: ${mixDetail}. Consistent naming improves readability and reduces cognitive load.`,
                suggestion: `Standardize on ${dominant[0]} across the codebase.`
            });
        }
    }
    ...
    for (const group of Object.values(lowerDirs)) {
        if (group.length > 1) {
            findings.push({
                file: 'project-root',
                type: 'Case-Conflicting Directory Names',
                severity: 'medium',
                detail: `Directories differ only by case: ${group.join(', ')}. This causes problems on case-insensitive filesystems.`,
                suggestion: 'Consolidate to a single casing.'
            });
        }
    }
...
function analyzeRemovableFiles(files) {
    const categories = {
        node_modules: {
            ...
            bytes: 0,
            examples: [],
            action: 'Delete — run npm install to regenerate. Do not commit to Git.',
            removable: true
        },
        git: {
            label: '.git (version control metadata)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Keep for dev. Safe to remove from deployment artifacts only.',
            removable: false
        },
        build_artifacts: {
            label: 'Build output directories (dist, build, .next, out)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Delete — regenerated by build scripts. Add to .gitignore.',
            removable: true
        },
        cache: {
            label: 'Cache directories (.cache, .turbo, __pycache__)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Delete — temporary cache. Regenerated automatically.',
            removable: true
        },
        coverage: {
            ...
            bytes: 0,
            examples: [],
            action: 'Delete — regenerated by test runs. Add to .gitignore.',
            removable: true
        },
        logs: {
            label: 'Log files (*.log)',
            ...
            bytes: 0,
            examples: [],
            action: 'Delete — runtime logs. Add *.log to .gitignore.',
            removable: true
        },
        editor_files: {
            label: 'IDE / editor config files (.vscode, .idea)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Review — some teams share these, others keep them local.',
            removable: false
        },
        temp_backup: {
            label: 'Temporary / backup files (*.tmp, *.bak, *~)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Delete — temporary files. Add to .gitignore.',
            removable: true
        },
        lockfiles: {
            label: 'Package manager lockfiles',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Keep one per package.json. Extra lockfiles may be from nested projects.',
            removable: false
        },
        empty_files: {
            label: 'Empty files (0 bytes)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Delete — empty files serve no purpose.',
            removable: true
        },
        snapshots: {
            label: 'Test snapshots (__snapshots__)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Review — can be regenerated but may be needed for CI.',
            removable: false
        },
        scan_artifacts: {
            label: 'Self-generated scan artifacts (.simplebeacon)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Delete — generated by previous scans.',
            removable: true
        },
        os_metadata: {
            label: 'OS metadata files (.DS_Store, Thumbs.db)',
            count: 0,
            bytes: 0,
            examples: [],
            action: 'Delete — should never be in version control.',
            removable: true
        }
    };
    const pathToCategory = [
        ...
        { pattern: /(?:^|\/)\.git(?:\/|$)/, cat: 'git' },
        {
            pattern: /(?:^|\/)(dist|build|out|\.next|\.nuxt|\.output|target|public\/build)(?:\/|$)/,
            cat: 'build_artifacts'
        },
        {
            pattern:
                /(?:^|\/)(\.cache|\.turbo|\.parcel-cache|\.grunt|\.sass-cache|__pycache__|\.eslintcache|\.stylelintcache)(?:\/|$)/,
            cat: 'cache'
        },
        ...
        {
            pattern:
                /(?:^|\/)(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|poetry\.lock|Gemfile\.lock|composer\.lock|Cargo\.lock|mix\.lock|go\.sum)$/,
            cat: 'lockfiles'
        },
...
async function processLocalCLIScan(files) {
    var _a, _b, _c, _d, _e;
    appendTerminalLine(
        `<span style="color:#60A5FA;font-weight:700;">&#9654;</span> processLocalCLIScan: received ${files.length.toLocaleString()} files`,
        'info'
    );
    reportData = null;
    ...
    if (_deduped.length < files.length) {
        ...
        appendTerminalLine(
            `<span style="color:#F59E0B;">&#9888; WARN:</span> Removed ${removed.toLocaleString()} duplicate file(s) from symlink/junction traversal.`,
            'warn'
        );
        files = _deduped;
    }
    ...
    function isProjectRoot(prefix) {
        ...
        const hasJsRoot =
            paths.some(p => p === prefix + '/package.json') ||
    ...
    if (candidate) {
        ...
        if (localScanFileName) {
            ...
            appendTerminalLine(
                `<span style="color:#60A5FA;">&#9432;</span> Auto-detected project root: <strong>${detectedRoot}</strong> — scanning ${files.length.toLocaleString()} files.${dropMsg}`
            );
        }
    }
    ...
    if (downloadLogBtn) {
        ...
        downloadLogBtn.onclick = () => {
            try {
                ...
                const lines = Array.from(tc.children)
                    .map(div => {
                        var _a;
                        const ts =
                            ((_a = div.querySelector('span')) === null || _a === void 0 ? void 0 : _a.textContent) ||
                            '';
        ...
        copyLogBtn.onclick = () => {
            ...
            if (text.length > MAX_CLIPBOARD_CHARS) {
                text =
                    ...
                    '\n\n[...truncated ' +
                    (text.length - MAX_CLIPBOARD_CHARS).toLocaleString() +
                    ' chars...]';
                truncated = true;
            }
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard
                    ...
                    .then(() => {
                        showToast(
                            'Log copied to clipboard.' + (truncated ? ' (truncated to 100K chars)' : ''),
                            'success'
                        );
                    })
                    .catch(e => {
    ...
    function fallbackCopy(text, truncated) {
        try {
            ...
            ta.style.cssText =
                'position:fixed;top:0;left:0;opacity:0;width:2em;height:2em;padding:0;border:none;z-index:1;';
    ...
    try {
        ...
        const profile = (typeof BROWSER_SCAN_PROFILES !== 'undefined' && BROWSER_SCAN_PROFILES[profileKey]) || {
            label: 'Complete Scan',
            checkAi: true,
        ...
        const allowedSections =
            (typeof PROFILE_SECTIONS !== 'undefined' && (PROFILE_SECTIONS[profileKey] || PROFILE_SECTIONS.complete)) ||
            ALL_SECTIONS; // simplebeacon-ignore
        const tier =
            ((_a = window._tokenPayload) === null || _a === void 0 ? void 0 : _a.tier) ||
            ((_b = window._tokenPayload) === null || _b === void 0 ? void 0 : _b.product) ||
            'locked'; // simplebeacon-ignore
        ...
        const localScanFileName = document.getElementById('localScanFileName');
        if (localScanFileName)
            localScanFileName.textContent = `Preparing to scan ${files.length.toLocaleString()} files...`;
        appendTerminalLine('simplebeacon scan .', 'input');
        appendTerminalLine(
            `Initializing Local Web Worker Core... Profile: ${profile.label}. Allocating isolated browser memory structures.`
        );
        // Security baseline: only exclude dependency trees by default (user can override via checkbox)
        const includeDeps =
            ((_c = document.getElementById('includeDepsToggle')) === null || _c === void 0 ? void 0 : _c.checked) ||
            false;
        const deepScan =
            ((_d = document.getElementById('deepScanToggle')) === null || _d === void 0 ? void 0 : _d.checked) || false;
        ...
        const nodeModuleCount = files.filter(f =>
            /node_modules\//.test((f.webkitRelativePath || f.name).toLowerCase())
        ).length;
        const gitCount = files.filter(f => /\.git\//.test((f.webkitRelativePath || f.name).toLowerCase())).length;
        const buildArtifactCount = files.filter(f =>
            /\/(dist|build|\.next|out|coverage|frontend-build)\//i.test((f.webkitRelativePath || f.name).toLowerCase())
        ).length;
        const cacheDirCount = files.filter(f =>
            /\/(\.simplebeacon|\.github-sync|github-cache|\.cursor|\.windsurf)\//i.test(
                (f.webkitRelativePath || f.name).toLowerCase()
            )
        ).length;
        const lockfileCount = files.filter(f =>
            (f.webkitRelativePath || f.name).toLowerCase().endsWith('package-lock.json')
        ).length;
        const archiveCount = files.filter(f =>
            /\.(zip|tgz)$/.test((f.webkitRelativePath || f.name).toLowerCase())
        ).length;
        const binaryCount = files.filter(f =>
            /\.(png|jpe?g|gif|webp|ico|bmp|mp3|mp4|wav|zip|exe|dll|pdf|doc|docx)$/.test(
                (f.webkitRelativePath || f.name).toLowerCase()
            )
        ).length;
        ...
        appendTerminalLine(
            `Estimator: ~${estTimeSec}s scan time · ${nodeModuleCount} node_modules files · ${gitCount} .git files detected.`
        );
        if (nodeModuleCount > 100 && !includeDeps) {
            appendTerminalLine(
                `Note: ${nodeModuleCount} node_modules files will be excluded by default. If you intended to scan source only, this is expected.`,
                'success'
            );
        } else if (nodeModuleCount > 100 && includeDeps) {
            appendTerminalLine(
                `Warning: ${nodeModuleCount} node_modules files will be scanned. Expect slower performance && potential false positives.`,
                'warn'
            );
        }
        if (includeDeps)
        ...
        if (deepScan)
            appendTerminalLine(
                'DEEP SCAN MODE: All cache, docs, && scanner-page filters bypassed. Only secrets && 500MB+ files are excluded.',
                'warn'
            );
        ...
        const fileListHtml = files
            ...
            .map(f =>
                (f.webkitRelativePath || f.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
            )
            .join('\n');
        ...
        let todoFiles = [];
        // Synthetic TODOs (auto-generated by the scanner/auto-fix flow) are
        // tracked separately so they don't pollute user-visible TODO counts.
        let syntheticTodoFiles = [];
        const todoMarkerCounts = { tasks: 0, fixes: 0, hacks: 0, bugs: 0, notes: 0 };
        const syntheticTodoMarkerCounts = { total: 0 };
        let imageFiles = [];
        ...
        for (const ignoreFile of ignoreFiles) {
            try {
                ignorePatterns.push(...parseIgnoreFile(await ignoreFile.text()));
            } catch (e) {
                /* ignore parse errors */
            }
        }
        ...
        if (detectedProjectRoot && ignorePatterns.length) {
            ...
            ignorePatterns = ignorePatterns.filter(pat => {
                if (pat instanceof RegExp) {
                    ...
                    if (
                        rootRegex.test(src + '/') ||
                        (pat.test(detectedProjectRoot + '/') && !pat.test(detectedProjectRoot + '/sub/'))
                    ) {
                        return false;
                    }
                    return true;
        ...
        appendTerminalLine(
            `<span style="color:#60A5FA;font-weight:700;">&#9654; Stage 2/3:</span> Filtering ${files.length.toLocaleString()} files (excluding dependencies, secrets, generated artifacts)...`
        );
        ...
        for (const f of files) {
            ...
            const includeVendorDirs =
                ((_e = window._scanOptions) === null || _e === void 0 ? void 0 : _e.includeVendorDirs) || false;
            if (
                skipDeps &&
                !includeVendorDirs &&
                !reason &&
                (isSimplebeaconMonorepo
                    ? /(^|\/)(node_modules|\.git|\.github-sync|github-cache|\.cursor|\.windsurf|\.cursor-tutor|\.vscode|\.idea|\.husky|\.simplebeacon|backups|java-ai-vulnerable|out)\//i.test(
                          path
                      )
            ...
            if (
                !reason &&
                !deepScan &&
                /(^|\/)(docs\/|doc\/|third_party\/|thirdparty\/|geedocs\/|mapfiles\/|vendor\/|\.min\.js$|\.bundle\.min\.js$|\.pack\.js$)/i.test(
                    path
                )
            ) {
            ...
            if (isSimplebeaconMonorepo) {
                ...
                if (
                    !reason &&
                    /(^|\/)(simplebeacon-report-|simplebeacon-cascadeprojects-|simplebeacon-latest-|simplebeacon-full-|simplebeacon-export-|complete-scan|report-export|simplebeacon-report-export).*\.json$/i.test(
                        path
                    )
                ) {
                ...
                if (
                    !reason &&
                    /(^|\/)(knip-report|git-status|test-output-root|test-results|tier-accounts|tier-tokens|MASTER-TOKEN|ai-slop-cop-pro-token)\.txt$/i.test(
                        path
                    )
                ) {
                ...
                if (
                    !reason &&
                    !deepScan &&
                    /(^|\/)(dashboard-web|simplebeacon-dashboard)\/js\/(main\.js|components\/)/i.test(path)
                ) {
                    reason = 'Dashboard component (legitimate innerHTML)';
                }
                if (
                    !reason &&
                    /(^|\/)(codebase-analyzer|code-hygiene-certificate|zscript-parser)\.(cjs|js)$/i.test(path)
                ) {
                ...
                if (
                    !reason &&
                    !deepScan &&
                    /(^|\/)(dashboard-web|simplebeacon-dashboard)\/js\/(services|utils|components|controllers)\//i.test(
                        path
                    )
                ) {
                ...
                if (
                    !reason &&
                    !deepScan &&
                    /(^|\/)public\/dashboard\/js\/(main|services|utils|components)\//i.test(path)
                ) {
                    reason = 'Dashboard service/component (legitimate patterns)';
                }
                ...
                if (
                    !reason &&
                    /(^|\/)(ai-analyst|compliance-rules|aiProblemAnalyzerSuite|analyzeService)\.(cjs|js|mjs)$/i.test(
                        path
                    )
                ) {
                ...
                if (
                    !reason &&
                    /(^|\/)(orchestrator\.cjs|setup\.cjs|deep-check-welcome\.cjs|verify-deployment\.cjs)$/i.test(path)
                ) {
        ...
        const skipLabel = deepScan
            ? 'skipped (>2GB / .simplebeaconignore only)'
            : 'skipped (vendor/docs/build artifact)';
        appendTerminalLine(
            `<span style="color:#10B981;font-weight:700;">&#128310; Filter Summary:</span> ${sourceFiles.length.toLocaleString()} files ready · ${skipped.toLocaleString()} ${skipLabel}`
        );
        const sortedReasons = Object.entries(skipReasons).sort((a, b) => b[1] - a[1]);
        for (const [reason, count] of sortedReasons.slice(0, 8)) {
            appendTerminalLine(
                `  <span style="color:#64748B;">&#10148;</span> ${reason}: <strong>${count.toLocaleString()}</strong>`
            );
        }
        if (sourceFiles.length === 0) {
            appendTerminalLine(
                `<span style="color:#F59E0B;font-weight:700;">&#9888; No scannable files found.</span> The selected folder appears to be empty or contains only vendor/build directories.`,
                'warn'
            );
            showToast(
                'No scannable source files found. Select a folder with actual code, or enable Deep Scan.',
                'warning'
            );
        ...
        appendTerminalLine(
            `Found ${sourceFiles.length.toLocaleString()} files. All files will be analyzed.`,
            'success'
        );
        ...
        for (const f of sourceFiles) {
            ...
            if (/\.(js|cjs|mjs|ts|tsx|jsx|py|java|go|rs|php|rb|cs|swift|kt|scala|dart|vue|svelte)$/i.test(p)) {
                ...
                /\/(test-cert|java-ai-vulnerable|simplebeacon-rule-tests|__tests__|mocks|fixtures)[\/]|\.(test|spec)\./i.test(
                    p
                )
            ) {
        ...
        appendTerminalLine(
            `<span style="color:#60A5FA;font-weight:700;">&#128200; File Inventory:</span> ${sourceFiles.length.toLocaleString()} total files`
        );
        appendTerminalLine(
            `  <span style="color:#64748B;">&#10148;</span> Source code: <strong>${inventory.sourceCode.toLocaleString()}</strong>`
        );
        appendTerminalLine(
            `  <span style="color:#64748B;">&#10148;</span> Markup/style: <strong>${inventory.markup.toLocaleString()}</strong>`
        );
        appendTerminalLine(
            `  <span style="color:#64748B;">&#10148;</span> Config/data: <strong>${inventory.config.toLocaleString()}</strong>`
        );
        appendTerminalLine(
            `  <span style="color:#64748B;">&#10148;</span> Documentation: <strong>${inventory.docs.toLocaleString()}</strong>`
        );
        if (inventory.buildArtifacts > 0)
            appendTerminalLine(
                `  <span style="color:#64748B;">&#10148;</span> Build artifacts: <strong>${inventory.buildArtifacts.toLocaleString()}</strong>`
            );
        if (inventory.testFixtures > 0)
            appendTerminalLine(
                `  <span style="color:#64748B;">&#10148;</span> Test fixtures: <strong>${inventory.testFixtures.toLocaleString()}</strong>`
            );
        if (inventory.tempDev > 0)
            appendTerminalLine(
                `  <span style="color:#64748B;">&#10148;</span> Temp/dev files: <strong>${inventory.tempDev.toLocaleString()}</strong>`
            );
        if (inventory.other > 0)
            appendTerminalLine(
                `  <span style="color:#64748B;">&#10148;</span> Other: <strong>${inventory.other.toLocaleString()}</strong>`
            );
        if (localScanFileName)
        ...
        appendTerminalLine(
            `<span style="color:#60A5FA;font-weight:700;">&#9654; Stage 3/3:</span> Scanning ${sourceFiles.length.toLocaleString()} source files across ${activeEngineCount} analysis engine${activeEngineCount === 1 ? '' : 's'}...`
        );
        ...
        if (false && sourceFiles.length >= 1000 && typeof Worker !== 'undefined') {
            // simplebeacon-ignore: strict !== comparison
            try {
            ...
            workerPromise = new Promise(resolve => {
                scanWorker.onmessage = e => {
                    ...
                    if (msg.type === 'started') {
                        ...
                        if (localScanFileName)
                            localScanFileName.textContent = `Worker scanning ${msg.processed.toLocaleString()} of ${msg.total.toLocaleString()} files (${pct}%)...`;
                    } else if (msg.type === 'complete') {
                        appendTerminalLine(
                            `Worker scan complete: ${msg.issueCount} findings across ${msg.processed} files.`,
                            'success'
                        );
                        if (msg.findings) {
        ...
        for (let i = 0; i < sourceFiles.length; i++) {
            ...
            if (i > 0 && i % 1000 === 0) {
                appendTerminalLine(
                    `Scanning progress: ${i.toLocaleString()}/${sourceFiles.length.toLocaleString()} files...`
                );
            }
            ...
            try {
                ...
                if (e.message && e.message.includes('byte limit')) {
                    largeFileSkips++;
                    if (largeFileSkips <= 5)
                        appendTerminalLine(`Skipping ${path}: file exceeds 10 MB read limit.`, 'warn');
                } else {
                    readErrors++;
                }
                continue;
            ...
            const isTestOutput =
                /test-(output|debug|failures|log|complete|auto)\.(txt|log)$/i.test(path) ||
                (/\.(txt|log)$/i.test(path) && /test|debug|output|log/i.test(path.split('/').pop()));
            const isSampleOrTest =
                /[/-]test[-_]|\.(test|spec)\.|sample[-_]|demo-|fixture-|mock-|audit-|vulnerable|exposed|leaky_|positive-test|\.simplebeacon\/|test-cert\/|__tests__\/|mocks\/|fixtures\/|report\.json|\.simplebeaconignore|(^|[\/])tmp-[^\/]*\.js$/i.test(
                    lowerPath
                );
            const isTypeScriptDef = lowerPath.endsWith('.d.ts');
            const isCiWorkflow = /\.github\/workflows\//.test(lowerPath);
            const isSourceCode =
                /\.(js|cjs|mjs|ts|tsx|jsx|py|java|go|rs|c|cpp|h|hpp|cs|php|rb|swift|kt|scala|dart|vue|svelte)$/i.test(
                    path
                );
            const isServerEntry =
                /\/server\.cjs$|\/(server|api|scripts|reporters|bin|lib)\/|mcp-[^/]+\.cjs$|-helper\.cjs$|-gateway\.cjs$|AssessmentController\.cjs$|path-health\.cjs$|fetch-grammars\.js$/.test(
                    path
                );
            ...
            if (
                ...
                const hasLlmCall =
                    /\bopenai\.chat\.completions\.create\s*\(|\bopenai\.responses\.create\s*\(|\banthropic\.messages\.create\s*\(|\bstreamText\s*\(|\bgenerateText\s*\(|\buseChat\s*\(|\buseCompletion\s*\(/i.test(
                        text
                    );
                const hasTokenLimit = /\bmax_tokens\b|\bmax_completion_tokens\b|\bmaxOutputTokens\b/i.test(text);
                if (hasLlmCall && !hasTokenLimit) {
                    archDriftFindings.push({
                        file: path,
                        type: 'Token Bleed',
                        reason: 'LLM API call without max_tokens limit'
                    });
                }
                const hasHybridModel =
                    /\bmamba(?:-2)?\b|\bstate-spaces\/|\bstate-space-model\b|\bssm\b|\bjamba\b|\bhybrid-transformer\b|\bmixture-of-experts\b|\btitans\b|\brwkv\b|\bhyena\b|\bretnet\b/i.test(
                        text
                    );
                const hasValidator =
                    /\bzod\b|\bajv\b|\byup\b|\bio-ts\b|\bclass-validator\b|\bpydantic\.BaseModel\b|\bresponse_format\b|\bjson_schema\b|\bstructuredOutputs\b|\bsafeParse\s*\(|\b\.parse\s*\(|\bvalidate\s*\(/i.test(
                        text
                    );
                if (hasHybridModel && !hasValidator) {
                    archDriftFindings.push({
                        ...
                        reason: 'Hybrid/SSM model without schema validator'
                    });
                }
            }
            ...
            if (isSourceCode && !isNodeModule && !isSimplebeaconCache && !isScannerArtifact) {
                ...
                srcLines.forEach((line, idx) => {
                    ...
                    if (/JSON\.parse\s*\(\s*fs\.readFileSync\s*\(/.test(line)) {
                        if (
                            !/New folder\/|simplebeacon-export-operator|\.json$|scripts\/|ai-agent\/|ai-tools\/|bulk-fix-|export-findings|audit-token-bleed|add-sample-json-|complexity-gate|run-cli-scan|test-all-patterns|scanner-patterns/.test(
                                path
                            )
                        ) {
            ...
            if (!workerScanActive) {
                for (const analyzerId of activeAnalyzers) {
                    ...
                    const isKnownVendor =
                        /\/(jquery|modernizr|underscore|bootstrap|lodash|moment|react|vue|angular|backbone|ember|dojo|extjs|prototypejs)\b|\.pack\.js$|\.bundle\.js$|[\-.]min\.js$|[\-.]min\.css$|\.map$|(^|\/)(docs\/|doc\/|third_party\/|thirdparty\/|geedocs\/|mapfiles\/|vendor\/)/i.test(
                            path
                        );
                    ...
                    if (jsOnlyPatterns.includes(reg.id) && !/^(js|cjs|mjs|ts|tsx|jsx|html|htm)$/.test(fileExt))
                        continue;
                    ...
                    try {
                        ...
                        if (/java-ai-vulnerable[\/]|simplebeacon-rule-tests[\/]|archive[\/]|scripts[\/]/.test(path))
                            continue;
                        ...
                        if (
                            reg.id === 'configDrift' &&
                            (/\/analyzers\/|env-parser\.|env-profile-utils\.|commands\.|compliance-checklist\.|fix-dry-run\.|certificate-module\.|ui-renderer\.|app-links\.|site-config\.|playwright\.config\.|db\.cjs$|generate-license-token\.|generate-test-token\.|free-token\.|main\.js$|trello-roadmap-export\.|server\.cjs$|send-queued-emails\.|run-cli-scan\.|agency-handoff-patterns\.|simplebeacon-frameworkless\/app\.|config\.js$|SettingsView\.js$|test-all-patterns\.|upload\.spec\.|e2e\/|extension\.ts$|out\/extension\.js$|pattern-documentation\.js$|quick-actions\.js$|AnalyzeView\.js$|scanner-patterns\.js$|scanner-engine\.js$|browser-extension\//.test(
                                path
                            ) ||
                                /simplebeacon-vscode\/src\//.test(path))
                        )
                            continue;
                        if (
                            reg.id === 'sensitiveData' &&
                            (/outreach-prospects\.|agency-handoff-patterns\.|site-config\.|app-links\.|email\.cjs$|free-token\.|generate-license-token\.|generate-test-token\.|scan-github-repo\.|send-all-tier-emails\.|send-payment-tier-emails\.|main\.js$|demoMode\.|AssessmentView\.|OutreachView\.|send-queued-emails\.|repair\.|generate-token\.js$|LoginModal\.js$|AnalyzeView\.js$/.test(
                                path
                            ) ||
                                /simplebeacon-vscode\/src\//.test(path) ||
                                /ScanPaywall\.js$/.test(path))
                        )
                            continue;
                        if (
                            reg.id === 'securityHeaders' &&
                            (/certificate-module\.|main\.js$|ui-renderer\.|run-all-tier-scans\.cjs$|server\.cjs$|AnalyzeView\.js$|scanner-patterns\.js$/.test(
                                path
                            ) ||
                                /simplebeacon-vscode\/src\//.test(path) ||
                                /\/rules\/agency-handoff-patterns\.js$/.test(path))
                        )
                            continue;
                        if (
                            reg.id === 'configDrift' &&
                            (/scanService\.js$|site-config\.js$|scanner-engine\.js$/.test(path) ||
                                /simplebeacon-vscode\/src\//.test(path))
                        )
                            continue;
                        if (
                            reg.id === 'debugArtifacts' &&
                            /generate-license-token\.|generate-test-token\.|db\.cjs$|trello-roadmap-export\.|send-queued-emails\.|run-cli-scan\.|repair\.|fix-.*\.py$|simplebeacon-frameworkless\/app\.js$|scripts\/|ai-agent\/|ai-platform\/tools\/|coming-soon\/archive\//.test(
                                path
                            )
                        )
                            continue;
                        ...
                        if (
                            reg.id === 'i18nHardcoded' &&
                            /certificate-utils\.cjs$|certificates\.cjs$|checkout\.cjs$|server\.cjs$|services\/email\.cjs$|contact\.js$|send-queued-emails\.|llm-slop-patterns\.|tmp-js-check\.|simplebeacon-frameworkless\/app\.js$|LoginModal\.js$|PathHealthDashboard\.js$|AnalyzeView\.js$|SignInView\.js$|UploadView\.js$/.test(
                                path
                            )
                        )
                            continue;
                        if (
                            reg.id === 'a11yGap' &&
                            (/cleanupAssistant\.js$|AnalyzeView\.js$|OutreachView\.js$|SettingsView\.js$/.test(path) ||
                                /simplebeacon-vscode\/src\//.test(path))
                        )
                            continue;
                        if (
                            (reg.id === 'productionLeak' || reg.id === 'mockPathLeak' || reg.id === 'sampleJsonRef') &&
                            (/simplebeacon-vscode\/src\//.test(path) ||
                                /simplebeacon-dashboard/.test(path) ||
                                /\/analyzers\//.test(path) ||
                                /\/reporters\//.test(path) ||
                                /ai-agent\/|scripts\/|archive\/|assessment\.js$|project-detect\.js$|scan\.js$|data-lineage-analyzer\.js$|unused-file-detector\.js$|visualSidebarProvider\.ts$|analyzeService\.js$|jest\.config\.(js|cjs|mjs|ts)$/i.test(
                                    path
                                ))
                        )
                            continue;
                        if (reg.id === 'ssrfVector' && /browser-extension\//.test(path)) continue;
                        ...
                        if (
                            (/^aiResidue/.test(reg.id) || reg.id === 'deprecatedPattern') &&
                            (/\/vendor\/|\.min\.js$|\.bundle\.min\.js$/.test(path) ||
                                /deploy-auto\.|generate-license-token\.|generate-test-token\.|tmp-js-check\.|run-cli-scan\.|repair\.|send-queued-emails\.|trello-roadmap-export\.|themeService\.js$|file-reference-tracker\.js$|site-config\.js$|architecture-drift-patterns\.js$|fix-browser-require\.js$|realtimeMonitor\.ts$/.test(
                                    path
                                ))
                        )
                            continue;
                        if (
                            (/^aiResidue/.test(reg.id) ||
                                reg.id === 'roadmapMarker' ||
                                reg.id === 'architectureDrift') &&
                            (/server\.cjs$/.test(path) ||
                                /architecture-drift-patterns\.js$|fix-browser-require\.js$|realtimeMonitor\.ts$|AnalyzeView\.js$/.test(
                                    path
                                ))
                        )
                            continue;
                        if (
                            reg.id === 'apiContractDrift' &&
                            /routes\/[^/]+\.(cjs|js|mjs)$|server\.(cjs|js)$|_restore-eu-audit\.js$|extendedAnalyzers\.mjs$|AnalyzeView\.js$|main\.js$|scanner-patterns\.js$/.test(
                                path
                            )
                        )
                            continue;
                        ...
                        if (
                            reg.id === 'missingTest' &&
                            /trello-roadmap-export\.|\/out\/extension\.js$|\.min\.js$|\.bundle\.min\.js$|\.min\.css$|\.map$/.test(
                                path
                            )
                        )
                            continue;
                        if (
                            reg.id === 'complexityMetric' &&
                            (/contact\.js$|roadmap\.html$|code-map\.json$|New folder\/|simplebeacon-export-operator/.test(
                                path
                            ) ||
                                /simplebeacon-vscode\/src\/codeMapProvider\.ts$/.test(path))
                        )
                            continue;
                        if (reg.id === 'unusedDep' && /AnalyzeView\.js$|main\.js$/.test(path)) continue;
                        if (
                            reg.id === 'llmSlop' &&
                            /llm-slop-patterns|fiction-kpi|rejectedFiction|scanner-patterns|AnalyzeView\.js$|main\.js$/i.test(
                                path
                            )
                        )
                            continue;
                        ...
                        if (
                            reg.id === 'magicNumber' &&
                            (/vite\.config\.js$|simplebeacon-vscode\/src\//.test(path) ||
                                /ai-platform\/|ai-agent\/|ai-tools\/|coming-soon\/|archive\/|packages\/simplebeacon-cli\/src\/|scripts\//.test(
                                    path
                                ))
                        )
                            continue;
                        if (
                            reg.id === 'unvalidatedRedirect' &&
                            /simplebeacon-server\.cjs$|app-links\.js$|free-token\.cjs$|_restore-eu-audit\.js$/.test(
                                path
                            )
                        )
                            continue;
                        if (
                            reg.id === 'missingRateLimit' &&
                            /simplebeacon-server\.cjs$|health|server\.cjs$|_restore-eu-audit\.js$|local-agent[\\/]agent\.js$/i.test(
                                path
                            )
                        )
                            continue;
                        if (
                            reg.id === 'loggingSecrets' &&
                            /simplebeacon-server\.cjs$|server\.cjs$|generate-account-token\.js$|generate-license-token\.cjs$|generate-test-token\.cjs$|get-test-token\.cjs$|verify-v1-internal-profile\.cjs$/.test(
                                path
                            )
                        )
                            continue;
                        ...
                        if (
                            (reg.id === 'evalDanger' ||
                                reg.id === 'innerHtmlXss' ||
                                reg.id === 'prototypePollution' ||
                                reg.id === 'unvalidatedRedirect' ||
                                reg.id === 'missingRateLimit' ||
                                reg.id === 'insecureRandom' ||
                                reg.id === 'loggingSecrets') &&
                            /scripts\/|ai-tools\/|ai-agent\/|simplebeacon-frameworkless\/|bulk-fix-|export-findings|audit-token-bleed|complexity-gate|add-sample-json-|run-cli-scan|test-all-patterns|token-file-system\.js$|usb-token-manager\.js$/.test(
                                path
                            )
                        )
                            continue;
                        if (
                            reg.id === 'prototypePollution' &&
                            (/enhancedAIProvider\.ts$|compliance-checklist\.js$|workspaceAnalyzer\.ts$|remediationProvider\.ts$|architecture-drift-patterns\.js$/.test(
                                path
                            ) ||
                                /Object\.prototype\.hasOwnProperty\.call.*is the SAFE pattern/i.test(text))
                        )
                            continue;
                        if (reg.id === 'evalDanger' && /architecture-drift-patterns\.js$|AboutView\.js$/.test(path))
                            continue;
                        if (reg.id === 'insecureRandom' && /fixRegistry\.ts$/.test(path)) continue;
                        if (reg.id === 'unhandledPromise' && /scripts\/debug-.*\.js$/.test(path)) continue;
                        if (
                            reg.id === 'missingStrictMode' &&
                            /\.eslintrc\.js$|jest\.config\.js$|build-extension\.js$/.test(path)
                        )
                            continue;
                        ...
                        if (reg.contextFilter && typeof reg.contextFilter === 'function') {
                            filteredMatches = filteredMatches.filter(m =>
                                reg.contextFilter(m.snippet || '', path, m.context || [])
                            );
                        }
                        ...
                        if (reg.id === 'configDrift') {
                            ...
                            if (collector) {
                                collector(path, reg, finalMatches, {
                                    ...
                                    evalDangerFindings,
                                    innerHtmlXssHits,
                                    innerHtmlXssFindings,
                                    prototypePollutionHits,
                                    prototypePollutionFindings,
                                    unhandledPromiseHits,
                                    unhandledPromiseFindings,
                                    magicNumberHits,
                                    magicNumberFindings,
                                    missingStrictModeHits,
                                    missingStrictModeFindings,
                                    uninitializedReadHits,
                                    uninitializedReadFindings,
                                    unvalidatedRedirectHits,
                                    unvalidatedRedirectFindings,
                                    missingRateLimitHits,
                                    missingRateLimitFindings,
                                    insecureRandomHits,
                                    insecureRandomFindings,
                                    loggingSecretsHits,
                                    loggingSecretsFindings,
                                    hardcodedConfidenceHits,
                                    hardcodedConfidenceFindings,
                                    hardcodedCompletionHits,
                                    hardcodedCompletionFindings,
                                    mockPathLeakHits,
                                    mockPathLeakFindings,
                                    sampleJsonRefHits,
                                    sampleJsonRefFindings,
                                    governanceMarkerHits,
                                    governanceMarkerFindings,
                                    aiPlaceholderCommentHits,
                                    aiPlaceholderCommentFindings,
                                    aiPlaceholderBlockHits,
                                    aiPlaceholderBlockFindings,
                                    markdownFenceLeakHits,
                                    markdownFenceLeakFindings,
                                    emptyStubFunctionHits,
                                    emptyStubFunctionFindings,
                                    arrowStubHits,
                                    arrowStubFindings,
                                    roadmapMarkerHits,
                                    roadmapMarkerFindings
                                });
                            }
                            continue;
                        }
            ...
            if (allowedSections.includes('mockDataCategories')) {
                ...
                const isStructuralUtility =
                    /-(path-resolver|resolver|stub-api|schema-validator|schema|consistency-checker|overrides|config|checker|specs)\.(js|cjs|mjs|ts|json)$/i.test(
                        lowerPath
                    );
                ...
                if (
                    ...
                    !isStructuralUtility &&
                    !isBackupOrVuln &&
                    /mock|fixture|sample|test-data|testdata|fake-data|dummy|stub/i.test(lowerPath)
                ) {
                    mockFiles.push(path);
                }
            }
                ...
                if (!isNodeModule && !isCacheDir && lowerPath.endsWith('package.json')) {
                    try {
                        ...
                        if (depCount >= 0 && !/java-ai-vulnerable/i.test(path)) {
                            ...
                            if (allFilePaths.has(dir + '/package-lock.json')) {
                                hasLockfile = true;
                                lockfileType = 'package-lock';
                            } else if (allFilePaths.has(dir + '/yarn.lock')) {
                                hasLockfile = true;
                                lockfileType = 'yarn';
                            } else if (allFilePaths.has(dir + '/pnpm-lock.yaml')) {
                                hasLockfile = true;
                                lockfileType = 'pnpm';
                            } else if (allFilePaths.has(dir + '/bun.lockb')) {
                                hasLockfile = true;
                                lockfileType = 'bun';
                            } else if (allFilePaths.has(dir + '/bun.lock')) {
                                hasLockfile = true;
                                lockfileType = 'bun';
                            }
                            ...
                            if (!hasLockfile) {
                                ...
                                if (
                                    allFilePaths.has(parentDir + '/package-lock.json') &&
                                    allFilePaths.has(parentDir + '/package.json')
                                ) {
                                    hasLockfile = true;
                                    lockfileType = 'package-lock (workspace)';
                                } else if (
                                    allFilePaths.has(parentDir + '/yarn.lock') &&
                                    allFilePaths.has(parentDir + '/package.json')
                                ) {
                                    hasLockfile = true;
                                    lockfileType = 'yarn (workspace)';
                                } else if (
                                    allFilePaths.has(parentDir + '/pnpm-lock.yaml') &&
                                    allFilePaths.has(parentDir + '/package.json')
                                ) {
                                    hasLockfile = true;
                                    lockfileType = 'pnpm (workspace)';
                                } else if (
                                    allFilePaths.has(parentDir + '/bun.lockb') &&
                                    allFilePaths.has(parentDir + '/package.json')
                                ) {
                                    hasLockfile = true;
                                    lockfileType = 'bun (workspace)';
                                } else if (
                                    allFilePaths.has(parentDir + '/bun.lock') &&
                                    allFilePaths.has(parentDir + '/package.json')
                                ) {
                                    hasLockfile = true;
                                    lockfileType = 'bun (workspace)';
            ...
            const isBuildOutput = /\/(build|dist|frontend-build|out|\.next|\.output|target|coverage)\//i.test(
                lowerPath
            );
            if (allowedSections.includes('compliance') || allowedSections.includes('euAiActSummary')) {
                if (
                    !isNodeModule &&
                    !isBuildOutput &&
                    /license|licence/i.test(lowerPath) &&
                ...
                if (
                    !isNodeModule &&
                    !isBuildOutput &&
                    /security\.md|code_of_conduct|contributing|changelog|risk-assessment/i.test(lowerPath)
                ) {
            ...
            if (allowedSections.includes('dataQuality')) {
                const isTestFixture =
                    /\/test[-/]|\/test\.json$|\.simplebeacon\/|test-cert\/|__tests__\/|fixtures\/|mocks\//.test(
                        lowerPath
                    );
            ...
            if (
                (allowedSections.includes('consolidation') || allowedSections.includes('fileReduction')) &&
                text.length < 200000 &&
                !/node_modules|\.git|dist|build|coverage|java-ai-vulnerable|backups|\.husky|\.simplebeacon\/|\.github-sync\/|github-cache\/|\.cursor\/|\.windsurf\/|_restore-|fix-.*\.py$|patch-main|New folder\/|simplebeacon-export-operator/i.test(
                    lowerPath
                )
            ) {
                const baseName = path.split('/').pop().toLowerCase();
                // Skip common legitimately-duplicated files (licenses, package manifests, ignore files)
                const isCommonLegitDuplicate =
                    /^(license|licence|readme|changelog|changes|history|contributing|code_of_conduct|\.gitignore|\.npmignore|\.dockerignore|package\.json|tsconfig\.json|\.editorconfig|\.prettierrc|\.eslintrc)$/i.test(
                        baseName
                    );
            ...
            if (allowedSections.includes('roadmap')) {
                const isCacheOrBinary =
                    /\.git\/|\.simplebeacon\/|github-cache\/|\.github-sync\/|node_modules\/|\.pack$|\.idx$|\.objects\//i.test(
                        lowerPath
                    );
                ...
                const hasSourceExt =
                    /\.(js|cjs|mjs|ts|tsx|jsx|py|java|go|rs|c|cpp|h|hpp|cs|php|rb|swift|kt|scala|dart|vue|svelte|html|css|scss|sass|less|sql|sh|bash|ps1|yml|yaml|json|md|txt|xml|cfg|ini|toml|gradle|dockerfile)$/i.test(
                        path
                    );
                ...
                // Count occurrences of scanner-inserted synthetic markers so
                // they can be grouped separately. Standardized marker tokens
                // used by `buildSuggestedFixes` are: "[Auto-Generated Issue]"
                // and the inline tag `AUTO_FIX: ai_residue`.
                const syntheticMatches = (text.match(/\[Auto-Generated Issue\]|\bAUTO_FIX:\s*ai_residue\b/gi) || [])
                    .length;
                if (syntheticMatches > 0) fileMarkers.synthetic = syntheticMatches;
                const hasRealTodo = Object.keys(fileMarkers)
                    .filter(k => k !== 'synthetic')
                    .some(k => fileMarkers[k] > 0);
                if (!isCacheOrBinary && !isScanReport && !isTestFile && hasSourceExt && hasRealTodo) {
                    todoFiles.push({ path, markers: fileMarkers });
                    Object.keys(fileMarkers).forEach(k => {
                        if (k !== 'synthetic') todoMarkerCounts[k] += fileMarkers[k];
                    });
                }
                if (syntheticMatches > 0) {
                    syntheticTodoFiles.push({ path, synthetic: syntheticMatches });
                    syntheticTodoMarkerCounts.total += syntheticMatches;
                }
            }
            ...
            if (allowedSections.includes('fileReduction')) {
                ...
                const isStandardWebAsset =
                    /^(favicon|apple-touch-icon|mstile|browserconfig|site\.webmanifest|logo192\.png|logo512\.png|maskable-icon|splash-screen|og-image)/i.test(
                        baseName
                    );
            ...
            if (allowedSections.includes('junkFiles')) {
                ...
                const isJunk =
                    ...
                    baseName.endsWith('~') ||
                    /^(\.DS_Store|Thumbs\.db|desktop\.ini|npm-debug\.log|yarn-error\.log|\.eslintcache|\.node_repl_history)$/i.test(
                        baseName
                    ) ||
        ...
        if (fileNamingHits > 0) {
            appendTerminalLine(
                `<span style="color:#60A5FA;font-weight:700;">&#128451; File Naming Analysis:</span> ${fileNamingHits} naming issue(s) detected.`
            );
            for (const f of fileNamingResult.findings.slice(0, 8)) {
                appendTerminalLine(
                    `  <span style="color:#64748B;">&#10148;</span> ${f.type}: <strong>${f.file}</strong> — ${f.detail.slice(0, 80)}${f.detail.length > 80 ? '...' : ''}`
                );
            }
        } else {
            appendTerminalLine(
                '<span style="color:#10B981;font-weight:700;">&#128451; File Naming Analysis:</span> All file names look clean.',
                'success'
            );
        }
        ...
        if (removableResult.totalRemovable > 0) {
            appendTerminalLine(
                `<span style="color:#F59E0B;font-weight:700;">&#128465; Removable Files:</span> ${removableResult.summary}`
            );
            for (const cat of removableResult.categories.filter(c => c.removable).slice(0, 5)) {
                appendTerminalLine(
                    `  <span style="color:#64748B;">&#10148;</span> ${cat.label}: <strong>${cat.count.toLocaleString()} files</strong> — ${cat.action}`
                );
            }
        } else {
            appendTerminalLine(
                '<span style="color:#10B981;font-weight:700;">&#128465; Removable Files:</span> No obvious bloat detected.',
                'success'
            );
        }
        ...
        appendTerminalLine(
            `  After initial filter: <strong>${sourceFiles.length.toLocaleString()}</strong> (skipped ${skipped.toLocaleString()})`
        );
        ...
        if (largeFileSkips > 0)
            appendTerminalLine(
                `  Large files skipped (>10 MB): <strong>${largeFileSkips.toLocaleString()}</strong>`,
                'warn'
            );
        if (scanWorker) {
            appendTerminalLine(
                `  Worker files posted: <strong>${Math.min(sourceFiles.length, 100000).toLocaleString()}</strong> (cap: 100000)`
            );
            appendTerminalLine(
                `  <span style="color:#F59E0B;">&#9888; Worker receives path objects only — file.text() will silently fail. Main thread does actual scanning.</span>`,
                'warn'
            );
        }
        ...
        if (unaccounted > 0) {
            appendTerminalLine(
                `  <span style="color:#EF4444;">&#9888; Unaccounted files: ${unaccounted.toLocaleString()}</span>`,
                'warn'
            );
        }
        appendTerminalLine(
            `  <span style="color:#64748B;font-size:11px;">There is no file cap. All files are scanned regardless of repo size.</span>`
        );
        ...
        for (const file of sourceFiles) {
            ...
            for (const pat of bloatPatterns) {
                if (pat.test(path)) {
                    bloatFindings.push({
                        file: path,
                        reason: 'Self-generated scan artifact — safe to delete after review'
                    });
                    break;
                }
            }
        ...
        if (bloatFindings.length > 0) {
            appendTerminalLine(
                `Detected ${bloatFindings.length} self-generated bloat file(s) — consider cleanup`,
                'warn'
            );
        }
        ...
        if (imageFiles.length > 0) {
            ...
            const sourceTextFiles = sourceFiles.filter(f => {
                const p = (f.webkitRelativePath || f.name).toLowerCase();
                return (
                    /\.(js|cjs|mjs|ts|tsx|jsx|vue|svelte|html|htm|css|scss|sass|less|json|md|txt|xml)$/i.test(p) &&
                    !/node_modules\/|\.git\/|\.simplebeacon\/|github-cache\//i.test(p)
                );
            });
            ...
            if (imageFiles.length > CROSS_REF_MAX_IMAGES || sourceTextFiles.length > CROSS_REF_MAX_SOURCES) {
                appendTerminalLine(
                    `Skipped unused-asset cross-reference for large repository (${imageFiles.length} images, ${sourceTextFiles.length} source files).`,
                    'info'
                );
            } else {
                ...
                if (unusedAssetCandidates.length > 0) {
                    appendTerminalLine(
                        `${unusedAssetCandidates.length} of ${imageFiles.length} images appear unused in source code.`,
                        'warn'
                    );
                } else {
                    appendTerminalLine(
                        `All ${imageFiles.length} image assets are referenced in source code.`,
                        'success'
                    );
                }
            }
            ...
            Object.entries(keyToFiles).forEach(([key, refs]) => {
                ...
                if (uniqueValues.length > 1) {
                    envInconsistencyFindings.push({
                        ...
                        values: refs.map(r => ({ file: r.path, value: r.value }))
                    });
                }
            });
        }
        ...
        if (processEnvRefs.length > 0 && envFileData.length > 0) {
            ...
            Object.entries(missingMap).forEach(([key, refs]) => {
                missingEnvKeyFindings.push({
                    severity: 'medium',
                    type: 'Missing Environment Key',
                    key,
        ...
        const crossFileHitCount =
            envInconsistencyFindings.length +
            missingEnvKeyFindings.length +
            versionDriftFindings.length +
            syncIoFindings.length;
        if (crossFileHitCount > 0) {
            appendTerminalLine(
                `<span style="color:#60A5FA;font-weight:700;">Cross-file analysis:</span> ${envInconsistencyFindings.length} env inconsistencies, ${missingEnvKeyFindings.length} missing env keys, ${versionDriftFindings.length} version drifts, ${syncIoFindings.length} sync I/O patterns`
            );
        }
        // 4. Score & report
        ...
        const qualityScore = Math.max(
            ...
            100 -
                ...
                llmSlopPenalty -
                tokenBleedPenalty -
                productionLeakPenalty -
                fictionKpiPenalty -
                securityPenalty -
                qualityPenalty -
                maintainabilityPenalty -
                archDriftPenalty -
                envInconsistencyPenalty -
                missingEnvPenalty -
                versionDriftPenalty -
                syncIoPenalty
        );
        const schemaCompliance =
            totalJsonFiles === 0 ? 100 : Math.max(0, Math.round(100 - (emptyJsonFiles.length / totalJsonFiles) * 100));
        const consistencyScore =
            duplicateGroups === 0 ? 100 : Math.max(0, Math.round(100 - Math.min(duplicateGroups * 5, 50)));
        const gatePass =
            credentialHits === 0 && sensitiveDataHits === 0 && dbPatternHits === 0 && configDriftHits === 0;
        ...
        const HIDDEN_ROOT_EXCLUSIONS =
            /^(\..*|node_modules|\.git|\.github|\.github-sync|github-cache|\.cursor|\.windsurf|\.cursor-tutor|\.vscode|\.idea|\.husky|__tests__|fixtures?|mocks?|\.simplebeacon)$/i;
        ...
        const testFixturePattern =
            /test-all-patterns|\.test\.|\.spec\.|mock|fixture|sample|demo|test.*\.js$|test-technical|e2e\/|positive-test|negative-test|simplebeacon-rule-tests/i;
        const credentialGateFindings =
            profile.checkCredentials && credentialFindings.length
                ? credentialFindings
                      .filter(f => !testFixturePattern.test(f.file) && f.matches.some(m => m.confidence >= 0.75))
                      .slice(0, 5)
                      .map(f => ({
                          severity: 'medium',
                          type: 'Credential Pattern',
                          count: f.matches.filter(m => m.confidence >= 0.75).length,
                          filePath: f.file,
                          rule: 'CREDENTIAL_PATTERN_HEURISTIC',
                          impact: 'MEDIUM RISK: Hardcoded credential patterns in source increase breach surface.',
                          fix: 'Move secrets to environment variables || a secret manager; never commit keys to version control.',
                          findings: f.matches.filter(m => m.confidence >= 0.75)
                      }))
                : [];
        const sensitiveDataGateFindings =
            profile.checkAiResidue && sensitiveDataFindings.length
                ? sensitiveDataFindings
                      .filter(f => !testFixturePattern.test(f.file) && f.matches.some(m => m.confidence >= 0.75))
                      .slice(0, 3)
                      .map(f => ({
                          ...
                          impact: 'PRIVACY RISK: PII in logs or source code violates GDPR and increases breach liability.',
                          fix: 'Sanitize logs, remove PII from source, and use tokenization for identifiers.',
                          findings: f.matches.filter(m => m.confidence >= 0.75)
                      }))
                : [];
        const dbPatternGateFindings =
            profile.checkAiResidue && dbPatternFindings.length
                ? dbPatternFindings
                      .filter(f => !testFixturePattern.test(f.file) && f.matches.some(m => m.confidence >= 0.75))
                      .slice(0, 3)
                      .map(f => ({
                          severity: 'high',
                          type: 'Database Anti-Pattern',
                          count: f.matches.filter(m => m.confidence >= 0.75).length,
                          filePath: f.file,
                          rule: 'DB_PATTERN',
                          impact: 'SECURITY RISK: Raw SQL concatenation enables injection. Unbounded queries cause DoS.',
                          fix: 'Use parameterized queries, ORM methods, and always apply LIMIT/OFFSET.',
                          findings: f.matches.filter(m => m.confidence >= 0.75)
                      }))
                : [];
        const configDriftGateFindings =
            profile.checkAiResidue && configDriftFindings.length
                ? configDriftFindings
                      ...
                      .map(f => ({
                          ...
                          count: f.matches.filter(m => m.confidence >= 0.75).length,
                          filePath: f.file,
                          rule: 'CONFIG_DRIFT_PATTERN',
                          impact: 'SECURITY RISK: Literal endpoint values and secrets in config files bypass environment controls.',
                          fix: 'Store secrets outside source, inject endpoints via configuration, and audit config files.',
                          findings: f.matches.filter(m => m.confidence >= 0.75)
                      }))
                : [];
        const securityHeaderGateFindings =
            profile.checkAiResidue && securityHeaderFindings.length
                ? securityHeaderFindings
                      .filter(f => !testFixturePattern.test(f.file) && f.matches.some(m => m.confidence >= 0.75))
                      .slice(0, 3)
                      .map(f => ({
                          ...
                          count: f.matches.filter(m => m.confidence >= 0.75).length,
                          filePath: f.file,
                          rule: 'SECURITY_HEADER_PATTERN',
                          impact: 'SECURITY RISK: Missing CSP or X-Frame-Options enables XSS and clickjacking.',
                          fix: 'Add helmet middleware or configure reverse proxy with CSP, HSTS, X-Frame-Options.',
                          findings: f.matches.filter(m => m.confidence >= 0.75)
                      }))
                : [];
        const gateFindings = [
            ...credentialGateFindings,
            ...sensitiveDataGateFindings,
            ...dbPatternGateFindings,
            ...configDriftGateFindings,
            ...securityHeaderGateFindings
        ];
        ...
        reportData = {
            ...
            issueCount:
                (profile.checkAi ? aiHits.length : 0) +
                (profile.checkCredentials ? credentialHits : 0) +
                (profile.checkDebug ? debugHits.length : 0) +
                (profile.checkGov ? govHits.length : 0) +
                (profile.checkAiResidue
                    ? aiResidueHits +
                      perfHits +
                      typeSafetyHits +
                      testHits +
            ...
            repositoryFoldersTotal: [
                ...new Set(
                    files.map(f =>
                        (f.webkitRelativePath || f.name).replace(/\\/g, '/').split('/').slice(0, -1).join('/')
                    )
                )
            ...
            simplebeaconIssues:
                ...
                archDriftFindings.length,
            detectedIssues: [
                ...(profile.checkAi && aiFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'AI System Indicator',
                              count: aiFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: aiFindings.slice(0, 10).map(f => f.file),
                              rule: 'AI_IMPLEMENTATION_PATTERN',
                              impact: 'REVIEW REQUIRED: Left unchanged, this pattern may reappear in the next release cycle without a local gate on pull requests.',
                              fix: 'Review && remediate before enabling --gate on main.',
                              findings: aiFindings.slice(0, 5),
                              reasoning:
                                  'The scanner detected imports || function calls matching known AI SDK signatures (OpenAI, Anthropic, LangChain, HuggingFace). These patterns indicate AI system integration that may require compliance review under EU AI Act Article 6.',
                              confidence: 0.92,
                              humanReadable: `AI SDK usage detected in ${aiFindings.length} file(s). This suggests the project implements || integrates with third-party AI services.`
                          }
                      ]
                    : []),
                ...(profile.checkCredentials && credentialFindings.length
                    ? [
                          {
                              ...
                              severityBand: 'medium',
                              type: 'Credential Pattern',
                              count: credentialFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: credentialFindings.slice(0, 10).map(f => f.file),
                              rule: 'CREDENTIAL_PATTERN_HEURISTIC',
                              ...
                              fix: 'Move secrets to environment variables || a secret manager; never commit keys to version control.',
                              findings: credentialFindings.slice(0, 5),
                              reasoning:
                                  'Regex heuristics matched hardcoded credential patterns: password=, api_key=, secret_key=, || AWS access keys. These strings resemble common secret formats && are flagged for review even if they are dummy values.',
                              confidence: 0.78,
                              humanReadable: `Potential hardcoded secrets found in ${credentialFindings.length} file(s). The scanner matched patterns resembling API keys, passwords, || tokens assigned as string literals.`
                          }
                      ]
                    : []),
                ...(profile.checkDebug && debugFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Debug Artifact',
                              count: debugFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: debugFindings.slice(0, 10).map(f => f.file),
                              rule: 'DEBUG_ARTIFACT',
                              impact: 'HYGIENE RISK: Debug artifacts should ! reach production builds.',
                              fix: 'Remove console.log, debugger, TODO, && FIXME markers before release.',
                              findings: debugFindings.slice(0, 5),
                              reasoning:
                                  'Source code analysis detected console.log(), debugger statements, || alert()/prompt() calls. These are development-only artifacts that can leak internal state || interrupt user experience in production.',
                              confidence: 0.98,
                              humanReadable: `Debug artifacts (console.log, debugger, alert) found in ${debugFindings.length} file(s). These should be removed before production builds.`
                          }
                      ]
                    : []),
                ...(profile.checkGov && govFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'License/Governance Marker',
                              count: govFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: govFindings.slice(0, 10).map(f => f.file),
                              rule: 'GOVERNANCE_PATTERN',
                              impact: 'INFO: License headers && copyright notices detected. Verify compliance with open-source policy.',
                              fix: 'Review license compatibility with your product distribution model.',
                              findings: govFindings.slice(0, 5),
                              reasoning:
                                  'SPDX-License-Identifier tags, copyright headers, || license references were found  in source files. This indicates the project contains licensed third-party code that must be tracked for compliance.',
                              confidence: 0.95,
                              humanReadable: `License && copyright markers detected in ${govFindings.length} file(s). Verify open-source license compatibility with your distribution model.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && aiResidueFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'AI Residue',
                              count: aiResidueFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: aiResidueFindings.slice(0, 10).map(f => f.file),
                              rule: 'AI_RESIDUE_PATTERN',
                              impact: 'HYGIENE RISK: AI-generated stubs, deprecated idioms, and error-swallowing patterns reduce code quality.',
                              fix: 'Replace stubs with real implementations, modernize deprecated APIs, and add proper error handling.',
                              findings: aiResidueFindings.slice(0, 5),
                              reasoning:
                                  'The scanner detected patterns commonly left behind by AI coding assistants: empty function bodies, TODO placeholders, deprecated framework APIs, error-swallowing catch blocks, and large commented-out dead code.',
                              confidence: 0.85,
                              humanReadable: `AI residue (stubs, deprecated patterns, error swallowing, dead code) found in ${aiResidueFindings.length} file(s).`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && perfFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Performance Anti-Pattern',
                              count: perfFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: perfFindings.slice(0, 10).map(f => f.file),
                              rule: 'PERFORMANCE_PATTERN',
                              impact: 'PERFORMANCE RISK: Nested loops and leaked listeners degrade runtime performance.',
                              fix: 'Optimize hot paths, debounce event handlers, and remove listeners on unmount.',
                              findings: perfFindings.slice(0, 5),
                              reasoning:
                                  'Nested loops create O(n²) complexity. Leaked event listeners accumulate DOM references causing memory bloat.',
                              confidence: 0.8,
                              humanReadable: `${perfFindings.length} file(s) contain performance anti-patterns.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && typeSafetyFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Type Safety Gap',
                              count: typeSafetyFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: typeSafetyFindings.slice(0, 10).map(f => f.file),
                              rule: 'TYPE_SAFETY_PATTERN',
                              impact: 'MAINTAINABILITY RISK: any types and missing PropTypes erode compile-time guarantees.',
                              fix: 'Replace any with specific types, add PropTypes or migrate to TypeScript.',
                              findings: typeSafetyFindings.slice(0, 5),
                              reasoning:
                                  'any defeats TypeScript compiler checks. Missing PropTypes causes runtime prop mismatch errors in React.',
                              confidence: 0.82,
                              humanReadable: `${typeSafetyFindings.length} file(s) contain type safety gaps.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && testFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Missing Test Coverage',
                              count: testFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: testFindings.slice(0, 10).map(f => f.file),
                              rule: 'TEST_COVERAGE_PATTERN',
                              impact: 'QUALITY RISK: Skipped or empty tests give false confidence in coverage metrics.',
                              fix: 'Implement skipped tests or remove empty test placeholders.',
                              findings: testFindings.slice(0, 5),
                              reasoning: 'Empty test bodies pass silently while production code remains unverified.',
                              confidence: 0.88,
                              humanReadable: `${testFindings.length} file(s) have test coverage gaps.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && a11yFindings.length
                    ? [
                          {
                              severity: 'medium',
                              severityBand: 'medium',
                              type: 'Accessibility Gap',
                              count: a11yFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: a11yFindings.slice(0, 10).map(f => f.file),
                              rule: 'A11Y_PATTERN',
                              impact: 'INCLUSION RISK: Missing alt text and unlabeled inputs block assistive technology users.',
                              fix: 'Add alt attributes to images, aria-label to buttons, and associate labels with inputs.',
                              findings: a11yFindings.slice(0, 5),
                              reasoning:
                                  'Screen readers rely on alt text and ARIA labels. Color-only indicators fail for colorblind users.',
                              confidence: 0.92,
                              humanReadable: `${a11yFindings.length} file(s) have accessibility gaps.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && i18nFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'i18n Issue',
                              count: i18nFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: i18nFindings.slice(0, 10).map(f => f.file),
                              rule: 'I18N_PATTERN',
                              impact: 'GLOBAL RISK: Hardcoded English strings prevent localization.',
                              fix: 'Wrap UI strings in t()/i18n() functions and use locale-aware formatting.',
                              findings: i18nFindings.slice(0, 5),
                              reasoning:
                                  'Concatenated strings with variables are impossible to translate because word order varies by language.',
                              confidence: 0.78,
                              humanReadable: `${i18nFindings.length} file(s) have i18n issues.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && sensitiveDataFindings.length
                    ? [
                          {
                              ...
                              severityBand: 'high',
                              type: 'Sensitive Data Exposure',
                              count: sensitiveDataFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: sensitiveDataFindings.slice(0, 10).map(f => f.file),
                              rule: 'SENSITIVE_DATA_PATTERN',
                              impact: 'PRIVACY RISK: PII in logs or source code violates GDPR and increases breach liability.',
                              fix: 'Sanitize logs, remove PII from source, and use tokenization for identifiers.',
                              findings: sensitiveDataFindings.slice(0, 5),
                              reasoning:
                                  'Email addresses, phone numbers, and SSN-like patterns in non-test files indicate potential data leakage.',
                              confidence: 0.85,
                              humanReadable: `${sensitiveDataFindings.length} file(s) may expose sensitive data.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && configDriftFindings.length
                    ? [
                          {
                              ...
                              severityBand: 'medium',
                              type: 'Configuration Drift',
                              count: configDriftFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: configDriftFindings.slice(0, 10).map(f => f.file),
                              rule: 'CONFIG_DRIFT_PATTERN',
                              impact: 'SECURITY RISK: Literal endpoint values and secrets in config files bypass environment controls.',
                              fix: 'Store secrets outside source, inject endpoints via configuration, and audit config files.',
                              findings: configDriftFindings.slice(0, 5),
                              reasoning:
                                  'Config drift occurs when local settings are committed instead of environment-specific overrides.',
                              confidence: 0.8,
                              humanReadable: `${configDriftFindings.length} file(s) show configuration drift.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && securityHeaderFindings.length
                    ? [
                          {
                              ...
                              severityBand: 'medium',
                              type: 'Missing Security Header',
                              count: securityHeaderFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: securityHeaderFindings.slice(0, 10).map(f => f.file),
                              rule: 'SECURITY_HEADER_PATTERN',
                              impact: 'SECURITY RISK: Missing CSP or X-Frame-Options enables XSS and clickjacking.',
                              fix: 'Add helmet middleware or configure reverse proxy with CSP, HSTS, X-Frame-Options.',
                              findings: securityHeaderFindings.slice(0, 5),
                              reasoning:
                                  'Modern browsers enforce security headers to mitigate XSS, clickjacking, and MIME sniffing attacks.',
                              confidence: 0.83,
                              humanReadable: `${securityHeaderFindings.length} file(s) reference security headers — verify full coverage.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && dbPatternFindings.length
                    ? [
                          {
                              ...
                              severityBand: 'high',
                              type: 'Database Anti-Pattern',
                              count: dbPatternFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: dbPatternFindings.slice(0, 10).map(f => f.file),
                              rule: 'DB_PATTERN',
                              impact: 'SECURITY RISK: Raw SQL concatenation enables injection. Unbounded queries cause DoS.',
                              fix: 'Use parameterized queries, ORM methods, and always apply LIMIT/OFFSET.',
                              findings: dbPatternFindings.slice(0, 5),
                              reasoning:
                                  'String concatenation in SQL queries is the leading cause of injection vulnerabilities in web apps.',
                              confidence: 0.9,
                              humanReadable: `${dbPatternFindings.length} file(s) contain database anti-patterns.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && frameworkFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Framework Practice Issue',
                              count: frameworkFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: frameworkFindings.slice(0, 10).map(f => f.file),
                              rule: 'FRAMEWORK_PATTERN',
                              impact: 'MAINTAINABILITY RISK: Hook misuse and missing cleanup cause memory leaks and stale state.',
                              fix: 'Add dependency arrays to useEffect, unsubscribe in ngOnDestroy, avoid direct DOM access.',
                              findings: frameworkFindings.slice(0, 5),
                              reasoning:
                                  'React hooks without dependency arrays re-run on every render. Missing Angular unsubscribes leak memory.',
                              confidence: 0.85,
                              humanReadable: `${frameworkFindings.length} file(s) have framework practice issues.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && workspaceFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Workspace Health Issue',
                              count: workspaceFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: workspaceFindings.slice(0, 10).map(f => f.file),
                              rule: 'WORKSPACE_PATTERN',
                              impact: 'MAINTAINABILITY RISK: Circular imports and mismatched versions cause build failures.',
                              fix: 'Refactor shared code into a common package, align dependency versions across workspace.',
                              findings: workspaceFindings.slice(0, 5),
                              reasoning:
                                  'Relative imports between workspace packages suggest tight coupling that can become circular.',
                              confidence: 0.7,
                              humanReadable: `${workspaceFindings.length} file(s) show workspace health concerns.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && unusedDepFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Unused Dependency',
                              count: unusedDepFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: unusedDepFindings.slice(0, 10).map(f => f.file),
                              rule: 'UNUSED_DEP_PATTERN',
                              impact: 'BLOAT RISK: Unused packages increase install time, bundle size, and attack surface.',
                              fix: 'Remove unused packages from package.json and lockfile.',
                              findings: unusedDepFindings.slice(0, 5),
                              reasoning:
                                  'Dependencies not imported anywhere add zero value but increase supply chain risk.',
                              confidence: 0.75,
                              humanReadable: `${unusedDepFindings.length} file(s) suggest unused dependencies.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && apiContractFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'API Contract Drift',
                              count: apiContractFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: apiContractFindings.slice(0, 10).map(f => f.file),
                              rule: 'API_CONTRACT_PATTERN',
                              impact: 'INTEGRATION RISK: Unconsumed endpoints and stale OpenAPI specs mislead consumers.',
                              fix: 'Sync OpenAPI spec with implementation, remove dead endpoints, verify frontend coverage.',
                              findings: apiContractFindings.slice(0, 5),
                              reasoning:
                                  'Backend routes not called by frontend may be dead code or indicate missing frontend integration.',
                              confidence: 0.72,
                              humanReadable: `${apiContractFindings.length} file(s) show API contract drift.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && complexityFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'High Complexity',
                              count: complexityFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: complexityFindings.slice(0, 10).map(f => f.file),
                              rule: 'COMPLEXITY_PATTERN',
                              impact: 'MAINTAINABILITY RISK: Over-long functions and deep nesting increase bug density.',
                              fix: 'Extract helper functions, reduce nesting with early returns, apply cyclomatic complexity limits.',
                              findings: complexityFindings.slice(0, 5),
                              reasoning:
                                  'Functions over 50 lines or with deeply nested control flow are harder to test and reason about.',
                              confidence: 0.8,
                              humanReadable: `${complexityFindings.length} file(s) contain high complexity patterns.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && llmSlopFindings.length
                    ? [
                          {
                              severity: 'medium',
                              severityBand: 'medium',
                              type: 'LLM Slop',
                              count: llmSlopFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: llmSlopFindings.slice(0, 10).map(f => f.file),
                              rule: 'LLM_SLOP_PATTERN',
                              impact: 'HYGIENE RISK: Unresolved LLM placeholders and leaked markdown degrade source quality.',
                              fix: 'Remove placeholder debris and markdown code fences from source files.',
                              findings: llmSlopFindings.slice(0, 5),
                              reasoning:
                                  'AI coding assistants often leave behind placeholder text, markdown fences, or hardcoded default metrics that should not reach production.',
                              confidence: 0.85,
                              humanReadable: `${llmSlopFindings.length} file(s) contain LLM slop / placeholder debris.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && tokenBleedFindings.length
                    ? [
                          {
                              severity: 'medium',
                              severityBand: 'medium',
                              type: 'Token Bleed',
                              count: tokenBleedFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: tokenBleedFindings.slice(0, 10).map(f => f.file),
                              rule: 'TOKEN_BLEED_PATTERN',
                              impact: 'COST & PERFORMANCE RISK: Unbounded LLM API calls without token limits inflate costs.',
                              fix: 'Add max_tokens / max_completion_tokens to every LLM API call and chunk long literals.',
                              findings: tokenBleedFindings.slice(0, 5),
                              reasoning:
                                  'LLM API calls without max_tokens limits can consume excessive tokens and budget. Very long string literals in prompts risk context window overflow.',
                              confidence: 0.8,
                              humanReadable: `${tokenBleedFindings.length} file(s) show token bleed risk.`
                          }
                      ]
                    : []),
                    ...
                    ? [
                          {
                              ...
                              severityBand: 'medium',
                              type: 'Production Leak',
                              count: productionLeakFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: productionLeakFindings.slice(0, 10).map(f => f.file),
                              rule: 'PRODUCTION_LEAK_PATTERN',
                              impact: 'SECURITY RISK: Mock/fixture data paths referenced in production source can leak test data.',
                              fix: 'Replace hardcoded mock/fixture paths with environment-based configuration or runtime discovery.',
                              findings: productionLeakFindings.slice(0, 5),
                              reasoning:
                                  'References to mock, fixture, sample, or test-data directories in production source indicate potential test data leakage.',
                              confidence: 0.82,
                              humanReadable: `${productionLeakFindings.length} file(s) reference mock/fixture paths in production code.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && fictionKpiFindings.length
                    ? [
                          {
                              severity: 'medium',
                              severityBand: 'medium',
                              type: 'Fiction KPI',
                              count: fictionKpiFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: fictionKpiFindings.slice(0, 10).map(f => f.file),
                              rule: 'FICTION_KPI_PATTERN',
                              impact: 'TRUST RISK: Hardcoded metrics and confidence scores may be fabricated AI output.',
                              fix: 'Replace hardcoded KPIs with real data sources and dynamic calculations.',
                              findings: fictionKpiFindings.slice(0, 5),
                              reasoning:
                                  'Hardcoded completion rates, accuracy percentages, and AI confidence scores are commonly generated by LLMs as placeholder data.',
                              confidence: 0.88,
                              humanReadable: `${fictionKpiFindings.length} file(s) contain hardcoded fiction KPIs.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && archDriftFindings.length
                    ? [
                          {
                              severity: 'medium',
                              severityBand: 'medium',
                              ...
                              filePath: archDriftFindings.slice(0, 10).map(f => f.file),
                              rule: 'ARCH_DRIFT_PATTERN',
                              ...
                              findings: archDriftFindings.slice(0, 5),
                              reasoning:
                                  'Hybrid and state-space model identifiers without output schema validators indicate unguarded LLM integration.',
                              confidence: 0.75,
                              humanReadable: `${archDriftFindings.length} file(s) show architecture drift / unguarded LLM calls.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && securityFindings.length
                    ? [
                          {
                              severity: 'high',
                              severityBand: 'high',
                              type: 'Security Vulnerability',
                              count: securityFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: securityFindings.slice(0, 10).map(f => f.file),
                              rule: 'SECURITY_PATTERN',
                              impact: 'SECURITY RISK: eval(), XSS, prototype pollution, insecure random, or missing rate limits detected.',
                              fix: 'Replace eval with structured parsing, sanitize innerHTML, use crypto.randomBytes, add rate limiting.',
                              findings: securityFindings.slice(0, 5),
                              reasoning:
                                  'These patterns represent common attack vectors: code injection, XSS, prototype pollution, and predictable crypto.',
                              confidence: 0.85,
                              humanReadable: `${securityFindings.length} file(s) contain security vulnerabilities.`
                          }
                      ]
                    : []), // simplebeacon-ignore: scanner rule description text
                ...(profile.checkAiResidue && qualityFindings.length
                    ? [
                          {
                              severity: 'medium',
                              severityBand: 'medium',
                              type: 'Code Quality Issue',
                              count: qualityFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: qualityFindings.slice(0, 10).map(f => f.file),
                              rule: 'QUALITY_PATTERN',
                              impact: 'QUALITY RISK: Unhandled promises, missing strict mode, and uninitialized reads cause runtime errors.',
                              fix: 'Add .catch() to promises, use "use strict", initialize variables at declaration.',
                              findings: qualityFindings.slice(0, 5),
                              reasoning:
                                  'Unhandled rejections crash Node.js. Implicit globals from missing strict mode cause silent bugs.',
                              confidence: 0.82,
                              humanReadable: `${qualityFindings.length} file(s) have code quality issues.`
                          }
                      ]
                    : []),
                ...(profile.checkAiResidue && maintainabilityFindings.length
                    ? [
                          {
                              severity: 'low',
                              severityBand: 'low',
                              type: 'Maintainability Issue',
                              count: maintainabilityFindings.reduce((a, f) => a + f.matches.length, 0),
                              filePath: maintainabilityFindings.slice(0, 10).map(f => f.file),
                              rule: 'MAINTAINABILITY_PATTERN',
                              impact: 'MAINTAINABILITY RISK: Magic numbers and hardcoded literals make code harder to understand and modify.',
                              fix: 'Extract numeric literals to named constants (e.g., const MAX_RETRIES = 3).',
                              findings: maintainabilityFindings.slice(0, 5),
                              reasoning: 'Magic numbers obscure intent and make bulk updates error-prone.',
                              confidence: 0.78,
                              humanReadable: `${maintainabilityFindings.length} file(s) have maintainability issues.`
                          }
                      ]
                    : [])
            ],
            issues: [
                ...(profile.checkAi && aiHits.length
                    ? [{ severity: 'low', type: 'AI System Indicator', count: aiHits.length }]
                    : []),
                ...(profile.checkCredentials && credentialHits
                    ? [{ severity: 'medium', type: 'Credential Pattern', count: credentialHits }]
                    : []),
                ...(profile.checkDebug && debugHits.length
                    ? [{ severity: 'low', type: 'Debug Artifact', count: debugHits.length }]
                    : []),
                ...(profile.checkGov && govHits.length
                    ? [{ severity: 'low', type: 'License/Governance Marker', count: govHits.length }]
                    : []),
                ...(profile.checkAiResidue && aiResidueHits
                    ? [{ severity: 'low', type: 'AI Residue', count: aiResidueHits }]
                    : []),
                ...(profile.checkAiResidue && perfHits
                    ? [{ severity: 'low', type: 'Performance Anti-Pattern', count: perfHits }]
                    : []),
                ...(profile.checkAiResidue && typeSafetyHits
                    ? [{ severity: 'low', type: 'Type Safety Gap', count: typeSafetyHits }]
                    : []),
                ...(profile.checkAiResidue && testHits
                    ? [{ severity: 'low', type: 'Missing Test Coverage', count: testHits }]
                    : []),
                ...(profile.checkAiResidue && a11yHits
                    ? [{ severity: 'medium', type: 'Accessibility Gap', count: a11yHits }]
                    : []),
                ...(profile.checkAiResidue && i18nHits
                    ? [{ severity: 'low', type: 'i18n Issue', count: i18nHits }]
                    : []),
                ...(profile.checkAiResidue && sensitiveDataHits
                    ? [{ severity: 'high', type: 'Sensitive Data Exposure', count: sensitiveDataHits }]
                    : []),
                ...(profile.checkAiResidue && configDriftHits
                    ? [{ severity: 'medium', type: 'Configuration Drift', count: configDriftHits }]
                    : []),
                ...(profile.checkAiResidue && securityHeaderHits
                    ? [{ severity: 'medium', type: 'Missing Security Header', count: securityHeaderHits }]
                    : []),
                ...(profile.checkAiResidue && dbPatternHits
                    ? [{ severity: 'high', type: 'Database Anti-Pattern', count: dbPatternHits }]
                    : []),
                ...(profile.checkAiResidue && frameworkHits
                    ? [{ severity: 'low', type: 'Framework Practice Issue', count: frameworkHits }]
                    : []),
                ...(profile.checkAiResidue && workspaceHits
                    ? [{ severity: 'low', type: 'Workspace Health Issue', count: workspaceHits }]
                    : []),
                ...(profile.checkAiResidue && unusedDepHits
                    ? [{ severity: 'low', type: 'Unused Dependency', count: unusedDepHits }]
                    : []),
                ...(profile.checkAiResidue && apiContractHits
                    ? [{ severity: 'low', type: 'API Contract Drift', count: apiContractHits }]
                    : []),
                ...(profile.checkAiResidue && complexityHits
                    ? [{ severity: 'low', type: 'High Complexity', count: complexityHits }]
                    : []),
                ...(profile.checkAiResidue && llmSlopHits
                    ? [{ severity: 'medium', type: 'LLM Slop', count: llmSlopHits }]
                    : []),
                ...(profile.checkAiResidue && tokenBleedHits
                    ? [{ severity: 'medium', type: 'Token Bleed', count: tokenBleedHits }]
                    : []),
                ...(profile.checkAiResidue && productionLeakHits
                    ? [{ severity: 'medium', type: 'Production Leak', count: productionLeakHits }]
                    : []),
                ...(profile.checkAiResidue && fictionKpiHits
                    ? [{ severity: 'medium', type: 'Fiction KPI', count: fictionKpiHits }]
                    : []),
                ...(profile.checkAiResidue && archDriftFindings.length
                    ? [{ severity: 'medium', type: 'Architecture Drift', count: archDriftFindings.length }]
                    : []),
                ...(profile.checkAiResidue && securityHits
                    ? [{ severity: 'high', type: 'Security Vulnerability', count: securityHits }]
                    : []),
                ...(profile.checkAiResidue && qualityHits
                    ? [{ severity: 'medium', type: 'Code Quality Issue', count: qualityHits }]
                    : []),
                ...(profile.checkAiResidue && maintainabilityHits
                    ? [{ severity: 'low', type: 'Maintainability Issue', count: maintainabilityHits }]
                    : [])
            ],
            rawIssues: [
                ...(profile.checkAi && aiHits.length
            ...
            aiContext: {
                ...
                readerGuide: {
                    purpose:
                        'This report contains a static analysis of a JavaScript/TypeScript codebase. Each module section (gate, cleanup, performance, etc.) contains findings with file paths, line numbers, snippets, and suggested fixes.',
                    howToUse:
                        'Start with aiContext.suggestedFixes[] for prioritized actionable tasks. Each fix has file, line, currentCode, replacement, suggestedPatch (unified diff), and verificationCommand. For high-level planning, use moduleDependencies and completionCriteria.',
                    keyFields: {
                        ...
                        'aiContext.suggestedFixes[].suggestedPatch':
                            'Unified diff string — can be applied with git apply',
                        'aiContext.suggestedFixes[].context': 'Array of surrounding code lines for broader context',
                        'aiContext.suggestedFixes[].autoFixable':
                            'Boolean — if true, AI can apply without human review',
                        ...
                        'detectedIssues[].fix':
                            'Actionable remediation string — can be turned into a code change directly',
                ...
                moduleDependencies: [
                    {
                        id: 'security',
                        dependsOn: null,
                        reason: 'Fix credentials && secrets first — they are the highest-risk items'
                    },
                    { id: 'sensitive-data', dependsOn: null, reason: 'Remove PII from source before refactoring' },
                    {
                        id: 'database-patterns',
                        dependsOn: null,
                        reason: 'Fix SQL injection vectors before feature work'
                    },
                    {
                        id: 'config-drift',
                        dependsOn: null,
                        reason: 'Move hardcoded secrets to env before other changes'
                    },
                    {
                        id: 'data-quality',
                        dependsOn: 'security',
                        reason: 'Validate JSON integrity after security fixes'
                    },
                    {
                        id: 'cleanup',
                        dependsOn: 'security',
                        reason: 'Remove debug artifacts only after verifying no secrets are in them'
                    },
                    {
                        id: 'performance',
                        dependsOn: 'cleanup',
                        reason: 'Optimize after removing dead code && debug artifacts'
                    },
                    ...
                    { id: 'test-coverage', dependsOn: 'type-safety', reason: 'Write tests after types are stable' },
                    {
                        ...
                        reason: 'Document after APIs are tested && finalized'
                    }
                ],
                completionCriteria: [
                    {
                        ...
                        criteria: [
                            ...
                            'secretlint or grep scan returns clean'
                        ]
                    },
                    {
                        phase: 'cleanup',
                        criteria: [
                            ...
                            'No self-generated bloat files',
                            'eslint-plugin-no-console passes in CI'
                        ]
                    },
                    {
                        phase: 'performance',
                        criteria: [
                            'No nested O(n²) loops in hot paths',
                            'All event listeners properly removed on unmount',
                            'Benchmark tests pass within baseline'
                        ]
                    },
                    {
                        phase: 'type-safety',
                        criteria: [
                            ...
                            'No any types in src/',
                            'All public functions have JSDoc @param tags'
                        ]
                    },
                    {
                        phase: 'test-coverage',
                    ...
                    },
                    {
                        phase: 'documentation',
                        criteria: [
                            'All exported functions have JSDoc or docstrings',
                            'README updated for new APIs',
                    ...
                    },
                    {
                        phase: 'eu-ai-act',
                        criteria: [
                            'AI disclosure notices present in UI',
                            'No prohibited practices per Article 5',
                            'Risk assessment document current'
                        ]
                    }
                ],
            ...
            ...(allowedSections.includes('consolidation')
                ? {
                      consolidation: {
                          ...
                          duplicateGroups: duplicateGroups,
                          duplicateFiles: Array.from(duplicateHashes.values())
                              .filter(g => g.length > 1)
                              .slice(0, 250)
                              .map(g => ({ paths: g, count: g.length, sizeHint: g[0].split('.').pop() })),
                          summary:
                              duplicateGroups > 0
                                  ? `${duplicateGroups} duplicate file groups detected.`
            ...
            ...(allowedSections.includes('roadmap')
                ? {
                      roadmap: {
                          ...
                          syntheticTodoCount: typeof syntheticTodoFiles !== 'undefined' ? syntheticTodoFiles.length : 0,
                          syntheticTodoMarkers:
                              typeof syntheticTodoMarkerCounts !== 'undefined' ? syntheticTodoMarkerCounts.total : 0,
                          syntheticTodoFiles:
                              typeof syntheticTodoFiles !== 'undefined'
                                  ? syntheticTodoFiles.map(f => f.path || f).slice(0, 20)
                                  : [],
                          markerBreakdown: {
                              tasks: todoMarkerCounts.tasks,
                              fixes: todoMarkerCounts.fixes,
                              hacks: todoMarkerCounts.hacks,
                              bugs: todoMarkerCounts.bugs,
                              notes: todoMarkerCounts.notes,
                              reviews: todoMarkerCounts.reviews,
                              deprecateds: todoMarkerCounts.deprecateds,
                              optimizes: todoMarkerCounts.optimizes
                          },
                          summary:
                              todoFiles.length > 0
                                  ? `${todoFiles.length} files contain task/fix markers.`
                      ...
                      dataQuality: {
                          ...
                          summary:
                              emptyJsonFiles.length > 0
                                  ? `${emptyJsonFiles.length} empty || trivial JSON files detected.`
                                  : 'No empty JSON files detected.'
                      }
                  }
                : {}),
            ...(allowedSections.includes('cleanup')
                ? {
                      cleanup: {
                          ...
                          summary:
                              (debugHits.length > 0
                                  ? `${debugHits.length} debug artifacts detected.`
            ...
            ...(allowedSections.includes('junkFiles')
                ? {
                      junkFiles: {
                          ...
                          summary:
                              junkFiles.length > 0
                                  ? `${junkFiles.length} junk || temporary file${junkFiles.length === 1 ? '' : 's'} detected.`
                          ...
                          repositoryHygiene: {
                              ...
                              uselessPct:
                                  files.length > 0
                                      ? (((files.length - usefulCount) / files.length) * 100).toFixed(1)
                                      : '0.0',
                              breakdown: {
                      ...
                      npmAudit: {
                          ...
                          summary:
                              packageJsonFiles.length > 0
                                  ? `${packageJsonFiles.length} package.json file${packageJsonFiles.length === 1 ? '' : 's'} found with ${totalDependencyCount.toLocaleString()} total dependenc${totalDependencyCount === 1 ? 'y' : 'ies'}.`
            ...
            ...(allowedSections.includes('euAiActSummary')
                ? (() => {
                      ...
                      const highRiskIndicators = warnOrWorse.filter(
                      ...
                      return {
                          euAiActSummary: {
                              ...
                              deadlineNote:
                                  aiHits.length > 0
                                      ? `${aiHits.length} AI system indicator${aiHits.length === 1 ? '' : 's'} detected; ${warnOrWorse.length} control(s) require action, ${reviewItems.length} need review.`
            ...
            ...(allowedSections.includes('dependencyAudit')
                ? {
                      dependencyAudit: {
                          ...
                          outdatedPackages: [],
                          summary:
                              'Browser scan does ! execute npm audit. Run CLI scan with --audit flag for full CVE report.'
                      }
                  }
                : {}),
            ...
            fileList: files
                ...
                .filter(
                    p =>
                        !/node_modules\/|\.git\/|\.github-sync\/|github-cache\/|\.cursor\/|\.windsurf\/|\.vscode\/|\.idea\/|\.husky\/|\.simplebeacon\/|backups\/|java-ai-vulnerable\//.test(
                            p.toLowerCase()
                        )
                ),
            ...(allowedSections.includes('architectureDrift')
                ? {
                      architectureDrift: {
                          ...
                          count: archDriftFindings.length,
                          summary:
                              archDriftFindings.length > 0
                                  ? `${archDriftFindings.length} architecture drift / token bleed finding(s) detected.`
                                  : 'No architecture drift or token bleed issues detected.'
                      }
                  }
                : {}),
            ...
            ...(allowedSections.includes('fileNaming')
                ? {
                      fileNaming: {
                          ...
                          summary:
                              fileNamingHits > 0
                                  ? `${fileNamingHits} file naming issue(s) detected.`
                                  : 'No file naming issues detected.'
                      }
                  }
                : {}),
            ...
            ...(allowedSections.includes('buildReadiness')
                ? (() => {
                      ...
                      const isSubfolder =
                          allPaths.length > 0 &&
                          allPaths.every(p => p.startsWith(commonPrefix + '/') || p === commonPrefix);
                      ...
                      const brChecks = langChecks.map(check => ({
                          name: check.name,
                          found: check.customCheck ? check.customCheck(allPaths) : findAtAnyDepth(check.regex),
                      ...
                      const hasLockfileAnywhere = lowerPaths.some(p =>
                          /(^|\/)(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|bun\.lockb|bun\.lock)$/.test(p)
                      );
                      ...
                      return {
                          buildReadiness: {
                              ...
                              remediation:
                                  brMissingCritical.length > 0
                                      ? `Missing critical: ${brMissingCritical.map(c => c.name).join(', ')}.`
                                      : brMissingNice.length > 0
                                        ? `Missing recommended: ${brMissingNice.map(c => c.name).join(', ')}.`
                              ...
                              recommendations:
                                  brMissingCritical.length > 0
                                      ...
                                      : brMissingNice.length > 0
                                        ...
                                        : ['Project is fully ready for production. All checklist items present.'] // simplebeacon-ignore: recommendation text, not a hardcoded secret
                          }
                      };
                  })()
                : {}),
            ...
            scanProfile:
                (browserScanProfile === null || browserScanProfile === void 0 ? void 0 : browserScanProfile.value) ||
                'gate',
            ...
            remediationPhases: (() => {
                const getTaskLsDone = (projectName, phaseId, taskIdx) => {
                    try {
                        const pk =
                            'sbr_' + String(projectName).replace(/[^a-z0-9]/gi, '_') + '_' + phaseId + '_t' + taskIdx;
                ...
                const makePhase = (id, title, severity, effort, description, tasks, donePredicates, patches) => {
                    ...
                    const taskObjs = tasks.map((t, i) => {
                        ...
                        const autoDone =
                            ...
                            typeof t === 'object' &&
                            t !== null &&
                            (t.type === 'verify' || t.type === 'review');
                        if (typeof t === 'object' && t !== null) {
                            return { ...t, done: reportDone || lsDone || autoDone, patch: patches ? patches[i] : null };
                        }
                        return {
                            description: t,
                            done: reportDone || lsDone || autoDone,
                            patch: patches ? patches[i] : null
                        };
                    });
                ...
                return [
                    ...(gateBlockingCount > 0
                        ? [
                              makePhase(
                                  ...
                                  'critical',
                                  '1–2 days',
                                  `${gateBlockingCount} credential finding(s) detected.`,
                                  [
                                      {
                                          description: 'Review flagged credential patterns',
                                          type: 'review',
                                          isStructured: true
                                      },
                                      {
                                          ...
                                          type: 'fix',
                                          codeSnippet: 'mv secrets-file secrets-file.example',
                                          isStructured: true
                                      },
                                      {
                        ...
                        ? (() => {
                              ...
                              const tailTasks = [
                                  ...
                                  },
                                  {
                                      ...
                                      type: 'verify',
                                      codeSnippet: 'npx simplebeacon scan',
                                      isStructured: true
                                  }
                              ];
                              ...
                              return [
                                  makePhase(
                                      ...
                                      'high',
                                      '2–4 days',
                                      `${emptyJsonFiles.length} empty/trivial JSON file(s) detected.`,
                                      allTasks,
                                      allTasks.map(() => false)
                                  )
                              ];
                    ...
                    ...(duplicateGroups > 0
                        ? [
                              makePhase(
                                  ...
                                  duplicateGroups > 5 ? 'high' : 'medium',
                                  '3–5 days',
                                  `${duplicateGroups} duplicate file group(s) detected.`,
                                  [
                                      ...
                                      {
                                          description: 'Document canonical file locations',
                                          type: 'doc',
                                          isStructured: true
                                      }
                                  ],
                        ...
                        : []),
                    ...(debugHits.length > 0 || bloatFindings.length > 0
                        ? [
                              makePhase(
                                  'cleanup',
                                  'Cleanup & Hygiene',
                                  debugHits.length > 50 ? 'high' : 'medium',
                                  '1–2 days',
                                  `${debugHits.length} debug artifact(s)${bloatFindings.length > 0 ? ` + ${bloatFindings.length} bloat file(s)` : ''} detected.`,
                                  [
                                      {
                                          description: 'Remove console.log/debugger statements',
                                          type: 'fix',
                                          codeSnippet: 'eslint --fix --rule "no-console: error" src/',
                                          isStructured: true
                                      },
                                      {
                                          description: 'Delete self-generated bloat files',
                                          type: 'fix',
                                          isStructured: true
                                      },
                                      {
                                          description: 'Install eslint-plugin-no-console for CI',
                                          type: 'fix',
                                          codeSnippet: 'npm install eslint-plugin-no-console --save-dev',
                                          isStructured: true
                                      }
                                  ],
                                  [false, false, false]
                        ...
                        ? (() => {
                              ...
                              return [
                                  makePhase(
                                      ...
                                      'medium',
                                      '2–3 days',
                                      `${licenseFiles.length} license file(s), ${securityFiles.length} security file(s).`,
                                      tasks,
                                      tasks.map(() => false)
                                  )
                              ];
                    ...
                    ...(aiHits.length > 0
                        ? [
                              makePhase(
                                  ...
                                  aiHits.length > 0 ? 'high' : 'medium',
                                  '5–10 days',
                                  `${aiHits.length} AI system indicator(s) detected.`,
                                  [
                                      {
                    ...
                    ...(packageJsonFiles.length > 0
                        ? (() => {
                              ...
                              const perPkgTasks = packageJsonFiles
                                  .filter(p => p.depCount > 0 || p.devDepCount > 0)
                                  .slice(0, 5)
                                  .map(p => {
                                      ...
                                      ? [
                                            {
                                                ...
                                                location: packageJsonFiles
                                                    .filter(p => !p.hasLockfile)
                                                    .map(p => p.path)
                                                    .join(', '),
                                                isStructured: true
                                            }
                              ...
                              return [
                                  makePhase(
                                      'npmaudit',
                                      'npm Audit',
                                      missingLockfiles > 0 ? 'medium' : 'low',
                                      '1 day',
                                      `${packageJsonFiles.length} package.json file(s), ${totalDependencyCount} total dependencies${missingLockfiles > 0 ? `, ${missingLockfiles} missing lockfile(s)` : ''}.`,
                                      allTasks,
                                      allTasks.map(() => false)
                                  )
                              ];
                        ...
                        ? (() => {
                              ...
                              return [
                                  makePhase(
                                      ...
                                      'low',
                                      'Ongoing',
                                      `${todoFiles.length} TODO/FIXME marker(s) in source code.`,
                                      allTasks,
                                      allTasks.map(() => false)
                                  )
                              ];
        ...
        const derivedCounts = (reportData.detectedIssues || []).reduce(
            (acc, issue) => {
        ...
        appendTerminalLine(
            'Zero-Retention Safeguard Verified. Report compiled locally inside browser sandbox.',
            'success'
        );
        ...
        if (dq.overall === 'FAIL') {
            appendTerminalLine(
                'Certificate generation blocked: Data quality validation failed. Review the scorecard above.',
                'error'
            );
        ...
        const statusParts = [
            `Complete scan: ${files.length.toLocaleString()} files discovered · ${sourceFiles.length.toLocaleString()} analyzed`,
            `${selectedModules.size} modules selected for ZIP`
        ];
        ...
        pre.style.cssText =
            'margin-top:8px;padding:12px;background:#0B0F19;border:1px solid var(--border);border-radius:8px;font-size:0.75rem;color:#94A3B8;overflow-x:auto;max-height:200px;';
