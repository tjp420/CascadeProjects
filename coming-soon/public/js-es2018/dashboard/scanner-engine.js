    ...
    for (let li = 0; li < lines.length && matches.length < maxMatches; li++) {
        ...
        if (line.length > MAX_LINE_LEN) {
            ...
            if (/\{2000,\}/.test(pattern.source || pattern)) {
                if (!regex.test(line.slice(0, MAX_LINE_LEN)))
                    continue;
            }
            else {
                continue;
        ...
        if (regex.test(line)) {
            ...
            matches.push({
                ...
                snippet: snippet,
                context: [
                    ((_a = lines[Math.max(0, li - 1)]) === null || _a === void 0 ? void 0 : _a.trim().slice(0, 100)) || null,
                    snippet,
                    ((_b = lines[Math.min(lines.length - 1, li + 1)]) === null || _b === void 0 ? void 0 : _b.trim().slice(0, 100)) || null
                ].filter(Boolean)
            });
        }
    }
...
function buildSuggestedFixes(collections) {
    ...
    const isFalsePositive = (file, type, snippet) => {
        ...
        if (/scanner-engine\.js/.test(fp))
            return true;
        // Test files: fixtures are intentionally hardcoded
        if (/\.(test|spec)\.(js|ts|cjs|mjs)$/.test(fp))
            return true;
        // Dedicated test / false-positive directories
        if (/simplebeacon-rule-tests\//.test(fp))
            return true;
        if (/false-positive-audit\//.test(fp))
            return true;
        if (/\.vscode-test\//.test(fp))
            return true;
        if (/simplebeacon-vscode-merged\//.test(fp))
            return true;
        if (/\.vsix$/i.test(fp))
            return true;
        if (/coming-soon\/archive\/test-/.test(fp))
            return true;
        // Crypto token generation (random, not hardcoded secret)
        if (type === 'Credential Pattern' && /crypto\.randomBytes|Math\.random|Date\.now/.test(s))
            return true;
        // CLI help text
        if (type === 'Debug Artifact' && /console\.(error|warn|log)\s*\(['"`]\s*Usage:\s*node/.test(s))
            return true;
        // Gated debug logging (production-safe)
        if (type === 'Debug Artifact' && /if\s*\(\s*(DEBUG|PROCESSOR_DEBUG)\s*\)/.test(s))
            return true;
        // Error handling in catch blocks
        if (type === 'Debug Artifact' && /catch\s*\([^)]*\)\s*\{[^}]*console\.(error|warn)/.test(s))
            return true;
        // server.cjs — structured env/billing startup logging
        if (type === 'Debug Artifact' && /server\.cjs$/.test(fp) && /console\.(warn|error)\s*\(\s*['"`]\[[A-Z][a-zA-Z]*\]/.test(s))
            return true;
        // analyze-directory.js — CLI progress and summary output
        if (type === 'Debug Artifact' && /analyze-directory\.js$/.test(fp) && /console\.log\s*\(.*(?:TOTAL|FOLDERS|SIZE|LINES|DURATION|SKIPPED|Analysis complete|Report saved|Target:|Started:|files indexed|Top \d|Largest Files)/.test(s))
            return true;
        // main.js — structured UI event logging with bracket prefixes
        if (type === 'Debug Artifact' && /js\/dashboard\/main\.js$/.test(fp) && /console\.(log|warn|error|info|debug)\s*\(.*\[[A-Za-z][a-zA-Z]*\]/.test(s))
            return true;
        // token-manager.js — intentional or commented-out config logging
        if (type === 'Debug Artifact' && /js\/dashboard\/token-manager\.js$/.test(fp) && /console\.(log|warn|error|info|debug)\s*\(/.test(s))
            return true;
        ...
        if (type === 'Production Leak' && /\b(?:sampleFiles|sampleUrl|sampleData|recentSamples|sampleCount|recentFiles)\b/.test(s))
            return true;
        ...
        if (type === 'License/Governance Marker' && /ai_system\s*:\s*['"]ready['"]/.test(s))
            return true;
        // EU AI Act comments in server code
        if (type === 'License/Governance Marker' && /Article\s*50|EU AI Act|Transparency|high-risk/.test(s) && /server\/|tools\//.test(fp))
            return true;
        // simplebeacon-ignore comments
        if (/simplebeacon-ignore/.test(s))
            return true;
        // Synchronous fs in VS Code extension source files (standard for init/package reads)
        if (type === 'Synchronous File Operation' && /simplebeacon-vscode.*\/src\//.test(fp))
            return true;
        if (type === 'Synchronous File Operation' && /build-extension\.js|build-public\.js|replace-dashboard\.js/.test(fp))
            return true;
        if (type === 'Synchronous File Operation' && /extension\.ts.*readFileSync.*package\.json/.test(s))
            return true;
        ...
        if (type === 'Type Safety Gap' && /postJson\(.*payload:\s*any|_uploadReport\(.*data:\s*any/.test(s))
            return true;
        if (type === 'Type Safety Gap' && /buildHierarchy.*:\s*any/.test(s))
            return true;
        // innerHTML XSS: static template literals with no user input interpolation
        if (type === 'innerHTML XSS Risk' && /\.innerHTML\s*=\s*`[^`]*`/.test(s) && !/\$\{[^}]*\}/.test(s))
            return true;
        if (type === 'innerHTML XSS Risk' && /\/views\/(Audit|Quality|Security|Trust|Profile|Chatbot|SignIn)View\.js$/.test(fp) && /container\.innerHTML|recoveryForm\.innerHTML/.test(s))
            return true;
        // Config Drift: comment text and contextFilter documentation, not actual config
        if (type === 'Configuration Drift' && /\/\/.*hardcoded|contextFilter|scanner-engine\.js|CONFIG_DRIFT_PATTERN/.test(s))
            return true;
        if (type === 'Configuration Drift' && /browser-extension\//.test(fp))
            return true;
        if (type === 'Configuration Drift' && /Literal URL values and secrets in config files bypass environment controls/.test(s))
        ...
        if (type && type[0] === 'H' && EXCLUDED_FROM_HC_URL.test(fp))
            return true;
        // Missing Security Header: CSP builder comments and meta tag detection, not missing headers
        if (type === 'Missing Security Header' && /\/\*\*.*Build a Content-Security-Policy|Build a Content-Security-Policy meta tag/.test(s))
            return true;
        if (type === 'Missing Security Header' && /missing security header|security header|csp-source/.test(s) && /return true|return false/.test(s))
            return true;
        // AI Residue: legitimate vscode.postMessage error handling, not AI stubs
        if (type === 'Error Swallowing' && /vscode\.postMessage failed/.test(s))
            return true;
        if (type === 'AI Residue' && /catch\s*\([^)]*\)\s*\{[^}]*console\.warn.*vscode\.postMessage/.test(s))
            return true;
        // Accessibility Gap: inputs already wrapped in labels
        if (type === 'Accessibility Gap' && /<label[^>]*>.*<(input|textarea|select)/.test(s))
            return true;
        if (type === 'Accessibility Gap' && /id=["']?(layoutSelect|categoryFilter|autoScan|fullTreeCheck|chatbot-remove-filters)/.test(s))
            return true;
        return false;
    };
    Object.entries(collections).forEach(([collectionName, findings]) => {
        ...
        findings.forEach(f => {
            ...
            if (isFalsePositive(file, type, snippet))
                return;
            ...
            if (!verificationCommand) {
                ...
                if (['js', 'cjs', 'mjs'].includes(ext))
                    verificationCommand = 'node -c ' + file;
                else if (['ts', 'tsx'].includes(ext))
                    verificationCommand = 'npx tsc --noEmit';
                else if (['py'].includes(ext))
                    verificationCommand = 'python -m py_compile ' + file;
                else if (['java'].includes(ext))
                    verificationCommand = 'javac -d /tmp ' + file;
                else
...
function computeDynamicSeverity(baseSeverity, snippet, filePath, language) {
    if (isInComment(snippet, language))
        return 'low';
    ...
    if (baseSeverity === 'high' && isLiteral && !hasVariableRef)
        return 'medium';
    if (baseSeverity === 'medium' && isLiteral && !hasVariableRef)
        return 'low';
    if (baseSeverity === 'high' && hasVariableRef)
        return 'critical';
    return baseSeverity;
}
function computeConfidence(dynamicSeverity, snippet, filePath, language) {
    ...
    if (isInComment(snippet, language))
        ...
        confidence = 0.5;
    else if (dynamicSeverity === 'critical')
        confidence = 0.95;
    return confidence;
}
...
const CATEGORY_COLLECTORS = {
    ...
    sensitiveData: (path, reg, matches, ctx) => {
        ...
        const triaged = matches.map(m => {
            ...
            else if (/\.(md|txt|markdown)$/.test(lowerPath) && /support|contact|hello|press|security|docs/.test(snippet)) {
                triageConfidence = 'low';
            ...
            else if (/tools\/(generate|send|scan|test)/.test(lowerPath) || /demo|fixture|mock|sample|test/.test(lowerPath)) {
...
const CUSTOM_HANDLER_REGISTRY = {
    'gate': {
        ...
        collect: function (collector, ctx) { if (collector)
            ctx.credentialFindings = (ctx.credentialFindings || []).concat(collector); }
    },
    'consolidation': {
        category: 'governance',
        collect: function (collector, ctx) { if (collector)
            ctx.consolidation = collector; }
    },
    'mock-data': {
        category: 'aiResidue',
        collect: function (collector, ctx) { if (collector)
            ctx.mockDataCategories = collector; }
    },
    'roadmap': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.roadmap = collector; }
    },
    'codebase': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.codebase = collector; }
    },
    'file-reduction': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.fileReduction = collector; }
    },
    'data-quality': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.dataQuality = collector; }
    },
    'cleanup': {
        category: 'debug',
        collect: function (collector, ctx) { if (collector)
            ctx.cleanup = collector; }
    },
    'npm-audit': {
        category: 'unusedDeps',
        collect: function (collector, ctx) { if (collector)
            ctx.npmAudit = collector; }
    },
    'compliance': {
        category: 'governance',
        collect: function (collector, ctx) { if (collector)
            ctx.compliance = collector; }
    },
    'eu-ai-act': {
        category: 'aiIndicators',
        collect: function (collector, ctx) { if (collector)
            ctx.euAiActSummary = collector; }
    },
    'dependency-vulns': {
        category: 'unusedDeps',
        collect: function (collector, ctx) { if (collector)
            ctx.dependencyAudit = collector; }
    },
    'build-readiness': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.buildReadiness = collector; }
    },
    'junk-files': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.junkFiles = collector; }
    },
    'ai-residue': {
        category: 'aiResidue',
        collect: function (collector, ctx) { if (collector)
            ctx.aiResidueFindings = (ctx.aiResidueFindings || []).concat(collector); }
    },
    'performance': {
        category: 'performance',
        collect: function (collector, ctx) { if (collector)
            ctx.performanceFindings = (ctx.performanceFindings || []).concat(collector); }
    },
    'type-safety': {
        category: 'typeSafety',
        collect: function (collector, ctx) { if (collector)
            ctx.typeSafetyFindings = (ctx.typeSafetyFindings || []).concat(collector); }
    },
    'documentation': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.documentationFindings = (ctx.documentationFindings || []).concat(collector); }
    },
    'test-coverage': {
        category: 'testCoverage',
        collect: function (collector, ctx) { if (collector)
            ctx.testCoverageFindings = (ctx.testCoverageFindings || []).concat(collector); }
    },
    'accessibility': {
        category: 'accessibility',
        collect: function (collector, ctx) { if (collector)
            ctx.accessibilityFindings = (ctx.accessibilityFindings || []).concat(collector); }
    },
    'i18n': {
        category: 'i18n',
        collect: function (collector, ctx) { if (collector)
            ctx.i18nFindings = (ctx.i18nFindings || []).concat(collector); }
    },
    'database-patterns': {
        category: 'databasePatterns',
        collect: function (collector, ctx) { if (collector)
            ctx.databasePatternsFindings = (ctx.databasePatternsFindings || []).concat(collector); }
    },
    'framework-practices': {
        category: 'frameworkPractices',
        collect: function (collector, ctx) { if (collector)
            ctx.frameworkPracticesFindings = (ctx.frameworkPracticesFindings || []).concat(collector); }
    },
    'workspace-health': {
        category: 'workspaceHealth',
        collect: function (collector, ctx) { if (collector)
            ctx.workspaceHealthFindings = (ctx.workspaceHealthFindings || []).concat(collector); }
    },
    'unused-deps': {
        category: 'unusedDeps',
        collect: function (collector, ctx) { if (collector)
            ctx.unusedDepsFindings = (ctx.unusedDepsFindings || []).concat(collector); }
    },
    'api-contract': {
        category: 'apiContract',
        collect: function (collector, ctx) { if (collector)
            ctx.apiContractFindings = (ctx.apiContractFindings || []).concat(collector); }
    },
    'complexity': {
        category: 'complexity',
        collect: function (collector, ctx) { if (collector)
            ctx.complexityFindings = (ctx.complexityFindings || []).concat(collector); }
    },
    'fix-preview': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.fixPreview = collector; }
    },
    'file-naming': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.fileNamingFindings = (ctx.fileNamingFindings || []).concat(collector); }
    },
    'removable-files': {
        category: 'maintainability',
        collect: function (collector, ctx) { if (collector)
            ctx.removableFilesFindings = (ctx.removableFilesFindings || []).concat(collector); }
    }
};
/**
...
function getAnalyzersForLanguage(langKey, profile) {
    ...
    for (const [id, entry] of Object.entries(PATTERN_REGISTRY)) {
        if (!entry.appliesTo || !entry.appliesTo.includes(langKey))
            continue;
        const cat = ((_a = ANALYZER_SCHEMA[id]) === null || _a === void 0 ? void 0 : _a.category) || '';
        // Gate by profile flags based on category
        if (cat === 'aiIndicators' && !profile.checkAi)
            continue;
        if (cat === 'credentials' && !profile.checkCredentials)
            continue;
        if (cat === 'debug' && !profile.checkDebug)
            continue;
        if (cat === 'governance' && !profile.checkGov)
            continue;
        if (cat === 'aiResidue' && !profile.checkAiResidue)
            continue;
        if (cat === 'performance' && !profile.checkAiResidue)
            continue;
        if (cat === 'typeSafety' && !profile.checkAiResidue)
            continue;
        if (cat === 'testCoverage' && !profile.checkAiResidue)
            continue;
        if (cat === 'accessibility' && !profile.checkAiResidue)
            continue;
        if (cat === 'i18n' && !profile.checkAiResidue)
            continue;
        if (cat === 'sensitiveData' && !profile.checkAiResidue)
            continue;
        if (cat === 'configDrift' && !profile.checkAiResidue)
            continue;
        if (cat === 'securityHeaders' && !profile.checkAiResidue)
            continue;
        if (cat === 'databasePatterns' && !profile.checkAiResidue)
            continue;
        if (cat === 'frameworkPractices' && !profile.checkAiResidue)
            continue;
        if (cat === 'workspaceHealth' && !profile.checkAiResidue)
            continue;
        if (cat === 'unusedDeps' && !profile.checkAiResidue)
            continue;
        if (cat === 'apiContract' && !profile.checkAiResidue)
            continue;
        if (cat === 'complexity' && !profile.checkAiResidue)
            continue;
        if (cat === 'llmSlop' && !profile.checkAiResidue)
            continue;
        if (cat === 'tokenBleed' && !profile.checkAiResidue)
            continue;
        if (cat === 'productionLeak' && !profile.checkAiResidue)
            continue;
        if (cat === 'fictionKpi' && !profile.checkAiResidue)
            continue;
        if (cat === 'security' && !profile.checkAiResidue)
            continue;
        if (cat === 'quality' && !profile.checkAiResidue)
            continue;
        if (cat === 'maintainability' && !profile.checkAiResidue)
            continue;
        active.push(id);
    }
    return active;
...
function analyzeFileNaming(paths) {
    ...
    for (const rawPath of paths) {
        ...
        if (/^\./.test(fileName) || /node_modules|\.git|\.github|dist|build|coverage|\.next|out/.test(path))
            continue;
        // Extension tracking
...
function analyzeRemovableFiles(files) {
    ...
    for (const file of files) {
        ...
        for (const { pattern, cat } of pathToCategory) {
            if (pattern.test(filePath)) {
                ...
                if (categories[cat].examples.length < 5)
                    categories[cat].examples.push(filePath);
                matched = true;
                break;
            }
        }
        if (!matched && file.size === 0) {
            ...
            if (categories.empty_files.examples.length < 5)
                categories.empty_files.examples.push(filePath);
        }
    }
...
async function processLocalCLIScan(files) {
        ...
        for (const cfg of Object.values(LANGUAGE_REGISTRY)) {
            for (const rootFile of cfg.rootFiles) {
                ...
                if (rootFile.startsWith('*.')) {
                    ...
                    if (paths.some(p => p.startsWith(prefix + '/') && p.endsWith(ext)))
                        return true;
                }
            }
        }
        ...
        downloadLogBtn.onclick = () => {
            try {
                ...
                const lines = Array.from(tc.children).map(div => {
                    var _a;
                    const ts = ((_a = div.querySelector('span')) === null || _a === void 0 ? void 0 : _a.textContent) || '';
    ...
    try {
        ...
        const tier = ((_a = window._tokenPayload) === null || _a === void 0 ? void 0 : _a.tier) || ((_b = window._tokenPayload) === null || _b === void 0 ? void 0 : _b.product) || 'locked'; // simplebeacon-ignore
        ...
        if (browserFolderDropzone)
            browserFolderDropzone.classList.add('scanning');
        const localScanFileName = document.getElementById('localScanFileName');
        if (localScanFileName)
        ...
        const includeDeps = ((_c = document.getElementById('includeDepsToggle')) === null || _c === void 0 ? void 0 : _c.checked) || false;
        const deepScan = ((_d = document.getElementById('deepScanToggle')) === null || _d === void 0 ? void 0 : _d.checked) || false;
        ...
        for (const f of files) {
            ...
            const includeVendorDirs = ((_e = window._scanOptions) === null || _e === void 0 ? void 0 : _e.includeVendorDirs) || false;
        ...
        if (inventory.buildArtifacts > 0)
            appendTerminalLine(`  <span style="color:#64748B;">&#10148;</span> Build artifacts: <strong>${inventory.buildArtifacts.toLocaleString()}</strong>`);
        if (inventory.testFixtures > 0)
            appendTerminalLine(`  <span style="color:#64748B;">&#10148;</span> Test fixtures: <strong>${inventory.testFixtures.toLocaleString()}</strong>`);
        if (inventory.tempDev > 0)
            appendTerminalLine(`  <span style="color:#64748B;">&#10148;</span> Temp/dev files: <strong>${inventory.tempDev.toLocaleString()}</strong>`);
        if (inventory.other > 0)
            appendTerminalLine(`  <span style="color:#64748B;">&#10148;</span> Other: <strong>${inventory.other.toLocaleString()}</strong>`);
        if (localScanFileName)
        ...
        for (let i = 0; i < sourceFiles.length; i++) {
            ...
            if (!workerScanActive) {
                for (const analyzerId of activeAnalyzers) {
                    ...
                    if (isKnownVendor && reg.id !== 'governance')
                        continue; // simplebeacon-ignore: strict !== comparison
                    ...
                    try {
                        ...
                        if ((reg.id === 'evalDanger' || reg.id === 'innerHtmlXss' || reg.id === 'prototypePollution' || reg.id === 'unvalidatedRedirect' || reg.id === 'missingRateLimit' || reg.id === 'insecureRandom' || reg.id === 'loggingSecrets') && (/scripts\/|ai-tools\/|ai-agent\/|simplebeacon-frameworkless\/|bulk-fix-|export-findings|audit-token-bleed|complexity-gate|add-sample-json-|run-cli-scan|test-all-patterns|token-file-system\.js$|usb-token-manager\.js$/.test(path)))
                            continue;
                        if (reg.id === 'prototypePollution' && (/enhancedAIProvider\.ts$|compliance-checklist\.js$|workspaceAnalyzer\.ts$|remediationProvider\.ts$|architecture-drift-patterns\.js$/.test(path) || /Object\.prototype\.hasOwnProperty\.call.*is the SAFE pattern/i.test(text)))
                            continue;
                        if (reg.id === 'evalDanger' && (/architecture-drift-patterns\.js$|AboutView\.js$/.test(path)))
                            continue;
                        if (reg.id === 'insecureRandom' && (/fixRegistry\.ts$/.test(path)))
                            continue;
                        if (reg.id === 'unhandledPromise' && /scripts\/debug-.*\.js$/.test(path))
                            continue;
                        if (reg.id === 'missingStrictMode' && (/\.eslintrc\.js$|jest\.config\.js$|build-extension\.js$/.test(path)))
                            continue;
                        // pathPattern: only run on files whose path matches
                        if (reg.pathPattern && !reg.pathPattern.test(path))
                            continue;
                        // skipPathPattern: skip files whose path matches
                        if (reg.skipPathPattern && reg.skipPathPattern.test(path))
                            continue;
                        let matches;
                        try {
            ...
            if ((allowedSections.includes('consolidation') || allowedSections.includes('fileReduction')) && text.length < 200000 && !/node_modules|\.git|dist|build|coverage|java-ai-vulnerable|backups|\.husky|\.simplebeacon\/|\.github-sync\/|github-cache\/|\.cursor\/|\.windsurf\/|_restore-|fix-.*\.py$|patch-main|New folder\/|simplebeacon-export-operator/i.test(lowerPath)) {
                ...
                if (!isCommonLegitDuplicate && !isEmptyFile && !isBinaryArchive && !isIgnored) {
                    ...
                    if (duplicateHashes.has(hash)) {
                        ...
                        if (duplicateHashes.get(hash).length === 2)
                            duplicateGroups++;
                    }
                    else {
            ...
            if (allowedSections.includes('roadmap')) {
                ...
                const syntheticMatches = (text.match(/\[Auto-Generated Issue\]|\bAUTO_FIX:\s*ai_residue\b/gi) || []).length;
                if (syntheticMatches > 0)
                    fileMarkers.synthetic = syntheticMatches;
                const hasRealTodo = Object.keys(fileMarkers).filter(k => k !== 'synthetic').some(k => fileMarkers[k] > 0);
                if (!isCacheOrBinary && !isScanReport && !isTestFile && hasSourceExt && hasRealTodo) {
                    todoFiles.push({ path, markers: fileMarkers });
                    Object.keys(fileMarkers).forEach(k => { if (k !== 'synthetic')
            ...
            if (allowedSections.includes('junkFiles')) {
                const baseName = path.split('/').pop();
                const isJunk = (/\.(tmp|temp|bak|backup|swp|swo|swn|old|orig|rej|pid|seed|crdownload)$/i.test(baseName) ||
                    baseName.endsWith('~') ||
            ...
            if ((i + 1) % 250 === 0 || i === sourceFiles.length - 1) {
                ...
                if (profile.checkCredentials && credentialHits > 0)
                    findingParts.push(`Creds: ${credentialHits}`);
                if (profile.checkAi && aiHits.length > 0)
                    findingParts.push(`AI: ${aiHits.length}`);
                if (profile.checkDebug && debugHits.length > 0)
                    findingParts.push(`Debug: ${debugHits.length}`);
                if (profile.checkAiResidue && aiResidueHits > 0)
        ...
        for (const file of sourceFiles) {
            ...
            if (ignorePatterns.some(rx => rx.test(path)))
                continue;
                ...
                for (const sf of sourceTextFiles) {
                    try {
                        ...
                        for (const img of imageFiles) {
                            if (txt.includes(img.name))
                                referencedNames.add(img.name);
                        }
                    }
        ...
        if (processEnvRefs.length > 0 && envFileData.length > 0) {
            ...
            processEnvRefs.forEach(ref => {
                if (!allDefinedKeys.has(ref.key)) {
                    if (!Object.hasOwn(missingMap, ref.key))
                        missingMap[ref.key] = [];
        ...
        reportData = {
            ...
            ...(allowedSections.includes('roadmap') ? {
                roadmap: {
                    ...
                    syntheticTodoMarkers: (typeof syntheticTodoMarkerCounts !== 'undefined') ? syntheticTodoMarkerCounts.total : 0,
                    syntheticTodoFiles: (typeof syntheticTodoFiles !== 'undefined') ? syntheticTodoFiles.map(f => f.path || f).slice(0, 20) : [],
            ...
            ...(allowedSections.includes('buildReadiness') ? (() => {
                ...
                if (dominantLang === 'javascript' && !hasPackageJsonAnywhere) {
                    brChecks.push({ name: 'package.json', found: false, critical: true }, { name: 'Lockfile', found: hasLockfileAnywhere, critical: true });
                }
            ...
            scanProfile: (browserScanProfile === null || browserScanProfile === void 0 ? void 0 : browserScanProfile.value) || 'gate',
        ...
        if (aiHits.length)
            statusParts.push(`${aiHits.length} AI indicators`);
        if (debugHits.length)
            statusParts.push(`${debugHits.length} debug artifacts`);
        if (llmSlopHits)
            statusParts.push(`${llmSlopHits} LLM slop`);
        if (tokenBleedHits)
            statusParts.push(`${tokenBleedHits} token bleed`);
        if (productionLeakHits)
            statusParts.push(`${productionLeakHits} production leaks`);
        if (fictionKpiHits)
