...
const LANGUAGE_REGISTRY = {
    javascript: {
        ...
        buildChecks: [
            ...
            {
                name: 'CI/CD',
                regex: /\.github\/workflows|\.gitlab-ci|jenkins|\.circleci|\.travis|azure-pipelines|build\.yml|deploy\.yml/i,
                critical: true
            },
            ...
            { name: '.gitignore', regex: /\.gitignore/, critical: true },
            {
                ...
                regex: null,
                customCheck: paths =>
                    !paths.some(p => /\/(dist|build|\.next|out)\//.test(p) && !/node_modules\//.test(p)),
                critical: true
            },
...
const PATTERN_REGISTRY = {
    aiIndicators: {
        ...
        maxMatches: 5,
        selfReferenceFilter:
            /(?:const|let|var)\s+(?:aiPattern|AI_INDICATORS_PATTERNS)\b|{\s*id:\s*['"][^'"]+['"],\s*pattern:\s*\//,
        contextFilter: (snippet, filePath) => {
            // Skip dashboard UI labels/comments that merely mention AI providers
            if (
                /(label|placeholder|title|tooltip|hint|description|heading)\s*[:=]\s*['"].*\b(openai|anthropic|langchain|huggingface|gpt|claude)/i.test(
                    snippet
                )
            )
                return false;
            ...
            if (
                filePath &&
                /\/(simplebeacon-dashboard\/js-es2018\/views|coming-soon\/(js|js-es2018|public\/js-es2018)\/dashboard|coming-soon\/lib\/certificate-utils)\//.test(
                    filePath
                )
            )
                return false;
    ...
    credentials: {
        ...
        pattern: new RegExp(
            'password\\s*=\\s*[\'"][^\'"]{6,}|' +
                'api[_-]?key\\s*=\\s*[\'"][a-z0-9]{12,}|' +
                'secret[_-]?key\\s*=\\s*[\'"][a-z0-9]{12,}|' +
                'private_key\\s*=\\s*[\'"][^\'"]{8,}|' +
                'aws_access_key_id\\s*=\\s*[\'"][A-Z0-9]{16,}|' +
                'API_KEY\\s*=\\s*[\'"][A-Za-z0-9_\\-]{16,}|' +
                'SECRET\\s*=\\s*[\'"][A-Za-z0-9_\\-]{12,}|' +
                'AUTH_TOKEN\\s*=\\s*[\'"][A-Za-z0-9_\\-]{12,}',
            'i'
        ),
        maxMatches: 3,
        redact: true,
        selfReferenceFilter:
            /PATTERN_REGISTRY|TEST_CASES|shouldMatch|shouldNotMatch|test.*password|test.*api_key|mockFileContent|REDACTED|trello-board|test-all-patterns/i,
        contextFilter: (snippet, filePath) => {
    ...
    debugArtifacts: {
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (filePath && /\.(py)$/.test(filePath) && /\bprint\s*\(/.test(snippet)) {
                if (
                    /scons|build|test|packageUtils/.test(filePath) ||
                    /BUILD ERROR|Test is now looking|Basic checks for/.test(snippet)
                )
                    return false;
            }
            ...
            if (
                /console\.(warn|error|log)\s*\(\s*['"`]\[[^\]]+\]\s+\w+\s+.*not\s+loaded|setup\s+skipped/i.test(snippet)
            )
                return false;
            return true;
        },
    ...
    governance: {
        ...
        severity: 'low',
        pattern:
            /SPDX-License-Identifier|MIT License|Apache License|GNU General Public License|All rights reserved|Copyright \(c\)|License: /i,
        maxMatches: 1,
    ...
    javaDebug: {
        ...
        maxMatches: 3,
        message:
            'Java debug artifact (System.out.print, e.printStackTrace) detected. Use logging framework in production.'
    },
    pythonFramework: {
        id: 'pythonFramework',
    ...
    phpFramework: {
        ...
        severity: 'medium',
        pattern:
            /APP_DEBUG\s*=>\s*true|APP_ENV\s*=>\s*['"]local['"]|DB::raw\s*\(|mysql_query\s*\(|mysqli_query\s*\(|PDO\s*::\s*query\s*\(/i,
        maxMatches: 3,
    ...
    dotnetDebug: {
        ...
        maxMatches: 3,
        message:
            '.NET debug artifact (Console.WriteLine, Debug.WriteLine, Debugger.Break) detected. Remove before production builds.'
    },
    dotnetFramework: {
        id: 'dotnetFramework',
        ...
        redact: true,
        message:
            '.NET framework issue: hardcoded connection string or in-memory database detected. Use secrets management.'
    },
    rubyDebug: {
        id: 'rubyDebug',
    ...
    rubyFramework: {
        ...
        maxMatches: 3,
        message:
            'Ruby framework anti-pattern: unsafe mass assignment, eval, or dynamic send detected. Review for security risks.'
    },
    aiResidueStub: {
        id: 'aiResidueStub',
        ...
        severity: 'low',
        pattern:
            /function\s+\w+\s*\([^)]*\)\s*\{\s*\/\/\s*TODO|def\s+\w+\s*\([^)]*\):\s*pass\b|public\s+\w+\s+\w+\s*\([^)]*\)\s*\{\s*\/\/\s*implement|TODO\s*:\s*implement|TODO\s*:\s*AI|TODO\s*:\s*generated|\/\/\s*AI\s+generated|\/\/\s*Generated\s+by\s+(ChatGPT|GPT|Claude[\w.-]*|Copilot|Gemini|LLM)/i,
        maxMatches: 3,
    ...
    aiResidueSwallow: {
        ...
        severity: 'medium',
        pattern:
            /catch\s*\([^)]*\)\s*\{\s*\}|catch\s*\([^)]*\)\s*\{\s*\/\/\s*ignore|catch\s*\([^)]*\)\s*\{\s*console\.(log|warn|error)\s*\(|except\s*[^:]+:\s*pass|catch\s*\([^)]*\)\s*\{\s*return\s*null|catch\s*\([^)]*\)\s*\{\s*return\s*;|rescue\s+=>\s*e\s*#\s*ignore/i,
        maxMatches: 3,
    ...
    aiResidueDeprecated: {
        ...
        severity: 'low',
        pattern:
            /jQuery\s*\(|\.ajax\s*\(|componentWillMount\s*\(|componentWillReceiveProps\s*\(|shouldComponentUpdate\s*\(|React\.createClass\s*\(|\.getInitialState\s*\(|asyncio\.coroutine|@asyncio\.coroutine|urllib\.urlopen\s*\(|urllib2\.urlopen\s*\(|xrange\s*\(|raw_input\s*\(/i,
        maxMatches: 3,
    ...
    aiResidueDeadCode: {
        ...
        maxMatches: 2,
        message:
            'Large commented-out code block detected. AI assistants often leave dead code behind. Remove or restore.'
    },
    perfNestedLoop: {
        id: 'perfNestedLoop',
    ...
    typeSafetyAny: {
        ...
        maxMatches: 3,
        selfReferenceFilter:
            /pattern\s*:\s*\/[^/]+\/|severity.*vscode\.DiagnosticSeverity|fixSuggestion.*Replace any with specific types|scanner-patterns|extension\.ts|out\/extension\.js|postMessage\s*\(/i,
        contextFilter: (snippet, filePath) => {
            ...
            if (/function\s+postJson\(.*payload:\s*any|private\s+async\s+_uploadReport\(.*data:\s*any/.test(snippet))
                return false;
    ...
    a11yGap: {
        ...
        severity: 'medium',
        pattern:
            /<img\b(?![^>]*\balt=)[^>]*>|<(input|textarea|select)\b(?![^>]*\b(?:aria-label|aria-labelledby|title|placeholder|hidden)=)[^>]*>|<button\b(?![^>]*\b(?:aria-label|aria-labelledby|title)=)[^>]*>\s*<\/button>|<div\b[^>]*\brole=["']button["'](?![^>]*\b(?:aria-label|aria-labelledby|tabindex))[^>]*>/i,
        maxMatches: 3,
    ...
    sensitiveData: {
        ...
        severity: 'high',
        pattern:
            /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z]{2,}\b|\b\d{3}-\d{2}-\d{4}\b|\b\d{3}-\d{3}-\d{4}\b|console\.(log|warn|error|info)\s*\(\s*(?:user|customer|email|password|token|ssn|phone)|localStorage\.setItem\s*\(\s*['"](?:token|auth|session|password)/i,
        maxMatches: 3,
        redact: true,
        selfReferenceFilter:
            /\b(?:dev@simplebeacon\.ai|demo@simplebeacon\.ai|test@example\.com|alice@example\.com|trevor_punt@live\.com|audit@simplebeacon\.ai|@live\.com|noreply@|no-reply@|example\.com|placeholder.*@|you@company\.com|user@example\.com|localhost|127\.0\.0\.1|0\.0\.0\.0|::1|generate.*token|send.*email|email.*template|outreach|prospect|marketing|invoice|billing|payment|tier|sandbox|demo|test|fixture|mock|sample|dummy|fake|stub)\b/i,
        contextFilter: (snippet, filePath) => {
            ...
            if (
                filePath &&
                /\/(jquery|modernizr|underscore|bootstrap|lodash|moment|react|vue|angular|protobuf|ossp|sphinxtogithub)\b|\.min\.js$|\.pack\.js$|(^|\/)(docs\/|doc\/|third_party\/|thirdparty\/|geedocs\/|vendor\/)\//i.test(
                    filePath
                )
            )
                return false;
    ...
    configDrift: {
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /(explanations|getSteps|fix:|message:).*(hardcoded|configuration values|deployments fragile)/i.test(
                    snippet
                )
            )
                return false;
            ...
            if (
                /isCliFalsePositive|isBuildArtifact/.test(snippet) &&
                /type\s*===|return true|return false/.test(snippet)
            )
                return false;
            return true;
        },
    ...
    securityHeaders: {
        ...
        severity: 'medium',
        pattern:
            /Content-Security-Policy|X-Frame-Options|Strict-Transport-Security|Referrer-Policy| helmet\(\)|helmet\s*\.\w+/i,
        maxMatches: 2,
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /(missing security header|security header|content-security-policy)/i.test(snippet) &&
                /return true|return false|\.test\(|\.match\(|contextFilter/i.test(snippet)
            )
                return false;
            ...
            if (
                /headers\.(get|set|append|has|delete)\s*\(\s*['"](Content-Security-Policy|X-Frame-Options|Strict-Transport-Security|Referrer-Policy)['"]/i.test(
                    snippet
                )
            )
                return false;
    ...
    dbAntiPattern: {
        ...
        severity: 'high',
        pattern:
            /SELECT\s+.*['"]\s*\+\s*['"]|query\s*\(\s*['"].*\+\s*['"]|raw\s*\(\s*['"].*\$\{|\.findAll\s*\(\s*\)(?!.*limit)|execute\s*\(\s*['"].*\+\s*['"]/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
            // Skip jQuery DOM element creation misidentified as SQL concatenation
            if (/jQuery\s*\(\s*['"]<[^'"]+>['"]\s*\)|\.appendTo\s*\(\s*document\.body\s*\)|nodeName/.test(snippet))
                return false;
            return true;
        },
    ...
    innerHtmlXss: {
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /webviewPanel|web2Panel|dashboard2_0|enhancedDashboard|browserPreview|codeMapProvider|scanPanel|uploadPanel/i.test(
                    filePath
                )
            )
                return false;
            ...
            if (
                filePath &&
                /\/views\/(Audit|Quality|Security|Trust|Profile|Chatbot|SignIn)View\.js$/i.test(filePath) &&
                /container\.innerHTML|recoveryForm\.innerHTML/.test(snippet)
            )
                return false;
            return true;
        },
    ...
    evalDanger: {
        ...
        severity: 'high',
        pattern:
            /\beval\s*\(|\bnew\s+Function\s*\(|\bsetTimeout\s*\(\s*['"`]|\bsetInterval\s*\(\s*['"`]|\bsystem\s*\(/i,
        maxMatches: 3,
        selfReferenceFilter:
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                filePath &&
                /\.(cpp|c|h|hpp|cc)$/.test(filePath) &&
                /cmdline\.System|CmdLine::System|\.System\s*\(/.test(snippet)
            )
                return false;
            ...
            if (
                filePath &&
                /\.(py)$/.test(filePath) &&
                /os\.system\s*\(/.test(snippet) &&
                /build|version|update|test/.test(filePath)
            )
                return false;
        ...
        message:
            'eval(), new Function(), or dynamic code execution — code injection risk. Use structured parsing instead.'
    },
    committedEnvFile: {
        id: 'committedEnvFile',
    ...
    redosRisk: {
        ...
        message:
            'Regular expression with nested quantifiers — potential ReDoS. Refactor to avoid catastrophic backtracking.'
    },
    cicdSecretExposure: {
    ...
    frameworkPractice: {
        ...
        severity: 'low',
        pattern:
            /useEffect\s*\(\s*\(\)\s*=>\s*\{[\s\S]{0,60}\}\s*\)(?!.*,\s*\[)|useState\s*\(\s*\)\s*;(?=.*useState\s*\(\s*\))|this\.\$refs\.|ngOnInit\s*\(\s*\)[\s\S]{0,200}(?!.*ngOnDestroy)/i,
        maxMatches: 3,
    ...
    llmSlop: {
        ...
        maxMatches: 5,
        selfReferenceFilter:
            /llm-slop-patterns|fiction-kpi|rejectedFiction|not model output|baseline false|scanner-patterns/i,
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /readme|docs?|guide|tutorial|example/i.test(filePath) &&
                !/I have (written|implemented|created|updated)|as requested|Let me know if you need|AI Assistant Note/i.test(
                    snippet
                )
            )
                return false;
            ...
            if (
                /\/\/\s*TODO:?|\/\/\s*FIXME:?|#\s*TODO:?/i.test(snippet) &&
                !/AI\s+Generated|Placeholder/i.test(snippet)
            )
                return false;
            // Skip placeholder/test API keys
            if (
                /process\.env\.(RESEND_API_KEY|JWT_SECRET|SECRET|TOKEN)\s*=\s*['"][^'"]*(test|placeholder|example)/i.test(
                    snippet
                )
            )
                return false;
        ...
        message:
            'Unresolved LLM placeholder, markdown code fence leaked into source, or hardcoded AI-default metric copy detected.'
    },
    tokenBleed: {
        id: 'tokenBleed',
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                filePath &&
                /\/(mapfiles|poly|geometry|stats|usage|visualization|earthbuilder_impl)\.js$|\.pack\.js$|\.min\.js$/i.test(
                    filePath
                )
            )
                return false;
    ...
    productionLeak: {
        ...
        severity: 'medium',
        pattern:
            /['"`][^'"`]*(?:\/|\\)mock(?:\/|\\)[^'"`]+['"`]|['"`][^'"`]*(?:\/|\\)fixtures(?:\/|\\)[^'"`]+['"`]|['"`][^'"`]*(?:\/|\\)sample(?:\/|\\)[^'"`]+['"`]|['"`][^'"`]*web(?:\/|\\)data[^'"`]*['"`]|['"`][^'"`]*-sample\.json['"`]|['"`][^'"`]*(?:\/|\\)test-data(?:\/|\\)[^'"`]+['"`]/i,
        maxMatches: 3,
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /\b(?:const|let|var)\s+(?:sampleFiles|sampleUrl|sampleData|recentSamples|sampleCount|mockData|mockUrl)\b/.test(
                    snippet
                )
            )
                return false;
        ...
        message:
            'Mock, fixture, or sample data path referenced in source code. Verify no test data leaks to production.'
    },
    fictionKpi: {
        id: 'fictionKpi',
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (filePath && /uploadPanel\.ts$/i.test(filePath) && /return\s*\{\s*type:\s*['"]/.test(snippet))
                return false;
    ...
    syncIo: {
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (/extension\.ts.*readFileSync.*package\.json|extension\.ts.*readFileSync.*report\.json/.test(snippet))
                return false;
            return true;
        },
        message:
            'Synchronous fs read/write blocks the event loop. Use fs.promises instead. (fs.existsSync is excluded — it is a lightweight stat check.)'
    },
    magicNumber: {
        id: 'magicNumber',
        name: 'Magic Number',
        ...
        pattern:
            /(?<!\b(?:const|let|var)\s+\w+\s*=\s*)[<>!=]\s*\b\d{3,}\b|\b(?:timeout|delay|port|max|min|limit|size|count)\s*[:=]\s*\d{2,}\b/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /\b(?:slice|substr|substring|limit|truncate|maxLength|padStart|padEnd)\s*\(\s*[^)]*\d{3,}/.test(snippet)
            )
                return false;
    ...
    unvalidatedRedirect: {
        ...
        severity: 'high',
        pattern:
            /window\.location\s*=\s*[^'"]|window\.location\.href\s*=\s*[^'"]|window\.location\.replace\s*\(\s*[^'"]|res\.redirect\s*\(\s*[^'"]|res\.redirect\s*\(\s*req\.|location\.href\s*=\s*req\./i,
        maxMatches: 3,
    ...
    missingRateLimit: {
        ...
        severity: 'medium',
        pattern:
            /app\.(get|post|put|delete|patch)\s*\(\s*['"`][^'"`]+['"`]\s*,\s*(?!\s*(?:rateLimit|throttle|\w*[Ll]imiter|\w*[Rr]ate[Ll]imit))/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
            if (/rateLimit|throttle|[Ll]imiter|express-rate-limit|express-slow-down/.test(snippet)) return false;
    ...
    architectureDrift: {
        ...
        severity: 'medium',
        pattern:
            /\b(hybrid|state-space|long-context)\b(?!.*\b(schema|validator|zod|ajv|json_schema|response_format)\b)/i,
        maxMatches: 3,
        selfReferenceFilter:
            /scanner-patterns|scanner-engine|simplebeacon-intelligence|architecture-drift|@simplebeacon\/intelligence/,
        contextFilter: (snippet, filePath) => {
    ...
    roadmapMarker: {
        ...
        contextFilter: (snippet, filePath) => {
            ...
            if (
                /(?:^|\/)(scanner-patterns|scanner-engine|production-debug-guard|scan-github-repo|repository-scanner-api|index\.cjs)/.test(
                    p
                )
            )
                return false;
    ...
    llmApiEndpoint: {
        ...
        severity: 'low',
        pattern:
            /api\.openai\.com|api\.anthropic\.com|generativelanguage\.googleapis\.com|api\.groq\.com|api\.together\.xyz|api\.replicate\.com|proxy\.llm|ollama\.run|localhost:\d+\/v1\/chat\/completions|azure\.openai|\.openai\.azure\.com/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
        ...
        message:
            'Hardcoded LLM API endpoint detected. Consider making configurable for vendor flexibility and EU data residency.'
    },
    systemPromptFile: {
        id: 'systemPromptFile',
        ...
        severity: 'medium',
        pathPattern:
            /(^|[\\/])(system[-_]?prompt|prompt[-_]?system|ai[-_]?prompt|llm[-_]?prompt)(\.txt|\.md|\.json|\.yaml|\.yml|\.js|\.py|\.ts)?$/i,
        maxMatches: 2,
        contextFilter: (snippet, filePath) => {
        ...
        message:
            'System prompt file committed to repository. Prompts may contain business logic or injection surface — consider runtime configuration.'
    },
    insecureModelParam: {
        ...
        severity: 'medium',
        pattern:
            /temperature\s*:\s*2(?!\d)|max_tokens\s*:\s*1[5-9]\d{3}|max_tokens\s*:\s*\d{5,}|top_p\s*:\s*1\.0\b(?![,}\s]*\w)|presence_penalty\s*:\s*2\.0|frequency_penalty\s*:\s*2\.0/i,
        maxMatches: 3,
        message:
            'Insecure LLM parameter detected — high temperature, excessive max_tokens, or maxed penalties may cause quality/cost issues.'
    },
    webhookUrl: {
        id: 'webhookUrl',
        ...
        severity: 'high',
        pattern:
            /hooks\.slack\.com\/services\/[^\s'"`]+|discord\.com\/api\/webhooks\/[^\s'"`]+|hooks\.teams\.microsoft\.com\/[^\s'"`]+|webhook\.url|webhook_url\s*[:=]\s*['"`][^'"`]{20,}/i,
        maxMatches: 3,
        redact: true,
        contextFilter: (snippet, filePath) => {
    ...
    dbConnectionString: {
        ...
        message:
            'Database connection string with embedded password detected. Use environment variables or a secret manager.'
    },
    corsWildcard: {
        id: 'corsWildcard',
        ...
        severity: 'high',
        pattern:
            /Access-Control-Allow-Origin\s*:\s*\*|cors\s*\(\s*\{[^}]*origin\s*:\s*\*|app\.use\s*\(\s*cors\s*\(\s*\)\s*\)/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
    ...
    graphqlIntrospection: {
        ...
        severity: 'medium',
        pattern:
            /introspection\s*:\s*true|__schema|__type\s*\(|apollo-server.*introspection|graphql\s*\(\s*\{\s*__schema/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
        ...
        message:
            'GraphQL introspection enabled in production — schema exposure aids attackers. Disable or gate behind auth.'
    },
    dockerfileAntiPattern: {
        ...
        pattern:
            /FROM\s+\w+\s*:\s*latest\b|USER\s+root\b|RUN\s+.*curl\s+.*\|\s*bash|RUN\s+.*wget\s+.*\|\s*sh|ADD\s+https?:\/\/|ARG\s+.*SECRET|ARG\s+.*PASSWORD|ARG\s+.*TOKEN/i,
        maxMatches: 3,
        message:
            'Dockerfile anti-pattern: latest tag, root user, pipe-to-shell, or build-arg secrets. Pin versions and use non-root user.'
    },
    k8sMissingLimits: {
        id: 'k8sMissingLimits',
        ...
        pattern:
            /containers\s*:\s*\[[\s\S]{0,400}?\](?!.*resources)(?!.*limits)(?!.*requests)|kind\s*:\s*Deployment[\s\S]{0,600}?containers\s*:[\s\S]{0,400}(?!resources)(?!limits)(?!requests)/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
    ...
    dependencyGitUrl: {
        ...
        pattern:
            /"[^"]+"\s*:\s*"git\+(?:ssh|https)?:\/\/[^"]+"|"[^"]+"\s*:\s*"(?:https?:\/\/github\.com\/[^"]+#[^"]+|github:[^"]+)"/i,
        maxMatches: 5,
        contextFilter: (snippet, filePath) => {
            if (/\.env\.example|config\.example/.test(filePath)) return false;
            return true;
        ...
        message:
            'Git URL dependency in package.json — supply chain risk (unpublished, unvetted, no audit trail). Prefer npm registry versions.'
    },
    jwtSecretInConfig: {
        ...
        severity: 'critical',
        pattern:
            /jwt[_-]?secret\s*[:=]\s*['"`][^'"`]{8,}|JWT_SECRET\s*[:=]\s*['"`][^'"`]{8,}|secret[_-]?key\s*[:=]\s*['"`][^'"`]{8,}.*algorithm|signing[_-]?key\s*[:=]\s*['"`][^'"`]{8,}/i,
        maxMatches: 3,
        redact: true,
        contextFilter: (snippet, filePath) => {
            if (/\.env\.example|config\.example|\.env\.template/.test(filePath)) return false;
    ...
    insecureCookie: {
        ...
        severity: 'high',
        pattern:
            /res\.cookie\s*\([^)]*\)(?!.*httpOnly)(?!.*secure)(?!.*sameSite)|Set-Cookie\s*:[^;]*;(?!.*[Hh]ttpOnly)(?!.*[Ss]ecure)(?!.*[Ss]ameSite)/i,
        maxMatches: 3,
    ...
    pathTraversal: {
        ...
        severity: 'high',
        pattern:
            /fs\.readFile(?:Sync)?\s*\(\s*(?:req\.(?:query|body|params)|process\.argv|args)\b|fs\.readFile(?:Sync)?\s*\(\s*path\.join\s*\([^)]*\b(?:req\.|body\.|query\.|params\.|argv)/i,
        maxMatches: 3,
        message:
            'User input passed directly to file system read without path sanitization. Validate and whitelist paths.'
    },
    ssrfVector: {
        id: 'ssrfVector',
        name: 'SSRF Vector',
        ...
        severity: 'high',
        pattern:
            /fetch\s*\(\s*(?:req\.(?:query|body)|url|target)\b|axios\.(?:get|post|put|delete)\s*\(\s*(?:req\.(?:query|body)|url|target)\b|request\s*\(\s*(?:req\.(?:query|body)|url|target)\b|curl\s+['"`][^'"`]*\$\{|curl\s+['"`][^'"`]*\+ /i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
        ...
        message:
            'User-controlled URL passed to HTTP client without validation — SSRF risk. Validate against an allowlist.'
    },
    nosqlInjection: {
        id: 'nosqlInjection',
        ...
        severity: 'critical',
        pattern:
            /\$where\s*:\s*(?:req\.|body\.|query\.|params\.)|\.aggregate\s*\(\s*\[\s*\{[^}]*(?:req\.|body\.|query\.|params\.)|\.find\s*\(\s*\{[^}]*(?:req\.|body\.|query\.|params\.)[^}]*\$ne|\$eq[^}]*(?:req\.|body\.|query\.|params\.)/i,
        maxMatches: 3,
    ...
    noHelmet: {
        ...
        message:
            'Express app routes found but no helmet() or security middleware imported. Add helmet() for security headers.'
    },
    modelFallback: {
        id: 'modelFallback',
        ...
        severity: 'medium',
        pattern:
            /gpt-4.*gpt-3\.5-turbo|claude-3.*claude-2|model\s*:\s*['"`][^'"`]*['"`]\s*,\s*(?!.*log)(?!.*warn)(?!.*notify)(?!.*alert).*model\s*:\s*['"`][^'"`]*['"`]/i,
        maxMatches: 3,
    ...
    promptInjectionSurface: {
        ...
        severity: 'high',
        pattern:
            /(system|user|assistant)\s*[:=]\s*['"`][^'"`]*\$\{[^}]*(?:req\.|body\.|query\.|params\.|user|input|message)\b[^}]*\}|['"`][^'"`]*\$\{[^}]*(?:req\.|body\.|query\.|params\.|user|input|message)\b[^}]*\}['"`].*\.(?:chat\.completions|messages\.create|generate|complete|predict|embed)/i,
        maxMatches: 3,
        message:
            'User input interpolated directly into LLM prompt. Sanitize input or use structured message arrays to prevent prompt injection.'
    },
    mixedIndentation: {
        id: 'mixedIndentation',
...
const EXCLUSION_RULES = {
    aiIndicators: ctx =>
        ...
        ctx.isTestOutput ||
        ctx.isTestFile ||
        ctx.isSimplebeaconCache ||
        ctx.isScannerArtifact ||
        ctx.isBuildArtifact,
