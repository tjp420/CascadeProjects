    ...
    function isPaidUser() {
        try {
            if (typeof global.SbAuth !== 'undefined') {
                ...
                if (typeof global.SbAuth.isAccountSignedIn === 'function' && global.SbAuth.isAccountSignedIn()) return true;
            }
        } catch (_) { /* ignore */ }
        return false;
    }
    ...
    function wireHandoffModal() {
        ...
        if (copyBtn) {
            copyBtn.addEventListener('click', function () {
                var report = global.currentReport || global.lastScanReport || global.reportData;
                if (!report) { setStatus('No report loaded', false); return; }
    ...
    function wireSendToAiButtons() {
        ...
        if (analyzeSendBtn) {
            analyzeSendBtn.addEventListener('click', function () {
                ...
                if (vscode) {
                    var report = global.currentReport || global.lastScanReport;
                    if (!report) { alert('No scan report to send'); return; }
        ...
        if (aiConfirm) {
            aiConfirm.addEventListener('click', function () {
                var report = global.currentReport || global.lastScanReport;
                if (!report) {
                    if (aiStatus) { aiStatus.textContent = 'No report loaded'; aiStatus.style.color = 'var(--error)'; }
                    return;
                }
