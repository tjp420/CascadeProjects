...
(function () {
    ...
    function isVaultTokenUsed(token) {
        try {
            ...
            if (!Array.isArray(vault))
                return false;
            const entry = vault.find(function (v) { return v && v.token === token; });
    ...
    function applyTokenToInput(input, token, options) {
        if (!input)
            return { ok: false, error: 'Token field not found.' };
    ...
    function bindLicenseTokenInput(input, errorEl) {
        if (!input || input.dataset.newTokenGuard === '1')
            return;
