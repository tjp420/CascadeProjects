...
function renderDashboard(report, roadmap) {
    ...
    const cards = [
            ...
            : null,
        {
            ...
            value: roi.developerHoursSaved + 'h',
            pct: Math.min(100, Math.round(roi.developerHoursSaved * 6)),
            cls: 'score-good'
        },
        {
            label: 'Audit Friction Reduction',
            value: roi.auditFrictionReduction + '%',
            pct: roi.auditFrictionReduction,
            cls: roi.auditFrictionReduction >= 75 ? 'score-good' : 'score-warn'
        },
        {
            label: 'EU AI Act Risk Reduction',
            value: roi.euAiActRiskReduction + '%',
            pct: roi.euAiActRiskReduction,
            cls: roi.euAiActRiskReduction >= 75 ? 'score-good' : 'score-warn'
        },
        issues != null
            ? {
                  label: 'Total Issues',
...
function exportRoadmapJson() {
    ...
    const exportIssues = (currentReport.rawIssues || currentReport.issues || currentReport.detectedIssues || []).map(
        issue => ({ sev: issue.severity || issue.severityBand || 'low' })
    );
    ...
    currentRoadmap.phases.forEach(p => {
        p.tasks.forEach((t, idx) => {
            ...
            rows.push([
                ...
                String(done),
                loc,
                String(roi.developerHoursSaved),
                String(roi.auditFrictionReduction),
                String(roi.euAiActRiskReduction),
                roi.label
            ]);
        });
    });
