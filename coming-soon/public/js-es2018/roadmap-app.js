...
if (analyzeReportBtn && reportUrlInput) {
    ...
    reportUrlInput.addEventListener('keydown', e => { if (e.key === 'Enter')
...
function loadReport(report) {
    ...
    const extractedName = src.projectRoot || src.projectPath || src.projectName || src.scanTargetRoot || ((_b = report.metadata) === null || _b === void 0 ? void 0 : _b.project) || ((_c = report.metadata) === null || _c === void 0 ? void 0 : _c.projectName) || 'Unknown';
...
function loadTaskState(project, phaseId, taskIdx) { try {
    return localStorage.getItem(getTaskKey(project, phaseId, taskIdx)) === '1';
}
catch (e) {
    return false;
} }
function saveTaskState(project, phaseId, taskIdx, done) { try {
...
function loadTaskTime(project, phaseId, taskIdx) { try {
...
function saveTaskTime(project, phaseId, taskIdx, seconds) { try {
...
function fireConfetti(container) {
    ...
    for (let i = 0; i < 18; i++) {
        ...
        setTimeout(() => { if (piece.parentNode)
            piece.parentNode.removeChild(piece); }, 900);
    }
}
...
function buildPhaseCardHtml(phase, phaseIdx, projectKey, roadmap) {
    ...
    const depBlock = phase.dependsOn ? `<div class="dep-line"><span class="dep-arrow">↳</span> Depends on <strong>${escapeHtml(((_a = roadmap.phases.find(x => x.id === phase.dependsOn)) === null || _a === void 0 ? void 0 : _a.title) || phase.dependsOn)}</strong></div>` : '';
...
function generateRoadmap(report) {
    ...
    const dupes = src.duplicateGroups != null ? Number(src.duplicateGroups) : (((_c = src.consolidation) === null || _c === void 0 ? void 0 : _c.duplicateGroups) != null ? Number(src.consolidation.duplicateGroups) : null);
    const consistency = src.consistencyScore != null ? Number(src.consistencyScore) : (((_d = src.consolidation) === null || _d === void 0 ? void 0 : _d.duplicateGroups) != null ? (src.consolidation.duplicateGroups === 0 ? 100 : 50) : null);
    ...
    const credFindings = src.credentialFindings != null ? Number(src.credentialFindings) : (((_e = src.gate) === null || _e === void 0 ? void 0 : _e.blockingCount) != null ? Number(src.gate.blockingCount) : null);
    const leakFindings = src.productionLeakFindings != null ? Number(src.productionLeakFindings) : null;
    const euAiAct = src.euAiActFindings != null ? Number(src.euAiActFindings) : (((_f = src.euAiActSummary) === null || _f === void 0 ? void 0 : _f.aiSystemIndicators) != null ? Number(src.euAiActSummary.aiSystemIndicators) : null);
    const todoMarkers = src.todoMarkerCount != null ? Number(src.todoMarkerCount) : (((_g = src.roadmap) === null || _g === void 0 ? void 0 : _g.todoCount) != null ? Number(src.roadmap.todoCount) : null);
    ...
    // Phase 1: Security — always show
    {
        ...
        if (t.length === 0)
            t.push({ description: 'No security issues detected — credentials && secrets verified.', type: 'verify', done: secClean, isStructured: true });
        t.push({ description: 'Add .env to .gitignore', type: 'fix', codeSnippet: 'echo ".env" >> .gitignore', done: secClean, isStructured: true }, { description: 'Re-run gate scan', type: 'verify', codeSnippet: 'npx simplebeacon scan --gate', done: secClean, isStructured: true });
        ...
        const impactHtml = (credIssue === null || credIssue === void 0 ? void 0 : credIssue.impact) ? `<div class="phase-impact">Impact: ${escapeHtml(credIssue.impact)}</div>` : '';
    ...
    // Phase 2: Data Integrity — always show
    {
        ...
        t.push({ description: 'Validate all JSON', type: 'verify', codeSnippet: 'npx simplebeacon scan --json', done: allClean, isStructured: true }, { description: 'Re-run scan', type: 'verify', codeSnippet: 'npx simplebeacon scan', done: allClean, isStructured: true });
    ...
    // Phase 3: Consistency — always show
    {
        ...
        if (!autoComplete)
            t.push({ description: 'Standardize naming conventions', type: 'doc', done: autoComplete, isStructured: true }, { description: 'Document canonical file locations', type: 'doc', done: autoComplete, isStructured: true });
        if (autoComplete)
            t.push({ description: 'Verified — duplicates are structural/intentional', type: 'verify', done: autoComplete, isStructured: true });
    ...
    const bloatCount = ((_2 = report.cleanup) === null || _2 === void 0 ? void 0 : _2.bloatArtifactCount) || 0;
    {
        ...
        if (debugFindings.length > 0) {
            ...
            t.push({ description: `Remove ${dc} debug artifact${dc === 1 ? '' : 's'} (console.log, debugger, alert)`, type: 'fix', isStructured: true });
        }
        if (bloatCount > 0) {
            ...
            t.push({ description: 'Add bloat patterns to .simplebeaconignore', type: 'fix', codeSnippet: 'echo "*.simplebeacon-backup.*" >> .simplebeaconignore', isStructured: true }, { description: 'Purge old scan backups with retention policy', type: 'fix', codeSnippet: 'find .simplebeacon -name "*.simplebeacon-backup.*" -mtime +30 -delete', isStructured: true });
        }
        if (dc > 0)
            t.push({ description: 'Install eslint-plugin-no-console for CI', type: 'fix', codeSnippet: 'npm install eslint-plugin-no-console --save-dev', isStructured: true }, { description: 'Add pre-commit hook to reject console.log', type: 'fix', codeSnippet: 'npx husky add .husky/pre-commit "npx eslint --rule \'no-console: error\' src/"', isStructured: true });
        if (t.length === 0)
            t.push({ description: 'No debug artifacts or bloat detected — codebase is clean.', type: 'verify', done: true, isStructured: true });
        ...
        const impactHtml = (debugIssue === null || debugIssue === void 0 ? void 0 : debugIssue.impact) ? `<div class="phase-impact">Impact: ${escapeHtml(debugIssue.impact)}</div>` : '';
    ...
    const foundGovCount = (licenseCount || 0) + (securityCount || 0);
    {
        ...
        if (licenseCount > 0)
            t.push({ description: `Audit ${licenseCount} open-source license file(s)`, type: 'review', done: true, isStructured: true });
        else
            t.push({ description: 'Add LICENSE file to clarify distribution terms', type: 'fix', codeSnippet: 'touch LICENSE', done: hasGovDocs, isStructured: true });
        if (securityCount > 0)
            t.push({ description: `Review ${securityCount} security/governance file(s)`, type: 'review', done: true, isStructured: true });
        else
            t.push({ description: 'Add SECURITY.md to disclose vulnerability reporting', type: 'fix', codeSnippet: 'touch SECURITY.md', done: hasGovDocs, isStructured: true });
        const missingGov = (comp.missingGovernanceFiles || []).slice(0, 3);
        if (missingGov.length > 0)
            missingGov.forEach(f => t.push({ description: `Add ${f}`, type: 'fix', codeSnippet: `touch ${f}`, done: false, isStructured: true }));
        t.push({ description: 'Verify license compatibility with distribution model', type: 'verify', codeSnippet: 'npx license-checker --summary', done: hasGovDocs, isStructured: true }, { description: 'Document governance policies', type: 'doc', done: hasGovDocs, isStructured: true });
        if (t.length === 0)
            t.push({ description: 'Governance files verified — all standard policies present.', type: 'verify', done: true, isStructured: true });
        ...
        const impactHtml = (govIssue === null || govIssue === void 0 ? void 0 : govIssue.impact) ? `<div class="phase-impact">Impact: ${escapeHtml(govIssue.impact)}</div>` : '';
    ...
    // Phase 5: EU AI Act — always show
    {
        ...
        const t = [];
        if (hr > 0)
            t.push({ description: `Address ${hr} high-risk indicator(s)`, type: 'fix', done: false, isStructured: true });
        if (tg > 0)
            t.push({ description: `Close ${tg} transparency gap(s)`, type: 'fix', done: false, isStructured: true });
        if (ai > 0)
            t.push({ description: `Review ${ai} AI system indicator(s) (Art. 6)`, type: 'review', done: false, isStructured: true });
        t.push({ description: 'Generate documentation artifacts', type: 'doc', done: clean, isStructured: true }, { description: 'Review AI system classification (Art. 6)', type: 'review', done: clean, isStructured: true }, { description: 'Schedule legal review', type: 'review', done: clean, isStructured: true });
        ...
        const status = progress >= 95 ? 'completed' : (progress > 0 ? 'in-progress' : 'pending');
        if (t.length === 0)
            t.push({ description: 'No AI system indicators detected — EU AI Act compliance verified.', type: 'verify', isStructured: true });
    ...
    const mockAutoComplete = isLegitimateDemo || scanIsClean || mockTotal === 0;
    {
        ...
        if (!mockAutoComplete)
            t.push({ description: 'Add .simplebeaconignore patterns for fixtures', type: 'fix', codeSnippet: 'echo "*.fixture.*" >> .simplebeaconignore', isStructured: true }, { description: 'Exclude test data from production builds', type: 'fix', isStructured: true });
        if (mockAutoComplete)
            t.push({ description: 'Demo data verified — excluded from production builds', type: 'verify', done: mockAutoComplete, isStructured: true });
    ...
    const packages = (npm.packages || []);
    {
        ...
        if (depCount > 0)
            t.push({ description: `Audit ${depCount} total dependencies`, type: 'audit', codeSnippet: 'npm audit', done: false, isStructured: true });
        if (outdatedCount > 0)
            t.push({ description: `Update ${outdatedCount} outdated package(s)`, type: 'fix', codeSnippet: 'npm update', done: false, isStructured: true });
        if (missingLockfiles > 0)
            t.push({ description: `Add missing lockfiles (${missingLockfiles} package(s))`, type: 'fix', codeSnippet: 'npm install', location: pkgList.filter(p => p.hasLockfile === false).map(p => p.path || String(p)).join(', '), done: false, isStructured: true });
        t.push({ description: 'Run npm audit', type: 'audit', codeSnippet: 'npm audit', done: npmClean, isStructured: true }, { description: 'Verify lockfile integrity', type: 'verify', codeSnippet: 'npm ci', done: npmClean, isStructured: true }, { description: 'Review dependency update policy', type: 'review', done: npmClean, isStructured: true });
        ...
        if (t.length === 0)
            t.push({ description: 'Dependencies verified — no audit issues detected.', type: 'verify', isStructured: true });
    ...
    // Phase 6: Quality Optimization — always show
    {
        ...
        if (todoDetail.length > 0) {
            ...
            t.push({ description: `Address ${todoMarkers} TODO/FIXME marker(s) in source code`, type: 'fix', isStructured: true });
        }
        ...
        if (qs < 85)
            t.push({ description: 'Refactor low-quality modules (quality score < 85)', type: 'fix', done: false, isStructured: true });
        t.push({ description: 'Add test coverage for uncovered modules', type: 'fix', codeSnippet: 'npm test -- --coverage', done: qs >= 90, isStructured: true }, { description: 'Install pre-commit hooks for automated scanning', type: 'fix', codeSnippet: 'npx husky install', done: qs >= 90, isStructured: true }, { description: 'Schedule monthly quality gate reviews', type: 'review', done: qs >= 90, isStructured: true });
    ...
    // Phase 6: Junk & Temporary Files — always show
    {
        ...
        t.push({ description: 'Add .simplebeaconignore patterns for temp files', type: 'fix', codeSnippet: 'echo "*.tmp" >> .simplebeaconignore', done: junkClean, isStructured: true }, { description: 'Schedule monthly cleanup sweep', type: 'review', done: junkClean, isStructured: true });
    ...
    // Phase 8: Dependency Vulnerability Audit — always show
    {
        ...
        t.push({ description: 'Run npm audit fix', type: 'fix', codeSnippet: 'npm audit fix', done: vulnClean, isStructured: true }, { description: 'Review dependency update policy', type: 'review', done: vulnClean, isStructured: true }, { description: 'Enable Dependabot or Snyx', type: 'fix', codeSnippet: 'Enable Dependabot in repo settings', done: vulnClean, isStructured: true });
    ...
    phases.forEach(p => {
        if (p.status === 'completed' && Array.isArray(p.tasks)) {
            p.tasks.forEach(t => { if (t && (t.type === 'verify' || t.type === 'review'))
                t.done = true; });
        }
    });
...
function renderDashboard(report, roadmap) {
    ...
    if (roadmap && Array.isArray(roadmap.phases)) {
        roadmap.phases.forEach(phase => {
            if ((phase.status === 'completed' || phase.progress >= 95) && Array.isArray(phase.tasks)) {
                phase.tasks.forEach(task => { if (task && (task.type === 'verify' || task.type === 'review'))
                    task.done = true; });
            }
        });
    }
    ...
    function renderIssues() {
        ...
        if (issueSearchHitsEl)
            issueSearchHitsEl.textContent = "Showing " + Math.min(_issuesChunkSize, filtered.length) + ' of ' + filtered.length + ' issues (lazy)';
        ...
        _issuesObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (issueSearchHitsEl)
                        issueSearchHitsEl.textContent = "Showing " + Math.min(nextIdx, filtered.length) + ' of ' + filtered.length + ' issues';
...
function generatePdf() {
    ...
    clone.querySelectorAll('*').forEach(el => {
        ...
        const fix = (val, map) => { if (!val || val === 'transparent' || val === 'rgba(0, 0, 0, 0)')
            return; const rgb = val.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/); if (rgb) {
            const hex = '#' + [1, 2, 3].map(i => ('0' + parseInt(rgb[i]).toString(16)).slice(-2)).join('');
            if (map[hex]) {
                el.style.color = map[hex];
            }
        } const short = Object.keys(map).find(k => val.includes(k)); if (short)
...
function exportRoadmapJson() {
    ...
    const projectName = currentReport.projectRoot || currentReport.projectPath || currentReport.projectName || currentReport.scanTargetRoot || ((_a = currentReport.metadata) === null || _a === void 0 ? void 0 : _a.project) || ((_b = currentReport.summary) === null || _b === void 0 ? void 0 : _b.project) || (currentReport.sourceReport && currentReport.sourceReport.projectRoot) || 'Unknown';
    ...
    const phasesWithState = currentRoadmap.phases.map(p => {
        ...
        if (p.extraHtml) {
            ...
            if (imp)
                impact = imp[1].replace(/&amp;&amp;/g, 'and').replace(/&amp;/g, '&').trim();
            if (fx)
                fix = fx[1].replace(/&amp;&amp;/g, 'and').replace(/&amp;/g, '&').trim();
        }
...
document.getElementById('copySummaryBtn').addEventListener('click', () => { if (!currentRoadmap || !currentReport) {
    showToast('Load a report first', 'warning');
    return;
} const pk = String(currentReport.projectRoot || currentReport.projectPath || currentReport.projectName || 'Unknown').replace(/[^a-z0-9]/gi, '_'); const total = currentRoadmap.phases.length; const completed = currentRoadmap.phases.filter(p => p.status === 'completed').length; const blocked = currentRoadmap.phases.filter(p => p.status === 'blocked').length; const inProgress = currentRoadmap.phases.filter(p => p.status === 'in-progress').length; const critical = currentRoadmap.phases.filter(p => p.severity === 'critical' && p.status !== 'completed').length; const lines = ['SimpleBeacon Scan Summary', 'Project: ' + pk, 'Generated: ' + new Date().toLocaleDateString(), '', 'Phases: ' + total + ' | Completed: ' + completed + ' | In Progress: ' + inProgress + ' | Blocked: ' + blocked + ' | Critical: ' + critical, '']; currentRoadmap.phases.forEach(p => { const icon = p.status === 'completed' ? '✅' : p.status === 'blocked' ? '🚫' : p.status === 'in-progress' ? '🟡' : '⬜'; lines.push(icon + ' ' + p.title + ' — ' + p.status + ' (' + p.progress + '%) — ' + p.severity); if (p.tasks.length > 0) {
    const done = p.tasks.filter((t, idx) => loadTaskState(pk, p.id, idx)).length;
    lines.push('   Tasks: ' + done + '/' + p.tasks.length + ' done');
} }); lines.push('', 'Overall Health: ' + (currentRoadmap.phases.length ? Math.round(currentRoadmap.phases.reduce((a, p) => a + (p.status === 'completed' ? 1 : p.status === 'in-progress' ? 0.5 : 0), 0) / currentRoadmap.phases.length * 100) : 0) + '%'); if (!navigator.clipboard) {
    showToast('Clipboard not available — use HTTPS or copy manually', 'error');
    return;
...
document.getElementById('exportGitHubBtn').addEventListener('click', () => { if (!currentRoadmap)
    return; const pk = String(currentReport.projectRoot || currentReport.projectPath || currentReport.projectName || 'Unknown').replace(/[^a-z0-9]/gi, '_'); let issueCount = 0; const files = []; currentRoadmap.phases.forEach(p => { if (p.status === 'completed')
    return; p.tasks.forEach((t, taskIdx) => { var _a; const done = loadTaskState(pk, p.id, taskIdx); if (done)
    return; const taskStr = typeof t === 'string' ? t : (typeof t === 'object' && t != null && t.description ? t.description : (t.html || t.text || '')); if (/^\.\.\.and\s+\d+\s+more/.test(taskStr))
    return; issueCount++; const shortTask = taskStr.length > 80 ? taskStr.slice(0, 77) + '...' : taskStr; const title = '[' + p.title + '] ' + shortTask; const bodyParts = ['## Context', '**Phase:** ' + p.title, '**Severity:** ' + p.severity, '**Effort:** ' + p.effort, '**Progress:** ' + p.progress + '%', '', p.description]; if (p.dependsOn) {
...
document.getElementById('exportJiraBtn').addEventListener('click', () => { if (!currentRoadmap)
    return; const pk = String(currentReport.projectRoot || currentReport.projectPath || currentReport.projectName || 'Unknown').replace(/[^a-z0-9]/gi, '_'); const lines = ['Issue Type,Summary,Description,Priority,Status,Labels,Linked Issues']; currentRoadmap.phases.forEach(p => { if (p.status === 'completed')
    return; const jiraPriority = p.severity === 'critical' ? 'Highest' : p.severity === 'high' ? 'High' : p.severity === 'medium' ? 'Medium' : 'Low'; const jiraStatus = p.status === 'blocked' ? 'Blocked' : p.status === 'in-progress' ? 'In Progress' : 'To Do'; p.tasks.forEach((t, taskIdx) => { const done = loadTaskState(pk, p.id, taskIdx); if (done)
    return; const taskStr = typeof t === 'string' ? t : (typeof t === 'object' && t != null && t.description ? t.description : (t.html || t.text || '')); if (/^\.\.\.and\s+\d+\s+more/.test(taskStr))
    return; const summary = '[' + p.title + '] ' + taskStr; const desc = p.description + (p.dependsOn ? ' Depends on: ' + p.dependsOn : ''); const labels = 'simplebeacon ' + p.severity; const linked = p.dependsOn || ''; lines.push('Task,"' + summary.replace(/"/g, '""') + '","' + desc.replace(/"/g, '""') + '",' + jiraPriority + ',' + jiraStatus + ',' + labels + ',' + linked); }); }); if (lines.length === 1) {
    showToast('No pending tasks to export', 'warning');
    return;
...
document.getElementById('phaseImportInput').addEventListener('change', e => { if (e.target.files[0]) {
