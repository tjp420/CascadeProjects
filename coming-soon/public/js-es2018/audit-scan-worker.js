...
const PATTERN_REGISTRY = {
    ...
    fictionKpi: {
        appliesTo: ['javascript', 'python', 'java', 'go', 'rust', 'php', 'ruby', 'dotnet', 'generic'],
        pattern:
            /\b(?:99\.9|99\.99|100)\s*%?\s*(?:uptime|availability|accuracy|success\s*rate|coverage|pass\s*rate|reliability)|\b(?:revenue|users?|customers?|downloads?|requests?|transactions?)\s*(?:=|:)\s*(?:["']?\d{4,}["']?|\d{4,}\s*(?:M|B|K|million|billion|thousand))|\b(?:TODO|FIXME)\b.*(?:fictional|placeholder|mock|sample|dummy|fake)\s+(?:data|numbers?|metrics?|kpi)/i,
        maxMatches: 3,
        selfReferenceFilter:
            /fiction-kpi|llm-slop-patterns|rejectedFiction|scanner-patterns|pricing\.html|site-config/i,
        contextFilter: (snippet, filePath) => {
            ...
            if (/\/\/.*(fictional|placeholder|mock|sample|dummy|fake|not.*real|demo|example)/i.test(snippet))
                return false;
            return true;
        }
    },
    ...
    mockPathLeak: {
        appliesTo: ['javascript', 'python', 'java', 'go', 'rust', 'php', 'ruby', 'dotnet', 'generic'],
        pattern:
            /['"`](?:[^'"`]*\/)?mock-?data\/|['"`](?:[^'"`]*\/)?test-?fixtures\/|['"`](?:[^'"`]*\/)?sample-?data\//i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
            if (/test|spec|__tests__|\.test\.|\.spec\./i.test(filePath)) return false;
            return true;
    ...
    emptyStubFunction: {
        ...
        contextFilter: (snippet, filePath) => {
            if (
                /test|spec|__tests__|\.test\.|\.spec\.|fixture|mock|stub|interface|abstract|protocol|trait/i.test(
                    filePath
                )
            )
                return false;
            if (/\/\/\s*(?:stub|placeholder|todo|implement later|not yet implemented)/i.test(snippet)) return true;
            if (/export\s+(?:async\s+)?function|export\s+const|export\s+default|module\.exports/i.test(snippet))
                return false;
            return true;
        }
    },
    roadmapMarker: {
        appliesTo: ['javascript', 'python', 'java', 'go', 'rust', 'php', 'ruby', 'dotnet', 'generic'],
        pattern:
            /\/\/\s*(?:ROADMAP|PLANNED|FUTURE|NEXT|BACKLOG|LATER)\b|\/\*\s*(?:ROADMAP|PLANNED|FUTURE|NEXT|BACKLOG|LATER)\b|#\s*(?:ROADMAP|PLANNED|FUTURE|NEXT|BACKLOG|LATER)\b/i,
        maxMatches: 3
    },
    aiPlaceholderComment: {
        appliesTo: ['javascript', 'python', 'java', 'go', 'rust', 'php', 'ruby', 'dotnet', 'generic'],
        pattern:
            /\/\/\s*(?:AI\s+generated|Generated\s+by\s+(?:AI|Copilot|ChatGPT|Claude|GPT)|This\s+code\s+was\s+generated|Auto-generated|Do\s+not\s+edit|This\s+file\s+was\s+auto)/i,
        maxMatches: 3
    },
    markdownFenceLeak: {
        appliesTo: ['javascript', 'python', 'java', 'go', 'rust', 'php', 'ruby', 'dotnet', 'generic'],
    ...
    awsSecretKey: {
        appliesTo: ['javascript', 'python', 'java', 'go', 'rust', 'php', 'ruby', 'dotnet', 'generic'],
        pattern:
            /AKIA[0-9A-Z]{16}|ASIA[0-9A-Z]{16}|aws_secret_access_key\s*[:=]\s*['"`][A-Za-z0-9/+=]{40}['"`]|aws_access_key_id\s*[:=]\s*['"`]AKIA[0-9A-Z]{16}['"`]/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
            if (/test|spec|fixture|mock|example|sample|placeholder|template|documentation/i.test(filePath))
                return false;
    ...
    privateKeyBlock: {
        ...
        maxMatches: 2,
        contextFilter: (snippet, filePath) => {
            if (/test|spec|fixture|mock|example|sample|placeholder|template|\.pem\.example/i.test(filePath))
                return false;
            return true;
        }
    },
    ...
    bearerToken: {
        ...
        contextFilter: (snippet, filePath) => {
            if (/test|spec|fixture|mock|example|sample|placeholder|template/i.test(filePath)) return false;
            if (
                /Bearer\s+your_token|Bearer\s+\$\{|Bearer\s+\+|Bearer\s+tokenVar|Bearer\s+accessToken|Bearer\s+authToken|Bearer\s+jwt/i.test(
                    snippet
                )
            )
                return false;
    ...
    dockerRootUser: {
        ...
        maxMatches: 2,
        contextFilter: (snippet, filePath) => {
            if (!/Dockerfile|dockerfile|\.docker/i.test(filePath) && !/docker-compose|compose\.ya?ml/i.test(filePath))
                return false;
            if (/test|spec|fixture|mock|example/i.test(filePath)) return false;
            return true;
        }
    ...
    suspiciousPackage: {
        appliesTo: ['javascript', 'generic'],
        pattern:
            /(?:from\s+['"]|require\s*\(\s*['"]|import\s+['"])(?:[^'"]*npm[^'"]*|[^'"]*typosquat)[^'"]*['"]|(?:npm|yarn|pnpm)\s+install\s+[@a-z0-9_\-]+(?:[a-z0-9_\-]*\.js|nodejs|nodjs|nodel|node-js)/i,
        maxMatches: 3,
        contextFilter: (snippet, filePath) => {
            if (/test|spec|fixture|mock|example/i.test(filePath)) return false;
...
function shouldSkipAnalyzerFile(name, filePath) {
    ...
    if (securityRules.includes(name)) {
        ...
        if (/(?:^|\/)scanner-patterns|credential-pattern|test-all-patterns|pattern-documentation/i.test(normalized))
            return true;
    }
    return false;
}
...
function shouldSkipFile(path, deepScan, ignoreCtx) {
    ...
    if (
        ...
        return true;
    if (!deepScan && /(^|[\/])(docs\/|doc\/|third_party\/|thirdparty\/|vendor\/)/i.test(normalized)) return true;
