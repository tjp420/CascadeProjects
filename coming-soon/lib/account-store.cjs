...
function ensureAccountsTable() {
  ...
  try { db.exec(`ALTER TABLE time_tokens ADD COLUMN last_validated_at TEXT`); } catch {}
  try { db.exec(`ALTER TABLE time_tokens ADD COLUMN last_context_hash TEXT`); } catch {}
  try { db.exec(`ALTER TABLE time_tokens ADD COLUMN lease_expires_at TEXT`); } catch {}
  try { db.exec(`ALTER TABLE time_tokens ADD COLUMN use_count INTEGER DEFAULT 0`); } catch {}
  try { db.exec(`ALTER TABLE time_tokens ADD COLUMN first_used_at TEXT`); } catch {}
}
...
function revokeAccount(accountId) {
  ...
  db.prepare("UPDATE time_tokens SET status = 'revoked', revoked_at = ? WHERE account_id = ? AND status != 'revoked'").run(now, accountId);
  return { success: true };
}
...
function registerTimeTokenHash(timeTokenId, jwtString) {
  ...
  if (ttRow?.account_id) {
    ...
    if (account && account.tier === 'starter' && account.subscriptionTier && account.subscriptionTier !== 'starter') {
      const restorePlan = getPlan(account.subscriptionTier);
      const nowIso = new Date().toISOString();
      db.prepare(
        `UPDATE accounts SET tier = ?, features = ?, limits = ?, updated_at = ? WHERE id = ?`
      ).run(
...
function validateTimeToken(jwtString, context = {}) {
  ...
  const activeRemaining = db.prepare(
    `SELECT COUNT(*) AS count FROM time_tokens WHERE account_id = ? AND status = 'active'`
  ).get(row.account_id);
  let downgraded = false;
  if (activeRemaining.count === 0 && account.tier !== 'starter') {
    const starterPlan = getPlan('starter');
    db.prepare(
      `UPDATE accounts SET tier = 'starter', features = ?, limits = ?, updated_at = ? WHERE id = ?`
    ).run(
