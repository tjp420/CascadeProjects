...
function buildEuComplianceCertificateHtml(reportJson, payload) {
    ...
    const controlRows =
        controls
            ...
            .join('') ||
        '<tr><td colspan="4" style="text-align:center;color:#94a3b8;"><em>No compliance checks were recorded for this project.</em></td></tr>';
    ...
    const docList = docsFound.length
        ...
        : '<em style="color:#94a3b8;">No governance documentation was found in your repository. Consider adding AI system documentation, risk assessments, and transparency records.</em>';
    ...
    const statusText = overallPass
        ? 'Your project meets the baseline requirements for EU AI Act readiness. No high-risk AI indicators or transparency gaps were found — your code is ready for EU AI Act compliance review.'
        : 'Your project has some areas that need attention before it meets EU AI Act requirements. Review the issues below and fix them before submitting for compliance review.';
...
<div class="metrics">
  <div class="metric"><div class="num">${aiCount}</div><div class="lbl">AI system indicators</div></div>
  <div class="metric"><div class="num">${hrCount}</div><div class="lbl">High-risk markers</div></div>
  <div class="metric"><div class="num">${tgCount}</div><div class="lbl">Transparency gaps</div></div>
  <div class="metric"><div class="num">${docCount}</div><div class="lbl">Docs found</div></div>
</div>

<h2 style="font-size:1rem;font-weight:700;color:#1e293b;margin-top:24px;margin-bottom:10px;">Compliance Checks</h2>
<table>
  <thead><tr><th>Status</th><th>Check</th><th>Result</th><th>EU AI Act Article</th></tr></thead>
  <tbody>${controlRows}</tbody>
</table>

<h2 style="font-size:1rem;font-weight:700;color:#1e293b;margin-top:20px;margin-bottom:10px;">Governance Documentation Found</h2>
<div style="font-size:0.88rem;color:#334155;">${docList}</div>

<div class="footer">
  Generated by SimpleBeacon · simplebeacon.ai · This certificate is an automated assessment for attestation purposes and does not constitute legal advice.<br>
...
function buildCertificateHtml(reportJson, payload) {
    ...
    <section class="section">
      ...
      <h2>Fix Roadmap</h2>
      <p class="meta">Step-by-step action plan generated from your scan results. ${completedPhases} of ${remediationPhases.length} steps complete. ${pendingPhases} pending${criticalPhases > 0 ? `, ${criticalPhases} critical` : ''}.</p>
      <div class="kpi-strip">
        ...
        <div class="kpi"><strong>${remediationPhases.length}</strong><span>Total steps</span></div>
      </div>
      <table class="data-table">
        <tr><th>Severity</th><th>Step</th><th>Status</th><th>Tasks</th><th>Time needed</th></tr>
        ${phaseRows}
      </table>
    </section>`
        : '';
    ...
    <section class="section">
      ...
      <p class="meta">Checks if your project meets EU AI Act requirements — risk classification, transparency, and documentation.</p>
      <div class="kpi-strip">
    ...
    <section class="section">
      ...
      <p class="meta">Results from all SimpleBeacon analysis checks. Only checks that found issues are shown.</p>
      ${allSubs}
    </section>`
        : '';
...
<section class="certificate cover-page">
  ...
  <p class="confidential">Prepared for authorized business and engineering recipients. This document combines executive risk metrics for leadership and step-by-step fix instructions for developers.</p>
</section>
<main>
  <section class="section">
    <div class="section-num">Section 01</div>
    <h2>Audit Metadata &amp; Ledger</h2>
    <p class="meta">Scan details — when it ran, what was scanned, and how it was performed.</p>
    <table class="data-table">
      <tr><td>Client name</td><td>${safeClientName}</td></tr>
      <tr><td>Repository / branch</td><td><code>${safeProjectName}</code> / <code>main</code></td></tr>
      <tr><td>Scan date</td><td>${safeNowStr}</td></tr>
      <tr><td>Scanner version</td><td>SimpleBeacon Engine v1.3.0 (Zero-Dependency)</td></tr>
      <tr><td>Files scanned</td><td>${safeTotalFiles} files</td></tr>
      <tr><td>Assessed by</td><td>SimpleBeacon</td></tr>
  ...
  <section class="section">
    ...
    <p class="meta">High-level summary for leadership — generated directly from scan results, no AI inference on counts or findings.</p>
  ...
  <section class="section">
    ...
    <h2>Developer Action Plan (Step-by-Step Fixes)</h2>
    <p class="meta">Each row shows the file, what's wrong, why it matters, and how to fix it. Showing up to 100 prioritized issues.</p>
    <table class="data-table">
      <tr><th>Severity</th><th>File</th><th>Issue type</th><th>Why it matters</th><th>How to fix</th></tr>
      ${issueRows || '<tr><td colspan="5" style="text-align:center;color:#8b949e;">No issues found — your code passed all checks.</td></tr>'}
    </table>
    <div class="verify-block">
      <h3>Verify your fixes locally</h3>
      <p class="meta">After applying the fixes above, run this command to verify your code passes the quality gate — no need to wait for a re-audit.</p>
      <div class="command-box">npx simplebeacon scan --path ./${safeProjectName} --gate</div>
    </div>
  </section>
  <section class="section">
    <div class="section-num">Section 04</div>
    <h2>Compliance Checklist &amp; Prevention Steps</h2>
    <p class="meta">Quality checks and automated tools to keep your code secure going forward.</p>
    <h3>Quality checklist</h3>
    <table class="data-table">
      <tr><th>Check</th><th>Status</th><th>Notes</th></tr>
      <tr><td>No hardcoded passwords or API keys</td><td><strong>${credentialHits > 0 ? 'FAIL' : 'PASS'}</strong></td><td>${credentialHits > 0 ? safeCredentialHits + ' hardcoded password/key(s) found' : 'Scanned ' + safeTotalFiles + ' file(s) — no hardcoded passwords or keys found'}</td></tr>
      <tr><td>No test data in production code</td><td><strong>PASS</strong></td><td>Scanned ${safeTotalFiles} production file(s) — no test data leaks found</td></tr>
      <tr><td>Sample data schema matches specs</td><td><strong>N/A</strong></td><td>No registered page samples to check</td></tr>
      <tr><td>No AI-generated placeholder values</td><td><strong>N/A</strong></td><td>Consistency checks not configured for this project</td></tr>
    </table>
    <h3>Prevent issues before they're committed — install a pre-commit hook</h3>
    <div class="command-box">npx simplebeacon hook install</div>
    <p class="meta">This installs a local hook that automatically checks for hardcoded passwords, test data leaks, and AI placeholder values before each commit.</p>
    <h3>Set up automated checks in CI</h3>
    <div class="command-box">npx simplebeacon scan --gate --format json --output .simplebeacon/report.json</div>
    <p class="meta">Add .github/workflows/simplebeacon-gate.yml from the SimpleBeacon examples so pull requests automatically fail when high-severity issues are found.</p>
    <div class="disclaimer-box">
      <strong>About this report.</strong> This is an automated technical review of the source code at the time of evaluation. It is not a legal compliance guarantee, formal penetration test, SOC 2 attestation, or certification that the system is secure in production. The client remains responsible for remediation, release authorization, and ongoing security posture.
    </div>
  </section>
  ...
  <section class="section">
    ...
    <h2>Production Sign-Off</h2>
    <p class="meta">Complete this section after all issues are fixed and a re-scan shows zero critical or high issues.</p>
    <div class="signoff-grid">
      <span class="signoff-check"><span class="signoff-box" aria-hidden="true"></span> Step 1: All issues fixed by the engineering team.</span>
      <span class="signoff-check"><span class="signoff-box" aria-hidden="true"></span> Step 2: Re-scan completed with zero critical or high issues remaining.</span>
    </div>
  ...
  <section class="section">
    ...
    <ul><li>Scanned ${safeTotalFiles} files in your repository.</li><li>Pattern matching checks for hardcoded passwords/keys, AI-generated code issues, and code quality problems — not a semantic AI review.</li><li>Processing runs 100% locally. No source code leaves your computer.</li><li>Checks include: hardcoded credentials, AI-generated placeholder values, and code quality rules.</li></ul>
