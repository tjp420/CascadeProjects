...
function createTokenChain(ownerEmail, ownerTokenPayload, ownerJwt, ttlMinutes, attached = [], attachedJwts = []) {
    ...
    for (let i = 0; i < attached.length; i++) {
        ...
        if (aHash) {
            const row = db
                .prepare('SELECT id, token_hash, status, token_type FROM token_nodes WHERE token_hash = ?')
                .get(aHash);
...
function revokeToken(tokenHash, cascade = false) {
    ...
    if (cascade && node.token_type === 'owner') {
        db.prepare("UPDATE token_nodes SET status = 'revoked' WHERE parent_id = ? AND status != 'revoked'").run(
            node.id
        );
    }
