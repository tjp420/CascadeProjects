            ...
            if (!reason && ignorePatterns.length) {
                const isIgnoredBySimplebeacon = ignorePatterns.some(pat => {
                    if (typeof pat !== 'string')
                        return false; // simplebeacon-ignore: strict !== comparison
                    if (pat.startsWith('*')) {
                ...
                for (const analyzerId of activeAnalyzers) {
                    ...
                    try {
                        ...
                        if (reg.id === 'unvalidatedRedirect' && (/simplebeacon-server\.cjs$|app-links\.js$|free-token\.cjs$|_restore-eu-audit\.js$/.test(path)))
                            continue;
                        if (reg.id === 'missingRateLimit' && (/simplebeacon-server\.cjs$|health|server\.cjs$|_restore-eu-audit\.js$/.test(path)))
                            continue;
