...
function renderAuditFileAnalysisPanel(summary, meta) {
    ...
    Object.keys(FILE_ANALYSIS_CATEGORY_LABELS).forEach(function (key) {
        ...
        rows.push('<div class="file-analysis-row">'
...
function downloadSelectedModule(btn) {
    ...
    try {
        ...
        const totalFiles = ((_c = data.codebase) === null || _c === void 0 ? void 0 : _c.totalFiles) || data.totalFiles || data.filesAnalyzed || 1;
        ...
        if (num === '1') {
            ...
            const mockTotal = (_f = data.mockSampleFiles) !== null && _f !== void 0 ? _f : mockCats.reduce((a, c) => a + (c.fileCount || 0), 0);
            ...
            if (debugCount > 0)
                actionItems.push({ task: `Remove ${debugCount} build artifact${debugCount === 1 ? '' : 's'} (log statements, breakpoint statements, alerts)`, priority: 'high', source: 'cleanup' });
            // Data quality
            const dq = data.dataQuality || {};
            const emptyCount = dq.emptyJsonCount || (dq.emptyJsonFiles || []).length || 0;
            if (emptyCount > 0)
                actionItems.push({ task: `Clean up ${emptyCount} empty JSON file${emptyCount === 1 ? '' : 's'}`, priority: 'low', source: 'data-quality' });
            // File reduction
            const fr = data.fileReduction || data.fileReductionPlan || {};
            const assetCount = (fr.unusedAssetCandidates || []).length;
            if (assetCount > 0)
                actionItems.push({ task: `Review && remove ${assetCount} unused asset${assetCount === 1 ? '' : 's'}`, priority: 'medium', source: 'file-reduction' });
            // Consolidation
            const cons = data.consolidation || {};
            const dupCount = cons.duplicateGroups || 0;
            if (dupCount > 0)
                actionItems.push({ task: `Consolidate ${dupCount} duplicate file group${dupCount === 1 ? '' : 's'}`, priority: 'medium', source: 'consolidation' });
            // EU AI Act
            const eu = data.euAiActSummary || {};
            if ((eu.highRiskIndicators || 0) > 0)
                actionItems.push({ task: 'Schedule EU AI Act legal review (high-risk indicators detected)', priority: 'critical', source: 'eu-ai-act' });
            if ((eu.aiSystemIndicators || 0) > 0)
                actionItems.push({ task: 'Document AI system classification under EU AI Act Article 6', priority: 'medium', source: 'eu-ai-act' });
            if ((eu.transparencyGaps || 0) > 0)
                actionItems.push({ task: 'Address transparency gaps per EU AI Act Article 10', priority: 'medium', source: 'eu-ai-act' });
            // npm audit
            ...
            if (pkgCount > 0 && depCount / pkgCount > 50)
                actionItems.push({ task: `Audit ${depCount} dependencies — dependency density is high`, priority: 'low', source: 'npm-audit' });
            // Mock data
            const mockCats = data.mockDataCategories || [];
            const mockTotal = (_g = data.mockSampleFiles) !== null && _g !== void 0 ? _g : mockCats.reduce((a, c) => a + (c.fileCount || 0), 0);
            if (mockTotal > 0)
                actionItems.push({ task: `Ensure ${mockTotal} mock/fixture file${mockTotal === 1 ? '' : 's'} are excluded from production builds`, priority: 'low', source: 'mock-data' });
            ...
            moduleData = { ...moduleData, debugArtifacts: artifacts.slice(0, 10), debugArtifactCount: ac, findings: ((debugIssue === null || debugIssue === void 0 ? void 0 : debugIssue.findings) || []).slice(0, 5).map(f => ({ file: f.file, matches: (f.matches || []).slice(0, 3).map(m => ({ line: m.line, snippet: m.snippet })) })), summary: ac ? `${ac} debug artifact${ac === 1 ? '' : 's'} detected.` : 'No debug artifacts found.' };
        }
        else if (num === '9') {
            const npm = data.npmAudit || {};
            ...
            const allFiles = data.fileList || ((_h = data.repositoryInventory) === null || _h === void 0 ? void 0 : _h.totalFiles) || [];
...
function registerTokenInVault(token) {
    ...
    const features = Array.isArray(payload === null || payload === void 0 ? void 0 : payload.features) ? payload.features : (Array.isArray(payload === null || payload === void 0 ? void 0 : payload.modules) ? payload.modules : null);
...
function wireRuntimeModeTabs() {
    ...
    if (!cliTab || !browserTab || !cliView || !browserView)
        return;
...
function triggerDirectoryPicker(options) {
    ...
    if (tokenJustGenerated) {
        ...
        Promise.resolve().then(function () {
            ...
            if (typeof applyProductFromToken === 'function')
                applyProductFromToken(_token);
            if (typeof updateDropzoneGate === 'function')
                updateDropzoneGate();
            if (typeof showToast === 'function')
                showToast('Sandbox ready — you can scan now', 'success');
...
function validateReportQuality(report) {
    ...
    const accChecks = [
        ...
        report.filesAnalyzed === (((_a = report.codebase) === null || _a === void 0 ? void 0 : _a.totalFiles) || report.filesAnalyzed),
    ...
    const sevSum = (((_b = report.severityCounts) === null || _b === void 0 ? void 0 : _b.critical) || 0) + (((_c = report.severityCounts) === null || _c === void 0 ? void 0 : _c.high) || 0) + (((_d = report.severityCounts) === null || _d === void 0 ? void 0 : _d.medium) || 0) + (((_e = report.severityCounts) === null || _e === void 0 ? void 0 : _e.low) || 0);
    const summaryFindings = (_g = (_f = report.summary) === null || _f === void 0 ? void 0 : _f.totalFindings) !== null && _g !== void 0 ? _g : -1;
    const consChecks = [
        sevSum === report.issueCount,
        summaryFindings === report.issueCount || summaryFindings === -1,
        ((_j = (_h = report.gateReport) === null || _h === void 0 ? void 0 : _h.blockingCount) !== null && _j !== void 0 ? _j : 0) === ((_l = (_k = report.gate) === null || _k === void 0 ? void 0 : _k.blockingCount) !== null && _l !== void 0 ? _l : 0)
    ];
