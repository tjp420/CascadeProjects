...
function downloadSelectedModule(btn) {
    ...
    try {
        ...
        if (num === '1') {
            ...
            if (!foundFiles.some(f => f.includes('CODE_OF_CONDUCT')))
                recs.push('Add CODE_OF_CONDUCT.md to set community standards.');
...
if (tryFreeBtn) {
    tryFreeBtn.addEventListener('click', async () => {
        ...
        const email = (((_a = document.getElementById('resendEmail')) === null || _a === void 0 ? void 0 : _a.value) || '').trim();
        btn.disabled = true;
...
async function probeLocalServer() {
    ...
    for (const port of SERVER_PORTS) {
        ...
        try {
            ...
            if (vaultLink)
                vaultLink.href = '/dashboard/dashboard';
            return;
        }
        catch (_) {
