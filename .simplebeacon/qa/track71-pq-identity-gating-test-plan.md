...
## Extension scope (Track 71 Phase 2)
...
- **Batch pool initialization** — initialize multiple identity gating pools in a single batch call with per-pool results.
...
- **Batch attribute claim verification** — verify multiple attribute claims in a single batch call with per-claim results.
...
- **Slashing event recording** — record slash events with reason codes (malformed_claim, duplicate_claim, contract_lifetime_out_of_bounds, pool_not_found, banned_peer, out_of_window).
...
- [x] Batch attribute claim verification processes multiple proofs.
...
- [x] Reject empty batch attribute claim verification.
- [x] Reject batch attribute claim verification exceeding max size.
...
- [x] Record slashes for out-of-bounds contract lifetime.
...
- `ai-platform/server/lib/hsm-adapter/pqc-identity-gating-hub.cjs` *(extended)*
- `ai-platform/server/lib/hsm-adapter/zk-identity-gating-validator.cjs` *(extended)*
- `ai-platform/server/lib/hsm-adapter/hsm-metrics.cjs` *(14 new counters)*
- `ai-platform/server/lib/hsm-adapter/__tests__/pq-identity-gating-extensions.test.cjs` *(new, 54 tests)*
