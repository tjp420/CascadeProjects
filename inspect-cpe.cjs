...
const cpePath = path.join(__dirname, 'ai-platform', 'server', 'lib', 'hsm-adapter', 'crypto-policy-engine.cjs');
...
lines.forEach((l, i) => {
    ...
    if (mm && !mm[1].match(/^(if|for|while|switch|return|throw|const|let|var|case|default)/)) {
        methods.push({ line: i + 1, name: mm[1] });
    }
});
console.log('\nClass methods (approximate):', methods.length);
for (const m of methods.slice(0, 50)) console.log('  L' + m.line + ': ' + m.name);
if (methods.length > 50) console.log('  ... and ' + (methods.length - 50) + ' more');
