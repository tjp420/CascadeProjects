...
function extractMatches(text: string, pattern: RegExp, maxMatches = 5, redact = false, multiline = false): MatchEntry[] {
...
function shouldSkipFile(filePath: string): boolean {
  ...
  // Skip non-production directories (dev tools, demos, scripts, archives)
  ...
  if (/(^|\/)simplebeacon-vscode-merged\//.test(normalized)) return true;
  if (/(^|\/)ai-platform\//.test(normalized)) return true;
...
const PATTERN_REGISTRY: Record<string, PatternDef> = {
  credentials: {
    id: 'credentials', name: 'Credential Pattern',
    ...
    maxMatches: 3, redact: true, realtime: true,
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (/scanner-patterns\.js|scanner-engine\.js|pattern-documentation\.js/i.test(filePath) && /label.*Secret|section.*credentials|message.*secret/i.test(snippet)) return false;
  ...
  debugArtifacts: {
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (isConsoleLog) {
        ...
        // Conditional debug loggers (behind PROCESSOR_DEBUG, options.debug, etc.)
        if (/PROCESSOR_DEBUG|options\.debug/.test(snippet) && /console\.(log|error|warn)/.test(snippet)) return false;
        ...
        if (/workspaceAnalyzer\.ts/i.test(filePath) && /pattern.*console|contextFilter.*console|console\.log|debugger/.test(snippet)) return false;
        // CLI entry points
  ...
  aiResidueStub: {
    id: 'aiResidueStub', name: 'Stub Implementation',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'low',
    pattern: /function\s+\w+\s*\([^)]*\)\s*\{\s*\/\/\s*TODO|def\s+\w+\s*\([^)]*\):\s*pass\b|TODO\s*:\s*implement|TODO\s*:\s*AI|\/\/\s*AI\s+generated|\/\/\s*Generated\s+by\s+(ChatGPT|GPT|Claude|Copilot|Gemini|LLM)/i,
    maxMatches: 3,
    contextFilter: (snippet: string, filePath: string) => {
      // Exclude pattern definition files that describe stub detection
      if (/scanner-patterns\.js|scanner-engine\.js|pattern-documentation\.js|main\.js/i.test(filePath) && /label.*Placeholder|desc.*TODO|id.*ai-placeholder/i.test(snippet)) return false;
      // Exclude workspaceAnalyzer.ts pattern definition
  ...
  aiResidueSwallow: {
    id: 'aiResidueSwallow', name: 'Error Swallowing',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'medium',
    pattern: /catch\s*\([^)]*\)\s*\{\s*\}|except\s*[^:]+:\s*pass|catch\s*\([^)]*\)\s*\{\s*return\s*null|rescue\s*=>\s*e\s*#\s*ignore/i,
    maxMatches: 3,
    contextFilter: (snippet: string, filePath: string) => {
      // Exclude pattern definition files that describe error-swallowing detection
      if (/scanner-patterns\.js|scanner-engine\.js|pattern-documentation\.js/i.test(filePath) && /label.*Error|message.*swallow|desc.*catch/i.test(snippet)) return false;
  ...
  perfNestedLoop: {
    id: 'perfNestedLoop', name: 'Performance Anti-Pattern',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'low',
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (/scanner-patterns\.js|scanner-engine\.js|pattern-documentation\.js/i.test(filePath) && /label.*Nested|message.*O\(n\)|desc.*loop/i.test(snippet)) return false;
      // Exclude test-all-patterns.js fixture data
      if (/test-all-patterns\.js$/i.test(filePath)) return false;
      return true;
    },
  ...
  typeSafetyAny: {
    id: 'typeSafetyAny', name: 'Type Safety Gap',
    appliesTo: ['javascript','typescript'], severity: 'low',
    pattern: /:\s*any\b(?!\s*\[\])|@ts-ignore|@ts-nocheck/i,
    maxMatches: 3,
    selfReferenceFilter: /id:\s*['"]typeSafetyAny['"]|message:\s*['"]Type safety gap|pattern:\s*\/.*any\b|label:\s*['"]Explicit any|label:\s*['"]TypeScript error suppression/i,
    contextFilter: (snippet: string, filePath: string) => {
      // Exclude suggestion text in findingConverter that mentions 'any' as advice
      if (/findingConverter\.ts/i.test(filePath) && /Replace any with specific types|@ts-ignore/.test(snippet)) return false;
      ...
      if (/remediationProvider\.ts/i.test(filePath) && /Replace : any|type safety|@ts-ignore/.test(snippet)) return false;
      // Exclude scanner-engine.js which builds type-safety finding objects
      if (/scanner-engine\.js$/i.test(filePath) && /typeSafetyFindings|severity.*low.*type.*Type Safety/.test(snippet)) return false;
      return true;
    },
    message: 'Type safety gap: any type, missing PropTypes, or excessive parameters detected.',
  },
  missingTest: {
    id: 'missingTest', name: 'Missing Test Coverage',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'low',
  ...
  a11yGap: {
    id: 'a11yGap', name: 'Accessibility Gap',
    appliesTo: ['javascript','typescript','html'], severity: 'medium',
    pattern: /<img\b(?![^>]*\balt=)[^>]*>|<(input|textarea|select)\b(?![^>]*\b(?:aria-label|aria-labelledby|title|placeholder)=)(?![^>]*\b(?:hidden|id)\b)[^>]*>|<button\b(?![^>]*\b(?:aria-label|aria-labelledby|title)=)[^>]*>\s*<\/button>/i,
    maxMatches: 3,
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (/<input[^>]*\bhidden\b/i.test(snippet)) return false;
      // Exclude VS Code webview panel HTML templates
      ...
      if (/workspaceAnalyzer\.ts/i.test(filePath) && (/pattern.*img|message.*alt|id:\s*['"]a11yGap/i.test(snippet))) return false;
      // Exclude inputs with an ID that are likely labeled externally
      if (/<input[^>]*\bid=['"]/i.test(snippet)) return false;
  ...
  sensitiveData: {
    id: 'sensitiveData', name: 'Sensitive Data Exposure',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'high',
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (/\b(?:dev|audit|outreach|press|hello|certificates|sandbox|admin|user)@simplebeacon\.(?:ai|dev|local)\b/.test(snippet)) return false;
      ...
      if (/scanner-patterns\.js/i.test(filePath) && /label.*Sensitive|section.*sensitiveData/i.test(snippet)) return false;
  ...
  configDrift: {
    id: 'configDrift', name: 'Configuration Drift',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'medium',
    pattern: /localhost:\d+|127\.0\.0\.1:\d+|0\.0\.0\.0:\d+|hardcoded.*url|\bpassword\s*=\s*['"]|\bsecret\s*=\s*['"]|\bapi_key\s*=\s*['"]/i,
    maxMatches: 3, redact: true,
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (/['"]http[s]?:\/\/.*['"]\s*[,)]/.test(snippet) && !/\bpassword\b|\bsecret\b|\bapi_key\b/.test(snippet)) return false;
      // Exclude pattern definitions that describe config drift detection
      if (/scanner-patterns\.js|scanner-engine\.js|certificate-module\.js/i.test(filePath) && /pattern.*localhost|label.*Config|section.*configDrift|detail.*hardcoded.*url/i.test(snippet)) return false;
      if (/test-all-patterns\.js/i.test(filePath)) return false;
      ...
      if (/workspaceAnalyzer\.ts/i.test(filePath) && /pattern:|message:.*config|hardcoded.*url/i.test(snippet)) return false;
      ...
      if (/findingConverter\.ts$/i.test(filePath) && /Configuration Drift|Move hardcoded|secrets to \.env/i.test(snippet)) return false;
      if (/remediationProvider\.ts$/i.test(filePath) && /hardcoded URLs|configuration values|deployments frag/i.test(snippet)) return false;
      ...
      if (/AnalyzeView\.js$/i.test(filePath) && /localhost|127\.0\.0\.1|deploy leaks|hardcoded URLs|\.env files/i.test(snippet)) return false;
  ...
  dbAntiPattern: {
    id: 'dbAntiPattern', name: 'Database Anti-Pattern',
    appliesTo: ['javascript','python','java','go','php','ruby'], severity: 'high',
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      if (/scanner-patterns\.js|scanner-engine\.js|pattern-documentation\.js/i.test(filePath) && /label.*SQL|message.*query|desc.*parameterized/i.test(snippet)) return false;
      return true;
    },
    message: 'Raw SQL concatenation or unbounded query detected. Use parameterized queries and pagination.',
  },
  evalDanger: {
    id: 'evalDanger', name: 'Dangerous eval() Usage',
    appliesTo: ['javascript','python','php','ruby'], severity: 'high',
    pattern: /\beval\s*\(|\bnew\s+Function\s*\(|\bsetTimeout\s*\(\s*['"`]|\bsetInterval\s*\(\s*['"`]|\bsystem\s*\(/i,
    maxMatches: 3,
    selfReferenceFilter: /id:\s*['"]evalDanger['"]|\bname:\s*['"]Dangerous eval\(\)|message:\s*['"]eval\(\)|\/\/\s*Scanner rule definitions that detect eval|\/\/\s*CLI rule files use new Function/i,
    contextFilter: (snippet: string, filePath: string) => {
      // Exclude scripts/, coming-soon/, tools/, test files entirely
      if (/(^|\/)scripts\//i.test(filePath)) return false;
      if (/(^|\/)coming-soon(-dev)?\//i.test(filePath)) return false;
      ...
      if (/scanner-patterns\.js|scanner-engine\.js|certificate-module\.js|ui-renderer\.js|main\.js/i.test(filePath) && (/section:\s*['"]evalDanger['"]|label.*Eval|desc.*eval\(\)|id:\s*['"]eval-danger['"]/i.test(snippet))) return false;
      // Exclude workspaceAnalyzer.ts pattern definition lines
      if (/workspaceAnalyzer\.ts/i.test(filePath) && (/id:\s*['"]evalDanger['"]|name:\s*['"]Dangerous eval|message:\s*['"]eval\(\)/i.test(snippet))) return false;
  ...
  innerHtmlXss: {
    id: 'innerHtmlXss', name: 'innerHTML XSS Risk',
    appliesTo: ['javascript','typescript','html'], severity: 'medium',
  ...
  prototypePollution: {
    id: 'prototypePollution', name: 'Prototype Pollution Risk',
    appliesTo: ['javascript'], severity: 'high',
  ...
  unhandledPromise: {
    id: 'unhandledPromise', name: 'Unhandled Promise',
    appliesTo: ['javascript','typescript'], severity: 'medium',
  ...
  insecureRandom: {
    id: 'insecureRandom', name: 'Insecure Random for Security',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'high',
  ...
  loggingSecrets: {
    id: 'loggingSecrets', name: 'Sensitive Data in Logs',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet'], severity: 'high',
    ...
    contextFilter: (snippet: string, filePath: string) => {
      ...
      const hasVariable = /\b(?:password|token|secret|apiKey|api_key|privateKey|private_key|credential)\s*[,+)]/.test(snippet);
      const onlyInString = /['"][^'"]*(?:password|token|secret|apiKey|api_key|privateKey|private_key|credential)[^'"]*['"]/.test(snippet);
      if (!hasVariable && onlyInString) return false;
      ...
      if (/scanner-patterns\.js|codebase-analyzer\.cjs|workspaceAnalyzer\.ts/i.test(filePath) && /label.*Sensitive|message.*log|pattern.*console/i.test(snippet)) return false;
  ...
  llmSlop: {
    id: 'llmSlop', name: 'LLM Slop / Placeholder',
    appliesTo: ['javascript','python','java','go','rust','php','ruby','dotnet','html','json','yaml','yml'], severity: 'medium',
    pattern: /YOUR_[A-Z0-9_]+_HERE|INSERT_[A-Z0-9_]+_HERE|\[Insert\s[^\]]+\]|\/\/\s*Handle\s+this\s+later|99\.99\s*%?\s*Uptime|100\s*%?\s*Secure|Lorem\s+Ipsum\s+Dolor|9,999\s*Users/i,
    maxMatches: 5,
    contextFilter: (snippet: string, filePath: string) => {
...
export const ANALYZER_PRESETS: Record<string, AnalyzerPreset> = {
  essential: {
    ...
    patternIds: ['credentials', 'sensitiveData', 'evalDanger', 'dbAntiPattern', 'aiResidueSwallow', 'loggingSecrets', 'innerHtmlXss', 'prototypePollution'],
  },
  security: {
    label: 'Security',
    description: 'All security-focused scans',
    patternIds: ['credentials', 'sensitiveData', 'evalDanger', 'dbAntiPattern', 'configDrift', 'innerHtmlXss', 'prototypePollution', 'loggingSecrets', 'insecureRandom'],
  },
  full: {
    label: 'Full',
    description: 'All available scans',
    patternIds: Object.keys(PATTERN_REGISTRY),
  },
  custom: {
    label: 'Custom',
...
function analyzeBuildReadiness(allPaths: string[]): BuildReadinessResult {
  ...
  return {
    ...
    recommendations: missingCritical.length > 0
      ? ['Add all critical files before production deployment.', 'Start with package.json, README, .gitignore, && .env.example.']
...
export async function analyzeWorkspace(
  ...
  const dirPatterns = buildArtifactPatterns.filter(p => !p.startsWith('.') || p === '.git' || p === '.next' || p === '.vscode-test');
  ...
  const allWorkspacePaths: string[] = files.map(f => path.relative(rootPath, f.fsPath));
  ...
  outputChannel.appendLine(`[WorkspaceAnalyzer] Scan complete: ${filesAnalyzed} files analyzed, ${findings.length} findings`);
  ...
  const aiAssistedFindings = findings.filter(f =>
    f.patternId === 'llmSlop' || f.patternId === 'aiResidueStub' || f.patternId === 'aiResidueSwallow'
  ).length;
  ...
  return {
    ...
    gate: {
      ...
      blockingIssues: findings.filter(f => f.severity === 'high' || f.severity === 'critical').map(f => ({
        file: f.file,
        type: f.type,
        severity: f.severity,
...
function resolveActivePatterns(profile: ScanProfile, selectedModules?: string[]): string[] {
  switch (profile) {
    ...
    case 'euai':
      return ['llmSlop', 'aiResidueStub', 'aiResidueSwallow', 'evalDanger', 'dbAntiPattern', 'configDrift', 'sensitiveData', 'credentials'];
    case 'compliance':
      return ['credentials', 'sensitiveData', 'configDrift', 'evalDanger', 'dbAntiPattern', 'loggingSecrets', 'innerHtmlXss', 'prototypePollution'];
    case 'hygiene':
...
function analyzeEuAiAct(allPaths: string[], findings: Finding[]): EuAiActResult {
  ...
  const aiSdkPattern = /(^|\/)(openai|@anthropic-ai|@google\/generative-ai|@google-ai|@huggingface|ollama|langchain|ai-sdk|vertexai|gemini|claude|gpt-4|gpt-3)/i;
  ...
  controls.push({
    ...
    evidence: aiSdkCount > 0 ? `${aiSdkCount} file(s) with AI SDK imports (e.g., openai, @anthropic-ai, @google/generative-ai)` : 'None detected',
    action: aiSdkCount > 0
  ...
  controls.push({
    ...
    evidence: aiSdkCount > 0 ? `${aiSdkCount} AI indicator(s); ${hasGovDocs ? govDocFiles.length + ' governance doc(s) present — verify Annex III classification is explicitly documented' : '0 governance docs — add risk-assessment.md'}` : 'None detected',
    action: aiSdkCount > 0
      ? (hasGovDocs ? 'Review existing governance docs to confirm Annex III classification is explicitly documented. Do not assume presence of docs equals correct classification.' : 'Add risk-assessment.md documenting whether the system is high-risk under Annex III.')
      : 'No action needed.',
  });
  ...
  controls.push({
    ...
    evidence: aiSdkCount > 0 ? `${hasGovDocs ? govDocFiles.length + ' doc(s) present — verify risk management coverage' : 'No risk management documentation detected'}` : 'None detected',
    action: aiSdkCount > 0
