...
export class Web2Panel {
  ...
  private constructor(panel: vscode.WebviewPanel, extUri: vscode.Uri) {
    ...
    this.panel.onDidDispose(() => { Web2Panel.currentPanel = undefined; }, null, []);
    this.panel.webview.onDidReceiveMessage(async (msg) => {
      ...
      switch (msg.command) {
        ...
        case 'updateMaxFiles':
          ...
          break;
        case 'updateExclude':
          await config.update('excludePatterns', msg.value.split('\n').map((s: string) => s.trim()).filter(Boolean), true);
          break;
        case 'updateDashboardPort':
          await config.update('dashboardPort', parseInt(msg.value, 10) || 3001, true);
          break;
        case 'updateApiServerPort':
          await config.update('apiServerPort', parseInt(msg.value, 10) || 3000, true);
          break;
        case 'runScan':
  ...
  private buildHtml(): string {
    ...
    const dashboardPort = config.get<number>('dashboardPort', 3001);
    const apiServerPort = config.get<number>('apiServerPort', 3000);
    const workspaceFolders = vscode.workspace.workspaceFolders;
    const workspaceName = workspaceFolders && workspaceFolders.length > 0 ? workspaceFolders[0].name : 'No workspace open';
  ...
  <div class="card">
    ...
    <div class="card-body">
      <label class="checkbox-row">
        <input type="checkbox" id="autoScan" ${autoScan ? 'checked' : ''} />
      ...
      <div class="form-row">
        ...
        <input type="number" id="maxFiles" value="${maxFiles}" />
      </div>
      <div class="form-row">
        <label for="excludePatterns">Exclude patterns</label>
        <div class="desc">One glob pattern per line (e.g. node_modules, .git, dist).</div>
        <textarea id="excludePatterns">${escapeHtml(excludePatterns.join('\n'))}</textarea>
      </div>
      <div class="form-row">
        <label for="dashboardPort">Dashboard server port</label>
        <div class="desc">Local port for the SimpleBeacon dashboard (coming-soon). Default: 3001.</div>
        <input type="number" id="dashboardPort" value="${dashboardPort}" />
      </div>
      <div class="form-row">
        <label for="apiServerPort">API server port</label>
        <div class="desc">Local port for the SimpleBeacon analysis server (ai-platform). Default: 3000.</div>
        <input type="number" id="apiServerPort" value="${apiServerPort}" />
      </div>
      <div class="btn-group">
...
document.getElementById('saveConfig').addEventListener('click',()=>{
  ...
  vscode.postMessage({
    command:'updateDashboardPort',
    value:document.getElementById('dashboardPort').value
  });
  vscode.postMessage({
    command:'updateApiServerPort',
    value:document.getElementById('apiServerPort').value
  });
});
