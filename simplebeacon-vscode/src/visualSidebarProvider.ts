...
// Enhanced visual sidebar with rich UI elements
export class VisualSidebarProvider implements vscode.TreeDataProvider<VisualNode> {
  ...
  setScanning(isScanning: boolean, progress?: { phase: string; progress: number; total: number; currentFile?: string }) {
  ...
  private createScanningSection(): VisualNode[] {
    ...
    nodes.push(new StatusNode(
      ...
      'loading',
      'Analyzing your codebase for security and quality issues'
    ));

    if (this.scanProgress) {
      nodes.push(new ProgressNode(
  ...
  private createResultsOverview(): VisualNode[] {
    ...
    const severityTotal = r.severityCounts
      ? (r.severityCounts.critical || 0) + (r.severityCounts.high || 0) + (r.severityCounts.medium || 0) + (r.severityCounts.low || 0)
      : 0;
    const categoryTotal = categories.reduce((sum, c) => sum + c.count, 0);
    const totalFindings = severityTotal || categoryTotal || r.issueCount || 0;

    // Score Card
    nodes.push(new ScoreCardNode(
    ...
    // Metrics Grid
    nodes.push(new MetricsGridNode([
      ...
      { label: 'Gate Status', value: gatePass ? 'PASS' : 'FAIL', icon: gatePass ? 'check' : 'error', color: gatePass ? '#10B981' : '#EF4444' }
    ]));
  ...
  private extractCategories(report: unknown): { label: string; count: number; severity: string; icon: string }[] {
    ...
    push('Warning Issues', 'warn', r.gate?.warningIssues, 'warning');
    ...
    // Also use when structured categories undercount vs severityCounts
    const structuredTotal = cats.reduce((sum, c) => sum + c.count, 0);
    const sc = r.severityCounts;
    const severityTotal = sc ? (sc.critical || 0) + (sc.high || 0) + (sc.medium || 0) + (sc.low || 0) : 0;
    if ((cats.length === 0 || severityTotal > structuredTotal) && sc) {
  ...
  private extractAllFindings(report: unknown): { category: string; severity: string; description: string; file: string; line: number }[] {
    const r = report as any;
    ...
    push('Warnings', 'medium', r.gate?.warningIssues);
  ...
  private extractDetailedFileIssues(report: unknown): { file: string; issues: { severity: string; description: string; line: number }[] }[] {
    const r = report as any;
  ...
  private groupFindingsBySeverity(findings: any[]): { label: string; count: number; severity: string; icon: string; children: FindingNode[] }[] {
    ...
    return Object.entries(grouped).map(([severity, items]: [string, any[]]) => {
      ...
      const findingNodes = items.map(item => 
        new FindingNode(
          item.description,
          item.file,
          item.line,
...
// Visual Node Classes
export type VisualNode = HeaderNode | StatusNode | ProgressNode | ScoreCardNode | MetricsGridNode | ActionsGroupNode | QuickActionNode | CategoryNode | CategoryItemNode | FindingNode | ScanDetailsNode | FileIssuesCategoryNode | FileIssuesNode | IssueNode;
...
export class HeaderNode extends vscode.TreeItem {
  constructor(
    ...
    this.iconPath = new vscode.ThemeIcon('shield', new vscode.ThemeColor(this.gatePass ? 'testing.iconPassed' : 'testing.iconFailed'));
...
export class FileIssuesCategoryNode extends vscode.TreeItem {
  ...
  getChildren(): Promise<FileIssuesNode[]> {
    return Promise.resolve(
      this.fileIssues.map(file => 
        new FileIssuesNode(file.file, file.issues)
      )
    );
  }
}
...
export class ScanDetailsNode extends vscode.TreeItem {
  constructor(public readonly report: unknown) {
    ...
    const fileDisplay = totalRepositoryFiles > 0 
      ? `${filesAnalyzed}/${totalRepositoryFiles}` 
      : `${filesAnalyzed}`;
  ...
  getChildren(): Promise<DetailNode[]> {
    ...
    details.push(new DetailNode('Files Analyzed', String(r.ruleScopedFilesAnalyzed || r.filesAnalyzed || r.totalFiles || 0), 'file'));
...
export class DetailNode extends vscode.TreeItem {
  constructor(label: string, public readonly value: string, public readonly icon: string) {
    super(label, vscode.TreeItemCollapsibleState.None);
