...
export class AICodeAnalyzer {
  ...
  async analyzeCode(document: vscode.TextDocument): Promise<CodeAnalysis> {
    ...
    try {
      ...
      this.outputChannel.appendLine(`📊 Found ${analysis.issues.length} issues (${analysis.metrics.autoFixableIssues} auto-fixable)`);
  ...
  private generateSecretFix(line: string): string {
    ...
    return line.replace(/(password|api[_-]?key|token|secret)\s*=\s*["'][^"']+["']/gi, 
      (match, secretType) => {
  ...
  private async applyFix(editor: vscode.TextEditor, issue: CodeIssue): Promise<{success: boolean; message: string; error?: string}> {
    try {
