...
export class SummaryProvider implements vscode.TreeDataProvider<SummaryNode> {
  ...
  private getRootItems(): SummaryNode[] {
    ...
    items.push(new SummaryNode(
      `Files: ${files}${folders ? ` (${folders} folders)` : ''}`,
      'info'
    ));
    ...
    push('Warnings', 'warn', r.gate?.warningIssues);
    ...
    // Fallback: show severity breakdown when no structured categories found
    if (items.length <= 1 && r.severityCounts) {
