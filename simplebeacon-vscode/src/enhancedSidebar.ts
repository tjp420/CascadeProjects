...
// Enhanced UI elements for better sidebar experience
...
export class FindingGroupNode extends vscode.TreeItem {
  ...
  constructor(
    ...
    this.iconPath = new vscode.ThemeIcon(iconMap[severity] || 'info', new vscode.ThemeColor(colorMap[severity] || 'info'));
...
// Utility functions for creating enhanced sidebar content
export class SidebarUIHelper {
  ...
  static createMetricSection(label: string, value: number | string, icon: string, trend?: 'up' | 'down' | 'stable'): MetricNode {
  ...
  static createStatusCard(label: string, status: 'pass' | 'fail' | 'warn' | 'info' | 'loading', details?: string): StatusCardNode {
  ...
  static createFindingGroups(findings: unknown[]): FindingGroupNode[] {
    ...
    return Object.entries(grouped).map(([severity, items]: [string, RawIssue[]]) => {
      const findingNodes = items.map((item: RawIssue) => 
        new FindingNode(
