...
export class EnhancedScanProvider implements vscode.TreeDataProvider<TreeNode> {
  ...
  private getRootNodes(): TreeNode[] {
    ...
    if (this.isScanning && this.scanProgress) {
      nodes.push(
        ...
        new ScanProgressNode(this.scanProgress.phase, this.scanProgress.progress, this.scanProgress.total, this.scanProgress.currentFile)
      );
    } else if (this.report) {
      ...
      nodes.push(
        SidebarUIHelper.createStatusCard('No Scan Data', 'info', 'Run a scan to see results')
      );
    }
  ...
  private createQuickActionsSection(): ActionGroupNode {
    const actions = [
      ...
      SidebarUIHelper.createQuickAction('Generate Certificate', 'simplebeacon.generateCertificate', 'verified', 'Ctrl+Shift+G'),
  ...
  private createResultsSection(): TreeNode[] {
    ...
    const totalFindings = r.issueCount || categories.reduce((sum, c) => sum + c.count, 0);
  ...
  private createRecentScansSection(): TreeNode[] {
    // Show last 3 scans
    return this.recentScans.slice(-3).map(scan => 
  ...
  private extractCategories(report: unknown): { label: string; count: number; severity: 'pass' | 'fail' | 'warn' | 'info' }[] {
    ...
    push('Warning Issues', 'warn', r.gate?.warningIssues);
    ...
    // Fallback: use severity counts when structured category arrays are empty
  ...
  private addToRecentScans(report: unknown) {
    ...
    this.recentScans = this.recentScans.filter(scan => 
      scan.generatedAt !== r.generatedAt
    );
