...
export function generateRoadmap(report: ScanResult): RoadmapResult {
  ...
  phases.push(makePhase(
    ...
    'low',
    [
      { description: 'No security issues detected — credentials && secrets verified.', type: 'verify', done: secFindings === 0 },
      { description: 'Add .env to .gitignore', type: 'fix', done: scanIsClean },
      { description: 'Re-run gate scan', type: 'verify', done: gatePass, codeSnippet: 'npx simplebeacon scan --gate' },
    ],
    secFindings === 0,
  ...
  phases.push(makePhase(
    ...
    'low',
    [
      { description: 'Validate all JSON', type: 'verify', done: scanIsClean, codeSnippet: 'npx simplebeacon scan --json' },
  ...
  phases.push(makePhase(
    ...
    'low',
    [
      { description: 'No debug artifacts or bloat detected — codebase is clean.', type: 'verify', done: cleanupCount === 0 },
    ],
    cleanupCount === 0,
  ...
  phases.push(makePhase(
    ...
    'low',
    [
      { description: 'Add LICENSE file to clarify distribution terms', type: 'fix', done: govCount > 0 || scanIsClean, codeSnippet: 'touch LICENSE' },
      { description: 'Add SECURITY.md to disclose vulnerability reporting', type: 'fix', done: govCount > 0 || scanIsClean, codeSnippet: 'touch SECURITY.md' },
      { description: 'Verify license compatibility with distribution model', type: 'verify', done: scanIsClean, codeSnippet: 'npx license-checker --summary' },
  ...
  // Phase 9: npm Audit
  const hasPackageJson = report.buildReadiness?.checklist?.some((c: { name: string; found: boolean }) => c.name === 'package.json' && c.found) ?? false;
  phases.push(makePhase(
    'npmaudit',
    'Phase 9: npm Audit',
    'low',
    [
      ...
      { description: 'Verify lockfile integrity', type: 'verify', done: !hasPackageJson || scanIsClean, codeSnippet: 'npm ci' },
  ...
  phases.push(makePhase(
    ...
    'low',
    [
      { description: 'Add test coverage for uncovered modules', type: 'fix', done: qualityScore >= 80, codeSnippet: 'npm test -- --coverage' },
      { description: 'Install pre-commit hooks for automated scanning', type: 'fix', done: qualityScore >= 90, codeSnippet: 'npx husky install' },
  ...
  phases.push(makePhase(
    ...
    'low',
    [
      { description: 'Add .simplebeaconignore patterns for temp files', type: 'fix', done: scanIsClean, codeSnippet: 'echo "*.tmp" >> .simplebeaconignore' },
  ...
  phases.push(makePhase(
    ...
    [
      ...
      { description: 'Review dependency update policy', type: 'review', done: true },
      { description: 'Enable Dependabot or Snyx', type: 'fix', done: scanIsClean, codeSnippet: 'Enable Dependabot in repo settings' },
    ],
    scanIsClean,
...
export class RoadmapProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  getChildren(element?: vscode.TreeItem): Thenable<vscode.TreeItem[]> {
    ...
    if (!element) {
      ...
      healthItem.iconPath = new vscode.ThemeIcon(summary.healthScore >= 80 ? 'pass' : summary.healthScore >= 50 ? 'warning' : 'error');
      ...
      const phaseItems = this.roadmap.phases.map(phase => {
        ...
        item.iconPath = new vscode.ThemeIcon(
          phase.status === 'completed' ? 'pass' :
          phase.status === 'blocked' ? 'error' :
          phase.status === 'inProgress' ? 'sync' : 'circle-outline'
        );
