...
export class EnhancedAIProvider implements vscode.TreeDataProvider<EnhancedAINode> {
  ...
  public setScanResult(result: ScanResult | any): void {
    ...
    const isBuildArtifact = (filePath: string, fallbackText?: string): boolean => {
      ...
      const pathMatch = /(^|\/)(node_modules|\.git|dist|build|\.next|out|coverage|frontend-build|vendor)\//i.test(normalized)
        ...
        || /(^|\/)simplebeacon-vscode-merged\//i.test(normalized)
      ...
      if (!normalized && text) {
        return /code-map\.json/i.test(text)
          || /node_modules\/|\.git\/|dist\/|build\/|\.next\/|out\/|coverage\//i.test(text);
      }
      return false;
    };
    ...
    const isCliFalsePositive = (f: Finding): boolean => {
      ...
      if (/(^|\/)(ai-agent|ai-tools|scripts|packages|simplebeacon-vscode-merged)\//i.test(file)) return true;
      ...
      // 48. Unhandled Promise on .then(resolve) is a valid Promise pattern
      if (type === 'unhandled promise' || /unhandled promise/i.test(msg)) {
        if (/\.then\(resolve\)/i.test(snippet)) return true;
        if (/\.then\(reject\)/i.test(snippet)) return true;
      }
      // 49. Error swallowing on console.warn in dashboard view catch blocks
      if (type === 'error swallowing' || /error swallowing/i.test(msg)) {
        if (/catch.*console\.(warn|error|log)/i.test(snippet)) return true;
        if (/dashboard.*js\/views\//i.test(file)) return true;
      }
      // 50. Magic numbers in CSS/config/timeout defaults
      if (type === 'magic number' || /magic number/i.test(msg)) {
        if (/timeout\s*=\s*\d+/i.test(snippet)) return true;
        if (/maxRedirects\s*=\s*\d+/i.test(snippet)) return true;
        if (/font-size:|padding:|margin:|width:|height:/i.test(snippet)) return true;
        if (/score\s*===?\s*100/i.test(snippet)) return true;
        if (/percent\s*>=?\s*100/i.test(snippet)) return true;
      }
      return false;
    };
    ...
    if (result && result.findings && result.summary) {
      ...
      this.scanResult = {
        ...
        summary: {
          ...
          severityCounts: filtered.reduce((acc: Record<string, number>, f: Finding) => {
            acc[f.severity] = (acc[f.severity] || 0) + 1;
            return acc;
          }, { critical: 0, high: 0, medium: 0, low: 0 })
        },
        categories: filtered.reduce((acc: Record<string, Finding[]>, f: Finding) => {
  ...
  private initializeDefaultData() {
    ...
    this.modelHealth = {
      overall: 'unknown',
      models: [
        { id: 'demo', name: 'Demo Model', provider: 'demo', available: true, confidence: 0.5 }
      ]
    };
  ...
  private getRootNodes(): EnhancedAINode[] {
    ...
    nodes.push(new QuickActionNode(
  ...
  async startEnhancedAnalysis(profile?: ScanProfile): Promise<void> {
    ...
    try {
      ...
      if (workspaceFolder) {
        ...
        vscode.window.showInformationMessage(`Workspace analysis complete: ${result?.summary?.totalFindings ?? 0} issues found across ${result?.summary?.filesAnalyzed ?? 0} files`);
      } else if (this.scanResult) {
        // No workspace open but we have existing scan data (e.g. from CLI)
        this.outputChannel.appendLine(`[EnhancedAI] No workspace open; using existing scan result with ${this.scanResult.summary?.totalFindings ?? 0} findings`);
        result = this.scanResult;
        vscode.window.showInformationMessage(`Using existing scan data. Open the workspace folder to run live analysis.`);
      } else {
      ...
      this.outputChannel.appendLine(`[EnhancedAI] Analysis complete: ${result.summary?.filesAnalyzed || 0} files, ${result.summary?.totalFindings || 0} findings`);
  ...
  public convertScanResultToReport(result: ScanResult): unknown {
    const s = result.summary || { totalFiles: 0, filesAnalyzed: 0, totalFindings: 0, severityCounts: { critical: 0, high: 0, medium: 0, low: 0 }, categoryCounts: {} };
    ...
    return {
      ...
      detectedIssues: result.findings || [],
      rawIssues: result.findings || [],
  ...
  private async callPatternDetectionAPI(content: string, filePath: string): Promise<PatternResult[]> {
    ...
    if (lower.includes('import') && lower.includes('export')) {
      patterns.push({ category: 'architecture', type: 'esm-module', description: 'ES module structure detected', confidence: 0.7, location: filePath });
    }
    if (lower.includes('function') || lower.includes('=>')) {
      patterns.push({ category: 'architecture', type: 'functional-style', description: 'Functional programming patterns detected', confidence: 0.6, location: filePath });
    }
    if (lower.includes('try') && lower.includes('catch')) {
      patterns.push({ category: 'security', type: 'error-handling', description: 'Error handling pattern detected', confidence: 0.8, location: filePath });
    }
    return patterns;
  }
  ...
  private async callModelHealthAPI(): Promise<ModelHealthStatus> {
    ...
    if (!ollamaUrl) {
      return {
        ...
        models: [
          { id: 'local-default', name: modelName, provider: 'none', available: false, confidence: 0 }
        ]
      };
    }
    try {
  ...
  private startRealtimeMonitoring(): void {
    if (this.sessionMonitorInterval) { return; }
    this.sessionMonitorInterval = setInterval(() => {
      if (this.analysisSessions.size > 0) {
        const now = Date.now();
        for (const [id, session] of this.analysisSessions) {
          if (now - session.createdAt > 3600000) { // 1 hour stale
  ...
  private getAnalysisResultsHtml(results: ScanResult): string {
    const summary = results.summary || { totalFiles: 0, filesAnalyzed: 0, totalFindings: 0, severityCounts: { critical: 0, high: 0, medium: 0, low: 0 }, categoryCounts: {} };
...
// Node Classes
export type EnhancedAINode = ModelHealthNode | StatusNode | SessionsHeaderNode | AnalysisSessionNode |
  PatternHeaderNode | PatternCategoryNode | PatternItemNode | FindingCategoryNode | FindingItemNode | ActionsHeaderNode | QuickActionNode;
...
export class ModelHealthNode extends vscode.TreeItem {
  constructor(public readonly health: ModelHealthStatus) {
    ...
    this.iconPath = new vscode.ThemeIcon('heart', new vscode.ThemeColor(
      health.overall === 'healthy' ? 'testing.iconPassed' : 'testing.iconFailed'
    ));
  ...
  getChildren(): Thenable<ModelItemNode[]> {
    return Promise.resolve(
      this.health.models.map(model => new ModelItemNode(model))
    );
  }
}
...
export class StatusNode extends vscode.TreeItem {
  constructor(label: string, public readonly status: string, public readonly description?: string) {
...
export class SessionsHeaderNode extends vscode.TreeItem {
  constructor(label: string, public readonly count: number) {
...
export class SessionDetailNode extends vscode.TreeItem {
  constructor(label: string, public readonly value: string) {
    super(label, vscode.TreeItemCollapsibleState.None);
...
export class FindingItemNode extends vscode.TreeItem {
  ...
  constructor(finding: Finding) {
    ...
    this.iconPath = new vscode.ThemeIcon('warning', new vscode.ThemeColor(colorMap[finding.severity] || 'testing.iconQueued'));
    this.command = {
      command: 'vscode.open',
      title: 'Open File',
      arguments: [vscode.Uri.file(finding.file), { selection: new vscode.Range((finding.matches[0]?.line || 1) - 1, 0, (finding.matches[0]?.line || 1) - 1, 0) }]
    };
  }
}
