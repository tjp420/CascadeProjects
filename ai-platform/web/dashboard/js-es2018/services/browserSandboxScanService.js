...
const SKIP_FILE_PATTERNS = [/^headers\d*\.txt$/i, /^audit\.log$/i, /^.*\.log$/i];
...
async function crawlSandboxedTree(dirHandle, currentPath, queue, options) {
  ...
  for await (const [name, handle] of dirHandle.entries()) {
    ...
    if (
      ignoreCtx &&
      isIgnoredVirtualPath(nextVirtualPath, ignoreCtx.scanRootName, ignoreCtx.patterns)
    ) {
      continue;
    }
...
function gradeFindings(highRiskCount, mediumRiskCount, criticalCount, lowRiskCount) {
  ...
  return {
    ...
    complianceStatus:
      letterGrade === 'F' ? 'NON-COMPLIANT (CRITICAL DEBT)' : 'APPROVED FOR PRODUCTION RELEASE',
  };
}
...
function pickLegacyDirectory({ maxFiles, onLog }) {
  return new Promise((resolve, reject) => {
    ...
    input.addEventListener('change', async (e) => {
      ...
      const rootName =
        (files[0].webkitRelativePath || files[0].name).split('/')[0] || 'selected-folder';
      logLine(
        onLog,
      ...
      for (const file of files) {
        ...
        if (
          ignoreCtx &&
          isIgnoredVirtualPath(virtualPath, ignoreCtx.scanRootName, ignoreCtx.patterns)
        )
          continue;
...
async function analyzeDirectory(
  ...
  for (let i = 0; i < filteredQueue.length; i++) {
    ...
    fileReport.push({
      ...
      size: result.size,
      status:
        result.fileIssues.length > 0 ? `Issues Flagged: ${result.fileIssues.join(', ')}` : 'Clean',
    });
...
async function crawlWebkitEntryTree(entry, currentPath, queue, options) {
  ...
  if (!entry.isDirectory) {
    if (
      ignoreCtx &&
      isIgnoredVirtualPath(currentPath, ignoreCtx.scanRootName, ignoreCtx.patterns)
    ) {
      return;
    }
  ...
  do {
    batch = await new Promise((resolve, reject) =>
      reader.readEntries(resolve, (err) => {
        logLine(onLog, `readEntries error at ${currentPath}: ${err}`, 'warning');
        resolve([]);
      })
    );
    for (const child of batch) {
      ...
      if (
        ignoreCtx &&
        isIgnoredVirtualPath(childPath, ignoreCtx.scanRootName, ignoreCtx.patterns)
      ) {
        continue;
      }
