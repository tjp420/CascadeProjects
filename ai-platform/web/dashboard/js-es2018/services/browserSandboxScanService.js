...
} from "../utils-lib/dom.js?v=20260721corsfix1";
import {
  createIgnoreContext,
...
function detectSimplebeaconMonorepo(scanRootName, fileQueue) {
  ...
  if (
    /^(coming-soon|ai-platform|simplebeacon-vscode-merged|CascadeProjects(?:_BACKUP_\d+)?)$/i.test(
      root,
    )
  ) {
    return true;
...
const RULES = [
  {
    ...
    severity: "HIGH",
    regex:
      /(sk_live_[a-zA-Z0-9]{24,}|sk_test_[a-zA-Z0-9]{24,}|AKIA[0-9A-Z]{16}|ASIA[0-9A-Z]{16}|ghp_[a-zA-Z0-9]{36}|github_pat_[a-zA-Z0-9_]{22,}|xoxb-[0-9]{10,}-[0-9]{10,}-[a-zA-Z0-9]{24}|xoxp-[0-9]{10,}-[0-9]{10,}-[a-zA-Z0-9]{24}|SG\.[a-zA-Z0-9_\-]{22}\.[a-zA-Z0-9_\-]{43}|private[_\-]?key|-----BEGIN (RSA |DSA |EC |OPENSSH )?PRIVATE KEY-----)/gi,
...
async function crawlSandboxedTree(dirHandle, currentPath, queue, options) {
  ...
  if (SKIP_DIRS.has(dirHandle.name) || SKIP_DOT_DIRS.has(dirHandle.name)) {
    logLine(
      onLog,
      `Skipping dependency/build directory: ${currentPath}`,
      "info",
    );
    return;
  }
  ...
  for await (const [name, handle] of dirHandle.entries()) {
    if (queue.length >= maxFiles) {
      logLine(
        onLog,
        `Reached max file limit (${maxFiles}); stopping traversal.`,
        "warning",
      );
      break;
    }
...
function gradeFindings(
  ...
  const estimatedLiability =
    crit * 100000 + highRiskCount * 25000 + mediumRiskCount * 1250;
...
function analyzeFile(content, virtualPath) {
  ...
  if (
    ...
    shouldSkipSandboxScanFile(virtualPath, _monorepoFlag)
  ) {
    return { fileIssues, fileFindings };
  }

  for (const rule of RULES) {
    if (rule.id === "SB-05" && shouldSkipSandboxComplianceDrift(virtualPath))
      continue;
...
async function pickFileSystemAccessDirectory({ maxFiles, onLog }) {
  ...
  logLine(
    onLog,
    `Access granted. Initializing scan over boundary: ${rootName}`,
    "info",
  );
  const ignoreLoad = await loadIgnorePatternsFromDirHandle(directoryHandle);
  const ignoreCtx = createIgnoreContext(
    ignoreLoad.patterns,
    rootName,
    ignoreLoad.source,
    ignoreLoad.isSimplebeaconMonorepo,
  );
  logLine(
    onLog,
  ...
  const fileQueue = [];
  await crawlSandboxedTree(directoryHandle, rootName, fileQueue, {
    maxFiles,
    onLog,
    ignoreCtx,
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    input.addEventListener("change", async (e) => {
      ...
      if (files.length === 0) {
        reject(
          new Error(
            "Directory selection was cancelled or no files were chosen.",
          ),
        );
        return;
      }
      ...
      logLine(
        onLog,
        `Legacy directory input selected. Streaming analysis over ${files.length} items...`,
        "info",
      );
      ...
      for (const file of files) {
        ...
        if (
          pathParts.some(
            (part) => SKIP_DIRS.has(part) || SKIP_DOT_DIRS.has(part),
          )
        )
          continue;
        ...
        const ext =
          extIndex >= 0 ? file.name.substring(extIndex).toLowerCase() : "";
        if (!ALLOWED_EXTENSIONS.has(ext)) continue;
...
function createScanWorker() {
  ...
  try {
    return new Worker(
      new URL("../workers/scan-worker.js?v=20260728dropfix2", import.meta.url),
    );
  } catch (err) {
    window["console"]["warn"](
...
async function analyzeDirectory(
  { rootName, fileQueue, ignoreCtx },
  { maxFileSize, onLog, onProgress },
) {
  ...
  if (ignoreCtx && filteredQueue.length < fileQueue.length) {
    logLine(
      onLog,
      `Excluded ${fileQueue.length - filteredQueue.length} paths via .simplebeaconignore.`,
      "info",
    );
  }
  ...
  if (worker) {
    logLine(
      onLog,
      "Using background scan worker to keep the UI responsive.",
      "info",
    );
    ...
    worker.onmessageerror = (err) => {
      window["console"]["error"](
        "[SimpleBeacon] Scan worker message error:",
        err,
      );
      for (const [virtualPath, resolve] of pending) {
        resolve({
    ...
    for (let i = 0; i < filteredQueue.length; i++) {
      ...
      try {
        ...
        if (file.size > maxFileSize) {
          logLine(
            onLog,
            `Large file ${item.virtualPath} (${file.size.toLocaleString()} bytes) — scanning first ${maxFileSize.toLocaleString()} bytes.`,
            "info",
          );
        ...
        logLine(
          onLog,
          `Could not read ${item.virtualPath}: ${err.message}`,
          "warning",
        );
      }
    }
    ...
    logLine(
      onLog,
      "Scan worker not available; running scan on the main thread.",
      "warning",
    );
    for (let i = 0; i < filteredQueue.length; i++) {
      const item = filteredQueue[i];
      try {
        ...
        const { fileIssues, fileFindings } = analyzeFile(
          content,
          item.virtualPath,
        );
    ...
    fileReport.push({
      ...
      status:
        result.fileIssues.length > 0
          ? `Issues Flagged: ${result.fileIssues.join(", ")}`
          : "Clean",
    });
  ...
  const certificate = gradeFindings(
    highRiskCount,
    mediumRiskCount,
    criticalCount,
    lowRiskCount,
  ...
  if (globalIssuesQueue.length >= MAX_FINDINGS) {
    ...
    logLine(
      onLog,
      `Findings capped at ${MAX_FINDINGS.toLocaleString()} for browser memory. Use CLI export for full list.`,
      "warning",
    );
  }
  ...
  logLine(
    onLog,
    `Sandboxed drive sweep complete. Grade ${certificate.letterGrade} | ${fileReport.length}/${filteredQueue.length} files (${skippedLarge + skippedError} skipped).`,
    "success",
  );
...
export async function runSandboxedDirectoryScan(options = {}) {
  ...
  if (isSupported()) {
    try {
      ...
      if (isFilePickerBlockedError(err)) {
        logLine(
          onLog,
          "Native folder picker blocked in embed — using legacy folder dialog.",
          "warning",
        );
  ...
  logLine(
    onLog,
    `Discovered ${picked.fileQueue.length} text/code targets.`,
    "info",
  );
...
export async function scanDroppedItems(items, options = {}) {
  ...
  if (!items || items.length === 0) {
    throw new Error(
      "No items were dropped. Drop a folder or supported code files to scan.",
    );
  }
  ...
  try {
    firstFile =
      first && typeof first.getAsFile === "function" ? first.getAsFile() : null;
  } catch (_a) {
    firstFile = null;
  }
  ...
  if (entry && entry.isDirectory) {
    ...
    const fileQueue = [];
    await crawlWebkitEntryTree(entry, entry.name, fileQueue, {
      maxFiles,
      onLog,
      ignoreCtx,
    });
    ...
    logLine(
      onLog,
      `Dropped directory "${entry.name}" — ${fileQueue.length} targets queued.`,
      "info",
    );
    return analyzeDirectory(
      { rootName: entry.name, fileQueue, ignoreCtx },
      { maxFileSize, onLog, onProgress },
    );
  }
  ...
  if (typeof first.getAsFileSystemHandle === "function") {
    try {
      ...
      if (handle && handle.kind === "directory") {
        ...
        const fileQueue = [];
        await crawlSandboxedTree(handle, handle.name, fileQueue, {
          maxFiles,
          onLog,
          ignoreCtx,
        });
        ...
        logLine(
          onLog,
          `Dropped directory "${handle.name}" — ${fileQueue.length} targets queued.`,
          "info",
        );
        return analyzeDirectory(
          { rootName: handle.name, fileQueue, ignoreCtx },
          { maxFileSize, onLog, onProgress },
        );
      }
    } catch (_a) {
      /* fall through to stale webkit entry / file fallback */
    }
  }

  // Fallback: webkitGetAsEntry traversal (may already be stale if not captured synchronously).
  const staleEntry =
    typeof first.webkitGetAsEntry === "function"
      ? first.webkitGetAsEntry()
      : null;
  if (staleEntry && staleEntry.isDirectory) {
    const ignoreCtx = createIgnoreContext(null, staleEntry.name, "builtin");
    const fileQueue = [];
    await crawlWebkitEntryTree(staleEntry, staleEntry.name, fileQueue, {
      maxFiles,
      onLog,
      ignoreCtx,
    });
    ...
    logLine(
      onLog,
      `Dropped directory "${staleEntry.name}" — ${fileQueue.length} targets queued.`,
      "info",
    );
    return analyzeDirectory(
      { rootName: staleEntry.name, fileQueue, ignoreCtx },
      { maxFileSize, onLog, onProgress },
    );
  }
  ...
  logLine(
    onLog,
    `Dropped ${fileQueue.length} file(s) — scanning locally.`,
    "info",
  );
  ...
  return analyzeDirectory(
    { rootName: name, fileQueue, ignoreCtx },
    { maxFileSize, onLog, onProgress },
  );
}
  ...
  do {
    batch = await new Promise((resolve, reject) =>
      reader.readEntries(resolve, (err) => {
        logLine(
          onLog,
          `readEntries error at ${currentPath}: ${err}`,
          "warning",
        );
        resolve([]);
      }),
