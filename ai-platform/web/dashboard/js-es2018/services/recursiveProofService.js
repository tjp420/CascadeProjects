...
function _ensureBuffer(counterKey) {
    if (!_history.has(counterKey)) _history.set(counterKey, createRingBuffer(MAX_SAMPLES));
...
export async function fetchRecursiveProofTelemetry() {
    try {
        ...
        if (!response.ok) {
            if (response.status === 403) return { status: 'forbidden', counters: {}, stats: {} };
            if (response.status === 404) return { status: 'unavailable', counters: {}, stats: {} };
            throw new Error('HTTP ' + response.status + ': ' + response.statusText);
        }
        const data = await response.json();
        if (data.success !== true) {
            throw new Error(data.error || 'Failed to retrieve recursive proof telemetry');
        }
        const counters = data.counters || {};
        _recordSnapshot(counters);
        return { status: 'success', counters, stats: data.stats || {}, timestamp: data.timestamp };
    }
    catch (error) {
        ...
        window["console"]["error"]('[recursiveProofService] Error fetching telemetry:', msg);
        throw error;
    }
}
