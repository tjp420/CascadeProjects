...
export async function fetchSwarmRoboticsKineticAssemblyPolicy() {
    const response = await fetch('/api/vault/swarm-robotics-kinetic-assembly/policy');
...
export async function validateSwarmRoboticsKineticAssemblyPolicy(payload) {
    const response = await fetch('/api/vault/swarm-robotics-kinetic-assembly/policy/validate', {
        method: 'POST',
...
export async function fetchSwarmRoboticsKineticAssemblyTelemetry() {
    const response = await fetch('/api/vault/swarm-robotics-kinetic-assembly/telemetry');
    if (!response.ok) throw new Error('HTTP ' + response.status + ': ' + response.statusText);
    const data = await response.json();
