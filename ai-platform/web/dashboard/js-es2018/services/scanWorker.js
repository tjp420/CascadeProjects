...
const SIGNATURE_ENGINE = [
  ...
  },
  {
    ...
    severity: "LOW",
    regex:
      /catch\s*\(\s*\w+\s*\)\s*\{\s*\/*\s*(TODO|FIXME|ignore)?\s*\*\/\s*\}/g,
...
function shouldSkipSandboxScanFile(virtualPath, isSimplebeaconMonorepo) {
  ...
  if (/(?:^|\/)(?:node_modules|\.git|\.simplebeacon)(?:\/|$)/i.test(normalized))
    return true;
...
function shouldSkipRuleLine(ruleId, virtualPath, line) {
  ...
  if (ruleId === "SB-05" && shouldSkipSandboxComplianceDrift(virtualPath))
    return true;
  if (!CREDENTIAL_RULE_IDS.has(ruleId)) return false;
  const normalized = normalizeSandboxScanPath(virtualPath);
  if (isTestOrFixturePath(normalized) || isComplianceToolingPath(normalized))
    return true;
  if (/^\s*\/\//.test(line) && /private[_\-]?key|signing key/i.test(line))
    return true;
...
function isCommonFalsePositive(str) {
  ...
  if (/={2}$/.test(str) || /P={2}$/.test(str) || /A={2}$/.test(str))
    return true;
