...
const WORKER_URL = new URL(
  "../workers/scan-worker.js?v=20260728dropfix2",
  import.meta.url,
);
...
async function* listDirEntries(dirHandle) {
  if (typeof dirHandle.entries === "function") {
    ...
    do {
      batch = await new Promise((resolve, reject) => {
        reader.readEntries(resolve, (err) =>
          reject(err || new Error("readEntries failed")),
        );
      });
    ...
    throw new Error(
      `Directory handle does not support entries() or createReader(): ${dirHandle.name || ""}`,
    );
  }
}
/**
...
async function collectFiles(dirHandle, options = {}) {
  ...
  if (SKIP_DIRS.test(pathPrefix)) return files;
  let entryCount = 0;
  for await (const { name, kind, handle, getFile } of listDirEntries(
    dirHandle,
  )) {
    ...
    if (kind === "directory") {
      ...
      if (getFile) {
        try {
          ...
          window["console"]["warn"](
            `[localScan] Could not read legacy file ${fullPath}: ${err && err.message}`,
          );
          continue;
        }
      }
  ...
  if (typeof window !== "undefined") {
    window["console"]["log"](
      `[localScan] dir "${dirHandle.name || pathPrefix || "?"}" entries=${entryCount} runningFiles=${files.length} folders=${stats.folders}`,
    );
  }
  return files;
}
  ...
  return {
    ...
    scanLimitNote: meta.issuesTruncated
      ...
      : totalFiles >= MAX_FILES
        ? `File inventory capped at ${MAX_FILES.toLocaleString()} files. Use the CLI for full monorepo coverage.`
        : null,
  };
}
/**
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = async (e) => {
      ...
      if (type === "complete") {
        ...
        const computedFolderCount = countFoldersFromPaths(
        ...
        if (typeof window !== "undefined") {
          window["console"]["log"](
            `[localScan] worker complete: total=${resolvedTotal}, processed=${processed || 0}, analyzed=${analyzedFiles}, folders=${folderCount}, binarySkipped=${binarySkipped || 0}, textErrors=${textErrors || 0}, ignoredDir=${ignoredDir || 0}, ignoredByPattern=${ignoredByPattern || 0}, heavyVendor=${heavyVendor || 0}`,
          );
        }
        resolve(
          buildReport(
    ...
    worker.postMessage({
      ...
      ignoreCtx: ignoreCtx
        ? {
            ...
            isSimplebeaconMonorepo: ignoreCtx.isSimplebeaconMonorepo,
          }
        : null,
    });
    (async () => {
...
export async function runLocalScan(options = {}) {
  ...
  if (typeof window !== "undefined") {
    window["console"]["log"](
      `[localScan] start: projectPath=${options.projectPath || ""}, dirHandleName=${options.dirHandle?.name || ""}, droppedFiles=${options.files?.length || 0}`,
    );
  }
  if (options.files && options.files.length) {
    ...
    if (!dirHandle) {
      ...
      if (options.projectPath && picked.name) {
        ...
        if (
          expectedBasename &&
          picked.name.toLowerCase() !== expectedBasename.toLowerCase()
        ) {
          window["console"]["warn"](
            `[localScan] folder picker name "${picked.name}" does not match typed path basename "${expectedBasename}". The selected folder may not be the intended one.`,
          );
        }
      }
      ...
      files = await collectFiles(picked, {
        files: [],
        ignoreCtx,
        stats: folderStats,
      });
  ...
  if (typeof window !== "undefined") {
    window["console"]["log"](
      `[localScan] files before ignore filter: ${files.length}${folderCount ? `, folders=${folderCount}` : ""}`,
    );
  }
  ...
  if (typeof window !== "undefined") {
    window["console"]["log"](
      `[localScan] files after ignore filter: ${files.length}`,
    );
  }
  ...
  if (files.length >= MAX_FILES) {
    showToast(
      `Large repo — scanning first ${MAX_FILES.toLocaleString()} files. Use CLI for unlimited coverage.`,
      "warning",
      { duration: 8000 },
    );
