...
export async function fetchBioDigitalInterfaceNeuralTelemetryPolicy() {
  const response = await fetch(
    "/api/vault/bio-digital-neural-telemetry/policy",
  );
  if (!response.ok)
...
export async function validateBioDigitalInterfaceNeuralTelemetryPolicy(
  payload,
) {
  const response = await fetch(
    "/api/vault/bio-digital-neural-telemetry/policy/validate",
    {
      method: "POST",
...
export async function fetchBioDigitalInterfaceNeuralTelemetryTelemetry() {
  const response = await fetch(
    "/api/vault/bio-digital-neural-telemetry/telemetry",
  );
  if (!response.ok)
    throw new Error("HTTP " + response.status + ": " + response.statusText);
  const data = await response.json();
