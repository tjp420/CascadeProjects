...
import {
  apiBase,
  getToken,
} from "../services/authService.js?v=20260720adminfix1";

export async function fetchMultiEnclaveConfidentialMeshStateReconciliationPolicy() {
  const res = await fetch(
    `${apiBase}/api/vault/multi-enclave-confidential-mesh-state-reconciliation/policy`,
    {
...
export async function validateMultiEnclaveConfidentialMeshStateReconciliationPolicy(
  config,
) {
  const res = await fetch(
    `${apiBase}/api/vault/multi-enclave-confidential-mesh-state-reconciliation/policy/validate`,
    {
      method: "POST",
      headers: {
...
export async function fetchMultiEnclaveConfidentialMeshStateReconciliationTelemetry() {
  const res = await fetch(
    `${apiBase}/api/vault/multi-enclave-confidential-mesh-state-reconciliation/telemetry`,
    {
      headers: { Authorization: `Bearer ${getToken()}` },
    },
  );
