...
export class AuthService {
  ...
  setSession(token, user, options = {}) {
    ...
    if (typeof window !== 'undefined' && window.parent === window) {
      try {
        ...
        if (
          redirectUri &&
          redirectUri.startsWith('vscode://simplebeacon.simplebeacon-vscode/relay/auth')
        ) {
  ...
  isAuthenticated() {
    ...
    return (
      Boolean((_a = this.user) === null || _a === void 0 ? void 0 : _a.vaultSession) &&
      !_isCloudApiBase()
    );
  }
  getAuthHeaders() {
  ...
  async login(email, password) {
    ...
    if (
      ...
      throw new Error(
        loginErrorMessage(loginHttpResponse, loginResponseBody, 'Login response missing token')
      );
    }
  ...
  isTokenActivated(token) {
    const binding = this.getTokenBinding(token);
    return (
      (binding === null || binding === void 0 ? void 0 : binding.tokenClass) === 'account' || false
    );
  }
  /**
  ...
  async probeVaultOperatorSession() {
    ...
    if (
      res.status === 403 &&
      (body === null || body === void 0 ? void 0 : body.error) === 'vault_required'
    )
      return false;
