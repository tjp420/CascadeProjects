...
export function getBrowserBuiltinIgnorePatterns(isSimplebeaconMonorepo) {
  if (isSimplebeaconMonorepo) {
    return [
      ...BROWSER_BUILTIN_IGNORE_GENERIC,
      ...BROWSER_BUILTIN_IGNORE_SIMPLEBEACON,
    ];
  }
...
export function shouldSkipSandboxScanFile(virtualPath, isSimplebeaconMonorepo) {
  ...
  if (/(?:^|\/)(?:tests?|fixtures?|mocks?)(?:\/|$)/i.test(normalized))
    return true;
  ...
  if (isSimplebeaconMonorepo) {
    if (
      ...
      return true;
    if (
      /(?:^|\/)(?:simplebeacon-rule-tests|guardrail-test-bench)(?:\/|$)/i.test(
        normalized,
      )
    )
      return true;
    if (/(?:^|\/)(?:scan-exports|out|\.vscode-test)(?:\/|$)/i.test(normalized))
      return true;
    ...
    if (
      ...
      return true;
    if (
      /(?:^|\/)packages\/simplebeacon-cli\/src\/(?:compliance-rules|proxy)\//i.test(
        normalized,
      )
    )
      return true;
    if (/(?:^|\/)packages\/simplebeacon-intelligence\//i.test(normalized))
      return true;
    ...
    if (/src\/api\/billing\/email-templates\.cjs$/i.test(normalized))
      return true;
    if (/src\/core\/GlobalContextManager\.cjs$/i.test(normalized)) return true;
    if (/(?:^|\/)simplebeacon-vscode-merged(?:\/|$)/i.test(normalized))
      return true;
    ...
    if (/(?:^|\/)server\/middleware\/security\.cjs$/i.test(normalized))
      return true;
    if (/(?:^|\/)server\/lib\/flexible-analyze-utils\.cjs$/i.test(normalized))
      return true;
    if (
      /(?:^|\/)server\/lib\/code-understanding\/semantic-analyzer\.cjs$/i.test(
        normalized,
      )
    )
      return true;
    if (/(?:^|\/)server\/lib\/file-audit-context\.cjs$/i.test(normalized))
      return true;
    if (
      /(?:^|\/)server\/lib\/trust-verification-payload\.cjs$/i.test(normalized)
    )
      return true;
    if (
      /(?:^|\/)server\/lib\/language-patterns\/(?:go|rust|sql)-patterns\.cjs$/i.test(
        normalized,
      )
    )
      return true;
    ...
    if (/(?:^|\/)ai-platform\/start-dashboard\.bat$/i.test(normalized))
      return true;
    if (/(?:^|\/)ai-platform\/test-output\.txt$/i.test(normalized)) return true;
    if (/(?:^|\/)ai-platform\/test-patch\.bat$/i.test(normalized)) return true;
    if (/(?:^|\/)web\/simplebeacon-dashboard\/css\//i.test(normalized))
      return true;
    if (/(?:^|\/)web\/simplebeacon-dashboard\/index\.html$/i.test(normalized))
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/config\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/demoMode\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/services\/aiKeysService\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/services\/scanService\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/utils\/funnelTrigger\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/components\/ScanStatus\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/utils-lib\/ideDeepLink\.js$/i.test(
        normalized,
      )
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/utils-lib\/test-out\.txt$/i.test(
        normalized,
      )
    )
      return true;
    ...
    if (/(?:^|\/)simplebeacon-guardrails-public\//i.test(normalized))
      return true;
    if (/simplebeacon-workflow\.ps1$/i.test(normalized)) return true;
    if (
      /(?:^|\/)packages\/simplebeacon-cli\/test-output\.txt$/i.test(normalized)
    )
      return true;
    if (/(?:^|\/)web\/simplebeacon-dashboard\/package\.json$/i.test(normalized))
      return true;
...
export function isIgnoredVirtualPath(
  ...
  return pathMatchCandidates(virtualPath, scanRootName).some((rel) =>
    isIgnoredPath(rel, ignorePatterns),
  );
}
...
export function createIgnoreContext(
  ...
  const resolved = hasPatterns
    ? patterns.slice()
    : getBrowserBuiltinIgnorePatterns(isSimplebeaconMonorepo);
  return {
    patterns: resolved,
...
export function filterQueueByIgnore(fileQueue, ignoreCtx) {
  if (!ignoreCtx?.patterns?.length || !Array.isArray(fileQueue))
    return fileQueue || [];
...
export async function loadIgnorePatternsFromDirHandle(dirHandle) {
  const isSimplebeaconMonorepo =
    await detectSimplebeaconMonorepoAsync(dirHandle);
  ...
  try {
    ...
    if (patterns.length)
      return { patterns, source: "simplebeaconignore", isSimplebeaconMonorepo };
  } catch {
...
export async function extractIgnorePatternsFromLegacyFiles(files) {
  ...
  const ignoreFile = list.find((file) => {
    ...
    return (
      /(?:^|\/)\.simplebeaconignore$/i.test(path) ||
      path.endsWith(".simplebeaconignore")
    );
  });
  if (ignoreFile) {
    try {
