...
import {
  ...
  downloadJson,
  setHtml,
} from "../utils.js?v=20260720adminfix1";
import {
  renderCoreReplicationTelemetryDashboard,
  cleanupCoreReplicationTelemetryDashboard,
} from "../components/CoreReplicationTelemetryDashboard.js";
import {
  renderDropTelemetryDashboard,
  cleanupDropTelemetryDashboard,
} from "../components/DropTelemetryDashboard.js";
import {
  renderEnclaveTelemetryDashboard,
  cleanupEnclaveTelemetryDashboard,
} from "../components/EnclaveTelemetryDashboard.js";
import {
  renderRecursiveProofAggregationDashboard,
  cleanupRecursiveProofAggregationDashboard,
} from "../components/RecursiveProofAggregationDashboard.js";
import {
  renderDecentralizedIdentityGatingDashboard,
  cleanupDecentralizedIdentityGatingDashboard,
} from "../components/DecentralizedIdentityGatingDashboard.js";
import {
  renderSpaceBasedLaserCommunicationMeshGatingDashboard,
  cleanupSpaceBasedLaserCommunicationMeshGatingDashboard,
} from "../components/SpaceBasedLaserCommunicationMeshGatingDashboard.js";
import {
  renderQkdLinkSwitchGatingDashboard,
  cleanupQkdLinkSwitchGatingDashboard,
} from "../components/QkdLinkSwitchGatingDashboard.js";
import {
  renderHolographicStorageContentAddressableGatingDashboard,
  cleanupHolographicStorageContentAddressableGatingDashboard,
} from "../components/HolographicStorageContentAddressableGatingDashboard.js";
import {
  renderSupplyChainProvenanceGatingDashboard,
  cleanupSupplyChainProvenanceGatingDashboard,
} from "../components/SupplyChainProvenanceGatingDashboard.js";
import {
  renderZkDecentralizedStorageAttestationGatingDashboard,
  cleanupZkDecentralizedStorageAttestationGatingDashboard,
} from "../components/ZkDecentralizedStorageAttestationGatingDashboard.js";
import {
  renderBioDigitalInterfaceNeuralTelemetryDashboard,
  cleanupBioDigitalInterfaceNeuralTelemetryDashboard,
} from "../components/ZkBioDigitalInterfaceNeuralTelemetryDashboard.js";
import {
  renderSwarmRoboticsKineticAssemblyDashboard,
  cleanupSwarmRoboticsKineticAssemblyDashboard,
} from "../components/ZkSwarmRoboticsKineticAssemblyDashboard.js";
import {
  renderMultiEnclaveConfidentialMeshStateReconciliationDashboard,
  cleanupMultiEnclaveConfidentialMeshStateReconciliationDashboard,
} from "../components/ZkMultiEnclaveConfidentialMeshStateReconciliationDashboard.js";
...
export class AdminPanelView {
  ...
  refreshPaginationState() {
    const totalPages = Math.max(
      1,
      Math.ceil((Number(this.totalUsers) || 0) / (this.pageLimit || PAGE_SIZE)),
    );
  ...
  adminFetch(path, options = {}) {
    ...
    try {
      ...
      const sameOrigin =
        typeof location !== "undefined" ? apiOrigin === location.origin : true;
  ...
  buildUsersQuery() {
    ...
    if (this.trustFilter && this.trustFilter !== "all")
      params.set("trust", this.trustFilter);
    if (this.pageIndex > 1) {
  ...
  async loadData(options = {}) {
    ...
    try {
      ...
      const statsPromise = this.adminFetch("/api/admin/stats").catch(
        () => null,
      );
      await usersPromise;
      ...
      if (!sessionsRes.ok)
        throw new Error(`Sessions API ${sessionsRes.status}`);
  ...
  async loadAllAccounts() {
    ...
    try {
      ...
      showToast(
        "Load all failed: " + (err.message || "unknown error"),
        "error",
      );
    } finally {
      this.loading = false;
  ...
  async unsuspendUser(id) {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(data.error || `Unsuspend failed ${res.status}`);
      user.status = "active";
      showToast(
        wasPending ? "Account approved" : "Account reactivated",
        "success",
      );
      this.render();
    } catch (err) {
  ...
  openTierModal(id) {
    ...
    overlay
      .querySelector("#admin-tier-cancel")
      .addEventListener("click", finish);
    overlay.addEventListener("click", (e) => {
      if (e.target === overlay) finish();
    });
    overlay
      .querySelector("#admin-tier-save")
      .addEventListener("click", async () => {
        const trustLevel = overlay.querySelector("#admin-tier-trust").value;
        const subscriptionTier = overlay.querySelector(
          "#admin-tier-subscription",
        ).value;
        const subscriptionStatus =
          overlay.querySelector("#admin-tier-status").value;
        ...
        try {
          const res = await this.adminFetch(
            `/api/admin/users/${encodeURIComponent(id)}/trust-level`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
  ...
  openEditModal(id) {
    ...
    const finish = () => overlay.remove();
    overlay
      .querySelector("#admin-edit-cancel")
      .addEventListener("click", finish);
    overlay.addEventListener("click", (e) => {
      if (e.target === overlay) finish();
    });
    overlay
      .querySelector("#admin-edit-save")
      .addEventListener("click", async () => {
        ...
        try {
          const res = await this.adminFetch(
            `/api/admin/users/${encodeURIComponent(id)}/details`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
  ...
  openRefundModal(id) {
    ...
    const finish = () => overlay.remove();
    overlay
      .querySelector("#admin-refund-cancel")
      .addEventListener("click", finish);
    overlay.addEventListener("click", (e) => {
      if (e.target === overlay) finish();
    });
    overlay
      .querySelector("#admin-refund-confirm")
      .addEventListener("click", async () => {
        const reason = overlay
          .querySelector("#admin-refund-reason")
          .value.trim();
        ...
        try {
          const res = await this.adminFetch(
            `/api/admin/customers/${encodeURIComponent(user.email)}/refund`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
          ...
          showToast(
            `Refund processed (${data.refundedCount || 0} subscriptions)`,
            "success",
          );
        } catch (err) {
  ...
  openDetailsModal(id) {
    ...
    this.adminFetch(`/api/admin/users/${encodeURIComponent(id)}/details`)
      ...
      .catch((err) => {
        ...
        overlay
          .querySelector("#admin-details-close")
  ...
  renderOverviewTab(user, u) {
    ...
    const items = [
      ...
      [
        "Trust tier",
        this.badge(u.trustLevel, "trust-" + (u.trustLevel || "bronze")),
      ],
      ["Account status", this.badge(u.status, "status-" + statusClass)],
      [
        "Verification",
        this.badge(u.verificationStatus, "status-" + statusClass),
      ],
      ["Online now", u.online ? "Yes" : "No"],
      [
        "Last seen",
        this.formatDate(u.lastSeen) +
          (u.lastSeen ? " (" + this.formatRelative(u.lastSeen) + ")" : ""),
      ],
  ...
  renderTokenTab(token) {
    ...
    const items = [
      ...
      [
        "Registered",
        this.formatDate(token.registeredAt || status.registeredAt),
      ],
      ...
      [
        ...
        this.formatDate(status.expiresAt) +
          (status.expiringSoon
            ? ' <span class="status-badge expired">Expiring soon</span>'
            : ""),
      ],
      ["Scan quota", token.scanQuota != null ? token.scanQuota : "—"],
      [
        "Scans this period",
        token.scansThisPeriod != null ? token.scansThisPeriod : "—",
      ],
      [
        "API calls this period",
        token.apiCallsThisPeriod != null ? token.apiCallsThisPeriod : "—",
      ],
      [
        "Features",
        Array.isArray(status.features) && status.features.length
          ? status.features
              .map((f) => `<span class="admin-chip">${escapeHtml(f)}</span>`)
              .join(" ")
          : "—",
      ],
    ];
  ...
  renderBillingTab(billing) {
    ...
    const items = [
      [
        "Subscription status",
        this.badge(billing.subscriptionStatus, "status-" + statusClass),
      ],
      ["Plan / tier", billing.plan || "—"],
      [
        "Stripe customer",
        `<code>${escapeHtml(billing.stripeCustomerId || "—")}</code>`,
      ],
  ...
  bindDetailsModal(overlay, data) {
    ...
    overlay
      .querySelectorAll("#admin-reveal-token, #admin-reveal-api")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
  ...
  exportAdminData() {
    ...
    const payload = {
      ...
      exportScope:
        this.totalUsers > this.users.length ? "current-page" : "full-page",
    ...
    downloadJson(
      payload,
      `admin-panel-export-${new Date().toISOString().slice(0, 10)}.json`,
    );
    const scopeNote =
  ...
  render() {
    ...
    const onlineCount =
      this.stats?.onlineNow ?? filtered.filter((u) => u.online).length;
    const showIncidents =
      filtered.some((u) => (u.securityIncidents || 0) > 0) ||
      this.users.some((u) => (u.securityIncidents || 0) > 0);
  ...
  mountTelemetryDashboards() {
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount replication telemetry dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount drop telemetry dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount enclave telemetry dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount recursive proof aggregation dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount decentralized identity gating dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount space-based laser mesh gating dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount QKD link-switch gating dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount holographic storage gating dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount supply chain provenance gating dashboard:",
        e,
      );
    }
    // Mount ZK Decentralized Storage Attestation Gating (Track 111, policy + telemetry)
    try {
      const zkStorageDashboard =
        renderZkDecentralizedStorageAttestationGatingDashboard();
      grid.appendChild(zkStorageDashboard);
    } catch (e) {
      window["console"]["error"](
        "[AdminPanelView] Failed to mount ZK decentralized storage gating dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount bio-digital neural telemetry gating dashboard:",
        e,
      );
    }
    ...
    try {
      ...
      window["console"]["error"](
        "[AdminPanelView] Failed to mount swarm robotics kinetic assembly gating dashboard:",
        e,
      );
    }
    // Mount Multi-Enclave Confidential Mesh State-Reconciliation Gating (Track 115, policy + telemetry)
    try {
      const meshReconciliationDashboard =
        renderMultiEnclaveConfidentialMeshStateReconciliationDashboard();
      grid.appendChild(meshReconciliationDashboard);
    } catch (e) {
      window["console"]["error"](
        "[AdminPanelView] Failed to mount multi-enclave confidential mesh state-reconciliation gating dashboard:",
        e,
      );
    }
  }
  ...
  renderStats(onlineCount, tierCounts = {}, statusCounts = {}) {
    const s = this.stats;
    const totalAccounts =
      s?.totalAccounts ?? this.totalUsers ?? this.users.length;
    const items = [
      {
      ...
      {
        label: "Active",
        value: formatCompactNumber(statusCounts.active ?? "—"),
      },
      {
        label: "Suspended",
        value: formatCompactNumber(statusCounts.suspended ?? "—"),
      },
    ...
    if (s?.activeSubscriptions != null)
      items.push({ label: "Subscriptions", value: s.activeSubscriptions });
  ...
  renderFilterChip(kind, value, label, count) {
    const active =
      (kind === "status" ? this.statusFilter : this.trustFilter) === value;
    const countLabel =
      count != null && count !== "—" ? ` (${formatCompactNumber(count)})` : "";
    ...
    const groups = tiers
      .map((tier) => ({
        ...
        users: filtered.filter(
          (u) => String(u.trustLevel || "bronze").toLowerCase() === tier,
        ),
      }))
      .filter((g) => g.users.length > 0);
    const other = filtered.filter(
      (u) => !tiers.includes(String(u.trustLevel || "bronze").toLowerCase()),
    );
    ...
    this.container.querySelectorAll(".admin-actions-toggle").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        const menu = this.container.querySelector(
          `.admin-actions-menu[data-menu-id="${CSS.escape(id)}"]`,
        );
        if (!menu) return;
        const wasOpen = menu.classList.contains("open");
        this.container
          .querySelectorAll(".admin-actions-menu.open")
