...
import {
  ...
  registerSecurityKey,
  removeSecurityKey,
} from "../services/webauthnService.js?v=20260716cachefix1";
...
export class ProfileView {
  ...
  mount(container) {
    ...
    const user =
      authService.getUser() ||
      ((_a = this.app.state) === null || _a === void 0 ? void 0 : _a.user) ||
      {};
    ...
    const tokenTier =
      ...
      tier;
    const tokenExp =
      (payload === null || payload === void 0 ? void 0 : payload.exp) || null;
    const tokenIat =
      (payload === null || payload === void 0 ? void 0 : payload.iat) || null;
    const expiryInfo = formatExpiry(tokenExp);
    const boundAt =
      (binding === null || binding === void 0 ? void 0 : binding.boundAt) ||
      null;
    ...
    const stockpiledRows = listStockpiled(activeToken)
      .map(({ entry, index }) => {
    ...
    function promptForConfirmPassword(message) {
      return new Promise((resolve) => {
        ...
        overlay
          ...
          ?.addEventListener("click", () => finish(input ? input.value : ""));
        overlay
    ...
    // Save profile
    (_b = container.querySelector("#profile-save-btn")) === null ||
    _b === void 0
      ? void 0
      : _b.addEventListener("click", async () => {
          ...
          const data = {
            email:
              ((_b =
                (_a = container.querySelector("#profile-email")) === null ||
                _a === void 0
                  ? void 0
                ...
                : _b.trim()) || "",
            emailPassword:
              ((_c = container.querySelector("#profile-email-password")) ===
                null || _c === void 0
                ? void 0
                : _c.value) || "",
            tokenPassword:
              ((_d = container.querySelector("#profile-token-password")) ===
                null || _d === void 0
                ? void 0
                : _d.value) || "",
            loginMethod:
              ((_e = container.querySelector(
          ...
          const tokenVal =
            (_g =
              (_f = container.querySelector("#profile-token")) === null ||
              _f === void 0
                ? void 0
    ...
    // Sign out
    (_c = container.querySelector("#profile-signout-btn")) === null ||
    _c === void 0
      ? void 0
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", async () => {
        ...
        if (!input || !input.value) {
          (_b = (_a = this.app).showToast) === null || _b === void 0
            ? void 0
            : _b.call(_a, "No token to copy", "error");
          return;
        }
        let copied = false;
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          try {
            ...
            window["console"]["warn"](
              "[Profile] Clipboard API failed, trying fallback:",
              e,
            );
          }
        }
        ...
        if (copied) {
          ...
          setTimeout(() => {
            if (originalIcon) {
              if (typeof window.setSafeHTML === "function") {
                window.setSafeHTML(
                  copyBtn,
                  `<i data-lucide="${originalIcon}" class="icon-16"></i>`,
                );
              } else {
          ...
          (_d = (_c = this.app).showToast) === null || _d === void 0
            ? void 0
            : _d.call(_c, "Token copied", "success");
        } else {
          (_f = (_e = this.app).showToast) === null || _f === void 0
            ? void 0
            : _f.call(
                _e,
    ...
    // Clear cache
    (_d = container.querySelector("#profile-clear-cache-btn")) === null ||
    _d === void 0
      ? void 0
      : _d.addEventListener("click", () => {
          var _a, _b;
          const keys = Object.keys(localStorage).filter(
            (k) => k.startsWith("sb_") || k.includes("simplebeacon"),
          );
          keys.forEach((k) => localStorage.removeItem(k));
          ((_b = (_a = this.app).showToast) === null || _b === void 0
            ? void 0
            : _b.call(_a, "Local cache cleared", "success")) || void 0;
        });
    ...
    const serverStatusBanner = container.querySelector(
      "#security-key-server-status",
    );
      ...
      try {
        ...
        if (res.status === 404) {
          if (serverStatusBanner) {
            ...
            serverStatusBanner.className =
              "security-key-status-banner security-key-status--error";
            serverStatusBanner.textContent =
              "Security keys are not enabled on this server yet. Contact support to enable WebAuthn.";
          }
        ...
        if (data.success && data.storeWritable === false) {
          if (serverStatusBanner) {
            serverStatusBanner.style.display = "block";
            serverStatusBanner.className =
              "security-key-status-banner security-key-status--warn";
            serverStatusBanner.textContent =
              "Security keys are enabled but the credential store is read-only on this server. Registration may fail.";
          }
        }
      } catch {
        /* offline or local — silently continue */
      }
    })();
    ...
    const renderSecurityKeys = async () => {
      ...
      try {
        ...
        securityKeysList
          .querySelectorAll("[data-remove-key]")
          .forEach((btn) => {
        ...
        securityKeysList.querySelectorAll("[data-test-key]").forEach((btn) => {
          btn.addEventListener("click", async () => {
            ...
            try {
              const { authenticateWithSecurityKey } =
                await import("../services/webauthnService.js?v=20260716cachefix1");
        ...
        setHtml(
          securityKeysList,
          `<p class="profile-help">${escapeHtml(describeWebAuthnError(err))}</p>`,
        );
      }
    };
    ...
    if (typeGrid) {
      typeGrid.querySelectorAll(".security-key-type-card").forEach((card) => {
        card.addEventListener("click", () => {
          ...
          if (usbHelp)
            usbHelp.style.display =
              selectedKeyType === "usb" ? "block" : "none";
          // Update instructions
          ...
          if (instructions) {
            const labels = {
              hardware:
                "Insert your hardware key (YubiKey, Titan, etc.), then click Activate. Tap the key when your browser prompts you.",
              platform:
                "Click Activate, then confirm with Windows Hello, Touch ID, or Face ID when your browser prompts you.",
            ...
            setHtml(
              instructions,
              `<p class="profile-help">${labels[selectedKeyType] || labels.hardware}</p>`,
            );
          }
        });
      });
    ...
    if (activateBtn) {
      activateBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (typeGrid)
            typeGrid
              .querySelectorAll(".security-key-type-card")
              .forEach((c) => {
                c.classList.remove("selected");
                c.setAttribute("aria-checked", "false");
              });
