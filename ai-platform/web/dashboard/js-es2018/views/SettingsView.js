...
export class SettingsView {
  ...
  _resolveMountContainer(container) {
    if (container && container.id === "app-main") return container;
  ...
  _removeOrphanSettingsRoots(container) {
    document
      ...
      .forEach((el) => {
        if (!this._isSettingsRoot(el)) return;
  ...
  _paint(container) {
    ...
    void mountCheckoutSuccessBanner(root, {
      onTokenReady: (token, email) => {
        this.addToVault(token, {
          email: email || billingService.getEmail(),
          tier: "team",
        });
    ...
    if (!this.isDirty()) {
      this.savedSnapshot = JSON.stringify(
        this.buildConfigFromDom(this.draft, this._root),
      );
    }
    target.scrollTop = 0;
    return root;
  ...
  returnVaultToken(index, rerender) {
    ...
    if (!this.canReturnToken(index)) {
      showToast(
        "This token has already been used and cannot be returned",
        "error",
      );
      return;
    }
    const tHint =
      entry.token.length > 24
        ? `${entry.token.slice(0, 8)}…${entry.token.slice(-8)}`
        : entry.token;
    if (
      !globalThis.confirm(
  ...
  computeVaultMetrics() {
    ...
    const activeToken = authService.getToken();
    const activeIndex = this.tokenVault.findIndex(
      (v) => v.token === activeToken,
    );
    const stockpiled = this.tokenVault.filter((v) =>
      isStockpiledEntry(v, activeToken),
    ).length;
  ...
  renderOllamaModelSelect(keys) {
    const selected = this.matchOllamaModelOption(
      keys.ollamaModel,
      this.ollamaModels,
    );
  ...
  refreshOllamaModelSelect() {
    ...
    const wrap =
      (_a = this._root) === null || _a === void 0
        ? void 0
        : _a.querySelector("#settings-ai-ollama-model-wrap");
    if (!wrap) return;
  ...
  bindOllamaModelEvents(root = this._root) {
    var _a, _b;
    if (!root) return;
    (_a = root.querySelector("#settings-ai-ollama-model")) === null ||
    _a === void 0
      ? void 0
      : _a.addEventListener("change", (e) => {
          this.aiKeysFormDraft.ollamaModel = e.target.value;
        });
    (_b = root.querySelector("#settings-ai-refresh-models")) === null ||
    _b === void 0
      ? void 0
      : _b.addEventListener("click", () => {
          var _a, _b;
          const baseUrl =
            this.sanitizeOllamaBaseUrl(
              (_b =
                (_a = root.querySelector("#settings-ai-ollama")) === null ||
                _a === void 0
                  ? void 0
    ...
    try {
      ...
      if (!result.models.length) {
        this.ollamaModelsError =
          result.message || "No models returned — run `ollama pull <model>`";
      } else if (!result.ok) {
      ...
      if (options.toastOnSuccess && this.ollamaModels.length) {
        showToast(
          `${this.ollamaModels.length} Ollama model(s) loaded`,
          "success",
        );
      }
    }
  ...
  captureAiKeysFormDraft(root = this._root) {
    ...
    this.syncAiKeysFormDraft({
      ...
      ollamaModel:
        (_f =
          (_e =
            (_d = root.querySelector("#settings-ai-ollama-model")) === null ||
            _d === void 0
              ? void 0
    ...
    if (this.error && !this.draft) {
      el.innerHTML = `
        ...
        ${renderEmptyState({
          ...
          actions: [
            { label: "Retry", id: "settings-reload", className: "btn-primary" },
          ],
        })}
      `;
      ...
      <div class="card settings-grid mb-6" id="settings-section-rules">
        ...
        ${RULE_ORDER.map((name) => {
          ...
          return renderRuleRow(
            name,
            (_a = config.rules) === null || _a === void 0 ? void 0 : _a[name],
          );
        }).join("")}
      </div>
  ...
  renderTokenSection() {
    ...
    const hint =
      token && token.length > 24
        ...
        : token || "—";
    const expiresAt = (
      payload === null || payload === void 0 ? void 0 : payload.exp
    )
    ...
    const vaultRows = this.tokenVault
      .map((entry, idx) => {
        ...
        const loadLabel = isStockpiledEntry(entry, authService.getToken())
          ? "Load"
          : "Use";
        return `
  ...
  stockpileTokenFromInput(root, rerender) {
    var _a, _b, _c;
    const input =
      (_a = root || this._root) === null || _a === void 0
        ? void 0
        : _a.querySelector("#settings-token-input");
    const newToken =
      ((_b = input === null || input === void 0 ? void 0 : input.value) ===
        null || _b === void 0
        ? void 0
        : _b.trim()) || "";
    if (!newToken) {
  ...
  registerTokenWithEmail(rerender) {
    ...
    const email = window.prompt(
      "Enter email address to associate with this token (for recovery):",
      "",
    );
    if (!email || !email.trim()) {
  ...
  loadDownloadSettings() {
    ...
    try {
      ...
      return {
        ...
        agency: {
          projectName:
            ((_e = parsed.agency) === null || _e === void 0
              ? void 0
              : _e.projectName) || "CascadeProjects",
          devAgency:
            ((_f = parsed.agency) === null || _f === void 0
              ? void 0
              : _f.devAgency) || "Agency",
          client:
            ((_g = parsed.agency) === null || _g === void 0
              ? void 0
              : _g.client) || "",
          milestone:
            ((_h = parsed.agency) === null || _h === void 0
              ? void 0
              : _h.milestone) || "Release",
          date:
            ((_j = parsed.agency) === null || _j === void 0
              ? void 0
              : _j.date) || today,
          scanId:
            ((_k = parsed.agency) === null || _k === void 0
              ? void 0
              : _k.scanId) || "",
          score:
            (_m =
              (_l = parsed.agency) === null || _l === void 0
                ? void 0
                : _l.score) !== null && _m !== void 0
              ? _m
              : 100,
          status:
            ((_o = parsed.agency) === null || _o === void 0
              ? void 0
              : _o.status) || "PASS",
        },
        executive: {
          projectName:
            ((_p = parsed.executive) === null || _p === void 0
              ? void 0
              : _p.projectName) || "CascadeProjects",
          assessor:
            ((_q = parsed.executive) === null || _q === void 0
              ? void 0
              : _q.assessor) || "SimpleBeacon Operator",
          date:
            ((_r = parsed.executive) === null || _r === void 0
              ? void 0
              : _r.date) || today,
          reportId:
            ((_s = parsed.executive) === null || _s === void 0
              ? void 0
              : _s.reportId) || "",
          summary:
            ((_t = parsed.executive) === null || _t === void 0
              ? void 0
              : _t.summary) || "",
          verdict:
            ((_u = parsed.executive) === null || _u === void 0
              ? void 0
              : _u.verdict) || "READY",
        },
      };
  ...
  async loadAiKeys() {
    ...
    try {
      ...
      const isAuthError =
        ...
        /Authentication required|Unauthorized/i.test(err.message);
      if (!isAuthError) {
  ...
  updateAiKeysBusyUi() {
    ...
    (_c = root.querySelector("#settings-ai-test-ollama")) === null ||
    _c === void 0
      ? void 0
  ...
  async testOllamaConnection(root, rerender) {
        ...
        (_b =
          (_a =
            payloadRoot === null || payloadRoot === void 0
              ? void 0
              : payloadRoot.querySelector("#settings-ai-ollama")) === null ||
          _a === void 0
            ? void 0
            : _a.value) === null || _b === void 0
          ? void 0
    ...
    const model =
      ((_d =
        (_c =
          payloadRoot === null || payloadRoot === void 0
            ? void 0
            : payloadRoot.querySelector("#settings-ai-ollama-model")) ===
          null || _c === void 0
          ? void 0
          : _c.value) === null || _d === void 0
        ? void 0
        : _d.trim()) || "";
    const hostedBlockedMsg =
      "Local Ollama cannot be reached from the hosted dashboard. Use OpenAI or Anthropic keys here, or run the dashboard locally with ollama serve.";
    ...
    try {
      ...
      if (!this.ollamaModels.length) {
        showToast(
          this.ollamaModelsError || "Ollama connection failed",
          "error",
        );
        return;
      }
      const picked =
        model ||
        this.displayAiKeys().ollamaModel ||
        String(this.ollamaModels[0]).split(":")[0];
      this.syncAiKeysFormDraft({ ollamaBaseUrl: baseUrl, ollamaModel: picked });
      showToast(
        `Ollama connected — ${this.ollamaModels.length} model(s) available`,
        "success",
      );
      rerender();
    } catch (err) {
  ...
  async clearAllAiKeys(rerender) {
    if (
      !globalThis.confirm("Remove all saved AI provider keys for your account?")
    )
      return;
    ...
    try {
      ...
      const [config, presets] = await Promise.all([
        ...
        this.presets
          ...
          : scanService.fetchConfigPresets(),
        this.loadAiKeys(),
      ]);
      ...
      this.savedSnapshot = JSON.stringify(
        this.buildConfigFromDom(this.draft, null),
      );
    } catch (err) {
      if (token !== this._mountToken) return;
  ...
  mount(container) {
    ...
    if (!this.aiKeys && !this._loadAndMountActive && !this.aiKeysBusy) {
      void this.loadAiKeys().then(() => {
        ...
        if (main && this._root && main.contains(this._root)) this.mount(main);
      });
    }
  }
  resetDraft() {
  ...
  isDirty() {
    ...
    return (
      JSON.stringify(this.buildConfigFromDom(this.draft, this._root)) !==
      this.savedSnapshot
    );
  }
  bindEvents(root) {
    ...
    root
      .querySelectorAll(".settings-nav-link[data-scroll-to]")
      .forEach((link) => {
    ...
    (_b = root.querySelector("#settings-profile-select")) === null ||
    _b === void 0
      ? void 0
    ...
    (_e = root.querySelector("#settings-production-paths")) === null ||
    _e === void 0
      ? void 0
      : _e.addEventListener("input", onFieldChange);
    (_f = root.querySelector("#settings-full-directory-scan")) === null ||
    _f === void 0
      ? void 0
    ...
    (_r = root.querySelector("#settings-ai-test-ollama")) === null ||
    _r === void 0
      ? void 0
      : _r.addEventListener("click", () =>
          this.testOllamaConnection(root, rerender),
        );
    (_s = root.querySelector("#settings-download-save")) === null ||
    _s === void 0
      ? void 0
      : _s.addEventListener("click", () => {
          var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p;
          const autoPdfCheckbox = root.querySelector(
            "#settings-download-auto-pdf",
          );
          const promptCheckbox = root.querySelector(
            "#settings-download-prompt-credentials",
          );
          const projectInput = root.querySelector(
            "#settings-download-default-project",
          );
          const signatoryInput = root.querySelector(
            "#settings-download-default-signatory",
          );
          const titleInput = root.querySelector(
            "#settings-download-default-title",
          );
          const emailInput = root.querySelector(
            "#settings-download-default-email",
          );
          this.saveDownloadSettings({
            autoGeneratePdf: autoPdfCheckbox ? autoPdfCheckbox.checked : false,
            promptForCredentials: promptCheckbox
            ...
            agency: {
              projectName:
                ((_a = root.querySelector("#settings-agency-project")) ===
                  null || _a === void 0
                  ? void 0
                  : _a.value.trim()) || "",
              devAgency:
                ((_b = root.querySelector("#settings-agency-dev")) === null ||
                _b === void 0
                  ? void 0
                  : _b.value.trim()) || "",
              client:
                ((_c = root.querySelector("#settings-agency-client")) ===
                  null || _c === void 0
                  ? void 0
                  : _c.value.trim()) || "",
              milestone:
                ((_d = root.querySelector("#settings-agency-milestone")) ===
                  null || _d === void 0
                  ? void 0
                  : _d.value) || "Release",
              date:
                ((_e = root.querySelector("#settings-agency-date")) === null ||
                _e === void 0
                  ? void 0
                  : _e.value) || "",
              scanId:
                ((_f = root.querySelector("#settings-agency-scan-id")) ===
                  null || _f === void 0
                  ? void 0
                  : _f.value.trim()) || "",
              score: Number(
                ((_g = root.querySelector("#settings-agency-score")) === null ||
                _g === void 0
                  ? void 0
                  : _g.value) || 100,
              ),
              status:
                ((_h = root.querySelector("#settings-agency-status")) ===
                  null || _h === void 0
                  ? void 0
                  : _h.value) || "PASS",
            },
            executive: {
              projectName:
                ((_j = root.querySelector("#settings-exec-project")) === null ||
                _j === void 0
                  ? void 0
                  : _j.value.trim()) || "",
              assessor:
                ((_k = root.querySelector("#settings-exec-assessor")) ===
                  null || _k === void 0
                  ? void 0
                  : _k.value.trim()) || "",
              date:
                ((_l = root.querySelector("#settings-exec-date")) === null ||
                _l === void 0
                  ? void 0
                  : _l.value) || "",
              reportId:
                ((_m = root.querySelector("#settings-exec-report-id")) ===
                  null || _m === void 0
                  ? void 0
                  : _m.value.trim()) || "",
              summary:
                ((_o = root.querySelector("#settings-exec-summary")) === null ||
                _o === void 0
                  ? void 0
                  : _o.value) || "",
              verdict:
                ((_p = root.querySelector("#settings-exec-verdict")) === null ||
                _p === void 0
                  ? void 0
                  : _p.value) || "READY",
            },
    ...
    root.querySelectorAll("[data-cred-tab]").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        root
          .querySelectorAll("[data-cred-tab]")
          .forEach((t) => t.classList.remove("active"));
        root
          .querySelectorAll("[data-cred-panel]")
          .forEach((p) => p.classList.remove("active"));
        tab.classList.add("active");
        (_a = root.querySelector(`[data-cred-panel="${target}"]`)) === null ||
        _a === void 0
          ? void 0
          : _a.classList.add("active");
      });
    });
    (_t = root.querySelector("#settings-token-update")) === null ||
    _t === void 0
      ? void 0
      : _t.addEventListener("click", () => this.updateToken(root, rerender));
    (_u = root.querySelector("#settings-token-stockpile")) === null ||
    _u === void 0
      ? void 0
      : _u.addEventListener("click", () =>
          this.stockpileTokenFromInput(root, rerender),
        );
    ...
    (_y = root.querySelector("#settings-token-register-email")) === null ||
    _y === void 0
      ? void 0
      : _y.addEventListener("click", () =>
          this.registerTokenWithEmail(rerender),
        );
    (_z = root.querySelector("#settings-token-clear-vault")) === null ||
    _z === void 0
      ? void 0
    ...
    if (root.querySelector("#settings-ai-keys-card")) {
      const baseUrl =
        this.displayAiKeys().ollamaBaseUrl || "http://127.0.0.1:11434";
      if (
        !this.ollamaModels.length &&
        !this.ollamaModelsLoading &&
        shouldProbeOllamaModels(baseUrl)
      ) {
        void this.loadOllamaModels(baseUrl).catch(() => {});
      }
    }
  ...
  async applyProfilePreset(profile, rerender) {
    ...
    try {
      ...
      this.draft = {
        ...
        productionPaths: (
          (_a = preset.productionPaths) === null || _a === void 0
            ? void 0
            : _a.length
        )
      ...
      if (
        (_b = this.draft.rules) === null || _b === void 0
          ? void 0
          : _b["production-leak"]
      ) {
        this.draft.rules["production-leak"].productionPaths = [
          ...(this.draft.productionPaths || []),
        ];
      }
  ...
  buildConfigFromDom(base, rootEl) {
    ...
    if (root) {
      config.sampleDir =
        ((_b =
          (_a = root.querySelector("#settings-sample-dir")) === null ||
          _a === void 0
            ? void 0
            : _a.value) === null || _b === void 0
          ? void 0
      ...
      const scanPaths = textToPaths(
        (_c = root.querySelector("#settings-scan-paths")) === null ||
          _c === void 0
          ? void 0
          : _c.value,
      );
      const productionPaths = textToPaths(
        (_d = root.querySelector("#settings-production-paths")) === null ||
          _d === void 0
          ? void 0
          : _d.value,
      );
      ...
      config.fullDirectoryScan =
        (_f =
          (_e = root.querySelector("#settings-full-directory-scan")) === null ||
          _e === void 0
            ? void 0
    ...
    if (
      (_g = config.rules) === null || _g === void 0
        ? void 0
        : _g["production-leak"]
    ) {
      config.rules["production-leak"].productionPaths = [
  ...
  async discoverPathsFromProject(rerender) {
    ...
    try {
      ...
      const inventory =
        await this.app.scanService.fetchRepositoryInventory(projectPath);
      if (
        !(inventory === null || inventory === void 0
          ? void 0
          : inventory.directoryTree)
      ) {
        showToast("Could not read project structure", "error");
        return;
      }
      const dirs = inventory.directoryTree.map((d) =>
  ...
  async runScan(rerender) {
    ...
    try {
      ...
      await scanService.runScan(null, {
        fullDirectoryScan: config.fullDirectoryScan === true,
      });
...
function readGateSelection(root, group) {
  return SEVERITIES.filter((sev) => {
    const input = root.querySelector(
      `[data-gate-group="${group}"][value="${sev}"]`,
    );
