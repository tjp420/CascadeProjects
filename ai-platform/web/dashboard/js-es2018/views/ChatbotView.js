...
function readOllamaVerifiedFromSession() {
  try {
    return (
      typeof sessionStorage !== 'undefined' && sessionStorage.getItem(OLLAMA_VERIFIED_KEY) === '1'
    );
  } catch (_a) {
    return false;
  }
...
async function resolveChatbotProviders(options = {}) {
  ...
  const ollamaFromBridgeApi = Boolean(
    providers.find((provider) => provider.id === 'ollama' && provider.available)
  );
...
function buildChatbotSystemPrompt(options = {}) {
  ...
  const personalityPrompt =
    PERSONALITY_PROMPTS[options.personality || 'helpful'] || PERSONALITY_PROMPTS.helpful;
...
export class ChatbotView {
  ...
  async maybeAutoConnectBridgeOllama() {
    ...
    try {
      ...
      baseUrl =
        String(keys?.ollamaBaseUrl || BROWSER_OLLAMA_URL).replace(/\/$/, '') || BROWSER_OLLAMA_URL;
    } catch (_a) {
      /* default */
    }
  ...
  updateOllamaModelGroupVisibility() {
    ...
    if (!group) return;
    const show =
      this.useBrowserOllama || hasExtensionBridgeConfigured() || this.selectedProvider === 'ollama';
  ...
  async saveChatbotSettings(personalitySelect, removeFiltersCheckbox, settingsPanel) {
    ...
    this.showPromptToast(
      this.ollamaModel ? `Settings saved — model: ${this.ollamaModel}` : 'Settings saved'
    );
        ...
        getNoProviderMessage({
          ...
          hasSavedCloudKeys: Boolean(
            this._resolvedProviders?.some?.(
              (provider) =>
                (provider.id === 'openai' || provider.id === 'anthropic') && provider.available
            )
          ),
        }),
        true,
