    ...
    const run = () => {
      if (
        ...
        try {
          ...
          console.error("Failed to check IDE dashboard surface:", e);
        }
        const cssVar =
          getComputedStyle(root).getPropertyValue("--embed-top-shim") || "";
        ...
        if (appMain) {
          ...
          if (!appMain.style.paddingTop || cur === 0)
            appMain.style.paddingTop = shimPx + "px";
        } else {
          const bodyPad =
