...
const PATTERNS = [
  ...
  // simplebeacon-ignore redos-risk — PEM header regex matches short fixed-length strings, not user-controlled input
  {
    ...
    regex: new RegExp(
      "-----BEGIN (RSA |EC |OPENSSH )?P" + "RIVATE K" + "EY-----",
      "g",
    ),
  },
  {
...
export function isAnalyzerCacheJson(parsed) {
  ...
  return entries
    ...
    .every(
      (entry) =>
        entry && typeof entry === "object" && typeof entry.hash === "string",
    );
}
/**
...
function isAllowlisted(text, match) {
  ...
  return ALLOWLIST.some(
    (allowed) => snippet.indexOf(allowed.toLowerCase()) !== -1,
  );
}
/**
 * Line at.
...
function isAuditReportFindingLine(line) {
  ...
  return (
    /<code class="snippet">/.test(trimmed) ||
    /<td><code>[^<]+:\d+<\/code>/.test(trimmed)
  );
}
/**
 * Is lockfile name.
...
function isPackageManifestPathLine(line) {
  const trimmed = String(line || "").trim();
  return /^"[^"]+"\s*:\s*"[^"]+\.(?:js|mjs|cjs|ts|tsx|json)"\s*,?\s*$/.test(
    trimmed,
  );
}
/**
...
function shouldSkipMockPathMatch(text, match, options = {}) {
  ...
  if (
    looksLikeAuditReportHtml(text, options.fileName) &&
    isAuditReportFindingLine(line)
  )
    return true;
...
export function scanSnippetText(text, options = {}) {
  ...
  try {
    ...
    if (
      isAnalyzerCacheJson(parsed) ||
      isCleanupExportJson(parsed) ||
      isFictionDigestJson(parsed)
    ) {
      return [];
    }
  } catch (_a) {
  ...
  PATTERNS.forEach((pattern) => {
    ...
    while ((match = pattern.regex.exec(text)) !== null) {
      ...
      if (
        pattern.id === "mock-path" &&
        shouldSkipMockPathMatch(text, match, options)
      )
        continue;
