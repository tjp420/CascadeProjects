...
import { normalizeProjectPath } from '../services/analyzeService.js?v=20260716cachefix1';
...
export function expandDisplayPathToFull(inputValue, app, testSources = []) {
  ...
  if (suffix) {
    ...
    if (
      defaultPath &&
      (defaultPath.endsWith(suffix) || defaultPath.toLowerCase().endsWith(suffix.toLowerCase()))
    ) {
