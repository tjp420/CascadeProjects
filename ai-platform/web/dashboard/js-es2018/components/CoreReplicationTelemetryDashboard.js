...
import {
  fetchReplicationTelemetry,
  getReplicationHistory,
} from "../services/replicationTelemetryService.js";
...
const TRACK_DEFS = [
  {
    ...
    metrics: [
      ...
      {
        key: "hsm_migration_verification_failed_total",
        label: "Verify Failed",
      },
  ...
  },
  {
    ...
    metrics: [
      ...
      {
        key: "hsm_reconciliation_divergence_detected_total",
        label: "Divergence",
      },
      { key: "hsm_reconciliation_promoted_total", label: "Promoted" },
      { key: "hsm_reconciliation_quarantined_total", label: "Quarantined" },
      {
        key: "hsm_reconciliation_rollback_blocked_total",
        label: "Rollback Blocked",
      },
  ...
  },
  {
    ...
    metrics: [
      ...
      {
        key: "hsm_poa_double_count_blocked_total",
        label: "Double-Count Blocked",
      },
...
async function loadCoreReplicationData(container, isInitial) {
  ...
  try {
    ...
    if (data.status === "unavailable") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Replication telemetry endpoint not available.</div>';
      return;
    }
    ...
    if (data.status === "forbidden") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Admin access required to view replication telemetry.</div>';
      return;
    }
    ...
    for (const trackDef of TRACK_DEFS) {
      ...
      for (const def of trackDef.metrics) {
        ...
        const value =
          groupCounters[def.key] !== undefined ? groupCounters[def.key] : 0;
    ...
    content.innerHTML =
      '<div class="text-danger">Failed to load replication telemetry.</div>';
  } finally {
    isRefreshing = false;
