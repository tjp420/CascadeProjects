...
const COUNTER_DEFS = [
    { key: 'hsm_neurogate_pool_initialized_total', label: 'Neural Telemetry Pools Initialized', severity: 'positive' },
    { key: 'hsm_zk_neural_telemetry_verified_total', label: 'ZK Neural Telemetry Verified', severity: 'positive' },
    { key: 'hsm_synapse_accreditation_completed_total', label: 'Synapse Accreditation Completions', severity: 'positive' },
];

const FORM_FIELDS = [
    { name: 'neuralQuorum', label: 'Neural Quorum', type: 'number' },
    { name: 'neuralTelemetryWindowSeconds', label: 'Neural Telemetry Window (s)', type: 'number' },
...
const BOOLEAN_FIELDS = [
    { name: 'neuroTelemetryAuthorityInitializerAttestation', label: 'Require Neuro Telemetry Authority Initializer Attestation' },
    { name: 'bioEthicsOversightCommitteeAttestation', label: 'Require Bio Ethics Oversight Committee Attestation' },
    { name: 'banMalformedOrOutOfOrderNeuroClaims', label: 'Ban Malformed / Out-of-Order Neuro Claims' },
...
function bindBioDigitalInterfaceNeuralTelemetryDashboard(container) {
    ...
    const validateBtn = container.querySelector('#neural-telemetry-validate-policy');
...
async function loadBioDigitalInterfaceNeuralTelemetryData(container, isInitial) {
    ...
    try {
        ...
        content.innerHTML = '<div class="text-danger">Failed to load bio-digital neural telemetry data.</div>';
        window['console']['error']('[BioDigitalInterfaceNeuralTelemetryDashboard] Error:', error.message);
    } finally {
        isRefreshing = false;
...
async function onValidatePolicy(container) {
    const resultBox = container.querySelector('#neural-telemetry-validation-result');
    resultBox.textContent = 'Validating...';
    resultBox.className = 'mt-2 text-muted';

    try {
        const payload = collectPolicyPayload(container);
        const result = await validateBioDigitalInterfaceNeuralTelemetryPolicy(payload);
        if (result.valid) {
