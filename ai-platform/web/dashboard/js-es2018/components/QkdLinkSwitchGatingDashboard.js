...
const COUNTER_DEFS = [
  {
    key: "hsm_qkdswitchgate_pool_initialized_total",
    label: "QKD Link-Switch Pools Initialized",
    severity: "positive",
  },
  {
    key: "hsm_zk_qkd_link_claim_verified_total",
    label: "ZK QKD Link Claims Verified",
    severity: "positive",
  },
  {
    key: "hsm_entanglement_accreditation_completed_total",
    label: "Entanglement Accreditations Completed",
    severity: "positive",
  },
];
...
const FORM_FIELDS = [
  ...
  {
    name: "entanglementWindowSeconds",
    label: "Entanglement Window (s)",
    type: "number",
  },
  {
    name: "qkdSwitchChainDepth",
    label: "QKD Switch Chain Depth",
    type: "number",
  },
  {
...
const BOOLEAN_FIELDS = [
  {
    name: "qkdLinkAuthorityInitializerAttestation",
    label: "Require QKD Link Authority Initializer Attestation",
  },
  {
    name: "qkdEthicsOversightCommitteeAttestation",
    label: "Require QKD Ethics Oversight Attestation",
  },
  {
    name: "banMalformedOrOutOfOrderQkdLinkClaims",
    label: "Ban Malformed / Out-of-Order QKD Link Claims",
  },
...
function bindQkdLinkSwitchDashboard(container) {
  const refreshBtn = container.querySelector(
    "#qkd-link-switch-telemetry-refresh",
  );
  const validateBtn = container.querySelector(
    "#qkd-link-switch-validate-policy",
  );
...
async function loadQkdLinkSwitchData(container, isInitial) {
  ...
  try {
    ...
    content.innerHTML =
      '<div class="text-danger">Failed to load QKD link-switch gating data.</div>';
    window["console"]["error"](
      "[QkdLinkSwitchGatingDashboard] Error:",
      error.message,
    );
  } finally {
    isRefreshing = false;
...
async function onValidatePolicy(container) {
  const resultBox = container.querySelector(
    "#qkd-link-switch-validation-result",
  );
