...
import {
  fetchEnclaveTelemetry,
  getEnclaveHistory,
} from "../services/enclaveTelemetryService.js";
...
const COUNTER_DEFS = [
  {
    key: "hsm_enclave_bootstrap_total",
    label: "Bootstrap OK",
    severity: "positive",
  },
  {
    key: "hsm_enclave_bootstrap_failed_total",
    label: "Bootstrap Fail",
    severity: "danger",
  },
  { key: "hsm_enclave_seal_total", label: "Sealed", severity: "positive" },
  { key: "hsm_enclave_unseal_total", label: "Unsealed", severity: "positive" },
  {
    key: "hsm_enclave_unseal_failed_total",
    label: "Unseal Fail",
    severity: "danger",
  },
  {
    key: "hsm_enclave_key_provisioned_total",
    label: "Keys Provisioned",
    severity: "positive",
  },
  {
    key: "hsm_enclave_key_provision_blocked_total",
    label: "Provision Blocked",
    severity: "warning",
  },
  {
    key: "hsm_enclave_attestation_verified_total",
    label: "Attestation OK",
    severity: "positive",
  },
  {
    key: "hsm_enclave_attestation_rejected_total",
    label: "Attestation Rejected",
    severity: "danger",
  },
...
function getSeverityClass(def, value) {
  if (def.severity === "danger")
    return value > 0 ? "metric-chip-danger" : "metric-chip-ok";
  if (def.severity === "warning")
    return value > 0 ? "metric-chip-warning" : "metric-chip-ok";
  if (def.severity === "positive")
    return value > 0 ? "metric-chip-ok" : "metric-chip-neutral";
  if (def.severity === "active")
    return value > 0 ? "metric-chip-ok" : "metric-chip-warning";
...
async function loadEnclaveTelemetryData(container, isInitial) {
  ...
  try {
    ...
    if (data.status === "unavailable") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Enclave telemetry endpoint not available.</div>';
      return;
    }
    ...
    if (data.status === "forbidden") {
      ...
      stateBanner.innerHTML = "";
      content.innerHTML =
        '<div class="text-gray-500">Admin access required to view enclave telemetry.</div>';
      return;
    }

    statusBadge.className = "badge badge-success";
    statusBadge.textContent = data.registered
      ? data.initialized
        ? "Initialized"
        : "Registered"
      : "Not Registered";
    ...
    const mrenclave = data.mrenclave
      ? data.mrenclave.substring(0, 24) + "..."
      : "—";
    const initStatus = data.initialized ? "● Initialized" : "○ Not Initialized";
    const initClass = data.initialized
      ? "enclave-init-ok"
      : "enclave-init-pending";
    stateBanner.innerHTML =
      '<div class="enclave-state-row">' +
      '<span class="enclave-state-item"><strong>Backend:</strong> ' +
      backend +
      "</span>" +
      '<span class="enclave-state-item"><strong>MRENCLAVE:</strong> ' +
      mrenclave +
      "</span>" +
      '<span class="enclave-state-item ' +
      initClass +
      '">' +
      initStatus +
      "</span>" +
    ...
    stateBanner.innerHTML = "";
    content.innerHTML =
      '<div class="text-danger">Failed to load enclave telemetry.</div>';
  } finally {
    isRefreshing = false;
