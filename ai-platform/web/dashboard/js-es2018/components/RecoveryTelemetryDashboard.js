...
async function loadRecoveryTelemetryData(container, isInitial = false) {
  ...
  try {
    ...
    if (data.status === "unavailable") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Recovery telemetry endpoint not available.</div>';
      return;
    }
    ...
    if (data.status === "forbidden") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Admin access required to view recovery telemetry.</div>';
      return;
    }
    ...
    const metricDefs = [
      ...
      {
        key: "hsm_recovery_time_lock_blocked_total",
        label: "Time-Lock Blocked",
      },
    ...
    content.innerHTML =
      '<div class="text-danger">Failed to load recovery telemetry.</div>';
  } finally {
    isRefreshing = false;
