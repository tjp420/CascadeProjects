...
const COUNTER_DEFS = [
    { key: 'hsm_lasergate_pool_initialized_total', label: 'Laser Mesh Pools Initialized', severity: 'positive' },
    { key: 'hsm_zk_laser_mesh_claim_verified_total', label: 'ZK Laser Mesh Claims Verified', severity: 'positive' },
    { key: 'hsm_handoff_accreditation_completed_total', label: 'Handoff Accreditations Completed', severity: 'positive' },
];

const FORM_FIELDS = [
    { name: 'laserMeshQuorum', label: 'Laser Mesh Quorum', type: 'number' },
    { name: 'handoffWindowSeconds', label: 'Handoff Window (s)', type: 'number' },
    { name: 'laserMeshChainDepth', label: 'Laser Mesh Chain Depth', type: 'number' },
...
const BOOLEAN_FIELDS = [
    { name: 'laserMeshAuthorityInitializerAttestation', label: 'Require Laser Mesh Authority Initializer Attestation' },
    { name: 'laserEthicsOversightCommitteeAttestation', label: 'Require Laser Ethics Oversight Attestation' },
    { name: 'banMalformedOrOutOfOrderLaserMeshClaims', label: 'Ban Malformed / Out-of-Order Laser Mesh Claims' },
...
async function loadSpaceBasedLaserMeshData(container, isInitial) {
    ...
    try {
        ...
        content.innerHTML = '<div class="text-danger">Failed to load laser mesh gating data.</div>';
        window['console']['error']('[SpaceBasedLaserCommunicationMeshGatingDashboard] Error:', error.message);
    } finally {
        isRefreshing = false;
