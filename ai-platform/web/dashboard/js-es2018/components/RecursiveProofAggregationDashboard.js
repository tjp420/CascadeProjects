...
import {
  fetchRecursiveProofTelemetry,
  getRecursiveProofHistory,
} from "../services/recursiveProofService.js";
...
const METRIC_DEFS = [
  ...
  {
    key: "hsm_recursive_chain_aggregations_total",
    label: "Chain Aggregations",
  },
  ...
  {
    key: "hsm_recursive_mixnet_compressions_total",
    label: "Mixnet Compressions",
  },
...
async function loadRecursiveProofData(container, isInitial) {
  ...
  try {
    ...
    if (data.status === "unavailable") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Recursive proof aggregation endpoint not available.</div>';
      return;
    }
    ...
    if (data.status === "forbidden") {
      ...
      content.innerHTML =
        '<div class="text-gray-500">Admin access required to view recursive proof telemetry.</div>';
      return;
    }
    ...
    content.innerHTML =
      '<div class="text-danger">Failed to load recursive proof telemetry. Check console for details.</div>';
    window["console"]["error"](
      "[RecursiveProofAggregationDashboard] Error:",
      error,
    );
  } finally {
    isRefreshing = false;
