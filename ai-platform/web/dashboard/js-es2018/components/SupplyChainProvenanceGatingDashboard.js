...
const COUNTER_DEFS = [
    { key: 'hsm_supplygate_pool_initialized_total', label: 'Supply Chain Provenance Pools Initialized', severity: 'positive' },
    { key: 'hsm_zk_provenance_claim_verified_total', label: 'ZK Provenance Claims Verified', severity: 'positive' },
    { key: 'hsm_lineage_accreditation_completed_total', label: 'Lineage Accreditations Completed', severity: 'positive' },
    { key: 'hsm_supplygate_settled_total', label: 'Supply Chain Pools Settled', severity: 'positive' },
    { key: 'hsm_supplygate_rebalanced_total', label: 'Supply Chain Rebalances Completed', severity: 'positive' },
    { key: 'hsm_supplygate_slash_recorded_total', label: 'Provenance Claim Slashes', severity: 'negative' },
    { key: 'hsm_provenance_batch_verified_total', label: 'Provenance Batch Verifications', severity: 'positive' },
];

const FORM_FIELDS = [
    { name: 'supplierCheckpointQuorum', label: 'Supplier Checkpoint Quorum', type: 'number' },
    { name: 'transitExpirationSeconds', label: 'Transit Expiration (s)', type: 'number' },
    { name: 'componentLineageDepth', label: 'Component Lineage Depth', type: 'number' },
...
const BOOLEAN_FIELDS = [
    { name: 'factoryEndpointInitializerAttestation', label: 'Require Factory Endpoint Initializer Attestation' },
    { name: 'clearingCommitteeAttestation', label: 'Require Clearing Committee Attestation' },
    { name: 'banMalformedOrOutOfOrderProvenanceClaims', label: 'Ban Malformed / Out-of-Order Provenance Claims' },
...
function bindSupplyChainProvenanceDashboard(container) {
    const refreshBtn = container.querySelector('#supply-chain-provenance-telemetry-refresh');
    const validateBtn = container.querySelector('#supply-chain-provenance-validate-policy');
...
async function loadSupplyChainProvenanceData(container, isInitial) {
    const content = container.querySelector('#supply-chain-provenance-telemetry-content');
    const statusBadge = container.querySelector('#supply-chain-provenance-status');
    const refreshBtn = container.querySelector('#supply-chain-provenance-telemetry-refresh');
    ...
    try {
        ...
        content.innerHTML = '<div class="text-danger">Failed to load supply chain provenance gating data.</div>';
        window['console']['error']('[SupplyChainProvenanceGatingDashboard] Error:', error.message);
    } finally {
        isRefreshing = false;
...
function populatePolicyForm(container, policy) {
    for (const field of FORM_FIELDS) {
        const input = container.querySelector('#supply-chain-provenance-' + field.name);
...
async function onValidatePolicy(container) {
    const resultBox = container.querySelector('#supply-chain-provenance-validation-result');
