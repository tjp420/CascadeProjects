...
const COUNTER_DEFS = [
    { key: 'hsm_meshgate_pool_initialized_total', label: 'Mesh State-Reconciliation Pools Initialized', severity: 'positive' },
    { key: 'hsm_zk_mesh_state_reconciled_total', label: 'ZK Mesh State Reconciled', severity: 'positive' },
    { key: 'hsm_epoch_finality_completed_total', label: 'Epoch Finality Completions', severity: 'positive' },
];

const FORM_FIELDS = [
    { name: 'meshQuorum', label: 'Mesh Quorum', type: 'number' },
    { name: 'epochFinalityWindowSeconds', label: 'Epoch Finality Window (s)', type: 'number' },
    { name: 'reconciliationChainDepth', label: 'Reconciliation Chain Depth', type: 'number' },
...
const BOOLEAN_FIELDS = [
    { name: 'meshReconciliationAuthorityInitializerAttestation', label: 'Require Mesh Reconciliation Authority Initializer Attestation' },
    { name: 'meshEthicsOversightCommitteeAttestation', label: 'Require Mesh Ethics Oversight Committee Attestation' },
    { name: 'banMalformedOrOutOfOrderMeshStateReconciliationClaims', label: 'Ban Malformed / Out-of-Order Mesh State-Reconciliation Claims' },
...
export function renderMultiEnclaveConfidentialMeshStateReconciliationDashboard() {
    ...
    container.id = 'multi-enclave-confidential-mesh-state-reconciliation-dashboard';
    ...
    title.textContent = 'Multi-Enclave Confidential Mesh State-Reconciliation Gating';
...
function bindMultiEnclaveConfidentialMeshStateReconciliationDashboard(container) {
    const refreshBtn = container.querySelector('#mesh-reconciliation-refresh');
    const validateBtn = container.querySelector('#mesh-reconciliation-validate-policy');

    refreshBtn.addEventListener('click', () => {
        if (!isRefreshing) loadMultiEnclaveConfidentialMeshStateReconciliationData(container);
    });
...
async function loadMultiEnclaveConfidentialMeshStateReconciliationData(container, isInitial) {
    ...
    try {
        ...
        content.innerHTML = '<div class="text-danger">Failed to load multi-enclave confidential mesh state-reconciliation data.</div>';
        window['console']['error']('[MultiEnclaveConfidentialMeshStateReconciliationDashboard] Error:', error.message);
    } finally {
        isRefreshing = false;
...
async function onValidatePolicy(container) {
    const resultDiv = container.querySelector('#mesh-reconciliation-validation-result');
    ...
    try {
        const result = await validateMultiEnclaveConfidentialMeshStateReconciliationPolicy(data);
