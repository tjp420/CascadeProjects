 ...
 */
export const OLLAMA_DEFAULT_URL =
  env.OLLAMA_DEFAULT_URL || "http://127.0.0.1:11434"; // simplebeacon-ignore hardcoded-url — default local Ollama URL for static dashboards
/**
...
export const VSIX_DOWNLOAD_URL =
  ...
  (typeof window !== "undefined" &&
    ...
    window.SIMPLEBEACON_SITE.vsixDownloadUrl) ||
  "https://simplebeacon.ai/downloads/simplebeacon.vsix";
export const EXTENSION_ID =
  env.EXTENSION_ID || "simplebeacon.simplebeacon-vscode";
...
export const LOCAL_AGENT_DOWNLOAD_URL =
  ...
  (typeof window !== "undefined" &&
    window.SIMPLEBEACON_SITE &&
    window.SIMPLEBEACON_SITE.localAgentDownloadUrl) ||
...
export function apiUrl(path) {
  ...
  const detected =
    ...
    "";
  const base = String(envBase || detected)
    .replace(/\/+$/, "")
    .replace(/\/api$/i, "");
...
if (typeof window !== "undefined") {
  (function detectLocalApiBase() {
    try {
      ...
      var endpoints = [
        "/api/platform/status",
        "/api/prompts/get?userId=anonymous",
        "/api/health",
        "/api/theme",
      ];
