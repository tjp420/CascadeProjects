...
function resolveCoverageSnapshot(coverage, baseline, dashboardHome) {
  ...
  const passed =
    merged.passedTests ??
    baseline?.jestTestsPassing ??
    dashboardHome?.overview?.passedTests;
  const total =
    merged.totalTests ??
...
function coveragePendingMessage(coverage) {
  if (coverage?.branchCoverage != null || coverage?.lineCoverage != null)
    return "";
  return (
    coverage?.notes ||
...
export class QualityView {
  ...
  render() {
    ...
    el.querySelector("#run-audit-btn")?.addEventListener("click", () =>
      this.runAudit(),
    );
    el.querySelector("#quality-send-ai-btn")?.addEventListener(
      "click",
      async () => {
        const security = this.app.state.security || {};
        const coverage = resolveCoverageSnapshot(
        ...
        const payload = {
          ...
          reportSummary: {
            lineCoverage:
              coverage.overallCoverage ?? coverage.lineCoverage ?? "N/A",
