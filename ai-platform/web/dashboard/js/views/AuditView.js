...
export class AuditView {
  ...
  render() {
    ...
    if (this.authRequired && !this.audit) {
      ...
      el.querySelector("#audit-import-report")?.addEventListener(
        "click",
        () => {
      ...
      el.querySelector("#audit-retry")?.addEventListener(
        "click",
    ...
    el.querySelector("#audit-download-assessment")?.addEventListener(
      "click",
      () => {
        if (!assessment) {
    ...
    el.querySelector("#audit-open-assessments")?.addEventListener(
      "click",
      () => {
    ...
    el.querySelector("#audit-send-ai-btn")?.addEventListener(
      "click",
      async () => {
        ...
        const vulnList = [];
        const rawVulns =
          npmAudit?.vulnerabilities || npmAudit?.advisories || {};
        if (typeof rawVulns === "object" && rawVulns !== null) {
          for (const [pkg, info] of Object.entries(rawVulns)) {
            if (info && typeof info === "object") {
              ...
              const title =
                info.via?.[0]?.title || info.title || info.overview || "";
        ...
        const payload = {
          ...
          reportSummary: {
            ...
            npmAuditVulnerabilities:
              npmAudit?.summary?.vulnerabilityTotal ?? "N/A",
  ...
  mergeLiveReport(audit) {
    ...
    return {
      ...
      report: {
        ...
        ...live,
        repositoryInventory:
          live.repositoryInventory ?? audit.report?.repositoryInventory,
      },
  ...
  scrollToAssessmentSummary() {
    requestAnimationFrame(() => {
      document
        .getElementById("audit-assessment-summary")
  ...
  async handleAction(action, container) {
    if (action === "results") {
      ...
      const issueCount =
        report?.issueCount ??
        (report?.rawIssues ?? []).reduce((s, i) => s + (i.count || 1), 0);
