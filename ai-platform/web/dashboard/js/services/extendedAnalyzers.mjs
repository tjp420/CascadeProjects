...
export function createExtendedAnalyzers(deps) {
  ...
  function runPromptEngineeringDifficultyAnalyzer(definition, issueId, input = {}) {
    ...
    const expertAvg = expertRows.length
      ? expertRows.reduce((sum, row) => sum + Math.max(1, Number(row.attempts ?? 1)), 0) /
        expertRows.length
      : attemptsToSuccess;
    const noviceAvg = noviceRows.length
      ? noviceRows.reduce((sum, row) => sum + Math.max(1, Number(row.attempts ?? 1)), 0) /
        noviceRows.length
      : attemptsToSuccess;
  ...
  function runLanguageLimitationAnalyzer(definition, issueId, input = {}) {
    ...
    const qualityScores = evaluations.map((row) =>
      clampScore(Number(row.qualityScore ?? (row.passed ? 80 : 40)))
    );
  ...
  function runDependenceRiskAnalyzer(definition, issueId, input = {}) {
    ...
    const positiveHits = text.trim()
      ? DEPENDENCE_POSITIVE_MARKERS.filter((pattern) => pattern.test(text)).length
      : 0;
    const negativeHits = text.trim()
      ? DEPENDENCE_NEGATIVE_MARKERS.filter((pattern) => pattern.test(text)).length
      : 0;
