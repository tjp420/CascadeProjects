...
describe("js/utils.js barrel", () => {
  ...
  it("default export contains all namespaces", () => {
    ...
    for (const key of expected) {
      ...
      assert.strictEqual(
        typeof Utils[key],
        "object",
        `namespace "${key}" should be an object`,
      );
    }
  });
  ...
  it("deepFreeze works on plain objects", async () => {
    ...
    assert.ok(
      Utils.string,
      "namespace should still be accessible even if not frozen",
    );
  });
  ...
  it("barrel re-exports submodule functions (integrity spot-check)", async () => {
    ...
    assert.ok(
      names.has("escapeHtml"),
      "escapeHtml from string.js should be re-exported",
    );
    assert.ok(
      names.has("camelCase"),
      "camelCase from string.js should be re-exported",
    );

    // Spot-check number exports
    assert.ok(names.has("clamp"), "clamp from number.js should be re-exported");
    assert.ok(
      names.has("formatBytes"),
      "formatBytes from number.js should be re-exported",
    );

    // Verify getExportNames itself is present
    assert.ok(
      names.has("getExportNames"),
      "getExportNames should be self-listed",
    );
  });
  ...
  it("new inline utilities appear in getExportNames", () => {
    ...
    assert.ok(
      names.includes("parseJsonSafe"),
      "parseJsonSafe should be exported",
    );
    assert.ok(
      names.includes("parseResponseJson"),
      "parseResponseJson should be exported",
    );
    assert.ok(
      names.includes("stringifySafe"),
      "stringifySafe should be exported",
    );
