...
function sanitizeComplianceForCompleteScan(compliance, gateReport, projectPath, context = {}) {
  ...
  const ruleScoped =
    gateReport?.ruleScopedFilesAnalyzed ?? gateReport?.scanScope?.ruleScopedFilesAnalyzed ?? 0;
  if (ruleScoped === 0) {
...
export function sanitizeCompleteScanBundle(bundle, options = {}) {
  ...
  if (!benchmarkScan) {
    ...
    const fictionJson =
      sbFinal?.fictionJsonFilesScanned ?? sbFinal?.scanScope?.fictionJsonFilesScanned;
