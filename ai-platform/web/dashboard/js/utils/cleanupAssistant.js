...
export function resolveFileReductionPlan(fileReduction) {
  ...
  const unusedCandidates =
    unusedSummary.unusedCandidates ?? fileReduction.summary?.unusedFileCandidates ?? 0;
...
export function buildCleanupAssistantBrief({
  ...
  const inventory = {
    ...
    totalFolders:
      repositoryInventory?.totalFolders ?? fileReduction?.inventory?.totalDirectories ?? null,
  };
  ...
  return {
    ...
    sourceScans: {
      ...
      fileReductionPlanPresent: Boolean(
        fileReduction?.fileReductionPlan || plan?.safeToDelete?.topDirectories?.length
      ),
...
function resolveCleanupScanInputs(lastResult) {
  ...
  if (lastResult.kind === 'complete' && Array.isArray(lastResult.steps)) {
    ...
    return {
      ...
      repositoryInventory:
        cleanupStep?.repositoryInventory || lastResult.repositoryInventory || null,
