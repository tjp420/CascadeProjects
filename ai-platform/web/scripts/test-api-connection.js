...
testAPIServer().then(result => {
    if (result.success) {
        ...
        console.log('   1. Check if API server is running on port 8081');
        console.log('   2. Verify no firewall is blocking the connection');
        console.log('   3. Try starting the API server with: python api/server.py');
        console.log('   4. Check if the server is listening on the correct port');
    }
});
