// Dashboard Sidebar Button Fix - Corrected Version
// Specific fix for the sidebar toggle button in ai_dashboard.html

console.log('🔧 Loading dashboard sidebar button fix v2...');
...
window.toggleSidebar = function() {
    ...
    const _willBeCollapsed = !isCurrentlyCollapsed;
    
    console.log('Current state: ' + (isCurrentlyCollapsed ? 'collapsed' : 'expanded') + ', will become: ' + (shouldBeCollapsed ? 'collapsed' : 'expanded'));
    ...
    // Save preference
    localStorage.setItem('sidebar-collapsed', shouldBeCollapsed);
    
    // Trigger custom event
    window.dispatchEvent(new CustomEvent('sidebarToggle', {
        detail: { collapsed: shouldBeCollapsed, timestamp: Date.now() }
    }));
    
    // Show notification
    showNotification('Sidebar ' + (shouldBeCollapsed ? 'collapsed' : 'expanded') + ' (' + (shouldBeCollapsed ? '60px' : '250px') + ')', 'success');
};
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔧 Dashboard sidebar button fix v2 loaded');
    ...
    console.log('✅ Dashboard sidebar button fix v2 initialized');
});
