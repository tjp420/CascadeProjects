...
class TechnicalDebtAnalyzer {
    ...
    renderOverview() {
        ...
        const _totalDebt = this.data.categories.reduce((sum, cat) => sum + cat.score, 0);
        ...
        const _lowDebt = this.data.categories.filter(cat => cat.severity === 'low').reduce((sum, cat) => sum + cat.score, 0);
    ...
    renderDebtGrid() {
        ...
        container.textContent = this.data.categories.map((category, _index) => `
            <div class="debt-card">
     ...
     */
    highlightRelatedData(_insightCard) {
