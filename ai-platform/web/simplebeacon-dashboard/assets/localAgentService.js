import { g as ve, E as Se, c as P, p as Ae, L as D, V as Ee } from './main.js';
function $e(e) {
...
function _e(e, t = 1, n = {}) {
  const r = pe(e, n.projectRoot);
  if (!r || !de(r)) return null;
  const a =
      n.scheme ||
      (typeof navigator < 'u' && /Cursor/i.test(navigator.userAgent || '') ? 'cursor' : 'vscode'),
    o = $e(r),
    s = Math.max(1, Number(t) || 1);
  return /^\/\//.test(o) || /^[A-Za-z]:\//.test(o)
    ? `${a}://file/${o}:${s}`
    : `${a}://file/${o}:${s}`;
}
function ce(e, t = 1, n = {}) {
...
const G = 'http://127.0.0.1:55432',
  Y = 'http://127.0.0.1:4000',
  le = 'sb_api_base',
  ae = 3e3,
  ue = { windows: D, linux: D, macos: D, unknown: D };
let A = null,
  F = 0;
const Ce = 3e4,
  Be = 12e4;
function fe(e, t) {
  const n = t && t.available ? Ce : Be;
  return e + n > Date.now();
}
let R = null,
  E = null,
...
function He() {
  ...
  if (e)
    try {
      ee(new URL(String(e).replace(/\/api\/?$/, '')).origin) && (P({ updateUrl: !0 }), (t = !0));
    } catch {}
  if (typeof window < 'u' && window.__SB_BRIDGE_HOST__ && ee(window.__SB_BRIDGE_HOST__)) {
    try {
    ...
    : he()
      ? async (e, t) => {
          try {
            ...
            if (
              r.includes('Parent bridge fetch timeout') ||
              r.includes('Parent bridge unavailable')
            )
              return fetch(e, t);
            throw n;
          }
...
function M() {
  ...
  try {
    const e = new URLSearchParams(window.location.search),
      t = e.get(le) || e.get('sb_notify_base');
    if (t) return t;
  } catch {}
  if (typeof sessionStorage < 'u')
    try {
      return sessionStorage.getItem(le) || sessionStorage.getItem('sb_notify_base');
    } catch {}
  return null;
}
function b() {
  ...
  try {
    ...
    return (r !== '127.0.0.1' && r !== 'localhost') || ($() && ee(n.origin)) ? null : n.origin;
  } catch {
    return null;
  }
}
...
const y = 'http://127.0.0.1:11434',
  ge = [54358, 54697, 58681];
function We() {
  return Ee;
}
function qe(e = 'chatbot') {
...
async function ne(e = ge, t = {}) {
  ...
  for (const r of e) {
    ...
    if (!(I(a) && !g()))
      try {
        const o = await n(`${a}/api/health`, { signal: AbortSignal.timeout(2200) });
        if (o.ok) {
          const l = await o.json().catch(() => ({}));
          if (l && (l.service === 'simplebeacon-bridge' || l.platform === 'Simplebeacon'))
            return `${a}/api`;
        }
        const s = await n(`${a}/api/ping`, { signal: AbortSignal.timeout(2200) });
        if (!s.ok) continue;
        const c = await s.json().catch(() => ({}));
        if (c && (c.online === !0 || c.status === 'ok')) return `${a}/api`;
      } catch {}
  }
  return null;
}
async function Je(e = {}) {
  ...
  if (V() && n) {
    if (!T())
      return e.userInitiated && (await H()).ok
  ...
  const r = await ne();
  return r
    ? Ae(r, { websiteMode: !0, updateUrl: !0 })
...
async function Ke(e = y) {
  ...
  try {
    ...
    return (await re(a)).ok;
  } catch {
    return !1;
  }
}
async function Qe(e = y) {
    ...
    return o.ok && c.status === 404
      ...
      : o.ok && c.bridgeReachable && c.error
        ? {
            ...
            corsBlocked: !1,
            status: c.status || 502,
            error: `Extension bridge connected. ${c.error}`,
          }
        : o.ok
          ? {
              ok: !1,
              ...
              status: c.status,
              error:
                c.error || 'Extension bridge could not reach Ollama. Run ollama serve locally.',
            }
          : {
              ok: !1,
...
async function Ye(e) {
  ...
  try {
    ...
    if (
      o.workspace &&
      (String(o.workspace).replace(/\\/g, '/').split('/').pop() || '').toLowerCase() ===
      ...
      : R ||
        ((R = (async () => {
          ...
          try {
            ...
            return ((A = r), (F = Date.now()), r);
          } catch (t) {
            return (
              (A = { available: !1, scannerAvailable: !1, likelyBlocked: !g() && Pe(e, t) }),
              (F = Date.now()),
              A
            );
          } finally {
...
async function nt(e, t = {}, n = G) {
  const r = await C(`${n}/inventory`, {
      method: 'POST',
...
async function at(e, t = G) {
  const n = new URLSearchParams({ projectPath: String(e || '') }),
    r = await C(`${t}/progress?${n}`, { headers: { Accept: 'application/json' } }, 15e3),
    a = await r.json().catch(() => ({}));
      ...
      : O ||
        ((O = (async () => {
          ...
          try {
            ...
            return ((E = a), (j = Date.now()), a);
          } catch {
            const r = { available: !1, extensionBridge: t, origin: e };
            return ((E = r), (j = Date.now()), r);
          } finally {
            O = null;
          }
        })()),
...
async function Ue(e, t, n) {
  ...
  let a = 0;
  for (; Date.now() - r < Ie;) {
    await new Promise((o) => setTimeout(o, Te));
    try {
      if (
        !(
          (
...
async function it(e, t = te()) {
  if (be(t)) {
    const a = _(),
      ...
      s = await o.json().catch(() => ({}));
    if (!o.ok || !s.success)
      throw new Error(s.error || s.warning || `Extension scan failed (${o.status})`);
    return s.scanning
      ? {
          success: !0,
          extensionBridge: !0,
          report: await Ue(t, e, a),
          path: e,
...
function ct(e, t) {
  ...
  ((i.className = 'sb-compliance-report'),
    i.style.setProperty('--sb-grade-color', n.badgeColor || '#6366f1'));
  const p = document.createElement('div');
  p.className = 'sb-compliance-hero';
  const f = document.createElement('div');
  f.className = 'sb-compliance-hero-main';
  const W = document.createElement('div');
  ((W.className = 'sb-compliance-eyebrow'), (W.textContent = 'Scan complete'));
  const q = document.createElement('h3');
  ((q.className = 'sb-compliance-title'), (q.textContent = 'Compliance summary'));
  const J = document.createElement('p');
  ((J.className = 'sb-compliance-status'),
    (J.textContent = n.complianceStatus || 'Review required'),
    f.appendChild(W),
    f.appendChild(q),
    f.appendChild(J));
  ...
  if (
    ((Z.className = 'sb-compliance-actions'),
    [
      ...
      ((m.type = 'button'),
        (m.className =
          d.route === 'results' ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'),
    ...
    const se = () => {
      ...
      v === 'issues'
        ? (S = a)
        : v === 'clean' && (S = r.filter((u) => !u.status || u.status === 'Clean'));
      const w = v === 'all' ? 80 : 120;
      if (
        (S.slice(0, w).forEach((u) => {
          const B = document.createElement('div'),
            ie = !u.status || u.status === 'Clean';
          B.className = `sb-compliance-file-row${ie ? ' is-clean' : ' is-issue'}`;
          const K = document.createElement('span');
          ((K.className = 'sb-compliance-file-badge'),
            (K.textContent = ie ? 'Clean' : String(u.status)));
          ...
          ((L.className = 'sb-compliance-file-path'),
            ...
            (u.absolutePath || u.name) &&
              ...
              L.addEventListener('click', () => {
                ce(u.absolutePath || u.name, u.line || 1, { projectRoot: l });
              })));
        ...
        w.addEventListener('click', () => {
          ((v = S.id),
            h
              ...
              .forEach((u) => u.classList.remove('is-active')),
  ...
  t.appendChild(i);
  const oe = t.closest('#analyze-path-dropzone');
  oe && oe.classList.add('has-compliance-report');
}
function lt(e, t) {
  return !(t != null && t.available) || !(t != null && t.scannerAvailable) ? !1 : Oe(e);
}
function k() {
...
const je = { available: !1, scannerAvailable: !1, hostedSkipped: !0 },
  ze = { available: !1, scannerAvailable: !1, integratedSkipped: !0 };
function ut(e) {
...
export {
  ...
  Ze as buildBridgeOllamaChatUrls,
  Re as buildBridgeOllamaProbeUrls,
  qe as buildExtensionConnectDeepLink,
  he as canUseParentBridgeFetch,
  He as clearStaleIntegratedBridgeParams,
  ye as detectPlatform,
  Je as discoverAndApplyExtensionBridge,
  nt as fetchInventoryViaAgent,
  at as fetchScanProgressViaAgent,
  ot as fetchScanProgressViaExtensionBridge,
  Ye as findFolderViaBridge,
  ut as formatAgentStatus,
  dt as getAgentDownloadUrl,
  pt as getAgentFallbackMessage,
  me as getBridgeFetch,
  b as getExtensionBridgeOrigin,
  ht as getInstallInstructions,
  _ as getLocalBridgeFetch,
  ft as getPlatformLabel,
  We as getVsixDownloadUrl,
  z as hasExplicitBridgeParam,
  V as hasExtensionBridgeConfigured,
  k as isHostedHttpsDashboard,
  $ as isIntegratedLocalDashboard,
  Oe as isLocalPath,
  et as pickFolderViaExtensionBridge,
  tt as probeAgent,
  st as probeAgent4000,
  H as probeExtensionBridgeHealth,
  ne as probeExtensionDataServer,
  Ke as probeLocalOllama,
  Qe as probeUserInitiatedOllama,
  ct as renderAgentCertificate,
  Ne as resolveOllamaProxyUrl,
  rt as scanViaAgent,
  it as scanViaAgent4000,
  De as shouldProbeAgent4000,
  Fe as shouldProbeLocalAgent,
  lt as shouldUseAgent,
  Xe as validateExtensionBridgeOnLoad,
};
