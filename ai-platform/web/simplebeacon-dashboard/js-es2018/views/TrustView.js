...
function extractLiveTrustData(data) {
  ...
  const headlineObj =
    typeof headline === 'object' && headline && headline.primary ? headline : null;
  return {
...
function renderSnapshot(snap, title) {
  ...
  const repoFiles =
    snap.repositoryFilesTotal != null ? formatNumber(snap.repositoryFilesTotal) : '—';
  const gateChecked =
    snap.ruleScopedFilesAnalyzed != null ? formatNumber(snap.ruleScopedFilesAnalyzed) : '—';
  const mockSample = snap.mockSampleFiles != null ? formatNumber(snap.mockSampleFiles) : '—';
  const fictionJson =
    snap.fictionJsonFilesScanned != null ? formatNumber(snap.fictionJsonFilesScanned) : '—';
