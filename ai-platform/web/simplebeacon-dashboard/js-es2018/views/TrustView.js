...
function extractLiveTrustData(data) {
  ...
  const headlineObj =
    typeof headline === "object" && headline && headline.primary
      ? headline
      : null;
  return {
    verificationId: source.verificationId || null,
    headlineSource: headlineObj
      ? headlineObj.source || null
      : source.headlineSource || null,
    headlineReason: headlineObj
      ? headlineObj.reason || null
      : source.headlineReason || null,
    headline: headlineObj
      ? headlineObj.primary || null
...
function renderSnapshot(snap, title) {
  ...
  const issueCount =
    snap.issueCount != null ? formatNumber(snap.issueCount) : "—";
  const schema =
    (snap.schemaPassed != null ? snap.schemaPassed : "—") +
    "/" +
    (snap.schemaChecked != null ? snap.schemaChecked : "—");
  const consistency =
    snap.consistencyScore != null ? snap.consistencyScore + "%" : "—";
  const repoFiles =
    snap.repositoryFilesTotal != null
      ? formatNumber(snap.repositoryFilesTotal)
      : "—";
  const gateChecked =
    snap.ruleScopedFilesAnalyzed != null
      ? formatNumber(snap.ruleScopedFilesAnalyzed)
      : "—";
  const mockSample =
    snap.mockSampleFiles != null ? formatNumber(snap.mockSampleFiles) : "—";
  const fictionJson =
    snap.fictionJsonFilesScanned != null
      ? formatNumber(snap.fictionJsonFilesScanned)
      : "—";
...
export class TrustView {
  ...
  render() {
    ...
    const reAttestation =
      this.app && this.app.state && this.app.state.reAttestation;
  ...
  downloadTrustData() {
    ...
    downloadJson(
      payload,
      `trust-verification-${new Date().toISOString().slice(0, 10)}.json`,
    );
  }
  async mount(container) {
    ...
    try {
      ...
      this.loading = false;
      setHtml(container, this.render());
      (_a = container.querySelector("#trust-download-json")) === null ||
      _a === void 0
        ? void 0
      ...
      (_b = container.querySelector("#trust-send-ai-btn")) === null ||
      _b === void 0
        ? void 0
        : _b.addEventListener("click", async () => {
            ...
            const payload = {
              ...
              reportSummary: {
                ...
                repositoryHealthScore:
                  (_c =
                    (_b =
                      (_a = live.repositoryHealth) === null || _a === void 0
                        ? void 0
                        : _a.headline) === null || _b === void 0
                      ? void 0
              ...
              notes:
                "Trust Verification — scoped scan results and repository health",
            };
            ...
            if (vscode) {
              try {
                ...
                window["console"]["warn"](
                  "[Trust-AI] vscode.postMessage failed:",
                  err,
                );
