...
function normalizeStaticRepositoryHealthPayload(payload) {
  const health =
    payload === null || payload === void 0 ? void 0 : payload.repositoryHealth;
  ...
  return {
    ...
    recommendations: Array.isArray(health.recommendations)
...
async function fetchStaticRepositoryHealthFallback() {
  const trustHttpResponse = await fetch("/trust-verification.json", {
    cache: "no-store",
  }).catch(() => null);
  if (!trustHttpResponse || !trustHttpResponse.ok) return null;
  const trustVerificationDocument = await trustHttpResponse
    .json()
    .catch(() => null);
...
export async function fetchRepositoryHealth() {
  ...
  if (!res.ok || data.success === false) {
    throw new Error(
      data.error || data.message || "Failed to load repository health",
    );
  }
  return data;
}
...
function renderHealthSnapshot(snap, title) {
  ...
  const scoreClass =
    snap.repositoryHealthScore >= 80
      ? "pass"
      : snap.repositoryHealthScore >= 60
        ? "warn"
        : "fail";
  return `
    <div class="card mb-4">
      ...
      <div class="metrics-row mb-4">
        <div class="metric-chip"><strong>${formatNumber((_a = snap.repositoryFilesTotal) !== null && _a !== void 0 ? _a : "—")}</strong> repo files</div>
        <div class="metric-chip"><strong>${formatNumber((_b = snap.repositoryFoldersTotal) !== null && _b !== void 0 ? _b : "—")}</strong> folders</div>
        <div class="metric-chip"><strong>${escapeHtml(snap.optimizationPotential || "—")}</strong> savings</div>
        <div class="metric-chip"><strong>${(_c = snap.duplicateGroups) !== null && _c !== void 0 ? _c : "—"}</strong> dup groups</div>
        <div class="metric-chip"><strong>${(_d = snap.oversizedFiles) !== null && _d !== void 0 ? _d : "—"}</strong> oversized</div>
        <div class="metric-chip"><strong>${(_e = snap.reductionOpportunities) !== null && _e !== void 0 ? _e : "—"}</strong> reductions</div>
      </div>
 ...
 * @returns {any}
 */
export function renderRepositoryHealthSection(
  health,
  { compact = false } = {},
) {
  ...
  const scoreClass =
    h.repositoryHealthScore >= 80
      ? "pass"
      : h.repositoryHealthScore >= 60
        ? "warn"
        : "fail";
  if (compact) {
    return `
...
export class RepositoryHealthView {
  ...
  exportHealthData() {
    ...
    downloadJson(
      payload,
      `repository-health-${new Date().toISOString().slice(0, 10)}.json`,
    );
  ...
  render() {
    ...
    const staticHost = Boolean(
      health === null || health === void 0 ? void 0 : health.staticHost,
    );
    return `
      <div class="analyze-hero">
        <h1 class="page-title">Repository Health</h1>
        ...
        ${
          ...
          <div class="card mb-4">
            ...
            <div class="metrics-row">
              ...
              <div class="metric-chip"><strong>${(_a = headline.duplicateGroups) !== null && _a !== void 0 ? _a : "—"}</strong> exact duplicate groups</div>
              <div class="metric-chip"><strong>${(_b = headline.oversizedFiles) !== null && _b !== void 0 ? _b : "—"}</strong> oversized files</div>
              <div class="metric-chip"><strong>${(_c = headline.reductionOpportunities) !== null && _c !== void 0 ? _c : "—"}</strong> reduction opportunities</div>
              <div class="metric-chip"><strong>${formatNumber((_d = headline.repositoryFilesTotal) !== null && _d !== void 0 ? _d : "—")}</strong> repo files</div>
  ...
  resolvePreviewProjectPath() {
    if (
      this.candidatesProjectPath !== undefined &&
      this.candidatesProjectPath !== null
    ) {
  ...
  scrollPreviewIntoView(container) {
    requestAnimationFrame(() => {
      var _a;
      (_a = container.querySelector("#merge-preview-panel")) === null ||
      _a === void 0
        ? void 0
  ...
  bindEvents(container) {
    var _a, _b, _c, _d;
    this._root = container;
    (_d = container.querySelector("#export-health-json")) === null ||
    _d === void 0
      ? void 0
      : _d.addEventListener("click", () => this.exportHealthData());
    (_a = container.querySelector("#run-optimization-scan")) === null ||
    _a === void 0
      ? void 0
    ...
    (_b = container.querySelector("#quarantine-merge-btn")) === null ||
    _b === void 0
      ? void 0
      : _b.addEventListener("click", async () => {
          ...
          try {
            ...
            showToast(
              data.message || "Duplicates quarantined successfully",
              "success",
            );
            this.preview = null;
            this.previewError = null;
    ...
    (_c = container.querySelector("#send-health-ai-btn")) === null ||
    _c === void 0
      ? void 0
      : _c.addEventListener("click", async () => {
          ...
          if (vscode) {
            try {
              ...
              showToast(
                "Repository health sent to your AI coding agent",
                "success",
              );
              return;
            } catch (err) {
              window["console"]["warn"](
                "[Health-AI] vscode.postMessage failed:",
                err,
              );
            }
          }
    ...
    container.addEventListener("click", (event) => {
      ...
      if (!candidateId) {
        showToast(
          "Merge candidate id missing — re-run consolidation scan",
          "error",
        );
        return;
      }
