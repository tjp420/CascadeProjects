...
import {
  ...
  blockKey,
  releaseKey,
} from "../services/interdictionService.js";
import {
...
export class SecurityView {
  ...
  async handleExportCompliance(format = "json") {
    try {
      const authHeaders = this.app.authService
        ? this.app.authService.getAuthHeaders()
        : {};
      ...
      if (res && typeof res === "object") {
        ...
        showToast(
          id
            ? `Compliance report generated: ${id}`
  ...
  async loadTelemetry() {
    ...
    try {
      const { authService } =
        await import("../services/authService.js?v=20260722bridgefix1");
  ...
  renderKeyManagementSection() {
    ...
    const graceText = hasPrevious
      ? formatGraceCountdown(s.rotatedAt, s.graceMs)
      : "—";
    const graceColor =
      hasPrevious && s.graceExpired
        ? "var(--danger)"
        : hasPrevious
          ? "var(--warning)"
          : "var(--text-muted)";
    ...
    const rotatedDate =
      s && s.rotatedAt ? new Date(s.rotatedAt).toLocaleString() : "—";
    return `
      <div class="section-block">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--space-4);">
  ...
  async handlePurgeStaleKeys() {
    ...
    try {
      ...
      const { purgeStaleKeys } =
        await import("../services/keyManagementService.js?v=20260722bridgefix1");
      ...
      const purged =
        res && (res.purged || res.purgedCount || res.deleted)
          ...
          : 0;
      showToast(
        `Purge complete: ${purged} stale key(s) removed`,
        purged > 0 ? "success" : "info",
      );
      await this.loadKeyStatus();
    } catch (err) {
  ...
  async handleBlockKey() {
    ...
    try {
      ...
      showToast(
        `Key ${apiKey.slice(0, 4)}… blocked for ${durationMin} min`,
        "success",
      );
      keyInput.value = "";
  ...
  async handleReleaseKey(maskedKey) {
    try {
      ...
      if (result.wasBlocked) {
        ...
        showToast(
          `Key ${maskedKey.slice(0, 4)}… was not actively blocked`,
          "info",
        );
      }
      await this.loadInterdiction();
  ...
  renderRetentionSection() {
    ...
    const oldest = stats.oldestTimestamp
      ? new Date(stats.oldestTimestamp).toLocaleString()
      : "—";
    const newest = stats.newestTimestamp
      ? new Date(stats.newestTimestamp).toLocaleString()
      : "—";
    const autoPurge = stats.autoPurgeStats || {
      runs: 0,
      purged: 0,
      archived: 0,
      failed: 0,
    ...
    const formattedLastRun = autoPurge.lastRun
      ? new Date(autoPurge.lastRun).toLocaleString()
      : "Never";
  ...
  renderControlMappings(report) {
    const mappings =
      report && report.controlMappings ? report.controlMappings : [];
    ...
    const toggleLabel = this.showControlMappings
      ? "Hide control mappings"
  ...
  async handleSavePolicy() {
    ...
    try {
      ...
      await updateRetentionConfig(
        { retentionDays, maxEntries, archive },
        authHeaders,
      );
  ...
  async handlePurge() {
    ...
    try {
      ...
      if (result.purged === 0) {
        showToast(
          "Nothing to purge — no entries older than retention period",
          "info",
        );
      } else {
        showToast(
  ...
  async loadQuarantine() {
    ...
    try {
      const authHeaders = this.app.authService
        ? this.app.authService.getAuthHeaders()
        : {};
      this.quarantine = await fetchQuarantineEntries(
        this.quarantineAllOrgs,
        authHeaders,
      );
    } catch (err) {
  ...
  async handleVerifyEntry(entryId) {
    ...
    try {
      const authHeaders = this.app.authService
        ? this.app.authService.getAuthHeaders()
        : {};
      const result = await verifyQuarantineEntry(
        entryId,
        undefined,
        authHeaders,
      );
      ...
      showToast(
        result.hashMatches
          ? "Entry hash verified — match"
          : "Entry hash mismatch detected",
  ...
  render() {
    ...
    const gateIcon = summary.gatePass
      ? "✅"
      : summary.gatePass === false
        ? "⚠️"
        : "❓";
    ...
    const complianceScore =
      (_c =
        (_b = this.compliance) === null || _b === void 0
          ? void 0
          : _b.securityScore) !== null && _c !== void 0
        ? _c
        : null;
    const totalScanned =
      (summary.credentialScanned || 0) + (summary.productionLeakScanned || 0);
    el.innerHTML = `
      <!-- Hero -->
    ...
    if (_tlRefresh)
      _tlRefresh.addEventListener("click", () => this.loadTelemetry());
    if (_tlRetry)
      _tlRetry.addEventListener("click", () => this.loadTelemetry());
      ...
      : _f.addEventListener("click", async () => {
          ...
          const payload = {
            ...
            reportSummary: {
              ...
              complianceScore:
                (_b =
                  (_a = this.compliance) === null || _a === void 0
                    ? void 0
                    : _a.securityScore) !== null && _b !== void 0
                  ? _b
                  : "N/A",
            },
            notes:
    ...
    if (_kmRefresh)
      _kmRefresh.addEventListener("click", () => this.loadKeyStatus());
    if (_kmRetry)
      _kmRetry.addEventListener("click", () => this.loadKeyStatus());
    if (_kmRotate)
      _kmRotate.addEventListener("click", () => this.handleKeyRotation());
    if (_kmGenerate)
      _kmGenerate.addEventListener("click", () => this.handleGenerateKey());
    if (_kmRekeyNow)
      _kmRekeyNow.addEventListener("click", () => this.handleForceReKey());
    if (_kmPurge)
      _kmPurge.addEventListener("click", () => this.handlePurgeStaleKeys());
    ...
    if (_iRefresh)
      _iRefresh.addEventListener("click", () => this.loadInterdiction());
    if (_iRetry)
      _iRetry.addEventListener("click", () => this.loadInterdiction());
    if (_iBlockBtn)
      _iBlockBtn.addEventListener("click", () => this.handleBlockKey());
    // Wire up per-row release buttons
    if (this.interdiction && this.interdiction.keys) {
      for (const entry of this.interdiction.keys) {
        const releaseBtn = el.querySelector(
          `#interdiction-release-${CSS.escape(entry.apiKey || "")}`,
        );
        if (releaseBtn)
          releaseBtn.addEventListener("click", () =>
            this.handleReleaseKey(entry.apiKey),
          );
      }
    }
    ...
    if (_rRefresh)
      _rRefresh.addEventListener("click", () => this.loadRetention());
    ...
    if (_rPurgeBtn)
      _rPurgeBtn.addEventListener("click", () => this.showPurgeConfirmation());
    if (_rPurgeCancel)
      _rPurgeCancel.addEventListener("click", () => this.cancelPurge());
    if (_rPurgeConfirm)
      _rPurgeConfirm.addEventListener("click", () => this.handlePurge());
    ...
    if (_cGen)
      _cGen.addEventListener("click", () =>
        this.handleGenerateComplianceReport(),
      );
    if (_cCsv)
      _cCsv.addEventListener("click", () => this.handleDownloadComplianceCsv());
    ...
    if (_qRefresh)
      _qRefresh.addEventListener("click", () => this.loadQuarantine());
    ...
    if (this.quarantine && this.quarantine.entries) {
      for (const entry of this.quarantine.entries) {
        const toggleBtn = el.querySelector(
          `#quarantine-toggle-${CSS.escape(entry.id)}`,
        );
        if (toggleBtn)
          toggleBtn.addEventListener("click", () =>
            this.toggleQuarantineEntry(entry.id),
          );
        const verifyBtn = el.querySelector(
          `#quarantine-verify-${CSS.escape(entry.id)}`,
        );
        if (verifyBtn)
          verifyBtn.addEventListener("click", () =>
            this.handleVerifyEntry(entry.id),
          );
      }
    }
    return el;
