...
export function renderInsights(report, baseline, dashboardHome) {
  ...
  const healthClass =
    totalIssues === 0 ? "success" : totalIssues <= 5 ? "warning" : "danger";
  const healthLabel =
...
export class DashboardView {
  ...
  renderHeader(report) {
    ...
    header.className =
      "dashboard-header d-flex justify-content-between align-items-center mb-4";

    const projectName = report
      ? (report.projectRoot || report.projectPath || "Active Project")
          .split(/[\\/]/)
          .pop()
      : "No Active Project";
    const statusChip =
    ...
    const adminBtnHtml =
      this.app &&
      typeof this.app.isCurrentUserAdmin === "function" &&
      ...
      this._cliUploadZone = new CliReportUploadZone(zoneContainer, {
        onReportLoaded: (adapted, raw) => {
          ...
          if (this.app.scanService) {
            try {
              this.app.scanService.notifyReportListeners(raw);
            } catch {
              /* ignore */
            }
          }
  ...
  renderTeamCiMetrics(metrics, teamExtras = {}) {
    ...
    const blocked =
      metrics.merges_blocked_this_week ?? metrics.gates_tripped ?? 0;
    const criticals = metrics.criticals_blocked ?? 0;
    const hasTeamTelemetry =
      metrics.gate_pass_rate != null ||
      (trend && Array.isArray(trend.trend)) ||
      (distribution && distribution.p50 != null);

    const gatePassRateHtml =
      metrics.gate_pass_rate != null
        ? `
    ...
    const distributionHtml =
      dist && dist.sampleSize > 0
        ? this.renderQualityDistributionStrip(dist)
        : "";

    const sources = metrics.scan_sources || null;
    const sourcesHtml = sources ? this.renderScanSourcesBreakdown(sources) : "";

    const kAnonymityHtml =
      metrics.k_anonymity_met === false
        ? `
  ...
  renderQualityDistributionStrip(dist) {
    ...
    const toPct = (value) => {
      ...
      return Math.min(
        100,
        Math.max(0, ((n - scaleMin) / (scaleMax - scaleMin)) * 100),
      );
    };
  ...
  renderScanSourcesBreakdown(sources) {
    ...
    const total = entries.reduce(
      (sum, e) => sum + (Number(sources[e.key]) || 0),
      0,
    );
    if (total <= 0) {
      return "";
  ...
  async loadCiTeamMetrics(view) {
    ...
    if (trend && Array.isArray(trend.trend)) {
      requestAnimationFrame(() => {
        ...
        if (trendContainer) {
          this._teamTrendCleanup =
            mountTeamGatePassTrendChart(trendContainer, trend.trend) || null;
        }
      });
    }
  }
  ...
  renderResultsState(report, categories) {
    ...
    const gateScore =
      typeof (report.gate && report.gate.score) === "number"
        ? report.gate.score
        : null;
    const displayScore =
      resolveDisplayScore(report) != null
        ? resolveDisplayScore(report)
        : gateScore;
    ...
    const displayFilesAnalyzed =
      repoTotal > 0 && rawFilesAnalyzed > repoTotal
        ? repoTotal
        : rawFilesAnalyzed;
    // content where applicable.
    setSafeHTML(
  ...
  bindScanPanel(view) {
    ...
    bindScanStatus(scanSlot, {
      ...
      onLocalScanResult: (payload) => {
        if (!payload) return;
        const projectPath =
          payload.projectPath || payload.verifiedAddress || payload.path || "";
        if (projectPath) {
          this.app.state.lastProjectPath = projectPath;
        }
        if (payload.files || payload.certificate) {
          const sbReport = convertSandboxReportToSimplebeacon(
            payload,
            projectPath,
          );
  ...
  refreshScanStatus() {
    ...
    if (!updated) {
      const reviewMode =
        Boolean(this.app.state.report) && !this.app.state.scanning;
  ...
  handleHarExport() {
    try {
      if (!this._harExporter) {
        showToast(
          "HAR recorder not started — reload the page and try again",
          "error",
        );
        return;
      }
      ...
      if (entryCount === 0) {
        showToast(
          "No network requests captured yet — interact with the dashboard first",
          "info",
        );
        return;
      }
      ...
      showToast(
        "HAR export failed: " + ((err && err.message) || String(err)),
        "error",
      );
    }
  }
  ...
  async handleSendAi() {
    ...
    try {
      const res = await fetch("/api/ai-context", {
        ...
        body: JSON.stringify({
          projectPath:
            report.projectRoot || report.projectPath || window.location.origin,
          notes: "",
          reportSummary,
          issues: allIssues,
        }),
  ...
  mount(container) {
    ...
    if (this._teamTrendCleanup) this._teamTrendCleanup();
    this.stopScanProgressPolling();
    if (!this._harExporter) {
    ...
    requestAnimationFrame(() => {
      ...
      this._trendCleanup =
        mountTrendChart(trendSlot, this.app.state.history) || null;
    });
    requestAnimationFrame(() => {
      const policySlot = view.querySelector("#slot-policy-editor");
      if (policySlot) {
        this._policyEditorCleanup =
          mountPolicyEditor(policySlot, this.app) || null;
      }
    });
  ...
  destroy() {
    if (this._trendCleanup) this._trendCleanup();
    if (this._teamTrendCleanup) this._teamTrendCleanup();
    if (this._policyEditorCleanup) this._policyEditorCleanup();
    this.stopScanProgressPolling();
  }
}
