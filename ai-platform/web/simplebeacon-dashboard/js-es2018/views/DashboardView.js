...
export class DashboardView {
  ...
  renderResultsState(report, categories) {
    ...
    const gateScore =
      typeof (report.gate && report.gate.score) === 'number' ? report.gate.score : null;
    const displayScore =
      resolveDisplayScore(report) != null ? resolveDisplayScore(report) : gateScore;
    ...
    const displayFilesAnalyzed =
      repoTotal > 0 && rawFilesAnalyzed > repoTotal ? repoTotal : rawFilesAnalyzed;
    // content where applicable.
    setSafeHTML(
      grid,
      `
