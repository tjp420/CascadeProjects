...
import { fetchRepositoryInventory } from '../services/analyzeService.js?v=20260726sevfix1';
/**
...
function renderTreeHtml(node, name, depth = 0) {
  ...
  const indent =
    depth > 0 ? `<span style="display:inline-block;width:${depth * 16}px"></span>` : '';
  const isLeaf =
    keys.length === 0 || (keys.length === 1 && Object.keys(node[keys[0]]).length === 0);
  const icon = isLeaf ? '📄' : '📁';
  const caret = isLeaf
    ? ''
