...
export class SignInView {
      ...
      if (token && this._looksLikeJwt(token)) {
        ...
        if (payload && payload.email) {
          authService.setSession(token, {
            email: payload.email,
            tier:
              payload.tier || payload.product || payload.plan || "community",
            role:
              payload.role ||
              payload.tier ||
              payload.product ||
  ...
  async mount(container, mode = null) {
    ...
    if (!authed) {
      ...
      (_b = container.querySelector("#signin-email-form")) === null ||
      _b === void 0
        ? void 0
        : _b.addEventListener("submit", (e) => this.handleEmailSubmit(e));
      (_c = container.querySelector("#forgot-password-btn")) === null ||
      _c === void 0
        ? void 0
        : _c.addEventListener("click", () => this._showRecoveryModal());
      (_d = container.querySelector("#webauthn-signin-btn")) === null ||
      _d === void 0
        ? void 0
        : _d.addEventListener("click", () => this._handleWebAuthnSignIn());
      (_f = container.querySelector("#signin-token-form")) === null ||
      _f === void 0
        ? void 0
        : _f.addEventListener("submit", (e) => this.handleTokenSubmit(e));
    } else {
      (_e = container.querySelector("#signin-signout-btn")) === null ||
      _e === void 0
        ? void 0
  ...
  bindEmailModeToggle(container) {
    ...
    container
      .querySelector("#note-goto-register")
    ...
    container
      .querySelector("#note-goto-signin")
      ?.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.app.navigate("signin");
    ...
    container
      .querySelector("#goto-register-btn")
      ?.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.app.navigate("register");
  ...
  async handleEmailSubmit(e) {
    ...
    const confirmPasswordEl = form.querySelector(
      "#signin-confirm-password-input",
    );
    ...
    if (this._emailMode === "register") {
      ...
      const username =
        form
          .querySelector("#signin-username-input")
          ?.value.trim()
          .toLowerCase() || "";
    ...
    submitBtn.textContent =
      this._emailMode === "register" ? "Creating account…" : "Signing in…";
    if (errorEl) {
      errorEl.hidden = true;
      errorEl.textContent = "";
    }
    try {
      if (this._emailMode === "register") {
        const name =
          form.querySelector("#signin-name-input")?.value.trim() || "";
        const username =
          form
            .querySelector("#signin-username-input")
            ?.value.trim()
        ...
        if (result.pending) {
          ...
          if (card) {
            ...
            card
              .querySelector("#pending-goto-signin")
              ?.addEventListener("click", () => this.app.navigate("signin"));
          }
          return;
        }
      ...
      this.app.updateAuthUi();
      (_b = (_a = this.app).bootstrapAfterAuth) === null || _b === void 0
        ? void 0
        : _b.call(_a);
      this.app.navigate("dashboard");
    } catch (err) {
      ...
      if (
        ...
        message =
          "Unable to reach the account server. Check your connection and try again.";
      }
      window["console"]["error"]("[SignInView] submit error:", {
        mode: this._emailMode,
        error: err,
      });
      if (errorEl) {
