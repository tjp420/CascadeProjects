...
function renderChatbotOllamaModelSelect(state) {
  const selected = matchOllamaModelOption(
    state.ollamaModel,
    state.ollamaModels,
  );
...
function readOllamaVerifiedFromSession() {
  try {
    return (
      ...
      sessionStorage.getItem(OLLAMA_VERIFIED_KEY) === "1"
    );
  } catch (_a) {
    return false;
  }
...
function mergeProviderAvailability(providers, keyMap = {}, platformEnv = null) {
  return providers.map((provider) => {
    ...
    const label =
      provider.available && !configured && platform
...
async function resolveChatbotProviders(options = {}) {
  ...
  try {
    const keys = await fetchUserAiKeys(
      options.refreshKeys ? { refresh: true } : {},
    );
  ...
  let providers =
    apiProviders.length > 0
      ...
      : HARDCODED_PROVIDERS.map((provider) => ({
          ...provider,
          available: Boolean(
            keyMap[provider.id]?.configured || platformEnv?.[provider.id],
          ),
          label:
  ...
  const ollamaFromBridgeApi = Boolean(
    providers.find(
  ...
  const hasSavedCloudKeys = Boolean(
    keyMap.openai?.configured || keyMap.anthropic?.configured,
  );
  return {
    providers,
    available,
...
function markOllamaProviderConnected(providers, modelName = "") {
  ...
  if (ollama) {
    ...
    next.unshift({
      id: "ollama",
      label: `Ollama (local · 127.0.0.1:11434${modelLabel})`,
      available: true,
    });
  }
...
const PERSONALITY_PROMPTS = {
  ...
  professional:
    "You are a professional, concise code assistant for the SimpleBeacon platform.",
  casual:
    "You are a friendly, relaxed code assistant for the SimpleBeacon platform.",
  sarcastic:
    "You are a witty, sarcastic code assistant for the SimpleBeacon platform.",
  technical:
    "You are a deeply technical code assistant for the SimpleBeacon platform.",
  creative:
    "You are a creative, exploratory code assistant for the SimpleBeacon platform.",
  oracle:
    "You are The Unbreakable Oracle, an omniscient code assistant for the SimpleBeacon platform.",
};
...
function buildChatbotSystemPrompt(options = {}) {
  ...
  const personalityPrompt =
    PERSONALITY_PROMPTS[options.personality || "helpful"] ||
    PERSONALITY_PROMPTS.helpful;
...
export class ChatbotView {
  ...
  mount(container) {
    ...
    if (!this._aiKeysListenerBound) {
      this._onAiKeysUpdated = () => {
        void this.refreshProviders({ force: true }).catch(() => {
          /* ignore */
        });
      };
  ...
  async maybeAutoConnectBridgeOllama() {
    ...
    if (statusEl)
      statusEl.textContent =
        "Checking extension bridge (allow local network access if prompted)…";
    ...
    try {
      ...
      baseUrl =
        String(keys?.ollamaBaseUrl || BROWSER_OLLAMA_URL).replace(/\/$/, "") ||
        BROWSER_OLLAMA_URL;
    } catch (_a) {
      /* default */
    }
    ...
    if (probe.ok) {
      ...
      const connectStatus = document.getElementById(
        "chatbot-ollama-connect-status",
      );
      if (connectStatus)
        connectStatus.textContent =
          "Connected — Ollama is reachable via the extension bridge.";
    ...
    if (statusEl && health.ok)
      statusEl.textContent =
        "Extension bridge online — Ollama not connected yet.";
  }
  async refreshProviders(options = {}) {
    ...
    if (
      !options.force &&
      this._lastRefreshAt &&
      Date.now() - this._lastRefreshAt < 5000
    ) {
  ...
  updateOllamaSetupVisibility(resolved) {
    if (!isHostedDashboard()) return;
    const ollamaUp = resolved.available.some(
      (provider) => provider.id === "ollama",
    );
    const cloudUp = resolved.available.some(
      (provider) => provider.id === "openai" || provider.id === "anthropic",
    );
  ...
  getSelectedOllamaModel() {
    const fromSelect = document
      .getElementById("chatbot-ollama-model")
      ?.value?.trim();
    return (
      fromSelect ||
      this.ollamaModel ||
  ...
  async loadChatbotOllamaModels(options = {}) {
    ...
    try {
      ...
      const picked = matchOllamaModelOption(
        this.ollamaModel,
        this.ollamaModels,
      );
      if (picked) {
  ...
  syncSettingsFromUi() {
    ...
    const removeFiltersCheckbox = document.getElementById(
      "chatbot-remove-filters",
    );
  ...
  async tryConnectExtensionBridge() {
    ...
    const result = await discoverAndApplyExtensionBridge({
      userInitiated: true,
    });
    this._extensionConnectInFlight = false;
    if (connectBtn) {
      connectBtn.disabled = false;
      connectBtn.textContent = result.ok
        ? "Extension connected"
        : "Connect extension";
    }
    if (result.ok) {
  ...
  async tryConnectLocalOllama() {
    ...
    if (probe.ok) {
      ...
      if (statusEl)
        statusEl.textContent =
          "Connected — Ollama is reachable from this browser.";
      await this.refreshProviders({ force: true });
      void this.loadChatbotOllamaModels().catch(() => {});
      return;
    }
    if (statusEl) {
      const health = hasExtensionBridgeConfigured()
        ? await probeExtensionBridgeHealth()
        : null;
      if (probe.error) {
  ...
  applyProviderSelect(resolved) {
    ...
    if (firstAvailable) {
      ...
      this.showErrorBanner(getNoProviderMessage(resolved), false, {
        showSettingsLink:
          resolved.keysSessionReady !== false &&
          !hasExtensionBridgeConfigured(),
        showOllamaSetup: !isIdeDashboardSurface(),
        showRetry: hasExtensionBridgeConfigured(),
        showSignInLink:
          !hasExtensionBridgeConfigured() &&
          (!authService.isAuthenticated() || !resolved.keysSessionReady),
      });
    }
  }
    ...
    bindRoot?.querySelectorAll("[data-copy]").forEach((el) => {
      el.addEventListener("click", () => {
        ...
        void copyToClipboard(text)
          ...
          .catch(() =>
            this.showPromptToast("Copy failed — select text manually"),
          );
      });
    });
  }
  ...
  bindEvents() {
    ...
    if (promptToggle && promptPanel) {
      promptToggle.addEventListener("click", () => {
        promptPanel.style.display =
          promptPanel.style.display === "none" ? "block" : "none";
      });
    }
    if (promptSave && promptTextarea) {
      promptSave.addEventListener("click", async () => {
        ...
        const userId =
          ((_c =
            (_b =
              (_a = this.app) === null || _a === void 0 ? void 0 : _a.state) ===
              null || _b === void 0
              ? void 0
    ...
    if (settingsToggle && settingsPanel) {
      settingsToggle.addEventListener("click", () => {
        ...
        if (
          opening &&
          (this.useBrowserOllama || hasExtensionBridgeConfigured())
        ) {
          void this.loadChatbotOllamaModels().catch(() => {});
        }
      });
    ...
    if (settingsSave) {
      settingsSave.addEventListener("click", () => {
        void this.saveChatbotSettings(
          personalitySelect,
          removeFiltersCheckbox,
          settingsPanel,
        );
      });
    }
  ...
  async saveChatbotSettings(
    ...
    this.personality =
      (personalitySelect === null || personalitySelect === void 0
        ? void 0
        : personalitySelect.value) || "helpful";
    this.removeFilters =
    ...
    this.showPromptToast(
      this.ollamaModel
        ? `Settings saved — model: ${this.ollamaModel}`
        : "Settings saved",
    );
    if (settingsPanel) settingsPanel.style.display = "none";
    void this.refreshProviders({ force: true }).catch((err) => {
  ...
  async loadCustomPrompt() {
    ...
    const userId =
      ((_c =
        (_b = (_a = this.app) === null || _a === void 0 ? void 0 : _a.state) ===
          null || _b === void 0
          ? void 0
          : _b.user) === null || _c === void 0
        ? void 0
    ...
    try {
      const res = await fetch(
        apiBase() + "/api/prompts/get?userId=" + encodeURIComponent(userId),
  ...
  async sendMessage() {
    ...
    if (!this.selectedProvider) {
      this.showErrorBanner(
        ...
        true,
        {
          ...
          showRetry: hasExtensionBridgeConfigured(),
          showSignInLink:
            !hasExtensionBridgeConfigured() && !authService.isAuthenticated(),
        },
      );
      return;
    }
    ...
    try {
      if (this.useBrowserOllama && this.selectedProvider === "ollama") {
        ...
        const targetBubble = messageElements[
          assistantMessageIndex
        ]?.querySelector(".chatbot-message-text");
        if (!targetBubble) {
          throw new Error("Could not render chat response area.");
        }
        await this.sendBrowserOllamaMessage(message, targetBubble);
        this.conversationHistory[assistantMessageIndex].content =
      ...
      if (!res.ok) {
        if (res.status === 404) {
          throw new Error(
            "Chatbot API not found. Ensure the ai-platform server is running on port 54355.",
          );
        }
      ...
      if (targetBubble) {
        ...
        if (data.success) {
          ...
          this.showErrorBanner(
            data.message ||
              "The AI provider returned an error. Check provider configuration.",
          );
  ...
  async sendBrowserOllamaMessage(message, targetBubble) {
    ...
    const customPrompt = document
      .getElementById("chatbot-custom-prompt")
      ?.value?.trim();
        ...
        try {
          ...
          if (token) {
            ...
            targetBubble.innerHTML =
              this.formatStreamedMessage(accumulatedText);
            const container = document.getElementById("chatbot-messages");
            if (container) {
              container.scrollTo({
  ...
  async consumeTokenStream(response, targetBubble) {
    ...
    try {
      ...
      window.setSafeHTML(
        targetBubble,
        '<p class="error">[Stream Interrupted]</p>',
      );
    } finally {
      reader.releaseLock();
    }
  ...
  formatStreamedMessage(text) {
    ...
    inlineBlocks.forEach((code, index) => {
      ...
      processedText = processedText.replace(
        `__INLINE_PLACEHOLDER_${index}__`,
        safeInline,
      );
    });
  ...
  async copyMessage(index) {
    ...
    const btn = document.querySelector(
      `.chatbot-copy-btn[data-index="${index}"]`,
    );
    try {
