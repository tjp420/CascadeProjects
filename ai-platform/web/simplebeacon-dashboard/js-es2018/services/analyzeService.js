...
export async function fetchComplianceTrailExportJson(windowDays = 90) {
  ...
  const res = await fetch(
    resolveApiUrl(`/api/compliance-trail/export/json?${params}`),
    {
      cache: "no-store",
      headers: authService.getAuthHeaders(),
    },
...
export async function fetchComplianceTrailExportHtml(windowDays = 90) {
  ...
  const res = await fetch(
    resolveApiUrl(`/api/compliance-trail/export/pdf?${params}`),
    {
      cache: "no-store",
      headers: authService.getAuthHeaders(),
    },
...
export function mergeReportInventory(report, inventory) {
  ...
  const normalizedTotalFiles =
    report.totalFiles === 0 || report.totalFiles === 1
      ? inventory.totalFiles
      : report.totalFiles;
  const normalizedFilesAnalyzed =
    report.filesAnalyzed === 0 || report.filesAnalyzed === 1
      ? repoFiles
...
export function convertSandboxReportToSimplebeacon(report, projectPath) {
  ...
  const sampleFiles = (report.files || [])
    .slice(0, 96)
    .map((f) =>
      typeof f === "string" ? f : f.path || f.name || f.relativePath || "",
    )
    .filter(Boolean);
  return {
