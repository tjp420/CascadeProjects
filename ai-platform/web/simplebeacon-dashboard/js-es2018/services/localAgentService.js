...
export function clearStaleIntegratedBridgeParams() {
  ...
  if (override) {
    try {
      if (
        isAgentJs4000Origin(
          new URL(String(override).replace(/\/api\/?$/, "")).origin,
        )
      ) {
...
function getAgentFetch() {
  if (
    hasAgentBridge() &&
    typeof window.simplebeaconAgentBridge.fetch === "function"
  ) {
    return window.simplebeaconAgentBridge.fetch.bind(
...
export function bridgeFetchViaParent(url, init = {}, timeoutMs = 4000) {
  return new Promise((resolve, reject) => {
    ...
    const onMsg = (event) => {
      ...
      if (
        !data ||
        data.command !== "bridgeFetchResponse" ||
...
function readSbApiBaseOverride() {
  ...
  try {
    ...
    const fromQuery =
      params.get(SB_API_BASE_KEY) || params.get("sb_notify_base");
...
export function buildExtensionConnectDeepLink(route = "chatbot") {
  ...
  const scheme = /Cursor/i.test(navigator.userAgent || "")
    ? "cursor"
    : "vscode";
  const segment =
 ...
 */
export async function probeExtensionDataServer(
  ports = EXTENSION_PROBE_PORTS,
  _options = {},
) {
  if (typeof window === "undefined") return null;
  ...
  for (const port of ports) {
    ...
    try {
      ...
      const pingRes = await doFetch(`${origin}/api/ping`, {
        signal: AbortSignal.timeout(2200),
      });
...
export async function discoverAndApplyExtensionBridge(options = {}) {
  ...
  if (hasExtensionBridgeConfigured() && normalizedStored) {
    ...
    try {
      const port = Number(
        new URL(String(stored).replace(/\/api\/?$/, "")).port,
      );
      if (port) ports = [port];
    } catch (_a) {
      /* use defaults */
  ...
  const applied = persistExtensionBridge(apiBase, {
    websiteMode: true,
    updateUrl: true,
  });
  return applied
    ? { ok: true, source: "probe", apiBase }
    : { ok: false, source: "denied" };
}
 ...
 * @returns {string}
 */
export function resolveOllamaProxyUrl(
  ollamaPath,
...
export async function probeExtensionBridgeHealth() {
  ...
  const doFetch = getLocalBridgeFetch();
  try {
    const res = await doFetch(`${bridge}/api/ping`, {
...
export async function probeUserInitiatedOllama(
  baseUrl = DEFAULT_OLLAMA_ORIGIN,
) {
  ...
  try {
    const res = await doFetch(probeUrls[0], {
      signal: AbortSignal.timeout(4000),
    });
    const parsed = await parseOllamaProbeResponse(res);
    if (parsed.ok) {
...
export async function findFolderViaBridge(folderName) {
  ...
  try {
    ...
    if (status.workspace) {
      const wsName =
        String(status.workspace).replace(/\\/g, "/").split("/").pop() || "";
...
async function agentFetchWithTimeout(url, options = {}, timeoutMs = 300000) {
  ...
  const timeout = new Promise((_resolve, reject) => {
    setTimeout(
      () => reject(new Error("Local agent request timed out")),
      timeoutMs,
    );
  });
...
export async function probeAgent(origin = DEFAULT_AGENT_ORIGIN) {
  if (!shouldProbeLocalAgent()) {
    return isIntegratedLocalDashboard()
      ? INTEGRATED_AGENT_SKIP
      : HOSTED_AGENT_OFFLINE;
  }
  const now = Date.now();
  ...
  pendingProbe = (async () => {
    ...
    try {
      ...
      const likelyBlocked =
        !hasAgentBridge() && isMixedContentBlocked(origin, err);
      cachedAgentStatus = {
        available: false,
        scannerAvailable: false,
        likelyBlocked,
 ...
 * @returns {Promise<Object>}
 */
export async function scanViaAgent(
  projectPath,
  options = {},
  origin = DEFAULT_AGENT_ORIGIN,
) {
 ...
 * @returns {Promise<Object>}
 */
export async function fetchScanProgressViaAgent(
  projectPath,
  origin = DEFAULT_AGENT_ORIGIN,
) {
 ...
 * @param {string} [origin]
 */
export async function scanViaAgent4000(
  projectPath,
  origin = resolveBridgeOrigin(),
) {
  if (isExtensionBridgeOrigin(origin)) {
    ...
    if (!response.ok || !data.success) {
      throw new Error(
        data.error ||
          data.warning ||
          `Extension scan failed (${response.status})`,
      );
    }
    if (data.scanning) {
...
export function renderAgentCertificate(report, container) {
  ...
  const discovered =
    typeof report.discoveredFiles === "number"
      ? report.discoveredFiles
      : files.length;
  const skipped =
    typeof report.skippedFiles === "number" ? report.skippedFiles : 0;
  ...
  const metricDefs = [
    ...
    ...(skipped > 0
      ? [{ label: "Skipped", value: `${skipped} unreadable` }]
      : []),
  ];
    ...
    btn.addEventListener("click", () => {
      ...
      if (item.route === "export") {
        if (
          app &&
          app.scanService &&
          typeof app.scanService.exportReport === "function"
        ) {
      ...
      rows.slice(0, limit).forEach((f) => {
        ...
        if (f.absolutePath || f.name) {
          ...
          path.addEventListener("click", () => {
            openInIde(f.absolutePath || f.name, f.line || 1, {
              projectRoot: scannedPath,
            });
          });
        }
...
export function shouldUseAgent(projectPath, agentStatus) {
  if (
    !(agentStatus === null || agentStatus === void 0
      ? void 0
      : agentStatus.available) ||
    !(agentStatus === null || agentStatus === void 0
      ? void 0
      : agentStatus.scannerAvailable)
  )
    return false;
...
export function formatAgentStatus(agentStatus) {
  ...
  if (
    isHostedHttpsDashboard() &&
    (!agentStatus.available || agentStatus.hostedSkipped)
  )
    return "";
...
export function getAgentFallbackMessage(agentStatus) {
  ...
  if (
    (_a = agentStatus) === null || _a === void 0 ? void 0 : _a.likelyBlocked
  ) {
  ...
  if (
    !((_c = agentStatus) === null || _c === void 0
      ? void 0
      : _c.scannerAvailable)
  ) {
    var _d;
    const error = (
      (_d = agentStatus) === null || _d === void 0
        ? void 0
        : _d.scannerLoadError
    )
      ? ` (${agentStatus.scannerLoadError})`
