...
export async function fetchCliReport(reportId, apiKey) {
  const res = await fetch(
    `${apiPrefix()}/api/simplebeacon/report/${reportId}`,
    {
      headers: { Authorization: `Bearer ${apiKey}` },
    },
  );
...
export function renderCliUploadCard(container, apiKey) {
  ...
  desc.style.cssText =
    "font-size:13px; color:var(--text-secondary); margin:10px 0;";
...
export function renderCliReport(report, container) {
  ...
  const scannedPath =
    report.projectPath || report.projectRoot || report.scannedPath || "unknown";
  let grade = "A";
  if (score < 60) grade = "F";
