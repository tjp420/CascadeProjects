...
export function isIgnoredPath(rel, ignorePatterns) {
  if (!Array.isArray(ignorePatterns) || !ignorePatterns.length || typeof rel !== 'string')
    return false;
...
export function shouldSkipSandboxScanFile(virtualPath, isSimplebeaconMonorepo) {
  ...
  if (isSimplebeaconMonorepo) {
    if (
      ...
      return true;
    if (/(?:^|\/)(?:simplebeacon-rule-tests|guardrail-test-bench)(?:\/|$)/i.test(normalized))
      return true;
    ...
    if (
      ...
      return true;
    if (/(?:^|\/)packages\/simplebeacon-cli\/src\/(?:compliance-rules|proxy)\//i.test(normalized))
      return true;
    ...
    if (/(?:^|\/)server\/lib\/code-understanding\/semantic-analyzer\.cjs$/i.test(normalized))
      return true;
    ...
    if (/(?:^|\/)server\/lib\/language-patterns\/(?:go|rust|sql)-patterns\.cjs$/i.test(normalized))
      return true;
    ...
    if (/(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/config\.js$/i.test(normalized))
      return true;
    if (/(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/demoMode\.js$/i.test(normalized))
      return true;
    if (
      ...
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/services\/scanService\.js$/i.test(normalized)
    )
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/utils\/funnelTrigger\.js$/i.test(normalized)
    )
      return true;
    if (
    ...
    if (
      ...
      return true;
    if (
      /(?:^|\/)web\/simplebeacon-dashboard\/js-es2018\/utils-lib\/test-out\.txt$/i.test(normalized)
    )
      return true;
