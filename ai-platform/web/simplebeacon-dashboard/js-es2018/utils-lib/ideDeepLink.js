...
export function resolveAbsoluteFilePath(filePath, projectRoot) {
  ...
  try {
    ...
    if (
      rootName &&
      (raw === rootName || raw.indexOf(rootName + '/') === 0 || raw.indexOf(rootName + '\\') === 0)
    ) {
...
export function buildIdeFileUrl(filePath, line = 1, options = {}) {
  ...
  const scheme =
    options.scheme ||
    (typeof navigator !== 'undefined' && /Cursor/i.test(navigator.userAgent || '')
      ? 'cursor'
      : 'vscode');
...
export function renderIdeFileLink(filePath, line = 1, options = {}) {
  ...
  const canOpen =
    Boolean(resolveAbsoluteFilePath(filePath, options.projectRoot)) || isAbsolutePath(filePath);
  if (canOpen) {
