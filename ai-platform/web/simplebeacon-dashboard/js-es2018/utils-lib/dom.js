...
function writeNotificationsEnabled(enabled) {
  try {
    ...
    console.error("Failed to set notifications enabled state:", _a);
  }
}
...
export function showOSNotification(title, options) {
  ...
  try {
    ...
    console.error("Failed to show OS notification:", _b);
  }
  return null;
}
/**
...
function isIdeEmbedDownloadBridge() {
  ...
  try {
    ...
    console.error("Failed to check IDE embed bridge params:", _a);
  }
  try {
...
function wouldBeMixedOrLocalBridge(baseUrl) {
  try {
    ...
    console.error("Failed to check mixed or local bridge URL:", _b);
  }
  return false;
}
function getExtensionBridgeNotifyUrl() {
  ...
  try {
    ...
    console.error("Failed to resolve extension bridge notify URL:", _a);
  }
...
function notifyExtensionDownload(blob, filename) {
  ...
  reader.onload = () => {
    ...
    try {
      ...
      console.error("Failed to post download message to parent:", _a);
    }
    try {
      ...
      console.error("Failed to fetch extension bridge notify URL:", _b);
    }
  };
    ...
    try {
      ...
      reader.onerror = () => {
        ...
        try {
          ...
          console.error("Failed to download file with both methods:", _a);
        }
      };
      reader.readAsDataURL(blob);
      return;
...
if (typeof window !== "undefined") {
  if (!window.__SB_createEmptyStateElement) {
    window.__SB_createEmptyStateElement = function (opts) {
      ...
      if (body)
        container.appendChild(
          createElement("p", { className: "empty-state-body" }, [String(body)]),
        );
      if (actions.length) {
      ...
      attach(container) {
        const builder =
          typeof window !== "undefined" && window.__SB_createEmptyStateElement
            ...
            : null;
        const el = builder
          ? builder({ icon, title, body, actions: rawActions, iconWrapper })
          : null;
        // replace container contents rather than relying on innerHTML elsewhere
        if (el && container && typeof container.appendChild === "function")
...
export function isIdeDashboardSurface() {
  ...
  try {
    ...
    console.error("Failed to check IDE embed attribute:", _a);
  }
  ...
  try {
    ...
    console.error("Failed to check acquireVsCodeApi:", _b);
  }
  return window.self !== window.top;
}
...
export function isExtensionHostedTab() {
  ...
  try {
    ...
    console.error("Failed to check extension hosted tab params:", _b);
  }
  try {
    ...
    console.error("Failed to check sessionStorage for extension params:", _c);
  }
  return false;
}
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.error("Failed to load DOMPurify module:", _b);
      }
    }
...
if (typeof window !== "undefined") {
  try {
    ...
    console.error("Failed to set window.setSafeHTML:", e);
  }
}
    ...
    _vsCodeProxyCache = {
      postMessage(msg) {
        try {
          ...
          console.error(
            "Failed to post message to parent for VS Code proxy:",
            _b,
          );
        }
      },
