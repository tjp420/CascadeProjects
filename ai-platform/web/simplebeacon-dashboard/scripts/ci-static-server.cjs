...
const server = http.createServer((req, res) => {
  try {
    ...
    fs.stat(filePath, (err, stats) => {
      ...
      for (const altRoot of alternativeRoots) {
        try {
          ...
          if (fs.existsSync(altPath) && fs.statSync(altPath).isFile()) {
            ...
            console.log(
              "[HTTP] %s %s -> 200 (file via altRoot %s)",
              req.method,
              urlPath,
              altRoot,
            );
      ...
      if (ext) {
        console.log(
          "[HTTP] %s %s -> 404 (missing file with extension)",
          req.method,
          urlPath,
        );
      ...
      fs.readFile(indexPath, (rerr, data) => {
        if (rerr) {
          console.error(
            "[HTTP] %s %s -> 500 (index read error)",
            req.method,
            urlPath,
            rerr && rerr.stack ? rerr.stack : rerr,
          );
          res.writeHead(500, { "Content-Type": "text/plain" });
          return res.end("index.html not found");
        }
        console.log(
          "[HTTP] %s %s -> 200 (index fallback)",
          req.method,
          urlPath,
        );
...
server.listen(port, "127.0.0.1", () =>
  console.log("CI static server listening on", port),
);
