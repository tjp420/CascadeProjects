...
export function PostScanShareBanner({
  ...
  useEffect(() => {
    ...
    setLoadingLink(true);
    fetch(
      `${apiUrl("referral/link")}?email=${encodeURIComponent(userEmail)}&channel=results-share`,
    )
      .then((res) => res.json())
      .then((data) => {
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-foreground-muted text-sm mt-1 leading-relaxed">
              {gatePass ? (
                <>
                  ...
                  . Share the win — your link includes referral tracking when
                  signed in.
                </>
              ) : (
                <>
                  Your scan flagged compliance issues. Share SimpleBeacon so
                  your team can run their own local scans.
                </>
              )}
            </p>
            ...
            <div className="flex items-center gap-2 mt-3 bg-background/60 p-1.5 border border-border rounded-md">
              <input
                ...
                readOnly
                value={
                  loadingLink ? "Generating your tracking link…" : trackingUrl
                }
