...
export function PolicySyncer() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const orgsResp = await fetch(apiUrl("audit/pii/orgs"), {
        headers: authHeaders(),
        credentials: "include",
      });
      if (!orgsResp.ok)
        throw new Error(`Orgs request failed: ${orgsResp.status}`);
      const orgsJson = await orgsResp.json();
      const org = (orgsJson.orgs || [])[0];
      const orgId = org?.orgId || "org-source";

      const polResp = await fetch(
        apiUrl(`audit/pii/policies/${encodeURIComponent(orgId)}`),
        { headers: authHeaders(), credentials: "include" },
      );
      if (!polResp.ok)
        throw new Error(`Policy request failed: ${polResp.status}`);
      ...
      const rules: PolicyRule[] = policies.map((p) => ({
        ...
        remediation:
          p.description ||
          `Enforce ${p.name} for ${p.compliance?.join(", ") || "compliance"}`,
      }));
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl mt-6 animate-in fade-in duration-200">
      <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Tenant-scoped operational compliance guidelines, rule sets, and
            access barriers.
          </p>
        </div>
        <button
      ...
      {policy ? (
        ...
        !loading && (
          <div className="text-slate-500 font-mono text-center py-6">
            No policy blueprints currently mapped.
          </div>
        )
      )}
    </div>
