...
function syncReportToVscodeSidebar(reportData: any, fallbackProjectPath = ''): void {
  ...
  const payload = {
    ...
    ruleScopedFilesAnalyzed:
      ...
      0,
    issueCount:
      reportData?.issueCount ?? issues.reduce((sum, i) => sum + (Number(i.count) || 1), 0),
    qualityScore,
...
export function ResultsView() {
  ...
  const [selectedCell, setSelectedCell] = useState<{ impact: string; likelihood: string } | null>(
    null
  );
    ...
    <div className="mx-auto max-w-7xl p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            3x3 matrix of issue impact vs. likelihood. Click or press Enter on a cell to filter
            findings.
            {selectedCell &&
            ...
            {(['high', 'medium', 'low'] as const).map((imp) => (
              <div key={imp} className="contents">
                ...
                {(['low', 'medium', 'high'] as const).map((lk) => {
                  ...
                  const isSelected =
                    selectedCell?.impact === imp && selectedCell?.likelihood === lk;
                  return (
                    <div
                      key={`${imp}-${lk}`}
                      tabIndex={0}
                  ...
                  <div className="flex flex-col items-center gap-3 py-8 text-center">
                    ...
                    <div>
                      <p className="text-sm font-medium">
                        {(result?.issueCount ?? 0).toLocaleString()} issues found — detailed list
                        unavailable
                      </p>
                      <p className="text-xs text-foreground-muted mt-1">
                        The findings list was too large for browser storage. Export the JSON report
                        from the Analyze page or run{' '}
                  ...
                  <>
                    {filteredIssues.length > 500 && (
                      <p className="text-xs text-foreground-muted py-2">
                        Showing first 500 of {filteredIssues.length} filtered issues. Export for the
                        full set.
                      </p>
                    )}
