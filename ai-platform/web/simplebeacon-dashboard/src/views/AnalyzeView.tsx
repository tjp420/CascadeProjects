...
export function AnalyzeView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const resp = await fetch(apiUrl('/simplebeacon/report/browser-errors'), {
          headers: authHeaders(),
        });
        if (!cancelled) {
          if (resp.ok) {
    ...
    const buildCompactReport = (report: any) => {
      ...
      return {
        ...
        summary: {
          ...
          codeFilesAnalyzed:
            fullSummary.codeFilesAnalyzed ?? scanResult.scanScope?.codeFilesAnalyzed,
        ...
        ruleScopedFilesAnalyzed:
          report?.ruleScopedFilesAnalyzed ?? scanResult.scanScope?.codeFilesAnalyzed,
        ...
        scanScope: {
          ...
          resultsViewScope:
            ...
            'browser-local',
          codeFilesAnalyzed:
            report?.scanScope?.codeFilesAnalyzed ?? scanResult.scanScope?.codeFilesAnalyzed,
        },
  ...
  const debouncedAppendLog = useCallback((line: string) => {
    ...
    (window as any).__sb_debounce_append = (window as any).__sb_debounce_append || {
      timer: 0,
      queue: [],
    };
    ...
    try {
      ...
      if (!isUrl && !isGithubUrl(scanPath) && hosted && !isServerDefaultPath) {
        ...
        if (!dirHandlePick && folderInputRef.current) {
          ...
          toast.info(
            'Please select the folder to scan using the file picker (select any file in the folder).'
          );
          try {
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (capturedEntries.length > 0) {
        try {
          const { files, rootName, traverseErrors } = await collectFilesFromDrop(
            e.dataTransfer,
            capturedEntries
          );
          if (files.length > 0) {
      ...
      if (dtFiles.length > 0) {
        ...
        setScanState('idle');
        toast.warning(
          `Dropped "${dirName}" but could not read folder contents. Click Select Folder below, or use Browse Folder.`
        );
        return;
      }
    ...
    <div className="mx-auto max-w-7xl p-6 space-y-6">
                        ...
                        return (
                          <div key={i} className="mb-2">
                            ...
                            <div className="text-sm">
                              {be.errorName || be.error || be.message || JSON.stringify(be)}
                            </div>
                            <div className="text-foreground-muted text-[11px]">
      ...
      {scanState === 'complete' && fileErrorsCount ? (
        <div className="rounded-md border p-3 bg-muted/5 text-sm">
          <div className="flex items-center justify-between">
            <div>
              <strong>File access issues:</strong> {fileErrorsCount} file(s) could not be read
              during the scan.
          ...
          <Card>
            <CardContent className="p-4">
              <p className="text-sm text-foreground-muted">
                Deterministic gate scan (AI narrative hidden for compliance integrity). Source files
                are not semantically reviewed. Gate passes on configured severities. Scoped to
                configured scanPaths and production directories — pattern matching only.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
