...
export function UsageAnalyticsView() {
  ...
  const [ticketPayload, setTicketPayload] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const fetchFilters = useCallback(async () => {
    try {
      ...
      const resp = await fetch(
        apiUrl(`/enterprise/analytics/filters?${params}`),
        { headers: authHeaders() },
      );
      if (resp.ok) {
  ...
  const fetchTicketStatuses = useCallback(async () => {
    try {
      const resp = await fetch(
        apiUrl("/analytics/violations/ticket-statuses"),
        { headers: authHeaders() },
      );
      if (resp.ok) {
        const data = await resp.json();
  ...
  const fetchViolationSummary = useCallback(async () => {
    try {
      ...
      if (branchFilter) params.set("branch", branchFilter);
      const resp = await fetch(
        apiUrl(`/analytics/violations/summary?${params}`),
        { headers: authHeaders() },
      );
      if (resp.ok) {
        const data = await resp.json();
  ...
  const markTicketed = useCallback(
    async (
      ...
      try {
        const resp = await fetch(
          apiUrl("/analytics/violations/mark-ticketed"),
          {
            method: "POST",
  ...
  const unmarkTicketed = useCallback(
    async (scanId: string, category: string) => {
      try {
        const resp = await fetch(
          apiUrl("/analytics/violations/unmark-ticketed"),
          {
            method: "POST",
            headers: { ...authHeaders(), "Content-Type": "application/json" },
  ...
  const bulkMarkTicketed = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(
        apiUrl("/analytics/violations/bulk-mark-ticketed"),
        {
          method: "POST",
          headers: { ...authHeaders(), "Content-Type": "application/json" },
          body: JSON.stringify({
            violations,
            ticketRef: bulkTicketRef.trim(),
            ticketTarget,
          }),
        },
      );
      ...
      setBulkTicketRef("");
      toast.success(
        `Marked ${data.succeeded} of ${data.total} violations as ticketed`,
      );
    } catch {
  ...
  const bulkUnmarkTicketed = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(
        apiUrl("/analytics/violations/bulk-unmark-ticketed"),
        {
          method: "POST",
          headers: { ...authHeaders(), "Content-Type": "application/json" },
      ...
      setSelectedRows(new Set());
      toast.success(
        `Removed ticket status from ${data.succeeded} of ${data.total} violations`,
      );
    } catch {
  ...
  const deleteWebhookConfig = useCallback(
    async (target: string) => {
      try {
        const resp = await fetch(
          apiUrl(`/analytics/webhook/configs/${target}`),
          {
            method: "DELETE",
            headers: authHeaders(),
          },
  ...
  const dispatchTicket = useCallback(
    async (
      scanId: string,
      category: string,
      target: "jira" | "linear" | "github",
    ) => {
      const rowKey = `${scanId}-${category}`;
      setDispatchingRow(rowKey);
      try {
        const resp = await fetch(
          apiUrl("/analytics/violations/dispatch-ticket"),
          {
            method: "POST",
            headers: { ...authHeaders(), "Content-Type": "application/json" },
        ...
        if (!resp.ok) {
          if (data.error === "webhook_not_configured") {
            ...
            toast.error(
              `No webhook configured for ${target}. Please configure it first.`,
            );
          } else {
        ...
        toast.error(
          err instanceof Error ? err.message : "Failed to dispatch ticket",
        );
      } finally {
  ...
  const runReportSchedule = useCallback(
    async (id: string) => {
      ...
      try {
        const resp = await fetch(
          apiUrl(`/analytics/report/schedules/${id}/run`),
          {
            method: "POST",
            headers: authHeaders(),
          },
        ...
        toast.error(
          err instanceof Error ? err.message : "Failed to run report schedule",
        );
      } finally {
      ...
      if (resp.ok) {
        ...
        setGatePolicyForm({
          ...
          blockOnUnticketedCritical:
            data.policy?.blockOnUnticketedCritical ?? false,
        });
      }
    } catch {
      /* silent */
  ...
  const fetchAuditLog = useCallback(async () => {
    try {
      ...
      const resp = await fetch(apiUrl(`/audit/log?${params}`), {
        headers: authHeaders(),
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const fetchAuditStats = useCallback(async () => {
    try {
      const resp = await fetch(apiUrl("/audit/stats"), {
        headers: authHeaders(),
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const fetchEvalSuites = useCallback(async () => {
    try {
      const resp = await fetch(apiUrl("/model-eval/suites"), {
        headers: authHeaders(),
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const fetchEvalRuns = useCallback(async () => {
    try {
      const resp = await fetch(apiUrl("/model-eval/runs?limit=20"), {
        headers: authHeaders(),
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const fetchEvalStats = useCallback(async () => {
    try {
      const resp = await fetch(apiUrl("/model-eval/stats"), {
        headers: authHeaders(),
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const runEvaluation = useCallback(async () => {
    ...
    try {
      ...
      if (resp.ok && data.run) {
        ...
        fetchEvalStats();
        toast.success(
          `Evaluation complete: ${data.run.passed}/${data.run.totalTests} passed`,
        );
      } else {
  ...
  const fetchRunDetail = useCallback(async (runId: string) => {
    try {
      const resp = await fetch(apiUrl(`/model-eval/runs/${runId}`), {
        headers: authHeaders(),
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const exportLedger = useCallback(
    async (format: "csv" | "json") => {
      try {
        ...
        if (slaBreachedFilter) params.set("slaBreached", "true");
        const resp = await fetch(
          apiUrl(`/analytics/violations/export?${params}`),
          { headers: authHeaders() },
        );
  ...
  const generateTicket = useCallback(
    async (
      ...
      try {
        const resp = await fetch(
          apiUrl("/analytics/violations/ticket-payload"),
          {
            method: "POST",
            headers: { ...authHeaders(), "Content-Type": "application/json" },
            body: JSON.stringify({ scanId, category, target }),
  ...
  const handleExport = useCallback(
    async (format: "csv" | "json") => {
      try {
        ...
        if (repoFilter) params.set("repository", repoFilter);
        if (branchFilter) params.set("branch", branchFilter);
        const resp = await fetch(apiUrl(`/analytics/export?${params}`), {
          headers: authHeaders(),
        });
        if (!resp.ok) throw new Error("export_failed");
        const blob = await resp.blob();
  ...
  const severityData = stats
    ? [
        {
          name: "Critical",
          value: stats.severityTotals.critical,
          fill: SEVERITY_COLORS.critical,
        },
        {
          name: "High",
          value: stats.severityTotals.high,
          fill: SEVERITY_COLORS.high,
        },
        {
          name: "Medium",
          value: stats.severityTotals.medium,
          fill: SEVERITY_COLORS.medium,
        },
        {
          name: "Low",
          value: stats.severityTotals.low,
  ...
  const heatmapData = heatmap.slice(0, 10).map((h) => ({
    category:
      h.category.length > 20 ? h.category.slice(0, 18) + "..." : h.category,
    fullName: h.category,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Scan volumes, violation trends, and compliance posture across all
            tenants
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {stats?.totalOrgs ?? 0} organizations
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {stats?.severityTotals.critical ?? 0} critical,{" "}
              {stats?.severityTotals.high ?? 0} high
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Files analyzed and findings per {granularity}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {trend.length > 0 ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Average posture score per {granularity}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {trend.length > 0 ? (
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Files analyzed by programming language
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Most frequently scanned repositories
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {violationSummary.categories.length > 0 && (
                <div className="space-y-1.5 pt-2 border-t">
                  ...
                  {violationSummary.categories.length > 8 && (
                    <span className="text-xs text-muted-foreground italic">
                      +{violationSummary.categories.length - 8} more
                      categories...
                    </span>
                  )}
                </div>
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-[auto_auto_1fr_auto_auto_auto] gap-3 text-xs font-medium text-muted-foreground pb-2 border-b items-center">
                <input
                  ...
                  className="h-4 w-4 rounded"
                  checked={
                    selectedRows.size === violations.length &&
                    violations.length > 0
                  }
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isSlaBreached && (
                            ...
                            >
                              <Clock className="h-3 w-3" /> SLA +{v.slaDaysOver}
                              d
                            </span>
                          )}
                        </div>
                    ...
                    {isExpanded && (
                      <div className="ml-9 mr-2 mb-2 p-3 rounded-md bg-muted/30 border text-sm space-y-2">
                        ...
                        <div className="flex gap-3 text-xs text-muted-foreground pt-1 border-t">
                          ...
                          <span>
                            Commit:{" "}
                            <code className="font-mono">{v.commitSha}</code>
                          </span>
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t flex-wrap">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              generateTicket(v.scanId, v.category, ticketTarget)
                            }
                          >
                            {ticketLoading === rowKey
                              ? "Generating..."
                              : "Generate Ticket"}
                          </Button>
                          ...
                          {isTicketed ? (
                            <>
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  unmarkTicketed(v.scanId, v.category)
                                }
                              >
                            ...
                            <>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setTicketRefInput(e.target.value)
                                }
                                className="flex h-8 w-64 rounded-md border border-input bg-transparent px-2 py-1 text-xs"
                              />
                              <Button
              ...
              {showWebhookConfig && (
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground">
                      Configure the API endpoint and credentials for direct
                      ticket dispatch. Auth tokens are stored locally and masked
                      in API responses.
                    </p>
                      ...
                      <div className="flex gap-2">
                        {webhookConfigs[webhookForm.target] && (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              deleteWebhookConfig(webhookForm.target)
                            }
                          >
              ...
              {violationsTotal > violationsPageSize && (
                <div className="flex items-center justify-between pt-3 border-t">
                  <span className="text-xs text-muted-foreground">
                    ...
                    {Math.min(
                      (violationsPage + 1) * violationsPageSize,
                      violationsTotal,
                    )}{" "}
                    of {violationsTotal}
                  </span>
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() =>
                        setViolationsPage((p) => Math.max(0, p - 1))
                      }
                    >
                      Previous
                    </Button>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Schedule recurring compliance ledger reports delivered to SecOps
                mailboxes
              </CardDescription>
            </div>
            <Button
        ...
        <CardContent>
          {Object.keys(reportSchedules).length === 0 ? (
            <div className="flex h-[100px] items-center justify-center text-muted-foreground text-sm">
              No report schedules configured. Click "New Schedule" to create
              one.
            </div>
          ) : (
            <div className="space-y-2">
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      at {String(s.hour).padStart(2, "0")}:
                      {String(s.minute).padStart(2, "0")} UTC
                    </div>
                    <div className="text-xs text-muted-foreground">
                    ...
                    {s.lastRunAt && (
                      <div className="text-xs text-muted-foreground">
                        Last run: {s.lastRunAt.slice(0, 19).replace("T", " ")} —{" "}
                        {s.lastRunStatus}
      ...
      {showScheduleModal && (
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              Configure a recurring compliance report that generates the
              22-column ledger and emails it to the specified recipients.
              Without SMTP configuration, reports are saved as stub files on
              disk.
            </p>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Evaluate scan posture against organizational policy thresholds.
            Returns pass/fail for CI/CD pipeline integration.
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => setShowGatePolicyModal(true)}
              >
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-muted-foreground ml-auto">
                    Posture: {gateEvalResult.scan?.postureScore ?? "N/A"} ·
                    Gate: {gateEvalResult.scan?.gateStatus ?? "N/A"}
                  </span>
                </div>
                ...
                {gateEvalResult.scan && (
                  <div className="text-xs text-muted-foreground mt-2 pt-2 border-t">
                    Scan: {gateEvalResult.scan.scanId} ·{" "}
                    {gateEvalResult.scan.repository}@
          ...
          <div className="p-3 rounded-lg border bg-muted/20 space-y-2">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs">
              ...
              <div>
                ...
                <span className="font-medium">
                  {gatePolicy.blockOnGateFail ? "Yes" : "No"}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Block on SLA:</span>{" "}
                <span className="font-medium">
                  {gatePolicy.blockOnSlaBreached ? "Yes" : "No"}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">
                  Block Unticketed Critical:
                </span>{" "}
                <span className="font-medium">
    ...
    echo "Deployment gate PASSED"`}
      ...
      {showGatePolicyModal && (
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              Configure organizational thresholds for CI/CD deployment gate
              evaluations. These thresholds are applied to all gate evaluations
              unless overridden by query parameters.
            </p>
            <div className="space-y-3">
                ...
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setGatePolicyForm((prev) => ({
                        ...prev,
                        blockOnGateFail: e.target.checked,
                      }))
                    }
                    className="h-4 w-4 rounded"
                  />
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Immutable record of all administrative changes across ticket
            statuses, webhook configs, report schedules, and deployment gate
            policies.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs">
              ...
              <div className="p-2 rounded border bg-muted/20">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {Object.entries(auditStats.byAction || {}).map(
                    ([action, count]: any) => (
                      <Badge
                        key={action}
                        variant="outline"
              ...
              <div className="p-2 rounded border bg-muted/20">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {Object.entries(auditStats.byEntity || {}).map(
                    ([entity, count]: any) => (
                      <Badge
                        key={entity}
                        variant="secondary"
                      ...
                      <div className="mt-1 space-y-0.5">
                        {entry.changes.slice(0, 3).map((c: any, j: number) => (
                          ...
                          >
                            ...
                            {JSON.stringify(c.oldValue)?.slice(0, 40)} →{" "}
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Run adversarial test suites against LLM providers to benchmark bias,
            hallucination, injection resistance, and policy compliance.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Stats Summary */}
          {evalStats && (
            <div className="grid grid-cols-4 gap-2 text-xs">
              ...
              <div className="p-2 rounded border bg-muted/20">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {Object.entries(evalStats.byProvider || {}).map(
                    ([prov, count]: any) => (
                      <Badge
                        key={prov}
                        variant="outline"
          ...
          {evalStats?.byCategory &&
            Object.keys(evalStats.byCategory).length > 0 && (
              <div className="p-3 rounded-lg border bg-muted/20 space-y-2">
                <span className="text-xs font-medium text-muted-foreground">
                  Category Breakdown
                </span>
                <div className="grid grid-cols-5 gap-2 text-xs">
                  {Object.entries(evalStats.byCategory).map(
                    ([cat, data]: any) => (
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="font-medium">
                  ...
                  {evalRunDetail.totalTests > 0
                    ? Math.round(
                        (evalRunDetail.passed / evalRunDetail.totalTests) * 100,
                      )
                    : 0}
                  %)
                </span>
