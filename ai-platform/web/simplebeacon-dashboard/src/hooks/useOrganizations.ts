...
export function useOrganizations() {
  ...
  const createOrganization = useCallback(async (name: string, slug: string, plan?: string, maxSeats?: number) => {
    await waitForApiBase();
    const resp = await fetch(apiUrl('/api/orgs'), {
      method: 'POST',
  ...
  const updateOrganization = useCallback(async (orgId: string, updates: { name?: string; plan?: string; maxSeats?: number }) => {
    await waitForApiBase();
  ...
  const acceptInvitation = useCallback(async (orgId: string) => {
    await waitForApiBase();
