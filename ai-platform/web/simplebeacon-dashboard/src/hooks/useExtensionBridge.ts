...
export type ExtensionBridgeStatus =
  "idle" | "discovering" | "connected" | "unavailable" | "denied";
...
function readStoredBridgeBase(): string | null {
  ...
  try {
    return (
      sessionStorage.getItem(BRIDGE_BASE_KEY) || getExtensionBridgeOrigin()
    );
  } catch {
        ...
        if (storedBase) {
          ...
          setBridgeToken(
            typeof sessionStorage !== "undefined"
              ? sessionStorage.getItem(BRIDGE_TOKEN_KEY)
              : null,
          );
          setStatus("connected");
          return {
        ...
        const portsToProbe =
          isHostedHttpsDashboard() && canUseParentBridgeFetch()
            ? BRIDGE_PORTS.filter((port) => {
                ...
                return (
                  bridgeOrigin && `http://127.0.0.1:${port}` === bridgeOrigin
                );
              })
            : BRIDGE_PORTS;
        const probes = await Promise.all(
          portsToProbe.map((port) => probeBridgePort(port)),
        );
