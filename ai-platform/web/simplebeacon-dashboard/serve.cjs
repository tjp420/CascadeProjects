...
const server = http.createServer((req, res) => {
  ...
  res.setHeader(
    ...
    "Content-Type, Authorization, X-SimpleBeacon-Bridge-Token",
  );
  ...
  if (urlPath === "/api/simplebeacon/config") {
    ...
    res.end(
      JSON.stringify({
        success: true,
        features: { scan: true, analyze: true },
        env: "local",
      }),
    );
    return;
  ...
  if (urlPath === "/api/dashboard-home") {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(
      JSON.stringify({ success: true, data: { metrics: {}, recentScans: [] } }),
    );
    return;
  }
  ...
  if (urlPath === "/api/analyze/providers") {
    ...
    try {
      ...
      allowedRoots = Array.isArray(cfg.allowedAnalysisRoots)
        ? cfg.allowedAnalysisRoots
        : [];
      defaultProjectPath =
        Array.isArray(allowedRoots) && allowedRoots[0]
          ? allowedRoots[0]
          : repoRoot;
    } catch {
