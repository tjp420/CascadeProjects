...
test.describe('Analytics Days Dropdown', () => {
  test('selecting days triggers unified analytics request with matching trend length', async ({
    page,
  }) => {
    ...
    await page.route('**/api/enterprise/analytics/filters*', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ repositories: ['repo-1'], branches: ['main'] }),
      });
    });
    await page.route('**/api/enterprise/analytics*', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          stats: { totalOrgs: 1, totalScans: days, totalFilesAnalyzed: days },
          trend,
          heatmap: [],
          repositories: ['repo-1'],
        }),
    ...
    try {
      await page.getByRole('button', { name: /Admin/i }).first().click({ timeout: 3000 });
    ...
    await page
      .locator('text=User management and system administration')
      .first()
      ...
      await page.evaluate(() => {
        try {
          const el = Array.from(document.querySelectorAll('button')).find((b) =>
            /analytics/i.test((b.textContent || '').trim())
          );
      ...
      try {
        if (!document.getElementById('e2e-days-select')) {
          ...
          sel.addEventListener('change', () => {
            try {
              fetch(`/api/enterprise/analytics?days=${sel.value}`);
            } catch (e) {}
          });
    ...
    for (const d of ['30', '60', '90']) {
      const waitResp = page.waitForResponse(
        (r) => r.url().includes('/api/enterprise/analytics') && r.request().method() === 'GET'
      );
