...
test.describe('Dashboard Smoke Tests', () => {
  ...
  test('dashboard renders without console errors', async ({ page }) => {
    ...
    const filtered = consoleErrors.filter(e => !e.includes('favicon') && !e.includes('/api/health') && !e.includes('jszip') && !e.includes('Failed to load resource') && !e.includes('/api/config/pricing') && !e.includes('ECONNREFUSED'));
