...
import {
  analyzeFileChunks,
  findingsToIssues,
} from "../../js-es2018/workers/scan-wasm-bridge.js?v=20260716cachefix1";
...
const PATTERN_REGISTRY = {
  ...
  euAiAct: {
    ...
    pattern:
      /ai_system|high_risk|transparency|conformity|bias_audit|data_governance/gi,
  },
  hardcodedIp: {
    appliesTo: [
      "javascript",
  ...
  pythonDebug: {
    appliesTo: ["python"],
    pattern:
      /\bprint\s*\(|\bpprint\s*\(|\blogging\.debug\s*\(|\bbreakpoint\s*\(/i,
  },
  javaDebug: {
    appliesTo: ["java"],
    pattern:
      /\bSystem\.(out|err)\.(print|println)\s*\(|\be\.printStackTrace\s*\(|\bjava\.util\.logging\./i,
  },
  pythonFramework: {
    appliesTo: ["python"],
  ...
  javaFramework: {
    appliesTo: ["java"],
    pattern:
      /spring\.datasource\.(password|url)\s*=\s*['"][^'"]{4,}|log4j.*CVE|log4shell|jndi:ldap/i,
  },
  goDebug: {
    appliesTo: ["go"],
    pattern:
      /\bfmt\.Print(?:ln|f)?\s*\(|\blog\.Print(?:ln|f)?\s*\(|\blog\.Fatal(?:f|ln)?\s*\(|\bpanic\s*\(/i,
  },
  goFramework: {
    appliesTo: ["go"],
    pattern:
      /\bgin\.SetMode\s*\(\s*gin\.DebugMode|http\.ListenAndServe\s*\(\s*["'][^"']+["']\s*,\s*nil\s*\)/i,
  },
  rustDebug: {
    appliesTo: ["rust"],
    pattern:
      /\bprintln!\s*\(|\beprintln!\s*\(|\bdbg!\s*\(|\bprint!\s*\(|\bpanic!\s*\(/i,
  },
  rustFramework: {
    appliesTo: ["rust"],
    pattern:
      /\.unwrap\s*\(\s*\)(?:\s*\?\s*\.unwrap\s*\(\s*\))+|\.expect\s*\(\s*["']\s*["']\s*\)/i,
  },
  phpDebug: {
    appliesTo: ["php"],
    pattern:
  ...
  rubyFramework: {
    appliesTo: ["ruby"],
    pattern:
      /\.permit!\s*\)|\bskip_before_action\b|\beval\s*\(|\bsend\s*\(\s*params\[/i,
  },
  ...
  configDrift: {
    ...
    contextFilter: (snippet, filePath) => {
      if (
        /config\.get<|vscode\.workspace\.getConfiguration|\.get\(['"]\w+['"]\s*,\s*['"]/.test(
          snippet,
        )
      )
        return false;
      if (
        /\/\/.*hardcoded|\/\*.*hardcoded|move hardcoded|configuration drift/i.test(
          snippet,
        )
      )
        return false;
      if (
        /\/\/.*localhost|\/\*.*localhost|#.*localhost|default\s*=\s*['"]http:\/\/localhost/.test(
          snippet,
        )
      )
        return false;
  ...
  innerHtmlXss: {
    ...
    maxMatches: 3,
    selfReferenceFilter:
      /scanner-patterns|scanner-engine|ui-renderer|scan-worker|main\.js|certificate-module/i,
    contextFilter: (snippet, filePath) => {
      if (
        /scanner-patterns\.js|scanner-engine\.js|ui-renderer\.js|main\.js/i.test(
          filePath,
        )
      )
        return false;
      if (
        /\.innerHTML\s*=\s*`[^`]*`/.test(snippet) &&
        !/\$\{[^}]*\}/.test(snippet)
      )
        return false;
      return true;
  ...
  prototypePollution: {
    ...
    maxMatches: 3,
    selfReferenceFilter:
      /Object\.prototype\.hasOwnProperty\.call|Object\.prototype\.toString\.call/i,
  },
  unhandledPromise: {
    appliesTo: ["javascript", "generic"],
  ...
  insecureRandom: {
    ...
    maxMatches: 3,
    selfReferenceFilter:
      /scanner-patterns|scanner-engine|pattern-documentation|test-all-patterns/i,
  },
  loggingSecrets: {
    appliesTo: [
      "javascript",
    ...
    contextFilter: (snippet, filePath) => {
      const hasVariable =
        /\b(?:token|password|secret|apiKey|api_key|privateKey|private_key|credential)\s*[,+)]/.test(
          snippet,
        );
      const onlyInString =
  ...
  evalDanger: {
    ...
    pattern:
      /\beval\s*\(|\bnew\s+Function\s*\(|\bsetTimeout\s*\(\s*['"`]|\bsetInterval\s*\(\s*['"`]/i,
    maxMatches: 3,
    selfReferenceFilter:
  ...
  weakCryptography: {
    ...
    pattern:
      /\bmd5\s*\(|\bsha1\s*\(|\bDES\b|\bRC4\b|\bcrypto\.createHash\s*\(\s*['"`](?:md5|sha1)['"`]/i,
    maxMatches: 3,
  ...
  secretInComment: {
    ...
    contextFilter: (snippet, filePath) => {
      if (
        filePath &&
        /scanner-patterns|scanner-engine|pattern-documentation/i.test(filePath)
      )
        return false;
      return true;
    },
  ...
  llmSlop: {
    ...
    maxMatches: 5,
    selfReferenceFilter:
      /llm-slop-patterns|fiction-kpi|rejectedFiction|scanner-patterns/i,
    contextFilter: (snippet, filePath) => {
  ...
  productionLeak: {
    ...
    maxMatches: 3,
    selfReferenceFilter:
      /production-leak|llm-slop-patterns|fiction-kpi|scanner-patterns/i,
    contextFilter: (snippet, filePath) => {
  ...
  emptyStubFunction: {
    appliesTo: ["javascript", "generic"],
    pattern:
      /function\s+\w+\s*\([^)]*\)\s*\{\s*\}|(?:const|let|var)\s+\w+\s*=\s*\([^)]*\)\s*=>\s*\{\s*\}/i,
    maxMatches: 3,
  ...
  aiPlaceholderComment: {
    ...
    maxMatches: 3,
    selfReferenceFilter:
      /scanner-patterns|scanner-engine|pattern-documentation/i,
  },
  markdownFenceLeak: {
    appliesTo: [
      "javascript",
...
const IGNORE_LINE_RE =
  /simplebeacon-ignore\s+(?:credentials|credential-pattern|sensitive-data|euAiAct|eu-ai-act)/i;
...
function isTestOrFixturePath(normalized) {
  return (
    /(?:^|\/)(__tests__|tests?|fixtures?|mocks?|simplebeacon-rule-tests)(?:\/|$)/i.test(
      normalized,
...
function isComplianceToolingPath(normalized) {
  return (
    ...
    /web\/simplebeacon-dashboard\/js(?:-es2018)?\/(?:services|workers|views)\//i.test(
      normalized,
    ) ||
    /server\/routes\/(?:chatbot-api|flexible-analyze-api)\.cjs$/i.test(
      normalized,
    ) ||
...
function shouldSkipAnalyzerLine(name, filePath, line) {
  ...
  if (name === "euAiAct") {
    if (
      isComplianceToolingPath(normalized) ||
      EU_AI_ACT_COMPLIANCE_LINE_RE.test(line)
    )
      return true;
  }
  if (name === "hardcodedIp") {
...
function shouldSkipAnalyzerFile(name, filePath) {
  ...
  if (
    name === "credentials" &&
    /(?:^|\/)simplebeacon-rule-tests\//i.test(normalized)
  )
    return true;
...
function shouldSkipFile(path, deepScan, ignoreCtx) {
  ...
  if (
    ...
    return true;
  if (
    /complete-scan.*\.json$/i.test(normalized) ||
    /simplebeacon-export.*\.json$/i.test(normalized)
  )
    return true;
  if (
    !deepScan &&
    /(^|[\/])(docs\/|doc\/|third_party\/|thirdparty\/|geedocs\/|mapfiles\/|vendor\/)/i.test(
      normalized,
    )
  )
    return true;
  if (
    !deepScan &&
    /\.min\.js$|\.pack\.js$|\.bundle\.js$|\.map$/i.test(normalized)
  )
    return true;
  return false;
}
...
function runAnalyzer(name, text, filePath) {
  ...
  if (reg && reg.pattern) {
    ...
    const filtered = matches.filter((m) => {
      if (reg.selfReferenceFilter && reg.selfReferenceFilter.test(m.snippet))
        return false;
      if (reg.selfReferenceFilter && reg.selfReferenceFilter.test(filePath))
        return false;
      if (reg.contextFilter && !reg.contextFilter(m.snippet, filePath))
        return false;
      return true;
    });
    ...
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(
          () =>
            reject(
              new Error(`Timed out after ${Math.round(ms / 1000)}s: ${label}`),
            ),
          ms,
        );
      }),
  ...
  for (const file of files) {
    ...
    if (shouldSkipFile(file.path, deepScan, ignoreCtx)) {
      ...
      if (
        ...
        /(^|[\/])(docs\/|doc\/|third_party\/|thirdparty\/|vendor\/)/i.test(
          normalized,
        )
      ) {
        ignoredDir++;
    ...
    try {
      ...
      if (size > LARGE_FILE_THRESHOLD) {
        ...
        if (
          !deepScan &&
          /(?:\b|\/)(?:vendor|third_party|thirdparty|node_modules)(?:\b|\/)/i.test(
            normalized,
          )
        ) {
          heavyVendor += 1;
...
self.onmessage = async (e) => {
  ...
  if (type === "scan-start") {
    ...
    self.postMessage({
      type: "started",
      scanId,
      totalFiles: self.scanState.totalFiles,
    });
    return;
  }
  if (type === "scan-batch") {
    ...
    if (!state || state.scanId !== scanId) {
      self.postMessage({
        type: "error",
        scanId,
        error: "Scan batch received before scan-start",
      });
      return;
    }
    try {
  ...
  if (type === "scan-finish") {
    ...
    if (!state || state.scanId !== scanId) {
      self.postMessage({
        type: "error",
        scanId,
        error: "Scan finish received before scan-start",
      });
      return;
    }
    self.postMessage({
