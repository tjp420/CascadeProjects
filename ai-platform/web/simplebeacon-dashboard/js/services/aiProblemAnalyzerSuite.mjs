...
function parseClaims(input = {}) {
    ...
    return input.claims
      .map((claim, index) => ({
        ...
        evidenceConfidence: Number(
          claim.evidenceConfidence ?? (claim.hasEvidence ? 0.8 : 0.2),
        ),
        idx: index,
      }))
  ...
  return responseText
    ...
    .map((sentence, index) => ({
      ...
      evidenceConfidence:
        /\[[^\]]+\]|\b(source|citation|evidence|according to)\b/i.test(sentence)
          ? 0.8
          : 0.2,
      idx: index,
    }));
...
function runJobDisplacementImpactAnalyzer(definition, issueId, input = {}) {
  ...
  const exposureValues = tasks.map((task) =>
    clampScore(Number(task.exposureWeight ?? 25)),
  );
  ...
  const highRiskRoleCount = tasks.filter(
    (task) => Number(task.exposureWeight ?? 0) >= 30,
  ).length;
  const score = clampScore(
...
function runEnvironmentalImpactAnalyzer(definition, issueId, input = {}) {
  ...
  const energyPerRequest = Number(
    metrics.energyPerRequest ?? metrics.kwhPerRequest ?? NaN,
  );
  const scanDurationMs = Number(
    metrics.scanDurationMs ?? metrics.dataQualityScanDurationMs ?? 0,
  );
...
function collectCalibrationRecords(input = {}) {
  ...
  return source.map((row) => ({
    claimedConfidence: normalizeConfidenceValue(
      row.claimedConfidence ?? row.confidence ?? row.score,
    ),
    correct:
      row.correct === true ||
      row.isCorrect === true ||
    ...
    return input.reasoningTasks.map((row) => {
      ...
      const fallacyHits =
        REASONING_FALLACY_MARKERS.filter((pattern) => pattern.test(text))
          .length + Number(row.fallacyCount ?? 0);
      const expectedAnswer = String(row.expectedAnswer ?? row.expected ?? "")
        .trim()
        .toLowerCase();
      const actualAnswer = String(
        row.actualAnswer ?? row.answer ?? row.output ?? "",
      )
        .trim()
        .toLowerCase();
      const answerCorrect =
  ...
  register(entry = {}) {
    this._records.push({
      ...
      claimedConfidence: Number(
        entry.claimedConfidence ?? entry.confidence ?? 0,
      ),
...
function runGeneralizationAnalyzer(definition, issueId, input = {}) {
  ...
  const idScore = Number(
    benchmarks.inDistributionAccuracy ?? benchmarks.idAccuracy ?? NaN,
  );
  const oodScore = Number(
    benchmarks.outOfDistributionAccuracy ?? benchmarks.oodAccuracy ?? NaN,
  );
  const oodClaims = [
...
function deriveClaimsFromContext(context = {}) {
  ...
  return unique.slice(0, 20).map((text, index) => ({
    ...
    evidenceConfidence:
      /\b(scan|test|verified|eslint|benchmark|measured|observed)\b/i.test(text)
        ? 0.78
        : 0.35,
    source: "scan-derived",
    idx: index,
...
function deriveScalabilityMetrics(context = {}) {
  ...
  if (context.scan?.summary?.throughputRps)
    derived.throughputRps = context.scan.summary.throughputRps;
  if (context.scan?.summary?.p95LatencyMs)
...
function deriveBenchmarks(context = {}) {
  ...
  const explicit =
    context.healthScore ??
    context.scan?.healthScore ??
    context.report?.healthScore;
...
export const IMPLEMENTED_ANALYZER_ISSUE_IDS = AI_SYSTEM_ISSUES.filter(
  (issue) => ANALYZER_BY_ID.get(issue.analyzerId)?.status === "implemented",
).map((issue) => issue.id);
  ...
  return analyzerResults
    ...
    .map((result) => {
      ...
      return {
        ...
        detail:
          evidence?.detail || "Insufficient input for deterministic analysis.",
      };
    })
