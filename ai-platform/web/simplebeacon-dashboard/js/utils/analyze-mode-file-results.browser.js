...
function resolvePayload(mode, lastResult, report) {
  switch (mode) {
    ...
    case 'codebase':
      ...
      if (lastResult?.kind === 'complete')
        return { kind: 'codebase', scan: stepPayload(lastResult, 'codebase')?.scan };
      return null;
    case 'file-reduction':
    case 'data-quality':
    ...
    case 'roadmap':
      ...
      if (lastResult?.kind === 'complete')
        return { kind: 'roadmap', data: stepPayload(lastResult, 'roadmap')?.data };
      return null;
    case 'complete':
...
function buildRows(mode, payload) {
  ...
  if (payload.kind === 'codebase') {
    return {
      ...
      summary: {
        scoped: payload.scan?.summary?.filesScanned ?? payload.scan?.summary?.codeFilesAnalyzed,
      },
      note: null,
    };
  }
