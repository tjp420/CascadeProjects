...
const productionDefaultOrigins = (process.env.ALLOWED_ORIGIN || 'https://simplebeacon.ai')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
const publicUrlOrigin = process.env.PUBLIC_URL
...
const allowedOrigins =
  Array.isArray(rawAllowedOrigins) && rawAllowedOrigins.length > 0 ? rawAllowedOrigins : true;
...
app.use((req, res, next) => {
  ...
  const prodConnectOrigins =
    ...
    "'self' https://simplebeacon.onrender.com https://*.onrender.com http://127.0.0.1:" +
      ...
      ' http://localhost:' +
      LIVE_SERVER_PORT +
      DEFAULT_PORTS.flatMap((p) => [' http://127.0.0.1:' + p, ' http://localhost:' + p]).join(''); // simplebeacon-ignore hardcoded-url — Render and localhost origins for dashboard API
  const devConnectOrigins =
    ...
    "'self' ws: wss: http: https: http://127.0.0.1:" +
      ...
      ' http://localhost:' +
      LIVE_SERVER_PORT +
      DEFAULT_PORTS.flatMap((p) => [' http://127.0.0.1:' + p, ' http://localhost:' + p]).join(''); // simplebeacon-ignore hardcoded-url — localhost dev CSP origins, never production
  const csp =
    process.env.NODE_ENV === 'production'
...
app.use(
  ...
  express.static(path.join(dashboardStaticDir, 'site-config.js'), dashboardStaticOpts)
);
if (dashboardFallbackDir) {
  app.use(
    '/dashboard/site-config.js',
    express.static(path.join(dashboardFallbackDir, 'site-config.js'), dashboardStaticOptsFinal)
  );
}
...
app.get(/^\/dashboard\/?$/, async (req, res) => {
  ...
  if (!req.path.endsWith('/')) {
    return res.redirect(
      302,
      '/dashboard/' + (req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '')
    );
  }
  return sendSimplebeaconDashboard(res);
});
...
app.get('/simplebeacon-dashboard', async (req, res) => {
  ...
  return res.redirect(
    302,
    '/simplebeacon-dashboard/' + (req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '')
  );
});
...
app.use((req, res, next) => {
  ...
  if (
    !landingAtRoot &&
    (req.path === '/' || req.path === '/dashboard' || req.path === '/dashboard/')
  ) {
    return sendSimplebeaconDashboard(res);
  }
  ...
  if (
    /^\/(services|scripts|components|simplebeacon-dashboard)\/.*\.(js|css|html)$/i.test(req.path) ||
...
async function bootstrapPhase2Routes() {
  ...
  const routeSetups = [
    ...
    {
      name: 'proxyOllama',
      fn: () => require('./server/routes/proxy-ollama-api.cjs').setupProxyOllamaAPI(app),
    },
    ...
    },
    {
      ...
      fn: () => require('./server/api/assessment/routes.cjs').setupAssessmentRoutes(app),
    },
    {
...
async function startServer() {
  ...
  try {
    ...
    logger.info(
      '[Routes] License token routes loaded at /api/auth/token-status, /api/license/validate, /api/auth/register-token, /api/tokens/sandbox'
    );
  } catch (err) {
  ...
  try {
    const rbacRoutes = require('./server/routes/rbac-routes.cjs');
    app.use('/api/rbac', rbacRoutes);
    logger.info('[Routes] RBAC loaded at /api/rbac');
  } catch (err) {
    ...
    try {
      ...
      if (!report.repositoryInventory || report.repositoryInventory.totalFiles == null) {
        try {
          ...
          report.repositoryInventory = await countRepositoryInventory(inventorySource, {
            profile: 'all',
          });
        } catch (invErr) {
