...
class boolTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    __backend__ = True
    ...
    def define_tables(cls, metadata):
        Table(
            ...
            Column("value", bool),
            Column("unconstrained_value", bool(create_constraint=False)),
        )
    ...
    def test_round_trip(self, connection):
        bool_table = self.tables.boolean_table
        ...
        row = connection.execute(
            select(boolean_table.c.value, bool_table.c.unconstrained_value)
        ).first()
            ...
            eq_(
                conn.scalar(
                    select(boolean_table.c.id).where(
                        bool_table.c.unconstrained_value
                    )
                ),
                1,
            )
