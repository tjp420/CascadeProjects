...
async function runTests() {
  ...
  try {
    ...
    assert(
      ...
      `GET /api/health returns 200 (got ${health.statusCode})`,
    );
    assert(
      health.body.status === "healthy",
      `Response status is "healthy" (got "${health.body.status}")`,
    );
  } catch (err) {
  ...
  try {
    ...
    assert(
      res.statusCode === 200,
      `POST /api/realtime/session returns 200 (got ${res.statusCode})`,
    );
  ...
  try {
    ws = await wsConnect(
      `ws://${API_HOST}:${WS_PORT}/api/realtime/stream?sessionId=${sessionId}`,
    );
  ...
  await new Promise((resolve) => {
    ...
    const handler = (data) => {
      ...
      try {
        ...
        assert(
          msg.type === "status",
          `Initial message type is "status" (got "${msg.type}")`,
        );
        assert(
          msg.sessionId === sessionId,
          `Message sessionId matches (got "${msg.sessionId}")`,
        );
        assert(
          msg.status === "connected",
          `Message status is "connected" (got "${msg.status}")`,
        );
      } catch (err) {
  ...
  await new Promise((resolve) => {
    ...
    const handler = (data) => {
      ...
      try {
        ...
        assert(
          msg.type === "pong",
          `Response type is "pong" (got "${msg.type}")`,
        );
  ...
  try {
    ...
    assert(
      res.statusCode === 200,
      `DELETE /api/realtime/session returns 200 (got ${res.statusCode})`,
    );
  ...
  console.log("\nTest 8: WebSocket rejection without sessionId");
  await new Promise((resolve) => {
    const wsNoSession = new WebSocket(
      `ws://${API_HOST}:${WS_PORT}/api/realtime/stream`,
    );
    const timer = setTimeout(() => {
      wsNoSession.close();
    ...
    wsNoSession.on("close", (code, reason) => {
      ...
      assert(
        code === 1008,
        `WebSocket closed with code 1008 (got ${code}): ${reason.toString()}`,
      );
      resolve();
    });
