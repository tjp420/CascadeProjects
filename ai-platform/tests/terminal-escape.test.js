...
const mainJsPath = path.resolve(__dirname, '../../coming-soon/js/dashboard/main.js');
const mainJsSource = fs.readFileSync(mainJsPath, 'utf8');

// Minimal stub: extract just the escapeHtml function definition
const escapeHtmlMatch = mainJsSource.match(/function escapeHtml\(str\)\s*\{[^}]+\}/);
if (!escapeHtmlMatch) {
...
describe('Terminal line HTML escaping', () => {
  ...
  test('escapeHtml passes through safe text unchanged', () => {
    ...
    expect(escapeHtml('Scan complete — 42 files')).toBe('Scan complete — 42 files');
  });
