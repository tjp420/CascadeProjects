...
global.setInterval = function (fn, ms, ...args) {
  ...
  try {
    __jestIntervals.add(id);
  } catch (e) {}
  return id;
};
global.clearInterval = function (id) {
  try {
    __jestIntervals.delete(id);
  } catch (e) {}
...
process.env.JWT_REFRESH_SECRET =
  process.env.JWT_REFRESH_SECRET ||
  "test-jwt-refresh-secret-key-for-testing-32chars";
...
global.testUtils = {
  ...
  cleanupTestData: async () => {
    ...
    if (global.__testDbPool) {
      try {
        if (typeof global.__testDbPool.end === "function")
          await global.__testDbPool.end();
      } catch (e) {
        // ignore cleanup errors
      }
      try {
        delete global.__testDbPool;
      } catch (e) {}
    }
...
afterAll(async () => {
  ...
  try {
    ...
    if (
      reportScheduler &&
      typeof reportScheduler.stopScheduler === "function"
    ) {
      try {
