...
def run_openssl(cmd):
    OPENSSL = "openssl"
    p = /* SECURITY WARNING: Command execution - use subprocess.run with shell=False and validate inputs */
...
class ECDSA(unittest.TestCase):
    def test_basic(self):
        ...
        pub2 = VerifyingKey.from_str(pub.to_str())
    ...
    def test_lengths_default(self):
        ...
        priv = SigningKey.generate()
        pub = priv.get_verifying_key()
        self.assertEqual(len(pub.to_str()), default.verifying_key_length)
    ...
    def test_serialize(self):
        ...
        self.assertEqual(
            hexlify(priv1.to_str()), hexlify(priv2.to_str())
        )
        ...
        self.assertEqual(hexlify(pub1.to_str()), hexlify(pub2.to_str()))
    ...
    def test_nonrandom(self):
        ...
        self.assertEqual(
            hexlify(priv1.get_verifying_key().to_str()),
            hexlify(priv2.get_verifying_key().to_str()),
        )
    ...
    def test_pubkey_strings(self):
        ...
        pub2 = VerifyingKey.from_str(s1, curve=NIST256p)
        self.assertTruePubkeysEqual(pub1, pub2)
    ...
    def test_pubkey_strings_brainpool(self):
        ...
        pub2 = VerifyingKey.from_str(s1, curve=BRAINPOOLP512r1)
        self.assertTruePubkeysEqual(pub1, pub2)
    ...
    def test_vk_from_der_garbage_after_curve_oid(self):
        ...
        point_der = der.encode_bitstr(b"\x00\xff", None)
    ...
    def test_vk_from_der_garbage_after_point_str(self):
        type_oid_der = encoded_oid_ecPublicKey
        curve_oid_der = der.encode_oid(*(1, 2, 840, 10045, 3, 1, 1))
        enc_type_der = der.encode_sequence(type_oid_der, curve_oid_der)
        point_der = der.encode_bitstr(b"\x00\xff", None) + b"garbage"
        to_decode = der.encode_sequence(enc_type_der, point_der)

        with self.assertRaises(der.UnexpectedDER):
            VerifyingKey.from_der(to_decode)

    def test_vk_from_der_invalid_bitstr(self):
        type_oid_der = encoded_oid_ecPublicKey
        curve_oid_der = der.encode_oid(*(1, 2, 840, 10045, 3, 1, 1))
        enc_type_der = der.encode_sequence(type_oid_der, curve_oid_der)
        point_der = der.encode_bitstr(b"\x08\xff", None)
        to_decode = der.encode_sequence(enc_type_der, point_der)
    ...
    def test_vk_from_der_with_invalid_length_of_encoding(self):
        ...
        curve_oid_der = der.encode_oid(*(1, 2, 840, 10045, 3, 1, 1))
        enc_type_der = der.encode_sequence(type_oid_der, curve_oid_der)
        point_der = der.encode_bitstr(b"\xff" * 64, 0)
        to_decode = der.encode_sequence(enc_type_der, point_der)
    ...
    def test_vk_from_der_with_raw_encoding(self):
        ...
        curve_oid_der = der.encode_oid(*(1, 2, 840, 10045, 3, 1, 1))
        enc_type_der = der.encode_sequence(type_oid_der, curve_oid_der)
        point_der = der.encode_bitstr(b"\xff" * 48, 0)
        to_decode = der.encode_sequence(enc_type_der, point_der)
    ...
    def test_sk_from_string_with_wrong_len_str(self):
        with self.assertRaises(MalformedPointError):
            SigningKey.from_str(b"\x01")
    ...
    def test_sk_from_der_invalid_const_tag(self):
        ver_der = der.encode_integer(1)
        privkey_der = der.encode_octet_str(b"\x00\xff")
    ...
    def test_sk_from_p8_der_with_wrong_version(self):
        ...
        privkey_der = der.encode_octet_str(
            der.encode_sequence(
                der.encode_integer(1), der.encode_octet_str(b"\x00\xff")
            )
        )
    ...
    def test_hashfunc(self):
        ...
        sig = sk.sign(data)
        vk = VerifyingKey.from_str(
            sk.get_verifying_key().to_str(),
            curve=NIST256p,
        ...
        vk2 = VerifyingKey.from_str(
            sk2.get_verifying_key().to_str(),
            curve=NIST256p,
            hashfunc=hashlib.sha256,
        )
        self.assertTrue(vk2.verify(sig2, data))

        vk3 = VerifyingKey.from_str(
            sk.get_verifying_key().to_str(), curve=NIST256p
        )
    ...
    def test_encoding(self):
        ...
        self.assertEqual(vk.to_str(), exp)
        self.assertEqual(vk.to_str("raw"), exp)
        self.assertEqual(vk.to_str("uncompressed"), b"\x04" + exp)
        self.assertEqual(vk.to_str("compressed"), b"\x02" + exp[:24])
        self.assertEqual(vk.to_str("hybrid"), b"\x06" + exp)
    ...
    def test_decoding(self):
        ...
        from_raw = VerifyingKey.from_str(enc)
        self.assertEqual(from_raw.pubkey.point, vk.pubkey.point)

        from_uncompressed = VerifyingKey.from_str(b"\x04" + enc)
        self.assertEqual(from_uncompressed.pubkey.point, vk.pubkey.point)

        from_compressed = VerifyingKey.from_str(b"\x02" + enc[:24])
        self.assertEqual(from_compressed.pubkey.point, vk.pubkey.point)

        from_uncompressed = VerifyingKey.from_str(b"\x06" + enc)
        self.assertEqual(from_uncompressed.pubkey.point, vk.pubkey.point)
    ...
    def test_uncompressed_decoding_as_only_alowed(self):
        ...
        vk = VerifyingKey.from_str(enc, valid_encodings=("uncompressed",))
        sk = SigningKey.from_secret_exponent(123456789)
    ...
    def test_raw_decoding_with_blocked_format(self):
        ...
        with self.assertRaises(MalformedPointError) as exp:
            VerifyingKey.from_str(enc, valid_encodings=("hybrid",))

        self.assertIn("hybrid", str(exp.exception))

    def test_decoding_with_unknown_format(self):
        with self.assertRaises(ValueError) as e:
            VerifyingKey.from_str(b"", valid_encodings=("raw", "foobar"))
    ...
    def test_hybrid_decoding_with_blocked_format(self):
        ...
        with self.assertRaises(MalformedPointError) as exp:
            VerifyingKey.from_str(enc, valid_encodings=("uncompressed",))
    ...
    def test_hybrid_decoding_with_inconsistent_encoding_and_no_validation(
        ...
        enc = vk.to_str("hybrid")
        self.assertEqual(enc[:1], b"\x06")
        enc = b"\x07" + enc[1:]

        b = VerifyingKey.from_str(
    ...
    def test_compressed_decoding_with_blocked_format(self):
        ...
        with self.assertRaises(MalformedPointError) as exp:
            VerifyingKey.from_str(enc, valid_encodings=("hybrid", "raw"))
    ...
    def test_decoding_with_malformed_uncompressed(self):
        ...
        with self.assertRaises(MalformedPointError):
            VerifyingKey.from_str(b"\x02" + enc)
    ...
    def test_decoding_with_malformed_compressed(self):
        ...
        with self.assertRaises(MalformedPointError):
            VerifyingKey.from_str(b"\x01" + enc[:24])
    ...
    def test_decoding_with_inconsistent_hybrid(self):
        ...
        with self.assertRaises(MalformedPointError):
            VerifyingKey.from_str(b"\x07" + enc)
    ...
    def test_decoding_with_inconsistent_hybrid_odd_point(self):
        ...
        with self.assertRaises(MalformedPointError):
            b = VerifyingKey.from_str(enc, valid_encodings=("hybrid",))
    ...
    def test_from_string_with_invalid_curve_too_short_ver_key_len(self):
        ...
        with self.assertRaises(MalformedPointError):
            VerifyingKey.from_str(b"\x00" * 16, curve)
...
def test_VerifyingKey_decode_with_small_values(val, even):
    enc = number_to_str(val, NIST192p.order)
...
def test_VerifyingKey_encode_decode(curve, encoding):
    ...
    encoded = vk.to_str(encoding)

    from_enc = VerifyingKey.from_str(encoded, curve=curve)
...
def test_lengths(curve):
    ...
    pub2 = VerifyingKey.from_str(pub1.to_str(), curve)
    assert pub1.to_str() == pub2.to_str()
    assert len(pub1.to_str()) == curve.verifying_key_length
    sig = priv.sign(b"data")
...
def test_RFC4754_vectors(w, gwx, gwy, k, msg, md, r, s, curve):
    sk = SigningKey.from_str(unhexlify(w), curve)
    vk = VerifyingKey.from_str(unhexlify(gwx + gwy), curve)
