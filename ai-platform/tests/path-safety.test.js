...
describe("path-safety", () => {
  ...
  describe("assertSafeProjectPath", () => {
    ...
    test("throws when path is empty", () => {
      ...
      expect(() => assertSafeProjectPath(null, [tmpDir])).toThrow(
        /is required/,
      );
    });

    test("throws when path contains null bytes", () => {
      expect(() => assertSafeProjectPath("/tmp/\0evil", [tmpDir])).toThrow(
        /invalid characters/,
      );
    });
    ...
    test("throws when path does not exist", () => {
      expect(() =>
        assertSafeProjectPath(path.join(tmpDir, "does-not-exist"), [tmpDir]),
      ).toThrow(/does not exist/);
    });

    test("uses custom label in error", () => {
      expect(() => assertSafeProjectPath("", [tmpDir], "scanPath")).toThrow(
        /scanPath is required/,
      );
    });
  });
  ...
  describe("validateRepoUrl", () => {
    ...
    test("throws for invalid characters", () => {
      expect(() =>
        validateRepoUrl("https://github.com/user/repo;rm -rf"),
      ).toThrow(/invalid characters/);
      expect(() => validateRepoUrl("https://github.com/user/repo'")).toThrow(
        /invalid characters/,
      );
    });
    ...
    test("allows custom allowedSchemes", () => {
      expect(() =>
        validateRepoUrl("http://github.com/user/repo", {
  ...
  describe("assertSafeExecutablePath", () => {
    ...
    test("throws for relative path", () => {
      expect(() => assertSafeExecutablePath("./node")).toThrow(
        /must be an absolute path/,
      );
    });

    test("throws for path with invalid characters", () => {
      expect(() => assertSafeExecutablePath("/usr/bin/node;rm -rf")).toThrow(
        /invalid characters/,
      );
      expect(() => assertSafeExecutablePath("/usr/bin/node|cat")).toThrow(
        /invalid characters/,
      );
    });

    test("uses custom label", () => {
      expect(() => assertSafeExecutablePath("", "binary")).toThrow(
