...
class TestVerifyingKeyFromString(unittest.TestCase):
    """
    Verify that ecdsa.keys.VerifyingKey.from_str() can be used with
    bytes-like objects
    """
    ...
    def setUpClass(cls):
        ...
        cls.vk = VerifyingKey.from_str(cls.key_bytes)
    ...
    def test_bytes_memoryview(self):
        vk = VerifyingKey.from_str(buffer(self.key_bytes))

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_bytearray(self):
        vk = VerifyingKey.from_str(bytearray(self.key_bytes))

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_bytesarray_memoryview(self):
        vk = VerifyingKey.from_str(buffer(bytearray(self.key_bytes)))

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_array_array_of_bytes(self):
        arr = array.array("B", self.key_bytes)
        vk = VerifyingKey.from_str(arr)

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_array_array_of_bytes_memoryview(self):
        arr = array.array("B", self.key_bytes)
        vk = VerifyingKey.from_str(buffer(arr))
    ...
    def test_bytes_uncompressed(self):
        vk = VerifyingKey.from_str(b"\x04" + self.key_bytes)

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_bytearray_uncompressed(self):
        vk = VerifyingKey.from_str(bytearray(b"\x04" + self.key_bytes))

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_bytes_compressed(self):
        vk = VerifyingKey.from_str(b"\x02" + self.key_bytes[:24])

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_bytearray_compressed(self):
        vk = VerifyingKey.from_str(bytearray(b"\x02" + self.key_bytes[:24]))

        self.assertEqual(self.vk.to_str(), vk.to_str())

    def test_ed25519_VerifyingKey_from_string_imported(self):
        with self.assertRaises(MalformedPointError):
            VerifyingKey.from_str(b"AAA", Ed25519)
...
class TestVerifyingKeyFromDer(unittest.TestCase):
    ...
    def test_ed25519_VerifyingKey_repr__(self):
        sk = SigningKey.from_str(Ed25519.generator.to_bytes(), Ed25519)
    ...
    def test_edwards_precompute_no_side_effect(self):
        sk = SigningKey.from_str(Ed25519.generator.to_bytes(), Ed25519)
        vk = sk.verifying_key
        vk2 = VerifyingKey.from_str(vk.to_str(), Ed25519)
        vk.precompute()
    ...
    def test_parse_malfomed_eddsa_der_pubkey(self):
        der_str = encode_sequence(
            ...
            encode_bitstr(bytes(Ed25519.generator.to_bytes()), 0),
            encode_bitstr(b"\x00", 0),
        )
    ...
    def test_load_ed25519_from_pem(self):
        ...
        vk_2 = VerifyingKey.from_str(vk_str, Ed25519)
    ...
    def test_export_ed255_to_pem(self):
        ...
        vk = VerifyingKey.from_str(vk_str, Ed25519)
    ...
    def test_ed448_from_pem(self):
        ...
        vk2 = VerifyingKey.from_str(vk_str, Ed448)
    ...
    def test_ed448_to_pem(self):
        ...
        vk = VerifyingKey.from_str(vk_str, Ed448)
...
class TestSigningKey(unittest.TestCase):
    ...
    def test_ed25519_from_der_bad_alg_id_params(self):
        der_str = encode_sequence(
            ...
            encode_octet_str(encode_octet_str(b"A" * 32)),
        )
    ...
    def test_ed25519_from_der_junk_after_priv_key(self):
        der_str = encode_sequence(
            ...
            encode_octet_str(encode_octet_str(b"A" * 32) + b"B"),
        )
...
class TestTrivialCurve(unittest.TestCase):
    ...
    def test_public_key_sanity(self):
        self.assertEqual(self.sk.verifying_key.to_str(), b"\x98\x1e")
...
def test_SigningKey_from_str(convert):
    key = convert(prv_key_bytes)
    sk = SigningKey.from_str(key)

    assert sk.to_str() == prv_key_bytes
