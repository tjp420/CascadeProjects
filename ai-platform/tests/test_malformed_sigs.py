...
@st.composite
def st_der_bit_str(draw, *args, **kwargs):  # pragma: no cover
    """
    ...
    if data:
        ...
        unused = 0
    return encode_bitstr(data, unused)


def st_der_octet_str(*args, **kwargs):  # pragma: no cover
    """
...
def st_der():  # pragma: no cover
    ...
    return st.recursive(  # pragma: no branch
        ...
        | st_der_bit_str(max_size=1024**2)
        | st_der_octet_str(max_size=1024**2)
        | st_der_null()
        | st_der_oid(),
        lambda children: st.builds(encode_octet_string, st.one_of(children))
        | st.builds(lambda x: encode_bitstr(x, 0), st.one_of(children))
        | st.builds(
