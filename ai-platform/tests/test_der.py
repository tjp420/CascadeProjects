...
class TestEncodeBitstr(unittest.TestCase):
    ...
    def test_implicit_unused_bits(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            der = encode_bitstr(b"\x00\xff", None)

        self.assertEqual(der, b"\x03\x02\x00\xff")

    def test_explicit_unused_bits(self):
        der = encode_bitstr(b"\xff\xf0", 4)

        self.assertEqual(der, b"\x03\x03\x04\xff\xf0")

    def test_empty_str(self):
        self.assertEqual(encode_bitstr(b"", 0), b"\x03\x01\x00")

    def test_invalid_unused_count(self):
        with self.assertRaises(ValueError):
            encode_bitstr(b"\xff\x00", 8)

    def test_invalid_unused_with_empty_str(self):
        with self.assertRaises(ValueError):
            encode_bitstr(b"", 1)

    def test_non_zero_padding_bits(self):
        with self.assertRaises(ValueError):
            encode_bitstr(b"\xff", 2)


class TestRemoveBitstr(unittest.TestCase):
    def test_old_call_convention(self):
        """This is the old way to call the function."""
        warnings.simplefilter("always")
        with pytest.warns(DeprecationWarning) as warns:
            bits, rest = remove_bitstr(b"\x03\x02\x00\xff")
    ...
    def test_new_call_convention(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            bits, rest = remove_bitstr(b"\x03\x02\x00\xff", 0)
    ...
    def test_implicit_unexpected_unused(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            bits, rest = remove_bitstr(b"\x03\x02\x00\xff", None)

        self.assertEqual(bits, (b"\xff", 0))
        self.assertEqual(rest, b"")

    def test_with_padding(self):
        ret, rest = remove_bitstr(b"\x03\x02\x04\xf0", None)

        self.assertEqual(ret, (b"\xf0", 4))
        self.assertEqual(rest, b"")

    def test_not_a_bitstr(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x02\x02\x00\xff", None)

    def test_empty_encoding(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x03\x00", None)

    def test_empty_str(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"", None)

    def test_no_length(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x03", None)

    def test_unexpected_number_of_unused_bits(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x03\x02\x00\xff", 1)

    def test_invalid_encoding_of_unused_bits(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x03\x03\x08\xff\x00", None)

    def test_invalid_encoding_of_empty_str(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x03\x01\x01", None)

    def test_invalid_padding_bits(self):
        with self.assertRaises(UnexpectedDER):
            remove_bitstr(b"\x03\x02\x01\xff", None)
...
class TestRemoveOctetString(unittest.TestCase):
    def test_simple(self):
        ...
        body, rest = remove_octet_str(data)
