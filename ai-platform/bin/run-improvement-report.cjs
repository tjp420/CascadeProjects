...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (arg === "--dry-run" || arg === "-n") {
      ...
      console.log(
        "Usage: node run-improvement-report.cjs [--dry-run] [--days N]",
      );
      console.log("");
      console.log("Options:");
      console.log("  --dry-run, -n   Print report to stdout, do not email");
      console.log(
        "  --days N, -d N  Time window in days (default: 30, max: 365)",
      );
...
async function run() {
  ...
  console.log(
    "[" + timestamp + "] Initializing internal improvement report...",
  );
  console.log(
    "[ImprovementReportCLI] Mode: " + (args.dryRun ? "DRY RUN" : "EMAIL"),
  );
...
run().catch(function (err) {
  console.error(
    "[ImprovementReportCLI] Unhandled error: " +
