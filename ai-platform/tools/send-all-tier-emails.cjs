...
if (!APP_URL) {
  process.stderr.write(
    ["SIMPLEBEACON_APP_URL env var is required"].join(" ") + "\n",
  );
  process.exit(1);
}
...
function buildEmail(tier, config) {
  ...
  if (config.token) {
    token = generateLicenseToken(
      {
        email:
          process.env.SIMPLEBEACON_OWNER_EMAIL ||
          "admin@" + "simplebeacon.local",
        tier: tier,
  ...
  r(
    "{{INVOICE_LINE}}",
    config.price === "Free" ? "Community Access" : `Invoice #${invoiceId}`,
  );
  r("{{RECEIPT_CLASS}}", config.receiptClass || "");
  r("{{LICENSE_TOKEN}}", token);
  r(
    "{{PRIMARY_URL}}",
    config.primaryUrl || `${BASE_URL}?session_id=${sessionId}&tier=${tier}`,
  );
...
const TIERS = {
  ...
  executive: {
    ...
    `,
    stepsText:
      "Run scan, upload report, analyst review, receive certificate in 48h",
    privacyText:
  ...
  agency: {
    ...
    `,
    stepsText:
      "Run scan at milestones, upload reports, get co-branded certificates",
    privacyText:
  ...
  annual: {
    ...
    `,
    stepsText:
      "Priority audit, warranty re-scan, analyst access, all scan types",
    privacyText:
  ...
  euai: {
    ...
    `,
    stepsText:
      "Open dashboard, paste token, upload code, local scan, instant PDF",
    privacyText:
  ...
  enterprise: {
    ...
    primaryUrl:
      "mailto:enterprise@simplebeacon.ai?subject=Enterprise%20Onboarding%20Request",
    ...
    `,
    stepsText:
      "Analyst contact within 24h, onboarding call, dashboard setup, monitoring begins",
    privacyText:
