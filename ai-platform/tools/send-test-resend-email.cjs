...
const main = async () => {
  ...
  if (!apiKey) {
    console.error(
      "ERROR: RESEND_API_KEY is not set. Export RESEND_API_KEY=re_xxx",
    );
    process.exit(2);
  }
  if (!to) {
    console.error(
      "ERROR: Recipient not provided. Set RESEND_TO or pass recipient as first arg.",
    );
    process.exit(2);
  }
  ...
  if (typeof fetch === "undefined") {
    console.error(
      "ERROR: global fetch is not available in this Node runtime. Use Node 18+ or install node-fetch.",
    );
    process.exit(3);
  }
