...
const OUT_MD = path.join(process.cwd(), 'ai-platform', 'test-results', 'mock-pr-comment.md');
...
function main() {
  ...
  if (!current) {
    console.log('No current summary found at', CI_SUMMARY, 'using mock current run.');
    current = makeMockCurrent();
  }
  let history = loadJson(HISTORY);
  if (!history) {
    console.log('No history found at', HISTORY, 'using mock history (7 entries).');
