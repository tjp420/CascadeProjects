...
const REGRESSION_PERCENT = Number(process.env.CI_TELEMETRY_REGRESSION_PERCENT || 10);
...
if (!OWNER || !REPO_NAME || !TOKEN || !PR_NUMBER || !RUN_ID) {
  console.error('Missing required env vars. Ensure GITHUB_REPOSITORY, GITHUB_TOKEN, PR_NUMBER, and GITHUB_RUN_ID are set.');
  process.exit(2);
}
...
function downloadArtifactZip(url) {
  const res = spawnSync('curl', ['-sS', '-L', '-H', `Authorization: token ${TOKEN}`, url], { encoding: 'buffer' });
  if (res.status !== 0) throw new Error(`curl download failed: ${res.stderr.toString()}`);
  return res.stdout;
}
...
function unzipFileFromZipBuffer(zipBuffer, filename) {
  ...
  const res = spawnSync('unzip', ['-p', tmp, filename], { encoding: 'utf8', maxBuffer: 20 * 1024 * 1024 });
  fs.unlinkSync(tmp);
...
function upsertPrComment(prNumber, body) {
  ...
  if (existing) {
    ...
    const res = spawnSync('curl', ['-sS', '-X', 'PATCH', '-H', `Authorization: token ${TOKEN}`, '-H', 'Accept: application/vnd.github.v3+json', patchUrl, '-d', JSON.stringify({ body })], { encoding: 'utf8' });
    if (res.status !== 0) throw new Error(`Failed to edit comment: ${res.stderr}`);
    return JSON.parse(res.stdout);
  } else {
    const post = `https://api.github.com/repos/${OWNER}/${REPO_NAME}/issues/${prNumber}/comments`;
    const res = spawnSync('curl', ['-sS', '-X', 'POST', '-H', `Authorization: token ${TOKEN}`, '-H', 'Accept: application/vnd.github.v3+json', post, '-d', JSON.stringify({ body })], { encoding: 'utf8' });
    if (res.status !== 0) throw new Error(`Failed to create comment: ${res.stderr}`);
    return JSON.parse(res.stdout);
  }
}

function safeNumber(v) { return typeof v === 'number' ? v : (v ? Number(v) : 0); }
...
function computeBaseline(history, branch='main', lookback=7) {
  ...
  const vals = filtered.map(h => safeNumber(h.summary && h.summary.runTimeMs));
  ...
  const fileAvg = {};
  for (const [k,v] of Object.entries(fileMap)) fileAvg[k] = v.reduce((a,b)=>a+(b||0),0)/v.length;
...
async function run() {
  ...
  const artifact = (artifacts.artifacts || []).find(a => a.name === 'ci-test-metrics');
  if (!artifact) {
  ...
  if (!summaryContent) { console.log('ci-test-summary.json not found in artifact.'); return; }
  ...
  const wf = wfList.workflows.find(w => w.path && w.path.endsWith('ci-telemetry-aggregate.yml')) || wfList.workflows.find(w => w.name && w.name.includes('CI Telemetry Aggregation'));
  let history = [];
  if (wf) {
    // find latest successful run
    const runs = ghApi(`/actions/workflows/${wf.id}/runs?per_page=20`);
    const run = (runs.workflow_runs || []).find(r => r.conclusion === 'success');
    if (run) {
      const arts = ghApi(`/actions/runs/${run.id}/artifacts`);
      const histArt = (arts.artifacts || []).find(a => a.name === 'ci-telemetry-history');
      if (histArt) {
        const zip = downloadArtifactZip(histArt.archive_download_url);
        const histContent = unzipFileFromZipBuffer(zip, 'ci-telemetry-history.json');
  ...
  let deltaPct = (baselineMs && deltaMs !== null) ? (deltaMs / baselineMs * 100) : null;

  // top N slowest files current
  const files = Array.isArray(current.testFileSummaries) ? current.testFileSummaries.slice() : [];
  files.sort((a,b)=> (b.runTimeMs||0) - (a.runTimeMs||0));
  const topFiles = files.slice(0, TOP_N).map(f => ({ name: path.basename(f.filePath||''), runMs: safeNumber(f.runTimeMs||0) }));

  // for each top file compute baseline
  const fileRows = topFiles.map(f => {
    const base = baseline && baseline.fileAverages && baseline.fileAverages[f.name] ? baseline.fileAverages[f.name] : null;
    const dms = (base !== null) ? (f.runMs - base) : null;
    const dpct = (base !== null && base > 0) ? (dms / base * 100) : null;
    return { name: f.name, runMs: f.runMs, baseMs: base, deltaMs: dms, deltaPct: dpct };
  });

  // 5) build markdown
  const warning = (deltaMs !== null && (Math.abs(deltaPct) >= REGRESSION_PERCENT || Math.abs(deltaMs) >= REGRESSION_MS));
  ...
  md += '| File | Current (ms) | Baseline (ms) | Delta |\n|---|---:|---:|---:|\n';
