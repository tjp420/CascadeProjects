...
const OWNER = process.env.GITHUB_REPOSITORY ? process.env.GITHUB_REPOSITORY.split('/')[0] : process.argv[2];
const REPO = process.env.GITHUB_REPOSITORY ? process.env.GITHUB_REPOSITORY.split('/')[1] : process.argv[3];
const TOKEN = process.env.GITHUB_TOKEN || process.argv[4];
const OUT_DIR = process.argv[5] || path.join(process.cwd(), 'ai-platform', 'test-results');
const RETENTION_DAYS = Number(process.env.CI_TELEMETRY_RETENTION_DAYS || process.argv[6] || 30);
const BRANCH_FILTER = process.env.CI_TELEMETRY_BRANCHES || process.argv[7] || 'all'; // comma-separated or 'all'
const MAX_RUNS = Number(process.env.CI_TELEMETRY_MAX_RUNS || process.argv[8] || 0); // 0 => no max

if (!OWNER || !REPO || !TOKEN) {
  console.error('Usage: aggregate-artifacts.cjs <owner> <repo> <token> [outDir] [retentionDays] [branches] [maxRuns]');
...
function downloadArtifactZip(url) {
  ...
  const tmp = spawnSync('curl', ['-sS', '-L', '-H', `Authorization: token ${TOKEN}`, url], { encoding: 'buffer' });
  if (tmp.status !== 0) throw new Error(`curl download failed: ${tmp.stderr.toString()}`);
  return tmp.stdout;
}
...
function unzipFileFromZipBuffer(zipBuffer, filename) {
  ...
  const res = spawnSync('unzip', ['-p', tmpPath, filename], { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
...
async function run() {
  ...
  // filter by branch
  let branches = [];
  if (BRANCH_FILTER !== 'all') branches = BRANCH_FILTER.split(',').map(s => s.trim());
  ...
  for (const r of runs) {
    ...
    try {
      ...
      const target = artifacts.artifacts.find(a => a.name === 'ci-test-metrics');
      if (!target) continue;
      console.log(`Downloading artifact for run ${r.id} (${r.head_branch} #${r.run_number})`);
      ...
      if (!content) {
        console.warn('ci-test-summary.json not found in artifact for run', r.id);
        continue;
      }
  ...
  const rows = ['run_id,run_number,branch,created_at,totalTests,totalPassed,totalFailed,runTimeMs'];
  for (const h of history) {
    const s = h.summary || {};
    rows.push([h.run_id, h.run_number, h.branch, h.created_at, s.totalTests || 0, s.totalPassed || 0, s.totalFailed || 0, s.runTimeMs || 0].join(','));
  }
