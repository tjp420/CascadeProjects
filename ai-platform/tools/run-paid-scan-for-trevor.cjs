...
async function main() {
  ...
  if (!serverUp) {
    ...
    process.stderr.write(
      ["Please start it first: .\\start-dashboard.bat"].join(" ") + "\n",
    );
    process.stderr.write(
  ...
  process.stdout.write(
    [
      `Repository files:     ${report.repositoryInventory?.totalFiles ?? "—"}`,
    ].join(" ") + "\n",
  );
  process.stdout.write(
    [
  ...
  process.stdout.write(
    [`Quality score:        ${report.qualityScore ?? "—"}%`].join(" ") + "\n",
  );
