...
async function waitForServer(maxRetries = 30) {
  ...
  throw new Error(
    `Server not reachable at ${BASE}. Start it with: cd ai-platform && npm start`,
  );
}
...
async function run() {
  ...
  console.log(
    `  Expected Price : $${(EXPECTED_AMOUNT / 100).toFixed(2)} ${MODE === "annual" ? "/yr" : "/mo"}`,
  );
  if (EXTRA_SEATS > 0)
    console.log(
      `  Extra Seats    : ${EXTRA_SEATS} ($${((EXTRA_SEATS * (MODE === "annual" ? 15000 : 1500)) / 100).toFixed(2)})`,
    );
  ...
  log(
    2,
    "Stripe sends checkout.session.completed webhook → POST /api/simplebeacon/billing/webhook",
  );
  ...
  const webhookPayload = {
    ...
    data: {
      object: {
        ...
        mode:
          MODE === "annual" || MODE === "monthly" ? "subscription" : "payment",
        amount_total:
          EXPECTED_AMOUNT + EXTRA_SEATS * (MODE === "annual" ? 15000 : 1500),
        metadata: {
          tier: TIER,
          product:
            TIER === "developer"
  ...
  const { body: webhookBody, header: stripeSignature } = signWebhook(
    webhookPayload,
    WEBHOOK_SECRET,
  );
  ...
  const licenseResp = await httpRequest(
    "GET",
    `/api/simplebeacon/billing/license?email=${encodeURIComponent(EMAIL)}`,
  );
  ...
  log(
    5,
    "Customer uses license to check report status → GET /api/reports/status/{token}",
  );

  const statusResp = await httpRequest(
    "GET",
    `/api/reports/status/${encodeURIComponent(licenseToken)}`,
  );
  ...
  if (statusResp.body && typeof statusResp.body === "object") {
    console.log(
      "  Response:",
      JSON.stringify(statusResp.body, null, 2).split("\n").join("\n  "),
    );
  } else {
  ...
  if (EXTRA_SEATS > 0) {
    ...
    console.log(
      `  Seats    : ${EXTRA_SEATS} extra × $${MODE === "annual" ? 150 : 15}/seat = $${(seatCost / 100).toFixed(2)}`,
    );
    console.log(
      `  Total    : $${((EXPECTED_AMOUNT + seatCost) / 100).toFixed(2)}`,
    );
  }
  console.log(`  Tier     : ${payload.tier}`);
