...
function checkEnvironmentVariable(name, required = true) {
  ...
  if (!value) {
    if (required) {
      process.stderr.write([`❌ REQUIRED: ${name} is not set`].join(' ') + '\n');
      return false;
    } else {
      process.stderr.write([`⚠️  OPTIONAL: ${name} is not set`].join(' ') + '\n');
      return true;
    }
  }
  ...
  if (name.includes('JWT_SECRET') && value.length < 32) {
    process.stderr.write(
      [`❌ REQUIRED: ${name} must be at least 32 characters (current: ${value.length})`].join(' ') +
        '\n'
    );
    return false;
  }
...
function checkFileExists(filePath, description) {
  try {
    if (fs.existsSync(filePath)) {
      process.stdout.write([`✅ ${description} exists: ${filePath}`].join(' ') + '\n');
      return true;
    } else {
      process.stderr.write([`❌ ${description} missing: ${filePath}`].join(' ') + '\n');
      return false;
    }
  } catch (error) {
    process.stderr.write([`❌ Error checking ${description}: ${error.message}`].join(' ') + '\n');
    return false;
  }
}
...
function checkAuthConfiguration() {
  ...
  if (requireAuth === 'true') {
    ...
    if (!jwtRefreshSecret || jwtRefreshSecret.length < 32) {
      process.stderr.write(
        ['❌ REQUIRE_AUTH=true requires JWT_REFRESH_SECRET (32+ characters)'].join(' ') + '\n'
      );
      return false;
    }

    process.stdout.write(['✅ Authentication configuration is valid'].join(' ') + '\n');
    return true;
  }
...
function checkPhase2Readiness() {
  ...
  if (phase2Vars.length === 0) {
    process.stderr.write(
      ['⚠️  No Phase 2 variables configured - Phase 2 features will be disabled'].join(' ') + '\n'
    );
    return false;
  }
  ...
  if (allPhase2VarsSet) {
    process.stdout.write(['✅ All Phase 2 variables configured'].join(' ') + '\n');
    return true;
  } else {
    process.stderr.write(
      ['⚠️  Some Phase 2 variables missing - Phase 2 features may be limited'].join(' ') + '\n'
    );
    return false;
  }
}
...
function main() {
  process.stdout.write(['🔍 Verifying v1-Internal Profile Configuration\n'].join(' ') + '\n');

  let allChecksPassed = true;

  // Check required environment variables
  process.stdout.write(['📋 Checking required environment variables:'].join(' ') + '\n');
  ...
  if (allChecksPassed) {
    ...
    process.stdout.write(['   3. Run smoke tests: npm run smoke:test'].join(' ') + '\n');
    process.exit(0);
  } else {
    process.stderr.write(['❌ Some checks failed - please fix the issues above'].join(' ') + '\n');
    process.stdout.write(['\n📚 Documentation:'].join(' ') + '\n');
    process.stdout.write(['   - Read: docs/v1-internal-runbook.md'].join(' ') + '\n');
    process.stdout.write(['   - Reference: .env.v1-internal.example'].join(' ') + '\n');
    process.exit(1);
  }
}
