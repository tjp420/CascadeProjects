...
try {
  ...
  console.error(
    "❌ Deploy gate failed: predeploy sequence checks did not pass.",
  );
  process.exit(1);
}
...
try {
  ...
  console.error(
    "❌ Deploy gate failed: production readiness checks did not pass.",
  );
  process.exit(1);
}
...
try {
  ...
  console.error(
    "❌ Deploy gate failed: v1-internal profile checks did not pass.",
  );
  process.exit(1);
}
...
try {
  ...
  if (isWindows) {
    ...
    readline.question(
      "Continue deploy despite smoke test warnings? (y/N): ",
      (answer) => {
        readline.close();
