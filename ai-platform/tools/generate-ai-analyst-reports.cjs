/**
 * Generate AI Analyst Reports
 *
 * Uses SimpleBeacon's built-in AI Analyst (server/lib/ai-analyst.cjs) to analyze
 * scan findings and produce board-ready documentation.
 *
 * Usage:
 *   node tools/generate-ai-analyst-reports.cjs
 *
 * Env:
 *   OPENAI_API_KEY or ANTHROPIC_API_KEY (optional — falls back to deterministic mode)
...
function normalizeScanForAiAnalyst(raw) {
  ...
  const overview = raw.analysisOverview || {};
...
async function main() {
  ...
  const technicalPayload = buildTechnicalPayload(scanData, verdict);
  const compliancePayload = buildCompliancePayload(scanData, verdict);
  const productPayload = buildProductPayload(scanData, verdict);
...
function buildExecutivePayload(scan, verdict) {
  const gate = scan.gate || {};
  const inventory = scan.repositoryInventory || {};
  return {
    verdict,
    ...
    repositoryFilesTotal: inventory.totalFiles,
    repositoryFoldersTotal: inventory.totalFolders,
    filesAnalyzed: scan.filesAnalyzed,
    credentialFindings: scan.credentialFindings,
    productionLeakFindings: scan.productionLeakFindings,
...
function buildTechnicalPayload(scan, verdict) {
  ...
  return {
    ...
    credentialFindings: scan.credentialFindings,
    productionLeakFindings: scan.productionLeakFindings,
    llmSlopHits: scan.llmSlopPatternHits,
...
function buildCompliancePayload(scan, verdict) {
  return {
    verdict,
    euAiAct: scan.euAiAct || {},
    dependencyAudit: scan.dependencyAudit || {},
    compliance: scan.compliance || {},
    buildReadiness: scan.buildReadiness || {},
...
function buildProductPayload(scan, verdict) {
  const inventory = scan.repositoryInventory || {};
  return {
    verdict,
    ...
    filesAnalyzed: scan.filesAnalyzed,
    totalSizeLabel: scan.totalSizeLabel,
    mockSampleFiles: scan.mockSampleFiles,
...
function buildQaPayload(scan, verdict) {
  const gate = scan.gate || {};
  return {
    verdict,
    jestBaselinePassed: scan.jestBaselinePassed,
    jestBaselineChecked: scan.jestBaselineChecked,
    qualityScore: scan.qualityScore,
    schemaCompliance: scan.schemaCompliance,
    consistencyScore: scan.consistencyScore,
    duplicateGroups: scan.duplicateGroups,
    invalidJson: scan.invalidJson,
    emptyFiles: scan.emptyFiles,
    gateBlockingCount: gate.blockingCount,
...
function renderExecutive(p) {
  return `# SimpleBeacon Platform — Executive Board Summary
**AI Analyst Verdict | Generated: ${new Date().toISOString()}**
...
| Metric | Value |
|--------|-------|
| **Repository Files** | ${p.repositoryFilesTotal} |
| **Repository Folders** | ${p.repositoryFoldersTotal} |
| **Files Analyzed** | ${p.filesAnalyzed} |
| **Credential Findings** | ${p.credentialFindings} |
| **Production Leak Findings** | ${p.productionLeakFindings} |
...
*Generated by SimpleBeacon AI Analyst | ${new Date().toISOString()}*
`;
}

function renderTechnical(p) {
  return `# SimpleBeacon Platform — Technical Architecture & Security
**AI Analyst Report | Generated: ${new Date().toISOString()}**
...
| Check | Result |
|-------|--------|
| **Credential Scan** | ${p.credentialFindings} findings |
| **Production Leak Scan** | ${p.productionLeakFindings} findings |
| **LLM Slop Detection** | ${p.llmSlopHits} hits |
| **EU AI Act Scan** | ${p.euAiActFindings} findings |
| **Schema Compliance** | ${p.schemaCompliance}% |
| **Data Consistency** | ${p.consistencyScore}% |

---

## Issue Breakdown

- **Critical**: ${p.criticalIssues.length}
...
function renderCompliance(p) {
  return `# SimpleBeacon Platform — Compliance & Governance
**AI Analyst Report | Generated: ${new Date().toISOString()}**

---

## Compliance Grade: ${p.verdict.complianceGrade}
...
**Liability Estimate**: ${p.verdict.estimatedLiability}
...
function renderProduct(p) {
  return `# SimpleBeacon Platform — Product Capabilities & Market Position
**AI Analyst Report | Generated: ${new Date().toISOString()}**
...
| Metric | Value |
|--------|-------|
| **Repository Files** | ${p.totalFiles} |
| **Repository Folders** | ${p.totalFolders} |
| **Files Analyzed** | ${p.filesAnalyzed} |
...
function renderQa(p) {
  return `# SimpleBeacon Platform — Quality Assurance & Testing
**AI Analyst Report | Generated: ${new Date().toISOString()}**
...
| **Duplicate File Groups** | ${p.duplicateGroups} |
| **Invalid JSON Files** | ${p.invalidJson} |
| **Empty Files** | ${p.emptyFiles} |
...
| Metric | Value |
|--------|-------|
| **Blocking Issues** | ${p.gateBlockingCount} |
| **Warning Issues** | ${p.gateWarningCount} |
