...
function toAiRoadmapReportModel(roadmap) {
  ...
  return {
    ...
    projectOverview: roadmap.projectOverview || {
      ...
      projectHealth:
        roadmap.projectOverview?.projectHealth || roadmap.executiveSummary?.projectHealth || null,
    },
    ...
    roadmapScanMeta: {
      ...
      sprintCompletionRate:
        roadmap.hygieneSummary?.sprintCompletionRate ?? roadmap.executiveSummary?.completionRate,
...
function main() {
  ...
  if (!validation.valid) {
    process.stderr.write(
      'Canonical roadmap failed validation: ' + JSON.stringify(validation.violations) + '\n'
    );
    process.exit(1);
  }
