...
function main() {
    ...
    if (!inputPath) {
        process.stderr.write('Usage: node tools/apply-roadmap-export.cjs <export.json>\n');
        process.exit(1);
    }
    ...
    if (raw.type !== 'dynamic-project-roadmap-analysis') {
        process.stderr.write(`Expected type dynamic-project-roadmap-analysis, got ${raw.type || 'unknown'}\n`);
        process.exit(1);
    }
    ...
    if (sanitized.benchmarkScan) {
        process.stderr.write('Export resolved as benchmark-cache — use a product-scoped ai-platform scan, not github-cache.\n');
        process.exit(1);
    }
    ...
    if (!validation.valid) {
        process.stderr.write('Canonical roadmap failed validation: ' + JSON.stringify(validation.violations) + '\n');
        process.exit(1);
    }

    process.stdout.write('Applied roadmap export:\n');
    process.stdout.write(`  dynamic (full): ${path.relative(ROOT, DYNAMIC_ARCHIVE)}\n`);
    process.stdout.write(`  canonical:      ${path.relative(ROOT, CANONICAL)}\n`);
    process.stdout.write(`  sprints:        ${pageModel.developmentPhases.length} phases, ${pageModel.projectOverview?.completionRate ?? '—'}% complete\n`);
    process.stdout.write(`  profile:        ${sanitized.scanTargetProfile || 'product'}\n`);
}
