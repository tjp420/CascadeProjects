...
const SUBSCRIPTION_STORE = path.join(
  PLATFORM_ROOT,
  ".simplebeacon",
  "subscriptions.json",
);
  ...
  return {
    ...
    sections: {
      ...
      securityHeaders: {
        ...
        grade: "C",
        details:
          "HSTS present, X-Frame-Options OK. Missing: CSP, X-Content-Type-Options",
      },
      seo: {
        pass: true,
        grade: "B",
        details:
          "Title and description present. Missing: canonical URL, incomplete Open Graph",
      },
      mobile: {
        pass: true,
        grade: "A-",
    ...
    summary:
      "6 categories scanned · 4 passed · 2 partial · 0 critical failures",
  };
}
...
function uploadReport(report, licenseToken) {
  ...
  fs.writeFileSync(
    payloadPath,
    JSON.stringify({ reportJson: report, licenseToken }),
  );
  try {
...
function checkServer() {
  try {
    execSync(`curl -s http://127.0.0.1:${PORT}/api/health`, {
...
const TIERS = [
  ...
  },
  {
    ...
    price: "$2,499",
    cliFlags:
      "--full --offline --config .simplebeacon/config-full-coverage.json",
    licenseTier: "executive",
    certLimit: 10,
