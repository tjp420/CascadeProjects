...
const featPkgRaw = gitShow(
  "feature/track18-groundwork:ai-platform/package.json",
);
...
let featPkg = {};
try {
  mainPkg = mainPkgRaw ? JSON.parse(mainPkgRaw) : {};
} catch (e) {
  console.error("Failed parse main package.json", e.message);
  process.exit(2);
}
try {
  featPkg = featPkgRaw ? JSON.parse(featPkgRaw) : {};
} catch (e) {
  console.error("Failed parse feature package.json", e.message);
  process.exit(2);
}
...
merged.devDependencies = mergeObjects(
  mainPkg.devDependencies,
  featPkg.devDependencies,
);
...
merged.keywords = Array.from(
  new Set([].concat(mainPkg.keywords || [], featPkg.keywords || [])),
);
...
const mainSoft = gitShow(
  "origin/main:ai-platform/server/lib/hsm-adapter/softHsmAdapter.cjs",
);
const featSoft = gitShow(
  "feature/track18-groundwork:ai-platform/server/lib/hsm-adapter/softHsmAdapter.cjs",
);
