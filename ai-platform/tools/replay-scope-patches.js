...
const file = require('path').join(__dirname, '../web/simplebeacon-dashboard/js/utils/analyze-mode-file-scope.browser.js');
  ...
  for (const line of fs.readFileSync(jl, 'utf8').split('\n')) {
    ...
    try {
      ...
      for (const c of (o.message?.content || [])) {
        if (c.name !== 'StrReplace' || !c.input?.path?.includes('analyze-mode-file-scope')) continue;
...
console.log('applied', applied, 'missed', missed, 'hasExport', content.includes('export function extractRoadmapFileMetrics'));
