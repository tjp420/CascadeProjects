...
const serverEnv = Object.assign({}, process.env, {
  ...
  STRIPE_WEBHOOK_SECRET: process.env.STRIPE_WEBHOOK_SECRET || 'whsec_test_secret_placeholder_54200',
...
async function run() {
  ...
  try {
    ...
    execFileSync(process.execPath, [path.join(TOOLS_DIR, 'send-custom-stripe-webhook.cjs')], { env: serverEnv, cwd: ROOT, stdio: 'inherit' });
    ...
    const licenseLookup = execFileSync('curl', ['-sS', `http://127.0.0.1:${serverEnv.PORT}/api/simplebeacon/billing/license?email=test-customer@simplebeacon.ai`], { encoding: 'utf8' });
    ...
    const uploadEnv = Object.assign({}, serverEnv, { LICENSE_TOKEN: licenseToken });
    execFileSync(process.execPath, [path.join(TOOLS_DIR, 'upload-test-report.cjs')], { env: uploadEnv, cwd: ROOT, stdio: 'inherit' });
    ...
    const deliveryId = statusResp.lastDeliveryId || statusResp.lastDeliveryId || null;
    if (!deliveryId) throw new Error('no deliveryId found in /api/reports/status response');

    const { REPORT_STORE_DIR } = require(path.join(ROOT, 'src', 'api', 'billing', 'billing-utils.cjs'));
    ...
    const signer = require(path.join(ROOT, 'server', 'lib', 'report-signer.cjs'));
    ...
    if (!valid) throw new Error('Signature verification failed for generated report');
    ...
    if (typeof tampered.qualityScore === 'number') tampered.qualityScore = tampered.qualityScore + 1; else tampered.qualityScore = 9999;
    const tamperedValid = signer.verifyReportSignature(tampered, signingKey);
    if (tamperedValid) throw new Error('Tamper test failed: modified report still verifies');
