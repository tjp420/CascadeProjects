...
} = require("../server/services/local-model-service.cjs");
...
async function main() {
  ...
  process.stdout.write(
    [`   Preferred model: ${modelName || "(none)"}`].join(" ") + "\n",
  );
  ...
  try {
    ...
    if (result.ok) {
      ...
      process.stdout.write([""].join(" ") + "\n");
      process.stdout.write(
        ["🎉 Ollama is ready for optional Phase 2 inference."].join(" ") + "\n",
      );
      process.exit(0);
    } else {
      ...
      process.stdout.write(
        ["   Run: ollama pull " + modelName].join(" ") + "\n",
      );
      process.exit(0); // soft fail — optional Phase 2
    }
  } catch (error) {
    process.stdout.write(
      ["⚠️  Ollama connection failed:", error.message].join(" ") + "\n",
    );
    process.stdout.write([""].join(" ") + "\n");
    process.stdout.write(
      ["ℹ️  Ollama is optional. To enable Phase 2 inference:"].join(" ") + "\n",
    );
    process.stdout.write(
      ["   1. Install Ollama: https://ollama.com"].join(" ") + "\n",
    );
    process.stdout.write(["   2. Start the Ollama service"].join(" ") + "\n");
    process.stdout.write(
      [`   3. Run: ollama pull ${modelName || "<model>"}`].join(" ") + "\n",
    );
    process.stdout.write(
      [
        `   4. Set OLLAMA_BASE_URL in .env.v1-internal (default: ${baseUrl})`,
      ].join(" ") + "\n",
    );
    process.exit(0); // soft fail — optional Phase 2
  }
