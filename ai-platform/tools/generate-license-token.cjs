...
async function main() {
  ...
  if (!tier) {
    tier = await prompt(
      "Tier (executive / instant / euai / universal) [executive]: ",
    );
    tier = tier || "executive";
  }
  ...
  if (!days) {
    const defaultDays =
      { executive: 90, instant: 7, euai: 30, universal: 365 }[tier] || 90;
  ...
  const hasEuAiIndicator = features.some((f) =>
    euAiIndicators.includes(f.toLowerCase()),
  );
