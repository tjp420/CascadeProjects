...
describe("Alertmanager routing (synthetic validation)", () => {
  ...
  test("mesh reconciliation drift alerts route to hsm-crypto-ops-pager", () => {
    const labels = {
      alertname: "MeshReconciliationBoundaryDrift",
      track: "115",
      service: "hsm-vault-mesh",
    };
    const rs = receiversForAlert(routes, labels);
  ...
  test("ShardRepairStallDetected alerts route to hsm-crypto-ops-pager", () => {
    const labels = {
      alertname: "ShardRepairStallDetected",
      component: "hsm-mesh-vault",
      severity: "critical",
    };
    const rs = receiversForAlert(routes, labels);
  ...
  test("ShardReassemblyHighFailureRate alerts route to hsm-crypto-ops-pager", () => {
    const labels = {
      alertname: "ShardReassemblyHighFailureRate",
      component: "hsm-mesh-vault",
      severity: "warning",
    };
    const rs = receiversForAlert(routes, labels);
  ...
  test("Track114LatticeVssReconciliationStall alerts route to hsm-crypto-ops-pager", () => {
    const labels = {
      alertname: "Track114LatticeVssReconciliationStall",
      component: "hsm-mesh-vault",
      severity: "critical",
    };
    const rs = receiversForAlert(routes, labels);
  ...
  test("Track114HighValidationFailureRate alerts route to hsm-crypto-ops-pager", () => {
    const labels = {
      alertname: "Track114HighValidationFailureRate",
      component: "hsm-mesh-vault",
      severity: "warning",
    };
    const rs = receiversForAlert(routes, labels);
