...
function advanceStage(id, newStage, note) {
  if (!STAGES.includes(newStage)) {
    logError(
      `[Pipeline] Invalid stage: ${newStage}. Valid: ${STAGES.join(", ")}`,
    );
    return null;
  }
  const pipeline = loadPipeline();
...
function list(stageFilter) {
  ...
  for (const p of filtered) {
    log(
      `  ${p.name} | ${p.company} | ${p.industry} | ${p.stage} | Last touch: ${p.touchHistory.length > 0 ? p.touchHistory[p.touchHistory.length - 1].at.slice(0, 10) : "never"}`,
    );
  }
  log("");
}
...
if (cmd === "add") {
  ...
  if (!name || !company) {
    logError(
      'Usage: node outreach-prospects.js add "Name" "Company" "Industry" "Title"',
    );
    process.exit(1);
  }
  addProspect({
