# 🎯 FINAL WORKING APPROACH - Agent Zero + Ollama
...
```bash
docker run -d --name agent-zero-ollama-final -p 32781:80 agent0ai/agent-zero:latest
```
...
docker exec -it agent-zero-ollama-final bash
...
# Test the interface
curl -s http://localhost:32781 | head -5
```

### **Step 6: Test Functionality**
1. **Open Browser**: Navigate to `http://localhost:32781`
