...
describe('assessment router', () => {
  ...
  it('health endpoint returns 200 without rate limit', async () => {
    ...
    const healthLayer = router.stack.find((layer) => layer.route?.path === '/health');
    ...
    assert.strictEqual(hasRateLimit, false, 'health should not have rate limit');
  });

  it('scan endpoint has rate limit and validation', async () => {
    const scanLayer = router.stack.find((layer) => layer.route?.path === '/scan');
  ...
  it('report/:id endpoint has rate limit and validation', async () => {
    const reportLayer = router.stack.find((layer) => layer.route?.path === '/report/:id');
