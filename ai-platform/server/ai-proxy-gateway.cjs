...
const {
  ...
  isBlockingFinding,
} = require("./enterprise-patterns");
const logger = require("./lib/app-logger");
const requireProject = require("../shared-utils/index.cjs");
const { scanTextContent } = requireProject(
...
class AIProxyGateway {
  ...
  start() {
    ...
    this.server.listen(this.config.port, () => {
      ...
      logger.info(
        `Blocking mode: ${this.config.blockOnMatch ? "ENABLED" : "MONITOR ONLY"}`,
      );
  ...
  logViolation(scanResult, req, requestUrl) {
    ...
    logger.info(
      `Violation #${this.violationCount}: ${scanResult.patterns.join(", ")}`,
    );
  }
  ...
  sendViolationResponse(res, scanResult) {
    ...
    res.end(
      JSON.stringify({
        ...
        count: scanResult.count,
        message:
          "This request contains sensitive data that violates data protection policies. Remove the sensitive information and try again.",
