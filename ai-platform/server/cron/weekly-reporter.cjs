...
function compileWeeklyReportHTML(orgName, metrics) {
  var dashboardUrl =
    process.env.DASHBOARD_URL || "https://simplebeacon.ai/dashboard";
  ...
  return [
    ...
    "</div>",
    "<p>Hello Team,</p>",
    "<p>Here is your automated engineering quality rollup for <strong>" +
      orgName +
      "</strong> over the past 7 days:</p>",
    ...
    '<div style="font-size:32px;font-weight:700;color:#3b82f6;margin-top:4px;">' +
      metrics.currentGrade +
      "</div>",
    "</div>",
    '<div style="padding:8px;">',
    '<span style="font-size:12px;color:#64748b;text-transform:uppercase;font-weight:600;">Dev Hours Saved</span>',
    '<div style="font-size:32px;font-weight:700;color:#10b981;margin-top:4px;">' +
      metrics.developerHoursSaved +
      " hrs</div>",
    "</div>",
    "</div>",
    '<div style="margin-top:16px;border-top:1px solid #e2e8f0;padding-top:16px;font-size:14px;color:#475569;">',
    "&bull; <strong>Total Scans Conducted:</strong> " +
      metrics.totalScans +
      "<br />",
    "&bull; <strong>Files Analyzed:</strong> " +
      metrics.totalFilesAnalyzed +
      "<br />",
    "&bull; <strong>AI Issues Remediated:</strong> " +
      metrics.totalIssuesRemediated +
      "<br />",
    "&bull; <strong>Average Code Quality Score:</strong> " +
    ...
    '<a href="' +
      dashboardUrl +
      '" style="background-color:#3b82f6;color:white;padding:12px 24px;text-decoration:none;border-radius:6px;font-weight:500;display:inline-block;">Open Analytics Dashboard</a>',
    "</div>",
...
async function executeWeeklyReportingJob(activeSubscriptions) {
  ...
  console.log(
    "[WeeklyReporter] Starting weekly compliance reporting for " +
  ...
  for (var i = 0; i < activeSubscriptions.length; i++) {
    ...
    try {
      ...
      var orgKey =
        subscription.orgKey ||
        resolveOrgKey(subscription.adminEmail, subscription);
      ...
      await emailService.sendEmail({
        ...
        subject:
          "SimpleBeacon Weekly Compliance Report: Grade " +
          metrics.currentGrade +
          " for " +
      ...
      console.error(
        "[WeeklyReporter] Failed for Org " +
          (subscription.orgId || "unknown") +
          ": " +
