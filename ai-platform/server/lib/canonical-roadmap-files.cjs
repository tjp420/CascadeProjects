...
function isDistinctCanonicalRoadmapPair(fileA, fileB) {
  const relA = normalizeRelativePath(
    typeof fileA === "string"
      ? fileA
      : fileA.relativePath || fileA.path || fileA,
  );
  const relB = normalizeRelativePath(
    typeof fileB === "string"
      ? fileB
      : fileB.relativePath || fileB.path || fileB,
  );
  return (
    DISTINCT_CANONICAL_ROADMAP_FILES.has(relA) &&
    DISTINCT_CANONICAL_ROADMAP_FILES.has(relB)
  );
}
