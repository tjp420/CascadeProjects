...
async function progressiveAnalysis(content, context, options = {}) {
  const profile =
    ANALYSIS_PROFILES[options.profile] || ANALYSIS_PROFILES.balanced;
  const results = {
  ...
  try {
    ...
    if (profile.useSemantic) {
      try {
        results.layers.semantic = await analyzeSemanticLayer(
          content,
          context,
          options,
        );
        ...
        logger.warn(
          "[Enhanced Orchestrator] Semantic analysis failed:",
          error.message,
        );
      }
    }
    ...
    if (profile.useContextual && context.projectPath) {
      try {
        ...
        logger.warn(
          "[Enhanced Orchestrator] Contextual analysis failed:",
          error.message,
        );
      }
    }
    ...
    if (optimalModel) {
      try {
        results.layers.ai = await performAIAnalysis(
          content,
          context,
          optimalModel,
          options,
        ...
        logger.warn(
          "[Enhanced Orchestrator] AI analysis failed:",
          error.message,
        );

        // Fallback to deterministic if enabled
        if (profile.fallbackToDeterministic) {
          results.layers.deterministic = await performDeterministicAnalysis(
            results.layers,
          );
...
async function performStaticAnalysis(content, context) {
  ...
  const detection = detector.detectLanguage(
    context.filePath || "snippet.txt",
    content,
  );
...
async function performAIAnalysis(content, context, model, options) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      logger.warn(
        `[Enhanced Orchestrator] Chunk ${i + 1}/${totalChunks} failed: ${error.message}`,
      );
      chunkResults.push({
        chunkIndex: i,
        error: error.message,
  ...
  const avgConfidence =
    chunkResults.length > 0
      ? chunkResults.reduce((sum, r) => sum + (r.confidence || 0), 0) /
        chunkResults.length
      : 0.5;
