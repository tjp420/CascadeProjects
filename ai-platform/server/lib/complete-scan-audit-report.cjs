...
const { assessAuditExportTier, resolveAuditClientName } = require('./audit-export-tier.cjs');
const {
...
const { buildCleanScanRemediationMessage } = require('./audit-report-html-rows.cjs');
...
async function buildCompleteAuditModel(completeScan, options = {}) {
  ...
  const projectPath =
    normalizedScan?.projectPath || simplebeacon?.projectRoot || dataQuality?.projectRoot || '';
  ...
  const model = {
    ...
    summary: {
      ...
      ruleScopedFiles:
        ...
        null,
      codebaseHealth:
        codebase?.summary?.healthScore ?? normalizedScan?.summary?.codebaseHealthScore ?? null,
      ...
      dataQualityFindings:
        dataQuality?.summary?.totalFindings ?? normalizedScan?.summary?.dataQualityFindings ?? null,
...
async function buildCompleteAuditReport(completeScan, options = {}) {
  ...
  const allowAiExecutive =
    options.enhanceExecutive === true && aiProvider && aiProvider !== 'demo' && options.summarizeFn;
