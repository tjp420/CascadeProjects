...
function buildCleanScanRemediationMessage(summary = {}) {
  const parts = [
    "No production-path issues detected under this audit profile.",
  ];
  if (summary.documentationFindings > 0) {
    parts.push(
      `${Number(summary.documentationFindings).toLocaleString()} documentation-tier marker(s) tracked for hygiene (non-blocking).`,
    );
  }
  if (summary.generalFindings > 0) {
    parts.push(
      `${Number(summary.generalFindings).toLocaleString()} tooling/script-tier marker(s) in dev paths (non-blocking).`,
    );
  }
  if (summary.codebaseHealth != null && summary.codebaseHealth < 90) {
    parts.push(
      `Consider addressing hygiene markers to improve code health from ${summary.codebaseHealth}%.`,
    );
  } else if (summary.codebaseHealth != null) {
    parts.push(
      `Code health ${summary.codebaseHealth}% — production paths are clean.`,
    );
  }
  if (summary.codeFilesAnalyzed != null) {
    parts.push(
      `${Number(summary.codeFilesAnalyzed).toLocaleString()} code files deep-scanned with zero runtime-path blockers.`,
    );
  }
  return parts.join(" ");
}
...
function renderCategoryRollupRows(categories) {
  if (!categories.length)
    return '<tr><td colspan="5" class="empty">No codebase categories in bundle.</td></tr>';
  return categories
    .map(
      (c) => `
        <tr>
