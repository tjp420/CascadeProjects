...
function buildSampleAuditReportModel(ENGINE_VERSION) {
    ...
    const remediationRows = [
        {
            ...
            remediation: 'Remove hardcoded pattern; load from environment or secret manager; rotate if ever deployed.'
        },
        {
            severity: 'high',
        ...
        },
        {
            ...
            remediation: 'Route through database/API layer; restrict sample paths to dev profiles.'
        },
        {
            severity: 'medium',
    ...
    return {
        ...
        branch: 'staging',
        engineLabel: 'Simplebeacon Engine v' + ENGINE_VERSION + ' (Zero-Dependency)',
        scanDurationMs: 1840,
        ...
        remediationRows,
        categoryRollup: [
            { category: 'credentials', count: 1, production: 1, high: 1, medium: 0, low: 0 },
            { category: 'production-leak', count: 2, production: 2, high: 2, medium: 0, low: 0 },
            { category: 'debug-artifact', count: 1, production: 1, high: 0, medium: 1, low: 0 }
        ],
        markdown: {
            compliance: [
