...
function registerOperatorDeliverableRoute(app, options = {}) {
  ...
  const bookingsPath =
    ...
    path.join(
      options.dataDir || path.join(projectRoot, "data"),
      "audit-bookings.json",
    );
    ...
    try {
      ...
      if (!booking && body.bookingKey) {
        const rows = await loadBookings({
          dataDir: path.dirname(bookingsPath),
        });
      ...
      if (!booking && body.bookingEmail) {
        ...
        const matches = rows.filter(
          (r) => String(r.contactEmail || "").toLowerCase() === email,
        );
      ...
      const product =
        body.product ||
        (booking ? inferProductFromBooking(booking) : "clearance499");
      ...
      return res
        .status(500)
        .json({ ok: false, error: "workspace_failed", message: err.message });
    }
  });
}
