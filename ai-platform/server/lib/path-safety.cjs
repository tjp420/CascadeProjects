...
function loadConfigAnalyzeRoots(platformRoot) {
  try {
    ...
    return roots
      ...
      .map((entry) =>
        path.isAbsolute(entry) ? path.resolve(entry) : path.resolve(platform, entry)
      );
  } catch {
    return [];
  }
...
function isPathWithinRoots(targetPath, allowedRoots) {
  ...
  const isBareName =
    !path.isAbsolute(targetPath) && !targetPath.includes(path.sep) && !targetPath.includes('/');
  if (isBareName) {
  ...
  if (!fs.existsSync(resolved) && allowedRoots.length) {
    ...
    const monoRoot =
      ...
      allowedRoots.find((root) => {
        const normalized = normalizePathKey(root);
        return (
          !normalized.endsWith('/ai-platform') && normalizedResolved.startsWith(normalized + '/')
        );
      });
    if (monoRoot) {
