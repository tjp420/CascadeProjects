...
function shouldLogPathAccess() {
  return (
    process.env.LOG_ANALYZE_PATH_ACCESS === "true" ||
...
function formatAllowedRootsSummary(allowedRoots, limit = 8) {
  return dedupeResolvedRoots(allowedRoots).slice(0, limit).join("; ");
}
...
function logResolvedAllowedRoots(allowedRoots, context = "startup") {
  ...
  logger.info(
    `[path-safety] ${context}: allowed analysis roots -> ${summary || "(none)"}`,
  );
}
...
function parseAllowedRoots(envValue, fallbackRoots = []) {
  ...
  const roots = [
    ...fallbackRoots.map((entry) => path.resolve(entry)),
    ...fromEnv,
  ];
...
function detectMonorepoRoot(platformRoot) {
  ...
  for (const name of PLATFORM_DIR_NAMES) {
    ...
    if (
      fs.existsSync(path.join(sibling, "simplebeacon-server.js")) ||
  ...
  if (
    fs.existsSync(path.join(resolved, "packages", "simplebeacon-cli")) &&
...
function loadConfigAnalyzeRoots(platformRoot) {
  try {
    ...
    const isWindowsHost =
      process.platform === "win32" || /^win/.test(process.platform);
    return roots
...
function listLocalDriveRoots() {
  ...
  for (let i = 65; i <= 90; i++) {
    ...
    try {
      ...
      console.error("path-safety.cjs error:", e); /* skip inaccessible drives */
    }
  }
  return drives;
}
...
function resolveDefaultAllowedRoots(platformRoot, options = {}) {
  ...
  if (
    ...
    platform.includes("/opt/render/project/src/")
  ) {
    const renderProjectRoot = path.resolve("/opt/render/project/src");
    const platformParent = path.resolve(path.join(platform, ".."));
    if (
      platformParent.startsWith(renderProjectRoot) &&
      fs.existsSync(platformParent)
    ) {
 ...
 * @returns {any}
 */
function assertSafeProjectPath(
  targetPath,
  allowedRoots,
  label = "projectPath",
) {
    ...
    const monoRoot =
      allowedRoots.find((root) => {
        ...
        return (
          ...
          normalizedResolved.startsWith(normalized + "/") &&
  ...
  if (!isPathWithinRoots(resolved, allowedRoots)) {
    // Bare directory name: try resolving against each allowed root
    const isBareName =
      !path.isAbsolute(raw) && !raw.includes(path.sep) && !raw.includes("/");
    if (isBareName) {
