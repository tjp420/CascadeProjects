...
function generateDashboardMetrics(scanRecords = []) {
  ...
  const sortedScans = [...scanRecords].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
  ...
  summary.averageComplianceScore = Math.round(totalScoreSum / scanRecords.length);
  summary.currentGrade = calculateComplianceGrade(summary.averageComplianceScore);

  // 1 remediated item roughly equals 12 minutes of senior dev pipeline cleaning slop manually
  summary.developerHoursSaved = parseFloat(((summary.totalIssuesRemediated * 12) / 60).toFixed(1));
