...
function isExcludedConsolidationPath(filePath) {
  ...
  if (rel.startsWith("node_modules/") || rel.includes("/node_modules/"))
    return true;
  if (rel.startsWith("deliverables/") || rel.includes("/deliverables/"))
    return true;
  if (rel.startsWith("github-cache/") || rel.includes("/github-cache/"))
    return true;
  if (rel.startsWith(".github-sync/") || rel.includes("/.github-sync/"))
    return true;
  // Root npm publish tree mirrors ai-platform/packages/simplebeacon-cli (intentional).
  if (
    /^packages\/simplebeacon-cli\//.test(rel) &&
    !rel.startsWith("ai-platform/")
  )
    return true;
  return false;
}
...
function candidateTouchesExcludedPath(candidate) {
  ...
  const paths = (candidate.files || []).map(
    (file) => file.path || file.relativePath || file.name,
  );
...
function filterAdvancedAnalysis(analysis) {
  ...
  const fuzzyPairs = (analysis.fuzzyNearDuplicates?.pairs || []).filter(
    (pair) =>
      !isExcludedConsolidationPath(pair.fileA) &&
      !isExcludedConsolidationPath(pair.fileB),
  );
  const patternGroups = (
    ...
    (group) =>
      !(group.files || []).every((file) =>
        isExcludedConsolidationPath(file.path),
      ),
  );
  return {
    ...analysis,
...
const SIGNIN_SITE_CANONICAL_DUPE_PAIRS = [
  [
    ...
    "deployments/signin-site/trust-verification.json",
  ],
  [
    "coming-soon/stripe-audit-product.ids.json",
    "deployments/signin-site/stripe-audit-product.ids.json",
  ],
];
...
function isKnownSampleAliasPair(fileA, fileB) {
  ...
  if (
    Object.values(SAMPLE_FILE_OVERRIDES).includes(relA) &&
...
function isEligibleStructurePair(fileA, fileB) {
  ...
  if (isDashboardSample(fileA.name) && isDashboardSample(fileB.name))
    return false;
...
function isKnownCanonicalLinkPair(pathA, pathB) {
  ...
  const sorted = [
    normalizeRelativePath(pathA),
    normalizeRelativePath(pathB),
  ].sort();
...
async function collectSampleDataFiles(baseDir, extraPaths = []) {
  ...
  const scanPaths = resolveMockDataScanPaths(baseDir, [
    ...DEFAULT_EXTRA_PATHS,
    ...extraPaths,
  ]);
  const files = [];
...
function extractJsonStructure(node, depth = 0) {
  ...
  if (Array.isArray(node)) {
    return node.length
      ? ["array", extractJsonStructure(node[0], depth + 1)]
      : ["array", "empty"];
  }
  if (typeof node === "object") {
    return Object.keys(node)
      .sort()
      .map((key) => [key, extractJsonStructure(node[key], depth + 1)]);
  }
  return typeof node;
}
...
function buildExactDuplicateCandidates(groups) {
  ...
  return groups.map((group, index) => {
    ...
    return {
      ...
      effort: "low",
      recommendation:
        "Keep one canonical file; remove or symlink duplicates after review",
...
function buildOversizedOpportunities(files) {
  ...
  return files
    .filter((file) => {
      ...
      if (OVERSIZED_SKIP_PATH_SEGMENTS.some((segment) => rel.includes(segment)))
        return false;
...
async function findStructureSimilarPairs(jsonFiles) {
  ...
  for (let i = 0; i < loaded.length; i++) {
    for (let j = i + 1; j < loaded.length; j++) {
      ...
      if (
        loaded[i].contentHash &&
        loaded[i].contentHash === loaded[j].contentHash
      )
        continue;
      if (loaded[i].signature === loaded[j].signature) continue;
      const similarity = compareStructureSignatures(
        loaded[i].structure,
        loaded[j].structure,
      );
      if (similarity >= STRUCTURE_SIMILARITY_THRESHOLD) {
        pairs.push({
          ...
          savingsLabel: formatBytes(
            Math.min(loaded[i].file.size, loaded[j].file.size),
          ),
          risk: "medium",
          effort: "medium",
          recommendation:
            "Review shared schema fields — consider consolidating into one sample",
...
async function scanFileMergerReduction(baseDir, options = {}) {
  if (typeof baseDir !== "string" || !baseDir) {
    throw new TypeError(
      "scanFileMergerReduction requires a valid baseDir string",
    );
  }
  const resolvedBase = path.resolve(baseDir);
  const scope =
    options.scope === "sample-data-only" ? "sample-data-only" : "repository";
  const includeRepositoryInventory =
    options.includeRepositoryInventory !== false;
  ...
  const totalSizeBytes = repositoryFiles.reduce(
  ...
  const repositoryFilesTotal =
    repositoryInventory?.totalFiles ?? repositoryFiles.length;
  const filesAnalyzed =
    scope === "repository" ? repositoryFilesTotal : sampleDataFilesAnalyzed;

  const duplicateGroups = await filterResolvedDuplicateGroups(
    findDuplicateContentGroups(hashEntries).map((group) =>
      group.map((entry) => {
        const file =
          repositoryFiles.find((f) => f.path === entry.path) ||
  ...
  const fuzzyScopeFiles = (
    scope === "repository" ? repositoryFiles : sampleFiles
  ).filter(
    (file) => !isExcludedConsolidationPath(file.relativePath || file.path),
  );
  ...
  const platformDuplicateGroups = duplicateGroups.filter(
    (group) =>
      !group.every((entry) =>
        isExcludedConsolidationPath(entry.relativePath || entry.path),
      ),
  );
  const benchmarkCacheCandidatesExcluded =
    duplicateGroups.length -
    platformDuplicateGroups.length +
    ((rawAdvancedAnalysis.fuzzyNearDuplicates?.pairs?.length || 0) -
      (advancedAnalysis.fuzzyNearDuplicates?.pairs?.length || 0));

  const exactMergeCandidates = buildExactDuplicateCandidates(
    platformDuplicateGroups,
  );
  ...
  const mergeCandidates = [
    ...exactMergeCandidates,
    ...(await findStructureSimilarPairs(
      sampleFiles.filter((f) => f.ext === ".json"),
    )),
    ...fuzzyCandidates,
  ...
  const reductionOpportunities = [
    ...
    ...platformDuplicateGroups.map((group, index) => ({
      ...
      savingsLabel: formatBytes(
        group.slice(1).reduce((sum, e) => sum + e.size, 0),
      ),
      confidence: 1,
      effort: "low",
      risk: "low",
  ...
  return {
    ...
    repositoryInventory: repositoryInventory
      ? {
          projectRoot:
            path
              .relative(resolvedBase, repositoryInventory.projectRoot)
              .replace(/\\/g, "/") || repositoryInventory.projectRoot,
      ...
      ...(intentionalPairExclusions.monorepoAliasPairsExcluded > 0
        ? {
            monorepoAliasPairsExcluded:
              intentionalPairExclusions.monorepoAliasPairsExcluded,
            intentionalPairsExcluded:
              intentionalPairExclusions.intentionalPairsExcluded,
          }
        : {}),
      structureSimilarPairs: mergeCandidates.filter(
        (c) => c.mergeType === "structure-based",
      ).length,
      fuzzyNearDuplicatePairs: advancedAnalysis.fuzzyNearDuplicates.pairsFound,
      patternConsolidationGroups:
        advancedAnalysis.patternConsolidation.groupsFound,
      oversizedFiles: reductionOpportunities.filter(
        (o) => o.type === "format-optimization",
      ).length,
    },
    mergeCandidates,
    ...
    implementationPhases: [
      ...
      },
      {
        ...
        status: "complete",
        items: [
          "Merge plan preview API",
          "Confirmation + quarantine (no auto-delete)",
          "Audit log + rollback",
        ],
      },
    ],
...
function buildRecommendations(mergeCandidates, reductionOpportunities) {
  ...
  for (const opp of reductionOpportunities
    .filter((o) => o.type === "format-optimization")
    .slice(0, 3)) {
    items.push({
      priority: "medium",
      action: opp.method,
...
function buildPatternRecommendations(advancedAnalysis) {
  ...
  return (advancedAnalysis.patternConsolidation?.recommendations || []).slice(
    0,
    5,
  );
}
...
function buildConsolidationConclusion(report) {
  ...
  const repoFiles =
    s.repositoryFilesTotal ?? report.repositoryInventory?.totalFiles;
  ...
  const repoNote =
    repoFiles != null
      ...
      : jsonScanned != null
        ? ` ${Number(jsonScanned).toLocaleString("en-US")} JSON files hashed for duplicates (${Number(duplicateGroups).toLocaleString("en-US")} duplicate groups).`
        : "";
  if (!candidates) {
