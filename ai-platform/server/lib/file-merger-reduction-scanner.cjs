...
const { hashFileContent, findDuplicateContentGroups } = require('./mock-data-schema-validator.cjs');
...
function filterAdvancedAnalysis(analysis) {
  ...
  const fuzzyPairs = (analysis.fuzzyNearDuplicates?.pairs || []).filter(
    (pair) => !isExcludedConsolidationPath(pair.fileA) && !isExcludedConsolidationPath(pair.fileB)
  );
  const patternGroups = (analysis.patternConsolidation?.recommendations || []).filter(
    (group) => !(group.files || []).every((file) => isExcludedConsolidationPath(file.path))
  );
  return {
    ...analysis,
    fuzzyNearDuplicates: {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      results.push({
        ...
        size: stat.size,
        relativePath: baseDir ? path.relative(baseDir, fullPath).replace(/\\/g, '/') : fullPath,
      });
    } catch {
      /* skip */
    }
...
async function findStructureSimilarPairs(jsonFiles) {
  ...
  return pairs.sort((a, b) => b.similarity - a.similarity).slice(0, MAX_STRUCTURE_PAIRS);
}
...
async function scanFileMergerReduction(baseDir, options = {}) {
  ...
  const repositoryFiles =
    scope === 'repository' ? await collectRepositoryFiles(resolvedBase) : sampleFiles;
  ...
  const fuzzyScopeFiles = (scope === 'repository' ? repositoryFiles : sampleFiles).filter(
  ...
  const platformDuplicateGroups = duplicateGroups.filter(
    (group) =>
      !group.every((entry) => isExcludedConsolidationPath(entry.relativePath || entry.path))
  );
  ...
  const reductionOpportunities = [
    ...
    (opportunity) =>
      !(opportunity.files || []).every((file) => isExcludedConsolidationPath(file.path))
  );
  ...
  return {
    ...
    repositoryInventory: repositoryInventory
      ? {
          projectRoot:
            path.relative(resolvedBase, repositoryInventory.projectRoot).replace(/\\/g, '/') ||
    ...
    summary: {
      ...
      ...(intentionalPairExclusions.monorepoAliasPairsExcluded > 0
        ...
        : {}),
      structureSimilarPairs: mergeCandidates.filter((c) => c.mergeType === 'structure-based')
        .length,
