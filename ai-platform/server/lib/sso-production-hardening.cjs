...
async function fetchJwks(jwksUri) {
  ...
  if (result.status !== 200 || !result.data || !Array.isArray(result.data.keys)) {
...
async function verifyIdTokenSignature(idToken, discovery, clientSecret) {
  ...
  if (header.alg === 'RS256' || header.alg === 'RS384' || header.alg === 'RS512') {
    if (!discovery.jwks_uri) throw new Error('No jwks_uri in discovery document');
...
const PROVIDER_PRESETS = {
  okta: {
    ...
    label: 'Okta',
    method: 'oidc',
    description: 'Okta Workforce Identity — OIDC authorization code flow with PKCE',
    fields: {
      issuer: { label: 'Okta Domain', placeholder: 'https://your-org.okta.com', required: true },
      ...
      scope: { label: 'Scopes', placeholder: 'openid profile email groups', required: false },
    },
    buildIssuer: (domain) => domain.replace(/\/$/, ''),
    discoveryPath: '/.well-known/openid-configuration',
    notes: 'Ensure your Okta app has "Authorization Code" grant type enabled and PKCE required.',
  },
...
function validateSamlSignature(xml, certPem) {
  if (!certPem) {
    return { valid: false, reason: 'No IdP certificate provided for signature validation' };
  }
  ...
  if (!hasSignature) {
    return { valid: false, reason: 'SAML response is not signed — signature validation required' };
  }
  ...
  try {
    const verifier = crypto.createVerify(sigAlgorithm.includes('sha') ? sigAlgorithm : 'sha256');
...
async function validateIdTokenProduction(idToken, config, discovery, expectedNonce, clientSecret) {
  ...
  if (payload.iss !== expectedIssuer) {
    ...
    if (!(payload.iss === expectedIssuer + '/' || payload.iss + '/' === expectedIssuer)) {
      throw new Error(`ID token issuer mismatch: ${payload.iss} != ${expectedIssuer}`);
    }
  }
  ...
  if (config.providerType === 'azure_ad' && config.oidc.tenantId) {
    if (payload.tid && payload.tid !== config.oidc.tenantId) {
      throw new Error(`Azure AD tenant mismatch: ${payload.tid} != ${config.oidc.tenantId}`);
    }
  }

  logger.info(`[SSO] ID token verified: alg=${algorithm}, sub=${payload.sub || 'N/A'}`);
  return payload;
}
