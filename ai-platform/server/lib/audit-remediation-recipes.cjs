...
function inferAutoFixConfidence(kind, snippet, rule) {
  ...
  if (/assigned a value but never used|no-unused-vars|unused import/i.test(text + ruleText))
    return 'high';
...
function buildFixSpec(row = {}, options = {}) {
  ...
  return {
    ...
    businessImpact:
      mergedRow.businessImpact || BUSINESS_IMPACT_BY_KIND[kind] || BUSINESS_IMPACT_BY_KIND.general,
    changes,
    rotationSteps,
