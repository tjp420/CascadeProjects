...
function inferAutoFixConfidence(kind, snippet, rule) {
  ...
  if (
    /assigned a value but never used|no-unused-vars|unused import/i.test(
      text + ruleText,
    )
  )
    return "high";
...
function inferBlocksGate(kind, severity, row = {}) {
  ...
  const file = normalizePathKey(
    parseLocation(row.location).file || row.filePath || "",
  );
  ...
  if (
    kind === "env-secret" &&
    severity === "high" &&
    /\.env\.production/.test(file)
  ) {
    return true;
  }
  if (
    GATE_BLOCKING_KINDS.has(kind) &&
...
function buildStructuredChanges(
  ...
  if (new RegExp("con" + "sole\\s*\\.\\s*log").test(lower)) {
    ...
    changes.push({
      type: "manual",
      instruction:
        "Use your project logger (or add one under lib/logger.js). Never log tokens, API keys, or session identifiers.",
    });
    return changes;
  }
  ...
  if (/sk_(live|test)_|rk_(live|test)_|akia[0-9a-z]{4}|whsec_/i.test(text)) {
    ...
    changes.push({
      type: "manual",
      instruction:
        "Rotate the exposed secret in the vendor console before merging.",
    });
    return changes;
  }
  ...
  if (
    /assigned a value but never used|no-unused-vars/i.test(
      text + String(rule || ""),
    )
  ) {
    const unusedMatch = String(snippet || "").match(
      /'([^']+)'\s+is assigned a value but never used/i,
    );
    ...
    changes.push({
      ...
      after:
        `Prefix with _ (e.g. _${varName}) or delete line ${parsed.line || ""}`.trim(),
    });
    changes.push({
      type: "manual",
  ...
  if (kind === "env-secret") {
    changes.push({
      ...
      after:
        "STRIPE_SECRET_KEY=<set-on-deploy-host-only — never commit live keys>",
    });
    changes.push({
      type: "manual",
      instruction:
        "Rotate the key in Stripe Dashboard before production deploy.",
    });
    return changes;
  }
  ...
  if (kind === "config-sprawl") {
    changes.push({
      type: "manual",
      instruction:
        "Consolidate to .env.example + .env.v1-internal + .env.production.example; keep live secrets on the deploy host only.",
    });
    return changes;
  }
  ...
  if (kind === "pii") {
    ...
    changes.push({
      type: "manual",
      instruction:
        "Replace realistic PII in docs/samples with obviously synthetic tokens.",
    });
    return changes;
  }
  ...
  if (kind === "orphaned-data") {
    changes.push({
      type: "manual",
      instruction:
        "Archive the file or add a documented consumer — data-lineage reported consumerCount 0.",
    });
    return changes;
  }
  ...
  if (kind === "file-reduction") {
    changes.push({
      type: "manual",
      instruction:
        "Regenerable artifacts only (node_modules, coverage). Run npm install after removing node_modules. Do not bulk-delete unused-file candidates without import verification.",
    });
    return changes;
  }
  ...
  if (/import pytest|from pytest/.test(lower)) {
    ...
    changes.push({
      type: "manual",
      instruction:
        "Move pytest usage to tests/ and add pytest to devDependencies.",
    });
    return changes;
  }
  ...
  if (/sample\.json|mock\/|demo-metrics/.test(lower)) {
    changes.push({
      type: "manual",
      instruction:
        "Replace static sample import with a runtime API call or env-based config loader.",
    });
    changes.push({
      type: "manual",
      instruction:
        "Move fixtures to tests/ or fixtures/ excluded from production bundles.",
    });
    return changes;
  }
  ...
  if (/completion_rate|success_rate|98\.5|99\.9/.test(lower)) {
    changes.push({
      type: "manual",
      instruction:
        "Remove fiction KPI literal and bind to `.simplebeacon/baseline.json` or a live reporting API.",
    });
    changes.push({
      type: "manual",
      instruction:
  ...
  if (/[`]{3}(?:json|javascript|typescript|python)/i.test(text)) {
    ...
    changes.push({
      type: "manual",
      instruction:
        "Extract only the code body from LLM output — do not paste markdown fences into source.",
    });
    return changes;
  }
...
function recipeFromFixSpec(fixSpec) {
  ...
  for (const change of fixSpec.changes) {
    if (change.type === "replace-line" && change.before && change.after) {
      ...
      parts.push(
        `Remove line${change.line ? ` ${change.line}` : ""}: \`${change.before}\``,
      );
    } else if (change.type === "add-env" && change.key) {
      parts.push(
        `Add to \`.env.example\`: \`${change.key}=${change.example || "<value>"}\``,
      );
  ...
  return (
    parts.join("\n\n") ||
    DEFAULT_RECIPES[fixSpec.kind] ||
    DEFAULT_RECIPES.general
  );
}
...
function buildFixSpec(row = {}, options = {}) {
  ...
  const mergedRow = artifact
    ? { ...row, kind, severity, ...artifact }
    : { ...row, kind, severity };
  ...
  return {
    ...
    autoFixConfidence: inferAutoFixConfidence(
      kind,
      mergedRow.snippet,
      mergedRow.rule,
    ),
    estimatedMinutes:
      ESTIMATED_MINUTES_BY_KIND[kind] || ESTIMATED_MINUTES_BY_KIND.general,
    businessImpact:
      mergedRow.businessImpact ||
...
function enrichRemediationRow(row = {}, options = {}) {
  ...
  const severity = String(row.severity || "medium").toLowerCase();
  const fixSpec =
    row.fixSpec || buildFixSpec({ ...row, kind, severity }, options);
  const recipe =
    row.recipe ||
    recipeFromFixSpec(fixSpec) ||
    buildCodeRecipe(kind, row.snippet, row.rule, row.remediation);
...
function sortFixRows(rows = []) {
  return [...rows].sort((a, b) => {
    const gateDelta =
      Number(b.fixSpec?.blocksGate) - Number(a.fixSpec?.blocksGate);
    if (gateDelta !== 0) return gateDelta;
    const left =
      SEVERITY_ORDER[String(a.severity || "medium").toLowerCase()] ?? 9;
    const right =
      SEVERITY_ORDER[String(b.severity || "medium").toLowerCase()] ?? 9;
    return left - right;
  });
}
...
function buildSortedRemediationRows(scanPayload = {}, options = {}) {
  const { issues, codebaseFindings, dataQualityFindings } =
    extractFixInputsFromScan(scanPayload);
  const scopedIssues = filterFindingsByProjectScope(issues, options);
  const scopedCodebaseFindings = filterFindingsByProjectScope(
    codebaseFindings,
    options,
  );
  const scopedDataQualityFindings = filterFindingsByProjectScope(
    dataQualityFindings,
    options,
  );
  const rows = [];
  ...
  for (const issue of scopedIssues) {
    rows.push(
      enrichRemediationRow(
        normalizeScanFinding(issue, "Simplebeacon gate"),
        options,
      ),
    );
  }
  ...
  for (const finding of scopedDataQualityFindings) {
    rows.push(
      enrichRemediationRow(normalizeDataQualityFinding(finding), options),
    );
  }
...
function buildRemediationRowsFromScan(scanPayload = {}, options = {}) {
  ...
  return {
    ...
    summary: {
      gateBlockingCount: sortedRows.filter((row) => row.fixSpec?.blocksGate)
        .length,
...
function buildFixPlanFromScan(scanPayload = {}, options = {}) {
  ...
  return {
    ...
    projectPath:
      options.projectPath ||
      scanPayload.projectRoot ||
      scanPayload.projectPath ||
      null,
    gatePass,
    fixCount: sortedRows.length,
    summary: {
...
module.exports = {
  ...
  shouldScopeFindingsToProject,
  inferArtifactContext,
  isHandoffArtifactPath: require("./audit-remediation-recipes/paths.cjs")
    .isHandoffArtifactPath,
  isIntentionalFixturePath: require("./audit-remediation-recipes/paths.cjs")
    .isIntentionalFixturePath,
  isDocumentationPath: require("./audit-remediation-recipes/paths.cjs")
    .isDocumentationPath,
  IMPACT_BY_KIND: require("./audit-remediation-recipes/data.cjs")
    .IMPACT_BY_KIND,
  DEFAULT_RECIPES: require("./audit-remediation-recipes/data.cjs")
    .DEFAULT_RECIPES,
  ESTIMATED_MINUTES_BY_KIND: require("./audit-remediation-recipes/data.cjs")
    .ESTIMATED_MINUTES_BY_KIND,
  BUSINESS_IMPACT_BY_KIND: require("./audit-remediation-recipes/data.cjs")
    .BUSINESS_IMPACT_BY_KIND,
};
