...
function buildExecutiveDashboardBanner(model) {
  ...
  const scopeNote =
    '<p class="meta" style="margin-top:10px">Gate attestation not included in this export — run Simplebeacon gate or Complete scan, or attach a gate PDF separately.</p>';
  ...
  if (tier.tier === "codebase-only") {
    ...
    const files =
      s.codeFilesAnalyzed != null
        ? Number(s.codeFilesAnalyzed).toLocaleString()
        : "—";
...
function buildExecutiveKpiStrip(model) {
  ...
  const pushKpi = (value, label) => {
    kpis.push(
      `<div class="kpi"><strong>${escapeHtml(String(value))}</strong><span>${escapeHtml(label)}</span></div>`,
    );
  };
  ...
  if (tier === "handoff" || tier.tier === "gate-only") {
    ...
    switch (stepKey) {
      ...
      case "file-reduction":
        ...
        pushKpi(
          s.fileReductionReclaimableBytes != null
            ? `${Math.round(s.fileReductionReclaimableBytes / 1024 / 1024)} MB`
            : "—",
          "Reclaimable",
        );
        pushKpi(s.repositoryFiles ?? "—", "Repo files");
      ...
      case "roadmap":
        ...
        pushKpi(
          s.roadmapCompletion != null ? `${s.roadmapCompletion}%` : "—",
          "Complete",
        );
...
function buildCoverPresentation(model) {
  ...
  const readinessScore =
    readiness && typeof readiness.score === "number"
  ...
  if (tier.tier === "handoff") {
    ...
    subtitle =
      `Supplementary scan deliverable — not a standalone pre-launch security handoff. ${tier.handoffHint || ""}`.trim();
  }
  ...
  const supplementaryCallout =
    tier.tier !== "handoff" && tier.missingForHandoff?.length
