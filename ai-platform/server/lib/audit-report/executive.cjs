...
function buildExecutivePriorities(summary) {
  ...
  if (summary.productionFindings === 0) {
    const filesLabel =
      summary.codeFilesAnalyzed != null ? summary.codeFilesAnalyzed.toLocaleString() : 'all scoped';
    return [
...
function buildLaunchReadiness(model) {
  ...
  if (s.gatePass === true) {
    const docNote =
      s.documentationFindings > 0 ? ' — documentation markers tracked separately' : '';
    return {
...
function buildDeterministicExecutive(model) {
  ...
  const sev = s.severityCounts || { critical: 0, high: 0, medium: 0, low: 0 };
  const gate =
    s.gatePass === true ? 'PASS' : s.gatePass === false ? 'REVIEW REQUIRED' : 'NOT EVALUATED';
