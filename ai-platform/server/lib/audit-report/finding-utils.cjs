...
const { redactSnippet, truncateForDisplay } = require('../audit-report-utils.cjs');
...
function normalizeFindingDescription(finding) {
  ...
  if (
    /\b(python\s+todo|todo\s+comment|todo\s+marker|fixme\s+marker)\b/.test(raw) ||
    /\btodo\b/.test(raw)
  ) {
    return 'todo-marker';
  }
...
function scoreFinding(finding) {
  ...
  if (finding.category === 'debug-artifact' && isAuditProductionRuntimePath(finding.filePath))
    score += 20;
    ...
    rows.push(
      enrichRemediationRow({
        ...
        remediation: truncateForDisplay(
          finding.recommendedAction || 'Review and remediate before client handoff.',
          160
        ),
...
function normalizeSimplebeaconForCompliance(simplebeacon, summary = {}) {
  ...
  const credentialScanned =
    simplebeacon.credentialScanned ?? simplebeacon.scanScope?.credentialScanned ?? ruleScoped ?? 0;
