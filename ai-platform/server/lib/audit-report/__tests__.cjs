...
describe('finding-utils.cjs', () => {
  ...
  it('isAuditProductionRuntimePath excludes scripts and reporters', () => {
    ...
    assert.strictEqual(isAuditProductionRuntimePath('server/scripts/deploy.sh'), false);
    assert.strictEqual(isAuditProductionRuntimePath('packages/simplebeacon-cli/src/scan.js'), true);
    assert.strictEqual(isAuditProductionRuntimePath('packages/simplebeacon-cli/bin/cli.js'), false);
  });
  ...
  it('enrichFindings adds tier and priority', () => {
    const findings = [{ severity: 'high', filePath: 'server/lib/db.cjs', category: 'broken' }];
  ...
  it('countBySeverity aggregates correctly', () => {
    const findings = [{ severity: 'high' }, { severity: 'high' }, { severity: 'low' }];
  ...
  it('isPlaceholderExecutiveText detects placeholders', () => {
    ...
    assert.strictEqual(isPlaceholderExecutiveText('Real executive summary here'), false);
  });
...
describe('executive.cjs', () => {
  it('calculateAuditConfidence penalizes missing gate scan', () => {
    const score = calculateAuditConfidence({ ruleScopedFiles: 0, gatePass: null }, {});
    assert.ok(score < 100);
  });

  it('calculateAuditConfidence rewards high file counts', () => {
    const score = calculateAuditConfidence({ codeFilesAnalyzed: 9999, gatePass: true }, {});
    assert.ok(score > 80);
  });
...
describe('html-sections.cjs', () => {
  ...
  it('buildCoverPresentation returns cover metadata', () => {
    const model = {
      ...
      exportTier: { tier: 'handoff', showReadinessScore: true, showSignOffBlock: true },
      readiness: { score: 90 },
      client: 'Acme Corp',
      company: 'Acme Corp',
      reportId: 'R-123',
