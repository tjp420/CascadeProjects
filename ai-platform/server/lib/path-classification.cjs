...
function isDuplicateStagingPath(relativePath, groupPaths) {
  ...
  if (
    ...
    groupPaths.some((p) => normalizedAuditPath(p).endsWith(`/routers/${rel.split('/').pop()}`))
  ) {
    return true;
  }
  if (!rel.includes('/') && groupPaths.some((p) => normalizedAuditPath(p) === `ai-platform/${rel}`))
    return true;
  if (
...
function isIntentionalCliPublishBasenameGroup(basename, groupPaths) {
  ...
  return (
    normalized.length >= 2 &&
    normalized.every((p) => /^packages\/simplebeacon-cli\/(?:scripts\/)?publish\.ps1$/i.test(p))
  );
}
