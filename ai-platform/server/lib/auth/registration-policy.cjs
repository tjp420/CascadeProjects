...
function isPublicRegistrationAllowed() {
  return (
    String(
      process.env.SIMPLEBEACON_ALLOW_PUBLIC_REGISTRATION || "",
    ).toLowerCase() === "true"
  );
}
...
function registrationRequiresApproval() {
  return (
    String(
      process.env.SIMPLEBEACON_REGISTRATION_AUTO_ACTIVATE || "",
    ).toLowerCase() !== "true"
  );
}
...
function validateRegistrationPayload(body = {}) {
  ...
  const confirmProvided =
    body.confirmPassword !== undefined || body.confirm_password !== undefined;
