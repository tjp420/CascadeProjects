...
async function handleRegister(req, res) {
  if (!isPublicRegistrationAllowed()) {
    return res.status(403).json({
      ...
      message:
        "Self-serve account creation is disabled. Contact support@simplebeacon.ai for access.",
    });
  }
  ...
  if (result.error) {
    const status =
      result.error.includes("exists") || result.error.includes("taken")
        ? 409
        : 400;
    return res
      .status(status)
  ...
  try {
    ...
    logger.warn("Trial grant failed during registration", {
  ...
  if (pending) {
    return res.status(201).json({
      ...
      pending: true,
      message:
        "Account submitted. An operator will review and activate your access before you can sign in.",
      user: {
        id: result.user.id,
