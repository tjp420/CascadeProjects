...
const GZDoom_API_PATTERNS = [
  {
    id: "a_action",
    pattern: /\bA_[A-Za-z0-9_]+\s*\(/g,
    label: "Actor action function",
  },
  { id: "cvar", pattern: /\bCVar\.FindCVar\s*\(/g, label: "CVAR lookup" },
  {
    id: "dynamic_light",
    pattern: /\bDynamicLight\b|\bA_Light\b/g,
    label: "Dynamic light API",
  },
  {
    id: "event_handler",
    pattern: /\bEventHandler\b|\bWorldTick\b|\bOnRegister\b/g,
    label: "Event handler lifecycle",
  },
  {
    id: "console",
    pattern: /\bConsole\.(Command|Print)\b/g,
    label: "Console output/command",
  },
];
...
class ZScriptParser extends LanguageParser {
  ...
  parse(code, context = {}) {
    ...
    return {
      ...
      states,
      gzdoomAPI,
      summary: this.generateZScriptSummary(
        classes,
        states,
        gzdoomAPI,
        context.filePath,
  ...
  extractClasses(content) {
    const classes = [];
    const re =
      /\bclass\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?::\s*([A-Za-z_][A-Za-z0-9_.]*))?\s*\{/g;
    let match;
    while ((match = re.exec(content)) !== null) {
      const snippet = content.slice(
        match.index,
        match.index + constants.TIMEOUT_5S,
      );
      classes.push({
        name: match[1],
  ...
  extractStates(content) {
    ...
    while ((match = re.exec(content)) !== null) {
      const block = match[1];
      const labels = [...block.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\s*:/g)].map(
        (m) => m[1],
      );
      states.push({
  ...
  extractProperties(snippet) {
    const props = [];
    const re =
      /\b(?:private|protected|public)?\s*(?:bool|int|float|double|string|color|Vector2|Vector3|Array<[^>]+>)\s+([A-Za-z_][A-Za-z0-9_]*)\s*;/g;
    let match;
    while ((match = re.exec(snippet)) !== null) {
  ...
  generateZScriptSummary(classes, states, gzdoomAPI, filePath = "") {
    ...
    if (
      rel.includes("/lights/") ||
      classes.some((c) => /Light/i.test(c.name))
    ) {
    ...
    const complexity =
      classes.length > 3 || states.length > 2
        ? "high"
        : classes.length > 1 || states.length > 0
          ? "medium"
          : "low";
    ...
    if (classes.some((c) => c.hasStatesBlock && !c.methods.length)) {
      recommendations.push(
        "State-only classes — verify state chains reference valid sprites",
      );
    }
    if (
      gzdoomAPI.some((a) => a.id === "cvar") &&
      !gzdoomAPI.some((a) => a.id === "dynamic_light")
    ) {
      recommendations.push(
        "CVAR reads without light API in same file — intensity may not reach renderer",
      );
    }
    ...
    return {
      purpose,
      complexity,
      gzdoomFeatures: gzdoomAPI.map((g) => g.id),
      recommendations,
    };
  }
}
