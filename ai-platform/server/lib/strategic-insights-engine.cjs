...
function aggregateRoadmapMetrics(roadmap = {}) {
  ...
  const metrics =
    roadmap.codeAnalysis?.metrics || roadmap.progressMetrics?.metrics || {};
  const aiIntegration =
    roadmap.codeAnalysis?.aiIntegration || roadmap.aiIntegration || {};
  const summary = roadmap.executiveSummary || roadmap.projectOverview || {};
  const phases =
    roadmap.developmentPhases || roadmap.phases || roadmap.sprintPhases || [];
  ...
  return {
    ...
    completionRate: num(
      summary.completionRate ?? roadmap.projectOverview?.completionRate,
    ),
    testCoverage: num(metrics.testCoverage ?? metrics.lineCoverage),
    lineCoverage: num(metrics.lineCoverage),
    featureCompleteness: num(
      metrics.featureCompleteness ?? summary.completionRate,
    ),
    phaseCount: phases.length,
    totalFeatures: num(summary.totalFeatures),
    completedFeatures: num(summary.completedFeatures),
    projectHealth:
      summary.projectHealth || roadmap.projectOverview?.projectHealth || null,
...
function scorePerformance(metrics) {
  if (metrics.totalFiles != null && metrics.totalFiles > constants.TIMEOUT_30S)
    return "MODERATE";
  if (metrics.totalFiles != null && metrics.totalFiles > constants.TIMEOUT_15S)
    return "LOW";
  return "MINIMAL";
}
...
function buildDeterministicStrategicInsights(metrics) {
  ...
  const filesLabel =
    metrics.totalFiles != null ? metrics.totalFiles.toLocaleString() : "—";
...
function buildStrategicMetricsPayload(metrics, _projectPath) {
  return {
    ...
    detectedIssues: (metrics.immediateActions || [])
      .slice(0, 5)
      .map((action) => ({
        type: "roadmap-action",
        severity: "medium",
...
function sanitizeLlmStrategicSummary(summary, deterministic, metrics) {
  ...
  text = text.replace(
    /(\*{0,2}risk\s*level\*{0,2}\s*:\s*)(\*{0,2})(?:low|moderate|high)(?:\s*-\s*(?:low|moderate|high))?(\*{0,2})/gi,
    (_m, prefix, openStars, closeStars) =>
      `${prefix}${openStars}${riskHuman}${closeStars}`,
  );
  text = text.replace(
    /(\*{1,2}risk\s*level:\*{1,2}\s*)(?:low|moderate|high)(?:\s*-\s*(?:low|moderate|high))?/gi,
    (_m, prefix) => `${prefix}${riskHuman}`,
  );
  text = text.replace(
    /(risk\s*level\s*(?:\*{0,2}\s*:\s*\*{0,2}|\s+is\s+|:\s*))((?:low|moderate|high)(?:\s*-\s*(?:low|moderate|high))?)/gi,
    (_m, prefix) => `${prefix}${riskHuman}`,
  );
  text = text.replace(
    /(risk\s+if\s+not\s+addressed\s+promptly\s+poses\s+a\s+)(low|moderate|high)\s+risk/gi,
    (_m, prefix) => `${prefix}${riskHuman} risk`,
  );
  text = text.replace(
    /\bposes\s+a\s+(low|moderate|high)\s+risk\b/gi,
    `poses a ${riskHuman} risk`,
  );
  text = text.replace(/(?:^|\n)Risk Level\s*:\s*[A-Za-z-]+/gi, "");
  text = `${text.trim()}\n\nRisk Level: ${riskLabel}`;

  // Normalize common prose variants so LLM narrative cannot contradict deterministic risk.
  text = text.replace(
    /(overall\s+risk(?:\s+level)?\s+(?:is|:\s*)\s*)(low|moderate|high)(?:\s*-\s*(low|moderate|high))?/gi,
    (_m, prefix) => `${prefix}${riskHuman}`,
  );
  text = text.replace(
    /(risk\s+level\s+(?:is|:\s*)\s*)(low|moderate|high)(?:\s*-\s*(low|moderate|high))?/gi,
    (_m, prefix) => `${prefix}${riskHuman}`,
  );
  text = text.replace(
    /\b(?:low|moderate|high)\s*-\s*(?:low|moderate|high)\s+risk\b/gi,
    `${riskHuman} risk`,
  );
...
async function analyzeStrategicInsights(options = {}) {
  ...
  if (aiProvider === "demo" || aiProvider === "active") {
    deterministic.llmNote =
      "Select a local or cloud provider for the strategic analysis layer.";
    return deterministic;
  }
  ...
  try {
    ...
    if (enhanced.enhanced && enhanced.summary) {
      ...
      return {
        ...
        llmDisclaimer:
          "LLM narrative is advisory; deterministic metrics are source of truth.",
      };
    }
  } catch (err) {
