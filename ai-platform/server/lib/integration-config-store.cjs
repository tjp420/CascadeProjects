...
function maskConfig(config) {
  ...
  if (type) {
    for (const field of type.secretFields) {
      if (maskedConfig[field]) maskedConfig[field] = maskSecret(maskedConfig[field]);
    }
  }
...
function createConfig(params) {
  ...
  writeStore(store);
  logger.info(`[Integrations] Created ${params.type} config ${configId} for org ${params.orgId}`);
