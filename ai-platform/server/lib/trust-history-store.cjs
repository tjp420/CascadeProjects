...
function appendTrustSnapshot({
  ...
  const dedupeKey =
    nextEntry.verificationId ||
    `${nextEntry.generatedAt}|${nextEntry.issues}|${nextEntry.qualityScore}`;
  const filtered = existing.filter((entry) => {
    const key =
      entry.verificationId ||
      `${entry.generatedAt}|${entry.issues}|${entry.qualityScore}`;
...
function buildTrustTrend(entries, windowSize = 30) {
  ...
  const scored = scoped.filter((entry) =>
    Number.isFinite(numeric(entry.qualityScore)),
  );
  const issueEntries = scoped.filter((entry) =>
    Number.isFinite(numeric(entry.issues)),
  );
  ...
  const avgQuality = scored.length
    ? Number(
        (
          scored.reduce((sum, entry) => sum + numeric(entry.qualityScore), 0) /
          scored.length
        ).toFixed(2),
      )
    : null;
  const avgIssues = issueEntries.length
    ? Number(
        (
          issueEntries.reduce((sum, entry) => sum + numeric(entry.issues), 0) /
          issueEntries.length
        ).toFixed(2),
      )
    : null;
  ...
  return {
    ...
    passRatePercent: scoped.length
      ? Number(((passCount / scoped.length) * 100).toFixed(1))
      : null,
    ...
    qualityDelta:
      ...
      Number.isFinite(numeric(oldest.qualityScore))
        ? Number(
            (
              numeric(newest.qualityScore) - numeric(oldest.qualityScore)
            ).toFixed(2),
          )
        : null,
    latest: newest
