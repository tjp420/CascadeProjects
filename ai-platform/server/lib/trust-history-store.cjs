...
function appendTrustSnapshot({
  ...
  const filtered = existing.filter((entry) => {
    const key =
      entry.verificationId || `${entry.generatedAt}|${entry.issues}|${entry.qualityScore}`;
...
function buildTrustTrend(entries, windowSize = 30) {
  ...
  const avgQuality = scored.length
    ? Number(
        (
          scored.reduce((sum, entry) => sum + numeric(entry.qualityScore), 0) / scored.length
        ).toFixed(2)
      )
    : null;
  const avgIssues = issueEntries.length
    ? Number(
        (
          issueEntries.reduce((sum, entry) => sum + numeric(entry.issues), 0) / issueEntries.length
        ).toFixed(2)
      )
    : null;
  const issueDelta =
