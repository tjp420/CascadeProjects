const { generateAndStore, getStoredProof, listProofsByPolicy, clearStore } = require('../index.cjs');

describe('ProofStore integration', () => {
  beforeEach(() => clearStore());

  test('generateAndStore persists a proof and returns record', () => {
    const { record, proof } = generateAndStore({ policyId: 'policy-0001', publicInputs: { x: 1 }, scheme: 'mock' });
