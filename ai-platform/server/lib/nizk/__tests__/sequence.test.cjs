const { clearStore, generateAndStore, getStoredProof, listProofsByPolicy } = require('../index.cjs');
...
describe('NIZK monotonic sequencing', () => {
  ...
  test('generateAndStore assigns consecutive sequence numbers', () => {
    const p1 = generateAndStore({ policyId: 'p1', publicInputs: { policyId: 'p1', value: 1 } });
    const p2 = generateAndStore({ policyId: 'p1', publicInputs: { policyId: 'p1', value: 2 } });
  ...
  test('saving an imported proof with too-small sequence is rejected', () => {
    ...
    expect(() => {
      store.saveProof({ policyId: 'x', proof_bundle: 'b64', meta: { sequence: 1 } });
    }).toThrow(/sequence-too-small/);

    // but allow if force=true
    expect(() => {
      store.saveProof({ policyId: 'x', proof_bundle: 'b64', meta: { sequence: 1 }, force: true });
    }).not.toThrow();
  });
});
