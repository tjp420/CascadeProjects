...
describe("NIZK routes (contract tests)", () => {
  ...
  test("POST /nizk/verify returns valid for stored proof", async () => {
    const gen = await request(app)
      .post("/nizk/generate")
      .send({ policy_id: "p", public_inputs: { x: 1 }, scheme: "mock" });
    const proof = gen.body.proof.proof_bundle;
    const v = await request(app)
      .post("/nizk/verify")
      .send({ policy_id: "p", public_inputs: { x: 1 }, proof_bundle: proof })
      .expect(200);
  ...
  test("GET /nizk/proofs/:id returns stored record", async () => {
    const gen = await request(app)
      .post("/nizk/generate")
      .send({ policy_id: "p2", public_inputs: {}, scheme: "mock" });
  ...
  test("POST /nizk/verify rejects bad proof", async () => {
    const v = await request(app)
      ...
      .send({
        policy_id: "p",
        public_inputs: {},
        proof_bundle: Buffer.from("bad").toString("base64"),
      })
      .expect(200);
