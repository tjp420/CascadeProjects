...
function generateProof({publicInputs = {}, secretInputs = {}, scheme = 'mock-scheme', version = '1.0.0'} = {}){
  // deterministic mock proof bundle (includes canonicalized publicInputs)
  const payload = JSON.stringify(publicInputs);
  const proof = Buffer.from(`mock-proof:${scheme}:${payload}`).toString('base64');
...
function verifyProof({publicInputs = {}, proof_bundle, meta = {}}){
  try{
    ...
    if(meta && meta.provenanceHash){
      const expected = sha256HexFromObject({ policyId: publicInputs.policyId || null, proof_bundle, meta: { scheme: meta.scheme, version: meta.version, engineTimestamp: meta.engineTimestamp } });
      if(expected !== meta.provenanceHash){
        return { is_valid: false, error_context: 'provenance-hash-mismatch' };
      }
    }

    return { is_valid: okPrefix, error_context: okPrefix ? null : 'invalid-mock-proof' };
  }catch(e){
