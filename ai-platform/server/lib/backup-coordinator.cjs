...
class BackupCoordinator {
  ...
  constructor(opts) {
    if (!opts || !Buffer.isBuffer(opts.kek) || opts.kek.length !== KEK_LENGTH) {
      throw new Error(
        "INVALID_KEK: backup-coordinator: KEK must be a 32-byte Buffer",
      );
    }
    this.kek = opts.kek;
    ...
    return Buffer.from(
      JSON.stringify({
        ...
        auditLog: Buffer.isBuffer(bundle.auditLog)
          ...
          : Buffer.from(JSON.stringify(bundle.auditLog), "utf8").toString(
              "base64",
            ),
        resumptionTickets,
  ...
  async backup(bundle) {
    ...
    if (invalid) {
      const err = new Error(
        "INVALID_BUNDLE: backup-coordinator: state bundle is invalid",
      );
      err.code = invalid;
      throw err;
    }
  ...
  verifyArchive(archiveBuffer, archiveId) {
    if (
      ...
      archiveBuffer.length < HEADER_LENGTH + TAG_LENGTH
    ) {
    ...
    const archiveKey = BackupCoordinator.deriveArchiveKey(this.kek, archiveId);
    const ciphertext = archiveBuffer.slice(
      HEADER_LENGTH,
      HEADER_LENGTH + ciphertextLen,
    );
    ...
    try {
      ...
      const { metadata, bundle: bundleB64 } = JSON.parse(
      ...
      return {
        valid: true,
        metadata,
        bundle: JSON.parse(bundleJson.toString("utf8")),
      };
    } catch (err) {
  ...
  async restore(archiveId, { dryRun = false, asOf } = {}) {
    ...
    if (asOf !== undefined) {
      ...
      if (!candidate) {
        const err = new Error(
          "BACKUP_NOT_FOUND: backup-coordinator: no archive at or before asOf",
        );
    ...
    if (!archiveBuffer) {
      const err = new Error(
        "BACKUP_NOT_FOUND: backup-coordinator: archive not found",
      );
      err.code = "BACKUP_NOT_FOUND";
      throw err;
    }
    ...
    if (!verified.valid) {
      const err = new Error(
        `RESTORE_FAILED: backup-coordinator: ${verified.reason}`,
      );
    ...
    const bundle = {
      ...
      auditLog: Buffer.isBuffer(Buffer.from(verified.bundle.auditLog, "base64"))
        ? JSON.parse(
            Buffer.from(verified.bundle.auditLog, "base64").toString("utf8"),
          )
        : [],
  ...
  async prune(beforeTimestamp) {
    ...
    for (const archive of archives) {
      if (archive.timestamp < beforeTimestamp) {
        ...
        if (this.onEvent)
          this.onEvent("BACKUP_PRUNED", { archiveId: archive.archiveId });
      }
    }
