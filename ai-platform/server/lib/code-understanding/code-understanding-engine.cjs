...
function synthesizeSummary(layers) {
  ...
  if (layers.contextual?.contextSummary)
    parts.push(layers.contextual.contextSummary);
  if (layers.expert?.consensus?.latestNote)
    parts.push(`Expert note: ${layers.expert.consensus.latestNote}`);
  if (layers.semantic?.aiSummary) parts.push(layers.semantic.aiSummary);
  return (
    parts.join(" ").trim() || "Insufficient data for a consolidated summary."
  );
}
...
async function understandCodeSnippet(content, context = {}, options = {}) {
  ...
  const semanticLayer = await analyzeSemanticLayer(
    content,
    {
      ...context,
      filePath,
  ...
  const contextualLayer = context.projectPath
    ...
    : {
        layer: "contextual",
        available: false,
        contextSummary: "No project root provided for git/doc context.",
      };

  let expertLayer = {
    layer: "expert",
    reviews: [],
    consensus: { count: 0, consensus: null, confidence: 0 },
  };
  if (context.projectPath) {
    const relPath = String(filePath).replace(/\\/g, "/");
    const reviews = await loadExpertReviews(
      context.platformRoot || context.projectPath,
      {
        filePath: relPath,
 ...
 * @returns {any}
 */
async function attachUnderstandingToCodebaseReport(
  report,
  projectPath,
  options = {},
) {
  ...
  const topFiles = [...new Set(sampleFindings.map((f) => f.filePath))].slice(
    0,
    3,
  );
  const fileInsights = [];
  ...
  for (const rel of topFiles) {
    ...
    try {
      ...
      const understanding = await understandCodeSnippet(
        content,
        {
          filePath: rel,
          projectPath,
