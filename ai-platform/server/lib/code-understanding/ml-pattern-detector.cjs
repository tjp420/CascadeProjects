...
class StatisticalPatternDetector {
  ...
  initializePatterns() {
    ...
    this.patterns.set("mvc-pattern", {
      ...
      indicators: [
        {
          type: "class_names",
          pattern: /(Controller|Model|View)$/i,
          weight: 0.3,
        },
        {
          type: "file_structure",
          pattern: /controllers\/|models\/|views\//i,
          weight: 0.4,
        },
        {
          type: "imports",
          pattern: /import.*from.*['"]\.\.\/(models|views|controllers)/i,
          weight: 0.3,
        },
      ],
    });
    ...
    this.patterns.set("repository-pattern", {
      ...
      indicators: [
        ...
        {
          type: "method_names",
          pattern: /(save|find|delete|update)All/i,
          weight: 0.3,
        },
    ...
    this.patterns.set("input-validation", {
      ...
      indicators: [
        {
          type: "function_calls",
          pattern: /(validate|sanitize|clean|escape)/i,
          weight: 0.4,
        },
        {
          type: "conditionals",
          pattern: /if\s*\(\s*.*\.(length|size|trim)/i,
          weight: 0.3,
        },
    ...
    this.patterns.set("authentication-flow", {
      ...
      indicators: [
        {
          type: "function_calls",
          pattern: /(authenticate|login|signin|jwt|token)/i,
          weight: 0.4,
        },
        {
          type: "imports",
          pattern: /import.*(?:bcrypt|jsonwebtoken|passport)/i,
          weight: 0.4,
        },
        {
          type: "variables",
          pattern: /(password|token|auth|session)/i,
          weight: 0.2,
        },
      ],
    });
    ...
    this.patterns.set("caching-pattern", {
      ...
      indicators: [
        { type: "function_calls", pattern: /(cache|memo|store)/i, weight: 0.4 },
        {
          ...
          pattern: /(Map|Set|Object)\.set\(|\.get\(/i,
          weight: 0.3,
        },
    ...
    this.patterns.set("async-pattern", {
      ...
      indicators: [
        {
          type: "keywords",
          pattern: /\b(async|await|Promise|callback)/i,
          weight: 0.4,
        },
        {
          type: "function_calls",
          pattern: /\.then\(|\.catch\(|setTimeout\(/i,
          weight: 0.3,
        },
    ...
    this.patterns.set("unit-test-pattern", {
      ...
      indicators: [
        {
          type: "function_calls",
          pattern: /(describe|it|test|expect|assert)/i,
          weight: 0.4,
        },
        {
          type: "imports",
          pattern: /import.*(?:jest|test|chai|mocha)/i,
          weight: 0.4,
        },
    ...
    this.patterns.set("data-pipeline", {
      ...
      indicators: [
        {
          type: "function_calls",
          pattern: /(pipe|map|filter|reduce|transform)/i,
          weight: 0.4,
        },
        {
          type: "method_chaining",
          pattern: /\.pipe\(\s*\w+\)|\.map\(\s*\w+\)/i,
          weight: 0.3,
        },
        {
          type: "data_operations",
          pattern: /(forEach|map|filter|reduce)/i,
          weight: 0.3,
        },
      ],
    });
    ...
    this.patterns.set("error-handling-pattern", {
      ...
      indicators: [
        {
          type: "keywords",
          pattern: /\b(try|catch|throw|Error)/i,
          weight: 0.4,
        },
        {
          type: "error_objects",
          pattern: /new\s+Error\(|\.message|\.stack/i,
          weight: 0.3,
        },
        {
          type: "logging",
          pattern: /console\.(error|warn|log)|logger\./i,
          weight: 0.3,
        },
      ],
    });
    ...
    this.patterns.set("rest-api-pattern", {
      ...
      indicators: [
        {
          type: "http_methods",
          pattern: /\b(get|post|put|delete|patch)\b/i,
          weight: 0.4,
        },
        {
          type: "routing",
          pattern: /(router|route|app\.(get|post|put|delete))/i,
          weight: 0.4,
        },
        {
          type: "status_codes",
          pattern: /(status|statusCode)\s*:\s*\d{3}/i,
          weight: 0.2,
        },
      ],
    });
  ...
  analyzePattern(content, contentLower, pattern, context) {
    ...
    for (const indicator of pattern.indicators) {
      const match = this.checkIndicator(
        content,
        contentLower,
        indicator,
        context,
  ...
  getPatternSummary(detectedPatterns) {
    const summary = {
      ...
      mediumConfidence: detectedPatterns.filter(
        (p) => p.confidence > 0.4 && p.confidence <= 0.7,
      ).length,
...
function generatePatternInsights(patterns, summary) {
  ...
  if (summary.categories[PATTERN_CATEGORIES.SECURITY]?.count > 0) {
    ...
    insights.push({
      ...
      level: "warning",
      message:
        "No explicit security patterns detected - consider adding input validation and authentication",
    });
  }
  ...
  if (summary.categories[PATTERN_CATEGORIES.ERROR_HANDLING]?.count > 0) {
    ...
    insights.push({
      ...
      level: "warning",
      message:
        "Limited error handling detected - consider adding try-catch blocks",
    });
  }
