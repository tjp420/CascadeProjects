...
const INTENT_TEMPLATES = {
  zscript: {
    actor: 'Defines or extends a game actor (GZDoom ZScript) — likely gameplay entity behavior.',
    ...
    debug: 'Contains developer-only diagnostics that should be removed before release.',
  },
  javascript: {
...
function classifyPurpose(content, language, businessLogic, filePath) {
  ...
  if (language === 'zscript' || /\.zs$|zscript/.test(rel)) {
    if (/\bclass\s+\w+\s*:\s*(Weapon|CustomInventory)/.test(content)) purposes.push('weapon');
    if (/\bclass\s+\w+\s*:\s*Actor/.test(content)) purposes.push('actor');
    if (/\bStates\s*\{/.test(content)) purposes.push('state');
    if (/\b(Console\.Command|A_Log|DEVONLY)\b/.test(content)) purposes.push('debug');
  }
  ...
  const statements = purposes.filter((p) => templates[p]).map((p) => templates[p]);
  ...
  if (!statements.length) {
    statements.push('General-purpose code module — purpose inferred from structure and path only.');
  }
...
function assessAssumptions(content, language) {
  ...
  if (language === 'zscript' && /\bversion\s*["']\s*4\./.test(content)) {
    assumptions.push('Uses GZDoom 4.x+ ZScript features — requires compatible engine version.');
  }
...
async function analyzeSemanticLayer(content, context = {}, options = {}) {
  ...
  const detection =
    context.languageDetection || detector.detectLanguage(filePath || 'snippet.txt', content);
