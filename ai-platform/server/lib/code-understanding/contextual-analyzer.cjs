...
async function analyzeGitContext(relativePath, projectRoot) {
  ...
  const logLine = await runGit(root, [
    "log",
    "-1",
    "--format=%H|%an|%ad|%s",
    "--date=short",
  ...
  const recent = await runGit(root, [
    "log",
    "-5",
    "--format=%h %ad %s",
    "--date=short",
  ...
  if (!logLine) {
    return {
      available: false,
      reason: "No git history for path or not a git repository.",
    };
  }
...
function inferIntentFromCommitMessage(line) {
  ...
  if (/feat|add|implement/.test(msg))
    return "Feature addition or capability expansion";
...
async function analyzeContextualLayer(filePath, projectRoot, _content = "") {
  ...
  if (gitHistory.available && gitHistory.lastCommit?.message) {
    contextSummary.push(
      `Last change: "${gitHistory.lastCommit.message}" (${gitHistory.lastCommit.date}).`,
    );
  }
  if (documentation.length) {
    contextSummary.push(
      `Adjacent docs: ${documentation.map((d) => d.name).join(", ")}.`,
    );
  }
  if (!contextSummary.length) {
    contextSummary.push(
      "Limited contextual metadata — no git history or nearby README found.",
    );
  }
