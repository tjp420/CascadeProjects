...
function parseCvarInfoFile(content, sourceFile = "CVARINFO") {
  const definitions = [];
  const lineRe =
    /^\s*(user|server|nosave|archive|latched|cheat)?\s*(bool|int|float|color|string)\s+([a-zA-Z0-9_]+)\s*=\s*([^;]+);/gim;
...
function extractIntensityScalingHints(content, relativePath) {
  ...
  const patterns = [
    {
      id: "find-cvar-intensity",
      re: /FindCVar\s*\(\s*["'][^"']*intensity[^"']*["']\s*\)/gi,
      label: "CVAR intensity lookup",
    },
    {
      id: "master-intensity-var",
      re: /\bmasterIntensity\b/g,
      label: "masterIntensity field usage",
    },
    {
      id: "adjust-brightness",
      re: /\bAdjustBrightness\s*\(/g,
      label: "AdjustBrightness call",
    },
    {
      id: "calculate-light-intensity",
      re: /\bCalculateLightIntensity\s*\(/g,
      label: "CalculateLightIntensity call",
    },
    {
      id: "clamp-intensity",
      re: /\bclamp\s*\([^)]*intensity[^)]*\)/gi,
      label: "Intensity clamp",
    },
    {
      id: "multiply-intensity",
      re: /intensity\s*\*\s*[a-zA-Z_]/gi,
      label: "Intensity multiplication",
    },
  ];
...
async function buildCvarReport(rootDir, zscriptFiles, _options = {}) {
  ...
  for (const file of zscriptFiles) {
    for (const usage of extractFindCvarUsages(
      file.content,
      file.relativePath,
    )) {
    ...
    scalingHints.push(
      ...extractIntensityScalingHints(file.content, file.relativePath),
    );
  }
  ...
  const intensityCvars = Object.values(cvars).filter((c) =>
    /intensity/i.test(c.name),
  );
  const problemHints = buildIntensityProblemHints(intensityCvars, scalingHints);

  return {
    cvarInfoFiles: cvarInfoPaths.map((p) =>
...
function buildIntensityProblemHints(intensityCvars, scalingHints) {
  ...
  const globalDef = intensityCvars.find(
    (c) =>
      c.name.includes("lite_intensity") &&
      !c.name.includes("player") &&
      !c.name.includes("item"),
  );
  const playerDef = intensityCvars.find((c) =>
    c.name.includes("player_intensity"),
  );
  ...
  if (globalDef && playerDef) {
    hints.push({
      ...
      message:
        "Global intensity uses large float range (e.g. 750) while player intensity uses small multiplier (e.g. 1.0) — changing one may not affect visuals if code path reads the other.",
  ...
  if (
    !scalingHints.some(
      (h) => h.id === "multiply-intensity" || h.id === "adjust-brightness",
    )
  ) {
    hints.push({
      severity: "medium",
      ...
      message:
        "No obvious intensity multiplication/AdjustBrightness chain detected in scanned files — CVAR value may be read but not applied to rendered light.",
    });
  }
