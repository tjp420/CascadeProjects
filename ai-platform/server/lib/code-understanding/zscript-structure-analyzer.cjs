...
function parseZscriptFile(content, relativePath) {
    ...
    const legacy = {
        ...
        parentClasses: [...new Set((parsed.classes || []).map((c) => c.parentClass))],
        keyFunctions: (parsed.classes || []).flatMap((c) => c.methods || []).slice(0, 30),
        dependencies: parsed.ast?.includes || [],
        purpose: parsed.summary?.purpose || inferPurpose(relativePath, parsed.classes || []),
...
function inferPurpose(relativePath, classes) {
    ...
    if (rel.includes('handler') || rel.includes('system')) return 'Event handler / system coordinator';
    if (rel.includes('util') || rel.includes('helper')) return 'Shared helper utilities';
    if (rel.includes('debug')) return 'Debug/diagnostic tooling';
    if (classes.some((c) => /Light/i.test(c.name))) return 'Dynamic or actor-attached lighting';
    if (classes.some((c) => c.parentClass === 'EventHandler')) return 'GZDoom event handler registration';
...
function buildClassHierarchy(zscriptFiles) {
    ...
    return {
        ...
        roots: [...nodes.values()].filter((n) => !nodes.has(n.parentClass) || n.parentClass === 'Object').map((n) => n.className),
        nodes: Object.fromEntries([...nodes.entries()].map(([k, v]) => [k, {
...
function analyzeFunctionLogic(zscriptFiles) {
    ...
    for (const file of zscriptFiles) {
        for (const fn of targets) {
            const re = new RegExp(`\\b(?:override\\s+)?(?:static\\s+)?(?:void|float|bool|int|double)[^{;]*\\b${fn}\\s*\\([^)]*\\)\\s*\\{`, 'g');
...
function summarizeFunctionBlock(block, fnName) {
    ...
    if (/GetFloat|GetBool|GetInt/.test(block)) steps.push('Extracts typed value from CVAR object');
    if (/masterIntensity/.test(block)) steps.push('Updates masterIntensity field from CVAR');
    if (/AdjustBrightness/.test(block)) steps.push('Applies AdjustBrightness to color/intensity');
    if (/CalculateLightIntensity/.test(block)) steps.push('Uses distance-based intensity falloff');
    if (/A_Light|DynamicLight|light/i.test(block)) steps.push('Creates or updates dynamic light output');
    if (/return/.test(block) && fnName.startsWith('Get')) steps.push('Returns computed scalar to caller');
    if (!steps.length) steps.push('Logic present — review block manually for rendering side effects');
...
function flagFunctionIssues(fn, block) {
    ...
    if (fn === 'UpdateCVARs' && /FindCVar/.test(block) && !/masterIntensity\s*=/.test(block) && !/intensity/i.test(block)) {
        issues.push('CVARs read but intensity fields may not be reassigned in this function');
    }
    if (/GetFloat\(\)/.test(block) && !/\*|AdjustBrightness|CalculateLightIntensity|A_Light/.test(block)) {
        issues.push('CVAR float read without visible scaling/application in same function');
    }
    if (/clamp\s*\([^)]+\)/.test(block) && /1500/.test(block)) {
        issues.push('Intensity clamped to 0–1500 — small CVAR changes may be visually subtle after normalization');
    }
    return issues;
}
...
function inferDataFlow(fn, block) {
    ...
    if (/masterIntensity|GetIntensityCVAR/.test(block)) flow.push('masterIntensity');
...
async function buildStructureReport(rootDir, options = {}) {
    ...
    for (const file of zscriptFiles) {
        ...
        zscript_files[file.relativePath] = parseZscriptFile(file.content, file.relativePath);
    }
    ...
    return {
        ...
        zscript_files,
        classHierarchy: buildClassHierarchy(zscriptFiles.filter((f) => !f.truncated)),
        function_analysis: analyzeFunctionLogic(zscriptFiles.filter((f) => !f.truncated)),
