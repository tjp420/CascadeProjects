...
describe("PartialShareProofManager proof creation and verification", () => {
  test("valid proof verifies and invalid variants are rejected", () => {
    ...
    assert(
      ...
      "proof must contain evidence_id",
    );
    ...
    assert(
      resBad1 && resBad1.ok === false && resBad1.reason === "schema",
      "missing proof_material should fail with schema error",
    );
    ...
    assert(
      resBad2 && resBad2.ok === false && resBad2.reason === "schema",
      "wrong typed signature should fail with schema error",
    );
    ...
    assert(
      resBad3 && resBad3.ok === false && resBad3.reason === "canonicalization",
      "__bigint_hex injected envelope should fail with canonicalization error",
    );
  });
});
