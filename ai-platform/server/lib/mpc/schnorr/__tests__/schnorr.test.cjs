...
describe("Threshold Schnorr Signature Aggregation Invariants", () => {
  ...
  test("computes Lagrange weights and they sum to 1 mod q for quorum [1,2,3]", () => {
    const quorum = [1, 2, 3];
    const weights = quorum.map((i) =>
      aggregator.computeLagrangeWeight(i, quorum),
    );
    // sum them
  ...
  test("Should evaluate partial shares and validate linear aggregation for 2-of-3 committee", () => {
    ...
    const sumPartials = partials.reduce(
      (a, b) => aggregator.field.add(a, b),
      0n,
    );
    ...
    const nonceSum = nonces.reduce((acc, nn) => {
      return evaluator.field.add(
        acc,
        evaluator.field.add(nn.k1, evaluator.field.mul(binding, nn.k2)),
      );
    }, 0n);
...
describe("MuSig2 Core Protocol Primitives", () => {
  ...
  test("L2-06: Full signing round-trip — 2-of-3 threshold", () => {
    ...
    const bindingFactor = aggregator.computeBindingFactor(
      aggPubKey,
      nonceCommitments,
    );

    // 4. Aggregate nonces (use h1 values as public nonce stand-ins)
    const publicNonces = noncePairs.map(
      (np) => BigInt("0x" + np.publicCommitment.h1) % aggregator.field.q,
    );
    const aggNonce = aggregator.aggregateNonces(publicNonces, bindingFactor);

    // 5. Compute challenge
    const challenge = aggregator.computeChallenge(
      aggPubKey,
      aggNonce,
      messageHash,
    );
    ...
    const aggSecretNonce = noncePairs.reduce((acc, np, idx) => {
      const blended = aggregator.field.add(
        np.secret.k1,
        aggregator.field.mul(bindingFactor, np.secret.k2),
      );
      return aggregator.field.add(acc, blended);
    }, 0n);

    // 10. Verify signature
    const valid = aggregator.verifySignature(
      aggPubKey,
      aggNonce,
      signature,
      messageHash,
  ...
  test("L2-08: verifySignature — valid signature returns true", () => {
    ...
    const sig = { R, s };
    expect(
      aggregator.verifySignature(P, R, sig, msgHash, {
        aggPrivateKey: x_agg,
        aggSecretNonce: k_agg,
      }),
    ).toBe(true);
  });
  ...
  test("L2-11: verifySignature — wrong message returns false", () => {
    ...
    const sig = { R, s };
    expect(
      aggregator.verifySignature(P, R, sig, wrongMsgHash, {
        aggPrivateKey: x_agg,
        aggSecretNonce: k_agg,
      }),
    ).toBe(false);
  ...
  test("L2-13: partial share linear aggregation matches expected sum", () => {
    ...
    const secretSum = keyShares.reduce((acc, ks, idx) => {
      return aggregator.field.add(
        acc,
        aggregator.field.mul(aggregator.field.mul(challenge, ks), weights[idx]),
      );
    }, 0n);
    const nonceSum = nonces.reduce((acc, nn) => {
      return aggregator.field.add(
        acc,
        aggregator.field.add(nn.k1, aggregator.field.mul(binding, nn.k2)),
      );
    }, 0n);
