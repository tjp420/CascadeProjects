...
class Musig2NonceGenerator {
  ...
  generateNoncePair(sessionId) {
    if (sessionId === undefined || sessionId === null) {
      throw new Error('Musig2NonceGenerator.generateNoncePair: sessionId is required');
    }
    if (typeof sessionId !== 'string' || sessionId.length === 0) {
      throw new Error('Musig2NonceGenerator.generateNoncePair: sessionId must be a non-empty string');
    }
    ...
    const h1 = crypto.createHash('sha256').update(safeK1.toString() + sessionId).digest('hex');
    const h2 = crypto.createHash('sha256').update(safeK2.toString() + sessionId).digest('hex');
