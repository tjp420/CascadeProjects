...
class SchnorrShareEvaluator {
  ...
  _validateBigInt(value, name, allowZero = true) {
    ...
    try {
      ...
      throw new Error(
        `SchnorrShareEvaluator: ${name} must be a BigInt or BigInt-coercible, got ${typeof value}`,
      );
    }
    if (!allowZero && bg === 0n) {
    ...
    if (bg < 0n || bg >= this.subgroupOrder) {
      throw new Error(
        `SchnorrShareEvaluator: ${name} must be in subgroup range [0, q-1]`,
      );
    }
    return bg;
  }
  ...
  evaluatePartialShare({
    ...
    if (challenge === undefined || challenge === null) {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: challenge is required",
      );
    }
    if (secretKeyShare === undefined || secretKeyShare === null) {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: secretKeyShare is required",
      );
    }
    if (lagrangeWeight === undefined || lagrangeWeight === null) {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: lagrangeWeight is required",
      );
    }
    if (!secretNonces || typeof secretNonces !== "object") {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: secretNonces object is required",
      );
    }
    if (secretNonces.k1 === undefined || secretNonces.k1 === null) {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: secretNonces.k1 is required",
      );
    }
    if (secretNonces.k2 === undefined || secretNonces.k2 === null) {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: secretNonces.k2 is required",
      );
    }
    if (bindingFactor === undefined || bindingFactor === null) {
      throw new Error(
        "SchnorrShareEvaluator.evaluatePartialShare: bindingFactor is required",
      );
    }
