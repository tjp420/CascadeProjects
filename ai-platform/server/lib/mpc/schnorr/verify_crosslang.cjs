...
function runBinary(path, input) {
  try {
    const proc = spawnSync(path, {
      input: JSON.stringify(input),
      encoding: "utf8",
    });
    if (proc.error) throw proc.error;
    if (proc.status !== 0)
      throw new Error(`exit ${proc.status}: ${proc.stderr}`);
...
function run() {
  ...
  const goPath = "go";
  const rustPath =
    "./ai-platform/server/lib/mpc/schnorr/reference-runner-rust/target/release/reference-runner-rust";
  ...
  for (const s of samples) {
    ...
    try {
      ...
      console.log(
        "Go runner dir listing:",
        fs.readdirSync(
          "ai-platform/server/lib/mpc/schnorr/reference-runner-go",
        ),
      );
      // run 'go run main.go' in the Go runner directory so local go.mod is used
      const proc = spawnSync(goPath, ["run", "main.go"], {
        cwd: "ai-platform/server/lib/mpc/schnorr/reference-runner-go",
