...
function findPackageJson(startDir) {
  ...
  try {
    ...
    /* directory read may fail on restricted paths */
  }
  return null;
}
...
function aggregateNpmAudits(results) {
  ...
  return {
    ...
    auditRoots: valid.map((r, i) => ({
      index: i,
      root: r.auditRoot || "unknown",
    })),
  };
}
...
function runNpmAudit(projectRoot, options = {}) {
  ...
  const packageRoots =
    options.monorepo !== false ? discoverPackageJsonLocations(root) : [root];
