...
describe('daily-ops-report', () => {
  ...
  beforeEach(() => {
    ...
    tempQueueDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb-ops-report-email-'));
    process.env.WEBHOOK_EVENT_LOG = path.join(tempLogDir, 'webhook-event-log.json');
  ...
  it('gatherDailyData categorizes events correctly', async () => {
    const { logWebhookEvent, clearCache } = require('../lib/webhook-event-log.cjs');
    clearCache();

    await logWebhookEvent({ eventId: 'evt_1', eventType: 'invoice.payment_failed', status: 'processed', customerEmail: 'fail@test.com', reason: 'card_declined' });
    await logWebhookEvent({ eventId: 'evt_2', eventType: 'charge.dispute.created', status: 'processed', reason: 'fraudulent', amount: '$49.00 USD' });
    await logWebhookEvent({ eventId: 'evt_3', eventType: 'checkout.session.completed', status: 'processed', customerEmail: 'ok@test.com', amount: '$49.00' });
    await logWebhookEvent({ eventId: 'evt_4', eventType: 'customer.subscription.deleted', status: 'processed', customerEmail: 'cancel@test.com' });
    await logWebhookEvent({ eventId: 'evt_5', eventType: 'customer.subscription.trial_will_end', status: 'processed', customerEmail: 'trial@test.com' });
    await logWebhookEvent({ eventId: 'evt_6', eventType: 'invoice.paid', status: 'error', detail: 'handler crashed' });
  ...
  it('generateTextReport produces readable text with all sections', () => {
    ...
    const summary = {
      total: 3,
      byType: { 'invoice.payment_failed': 1, 'charge.dispute.created': 1, 'checkout.session.completed': 1 },
      byStatus: { processed: 3 },
      paymentFailures: [{ customerEmail: 'fail@test.com', reason: 'card_declined', timestamp: new Date().toISOString() }],
      disputes: [{ reason: 'fraudulent', amount: '$49.00 USD', timestamp: new Date().toISOString() }],
      trialWarnings: [],
      successfulPayments: [{ eventType: 'checkout.session.completed', customerEmail: 'ok@test.com', amount: '$49.00', timestamp: new Date().toISOString() }],
      cancellations: [],
      errors: []
    };
    ...
    assert.ok(!text.includes('Trial Ending Warnings'), 'empty sections should not appear');
  });
  ...
  it('generateHtmlReport produces valid HTML with stat cards', () => {
    ...
    const summary = {
      total: 5,
      byType: { 'invoice.payment_failed': 2, 'charge.dispute.created': 1, 'checkout.session.completed': 2 },
      byStatus: { processed: 4, error: 1 },
      paymentFailures: [{ customerEmail: 'a@test.com', reason: 'declined', amount: null, timestamp: new Date().toISOString() }],
      disputes: [{ reason: 'fraudulent', amount: '$49.00 USD', timestamp: new Date().toISOString() }],
      trialWarnings: [],
      successfulPayments: [],
      cancellations: [],
      errors: [{ eventType: 'invoice.paid', detail: 'crash', timestamp: new Date().toISOString() }]
    };
  ...
  it('sendDailyReport queues email when no provider configured', async () => {
    ...
    if (result.queued) {
      ...
      assert.ok(queuedEmails.length > 0, 'report email should be queued to disk');
  ...
  it('startScheduler does nothing when OPS_REPORT_ENABLED is not set', () => {
    const { startScheduler, stopScheduler } = require('../lib/daily-ops-report.cjs');
    startScheduler();
    stopScheduler();
  });
  ...
  it('report excludes events older than 24 hours', async () => {
    ...
    fs.writeFileSync(storePath, JSON.stringify({ events: [recentEvent, oldEvent], updatedAt: new Date().toISOString(), count: 2 }, null, 2));

    delete require.cache[require.resolve('../lib/daily-ops-report.cjs')];
    const { gatherDailyData } = require('../lib/daily-ops-report.cjs');
    const { summary } = gatherDailyData();

    assert.strictEqual(summary.total, 1, 'should only count events from last 24h');
    assert.strictEqual(summary.successfulPayments.length, 1);
    assert.strictEqual(summary.paymentFailures.length, 0, 'old payment failure should be excluded');
  });
});
