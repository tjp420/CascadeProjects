...
function patchPhaseDescription(phase, report) {
  ...
  const cleanupAssistant =
    report.cleanupAssistant || results.cleanupAssistant || {};
  ...
  if (id === "compliance") {
    ...
    if (licenseCount === null && securityCount === null) {
      return {
        ...phase,
        description: "Compliance scan incomplete — re-run with full inventory.",
      };
    }
    return {
      ...phase,
  ...
  if (id === "optimization") {
    ...
    if (healthScore != null && healthScore >= 100) {
      return {
        ...phase,
        description: `Maintain quality score at ${healthScore}/100.`,
      };
    }
    if (healthScore != null) {
      return {
        ...phase,
        description: `Drive quality score from ${healthScore}/100 toward 95+.`,
      };
    }
    return phase;
  }
  ...
  if (id === "npmaudit") {
    const govPkgCount =
      codebase?.summary?.governanceFiles?.packageJsonCount ?? null;
    const hasPackageJson =
      govPkgCount != null
        ? govPkgCount > 0
        : simplebeacon?.repositoryFilesTotal != null ||
          summary.totalFiles != null ||
          npmAudit?.packageCount != null;
    const vulnCount =
      npmAudit?.vulnerabilities?.total ??
      npmAudit?.metadata?.vulnerabilities?.total ??
      0;
    const pkgCount =
      npmAudit?.packageCount ?? npmAudit?.metadata?.dependencies ?? 0;
    if (hasPackageJson && pkgCount > 0) {
      return {
        ...phase,
        description: `${pkgCount} package(s) scanned. ${vulnCount} vulnerability(ies) found.`,
      };
    }
    if (hasPackageJson) {
      return {
        ...phase,
        description: `package.json detected — npm audit results pending.`,
      };
    }
    if (govPkgCount === 0) {
      return {
        ...phase,
        description: `No package.json detected — verify project has dependencies.`,
      };
    }
    return {
      ...phase,
      description: `Scan data incomplete — package.json status unknown.`,
    };
  }

  if (id === "buildreadiness") {
    const buildIssues =
      codebase?.findings?.filter(
        (f) => f.category === "build" || f.type === "build",
      ).length ?? 0;
    const health = codebase?.summary?.healthScore ?? null;
    if (buildIssues === 0 && (health == null || health >= 80)) {
      return {
        ...phase,
        description: `Build readiness verified — no issues detected.`,
      };
    }
    return {
      ...phase,
      description: `${buildIssues} build issue(s) detected. Health: ${health ?? "unknown"}%.`,
    };
  }
  ...
  if (id === "security") {
    ...
    const hasScanData =
      simplebeacon?.issueCount != null || Object.keys(sev).length > 0;
    const credIssues = simplebeacon?.issueCount ?? sev.critical ?? 0;
    if (!hasScanData) {
      return {
        ...phase,
        description: `Security scan incomplete — re-run with full scan data.`,
      };
    }
    if (credIssues === 0) {
      return {
    ...
    return {
      ...phase,
      description: `${credIssues} security issue(s) detected — review credentials && secrets.`,
    };
  }
  ...
  if (id === "integrity") {
    ...
    return {
      ...phase,
      description: `${structIssues} structural issue(s) detected.`,
    };
  }
  ...
  if (id === "consistency") {
    ...
    return {
      ...phase,
      description: `${dupGroups} duplicate group(s) detected — review merge candidates.`,
    };
  }
  ...
  if (id === "mockdata") {
    const fictionHits =
      ...
      summary.fictionKpiHits ??
      0;
    if (fictionHits === 0) {
      return {
        ...phase,
        description: `No mock data issues — fixtures verified or none detected.`,
      };
    }
    return {
      ...phase,
      description: `${fictionHits} fiction/mock data issue(s) detected.`,
    };
  }
  ...
  if (id === "junkfiles") {
    ...
    return {
      ...phase,
      description: `${totalCount} junk / bloat item(s) detected (${totalReclaimable} bytes reclaimable).`,
    };
  }
  ...
  if (id === "vulns") {
    ...
    return {
      ...phase,
      description: `${vulnTotal} vulnerable dependency(ies) detected.`,
    };
  }

  if (id === "cleanup") {
    const safeFiles =
      cleanupAssistant?.estimatedReduction?.files ??
      summary.cleanupSafeFiles ??
      0;
    const dqFindings =
      dataQuality?.summary?.totalFindings ?? summary.dataQualityFindings ?? 0;
    const bloatCount = fileReduction?.summary?.directoryBloatFindings ?? 0;
    const bloatBytes =
      fileReduction?.summary?.directoryBloatReclaimableBytes ?? 0;
    ...
    return {
      ...phase,
      description: `${totalSafe} safe-to-delete item(s) identified (${bloatBytes} bytes from bloat). ${dqFindings} data-quality finding(s).`,
    };
  }
...
function normalizeDashboardReport(report, requestedPath = "") {
  ...
  const requested = String(
    requestedPath || report.projectRoot || report.projectPath || "",
  ).trim();
  const requestedForward = requested ? requested.replace(/\\/g, "/") : "";
  const scanRoot =
    requestedForward ||
    String(
      report.projectRoot || report.projectPath || report.scanTargetRoot || "",
    ).replace(/\\/g, "/");
  ...
  const repositoryInventory =
    ...
    (report.inventory && typeof report.inventory === "object"
      ? {
          totalFiles:
            report.inventory.totalFiles ??
            report.inventory.scannedFiles ??
            null,
  ...
  const reportVersion =
    Number(report.reportVersion) >= 2
      ? Number(report.reportVersion)
      : report.version === "1.0.0" || report.type === "simplebeacon-report"
        ? 2
        : (report.reportVersion ?? 2);

  const mockSampleFiles = report.mockSampleFiles ?? summary.totalFiles ?? null;

  let qualityScore = report.qualityScore ?? null;
  if (qualityScore == null && !report.qualityScoreHidden) {
    const gateScore =
      report.gate && typeof report.gate.score === "number"
        ? report.gate.score
        : null;
    if (gateScore != null) {
  ...
  const scanScope = report.scanScope || {
    ...
    gatePolicy: (report.gate && {
      failOn: report.gate.failOn,
      warnOn: report.gate.warnOn,
    }) || { failOn: ["high"], warnOn: ["medium", "low"] },
