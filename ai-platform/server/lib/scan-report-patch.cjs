...
function patchPhaseDescription(phase, report) {
  ...
  if (id === 'buildreadiness') {
    const buildIssues =
      codebase?.findings?.filter((f) => f.category === 'build' || f.type === 'build').length ?? 0;
  ...
  if (id === 'junkfiles') {
    const reclaimable =
      fileReduction?.summary?.reclaimableBytes ?? summary.fileReductionReclaimableBytes ?? 0;
...
function normalizeDashboardReport(report, requestedPath = '') {
  ...
  const scanRoot =
    requestedForward ||
    String(report.projectRoot || report.projectPath || report.scanTargetRoot || '').replace(
      /\\/g,
      '/'
    );
  ...
  if (qualityScore == null && !report.qualityScoreHidden) {
    const gateScore =
      report.gate && typeof report.gate.score === 'number' ? report.gate.score : null;
    if (gateScore != null) {
  ...
  const scanScope = report.scanScope || {
    ...
    gatePolicy: (report.gate && { failOn: report.gate.failOn, warnOn: report.gate.warnOn }) || {
      failOn: ['high'],
      warnOn: ['medium', 'low'],
    },
