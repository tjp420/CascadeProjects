...
async function evaluateHumanOversightCompliance(scanJson, options = {}) {
  const provider =
    options.provider || process.env.COMPLIANCE_PROVIDER || "openai";
  ...
  try {
    const result = await summarizeScanWithProvider(provider, scanJson, {
      customPrompt: prompt,
      projectPath:
        options.projectPath || scanJson.projectPath || "unknown-project",
    ...
    return {
      ...
      findings: [
        {
          severity: "HIGH",
          pillar: "LLM Output Parsing",
          description:
            "AI provider returned unstructured output — manual review required.",
        },
      ],
      humanRemediationPlan:
        "Re-run with a configured AI provider to receive structured compliance scoring.",
      overallScore: "D",
    };
  } catch (err) {
    logger.warn(
      "[Compliance Rules] LLM evaluation failed, using deterministic fallback:",
      err.message,
    );
    return buildDeterministicEvaluation(scanJson);
  }
...
function buildHumanOversightPrompt(scanJson) {
  ...
  const apiPaths = allPaths.filter((p) =>
    /(api|route|server|handler)/i.test(p),
  );
  ...
  const payload = JSON.stringify(
    {
      repositoryFilesTotal:
        overview.repositoryFilesTotal ?? repoInv.totalFiles ?? "—",
...
function buildDeterministicEvaluation(scanJson) {
  ...
  const allText = issues
    .map((i) => `${i.type || ""} ${i.description || ""} ${i.filePath || ""}`)
    .join(" ")
    .toLowerCase();

  const hasConfidence = /confidence|probability|uncertainty|score/i.test(
    allText,
  );
  const hasAbort = /abort|kill|terminate|cancel|stop|emergency/i.test(allText);
  const hasExplain =
    /log|audit|metadata|explain|feature|weight|attribution/i.test(allText);
  const hasSanitize = /sanit|validat|regex|filter|inject|scrub|clean/i.test(
    allText,
  );
  ...
  if (hasConfidence)
    findings.push({
      severity: "LOW",
      pillar: "Automation Bias Prevention",
      description:
        "Repository contains references to confidence scoring or uncertainty metrics.",
    });
  else
    findings.push({
      severity: "HIGH",
      pillar: "Automation Bias Prevention",
      description:
        "No confidence-score or uncertainty UI indicators detected in scanned files.",
    });

  if (hasAbort)
    findings.push({
      severity: "LOW",
      pillar: "Intervention & Kill-Switch",
      description: "Abort, terminate, or stop hooks detected in source.",
    });
  else
    findings.push({
      severity: "HIGH",
      pillar: "Intervention & Kill-Switch",
      description:
        "No cancellation tokens, abort handlers, or emergency-stop functions detected.",
    });

  if (hasExplain)
    findings.push({
      severity: "LOW",
      pillar: "Explainability & Interpretability",
      description:
        "Logging or metadata exports found that could support human review.",
    });
  else
    findings.push({
      severity: "HIGH",
      pillar: "Explainability & Interpretability",
      description:
        "No decision-metadata, feature-importance, or inference-telemetry logging detected.",
    });

  if (hasSanitize)
    findings.push({
      severity: "LOW",
      pillar: "Data Hygiene & Input Scrubbing",
      description:
        "Input-validation or sanitization patterns found in codebase.",
    });
  else
    findings.push({
      severity: "HIGH",
      pillar: "Data Hygiene & Input Scrubbing",
      description:
        "No strict input-validation, regex filters, or prompt-injection guards detected at API boundaries.",
    });

  const passCount = [hasConfidence, hasAbort, hasExplain, hasSanitize].filter(
    Boolean,
  ).length;
  let overallScore;
