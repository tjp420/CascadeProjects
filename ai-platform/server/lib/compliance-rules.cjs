      ...
      findings: [
        {
          ...
          description: 'AI provider returned unstructured output — manual review required.',
        },
      ],
...
function buildDeterministicEvaluation(scanJson) {
  ...
  if (hasConfidence)
    findings.push({
      ...
      description: 'Repository contains references to confidence scoring or uncertainty metrics.',
    });
  else
    findings.push({
      severity: 'HIGH',
      pillar: 'Automation Bias Prevention',
      description: 'No confidence-score or uncertainty UI indicators detected in scanned files.',
    });
  ...
  if (hasAbort)
    ...
    findings.push({
      severity: 'HIGH',
      pillar: 'Intervention & Kill-Switch',
      description: 'No cancellation tokens, abort handlers, or emergency-stop functions detected.',
    });
  ...
  if (hasExplain)
    findings.push({
      ...
      description: 'Logging or metadata exports found that could support human review.',
    });
  else
    findings.push({
      severity: 'HIGH',
  ...
  if (hasSanitize)
    findings.push({
      ...
      description: 'Input-validation or sanitization patterns found in codebase.',
    });
  else
    findings.push({
      severity: 'HIGH',
