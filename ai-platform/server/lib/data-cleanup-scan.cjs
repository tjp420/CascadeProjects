...
async function isCachedResultFresh(payload, projectPath) {
  ...
  const envInconsistencies = findings.filter(
    (f) => f.type === "env-inconsistency",
  );
  ...
  for (const finding of envInconsistencies) {
    ...
    const uniqueCurrent = [...new Set(currentValues.values())].filter(
      (v) => v !== undefined,
    );
  ...
  for (const finding of dirBloat) {
    ...
    if (!dirRel) continue;
    const absDir = path.isAbsolute(dirRel)
      ? dirRel
      : path.join(projectPath, dirRel);
    try {
      ...
      if (
        ...
        finding.category === "Empty directory"
      ) {
...
const ALL_DATA_CLEANUP_SCANNER_IDS = [
  ...FILE_REDUCTION_SCANNER_IDS,
  ...DATA_QUALITY_SCANNER_IDS,
];
const DEFAULT_SCAN_CACHE_TTL_MS =
  Number(process.env.DATA_CLEANUP_SCAN_CACHE_TTL_MS) ||
  5 * constants.ONE_MINUTE_MS;
...
function resolveDataCleanupScannerConfig(profile, singleScanner) {
  if (singleScanner) {
    ...
    if (!ALL_DATA_CLEANUP_SCANNER_IDS.includes(id)) {
      throw new Error(
        `Unknown scanner "${id}". Valid: ${ALL_DATA_CLEANUP_SCANNER_IDS.join(", ")}`,
      );
    }
    return Object.fromEntries(
      ALL_DATA_CLEANUP_SCANNER_IDS.map((scannerId) => [
        scannerId,
        { enabled: scannerId === id },
      ]),
    );
  }
  ...
  return Object.fromEntries(
    ALL_DATA_CLEANUP_SCANNER_IDS.map((scannerId) => [
      scannerId,
      { enabled: enabledSet.has(scannerId) },
    ]),
  );
}
...
function resolveSafeProjectPath(rawPath, options = {}) {
  ...
  const allowedRoots =
    ...
    resolveDefaultAllowedRoots(options.baseDir, {
...
async function runDataCleanupScan(projectPath, options = {}) {
  const profile = String(
    options.profile || options.mode || "all",
  ).toLowerCase();
  const singleScanner = options.scanner ? String(options.scanner).trim() : "";
  const scanners = resolveDataCleanupScannerConfig(
    profile,
    singleScanner || undefined,
  );
  const startedAt = Date.now();
  const compact = options.compact !== false;
  const bypassCache = options.bypassCache === true;
  const cacheKey = buildScanCacheKey(projectPath, {
    profile,
  ...
  if (!bypassCache) {
    ...
    if (cached && cached.expiresAt > Date.now()) {
      ...
      if (fresh) {
        logger.info(
          `[analyze] data-cleanup cache hit path=${projectPath} profile=${profile}${singleScanner ? ` scanner=${singleScanner}` : ""}`,
        );
        return {
          ...cached.payload,
          cacheHit: true,
      ...
      logger.info(
        `[analyze] data-cleanup cache stale path=${projectPath} profile=${profile}${singleScanner ? ` scanner=${singleScanner}` : ""} — re-scanning`,
      );
  ...
  logger.info(
    `[analyze] data-cleanup start path=${projectPath} profile=${profile}${singleScanner ? ` scanner=${singleScanner}` : ""}`,
  );
  ...
  if (
    ...
    try {
      ...
      if (bloat?.findings?.directoryBloat?.length) {
        ...
        if (report.summary) {
          report.summary.directoryBloatFindings =
            bloat.summary.directoryBloatFindings;
          report.summary.directoryBloatReclaimableBytes =
            bloat.summary.directoryBloatReclaimableBytes;
        ...
        if (report.executiveSummary?.fileReduction) {
          report.executiveSummary.fileReduction.estimatedImmediateSavingsBytes =
            (report.executiveSummary.fileReduction
              .estimatedImmediateSavingsBytes || 0) +
            bloat.summary.directoryBloatReclaimableBytes;
        }
      }
    } catch (bloatErr) {
      logger.warn("[analyze] directory-bloat scan failed", {
        error: bloatErr.message,
      });
    }
  }
  ...
  const payload = compact
    ? compactDataCleanupReportForClient(enriched)
    : enriched;
  payload.durationMs = Date.now() - startedAt;
  logger.info(
    `[analyze] data-cleanup done path=${projectPath} profile=${profile} ms=${payload.durationMs} findings=${payload.summary?.totalFindings ?? "—"} compact=${compact}`,
  );
...
function registerDataCleanupAnalyzeRoute(app, options = {}) {
  ...
  const monorepoRoot =
    options.monorepoRoot || path.resolve(path.join(baseDir, ".."));
  ...
  async function handleDataCleanup(req, res) {
    try {
      ...
      const profile = String(
        req.query.profile || req.query.mode || "all",
      ).toLowerCase();
      const singleScanner = req.query.scanner
        ? String(req.query.scanner).trim()
        : "";
      const full = ["1", "true", "yes"].includes(
        String(
          req.query.full || req.query.compact === "0" ? "1" : "",
        ).toLowerCase(),
      );
      ...
      const report = await withTimeout(
        runDataCleanupScan(projectPath, {
          profile,
