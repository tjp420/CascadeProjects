...
const { resolveDefaultAllowedRoots, assertSafeProjectPath } = require('./path-safety.cjs');
...
const DEFAULT_SCAN_CACHE_TTL_MS =
  Number(process.env.DATA_CLEANUP_SCAN_CACHE_TTL_MS) || 5 * constants.ONE_MINUTE_MS;
      ...
      if (bloat?.findings?.directoryBloat?.length) {
        ...
        if (report.summary) {
          ...
          report.summary.totalFindings =
            (report.summary.totalFindings || 0) + bloat.summary.directoryBloatFindings;
          report.summary.reclaimableBytes =
            (report.summary.reclaimableBytes || 0) + bloat.summary.directoryBloatReclaimableBytes;
        }
        if (report.executiveSummary?.fileReduction) {
          report.executiveSummary.fileReduction.estimatedImmediateSavingsBytes =
            (report.executiveSummary.fileReduction.estimatedImmediateSavingsBytes || 0) +
            bloat.summary.directoryBloatReclaimableBytes;
        }
      }
    } catch (bloatErr) {
...
function registerDataCleanupAnalyzeRoute(app, options = {}) {
  ...
  const sendJson =
    typeof options.sendJson === 'function' ? options.sendJson : (res, payload) => res.json(payload);
  ...
  async function handleDataCleanup(req, res) {
    try {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ success: false, error: toClientError(error, 'Invalid projectPath') });
      }
      if (!projectPath) {
      ...
      const full = ['1', 'true', 'yes'].includes(
        String(req.query.full || req.query.compact === '0' ? '1' : '').toLowerCase()
      );
      ...
      return res
        .status(400)
        .json({ success: false, error: toClientError(error, 'Data cleanup analysis failed') });
    }
  }
