...
if (_disableRedis) {
  ...
  try {
    ...
    usingRedis = false;
    logger.warn(
      "admin-throttle: Redis initialization failed, using in-memory mode",
      {
...
async function _consumeFromRedis(bucketKey, consume, reserve) {
  ...
  try {
    ...
    logger.debug(
      "Redis hmget snapshot failed; proceeding with scripted consume",
      { error: e.message },
    );
  }
  const script = `
  ...
  try {
    ...
    try {
      ...
      logger.debug("Redis tokenBucketConsume failed; trying legacy EVAL", {
        error: err.message,
      });
    }
    ...
    try {
      ...
      if (redisClient && typeof redisClient.removeAllListeners === "function") {
        try {
          ...
          logger.warn("admin-throttle: removeAllListeners ready failed", {
            error: e && e.message,
          });
        }
        try {
          ...
          logger.warn("admin-throttle: removeAllListeners error failed", {
            error: e && e.message,
          });
        }
        try {
          ...
          logger.warn("admin-throttle: removeAllListeners close failed", {
            error: e && e.message,
          });
        }
      }
      ...
      try {
        ...
        logger.warn(
          "admin-throttle: inMemoryBuckets.clear failed during shutdown",
          { error: e && e.message },
        );
      }
      try {
        ...
        logger.warn(
          "admin-throttle: reset redisClient failed during shutdown",
          { error: e && e.message },
        );
      }
      usingRedis = false;
