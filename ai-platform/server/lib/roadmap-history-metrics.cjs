...
function extractRoadmapHistoryMetrics(roadmap) {
  ...
  const filesScanned =
    roadmap?.codeAnalysis?.structure?.totalFiles ?? roadmap?.projectStructure?.totalFiles ?? null;

  return {
    progressPercent: normalizeProgressPercent(progressRaw),
    filesScanned: filesScanned != null ? Number(filesScanned) : null,
    projectHealth:
      roadmap?.executiveSummary?.projectHealth ?? roadmap?.projectOverview?.projectHealth ?? null,
  };
}
