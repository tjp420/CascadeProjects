...
function extractRoadmapHistoryMetrics(roadmap) {
  ...
  const filesScanned =
    roadmap?.codeAnalysis?.structure?.totalFiles ??
...
function buildHistoryEntryFromRoadmap(roadmap, options = {}) {
  const { projectPath, title, scanOptions = {}, id, timestamp } = options;
  ...
  return {
    ...
    includePaths: Array.isArray(scanOptions.includePaths)
      ? scanOptions.includePaths
      : [],
    excludePatterns: Array.isArray(scanOptions.excludePatterns)
...
function isMisScopedFilesystemScan(roadmap) {
  ...
  const missedPlatformSignals =
    signals.serverEntry === true &&
    (signals.stubApi === false ||
      signals.pageSampleDir === false ||
      signals.phase2Auth === false);
