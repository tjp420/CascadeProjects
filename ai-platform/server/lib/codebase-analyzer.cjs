...
if (process.env.SIMPLEBEACON_DEBUG) {
  logger.debug(
    "[CodebaseAnalyzer] Module loaded: PATCHED v2.1 (excludes scanner files)",
  );
}
...
const _repoInventoryPath =
  "../../../packages/simplebeacon-cli/src/lib/repository-inventory";
const { countRepositoryInventory } = require(_repoInventoryPath);
const _platformRootPath =
  "../../../packages/simplebeacon-cli/src/project-detect";
...
const {
  getCodeExtensions,
  resolveScanProfile,
} = require("./universal-language-config.cjs");
const {
...
const _consolidationPath =
  "../../../packages/simplebeacon-cli/src/lib/consolidation-path-exclusions";
const { isConsolidationExcludedPair } = require(_consolidationPath);

const constants = require("../config/constants.cjs");
const { isBlank, isEmpty, ensureArray, capitalize, pluralize, truncate } =
  constants;
...
function summarizeFindings(findings) {
  if (!Array.isArray(findings))
    return { byCategory: {}, bySeverity: {}, byType: {}, total: 0 };
  const byCategory = {};
  const bySeverity = {};
  const byType = {};
...
const MAX_FINDINGS_COMPLETE =
  Number(process.env.CODEBASE_COMPLETE_MAX_FINDINGS) || 10000;
const MAX_DEEP_ANALYZE = Number.POSITIVE_INFINITY;
const MAX_DEEP_ANALYZE_DASHBOARD =
  Number(process.env.CODEBASE_DASHBOARD_MAX_FILES) || Number.POSITIVE_INFINITY;
const MAX_DEEP_ANALYZE_COMPLETE =
  Number(process.env.CODEBASE_COMPLETE_MAX_FILES) || Number.POSITIVE_INFINITY;
const ANALYZE_FILE_CONCURRENCY =
  Number(process.env.CODEBASE_ANALYZE_CONCURRENCY) || 24;
...
const COMMON_JS_KEYWORDS = new Set([
  "true",
  "false",
  "null",
  "undefined",
...
const EXCLUDED_UNSCOPED_PACKAGES = new Set([
...
const EXCLUDED_ANALYZER_PATHS = [
  ...
  /(?:^|\/)tmp_auth.*\.txt$/,
  /(?:^|\/)tmp_prompt.*\.txt$/,
  /(?:^|\/)tmp_extract.*\.(txt|json)$/,
  /(?:^|\/)tmp_index\.html$/,
  /(?:^|\/)tmp-/,
  /(?:^|\/)write-report.*\.mjs$/,
  /(?:^|\/)run\.mjs$/,
  /(?:^|\/)server-startup-(?:error|stderr|output)\.txt$/,
  /(?:^|\/)_fix_refusal\.js$/,
  /(?:^|\/)tmp-test-redact\.cjs$/,
  /(?:^|\/)\.vsix$/,
  /(?:^|\/)\.log$/,
  ...
  /(?:^|\/)orphan-audit-summary\.json$/,
  /(?:^|\/)remediation-roadmap.*\.json$/,
  /(?:^|\/)simplebeacon-report\.(html|json)$/,
  /(?:^|\/)report\.json$/,
  /(?:^|\/)test-tokens\.txt$/,
  /(?:^|\/)universal\.tokenkey$/,
  /(?:^|\/)__check_.*\.(mjs|js)$/,
  ...
  /(?:^|\/)tmp-ide-scroll\.mjs$/,
  /(?:^|\/)tmp-serve3\.mjs$/,
  /(?:^|\/)scan_trust_scripts\.js$/,
  /(?:^|\/)serve-codemap\d*\.js$/,
  /(?:^|\/)test_server\.js$/,
  /(?:^|\/)test_signin_route\.js$/,
  /(?:^|\/)test_url_parse\.js$/,
  /(?:^|\/)test_dashboard_route\.js$/,
  /(?:^|\/)verify-codemap\.js$/,
  /(?:^|\/)verify-deployment\.cjs$/,
  /(?:^|\/)test-jwt-rotation\.cjs$/,
  /(?:^|\/)SimpleBeaconPoC\.(js|mjs)$/,
  /(?:^|\/)local_dashboard_server\.js$/,
  /(?:^|\/)\.vscode-test\//,
  /(?:^|\/)\.wrangler\//,
  /(?:^|\/)\.windsurf\//,
  /(?:^|\/)\.cursor\//,
...
function resolveScanContext(options = {}) {
  return normalizeScanContext(
    options.context || options.scanContext || options.scanMode,
  );
}
...
function resolveDeepAnalyzeCap(options = {}, context = "cli") {
  if (
    options.maxDeepAnalyze != null &&
    Number.isFinite(Number(options.maxDeepAnalyze))
  ) {
...
async function analyzeFilesInBatches(files, rootDir, options = {}) {
  ...
  const concurrency = Math.max(
    1,
    options.concurrency || ANALYZE_FILE_CONCURRENCY,
  );
  ...
  async function processOne(file) {
    ...
    if (
      ...
      structureSamples.length < MAX_STRUCTURE_SAMPLES
    ) {
...
const FINDING_RUBRIC = {
  ...
  severityBands: {
    ...
    medium:
      "Should-fix quality issue with direct maintainability/runtime impact",
...
function isExcludedTechDebtLine(content, matchIndex) {
  ...
  if (/\*\s*@deprecated\b/.test(raw) || /@deprecated\s+use\b/i.test(raw))
    return true;
  if (/\[if deprecated/.test(line)) return true;
  if (
    /deprecatednarrative|rejectedfiction|legacy_skip_path|optionalobjectkeys.*deprecated/.test(
      line,
    )
  )
    return true;
  if (
    /deprecated marker|update deprecated|deprecated code analysis|previously returned hardcoded/.test(
      line,
    )
  )
    return true;
  if (
    /not implemented yet|not implemented in v\d|enterprise design claims not implemented/.test(
      line,
    )
  )
    return true;
  if (
    /oauth not configured|social sign-in requires oauth|use email login/.test(
      line,
    )
  )
    return true;
  if (
    ...
    /status|statuses|active|inactive|maintenance|colors|testing|'deprecated'\s*:/.test(
      line,
    )
  )
    return true;
  if (
    /todo\/fixme\/hack|todo\/fixme markers|get \/api\/backlog|isExcludedTechDebtLine|isRemediationContextLine|file-quality-heuristics|TECH_DEBT_MARKERS|contentNeedsValidation/.test(
      line,
    )
  )
    return true;
  if (/id:\s*['"]placeholder-todo['"]|id:\s*['"]placeholder-tbd['"]/.test(line))
    return true;
  if (/default_patterns|coming-soon-features|'pattern':\s*r'coming/.test(line))
    return true;
  if (
    /engineering debt marker|debt marker comment|marker comment found/.test(
      line,
    )
  )
    return true;
  if (
    /component render method not implemented|override in subclasses/.test(line)
  )
    return true;
  ...
  if (
    /normalizeFindingDescription|stub-not-implemented|deprecated-marker|fixme-marker|todo-marker|likely ai stub/.test(
      line,
    )
  )
    return true;
...
function isExcludedPatternCatalogLine(content, matchIndex) {
  ...
  if (
    /_PATTERNS\s*[=:\[]|PATTERNS\.(?:some|filter|find|map|forEach|length|push)/.test(
      normalized,
    )
  )
    return true;
  if (/\{\s*id:\s*['"]/.test(normalized) && /pattern:\s*\//.test(normalized))
    return true;
  if (/pattern:\s*\//.test(normalized) && /label:\s*['"]/.test(normalized))
    return true;
  // Skip analyzer catalog entries like { id: 'roadmap', label: 'Roadmap generation', category: 'Core Scans', desc: 'Tasks...' }
  if (
    /\{\s*id:\s*['"]/.test(normalized) &&
    /label:\s*['"]/.test(normalized) &&
    /(?:category|desc|description):\s*['"]/.test(normalized)
  )
    return true;
  // Skip analyzer detection logic where regexes test snippets/lines/content
  if (
    /\.test\s*\(\s*(?:snippet|line|content|text|raw|normalized|code|source|input|str|value)\s*\)/.test(
      normalized,
    )
  )
    return true;
...
const WELL_KNOWN_CONSTANTS = new Set([
  ...
  // time (ms)
  1000, 2000, 3000, 4000, 5000, 8000, 10000, 15000, 30000, 60000, 120000,
...
async function walkCodeFiles(rootDir, options = {}) {
  ...
  const codeExtensions =
    options.codeExtensions || getCodeExtensions(options.scanProfile);
  ...
  while (stack.length > 0) {
    ...
    if (depth > maxDepth) {
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(`[CodebaseAnalyzer] Skip (max depth): ${dir}`);
      continue;
    }
    if (results.length >= maxFiles) {
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(
          `[CodebaseAnalyzer] Skip (max files cap): ${results.length} files reached`,
        );
      break;
    }
    ...
    if (visited.has(realDir)) {
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(`[CodebaseAnalyzer] Skip (circular symlink): ${dir}`);
      continue;
    }
    visited.add(realDir);
    let entries;
    try {
      ...
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(`[CodebaseAnalyzer] Skip (unreadable dir): ${dir}`);
      continue;
    }
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (skipDirs.has(entry.name)) {
          if (process.env.SIMPLEBEACON_DEBUG)
            logger.debug(`[CodebaseAnalyzer] Skip (excluded dir): ${fullPath}`);
          continue;
        }
        ...
        try {
          ...
          if (stat.isDirectory()) {
            if (skipDirs.has(entry.name)) {
              if (process.env.SIMPLEBEACON_DEBUG)
                logger.debug(
                  `[CodebaseAnalyzer] Skip (excluded dir): ${fullPath}`,
                );
              continue;
            }
            stack.push({ dir: fullPath, depth: depth + 1 });
          ...
          if (process.env.SIMPLEBEACON_DEBUG)
            logger.debug(
              `[CodebaseAnalyzer] Skip (broken symlink): ${fullPath}`,
            );
          continue;
        }
      }
      if (!entry.isFile()) {
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(`[CodebaseAnalyzer] Skip (not a file): ${fullPath}`);
        continue;
      }
      ...
      const isArtifact =
        ARTIFACT_EXTENSIONS.some((suffix) => baseName.endsWith(suffix)) ||
      ...
      if (!isCode) {
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(
            `[CodebaseAnalyzer] Skip (unknown extension): ${fullPath} (${ext || "no ext"})`,
          );
        continue;
      }
      if (isGovernance || isBasenameMatch) {
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(
            `[CodebaseAnalyzer] Include (governance/basename): ${fullPath}`,
          );
      }
      ...
      if (shouldSkipLegacyExperimentalAnalysis(relativePath, options)) {
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(
            `[CodebaseAnalyzer] Skip (legacy/experimental): ${relativePath}`,
          );
        continue;
      }
      ...
      try {
        ...
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(
            `[CodebaseAnalyzer] Skip (unreadable file): ${fullPath}`,
          );
      }
    }
...
function isExcludedPythonMockProductionMatch(
  content,
  matchIndex,
  relativePath,
) {
  ...
  if (/__(tablename|table_name)__\s*=\s*['"]mock_/.test(normalized))
    return true;
  if (/data_source\s*=\s*['"]mock_/.test(normalized)) return true;
  if (/\bfrom\s+mock_[\w.]+\s+import\b/.test(normalized)) return true;
  if (
    /\bfrom\s+[\w.]+\s+import\b[\w.,\s]*mock_[\w.]+/.test(normalized) &&
    !/unittest\.mock/.test(normalized)
  )
    return true;
  if (
    /\bimport\s+[\w.,\s]*mock_[\w.]+/.test(normalized) &&
    !/unittest\.mock/.test(normalized)
  )
    return true;
  if (
    /\bmock_data_\w*/.test(normalized) &&
    !/\bunittest\.mock\b/.test(normalized)
  )
    return true;
  ...
  if (
    /['"]mock_[^'"]+['"]/.test(normalized) &&
    !/\bmock\.(patch|MagicMock|Mock)\b/.test(normalized)
  )
    return true;
  return false;
}
...
function isExcludedPlaceholderMatch(content, matchIndex) {
  ...
  if (
    /\.(?:monaco-)?(?:snippet-)?placeholder|finish-snippet-placeholder/.test(
      line,
    )
  )
    return true;
  if (/placeholder-token|UNIVERSAL_PLACEHOLDERS|scanContentPatterns/.test(line))
    return true;
  if (/^\s*#+\s+Placeholder\b/i.test(line)) return true;
  if (
    /::placeholder|:placeholder\b|\.placeholder\b|\bplaceholder\s*=/.test(line)
  )
    return true;
  if (/\{[a-z_]+\}.*placeholder|placeholder.*\{[a-z_]+\}/i.test(line))
    return true;
  return false;
}
...
function isExcludedRoadmapMarkerLine(content, matchIndex, relativePath) {
  ...
  if (
    /app\.(?:get|post|put|delete|use)\s*\(\s*['"`][^'"`]*(?:roadmap|milestone)/i.test(
      normalized,
    )
  )
    return true;
  ...
  if (
    /\b(?:require|import)\s*[(\s]['"`][^'"`]*(?:roadmap|RoadmapDataAnalyzer)/i.test(
      normalized,
    )
  )
    return true;
  ...
  if (/['"]\w*['"].*roadmap|roadmap.*['"]\w*['"]/i.test(normalized))
    return true;
  // Skip files whose purpose is roadmap-related by filename
  if (
    /roadmap|milestone|deliverable-access|export-tier|export-bundle|hygiene-certificate|snapshot-seeds|subscription-store|operator-deliverable|file-merger-reduction/i.test(
      relativePath,
    )
  )
    return true;
  ...
  // simplebeacon-ignore
  if (
    /TODO\|FIXME\|HACK\|XXX|regex:\s*['"].*TODO|roadmap.*marker|scan.*todo/i.test(
      normalized,
    )
  )
    return true;
...
function scanContentPatterns(
  ...
  if (
    isVendorBundledAssetPath(relativePath) &&
    (category === "meaningless-data" || category === "tech-debt")
  ) {
    return hits;
  }
  ...
  if (
    ...
    /intelligence-bridge\.js$/.test(relativePath)
  ) {
    return hits;
  }
  ...
  if (
    category === "eval-danger" &&
    /simplebeacon-vscode.*\/(?:realtimeMonitor|workspaceAnalyzer|enhancedAIProvider|enhancedDashboard2_0|findingConverter|remediationProvider)\.ts$/i.test(
      relativePath,
    )
  ) {
    return hits;
  ...
  if (
    (category === "eval-danger" || category === "inner-html-xss") &&
    /\/d3\.v\d+\.min\.js$|\.min\.js$|\.pack\.js$|\.bundle\.js$/.test(
      relativePath,
    )
  ) {
    return hits;
  ...
  for (const item of patterns) {
    ...
    const cacheKey =
      item.id + "|" + item.pattern.source + "|" + item.pattern.flags;
    let pattern = patternCache.get(cacheKey);
    if (!pattern) {
      try {
        pattern = new RegExp(
          item.pattern.source,
          ensureGlobalPatternFlags(item.pattern.flags),
        );
        patternCache.set(cacheKey, pattern);
      } catch (compileErr) {
        if (process.env.SIMPLEBEACON_DEBUG) {
          logger.warn(
            `[CodebaseAnalyzer] Skipping bad pattern ${item.id}: ${compileErr.message}`,
          );
        }
        continue;
      }
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (
        category === "tech-debt" &&
        isExcludedTechDebtLine(content, match.index)
      )
        continue;
      if (
        item.id === "python-mock-in-prod" ||
        item.id === "python-unittest-mock" ||
        item.id === "python-magic-mock" ||
        item.id === "python-mock-module-call"
      ) {
        if (
          isExcludedPythonMockProductionMatch(
            content,
            match.index,
            relativePath,
          )
      ...
      if (
        PLACEHOLDER_PATTERNS.some((p) => p.id === item.id) &&
        isExcludedPlaceholderMatch(content, match.index)
      )
        continue;
      if (
        category === "roadmap-marker" &&
        isExcludedRoadmapMarkerLine(content, match.index, relativePath)
      )
        continue;
      if (category === "missing-rate-limit") {
        ...
        if (
          /simplebeacon-dashboard/.test(relativePath) ||
          /(?:^|\/)coming-soon\//.test(relativePath)
        )
          continue;
        if (
          /server\/lib\/codebase-analyzer\.cjs$/.test(relativePath) ||
          /server\/lib\/file-audit-context\.cjs$/.test(relativePath)
        )
          continue;
        if (
          /server\/bootstrap\//.test(relativePath) ||
          /server\/index\.cjs$/.test(relativePath)
        )
          continue;
        // Skip scanner library, route, API, and server entry files where rate limiting is handled at a gateway/middleware layer
        if (
          /server\/lib\//.test(relativePath) ||
          /server\/routes\//.test(relativePath)
        )
          continue;
        if (/src\/api\//.test(relativePath)) continue;
        if (
          /simplebeacon-server\.cjs$/.test(relativePath) ||
          /tools\//.test(relativePath)
        )
          continue;
        ...
        const rllLineText = content.slice(
          rllLineStart,
          rllLineEnd === -1 ? undefined : rllLineEnd,
        );
        if (
          /(?:\/health|\/schema|\['"`]\/(?:health|schema)['"`]|['"`]\/['"`])/.test(
            rllLineText,
          )
        )
          continue;
        ...
        if (
          /rateLimit|throttle|[Ll]imiter|express-rate-limit|express-slow-down/.test(
            rllLineText,
          )
        )
          continue;
      ...
      if (category === "fix-preview") {
        ...
        if (
          ...
          continue;
        if (/server\/lib\/complete-scan-audit-report\.cjs$/.test(relativePath))
          continue;
        if (/server\/lib\/audit-remediation-recipes\.cjs$/.test(relativePath))
          continue;
      ...
      if (category === "token-bleed") {
        ...
        if (
          /server\/lib\//.test(relativePath) ||
          /server\/routes\//.test(relativePath) ||
          /server\/services\//.test(relativePath)
        )
          continue;
      }
      if (category === "sync-io") {
        ...
        if (
          ...
          /server\/dlp-dashboard\.cjs$/.test(relativePath) ||
          /server\/index\.cjs$/.test(relativePath)
        )
          continue;
      ...
      if (category === "magic-number") {
        ...
        if (
          /server\/config\/constants\.cjs$/.test(relativePath) ||
          /server\/bootstrap\//.test(relativePath)
        )
          continue;
        if (
          /packages\/[^/]+\/src\//.test(relativePath) ||
          /server\/ai-proxy-gateway\.cjs$/.test(relativePath)
        )
          continue;
        ...
        if (
          ...
          continue;
        if (
          /\btest-[\w-]+\.(js|cjs|mjs)$/.test(relativePath) ||
          /[\w-]-test\.(js|cjs|mjs)$/.test(relativePath)
        )
          continue;
        ...
        if (
          ...
          /auto-processor\.js$/.test(relativePath) ||
          /^simplebeacon-server\.cjs$/.test(relativePath)
        )
          continue;
        ...
        const mnLineText = content.slice(
          mnLineStart,
          mnLineEnd === -1 ? undefined : mnLineEnd,
        );
        if (/\b(?:const|let|var)\s+\w+\s*=\s*[^;]*\d{4,}/.test(mnLineText))
          continue;
      ...
      if (category === "config-drift") {
        ...
        if (
          /simplebeacon-dashboard/.test(relativePath) ||
          /simplebeacon-vscode/.test(relativePath) ||
          /src\/api\//.test(relativePath) ||
          /tests\//.test(relativePath)
        )
          continue;
        ...
        if (
          /\/rules\//.test(relativePath) ||
          /\/reporters\//.test(relativePath)
        )
          continue;
        if (
          /\/tests\//.test(relativePath) ||
        ...
        const cdLineText = content.slice(
          cdLineStart,
          cdLineEnd === -1 ? undefined : cdLineEnd,
        );
        // Skip version strings in report metadata (e.g., version: '1.0.0')
        if (/version\s*[:=]\s*['"`]\d+\.\d+\.\d+['"`]/.test(cdLineText))
          continue;
        // Skip DOM/JS toggle method calls (.toggle(), .toggleClass(), classList.toggle)
        if (/\.(?:classList)?\.?toggle(?:Class)?\s*\(/.test(cdLineText))
          continue;
        ...
        if (
          /\b(?:featureFlag|feature_flag|enableFeature|disableFeature)\b.*\b(?:pattern|regex|scanner|rule|catalog)\b/i.test(
            cdLineText,
          )
        )
          continue;
        ...
        if (
          /move hardcoded urls and secrets to \.env|Move hardcoded URLs and secrets to \.env/i.test(
            cdLineText,
          )
        )
          continue;
      ...
      if (category === "inner-html-xss") {
        ...
        if (
          /\b(?:escapeHtml|sanitizeHtml|htmlEscape|renderSafe|DOMPurify\.sanitize)\b/.test(
            mText,
          )
        )
          continue;
      ...
      if (category === "inner-html-xss") {
        ...
        if (
          /(?:^|\/)src\/(?:webviewPanel|web2Panel|enhancedDashboard|codeMapProvider)\.ts$/.test(
            relativePath,
          )
        )
          continue;
        if (/(?:^|\/)src\/(?:fixes|views|components)\//.test(relativePath))
          continue;
      }
      if (category === "build-readiness") {
        if (
          /^coming-soon\//.test(relativePath) ||
          /\btools\//.test(relativePath)
        )
          continue;
        ...
        if (
          /simplebeacon-dashboard/.test(relativePath) ||
          /\.browser\.(js|mjs|cjs)$/.test(relativePath)
        )
          continue;
      ...
      if (category === "logging-secrets") {
        ...
        if (
          /not\s+configured|missing|required|set\s+\w+|OFF\s*—\s*set|\*\*\*REDACTED\*\*\*|saved\s+to|file\s*:|path\s*:|===.*===|^\s*\/?\/?\s*(Test|Token|Auth|License)/i.test(
            lineText,
          )
        )
          continue;
        ...
        if (
          /console\.(log|warn|error)\s*\(\s*['`"](Report keys|Keys|Found|Total)/i.test(
            lineText,
          )
        )
          continue;
        ...
        if (
          /generate-license-token|Failed to reload API key|credentials needing review/i.test(
            lineText,
          )
        )
          continue;
        ...
        if (
          /console\.(warn|error|log)\s*\([^)]*(?:not\s+set|is\s+not|requires?|missing|JWT_SECRET|SIMPLEBEACON)/i.test(
            lineText,
          )
        )
          continue;
      ...
      if (category === "secret-in-comment") {
        ...
        if (
          /scanner-patterns|scanner-engine|pattern-documentation|codebase-analyzer/.test(
            relativePath,
          )
        )
          continue;
        ...
        const sicLineText = content.slice(
          sicLineStart,
          sicLineEnd === -1 ? undefined : sicLineEnd,
        );
        // Skip when it's describing a security pattern (not an actual secret)
        if (
          /\b(?:pattern|regex|scanner|detection|rule)\b.*\b(?:api[_-]?key|secret|token|password)/i.test(
            sicLineText,
          )
        )
          continue;
      ...
      if (category === "weak-cryptography") {
        ...
        if (
          /scanner-patterns|scanner-engine|codebase-analyzer/.test(relativePath)
        )
          continue;
        const wcLineStart = content.lastIndexOf("\n", match.index) + 1;
        const wcLineEnd = content.indexOf("\n", match.index);
        const wcLineText = content.slice(
          wcLineStart,
          wcLineEnd === -1 ? undefined : wcLineEnd,
        );
        // Skip when inside a comment describing the weakness
        if (
          /\/\/.*(?:weak|deprecated|do not use|avoid|legacy|insecure)/i.test(
            wcLineText,
          )
        )
          continue;
      ...
      if (category === "redos-risk") {
        ...
        const rdLineText = content.slice(
          rdLineStart,
          rdLineEnd === -1 ? undefined : rdLineEnd,
        );
        // Skip when regex is inside a string literal (not dynamically constructed)
        if (
          /['"`].*\(\[\^\]\]/i.test(rdLineText) &&
          !/new\s+RegExp/i.test(rdLineText)
        )
          continue;
      }
      if (category === "cicd-secret-exposure") {
        ...
        if (
          /scanner-patterns|scanner-engine|codebase-analyzer|\.test\./.test(
            relativePath,
          )
        )
          continue;
        ...
        const cicdLineText = content.slice(
          cicdLineStart,
          cicdLineEnd === -1 ? undefined : cicdLineEnd,
        );
        ...
        if (
          /example|placeholder|your_|my_|changeme|fake|dummy|test_|mock_/i.test(
            cicdLineText,
          )
        )
          continue;
      ...
      if (category === "committed-env-file") {
        ...
        if (
          /\.env\.(example|sample|template|local\.example)$/.test(relativePath)
        )
          continue;
      }
      if (category === "sensitive-data") {
        ...
        if (
          /[:=]\s*(?:get|resolve|generate|decode|create|build|fetch|load|read)[A-Z]\w+\s*\(/.test(
            lineText,
          )
        )
          continue;
        ...
        if (
          /['"`][^'"`]*(?:your-api-key-here|changeme|example|placeholder|sample|demo)['"`]/i.test(
            lineText,
          )
        )
          continue;
        ...
        if (
          /Exclude.*placeholder.*email|Exclude.*HTML placeholder/i.test(
            lineText,
          )
        )
          continue;
        ...
        if (
          /coming-soon\/js\/dashboard\/(?:pattern-documentation|quick-actions)\.js/.test(
            relativePath,
          )
        )
          continue;
      ...
      if (category === "sample-json-ref") {
        ...
        if (
          /src\/api\/simplebeacon-api\.cjs$/.test(relativePath) ||
          /simplebeacon-billing-api\.cjs$/.test(relativePath)
        )
          continue;
      ...
      if (category === "mock-path-leak") {
        ...
        if (/server\/lib\//.test(relativePath)) continue;
        if (/server\/routes\/flexible-analyze-api\.cjs$/.test(relativePath))
          continue;
        ...
        if (
          /sample-consistency-checker/.test(relativePath) ||
          /scan-conclusion/.test(relativePath)
        )
          continue;
        ...
        if (
          /Mock\/fixture\/sample data paths|mock\/fixture\/sample data references|mock\/fixture\/sample path references|development-only data paths|Blocks API keys/i.test(
            lineText,
          )
        )
          continue;
      ...
      if (
        category === "placeholder-fictional-data" &&
        ...
        if (/roadmap/i.test(relativePath) || /analyzer/i.test(relativePath))
          continue;
        // Skip common progress state values (0, 1, 100 are standard in progress tracking)
        const mVal = match[0].match(
          /(?:completionRate|completion|progress|done)\s*[:=]\s*['"`]?(\d+(?:\.\d+)?)/i,
        );
      ...
      if (category === "eval-danger") {
        ...
        const edLineText = content.slice(
          edLineStart,
          edLineEnd === -1 ? undefined : edLineEnd,
        );
        // Skip static require('literal-string') — standard Node.js import, not dynamic eval
        if (/\brequire\s*\(\s*['"`][^'"`]+['"`]\s*\)/.test(edLineText))
          continue;
        ...
        if (
          /['"`].*require\s*\(/.test(edLineText) ||
          /^\s*\/\//.test(edLineText)
        )
          continue;
        // Skip require(variable) when preceded by fs.existsSync(variable) in the same block
        if (/require\s*\(\s*\w+\s*\)/.test(edLineText)) {
          const prevBlock = content.slice(
            Math.max(0, edLineStart - 800),
            edLineStart,
          );
          const varName = (edLineText.match(/require\s*\(\s*(\w+)\s*\)/) ||
            [])[1];
          if (
            varName &&
            new RegExp("fs\\.existsSync\\s*\\(\\s*" + varName + "\\s*\\)").test(
              prevBlock,
            )
          )
            continue;
          ...
          if (
            /\btry\b[\s\S]{0,200}\bcatch(?:\s*\{|\s*\()/.test(
              prevBlock.slice(-400),
            )
          )
            continue;
        ...
        if (
          /\.(exec|match|test|search)\s*\(/.test(edLineText) &&
          !/eval\s*\(|new\s+Function/.test(edLineText)
        )
          continue;
        // Skip comments about eval safety patterns
        if (
          /^\s*\/\/.*(?:eval|exec|Function|is NOT eval|SAFE pattern)/.test(
            edLineText,
          )
        )
          continue;
      ...
      if (category === "dependency-vulns") {
        ...
        const dvLineText = content.slice(
          dvLineStart,
          dvLineEnd === -1 ? undefined : dvLineEnd,
        );
        ...
        if (
          /server\/config\//.test(relativePath) ||
          /server\/dlp-dashboard\.cjs$/.test(relativePath) ||
          /server\/lib\/trust-verification-payload\.cjs$/.test(relativePath)
        )
          continue;
        if (/src\/api\//.test(relativePath)) continue;
      ...
      if (
        category === "framework-practices" &&
        (/^coming-soon\//.test(relativePath) ||
          /simplebeacon-dashboard/.test(relativePath))
      )
        continue;
      if (category === "empty-stub-function") {
        if (/vendor\//.test(relativePath) || /\.min\.js$/i.test(relativePath))
          continue;
        if (
          /\b(?:docs|tests?)\//.test(relativePath) ||
          /\.(test|spec)\./i.test(relativePath)
        )
          continue;
        if (/^coming-soon\//.test(relativePath)) continue;
      ...
      if (category === "prototype-pollution") {
        ...
        if (
          item.id === "for-in-no-guard" &&
          /middleware\/security/.test(relativePath)
        )
          continue;
        // Skip __proto__ references in view files (prototype checks are common)
        if (item.id === "proto-in-key" && /\/views\//.test(relativePath))
          continue;
        ...
        const ppLineText = content.slice(
          ppLineStart,
          ppLineEnd === -1 ? undefined : ppLineEnd,
        );
        if (
          /^\s*\/\/.*(?:SAFE pattern|is the SAFE|hasOwnProperty\.call|Object\.prototype)/.test(
            ppLineText,
          )
        )
          continue;
      }
      if (category === "roadmap-marker" && /^coming-soon\//.test(relativePath))
        continue;
...
function isExcludedDebugLine(line) {
  ...
  if (/\.includes\s*\(\s*['"]console\.log['"]\s*\)/.test(normalized))
    return true;
  if (/includes\s*\(\s*['"]debugger['"]\s*\)/.test(normalized)) return true;
  if (/content\.includes\s*\(\s*['"]debugger['"]\s*\)/.test(normalized))
    return true;
  if (/content\.includes\s*\(\s*['"]console\.log['"]\s*\)/.test(normalized))
    return true;
  ...
  if (
    /DEBUG_PATTERNS|PLACEHOLDER_PATTERNS|TECH_DEBT_PATTERNS|debugger-statement/.test(
      normalized,
    )
  )
    return true;
  if (/pattern:\s*\//.test(normalized) && /\bdebugger\b/.test(normalized))
    return true;
  if (
    /pattern:\s*\//.test(normalized) &&
    /console\.(log|debug|info)\s*\(/.test(normalized)
  )
    return true;
  if (/\/[^/]*\bdebugger\b[^/]*\/[gimsuy]*/.test(normalized)) return true;
  if (
    /\bdebugger\s*statement\b/i.test(normalized) &&
    !/^\s*debugger\s*;?\s*$/.test(normalized)
  )
    return true;
  if (
    /Remove debugger|debug-artifact|debug logging \/ debugger|neutraliz|remediation|hardcoded-perfect|fiction.kpi/.test(
      normalized,
    )
  )
    return true;
  if (/console\.log statements found|console_statements/.test(normalized))
    return true;
  if (
    /no-debugger|no-console|app-logger|logger\.(debug|info|warn|error)\(/.test(
      normalized,
    )
  )
    return true;
  if (/process\.env\.[A-Z0-9_]*DEBUG|_DEBUG\s*===|LOG_LEVEL/.test(normalized))
    return true;
  if (/__resolveAppLogger|src\/lib\/app-logger/.test(normalized)) return true;
  if (/\bhasDebugger\b/.test(normalized)) return true;
  if (/if\s*\(\s*DEBUG\s*\)|if\s*\(\s*\w*DEBUG\w*\s*\)/.test(normalized))
    return true;
  if (/NODE_ENV\s*!==\s*['"]production['"]/.test(normalized)) return true;
  if (
    /isDebugScanPath|detectDebugArtifacts|isExcludedDebugLine|isExcludedPatternCatalogLine|calculateFileQuality|mapEslintRuleCategory/.test(
      normalized,
    )
  )
    return true;
...
function isExcludedDynamicEvalLine(line) {
  ...
  if (
    /Use of eval|eval\(\) function|code-injection|dynamic.eval|no-eval|security.*eval/.test(
      normalized,
    )
  )
    return true;
  ...
  if (/\brequire\s*\(\s*['"][A-Za-z0-9_-]+['"]\s*\)/.test(normalized))
    return true;
  return false;
}
...
function detectDynamicEval(content, relativePath) {
  ...
  if (/simplebeacon-dashboard/.test(rel) || /(?:^|\/)coming-soon\//.test(rel))
    return false;
  if (/\/(?:vendor|dist|build)\//.test(rel) || /\.min\.(js|cjs)$/.test(rel))
    return false;
  if (/\/(?:test|tests|__tests__)\//.test(rel) || /\.(test|spec)\./.test(rel))
    return false;
  ...
  if (/packages\/[^/]+\/src\//.test(rel) || /(?:^|\/)scripts\//.test(rel))
    return false;
  if (/(?:^|\/)ai-agent\//.test(rel) || /(?:^|\/)ai-tools\//.test(rel))
    return false;
  ...
  for (const line of content.split("\n")) {
    ...
    if (
      /\beval\s*\(|\bnew\s+Function\s*\(|\bFunction\s*\(|child_process\.(?:exec|execSync)\s*\(|vm\.(?:runInContext|runInNewContext|runInThisContext)\s*\(/.test(
        normalized,
      )
    )
      return true;
...
function isCliToolingPath(relativePath) {
  ...
  if (
    /(?:^|\/)packages\/[^/]+\/src\/(?:reporters|bin|scripts|tools)\//.test(rel)
  )
    return true;
  return false;
}
...
function shouldSkipSyntaxCheck(relativePath) {
  ...
  if (basename.includes("jest.setup") || basename.endsWith(".setup.js"))
    return true;
...
function isDebugScanPath(relativePath) {
  ...
  if (
    /^[a-z0-9_.-]+\/server\//.test(rel) ||
    /^[a-z0-9_.-]+\/packages\//.test(rel)
  )
    return true;
  return false;
}
...
function detectDebugArtifacts(content, relativePath) {
  ...
  if (
    !isDebugScanPath(relativePath) ||
    !isProductionRelevantPath(relativePath)
  ) {
    return [];
  }
  const findings = [];
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const hasConsole =
      /\bconsole\.(log|debug|info|trace)\s*\(/.test(line) &&
      !/\/[^/]*console\.(log|debug|info)[^/]*\//.test(line);
    if (!hasDebugger && !hasConsole) continue;

    const type = hasDebugger ? "debugger" : "console-log";
    const matchText = hasDebugger
      ? "debugger"
      : line.match(/\bconsole\.(log|debug|info|trace|warn)\s*\(/)?.[0] ||
        "console-log(";
...
function classifyPlaceholderSeverity(patternId, relativePath) {
  if (patternId === "fiction-kpi")
    return isProductionRelevantPath(relativePath) ? "medium" : "low";
  if (patternId === "hardcoded-perfect") return "medium";
  if (patternId === "hardcoded-completion") return "low";
  if (patternId === "coming-soon")
    return isProductionRelevantPath(relativePath) ? "medium" : "low";
  if (patternId === "tbd")
    return isProductionRelevantPath(relativePath) ? "medium" : "low";
  return "low";
...
function detectApiContractIssues(content, relativePath) {
  ...
  if (
    /server\/lib\/codebase-analyzer\.cjs$/.test(rel) ||
    /server\/lib\/file-audit-context\.cjs$/.test(rel)
  )
    return [];
  const hits = [];
  const seen = new Set();
...
function detectArrowStubs(content, relativePath) {
  ...
  const seen = new Set();
  const pattern =
    /(?:const|let|var|export\s+(?:const|let|var))\s+\w+\s*=\s*(?:async\s+)?\([^)]*\)\s*=>\s*(?:\{\s*\}|\{\s*return\s+(?:null|undefined|0|''|""|\[\]|true|false|\{\s*\});?\s*\}|\(\s*\{\s*\}\s*\)|null|undefined|0|''|""|\[\]|true|false|\{\s*\})/g;
  let match;
  while ((match = pattern.exec(content)) !== null) {
    if (isExcludedPatternCatalogLine(content, match.index)) continue;
    const line = lineNumberAt(content, match.index);
    ...
    hits.push({
      ...
      line,
      description:
        "Arrow function with empty or trivial return — likely AI stub. Implement return value.",
      match: matchText,
...
function detectEmptyStubFunctions(content, relativePath) {
  ...
  const seen = new Set();
  const pattern =
    /function\s+\w+\s*\([^)]*\)\s*\{\s*(?:return\s+(?:null|undefined|\{\}|\[\]|0|''|"")?;?\s*)?\}/g;
  let match;
  while ((match = pattern.exec(content)) !== null) {
    if (isExcludedPatternCatalogLine(content, match.index)) continue;
    const line = lineNumberAt(content, match.index);
    ...
    hits.push({
      ...
      line,
      description:
        "Empty or trivial function body — likely unimplemented stub.",
      match: matchText,
...
function detectFixPreviewIssues(content, relativePath) {
  ...
  if (
    /vendor\//.test(rel) ||
    /\.min\.js$/i.test(rel) ||
    /\.browser\.js$/i.test(rel)
  )
    return [];
  if (
    /^coming-soon\/(js\/dashboard\/|app-links\.js|contact\.js|site-config\.js)/.test(
      rel,
    )
  )
    return [];
  ...
  for (const item of FIX_PREVIEW_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      const fpLineText = content.slice(
        fpLineStart,
        fpLineEnd === -1 ? undefined : fpLineEnd,
      );
      ...
      if (item.id === "var-declaration") {
        const inStringLiteral = /['"`][^'"`]*\bvar\b[^'"`]*['"`]/.test(
          fpLineText,
        );
        const inComment =
          /^\s*\/\//.test(fpLineText) || /^\s*\/\*/.test(fpLineText);
        const inIIFE =
          /\(function\s*\(/.test(fpLineText) || /window\./.test(fpLineText);
...
function detectMissingStrictMode(content, relativePath) {
  ...
  if (
    ...
    /\/js\/(utils|components|views)\//.test(rel)
  )
    return [];
  if (
    /\.(config|test|spec)\./.test(rel) ||
    /jest\.config|eslint\.config|babel\.config/.test(rel)
  )
    return [];
  ...
  if (
    /(^|\/)(?:docs|templates|scripts|tools|fixtures|mocks|samples|tests)\//.test(
      rel,
    )
  )
    return [];
  ...
  if (/^server\//.test(rel) || /^src\//.test(rel) || /^packages\//.test(rel))
    return [];
  ...
  if (
    ...
    /^ai-agent\//.test(rel) ||
    /^ai-tools\//.test(rel) ||
    /^simplebeacon-frameworkless\//.test(rel)
  )
    return [];
  // Skip root-level server entry files and web data injection scripts
  if (/^simplebeacon-server\.js$/.test(rel) || /^web\/data\//.test(rel))
    return [];
  ...
  if (/outreach-prospects\.js$/.test(rel) || /site-config\.js$/.test(rel))
    return [];
  if (
    /test-(certificate|normalize|renderPreview|technical-audit|roadmap|zip-data|all-patterns|generate-roadmap)\.js$/.test(
      rel,
    )
  )
    return [];
  ...
  if (
    /^\s*\/\/?\s*SPDX-License-Identifier:/m.test(content) ||
    /^\s*\/\*\s*(?:MIT|Apache|GPL|BSD|Mozilla|ISC)\s+License/m.test(content) ||
    /^\s*\/\/\s*(?:MIT|Apache|GPL|BSD|Mozilla|ISC)\s+License/m.test(content)
  )
    return [];
  return [
...
function detectPerformanceIssues(content, relativePath) {
  ...
  if (/server\/dlp-dashboard\.cjs$/.test(rel) || /web\/data\//.test(rel))
    return [];
  ...
  for (const item of PERFORMANCE_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText.slice(0, 80),
        recommendedAction:
          "Refactor to avoid blocking or O(n²) operations in production paths",
      });
    }
  }
  return hits;
...
function detectSyncIoIssues(content, relativePath) {
  ...
  if (
    /server\/index\.cjs$/.test(rel) ||
    /server\/utils\//.test(rel) ||
    /auto-processor\.js$/.test(rel)
  )
    return [];
  // Skip simplebeacon-server, DLP dashboard, and package source files where sync I/O is standard
  if (
    /^simplebeacon-server\.(cjs|js)$/.test(rel) ||
    /server\/dlp-dashboard\.cjs$/.test(rel) ||
    /packages\/[^/]+\/src\//.test(rel)
  )
    return [];
  ...
  for (const item of SYNC_IO_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Replace synchronous file I/O with async equivalents (fs.promises or callbacks)",
      });
    }
  }
  return hits;
...
function isExcludedPrototypePollutionLine(content, matchIndex) {
  ...
  if (
    /===\s*['"]__proto__['"]\s*\|\|\s*key\s*===|!==\s*['"]__proto__['"]|hasOwnProperty\s*\(\s*['"]__proto__['"]|\.hasOwnProperty\s*\(/.test(
      normalized,
    )
  )
    return true; // simplebeacon:audit-ignore:prototype-pollution
  if (/Object\.create\s*\(\s*null\s*\)/.test(normalized)) return true; // simplebeacon:audit-ignore:prototype-pollution
  if (
    /['"]__proto__['"]\s*===\s*key|key\s*!==\s*['"]__proto__['"]/.test(
      normalized,
    )
  )
    return true; // simplebeacon:audit-ignore:prototype-pollution
...
function detectPrototypePollution(content, relativePath) {
  ...
  for (const item of PROTOTYPE_POLLUTION_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      seen.add(dedupeKey);
      const severity =
        item.id === "for-in-no-guard" || item.id === "object-assign-untrusted"
          ? "medium"
          : "high";
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Avoid assigning to __proto__ or dynamic prototype properties; use Object.create(null) or Map instead",
      });
    }
  }
  return hits;
...
function detectSampleJsonRef(content, relativePath) {
  ...
  // Skip API route files where sample JSON references are demo endpoints
  if (
    /src\/api\/simplebeacon-api\.cjs$/.test(rel) ||
    /simplebeacon-billing-api\.cjs$/.test(rel)
  )
    return [];
  // Skip test files — they legitimately reference sample JSON fixtures
  if (/\.(test|spec)\.(js|cjs|mjs|ts)$/.test(rel) || /\/tests?\//.test(rel))
    return [];
  ...
  if (
    /(?:\/|^)(?:page-sample-specs|sample-consistency-checker|sample-path-resolver|sample-path-matcher)\.js$/.test(
      rel,
    )
  )
    return [];
  if (/(?:\/|^)(?:data-lineage-analyzer|unused-file-detector)\.js$/.test(rel))
    return [];
  if (
    /(?:\/|^)(?:scan|project-detect|marketing-content-generator|remediation-guides)\.js$/.test(
      rel,
    )
  )
    return [];
  ...
  for (const item of SAMPLE_JSON_REF_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (
        /not\s+product|not\s+a?\s*sample|warning|avoid|fake|fictional/.test(
          lineText,
        )
      )
        continue;
      ...
      hits.push({
        ...
        match: matchText.slice(0, 80),
        recommendedAction:
          "Replace sample JSON reference with production data source",
      });
    }
  }
  return hits;
...
function detectSecurityHeaders(content, relativePath) {
  ...
  if (
    /(?:^|\/)packages\/simplebeacon-cli\/src\/(?:rules|analyzers|reporters|lib)\//.test(
      rel,
    )
  )
    return [];
  ...
  if (/(?:^|\/)New folder\/|complete-scan\.json$|\.simplebeacon\//.test(rel))
    return [];
  // Skip bootstrap/integration files where security headers are intentionally configured
  if (/server\/bootstrap\//.test(rel) || /phase\d+-integration\.cjs$/.test(rel))
    return [];
  ...
  if (
    relativePath
      .replace(/\\/g, "/")
      .endsWith("server/lib/codebase-analyzer.cjs")
  )
    return [];
  // Skip dashboard service files containing pattern catalogs and license matchers
  if (
    /simplebeacon-dashboard\/js\/services\/aiProblemAnalyzerSuite\.mjs$/.test(
      rel,
    )
  )
    return [];
  ...
  for (const item of UNVALIDATED_REDIRECT_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Validate redirect targets against an allowlist before calling res.redirect()",
      });
    }
  }
  return hits;
...
function detectUninitializedRead(content, relativePath) {
  ...
  if (
    /simplebeacon-dashboard/.test(rel) ||
    /\.browser\.(js|cjs)$/.test(rel) ||
    /\.min\.(js|cjs)$/.test(rel)
  )
    return [];
  if (
    /(^|\/)(?:docs|templates|scripts|tools|fixtures|mocks|samples)\//.test(rel)
  )
    return [];
  ...
  return [
    {
      ...
      match: matches[0],
      recommendedAction:
        "Initialize variables at declaration or ensure assignment before first read",
    },
  ];
}
...
function detectTokenBleed(content, relativePath) {
  ...
  if (
    /server\/lib\//.test(rel) ||
    /server\/routes\//.test(rel) ||
    /server\/services\//.test(rel)
  )
    return [];
  ...
  if (/packages\/[^/]+\/tests\//.test(rel) || /\.(test|spec)\./.test(rel))
    return [];
  ...
  for (const item of TOKEN_BLEED_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText.slice(0, 80),
        recommendedAction:
          "Break up very long string literals or load large content from external files",
      });
    }
  }
  return hits;
...
function detectUnhandledPromise(content, relativePath) {
  ...
  if (
    /packages\/[^/]+\/src\/(mcp|proxy|scan)\//.test(rel) ||
    /packages\/[^/]+\/src\/scan\.js$/.test(rel)
  )
    return [];
  ...
  for (const item of UNHANDLED_PROMISE_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText.slice(0, 80),
        recommendedAction:
          "Add .catch() or wrap in try/catch to handle promise rejections",
      });
    }
  }
  return hits;
...
function detectWorkspaceHealth(content, relativePath) {
  ...
  if (
    /(^|\/)server\/|(^|\/)src\/api\/|(^|\/)tools\/|(^|\/)web\/simplebeacon-dashboard\/|(^|\/)coming-soon\/|(^|\/)packages\/simplebeacon-cli\//.test(
      relativePath,
    )
  )
    return [];
  if (
    /\.(test|spec)\.(js|cjs|mjs|ts)|patch-strategies|scanner-engine|scanner-patterns/.test(
      relativePath,
    )
  )
    return [];
  ...
  for (const item of WORKSPACE_HEALTH_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Review module structure to avoid circular imports and barrel-file anti-patterns",
      });
    }
  }
  return hits;
...
function detectAiIndicators(content, relativePath) {
  ...
  const basename = rel.split("/").pop() || "";
  if (
    /(^|[-_.])(?:ai|model|ollama|semantic|inference|chatbot|llm|gpt|claude|openai|anthropic|langchain|huggingface|vertex)([-_.]|$)/i.test(
      basename,
    )
  )
    return [];
  if (
    /\/(?:services|routes|lib)\/(?:ai-|model-|ollama-|semantic-|inference-|chatbot-)/i.test(
      rel,
    )
  )
    return [];
  if (
    /\/ai[-_]/.test(rel) ||
    /\/(?:ai-proxy-gateway|cloud-inference-service|model-inference-service|local-model-service|ollama-client|chatbot-api|local-models-api)/i.test(
      rel,
    )
  )
    return [];
  ...
  for (const item of AI_INDICATORS_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (
        /require\s*\(\s*['"`].*(?:model-inference-service|local-model-service|cloud-inference-service|ai-proxy-gateway|ai-analyst|ollama-client|local-models-api|chatbot-api|semantic-analyzer|strategic-insights-engine)/.test(
          lineStr,
        )
      )
        continue;
      if (
        /import\s+.*\s+from\s+['"`].*(?:model-inference-service|local-model-service|cloud-inference-service|ai-proxy-gateway)/.test(
          lineStr,
        )
      )
        continue;
      ...
      if (
        item.id === "ai-inference-service" &&
        /\b(?:model-inference-service|local-model-service|cloud-inference-service|ai-proxy-gateway)\b/.test(
          fullMatch,
        )
      )
        continue;
      ...
      if (
        /\b(?:ollamaBaseUrl|DEFAULT_OLLAMA_URL|ollamaModel)\b/.test(fullMatch)
      )
        continue;
      const line = lineNumberAt(content, match.index);
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Review EU AI Act Article 6 applicability for AI SDK usage",
      });
    }
  }
  return hits;
...
function detectUnusedDeps(content, relativePath) {
  ...
  for (const item of UNUSED_DEPS_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (/\/(?:api|routes)\//.test(rel) && /controller/i.test(varName))
        continue;
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Remove unused dependency or verify it is required indirectly",
      });
    }
  }
  return hits;
  ...
  for (const item of I18N_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Wrap hardcoded UI strings with i18n function or translation key",
      });
    }
  }
  return hits;
  ...
  for (const item of COMPLEXITY_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Extract helper functions and reduce nesting with early returns",
      });
    }
  }
  return hits;
...
function detectDatabasePatterns(content, relativePath) {
  ...
  const isReportGeneration =
    /\/(?:audit-remediation-recipes|scan-report-patch|report-builder|audit-export)\.cjs$/i.test(
      rel,
    );
  const isDashboardUtility =
    /\/simplebeacon-dashboard\/(?:js-es2018\/|js\/)(?:utils|components|views|services|lib)\//i.test(
      rel,
    );
  ...
  for (const item of patterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (isReportGeneration || isDashboardUtility || isBrowserFile) {
        ...
        if (
          item.id === "sql-template-injection" ||
          item.id === "unbounded-select"
        )
          continue;
      }
      ...
      if (item.id === "unparameterized-query") {
        if (
          /SELECT\s+1\s+AS|SELECT\s+CURRENT_TIMESTAMP|SELECT\s+version\(\)/i.test(
            match[0],
          )
        )
          continue;
        if (/\b(?:BEGIN|COMMIT|ROLLBACK|SAVEPOINT|RELEASE)\b/i.test(match[0]))
          continue;
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Use parameterized queries and add pagination limits",
      });
    }
  }
  return hits;
...
function detectLicenseHeaders(content, relativePath) {
  const hits = [];
  const seen = new Set();
  const isSourceFile = /\.(js|mjs|cjs|ts|tsx|jsx|vue|svelte)$/i.test(
    relativePath,
  );
  ...
  for (const item of LICENSE_HEADER_PATTERNS) {
    ...
    if (
      /eslint\.config\.(js|cjs|mjs)$/.test(relativePath) ||
      /jest\.config\.(cjs|js|mjs)$/.test(relativePath)
    )
      continue;
    // Skip VS Code extension and CLI rule files where license patterns are definitions
    if (
      /(?:^|\/)simplebeacon-vscode\/src\//.test(relativePath) ||
      /(?:^|\/)packages\/simplebeacon-cli\/src\/(?:rules|analyzers|lib)\//.test(
        relativePath,
      )
    )
      continue;
    ...
    if (
      /(?:^|\/)ai-agent\//.test(relativePath) ||
      /(?:^|\/)ai-platform\/packages\/simplebeacon-intelligence\/src\//.test(
        relativePath,
      )
    )
      continue;
  ...
  for (const item of MARKDOWN_FENCE_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText.slice(0, 80),
        recommendedAction:
          "Remove leaked markdown code fences from source files",
      });
    }
  }
  return hits;
...
function detectLlmSlop(content, relativePath) {
  ...
  if (/simplebeacon-dashboard/.test(rel) || /server\/lib\//.test(rel))
    return [];
  const hits = [];
  const seen = new Set();
  for (const item of LLM_SLOP_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      hits.push({
        ...
        match: matchText,
        recommendedAction:
          "Replace AI-generated slop with real content or verified data",
      });
    }
  }
  return hits;
  ...
  for (const item of PLACEHOLDER_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (item.id === "ai-placeholder-block") {
        ...
        if (
          /\b(?:unavailable|dev\s+setups?|platform|configuration|config|setup|warning|caution|note)\b/i.test(
            blockText,
          )
        )
          continue;
...
function mapEslintRuleCategory(ruleId) {
  ...
  if (id.includes("no-console") || id.includes("no-debugger"))
    return "debug-hygiene";
  if (id.startsWith("security/") || id.includes("security")) return "security";
  if (
    id.includes("no-undef") ||
    id.includes("no-unreachable") ||
    id.includes("eqeqeq")
  )
    return "correctness";
  if (
...
function buildEslintSummary(messages, totals = {}) {
  ...
  const topRules = [...byRule.entries()]
    ...
    .map(([ruleId, count]) => ({
      ruleId,
      count,
      category: mapEslintRuleCategory(ruleId),
    }));
    ...
    for (const relPath of ESLINT_REPORT_CANDIDATES) {
      ...
      try {
        ...
        for (const fileReport of reportItems) {
          ...
          const fileMessages = normalizeEslintMessages(
            scanRoot,
            fileReport.filePath || "",
            fileReport.messages || [],
          );
...
async function analyzeFileContent(file, rootDir, options = {}) {
  ...
  if (file.isArtifact || rel.includes("security-reports/fixes/")) {
    ...
    if (
      /\b(logs|cache|tmp|temp|uploads|downloads|dist|build|coverage)\//.test(
        rel,
      ) ||
      /simplebeacon-dashboard\//.test(rel) ||
      /^[\w-]+\.log$/i.test(rel)
  ...
  if (file.size === 0) {
    pushFinding(findings, {
      ...
      recommendedAction:
        "Delete or populate — empty files add noise to the tree",
    });
    return finalizeFileAnalysis(findings, rel, structure);
  }
  ...
  if (file.ext === ".json") {
    ...
    const isConfigJson =
      file.name === "package.json" || file.name === "tsconfig.json";
    if (!isConfigJson) {
      try {
        JSON.parse(content);
      } catch (error) {
  ...
  if (
    /(?:^|[\\/])\.env$/.test(rel) &&
    !/\.env\.(example|sample|template|local\.example)$/.test(rel)
  ) {
    findings.push({
      ...
      line: 1,
      description:
        ".env file committed to repository — environment secrets may be exposed",
      match: rel.split(/[\\/]/).pop(),
      recommendedAction:
  ...
  if (
    [".c", ".cpp", ".cc", ".cxx", ".h", ".hpp", ".hh", ".hxx"].includes(
      file.ext,
    )
  ) {
    findings.push(
...
function detectTestCoverage(files) {
  ...
  for (const file of files) {
    ...
    if (
      ["config.json", "package.json", "tsconfig.json", "readme.md"].includes(
        name,
      )
    )
      continue;
    ...
    if (
      ...
      continue;
    if (
      /\.(config|test|spec)\./.test(name) ||
      /jest\.config|eslint\.config|babel\.config|webpack\.config/.test(name)
    )
      continue;
    if (
      /(?:^|\/)(?:docs|templates|scripts|tools|fixtures|mocks|samples|config|bootstrap|bin)\//.test(
        rel,
      )
    )
      continue;
    ...
    if (
      /-dashboard\./.test(name) ||
      /[-_]gateway\./.test(name) ||
      /enterprise[-_]/.test(name)
    )
      continue;
    if (/orchestrator\./.test(name) || /auto[-_]processor\./.test(name))
      continue;
    ...
    for (const entry of entries) {
      ...
      if (children.length === 0) {
        findings.push({
          ...
          recommendedAction:
            "Remove empty directory or document its purpose if still needed",
        });
      }
    }
  }
...
function classifyFindingTier(filePath) {
  ...
  if (
    DOCUMENTATION_PATH_PREFIXES.some(
...
function sortFindingsForReport(findings = []) {
  ...
  return [...findings].sort((a, b) => {
    const tierDelta =
      (tierRank[classifyFindingTier(a.filePath)] ?? 9) -
...
function aggregateCategories(findings = []) {
  ...
  for (const finding of findings) {
    ...
    else if (finding.severity === "medium" && bucket.severity !== "high")
...
function computeHealthScore(findings, codeFilesAnalyzed) {
  ...
  function tierDeduction(tierFindings, weight, maxDeduction) {
    ...
    const normalized = Math.min(
      100,
      Math.round((penalty / Math.max(codeFilesAnalyzed, 1)) * 120),
    );
    return Math.min(maxDeduction, normalized);
  }

  const deduction =
    tierDeduction(byTier.production, 1, 70) +
  ...
  try {
    const { stdout } = await execFileAsync(
      ...
      ],
      {
        ...
        timeout: 120000,
        encoding: "utf8",
        maxBuffer: 8 * constants.BYTES_PER_KB * constants.BYTES_PER_KB,
      },
    );
    ...
    for (const fileReport of reports) {
      ...
      const normalized = normalizeEslintMessages(
        projectRoot,
        fileReport.filePath,
        fileReport.messages || [],
      );
    ...
    if (error.stdout) {
      try {
        ...
        return {
          ...
          errors,
          warnings,
          filesWithIssues: reports.filter(
            (r) => (r.errorCount || 0) + (r.warningCount || 0) > 0,
          ).length,
          findings: [],
          summary: buildEslintSummary([], {
            source: "command",
...
async function analyzeCodebase(baseDir, options = {}) {
  ...
  const [repositoryInventory, files] = await Promise.all([
    countRepositoryInventory(scanRoot, {
      profile: opts.inventoryProfile || "audit",
    }),
  ...
  if (process.env.SIMPLEBEACON_DEBUG)
    logger.debug(
      `[CodebaseAnalyzer] Governance counts: license=${governanceCounts.licenseCount}, security=${governanceCounts.securityCount}, package.json=${governanceCounts.packageJsonCount}`,
    );
  ...
  const nodeModulesFiltered = files.filter((f) => {
    ...
    if (isNodeModules) {
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(
          `[CodebaseAnalyzer] Skip (node_modules): ${f.relativePath}`,
        );
    }
  ...
  if (nodeModulesFiltered.length > deepAnalyzeCap) {
    if (process.env.SIMPLEBEACON_DEBUG)
      logger.debug(
        `[CodebaseAnalyzer] Skip (deepAnalyzeCap): ${nodeModulesFiltered.length - deepAnalyzeCap} files truncated (cap: ${deepAnalyzeCap})`,
      );
  }
  ...
  if (analyzerFilesExcluded.length < nodeModulesFiltered.length) {
    if (process.env.SIMPLEBEACON_DEBUG)
      logger.debug(
        `[CodebaseAnalyzer] Skip (analyzer paths): ${nodeModulesFiltered.length - analyzerFilesExcluded.length} scanner/utility/subproject files excluded`,
      );
  } else {
    if (process.env.SIMPLEBEACON_DEBUG)
      logger.debug(
        "[CodebaseAnalyzer] No analyzer paths excluded (check if exclusions are needed)",
      );
  }
  const filesToAnalyze = analyzerFilesExcluded.slice(0, deepAnalyzeCap);

  const { findings, structureSamples } = await analyzeFilesInBatches(
    filesToAnalyze,
    codeWalkRoot,
    {
      concurrency:
        context === "complete"
          ? Number(process.env.CODEBASE_COMPLETE_CONCURRENCY) ||
            Math.max(ANALYZE_FILE_CONCURRENCY, 24)
          : opts.concurrency,
  ...
  let eslintSummary = buildEslintSummary([], {
    source: "none",
    errors: 0,
    warnings: 0,
    filesWithIssues: 0,
  ...
  if (includeEslint) {
    ...
    const artifactOnly = await loadEslintReportFromDisk(
      scanRoot,
      platformRoot || scanRoot,
    );
    if (artifactOnly) {
      ...
      eslintSummary = buildEslintSummary(
        artifactOnly.messages || [],
        artifactOnly,
      );
      ...
      eslintSkipped =
        "ESLint command disabled by default in request path; set includeEslint=true to run CLI scan.";
    }
  }
  ...
  if (process.env.SIMPLEBEACON_DEBUG)
    logger.debug(
      `[CodebaseAnalyzer] Scan summary: discovered=${files.length}, analyzed=${codeFilesAnalyzed}, cap=${deepAnalyzeCap}, findings=${findings.length}`,
    );
  ...
  return {
    ...
    summary: {
      ...
      analyzerCounts: {
        debugArtifacts: findings.filter((f) => f.category === "debug-artifact")
          .length,
        placeholderOrFictionalData: findings.filter(
          (f) => f.analyzer === "placeholder-fictional-data-analyzer",
        ).length,
    ...
    scanScope: {
      mode:
        context === "complete" ? "codebase-audit-complete" : "codebase-audit",
      scanProfile,
      scanContext: context,
      universalLanguageCount: languagePluginManager.listLanguages().length,
      dedicatedLanguagePlugins: [
        "zscript",
        "acs",
        "decorate",
        "glsl",
