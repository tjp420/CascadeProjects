...
const { isBlank, isEmpty, ensureArray, capitalize, pluralize, truncate } = constants;
...
async function analyzeFilesInBatches(files, rootDir, options = {}) {
  ...
  for (let offset = 0; offset < files.length; offset += concurrency) {
    ...
    const results = await Promise.all(
      batch.map((file) => analyzeFileContent(file, rootDir, { ...options, findingsCap: cap }))
    );
...
function isExcludedTechDebtLine(content, matchIndex) {
  ...
  if (
    /deprecatednarrative|rejectedfiction|legacy_skip_path|optionalobjectkeys.*deprecated/.test(line)
  )
    return true;
  if (
    ...
    return true;
  if (
    /not implemented yet|not implemented in v\d|enterprise design claims not implemented/.test(line)
  )
    return true;
  ...
  if (
    ...
    /status|statuses|active|inactive|maintenance|colors|testing|'deprecated'\s*:/.test(line)
  )
    return true;
  if (
...
function isExcludedPatternCatalogLine(content, matchIndex) {
  ...
  if (/_PATTERNS\s*[=:\[]|PATTERNS\.(?:some|filter|find|map|forEach|length|push)/.test(normalized))
    return true;
...
const WELL_KNOWN_CONSTANTS = new Set([
  ...
  // time (ms)
  1000, 2000, 3000, 4000, 5000, 8000, 10000, 15000, 30000, 60000, 120000, 180000, 300000, 600000,
  900000,
  // time conversions
...
async function walkCodeFiles(rootDir, options = {}) {
  ...
  while (stack.length > 0) {
    ...
    if (results.length >= maxFiles) {
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(`[CodebaseAnalyzer] Skip (max files cap): ${results.length} files reached`);
      break;
    }
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        try {
          ...
          if (process.env.SIMPLEBEACON_DEBUG)
            logger.debug(`[CodebaseAnalyzer] Skip (broken symlink): ${fullPath}`);
          continue;
        }
      }
      if (!entry.isFile()) {
      ...
      if (isGovernance || isBasenameMatch) {
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(`[CodebaseAnalyzer] Include (governance/basename): ${fullPath}`);
      }

      const relativePath = normalizeRelativePath(rootDir, fullPath);
      if (shouldSkipLegacyExperimentalAnalysis(relativePath, options)) {
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(`[CodebaseAnalyzer] Skip (legacy/experimental): ${relativePath}`);
        continue;
      }
      ...
      try {
        ...
        if (process.env.SIMPLEBEACON_DEBUG)
          logger.debug(`[CodebaseAnalyzer] Skip (unreadable file): ${fullPath}`);
      }
    }
...
function isExcludedPythonMockProductionMatch(content, matchIndex, relativePath) {
  ...
  if (
    ...
    return true;
  if (/\bimport\s+[\w.,\s]*mock_[\w.]+/.test(normalized) && !/unittest\.mock/.test(normalized))
    return true;
...
function isExcludedRoadmapMarkerLine(content, matchIndex, relativePath) {
  ...
  if (/app\.(?:get|post|put|delete|use)\s*\(\s*['"`][^'"`]*(?:roadmap|milestone)/i.test(normalized))
    return true;
  // Skip require/import of roadmap modules
  if (/\b(?:require|import)\s*[(\s]['"`][^'"`]*(?:roadmap|RoadmapDataAnalyzer)/i.test(normalized))
    return true;
  // Skip comment lines
  ...
  // simplebeacon-ignore
  if (/TODO\|FIXME\|HACK\|XXX|regex:\s*['"].*TODO|roadmap.*marker|scan.*todo/i.test(normalized))
    return true;
  return false;
}
...
function scanContentPatterns(
  ...
  if (
    ...
    /\/d3\.v\d+\.min\.js$|\.min\.js$|\.pack\.js$|\.bundle\.js$/.test(relativePath)
  ) {
    return hits;
  }
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (category === 'missing-rate-limit') {
        ...
        if (
          ...
          continue;
        if (/server\/bootstrap\//.test(relativePath) || /server\/index\.cjs$/.test(relativePath))
          continue;
        ...
        if (/simplebeacon-server\.cjs$/.test(relativePath) || /tools\//.test(relativePath))
          continue;
        ...
        if (/(?:\/health|\/schema|\['"`]\/(?:health|schema)['"`]|['"`]\/['"`])/.test(rllLineText))
          continue;
        // Skip lines where rate limiting middleware is already present
        if (/rateLimit|throttle|[Ll]imiter|express-rate-limit|express-slow-down/.test(rllLineText))
          continue;
      }
      ...
      if (category === 'performance') {
        ...
        if (/web\/data\//.test(relativePath) || /simplebeacon-dashboard/.test(relativePath))
          continue;
      ...
      if (category === 'magic-number') {
        ...
        if (/simplebeacon-dashboard/.test(relativePath) || /src\/api\//.test(relativePath))
          continue;
        ...
        if (/(?:^|\/)coming-soon\//.test(relativePath) || /(?:^|\/)vendor\//.test(relativePath))
          continue;
        if (
      ...
      if (category === 'config-drift') {
        ...
        if (/(?:^|\/)server\//.test(relativePath) || /packages\/[^/]+\/src\//.test(relativePath))
          continue;
      ...
      if (category === 'architecture-drift') {
        ...
        if (/(?:^|\/)scripts\//.test(relativePath) || /(?:^|\/)tools\//.test(relativePath))
          continue;
      ...
      if (category === 'inner-html-xss') {
        ...
        if (/\b(?:escapeHtml|sanitizeHtml|htmlEscape|renderSafe|DOMPurify\.sanitize)\b/.test(mText))
          continue;
      }
      if (category === 'inner-html-xss') {
        if (/^coming-soon\//.test(relativePath)) continue;
      ...
      if (category === 'logging-secrets') {
        ...
        if (/console\.(log|warn|error)\s*\(\s*['`"](Report keys|Keys|Found|Total)/i.test(lineText))
          continue;
      ...
      if (category === 'weak-cryptography') {
        ...
        if (/\/\/.*(?:weak|deprecated|do not use|avoid|legacy|insecure)/i.test(wcLineText))
          continue;
        if (/\/\*.*(?:weak|deprecated|do not use|avoid|legacy|insecure)/i.test(wcLineText))
          continue;
      }
      ...
      if (category === 'cicd-secret-exposure') {
        ...
        if (/scanner-patterns|scanner-engine|codebase-analyzer|\.test\./.test(relativePath))
          continue;
        ...
        if (/example|placeholder|your_|my_|changeme|fake|dummy|test_|mock_/i.test(cicdLineText))
          continue;
      }
      ...
      if (category === 'mock-path-leak') {
        ...
        if (/sample-consistency-checker/.test(relativePath) || /scan-conclusion/.test(relativePath))
          continue;
      ...
      if (category === 'eval-danger') {
        ...
        if (/require\s*\(\s*\w+\s*\)/.test(edLineText)) {
          ...
          if (
            varName &&
            new RegExp('fs\\.existsSync\\s*\\(\\s*' + varName + '\\s*\\)').test(prevBlock)
          )
            continue;
      ...
      if (
        ...
        (/^coming-soon\//.test(relativePath) || /simplebeacon-dashboard/.test(relativePath))
      )
        continue;
      if (category === 'empty-stub-function') {
        if (/vendor\//.test(relativePath) || /\.min\.js$/i.test(relativePath)) continue;
        if (/\b(?:docs|tests?)\//.test(relativePath) || /\.(test|spec)\./i.test(relativePath))
          continue;
        if (/^coming-soon\//.test(relativePath)) continue;
      }
...
function isExcludedDebugLine(line) {
  ...
  if (/DEBUG_PATTERNS|PLACEHOLDER_PATTERNS|TECH_DEBT_PATTERNS|debugger-statement/.test(normalized))
    return true;
  if (/pattern:\s*\//.test(normalized) && /\bdebugger\b/.test(normalized)) return true;
  if (/pattern:\s*\//.test(normalized) && /console\.(log|debug|info)\s*\(/.test(normalized))
    return true;
  if (/\/[^/]*\bdebugger\b[^/]*\/[gimsuy]*/.test(normalized)) return true;
  if (/\bdebugger\s*statement\b/i.test(normalized) && !/^\s*debugger\s*;?\s*$/.test(normalized))
    return true;
  if (
  ...
  if (/no-debugger|no-console|app-logger|logger\.(debug|info|warn|error)\(/.test(normalized))
    return true;
...
function detectDebugArtifacts(content, relativePath) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const matchText = hasDebugger
      ? 'debugger'
      : line.match(/\bconsole\.(log|debug|info|trace|warn)\s*\(/)?.[0] || 'console-log(';
...
function detectFixPreviewIssues(content, relativePath) {
  ...
  if (/^coming-soon\/(js\/dashboard\/|app-links\.js|contact\.js|site-config\.js)/.test(rel))
    return [];
...
function detectMissingStrictMode(content, relativePath) {
  ...
  if (/\.(config|test|spec)\./.test(rel) || /jest\.config|eslint\.config|babel\.config/.test(rel))
    return [];
...
function detectPerformanceIssues(content, relativePath) {
  ...
  const rel = normalizedAuditPath(relativePath);
  // Skip scanner/build tools where for...of loops are standard for batch processing
  if (/server\/lib\//.test(rel) || /server\/config\//.test(rel) || /(?:^|\/)tools\//.test(rel))
    return [];
  // Skip src/core and shared-utils where loops are standard for data processing
...
function detectPrototypePollution(content, relativePath) {
  ...
  for (const item of PROTOTYPE_POLLUTION_PATTERNS) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      seen.add(dedupeKey);
      const severity =
        item.id === 'for-in-no-guard' || item.id === 'object-assign-untrusted' ? 'medium' : 'high';
      hits.push({
...
function detectSampleJsonRef(content, relativePath) {
  ...
  // Skip API route files where sample JSON references are demo endpoints
  if (/src\/api\/simplebeacon-api\.cjs$/.test(rel) || /simplebeacon-billing-api\.cjs$/.test(rel))
    return [];
  ...
  if (
    /(?:\/|^)(?:scan|project-detect|marketing-content-generator|remediation-guides)\.js$/.test(rel)
  )
    return [];
  const hits = [];
  const seen = new Set();
...
function detectSecurityHeaders(content, relativePath) {
  ...
  if (/(?:^|\/)packages\/simplebeacon-cli\/src\/(?:rules|analyzers|reporters|lib)\//.test(rel))
    return [];
...
function detectTokenBleed(content, relativePath) {
  ...
  const rel = normalizedAuditPath(relativePath);
  // Skip scanner catalog, route, and service files where long strings are standard
  if (/server\/lib\//.test(rel) || /server\/routes\//.test(rel) || /server\/services\//.test(rel))
    return [];
  ...
  if (/(?:^|\/)coming-soon\//.test(rel) || /(?:^|\/)simplebeacon-frameworkless\//.test(rel))
    return [];
...
function detectAiIndicators(content, relativePath) {
  ...
  if (
    ...
    return [];
  if (/\/(?:services|routes|lib)\/(?:ai-|model-|ollama-|semantic-|inference-|chatbot-)/i.test(rel))
    return [];
  if (
    /\/ai[-_]/.test(rel) ||
  ...
  for (const item of patterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (item.id === 'unparameterized-query') {
        if (/SELECT\s+1\s+AS|SELECT\s+CURRENT_TIMESTAMP|SELECT\s+version\(\)/i.test(match[0]))
          continue;
      ...
      if (item.id === 'sql-template-injection' && /<\s*(?:select|div|span|option)/i.test(match[0]))
        continue;
...
function detectInsecureRandom(content, relativePath) {
  ...
  if (/server\/lib\//.test(rel) || /server\/routes\//.test(rel) || /server\/middleware\//.test(rel))
    return [];
...
function detectLicenseHeaders(content, relativePath) {
  ...
  for (const item of LICENSE_HEADER_PATTERNS) {
    ...
    if (
      isMarkdown &&
      (item.id === 'license-header' || item.id === 'licensed-under' || item.id === 'spdx-license')
    )
      continue;
    ...
    if (
      ...
      /(?:^|\/)packages\/simplebeacon-cli\/src\/(?:rules|analyzers|lib)\//.test(relativePath)
    )
      continue;
    // Skip subproject source files and package source with expected SPDX headers
    if (
      /(?:^|\/)ai-agent\//.test(relativePath) ||
      /(?:^|\/)ai-platform\/packages\/simplebeacon-intelligence\/src\//.test(relativePath)
    )
      continue;
    // Skip dashboard service files containing license pattern catalogs
    if (/simplebeacon-dashboard\/js\/services\/aiProblemAnalyzerSuite\.mjs$/.test(relativePath))
      continue;
    const pattern = new RegExp(item.pattern.source, ensureGlobalPatternFlags(item.pattern.flags));
    let match;
    while ((match = pattern.exec(content)) !== null) {
...
function mapEslintRuleCategory(ruleId) {
  ...
  if (id.includes('no-undef') || id.includes('no-unreachable') || id.includes('eqeqeq'))
    return 'correctness';
  if (id.includes('complexity') || id.includes('max-') || id.includes('sonarjs/'))
...
async function analyzeFileContent(file, rootDir, options = {}) {
  ...
  if (file.isArtifact || rel.includes('security-reports/fixes/')) {
    ...
    if (
      /\b(logs|cache|tmp|temp|uploads|downloads|dist|build|coverage)\//.test(rel) ||
      /simplebeacon-dashboard\//.test(rel) ||
      /^[\w-]+\.log$/i.test(rel)
    ) {
      return finalizeFileAnalysis(findings, rel, structure);
  ...
  if (['.js', '.mjs', '.cjs', '.ts', '.tsx', '.jsx'].includes(file.ext)) {
    ...
    if (
      ...
      findings.push(
        ...scanContentPatterns(content, rel, TECH_DEBT_PATTERNS, 'tech-debt', 'medium')
      );
    }
    // Skip scanner rule catalogs and server infrastructure files for architecture-drift
    const isScannerCatalog =
      /(?:^|\/)packages\/[^/]+\/(?:src\/)?rules\//.test(rel) || /(?:^|\/)server\/lib\//.test(rel);
    if (!isScannerCatalog) {
      findings.push(
        ...scanContentPatterns(
          content,
    ...
    findings.push(
      ...scanContentPatterns(content, rel, BUILD_READINESS_PATTERNS, 'build-readiness', 'low')
    );
    findings.push(
      ...scanContentPatterns(content, rel, CONFIG_DRIFT_PATTERNS, 'config-drift', 'low')
    );
    findings.push(
      ...scanContentPatterns(
        content,
    ...
    if (!/src\/proxy\/gateway\.js$/.test(rel)) {
      findings.push(
        ...scanContentPatterns(content, rel, DEPENDENCY_VULN_PATTERNS, 'dependency-vulns', 'low')
      );
    }
  ...
  if (['.js', '.mjs', '.cjs', '.ts', '.tsx', '.jsx'].includes(file.ext)) {
    ...
    const skipSecretInComment =
      /scanner-patterns|scanner-engine|pattern-documentation|\.test\./.test(rel);
    ...
    findings.push(
      ...scanContentPatterns(content, rel, MAGIC_NUMBER_PATTERNS, 'magic-number', 'low')
    );
    findings.push(
      ...scanContentPatterns(content, rel, MOCK_PATH_LEAK_PATTERNS, 'mock-path-leak', 'low')
    );
    findings.push(
      ...scanContentPatterns(content, rel, PRODUCTION_LEAK_PATTERNS, 'production-leak', 'low')
    );
    findings.push(
      ...scanContentPatterns(content, rel, ROADMAP_MARKER_PATTERNS, 'roadmap-marker', 'low')
    );
    if (!isNonProductionAuditContentPath(rel)) {
      findings.push(
        ...scanContentPatterns(content, rel, SENSITIVE_DATA_PATTERNS, 'sensitive-data', 'high')
      );
    }
    ...
    const isTestFileTypeSafety =
      /\.(test|spec)\./.test(rel) || /(?:^|\/)tests?\//.test(rel) || /test-all-patterns/.test(rel);
    ...
    if (isTypeScript && !isNonProdForTypeSafety && !isTestFileTypeSafety && !isVscodeExtension) {
      findings.push(
        ...scanContentPatterns(content, rel, TYPE_SAFETY_PATTERNS, 'type-safety', 'low')
      );
    }
  ...
  if (['.html', '.htm', '.jsx', '.tsx', '.vue'].includes(file.ext)) {
    ...
    if (
      ...
      findings.push(
        ...scanContentPatterns(content, rel, ACCESSIBILITY_PATTERNS, 'accessibility', 'low')
      );
    }
  }
  ...
  if (['.c', '.cpp', '.cc', '.cxx', '.h', '.hpp', '.hh', '.hxx'].includes(file.ext)) {
    findings.push(
      ...scanContentPatterns(content, rel, C_TYPE_SAFETY_PATTERNS, 'type-safety', 'low')
    );
    findings.push(
      ...scanContentPatterns(content, rel, C_RATE_LIMIT_PATTERNS, 'missing-rate-limit', 'medium')
    );
    findings.push(
      ...scanContentPatterns(content, rel, C_LOGGING_SECRET_PATTERNS, 'logging-secrets', 'medium')
    );
    findings.push(
      ...scanContentPatterns(content, rel, C_SAMPLE_DATA_PATTERNS, 'sample-json-ref', 'medium')
    );
    findings.push(
      ...scanContentPatterns(content, rel, C_ROADMAP_PATTERNS, 'roadmap-marker', 'low')
    );
    findings.push(...detectDatabasePatterns(content, rel));
  }
...
function detectTestCoverage(files) {
  ...
  for (const file of files) {
    ...
    if (/simplebeacon-dashboard/.test(rel) || /\/js\/(utils|components|views)\//.test(rel))
      continue;
    if (
    ...
    if (/-dashboard\./.test(name) || /[-_]gateway\./.test(name) || /enterprise[-_]/.test(name))
      continue;
    ...
    if (/shared-utils\//.test(rel) || /^server\/cron\//.test(rel) || /^server\/config\//.test(rel))
      continue;
    if (
      /(?:^|\/)coming-soon\//.test(rel) ||
      /(?:^|\/)simplebeacon-rule-tests\//.test(rel) ||
...
function classifyFindingTier(filePath) {
  ...
  if (rel.startsWith('packages/') || rel.includes('/packages/')) {
    ...
    if (/(?:^|\/)packages\/[^/]+\/src\/(?:reporters|bin|scripts|tools)\//.test(rel))
      return 'general';
  ...
  if (
    PRODUCTION_PATH_PREFIXES.some((prefix) => rel.startsWith(prefix) || rel.includes(`/${prefix}`))
  ) {
    return 'production';
  }
  return 'general';
    ...
    if (error.stdout) {
      try {
        ...
        return {
          ...
          errors,
          warnings,
          filesWithIssues: reports.filter((r) => (r.errorCount || 0) + (r.warningCount || 0) > 0)
            .length,
          findings: [],
          summary: buildEslintSummary([], {
            source: 'command',
...
async function analyzeCodebase(baseDir, options = {}) {
  ...
  const nodeModulesFiltered = files.filter((f) => {
    const isNodeModules =
      f.relativePath.includes('/node_modules/') || f.relativePath.startsWith('node_modules/');
    if (isNodeModules) {
      if (process.env.SIMPLEBEACON_DEBUG)
        logger.debug(`[CodebaseAnalyzer] Skip (node_modules): ${f.relativePath}`);
    }
