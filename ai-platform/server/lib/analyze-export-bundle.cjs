...
function extractCompleteResults(completeScan) {
  const normalized =
    resolveCompleteScanExportBundle(completeScan) || completeScan;
...
async function generateExecutiveAuditHtml(completeScan, options = {}) {
  ...
  } = require("./complete-scan-audit-report.cjs");
  ...
  const payload =
    completeScan && completeScan.results
      ...
      : {
          ...
          projectPath:
            completeScan?.projectRoot || completeScan?.projectPath || "",
  ...
  const report = await buildCompleteAuditReport(payload, {
    ...
    assessor:
      creds.signatoryName || options.assessor || "SimpleBeacon Operator",
  ...
  return {
    skipped: false,
    html: report.html,
    filename: report.filename || "executive-audit.html",
  };
}
...
async function generateEuAiActAuditHtml(projectPath, options = {}) {
  ...
  return {
    html: report.html,
    filename: report.filename || "eu-ai-act-audit.html",
  };
}
...
async function generateAgencyCertificateHtml(completeScan, options = {}) {
  ...
  const gateReport =
    results.simplebeacon || normalized?.results?.simplebeacon || normalized;
  if (!gateReport?.gate) {
    return {
      skipped: true,
      reason: "Gate report required for agency certificate",
    };
  }
...
async function collectExportArtifacts(completeScan, tierId, options = {}) {
  ...
  const selectedEngines = resolveSelectedEnginesForExport(
    completeScan,
    options,
  );
  ...
  if (
    kind === "complete" &&
    normalized?.type === "simplebeacon-complete-scan"
  ) {
  ...
  const artifacts = manifest.artifacts.filter((artifact) => {
    if (exportEngineSet) {
      if (
        !artifactAllowedForEngines(artifact.id, exportEngineSet, {
          includeEuAiAct,
          scanKind: kind,
        })
      ) {
        if (EU_AI_ACT_ARTIFACT_IDS.has(artifact.id) && !includeEuAiAct) {
          ...
          warnings.push(
            `${artifact.id}: excluded - not selected in export queue`,
          );
        }
        return false;
      }
      ...
      switch (artifact.id) {
        ...
        case "simplebeacon-gate": {
          const gate =
            ...
            (kind === "simplebeacon-report" ? normalized : null);
          if (!gate) {
            ...
            files.push({
              ...
              content: tryStringify(
                sanitizeFictionDigestExport(results.mockScan, {
                  projectPath,
                  repositoryFilesTotal: resolveRepoFilesTotal(
                    results.simplebeacon,
                  ),
        ...
        case "complete-scan-bundle":
          files.push({
            path: artifact.filename,
            content: tryStringify(normalized, null, 2),
          });
          break;
        case "consolidation":
        ...
        case "eu-ai-act-sprint": {
          ...
          if (raw) {
            euOpts.projectPath =
              raw.projectPath || projectPath || options.baseDir;
            files.push({
              path: artifact.filename,
              content: tryStringify(
                sanitizeEuAiActSprintArtifactExport(raw, euOpts),
                null,
                2,
              ),
            });
            ...
            euOpts.projectPath =
              normalized.sprint.projectPath || projectPath || options.baseDir;
            files.push({
              path: artifact.filename,
              content: tryStringify(
        ...
        case "eu-ai-act-audit": {
          if (!includeEuAiAct) {
            warnings.push(
              "eu-ai-act-audit: skipped - EU AI Act sprint not part of this scan",
            );
            break;
          }
          ...
          if (sprintPayload?.html) {
            ...
            const euProjectPath =
              sprintPayload?.projectPath || projectPath || options.baseDir;
            const eu = await generateEuAiActAuditHtml(euProjectPath, {
              ...
              sprintPayload: {
                ...sprintPayload,
                report:
                  sprintPayload?.report || results.simplebeacon || normalized,
  ...
  const enginesRunList =
    selectedEngines || resolveEnginesRun(normalized, options);
  ...
  for (let i = 0; i < scanSteps.length; i++) {
    ...
    try {
      ...
      warnings.push(
        `step-export-${step.id}: ${String(err.message || err).slice(0, 120)}`,
      );
    }
  }
...
async function buildAnalyzeExportZipStream(completeScan, options = {}) {
  ...
  if (
    tierId === "community" &&
    options.publicGateLocked &&
    !options.internalDashboard
  ) {
    const err = new Error(
  ...
  const { files, manifest, warnings } = await collectExportArtifacts(
    completeScan,
    tierId,
    options,
  );
  ...
  if (warnings.length)
    logger.debug("[Export Bundle] Warnings:", warnings.slice(0, 10));
  ...
  if (options.outputStream) {
    ...
    return {
      ...
      manifest,
      tierId,
      warnings: warnings.concat(archiveWarnings),
    };
  }
  ...
  logger.debug(
    `[Export Bundle] ZIP size: ${(buffer.length / 1024 / 1024).toFixed(2)} MB`,
  );
  return {
    buffer,
    stream,
