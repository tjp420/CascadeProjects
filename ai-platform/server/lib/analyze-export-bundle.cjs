...
async function collectExportArtifacts(completeScan, tierId, options = {}) {
    ...
    for (const artifact of artifacts) {
        try {
            ...
            switch (artifact.id) {
                case 'public-summary':
                    files.push({ path: artifact.filename, content: tryStringify(buildPublicSummary(normalized), null, 2) });
                    break;
                ...
                case 'eu-ai-act-sprint': {
                    ...
                    if (raw) {
                        ...
                        files.push({ path: artifact.filename, content: tryStringify(sanitizeEuAiActSprintArtifactExport(normalized.sprint, euOpts), null, 2) });
                    } else { warnings.push('eu-ai-act-sprint: run EU AI Act sprint mode or include sprint in payload'); }
                    break;
                }
                case 'executive-audit': {
                    const audit = await generateExecutiveAuditHtml(normalized, options);
                    if (audit.skipped) { warnings.push(`executive-audit: ${audit.reason}`); }
                    else { files.push({ path: artifact.filename, content: sanitizeFrozenAuditDeliverableHtml(audit.html) }); }
                    break;
                }
                case 'eu-ai-act-audit': {
                    if (!includeEuAiAct) { warnings.push('eu-ai-act-audit: skipped - EU AI Act sprint not part of this scan'); break; }
                    ...
                    if (sprintPayload?.html) {
                        ...
                        const eu = await generateEuAiActAuditHtml(euProjectPath, { ...options, euProjectPath, sprintPayload: { ...sprintPayload, report: sprintPayload?.report || results.simplebeacon || normalized, platformRoot: sprintPayload?.platformRoot || euProjectPath } });
                ...
                case 'agency-certificate': {
                    ...
                    if (cert.skipped) { warnings.push(`agency-certificate: ${cert.reason}`); }
                    else { files.push({ path: artifact.filename, content: sanitizeFrozenAuditDeliverableHtml(cert.html) }); }
                    break;
                }
    ...
    for (let i = 0; i < scanSteps.length; i++) {
        ...
        try {
            ...
            const slimStep = isBrowserAnalyzer ? {
                id: step.id, status: step.status || 'unknown', error: step.error || null, metric: step.metric || null,
                findingsCount: step.findingsCount ?? null, fileCount: step.fileCount ?? null, severity: step.severity || null,
                findings: step.findings || null, category: step.category || null
            } : {
                id: step.id, status: step.status || step.report?.status || 'unknown', error: step.error || null, metric: step.metric || null,
                gatePass: step.gatePass ?? step.report?.gate?.pass ?? null, publicGateLocked: step.publicGateLocked ?? null
            };
