...
function getCanaryConfig() {
  ...
  if (enabled !== undefined) {
    config.canary_parameters.default_enabled = enabled === '1' || enabled === 'true';
  }
...
function checkRollback(metrics, config) {
  ...
  if ((metrics.handshakeFailureRatePct || 0) > (th.handshake_failure_rate_pct || 10.0)) {
    reasons.push(`handshake_failure_rate: ${metrics.handshakeFailureRatePct.toFixed(2)}%`);
  }

  if ((metrics.downgradeRejectedRatePct || 0) > (th.downgrade_rejected_rate_pct || 5.0)) {
    reasons.push(`downgrade_rejected_rate: ${metrics.downgradeRejectedRatePct.toFixed(2)}%`);
  }

  const hbMultiplier = metrics.baselineHeartbeatTimeoutRatePct
    ? (metrics.heartbeatTimeoutRatePct || 0) / metrics.baselineHeartbeatTimeoutRatePct
    : 0;
  if (hbMultiplier > (th.heartbeat_timeout_multiplier || 2.0) && (metrics.heartbeatTimeoutRatePct || 0) > 0) {
