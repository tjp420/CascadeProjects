...
async function testStageAndFinalizeSuccess() {
  ...
  // metrics recorded
  const found = metrics.calls.find(
    (c) => c.name === "hsm_shard_reconstructed_blocks_total",
  );
...
async function testFinalizeFailureCleansStaging() {
  ...
  const chunk = {
    filename: "blk2.json",
    payload: JSON.stringify({ seq: 2, data: "y" }),
  };
  await reassembler.stageChunks(staging, [chunk]);
  // provide wrong expected hash to force validation failure
  const manifest = {
    expectedLeafHashes: { "blk2.json": "deadbeef" },
    labels: { tenantId: "tenant2", shardId: "shardB" },
  };
  const metrics = {
    calls: [],
    inc(name, v, labels) {
