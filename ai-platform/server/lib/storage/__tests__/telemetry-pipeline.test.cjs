...
describe("CI Telemetry Pipeline: reassembler + repair-worker + reconciler", () => {
  ...
  test("L2-01: reconciler increments hsm-metrics on out-of-sync gap", async () => {
    ...
    expect(
      metrics.hsm_shard_reconciler_repair_requested_total,
  ...
  test("L2-02: repair-worker increments hsm-metrics on completion", async () => {
    ...
    const completed = await waitForMetric(
      () => hsmMetrics.getMetrics().hsm_repair_worker_completed_total,
      1,
      500,
    );
  ...
  test("L2-03: reassembler success increments hsm-metrics via bridge", async () => {
    ...
    const chunk = {
      filename: "blk1.json",
      payload: JSON.stringify({ seq: 1, data: "x" }),
    };
    ...
    const res = await reassembler.finalizeRehydration(
      staging,
      live,
      manifest,
      metricsBridge,
    ...
    const metrics = hsmMetrics.getMetrics();
    expect(metrics.hsm_shard_reconstructed_blocks_total).toBeGreaterThanOrEqual(
      1,
    );
    expect(metrics.hsm_shard_reassembly_attempts_total).toBeGreaterThanOrEqual(
      1,
    );
  });
  ...
  test("L2-04: reassembler failure increments attempt counter", async () => {
    ...
    const metricsBridge = createCentralMetricsBridge();
    await expect(
      reassembler.finalizeRehydration(staging, live, manifest, metricsBridge),
    ).rejects.toThrow();
  ...
  test("L2-05: cooldown skip increments skip counter", async () => {
    ...
    const metrics = hsmMetrics.getMetrics();
    expect(
      metrics.hsm_shard_reconciler_repair_skipped_total,
    ).toBeGreaterThanOrEqual(1);
  });
  ...
  test("L3-02: local reconciler metrics still work (backward compat)", async () => {
    ...
    await reconciler.verifyShardContinuity();
    expect(
      reconciler.metrics.hsm_shard_out_of_sync_total,
    ).toBeGreaterThanOrEqual(1);
    expect(
      reconciler.metrics.hsm_shard_reconciler_repair_requested_total,
    ).toBeGreaterThanOrEqual(1);
  });
  ...
  test("L3-03: end-to-end pipeline — reconciler detects gap, repair-worker completes, all counters increment", async () => {
    ...
    reconciler.on("shard:reconciler:reconcile_requested", (p) =>
      worker.handle(p),
    );
    ...
    expect(completed).toBeGreaterThanOrEqual(1);
    const processed = await waitForMetric(
      () => worker.processed.length,
      1,
      1000,
    );
