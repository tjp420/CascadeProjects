...
function validateEnvelope(payload) {
  if (!payload || typeof payload !== "object")
    throw new Error("TRACK123_MISSING_PAYLOAD");
  if (payload.encrypted) {
    ...
    try {
      if (typeof cipher !== "string" || cipher.length === 0)
        throw new Error("TRACK123_MISSING_CIPHER");
      if (typeof iv !== "string" || iv.length === 0)
        throw new Error("TRACK123_MISSING_IV");
      if (typeof authTag !== "string" || authTag.length === 0)
        throw new Error("TRACK123_MISSING_AUTH_TAG");
...
class RepairWorker extends EventEmitter {
  constructor(opts = {}) {
    ...
    if (this.emitter) {
      this.emitter.on("shard:reconciler:reconcile_requested", (p) =>
        this.handle(p),
      );
  ...
  handle(payload = {}) {
    try {
      ...
      this.emit("error", err);
      if (
        err.message.startsWith("TRACK123_MISSING_") ||
        err.message.startsWith("TRACK123_INVALID_")
      ) {
    ...
    const jitterMax = Number(
      payload.repairJitterMs || this.repairJitterMs || 0,
    );
    const delay =
      jitterMax === 0 ? 0 : Math.floor(Math.random() * (jitterMax + 1));
    ...
    setTimeout(async () => {
      try {
        ...
        this.emit("shard:reconciler:repair_completed", {
          key,
          payload,
          result,
        });
