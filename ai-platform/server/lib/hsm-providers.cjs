...
class BaseHsmProvider {
  ...
  async derive(_orgId) { throw new Error('derive() not implemented'); }

  handshake() {
    const handle = `${this.provider}:${this.keyId}@${this.region}`;
    const seed = (this.accessToken || 'no-token').slice(0, 8);
    const fingerprint = crypto.createHash('sha256').update(`${handle}:${seed}`).digest('hex');
    return {
...
class CloudKmsProvider extends BaseHsmProvider {
  async derive(orgId) {
    ...
    if (!res || !res.mac) throw new Error('Cloud KMS did not return a valid MAC');
    const mac = Buffer.from(res.mac, 'base64');
    return mac.length >= 32 ? mac.slice(0, 32) : crypto.createHash('sha256').update(mac).digest();
  }
}
...
class AzureKmsProvider extends BaseHsmProvider {
  async derive(orgId) {
    ...
    const body = JSON.stringify({ alg: 'RS256', value: digest.toString('base64') });
    const keyVersion = this.keyId.includes('/') ? this.keyId : `keys/${this.keyId}`;
    ...
    if (!res || !res.value) throw new Error('Azure Key Vault did not return a valid signature');
