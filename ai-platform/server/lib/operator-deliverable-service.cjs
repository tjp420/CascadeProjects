...
const PRODUCTS = {
  ...
  clearance499: {
    ...
    notIncluded:
      "Hosted dashboard, milestone certificates, EU legal certification",
  },
  agency999: {
...
function inferProductFromBooking(booking = {}) {
  ...
  if (/money.?printer|instant.?audit|19/.test(combined))
    return "moneyPrinter19";
  ...
  if (/project pack|agency pack|999|milestone certificate/.test(combined))
    return "agency999";
...
function validateGateReport(reportPath) {
  ...
  return {
    ...
    issueCount:
      payload.issueCount ?? payload.results?.simplebeacon?.issueCount ?? null,
    generatedAt:
      payload.generatedAt || payload.results?.simplebeacon?.generatedAt || null,
  };
}
...
function buildOutboundMd(product, ctx) {
  ...
  if (ctx.gate) {
    ...
    if (ctx.gate.blockingCount != null)
      lines.push(`- Blocking: **${ctx.gate.blockingCount}**`);
    if (ctx.gate.generatedAt)
      lines.push(`- Generated: ${ctx.gate.generatedAt}`);
    lines.push("");
  }
  if (ctx.project) {
  ...
  if (product.sku === "clearance499" || product.sku === "warranty199") {
    lines.push(
      "- Attached: executive PDF with gate verdict and developer remediations.",
    );
  }
  if (product.sku.startsWith("agency")) {
    lines.push(
      "- Attached: co-branded hygiene certificate for this milestone.",
    );
  }
 ...
 * @returns {any}
 */
async function updateBookingDeliverable(
  projectRoot,
  bookingKey,
  patch,
  options = {},
  ...
  const file =
    ...
    path.join(projectRoot, "data", "audit-bookings.json");
  let rows;
  try {
...
async function createDeliverableWorkspace(input, options = {}) {
  ...
  if (!product) {
    return {
      ...
      message: `Unknown product "${productKey}"`,
    };
  }
  ...
  if (reportPath) {
    await fs.promises.copyFile(
      reportPath,
      path.join(workspaceDir, "report.json"),
    );
  }
  await fs.promises.writeFile(
  ...
  if (booking?.receivedAt) {
    await updateBookingDeliverable(
      ...
      booking.receivedAt,
      {
        deliverableWorkspace: path
          .relative(projectRoot, workspaceDir)
          .replace(/\\/g, "/"),
