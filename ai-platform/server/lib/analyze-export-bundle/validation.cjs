...
function enrichExportBundleManifest(manifest, { tierId, projectPath } = {}) {
  ...
  const redactedPath =
    redactProjectPathForExport(projectPath, label) || manifest.projectPath;
  const exportNotes = [
...
function resolveCompleteScanExportBundle(normalized, projectPath) {
  if (!normalized || normalized.type !== "simplebeacon-complete-scan")
    return normalized;
...
function buildPublicSummary(completeScan) {
  ...
  const normalized =
    kind === "complete" && completeScan?.type === "simplebeacon-complete-scan"
      ? completeScan
      : completeScan;
  ...
  const publicBlock =
    gated?.publicSummary || sanitizePublicOutput(simplebeacon || normalized);
...
function validateScanForTier(tierId, scanKind) {
  ...
  if (tier.requiresCompleteScan && scanKind !== "complete") {
    return {
      ok: false,
      error:
        "This deliverable tier requires Analyze → Complete (all ten engines). Run a complete scan and retry.",
    };
  }
