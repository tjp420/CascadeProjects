...
function resolveEnginesRun(payload, options = {}) {
  ...
  if (
    Array.isArray(payload?.analysisConfig?.enginesRun) &&
    payload.analysisConfig.enginesRun.length
  ) {
...
function resolveSelectedEnginesForExport(payload, options = {}) {
  if (
    Array.isArray(options.selectedEngines) &&
    options.selectedEngines.length
  ) {
    return [...new Set(options.selectedEngines.filter(Boolean))];
  }
  const fromPayload =
    payload?.enginesRun ||
    payload?.analysisConfig?.selectedEngines ||
    payload?.selectedEngines;
...
function filterCompleteScanForEngines(completeScan, engineIds = []) {
  ...
  const enginesRun = (
    normalized.enginesRun ||
    normalized.analysisConfig?.enginesRun ||
    []
...
function artifactAllowedForEngines(
  artifactId,
  engineSet,
  { includeEuAiAct = false, scanKind = "unknown" } = {},
) {
