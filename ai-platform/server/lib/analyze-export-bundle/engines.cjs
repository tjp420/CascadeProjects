...
function resolveSelectedEnginesForExport(payload, options = {}) {
  ...
  const fromPayload =
    payload?.enginesRun || payload?.analysisConfig?.selectedEngines || payload?.selectedEngines;
...
function filterCompleteScanForEngines(completeScan, engineIds = []) {
  ...
  const enginesRun = (normalized.enginesRun || normalized.analysisConfig?.enginesRun || []).filter(
    (id) => selected.has(id)
  );
