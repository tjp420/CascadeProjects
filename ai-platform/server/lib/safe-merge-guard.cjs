...
function assertConfirmation(options = {}) {
  if (options.confirmed !== true) {
    throw new Error(
      "Confirmation required — set confirmed: true after reviewing preview",
    );
  }
  if (String(options.confirmationPhrase || "") !== CONFIRMATION_PHRASE) {
    throw new Error(
      `Invalid confirmation phrase — expected "${CONFIRMATION_PHRASE}"`,
    );
  }
}
...
async function executeSafeMerge(options = {}) {
  ...
  const deleteAfter = new Date(
    Date.now() +
      (preview.gracePeriodHours || 24) * 60 * constants.ONE_MINUTE_MS,
  ).toISOString();
  ...
  for (const relativePath of preview.removeFiles || []) {
    try {
      ...
      if (!fs.existsSync(absPath)) {
        // simplebeacon-ignore sync-io — existence check before async backup/quarantine
        errors.push({
          file: relativePath,
          reason: "File not found at execution time",
        });
        continue;
      }
