...
function inferArtifactContext(row = {}) {
  ...
  if (isIntentionalFixturePath(file)) {
    return {
      ...
      businessImpact:
        "Intentional test/sample fixture — verify purpose before changing",
    };
  }
  if (isDocumentationPath(file)) {
    return {
      ...
      businessImpact:
        "Documentation example — tokenize PII when convenient; not a deploy blocker",
    };
  }
  ...
  if (/audit-remediation-recipes\.c?js$/.test(normalizePathKey(file))) {
    return {
      ...
      artifactType: "scanner-rule",
      businessImpact:
        "Remediation recipe source — contains debug pattern match tokens by design",
    };
  }
  return null;
}
...
function resolveFindingFilePath(finding = {}, options = {}) {
  const raw =
    finding.filePath || finding.path || finding.metadata?.filePath || "";
...
function filterFindingsByProjectScope(findings = [], options = {}) {
  ...
  return findings.filter((finding) =>
    isFindingInProjectScope(finding, options),
  );
}
