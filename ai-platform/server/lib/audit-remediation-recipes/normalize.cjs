...
function extractFixInputsFromScan(scanPayload = {}) {
  if (scanPayload.type === "simplebeacon-report") {
    return {
      ...
      dataQualityFindings: [],
      gatePass:
        scanPayload.gate?.pass ?? scanPayload.simplebeaconGatePass ?? null,
    };
  }
  ...
  if (Array.isArray(scanPayload.issues) && scanPayload.issues.length) {
    return {
      ...
      codebaseFindings:
        scanPayload.codebaseAnalysis?.findings ||
        scanPayload.codebase?.findings ||
        [],
      dataQualityFindings: flattenDataQualityFindings(
        scanPayload.dataQuality || {},
      ),
      gatePass:
        scanPayload.gate?.pass ?? scanPayload.simplebeaconGatePass ?? null,
    };
  ...
  return {
    ...
    codebaseFindings: Array.isArray(codebase?.findings)
      ? codebase.findings
      : [],
    dataQualityFindings: flattenDataQualityFindings(dataQuality || {}),
    gatePass:
      simplebeacon?.gate?.pass ??
      scanPayload.summary?.simplebeaconGatePass ??
      null,
  };
}
