...
function classifyRowKind(row = {}) {
  ...
  if (
    /debug|console_or_debugger|console\.log|debugger/.test(rule) ||
    (/console\s*\.\s*log|debugger/.test(snippet) &&
      !/api[_-]?key|sk_|akia/.test(snippet))
  ) {
  ...
  if (
    /data-privacy|pii|realistic-email|ssn-pattern|remove-or-tokenize-pii/.test(
      haystack,
    )
  ) {
    return "pii";
  ...
  if (
    /build-artifact|safe-to-delete|file-reduction|unused-file|consolidate-duplicates/.test(
      haystack,
    )
  ) {
  ...
  if (
    /credential|aws|stripe|secret|api[-_]?key|token|akia|sk_|rk_|whsec_/.test(
      haystack,
    )
  ) {
    return "credentials";
  ...
  if (
    /production.leak|mock|sample\.json|demo-metrics|fixtures\//.test(haystack)
  ) {
  ...
  if (
    /fiction|kpi|completion_rate|success_rate|98\.5|meaningless|placeholder metric/.test(
      haystack,
    )
  ) {
    return "fiction-kpi";
...
function impactBandClass(kind, severity) {
  if (kind === "credentials" || severity === "critical")
...
function buildImpactRisk(kind, severity) {
  ...
  const prefix =
    kind === "credentials" || severity === "critical"
      ? "CRITICAL"
...
function buildCodeRecipe(kind, snippet, rule, fallbackRemediation) {
  ...
  if (
    /sk_(live|test)_|rk_(live|test)_|akia[0-9a-z]{4}/i.test(
      String(snippet || ""),
    )
  ) {
  ...
  if (
    /api[_-]?key\s*[:=]|secret\s*[:=]|password\s*[:=]/i.test(
      String(snippet || ""),
    )
  ) {
  ...
  if (
    /[`]{3}(?:json|javascript|typescript|python)/i.test(String(snippet || ""))
  ) {
...
function buildVerificationCommand(projectPath, options = {}) {
  ...
  const normalized = String(platformRoot || projectPath || "")
    .replace(/\\/g, "/")
    .trim();
  if (!normalized) {
