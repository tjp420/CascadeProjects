...
function classifyRowKind(row = {}) {
  ...
  if (
    ...
    (/console\s*\.\s*log|debugger/.test(snippet) && !/api[_-]?key|sk_|akia/.test(snippet))
  ) {
  ...
  if (
    /build-artifact|safe-to-delete|file-reduction|unused-file|consolidate-duplicates/.test(haystack)
  ) {
  ...
  if (
    /fiction|kpi|completion_rate|success_rate|98\.5|meaningless|placeholder metric/.test(haystack)
  ) {
    return 'fiction-kpi';
  }
