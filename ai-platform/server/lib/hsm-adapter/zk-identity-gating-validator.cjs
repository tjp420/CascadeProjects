 ...
 * batch attribute claim verification, slashing window
...
const SLASH_REASON = {
  ...
  CONTRACT_LIFETIME_OUT_OF_BOUNDS: "contract_lifetime_out_of_bounds",
...
class ZkIdentityGatingValidator {
  ...
  verifyAttributeClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "ATTRCLAIM_HUB_MISSING",
        "identity gating hub is required",
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "ATTRCLAIM_COMMITTEE_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "ATTRCLAIM_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkAttributeRangeProofHash ||
      typeof request.zkAttributeRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "ATTRCLAIM_ZK_PROOF_MISSING",
        "zero-knowledge attribute range proof hash is required",
      );
    }
    if (
    ...
    if (!pool) {
      ...
      throw new HsmAdapterError(
        "ATTRCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (
      typeof request.contractLifetimeSeconds === "number" &&
      request.contractLifetimeSeconds >
      ...
      this._recordSlash(
        request.poolId,
        request.peerId,
        SLASH_REASON.CONTRACT_LIFETIME_OUT_OF_BOUNDS,
      );
      throw new HsmAdapterError(
        "ATTRCLAIM_CONTRACT_LIFETIME_OUT_OF_BOUNDS",
        `contract lifetime seconds ${request.contractLifetimeSeconds} exceeds maximum ${this.policy.maxAttestationContractLifetimeSeconds}`,
      );
    }
    ...
    if (this._verifiedClaims.has(claimKey)) {
      ...
      throw new HsmAdapterError(
        "ATTRCLAIM_DUPLICATE",
        `attribute claim for pool ${request.poolId} already verified`,
      );
    }
    const claimId =
    ...
    const claim = {
      ...
      blindedAttributeMetricCommitment:
        request.blindedAttributeMetricCommitment || "unspecified",
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an attribute claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "ATTRCLAIM_HW_PROOF_FIELDS_MISSING",
        "poolId is required",
      );
    }
    if (
      typeof request.attributeMetric !== "number" ||
      typeof request.claimValue !== "number"
    ) {
      throw new HsmAdapterError(
        "ATTRCLAIM_HW_PROOF_FIELDS_MISSING",
        "attributeMetric and claimValue numbers are required",
      );
    }
    if (!this._hub) {
      throw new HsmAdapterError(
    ...
    const proofHash = crypto
      .createHash("sha256")
      .update(
        `${request.poolId}:${request.attributeMetric}:${request.claimValue}:${this._hwAccelType}`,
      )
      .digest("hex");
    const proof = {
      zkAttributeRangeProofHash: proofHash,
      poolId: request.poolId,
      attributeMetric: request.attributeMetric,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit("ATTRCLAIM_HW_SNARK_PROOF_GENERATED", { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple attribute claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyAttributeClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "ATTRCLAIM_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "ATTRCLAIM_BATCH_TOO_LARGE",
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyAttributeClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || "ATTRCLAIM_BATCH_ERROR",
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit("ATTRCLAIM_BATCH_VERIFIED", {
        verifiedCount,
        failedCount,
        batchSize: requests.length,
      });
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError(
        "ATTRCLAIM_WINDOW_FIELDS_MISSING",
        "poolId is required",
      );
    }
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    ...
    if (this._audit) {
      this._audit("ATTRCLAIM_PARTIAL_SIGS_AGGREGATED", {
        poolId,
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrOutOfOrderIdentityClaims &&
      typeof request.peerId === "string"
    ) {
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit("ATTRCLAIM_SLASHED", { poolId, peerId, reason });
    }
  }
}
