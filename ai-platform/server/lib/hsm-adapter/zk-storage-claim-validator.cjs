...
class ZkStorageClaimValidator {
  ...
  validateClaim(claim) {
    ...
    if (
      typeof claim.proofOfSpaceTimeWindowSeconds === "number" &&
      this.policy.maxProofOfSpaceTimeWindowSeconds !== undefined &&
      claim.proofOfSpaceTimeWindowSeconds >
        this.policy.maxProofOfSpaceTimeWindowSeconds
    ) {
      this._issueChallenge(claim.poolId, "proof_of_space_time_window_exceeded");
      throw new HsmAdapterError(
        "ZKSTORAGE_PROOF_OF_SPACE_TIME_WINDOW_EXCEEDED",
        `proof-of-space-time window seconds ${claim.proofOfSpaceTimeWindowSeconds} exceeds maximum ${this.policy.maxProofOfSpaceTimeWindowSeconds}`,
      );
    }
    if (
      typeof claim.storageAttestationChainDepth === "number" &&
      this.policy.maxStorageAttestationChainDepth !== undefined &&
      claim.storageAttestationChainDepth >
        this.policy.maxStorageAttestationChainDepth
    ) {
      this._issueChallenge(
        claim.poolId,
        "storage_attestation_chain_depth_exceeded",
      );
      throw new HsmAdapterError(
        "ZKSTORAGE_CHAIN_DEPTH_EXCEEDED",
        `storage attestation chain depth ${claim.storageAttestationChainDepth} exceeds maximum ${this.policy.maxStorageAttestationChainDepth}`,
      );
    }
    if (
      typeof claim.replicaDispersalDistance === "number" &&
      this.policy.maxReplicaDispersalDistance !== undefined &&
      claim.replicaDispersalDistance > this.policy.maxReplicaDispersalDistance
    ) {
      this._issueChallenge(claim.poolId, "replica_dispersal_distance_exceeded");
      throw new HsmAdapterError(
        "ZKSTORAGE_REPLICA_DISPERSAL_DISTANCE_EXCEEDED",
        `replica dispersal distance ${claim.replicaDispersalDistance} exceeds maximum ${this.policy.maxReplicaDispersalDistance}`,
      );
    }
    if (
    ...
    if (claim.proofValid === false) {
      ...
      throw new HsmAdapterError(
        ...
        `storage proof for pool ${claim.poolId} is invalid`,
      );
    }
...
function _validateClaimShape(claim) {
  ...
  if (
    !claim.proofOfSpaceTimeCommitment ||
    !claim.storageSectorDigestCommitment ||
    !claim.replicaDispersalCommitment
  ) {
    throw new HsmAdapterError(
      "ZKSTORAGE_CLAIM_SHAPE_INVALID",
      "proofOfSpaceTimeCommitment, storageSectorDigestCommitment, and replicaDispersalCommitment are required",
    );
  }
}
