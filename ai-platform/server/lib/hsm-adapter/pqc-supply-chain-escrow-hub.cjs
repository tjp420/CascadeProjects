...
class PqcSupplyChainEscrowHub {
  ...
  initializeOrder(request) {
    ...
    if (
      this.policy.requireProcurementInitiatorAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.procurementInitiatorAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "SCORDER_PROCUREMENT_INITIATOR_UNATTESTED",
            "procurement initiator attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SCORDER_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SCORDER_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.deliveryEpochs === "number" &&
      request.deliveryEpochs > (this.policy.maxProcurementDeliveryEpochs || 30)
    ) {
      throw new HsmAdapterError(
        "SCORDER_DELIVERY_EPOCHS_EXCEEDED",
        `delivery epochs ${request.deliveryEpochs} exceeds maximum ${this.policy.maxProcurementDeliveryEpochs}`,
      );
    }
    if (
      typeof request.escrowFundingCap === "number" &&
      request.escrowFundingCap > (this.policy.maxEscrowFundingCap || 1000000000)
    ) {
      throw new HsmAdapterError(
        "SCORDER_ESCROW_CAP_EXCEEDED",
        `escrow funding cap ${request.escrowFundingCap} exceeds maximum ${this.policy.maxEscrowFundingCap}`,
      );
    }
    const orderId =
      request.orderId || `order-${crypto.randomBytes(4).toString("hex")}`;
    ...
    const order = {
      ...
      blindedLogisticsVolumeCommitment:
        request.blindedLogisticsVolumeCommitment,
  ...
  batchInitializeOrders(requests) {
    ...
    if (this._audit) {
      this._audit("SCORDER_BATCH_INITIALIZED", {
        successCount,
        failedCount,
  ...
  rebalanceDeliveryEpochs(request) {
    if (!request || !request.orderId) {
      throw new HsmAdapterError(
        "SCORDER_REBALANCE_FIELDS_MISSING",
        "orderId is required",
      );
    }
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError(
        "SCORDER_REBALANCE_DIRECTION_INVALID",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SCORDER_REBALANCE_AMOUNT_INVALID",
  ...
  releaseEscrow(request) {
    ...
    if (!order.milestoneVerified) {
      throw new HsmAdapterError(
        "SCORDER_MILESTONE_NOT_VERIFIED",
        `order ${request.orderId} milestone not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "SCORDER_CLEARING_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minOrderMatchingQuorum || 3)) {
      throw new HsmAdapterError(
        "SCORDER_RELEASE_QUORUM_INSUFFICIENT",
        `milestone signatures ${signatures.length} below minimum ${this.policy.minOrderMatchingQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  aggregateCommitteeSignatures(orderId, partialSignatures) {
    ...
    if (this._audit) {
      this._audit("SCORDER_SIGNATURES_AGGREGATED", {
        orderId,
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "SCORDER_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedOrderValueCommitment ||
    !request.blindedLogisticsVolumeCommitment ||
    !request.blindedDepositMarginCommitment
  ) {
    throw new HsmAdapterError(
      "SCORDER_FIELDS_MISSING",
      "blindedOrderValueCommitment, blindedLogisticsVolumeCommitment, and blindedDepositMarginCommitment are required",
    );
  }
  if (typeof request.deliveryEpochs !== "number") {
    throw new HsmAdapterError(
      "SCORDER_FIELDS_MISSING",
      "deliveryEpochs is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "SCORDER_PROCUREMENT_INITIATOR_ATTESTATION_MISSING",
...
function _validateReleaseRequest(policy, request) {
  if (!request.orderId) {
    throw new HsmAdapterError(
      "SCORDER_RELEASE_FIELDS_MISSING",
      "orderId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "SCORDER_CLEARING_ATTESTATION_MISSING",
