  ...
  verifyLaserMeshClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "LASERCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (pool.status !== "open") {
      throw new HsmAdapterError(
        "LASERCLAIM_POOL_NOT_OPEN",
    ...
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "LASERCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (
      this.policy.banMalformedOrOutOfOrderLaserMeshClaims &&
      request.peerId &&
      this._bannedPeers.has(request.peerId)
    ) {
      throw new HsmAdapterError(
        "LASERCLAIM_PEER_BANNED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "LASERCLAIM_DUPLICATE",
        `duplicate laser mesh claim for pool ${request.poolId}`,
      );
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.zkLaserMeshRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderLaserMeshClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "LASERCLAIM_ZK_PROOF_MISSING",
      "zkLaserMeshRangeProofHash is required",
    );
  }
  if (!request.timedReleaseKeyDigest) {
    if (policy.banMalformedOrOutOfOrderLaserMeshClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "LASERCLAIM_TIMED_RELEASE_KEY_DIGEST_MISSING",
      "timedReleaseKeyDigest is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "LASERCLAIM_OVERSIGHT_ATTESTATION_MISSING",
