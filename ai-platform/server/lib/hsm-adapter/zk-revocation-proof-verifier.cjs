  ...
  verifyNonMembershipProof(request) {
    ...
    if (!this._registry) {
      throw new HsmAdapterError(
        "REVOK_PROOF_REGISTRY_MISSING",
        "revocation registry is required",
      );
    }
    if (this.policy.requireVerifierAttestation && this._attestationClient) {
      try {
        const result = this._attestationClient.verify(
          request.verifierAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "REVOK_VERIFIER_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "REVOK_PROOF_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (!request.zkProofHash || typeof request.zkProofHash !== "string") {
      if (
        this.policy.banMalformedNonMembershipProofs &&
        typeof request.peerId === "string"
      ) {
        this._bannedPeers.add(request.peerId);
      }
      throw new HsmAdapterError(
        "REVOK_ZK_PROOF_MISSING",
        "zero-knowledge proof hash is required",
      );
    }
    if (
      !request.nonMembershipWitnessHash ||
      typeof request.nonMembershipWitnessHash !== "string"
    ) {
      if (
        this.policy.banMalformedNonMembershipProofs &&
        typeof request.peerId === "string"
      ...
      throw new HsmAdapterError(
        "REVOK_WITNESS_MISSING",
        "non-membership witness hash is required",
      );
    }
    if (
    ...
    if (now - proofTimestamp > maxExpiration) {
      ...
      throw new HsmAdapterError(
        "REVOK_PROOF_EXPIRED",
        `proof timestamp ${proofTimestamp} expired (age ${now - proofTimestamp}s exceeds max ${maxExpiration}s)`,
      );
    }
    ...
    if (this._registry.isRevoked(entityBlindedHash)) {
      ...
      throw new HsmAdapterError(
        "REVOK_IDENTITY_REVOKED",
        `entity ${entityBlindedHash} is in the revocation list`,
      );
    }
    ...
    if (this._authenticatedProofs.has(proofKey)) {
      ...
      throw new HsmAdapterError(
        "REVOK_PROOF_DUPLICATE",
        `proof for entity ${entityBlindedHash} already authenticated`,
      );
    }
    const proofId =
...
function _validateProofRequest(policy, request) {
  if (!request.entityBlindedHash) {
    throw new HsmAdapterError(
      "REVOK_PROOF_FIELDS_MISSING",
      "entityBlindedHash is required",
    );
  }
  if (policy.requireVerifierAttestation && !request.verifierAttestation) {
    throw new HsmAdapterError(
      "REVOK_PROOF_ATTESTATION_MISSING",
