...
class PqcStratosphericAerosolMonitoringGatingHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('STRATOGATE_MAX_POOLS',
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('STRATOGATE_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('STRATOGATE_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('STRATOGATE_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  rebalanceMonitoringChainDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('STRATOGATE_REBALANCE_FIELDS_MISSING', 'poolId is required');
    }
    ...
    if (pool.status !== POOL_STATUS.OPEN && pool.status !== POOL_STATUS.REBALANCING) {
      throw new HsmAdapterError('STRATOGATE_NOT_REBALANCEABLE',
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError('STRATOGATE_REBALANCE_DIRECTION_INVALID',
    ...
    if (typeof request.rebalanceAmount !== 'number' || request.rebalanceAmount <= 0) {
      throw new HsmAdapterError('STRATOGATE_REBALANCE_AMOUNT_INVALID',
    ...
    const rebalance = {
      ...
      newMonitoringChainDepth: request.newMonitoringChainDepth !== undefined ? request.newMonitoringChainDepth : pool.monitoringChainDepth,
    ...
    if (request.newMonitoringChainDepth !== undefined) {
      if (request.newMonitoringChainDepth > (this.policy.maxMonitoringChainDepth || 16)) {
        throw new HsmAdapterError('STRATOGATE_MONITORING_DEPTH_EXCEEDED',
  ...
  completeAccreditation(request) {
    ...
    if (signatures.length < (this.policy.minClimateQuorum || 4)) {
      throw new HsmAdapterError('STRATOGATE_ACCREDITATION_QUORUM_INSUFFICIENT', `accreditation signatures ${signatures.length} below minimum ${this.policy.minClimateQuorum}`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('STRATOGATE_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('STRATOGATE_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.ACCREDITED) {
      throw new HsmAdapterError('STRATOGATE_NOT_ACCREDITED',
    ...
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('STRATOGATE_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('STRATOGATE_SETTLE_CHAIN_MISMATCH',
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('STRATOGATE_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minClimateQuorum || 4)) {
      throw new HsmAdapterError('STRATOGATE_ACCREDITATION_QUORUM_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('STRATOGATE_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.ACCREDITED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('STRATOGATE_ALREADY_ACCREDITED',
        `pool ${poolId} has been accredited/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('STRATOGATE_ALREADY_CANCELLED',
...
module.exports = { PqcStratosphericAerosolMonitoringGatingHub, POOL_STATUS, REBALANCE_DIRECTION };
