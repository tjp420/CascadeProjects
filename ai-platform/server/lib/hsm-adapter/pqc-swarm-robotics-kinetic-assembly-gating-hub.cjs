...
class PqcSwarmRoboticsKineticAssemblyGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireKineticAssemblyAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.kineticAssemblyAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "KINETICGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "kinetic assembly authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "KINETICGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "KINETICGATE_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.kineticValidationWindowSeconds === "number" &&
      request.kineticValidationWindowSeconds >
        (this.policy.maxKineticValidationWindowSeconds || 1)
    ) {
      throw new HsmAdapterError(
        "KINETICGATE_POSTURE_VALIDATION_WINDOW_EXCEEDED",
        `kinetic validation window seconds ${request.kineticValidationWindowSeconds} exceeds maximum ${this.policy.maxKineticValidationWindowSeconds}`,
      );
    }
    if (
      typeof request.kineticAssemblyChainDepth === "number" &&
      request.kineticAssemblyChainDepth >
        (this.policy.maxKineticAssemblyChainDepth || 80)
    ) {
      throw new HsmAdapterError(
        "KINETICGATE_ASSEMBLY_CHAIN_DEPTH_EXCEEDED",
        `kinetic assembly chain depth ${request.kineticAssemblyChainDepth} exceeds maximum ${this.policy.maxKineticAssemblyChainDepth}`,
      );
    }
    const poolId =
      request.poolId || `kinetic-${crypto.randomBytes(4).toString("hex")}`;
    if (this._pools.has(poolId)) {
      throw new HsmAdapterError(
        "KINETICGATE_DUPLICATE",
    ...
    const pool = {
      ...
      blindedIsogenyKeyExchangeDigestCommitment:
        request.blindedIsogenyKeyExchangeDigestCommitment,
  ...
  verifyKineticPosture(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "KINETICGATE_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (!request.proofValid) {
      throw new HsmAdapterError(
        "KINETICGATE_PROOF_INVALID",
        `kinetic posture proof for pool ${request.poolId} is invalid`,
      );
    }
  ...
  completeAssemblyAccreditation(request) {
    ...
    if (!pool.kineticPostureVerified) {
      throw new HsmAdapterError(
        "KINETICGATE_POSTURE_NOT_VERIFIED",
        `pool ${request.poolId} kinetic posture not verified`,
      );
    }
    const signatures = request.roboticSignatures || [];
    if (signatures.length < (this.policy.minRoboticQuorum || 40)) {
      throw new HsmAdapterError(
        "KINETICGATE_QUORUM_INSUFFICIENT",
        `robotic quorum ${signatures.length} below minimum ${this.policy.minRoboticQuorum}`,
      );
    }
    if (
      this.policy.requireAssemblyEthicsOversightCommitteeAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.assemblyEthicsOversightCommitteeAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "KINETICGATE_OVERSIGHT_COMMITTEE_UNATTESTED",
            "assembly ethics oversight committee attestation invalid",
          );
        }
      } catch (err) {
        if (err instanceof HsmAdapterError) throw err;
    ...
    if (this._audit) {
      this._audit("ASSEMBLY_ACCREDITATION_COMPLETED", {
        poolId: request.poolId,
        roboticQuorum: signatures.length,
      });
    }
    return pool;
  }
...
function _validateInitRequest(policy, request) {
  if (!request || typeof request !== "object") {
    throw new HsmAdapterError(
      "KINETICGATE_INIT_SHAPE_INVALID",
      "request must be an object",
    );
  }
  if (
    !request.blindedIsogenyKeyExchangeDigestCommitment ||
    !request.blindedKineticPostureCommitment
  ) {
    throw new HsmAdapterError(
      "KINETICGATE_INIT_SHAPE_INVALID",
      "blindedIsogenyKeyExchangeDigestCommitment and blindedKineticPostureCommitment are required",
    );
  }
  if (
    typeof request.roboticQuorum === "number" &&
    request.roboticQuorum < (policy.minRoboticQuorum || 40)
  ) {
    throw new HsmAdapterError(
      "KINETICGATE_QUORUM_INSUFFICIENT",
      `robotic quorum ${request.roboticQuorum} below minimum ${policy.minRoboticQuorum || 40}`,
    );
  }
}
...
function _validateProofRequest(request) {
  if (!request || typeof request !== "object" || !request.poolId) {
    throw new HsmAdapterError(
      "KINETICGATE_PROOF_SHAPE_INVALID",
      "poolId is required",
    );
  }
}
...
function _validateAccreditationRequest(policy, request) {
  if (!request || typeof request !== "object" || !request.poolId) {
    throw new HsmAdapterError(
      "KINETICGATE_ACCREDITATION_SHAPE_INVALID",
      "poolId is required",
    );
  }
  const signatures = request.roboticSignatures || [];
  if (signatures.length < (policy.minRoboticQuorum || 40)) {
    throw new HsmAdapterError(
      "KINETICGATE_QUORUM_INSUFFICIENT",
      `robotic quorum ${signatures.length} below minimum ${policy.minRoboticQuorum || 40}`,
    );
  }
}
