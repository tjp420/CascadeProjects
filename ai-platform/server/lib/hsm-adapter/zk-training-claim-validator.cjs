 ...
 *
 * Succinct training verifier that processes non-interactive
 * zero-knowledge range and accreditation proofs, ensuring
 * that an entity's hidden training claim status strictly
 * satisfies policy-defined thresholds without disclosing
 * individual model or dataset attributes. Triggers defensive node
 * bans for malformed or out-of-order training claims.
 *
 * Extended with hardware-accelerated SNARK proof generation,
 * batch training claim verification, slashing window
...
const SLASH_REASON = {
  ...
  TRAINING_WINDOW_OUT_OF_BOUNDS: 'training_window_out_of_bounds',
...
class ZkTrainingClaimValidator {
  ...
  verifyTrainingClaim(request) {
    ...
    if (typeof request.trainingWindowSeconds === 'number' && request.trainingWindowSeconds > (this.policy.maxTrainingWindowSeconds || 63072000)) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.TRAINING_WINDOW_OUT_OF_BOUNDS);
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an training claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('TRAINCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.datasetProvenanceMetric !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('TRAINCLAIM_HW_PROOF_FIELDS_MISSING',
        'datasetProvenanceMetric and claimValue numbers are required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('TRAINCLAIM_HUB_MISSING', 'AI model training gating hub is required');
    }
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.datasetProvenanceMetric}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
      zkTrainingRangeProofHash: proofHash,
      poolId: request.poolId,
      datasetProvenanceMetric: request.datasetProvenanceMetric,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit('TRAINCLAIM_HW_SNARK_PROOF_GENERATED', { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple training claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyTrainingClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('TRAINCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('TRAINCLAIM_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyTrainingClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || 'TRAINCLAIM_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit('TRAINCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('TRAINCLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('TRAINCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('TRAINCLAIM_HUB_MISSING', 'AI model training gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('TRAINCLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
    const maxWindow = this.policy.maxTrainingWindowSeconds || 63072000;
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId) {
      throw new HsmAdapterError('TRAINCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('TRAINCLAIM_AGG_NO_SIGNATURES', 'partialSignatures array is required');
    }
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('TRAINCLAIM_PEER_BANNED',
    ...
    if (partialSignatures.length < (this.policy.minTrainingOversightQuorum || 3)) {
      throw new HsmAdapterError('TRAINCLAIM_AGG_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('TRAINCLAIM_PARTIAL_SIGS_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit('TRAINCLAIM_SLASHED', { poolId, peerId, reason });
    }
  }
}
