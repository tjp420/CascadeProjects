...
class PqcMultiEnclaveConfidentialMeshStateReconciliationGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "MESHGATE_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.epochFinalityWindowSeconds === "number" &&
      request.epochFinalityWindowSeconds >
        (this.policy.maxEpochFinalityWindowSeconds || 10)
    ) {
      throw new HsmAdapterError(
        "MESHGATE_EPOCH_FINALITY_WINDOW_EXCEEDED",
        `epoch finality window ${request.epochFinalityWindowSeconds} exceeds maximum ${this.policy.maxEpochFinalityWindowSeconds}`,
      );
    }
    if (
      typeof request.reconciliationChainDepth === "number" &&
      request.reconciliationChainDepth >
        (this.policy.maxReconciliationChainDepth || 100)
    ) {
      throw new HsmAdapterError(
        "MESHGATE_RECONCILIATION_CHAIN_DEPTH_EXCEEDED",
        `reconciliation chain depth ${request.reconciliationChainDepth} exceeds maximum ${this.policy.maxReconciliationChainDepth}`,
      );
    }
    if (
      ...
      throw new HsmAdapterError(
        "MESHGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    const pool = {
      poolId,
      blindedConfidentialStateReconciliationDigestCommitment:
        request.blindedConfidentialStateReconciliationDigestCommitment,
  ...
  reconcileMeshState(request) {
    ...
    if (request.proofValid === false) {
      throw new HsmAdapterError(
        "MESHGATE_PROOF_INVALID",
        `mesh reconciliation proof invalid for pool ${request.poolId}`,
      );
    }
  ...
  completeEpochFinality(request) {
    ...
    if (!pool.meshStateReconciled) {
      throw new HsmAdapterError(
        "MESHGATE_MESH_STATE_NOT_RECONCILED",
        `pool ${request.poolId} mesh state not reconciled`,
      );
    }
    ...
    if (signatures.length < minQuorum) {
      throw new HsmAdapterError(
        "MESHGATE_QUORUM_INSUFFICIENT",
        `mesh quorum signatures ${signatures.length} below minimum ${minQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  if (!request.blindedConfidentialStateReconciliationDigestCommitment) {
    throw new HsmAdapterError(
      "MESHGATE_FIELDS_MISSING",
      "blindedConfidentialStateReconciliationDigestCommitment is required",
    );
  }
  if (!request.blindedEpochFinalityCommitment) {
    throw new HsmAdapterError(
      "MESHGATE_FIELDS_MISSING",
      "blindedEpochFinalityCommitment is required",
    );
  }
}
