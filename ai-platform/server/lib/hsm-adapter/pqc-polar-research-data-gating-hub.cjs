 ...
 *
 * Extended with batch pool initialization, research chain depth
...
class PqcPolarResearchDataGatingHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('POLARGATE_MAX_POOLS',
  ...
  /**
   * Batch initialize multiple polar research data gating pools.
   * @param {object[]} requests
   * @returns {object}
   */
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('POLARGATE_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('POLARGATE_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        ...
        results.push({
          ...
          initialized: false,
          error: err.code || 'POLARGATE_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    if (this._audit) {
      this._audit('POLARGATE_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  /**
   * Rebalance research chain depth for a pool.
   * @param {object} request
   * @returns {object}
   */
  rebalanceResearchChainDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('POLARGATE_REBALANCE_FIELDS_MISSING', 'poolId is required');
    }
    ...
    if (pool.status !== POOL_STATUS.OPEN && pool.status !== POOL_STATUS.REBALANCING) {
      throw new HsmAdapterError('POLARGATE_NOT_REBALANCEABLE',
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError('POLARGATE_REBALANCE_DIRECTION_INVALID',
    ...
    if (typeof request.rebalanceAmount !== 'number' || request.rebalanceAmount <= 0) {
      throw new HsmAdapterError('POLARGATE_REBALANCE_AMOUNT_INVALID',
    ...
    const rebalance = {
      ...
      newResearchChainDepth: request.newResearchChainDepth !== undefined ? request.newResearchChainDepth : pool.researchChainDepth,
      rebalancedAt: Math.floor(Date.now() / 1000),
    };
    this._rebalances.set(rebalanceId, rebalance);
    this._rebalanceCount++;
    if (request.newResearchChainDepth !== undefined) {
      if (request.newResearchChainDepth > (this.policy.maxResearchChainDepth || 14)) {
        throw new HsmAdapterError('POLARGATE_RESEARCH_DEPTH_EXCEEDED',
          `new research chain depth ${request.newResearchChainDepth} exceeds maximum ${this.policy.maxResearchChainDepth}`);
      }
      pool.researchChainDepth = request.newResearchChainDepth;
    }
    if (this._audit) {
      this._audit('POLARGATE_RESEARCH_DEPTH_REBALANCED', { ...rebalance });
    }
    return rebalance;
  }
  ...
  completeAccreditation(request) {
    ...
    if (signatures.length < (this.policy.minPolarQuorum || 5)) {
      throw new HsmAdapterError('POLARGATE_ACCREDITATION_QUORUM_INSUFFICIENT', `accreditation signatures ${signatures.length} below minimum ${this.policy.minPolarQuorum}`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('POLARGATE_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('POLARGATE_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.ACCREDITED) {
      throw new HsmAdapterError('POLARGATE_NOT_ACCREDITED',
    ...
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('POLARGATE_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('POLARGATE_SETTLE_CHAIN_MISMATCH',
    ...
    this._settleCount++;
    if (this._audit) {
      this._audit('POLARGATE_SETTLED', { ...settlement });
    }
    return settlement;
  }
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('POLARGATE_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minPolarQuorum || 5)) {
      throw new HsmAdapterError('POLARGATE_ACCREDITATION_QUORUM_INSUFFICIENT',
        `${partialSignatures.length} signatures below minimum ${this.policy.minPolarQuorum || 5}`);
    }
    ...
    if (this._audit) {
      this._audit('POLARGATE_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.ACCREDITED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('POLARGATE_ALREADY_ACCREDITED',
        `pool ${poolId} has been accredited/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('POLARGATE_ALREADY_CANCELLED',
    ...
    this._cancelCount++;
    if (this._audit) {
      this._audit('POLARGATE_CANCELLED', { poolId });
    }
  ...
  getPools() {
    return Array.from(this._pools.values()).map(p => ({
      ...
      status: p.status,
      researchChainDepth: p.researchChainDepth,
      dataRetentionWindowSeconds: p.dataRetentionWindowSeconds,
      researchClaimVerified: p.researchClaimVerified,
    }));
  }
...
module.exports = { PqcPolarResearchDataGatingHub, POOL_STATUS, REBALANCE_DIRECTION };
