...
class TokenClaimVerifier {
  ...
  verify(token, claim = {}) {
    ...
    if (age < minAge) {
      throw new HsmAdapterError('ISSUANCE_PROOF_TOO_YOUNG', `proof age ${age}s below minimum ${minAge}s`);
    }
    if (age > maxAge) {
      throw new HsmAdapterError('ISSUANCE_PROOF_TOO_OLD', `proof age ${age}s exceeds maximum ${maxAge}s`);
    }
    if (!token.proof) {
      throw new HsmAdapterError('ISSUANCE_PROOF_MISSING', 'token has no proof');
    }
    if (!claim.commitment || claim.commitment !== token.commitment) {
      throw new HsmAdapterError('ISSUANCE_CLAIM_MISMATCH', 'claim commitment does not match token');
    }
    const expected = _recomputeProof(token.assetId, claim.amount, claim.blinding);
    if (token.proof !== expected) {
      throw new HsmAdapterError('ISSUANCE_PROOF_INVALID', 'zk-snark proof verification failed');
    }
    if (this._audit) {
      this._audit('ISSUANCE_PROOF_VALIDATED', { assetId: token.assetId, timestamp: now });
    }
