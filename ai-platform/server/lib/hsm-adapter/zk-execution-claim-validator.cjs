  ...
  verifyExecutionClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "EXECCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (pool.status !== "open") {
      throw new HsmAdapterError(
        "EXECCLAIM_POOL_NOT_OPEN",
    ...
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "EXECCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (
      this.policy.banMalformedOrOutOfOrderExecutionClaims &&
      request.peerId &&
      this._bannedPeers.has(request.peerId)
    ) {
      throw new HsmAdapterError(
        "EXECCLAIM_PEER_BANNED",
    ...
    const claimHash = crypto
      ...
      .update(
        JSON.stringify({
          ...
          blindedExecutionStateCommitment:
          ...
          verifiableComputationProofHash:
            request.verifiableComputationProofHash,
        }),
      )
      .digest("hex");
    if (
      ...
      throw new HsmAdapterError(
        "EXECCLAIM_DUPLICATE",
        `duplicate execution claim for pool ${request.poolId}`,
      );
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.zkExecutionRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderExecutionClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "EXECCLAIM_ZK_PROOF_MISSING",
      "zkExecutionRangeProofHash is required",
    );
  }
  if (!request.verifiableComputationProofHash) {
    if (policy.banMalformedOrOutOfOrderExecutionClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "EXECCLAIM_VC_PROOF_HASH_MISSING",
      "verifiableComputationProofHash is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "EXECCLAIM_OVERSIGHT_ATTESTATION_MISSING",
