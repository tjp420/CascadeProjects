 ...
 * that an entity's hidden provenance claim status strictly
 * satisfies policy-defined thresholds without disclosing
 * individual supplier or manufacturing attributes. Triggers defensive node
 * bans for malformed or out-of-order provenance claims.
 *
 * Extended with hardware-accelerated SNARK proof generation,
 * batch provenance claim verification, slashing window
...
class ZkProvenanceClaimValidator {
  ...
  verifyProvenanceClaim(request) {
    ...
    if (typeof request.transitExpirationSeconds === 'number' && request.transitExpirationSeconds > (this.policy.maxTransitExpirationSeconds || 7776000)) {
      ...
      throw new HsmAdapterError('SUPPLYCLAIM_TRANSIT_EXPIRATION_OUT_OF_BOUNDS', `transcript expiration seconds ${request.transitExpirationSeconds} exceeds maximum ${this.policy.maxTransitExpirationSeconds}`);
    }
    ...
    const claim = {
      ...
      blindedManufacturingMetricCommitment: request.blindedManufacturingMetricCommitment || 'unspecified',
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an provenance claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('SUPPLYCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.manufacturingMetric !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('SUPPLYCLAIM_HW_PROOF_FIELDS_MISSING',
        'manufacturingMetric and claimValue numbers are required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('SUPPLYCLAIM_HUB_MISSING', 'supply chain provenance gating hub is required');
    }
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.manufacturingMetric}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
      zkProvenanceRangeProofHash: proofHash,
      poolId: request.poolId,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit('SUPPLYCLAIM_HW_SNARK_PROOF_GENERATED', { ...proof });
    }
    return proof;
  }
  ...
  batchVerifyProvenanceClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('SUPPLYCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('SUPPLYCLAIM_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        ...
        results.push({
          ...
          verified: false,
          error: err.code || 'SUPPLYCLAIM_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit('SUPPLYCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    ...
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('SUPPLYCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('SUPPLYCLAIM_HUB_MISSING', 'supply chain provenance gating hub is required');
    }
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId) {
      throw new HsmAdapterError('SUPPLYCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('SUPPLYCLAIM_AGG_NO_SIGNATURES', 'partialSignatures array is required');
    }
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('SUPPLYCLAIM_PEER_BANNED',
    ...
    if (partialSignatures.length < (this.policy.minSupplierCheckpointQuorum || 3)) {
      throw new HsmAdapterError('SUPPLYCLAIM_AGG_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('SUPPLYCLAIM_PARTIAL_SIGS_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit('SUPPLYCLAIM_SLASHED', { poolId, peerId, reason });
    }
  }
}
