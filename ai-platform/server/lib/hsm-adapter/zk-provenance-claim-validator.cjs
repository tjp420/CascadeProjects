...
class ZkProvenanceClaimValidator {
  ...
  verifyProvenanceClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "SUPPLYCLAIM_HUB_MISSING",
        "supply chain provenance gating hub is required",
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "SUPPLYCLAIM_COMMITTEE_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SUPPLYCLAIM_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkProvenanceRangeProofHash ||
      typeof request.zkProvenanceRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "SUPPLYCLAIM_ZK_PROOF_MISSING",
        "zero-knowledge provenance range proof hash is required",
      );
    }
    if (
      ...
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "SUPPLYCLAIM_PARTIAL_SIG_MISSING",
    ...
    if (
      ...
      this._recordSlash(
        request.poolId,
        request.peerId,
        SLASH_REASON.TRANSIT_EXPIRATION_OUT_OF_BOUNDS,
      );
      throw new HsmAdapterError(
        "SUPPLYCLAIM_TRANSIT_EXPIRATION_OUT_OF_BOUNDS",
    ...
    if (this._verifiedClaims.has(claimKey)) {
      ...
      throw new HsmAdapterError(
        "SUPPLYCLAIM_DUPLICATE",
        `provenance claim for pool ${request.poolId} already verified`,
      );
    }
    const claimId =
    ...
    const claim = {
      ...
      blindedSupplierHashCommitment:
        request.blindedSupplierHashCommitment || "unspecified",
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "PROVCLAIM_HW_PROOF_FIELDS_MISSING",
        "poolId is required",
      );
    }
    ...
    if (!pool)
      throw new HsmAdapterError(
        "PROVCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    if (
      typeof request.manufacturingMetric !== "number" ||
    ...
    const proofHash = crypto
      .createHash("sha256")
      .update(
        `${request.poolId}:${request.manufacturingMetric}:${request.claimValue}:${this._hwAccelType}`,
      )
      .digest("hex");
    const proof = {
    ...
    if (this._audit)
      this._audit("PROVCLAIM_HW_SNARK_PROOF_GENERATED", { ...proof });
    this._hwProofCount++;
    return proof;
  }
  ...
  batchVerifyProvenanceClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "PROVCLAIM_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > (this.policy.maxBatchSize || 100)) {
      throw new HsmAdapterError(
        "PROVCLAIM_BATCH_TOO_LARGE",
        `${requests.length} exceeds max batch size ${this.policy.maxBatchSize || 100}`,
      );
    }
    const results = [];
    let verifiedCount = 0;
    ...
    for (const req of requests) {
      try {
        ...
        results.push({
          ...
          verified: false,
          error: err.code || "PROVCLAIM_BATCH_ERROR",
        });
        failedCount++;
      }
    }
    ...
    if (this._audit)
      this._audit("PROVCLAIM_BATCH_VERIFIED", {
        verifiedCount,
        failedCount,
        batchSize: requests.length,
      });
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId)
      throw new HsmAdapterError(
        "PROVCLAIM_WINDOW_FIELDS_MISSING",
        "poolId is required",
      );
    if (typeof claimTimestamp !== "number" || claimTimestamp <= 0)
      throw new HsmAdapterError(
        "PROVCLAIM_WINDOW_FIELDS_MISSING",
        "claimTimestamp must be positive number",
      );
    if (!this._hub)
      throw new HsmAdapterError("PROVCLAIM_HUB_MISSING", "hub is required");
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId)
      throw new HsmAdapterError(
        "PROVCLAIM_AGG_FIELDS_MISSING",
        "poolId is required",
      );
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0)
      throw new HsmAdapterError(
        "PROVCLAIM_AGG_NO_SIGNATURES",
        "partialSignatures array is required",
      );
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId))
        throw new HsmAdapterError(
          "PROVCLAIM_PEER_BANNED",
          `peer ${sig.peerId} is banned`,
        );
    }
    if (
      partialSignatures.length < (this.policy.minSupplierCheckpointQuorum || 3)
    )
      throw new HsmAdapterError(
        "PROVCLAIM_AGG_INSUFFICIENT",
    ...
    if (this._audit)
      this._audit("PROVCLAIM_PARTIAL_SIGS_AGGREGATED", {
        poolId,
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrOutOfOrderProvenanceClaims &&
      typeof request.peerId === "string"
    ) {
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit)
      this._audit("PROVCLAIM_SLASHED", { poolId, peerId, reason });
  }
}
...
function _validateClaimRequest(policy, request) {
  ...
  if (
    ...
    throw new HsmAdapterError(
      "SUPPLYCLAIM_ATTESTATION_MISSING",
