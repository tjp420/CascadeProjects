...
class PqcScientificReproducibilityGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireResearchAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.researchAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "RESEARCHGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "research authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "RESEARCHGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "RESEARCHGATE_PQC_SCHEME_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "RESEARCHGATE_REPLICATION_WINDOW_EXCEEDED",
        `replication window seconds ${request.replicationWindowSeconds} exceeds maximum ${this.policy.maxReplicationWindowSeconds}`,
      );
    }
    if (
      typeof request.citationDepth === "number" &&
      request.citationDepth > (this.policy.maxCitationDepth || 48)
    ) {
      throw new HsmAdapterError(
        "RESEARCHGATE_CITATION_DEPTH_EXCEEDED",
        `citation depth ${request.citationDepth} exceeds maximum ${this.policy.maxCitationDepth}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedReplicationResultCommitment:
        request.blindedReplicationResultCommitment,
      blindedReviewerIdentityCommitment:
        request.blindedReviewerIdentityCommitment,
  ...
  completeAccreditation(request) {
    ...
    if (!pool.replicationClaimVerified) {
      throw new HsmAdapterError(
        "RESEARCHGATE_REPLICATION_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} replication claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "RESEARCHGATE_INTEGRITY_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minPeerReviewQuorum || 3)) {
      throw new HsmAdapterError(
        "RESEARCHGATE_QUORUM_INSUFFICIENT",
        `peer review signatures ${signatures.length} below minimum ${this.policy.minPeerReviewQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "RESEARCHGATE_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedExperimentHashCommitment ||
    !request.blindedReplicationResultCommitment ||
    !request.blindedReviewerIdentityCommitment
  ) {
    throw new HsmAdapterError(
      "RESEARCHGATE_FIELDS_MISSING",
      "blindedExperimentHashCommitment, blindedReplicationResultCommitment, and blindedReviewerIdentityCommitment are required",
    );
  }
  if (typeof request.replicationWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "RESEARCHGATE_FIELDS_MISSING",
      "replicationWindowSeconds is required",
    );
  }
  if (typeof request.citationDepth !== "number") {
    throw new HsmAdapterError(
      "RESEARCHGATE_FIELDS_MISSING",
      "citationDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "RESEARCHGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "RESEARCHGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "RESEARCHGATE_INTEGRITY_ATTESTATION_MISSING",
