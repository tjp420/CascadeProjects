...
class HomomorphicDbLookupEngine {
  ...
  execute(query) {
    if (this.policy.requireQueryAttestation && this._attestationClient) {
      ...
      if (!result.verified) {
        throw new HsmAdapterError(
          "DB_LOOKUP_ATTESTATION_INVALID",
          "query attestation is not valid",
        );
      }
    }
    if (
      !Array.isArray(query.encryptedFilters) ||
      query.encryptedFilters.length === 0
    ) {
      throw new HsmAdapterError(
        "DB_LOOKUP_FILTERS_MISSING",
        "encrypted filters are required",
      );
    }
    if (
      query.encryptedFilters.length >
      (this.policy.maxEncryptedColumnsPerQuery || 8)
    ) {
      throw new HsmAdapterError(
        "DB_LOOKUP_COLUMNS_EXCEEDED",
        `filters ${query.encryptedFilters.length} exceed maximum ${this.policy.maxEncryptedColumnsPerQuery}`,
      );
    }
    if (
      typeof query.blindingType === "string" &&
      !this.policy.allowedBlindingTypes.includes(query.blindingType)
    ) {
      throw new HsmAdapterError(
        "DB_LOOKUP_BLINDING_BLOCKED",
        `blinding type ${query.blindingType} is not allowed`,
      );
    }
    if (query.crossTenant && !this.policy.allowCrossTenantTables) {
      throw new HsmAdapterError(
        "DB_LOOKUP_CROSS_TENANT_BLOCKED",
    ...
    if (age > (this.policy.maxQueryAgeSeconds || 60)) {
      throw new HsmAdapterError(
        "DB_LOOKUP_QUERY_EXPIRED",
        `query age ${age}s exceeds maximum ${this.policy.maxQueryAgeSeconds}s`,
      );
    }
    const matches = [];
    for (const record of query.records || []) {
      const score = _dotProduct(
        query.encryptedFilters,
        record.encryptedColumns,
      );
      if (score > 0n) {
