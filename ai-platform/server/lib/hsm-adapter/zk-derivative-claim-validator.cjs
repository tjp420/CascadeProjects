 ...
 * that an entity's hidden derivative claim status strictly
 * satisfies policy-defined thresholds without disclosing
 * individual derivative or counterparty risk attributes. Triggers defensive node
 * bans for malformed or out-of-order derivative claims.
 *
 * Extended with hardware-accelerated SNARK proof generation,
 * batch derivative claim verification, slashing window
...
const SLASH_REASON = {
  ...
  CONTRACT_EXPIRATION_OUT_OF_BOUNDS: 'contract_expiration_out_of_bounds',
...
class ZkDerivativeClaimValidator {
  ...
  verifyDerivativeClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError('DERIVCLAIM_HUB_MISSING', 'financial derivatives gating hub is required');
    }
    if (this.policy.requireRiskCommitteeAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('DERIVCLAIM_COMMITTEE_UNATTESTED', 'risk committee attestation invalid');
        }
      } catch (err) {
    ...
    if (typeof request.attestationAuthority === 'string' && !this.policy.allowedAttestationAuthorities.includes(request.attestationAuthority)) {
      throw new HsmAdapterError('DERIVCLAIM_AUTHORITY_BLOCKED', `attestation authority ${request.attestationAuthority} is not allowed; permitted: ${this.policy.allowedAttestationAuthorities.join(', ')}`);
    }
    if (typeof request.peerId === 'string' && this._bannedPeers.has(request.peerId)) {
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.BANNED_PEER);
      throw new HsmAdapterError('DERIVCLAIM_PEER_BANNED', `peer ${request.peerId} is banned`);
    }
    if (!request.zkDerivativeRangeProofHash || typeof request.zkDerivativeRangeProofHash !== 'string') {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError('DERIVCLAIM_ZK_PROOF_MISSING', 'zero-knowledge derivative range proof hash is required');
    }
    if (!request.partialSignature || typeof request.partialSignature !== 'string') {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError('DERIVCLAIM_PARTIAL_SIG_MISSING', 'partial signature is required');
    }
    const pool = this._hub.getPool(request.poolId);
    if (!pool) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.POOL_NOT_FOUND);
      throw new HsmAdapterError('DERIVCLAIM_POOL_NOT_FOUND', `pool ${request.poolId} not found`);
    }
    if (typeof request.contractExpirationSeconds === 'number' && request.contractExpirationSeconds > (this.policy.maxContractExpirationSeconds || 31536000)) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.CONTRACT_EXPIRATION_OUT_OF_BOUNDS);
      throw new HsmAdapterError('DERIVCLAIM_CONTRACT_EXPIRATION_OUT_OF_BOUNDS', `transcript expiration seconds ${request.contractExpirationSeconds} exceeds maximum ${this.policy.maxContractExpirationSeconds}`);
    }
    ...
    if (this._verifiedClaims.has(claimKey)) {
      ...
      throw new HsmAdapterError('DERIVCLAIM_DUPLICATE', `derivative claim for pool ${request.poolId} already verified`);
    }
    ...
    const claim = {
      ...
      blindedCounterpartyRiskCommitment: request.blindedCounterpartyRiskCommitment || 'unspecified',
      ...
      riskCommitteeAttestationHash: request.riskCommitteeAttestationHash || 'unspecified',
      verifiedAt: now,
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an derivative claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('DERIVCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.counterpartyRiskMetric !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('DERIVCLAIM_HW_PROOF_FIELDS_MISSING',
        'counterpartyRiskMetric and claimValue numbers are required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('DERIVCLAIM_HUB_MISSING', 'financial derivatives gating hub is required');
    }
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.counterpartyRiskMetric}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
      zkDerivativeRangeProofHash: proofHash,
      poolId: request.poolId,
      counterpartyRiskMetric: request.counterpartyRiskMetric,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit('DERIVCLAIM_HW_SNARK_PROOF_GENERATED', { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple derivative claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyDerivativeClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('DERIVCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('DERIVCLAIM_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyDerivativeClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || 'DERIVCLAIM_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit('DERIVCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('DERIVCLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('DERIVCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('DERIVCLAIM_HUB_MISSING', 'financial derivatives gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('DERIVCLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
    const maxWindow = this.policy.maxContractExpirationSeconds || 31536000;
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId) {
      throw new HsmAdapterError('DERIVCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('DERIVCLAIM_AGG_NO_SIGNATURES', 'partialSignatures array is required');
    }
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('DERIVCLAIM_PEER_BANNED',
    ...
    if (partialSignatures.length < (this.policy.minClearingHouseQuorum || 3)) {
      throw new HsmAdapterError('DERIVCLAIM_AGG_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('DERIVCLAIM_PARTIAL_SIGS_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  _banPeerIfPolicy(request) {
    if (this.policy.banMalformedOrOutOfOrderDerivativeClaims && typeof request.peerId === 'string') {
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit('DERIVCLAIM_SLASHED', { poolId, peerId, reason });
    }
  }
}
...
function _validateClaimRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError('DERIVCLAIM_FIELDS_MISSING', 'poolId is required');
  }
  if (policy.requireRiskCommitteeAttestation && !request.riskCommitteeAttestation) {
    throw new HsmAdapterError('DERIVCLAIM_ATTESTATION_MISSING', 'risk committee attestation is required');
  }
}
