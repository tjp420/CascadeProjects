...
class ZkQkdLinkClaimValidator {
  ...
  verifyQkdLinkClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError('QKDSWITCHCLAIM_POOL_NOT_FOUND', `pool ${request.poolId} not found`);
    }
    if (pool.status !== 'open') {
      throw new HsmAdapterError('QKDSWITCHCLAIM_POOL_NOT_OPEN', `pool ${request.poolId} is not open`);
    }
    if (this.policy.requireQkdEthicsOversightCommitteeAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('QKDSWITCHCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED', 'qkd ethics oversight committee attestation invalid');
        }
      } catch (err) {
    ...
    if (this.policy.banMalformedOrOutOfOrderQkdLinkClaims && request.peerId && this._bannedPeers.has(request.peerId)) {
      throw new HsmAdapterError('QKDSWITCHCLAIM_PEER_BANNED', `peer ${request.peerId} is banned`);
    }
    ...
    if (this.policy.banMalformedOrOutOfOrderQkdLinkClaims && this._verifiedClaims.has(claimHash)) {
      if (request.peerId) this._bannedPeers.add(request.peerId);
      throw new HsmAdapterError('QKDSWITCHCLAIM_DUPLICATE', `duplicate qkd link claim for pool ${request.poolId}`);
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  if (!request.poolId) {
    throw new HsmAdapterError('QKDSWITCHCLAIM_FIELDS_MISSING', 'poolId is required');
  }
  if (!request.blindedOpticalLinkPathCommitment || !request.blindedQuantumSecretSharingCommitment || !request.blindedEntanglingChannelCommitment) {
    throw new HsmAdapterError('QKDSWITCHCLAIM_FIELDS_MISSING', 'blindedOpticalLinkPathCommitment, blindedQuantumSecretSharingCommitment, and blindedEntanglingChannelCommitment are required');
  }
  if (!request.zkQkdLinkRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderQkdLinkClaims && request.peerId) bannedPeers.add(request.peerId);
    throw new HsmAdapterError('QKDSWITCHCLAIM_ZK_PROOF_MISSING', 'zkQkdLinkRangeProofHash is required');
  }
  if (!request.quantumSecretSharingDigest) {
    if (policy.banMalformedOrOutOfOrderQkdLinkClaims && request.peerId) bannedPeers.add(request.peerId);
    throw new HsmAdapterError('QKDSWITCHCLAIM_QUANTUM_SECRET_SHARING_DIGEST_MISSING', 'quantumSecretSharingDigest is required');
  }
  if (policy.requireQkdEthicsOversightCommitteeAttestation && !request.qkdEthicsOversightCommitteeAttestation) {
    throw new HsmAdapterError('QKDSWITCHCLAIM_OVERSIGHT_ATTESTATION_MISSING', 'qkd ethics oversight committee attestation is required');
  }
}
