...
class HomomorphicMasker {
    ...
    if (options.modulus) {
      if (typeof options.modulus !== "bigint" || options.modulus <= 0n) {
        throw new HsmAdapterError(
          "INVALID_INPUT",
          "modulus must be a positive bigint",
        );
      }
      this._p = options.modulus;
      this._bits = Number(options.modulus.toString(2).length);
    } else {
      this._p = crypto.generatePrimeSync(this._bits, {
        safe: true,
        bigint: true,
      });
    }
  ...
  _ensureInField(value) {
    ...
    if (value >= this._overflow) {
      throw new HsmAdapterError(
        "HOMOMORPHIC_OVERFLOW",
        `value ${value} exceeds 2p`,
      );
    }
  }
  ...
  unmask(ciphertext, blindingFactor) {
    if (typeof ciphertext !== "bigint" || typeof blindingFactor !== "bigint") {
      throw new HsmAdapterError(
        "INVALID_BLIND",
        "ciphertext and blindingFactor must be BigInt",
      );
    }
    if (
      ciphertext < 0n ||
      ciphertext >= this._p ||
      blindingFactor < 0n ||
      blindingFactor >= this._p
    ) {
      throw new HsmAdapterError(
        "INVALID_BLIND",
        "ciphertext and blindingFactor must be in [0, p)",
      );
    }
  ...
  combineBlinds(blinds) {
    if (!Array.isArray(blinds) || blinds.some((b) => typeof b !== "bigint")) {
      throw new HsmAdapterError(
        "INVALID_BLIND",
        "blinds must be an array of BigInt",
      );
    }
    ...
    if (total >= this._overflow) {
      throw new HsmAdapterError(
        "HOMOMORPHIC_OVERFLOW",
        `combined blinding ${total} exceeds 2p`,
      );
    }
