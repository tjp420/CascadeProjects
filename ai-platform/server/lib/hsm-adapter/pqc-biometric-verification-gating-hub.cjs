...
class PqcBiometricVerificationGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireBiometricAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.biometricAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "BIOMETRICGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "biometric authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "BIOMETRICGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "BIOMETRICGATE_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.templateExpirationSeconds === "number" &&
      request.templateExpirationSeconds >
        (this.policy.maxTemplateExpirationSeconds || 15552000)
    ) {
      throw new HsmAdapterError(
        "BIOMETRICGATE_TEMPLATE_EXPIRATION_EXCEEDED",
        `template expiration seconds ${request.templateExpirationSeconds} exceeds maximum ${this.policy.maxTemplateExpirationSeconds}`,
      );
    }
    if (
      typeof request.livenessMetricDepth === "number" &&
      request.livenessMetricDepth > (this.policy.maxLivenessMetricDepth || 16)
    ) {
      throw new HsmAdapterError(
        "BIOMETRICGATE_LIVENESS_DEPTH_EXCEEDED",
        `liveness metric depth ${request.livenessMetricDepth} exceeds maximum ${this.policy.maxLivenessMetricDepth}`,
      );
    }
    const poolId =
  ...
  completeAccreditation(request) {
    ...
    if (!pool.biometricClaimVerified) {
      throw new HsmAdapterError(
        "BIOMETRICGATE_BIOMETRIC_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} biometric claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "BIOMETRICGATE_CLEARING_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minBiometricAuthorityQuorum || 3)) {
      throw new HsmAdapterError(
        "BIOMETRICGATE_QUORUM_INSUFFICIENT",
        `biometric authority signatures ${signatures.length} below minimum ${this.policy.minBiometricAuthorityQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
    ...
    if (this._audit) {
      this._audit("LIVENESS_ATTESTATION_ACCREDITATION_COMPLETED", {
        ...completion,
      });
    }
    return completion;
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "BIOMETRICGATE_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedTemplateHashCommitment ||
    !request.blindedLivenessMetricCommitment ||
    !request.blindedSubjectHashCommitment
  ) {
    throw new HsmAdapterError(
      "BIOMETRICGATE_FIELDS_MISSING",
      "blindedTemplateHashCommitment, blindedLivenessMetricCommitment, and blindedSubjectHashCommitment are required",
    );
  }
  if (typeof request.templateExpirationSeconds !== "number") {
    throw new HsmAdapterError(
      "BIOMETRICGATE_FIELDS_MISSING",
      "templateExpirationSeconds is required",
    );
  }
  if (typeof request.livenessMetricDepth !== "number") {
    throw new HsmAdapterError(
      "BIOMETRICGATE_FIELDS_MISSING",
      "livenessMetricDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "BIOMETRICGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "BIOMETRICGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "BIOMETRICGATE_CLEARING_ATTESTATION_MISSING",
