...
class ZkSolvencyProofProcessor {
  ...
  verifySolvencyProof(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "SOLVENCYPROOF_HUB_MISSING",
        "lending collateral hub is required",
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "SOLVENCYPROOF_COMMITTEE_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SOLVENCYPROOF_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SOLVENCYPROOF_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkSolvencyRangeProofHash ||
      typeof request.zkSolvencyRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "SOLVENCYPROOF_ZK_PROOF_MISSING",
        "zero-knowledge solvency range proof hash is required",
      );
    }
    if (
      ...
      this._recordSlash(request, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "SOLVENCYPROOF_PARTIAL_SIG_MISSING",
    ...
    if (
      ...
      if (ltvRatio > 100) {
        ...
        throw new HsmAdapterError(
          "SOLVENCYPROOF_SUB_SOLVENCY",
          `LTV ratio ${ltvRatio}% exceeds 100% (collateral insufficient)`,
        );
      }
      if (ltvRatio < (this.policy.minLtvRatio || 50)) {
        ...
        this._recordSlash(request, SLASH_REASON.SUB_SOLVENCY);
        throw new HsmAdapterError(
          "SOLVENCYPROOF_LTV_BELOW_MINIMUM",
          `LTV ratio ${ltvRatio}% below minimum ${this.policy.minLtvRatio}%`,
        );
      }
    }
    ...
    if (this._verifiedProofs.has(proofKey)) {
      ...
      throw new HsmAdapterError(
        "SOLVENCYPROOF_DUPLICATE",
        `proof for pool ${request.poolId} already verified`,
      );
    }
    const proofId =
      request.proofId || `solvency-${crypto.randomBytes(4).toString("hex")}`;
    ...
    const proof = {
      ...
      blindedBorrowValueCommitment:
        request.blindedBorrowValueCommitment ||
        pool.blindedBorrowValueCommitment,
  ...
  batchVerifySolvencyProofs(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "SOLVENCYPROOF_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "SOLVENCYPROOF_BATCH_TOO_LARGE",
    ...
    if (this._audit) {
      this._audit("SOLVENCYPROOF_BATCH_VERIFIED", {
        verifiedCount,
        failedCount,
        batchSize: requests.length,
      });
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "SOLVENCYPROOF_GEN_FIELDS_MISSING",
        "poolId is required",
      );
    }
    if (!this._hub) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SOLVENCYPROOF_GEN_VALUES_MISSING",
    ...
    const zkSolvencyRangeProofHash = crypto
      .createHash("sha256")
      .update(
        `snark:${proofSeed.toString("hex")}:${request.poolId}:${request.collateralValue}:${request.borrowValue}`,
      )
      .digest("hex");
    const proof = {
      poolId: request.poolId,
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SOLVENCYPROOF_QUORUM_INSUFFICIENT",
    ...
    if (this._audit) {
      this._audit("SOLVENCYPROOF_SIGNATURES_AGGREGATED", {
        poolId,
  ...
  validateSlashingWindow(poolId, proofTimestamp) {
    ...
    if (typeof proofTimestamp !== "number") {
      throw new HsmAdapterError(
        "SOLVENCYPROOF_TIMESTAMP_INVALID",
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrSubSolvencyClaims &&
      typeof request.peerId === "string"
    ) {
  ...
  _recordSlash(request, reason) {
    ...
    this._slashCount++;
    if (this._audit) {
      this._audit("SOLVENCYPROOF_SLASHED", {
        poolId: request.poolId,
...
function _validateProofRequest(policy, request) {
  ...
  if (
    ...
    throw new HsmAdapterError(
      "SOLVENCYPROOF_ATTESTATION_MISSING",
