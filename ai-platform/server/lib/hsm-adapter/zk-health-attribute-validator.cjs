 ...
 * batch health claim verification, slashing window
...
const SLASH_REASON = {
  ...
  EXPIRATION_LIFETIME_OUT_OF_BOUNDS: "expiration_lifetime_out_of_bounds",
...
class ZkHealthAttributeValidator {
  ...
  verifyHealthClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "HEALTHCLAIM_HUB_MISSING",
        "health data gating hub is required",
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "HEALTHCLAIM_COMMITTEE_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "HEALTHCLAIM_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkHealthRangeProofHash ||
      typeof request.zkHealthRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "HEALTHCLAIM_ZK_PROOF_MISSING",
        "zero-knowledge health range proof hash is required",
      );
    }
    if (
      ...
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "HEALTHCLAIM_PARTIAL_SIG_MISSING",
    ...
    if (
      ...
      this._recordSlash(
        request.poolId,
        request.peerId,
        SLASH_REASON.EXPIRATION_LIFETIME_OUT_OF_BOUNDS,
      );
      throw new HsmAdapterError(
        "HEALTHCLAIM_EXPIRATION_LIFETIME_OUT_OF_BOUNDS",
    ...
    if (this._verifiedClaims.has(claimKey)) {
      ...
      throw new HsmAdapterError(
        "HEALTHCLAIM_DUPLICATE",
        `health claim for pool ${request.poolId} already verified`,
      );
    }
    const claimId =
    ...
    const claim = {
      ...
      blindedDiagnosticObservationCommitment:
        request.blindedDiagnosticObservationCommitment || "unspecified",
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for a health claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "HEALTHCLAIM_HW_PROOF_FIELDS_MISSING",
        "poolId is required",
      );
    }
    if (
      typeof request.diagnosticObservation !== "number" ||
      typeof request.claimValue !== "number"
    ) {
      throw new HsmAdapterError(
        "HEALTHCLAIM_HW_PROOF_FIELDS_MISSING",
        "diagnosticObservation and claimValue numbers are required",
      );
    }
    if (!this._hub) {
      throw new HsmAdapterError(
    ...
    const proofHash = crypto
      .createHash("sha256")
      .update(
        `${request.poolId}:${request.diagnosticObservation}:${request.claimValue}:${this._hwAccelType}`,
      )
      .digest("hex");
    const proof = {
      zkHealthRangeProofHash: proofHash,
      poolId: request.poolId,
      diagnosticObservation: request.diagnosticObservation,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit("HEALTHCLAIM_HW_SNARK_PROOF_GENERATED", { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple health claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyHealthClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "HEALTHCLAIM_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "HEALTHCLAIM_BATCH_TOO_LARGE",
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyHealthClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || "HEALTHCLAIM_BATCH_ERROR",
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit("HEALTHCLAIM_BATCH_VERIFIED", {
        verifiedCount,
        failedCount,
        batchSize: requests.length,
      });
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError(
        "HEALTHCLAIM_WINDOW_FIELDS_MISSING",
        "poolId is required",
      );
    }
    ...
    const now = Math.floor(Date.now() / 1000);
    const maxWindow = this.policy.maxRecordExpirationLifetimeSeconds || 7776000;
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    ...
    if (this._audit) {
      this._audit("HEALTHCLAIM_PARTIAL_SIGS_AGGREGATED", {
        poolId,
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrOutOfOrderHealthClaims &&
      typeof request.peerId === "string"
    ) {
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit("HEALTHCLAIM_SLASHED", { poolId, peerId, reason });
    }
  }
}
...
function _validateClaimRequest(policy, request) {
  ...
  if (
    ...
    throw new HsmAdapterError(
      "HEALTHCLAIM_ATTESTATION_MISSING",
