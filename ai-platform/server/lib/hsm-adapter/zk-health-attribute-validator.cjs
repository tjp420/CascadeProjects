...
class ZkHealthAttributeValidator {
  ...
  verifyHealthClaim(request) {
    ...
    if (typeof request.recordExpirationLifetimeSeconds === 'number' && request.recordExpirationLifetimeSeconds > (this.policy.maxRecordExpirationLifetimeSeconds || 7776000)) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.EXPIRATION_LIFETIME_OUT_OF_BOUNDS);
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('HEALTHCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.diagnosticObservation !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('HEALTHCLAIM_HW_PROOF_FIELDS_MISSING',
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.diagnosticObservation}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
  ...
  batchVerifyHealthClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('HEALTHCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('HEALTHCLAIM_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('HEALTHCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('HEALTHCLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('HEALTHCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('HEALTHCLAIM_HUB_MISSING', 'health data gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('HEALTHCLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId) {
      throw new HsmAdapterError('HEALTHCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('HEALTHCLAIM_AGG_NO_SIGNATURES', 'partialSignatures array is required');
    }
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('HEALTHCLAIM_PEER_BANNED',
    ...
    if (partialSignatures.length < (this.policy.minVerificationQuorum || 3)) {
      throw new HsmAdapterError('HEALTHCLAIM_AGG_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('HEALTHCLAIM_PARTIAL_SIGS_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
