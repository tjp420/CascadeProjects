...
class EncryptedStorageDeduplicator {
  ...
  submit(request) {
    ...
    if (this._bannedPeers.has(request.submitterPeerId)) {
      throw new HsmAdapterError('DEDUP_PEER_BANNED', `peer ${request.submitterPeerId} is banned`);
    }
    if (this.policy.requireSubmitterAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('DEDUP_SUBMITTER_UNATTESTED', 'submitter attestation invalid');
        }
      } catch (err) {
    ...
    if (chunkBitLength < (this.policy.minChunkBitLength || 256)) {
      throw new HsmAdapterError('DEDUP_CHUNK_TOO_SMALL', `chunk bit length ${chunkBitLength} below minimum ${this.policy.minChunkBitLength}`);
    }
    if (chunkBitLength > (this.policy.maxChunkBitLength || 4096)) {
      throw new HsmAdapterError('DEDUP_CHUNK_TOO_LARGE', `chunk bit length ${chunkBitLength} exceeds maximum ${this.policy.maxChunkBitLength}`);
    }
    if (typeof request.crossTenantAllocations === 'number' && request.crossTenantAllocations > (this.policy.maxCrossTenantChunkAllocations || 16)) {
      throw new HsmAdapterError('DEDUP_ALLOCATIONS_EXCEEDED', `cross-tenant allocations ${request.crossTenantAllocations} exceeds maximum ${this.policy.maxCrossTenantChunkAllocations}`);
    }
    if (typeof request.blindingGroup === 'string' && !this.policy.permittedBlindingGroups.includes(request.blindingGroup)) {
      throw new HsmAdapterError('DEDUP_BLINDING_GROUP_BLOCKED', `blinding group ${request.blindingGroup} is not permitted; allowed: ${this.policy.permittedBlindingGroups.join(', ')}`);
    }
    let ciphertextTagHash = crypto.createHash('sha256').update(request.chunk).digest('hex');
    if (this._convergenceGuard) {
      ciphertextTagHash = this._convergenceGuard.blind(ciphertextTagHash, request.blindingGroup || 'P-256');
    }
    ...
    if (existing) {
      ...
      return { deduplicated: true, chunkId, existingChunkId: existing.chunkId, ciphertextTagHash };
    }
    const record = {
      chunkId,
      sourceTenantId: request.sourceTenantId,
...
function _validateRequest(policy, request) {
  if (!request.sourceTenantId || !request.chunk) {
    throw new HsmAdapterError('DEDUP_FIELDS_MISSING', 'sourceTenantId and chunk are required');
  }
  if (policy.requireSubmitterAttestation && !request.submitterAttestation) {
    throw new HsmAdapterError('DEDUP_ATTESTATION_MISSING', 'submitter attestation is required');
  }
}
