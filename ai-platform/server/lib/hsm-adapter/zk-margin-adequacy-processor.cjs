...
class ZkMarginAdequacyProcessor {
  ...
  verifyMarginAdequacy(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "MARGINPROOF_HUB_MISSING",
        "blind option pool hub is required",
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "MARGINPROOF_COMMITTEE_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "MARGINPROOF_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkRangeProofHash ||
      typeof request.zkRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "MARGINPROOF_ZK_PROOF_MISSING",
        "zero-knowledge range proof hash is required",
      );
    }
    if (
      ...
      this._recordSlash(request, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "MARGINPROOF_PARTIAL_SIG_MISSING",
    ...
    if (!pool) {
      ...
      this._recordSlash(request, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "MARGINPROOF_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (
      typeof request.collateralValue === "number" &&
      typeof request.strikeValue === "number"
    ) {
      ...
      if (ratio < (this.policy.minCollateralRatio || 150)) {
        ...
        throw new HsmAdapterError(
          "MARGINPROOF_SUB_COLLATERAL",
          `collateral ratio ${ratio}% below minimum ${this.policy.minCollateralRatio}%`,
        );
      }
    }
    if (
      typeof request.collateralValue === "number" &&
      typeof request.strikeValue === "number" &&
      request.collateralValue < request.strikeValue
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request, SLASH_REASON.COLLATERAL_BELOW_STRIKE);
      throw new HsmAdapterError(
        "MARGINPROOF_COLLATERAL_BELOW_STRIKE",
        `collateral ${request.collateralValue} below strike ${request.strikeValue}`,
      );
    }
    ...
    if (this._verifiedProofs.has(proofKey)) {
      ...
      throw new HsmAdapterError(
        "MARGINPROOF_DUPLICATE",
        `proof for pool ${request.poolId} already verified`,
      );
    }
    const proofId =
      request.proofId || `margin-${crypto.randomBytes(4).toString("hex")}`;
    const now = Math.floor(Date.now() / 1000);
    const hwAccelUsed = request.hwAccelType || this._hwAccelType;
    const proof = {
      proofId,
      poolId: request.poolId,
      blindedCollateralCommitment:
        request.blindedCollateralCommitment || pool.blindedCollateralCommitment,
      blindedStrikeCommitment:
        request.blindedStrikeCommitment || pool.blindedStrikeCommitment,
  ...
  batchVerifyMarginAdequacy(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "MARGINPROOF_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "MARGINPROOF_BATCH_TOO_LARGE",
    ...
    if (this._audit) {
      this._audit("MARGINPROOF_BATCH_VERIFIED", {
        verifiedCount,
        failedCount,
        batchSize: requests.length,
      });
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "MARGINPROOF_GEN_FIELDS_MISSING",
        "poolId is required",
      );
    }
    if (!this._hub) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "MARGINPROOF_GEN_VALUES_MISSING",
    ...
    const zkRangeProofHash = crypto
      .createHash("sha256")
      .update(
        `snark:${proofSeed.toString("hex")}:${request.poolId}:${request.collateralValue}:${request.strikeValue}`,
      )
      .digest("hex");
    const proof = {
      poolId: request.poolId,
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "MARGINPROOF_QUORUM_INSUFFICIENT",
    ...
    if (this._audit) {
      this._audit("MARGINPROOF_SIGNATURES_AGGREGATED", {
        poolId,
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrSubCollateralProofs &&
      typeof request.peerId === "string"
    ) {
  ...
  _recordSlash(request, reason) {
    ...
    this._slashCount++;
    if (this._audit) {
      this._audit("MARGINPROOF_SLASHED", {
        poolId: request.poolId,
...
function _validateProofRequest(policy, request) {
  ...
  if (
    ...
    throw new HsmAdapterError(
      "MARGINPROOF_ATTESTATION_MISSING",
