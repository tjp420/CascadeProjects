...
class PqcFinancialDerivativesGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireClearingHouseInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.clearingHouseInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "DERIVGATE_CLEARING_HOUSE_INITIALIZER_UNATTESTED",
            "clearing house initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "DERIVGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "DERIVGATE_CONTRACT_EXPIRATION_EXCEEDED",
        `contract expiration seconds ${request.contractExpirationSeconds} exceeds maximum ${this.policy.maxContractExpirationSeconds}`,
      );
    }
    if (
      typeof request.riskMetricDepth === "number" &&
      request.riskMetricDepth > (this.policy.maxRiskMetricDepth || 32)
    ) {
      throw new HsmAdapterError(
        "DERIVGATE_RISK_DEPTH_EXCEEDED",
        `risk metric depth ${request.riskMetricDepth} exceeds maximum ${this.policy.maxRiskMetricDepth}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedCounterpartyRiskCommitment:
        request.blindedCounterpartyRiskCommitment,
  ...
  completeAccreditation(request) {
    ...
    if (!pool.derivativeClaimVerified) {
      throw new HsmAdapterError(
        "DERIVGATE_DERIVATIVE_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} derivative claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "DERIVGATE_RISK_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minClearingHouseQuorum || 3)) {
      throw new HsmAdapterError(
        "DERIVGATE_QUORUM_INSUFFICIENT",
        `clearing house signatures ${signatures.length} below minimum ${this.policy.minClearingHouseQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
    ...
    if (this._audit) {
      this._audit("COUNTERPARTY_RISK_ACCREDITATION_COMPLETED", {
        ...completion,
      });
    }
    return completion;
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "DERIVGATE_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedContractTermsCommitment ||
    !request.blindedCounterpartyRiskCommitment ||
    !request.blindedSettlementHashCommitment
  ) {
    throw new HsmAdapterError(
      "DERIVGATE_FIELDS_MISSING",
      "blindedContractTermsCommitment, blindedCounterpartyRiskCommitment, and blindedSettlementHashCommitment are required",
    );
  }
  if (typeof request.contractExpirationSeconds !== "number") {
    throw new HsmAdapterError(
      "DERIVGATE_FIELDS_MISSING",
      "contractExpirationSeconds is required",
    );
  }
  if (typeof request.riskMetricDepth !== "number") {
    throw new HsmAdapterError(
      "DERIVGATE_FIELDS_MISSING",
      "riskMetricDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "DERIVGATE_CLEARING_HOUSE_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "DERIVGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "DERIVGATE_RISK_COMMITTEE_ATTESTATION_MISSING",
