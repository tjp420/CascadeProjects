...
class PqcNuclearSafeguardsMonitoringGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireSafeguardsAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.safeguardsAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "NUCLEARGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "safeguards authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "NUCLEARGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "NUCLEARGATE_PQC_SCHEME_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "NUCLEARGATE_INSPECTION_WINDOW_EXCEEDED",
    ...
    if (
      ...
      request.telemetryChainDepth > (this.policy.maxTelemetryChainDepth || 12)
    ) {
      throw new HsmAdapterError(
        "NUCLEARGATE_TELEMETRY_DEPTH_EXCEEDED",
    ...
    const pool = {
      ...
      blindedReactorTelemetryCommitment:
        request.blindedReactorTelemetryCommitment,
      blindedInspectionReportCommitment:
        request.blindedInspectionReportCommitment,
      blindedFacilityIdentityCommitment:
        request.blindedFacilityIdentityCommitment,
  ...
  completeAccreditation(request) {
    ...
    if (!pool.safeguardsClaimVerified) {
      throw new HsmAdapterError(
        "NUCLEARGATE_SAFEGUARDS_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} safeguards claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "NUCLEARGATE_NUCLEAR_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minSafeguardsQuorum || 6)) {
      throw new HsmAdapterError(
        "NUCLEARGATE_QUORUM_INSUFFICIENT",
        `safeguards signatures ${signatures.length} below minimum ${this.policy.minSafeguardsQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  ...
  if (
    ...
    throw new HsmAdapterError(
      "NUCLEARGATE_FIELDS_MISSING",
      "blindedReactorTelemetryCommitment, blindedInspectionReportCommitment, and blindedFacilityIdentityCommitment are required",
    );
  }
  if (typeof request.inspectionWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "NUCLEARGATE_FIELDS_MISSING",
      "inspectionWindowSeconds is required",
    );
  }
  ...
  if (
    ...
    throw new HsmAdapterError(
      "NUCLEARGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "NUCLEARGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "NUCLEARGATE_NUCLEAR_ATTESTATION_MISSING",
