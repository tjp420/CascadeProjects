 ...
 *
 * Interlocking nuclear safeguards monitoring coordinator that
 * instantiates multi-party nuclear verification
 * pools using homomorphically split Pedersen commitments
 * over reactor telemetry hashes, inspection report digests, and
 * facility identity hashes. Parses NUCLEARGATE packets,
 * enforces maxTelemetryChainDepth, and tracks state
 * transitions alongside the minSafeguardsQuorum
 * boundary.
 *
...
class PqcNuclearSafeguardsMonitoringGatingHub {
  ...
  /**
   * Initialize an nuclear safeguards monitoring gating pool.
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('NUCLEARGATE_MAX_POOLS',
    ...
    if (this.policy.requireSafeguardsAuthorityInitializerAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('NUCLEARGATE_INSTITUTION_INITIALIZER_UNATTESTED', 'safeguards authority initializer attestation invalid');
        }
      } catch (err) {
  ...
  /**
   * Batch initialize multiple nuclear safeguards monitoring gating pools.
   * @param {object[]} requests
   * @returns {object}
   */
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('NUCLEARGATE_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('NUCLEARGATE_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        ...
        results.push({
          ...
          initialized: false,
          error: err.code || 'NUCLEARGATE_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    if (this._audit) {
      this._audit('NUCLEARGATE_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  rebalanceTelemetryChainDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('NUCLEARGATE_REBALANCE_FIELDS_MISSING', 'poolId is required');
    }
    ...
    if (pool.status !== POOL_STATUS.OPEN && pool.status !== POOL_STATUS.REBALANCING) {
      throw new HsmAdapterError('NUCLEARGATE_NOT_REBALANCEABLE',
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError('NUCLEARGATE_REBALANCE_DIRECTION_INVALID',
    ...
    if (typeof request.rebalanceAmount !== 'number' || request.rebalanceAmount <= 0) {
      throw new HsmAdapterError('NUCLEARGATE_REBALANCE_AMOUNT_INVALID',
    ...
    if (request.newTelemetryChainDepth !== undefined) {
      if (request.newTelemetryChainDepth > (this.policy.maxTelemetryChainDepth || 12)) {
        throw new HsmAdapterError('NUCLEARGATE_TELEMETRY_DEPTH_EXCEEDED',
    ...
    if (this._audit) {
      this._audit('NUCLEARGATE_TELEMETRY_DEPTH_REBALANCED', { ...rebalance });
    }
    return rebalance;
  }
  ...
  completeAccreditation(request) {
    ...
    if (signatures.length < (this.policy.minSafeguardsQuorum || 6)) {
      throw new HsmAdapterError('NUCLEARGATE_ACCREDITATION_QUORUM_INSUFFICIENT', `accreditation signatures ${signatures.length} below minimum ${this.policy.minSafeguardsQuorum}`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('NUCLEARGATE_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('NUCLEARGATE_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.ACCREDITED) {
      throw new HsmAdapterError('NUCLEARGATE_NOT_ACCREDITED',
    ...
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('NUCLEARGATE_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('NUCLEARGATE_SETTLE_CHAIN_MISMATCH',
    ...
    this._settleCount++;
    if (this._audit) {
      this._audit('NUCLEARGATE_SETTLED', { ...settlement });
    }
    return settlement;
  }
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('NUCLEARGATE_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minSafeguardsQuorum || 6)) {
      throw new HsmAdapterError('NUCLEARGATE_ACCREDITATION_QUORUM_INSUFFICIENT',
        `${partialSignatures.length} signatures below minimum ${this.policy.minSafeguardsQuorum || 6}`);
    }
    ...
    if (this._audit) {
      this._audit('NUCLEARGATE_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.ACCREDITED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('NUCLEARGATE_ALREADY_ACCREDITED',
        `pool ${poolId} has been accredited/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('NUCLEARGATE_ALREADY_CANCELLED',
    ...
    this._cancelCount++;
    if (this._audit) {
      this._audit('NUCLEARGATE_CANCELLED', { poolId });
    }
  ...
  getPools() {
    return Array.from(this._pools.values()).map(p => ({
      ...
      inspectionWindowSeconds: p.inspectionWindowSeconds,
      safeguardsClaimVerified: p.safeguardsClaimVerified,
    }));
  }
...
function _validateInitRequest(policy, request) {
  ...
  if (policy.requireSafeguardsAuthorityInitializerAttestation && !request.safeguardsAuthorityInitializerAttestation) {
    throw new HsmAdapterError('NUCLEARGATE_INSTITUTION_INITIALIZER_ATTESTATION_MISSING', 'safeguards authority initializer attestation is required');
  }
}
...
function _validateCompleteRequest(policy, request) {
  ...
  if (policy.requireNuclearOversightCommitteeAttestation && !request.nuclearOversightCommitteeAttestation) {
    throw new HsmAdapterError('NUCLEARGATE_CLEARING_ATTESTATION_MISSING', 'nuclear oversight committee attestation is required');
  }
}
