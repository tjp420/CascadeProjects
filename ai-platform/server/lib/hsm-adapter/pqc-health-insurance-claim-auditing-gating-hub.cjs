 ...
 *
 * Interlocking health insurance claim auditing coordinator that
 * instantiates multi-party actuarial verification
 * pools using homomorphically split Pedersen commitments
 * over encrypted diagnostic billing sequences, actuarial risk codes, and
 * policy payout commitment hashes. Parses INSURANCEGATE packets,
 * enforces maxBillingSequenceDepth, and tracks state
 * transitions alongside the minClaimsAuditQuorum
 * boundary.
 *
 * Extended with batch pool initialization, billing sequence
...
class PqcHealthInsuranceClaimAuditingGatingHub {
  ...
  /**
   * Initialize an health insurance claim auditing gating pool.
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError(
        "INSURANCEGATE_MAX_POOLS",
        `maximum ${this._maxPools} pools reached`,
      );
    }
    if (
      this.policy.requireInsuranceAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.insuranceAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "INSURANCEGATE_INSTITUTION_INITIALIZER_UNATTESTED",
            "insurance authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "INSURANCEGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "INSURANCEGATE_PQC_SCHEME_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "INSURANCEGATE_CLAIM_WINDOW_EXCEEDED",
        `claim window seconds ${request.claimWindowSeconds} exceeds maximum ${this.policy.maxClaimWindowSeconds}`,
      );
    }
    if (
      typeof request.billingSequenceDepth === "number" &&
      request.billingSequenceDepth > (this.policy.maxBillingSequenceDepth || 24)
    ) {
      throw new HsmAdapterError(
        "INSURANCEGATE_BILLING_DEPTH_EXCEEDED",
        `billing sequence depth ${request.billingSequenceDepth} exceeds maximum ${this.policy.maxBillingSequenceDepth}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedDiagnosticBillingCommitment:
        request.blindedDiagnosticBillingCommitment,
      blindedActuarialRiskCodeCommitment:
        request.blindedActuarialRiskCodeCommitment,
  ...
  /**
   * Batch initialize multiple health insurance claim auditing gating pools.
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "INSURANCEGATE_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "INSURANCEGATE_BATCH_TOO_LARGE",
    ...
    for (const req of requests) {
      try {
        ...
        results.push({
          ...
          initialized: false,
          error: err.code || "INSURANCEGATE_BATCH_ERROR",
        });
        failedCount++;
      }
    }
    if (this._audit) {
      this._audit("INSURANCEGATE_BATCH_INITIALIZED", {
        successCount,
        failedCount,
  ...
  /**
   * Rebalance billing sequence depth for a pool.
   * @param {object} request
   * @returns {object}
   */
  rebalanceBillingSequenceDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "INSURANCEGATE_REBALANCE_FIELDS_MISSING",
        "poolId is required",
      );
    }
    ...
    if (
      ...
      throw new HsmAdapterError(
        "INSURANCEGATE_NOT_REBALANCEABLE",
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError(
        "INSURANCEGATE_REBALANCE_DIRECTION_INVALID",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "INSURANCEGATE_REBALANCE_AMOUNT_INVALID",
    ...
    const rebalance = {
      ...
      newBillingSequenceDepth:
        request.newBillingSequenceDepth !== undefined
          ? request.newBillingSequenceDepth
          : pool.billingSequenceDepth,
      rebalancedAt: Math.floor(Date.now() / 1000),
    };
    this._rebalances.set(rebalanceId, rebalance);
    this._rebalanceCount++;
    if (request.newBillingSequenceDepth !== undefined) {
      if (
        request.newBillingSequenceDepth >
        (this.policy.maxBillingSequenceDepth || 24)
      ) {
        throw new HsmAdapterError(
          "INSURANCEGATE_BILLING_DEPTH_EXCEEDED",
          `new billing sequence depth ${request.newBillingSequenceDepth} exceeds maximum ${this.policy.maxBillingSequenceDepth}`,
        );
      }
      pool.billingSequenceDepth = request.newBillingSequenceDepth;
    }
    if (this._audit) {
      this._audit("INSURANCEGATE_BILLING_DEPTH_REBALANCED", { ...rebalance });
    }
    return rebalance;
  }
  ...
  completeAccreditation(request) {
    ...
    if (!pool.claimAuditVerified) {
      throw new HsmAdapterError(
        "INSURANCEGATE_CLAIM_AUDIT_NOT_VERIFIED",
        `pool ${request.poolId} claim audit not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "INSURANCEGATE_ACTUARIAL_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minClaimsAuditQuorum || 3)) {
      throw new HsmAdapterError(
        "INSURANCEGATE_ACCREDITATION_QUORUM_INSUFFICIENT",
        `accreditation signatures ${signatures.length} below minimum ${this.policy.minClaimsAuditQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "INSURANCEGATE_SETTLE_FIELDS_MISSING",
        "poolId is required",
      );
    }
    const pool = this._pools.get(request.poolId);
    ...
    this._settleCount++;
    if (this._audit) {
      this._audit("INSURANCEGATE_SETTLED", { ...settlement });
    }
    return settlement;
  }
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError(
        "INSURANCEGATE_NO_SIGNATURES",
        "partialSignatures array is required",
      );
    }
    if (partialSignatures.length < (this.policy.minClaimsAuditQuorum || 3)) {
      throw new HsmAdapterError(
        "INSURANCEGATE_ACCREDITATION_QUORUM_INSUFFICIENT",
        `${partialSignatures.length} signatures below minimum ${this.policy.minClaimsAuditQuorum || 3}`,
      );
    }
    ...
    if (this._audit) {
      this._audit("INSURANCEGATE_SIGNATURES_AGGREGATED", {
        poolId,
  ...
  cancelPool(poolId) {
    ...
    this._cancelCount++;
    if (this._audit) {
      this._audit("INSURANCEGATE_CANCELLED", { poolId });
    }
  ...
  getPools() {
    return Array.from(this._pools.values()).map((p) => ({
      ...
      status: p.status,
      billingSequenceDepth: p.billingSequenceDepth,
      claimWindowSeconds: p.claimWindowSeconds,
      claimAuditVerified: p.claimAuditVerified,
    }));
  }
...
function _validateInitRequest(policy, request) {
  ...
  if (
    !request.blindedDiagnosticBillingCommitment ||
    !request.blindedActuarialRiskCodeCommitment ||
    !request.blindedPayoutCommitment
  ) {
    throw new HsmAdapterError(
      "INSURANCEGATE_FIELDS_MISSING",
      "blindedDiagnosticBillingCommitment, blindedActuarialRiskCodeCommitment, and blindedPayoutCommitment are required",
    );
  }
  if (typeof request.claimWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "INSURANCEGATE_FIELDS_MISSING",
      "claimWindowSeconds is required",
    );
  }
  if (typeof request.billingSequenceDepth !== "number") {
    throw new HsmAdapterError(
      "INSURANCEGATE_FIELDS_MISSING",
      "billingSequenceDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "INSURANCEGATE_INSTITUTION_INITIALIZER_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "INSURANCEGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "INSURANCEGATE_CLEARING_ATTESTATION_MISSING",
