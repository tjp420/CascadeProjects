...
class PqcHealthInsuranceClaimAuditingGatingHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('INSURANCEGATE_MAX_POOLS',
    ...
    if (this.policy.requireInsuranceAuthorityInitializerAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('INSURANCEGATE_INSTITUTION_INITIALIZER_UNATTESTED', 'insurance authority initializer attestation invalid');
        }
      } catch (err) {
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('INSURANCEGATE_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('INSURANCEGATE_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('INSURANCEGATE_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  rebalanceBillingSequenceDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('INSURANCEGATE_REBALANCE_FIELDS_MISSING', 'poolId is required');
    }
    ...
    if (pool.status !== POOL_STATUS.OPEN && pool.status !== POOL_STATUS.REBALANCING) {
      throw new HsmAdapterError('INSURANCEGATE_NOT_REBALANCEABLE',
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError('INSURANCEGATE_REBALANCE_DIRECTION_INVALID',
    ...
    if (typeof request.rebalanceAmount !== 'number' || request.rebalanceAmount <= 0) {
      throw new HsmAdapterError('INSURANCEGATE_REBALANCE_AMOUNT_INVALID',
    ...
    const rebalance = {
      ...
      newBillingSequenceDepth: request.newBillingSequenceDepth !== undefined ? request.newBillingSequenceDepth : pool.billingSequenceDepth,
    ...
    if (request.newBillingSequenceDepth !== undefined) {
      if (request.newBillingSequenceDepth > (this.policy.maxBillingSequenceDepth || 24)) {
        throw new HsmAdapterError('INSURANCEGATE_BILLING_DEPTH_EXCEEDED',
  ...
  completeAccreditation(request) {
    ...
    if (signatures.length < (this.policy.minClaimsAuditQuorum || 3)) {
      throw new HsmAdapterError('INSURANCEGATE_ACCREDITATION_QUORUM_INSUFFICIENT', `accreditation signatures ${signatures.length} below minimum ${this.policy.minClaimsAuditQuorum}`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('INSURANCEGATE_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('INSURANCEGATE_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.ACCREDITED) {
      throw new HsmAdapterError('INSURANCEGATE_NOT_ACCREDITED',
    ...
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('INSURANCEGATE_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('INSURANCEGATE_SETTLE_CHAIN_MISMATCH',
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('INSURANCEGATE_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minClaimsAuditQuorum || 3)) {
      throw new HsmAdapterError('INSURANCEGATE_ACCREDITATION_QUORUM_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('INSURANCEGATE_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.ACCREDITED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('INSURANCEGATE_ALREADY_ACCREDITED',
        `pool ${poolId} has been accredited/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('INSURANCEGATE_ALREADY_CANCELLED',
...
function _validateInitRequest(policy, request) {
  ...
  if (policy.requireInsuranceAuthorityInitializerAttestation && !request.insuranceAuthorityInitializerAttestation) {
    throw new HsmAdapterError('INSURANCEGATE_INSTITUTION_INITIALIZER_ATTESTATION_MISSING', 'insurance authority initializer attestation is required');
  }
}
...
function _validateCompleteRequest(policy, request) {
  ...
  if (policy.requireActuarialCommitteeAttestation && !request.actuarialCommitteeAttestation) {
    throw new HsmAdapterError('INSURANCEGATE_CLEARING_ATTESTATION_MISSING', 'actuarial committee attestation is required');
  }
}
