...
class PqcRealEstateTokenizationHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('REPOOL_MAX_POOLS',
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('REPOOL_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('REPOOL_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('REPOOL_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  rebalanceValuation(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('REPOOL_REBALANCE_FIELDS_MISSING', 'poolId is required');
    }
    ...
    if (pool.status !== POOL_STATUS.OPEN && pool.status !== POOL_STATUS.REBALANCING) {
      throw new HsmAdapterError('REPOOL_NOT_REBALANCEABLE',
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError('REPOOL_REBALANCE_DIRECTION_INVALID',
    ...
    if (typeof request.rebalanceAmount !== 'number' || request.rebalanceAmount <= 0) {
      throw new HsmAdapterError('REPOOL_REBALANCE_AMOUNT_INVALID',
    ...
    const rebalance = {
      ...
      newAssetValuationCap: request.newAssetValuationCap || pool.assetValuationCap,
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('REPOOL_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('REPOOL_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.FINALIZED) {
      throw new HsmAdapterError('REPOOL_NOT_FINALIZED',
        `pool ${request.poolId} status is ${pool.status}, expected finalized`);
    }
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('REPOOL_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('REPOOL_SETTLE_CHAIN_MISMATCH',
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('REPOOL_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minCoSignerQuorum || 3)) {
      throw new HsmAdapterError('REPOOL_TRANSFER_QUORUM_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('REPOOL_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.FINALIZED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('REPOOL_ALREADY_FINALIZED',
        `pool ${poolId} has been finalized/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('REPOOL_ALREADY_CANCELLED',
