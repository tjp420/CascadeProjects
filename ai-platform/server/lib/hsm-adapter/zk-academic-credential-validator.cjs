...
class ZkAcademicCredentialValidator {
  ...
  verifyAcademicClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "EDUCLAIM_HUB_MISSING",
        "education credential gating hub is required",
      );
    }
    if (
    ...
    if (
      ...
      throw new HsmAdapterError(
        "EDUCLAIM_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkAcademicRangeProofHash ||
      typeof request.zkAcademicRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError(
        "EDUCLAIM_ZK_PROOF_MISSING",
        "zero-knowledge academic range proof hash is required",
      );
    }
    if (
    ...
    if (
      ...
      this._recordSlash(
        request.poolId,
        request.peerId,
        SLASH_REASON.TRANSCRIPT_EXPIRATION_OUT_OF_BOUNDS,
      );
      throw new HsmAdapterError(
        "EDUCLAIM_TRANSCRIPT_EXPIRATION_OUT_OF_BOUNDS",
    ...
    if (this._verifiedClaims.has(claimKey)) {
      ...
      throw new HsmAdapterError(
        "EDUCLAIM_DUPLICATE",
        `academic claim for pool ${request.poolId} already verified`,
      );
    }
    const claimId =
    ...
    const claim = {
      ...
      blindedAccreditationMetricCommitment:
        request.blindedAccreditationMetricCommitment || "unspecified",
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "EDUCLAIM_HW_PROOF_FIELDS_MISSING",
        "poolId is required",
      );
    }
    if (
      typeof request.accreditationMetric !== "number" ||
    ...
    const proofHash = crypto
      .createHash("sha256")
      .update(
        `${request.poolId}:${request.accreditationMetric}:${request.claimValue}:${this._hwAccelType}`,
      )
      .digest("hex");
    const proof = {
  ...
  batchVerifyAcademicClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "EDUCLAIM_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "EDUCLAIM_BATCH_TOO_LARGE",
    ...
    if (this._audit) {
      this._audit("EDUCLAIM_BATCH_VERIFIED", {
        verifiedCount,
        failedCount,
        batchSize: requests.length,
      });
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError(
        "EDUCLAIM_WINDOW_FIELDS_MISSING",
        "poolId is required",
      );
    }
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    ...
    if (this._audit) {
      this._audit("EDUCLAIM_PARTIAL_SIGS_AGGREGATED", {
        poolId,
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrOutOfOrderCredentialClaims &&
      typeof request.peerId === "string"
    ) {
