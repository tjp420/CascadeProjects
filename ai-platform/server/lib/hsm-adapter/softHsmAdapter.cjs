...
class SoftHsmAdapter {
  ...
  initialize() {
    ...
    if (!this.pkcs11)
      throw new HsmAdapterError(
        "PKCS11_MISSING",
        "pkcs11js native module not available",
      );
    ...
    try {
      ...
      for (const s of slots) {
        try {
          ...
          if (
            info &&
            info.label &&
            info.label.toString().trim() === this.slotLabel
          ) {
            target = s;
            break;
          }
      ...
      if (target === null)
        throw new HsmAdapterError(
          "TOKEN_NOT_FOUND",
          `Token label ${this.slotLabel} not found`,
        );
      this.slot = target;
  ...
  createKEK({ label, length = 32 } = {}) {
    ...
    if (!handle)
      throw new HsmAdapterError(
        "KEK_GEN_FAILED",
        "Failed to generate KEK on token",
      );
    return kekLabel;
  }
  ...
  listKEKs() {
    ...
    for (const h of found) {
      const attrs = this.pkcs11.C_GetAttributeValue(this.session, h, [
        { type: pkcs11js.CKA_LABEL },
      ]);
      const label =
        attrs[0] && attrs[0].value ? attrs[0].value.toString() : null;
  ...
  wrap(kekLabel, cekBuffer) {
    ...
    if (!kekHandle)
      throw new HsmAdapterError("KEK_NOT_FOUND", `KEK ${kekLabel} not found`);
    ...
    try {
      ...
      try {
        this.pkcs11.C_DestroyObject(this.session, cekHandle);
      } catch (e) {}
  ...
  unwrap(kekLabel, wrappedCekBuffer) {
    ...
    try {
      const newKeyHandle = this.pkcs11.C_UnwrapKey(
        this.session,
        mechanism,
        kekHandle,
        wrappedCekBuffer,
      ...
      if (!newKeyHandle)
        throw new HsmAdapterError(
          "UNWRAP_FAILED",
          "C_UnwrapKey returned no handle",
        );
  ...
  decryptPayload(ciphertext, iv, tag, aad, ceKeyHandle) {
    ...
    if (!ceKeyHandle)
      throw new HsmAdapterError(
        "INVALID_KEY_HANDLE",
        "CE key handle required for in-token decryption",
      );
    try {
      ...
      if (!usedVariant) {
        throw (
          lastErr ||
          new Error("CKM_AES_GCM init failed for all known parameter encodings")
        );
      }
      ...
      try {
        ...
        console.debug(
          "HSM decrypt succeeded using CKM_AES_GCM variant:",
          usedVariant,
        );
        return plain;
      } catch (err) {
        ...
        console.debug(
          "HSM decrypt succeeded (outBuf) using CKM_AES_GCM variant:",
          usedVariant,
        );
        return plain;
      }
  ...
  finalize() {
    this.clearSensitiveData();
    try {
      if (this.pkcs11) this.pkcs11.C_Finalize();
    } catch (e) {}
  }
}
