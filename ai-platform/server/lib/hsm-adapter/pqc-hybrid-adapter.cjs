...
class PqcHybridAdapter {
  ...
  constructor(tenantId, options = {}) {
    ...
    if (this._recipient && this._recipient.pqc) {
      this._pqcEngine = new PqcEncapsulationEngine(this._recipient.pqc.kemLevel);
    }
  }

  _audit(event, extra = {}) {
    if (!this._logger || !this._logger.info) return;
    this._logger.info(event, { sub: 'hsm-adapter', provider: 'pqc-hybrid', tenantId: this._tenantId, ...extra });
  }

  _validatePqc(kemLevel, hybridMode = true) {
    if (this._policyEngine) {
      this._policyEngine.validate(this._tenantId, 'pqc', { kemLevel, hybridMode });
    }
  }
  ...
  generateRecipientKeypair() {
    ...
    return {
      classic: { privateKey: ecdh.getPrivateKey(), publicKey: ecdh.getPublicKey() },
  ...
  encapsulate(recipient) {
    ...
    const pqc = new PqcEncapsulationEngine(recipient.kemLevel || this._kemLevel);
    const { ciphertext, sharedSecret: quantumSecret } = pqc.encapsulate(recipient.pqc.publicKey);

    this._kemLevel = recipient.kemLevel || this._kemLevel;
    const rootKey = this._combineRootKey(classicSecret, quantumSecret);

    const publicKeyHash = crypto.createHash('sha256').update(recipient.pqc.publicKey.seed).digest();
    const combinedKeyHash = crypto.createHash('sha256').update(rootKey).digest();
  ...
  decapsulate(payload) {
    if (!this._recipient || !this._recipient.classic || !this._recipient.pqc) {
      throw new HsmAdapterError('PQC_KEY_INTEGRITY', 'recipient private keys are required to decapsulate');
    }
    ...
    const quantumSecret = pqc.decapsulate(ciphertext, this._recipient.pqc.secretKey);
    ...
    if (!combinedKeyHash.equals(expectedHash)) {
      ...
      throw new HsmAdapterError('HYBRID_TRANSITION_FAILED', 'combined key hash mismatch');
    }
