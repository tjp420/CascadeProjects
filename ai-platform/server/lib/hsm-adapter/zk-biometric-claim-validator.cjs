...
class ZkBiometricClaimValidator {
  ...
  verifyBiometricClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_HUB_MISSING",
        "biometric verification gating hub is required",
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "BIOMETRICCLAIM_COMMITTEE_UNATTESTED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_PEER_BANNED",
        `peer ${request.peerId} is banned`,
      );
    }
    if (
      !request.zkBiometricRangeProofHash ||
      typeof request.zkBiometricRangeProofHash !== "string"
    ) {
      this._banPeerIfPolicy(request);
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_ZK_PROOF_MISSING",
        "zero-knowledge biometric range proof hash is required",
      );
    }
    if (
      ...
      this._banPeerIfPolicy(request);
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_PARTIAL_SIG_MISSING",
    ...
    if (
      ...
      this._banPeerIfPolicy(request);
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_TEMPLATE_EXPIRATION_OUT_OF_BOUNDS",
    ...
    if (this._verifiedClaims.has(claimKey)) {
      this._banPeerIfPolicy(request);
      throw new HsmAdapterError(
        "BIOMETRICCLAIM_DUPLICATE",
        `biometric claim for pool ${request.poolId} already verified`,
      );
    }
    const claimId =
    ...
    const claim = {
      ...
      blindedLivenessMetricCommitment:
        request.blindedLivenessMetricCommitment || "unspecified",
  ...
  _banPeerIfPolicy(request) {
    if (
      this.policy.banMalformedOrOutOfOrderBiometricClaims &&
      typeof request.peerId === "string"
    ) {
...
function _validateClaimRequest(policy, request) {
  ...
  if (
    ...
    throw new HsmAdapterError(
      "BIOMETRICCLAIM_ATTESTATION_MISSING",
