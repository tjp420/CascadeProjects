...
class UploadSession {
  constructor(opts = {}) {
    ...
    this.requireSignature = opts.requireSignature !== undefined ? opts.requireSignature : true;
  }
  ...
  async appendChunk(id, offset, buf) {
    ...
    // boundary checks
    if (offset % s.reassembler.leafSize !== 0) throw new Error('offset_not_aligned');
    if (s.receivedBytes + buf.length > s.maxBytes) throw new Error('exceeds_max_bytes');
    // update reassembler
  ...
  async commit(id, { signature } = {}) {
    const s = this.sessions.get(id);
    if (!s) throw new Error('session_not_found');
    if (this.requireSignature && !signature) throw new Error('missing_signature');
    ...
    if (this.requireSignature) {
      ...
      try { /* validate signature */ } catch (e) { throw new Error('invalid_signature'); }
    }
    this.sessions.delete(id);
    return { id, root };
  }
