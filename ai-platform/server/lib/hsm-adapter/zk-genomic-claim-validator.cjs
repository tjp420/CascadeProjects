  ...
  verifyGenomicClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "GENOCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (pool.status !== "open") {
      throw new HsmAdapterError(
        "GENOCLAIM_POOL_NOT_OPEN",
    ...
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "GENOCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (
      this.policy.banMalformedOrOutOfOrderGenomicClaims &&
      request.peerId &&
      this._bannedPeers.has(request.peerId)
    ) {
      throw new HsmAdapterError(
        "GENOCLAIM_PEER_BANNED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "GENOCLAIM_DUPLICATE",
        `duplicate genomic claim for pool ${request.poolId}`,
      );
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.zkGenomicRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderGenomicClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "GENOCLAIM_ZK_PROOF_MISSING",
      "zkGenomicRangeProofHash is required",
    );
  }
  if (!request.symmetricPredicateProofDigest) {
    if (policy.banMalformedOrOutOfOrderGenomicClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "GENOCLAIM_PREDICATE_PROOF_MISSING",
      "symmetricPredicateProofDigest is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "GENOCLAIM_OVERSIGHT_ATTESTATION_MISSING",
