...
process.env.KEY_ROTATION_STORE_PATH = path.join(
  os.tmpdir(),
  "sb-snapshot-test-key.json",
);
process.env.AUDIT_LOG_PATH = path.join(
  os.tmpdir(),
  "sb-snapshot-test-audit.json",
);
...
describe("State Snapshot Checkpoint Utility", () => {
  ...
  describe("L2-03: Snapshot triggered on EPOCH_DRIFT", () => {
    test("STATE_SNAPSHOT event recorded with reason epoch_drift", () => {
      ...
      const snapshotEvents = clusterSync.queryEvents({
  ...
  describe("L3-04: Restore with wrong snapshotId format", () => {
    test("throws validation error", () => {
      ...
      expect(() => clusterSync.restoreStateSnapshot(badSnapshot)).toThrow(
        "STATE_SNAPSHOT_INVALID",
      );
    });
  });
  ...
  describe("S-04: Snapshot restore validates schema before applying", () => {
    test("null snapshot throws validation error", () => {
      expect(() => clusterSync.restoreStateSnapshot(null)).toThrow(
        "STATE_SNAPSHOT_INVALID",
      );
    });

    test("non-object snapshot throws validation error", () => {
      expect(() => clusterSync.restoreStateSnapshot("not-an-object")).toThrow(
        "STATE_SNAPSHOT_INVALID",
      );
    });
