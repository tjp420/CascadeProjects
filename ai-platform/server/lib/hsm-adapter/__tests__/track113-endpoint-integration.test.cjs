...
process.env.RATCHET_SESSIONS_DIR = path.join(
  __dirname,
  "..",
  "..",
  "..",
  ".data",
  "ratchet-sessions-track113-test",
);
...
describe("Track 113 handshake endpoint integration", () => {
  ...
  test("POST /api/vault/handshake/init creates encrypted session", async () => {
    const res = await request(app)
      ...
      .send({
        clientId: "client-a",
        handshakeDigest: "track113-digest-001",
        lifecycleTimeout: 60000,
      })
      .expect(201);
  ...
  test("POST /api/vault/handshake/verify authenticates valid proof", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        sessionId,
        clientProof: expected,
        expectedStateDigest: "any-digest",
      })
      .expect(200);
    expect(res.body.success).toBe(true);
