 ...
 */
const { PqcCrossBorderLogisticsGatingHub } = require('../pqc-cross-border-logistics-gating-hub.cjs');
const { ZkManifestClaimValidator } = require('../zk-manifest-claim-validator.cjs');
...
describe('Track 81 PQ cross-border logistics gating', () => {
  test('PqcCrossBorderLogisticsGatingHub initializes a pool and emits LOGISTICS_GATING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'LOGISTICS_GATING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkManifestClaimValidator verifies a manifest claim and emits ZK_MANIFEST_CLAIM_VERIFIED', () => {
    ...
    expect(events.some((e) => e.event === 'ZK_MANIFEST_CLAIM_VERIFIED')).toBe(true);
  });
  ...
  test('PqcCrossBorderLogisticsGatingHub completes accreditation after claim and emits CARRIER_ACCREDITATION_COMPLETED', () => {
    ...
    expect(events.some((e) => e.event === 'CARRIER_ACCREDITATION_COMPLETED')).toBe(true);
  });
  ...
  test('PqcCrossBorderLogisticsGatingHub rejects un-attested customs authority initializer', () => {
    const attestationClient = new MockAttestationClient();
    const hub = new PqcCrossBorderLogisticsGatingHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkManifestClaimValidator rejects un-attested trade corridor committee', () => {
    ...
    const attestationClient = new MockAttestationClient();
    const validator = new ZkManifestClaimValidator({ policy: POLICY, hub, attestationClient });
  ...
  test('CryptoPolicyEngine validates pq logistics gating configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqLogisticsGating', {
      customsQuorum: 3,
      transitWindowSeconds: 3888000,
      manifestDepth: 16,
      pqcSignatureScheme: 'ML-DSA-65',
    ...
    expect(() => engine.validate('t1', 'pqLogisticsGating', { customsQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { transitWindowSeconds: 9999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { manifestDepth: 64 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { customsAuthorityInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { tradeCorridorCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { banMalformedOrOutOfOrderManifestClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLogisticsGating', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
