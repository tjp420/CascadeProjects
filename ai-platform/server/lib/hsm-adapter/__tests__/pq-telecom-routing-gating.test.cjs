...
} = require("../pqc-telecom-routing-gating-hub.cjs");
const {
  ZkBandwidthClaimValidator,
} = require("../zk-bandwidth-claim-validator.cjs");
const {
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyBandwidthClaim(
...
describe("Track 85 PQ telecom routing gating", () => {
  test("PqcTelecomRoutingGatingHub initializes a pool and emits TELECOM_ROUTING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "TELECOM_ROUTING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkBandwidthClaimValidator verifies a bandwidth claim and emits ZK_BANDWIDTH_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_BANDWIDTH_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcTelecomRoutingGatingHub completes accreditation after claim and emits ROUTING_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "ROUTING_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkBandwidthClaimValidator rejects un-attested routing committee", () => {
    ...
    expect(() => validator.verifyBandwidthClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq telecom gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTelecomGating", { telecomPeeringQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
        allocationWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", { networkRoutingDepth: 64 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
        carrierEndpointInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
        routingCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
        banMalformedOrOutOfOrderTelecomClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTelecomGating", {
