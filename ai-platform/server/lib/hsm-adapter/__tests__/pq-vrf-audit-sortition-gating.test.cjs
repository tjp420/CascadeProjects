 ...
 */
const { PqcVrfAuditSortitionGatingHub } = require('../pqc-vrf-audit-sortition-gating-hub.cjs');
const { ZkSortitionClaimValidator } = require('../zk-sortition-claim-validator.cjs');
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifySortitionClaim(baseClaimRequest(ctx.pool.poolId));
  return { ...ctx, claim };
}
...
describe('Track 80 PQ VRF audit sortition gating', () => {
  test('PqcVrfAuditSortitionGatingHub initializes a pool and emits SORTITION_GATING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'SORTITION_GATING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkSortitionClaimValidator verifies a sortition claim and emits ZK_SORTITION_CLAIM_VERIFIED', () => {
    ...
    expect(events.some((e) => e.event === 'ZK_SORTITION_CLAIM_VERIFIED')).toBe(true);
  });
  ...
  test('PqcVrfAuditSortitionGatingHub completes accreditation after claim and emits VALIDATOR_ACCREDITATION_COMPLETED', () => {
    ...
    expect(events.some((e) => e.event === 'VALIDATOR_ACCREDITATION_COMPLETED')).toBe(true);
  });
  ...
  test('PqcVrfAuditSortitionGatingHub rejects un-attested sortition authority initializer', () => {
    ...
    const hub = new PqcVrfAuditSortitionGatingHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkSortitionClaimValidator rejects un-attested audit committee', () => {
    ...
    const validator = new ZkSortitionClaimValidator({ policy: POLICY, hub, attestationClient });
    const clReq = baseClaimRequest(pool.poolId);
    clReq.auditCommitteeAttestation = { authority: 'bad' };
    expect(() => validator.verifySortitionClaim(clReq)).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq sortition gating configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqSortitionGating', {
      sortitionQuorum: 3,
      sortitionEpochSeconds: 1296000,
      entropyDepth: 8,
      pqcSignatureScheme: 'ML-DSA-65',
    ...
    expect(() => engine.validate('t1', 'pqSortitionGating', { sortitionQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { sortitionEpochSeconds: 9999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { entropyDepth: 32 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { sortitionAuthorityInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { auditCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { banMalformedOrOutOfOrderSortitionClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSortitionGating', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
