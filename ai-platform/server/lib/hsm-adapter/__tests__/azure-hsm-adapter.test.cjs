...
function setupMocks() {
  ...
  const adapterModule =
    require.cache[require.resolve("../azureKeyVaultHsmAdapter.cjs")];
  if (adapterModule) {
...
const originalLoadAzureSDKs =
  azureAdapter.AzureKeyVaultHsmAdapter.prototype._initialize;
...
describe("AzureKeyVaultHsmAdapter", () => {
  ...
  describe("constructor", () => {
    test("rejects missing vaultUrl", () => {
      ...
      expect(() => new AzureKeyVaultHsmAdapter({})).toThrow(
  ...
  describe("wrap", () => {
    ...
    test("rejects unknown KEK", async () => {
      ...
      adapter._wrap = async (tenantId, kekId, plaintext) => {
        throw new HsmAdapterError(
          "KEK_NOT_FOUND",
          "KEK nonexistent not found in vault",
        );
      };
      await expect(
        adapter.wrap("tenant-1", "nonexistent", Buffer.alloc(32)),
    ...
    test("rejects non-Buffer plaintext", async () => {
      ...
      await adapter.initialize();
      await expect(
        adapter.wrap("tenant-1", "kek1", "not-a-buffer"),
      ).rejects.toMatchObject({
        code: "INVALID_INPUT",
      });
    });
  ...
  describe("unwrap", () => {
    ...
    test("rejects corrupted wrapped key", async () => {
      ...
      await expect(
        adapter.unwrap("tenant-1", kekId, corrupted),
      ).rejects.toMatchObject({
        code: "UNWRAP_FAILED",
      });
    });
  ...
  describe("audit interceptor", () => {
    test("logs success and failure for SDK calls", async () => {
      ...
      const wrapEvent = auditLog.find(
        (e) => e.event === "WRAP" && e.status === "success",
      );
  ...
  describe("tenant isolation", () => {
    test("wrap with wrong tenantId throws UNAUTHORIZED_KEY_ACCESS", async () => {
      ...
      await expect(
        adapter.wrap("", "kek1", Buffer.alloc(32)),
      ).rejects.toMatchObject({
  ...
  describe("error mapping (_mapAzureError)", () => {
    ...
    test("maps 429 to RATE_LIMITED", async () => {
      ...
      await adapter.initialize();
      await expect(
        adapter.wrap("t1", "k1", Buffer.alloc(32)),
      ).rejects.toMatchObject({
        code: "RATE_LIMITED",
      });
    });
    ...
    test("maps PARSE_ERROR to CONNECTION_FAILURE", async () => {
      ...
      await adapter.initialize();
      await expect(
        adapter.unwrap("t1", "k1", Buffer.alloc(40)),
      ).rejects.toMatchObject({
        code: "CONNECTION_FAILURE",
      });
    });
