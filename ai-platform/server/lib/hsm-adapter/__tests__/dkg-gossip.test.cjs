...
process.env.KEY_ROTATION_STORE_PATH = require('path').join(require('os').tmpdir(), 'sb-dkg-gossip-key-state.json');
process.env.AUDIT_LOG_PATH = require('path').join(require('os').tmpdir(), 'sb-dkg-gossip-audit.json');
...
function createDkgEngine(nodeIds = ['node-1', 'node-2', 'node-3'], threshold = 2) {
...
describe('DKG Transcript Gossip Transport', () => {
  ...
  describe('serialization helpers', () => {
    ...
    test('_validateDkgHex accepts valid lowercase hex', () => {
      ...
      expect(clusterSync._validateDkgHex('abcdef0123456789', 'test')).toBe(true);
    });
  ...
  describe('L2-01: Full DKG round (3 nodes, t=2)', () => {
    test('initDkgSession creates session and broadcasts DKG_COMMIT', () => {
      ...
      const status = clusterSync.initDkgSession({ dkgEngine: engine, nodeId: 'node-1' });
    ...
    test('initDkgSession rejects when session already active', () => {
      ...
      expect(() => clusterSync.initDkgSession({ dkgEngine: engine, nodeId: 'node-1' }))
  ...
  describe('L2-02: Share verification failure', () => {
    test('invalid share triggers DKG_SHARE_REJECTED and DKG_COMPLAINT_FILED', () => {
      ...
      clusterSync._handleDkgMessage({
        ...
        nodeId: 'node-2',
        commitments: contrib.commitments.map((c) => clusterSync._serializeDkgBigInt(c)),
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_share_rejected' });
      expect(events.events.length).toBeGreaterThan(0);

      const complaints = clusterSync.queryEvents({ eventType: 'dkg_complaint_filed' });
  ...
  describe('L2-03: Non-leader sends DKG_FINALIZE', () => {
    test('when leader is known, non-leader DKG_FINALIZE is rejected', () => {
      ...
      const completedEvents = clusterSync.queryEvents({ eventType: 'dkg_session_completed' });
  ...
  describe('L2-06: Malformed DKG message', () => {
    test('DKG_COMMIT with missing commitments field', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      expect(events.events.length).toBeGreaterThan(0);
  ...
  describe('L3-01: Oversize commitment rejected', () => {
    test('oversize hex commitment is still accepted at transport (engine validates)', () => {
      ...
      const commitEvents = clusterSync.queryEvents({ eventType: 'dkg_commit_received' });
  ...
  describe('L3-02: DKG_SHARE with wrong recipient rejected', () => {
    test('DKG_SHARE intended for another node is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const shareMustUnicast = events.events.find((e) => e.details.reason === 'share_must_unicast');
  ...
  describe('L3-03: Duplicate DKG_COMMIT rejected', () => {
    test('second DKG_COMMIT from same node is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const dup = events.events.find((e) => e.details.reason === 'duplicate_commit');
  ...
  describe('L3-04: DKG message outside active session', () => {
    test('DKG_COMMIT with no active session is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const noSession = events.events.find((e) => e.details.reason === 'no_active_session');
  ...
  describe('L3-06 / S-02: Private shares not in telemetry', () => {
    test('DKG_SHARE_RECEIVED event does not contain share value', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_share_received' });
  ...
  describe('S-05: Hex fields validated as strict lowercase', () => {
    test('uppercase hex in DKG_COMMIT commitments is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const invalidHex = events.events.find((e) => e.details.reason === 'invalid_hex');
  ...
  describe('DKG_COMPLAINT handling', () => {
    test('valid complaint is stored and recorded', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_complaint_filed' });
      expect(events.events.length).toBe(1);
  ...
  describe('DKG_FINALIZE handling', () => {
    test('valid DKG_FINALIZE completes session', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_session_completed' });
      expect(events.events.length).toBe(1);
    });
    ...
    test('DKG_FINALIZE with missing masterPublicKey is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const missing = events.events.find((e) => e.details.reason === 'missing_masterPublicKey');
  ...
  describe('session ID mismatch', () => {
    test('DKG_COMMIT with wrong session ID is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const mismatch = events.events.find((e) => e.details.reason === 'session_mismatch');
  ...
  describe('DKG_DISQUALIFY handling', () => {
    test('valid DKG_DISQUALIFY adds nodes to disqualified list', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_node_disqualified' });
      expect(events.events.length).toBe(1);
    });
    ...
    test('DKG_DISQUALIFY with missing list is rejected', () => {
      ...
      const events = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
      const missing = events.events.find((e) => e.details.reason === 'missing_disqualified_list');
      expect(missing).toBeDefined();
    });
  });
});
