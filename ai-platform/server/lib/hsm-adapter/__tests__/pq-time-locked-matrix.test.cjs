...
describe("Track 62 PQ time-locked matrix", () => {
  test("PqcTimeLockedMatrixRouter initializes a matrix and emits TIME_LOCK_MATRIX_INITIALIZED", () => {
    ...
    expect(events.some((e) => e.event === "TIME_LOCK_MATRIX_INITIALIZED")).toBe(
      true,
    );
  });
  ...
  test("MpcTemporalValidityVerifier verifies a valid temporal proof and emits TEMPORAL_DECRYPTION_PROVE_VERIFIED", () => {
    ...
    const proof = verifier.verifyTemporalProof(
      baseProofRequest(matrix.matrixId),
    );
    expect(proof.proofId).toBeDefined();
    expect(
      events.some((e) => e.event === "TEMPORAL_DECRYPTION_PROVE_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("MpcTemporalValidityVerifier rejects un-attested verifier relay", () => {
    ...
    const proofReq = baseProofRequest(
      router.getMatrixCount() > 0 ? "matrix-001" : "matrix-001",
    );
    proofReq.verifierRelayAttestation = { authority: "bad" };
    expect(() => verifier.verifyTemporalProof(proofReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq time-locked matrix configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", {
        timeDelaySeconds: 3600,
        committeeQuorum: 3,
        payloadBytes: 1024,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", { timeDelaySeconds: 60 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", { committeeQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", { payloadBytes: 2000000 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", {
    ...
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", {
        banPrematureOrMalformedProofs: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTimeLockedMatrix", {
