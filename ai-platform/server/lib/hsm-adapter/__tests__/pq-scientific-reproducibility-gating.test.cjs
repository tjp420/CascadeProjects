 ...
 */
const { PqcScientificReproducibilityGatingHub } = require('../pqc-scientific-reproducibility-gating-hub.cjs');
const { ZkReplicationClaimValidator } = require('../zk-replication-claim-validator.cjs');
...
describe('Track 83 PQ scientific reproducibility gating', () => {
  test('PqcScientificReproducibilityGatingHub initializes a pool and emits RESEARCH_GATING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'RESEARCH_GATING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkReplicationClaimValidator verifies a replication claim and emits ZK_REPLICATION_CLAIM_VERIFIED', () => {
    ...
    const claim = validator.verifyReplicationClaim(baseClaimRequest(pool.poolId));
    expect(claim.claimId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_REPLICATION_CLAIM_VERIFIED')).toBe(true);
  });
  ...
  test('PqcScientificReproducibilityGatingHub completes accreditation after claim and emits PEER_REVIEW_ACCREDITATION_COMPLETED', () => {
    ...
    expect(events.some((e) => e.event === 'PEER_REVIEW_ACCREDITATION_COMPLETED')).toBe(true);
  });
  ...
  test('ZkReplicationClaimValidator rejects un-attested integrity committee', () => {
    ...
    expect(() => validator.verifyReplicationClaim(clReq)).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq research gating configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqResearchGating', {
      peerReviewQuorum: 3,
      replicationWindowSeconds: 7884000,
      citationDepth: 24,
      pqcSignatureScheme: 'ML-DSA-65',
    ...
    expect(() => engine.validate('t1', 'pqResearchGating', { peerReviewQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { replicationWindowSeconds: 99999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { citationDepth: 96 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { researchAuthorityInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { integrityCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { banMalformedOrOutOfOrderReplicationClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqResearchGating', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
