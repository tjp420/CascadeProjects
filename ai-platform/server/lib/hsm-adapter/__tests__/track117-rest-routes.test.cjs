...
describe("Track 117 bft-shard-sync REST route integration", () => {
  ...
  test("ROUTE117-L2-07: telemetry counters match hsmMetrics.getMetrics()", async () => {
    ...
    const res = await request(app)
      .get("/api/vault/bft-shard-sync/telemetry")
      .expect(200);
    expect(res.body.telemetry.hsm_shard_append_total).toBe(
      expectedMetrics.hsm_shard_append_total || 0,
    );
    expect(res.body.telemetry.hsm_shard_ack_total).toBe(
      expectedMetrics.hsm_shard_ack_total || 0,
    );
    expect(res.body.telemetry.hsm_shard_commit_total).toBe(
      expectedMetrics.hsm_shard_commit_total || 0,
    );
    expect(res.body.telemetry.hsm_shard_catchup_batch_total).toBe(
      expectedMetrics.hsm_shard_catchup_batch_total || 0,
    );
    expect(res.body.telemetry.hsm_shard_byzantine_detected_total).toBe(
      expectedMetrics.hsm_shard_byzantine_detected_total || 0,
    );
    expect(res.body.telemetry.hsm_shard_limit_exceeded_total).toBe(
      expectedMetrics.hsm_shard_limit_exceeded_total || 0,
    );
    expect(res.body.telemetry.hsm_shard_lagging_nodes).toBe(
      expectedMetrics.hsm_shard_lagging_nodes || 0,
    );
    expect(res.body.telemetry.hsm_shard_active).toBe(
      expectedMetrics.hsm_shard_active || 0,
    );
  });
