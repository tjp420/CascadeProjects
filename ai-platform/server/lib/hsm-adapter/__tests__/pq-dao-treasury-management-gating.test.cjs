...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyProposalClaim(
...
describe("Track 84 PQ DAO treasury management gating", () => {
  test("PqcDaoTreasuryManagementGatingHub initializes a pool and emits TREASURY_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "TREASURY_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkProposalClaimValidator verifies a proposal claim and emits ZK_PROPOSAL_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_PROPOSAL_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcDaoTreasuryManagementGatingHub completes accreditation after claim and emits VOTER_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "VOTER_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq treasury gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
        proposalQuorum: 3,
        proposalWindowSeconds: 1296000,
        allocationDepth: 8,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", { proposalQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
        proposalWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", { allocationDepth: 32 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
        governanceAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
        treasuryOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
        banMalformedOrOutOfOrderProposalClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTreasuryGating", {
