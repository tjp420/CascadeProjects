...
describe("Track 72 PQ health data gating", () => {
  test("PqcHealthDataGatingHub initializes a pool and emits HEALTH_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "HEALTH_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkHealthAttributeValidator verifies a health claim and emits ZK_HEALTH_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_HEALTH_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcHealthDataGatingHub completes gating after claim and emits HEALTH_RECORD_GATING_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "HEALTH_RECORD_GATING_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq health data gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", { verificationQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
        recordExpirationLifetimeSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
        diagnosticObservationDepth: 64,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
        recordInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
        banMalformedOrOutOfOrderHealthClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHealthDataGating", {
