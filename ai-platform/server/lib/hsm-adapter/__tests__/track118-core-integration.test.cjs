...
describe('Track 118 core integration — distributedConsensusCoordinator', () => {
  ...
  test('CORE-118-01: constructor validation traps invalid configs with POLICY_VIOLATION_BLOCKED', () => {
    ...
    expectViolation({ coordinatorId: 'coord-1', nodeId: 'node-A', maxGroups: 999 });
    expectViolation({ coordinatorId: 'coord-2', nodeId: 'node-B', faultTimeoutMs: 100 });
    expectViolation({ coordinatorId: 'coord-3', nodeId: 'node-C', faultCheckIntervalMs: 9999 });
    expectViolation({ coordinatorId: 'coord-4', nodeId: 'node-D', viewChangeTimeoutMs: 100 });
  });
  ...
  test('CORE-118-02: group creation blocked when allowDynamicGroupCreation is false', () => {
    const coordinator = new DistributedConsensusCoordinator({
      coordinatorId: 'coord-block', nodeId: 'node-A',
      allowDynamicGroupCreation: false,
    });
    expect(coordinator.allowDynamicGroupCreation).toBe(false);
    try {
      coordinator.createGroup({ groupId: 'group-1', clusterNodes: ['node-A', 'node-B', 'node-C'] });
  ...
  test('CORE-118-03: cross-group routing blocked when allowCrossGroupRouting is false', () => {
    const coordinator = new DistributedConsensusCoordinator({
      coordinatorId: 'coord-routing', nodeId: 'node-A',
      allowCrossGroupRouting: false,
    });
    expect(coordinator.allowCrossGroupRouting).toBe(false);
    coordinator.createGroup({ groupId: 'group-1', clusterNodes: ['node-A', 'node-B', 'node-C'] });
    const result = coordinator.routeProposal({ groupId: 'group-1', crossGroup: true, command: { type: 'test' } });
  ...
  test('CORE-118-04: quorum bypass when requireQuorumForProposals is false', () => {
    const permissiveEngine = new CryptoPolicyEngine({
      default: {},
      tenants: { default: { distributedConsensusCoordinator: { requireQuorumForProposals: false } } },
    });
    const coordinator = new DistributedConsensusCoordinator({
      coordinatorId: 'coord-quorum', nodeId: 'node-A',
      requireQuorumForProposals: false, policyEngine: permissiveEngine,
    });
    ...
    const result = coordinator.routeProposal({ groupId: 'group-1', command: { type: 'test' } });
  ...
  test('CORE-118-05: all 10 telemetry counters fire correctly', () => {
    const coordinator = new DistributedConsensusCoordinator({
      coordinatorId: 'coord-telemetry', nodeId: 'node-A', maxGroups: 8,
    });
    coordinator.createGroup({ groupId: 'group-1', clusterNodes: ['node-A', 'node-B', 'node-C'] });
    coordinator.routeProposal({ groupId: 'group-1', command: { type: 'test' } });
    coordinator.routeProposal({ groupId: 'nonexistent', command: { type: 'test' } });
    ...
    coordinator.createGroup({ groupId: 'group-2', clusterNodes: ['node-A', 'node-B', 'node-C'] });
    ...
    const coord2 = new DistributedConsensusCoordinator({
      coordinatorId: 'coord-qd', nodeId: 'node-X', maxGroups: 8, requireQuorumForProposals: true,
    });
    coord2.createGroup({ groupId: 'group-qd', clusterNodes: ['node-X', 'node-Y', 'node-Z'] });
  ...
  test('CORE-118-06: backward compatibility — valid config passes and basic operations work', () => {
    const coordinator = new DistributedConsensusCoordinator({
      coordinatorId: 'coord-compat', nodeId: 'node-A',
      maxGroups: 8, faultTimeoutMs: 5000, faultCheckIntervalMs: 500, viewChangeTimeoutMs: 10000,
    });
    ...
    const group = coordinator.createGroup({ groupId: 'group-compat', clusterNodes: ['node-A', 'node-B', 'node-C'] });
    expect(group.groupId).toBe('group-compat');
    expect(group.state).toBe(GROUP_STATE.ACTIVE);
    const result = coordinator.routeProposal({ groupId: 'group-compat', command: { type: 'test' } });
    expect(result.accepted).toBe(true);
    const crossResult = coordinator.routeProposal({ groupId: 'group-compat', crossGroup: true, command: { type: 'test' } });
