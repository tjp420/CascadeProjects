...
} = require("../pqc-insurance-underwriting-hub.cjs");
const {
  ZkRiskExposureValidator,
} = require("../zk-risk-exposure-validator.cjs");
const {
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyClaimEligibility(
...
describe("Track 67 PQ insurance underwriting", () => {
  test("PqcInsuranceUnderwritingHub initializes a pool and emits INSURANCE_POOL_INITIALIZED", () => {
    ...
    expect(events.some((e) => e.event === "INSURANCE_POOL_INITIALIZED")).toBe(
      true,
    );
  });
  ...
  test("ZkRiskExposureValidator verifies claim eligibility and emits ZK_CLAIM_ELIGIBILITY_VERIFIED", () => {
    ...
    const claim = validator.verifyClaimEligibility(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(
      events.some((e) => e.event === "ZK_CLAIM_ELIGIBILITY_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("PqcInsuranceUnderwritingHub liquidates a pool after claim eligibility and emits UNDERWRITING_POOL_LIQUIDATED", () => {
    ...
    expect(events.some((e) => e.event === "UNDERWRITING_POOL_LIQUIDATED")).toBe(
      true,
    );
  });
  ...
  test("ZkRiskExposureValidator rejects un-attested clearing committee", () => {
    ...
    expect(() => validator.verifyClaimEligibility(claimReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq insurance underwriting configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
        reserveRatio: 50,
        claimQuorum: 3,
        poolRiskExposureCap: 1000000,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", { reserveRatio: 20 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", { claimQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
        poolRiskExposureCap: 2000000000,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
        coverageInitiatorAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
        banMalformedOrOutOfOrderClaimAssertions: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceUnderwriting", {
