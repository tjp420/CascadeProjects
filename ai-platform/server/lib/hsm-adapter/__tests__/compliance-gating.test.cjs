...
const {
  FipsSelfTestRunner,
  FipsCriticalFaultError,
} = require("../fips-self-test-runner.cjs");
const { CryptoPolicyEngine } = require("../crypto-policy-engine.cjs");
const {
  RobustnessTelemetryAgent,
} = require("../robustness-telemetry-agent.cjs");
...
describe("Track 25 Compliance Gating", () => {
  ...
  describe("FipsSelfTestRunner", () => {
    ...
    test("executePowerOnSelfTests locks on bad AES vector", () => {
      ...
      try {
        expect(() => FipsSelfTestRunner.executePowerOnSelfTests()).toThrow(
          FipsCriticalFaultError,
        );
  ...
  describe("CryptoPolicyEngine FIPS gating", () => {
    test("allows P-256 ecdh under FIPS default", () => {
      ...
      expect(() =>
        engine.validate("acme", "wrap", { algorithm: "ecdh", keySize: 256 }),
      ).not.toThrow();
    });
    ...
    test("blocks P-521 ecdh when FIPS disables it", () => {
      ...
      expect(() =>
        engine.validate("acme", "wrap", { algorithm: "ecdh", keySize: 521 }),
      ).toThrow(/P-521/);
    });
    ...
    test("blocks blinding token grace when FIPS enabled", () => {
      ...
      expect(() =>
        engine.validate("acme", "homomorphic", {
  ...
  describe("RobustnessTelemetryAgent", () => {
    test("records tamper-evident events", () => {
      ...
      const event = agent.record("time", "TEMPORAL_DRIFT", {
        drift: 12345,
        tenantId: "acme",
      });
    ...
    test("produces attestation snapshot", () => {
      ...
      expect(
        attestation.events.every((e) => typeof e.integrity === "string"),
      ).toBe(true);
    });
    ...
    test("sanitizes Buffer values to length only (no raw bytes)", () => {
      ...
      const event = agent.record("hsm", "KEY_OPERATION", {
