...
const { CircuitBreaker, STATES, DEFAULT_THRESHOLD, DEFAULT_COOLDOWN_MS } = require('../circuit-breaker.cjs');
...
describe('CircuitBreaker', () => {
  ...
  beforeEach(() => {
    ...
    breaker = new CircuitBreaker({
      ...
      cooldownMs: 100,
      name: 'test-hsm',
      onTransition: (newState, prev, info) => transitions.push({ newState, prev, info }),
    });
  });
  ...
  test('onTransition callback receives provider name', () => {
    ...
    const b = new CircuitBreaker({
      threshold: 1,
      name: 'azure-prod',
      onTransition: (_new, _prev, info) => { receivedName = info.name; },
    });
...
describe('HSM Metrics', () => {
  ...
  test('all expected counter names are present', () => {
    ...
    const expectedCounters = [
      'hsm_wrap_total', 'hsm_wrap_failures_total',
      'hsm_unwrap_total', 'hsm_unwrap_failures_total',
