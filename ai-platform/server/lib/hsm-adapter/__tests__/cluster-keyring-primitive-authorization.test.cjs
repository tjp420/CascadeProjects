...
const { ClusterKeyringPrimitiveAuthorization } = require('../cluster-keyring-primitive-authorization.cjs');
...
class MockAttestationClient {
  constructor() { this._verifiedNodes = new Set(); }
  verify(attestation) {
    if (!attestation || typeof attestation !== 'object') return { verified: false };
    if (!attestation.authority || attestation.authority !== 'mock-authority') return { verified: false };
    return { verified: true };
  }
  isNodeVerified(nodeId) { return this._verifiedNodes.has(nodeId); }
  isVerified(nodeId) { return this._verifiedNodes.has(nodeId); }
  markVerified(nodeId) { this._verifiedNodes.add(nodeId); }
}

class MockShardDisperser {
  constructor() { this.dispersed = []; }
  disperse(request) {
...
class MockRatchet {
  constructor() { this.evolved = []; }
  evolveShare(token, epochId) {
    const result = { nodeIndex: token.nodeIndex, sequence: token.sequence, value: token.value + 1n, ratchet: { derivedAt: Date.now(), epoch: epochId } };
...
class MockReconciler {
  constructor() { this.divergence = { severity: 'none', divergentNodes: [] }; }
  detectDivergence(keyId) { return { keyId, ...this.divergence }; }
}
...
function setupGate() {
  ...
  const hub = new PqcQuantumKeyDistributionLinkSwitchGatingHub({ policy: {
    minQkdQuorum: 18, maxEntanglementWindowSeconds: 60, maxQkdSwitchChainDepth: 42,
    allowedPqcSignatureSchemes: ['ML-DSA-44', 'ML-DSA-65', 'ML-DSA-87'],
    ...
    requireCanonicalPayloadLayout: true,
  }, attestationClient, audit: () => {} });
  const validator = new ZkQkdLinkClaimValidator({ policy: {
    minQkdQuorum: 18, maxEntanglementWindowSeconds: 60, maxQkdSwitchChainDepth: 42,
    allowedPqcSignatureSchemes: ['ML-DSA-44', 'ML-DSA-65', 'ML-DSA-87'],
    requireQkdLinkAuthorityInitializerAttestation: true,
    requireQkdEthicsOversightCommitteeAttestation: true,
    ...
    banMalformedOrOutOfOrderQkdLinkClaims: true,
    requireCanonicalPayloadLayout: true,
  }, hub, attestationClient, audit: () => {} });
  const pool = hub.initializePool({
    sourceTenantId: 'tenant-a', targetChainId: 'chain-b',
    sourceOpticalNodeId: 'optical-001', targetOpticalNodeId: 'optical-002',
    ...
    entanglementWindowSeconds: 60, qkdSwitchChainDepth: 22,
...
function setupAuthorization() {
  ...
  return { ...gate, auth, events, shardDisperser, ratchet, stateSync, reconciler, keyringSync };
}
...
describe('Cluster Keyring Sync Integration Layer', () => {
  test('L2-01: authorizes an accredited pool and emits PRIMITIVE_POOL_AUTHORIZED', () => {
    ...
    const rec = auth.authorizeAccreditedPool('qkd', pool.poolId, baseAuthorizeRequest(pool.poolId));
    expect(rec.authorizationId).toBeDefined();
    expect(rec.status).toBe('active');
    expect(rec.trackType).toBe('qkd');
    expect(events.some(e => e.event === 'PRIMITIVE_POOL_AUTHORIZED')).toBe(true);
    expect(keyringSync.events.some(e => e.eventType === 'primitive_pool_authorized')).toBe(true);
  });
  ...
  test('L2-02: rejects un-accredited pool', () => {
    ...
    const pool = hub.initializePool({
      sourceTenantId: 'tenant-a', targetChainId: 'chain-b',
      sourceOpticalNodeId: 'optical-001', targetOpticalNodeId: 'optical-002',
      blindedOpticalLinkPathCommitment: 'c1', blindedQuantumSecretSharingCommitment: 'c2',
      blindedEntanglingChannelCommitment: 'c3', entanglementWindowSeconds: 60,
      qkdSwitchChainDepth: 22, pqcSignatureScheme: 'ML-DSA-65',
      attestationAuthority: 'mock-authority',
    });
    const auth = new ClusterKeyringPrimitiveAuthorization({ policy: POLICY, attestationClient });
    auth.registerGate('qkd', hub);
    expect(() => auth.authorizeAccreditedPool('qkd', pool.poolId, baseAuthorizeRequest(pool.poolId))).toThrow(HsmAdapterError);
  });
  ...
  test('L2-03: rejects un-attested node', () => {
    ...
    expect(() => auth.authorizeAccreditedPool('qkd', pool.poolId, req)).toThrow(HsmAdapterError);
  });
  ...
  test('L2-04: syncs authorized pool to target enclave and emits PRIMITIVE_POOL_SYNCED', () => {
    ...
    auth.authorizeAccreditedPool('qkd', pool.poolId, baseAuthorizeRequest(pool.poolId));
    ...
    expect(keyringSync.events.some(e => e.eventType === 'primitive_pool_synced')).toBe(true);
  ...
  test('L2-05: revokes authorization and emits PRIMITIVE_AUTHORIZATION_REVOKED', () => {
    ...
    expect(events.some(e => e.event === 'PRIMITIVE_AUTHORIZATION_REVOKED')).toBe(true);
    expect(keyringSync.events.some(e => e.eventType === 'primitive_authorization_revoked')).toBe(true);
  });
  ...
  test('L3-01: rejects unregistered gate trackType', () => {
    ...
    expect(() => auth.authorizeAccreditedPool('unknown', 'pool-1', {})).toThrow(HsmAdapterError);
  });
  ...
  test('L3-03: rejects sync after sync window exceeded', () => {
    ...
    expect(() => auth.syncAuthorizedPool(pool.poolId, 'enclave-002')).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates cluster keyring primitive authorization configuration', () => {
    ...
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', {
      authorizationQuorum: 3, syncWindowSeconds: 300, authorizedPoolRetentionSeconds: 86400,
      pqcSignatureScheme: 'ML-DSA-65', nodeAttestation: true, accreditedPoolStatus: true,
      attestationAuthority: 'mock-authority', banUnauthorizedShareDispersal: true,
      canonicalPayloadLayout: true,
    })).not.toThrow();
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { authorizationQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { syncWindowSeconds: 999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { authorizedPoolRetentionSeconds: 999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { nodeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { accreditedPoolStatus: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { banUnauthorizedShareDispersal: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'clusterKeyringPrimitiveAuthorization', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
