...
const { PqcSwarmRoboticsKineticAssemblyGatingHub } = require('../pqc-swarm-robotics-kinetic-assembly-gating-hub.cjs');
const { ZkKineticClaimValidator } = require('../zk-kinetic-claim-validator.cjs');
...
function makeHub() {
  return new PqcSwarmRoboticsKineticAssemblyGatingHub({ policy: DEFAULT_POLICY });
}
...
describe('Track 114 PQC Swarm Robotics Kinetic Assembly Gating Hub', () => {
  ...
  test('initializes a kinetic assembly pool and increments counter', () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_kineticgate_pool_initialized_total).toBe(1);
  });
  ...
  test('verifies kinetic posture and increments counter', () => {
    ...
    const verified = hub.verifyKineticPosture({ poolId: pool.poolId, proofValid: true });
    expect(verified.kineticPostureVerified).toBe(true);
    expect(hsmMetrics.getMetrics().hsm_zk_kinetic_posture_verified_total).toBe(1);
  });
  ...
  test('completes assembly accreditation with sufficient quorum', () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_assembly_accreditation_completed_total).toBe(1);
  });
  ...
  test('rejects assembly accreditation with insufficient quorum', () => {
    ...
    hub.verifyKineticPosture({ poolId: pool.poolId, proofValid: true });
    const code = expectErrorCode(() => hub.completeAssemblyAccreditation({
      poolId: pool.poolId,
