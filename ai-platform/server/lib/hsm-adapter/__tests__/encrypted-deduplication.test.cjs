 ...
 */
const { EncryptedStorageDeduplicator } = require('../encrypted-storage-deduplicator.cjs');
...
describe('Track 55 encrypted storage deduplication', () => {
  ...
  test('EncryptedStorageDeduplicator detects duplicate chunks across tenants', () => {
    ...
    expect(events.some((e) => e.event === 'DUPLICATE_BLOCK_RECONCILED')).toBe(true);
  });
  ...
  test('BlindedConvergenceGuard bans peers after consecutive malformed tokens', () => {
    ...
    expect(events.some((e) => e.event === 'PEER_BANNED_MALFORMED_CHUNK')).toBe(true);
  });
  ...
  test('CryptoPolicyEngine validates encrypted deduplication configuration', () => {
    ...
    expect(() => engine.validate('t1', 'encryptedDeduplication', {
      chunkBitLength: 512,
    ...
    expect(() => engine.validate('t1', 'encryptedDeduplication', { chunkBitLength: 128 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { chunkBitLength: 8192 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { crossTenantChunkAllocations: 32 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { blindingGroup: 'secp256k1' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { submitterAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { banMalformedChunkPeers: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'encryptedDeduplication', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
