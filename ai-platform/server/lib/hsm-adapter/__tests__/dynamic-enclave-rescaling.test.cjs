...
describe("Track 49: DynamicEnclaveRescaler", () => {
  ...
  describe("registerEnclave", () => {
    ...
    test("enforces max enclaves", () => {
      const small = new DynamicEnclaveRescaler({
        minEnclaves: 1,
        maxEnclaves: 2,
      });
  ...
  describe("unregisterEnclave", () => {
    ...
    test("rejects unknown enclave", () => {
      expect(() => rescaler.unregisterEnclave("unknown")).toThrow(
        HsmAdapterError,
      );
    });
  });
  ...
  describe("recordLoad", () => {
    ...
    test("rejects unknown enclave", () => {
      expect(() => rescaler.recordLoad("unknown", 0.5)).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("executeScaling", () => {
    ...
    test("returns not executed for no-action", () => {
      const result = rescaler.executeScaling({
  ...
  describe("registerShard", () => {
    ...
    test("rejects duplicate shard", () => {
      ...
      expect(() => rescaler.registerShard("shard-1", [])).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("onChaosEvent", () => {
    ...
    test("returns not triggered when disabled", () => {
      const disabled = new DynamicEnclaveRescaler({
        chaosTriggerEnabled: false,
      });
