 ...
 */
const { PqcLendingCollateralHub } = require('../pqc-lending-collateral-hub.cjs');
const { ZkSolvencyProofProcessor } = require('../zk-solvency-proof-processor.cjs');
...
describe('Track 66 PQ lending pools', () => {
  test('PqcLendingCollateralHub initializes a pool and emits LENDING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'LENDING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkSolvencyProofProcessor verifies solvency and emits ZK_SOLVENCY_PROOF_VERIFIED', () => {
    ...
    const proof = processor.verifySolvencyProof(baseSolvencyRequest(pool.poolId));
    expect(proof.proofId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_SOLVENCY_PROOF_VERIFIED')).toBe(true);
  });
  ...
  test('PqcLendingCollateralHub liquidates a pool after solvency and emits COLLATERAL_POOL_LIQUIDATED', () => {
    ...
    expect(events.some((e) => e.event === 'COLLATERAL_POOL_LIQUIDATED')).toBe(true);
  });
  ...
  test('PqcLendingCollateralHub rejects un-attested borrower', () => {
    ...
    const hub = new PqcLendingCollateralHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkSolvencyProofProcessor rejects un-attested clearing committee', () => {
    ...
    const processor = new ZkSolvencyProofProcessor({ policy: POLICY, hub, attestationClient });
    const solvReq = baseSolvencyRequest(pool.poolId);
    solvReq.clearingCommitteeAttestation = { authority: 'bad' };
    expect(() => processor.verifySolvencyProof(solvReq)).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq lending pools configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqLendingPools', {
      ltvRatio: 75,
    ...
    expect(() => engine.validate('t1', 'pqLendingPools', { ltvRatio: 30 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { liquidationSignatureQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { borrowValueCap: 2000000000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { borrowerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { clearingCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { banMalformedOrSubSolvencyClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqLendingPools', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
