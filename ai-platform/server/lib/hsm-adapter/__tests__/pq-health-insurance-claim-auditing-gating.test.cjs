...
describe("Track 86 PQ health insurance claim auditing gating", () => {
  test("PqcHealthInsuranceClaimAuditingGatingHub initializes a pool and emits INSURANCE_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "INSURANCE_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkInsuranceClaimValidator verifies a claim audit and emits ZK_CLAIM_AUDIT_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_CLAIM_AUDIT_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcHealthInsuranceClaimAuditingGatingHub completes accreditation after claim and emits ACTUARIAL_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "ACTUARIAL_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq insurance gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
        claimsAuditQuorum: 3,
        claimWindowSeconds: 2592000,
        billingSequenceDepth: 12,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", { claimsAuditQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
        claimWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", { billingSequenceDepth: 48 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
        insuranceAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
        actuarialCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
        banMalformedOrOutOfOrderClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqInsuranceGating", {
