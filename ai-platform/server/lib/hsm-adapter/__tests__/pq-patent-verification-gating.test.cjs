...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyPatentClaim(
...
describe("Track 74 PQ patent verification gating", () => {
  test("PqcPatentVerificationGatingHub initializes a pool and emits PATENT_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "PATENT_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkPatentClaimValidator verifies a patent claim and emits ZK_PATENT_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_PATENT_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcPatentVerificationGatingHub completes accreditation after claim and emits PATENT_LICENSE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "PATENT_LICENSE_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq patent gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
        licensingQuorum: 3,
        patentExpirationSeconds: 23652000,
        claimScopeDepth: 16,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqPatentGating", { licensingQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
        patentExpirationSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPatentGating", { claimScopeDepth: 64 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
        patentOfficeInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqPatentGating", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
        banMalformedOrOutOfOrderPatentClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPatentGating", {
