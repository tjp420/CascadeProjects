...
const {
  PqcHolographicStorageContentAddressableGatingHub,
} = require("../pqc-holographic-storage-content-addressable-gating-hub.cjs");
const {
  ZkHolographicClaimValidator,
} = require("../zk-holographic-claim-validator.cjs");
...
function baseInitRequest() {
  return {
    ...
    sourceVolumetricSectorId: "sector-001",
    targetVolumetricSectorId: "sector-002",
    blindedVolumetricSectorDigestCommitment: "pedersen-volumetric-sector-001",
    blindedHolographicStateCommitment: "pedersen-holographic-state-001",
    blindedInterferencePatternPhaseCommitment:
      "pedersen-interference-phase-001",
    phaseValidationWindowSeconds: 10,
    volumetricChainDepth: 22,
    pqcSignatureScheme: "ML-DSA-65",
    holographicStorageAuthorityInitializerAttestation: mockAttestation(),
...
function baseClaimRequest(poolId) {
  return {
    ...
    blindedInterferencePatternPhaseCommitment:
      "pedersen-interference-phase-001",
    zkHolographicRangeProofHash: "zk-holographic-proof-001",
    holographicStateDigest: "hgs-digest-001",
    holographicEthicsOversightCommitteeAttestation: mockAttestation(),
    attestationAuthority: "mock-authority",
  };
}
...
function setupHubAndValidator() {
  ...
  const hub = new PqcHolographicStorageContentAddressableGatingHub({
    policy: POLICY,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
  });
  const validator = new ZkHolographicClaimValidator({
    policy: POLICY,
    hub,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyHolographicClaim(
...
describe("Track 110 PQ holographic storage content-addressable gating", () => {
  test("initializes a pool and emits HOLOGRAPHIC_POOL_INITIALIZED", () => {
    ...
    expect(events.some((e) => e.event === "HOLOGRAPHIC_POOL_INITIALIZED")).toBe(
      true,
    );
  });
  ...
  test("verifies a holographic claim and emits ZK_HOLOGRAPHIC_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyHolographicClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(claim.holographicStateDigest).toBe("hgs-digest-001");
    expect(
      events.some((e) => e.event === "ZK_HOLOGRAPHIC_CLAIM_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("completes accreditation after claim and emits PHASE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "PHASE_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects phase validation window exceeding maximum", () => {
    ...
    req.phaseValidationWindowSeconds = 999999;
  ...
  test("rejects un-attested holographic storage authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.holographicStorageAuthorityInitializerAttestation = {
      authority: "bad",
    };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested holographic ethics oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkHolographicClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.holographicEthicsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkHolographicRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkHolographicRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing holographicStateDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.holographicStateDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyHolographicClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq holographic storage content-addressable gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
        phaseValidationWindowSeconds: 999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
        volumetricChainDepth: 52,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
        holographicStorageAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
        holographicEthicsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
        banMalformedOrOutOfOrderHolographicClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHolographicStorageContentAddressableGating", {
