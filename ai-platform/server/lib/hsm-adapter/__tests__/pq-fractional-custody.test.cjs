 ...
 */
const { PqcFractionalCustodyHub } = require('../pqc-fractional-custody-hub.cjs');
const { ZkFractionalReleaseVerifier } = require('../zk-fractional-release-verifier.cjs');
...
describe('Track 65 PQ fractional custody', () => {
  test('PqcFractionalCustodyHub initializes a vault and emits FRACTIONAL_VAULT_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'FRACTIONAL_VAULT_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkFractionalReleaseVerifier records a release and emits FRACTIONAL_RELEASE_SIGNED', () => {
    ...
    expect(events.some((e) => e.event === 'FRACTIONAL_RELEASE_SIGNED')).toBe(true);
  });
  ...
  test('PqcFractionalCustodyHub liquidates a vault after reconciliation and emits CUSTODY_VAULT_LIQUIDATED', () => {
    ...
    const liquidation = hub.liquidateVault(baseLiquidateRequest(vault.vaultId, 300));
    expect(liquidation.liquidationId).toBeDefined();
    expect(events.some((e) => e.event === 'CUSTODY_VAULT_LIQUIDATED')).toBe(true);
  });
  ...
  test('ZkFractionalReleaseVerifier rejects un-attested custodian relay', () => {
    ...
    expect(() => verifier.verifyFractionalRelease(releaseReq)).toThrow(HsmAdapterError);
  });
  ...
  test('PqcFractionalCustodyHub rejects liquidation before quorum', () => {
    const { hub, vault } = setupAndInitVault();
    expect(() => hub.liquidateVault(baseLiquidateRequest(vault.vaultId, 0))).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq fractional custody configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqFractionalCustody', {
      custodianQuorum: 3,
      fractionalBits: 32,
      assetCustodyCap: 1000000,
      pqcSignatureScheme: 'ML-DSA-65',
    ...
    expect(() => engine.validate('t1', 'pqFractionalCustody', { custodianQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { fractionalBits: 128 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { assetCustodyCap: 2000000000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { claimantAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { custodianRelayAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { banMalformedOrOutOfOrderCustodyClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqFractionalCustody', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
