...
describe('Track 47: PqcEnclaveMigrationEngine', () => {
  ...
  describe('registerEnclave', () => {
    ...
    test('rejects unsupported classical algorithm', () => {
      expect(() => engine.registerEnclave('e1', { currentAlgorithm: 'DES' })).toThrow(HsmAdapterError);
    });

    test('rejects unsupported PQC algorithm', () => {
      expect(() => engine.registerEnclave('e1', { targetPqcAlgorithm: 'Kyber-512' })).toThrow(HsmAdapterError);
    });
  ...
  describe('activateHybrid', () => {
    ...
    test('requires attestation when configured', () => {
      const strict = new PqcEnclaveMigrationEngine({ requireAttestation: true });
  ...
  describe('satisfySignatureConstraint', () => {
    ...
    test('rejects invalid proof format', () => {
      ...
      engine.planMigration('e1');
      engine.activateHybrid('e1');
      expect(() => engine.satisfySignatureConstraint('e1', { algorithm: 'ML-DSA-65' }))
    ...
    test('rejects unsupported algorithm', () => {
      ...
      engine.planMigration('e1');
      engine.activateHybrid('e1');
      expect(() => engine.satisfySignatureConstraint('e1', {
        algorithm: 'RSA-2048',
        signature: 'a'.repeat(128),
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects unknown enclave', () => {
      expect(() => engine.satisfySignatureConstraint('unknown', {
        algorithm: 'ML-DSA-65',
        signature: 'a'.repeat(128),
      })).toThrow(HsmAdapterError);
    });
  ...
  describe('violateSignatureConstraint', () => {
    ...
    test('rejects unknown enclave', () => {
      expect(() => engine.violateSignatureConstraint('unknown')).toThrow(HsmAdapterError);
    });
  });
