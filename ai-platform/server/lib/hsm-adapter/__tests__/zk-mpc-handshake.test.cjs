 ...
 */
const { ZkMpcHandshake, DEFAULT_OPTIONS, HANDSHAKE_PHASE, PROOF_STATUS, generateZkProof } = require('../zk-mpc-handshake.cjs');
...
describe('Track 46: ZkMpcHandshake', () => {
  ...
  beforeEach(() => {
    engine = new ZkMpcHandshake({ minParticipants: 2, maxParticipants: 5, handshakeTimeoutMs: 5000 });
  });
  ...
  describe('initiate', () => {
    ...
    test('rejects insufficient participants', () => {
      expect(() => engine.initiate({ participantIds: ['e1'] })).toThrow(HsmAdapterError);
    });

    test('rejects too many participants', () => {
      const small = new ZkMpcHandshake({ minParticipants: 2, maxParticipants: 3 });
      expect(() => small.initiate({ participantIds: ['e1', 'e2', 'e3', 'e4'] })).toThrow(HsmAdapterError);
    });

    test('rejects duplicate participants', () => {
      expect(() => engine.initiate({ participantIds: ['e1', 'e1'] })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects missing participantIds', () => {
      expect(() => engine.initiate({ purpose: 'test' })).toThrow(HsmAdapterError);
    });

    test('enforces max concurrent handshakes', () => {
      const small = new ZkMpcHandshake({ minParticipants: 2, maxConcurrentHandshakes: 1 });
      small.initiate({ participantIds: ['e1', 'e2'] });
      expect(() => small.initiate({ participantIds: ['e3', 'e4'] })).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('commit', () => {
    test('accepts a valid commitment', () => {
      ...
      const commitResult = engine.commit(result.handshakeId, 'e1', 'commitment-hash-1');
    ...
    test('rejects commitment from unauthorized participant', () => {
      const result = engine.initiate({ participantIds: ['e1', 'e2'] });
      expect(() => engine.commit(result.handshakeId, 'e3', 'commit-3')).toThrow(HsmAdapterError);
    });

    test('rejects duplicate commitment', () => {
      const result = engine.initiate({ participantIds: ['e1', 'e2'] });
      engine.commit(result.handshakeId, 'e1', 'commit-1');
      expect(() => engine.commit(result.handshakeId, 'e1', 'commit-1b')).toThrow(HsmAdapterError);
    });

    test('rejects empty commitment', () => {
      const result = engine.initiate({ participantIds: ['e1', 'e2'] });
      expect(() => engine.commit(result.handshakeId, 'e1', '')).toThrow(HsmAdapterError);
    });

    test('rejects commitment to unknown handshake', () => {
      expect(() => engine.commit('fake-id', 'e1', 'commit')).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('prove', () => {
    ...
    test('rejects proof before commitment', () => {
      const result = engine.initiate({ participantIds: ['e1', 'e2'] });
      const proof = generateZkProof('sha256', 'secret-1', 'commit-1');
      expect(() => engine.prove(result.handshakeId, 'e1', proof)).toThrow(HsmAdapterError);
    });
    ...
    test('rejects invalid proof format', () => {
      ...
      engine.commit(result.handshakeId, 'e1', 'commit-1');
      engine.commit(result.handshakeId, 'e2', 'commit-2');
      expect(() => engine.prove(result.handshakeId, 'e1', { challenge: 'x' })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof from unauthorized participant', () => {
      ...
      expect(() => engine.prove(result.handshakeId, 'e3', proof)).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('verifyProofs', () => {
    ...
    test('aborts when proofs are invalid', () => {
      ...
      engine.prove(result.handshakeId, 'e1', { challenge: 'wrong', response: 'a'.repeat(64), publicCommitment: 'pc1' });
      engine.prove(result.handshakeId, 'e2', { challenge: 'wrong', response: 'b'.repeat(64), publicCommitment: 'pc2' });
      const verifyResult = engine.verifyProofs(result.handshakeId);
    ...
    test('rejects verification in wrong phase', () => {
      const result = engine.initiate({ participantIds: ['e1', 'e2'] });
      expect(() => engine.verifyProofs(result.handshakeId)).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('finalize', () => {
    test('finalizes a verified handshake', () => {
      ...
      engine.commit(result.handshakeId, 'e1', 'commit-1');
      engine.commit(result.handshakeId, 'e2', 'commit-2');
      engine.prove(result.handshakeId, 'e1', generateZkProof('sha256', 's1', 'commit-1'));
      engine.prove(result.handshakeId, 'e2', generateZkProof('sha256', 's2', 'commit-2'));
      engine.verifyProofs(result.handshakeId);
      const finalResult = engine.finalize(result.handshakeId, 'combined-pub-key-123');
    ...
    test('rejects finalization without verification', () => {
      const result = engine.initiate({ participantIds: ['e1', 'e2'] });
      expect(() => engine.finalize(result.handshakeId, 'key')).toThrow(HsmAdapterError);
    });
    ...
    test('rejects empty public key', () => {
      ...
      engine.prove(result.handshakeId, 'e2', generateZkProof('sha256', 's2', 'commit-2'));
      engine.verifyProofs(result.handshakeId);
      expect(() => engine.finalize(result.handshakeId, '')).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('full handshake flow', () => {
    test('complete 5-phase handshake with 3 participants', () => {
      ...
      // Phase 3: Prove
      for (const pid of ['e1', 'e2', 'e3']) {
        const proof = generateZkProof('sha256', 'secret-' + pid, 'commit-' + pid);
      ...
      // Phase 5: Finalize
      const finalResult = engine.finalize(result.handshakeId, 'aggregated-public-key');
      expect(finalResult.phase).toBe(HANDSHAKE_PHASE.FINALIZED);
  ...
  describe('getHandshake', () => {
    ...
    test('returns completed handshake from history', () => {
      ...
      engine.prove(result.handshakeId, 'e1', generateZkProof('sha256', 's1', 'c1'));
      engine.prove(result.handshakeId, 'e2', generateZkProof('sha256', 's2', 'c2'));
      engine.verifyProofs(result.handshakeId);
  ...
  describe('checkExpired', () => {
    test('expires handshakes past timeout', () => {
      const fast = new ZkMpcHandshake({ minParticipants: 2, handshakeTimeoutMs: 50 });
