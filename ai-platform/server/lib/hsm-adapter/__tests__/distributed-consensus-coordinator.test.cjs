...
describe("DistributedConsensusCoordinator", () => {
  ...
  describe("constructor", () => {
    test("requires coordinatorId", () => {
      expect(
        () => new DistributedConsensusCoordinator({ nodeId: "n1" }),
      ).toThrow();
    });

    test("requires nodeId", () => {
      expect(
        () => new DistributedConsensusCoordinator({ coordinatorId: "c1" }),
      ).toThrow();
    });

    test("creates coordinator with defaults", () => {
      const c = new DistributedConsensusCoordinator({
        coordinatorId: "c1",
        nodeId: "n1",
      });
  ...
  describe("createGroup", () => {
    ...
    test("requires groupId", () => {
      expect(() =>
        coordinator.createGroup({ clusterNodes: ["node-A"] }),
      ).toThrow();
    });
    ...
    test("requires non-empty clusterNodes", () => {
      expect(() =>
        coordinator.createGroup({ groupId: "g1", clusterNodes: [] }),
      ).toThrow();
    });
    ...
    test("emits GROUP_CREATED audit event", () => {
      ...
      expect(
        auditLog.some((e) => e.event === COORDINATOR_EVENT.GROUP_CREATED),
      ).toBe(true);
    });
  });
  ...
  describe("fault detection", () => {
    test("detects node failure after timeout", () => {
      ...
      const metrics = hsmMetrics.getMetrics();
      expect(metrics.hsm_consensus_coord_faults_detected_total).toBeGreaterThan(
        0,
      );
    });
  ...
  describe("recordHeartbeat", () => {
    ...
    test("throws for unknown group", () => {
      expect(() =>
        coordinator.recordHeartbeat("node-B", "nonexistent"),
      ).toThrow();
    });
  ...
  describe("listGroups", () => {
    test("returns all group IDs", () => {
      ...
      expect(coordinator.listGroups()).toEqual(
