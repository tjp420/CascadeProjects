...
describe('TimeAnchorEngine', () => {
  ...
  test('L2-03: fails with ORACLE_QUORUM_FAILED when quorum is not met', () => {
    ...
    let caught;
    try { engine.consensusTimestamp(1); } catch (e) { caught = e; }
  ...
  test('rejects invalid oracle signature with ORACLE_SIGNATURE_INVALID', () => {
    ...
    let caught;
    try { engine.submitPulse(keys[0].oracleId, now, badSig, 0); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
  ...
  test('L3-01: monotonic time violation throws on older timestamp', () => {
    ...
    let caught;
    try {
      engine.submitPulse(keys[0].oracleId, now - 10000, _sign(keys[0].oracleId, now - 10000, 1, keys[0].privateKey), 1);
    } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
  ...
  test('L3-01: monotonic time violation throws on older epoch number', () => {
    ...
    keys.forEach((k) => {
      engine.submitPulse(k.oracleId, now, _sign(k.oracleId, now, 5, k.privateKey), 5);
    });
    ...
    let caught;
    try {
      engine.submitPulse(keys[0].oracleId, now + 1000, _sign(keys[0].oracleId, now + 1000, 3, keys[0].privateKey), 3);
    } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
    expect(caught.code).toBe('MONOTONIC_TIME_VIOLATION');
  });
  ...
  test('S-03: median is robust to floor((N-1)/2) rogue oracles', () => {
    const { oracles, keys } = _makeOracles(5);
    const engine = new TimeAnchorEngine({ oracles, minQuorum: 3, maxDriftMs: 1000 });
    const t = Date.now();
...
describe('EpochFrame', () => {
  ...
  test('throws EPOCH_SIGNATURE_INVALID when verifying unsigned frame', () => {
    ...
    let caught;
    try { frame.verify(publicKey); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
...
describe('Policy and adapter integration', () => {
  test('CryptoPolicyEngine rejects excessive time drift', () => {
    ...
    let caught;
    try { policy.validate('t1', 'time', { maxDriftMs: 120000 }); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
  ...
  test('CryptoPolicyEngine rejects insufficient minQuorum', () => {
    ...
    let caught;
    try { policy.validate('t1', 'time', { minQuorum: 1 }); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
    expect(caught.code).toBe('POLICY_VIOLATION_BLOCKED');
  });
  ...
  test('L2-05: verifyTemporalGuard blocks drift with TEMPORAL_DRIFT_BLOCKED', async () => {
    ...
    let caught;
    try { adapter.verifyTemporalGuard('t1', now + 10000); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
  ...
  test('S-04: wrap is blocked on excessive clock drift', async () => {
    ...
    let caught;
    try { await adapter.wrap('t1', kekId, plaintext); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
    expect(caught.code).toBe('TEMPORAL_DRIFT_BLOCKED');
  });
  ...
  test('S-04: unwrap is blocked on excessive clock drift', async () => {
    ...
    const driftedEngine = new TimeAnchorEngine({ oracles, minQuorum: 3, maxDriftMs: 5000 });
    keys.forEach((k) => {
      driftedEngine.submitPulse(k.oracleId, now - 100000, _sign(k.oracleId, now - 100000, 1, k.privateKey), 1);
    });
    driftedEngine.consensusTimestamp(2);
    adapter._timeAnchor = driftedEngine;

    let caught;
    try { await adapter.unwrap('t1', kekId, wrapped); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
    expect(caught.code).toBe('TEMPORAL_DRIFT_BLOCKED');
  });
  ...
  test('S-04: rotateKEK is blocked on excessive clock drift', async () => {
    ...
    let caught;
    try { await adapter.rotateKEK('t1', kekId); } catch (e) { caught = e; }
    expect(caught).toBeInstanceOf(HsmAdapterError);
    expect(caught.code).toBe('TEMPORAL_DRIFT_BLOCKED');
  });
