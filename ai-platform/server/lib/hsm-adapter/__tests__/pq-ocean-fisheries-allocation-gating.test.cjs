...
describe("Track 94 PQ ocean fisheries allocation gating", () => {
  test("PqcOceanFisheriesAllocationGatingHub initializes a pool and emits FISHERIES_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "FISHERIES_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkCatchClaimValidator verifies a catch claim and emits ZK_CATCH_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_CATCH_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcOceanFisheriesAllocationGatingHub completes accreditation after claim and emits QUOTA_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "QUOTA_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq fisheries gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
        maritimeQuorum: 5,
        catchTrackingWindowSeconds: 1296000,
        vesselTelemetryChainDepth: 6,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", { maritimeQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
        catchTrackingWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
        vesselTelemetryChainDepth: 24,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
        rfmoAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
        marineSanctuaryOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
        banMalformedOrOutOfOrderCatchClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqFisheriesGating", {
