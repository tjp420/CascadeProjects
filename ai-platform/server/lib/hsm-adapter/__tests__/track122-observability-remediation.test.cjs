...
const HSM_VAULT_ROUTES = path.join(
  __dirname,
  "..",
  "..",
  "..",
  "routes",
  "hsm-vault-routes.cjs",
);
...
describe("Track 122 — Observability Remediation", () => {
  ...
  test("REMED-122-02d: Track31 alert uses + 1 division guard", () => {
    expect(yamlText).toContain(
      "(rate(hsm_lookupgate_pool_initialized_total[5m]) + 1)",
    );
  });

  test("REMED-122-02e: Track32 alert uses + 1 division guard", () => {
    expect(yamlText).toContain(
      "(rate(hsm_ringgate_pool_initialized_total[5m]) + 1)",
    );
  });
  ...
  test("REMED-122-03a: resolveOrgId wraps return value in String()", () => {
    ...
    expect(routesContent).toMatch(
      /function resolveOrgId\(req\)\s*\{[\s\S]*return String\(/,
    );
  });
  ...
  test("REMED-122-04b: TRACK113_PQC_HANDSHAKE_STALL.md exists on disk", () => {
    expect(
      fs.existsSync(path.join(RUNBOOK_DIR, "TRACK113_PQC_HANDSHAKE_STALL.md")),
    ).toBe(true);
  });
  ...
  test("REMED-122-04c: TRACK31_HOMOMORPHIC_LOOKUP_STALL.md exists on disk", () => {
    expect(
      fs.existsSync(
        path.join(RUNBOOK_DIR, "TRACK31_HOMOMORPHIC_LOOKUP_STALL.md"),
      ),
    ).toBe(true);
  });
  ...
  test("REMED-122-04d: TRACK117_BYZANTINE_SHARD_DIVERGENCE_DETECTED.md exists on disk", () => {
    expect(
      fs.existsSync(
        path.join(
          RUNBOOK_DIR,
          "TRACK117_BYZANTINE_SHARD_DIVERGENCE_DETECTED.md",
        ),
      ),
    ).toBe(true);
  });
  ...
  test("REMED-122-04e: MESH_RECONCILIATION_STALL.md exists on disk", () => {
    expect(
      fs.existsSync(path.join(RUNBOOK_DIR, "MESH_RECONCILIATION_STALL.md")),
    ).toBe(true);
  });
