...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyWaterClaim(
...
describe("Track 88 PQ water rights allocation gating", () => {
  test("PqcWaterRightsAllocationGatingHub initializes a pool and emits WATER_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "WATER_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkWaterRightsClaimValidator verifies a water claim and emits ZK_WATER_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_WATER_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcWaterRightsAllocationGatingHub completes accreditation after claim and emits WATERSHED_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "WATERSHED_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq water gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqWaterGating", {
        watershedQuorum: 4,
        allocationWindowSeconds: 15768000,
        flowChainDepth: 10,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqWaterGating", { watershedQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWaterGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqWaterGating", { flowChainDepth: 40 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWaterGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqWaterGating", {
        waterAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWaterGating", {
        watershedOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWaterGating", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWaterGating", {
        banMalformedOrOutOfOrderWaterClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWaterGating", { canonicalPayloadLayout: false }),
    ).toThrow(HsmAdapterError);
  });
});
