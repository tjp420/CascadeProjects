...
describe("Track 42 dynamic resharding", () => {
  ...
  test("EphemeralShareRatchet advances and zeroizes old state", () => {
    const ratchet = new EphemeralShareRatchet({
      seed: "initial-seed",
      epoch: 0,
    });
  ...
  test("CryptoPolicyEngine validates resharding configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "resharding", { threshold: 4, committeeSize: 9 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "resharding", { committeeSize: 100 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "resharding", { expansionFactor: 9 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "resharding", { epochIntervalMs: 100 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "resharding", { requireEphemeralRatchet: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "resharding", { newNodeAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "resharding", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
  });
});
