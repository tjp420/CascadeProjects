 ...
 */
const {
  ClusterRecoveryCoordinator,
} = require("../cluster-recovery-coordinator.cjs");
...
describe("Track 33 cluster recovery", () => {
  ...
  test("exponential back-off triggers on delivery failure", async () => {
    ...
    await expect(streamer.stream(session, coordinator)).rejects.toThrow(
      HsmAdapterError,
    );
  ...
  test("rejects non-cluster node recovery", () => {
    ...
    expect(() => coordinator.startRecovery("node-z", "shard-1", 0)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates recoverySync configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "recoverySync", {
        maxCatchUpBatchSize: 64,
        reSyncRetryLimit: 5,
    ...
    expect(() =>
      engine.validate("t1", "recoverySync", { maxCatchUpBatchSize: 100 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "recoverySync", { reSyncRetryLimit: 10 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "recoverySync", { backoffBaseIntervalMs: 120000 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "recoverySync", { catchUpMode: "burst" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "recoverySync", { bftAck: false }),
    ).toThrow(HsmAdapterError);
  });
  ...
  test("tenant-specific recoverySync policy overrides default", () => {
    ...
    expect(() =>
      engine.validate("t-permissive", "recoverySync", {
        maxCatchUpBatchSize: 128,
...
describe("Track 33 base-adapter recovery telemetry hooks", () => {
  ...
  test("recovery telemetry hooks throw when adapter not initialized", async () => {
    ...
    expect(() => adapter.emitNodeRecoveryStarted({ nodeId: "node-a" })).toThrow(
      HsmAdapterError,
    );
    expect(() => adapter.emitNodeRecoverySynced({ nodeId: "node-a" })).toThrow(
      HsmAdapterError,
    );
  });
