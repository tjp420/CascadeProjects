...
const {
  PqcGenomicPrivacyComplianceGatingHub,
} = require("../pqc-genomic-privacy-compliance-gating-hub.cjs");
const {
  ZkGenomicClaimValidator,
} = require("../zk-genomic-claim-validator.cjs");
...
function baseInitRequest() {
  return {
    ...
    blindedDnaSequenceAccessCommitment: "pedersen-dna-access-001",
    blindedConsentProbabilityCommitment: "pedersen-consent-prob-001",
    blindedGenomicPrivacyAuthorityIdentityCommitment:
      "pedersen-genomic-auth-001",
    consentWindowSeconds: 15552000,
    complianceChainDepth: 12,
    pqcSignatureScheme: "ML-DSA-65",
    genomicPrivacyAuthorityInitializerAttestation: mockAttestation(),
...
function baseClaimRequest(poolId) {
  return {
    ...
    blindedGenomicPrivacyAuthorityIdentityCommitment:
      "pedersen-genomic-auth-001",
    zkGenomicRangeProofHash: "zk-genomic-proof-001",
    symmetricPredicateProofDigest: "symmetric-predicate-digest-001",
    genomicEthicsOversightCommitteeAttestation: mockAttestation(),
    attestationAuthority: "mock-authority",
  };
}
...
function setupHubAndValidator() {
  ...
  const hub = new PqcGenomicPrivacyComplianceGatingHub({
    policy: POLICY,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
  });
  const validator = new ZkGenomicClaimValidator({
    policy: POLICY,
    hub,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyGenomicClaim(
...
describe("Track 99 PQ genomic privacy compliance gating", () => {
  test("initializes a pool and emits GENOMIC_COMPLIANCE_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "GENOMIC_COMPLIANCE_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("verifies a genomic claim and emits ZK_GENOMIC_CLAIM_VERIFIED", () => {
    ...
    expect(claim.symmetricPredicateProofDigest).toBe(
      "symmetric-predicate-digest-001",
    );
    expect(events.some((e) => e.event === "ZK_GENOMIC_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits CONSENT_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "CONSENT_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects consent window exceeding maximum", () => {
    ...
    req.consentWindowSeconds = 99999999;
  ...
  test("rejects un-attested genomic privacy authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.genomicPrivacyAuthorityInitializerAttestation = { authority: "bad" };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested genomic ethics oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkGenomicClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.genomicEthicsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkGenomicRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkGenomicRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing symmetricPredicateProofDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.symmetricPredicateProofDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyGenomicClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq genomic privacy compliance gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
        genomicQuorum: 6,
        consentWindowSeconds: 15552000,
        complianceChainDepth: 12,
        pqcSignatureScheme: "ML-DSA-65",
        genomicPrivacyAuthorityInitializerAttestation: true,
        genomicEthicsOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderGenomicClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
        consentWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
        complianceChainDepth: 30,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
        genomicPrivacyAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
        genomicEthicsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
        banMalformedOrOutOfOrderGenomicClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqGenomicPrivacyComplianceGating", {
