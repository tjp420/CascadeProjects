...
describe("Track 114 multi-tenant fuzzing matrix", () => {
  test("FUZZ-114-01: prototype pollution in latticeVssGating is blocked", () => {
    ...
    expect(resolved.latticeVssGating.requireEnclaveBindingAttestation).toBe(
      true,
    );
  ...
  test("FUZZ-114-05: strict reference sandboxing — mutation does not cross-tenant leak", () => {
    const policy = makeTrack114PrngDrivenMultiLayerPolicy(
    ...
    const originalB = b.latticeVssGating
      ? b.latticeVssGating.maxDegreeBound
      : undefined;
  ...
  test("FUZZ-114-07: PRNG-driven lattice VSS validation — 100 calls, no unhandled crash", () => {
    ...
    for (let i = 0; i < 100; i++) {
      const { tenantId, operation, config } =
        makeTrack114PrngDrivenValidateCall(prng);
      try {
  ...
  test("FUZZ-114-08: concurrent validation flood does not race or crash", async () => {
    ...
    for (let i = 0; i < 1000; i++) {
      const { tenantId, operation, config } =
        makeTrack114ConcurrentValidationCall(prng);
  ...
  test("FUZZ-114-09: cross-tenant latticeVssGating mutation isolation", () => {
    ...
    const originalBMax =
      engine.getPolicy("track114-clean").latticeVssGating.maxDegreeBound;
