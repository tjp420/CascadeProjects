...
const {
  PqcQuantumSensorCalibrationGatingHub,
} = require("../pqc-quantum-sensor-calibration-gating-hub.cjs");
const {
  ZkQuantumClaimValidator,
} = require("../zk-quantum-claim-validator.cjs");
...
function baseInitRequest() {
  return {
    ...
    blindedQuantumMeasurementCommitment: "pedersen-quantum-measurement-001",
    blindedCalibrationProbabilityCommitment: "pedersen-calibration-prob-001",
    blindedQuantumMetrologyAuthorityIdentityCommitment:
      "pedersen-quantum-auth-001",
    calibrationWindowSeconds: 3888000,
    calibrationChainDepth: 14,
    pqcSignatureScheme: "ML-DSA-65",
    quantumMetrologyAuthorityInitializerAttestation: mockAttestation(),
...
function baseClaimRequest(poolId) {
  return {
    ...
    blindedQuantumMetrologyAuthorityIdentityCommitment:
      "pedersen-quantum-auth-001",
    zkQuantumRangeProofHash: "zk-quantum-proof-001",
    accumulationTreeDigest: "accumulation-tree-digest-001",
    quantumStandardsOversightCommitteeAttestation: mockAttestation(),
    attestationAuthority: "mock-authority",
  };
}
...
function setupHubAndValidator() {
  ...
  const hub = new PqcQuantumSensorCalibrationGatingHub({
    policy: POLICY,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
  });
  const validator = new ZkQuantumClaimValidator({
    policy: POLICY,
    hub,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyQuantumClaim(
...
describe("Track 100 PQ quantum sensor calibration gating", () => {
  test("initializes a pool and emits QUANTUM_CALIBRATION_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "QUANTUM_CALIBRATION_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("verifies a quantum claim and emits ZK_QUANTUM_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_QUANTUM_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits CALIBRATION_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "CALIBRATION_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects calibration window exceeding maximum", () => {
    ...
    req.calibrationWindowSeconds = 99999999;
  ...
  test("rejects un-attested quantum metrology authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.quantumMetrologyAuthorityInitializerAttestation = { authority: "bad" };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested quantum standards oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkQuantumClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.quantumStandardsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkQuantumRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkQuantumRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing accumulationTreeDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.accumulationTreeDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyQuantumClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq quantum sensor calibration gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
        quantumQuorum: 7,
        calibrationWindowSeconds: 3888000,
        calibrationChainDepth: 14,
        pqcSignatureScheme: "ML-DSA-65",
        quantumMetrologyAuthorityInitializerAttestation: true,
        quantumStandardsOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderQuantumClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
        calibrationWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
        calibrationChainDepth: 30,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
        quantumMetrologyAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
        quantumStandardsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
        banMalformedOrOutOfOrderQuantumClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumSensorCalibrationGating", {
