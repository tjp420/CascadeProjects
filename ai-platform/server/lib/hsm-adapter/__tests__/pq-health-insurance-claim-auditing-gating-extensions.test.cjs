...
/**
 * Track 86: PQC Health Insurance Claim Auditing Gating & ZK Insurance
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, billing sequence depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch claim audit verification, slashing
...
const {
  ...
  POOL_STATUS,
  REBALANCE_DIRECTION,
} = require('../pqc-health-insurance-claim-auditing-gating-hub.cjs');
const {
  ZkInsuranceClaimValidator,
  CLAIM_STATUS,
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-insurance-claim-validator.cjs');
...
describe('Track 86 PQC Health Insurance Claim Auditing Gating extensions', () => {
  describe('PqcHealthInsuranceClaimAuditingGatingHub — billing sequence depth rebalancing', () => {
    test('rebalances billing sequence depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceBillingSequenceDepth({
    ...
    test('rebalances billing sequence depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceBillingSequenceDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates billingSequenceDepth on rebalance when newBillingSequenceDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceBillingSequenceDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newBillingSequenceDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.billingSequenceDepth).toBe(18);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceBillingSequenceDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceBillingSequenceDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcHealthInsuranceClaimAuditingGatingHub — batch initialization', () => {
  ...
  describe('PqcHealthInsuranceClaimAuditingGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcHealthInsuranceClaimAuditingGatingHub — committee aggregation', () => {
  ...
  describe('PqcHealthInsuranceClaimAuditingGatingHub — cancellation', () => {
  ...
  describe('PqcHealthInsuranceClaimAuditingGatingHub — queries and stats', () => {
  ...
  describe('ZkInsuranceClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        actuarialRiskCode: 500,
        claimValue: 450,
      });
      expect(proof.zkInsuranceRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ actuarialRiskCode: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        actuarialRiskCode: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkInsuranceClaimValidator — batch claim audit verification', () => {
    test('batch verifies multiple claim audits', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyClaimAudits(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyClaimAudits([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyClaimAudits(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyClaimAudits([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkInsuranceClaimValidator — partial signature aggregation', () => {
    test('aggregates partial signatures', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateTFheProofs(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
      ]);
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkInsuranceRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyClaimAudit(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateTFheProofs(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateTFheProofs('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkInsuranceClaimValidator — slashing window validation', () => {
  ...
  describe('ZkInsuranceClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.claimWindowSeconds = 999999999;
      try { ctx.validator.verifyClaimAudit(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyClaimAudit(cReq);
      try { ctx.validator.verifyClaimAudit(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.CLAIM_WINDOW_OUT_OF_BOUNDS).toBe('claim_window_out_of_bounds');
  ...
  describe('full Track 86 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkInsuranceRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkInsuranceRangeProofHash = snarkProof.zkInsuranceRangeProofHash;
      const claim = ctx.validator.verifyClaimAudit(cReq);
