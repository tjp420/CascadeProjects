...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyTrainingClaim(
...
describe("Track 82 PQ AI model training gating", () => {
  test("PqcAiModelTrainingGatingHub initializes a pool and emits TRAINING_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "TRAINING_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkTrainingClaimValidator verifies a training claim and emits ZK_TRAINING_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_TRAINING_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcAiModelTrainingGatingHub completes accreditation after claim and emits MODEL_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "MODEL_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq training gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTrainingGating", { trainingOversightQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
        trainingWindowSeconds: 999999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTrainingGating", { provenanceDepth: 128 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
        trainingAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
        modelAuditCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
        banMalformedOrOutOfOrderTrainingClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqTrainingGating", {
