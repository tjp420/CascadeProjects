...
describe("Track 118 Prometheus alert rule compliance", () => {
  ...
  test("ALERT-118-L2-03: Track118ViewChangeAbortSpike — correct structure", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118ViewChangeAbortSpike",
    );
    ...
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_consensus_coord_view_change_aborted_total\[5m\]\s*\)/,
    );
  ...
  test("ALERT-118-L2-04: Track118ViewChangeCompletionStall — correct structure", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118ViewChangeCompletionStall",
    );
    expect(rule).toBeDefined();
    expect(rule.labels.severity).toBe("critical");
    expect(rule.for).toBe("5m");
    expect(rule.expr).toContain(
      "hsm_consensus_coord_view_change_started_total[10m]",
    );
    expect(rule.expr).toContain(
      "hsm_consensus_coord_view_change_completed_total[10m]",
    );
  ...
  test("ALERT-118-L2-05: Track118FaultDetectionSpike — correct structure", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118FaultDetectionSpike",
    );
    expect(rule).toBeDefined();
    expect(rule.labels.severity).toBe("critical");
    ...
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_consensus_coord_faults_detected_total\[5m\]\s*\)/,
    );
  ...
  test("ALERT-118-L2-06: Track118ProposalRejectionRateHigh — correct structure with + 1 denominator offset", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118ProposalRejectionRateHigh",
    );
    expect(rule).toBeDefined();
    ...
    expect(rule.expr).toMatch(
      /hsm_consensus_coord_proposals_routed_total\[5m\]/,
    );
    expect(rule.expr).toMatch(
      /hsm_consensus_coord_proposals_rejected_total\[5m\]/,
    );
  });
  ...
  test("ALERT-118-L2-07: Track118QuorumDenialSpike — correct structure", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118QuorumDenialSpike",
    );
    expect(rule).toBeDefined();
    expect(rule.labels.severity).toBe("warning");
    expect(rule.for).toBe("2m");
    ...
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_consensus_coord_quorum_denied_total\[5m\]\s*\)/,
    );
  ...
  test("ALERT-118-L2-08: Track118GroupChurnImbalance — correct structure", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118GroupChurnImbalance",
    );
    expect(rule).toBeDefined();
    expect(rule.labels.severity).toBe("warning");
    expect(rule.for).toBe("10m");
    expect(rule.expr).toContain(
      "hsm_consensus_coord_groups_destroyed_total[15m]",
    );
    expect(rule.expr).toContain(
      "hsm_consensus_coord_groups_created_total[15m]",
    );
  });
  ...
  test("ALERT-118-L2-09: Track118QuorumVerificationStall — correct structure", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track118QuorumVerificationStall",
    );
    expect(rule).toBeDefined();
    expect(rule.labels.severity).toBe("critical");
    expect(rule.for).toBe("5m");
    expect(rule.expr).toContain(
      "hsm_consensus_coord_proposals_routed_total[10m]",
    );
    expect(rule.expr).toContain(
      "hsm_consensus_coord_quorum_verified_total[10m]",
    );
    expect(rule.expr).toContain("== 0");
  });
  ...
  test("ALERT-118-L2-10: all rules have correct labels (track, component, tier, service)", () => {
    for (const r of group.rules) {
      ...
      expect(r.labels.service).toBe(
        "hsm-vault-distributed-consensus-coordinator",
      );
    }
  });
  ...
  test("ALERT-118-L3-03: no secrets in Track 118 alert YAML section", () => {
    ...
    const track118Start = yamlText.indexOf(
      "track118_distributed_consensus_coordinator_alerts",
    );
  ...
  test("ALERT-118-L3-06: existing Track 115/116/117 alert groups unchanged", () => {
    const track115Group = findGroup(
      doc,
      "track115_lattice_vfhss_gating_alerts",
    );
    const track116Group = findGroup(
      doc,
      "track116_cluster_isolation_hardening_alerts",
    );
