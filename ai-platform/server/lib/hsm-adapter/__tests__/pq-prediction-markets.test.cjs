...
describe("Track 64 PQ prediction markets", () => {
  test("PqcPredictionMarketHub initializes a market and emits PREDICTION_MARKET_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "PREDICTION_MARKET_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkMarketResolutionValidator records a vote and emits ZK_RESOLUTION_VOTE_RECORDED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_RESOLUTION_VOTE_RECORDED")).toBe(
      true,
    );
  });
  ...
  test("PqcPredictionMarketHub finalizes a market after quorum and emits PREDICTION_MARKET_FINALIZED", () => {
    ...
    const finalization = hub.finalizeMarket(
      baseFinalizeRequest(market.marketId),
    );
    expect(finalization.finalId).toBeDefined();
    expect(events.some((e) => e.event === "PREDICTION_MARKET_FINALIZED")).toBe(
      true,
    );
  });
  ...
  test("ZkMarketResolutionValidator rejects un-attested reporter relay", () => {
    ...
    expect(() => validator.recordResolutionVote(voteReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("PqcPredictionMarketHub rejects finalization before quorum", () => {
    const { hub, market } = setupAndInitMarket();
    expect(() =>
      hub.finalizeMarket(baseFinalizeRequest(market.marketId)),
  ...
  test("CryptoPolicyEngine validates pq prediction markets configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
        reporterQuorum: 3,
    ...
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", { reporterQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
        disputeResolutionEpochs: 10,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
    ...
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", { assetWeightCap: 2000000 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
    ...
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
        marketInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
        reporterRelayAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
    ...
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
        banMalformedOrOutOfOrderResolutionClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPredictionMarkets", {
