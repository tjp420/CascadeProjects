...
describe("CryptoPolicyEngine", () => {
  test("default policy allows AES 256-bit KEK creation", () => {
    ...
    expect(
      engine.validate("t1", "createKEK", { algorithm: "aes-kw", kekBits: 256 }),
    ).toBe(true);
  });
  ...
  test("default policy allows ECDH P-256 key creation", () => {
    const engine = new CryptoPolicyEngine();
    expect(
      engine.validate("t1", "createKEK", { algorithm: "ecdh", keySize: 256 }),
    ).toBe(true);
  });
  ...
  test("blocks disallowed AES bit length", () => {
    ...
    expect(() =>
      engine.validate("t1", "createKEK", { algorithm: "aes-kw", kekBits: 128 }),
    ).toThrow(HsmAdapterError);
  });
  ...
  test("tenant-specific policy overrides default", () => {
    ...
    expect(
      engine.validate("tenant-a", "createKEK", {
        algorithm: "aes-kw",
        kekBits: 128,
      }),
    ).toBe(true);
    expect(() =>
      engine.validate("tenant-b", "createKEK", {
        algorithm: "aes-kw",
        kekBits: 128,
      }),
    ).toThrow(HsmAdapterError);
  ...
  test("unknown tenant falls back to default", () => {
    ...
    expect(
      engine.validate("unknown", "createKEK", {
        algorithm: "aes-kw",
        kekBits: 256,
      }),
    ).toBe(true);
    ...
    const engine = new CryptoPolicyEngine({
      default: {
        deprecatedAlgorithms: [
          { algorithm: "rsa-oaep", reason: "migrating to PQC" },
        ],
      },
    });
    expect(() =>
  ...
  test("blocks disabled ECDH curve", () => {
    ...
    expect(() =>
      engine.validate("t1", "createKEK", { algorithm: "ecdh", keySize: 384 }),
    ).toThrow(HsmAdapterError);
  });
