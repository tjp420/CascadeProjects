...
const {
  PqcAutonomousDroneSwarmMeshRoutingGatingHub,
} = require("../pqc-autonomous-drone-swarm-mesh-routing-gating-hub.cjs");
...
function makeHub() {
  return new PqcAutonomousDroneSwarmMeshRoutingGatingHub({
...
describe("Track 113 PQC Autonomous Drone Swarm Mesh-Routing Gating Hub", () => {
  ...
  test("initializes a drone swarm mesh-routing pool and increments counter", () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_dronegate_pool_initialized_total).toBe(
      1,
    );
  });
  ...
  test("verifies swarm routing and increments counter", () => {
    ...
    const verified = hub.verifySwarmRouting({
      poolId: pool.poolId,
      proofValid: true,
    });
  ...
  test("completes topology accreditation with sufficient quorum", () => {
    ...
    expect(
      hsmMetrics.getMetrics().hsm_topology_accreditation_completed_total,
    ).toBe(1);
  });
  ...
  test("rejects topology accreditation with insufficient quorum", () => {
    ...
    const code = expectErrorCode(
      () =>
        hub.completeTopologyAccreditation({
          poolId: pool.poolId,
