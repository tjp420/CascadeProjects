...
describe("EncryptedP2PRoutingEngine — Track 38 Encrypted P2P Routing", () => {
  ...
  describe("L2.03: OnionEncryption", () => {
    ...
    test("decryptLayer peels one layer at relay node", () => {
      ...
      const result = OnionEncryption.decryptLayer(
        bundle,
        "node-b",
        KEYS["node-b"],
      );
    ...
    test("decryptLayer fails with wrong key", () => {
      ...
      expect(() =>
        OnionEncryption.decryptLayer(bundle, "node-b", wrongKey),
      ).toThrow(HsmAdapterError);
    });

    test("encrypt throws for missing peer key", () => {
      const route = ["node-a", "node-b"];
      expect(() => OnionEncryption.encrypt("secret", route, new Map())).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("encrypt throws for route with < 2 nodes", () => {
      expect(() =>
        OnionEncryption.encrypt("secret", ["node-a"], new Map()),
      ).toThrow(HsmAdapterError);
    });
  });
  ...
  describe("L2.05: state machine transitions", () => {
    test("cannot establish without discovery", () => {
      ...
      expect(() => engine.encryptMessage(d.routeId, "msg")).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("cannot relay without encrypting", () => {
      ...
      expect(() => engine.relayMessage({ routeId: d.routeId })).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("L3.01: anti-replay protection", () => {
    test("replayed nonce rejected", () => {
      ...
      expect(() => engine.relayMessage(replayedBundle)).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("L3.02: tampered message rejected", () => {
    test("tampered onion layer fails decryption", () => {
      ...
      const tamperedBundle = {
        ...bundle,
        layers: [{ ...bundle.layers[0], encrypted: "tampered" }],
      };
      expect(() =>
        OnionEncryption.decryptLayer(tamperedBundle, "node-b", KEYS["node-b"]),
      ).toThrow(HsmAdapterError);
    });
  });
  ...
  describe("L3.03: route revocation", () => {
    ...
    test("blacklisted peer excluded from routes", () => {
      ...
      expect(engine.discoverRoute("node-c").route).toEqual([
        "node-a",
        "node-b",
        "node-c",
      ]);
  ...
  describe("error cases", () => {
    ...
    test("decryptAtRelay throws for unknown peer key", () => {
      const engine = new EncryptedP2PRoutingEngine({ localNodeId: "node-a" });
      const fakeBundle = {
        layers: [{ iv: "00", encrypted: "00", authTag: "00" }],
      };
      expect(() => engine.decryptAtRelay(fakeBundle, "unknown")).toThrow(
        HsmAdapterError,
      );
    });
  });
