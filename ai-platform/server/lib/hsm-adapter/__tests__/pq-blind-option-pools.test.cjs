...
const { ZkMarginAdequacyProcessor } = require('../zk-margin-adequacy-processor.cjs');
...
function setupInitAndMargin() {
  ...
  const proof = ctx.processor.verifyMarginAdequacy(baseMarginProofRequest(ctx.pool.poolId));
  return { ...ctx, proof };
}
...
describe('Track 63 PQ blind option pools', () => {
  test('PqcBlindOptionPoolHub initializes a pool and emits BLIND_OPTION_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'BLIND_OPTION_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkMarginAdequacyProcessor verifies margin adequacy and emits ZK_MARGIN_ADEQUACY_VERIFIED', () => {
    ...
    const proof = processor.verifyMarginAdequacy(baseMarginProofRequest(pool.poolId));
    expect(proof.proofId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_MARGIN_ADEQUACY_VERIFIED')).toBe(true);
  });
  ...
  test('PqcBlindOptionPoolHub executes a cleared contract and emits BLIND_OPTION_CONTRACT_EXECUTED', () => {
    ...
    expect(events.some((e) => e.event === 'BLIND_OPTION_CONTRACT_EXECUTED')).toBe(true);
  });
  ...
  test('PqcBlindOptionPoolHub rejects un-attested initializer', () => {
    ...
    const hub = new PqcBlindOptionPoolHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkMarginAdequacyProcessor rejects un-attested clearing committee', () => {
    ...
    const processor = new ZkMarginAdequacyProcessor({ policy: POLICY, hub, attestationClient });
    const proofReq = baseMarginProofRequest(pool.poolId);
    proofReq.clearingCommitteeAttestation = { authority: 'bad' };
    expect(() => processor.verifyMarginAdequacy(proofReq)).toThrow(HsmAdapterError);
  });
  ...
  test('PqcBlindOptionPoolHub rejects execution before margin verification', () => {
    const { hub, pool } = setupAndInitPool();
    expect(() => hub.executeContract(baseExecRequest(pool.poolId))).toThrow(HsmAdapterError);
  });
  ...
  test('ZkMarginAdequacyProcessor bans peers broadcasting duplicate proofs', () => {
    ...
    expect(() => processor.verifyMarginAdequacy(dupReq)).toThrow(HsmAdapterError);
    expect(processor.isPeerBanned('peer-bad')).toBe(true);
  });

  test('CryptoPolicyEngine validates pq blind option pools configuration', () => {
    const engine = new CryptoPolicyEngine({ default: {} });
    expect(() => engine.validate('t1', 'pqBlindOptionPools', {
      collateralRatio: 200,
    ...
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { collateralRatio: 100 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { executionSignatureQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { contractLifetimeSeconds: 31536000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { initializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { clearingCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { banMalformedOrSubCollateralProofs: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBlindOptionPools', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
