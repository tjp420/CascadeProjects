...
/**
 * Track 79: PQC Clinical Trial Verification Gating & ZK Trial
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, cohort metric depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch trial claim verification, slashing
...
const {
  ...
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-trial-claim-validator.cjs');
...
describe('Track 79 PQC Clinical Trial Verification Gating extensions', () => {
  describe('PqcClinicalTrialVerificationGatingHub — cohort metric depth rebalancing', () => {
    test('rebalances cohort metric depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceCohortMetricDepth({
    ...
    test('rebalances cohort metric depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceCohortMetricDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates cohortMetricDepth on rebalance when newCohortMetricDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceCohortMetricDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newCohortMetricDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.cohortMetricDepth).toBe(18);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceCohortMetricDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceCohortMetricDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcClinicalTrialVerificationGatingHub — batch initialization', () => {
  ...
  describe('PqcClinicalTrialVerificationGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcClinicalTrialVerificationGatingHub — committee aggregation', () => {
  ...
  describe('PqcClinicalTrialVerificationGatingHub — cancellation', () => {
  ...
  describe('PqcClinicalTrialVerificationGatingHub — queries and stats', () => {
  ...
  describe('ZkTrialClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      ...
      expect(proof.zkTrialRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ cohortMetric: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
  ...
  describe('ZkTrialClaimValidator — batch trial claim verification', () => {
    test('batch verifies multiple trial claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyTrialClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyTrialClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyTrialClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyTrialClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkTrialClaimValidator — partial signature aggregation', () => {
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkTrialRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyTrialClaim(cReq); } catch (e) { /* expected */ }
  ...
  describe('ZkTrialClaimValidator — slashing window validation', () => {
  ...
  describe('ZkTrialClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.trialDurationSeconds = 999999999;
      try { ctx.validator.verifyTrialClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyTrialClaim(cReq);
      try { ctx.validator.verifyTrialClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.TRIAL_DURATION_OUT_OF_BOUNDS).toBe('trial_duration_out_of_bounds');
  ...
  describe('full Track 79 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkTrialRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkTrialRangeProofHash = snarkProof.zkTrialRangeProofHash;
      const claim = ctx.validator.verifyTrialClaim(cReq);
