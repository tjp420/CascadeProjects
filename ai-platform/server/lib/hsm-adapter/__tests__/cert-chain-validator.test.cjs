...
const { generateChain, generateExpired } = require('./__fixtures__/attestation-certs.cjs');
...
describe('Certificate Chain Validator', () => {
  ...
  test('CERT-09: validateSevSnpChain validates ARK -> ASK -> VCEK chain', () => {
    const v = new CertChainValidator();
    const result = v.validateSevSnpChain(chain.leafCert, chain.interCert, chain.rootCert);
    expect(result.valid).toBe(true);
    expect(result.chain.length).toBe(3);
  });
  ...
  test('CERT-10: validateSgxChain validates Root CA -> PCK CA -> PCK chain', () => {
    const v = new CertChainValidator();
    const result = v.validateSgxChain(chain.leafCert, chain.interCert, chain.rootCert);
    expect(result.valid).toBe(true);
    expect(result.chain.length).toBe(3);
  });
