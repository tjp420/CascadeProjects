...
const {
  PqcZkDecentralizedStorageAttestationGatingHub,
} = require("../pqc-zk-decentralized-storage-gating-hub.cjs");
const {
  ZkStorageClaimValidator,
} = require("../zk-storage-claim-validator.cjs");
...
function buildHub(policy = DEFAULT_POLICY, attestationClient) {
  return new PqcZkDecentralizedStorageAttestationGatingHub({
    policy,
    attestationClient,
    audit: jest.fn(),
  });
...
describe("PqcZkDecentralizedStorageAttestationGatingHub", () => {
  ...
  test("initializePool creates a pool with correct defaults", () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_zkstorage_pool_initialized_total).toBe(
      1,
    );
  });
  ...
  test("verifyStorageProof increments counter on valid proof", () => {
    ...
    const result = hub.verifyStorageProof({
      poolId: pool.poolId,
      proofValid: true,
    });
  ...
  test("verifyStorageProof with invalid proof issues challenge and throws", () => {
    ...
    expect(() =>
      hub.verifyStorageProof({ poolId: pool.poolId, proofValid: false }),
    ).toThrow("storage proof");
    expect(hsmMetrics.getMetrics().hsm_zkstorage_challenge_issued_total).toBe(
      1,
    );
  });
  ...
  test("confirmReplicaConsensus requires proof first", () => {
    ...
    expect(() =>
      hub.confirmReplicaConsensus({
        poolId: pool.poolId,
        replicaSignatures: ["s1", "s2", "s3"],
        storageEthicsOversightCommitteeAttestation: "attest-2",
      }),
    ).toThrow("storage proof not verified");
  });
  ...
  test("completeDispersal increments counter after consensus", () => {
    ...
    expect(
      hsmMetrics.getMetrics().hsm_zkstorage_dispersal_completed_total,
    ).toBe(1);
