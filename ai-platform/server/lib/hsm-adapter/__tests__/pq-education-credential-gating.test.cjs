 ...
 */
const { PqcEducationCredentialGatingHub } = require('../pqc-education-credential-gating-hub.cjs');
const { ZkAcademicCredentialValidator } = require('../zk-academic-credential-validator.cjs');
...
describe('Track 73 PQ education credential gating', () => {
  test('PqcEducationCredentialGatingHub initializes a pool and emits EDUCATION_GATING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'EDUCATION_GATING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkAcademicCredentialValidator verifies an academic claim and emits ZK_ACADEMIC_CLAIM_VERIFIED', () => {
    ...
    expect(events.some((e) => e.event === 'ZK_ACADEMIC_CLAIM_VERIFIED')).toBe(true);
  });
  ...
  test('PqcEducationCredentialGatingHub completes accreditation after claim and emits CREDENTIAL_ACCREDITATION_COMPLETED', () => {
    ...
    expect(events.some((e) => e.event === 'CREDENTIAL_ACCREDITATION_COMPLETED')).toBe(true);
  });
  ...
  test('PqcEducationCredentialGatingHub rejects un-attested institution initializer', () => {
    ...
    const hub = new PqcEducationCredentialGatingHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkAcademicCredentialValidator rejects un-attested clearing committee', () => {
    ...
    const validator = new ZkAcademicCredentialValidator({ policy: POLICY, hub, attestationClient });
  ...
  test('CryptoPolicyEngine validates pq education gating configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqEducationGating', {
    ...
    expect(() => engine.validate('t1', 'pqEducationGating', { accreditationQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { transcriptExpirationSeconds: 99999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { academicCredentialDepth: 48 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { institutionInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { clearingCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { banMalformedOrOutOfOrderCredentialClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqEducationGating', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
