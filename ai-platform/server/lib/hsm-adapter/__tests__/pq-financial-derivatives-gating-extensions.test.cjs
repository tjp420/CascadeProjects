...
/**
 * Track 78: PQC Financial Derivatives Gating & ZK Derivative
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, risk metric depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch derivative claim verification, slashing
...
function baseInitRequest() {
  return {
    ...
    blindedContractTermsCommitment: 'pedersen-contract-001',
...
describe('Track 78 PQC Financial Derivatives Gating extensions', () => {
  describe('PqcFinancialDerivativesGatingHub — risk metric depth rebalancing', () => {
    test('rebalances risk metric depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceRiskMetricDepth({
    ...
    test('rebalances risk metric depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceRiskMetricDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates riskMetricDepth on rebalance when newRiskMetricDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceRiskMetricDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newRiskMetricDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.riskMetricDepth).toBe(18);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceRiskMetricDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceRiskMetricDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcFinancialDerivativesGatingHub — batch initialization', () => {
  ...
  describe('PqcFinancialDerivativesGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcFinancialDerivativesGatingHub — committee aggregation', () => {
  ...
  describe('PqcFinancialDerivativesGatingHub — cancellation', () => {
  ...
  describe('PqcFinancialDerivativesGatingHub — queries and stats', () => {
  ...
  describe('ZkDerivativeClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        counterpartyRiskMetric: 500,
        claimValue: 450,
      });
      expect(proof.zkDerivativeRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ counterpartyRiskMetric: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        counterpartyRiskMetric: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkDerivativeClaimValidator — batch derivative claim verification', () => {
    test('batch verifies multiple derivative claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyDerivativeClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyDerivativeClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyDerivativeClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyDerivativeClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkDerivativeClaimValidator — partial signature aggregation', () => {
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkDerivativeRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyDerivativeClaim(cReq); } catch (e) { /* expected */ }
  ...
  describe('ZkDerivativeClaimValidator — slashing window validation', () => {
  ...
  describe('ZkDerivativeClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.contractExpirationSeconds = 999999999;
      try { ctx.validator.verifyDerivativeClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyDerivativeClaim(cReq);
      try { ctx.validator.verifyDerivativeClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.CONTRACT_EXPIRATION_OUT_OF_BOUNDS).toBe('contract_expiration_out_of_bounds');
  ...
  describe('full Track 78 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkDerivativeRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkDerivativeRangeProofHash = snarkProof.zkDerivativeRangeProofHash;
      const claim = ctx.validator.verifyDerivativeClaim(cReq);
