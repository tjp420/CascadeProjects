 ...
 */
const { EnclaveSecretSealingPolicy, DEFAULT_SEALING_POLICY } = require('../enclave-secret-sealing-policy.cjs');
...
describe('Track 42: Enclave Secret-Sealing Policy', () => {
  ...
  describe('validateSeal', () => {
    test('accepts a valid seal config with allowed cipher and key size', () => {
      const result = policy.validateSeal({ cipher: 'aes-256-gcm', keyBits: 256 });
      expect(result.valid).toBe(true);
      expect(result.cipher).toBe('aes-256-gcm');
    });

    test('rejects an unsupported sealing cipher', () => {
      expect(() => policy.validateSeal({ cipher: 'rc4', keyBits: 256 })).toThrow(HsmAdapterError);
    });

    test('rejects a key size below minimum', () => {
      expect(() => policy.validateSeal({ cipher: 'aes-256-gcm', keyBits: 64 })).toThrow(HsmAdapterError);
    });

    test('rejects data exceeding max size', () => {
      expect(() => policy.validateSeal({
        cipher: 'aes-256-gcm',
        keyBits: 256,
  ...
  describe('validateUnseal', () => {
    ...
    test('rejects unseal outside enclave when not allowed', () => {
      expect(() => policy.validateUnseal({ insideEnclave: false })).toThrow(HsmAdapterError);
    });
  ...
  describe('validateAttestation', () => {
    ...
    test('rejects attestation without nonce when challenge-response required', () => {
      expect(() => policy.validateAttestation({
        timestamp: Math.floor(Date.now() / 1000),
        attestationAgeSeconds: 0,
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects replayed nonce', () => {
      ...
      expect(() => policy.validateAttestation(attestation)).toThrow(HsmAdapterError);
    });
  ...
  describe('validateKeyProvisioning', () => {
    ...
    test('rejects provisioning without attestation when required', () => {
      expect(() => policy.validateKeyProvisioning({
        keyType: 'kek',
    ...
    test('rejects when max provisioned keys reached', () => {
      ...
      smallPolicy.validateKeyProvisioning({ keyType: 'kek', attestationVerified: true });
      smallPolicy.validateKeyProvisioning({ keyType: 'kek', attestationVerified: true });
      expect(() => smallPolicy.validateKeyProvisioning({ keyType: 'kek', attestationVerified: true })).toThrow(HsmAdapterError);
    });

    test('rejects missing config', () => {
      expect(() => policy.validateKeyProvisioning(null)).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('sealed key tracking', () => {
    test('recordSealedKey and removeSealedKey track count', () => {
      ...
      policy.recordSealedKey('key-1', { keyType: 'kek', cipher: 'aes-256-gcm' });
      expect(policy.sealedKeyCount).toBe(1);
      policy.recordSealedKey('key-2', { keyType: 'wrap-key', cipher: 'aes-256-gcm' });
    ...
    test('provisionedKeyCount increments on successful provisioning', () => {
      ...
      policy.validateKeyProvisioning({ keyType: 'kek', attestationVerified: true });
  ...
  describe('policy overrides', () => {
    ...
    test('allows custom sealing ciphers', () => {
      ...
      const result = custom.validateSeal({ cipher: 'chacha20-poly1305', keyBits: 256 });
      expect(result.valid).toBe(true);
    });

    test('allows provisioning without attestation when policy permits', () => {
      const relaxed = new EnclaveSecretSealingPolicy({
        policy: { keyProvisioning: { requireAttestationBeforeProvision: false } },
      });
      const result = relaxed.validateKeyProvisioning({ keyType: 'kek', attestationVerified: false });
      expect(result.valid).toBe(true);
    });
  });
});
...
describe('Track 42: CryptoPolicyEngine secretSealing validation', () => {
  test('accepts a valid secret sealing config', () => {
    ...
    expect(() => engine.validate('t1', 'secretSealing', {
      cipher: 'aes-256-gcm',
      keyBits: 256,
