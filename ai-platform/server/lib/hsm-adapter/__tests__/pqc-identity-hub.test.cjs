...
describe("Track 51 PQC identity hub", () => {
  test("PqcIdentityHubRouter registers a new identity", () => {
    ...
    expect(events.some((e) => e.event === "PQC_IDENTITY_HUB_REGISTERED")).toBe(
      true,
    );
  });
  ...
  test("ThresholdIdentityIssuer reaches quorum and commits", () => {
    ...
    expect(
      events.some((e) => e.event === "IDENTITY_ISSUANCE_QUORUM_COMMITTED"),
    ).toBe(true);
  });
  ...
  test("ThresholdIdentityIssuer rejects un-attested committee member", () => {
    ...
    expect(() =>
      issuer.sign("entity-1", "c-1", { authority: "bad" }, "sig-1"),
  ...
  test("CryptoPolicyEngine validates PQC identity hub configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", {
        issuanceQuorum: 3,
        committeeSize: 5,
        kemAlgorithm: "ML-KEM-1024",
    ...
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { issuanceQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { committeeSize: 20 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { kemAlgorithm: "ML-KEM-512" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { hostAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { committeeAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { identityAgeSeconds: 100000 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", { banUnattestedPeers: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcIdentityHub", {
