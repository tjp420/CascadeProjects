...
describe("Track 34 consensus-gated keyring lifecycle", () => {
  test("createKEK succeeds when consensus engine is leader with quorum", async () => {
    ...
    const gatedCall = logger.info.mock.calls.find(
      (c) => c[0] === "CONSENSUS_GATED_COMMIT",
    );
  ...
  test("createKEK blocked when node is not leader", async () => {
    ...
    await expect(adapter.createKEK("tenant-1")).rejects.toThrow(
      HsmAdapterError,
    );
  ...
  test("rotateKEK gated by consensus when leader", async () => {
    ...
    const gatedCommits = logger.info.mock.calls.filter(
      (c) => c[0] === "CONSENSUS_GATED_COMMIT",
    );
  ...
  test("rotateKEK blocked when not leader", async () => {
    ...
    await expect(adapter.rotateKEK("tenant-1", kekId)).rejects.toThrow(
      "not leader",
    );
  ...
  test("importKeyring blocked when not leader", async () => {
    ...
    await expect(adapter.importKeyring(envelope, masterKek)).rejects.toThrow(
      "not leader",
    );
