...
describe('AsymmetricHsmAdapter', () => {
  ...
  describe('ECDH', () => {
    ...
    test('rejects context mismatch on unwrap', async () => {
      ...
      await expect(adapter.unwrap('t1', kekId, wrapped, 'context-b')).rejects.toMatchObject({
        name: 'HsmAdapterError',
        code: 'UNWRAP_FAILED',
      });
    });
  ...
  describe('attestation', () => {
    ...
    test('verifyAttestation fails for expired certificate', async () => {
      ...
      await expect(adapter.verifyAttestation('t1', kekId, cert)).rejects.toMatchObject({
        name: 'HsmAdapterError',
  ...
  describe('validation', () => {
    ...
    test('rejects unknown KEK', async () => {
      ...
      await adapter.initialize();
      await expect(adapter.wrap('t1', 'missing', Buffer.alloc(16))).rejects.toMatchObject({
        name: 'HsmAdapterError',
        code: 'UNKNOWN_KEK',
      });
    });
