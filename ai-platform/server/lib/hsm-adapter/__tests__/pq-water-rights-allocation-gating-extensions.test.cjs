...
/**
 * Track 88: PQC Water Rights Allocation Gating & ZK Water Rights
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, flow chain depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch water claim verification, slashing
...
const {
  ...
  POOL_STATUS,
  REBALANCE_DIRECTION,
} = require('../pqc-water-rights-allocation-gating-hub.cjs');
const {
  ZkWaterRightsClaimValidator,
  CLAIM_STATUS,
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-water-rights-claim-validator.cjs');
...
function baseInitRequest() {
  return {
    ...
    allocationWindowSeconds: 15552000,
    flowChainDepth: 12,
...
describe('Track 88 PQC Water Rights Allocation Gating extensions', () => {
  describe('PqcWaterRightsAllocationGatingHub — flow chain depth rebalancing', () => {
    test('rebalances flow chain depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceFlowChainDepth({
    ...
    test('rebalances flow chain depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceFlowChainDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates flowChainDepth on rebalance when newFlowChainDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceFlowChainDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newFlowChainDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.flowChainDepth).toBe(18);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceFlowChainDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceFlowChainDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcWaterRightsAllocationGatingHub — batch initialization', () => {
  ...
  describe('PqcWaterRightsAllocationGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcWaterRightsAllocationGatingHub — committee aggregation', () => {
  ...
  describe('PqcWaterRightsAllocationGatingHub — cancellation', () => {
  ...
  describe('PqcWaterRightsAllocationGatingHub — queries and stats', () => {
  ...
  describe('ZkWaterRightsClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      ...
      expect(proof.zkWaterRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ watershedFlow: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
  ...
  describe('ZkWaterRightsClaimValidator — batch water claim verification', () => {
    test('batch verifies multiple water claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyWaterClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyWaterClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyWaterClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyWaterClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkWaterRightsClaimValidator — partial signature aggregation', () => {
    test('aggregates MPC proofs', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateMpcProofs(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
        { peerId: 'p4', signature: 'sig-4' },
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkWaterRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyWaterClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateMpcProofs(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateMpcProofs('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkWaterRightsClaimValidator — slashing window validation', () => {
  ...
  describe('ZkWaterRightsClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.allocationWindowSeconds = 999999999;
      try { ctx.validator.verifyWaterClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyWaterClaim(cReq);
      try { ctx.validator.verifyWaterClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.ALLOCATION_WINDOW_OUT_OF_BOUNDS).toBe('allocation_window_out_of_bounds');
  ...
  describe('full Track 88 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkWaterRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkWaterRangeProofHash = snarkProof.zkWaterRangeProofHash;
      const claim = ctx.validator.verifyWaterClaim(cReq);
