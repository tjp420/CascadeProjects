...
/**
 * Track 87: PQC Space-Asset Telemetry Gating & ZK Orbital Slot
 ...
 * generation, batch telemetry claim verification, slashing
...
const {
  ...
  POOL_STATUS,
  REBALANCE_DIRECTION,
} = require('../pqc-space-asset-telemetry-gating-hub.cjs');
const {
  ZkOrbitalSlotClaimValidator,
  CLAIM_STATUS,
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-orbital-slot-claim-validator.cjs');
...
function baseInitRequest() {
  return {
    ...
    slotAllocationWindowSeconds: 15552000,
...
function baseClaimRequest(poolId) {
  return {
    ...
    attestationAuthority: 'mock-authority',
    thresholdSignature: 'partial-sig-001',
    slotAllocationWindowSeconds: 15552000,
  };
}
...
describe('Track 87 PQC Space-Asset Telemetry Gating extensions', () => {
  describe('PqcSpaceAssetTelemetryGatingHub — telemetry chain depth rebalancing', () => {
    ...
    test('updates telemetryChainDepth on rebalance when newTelemetryChainDepth provided', () => {
      ...
      ctx.hub.rebalanceTelemetryChainDepth({
        ...
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newTelemetryChainDepth: 15,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.telemetryChainDepth).toBe(15);
  ...
  describe('PqcSpaceAssetTelemetryGatingHub — batch initialization', () => {
  ...
  describe('PqcSpaceAssetTelemetryGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcSpaceAssetTelemetryGatingHub — committee aggregation', () => {
  ...
  describe('PqcSpaceAssetTelemetryGatingHub — cancellation', () => {
  ...
  describe('PqcSpaceAssetTelemetryGatingHub — queries and stats', () => {
  ...
  describe('ZkOrbitalSlotClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        slotAllocationMetric: 500,
        claimValue: 450,
      });
      expect(proof.zkOrbitalRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ slotAllocationMetric: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        slotAllocationMetric: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkOrbitalSlotClaimValidator — batch telemetry claim verification', () => {
    test('batch verifies multiple telemetry claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyTelemetryClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyTelemetryClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyTelemetryClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyTelemetryClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkOrbitalSlotClaimValidator — partial signature aggregation', () => {
    test('aggregates threshold signatures', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateThresholdSignatures(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
        { peerId: 'p4', signature: 'sig-4' },
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkOrbitalRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyTelemetryClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateThresholdSignatures(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateThresholdSignatures('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkOrbitalSlotClaimValidator — slashing window validation', () => {
  ...
  describe('ZkOrbitalSlotClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.slotAllocationWindowSeconds = 999999999;
      try { ctx.validator.verifyTelemetryClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyTelemetryClaim(cReq);
      try { ctx.validator.verifyTelemetryClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.SLOT_WINDOW_OUT_OF_BOUNDS).toBe('slot_window_out_of_bounds');
  ...
  describe('full Track 87 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkOrbitalRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkOrbitalRangeProofHash = snarkProof.zkOrbitalRangeProofHash;
      const claim = ctx.validator.verifyTelemetryClaim(cReq);
