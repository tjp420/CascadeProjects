...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyTelemetryClaim(
...
describe("Track 87 PQC Space-Asset Telemetry Gating extensions", () => {
  describe("PqcSpaceAssetTelemetryGatingHub — telemetry chain depth rebalancing", () => {
    ...
    test("rejects rebalance with missing poolId", () => {
      ...
      expect(() => hub.rebalanceTelemetryChainDepth({})).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("ZkOrbitalSlotClaimValidator — batch telemetry claim verification", () => {
    ...
    test("rejects empty batch", () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyTelemetryClaims([])).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects batch exceeding max size", () => {
      ...
      expect(() => validator.batchVerifyTelemetryClaims(bigBatch)).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("ZkOrbitalSlotClaimValidator — partial signature aggregation", () => {
    test("aggregates threshold signatures", () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateThresholdSignatures(
        ctx.pool.poolId,
        [
          { peerId: "p1", signature: "sig-1" },
          { peerId: "p2", signature: "sig-2" },
    ...
    test("rejects aggregation with banned peer", () => {
      ...
      expect(() =>
        ctx.validator.aggregateThresholdSignatures(ctx.pool.poolId, [
    ...
    test("rejects aggregation with missing poolId", () => {
      const { validator } = setupHubAndValidator();
      expect(() =>
        validator.aggregateThresholdSignatures("", [
          { peerId: "p1", signature: "s1" },
        ]),
      ).toThrow(HsmAdapterError);
    });
  ...
  describe("ZkOrbitalSlotClaimValidator — slashing and stats", () => {
    ...
    test("CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported", () => {
      ...
      expect(SLASH_REASON.SLOT_WINDOW_OUT_OF_BOUNDS).toBe(
        "slot_window_out_of_bounds",
      );
