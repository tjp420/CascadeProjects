...
describe("Track 71 PQ identity gating", () => {
  test("PqcIdentityGatingHub initializes a pool and emits IDENTITY_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "IDENTITY_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkIdentityGatingValidator verifies an attribute claim and emits ZK_ATTRIBUTE_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_ATTRIBUTE_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcIdentityGatingHub completes gating after claim and emits SOVEREIGN_IDENTITY_GATING_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "SOVEREIGN_IDENTITY_GATING_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkIdentityGatingValidator rejects un-attested clearing committee", () => {
    ...
    expect(() => validator.verifyAttributeClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("PqcIdentityGatingHub rejects gating completion before attribute claim verification", () => {
    const { hub, pool } = setupAndInitPool();
    expect(() => hub.completeGating(baseCompleteRequest(pool.poolId))).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq identity gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
        attestationQuorum: 3,
        attestationContractLifetimeSeconds: 15552000,
        credentialDepth: 8,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityGating", { attestationQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
        attestationContractLifetimeSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityGating", { credentialDepth: 32 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
        identityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
        banMalformedOrOutOfOrderIdentityClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityGating", {
