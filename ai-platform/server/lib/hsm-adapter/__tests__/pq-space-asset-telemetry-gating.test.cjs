...
describe("Track 87 PQ space-asset telemetry gating", () => {
  test("PqcSpaceAssetTelemetryGatingHub initializes a pool and emits ORBITAL_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "ORBITAL_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkOrbitalSlotClaimValidator verifies a telemetry claim and emits ZK_TELEMETRY_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_TELEMETRY_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcSpaceAssetTelemetryGatingHub completes accreditation after claim and emits ORBITAL_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "ORBITAL_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkOrbitalSlotClaimValidator rejects un-attested orbital oversight committee", () => {
    ...
    expect(() => validator.verifyTelemetryClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq space gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqSpaceGating", {
        orbitalSlotQuorum: 5,
        slotAllocationWindowSeconds: 15768000,
        telemetryChainDepth: 8,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqSpaceGating", { orbitalSlotQuorum: 3 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", {
        slotAllocationWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", { telemetryChainDepth: 32 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSpaceGating", {
        spaceAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", {
        orbitalOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", {
        banMalformedOrOutOfOrderOrbitalClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSpaceGating", { canonicalPayloadLayout: false }),
    ).toThrow(HsmAdapterError);
  });
});
