...
const {
  ...
  KEY_STATUS,
  HEARTBEAT_STATUS,
} = require("../enclave-key-rotation-heartbeat.cjs");
  ...
  describe("heartbeat challenge-response", () => {
    ...
    test("processHeartbeatResponse verifies valid response", () => {
      ...
      const expectedResponse = crypto
        .createHmac("sha256", keyState.keyMaterial)
        .update(challenge.nonce)
        .digest("hex");
      const result = engine.processHeartbeatResponse(
    ...
    test("processHeartbeatResponse rejects invalid response", () => {
      ...
      const challenge = engine.issueHeartbeat("e1");
      expect(() =>
        engine.processHeartbeatResponse(
          challenge.challengeId,
          "e1",
          "invalid-response",
        ),
    ...
    test("processHeartbeatResponse rejects unknown challenge", () => {
      engine.registerEnclave("e1");
      expect(() =>
        engine.processHeartbeatResponse("fake-id", "e1", "response"),
      ).toThrow(HsmAdapterError);
    });
