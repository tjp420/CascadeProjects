 ...
 */
const { HomomorphicContractEngine, _pedersenCommitment } = require('../homomorphic-contract-engine.cjs');
...
describe('Track 46 homomorphic computation', () => {
  test('HomomorphicContractEngine adds two encrypted commitments', () => {
    ...
    expect(events.some((e) => e.event === 'HOMOMORPHIC_CONTRACT_EXECUTED')).toBe(true);
  });
  ...
  test('HomomorphicContractEngine scalar multiplies a commitment', () => {
    ...
    const result = engine.execute('c-2', 'scalarMul', [a, { scalar: 4n }], mockAttestation());
  ...
  test('HomomorphicContractEngine rejects un-attested worker', () => {
    ...
    expect(() => engine.execute('c-1', 'add', [commitment(1n, 1n), commitment(2n, 2n)], { authority: 'bad' })).toThrow(HsmAdapterError);
  });
  ...
  test('HomomorphicContractEngine rejects a disallowed operation', () => {
    ...
    expect(() => engine.execute('c-1', 'mul', [commitment(1n, 1n), commitment(2n, 2n)], mockAttestation())).toThrow(HsmAdapterError);
  });
  ...
  test('ZkRangeProofProcessor generates and verifies a valid proof', () => {
    ...
    expect(events.some((e) => e.event === 'ZK_RANGE_PROOF_VERIFIED')).toBe(true);
  });
  ...
  test('ZkRangeProofProcessor rejects out-of-bounds value', () => {
    ...
    expect(() => processor.generate('c-4', c, 0, 100, mockAttestation())).toThrow(HsmAdapterError);
  });

  test('ZkRangeProofProcessor rejects bit width above maximum', () => {
    const processor = new ZkRangeProofProcessor({ policy: POLICY });
    const c = commitment(2n ** 70n, 7n);
    expect(() => processor.generate('c-5', c, 0, (2n ** 70n), mockAttestation())).toThrow(HsmAdapterError);
  });

  test('CryptoPolicyEngine validates homomorphic computation configuration', () => {
    const engine = new CryptoPolicyEngine({ default: {} });
    expect(() => engine.validate('t1', 'homomorphicComputation', {
      operation: 'add',
      rangeBitWidth: 32,
    ...
    expect(() => engine.validate('t1', 'homomorphicComputation', { operation: 'mul' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'homomorphicComputation', { rangeBitWidth: 4 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'homomorphicComputation', { rangeBitWidth: 8192 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'homomorphicComputation', { workerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'homomorphicComputation', { workerAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'homomorphicComputation', { contractVerificationWindowSeconds: 120 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'homomorphicComputation', { zkRangeProof: false })).toThrow(HsmAdapterError);
  });
});
