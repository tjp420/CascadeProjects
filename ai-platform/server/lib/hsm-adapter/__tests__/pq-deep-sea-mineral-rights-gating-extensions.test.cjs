...
/**
 * Track 95: PQC Deep-Sea Mineral Rights Gating & ZK Extraction
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, extraction chain depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch extraction claim verification, slashing
...
function baseClaimRequest(poolId) {
  return {
    ...
    attestationAuthority: 'mock-authority',
    abeKeyPolicyDigest: 'partial-sig-001',
    leaseWindowSeconds: 15552000,
  };
}
...
describe('Track 95 PQC Deep-Sea Mineral Rights Gating extensions', () => {
  describe('PqcDeepSeaMineralRightsGatingHub — extraction chain depth rebalancing', () => {
    test('rebalances extraction chain depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceExtractionChainDepth({
    ...
    test('rebalances extraction chain depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceExtractionChainDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates extractionChainDepth on rebalance when newExtractionChainDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceExtractionChainDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newExtractionChainDepth: 11,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.extractionChainDepth).toBe(11);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceExtractionChainDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceExtractionChainDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcDeepSeaMineralRightsGatingHub — batch initialization', () => {
  ...
  describe('PqcDeepSeaMineralRightsGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcDeepSeaMineralRightsGatingHub — committee aggregation', () => {
    test('aggregates committee signatures', () => {
      ...
      const result = ctx.hub.aggregateCommitteeSignatures(ctx.pool.poolId, [
        ...
        { peerId: 'p6', signature: 'sig-6' },
      ]);
      expect(result.signatureCount).toBe(6);
    ...
    test('rejects aggregation for unknown pool', () => {
      ...
      expect(() => hub.aggregateCommitteeSignatures('unknown', [
        ...
        { peerId: 'p6', signature: 's6' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('PqcDeepSeaMineralRightsGatingHub — cancellation', () => {
  ...
  describe('PqcDeepSeaMineralRightsGatingHub — queries and stats', () => {
  ...
  describe('ZkExtractionClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      ...
      expect(proof.zkExtractionRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ extractionVolume: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
  ...
  describe('ZkExtractionClaimValidator — batch extraction claim verification', () => {
    test('batch verifies multiple extraction claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyExtractionClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyExtractionClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyExtractionClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyExtractionClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkExtractionClaimValidator — partial signature aggregation', () => {
    test('aggregates ABE key policy digests', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateAbeKeyPolicyDigests(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
        { peerId: 'p4', signature: 'sig-4' },
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkExtractionRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyExtractionClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateAbeKeyPolicyDigests(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateAbeKeyPolicyDigests('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkExtractionClaimValidator — slashing window validation', () => {
  ...
  describe('ZkExtractionClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.leaseWindowSeconds = 999999999;
      try { ctx.validator.verifyExtractionClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyExtractionClaim(cReq);
      try { ctx.validator.verifyExtractionClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.LEASE_WINDOW_OUT_OF_BOUNDS).toBe('lease_window_out_of_bounds');
  ...
  describe('full Track 95 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkExtractionRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkExtractionRangeProofHash = snarkProof.zkExtractionRangeProofHash;
      const claim = ctx.validator.verifyExtractionClaim(cReq);
      ...
      const sigResult = ctx.hub.aggregateCommitteeSignatures(pool.poolId, [
        ...
        { peerId: 'peer-5', signature: 'sig-5' },
      ]);
      expect(sigResult.signatureCount).toBe(6);
