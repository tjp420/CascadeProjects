...
describe("Track 48 PQC asset bridge", () => {
  test("PqcAssetBridgeHub initiates and releases with escrow", () => {
    ...
    expect(events.some((e) => e.event === "ESCROW_RELEASE_FINALIZED")).toBe(
      true,
    );
  });
  ...
  test("CryptoPolicyEngine validates asset bridge configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "assetBridge", {
        committeeQuorum: 3,
    ...
    expect(() =>
      engine.validate("t1", "assetBridge", { committeeQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { assetTransactionValue: 2000000 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { lockEpochDuration: 30 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { claimExpirationEpochs: 20 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { sourceAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { targetAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { bridgeAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { timeLockEscrow: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "assetBridge", { canonicalPayloadLayout: false }),
    ).toThrow(HsmAdapterError);
  });
});
