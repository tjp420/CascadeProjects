...
const {
  ZkMembershipProofProcessor,
} = require("../zk-membership-proof-processor.cjs");
const {
...
describe("Track 57 PQ identity accumulator", () => {
  test("PqIdentityAccumulator adds a member and emits IDENTITY_ACCUMULATOR_UPDATED", () => {
    ...
    expect(events.some((e) => e.event === "IDENTITY_ACCUMULATOR_UPDATED")).toBe(
      true,
    );
  });
  ...
  test("ZkMembershipProofProcessor validates a valid membership proof", () => {
    ...
    expect(
      events.some((e) => e.event === "ZK_MEMBERSHIP_CLAIM_VALIDATED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq identity accumulator configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
        treeDepth: 10,
        membershipProofSystem: "groth16",
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", { treeDepth: 32 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
        membershipProofSystem: "bulletproofs",
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
        updateEpochSeconds: 60,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
        rootUpdateAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
        membershipProofAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
    ...
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
        banMalformedMembershipPeers: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqIdentityAccumulator", {
