...
describe("Track 60: MixnetBlindTransactionEngine", () => {
  ...
  describe("registerNode", () => {
    ...
    test("rejects missing nodeId", () => {
      expect(() => engine.registerNode({ enclaveId: "e1" })).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects duplicate nodeId", () => {
      ...
      expect(() =>
        engine.registerNode({ nodeId: "n1", enclaveId: "e2" }),
      ).toThrow(HsmAdapterError);
    });

    test("rejects missing enclaveId", () => {
      expect(() => engine.registerNode({ nodeId: "n1" })).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects when max nodes reached", () => {
      ...
      expect(() =>
        engine.registerNode({ nodeId: "n20", enclaveId: "e20" }),
      ).toThrow(HsmAdapterError);
    });
  });
  ...
  describe("confirmTransaction", () => {
    ...
    test("rejects unknown transaction", () => {
      expect(() => engine.confirmTransaction("unknown")).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("createPool", () => {
    ...
    test("rejects duplicate poolId", () => {
      ...
      expect(() => engine.createPool("pool-1", "shard-0")).toThrow(
        HsmAdapterError,
      );
    });
  });
  ...
  describe("addToPool", () => {
    ...
    test("rejects unknown pool", () => {
      createTx(engine);
      expect(() => engine.addToPool("unknown", "tx-1")).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects unknown transaction", () => {
      engine.createPool("pool-1", "shard-0");
      expect(() => engine.addToPool("pool-1", "unknown")).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects shard mismatch", () => {
      ...
      expect(() => engine.addToPool("pool-1", "tx-1")).toThrow(HsmAdapterError);
    });
  });
