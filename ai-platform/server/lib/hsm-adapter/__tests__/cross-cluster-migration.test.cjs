...
describe("CrossClusterMigrationEngine — Track 34 Cross-Cluster Migration", () => {
  ...
  const SHARDS = [
    {
      shardId: "shard-a",
      entryCount: 100,
      vectorClockSnapshot: { "node-1": 100, "node-2": 100 },
    },
    {
      shardId: "shard-b",
      entryCount: 50,
      vectorClockSnapshot: { "node-1": 50, "node-2": 50 },
    },
  ];
  ...
  describe("L2.01: happy-path migration lifecycle", () => {
    test("initiate → attest → transfer → verify → ack (quorum) → commit", () => {
      ...
      expect(engine.getMigrationState(manifest.migrationId).state).toBe(
  ...
  describe("L2.04: multiple shards in single migration", () => {
    test("multiple shards are migrated together", () => {
      ...
      expect(state.manifest.shardIds).toEqual([
        "shard-a",
        "shard-b",
        "shard-c",
      ]);
  ...
  describe("L2.05: state machine transitions", () => {
    test("valid transitions are enforced", () => {
      ...
      // Cannot skip attest
      expect(() => engine.beginTransfer(manifest.migrationId)).toThrow(
        HsmAdapterError,
      );

      engine.attest(manifest.migrationId, "mock-authority", "token");
      // Cannot skip transfer
      expect(() => engine.verify(manifest.migrationId, true)).toThrow(
        HsmAdapterError,
      );

      engine.beginTransfer(manifest.migrationId);
      // Cannot ack before verify
      expect(() => engine.acknowledge(manifest.migrationId, "dest-1")).toThrow(
        HsmAdapterError,
      );
  ...
  describe("L2.06/L2.07/L2.08: policy and attestation", () => {
    ...
    test("tenant policy can override crossClusterMigration settings", () => {
      ...
      const engine = new CryptoPolicyEngine({
        default: true,
        tenants: {
          "tenant-a": { crossClusterMigration: { minQuorumNodes: 5 } },
        },
      });
        ...
        shards: [
          {
            shardId: "shard-a",
            entryCount: 100,
            vectorClockSnapshot: { "node-1": 100, "node-2": 100, "node-3": 99 },
          },
        ],
      });

      expect(manifest.shards[0].vectorClockSnapshot).toEqual({
        "node-1": 100,
        "node-2": 100,
        "node-3": 99,
      });
  ...
  describe("L3.03/L3.04: replay protection", () => {
    test("migration IDs are unique and monotonic", () => {
      ...
      const m1 = engine.initiate({
        sourceCluster: "east",
        destinationCluster: "west",
        shards: SHARDS,
      });
      const m2 = engine.initiate({
        sourceCluster: "east",
        destinationCluster: "west",
        shards: SHARDS,
      });
    ...
    test("committed migration cannot be acked again", () => {
      ...
      expect(() => engine.acknowledge(manifest.migrationId, "dest-4")).toThrow(
        HsmAdapterError,
      );
    });
  });
  ...
  describe("L3.05: unauthorized attestation", () => {
    test("unauthorized attestation authority rejected", () => {
      ...
      expect(() =>
        engine.attest(manifest.migrationId, "evil-authority", "token"),
  ...
  describe("rollback edge cases", () => {
    test("cannot rollback committed migration", () => {
      ...
      expect(() => engine.rollback(manifest.migrationId)).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("error cases", () => {
    test("constructor throws for empty destination nodes", () => {
      expect(
        () => new CrossClusterMigrationEngine({ destinationNodes: [] }),
      ).toThrow(HsmAdapterError);
    });
    ...
    test("attest throws for unknown migration", () => {
      ...
      expect(() => engine.attest("unknown", "mock-authority", "token")).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("acknowledge throws for unknown node", () => {
      ...
      engine.verify(manifest.migrationId, true);
      expect(() =>
        engine.acknowledge(manifest.migrationId, "unknown-node"),
      ).toThrow(HsmAdapterError);
    });
    ...
    test("getMigrationState throws for unknown migration", () => {
      ...
      expect(() => engine.getMigrationState("unknown")).toThrow(
        HsmAdapterError,
      );
    });
  });
