...
describe('Track 54: ThresholdDecryptionCircuit', () => {
  ...
  describe('createKeySet', () => {
    ...
    test('rejects duplicate keySetId', () => {
      ...
      expect(() => engine.createKeySet({
        keySetId: 'ks1', threshold: 2, participants: 3,
      })).toThrow(HsmAdapterError);
    });

    test('rejects threshold below minimum', () => {
      expect(() => engine.createKeySet({
        keySetId: 'ks1', threshold: 1, participants: 3,
      })).toThrow(HsmAdapterError);
    });

    test('rejects participants < threshold', () => {
      expect(() => engine.createKeySet({
        keySetId: 'ks1', threshold: 5, participants: 3,
      })).toThrow(HsmAdapterError);
    });

    test('rejects too many participants', () => {
      expect(() => engine.createKeySet({
        keySetId: 'ks1', threshold: 2, participants: 20,
      })).toThrow(HsmAdapterError);
    });
  ...
  describe('encrypt', () => {
    ...
    test('rejects unknown key set', () => {
      expect(() => engine.encrypt('unknown', Buffer.from('hello')))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects compromised key set', () => {
      ...
      expect(() => engine.encrypt('ks1', Buffer.from('hello')))
        .toThrow(HsmAdapterError);
    });
  });
  ...
  describe('initiateCircuit', () => {
    ...
    test('rejects duplicate circuitId', () => {
      ...
      engine.initiateCircuit({
        circuitId: 'c1', keySetId: 'ks1',
        ciphertext: ct.ciphertext, nonce: ct.nonce,
      });
      expect(() => engine.initiateCircuit({
        circuitId: 'c1', keySetId: 'ks1',
        ciphertext: ct.ciphertext, nonce: ct.nonce,
    ...
    test('rejects unknown key set', () => {
      expect(() => engine.initiateCircuit({
        circuitId: 'c1', keySetId: 'unknown',
        ciphertext: Buffer.from('a'), nonce: Buffer.from('b'),
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects non-Buffer ciphertext', () => {
      ...
      expect(() => engine.initiateCircuit({
        circuitId: 'c1', keySetId: 'ks1',
        ciphertext: 'not-a-buffer', nonce: Buffer.from('b'),
      })).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('submitPartialDecryption', () => {
    test('accepts a partial decryption share', () => {
      ...
      engine.initiateCircuit({
        circuitId: 'c1', keySetId: 'ks1',
        ciphertext: ct.ciphertext, nonce: ct.nonce, authTag: ct.authTag,
      });
      const result = engine.submitPartialDecryption('c1', 'n1',
    ...
    test('assembles when threshold reached', () => {
      ...
      // Submit first share
      engine.submitPartialDecryption('c1', 'n1',
        engine.computePartialDecryption('ks1', 'n1'));
      // Submit second share — should trigger assembly
      const result = engine.submitPartialDecryption('c1', 'n2',
    ...
    test('rejects unknown circuit', () => {
      expect(() => engine.submitPartialDecryption('unknown', 'n1', Buffer.from('a')))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects unknown node', () => {
      ...
      expect(() => engine.submitPartialDecryption('c1', 'unknown', Buffer.from('a')))
        .toThrow(HsmAdapterError);
    });

    test('rejects duplicate submission', () => {
      engine.createKeySet({
        keySetId: 'ks1', threshold: 3, participants: 5,
      ...
      expect(() => engine.submitPartialDecryption('c1', 'n1', Buffer.from('b')))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects non-Buffer partial', () => {
      ...
      expect(() => engine.submitPartialDecryption('c1', 'n1', 'not-a-buffer'))
        .toThrow(HsmAdapterError);
    });
  });
  ...
  describe('assembleDecryption', () => {
    test('manually assembles when enough shares collected', () => {
      ...
      engine.submitPartialDecryption('c1', 'n1',
        engine.computePartialDecryption('ks1', 'n1'));
      engine.submitPartialDecryption('c1', 'n2',
        engine.computePartialDecryption('ks1', 'n2'));
      // Third share triggers auto-assembly (threshold=3)
      const result = engine.submitPartialDecryption('c1', 'n3',
    ...
    test('rejects unknown circuit', () => {
      expect(() => engine.assembleDecryption('unknown')).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('full distributed decryption flow', () => {
    test('complete encrypt -> distribute -> decrypt flow', () => {
      ...
      engine.createKeySet({
        ...
        threshold: 3,
        participants: 5,
        participantIds: ['enclave-a', 'enclave-b', 'enclave-c', 'enclave-d', 'enclave-e'],
      });
