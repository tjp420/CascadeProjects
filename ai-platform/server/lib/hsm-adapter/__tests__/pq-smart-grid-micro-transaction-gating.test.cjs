...
describe("Track 91 PQ smart-grid micro-transaction gating", () => {
  test("PqcSmartGridMicroTransactionGatingHub initializes a pool and emits SMARTGRID_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "SMARTGRID_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkMicroTransactionClaimValidator verifies a micro-transaction claim and emits ZK_MICRO_TRANSACTION_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyMicroTransactionClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(
      events.some((e) => e.event === "ZK_MICRO_TRANSACTION_CLAIM_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("PqcSmartGridMicroTransactionGatingHub completes accreditation after claim and emits LOAD_BALANCE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "LOAD_BALANCE_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkMicroTransactionClaimValidator rejects un-attested load balance oversight committee", () => {
    ...
    expect(() => validator.verifyMicroTransactionClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq smart grid gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", { gridOperatorQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
        transactionWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", { consumptionChainDepth: 36 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
        gridAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
        loadBalanceOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
        banMalformedOrOutOfOrderMicroTransactionClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSmartGridGating", {
