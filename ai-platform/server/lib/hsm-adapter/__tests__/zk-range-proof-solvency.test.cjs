...
describe("Track 53: ZkRangeProofSolvency", () => {
  ...
  describe("completeSolvencyAudit", () => {
    ...
    test("rejects unknown audit", () => {
      expect(() =>
        engine.completeSolvencyAudit("unknown", {
          totalAssets: 100,
          totalLiabilities: 50,
        }),
      ).toThrow(HsmAdapterError);
    ...
    test("rejects missing totals", () => {
      ...
      expect(() => engine.completeSolvencyAudit("audit-1", null)).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("verifyAuditLogIntegrity", () => {
    test("verifies intact log", () => {
      ...
      engine.verifyRangeProof(
        engine.getActiveProofs()[0]
          ? engine.getProof(engine.getActiveProofs()[0].proofId)
          : null,
      );
