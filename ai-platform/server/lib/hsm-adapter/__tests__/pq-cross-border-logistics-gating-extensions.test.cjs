...
/**
 * Track 81: PQC Cross-Border Logistics Gating & ZK Manifest
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, manifest depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch manifest claim verification, slashing
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyManifestClaim(baseClaimRequest(ctx.pool.poolId));
  return { ...ctx, claim };
}

describe('Track 81 PQC Cross-Border Logistics Gating extensions', () => {
  describe('PqcCrossBorderLogisticsGatingHub — manifest depth rebalancing', () => {
    test('rebalances manifest depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceManifestDepth({
    ...
    test('rebalances manifest depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceManifestDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates manifestDepth on rebalance when newManifestDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceManifestDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newManifestDepth: 18,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceManifestDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcCrossBorderLogisticsGatingHub — batch initialization', () => {
  ...
  describe('PqcCrossBorderLogisticsGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcCrossBorderLogisticsGatingHub — committee aggregation', () => {
  ...
  describe('PqcCrossBorderLogisticsGatingHub — cancellation', () => {
  ...
  describe('PqcCrossBorderLogisticsGatingHub — queries and stats', () => {
  ...
  describe('ZkManifestClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        transitLogMetric: 500,
        claimValue: 450,
      });
      expect(proof.zkManifestRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ transitLogMetric: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        transitLogMetric: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkManifestClaimValidator — batch manifest claim verification', () => {
    test('batch verifies multiple manifest claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyManifestClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyManifestClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyManifestClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyManifestClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkManifestClaimValidator — partial signature aggregation', () => {
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkManifestRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyManifestClaim(cReq); } catch (e) { /* expected */ }
  ...
  describe('ZkManifestClaimValidator — slashing window validation', () => {
  ...
  describe('ZkManifestClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.transitWindowSeconds = 999999999;
      try { ctx.validator.verifyManifestClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyManifestClaim(cReq);
      try { ctx.validator.verifyManifestClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.TRANSIT_WINDOW_OUT_OF_BOUNDS).toBe('transit_window_out_of_bounds');
  ...
  describe('full Track 81 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkManifestRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkManifestRangeProofHash = snarkProof.zkManifestRangeProofHash;
      const claim = ctx.validator.verifyManifestClaim(cReq);
