...
describe('Track 117 multi-tenant fuzzing matrix', () => {
  ...
  test('FUZZ-117-02: 5-level nested __proto__ / constructor pollution is blocked', () => {
    ...
    expect(Object.prototype).not.toHaveProperty('bftShardDeepMaxCatchUpBatchSize');
  ...
  test('FUZZ-117-05: strict reference sandboxing — mutation does not cross-tenant leak', () => {
    const policy = makeTrack117PrngDrivenMultiLayerPolicy(makeHashChainPrng(FUZZ_SEED + '-ref'));
    const engine = new CryptoPolicyEngine(policy, { strict: true });
    ...
    const originalB = b.bftShardSync ? b.bftShardSync.maxShardsPerCluster : undefined;
    if (a.bftShardSync) {
  ...
  test('FUZZ-117-07: PRNG-driven bftShardSync validation — 100 calls, no unhandled crash', () => {
    ...
    for (let i = 0; i < 100; i++) {
      const { tenantId, operation, config } = makeTrack117PrngDrivenValidateCall(prng);
      try {
  ...
  test('FUZZ-117-08: concurrent validation flood does not race or crash', async () => {
    ...
    const calls = [];
    for (let i = 0; i < 1000; i++) {
      const { tenantId, operation, config } = makeTrack117ConcurrentValidationCall(prng);
  ...
  test('FUZZ-117-09: cross-tenant bftShardSync mutation isolation', () => {
    ...
    const originalBMax = engine.getPolicy('track117-clean').bftShardSync.maxShardsPerCluster;
