...
const {
  PqcAutonomousVehicleFleetCoordinationGatingHub,
} = require("../pqc-autonomous-vehicle-fleet-coordination-gating-hub.cjs");
const {
  ZkAutonomousClaimValidator,
} = require("../zk-autonomous-claim-validator.cjs");
...
function baseInitRequest() {
  return {
    ...
    blindedTrajectoryMeasurementCommitment:
      "pedersen-trajectory-measurement-001",
    blindedCoordinationProbabilityCommitment: "pedersen-coordination-prob-001",
    blindedAutonomousMobilityAuthorityIdentityCommitment:
      "pedersen-autonomous-auth-001",
    coordinationWindowSeconds: 43200,
    coordinationChainDepth: 18,
    pqcSignatureScheme: "ML-DSA-65",
    autonomousMobilityAuthorityInitializerAttestation: mockAttestation(),
...
function baseClaimRequest(poolId) {
  return {
    ...
    blindedAutonomousMobilityAuthorityIdentityCommitment:
      "pedersen-autonomous-auth-001",
    zkAutonomousRangeProofHash: "zk-autonomous-proof-001",
    polynomialCommitmentDigest: "poly-commit-digest-001",
    autonomousEthicsOversightCommitteeAttestation: mockAttestation(),
    attestationAuthority: "mock-authority",
  };
}
...
function setupHubAndValidator() {
  ...
  const hub = new PqcAutonomousVehicleFleetCoordinationGatingHub({
    policy: POLICY,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
  });
  const validator = new ZkAutonomousClaimValidator({
    policy: POLICY,
    hub,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyAutonomousClaim(
...
describe("Track 102 PQ autonomous vehicle fleet coordination gating", () => {
  test("initializes a pool and emits AUTONOMOUS_COORDINATION_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some(
        (e) => e.event === "AUTONOMOUS_COORDINATION_POOL_INITIALIZED",
      ),
    ).toBe(true);
  });
  ...
  test("verifies an autonomous claim and emits ZK_AUTONOMOUS_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyAutonomousClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(claim.polynomialCommitmentDigest).toBe("poly-commit-digest-001");
    expect(events.some((e) => e.event === "ZK_AUTONOMOUS_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits COORDINATION_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "COORDINATION_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects coordination window exceeding maximum", () => {
    ...
    req.coordinationWindowSeconds = 999999;
  ...
  test("rejects un-attested autonomous mobility authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.autonomousMobilityAuthorityInitializerAttestation = {
      authority: "bad",
    };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested autonomous ethics oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkAutonomousClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.autonomousEthicsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkAutonomousRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkAutonomousRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing polynomialCommitmentDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.polynomialCommitmentDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyAutonomousClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq autonomous vehicle fleet coordination gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
        autonomousQuorum: 9,
        coordinationWindowSeconds: 43200,
        coordinationChainDepth: 18,
        pqcSignatureScheme: "ML-DSA-65",
        autonomousMobilityAuthorityInitializerAttestation: true,
        autonomousEthicsOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderAutonomousClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
        coordinationWindowSeconds: 999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
        coordinationChainDepth: 30,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
        autonomousMobilityAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
        autonomousEthicsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
        banMalformedOrOutOfOrderAutonomousClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqAutonomousVehicleFleetCoordinationGating", {
