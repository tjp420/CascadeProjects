...
describe("Track 34 cluster consensus engine", () => {
  ...
  test("startElection transitions to candidate and increments term", async () => {
    ...
    expect(events.some((e) => e.event === CONSENSUS_EVENT.LEADER_ELECTED)).toBe(
      true,
    );
    expect(events.some((e) => e.event === CONSENSUS_EVENT.VOTE_REQUESTED)).toBe(
      true,
    );
  ...
  test("appendAndReplicate throws when not leader", async () => {
    ...
    await expect(engine.appendAndReplicate({ type: "PUT" })).rejects.toThrow(
      HsmAdapterError,
    );
  ...
  test("receiveHeartbeat acknowledges valid leader", () => {
    ...
    const result = engine.receiveHeartbeat({
      term: 1,
      leaderId: "node-a",
      leaderCommit: 0,
    });
  ...
  test("receiveHeartbeat rejects unknown leader", () => {
    ...
    expect(() =>
      engine.receiveHeartbeat({ term: 1, leaderId: "node-z" }),
    ).toThrow(HsmAdapterError);
  ...
  test("requestVote grants vote for up-to-date candidate", () => {
    ...
    const result = engine.requestVote({
      term: 1,
      candidateId: "node-a",
      lastLogIndex: 0,
      lastLogTerm: 0,
  ...
  test("requestVote rejects candidate with stale log", () => {
    ...
    expect(result.voteGranted).toBe(false);
    expect(events.some((e) => e.event === CONSENSUS_EVENT.VOTE_REJECTED)).toBe(
      true,
    );
  ...
  test("requestVote rejects unknown candidate", () => {
    ...
    expect(() =>
      engine.requestVote({ term: 1, candidateId: "node-z" }),
    ).toThrow(HsmAdapterError);
  ...
  test("election fails when quorum is not reached", async () => {
    ...
    expect(events.some((e) => e.event === CONSENSUS_EVENT.QUORUM_LOST)).toBe(
      true,
    );
...
describe("Track 34 consensus policy validation", () => {
  test("CryptoPolicyEngine validates consensus configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "consensus", { minQuorumNodes: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "consensus", { heartbeatIntervalMs: 50 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "consensus", {
        heartbeatIntervalMs: 500,
    ...
    expect(() =>
      engine.validate("t1", "consensus", { maxLogBatchSize: 100 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "consensus", { consensusMode: "dictator" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "consensus", { requireLeaderHeartbeat: false }),
    ).toThrow(HsmAdapterError);
  });
  ...
  test("tenant-specific consensus policy overrides default", () => {
    ...
    expect(() =>
      engine.validate("t-large", "consensus", {
        minQuorumNodes: 5,
        maxLogBatchSize: 128,
        consensusMode: "paxos",
      }),
