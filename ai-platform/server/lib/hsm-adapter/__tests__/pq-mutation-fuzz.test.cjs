...
describe("Post-Quantum Mutation Fuzzing Utilities", () => {
  ...
  describe("mutateStructuralNoise", () => {
    ...
    test("MUT-14: can inject null bytes into string fields", () => {
      ...
      for (let i = 0; i < 20; i++) {
        const mutated = mutateStructuralNoise(original, {
          prng,
          injectCount: 3,
        });
  ...
  describe("applyRandomMutation", () => {
    ...
    test("MUT-19: handles Buffer payloads", () => {
      ...
      expect(Buffer.isBuffer(payload) || typeof payload === "object").toBe(
        true,
      );
    });
  });
  ...
  describe("buildValidMerkleProof", () => {
    ...
    test("MUT-23: is deterministic with same PRNG seed", () => {
      const p1 = buildValidMerkleProof(2, {
        prng: makeHashChainPrng(FUZZ_SEED + "-merkle-det"),
      });
      const p2 = buildValidMerkleProof(2, {
        prng: makeHashChainPrng(FUZZ_SEED + "-merkle-det"),
      });
...
describe("runMutationFuzzRunner — Fail-Closed Mutation Fuzzing", () => {
  ...
  test("ZK-RUN-02: 100% mutation rate rejects all mutated proofs", async () => {
    ...
    expect(
      result.failures.filter((f) => f.reason === "MUTATED_PROOF_ACCEPTED"),
    ).toHaveLength(0);
  ...
  test("ZK-RUN-04: fail-closed invariant — no mutated proof is ever accepted", async () => {
    const verifier = new PoRepVerifier();
    const validProofs = [
      buildValidMerkleProof(2, {
        prng: makeHashChainPrng(FUZZ_SEED + "-inv-2"),
      }),
      buildValidMerkleProof(4, {
        prng: makeHashChainPrng(FUZZ_SEED + "-inv-4"),
      }),
    ];
    const result = await runMutationFuzzRunner({
      verifyFn: (proof) => verifier.verify(proof),
    ...
    const acceptedMutated = result.failures.filter(
      (f) => f.reason === "MUTATED_PROOF_ACCEPTED",
    );
  ...
  test("ZK-RUN-12: high-volume stress run (500 iterations) maintains fail-closed", async () => {
    ...
    const validProofs = [
      buildValidMerkleProof(2, {
        prng: makeHashChainPrng(FUZZ_SEED + "-stress-2"),
      }),
      buildValidMerkleProof(4, {
        prng: makeHashChainPrng(FUZZ_SEED + "-stress-4"),
      }),
      buildValidMerkleProof(1, {
        prng: makeHashChainPrng(FUZZ_SEED + "-stress-1"),
      }),
    ];
    const result = await runMutationFuzzRunner({
      verifyFn: (proof) => verifier.verify(proof),
