 ...
 */
const {
  HomomorphicDbLookupEngine,
} = require("../homomorphic-db-lookup-engine.cjs");
...
describe("Track 49 homomorphic DB lookup", () => {
  test("HomomorphicDbLookupEngine returns matching records", () => {
    ...
    expect(
      events.some((e) => e.event === "HOMOMORPHIC_DB_QUERY_INITIATED"),
    ).toBe(true);
  });
  ...
  test("ZkMatchAttestation generates and verifies a proof", () => {
    ...
    expect(events.some((e) => e.event === "ZK_LOOKUP_MATCH_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("CryptoPolicyEngine validates homomorphic DB lookup configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", {
        encryptedColumns: 4,
        blindingType: "pedersen",
    ...
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", { encryptedColumns: 10 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", { blindingType: "plain" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", { queryAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", { queryAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", { queryAgeSeconds: 120 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", {
        zkMatchAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "homomorphicDbLookup", {
