...
function mockProposal(rotator) {
  ...
  return rotator.propose(1, Buffer.from('old-seed'), Buffer.from('new-seed'), 'proposer-1', ts);
}
...
describe('Track 47 hardware root rotation', () => {
  test('EnclaveRootRotator proposes and commits with quorum', () => {
    ...
    expect(events.some((e) => e.event === 'ENCLAVE_ROOT_ROTATION_INITIATED')).toBe(true);
    ...
    expect(events.some((e) => e.event === 'HARDWARE_SEED_COMMITTED')).toBe(true);
  });
  ...
  test('EnclaveRootRotator rejects un-attested admin', () => {
    ...
    expect(() => rotator.sign(1, 'admin-1', { authority: 'bad' }, 'sig-1')).toThrow(HsmAdapterError);
  });
  ...
  test('EnclaveRootRotator rejects expired signature', () => {
    ...
    mockProposal(rotator);
    expect(() => rotator.sign(1, 'admin-1', mockAttestation(100), 'sig-1')).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates hardware root rotation configuration', () => {
    ...
    expect(() => engine.validate('t1', 'hardwareRootRotation', {
      adminQuorum: 3,
    ...
    expect(() => engine.validate('t1', 'hardwareRootRotation', { adminQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'hardwareRootRotation', { signatureAgeSeconds: 120 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'hardwareRootRotation', { adminAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'hardwareRootRotation', { adminAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'hardwareRootRotation', { previousSeedZeroized: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'hardwareRootRotation', { rotationEpochIntervalSeconds: 60 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'hardwareRootRotation', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
