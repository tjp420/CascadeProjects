...
describe("Track 115 Prometheus alert rule compliance", () => {
  ...
  test("ALERT-115-02: Track115LatticeVfhssReconciliationStall — correct name, PromQL, severity, for window", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track115LatticeVfhssReconciliationStall",
    );
    ...
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_vfhssgate_pool_initialized_total\[5m\]\s*\)/,
    );
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_vfhss_accreditation_completed_total\[5m\]\s*\)/,
    );
    // Labels
  ...
  test("ALERT-115-03: Track115HighValidationFailureRate — correct name, PromQL, severity, for window", () => {
    const rule = group.rules.find(
      (r) => r.alert === "Track115HighValidationFailureRate",
    );
    expect(rule).toBeDefined();
    ...
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_zk_vfhss_claim_verified_total\[5m\]\s*\)/,
    );
    expect(rule.expr).toMatch(
      /rate\(\s*hsm_vfhssgate_pool_initialized_total\[5m\]\s*\)/,
    );
  ...
  test("ALERT-115-04: runbook URLs, no secrets, and VFHSS counters exist in hsm-metrics.cjs", () => {
    ...
    const track115Section = yamlText.slice(
      yamlText.indexOf("track115_lattice_vfhss_gating_alerts"),
    );
