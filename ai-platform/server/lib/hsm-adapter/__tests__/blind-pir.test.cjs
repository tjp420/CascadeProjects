...
describe('BlindSignatureIssuer edge cases (spec gaps)', () => {
  ...
  test('rejects a tampered blinded message m_blind', () => {
    ...
    // Tamper with the blinded message by flipping a low-order byte
    // (flip MSB can produce a value >= modulus triggering OpenSSL errors)
    const tampered = Buffer.from(blindedMessage);
    const idx = tampered.length - 1; // flip least-significant byte to avoid modulus overflow
