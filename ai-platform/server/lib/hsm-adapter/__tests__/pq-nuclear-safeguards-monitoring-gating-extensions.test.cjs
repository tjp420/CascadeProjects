...
/**
 * Track 89: PQC Nuclear Safeguards Monitoring Gating & ZK Safeguards
 ...
 * generation, batch safeguards claim verification, slashing
...
function baseClaimRequest(poolId) {
  return {
    ...
    attestationAuthority: 'mock-authority',
    thresholdRingSignature: 'partial-sig-001',
    inspectionWindowSeconds: 3888000,
  };
}
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifySafeguardsClaim(baseClaimRequest(ctx.pool.poolId));
  return { ...ctx, claim };
}

describe('Track 89 PQC Nuclear Safeguards Monitoring Gating extensions', () => {
  describe('PqcNuclearSafeguardsMonitoringGatingHub — telemetry chain depth rebalancing', () => {
    ...
    test('updates telemetryChainDepth on rebalance when newTelemetryChainDepth provided', () => {
      ...
      ctx.hub.rebalanceTelemetryChainDepth({
        ...
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newTelemetryChainDepth: 9,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.telemetryChainDepth).toBe(9);
  ...
  describe('PqcNuclearSafeguardsMonitoringGatingHub — batch initialization', () => {
  ...
  describe('PqcNuclearSafeguardsMonitoringGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcNuclearSafeguardsMonitoringGatingHub — committee aggregation', () => {
  ...
  describe('PqcNuclearSafeguardsMonitoringGatingHub — cancellation', () => {
  ...
  describe('PqcNuclearSafeguardsMonitoringGatingHub — queries and stats', () => {
  ...
  describe('ZkSafeguardsClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        inspectionReport: 500,
        claimValue: 450,
      });
      expect(proof.zkSafeguardsRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ inspectionReport: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        inspectionReport: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkSafeguardsClaimValidator — batch safeguards claim verification', () => {
    test('batch verifies multiple safeguards claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifySafeguardsClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifySafeguardsClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifySafeguardsClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifySafeguardsClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkSafeguardsClaimValidator — partial signature aggregation', () => {
    test('aggregates threshold ring signatures', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateThresholdRingSignatures(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
        { peerId: 'p4', signature: 'sig-4' },
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkSafeguardsRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifySafeguardsClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateThresholdRingSignatures(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateThresholdRingSignatures('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkSafeguardsClaimValidator — slashing window validation', () => {
  ...
  describe('ZkSafeguardsClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.inspectionWindowSeconds = 999999999;
      try { ctx.validator.verifySafeguardsClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifySafeguardsClaim(cReq);
      try { ctx.validator.verifySafeguardsClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.INSPECTION_WINDOW_OUT_OF_BOUNDS).toBe('inspection_window_out_of_bounds');
  ...
  describe('full Track 89 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkSafeguardsRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkSafeguardsRangeProofHash = snarkProof.zkSafeguardsRangeProofHash;
      const claim = ctx.validator.verifySafeguardsClaim(cReq);
