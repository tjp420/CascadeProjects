...
describe('Track 116 cluster-isolation REST route integration', () => {
  ...
  test('ROUTE116-L2-02: validate endpoint uses resolveOrgId as tenantId', async () => {
    const res = await request(app)
      .post('/api/vault/cluster-isolation/policy/validate?orgId=tenant-116-validate')
      .send({})
      .expect(200);
    expect(res.body.success).toBe(true);
  ...
  test('ROUTE116-L2-03: telemetry endpoint returns only numeric counters', async () => {
    ...
    expect(Object.keys(telemetry).sort()).toEqual(['hsm_isolation_violation_total', 'hsm_key_reject_total']);
  ...
  test('ROUTE116-L2-05: telemetry counters match hsmMetrics.getMetrics()', async () => {
    ...
    const res = await request(app)
      .get('/api/vault/cluster-isolation/telemetry')
      .expect(200);
    expect(res.body.telemetry.hsm_isolation_violation_total).toBe(expectedMetrics.hsm_isolation_violation_total || 0);
    expect(res.body.telemetry.hsm_key_reject_total).toBe(expectedMetrics.hsm_key_reject_total || 0);
  });
