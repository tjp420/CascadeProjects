 ...
 */
const { MultipartyAuditLog, DEFAULT_OPTIONS } = require('../multiparty-audit-log.cjs');
...
describe('Track 43: MultipartyAuditLog', () => {
  ...
  describe('verifier management', () => {
    ...
    test('registerVerifier enforces max limit', () => {
      const small = new MultipartyAuditLog({ minVerifiers: 1, maxVerifiers: 2 });
  ...
  describe('append', () => {
    test('appends a valid event and returns pending entry', () => {
      const result = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { node: 'node-1' } });
    ...
    test('rejects disallowed event type', () => {
      expect(() => log.append({ eventType: 'UNKNOWN_EVENT', data: {} })).toThrow(HsmAdapterError);
    });
    ...
    test('uses provided timestamp', () => {
      const ts = 1234567890;
      const result = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {}, timestamp: ts });
      expect(result.pending).toBe(true);
    });
  });
  ...
  describe('signEntry and commit', () => {
    test('commits entry when min verifiers sign', () => {
      ...
      const result = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { node: 'n1' } });
    ...
    test('rejects signature from unregistered verifier', () => {
      const result = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {} });
      expect(() => log.signEntry(result.entryId, 'unknown', 'sig')).toThrow(HsmAdapterError);
    });

    test('rejects signature for non-existent entry', () => {
      log.registerVerifier('v1');
      expect(() => log.signEntry('fake-id', 'v1', 'sig')).toThrow(HsmAdapterError);
    });
    ...
    test('rejects duplicate signature from same verifier', () => {
      ...
      const result = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {} });
      log.signEntry(result.entryId, 'v1', 'sig-1');
      expect(() => log.signEntry(result.entryId, 'v1', 'sig-2')).toThrow(HsmAdapterError);
    });
  });

  describe('verification timeout', () => {
    test('rejects signature after timeout', () => {
      const fastLog = new MultipartyAuditLog({ minVerifiers: 2, verifierTimeoutMs: 50 });
      fastLog.registerVerifier('v1');
      fastLog.registerVerifier('v2');
      const result = fastLog.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {} });
      fastLog.signEntry(result.entryId, 'v1', 'sig-1');
      return new Promise(resolve => setTimeout(resolve, 60)).then(() => {
        expect(() => fastLog.signEntry(result.entryId, 'v2', 'sig-2')).toThrow(HsmAdapterError);
      });
    });
  });
  ...
  describe('queryEntries', () => {
    test('returns all committed entries', () => {
      log.registerVerifier('v1');
      log.registerVerifier('v2');
      log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { n: 1 } });
      const r2 = log.append({ eventType: 'ENCLAVE_KEY_PROVISIONED', data: { n: 2 } });
      // Sign first entry
    ...
    test('filters by eventType', () => {
      log.registerVerifier('v1');
      log.registerVerifier('v2');
      const r1 = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {} });
      const r2 = log.append({ eventType: 'ENCLAVE_KEY_PROVISIONED', data: {} });
      log.signEntry(r1.entryId, 'v1', 's1');
      log.signEntry(r1.entryId, 'v2', 's2');
      log.signEntry(r2.entryId, 'v1', 's3');
      log.signEntry(r2.entryId, 'v2', 's4');
      const filtered = log.queryEntries({ eventType: 'ENCLAVE_KEY_PROVISIONED' });
    ...
    test('filters by since timestamp', () => {
      log.registerVerifier('v1');
      log.registerVerifier('v2');
      const r1 = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {}, timestamp: 1000 });
      const r2 = log.append({ eventType: 'ENCLAVE_KEY_PROVISIONED', data: {}, timestamp: 5000 });
      log.signEntry(r1.entryId, 'v1', 's1');
      log.signEntry(r1.entryId, 'v2', 's2');
      log.signEntry(r2.entryId, 'v1', 's3');
      log.signEntry(r2.entryId, 'v2', 's4');
    ...
    test('limits results', () => {
      ...
      log.registerVerifier('v2');
      for (let i = 0; i < 5; i++) {
        const r = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { i } });
  ...
  describe('getEntry', () => {
    test('returns entry by ID', () => {
      log.registerVerifier('v1');
      log.registerVerifier('v2');
      const r = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { x: 1 } });
  ...
  describe('verifyChain', () => {
    ...
    test('detects tampered chain', () => {
      log.registerVerifier('v1');
      log.registerVerifier('v2');
      const r = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { i: 1 } });
      log.signEntry(r.entryId, 'v1', 's1');
      log.signEntry(r.entryId, 'v2', 's2');
  ...
  describe('exportLog', () => {
    test('exports entries, verifiers, and counts', () => {
      log.registerVerifier('v1');
      log.registerVerifier('v2');
      const r = log.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: {} });
      log.signEntry(r.entryId, 'v1', 's1');
      log.signEntry(r.entryId, 'v2', 's2');
  ...
  describe('pruning', () => {
    test('prunes oldest entries when maxEntries exceeded', () => {
      const small = new MultipartyAuditLog({ minVerifiers: 1, maxVerifiers: 3, maxEntries: 3 });
      small.registerVerifier('v1');
      for (let i = 0; i < 5; i++) {
        const r = small.append({ eventType: 'ENCLAVE_HARDWARE_BOOTSTRAPPED', data: { i } });
