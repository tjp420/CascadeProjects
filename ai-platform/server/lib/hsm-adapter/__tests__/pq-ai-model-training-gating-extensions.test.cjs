...
/**
 * Track 82: PQC AI Model Training Gating & ZK Training
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, provenance depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch training claim verification, slashing
...
const {
  ...
  POOL_STATUS,
  REBALANCE_DIRECTION,
} = require('../pqc-ai-model-training-gating-hub.cjs');
const {
  ZkTrainingClaimValidator,
  CLAIM_STATUS,
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-training-claim-validator.cjs');
...
function baseInitRequest() {
  return {
    ...
    trainingWindowSeconds: 15552000,
    provenanceDepth: 12,
...
describe('Track 82 PQC AI Model Training Gating extensions', () => {
  describe('PqcAiModelTrainingGatingHub — provenance depth rebalancing', () => {
    test('rebalances provenance depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceProvenanceDepth({
    ...
    test('rebalances provenance depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceProvenanceDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates provenanceDepth on rebalance when newProvenanceDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceProvenanceDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newProvenanceDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.provenanceDepth).toBe(18);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceProvenanceDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceProvenanceDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcAiModelTrainingGatingHub — batch initialization', () => {
  ...
  describe('PqcAiModelTrainingGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcAiModelTrainingGatingHub — committee aggregation', () => {
  ...
  describe('PqcAiModelTrainingGatingHub — cancellation', () => {
  ...
  describe('PqcAiModelTrainingGatingHub — queries and stats', () => {
  ...
  describe('ZkTrainingClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        datasetProvenanceMetric: 500,
        claimValue: 450,
      });
      expect(proof.zkTrainingRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ datasetProvenanceMetric: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        datasetProvenanceMetric: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkTrainingClaimValidator — batch training claim verification', () => {
    test('batch verifies multiple training claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyTrainingClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyTrainingClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyTrainingClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyTrainingClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkTrainingClaimValidator — partial signature aggregation', () => {
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkTrainingRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyTrainingClaim(cReq); } catch (e) { /* expected */ }
  ...
  describe('ZkTrainingClaimValidator — slashing window validation', () => {
  ...
  describe('ZkTrainingClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.trainingWindowSeconds = 999999999;
      try { ctx.validator.verifyTrainingClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyTrainingClaim(cReq);
      try { ctx.validator.verifyTrainingClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.TRAINING_WINDOW_OUT_OF_BOUNDS).toBe('training_window_out_of_bounds');
  ...
  describe('full Track 82 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkTrainingRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkTrainingRangeProofHash = snarkProof.zkTrainingRangeProofHash;
      const claim = ctx.validator.verifyTrainingClaim(cReq);
