...
/**
 * Track 77: PQC Biometric Verification Gating & ZK Biometric
 * Claim Validators ΓÇö extension tests.
 *
 * Tests the new batch pool initialization, liveness metric depth
...
describe('Track 77 PQC Biometric Verification Gating extensions', () => {
  describe('PqcBiometricVerificationGatingHub ΓÇö liveness metric depth rebalancing', () => {
    ...
    test('updates livenessMetricDepth on rebalance when newLivenessMetricDepth provided', () => {
      ...
      ctx.hub.rebalanceLivenessMetricDepth({
        ...
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newLivenessMetricDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.livenessMetricDepth).toBe(18);
  ...
  describe('PqcBiometricVerificationGatingHub ΓÇö batch initialization', () => {
  ...
  describe('PqcBiometricVerificationGatingHub ΓÇö cross-chain settlement', () => {
  ...
  describe('PqcBiometricVerificationGatingHub ΓÇö committee aggregation', () => {
  ...
  describe('PqcBiometricVerificationGatingHub ΓÇö cancellation', () => {
  ...
  describe('PqcBiometricVerificationGatingHub ΓÇö queries and stats', () => {
  ...
  describe('ZkBiometricClaimValidator ΓÇö HW-SNARK proof generation', () => {
  ...
  describe('ZkBiometricClaimValidator ΓÇö batch biometric claim verification', () => {
  ...
  describe('ZkBiometricClaimValidator ΓÇö partial signature aggregation', () => {
  ...
  describe('ZkBiometricClaimValidator ΓÇö slashing window validation', () => {
  ...
  describe('ZkBiometricClaimValidator ΓÇö slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.templateExpirationSeconds = 999999999;
      try { ctx.validator.verifyBiometricClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
  ...
  describe('full Track 77 extended flow', () => {
    test('complete init ΓåÆ rebalance ΓåÆ claim ΓåÆ accredit ΓåÆ settle flow', () => {
      const ctx = setupHubAndValidator();
      const req = baseInitRequest();
