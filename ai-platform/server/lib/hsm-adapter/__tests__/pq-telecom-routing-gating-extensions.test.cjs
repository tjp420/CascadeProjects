...
/**
 * Track 85: PQC Telecom Routing Gating & ZK Bandwidth
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, network routing depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch bandwidth claim verification, slashing
...
function baseClaimRequest(poolId) {
  return {
    ...
    attestationAuthority: 'mock-authority',
    blindSignature: 'partial-sig-001',
    allocationWindowSeconds: 1296000,
  };
}
...
describe('Track 85 PQC Telecom Routing Gating extensions', () => {
  describe('PqcTelecomRoutingGatingHub — network routing depth rebalancing', () => {
    test('rebalances network routing depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceNetworkRoutingDepth({
    ...
    test('rebalances network routing depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceNetworkRoutingDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates networkRoutingDepth on rebalance when newNetworkRoutingDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceNetworkRoutingDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newNetworkRoutingDepth: 18,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.networkRoutingDepth).toBe(18);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceNetworkRoutingDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceNetworkRoutingDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcTelecomRoutingGatingHub — batch initialization', () => {
  ...
  describe('PqcTelecomRoutingGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcTelecomRoutingGatingHub — committee aggregation', () => {
  ...
  describe('PqcTelecomRoutingGatingHub — cancellation', () => {
  ...
  describe('PqcTelecomRoutingGatingHub — queries and stats', () => {
  ...
  describe('ZkBandwidthClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      const ctx = setupAndInitPool();
      const proof = ctx.validator.generateHwSnarkProof({
        poolId: ctx.pool.poolId,
        latencyBoundMetric: 500,
        claimValue: 450,
      });
      expect(proof.zkTelecomRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ latencyBoundMetric: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects proof generation for unknown pool', () => {
      ...
      expect(() => validator.generateHwSnarkProof({
        poolId: 'unknown',
        latencyBoundMetric: 100,
        claimValue: 50,
      })).toThrow(HsmAdapterError);
    });
  });

  describe('ZkBandwidthClaimValidator — batch bandwidth claim verification', () => {
    test('batch verifies multiple bandwidth claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyBandwidthClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyBandwidthClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyBandwidthClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyBandwidthClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkBandwidthClaimValidator — partial signature aggregation', () => {
    test('aggregates partial signatures', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateBlindSignatures(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
      ]);
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkTelecomRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyBandwidthClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateBlindSignatures(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateBlindSignatures('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkBandwidthClaimValidator — slashing window validation', () => {
  ...
  describe('ZkBandwidthClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyBandwidthClaim(cReq);
      try { ctx.validator.verifyBandwidthClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
  ...
  describe('full Track 85 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkTelecomRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkTelecomRangeProofHash = snarkProof.zkTelecomRangeProofHash;
      const claim = ctx.validator.verifyBandwidthClaim(cReq);
