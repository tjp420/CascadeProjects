...
/**
 * Track 94: PQC Ocean Fisheries Allocation Gating & ZK Catch
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, vessel telemetry chain depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch catch claim verification, slashing
...
const {
  ...
  POOL_STATUS,
  REBALANCE_DIRECTION,
} = require('../pqc-ocean-fisheries-allocation-gating-hub.cjs');
const {
  ZkCatchClaimValidator,
  CLAIM_STATUS,
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-catch-claim-validator.cjs');
...
function baseClaimRequest(poolId) {
  return {
    ...
    attestationAuthority: 'mock-authority',
    proxyReEncryptionKeyDigest: 'partial-sig-001',
    catchTrackingWindowSeconds: 1296000,
  };
}
...
describe('Track 94 PQC Ocean Fisheries Allocation Gating extensions', () => {
  describe('PqcOceanFisheriesAllocationGatingHub — vessel telemetry chain depth rebalancing', () => {
    test('rebalances vessel telemetry chain depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceVesselTelemetryChainDepth({
    ...
    test('rebalances vessel telemetry chain depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceVesselTelemetryChainDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates vesselTelemetryChainDepth on rebalance when newVesselTelemetryChainDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceVesselTelemetryChainDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newVesselTelemetryChainDepth: 9,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.vesselTelemetryChainDepth).toBe(9);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceVesselTelemetryChainDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceVesselTelemetryChainDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcOceanFisheriesAllocationGatingHub — batch initialization', () => {
    ...
    test('batch init handles mixed valid/invalid', () => {
      ...
      r3.poolId = 'pool-ok2';
      r3.vesselTelemetryChainDepth = 999;
  ...
  describe('PqcOceanFisheriesAllocationGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcOceanFisheriesAllocationGatingHub — committee aggregation', () => {
  ...
  describe('PqcOceanFisheriesAllocationGatingHub — cancellation', () => {
  ...
  describe('PqcOceanFisheriesAllocationGatingHub — queries and stats', () => {
  ...
  describe('ZkCatchClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      ...
      expect(proof.zkCatchRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ quotaAllocation: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
  ...
  describe('ZkCatchClaimValidator — batch catch claim verification', () => {
    test('batch verifies multiple catch claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyCatchClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyCatchClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyCatchClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyCatchClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkCatchClaimValidator — partial signature aggregation', () => {
    test('aggregates proxy re-encryption key digests', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateProxyReEncryptionKeyDigests(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
        { peerId: 'p4', signature: 'sig-4' },
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkCatchRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyCatchClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateProxyReEncryptionKeyDigests(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateProxyReEncryptionKeyDigests('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkCatchClaimValidator — slashing window validation', () => {
  ...
  describe('ZkCatchClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.catchTrackingWindowSeconds = 999999999;
      try { ctx.validator.verifyCatchClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyCatchClaim(cReq);
      try { ctx.validator.verifyCatchClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.TRACKING_WINDOW_OUT_OF_BOUNDS).toBe('tracking_window_out_of_bounds');
  ...
  describe('full Track 94 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkCatchRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkCatchRangeProofHash = snarkProof.zkCatchRangeProofHash;
      const claim = ctx.validator.verifyCatchClaim(cReq);
