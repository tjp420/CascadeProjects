...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyAuthenticationClaim(
...
describe("Track 93 PQ cultural heritage provenance gating", () => {
  test("PqcCulturalHeritageProvenanceGatingHub initializes a pool and emits HERITAGE_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "HERITAGE_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkAuthenticationClaimValidator verifies an authentication claim and emits ZK_AUTHENTICATION_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyAuthenticationClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(claim.fuzzyMatchThreshold).toBe(0.85);
    expect(
      events.some((e) => e.event === "ZK_AUTHENTICATION_CLAIM_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("PqcCulturalHeritageProvenanceGatingHub completes accreditation after claim and emits PROVENANCE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "PROVENANCE_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkAuthenticationClaimValidator rejects un-attested cultural heritage oversight committee", () => {
    ...
    expect(() => validator.verifyAuthenticationClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq heritage gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHeritageGating", { authenticationQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
        authenticationWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHeritageGating", { provenanceChainDepth: 40 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
        unescoAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
        culturalHeritageOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
        banMalformedOrOutOfOrderAuthenticationClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqHeritageGating", {
