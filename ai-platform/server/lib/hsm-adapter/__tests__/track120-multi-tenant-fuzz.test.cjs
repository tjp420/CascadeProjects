...
describe('Track 120 multi-tenant fuzzing matrix', () => {
  ...
  test('FUZZ-120-01: prototype pollution in clusterKeyReconciliation is blocked', () => {
    ...
    expect(Object.prototype).not.toHaveProperty('reconciliationConstructorPolluted');
    ...
    expect(resolved.clusterKeyReconciliation.quarantineOnCriticalDivergence).toBe(true);
  ...
  test('FUZZ-120-02: 5-level nested __proto__ / constructor pollution is blocked', () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(Object.prototype).not.toHaveProperty('reconciliationProtoLevel' + i);
      expect(Object.prototype).not.toHaveProperty('reconciliationCtorLevel' + i);
    }
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      for (const tid of tenantIds) {
        ...
        if (resolved.clusterKeyReconciliation) {
          expect(typeof resolved.clusterKeyReconciliation.minQuorumNodes).toBe('number');
        }
      }
    }
  ...
  test('FUZZ-120-06: Track 120 type confusion fails closed with structured HsmAdapterError', () => {
    ...
    for (const { value, label } of configs) {
      ...
      try {
        const result = engine.validate('default', 'clusterKeyReconciliation', value);
