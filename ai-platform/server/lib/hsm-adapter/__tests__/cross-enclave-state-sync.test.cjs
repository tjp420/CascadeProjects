...
const {
  ...
  ENCLAVE_STATUS,
  SYNC_OP_TYPE,
} = require("../cross-enclave-state-sync.cjs");
...
describe("Track 44: CrossEnclaveStateSync", () => {
  ...
  beforeEach(() => {
    sync = new CrossEnclaveStateSync({
      replicationFactor: 3,
      minSyncQuorum: 2,
      maxEnclaves: 10,
    });
  ...
  describe("writeState and readState", () => {
    ...
    test("writeState rejects unassigned enclave", () => {
      ...
      const shard = sync.getShard("shard-1");
      const unassigned = ["e1", "e2", "e3", "e4"].find(
        (e) => !shard.enclaveIds.includes(e),
      );
      expect(() => sync.writeState("shard-1", "key-1", {}, unassigned)).toThrow(
        HsmAdapterError,
      );
    });

    test("writeState rejects unknown shard", () => {
      expect(() => sync.writeState("unknown", "key", {}, "e1")).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("writeState rejects oversized state", () => {
      const small = new CrossEnclaveStateSync({
        replicationFactor: 1,
        minSyncQuorum: 1,
        maxStateSizeBytes: 100,
      });
      small.registerEnclave("e1");
      small.createShard("shard-1");
      const bigValue = { data: "x".repeat(200) };
      expect(() => small.writeState("shard-1", "key", bigValue, "e1")).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("syncState", () => {
    ...
    test("rejects sync from unassigned enclave", () => {
      ...
      expect(() => sync.syncState("shard-1", unassigned, {}, {})).toThrow(
        HsmAdapterError,
      );
    });

    test("rejects sync for unknown shard", () => {
      expect(() => sync.syncState("unknown", "e1", {}, {})).toThrow(
        HsmAdapterError,
      );
    });
  ...
  describe("detectStaleEnclaves", () => {
    test("marks enclaves with old heartbeats as offline", () => {
      ...
      return new Promise((resolve) => setTimeout(resolve, 150)).then(() => {
        ...
        expect(enclaves.every((e) => e.status === ENCLAVE_STATUS.OFFLINE)).toBe(
          true,
        );
      });
    });
  ...
  describe("getSyncLog", () => {
    test("returns recent sync operations", () => {
      ...
      sync.syncState(
        ...
        "e2",
        {
          k: { value: 1, timestamp: Date.now(), enclaveId: "e2", sequence: 1 },
        },
        { e2: 1 },
      );
