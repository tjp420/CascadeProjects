...
describe("Track 33 primitive groundwork", () => {
  test("DEFAULT_POLICY exposes accumulatorGating constraints", () => {
    ...
    expect(
      DEFAULT_POLICY.accumulatorGating.requireEnclaveMembershipAttestation,
    ).toBe(true);
  });
  ...
  test("hsm-metrics exposes Track 33 accumulator counters", () => {
    ...
    expect(typeof all.hsm_accumulatorgate_pool_initialized_total).toBe(
      "number",
    );
    expect(typeof all.hsm_zk_accumulator_claim_verified_total).toBe("number");
    expect(typeof all.hsm_accumulator_accreditation_completed_total).toBe(
      "number",
    );
  });
});
