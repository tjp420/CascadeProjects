...
process.env.KEY_ROTATION_STORE_PATH = require('path').join(require('os').tmpdir(), 'sb-epoch-test-key.json');
process.env.AUDIT_LOG_PATH = require('path').join(require('os').tmpdir(), 'sb-epoch-test-audit.json');
...
describe('Epoch-Frame Verification Hardening', () => {
  ...
  describe('L2-01: HEARTBEAT with matching epoch', () => {
    test('accepted with no epoch events', () => {
      ...
      const reconciledEvents = clusterSync.queryEvents({ eventType: 'epoch_reconciled' });
  ...
  describe('L2-06: SIEM hook fires on KEY_REJECT', () => {
    test('hook invoked with siemSeverity high', () => {
      ...
      clusterSync._handleMessage({
        type: 'KEY_COMMIT', from: 'node-1', leaderId: 'node-1', epoch: 1,
        activeHex: validHex, previousHex: null,
        activeFingerprint: 'ghi', previousFingerprint: null,
        rotatedAt: Date.now(), graceMs: null,
      }, s2);
  ...
  describe('L3-02 / S-07: Epoch reconciliation after partition', () => {
    test('node adopts higher epoch from peer', () => {
      ...
      clusterSync._handleMessage({
        type: 'HEARTBEAT', from: 'node-3', leaderId: 'node-1', epoch: 5,
        activeFingerprint: 'abc', previousFingerprint: null, rotatedAt: null,
      }, s2);
    ...
    test('node does NOT adopt lower epoch', () => {
      ...
      const s2 = createMockSocket('127.0.0.1', 7002);
      clusterSync._handleMessage({
        type: 'HEARTBEAT', from: 'node-3', leaderId: 'node-1', epoch: 3,
        activeFingerprint: 'abc', previousFingerprint: null, rotatedAt: null,
      }, s2);
