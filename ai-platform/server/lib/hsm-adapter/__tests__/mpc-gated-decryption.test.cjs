...
describe("Track 54 MPC gated decryption", () => {
  test("MpcCircuitProcessor initiates and completes circuit evaluation", () => {
    ...
    expect(
      events.some((e) => e.event === "MPC_CIRCUIT_EVALUATION_INITIATED"),
    ).toBe(true);
  ...
  test("MpcGatedDecryptor unlocks after valid circuit satisfaction", () => {
    ...
    expect(events.some((e) => e.event === "MPC_DECRYPTION_GATE_UNLOCKED")).toBe(
      true,
    );
  });
  ...
  test("CryptoPolicyEngine validates MPC gated decryption configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
        circuitNodes: 3,
    ...
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", { circuitNodes: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
        multiplicationGateDepth: 16,
    ...
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
        transactionAgeSeconds: 600,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
        enclaveAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
    ...
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
        circuitSatisfactionProof: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "mpcGatedDecryption", {
  ...
  test("CryptoPolicyEngine validates PQC threshold configuration", () => {
    const engine = new CryptoPolicyEngine({ default: {} });
    expect(() =>
      engine.validate("t1", "pqcThreshold", {
    ...
    expect(() =>
      engine.validate("t1", "pqcThreshold", { signatureThreshold: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcThreshold", { committeeSize: 20 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcThreshold", {
    ...
    expect(() =>
      engine.validate("t1", "pqcThreshold", { hybridMode: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcThreshold", { curve: "secp256k1" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcThreshold", { signatureAgeSeconds: 600 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcThreshold", { canonicalPayloadLayout: false }),
    ).toThrow(HsmAdapterError);
  });
});
