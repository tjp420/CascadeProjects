...
describe('Track 117 core integration — BFT shard sync engine wiring', () => {
  ...
  test('CORE-117-01: constructor validates config against policy', () => {
    ...
    expect(() => makeEngine({ maxCatchUpBatchSize: 999 })).toThrow(HsmAdapterError);
    try {
    ...
    expect(() => makeEngine({ requireQuorumCommit: false })).toThrow(HsmAdapterError);
    try {
    ...
    expect(() => makeEngine({ maxShardsPerCluster: 9999 })).toThrow(HsmAdapterError);
    try {
