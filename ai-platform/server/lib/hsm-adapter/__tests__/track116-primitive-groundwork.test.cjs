...
describe("Track 116 primitive groundwork", () => {
  ...
  test("GROUND-116-02: DEFAULT_POLICY merges cleanly with tenant overrides without property contamination", () => {
    ...
    expect(
      policy.clusterIsolationHardening.maxIsolationViolationThreshold,
    ).toBe(50);
    // Non-overridden attributes retain defaults
    expect(policy.clusterIsolationHardening.requireKnownPeerValidation).toBe(
      true,
    );
    expect(policy.clusterIsolationHardening.rejectNonLeaderKeyCommits).toBe(
      true,
    );
    expect(policy.clusterIsolationHardening.allowDkgNonLeaderMessages).toBe(
      false,
    );
