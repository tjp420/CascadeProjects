...
describe("Track 97 PQ stratospheric aerosol monitoring gating", () => {
  test("initializes a pool and emits STRATOSPHERIC_MONITORING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some(
        (e) => e.event === "STRATOSPHERIC_MONITORING_POOL_INITIALIZED",
      ),
    ).toBe(true);
  });
  ...
  test("verifies an aerosol claim and emits ZK_AEROSOL_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_AEROSOL_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits DEPLOYMENT_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "DEPLOYMENT_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects deployment window exceeding maximum", () => {
    ...
    req.deploymentWindowSeconds = 99999999;
  ...
  test("rejects un-attested climate authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.climateAuthorityInitializerAttestation = { authority: "bad" };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested stratospheric oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkAerosolClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.stratosphericOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkAerosolRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkAerosolRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing latticeSignatureDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.latticeSignatureDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyAerosolClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq stratospheric aerosol gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqStratosphericAerosolGating", {
        climateQuorum: 4,
        deploymentWindowSeconds: 15552000,
        monitoringChainDepth: 8,
        pqcSignatureScheme: "ML-DSA-65",
        climateAuthorityInitializerAttestation: true,
        stratosphericOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderAerosolClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqStratosphericAerosolGating", {
        climateAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqStratosphericAerosolGating", {
        stratosphericOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqStratosphericAerosolGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqStratosphericAerosolGating", {
        banMalformedOrOutOfOrderAerosolClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqStratosphericAerosolGating", {
