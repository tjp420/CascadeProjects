...
describe('EnclaveAttestationClient hardening', () => {
  test('rejects missing fields', async () => {
    ...
    await expect(c.verifyHandshake({})).rejects.toHaveProperty('code', 'MISSING_FIELDS');
  });

  test('rejects timestamp skew outside window', async () => {
    const c = new EnclaveAttestationClient({ timestampSkewMs: 30 * 1000 });
    const payload = { peerId: 'peer1', nonce: 'n1', timestamp: Date.now() - 60 * 1000 };
    await expect(c.verifyHandshake(payload)).rejects.toHaveProperty('code', 'TIMESTAMP_SKEW');
  });
  ...
  test('rejects replayed nonces', async () => {
    ...
    await expect(c.verifyHandshake(payload)).rejects.toHaveProperty('code', 'REPLAY_NONCE');
  });
