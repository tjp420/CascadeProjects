...
function resolveBaseTestFile(pattern) {
  ...
  for (const subdir of subdirs) {
    const subFiles = fs
      .readdirSync(path.join(__dirname, subdir.name))
      .filter(
        (f) =>
          f.endsWith(".test.cjs") &&
          !f.includes("-extensions") &&
      ...
      .sort((a, b) => a.length - b.length);
    if (subFiles[0])
      return `server/lib/hsm-adapter/__tests__/${subdir.name}/${subFiles[0]}`;
  }
  return pattern;
}
...
if (useParallel) {
  ...
  runSuitesParallel(SUITES, { progress: true }).then(
    ({ results, totalMs, passed, failed, throughput }) => {
