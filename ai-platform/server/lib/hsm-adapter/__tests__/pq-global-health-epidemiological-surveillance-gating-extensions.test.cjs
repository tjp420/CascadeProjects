...
/**
 * Track 92: PQC Global Health Epidemiological Surveillance Gating & ZK Epidemiological
 * Claim Validators — extension tests.
 *
 * Tests the new batch pool initialization, genomic chain depth
 * rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch epidemiological claim verification, slashing
...
const {
  ...
  POOL_STATUS,
  REBALANCE_DIRECTION,
} = require('../pqc-global-health-epidemiological-surveillance-gating-hub.cjs');
const {
  ZkEpidemiologicalClaimValidator,
  CLAIM_STATUS,
  SLASH_REASON,
  HW_ACCEL_TYPES,
} = require('../zk-epidemiological-claim-validator.cjs');
...
function baseClaimRequest(poolId) {
  return {
    ...
    attestationAuthority: 'mock-authority',
    functionalEncryptionKeyDigest: 'partial-sig-001',
    surveillanceWindowSeconds: 302400,
  };
}
...
describe('Track 92 PQC Global Health Epidemiological Surveillance Gating extensions', () => {
  describe('PqcGlobalHealthEpidemiologicalSurveillanceGatingHub — genomic chain depth rebalancing', () => {
    test('rebalances genomic chain depth with increase direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceGenomicChainDepth({
    ...
    test('rebalances genomic chain depth with decrease direction', () => {
      const ctx = setupAndInitPool();
      const rebalance = ctx.hub.rebalanceGenomicChainDepth({
        poolId: ctx.pool.poolId,
    ...
    test('updates genomicChainDepth on rebalance when newGenomicChainDepth provided', () => {
      const ctx = setupAndInitPool();
      ctx.hub.rebalanceGenomicChainDepth({
        poolId: ctx.pool.poolId,
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 3,
        newGenomicChainDepth: 11,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.genomicChainDepth).toBe(11);
      expect(pool.rebalanceEpoch).toBe(1);
    });

    test('rejects rebalance with invalid direction', () => {
      const ctx = setupAndInitPool();
      expect(() => ctx.hub.rebalanceGenomicChainDepth({
        poolId: ctx.pool.poolId,
        direction: 'invalid',
        rebalanceAmount: 3,
    ...
    test('rejects rebalance with missing poolId', () => {
      ...
      expect(() => hub.rebalanceGenomicChainDepth({})).toThrow(HsmAdapterError);
    });
  ...
  describe('PqcGlobalHealthEpidemiologicalSurveillanceGatingHub — batch initialization', () => {
  ...
  describe('PqcGlobalHealthEpidemiologicalSurveillanceGatingHub — cross-chain settlement', () => {
  ...
  describe('PqcGlobalHealthEpidemiologicalSurveillanceGatingHub — committee aggregation', () => {
  ...
  describe('PqcGlobalHealthEpidemiologicalSurveillanceGatingHub — cancellation', () => {
  ...
  describe('PqcGlobalHealthEpidemiologicalSurveillanceGatingHub — queries and stats', () => {
  ...
  describe('ZkEpidemiologicalClaimValidator — HW-SNARK proof generation', () => {
    test('generates a hardware-accelerated SNARK proof', () => {
      ...
      expect(proof.zkEpidemiologicalRangeProofHash).toBeDefined();
    ...
    test('rejects proof generation with missing poolId', () => {
      ...
      expect(() => validator.generateHwSnarkProof({ genomicSequence: 100, claimValue: 50 }))
        .toThrow(HsmAdapterError);
    });
  ...
  describe('ZkEpidemiologicalClaimValidator — batch epidemiological claim verification', () => {
    test('batch verifies multiple epidemiological claims', () => {
      const ctx = setupHubAndValidator();
      const pools = [];
      for (let i = 0; i < 3; i++) {
      ...
      const result = ctx.validator.batchVerifyEpidemiologicalClaims(batch);
    ...
    test('rejects empty batch', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.batchVerifyEpidemiologicalClaims([])).toThrow(HsmAdapterError);
    });

    test('rejects batch exceeding max size', () => {
      const { validator } = setupHubAndValidator();
      const bigBatch = Array.from({ length: 101 }, () => baseClaimRequest('x'));
      expect(() => validator.batchVerifyEpidemiologicalClaims(bigBatch)).toThrow(HsmAdapterError);
    });
    ...
    test('records batch history', () => {
      ...
      r.peerId = 'p-bh';
      ctx.validator.batchVerifyEpidemiologicalClaims([r]);
      expect(ctx.validator.getBatchHistory().length).toBe(1);
    });
  });

  describe('ZkEpidemiologicalClaimValidator — partial signature aggregation', () => {
    test('aggregates functional encryption key digests', () => {
      const ctx = setupAndInitPool();
      const result = ctx.validator.aggregateFunctionalEncryptionKeyDigests(ctx.pool.poolId, [
        { peerId: 'p1', signature: 'sig-1' },
        { peerId: 'p2', signature: 'sig-2' },
        { peerId: 'p3', signature: 'sig-3' },
        { peerId: 'p4', signature: 'sig-4' },
    ...
    test('rejects aggregation with banned peer', () => {
      ...
      cReq.zkEpidemiologicalRangeProofHash = null;
      cReq.peerId = 'bad-peer';
      try { ctx.validator.verifyEpidemiologicalClaim(cReq); } catch (e) { /* expected */ }
      expect(ctx.validator.isPeerBanned('bad-peer')).toBe(true);
      expect(() => ctx.validator.aggregateFunctionalEncryptionKeyDigests(ctx.pool.poolId, [
    ...
    test('rejects aggregation with missing poolId', () => {
      const { validator } = setupHubAndValidator();
      expect(() => validator.aggregateFunctionalEncryptionKeyDigests('', [
        { peerId: 'p1', signature: 's1' },
      ])).toThrow(HsmAdapterError);
    });
  });

  describe('ZkEpidemiologicalClaimValidator — slashing window validation', () => {
  ...
  describe('ZkEpidemiologicalClaimValidator — slashing and stats', () => {
    ...
    test('records slashes for out-of-bounds transcript expiration', () => {
      ...
      cReq.surveillanceWindowSeconds = 999999999;
      try { ctx.validator.verifyEpidemiologicalClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('records slashes for duplicate claims', () => {
      ...
      ctx.validator.verifyEpidemiologicalClaim(cReq);
      try { ctx.validator.verifyEpidemiologicalClaim(cReq); } catch (e) { /* expected */ }
      const stats = ctx.validator.getSlashingStats();
      expect(stats.totalSlashes).toBeGreaterThan(0);
    });
    ...
    test('CLAIM_STATUS, SLASH_REASON, and HW_ACCEL_TYPES constants are exported', () => {
      ...
      expect(SLASH_REASON.SURVEILLANCE_WINDOW_OUT_OF_BOUNDS).toBe('surveillance_window_out_of_bounds');
  ...
  describe('full Track 92 extended flow', () => {
    test('complete init → rebalance → claim → accredit → settle flow', () => {
      ...
      expect(snarkProof.zkEpidemiologicalRangeProofHash).toBeDefined();
      const cReq = baseClaimRequest(pool.poolId);
      cReq.peerId = 'peer-claim';
      cReq.zkEpidemiologicalRangeProofHash = snarkProof.zkEpidemiologicalRangeProofHash;
      const claim = ctx.validator.verifyEpidemiologicalClaim(cReq);
