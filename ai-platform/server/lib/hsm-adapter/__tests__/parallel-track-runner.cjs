...
function runSuite(pattern) {
  ...
  return new Promise((resolve) => {
    exec(
      `npx jest ${target} --silent --coverage=false`,
      {
        cwd,
...
async function runSuitesParallel(suites, options = {}) {
  ...
  for (let i = 0; i < suites.length; i += concurrency) {
    ...
    const chunkResults = await Promise.all(
      chunk.map((pattern) => runSuite(pattern)),
    );
    if (progress) {
      for (const r of chunkResults) {
        console.log(
          `  [${results.length + chunkResults.indexOf(r) + 1}/${suites.length}] ${r.pattern.toLowerCase().replace(/\s+/g, "-")} -> ${r.status} (${r.durationMs}ms) [${results.length + chunkResults.indexOf(r) + 1} ${r.status === "PASS" ? "PASS" : "FAIL"}, 0 FAIL] ${((Date.now() - start) / 1000).toFixed(1)}s`,
        );
      }
    }
