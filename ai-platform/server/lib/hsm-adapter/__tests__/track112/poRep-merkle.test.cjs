...
function proofForIndex(layers, index) {
  ...
  for (let level = 0; level < layers.length - 1; level++) {
    ...
    const sibling = siblingIndex < layers[level].length ? layers[level][siblingIndex] : Buffer.alloc(32, 0);
...
describe('PoRep Merkle verification', () => {
  test('verifier accepts correct proof and rejects tampered leaf', async () => {
    ...
    const proof = { root, challenges: [{ index: idx, leaf: Buffer.from(leaves[idx]).toString('hex'), path }] };
    ...
    // Tamper leaf
    const badProof = { root, challenges: [{ index: idx, leaf: Buffer.from('corrupt').toString('hex'), path }] };
