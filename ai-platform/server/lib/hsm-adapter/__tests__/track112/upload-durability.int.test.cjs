...
describe('Track 396: Multipart upload durability (integration)', () => {
  ...
  it('persists tree-state.json after each chunk write', async () => {
    ...
    assert.ok(fs.existsSync(statePath), 'tree-state.json should exist after chunk writes');
  ...
  it('resumes session from checkpoint after simulated crash', async () => {
    ...
    const h1 = crypto.createHash('sha256').update(Buffer.from('alpha')).digest();
    const h2 = crypto.createHash('sha256').update(Buffer.from('beta')).digest();
    const expectedRoot = crypto.createHash('sha256').update(Buffer.concat([h1, h2])).digest('hex');
  ...
  it('rebuilds tree from chunk files when tree-state.json is missing (legacy session)', async () => {
    ...
    const h1 = crypto.createHash('sha256').update(Buffer.from('legacy-0')).digest();
    const h2 = crypto.createHash('sha256').update(Buffer.from('legacy-1')).digest();
    const expectedRoot = crypto.createHash('sha256').update(Buffer.concat([h1, h2])).digest('hex');
    assert.strictEqual(rootHex, expectedRoot);
  });
  ...
  it('commit produces correct Merkle root and moves to _committed', async () => {
    ...
    const h1 = crypto.createHash('sha256').update(Buffer.from('data-a')).digest();
    const h2 = crypto.createHash('sha256').update(Buffer.from('data-b')).digest();
    const expectedRoot = crypto.createHash('sha256').update(Buffer.concat([h1, h2])).digest('hex');

    // Sign the JCS canonical commit payload
    const keyPair = crypto.generateKeyPairSync('ed25519');
    const publicKeyPem = keyPair.publicKey.export({ type: 'spki', format: 'pem' });
    const commitPayload = canonicalize({ root: expectedRoot, sessionId: sid, tenant: 't1' });
    ...
    assert.ok(!fs.existsSync(path.join(base, 't1', sid)), 'session should be removed from active');

    // Verify committed meta
    const committedMeta = JSON.parse(fs.readFileSync(path.join(committedDir, 'meta.json'), 'utf8'));
      ...
      while (level.length > 1) {
        ...
        for (let i = 0; i < level.length; i += 2) {
          if (i + 1 < level.length) {
            ...
            next.push(crypto.createHash('sha256').update(Buffer.concat([l, r])).digest('hex'));
          } else {
            next.push(level[i]);
          }
        }
  ...
  it('atomic chunk write does not leave temp files on success', async () => {
    ...
    assert.strictEqual(tempFiles.length, 0, 'no temp files should remain after successful write');
  });
  ...
  it('durability telemetry counters are incremented', async () => {
    ...
    const metrics = hsmMetrics.getMetrics();
    assert.ok(metrics.hsm_track112_chunk_write_atomic_total >= 2, 'atomic write counter should be >= 2');
    assert.ok(metrics.hsm_track112_merkle_leaf_hashed_total >= 2, 'merkle leaf hashed counter should be >= 2');
  });
});
