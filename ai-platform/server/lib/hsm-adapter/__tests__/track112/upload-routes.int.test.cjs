...
describe("Track112 upload routes (integration)", () => {
  ...
  test("create -> chunk -> commit flow increments counters", async () => {
    ...
    expect(
      hsmMetrics.getMetrics().hsm_track112_upload_commit_failed_total,
    ).toBe(0);
  }, 20000);
  ...
  test("invalid signature increments failure counter", async () => {
    ...
    const fakePublicKeyPem = fakeKey.publicKey.export({
      type: "spki",
      format: "pem",
    });
    const fakeSignature = crypto
      .sign(null, Buffer.from("nope"), fakeKey.privateKey)
      .toString("base64");
  ...
  test("chunk write to non-existent session increments chunk failure counter", async () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_track112_upload_chunk_failed_total).toBe(
      1,
    );
