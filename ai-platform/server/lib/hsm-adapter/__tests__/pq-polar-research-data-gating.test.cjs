...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyResearchClaim(
...
describe("Track 96 PQ polar research data gating", () => {
  test("initializes a pool and emits POLAR_RESEARCH_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "POLAR_RESEARCH_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("verifies a research claim and emits ZK_RESEARCH_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_RESEARCH_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits DATA_ACCREDITATION_COMPLETED", () => {
    ...
    expect(events.some((e) => e.event === "DATA_ACCREDITATION_COMPLETED")).toBe(
      true,
    );
  });
  ...
  test("rejects data retention window exceeding maximum", () => {
    ...
    req.dataRetentionWindowSeconds = 99999999;
  ...
  test("rejects un-attested Antarctic Treaty Secretariat initializer", () => {
    ...
    const req = baseInitRequest();
    req.antarcticTreatySecretariatInitializerAttestation = { authority: "bad" };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested polar research oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkResearchClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.polarResearchOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkResearchRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkResearchRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("CryptoPolicyEngine validates pq polar research gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
        polarQuorum: 5,
        dataRetentionWindowSeconds: 3888000,
        researchChainDepth: 7,
        pqcSignatureScheme: "ML-DSA-65",
        antarcticTreatySecretariatInitializerAttestation: true,
        polarResearchOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderResearchClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", { polarQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
        dataRetentionWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
        researchChainDepth: 30,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
        antarcticTreatySecretariatInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
        polarResearchOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
        banMalformedOrOutOfOrderResearchClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqPolarResearchGating", {
