...
const {
  ZkSettlementEqualityProver,
} = require("../zk-settlement-equality-prover.cjs");
const {
...
describe("Track 50 ZK cross-chain settlement", () => {
  test("ZkSettlementBroker initiates and finalizes a balanced settlement", () => {
    ...
    expect(
      events.some((e) => e.event === "CROSS_CHAIN_SETTLEMENT_INITIATED"),
    ).toBe(true);
    ...
    expect(events.some((e) => e.event === "ZK_SETTLEMENT_FINALIZED")).toBe(
      true,
    );
  });
  ...
  test("ZkSettlementBroker rejects un-attested node", () => {
    ...
    broker.initiate(baseRequest());
    expect(() =>
      broker.sign("settle-1", "node-a", { authority: "bad" }, "sig-a"),
  ...
  test("CryptoPolicyEngine validates ZK settlement configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "zkSettlement", {
    ...
    expect(() =>
      engine.validate("t1", "zkSettlement", { clearingNodeQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { settlementTimeoutSeconds: 600 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { assetBitWidth: 4 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { assetBitWidth: 512 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { nodeAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { nodeAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { equalityProof: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkSettlement", { canonicalPayloadLayout: false }),
    ).toThrow(HsmAdapterError);
  });
});
