...
describe('Track 34 Phase 6 snapshotting — post-compaction operations', () => {
  test('can append new entries after snapshot', async () => {
    ...
    const result = await engine.appendAndReplicate({ operation: 'post-snapshot' });
...
describe('Track 34 Phase 6 snapshotting — installSnapshot', () => {
  ...
  test('installSnapshot rejects stale term', async () => {
    ...
    follower.appendEntries({ ...aePayload, signature: signPayload(aePayload, leaderKeys.privateKey) });
  ...
  test('installSnapshot rejects older snapshot', async () => {
    ...
    follower.installSnapshot({ ...payload1, signature: signPayload(payload1, leaderKeys.privateKey) });
    ...
    const result = follower.installSnapshot({ ...payload2, signature: signPayload(payload2, leaderKeys.privateKey) });
    expect(result.success).toBe(false);
  ...
  test('installSnapshot rejects missing inputs', () => {
    ...
    expect(() => follower.installSnapshot({ term: 1, leaderId: 'node-a' })).toThrow(HsmAdapterError);
  });
  ...
  test('installSnapshot rejects unknown leader', () => {
    ...
    expect(() => follower.installSnapshot({
      term: 1,
      leaderId: 'node-z',
      lastIncludedIndex: 10,
      lastIncludedTerm: 1,
  ...
  test('installSnapshot restores state via callback', async () => {
    ...
    const follower = new ClusterConsensusEngine({
      ...
      requireRpcSigning: true,
      replayWindowMs: 60000,
      restoreSnapshotState: (state) => { restoredState = state; },
    });
    follower.start();
    ...
    follower.installSnapshot({ ...payload, signature: signPayload(payload, leaderKeys.privateKey) });
  ...
  test('installSnapshot keeps compatible log entries after snapshot point', async () => {
    ...
    for (let i = 0; i < 15; i++) {
      follower._log.push({ term: 1, index: i + 1, command: { seq: i }, committed: false });
    }
    ...
    const result = follower.installSnapshot({ ...payload, signature: signPayload(payload, leaderKeys.privateKey) });
    expect(result.success).toBe(true);
...
describe('Track 34 Phase 6 snapshotting — audit events', () => {
  test('SNAPSHOT_CREATED event emitted on createSnapshot', async () => {
    ...
    expect(events.some((e) => e.event === CONSENSUS_EVENT.SNAPSHOT_CREATED)).toBe(true);
    engine.stop();
  });
  ...
  test('SNAPSHOT_INSTALLED event emitted on installSnapshot', async () => {
    ...
    follower.installSnapshot({ ...payload, signature: signPayload(payload, leaderKeys.privateKey) });
    expect(events.some((e) => e.event === CONSENSUS_EVENT.SNAPSHOT_INSTALLED)).toBe(true);
    follower.stop();
  });
  ...
  test('SNAPSHOT_REJECTED event emitted on rejected snapshot', async () => {
    ...
    const p1 = {
      term: 1, leaderId: 'node-a', lastIncludedIndex: 20, lastIncludedTerm: 1,
      state: {}, nonce: 1, timestamp: Date.now(),
    };
    follower.installSnapshot({ ...p1, signature: signPayload(p1, leaderKeys.privateKey) });

    // Try older snapshot
    const p2 = {
      term: 1, leaderId: 'node-a', lastIncludedIndex: 10, lastIncludedTerm: 1,
      state: {}, nonce: 2, timestamp: Date.now(),
    };
    follower.installSnapshot({ ...p2, signature: signPayload(p2, leaderKeys.privateKey) });

    expect(events.some((e) => e.event === CONSENSUS_EVENT.SNAPSHOT_REJECTED)).toBe(true);
    follower.stop();
  });
});
...
describe('Track 34 Phase 6 snapshotting — Prometheus metrics', () => {
  ...
  test('installSnapshot increments hsm_consensus_snapshot_installed_total', async () => {
    ...
    const payload = {
      term: 1, leaderId: 'node-a', lastIncludedIndex: 5, lastIncludedTerm: 1,
      state: {}, nonce: 1, timestamp: Date.now(),
    };
    follower.installSnapshot({ ...payload, signature: signPayload(payload, leaderKeys.privateKey) });
  ...
  test('rejected snapshot increments hsm_consensus_snapshot_rejected_total', async () => {
    ...
    // Sign with wrong key
    follower.installSnapshot({ ...payload, signature: signPayload(payload, attackerKeys.privateKey) });
...
describe('Track 34 Phase 6 snapshotting — policy validation', () => {
  ...
  test('tenant can tighten snapshot threshold bounds', () => {
    ...
    // Below tenant min
    expect(() => engine.validate('t-tight', 'consensus', {
...
describe('Track 34 Phase 6 snapshotting — end-to-end compaction flow', () => {
  test('leader compacts, continues appending, follower catches up via snapshot', async () => {
    ...
    leaderCounter++;
    const postResult = await leader.appendAndReplicate({ operation: 'increment' });
    ...
    const follower = new ClusterConsensusEngine({
      ...
      requireRpcSigning: true,
      replayWindowMs: 60000,
      restoreSnapshotState: (state) => { leaderCounter = state.counter; },
    });
    follower.start();
    ...
    const installResult = follower.installSnapshot({ ...payload, signature: signPayload(payload, leaderKeys.privateKey) });
    ...
    const aePayload = {
      term: leader.getState().term,
      leaderId: 'node-a',
      entries: [{ term: leader.getState().term, index: 6, command: { operation: 'increment' } }],
      leaderCommit: 6,
      nonce: 2,
      timestamp: Date.now(),
    };
    const aeResult = follower.appendEntries({ ...aePayload, signature: signPayload(aePayload, leaderKeys.privateKey) });
