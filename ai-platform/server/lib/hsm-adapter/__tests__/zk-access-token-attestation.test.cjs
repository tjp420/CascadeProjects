...
const {
  ZkAttestationContractVerifier,
} = require("../zk-attestation-contract-verifier.cjs");
const {
...
describe("Track 52 ZK access token attestation", () => {
  ...
  test("ZkAttestationContractVerifier generates and verifies a proof", () => {
    ...
    expect(
      events.some((e) => e.event === "ATTESTATION_CONTRACT_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("ZkAttestationContractVerifier rejects expired token and bans node", () => {
    ...
    const proof = verifier.generate(token);
    expect(() =>
      verifier.verify(token, proof, mockAttestation(), 9999),
  ...
  test("CryptoPolicyEngine validates ZK token attestation configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", {
        signatureQuorum: 3,
        tokenLifetimeSeconds: 3600,
        curve: "P-256",
    ...
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", { signatureQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", {
    ...
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", { curve: "secp256k1" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", { brokerAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", {
    ...
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", { scopesPerToken: 10 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", {
        banExpiredProofNodes: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "zkTokenAttestation", {
