...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyProvenanceClaim(
...
describe("Track 76 PQ supply chain provenance gating", () => {
  test("PqcSupplyChainProvenanceGatingHub initializes a pool and emits SUPPLY_CHAIN_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "SUPPLY_CHAIN_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkProvenanceClaimValidator verifies a provenance claim and emits ZK_PROVENANCE_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyProvenanceClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(events.some((e) => e.event === "ZK_PROVENANCE_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcSupplyChainProvenanceGatingHub completes accreditation after claim and emits COMPONENT_LINEAGE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some(
        (e) => e.event === "COMPONENT_LINEAGE_ACCREDITATION_COMPLETED",
      ),
    ).toBe(true);
  });
  ...
  test("ZkProvenanceClaimValidator rejects un-attested clearing committee", () => {
    ...
    expect(() => validator.verifyProvenanceClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq supply chain gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
        transitExpirationSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
        componentLineageDepth: 128,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
        factoryEndpointInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
        banMalformedOrOutOfOrderProvenanceClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainGating", {
