...
const {
  PqcNeuralNetworkInferenceIntegrityGatingHub,
} = require("../pqc-neural-network-inference-integrity-gating-hub.cjs");
...
function baseInitRequest() {
  return {
    ...
    blindedNeuralMeasurementCommitment: "pedersen-neural-measurement-001",
    blindedInferenceProbabilityCommitment: "pedersen-inference-prob-001",
    blindedNeuralNetworkAuthorityIdentityCommitment: "pedersen-neural-auth-001",
    inferenceWindowSeconds: 302400,
    inferenceChainDepth: 16,
    pqcSignatureScheme: "ML-DSA-65",
    neuralNetworkAuthorityInitializerAttestation: mockAttestation(),
...
function baseClaimRequest(poolId) {
  return {
    ...
    blindedInferenceProbabilityCommitment: "pedersen-inference-prob-001",
    blindedNeuralNetworkAuthorityIdentityCommitment: "pedersen-neural-auth-001",
    zkNeuralRangeProofHash: "zk-neural-proof-001",
    merkleMountainRangeDigest: "mmr-digest-001",
    neuralEthicsOversightCommitteeAttestation: mockAttestation(),
    attestationAuthority: "mock-authority",
  };
}
...
function setupHubAndValidator() {
  ...
  const hub = new PqcNeuralNetworkInferenceIntegrityGatingHub({
    policy: POLICY,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
  });
  const validator = new ZkNeuralClaimValidator({
    policy: POLICY,
    hub,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyNeuralClaim(
...
describe("Track 101 PQ neural network inference integrity gating", () => {
  test("initializes a pool and emits NEURAL_INFERENCE_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "NEURAL_INFERENCE_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("verifies a neural claim and emits ZK_NEURAL_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_NEURAL_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits INFERENCE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "INFERENCE_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects inference window exceeding maximum", () => {
    ...
    req.inferenceWindowSeconds = 99999999;
  ...
  test("rejects un-attested neural network authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.neuralNetworkAuthorityInitializerAttestation = { authority: "bad" };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested neural ethics oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkNeuralClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.neuralEthicsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkNeuralRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkNeuralRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing merkleMountainRangeDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.merkleMountainRangeDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyNeuralClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq neural network inference integrity gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
        neuralQuorum: 8,
        inferenceWindowSeconds: 302400,
        inferenceChainDepth: 16,
        pqcSignatureScheme: "ML-DSA-65",
        neuralNetworkAuthorityInitializerAttestation: true,
        neuralEthicsOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderNeuralClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
        inferenceWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
        inferenceChainDepth: 30,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
        neuralNetworkAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
        neuralEthicsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
        banMalformedOrOutOfOrderNeuralClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqNeuralNetworkInferenceIntegrityGating", {
