...
describe('CryptographicRatchet', () => {
  ...
  test('multiple messages advance message keys', () => {
    ...
    const plaintexts = [crypto.randomBytes(64), crypto.randomBytes(128), crypto.randomBytes(32)];
    const aad = 'multi';
  ...
  test('session expiry throws SESSION_EXPIRED', () => {
    const rootKey = crypto.randomBytes(32);
    const alice = new CryptographicRatchet(rootKey, { sessionExpiryMs: 1, createdAt: Date.now() - 2 });
...
describe('RatchetMessageHandler', () => {
  ...
  test('rejects gaps larger than maxSkipped', () => {
    ...
    expect(() => handler.decrypt(bob, envelopes[3], aad)).toThrow(HsmAdapterError);
  });
...
describe('Policy and adapter integration', () => {
  ...
  test('CryptoPolicyEngine ratchet policy rejects oversized maxSkipped', () => {
    ...
    expect(() => policy.validate('t1', 'ratchet', { maxSkipped: 101 })).toThrow(HsmAdapterError);
  });
