...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyQkdLinkClaim(
...
describe("Track 109 PQ quantum key distribution link-switch gating", () => {
  test("initializes a pool and emits QKD_LINK_POOL_INITIALIZED", () => {
    ...
    expect(events.some((e) => e.event === "QKD_LINK_POOL_INITIALIZED")).toBe(
      true,
    );
  });
  ...
  test("verifies a qkd link claim and emits ZK_QKD_LINK_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_QKD_LINK_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits ENTANGLEMENT_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "ENTANGLEMENT_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects entanglement window exceeding maximum", () => {
    ...
    req.entanglementWindowSeconds = 999999;
  ...
  test("rejects un-attested qkd link authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.qkdLinkAuthorityInitializerAttestation = { authority: "bad" };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested qkd ethics oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkQkdLinkClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.qkdEthicsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkQkdLinkRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkQkdLinkRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing quantumSecretSharingDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.quantumSecretSharingDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyQkdLinkClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq quantum key distribution link-switch gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
        qkdQuorum: 18,
        entanglementWindowSeconds: 60,
        qkdSwitchChainDepth: 22,
        pqcSignatureScheme: "ML-DSA-65",
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
        entanglementWindowSeconds: 999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
        qkdSwitchChainDepth: 44,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
        qkdLinkAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
        qkdEthicsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
        banMalformedOrOutOfOrderQkdLinkClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqQuantumKeyDistributionLinkSwitchGating", {
