...
const { ZkMeshReconciliationClaimValidator } = require(
  path.join(__dirname, "..", _zkfn),
);
...
function _validateIpcMessage(msg) {
  ...
  if (msg.type === "run_batch") {
    ...
    const allowed = new Set([
      ...Object.keys(RUN_BATCH_SCHEMA.required),
      ...Object.keys(RUN_BATCH_SCHEMA.optional),
    ]);
...
function runBatch(job) {
  const { workerId, batchSize, delayMs, timestampOffsetMs, reconcileValid } =
    job;
  hsmMetrics.reset();
  const validator = new ZkMeshReconciliationClaimValidator({
