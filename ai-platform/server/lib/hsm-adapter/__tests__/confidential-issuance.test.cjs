...
describe("Track 44 confidential token issuance", () => {
  test("ConfidentialTokenIssuer mints an attested token and emits telemetry", () => {
    ...
    const result = issuer.mint("gold", 2n ** 260n, mockAttestation(), [
      "a",
      "b",
    ]);
    ...
    expect(events.some((e) => e.event === "CONFIDENTIAL_TOKEN_MINTED")).toBe(
      true,
    );
  });
  ...
  test("ConfidentialTokenIssuer rejects un-attested mint", () => {
    ...
    expect(() =>
      issuer.mint("gold", 2n ** 260n, { authority: "bad" }, ["a", "b"]),
  ...
  test("ConfidentialTokenIssuer rejects amount below bit length", () => {
    ...
    expect(() =>
      issuer.mint("gold", 100n, mockAttestation(), ["a", "b"]),
    ).toThrow(HsmAdapterError);
  });
  ...
  test("ConfidentialTokenIssuer rejects quorum below minimum", () => {
    ...
    expect(() =>
      issuer.mint("gold", 2n ** 260n, mockAttestation(), ["a"]),
    ).toThrow(HsmAdapterError);
  });
  ...
  test("TokenClaimVerifier validates a fresh ownership claim", () => {
    ...
    const { token, amount, blinding } = issuer.mint(
      "gold",
      2n ** 260n,
      mockAttestation(),
      ["a", "b"],
    ...
    expect(events.some((e) => e.event === "ISSUANCE_PROOF_VALIDATED")).toBe(
      true,
    );
  });
  ...
  test("TokenClaimVerifier rejects stale proof", () => {
    ...
    const { token } = issuer.mint("gold", 2n ** 260n, mockAttestation(), [
      "a",
      "b",
    ]);
    ...
    expect(() =>
      verifier.verify(token, { commitment: token.commitment }),
    ).toThrow(HsmAdapterError);
  });
  ...
  test("TokenClaimVerifier rejects commitment mismatch", () => {
    ...
    const verifier = new TokenClaimVerifier({ policy: POLICY });
    expect(() => verifier.verify(token, { commitment: "wrong" })).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates confidential issuance configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "confidentialIssuance", {
        tokenBitLength: 256,
    ...
    expect(() =>
      engine.validate("t1", "confidentialIssuance", { tokenBitLength: 128 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", {
        blindingScheme: "plain",
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", {
        mintingAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", {
        mintingAuthority: "bad",
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", { zkSnarkProof: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", { proofAgeSeconds: 120 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", {
        commitmentCurve: "ed25519",
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "confidentialIssuance", { issuanceQuorum: 1 }),
    ).toThrow(HsmAdapterError);
  });
});
