...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyEnergyClaim(
...
describe("Track 75 PQ energy certificate gating", () => {
  test("PqcEnergyCertificateGatingHub initializes a pool and emits ENERGY_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "ENERGY_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkEnergyClaimValidator verifies an energy claim and emits ZK_ENERGY_CLAIM_VERIFIED", () => {
    ...
    expect(events.some((e) => e.event === "ZK_ENERGY_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("PqcEnergyCertificateGatingHub completes accreditation after claim and emits CERTIFICATE_TRADING_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some(
        (e) => e.event === "CERTIFICATE_TRADING_ACCREDITATION_COMPLETED",
      ),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq energy gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqEnergyGating", { gridOperatorQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
        certificateExpirationSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEnergyGating", { productionMetricDepth: 96 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
        gridOperatorInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqEnergyGating", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
        banMalformedOrOutOfOrderEnergyClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEnergyGating", {
