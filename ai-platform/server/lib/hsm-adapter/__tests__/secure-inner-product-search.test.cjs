...
describe("Track 52: SecureInnerProductSearch", () => {
  ...
  describe("registerParty", () => {
    ...
    test("enforces max parties", () => {
      const small = new SecureInnerProductSearch({
        minParties: 1,
        maxParties: 2,
      });
  ...
  describe("buildIndex", () => {
    ...
    test("rejects empty index ID", () => {
      expect(() =>
        sip.buildIndex("", [{ id: "doc1", keywords: ["a"] }]),
    ...
    test("rejects duplicate index", () => {
      ...
      expect(() =>
        sip.buildIndex("idx1", [{ id: "doc2", keywords: ["b"] }]),
      ).toThrow(HsmAdapterError);
    });
    ...
    test("rejects document without id", () => {
      expect(() => sip.buildIndex("idx1", [{ keywords: ["a"] }])).toThrow(
        HsmAdapterError,
      );
    });

    test("rejects document without keywords", () => {
      expect(() => sip.buildIndex("idx1", [{ id: "doc1" }])).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects insufficient parties", () => {
      ...
      small.registerParty("p1");
      expect(() =>
        small.buildIndex("idx1", [{ id: "doc1", keywords: ["a"] }]),
      ).toThrow(HsmAdapterError);
    });
  });
