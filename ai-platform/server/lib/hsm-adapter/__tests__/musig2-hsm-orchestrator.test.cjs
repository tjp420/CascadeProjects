...
const { Musig2HsmOrchestrator, SESSION_STATE, ORCHESTRATOR_EVENT } = require('../musig2-hsm-orchestrator.cjs');
const { registerMusig2Orchestrator, getMusig2Orchestrator, HsmAdapterError } = require('../base-adapter.cjs');
...
function createMockHsmAdapter() {
  ...
  return {
    ...
    async createKEK(tenantId, meta = {}) {
      const kekId = (meta && meta.label) || 'kek-' + crypto.randomBytes(8).toString('hex');
    ...
    async wrap(tenantId, kekId, plaintext) {
      ...
      wrapped.set(id, { kekId, tenantId, plaintext: Buffer.from(plaintext), cipher });
...
async function runFullRoundTrip(orch, { tenantId = 'tenant-1', participants = 3, keyShares = null } = {}) {
...
describe('L1 — Deterministic syntax checks', () => {
  ...
  it('L1-06: schnorr math engine files unchanged (READ-ONLY)', () => {
    ...
    const files = ['protocol.cjs', 'signature_share.cjs', 'nonce.cjs', 'field.cjs'];
    for (const f of files) {
      assert.ok(fs.existsSync(path.join(schnorrDir, f)), `schnorr file missing: ${f}`);
    }
  });
  ...
  it('L1-09: hsm-metrics defines new orchestrator counters', () => {
    ...
    assert.strictEqual(typeof counters.hsm_musig2_orch_session_created_total, 'number');
    assert.strictEqual(typeof counters.hsm_musig2_orch_session_completed_total, 'number');
    assert.strictEqual(typeof counters.hsm_musig2_orch_session_failed_total, 'number');
    assert.strictEqual(typeof counters.hsm_musig2_orch_key_share_wrapped_total, 'number');
  });
});
...
describe('L2 — Behavioral: session lifecycle and signing flow', () => {
  ...
  beforeEach(() => {
    ...
    orch = new Musig2HsmOrchestrator({ hsmAdapter: mockAdapter, modulus: SECP256K1_Q });
  });
  ...
  it('L2-01: createSession with valid params returns sessionId', async () => {
    ...
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_orch_session_created_total, 1);
  });
  ...
  it('L2-02: generateNonces returns public commitments and advances state', async () => {
    ...
    assert.strictEqual(orch.getSessionStatus(sessionId).state, SESSION_STATE.NONCES_GENERATED);
  });
  ...
  it('L2-03: aggregateKeys returns aggregated public key and advances state', async () => {
    ...
    assert.ok(aggKey >= 0n);
    assert.strictEqual(orch.getSessionStatus(sessionId).state, SESSION_STATE.KEYS_AGGREGATED);
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_key_aggregation_total, 1);
  });
  ...
  it('L2-04: computeBindingFactor returns binding factor and advances state', async () => {
    ...
    assert.strictEqual(orch.getSessionStatus(sessionId).state, SESSION_STATE.BINDING_COMPUTED);
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_binding_factor_computed_total, 1);
  });
  ...
  it('L2-05: evaluateShares returns partial shares and advances state', async () => {
    ...
    assert.strictEqual(orch.getSessionStatus(sessionId).state, SESSION_STATE.SHARES_EVALUATED);
  });
  ...
  it('L2-06: assembleSignature returns {R, s} and advances state', async () => {
    ...
    assert.strictEqual(orch.getSessionStatus(sessionId).state, SESSION_STATE.SIGNATURE_ASSEMBLED);
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_signature_assembled_total, 1);
  });
  ...
  it('L2-07: verifySignature returns {valid: true} and advances to VERIFIED', async () => {
    ...
    assert.strictEqual(orch.getSessionStatus(sessionId).state, SESSION_STATE.VERIFIED);
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_signature_verified_total, 1);
  });
  ...
  it('L2-10: wrapKeyShare returns wrapped blob and increments counter', async () => {
    ...
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_orch_key_share_wrapped_total, 1);
  });
  ...
  it('L2-15: destroySession removes session from registry', async () => {
    ...
    assert.throws(() => orch.getSessionStatus(sessionId), assertErrorCode('SESSION_NOT_FOUND'));
  });
  ...
  it('L2-18: route file exposes GET /musig2/session/:sessionId/status endpoint', () => {
    ...
    assert.ok(routeSrc.includes("router.get('/musig2/session/:sessionId/status'"));
  });
  ...
  it('L2-19: route file exposes POST /musig2/session/:sessionId/sign endpoint', () => {
    ...
    assert.ok(routeSrc.includes("router.post('/musig2/session/:sessionId/sign'"));
  });
});
...
describe('L3 — Edge cases & regression', () => {
  ...
  it('L3-02: createSession with empty participantIds throws INVALID_INPUT', async () => {
    await assert.rejects(
      () => orch.createSession({ tenantId: 't1', participantIds: [], messageHash: 'abc' }),
      assertErrorCode('INVALID_INPUT')
    );
  });
  ...
  it('L3-03: createSession with quorum larger than participants throws INVALID_INPUT', async () => {
    await assert.rejects(
      () => orch.createSession({
        tenantId: 't1', participantIds: [1, 2], quorum: [1, 2, 3], messageHash: 'abc',
      }),
      assertErrorCode('INVALID_INPUT')
    );
  });
  ...
  it('L3-05: aggregateKeys before generateNonces throws INVALID_STATE', async () => {
    const sessionId = await orch.createSession({
      tenantId: 't1', participantIds: [1, 2], quorum: [1, 2], messageHash: 'abc',
    });
    await assert.rejects(
  ...
  it('L3-07: session timeout auto-destroys idle session', async () => {
    ...
    assert.throws(
      ...
      (err) => err.code === 'SESSION_TIMEOUT' || err.code === 'SESSION_NOT_FOUND'
    );
  });
  ...
  it('L3-08: max sessions exceeded throws MAX_SESSIONS_EXCEEDED', async () => {
    ...
    await smallOrch.createSession({ tenantId: 't1', participantIds: [1, 2], quorum: [1, 2], messageHash: 'a' });
    await smallOrch.createSession({ tenantId: 't1', participantIds: [1, 2], quorum: [1, 2], messageHash: 'b' });
    await assert.rejects(
      () => smallOrch.createSession({ tenantId: 't1', participantIds: [1, 2], quorum: [1, 2], messageHash: 'c' }),
  ...
  it('L3-11: verify invalid signature returns {valid: false} and increments failure counter', async () => {
    const sessionId = await orch.createSession({
      tenantId: 't1', participantIds: [1, 2, 3], quorum: [1, 2, 3],
      messageHash: 'abc', keyShares: [100n, 200n, 300n],
    });
    await orch.generateNonces(sessionId);
    await orch.aggregateKeys(sessionId);
    await orch.computeBindingFactor(sessionId);
    ...
    assert.ok(hsmMetrics.getMetrics().hsm_musig2_signature_verification_failed_total >= 1);
  });
  ...
  it('L3-13: existing schnorr tests still pass (regression)', () => {
    const schnorrTestPath = path.resolve(SERVER_ROOT, 'lib/mpc/schnorr/__tests__/schnorr.test.cjs');
  ...
  it('L3-14: hsm-metrics counters are properly reset between tests', () => {
    hsmMetrics.reset();
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_orch_session_created_total, 0);
    assert.strictEqual(hsmMetrics.getMetrics().hsm_musig2_signature_verified_total, 0);
  });
  ...
  it('L3-15: nonce zeroization after share evaluation (S-03)', async () => {
    ...
    for (const nonce of session.nonces) {
      assert.strictEqual(nonce.secret.k1, 0n, 'k1 zeroized after evaluateShares');
      assert.strictEqual(nonce.secret.k2, 0n, 'k2 zeroized after evaluateShares');
    }
  });
});
...
describe('Security — S-01 through S-07', () => {
  ...
  it('S-01: no credentials/PII in audit events', async () => {
    ...
    await auditedOrch.createSession({
      tenantId: 'tenant-1', participantIds: [1, 2], quorum: [1, 2],
      messageHash: 'abc', keyShares: [100n, 200n],
    });
  ...
  it('S-02: key shares wrapped via HSM KEK before storage (never plaintext)', async () => {
    const sessionId = await orch.createSession({
      tenantId: 't1', participantIds: [1, 2], quorum: [1, 2],
      messageHash: 'abc', keyShares: [12345n, 67890n],
    });
    const session = orch._sessions.get(sessionId);
  ...
  it('S-06: no private key material in route responses or metrics', () => {
    ...
    assert.ok(!counterStr.match(/[0-9a-f]{64}/), 'no hex key material in metrics');
  });
  ...
  it('S-07: audit callback receives events without sensitive payload values', async () => {
    ...
    const sessionId = await auditedOrch.createSession({
      tenantId: 't1', participantIds: [1, 2, 3], quorum: [1, 2, 3],
      messageHash: 'abc', keyShares: [111n, 222n, 333n],
    });
