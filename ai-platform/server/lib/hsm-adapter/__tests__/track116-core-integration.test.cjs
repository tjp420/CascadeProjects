...
process.env.KEY_ROTATION_STORE_PATH = path.join(
  require("os").tmpdir(),
  "sb-track116-core-key.json",
);
process.env.AUDIT_LOG_PATH = path.join(
  require("os").tmpdir(),
  "sb-track116-core-audit.json",
);
...
describe("Track 116 core integration", () => {
  ...
  test("CORE-116-02: non-leader KEY_COMMIT triggers KEY_REJECT and increments hsm_key_reject_total", () => {
    ...
    const rejectEvents = clusterSync.queryEvents({
      eventType: clusterSync.EVENT_TYPES.KEY_REJECT,
    });
  ...
  test("CORE-116-04: DKG non-leader rejected when allowDkgNonLeaderMessages is false (default)", () => {
    ...
    const dkgEvents = clusterSync.queryEvents({
      eventType: clusterSync.EVENT_TYPES.DKG_INVALID_MESSAGE,
    });
    const notLeaderEvents = dkgEvents.events.filter(
      (e) => e.details.reason === "not_leader",
    );
