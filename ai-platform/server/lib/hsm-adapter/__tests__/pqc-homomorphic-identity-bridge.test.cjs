 ...
 */
const {
  PqcHomomorphicIdentityBridgeHub,
} = require("../pqc-homomorphic-identity-bridge-hub.cjs");
const {
  MpcHomomorphicConsensusVerifier,
} = require("../mpc-homomorphic-consensus-verifier.cjs");
const {
...
describe("Track 60 PQC homomorphic identity bridge", () => {
  test("PqcHomomorphicIdentityBridgeHub initializes a bridge and emits HOMOMORPHIC_IDENTITY_BRIDGE_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "HOMOMORPHIC_IDENTITY_BRIDGE_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("MpcHomomorphicConsensusVerifier processes a valid cross-chain identity assertion", () => {
    ...
    const bridge = hub.initializeBridge(baseInitRequest());
    const assertion = verifier.processAssertion(
      baseAssertionRequest(bridge.bridgeId, "entity-001"),
    );
  ...
  test("MpcHomomorphicConsensusVerifier finalizes consensus after quorum and emits MPC_CROSS_CHAIN_CONSENSUS_FINALIZED", () => {
    ...
    const bridge = hub.initializeBridge(baseInitRequest());
    verifier.processAssertion(
      baseAssertionRequest(bridge.bridgeId, "entity-001"),
    );
    verifier.processAssertion(
      baseAssertionRequest(bridge.bridgeId, "entity-002"),
    );
    verifier.processAssertion(
      baseAssertionRequest(bridge.bridgeId, "entity-003"),
    );
    const result = verifier.checkAndFinalize(bridge.bridgeId);
    expect(result.finalized).toBe(true);
    expect(result.assertionCount).toBe(3);
    expect(
      events.some((e) => e.event === "MPC_CROSS_CHAIN_CONSENSUS_FINALIZED"),
    ).toBe(true);
  });
  ...
  test("PqcHomomorphicIdentityBridgeHub evaluateEncryptedDotProduct rejects mismatched vectors", () => {
    const hub = new PqcHomomorphicIdentityBridgeHub({ policy: POLICY });
    expect(() => hub.evaluateEncryptedDotProduct([1, 2], [1, 2, 3])).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pqc homomorphic identity bridge configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
        crossChainQuorum: 3,
    ...
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
        homomorphicMatrixDepth: 64,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
        identityProofWindowSeconds: 200000,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
    ...
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
        routerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
        committeeVerifierAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
    ...
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
        banMalformedOrOutOfOrderProofs: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcHomomorphicIdentityBridge", {
