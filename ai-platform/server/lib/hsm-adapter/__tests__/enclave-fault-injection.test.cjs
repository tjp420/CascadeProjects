...
describe('Track 48: EnclaveFaultInjection', () => {
  ...
  describe('injectFault', () => {
    ...
    test('enforces max concurrent faults', () => {
      ...
      small.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
      small.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e2' });
      expect(() => small.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e3' }))
        .toThrow(HsmAdapterError);
    });

    test('rejects disabled fault type', () => {
      const restricted = new EnclaveFaultInjection({ enableCrashFaults: false });
      expect(() => restricted.injectFault({
        faultType: FAULT_TYPE.ENCLAVE_CRASH,
        targetEnclaveId: 'e1',
      })).toThrow(HsmAdapterError);
    });
  ...
  describe('cancelFault', () => {
    test('cancels an active fault', () => {
      ...
      expect(engine.getFault(result.faultId).status).toBe(FAULT_STATUS.CANCELLED);
    });
  ...
  describe('resolveFault', () => {
    test('resolves an active fault', () => {
      ...
      expect(engine.getFault(result.faultId).status).toBe(FAULT_STATUS.RESOLVED);
    });
  ...
  describe('checkExpiredFaults', () => {
    test('expires faults past their duration', () => {
      ...
      fast.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
  ...
  describe('chaosStep', () => {
    ...
    test('deterministic mode produces reproducible results', () => {
      const seed1 = new EnclaveFaultInjection({ chaosProbability: 1.0, deterministicSeed: 99 });
      const seed2 = new EnclaveFaultInjection({ chaosProbability: 1.0, deterministicSeed: 99 });
  ...
  describe('runScenario', () => {
    test('runs a multi-step scenario', () => {
      const result = engine.runScenario({
        ...
        steps: [
          { faultType: FAULT_TYPE.BYZANTINE_EQUIVOCATION, targetEnclaveId: 'e1' },
    ...
    test('rejects invalid scenario', () => {
      ...
      expect(() => engine.runScenario({ name: 'test' })).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('validateRecovery', () => {
    ...
    test('returns not recovered when faults are active', () => {
      engine.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
      const result = engine.validateRecovery();
    ...
    test('counts resolved and expired faults', () => {
      const r1 = engine.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
      const r2 = engine.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e2' });
  ...
  describe('getFault', () => {
    test('returns active fault', () => {
      const result = engine.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
      const fault = engine.getFault(result.faultId);
  ...
  describe('getActiveFaults', () => {
    test('returns all active faults', () => {
      engine.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
      engine.injectFault({ faultType: FAULT_TYPE.NETWORK_PARTITION, targetEnclaveId: 'e2' });
  ...
  describe('getStats', () => {
    test('returns summary statistics', () => {
      engine.injectFault({ faultType: FAULT_TYPE.ENCLAVE_CRASH, targetEnclaveId: 'e1' });
      engine.injectFault({ faultType: FAULT_TYPE.BYZANTINE_EQUIVOCATION, targetEnclaveId: 'e2' });
