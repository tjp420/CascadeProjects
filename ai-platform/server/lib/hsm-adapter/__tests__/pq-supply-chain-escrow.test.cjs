 ...
 */
const { PqcSupplyChainEscrowHub } = require('../pqc-supply-chain-escrow-hub.cjs');
const { ZkOrderMilestoneValidator } = require('../zk-order-milestone-validator.cjs');
...
function setupInitAndMilestone() {
  ...
  const milestone = ctx.validator.verifyMilestone(baseMilestoneRequest(ctx.order.orderId));
...
describe('Track 68 PQ supply chain escrow', () => {
  test('PqcSupplyChainEscrowHub initializes an order and emits SUPPLY_CHAIN_ORDER_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'SUPPLY_CHAIN_ORDER_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkOrderMilestoneValidator verifies a milestone and emits ZK_DELIVERY_MILESTONE_VERIFIED', () => {
    ...
    const milestone = validator.verifyMilestone(baseMilestoneRequest(order.orderId));
    expect(milestone.milestoneId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_DELIVERY_MILESTONE_VERIFIED')).toBe(true);
  });
  ...
  test('PqcSupplyChainEscrowHub releases escrow after milestone and emits PROCUREMENT_ESCROW_RELEASED', () => {
    ...
    expect(events.some((e) => e.event === 'PROCUREMENT_ESCROW_RELEASED')).toBe(true);
  });
  ...
  test('PqcSupplyChainEscrowHub rejects escrow release before milestone verification', () => {
    const { hub, order } = setupAndInitOrder();
    expect(() => hub.releaseEscrow(baseReleaseRequest(order.orderId))).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq supply chain escrow configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', {
    ...
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { orderMatchingQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { procurementDeliveryEpochs: 60 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { escrowFundingCap: 2000000000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { procurementInitiatorAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { clearingCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { banMalformedOrOutOfOrderDeliveryAssertions: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqSupplyChainEscrow', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
