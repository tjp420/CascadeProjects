...
const {
  PqcSupplyChainResilienceIntegrityGatingHub,
} = require("../pqc-supply-chain-resilience-integrity-gating-hub.cjs");
const {
  ZkResilienceClaimValidator,
} = require("../zk-resilience-claim-validator.cjs");
...
function baseInitRequest() {
  return {
    ...
    blindedDisruptionPredictionCommitment: "pedersen-disruption-prediction-001",
    blindedSupplierDiversityCommitment: "pedersen-supplier-diversity-001",
    blindedSupplyChainResilienceAuthorityIdentityCommitment:
      "pedersen-resilience-auth-001",
    resilienceWindowSeconds: 86400,
    resilienceChainDepth: 20,
    pqcSignatureScheme: "ML-DSA-65",
    supplyChainResilienceAuthorityInitializerAttestation: mockAttestation(),
...
function baseClaimRequest(poolId) {
  return {
    ...
    blindedSupplyChainResilienceAuthorityIdentityCommitment:
      "pedersen-resilience-auth-001",
    zkResilienceRangeProofHash: "zk-resilience-proof-001",
    verifiableSecretSharingDigest: "vss-digest-001",
    supplyChainEthicsOversightCommitteeAttestation: mockAttestation(),
    attestationAuthority: "mock-authority",
  };
}
...
function setupHubAndValidator() {
  ...
  const hub = new PqcSupplyChainResilienceIntegrityGatingHub({
    policy: POLICY,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
  });
  const validator = new ZkResilienceClaimValidator({
    policy: POLICY,
    hub,
    attestationClient,
    audit: (event, info) => events.push({ event, info }),
...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyResilienceClaim(
...
describe("Track 103 PQ supply chain resilience integrity gating", () => {
  test("initializes a pool and emits SUPPLY_CHAIN_RESILIENCE_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some(
        (e) => e.event === "SUPPLY_CHAIN_RESILIENCE_POOL_INITIALIZED",
      ),
    ).toBe(true);
  });
  ...
  test("verifies a resilience claim and emits ZK_RESILIENCE_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyResilienceClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(claim.verifiableSecretSharingDigest).toBe("vss-digest-001");
    expect(events.some((e) => e.event === "ZK_RESILIENCE_CLAIM_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("completes accreditation after claim and emits RESILIENCE_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "RESILIENCE_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("rejects resilience window exceeding maximum", () => {
    ...
    req.resilienceWindowSeconds = 999999;
  ...
  test("rejects un-attested supply chain resilience authority initializer", () => {
    ...
    const req = baseInitRequest();
    req.supplyChainResilienceAuthorityInitializerAttestation = {
      authority: "bad",
    };
    expect(() => hub.initializePool(req)).toThrow(HsmAdapterError);
  });
  ...
  test("rejects un-attested supply chain ethics oversight committee", () => {
    ...
    const ac = new MockAttestationClient();
    const v = new ZkResilienceClaimValidator({
      policy: POLICY,
      hub,
      attestationClient: ac,
    });
    const cr = baseClaimRequest(pool.poolId);
    cr.supplyChainEthicsOversightCommitteeAttestation = { authority: "bad" };
  ...
  test("bans peers broadcasting malformed claims (missing zkResilienceRangeProofHash)", () => {
    ...
    const cr = baseClaimRequest(pool.poolId);
    cr.zkResilienceRangeProofHash = null;
    cr.peerId = "peer-bad";
  ...
  test("bans peers broadcasting missing verifiableSecretSharingDigest", () => {
    const { validator, pool } = setupAndInitPool();
    const cr = baseClaimRequest(pool.poolId);
    cr.verifiableSecretSharingDigest = null;
    cr.peerId = "peer-bad";
    expect(() => validator.verifyResilienceClaim(cr)).toThrow(HsmAdapterError);
    expect(validator.isPeerBanned("peer-bad")).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates pq supply chain resilience integrity gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
        resilienceQuorum: 10,
        resilienceWindowSeconds: 86400,
        resilienceChainDepth: 20,
        pqcSignatureScheme: "ML-DSA-65",
        supplyChainResilienceAuthorityInitializerAttestation: true,
        supplyChainEthicsOversightCommitteeAttestation: true,
        attestationAuthority: "mock-authority",
        banMalformedOrOutOfOrderResilienceClaims: true,
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
        resilienceWindowSeconds: 999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
        resilienceChainDepth: 32,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
        supplyChainResilienceAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
        supplyChainEthicsOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
        banMalformedOrOutOfOrderResilienceClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqSupplyChainResilienceIntegrityGating", {
