...
describe("Track 45 cross-tenant audit", () => {
  test("CrossTenantAccessAuditor recognizes a valid access and chains a receipt", () => {
    ...
    expect(
      events.some((e) => e.event === "CROSS_TENANT_ACCESS_RECOGNIZED"),
    ).toBe(true);
  ...
  test("AccessProofReceipt serializes and parses canonically", () => {
    ...
    expect(serialized).toMatch(
      /^AUDIT:tenant-a:tenant-b:key-escrow:resource-456:1234567890:/,
    );
  ...
  test("CryptoPolicyEngine validates cross-tenant audit configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "crossTenantAudit", {
    ...
    expect(() =>
      engine.validate("t1", "crossTenantAudit", {
        attestationForBothEndpoints: false,
      }),
    ...
    expect(() =>
      engine.validate("t1", "crossTenantAudit", { operation: "forbidden" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "crossTenantAudit", { dualLinkedProof: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "crossTenantAudit", {
        canonicalReceiptLayout: false,
      }),
    ).toThrow(HsmAdapterError);
  });
});
