...
describe("Track 41 hardware enclaves", () => {
  ...
  test("HardwareEnclaveAdapter initializes with a valid attestation", async () => {
    ...
    expect(
      events.some((e) => e.event === "ENCLAVE_HARDWARE_BOOTSTRAPPED"),
    ).toBe(true);
    expect(
      events.some((e) => e.event === "ATTESTATION_CHALLENGE_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("CryptoPolicyEngine validates enclave configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "enclave", { enclaveType: "arm-trustzone" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "enclave", {
        attestationAuthority: "evil-authority",
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "enclave", { attestationAgeSeconds: 120 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "enclave", { enclaveCipher: "rc4" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "enclave", { requireRemoteAttestation: false }),
    ).toThrow(HsmAdapterError);
  });
});
