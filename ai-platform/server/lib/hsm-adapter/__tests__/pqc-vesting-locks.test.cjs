...
describe("Track 58 PQC vesting locks", () => {
  test("PqcVestingEscrowHub initializes a vesting lock and emits VESTING_LOCK_INITIALIZED", () => {
    ...
    expect(events.some((e) => e.event === "VESTING_LOCK_INITIALIZED")).toBe(
      true,
    );
  });
  ...
  test("PqcVestingEscrowHub processes a valid epoch release claim", () => {
    ...
    expect(
      events.some((e) => e.event === "VESTING_EPOCH_RELEASE_CLAIMED"),
    ).toBe(true);
  });
  ...
  test("PqcVestingEscrowHub completes escrow after all epochs and emits VESTING_ESCROW_COMPLETED", () => {
    ...
    expect(events.some((e) => e.event === "VESTING_ESCROW_COMPLETED")).toBe(
      true,
    );
  });
  ...
  test("CryptoPolicyEngine validates pqc vesting locks configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", {
    ...
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", { vestingEpochSeconds: 60 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", { releaseSignatureQuorum: 1 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", { assetValue: 2000000 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", {
    ...
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", { claimantAttestation: false }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", {
        committeeRelayAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", { attestationAuthority: "bad" }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", {
        banExpiredOrDuplicateClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqcVestingLocks", {
