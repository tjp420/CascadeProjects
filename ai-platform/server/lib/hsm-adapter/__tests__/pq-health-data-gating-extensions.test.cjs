 ...
 * extension tests.
 *
 * Tests the new batch pool initialization, diagnostic observation
 * depth rebalancing, committee signature aggregation, pool
 * cancellation, cross-chain settlement, HW-SNARK proof
 * generation, batch health claim verification, slashing
...
describe('Track 72 PQC Health Data Gating extensions', () => {
  describe('PqcHealthDataGatingHub — observation depth rebalancing', () => {
    ...
    test('updates diagnosticObservationDepth on rebalance when newDiagnosticObservationDepth provided', () => {
      ...
      ctx.hub.rebalanceObservationDepth({
        ...
        direction: REBALANCE_DIRECTION.INCREASE,
        rebalanceAmount: 4,
        newDiagnosticObservationDepth: 24,
      });
      const pool = ctx.hub.getPool(ctx.pool.poolId);
      expect(pool.diagnosticObservationDepth).toBe(24);
  ...
  describe('full Track 72 extended flow', () => {
    test('complete init → rebalance → claim → complete → settle flow', () => {
      ...
      expect(hStats.completeCount).toBeGreaterThan(0);
