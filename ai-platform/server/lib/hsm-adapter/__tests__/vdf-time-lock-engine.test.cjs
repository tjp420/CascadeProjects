...
describe("Track 59: VdfTimeLockEngine", () => {
  ...
  describe("createVdf", () => {
    ...
    test("rejects missing vdfId", () => {
      expect(() => engine.createVdf({ input: 5n, difficulty: 100 })).toThrow(
        HsmAdapterError,
      );
    });
    ...
    test("rejects non-bigint input", () => {
      expect(() =>
        engine.createVdf({ vdfId: "v1", input: "bad", difficulty: 100 }),
      ).toThrow(HsmAdapterError);
    });

    test("rejects zero input", () => {
      expect(() =>
        engine.createVdf({ vdfId: "v1", input: 0n, difficulty: 100 }),
      ).toThrow(HsmAdapterError);
    });

    test("rejects difficulty below minimum", () => {
      expect(() =>
        engine.createVdf({ vdfId: "v1", input: 5n, difficulty: 0 }),
      ).toThrow(HsmAdapterError);
    });

    test("rejects difficulty too high", () => {
      expect(() =>
        engine.createVdf({ vdfId: "v1", input: 5n, difficulty: 10000 }),
      ).toThrow(HsmAdapterError);
    });
  ...
  describe("verifyPuzzleSolution", () => {
    ...
    test("rejects unknown puzzle", () => {
      expect(() =>
        engine.verifyPuzzleSolution("unknown", Buffer.from("a")),
      ).toThrow(HsmAdapterError);
    });
    ...
    test("rejects non-Buffer solution", () => {
      ...
      expect(() => engine.verifyPuzzleSolution("p1", "not-a-buffer")).toThrow(
        HsmAdapterError,
      );
    });
  });
  ...
  describe("full VDF + puzzle flow", () => {
    test("complete create -> evaluate -> verify -> puzzle -> solve flow", () => {
      ...
      // Verify the solution
      const verifySolution = engine.verifyPuzzleSolution(
        "consensus-puzzle",
        secret,
      );
