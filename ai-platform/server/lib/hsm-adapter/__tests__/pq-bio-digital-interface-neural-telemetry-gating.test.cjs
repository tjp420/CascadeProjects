...
const {
  PqcBioDigitalInterfaceNeuralTelemetryGatingHub,
} = require("../pqc-bio-digital-interface-neural-telemetry-gating-hub.cjs");
...
function makeHub() {
  return new PqcBioDigitalInterfaceNeuralTelemetryGatingHub({
...
describe("Track 112 PQC Bio-Digital Interface Neural-Telemetry Gating Hub", () => {
  ...
  test("initializes a neuro telemetry pool and increments counter", () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_neurogate_pool_initialized_total).toBe(
      1,
    );
  });
  ...
  test("verifies neural telemetry and increments counter", () => {
    ...
    const verified = hub.verifyNeuralTelemetry({
      poolId: pool.poolId,
      proofValid: true,
    });
    expect(verified.neuralTelemetryVerified).toBe(true);
    expect(hsmMetrics.getMetrics().hsm_zk_neural_telemetry_verified_total).toBe(
      1,
    );
  });
  ...
  test("completes synapse accreditation with sufficient quorum", () => {
    ...
    expect(
      hsmMetrics.getMetrics().hsm_synapse_accreditation_completed_total,
    ).toBe(1);
  });
  ...
  test("rejects synapse accreditation with insufficient quorum", () => {
    ...
    const code = expectErrorCode(
      () =>
        hub.completeSynapseAccreditation({
          poolId: pool.poolId,
