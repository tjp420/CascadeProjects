 ...
 */
const { PqcRealEstateTokenizationHub } = require('../pqc-real-estate-tokenization-hub.cjs');
const { ZkTitleDeedMilestoneValidator } = require('../zk-title-deed-milestone-validator.cjs');
...
describe('Track 69 PQ real estate tokenization', () => {
  test('PqcRealEstateTokenizationHub initializes a pool and emits REAL_ESTATE_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'REAL_ESTATE_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkTitleDeedMilestoneValidator verifies encumbrance clearance and emits ZK_ENCUMBRANCE_CLEARANCE_VERIFIED', () => {
    ...
    const clearance = validator.verifyEncumbranceClearance(baseClearanceRequest(pool.poolId));
    expect(clearance.clearanceId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_ENCUMBRANCE_CLEARANCE_VERIFIED')).toBe(true);
  });
  ...
  test('PqcRealEstateTokenizationHub finalizes a transfer after clearance and emits TITLE_DEED_TRANSFER_FINALIZED', () => {
    ...
    const transfer = hub.finalizeTitleDeedTransfer(baseTransferRequest(pool.poolId));
    expect(transfer.transferId).toBeDefined();
    expect(events.some((e) => e.event === 'TITLE_DEED_TRANSFER_FINALIZED')).toBe(true);
  });
  ...
  test('ZkTitleDeedMilestoneValidator rejects un-attested clearing committee', () => {
    ...
    expect(() => validator.verifyEncumbranceClearance(clReq)).toThrow(HsmAdapterError);
  });
  ...
  test('PqcRealEstateTokenizationHub rejects transfer before encumbrance clearance verification', () => {
    const { hub, pool } = setupAndInitPool();
    expect(() => hub.finalizeTitleDeedTransfer(baseTransferRequest(pool.poolId))).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq real estate tokenization configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', {
      coSignerQuorum: 3,
      legalDisputeSeconds: 1296000,
      assetValuationCap: 1000000,
      pqcSignatureScheme: 'ML-DSA-65',
    ...
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { coSignerQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { legalDisputeSeconds: 5000000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { assetValuationCap: 2000000000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { assetInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { clearingCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { banMalformedOrOutOfOrderTitleDeedAssertions: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqRealEstateTokenization', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
