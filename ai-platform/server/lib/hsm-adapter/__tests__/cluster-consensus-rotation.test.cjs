...
describe('Track 34 Phase 5 peer key rotation — addPeerKey', () => {
  test('leader can add a new peer key via quorum-gated consensus', async () => {
    ...
    expect(events.some((e) => e.event === CONSENSUS_EVENT.PEER_KEY_ADDED)).toBe(true);
  ...
  test('follower cannot add a peer key', async () => {
    ...
    const newKeys = generateKeyPair();
    await expect(consensus.addPeerKey('node-d', newKeys.publicKey)).rejects.toThrow(HsmAdapterError);
    await expect(consensus.addPeerKey('node-d', newKeys.publicKey)).rejects.toThrow('not leader');
    expect(events.some((e) => e.event === CONSENSUS_EVENT.PEER_KEY_ROTATION_BLOCKED)).toBe(true);
  ...
  test('addPeerKey rejects invalid inputs', async () => {
    ...
    await expect(consensus.addPeerKey('', generateKeyPair().publicKey)).rejects.toThrow(HsmAdapterError);
    await expect(consensus.addPeerKey('node-d', null)).rejects.toThrow(HsmAdapterError);
  ...
  test('addPeerKey updates existing key (rotation)', async () => {
    ...
    const payload = { term: 1, candidateId: 'node-b', nonce: 1, timestamp: Date.now() };
...
describe('Track 34 Phase 5 peer key rotation — revokePeerKey', () => {
  test('leader can revoke a peer key via quorum-gated consensus', async () => {
    ...
    expect(events.some((e) => e.event === CONSENSUS_EVENT.PEER_KEY_REVOKED)).toBe(true);
  ...
  test('follower cannot revoke a peer key', async () => {
    ...
    await expect(consensus.revokePeerKey('node-a')).rejects.toThrow(HsmAdapterError);
    await expect(consensus.revokePeerKey('node-a')).rejects.toThrow('not leader');
    expect(events.some((e) => e.event === CONSENSUS_EVENT.PEER_KEY_ROTATION_BLOCKED)).toBe(true);
  ...
  test('revokePeerKey rejects unknown peer', async () => {
    ...
    await expect(consensus.revokePeerKey('node-z')).rejects.toThrow(HsmAdapterError);
    expect(events.some((e) => e.event === CONSENSUS_EVENT.PEER_KEY_ROTATION_BLOCKED)).toBe(true);
  ...
  test('revoked peer RPCs are rejected after revocation', async () => {
    ...
    const payload2 = { term: 1, candidateId: 'node-b', nonce: 2, timestamp: Date.now() };
...
describe('Track 34 Phase 5 peer key rotation — registry helpers', () => {
  test('getRegisteredPeers returns list of registered peer IDs', async () => {
    ...
    const consensus = new ClusterConsensusEngine({
      nodeId: 'node-a',
      clusterNodes: ['node-a', 'node-b', 'node-c'],
      peerPublicKeys: new Map([['node-b', keysB.publicKey], ['node-c', keysC.publicKey]]),
    });
    consensus.start();
    await consensus.startElection();
...
describe('Track 34 Phase 5 peer key rotation — command application', () => {
  test('_applyConsensusCommand applies addPeerKey from committed log entry', () => {
    ...
    consensus._log = [{
      term: 1,
      index: 1,
      command: { operation: 'addPeerKey', nodeId: 'node-d', publicKey: newKey },
      committed: false,
    }];
...
describe('Track 34 Phase 5 peer key rotation — Prometheus metrics', () => {
  ...
  test('blocked rotation increments hsm_consensus_peer_key_rotation_blocked_total', async () => {
    ...
    consensus.start();
    // node-b is follower
    try { await consensus.addPeerKey('node-d', generateKeyPair().publicKey); } catch {}
