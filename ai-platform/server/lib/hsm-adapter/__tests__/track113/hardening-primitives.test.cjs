...
const {
  encryptEnvelope,
  decryptEnvelope,
} = require("../../../crypto/ratchet/envelope-crypto.cjs");
...
function testWriteAtomicCleanup() {
  ...
  try {
    ...
    threw = true;
    assert.ok(
      /simulated rename failure/.test(String(e)),
      "expected simulated rename failure",
    );
  }
  ...
  assert.ok(
    unlinkArg,
    "writeAtomicSync should attempt to unlink the temp file",
  );
...
function testEnvelopeRoundTripAndTamper() {
  ...
  assert.throws(
    () => decryptEnvelope("not-valid-envelope"),
    /invalid envelope/,
  );
