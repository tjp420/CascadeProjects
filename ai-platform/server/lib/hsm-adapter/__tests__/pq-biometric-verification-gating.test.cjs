 ...
 */
const { PqcBiometricVerificationGatingHub } = require('../pqc-biometric-verification-gating-hub.cjs');
const { ZkBiometricClaimValidator } = require('../zk-biometric-claim-validator.cjs');
...
describe('Track 77 PQ biometric verification gating', () => {
  test('PqcBiometricVerificationGatingHub initializes a pool and emits BIOMETRIC_GATING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'BIOMETRIC_GATING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkBiometricClaimValidator verifies a biometric claim and emits ZK_BIOMETRIC_CLAIM_VERIFIED', () => {
    ...
    expect(events.some((e) => e.event === 'ZK_BIOMETRIC_CLAIM_VERIFIED')).toBe(true);
  });
  ...
  test('PqcBiometricVerificationGatingHub completes accreditation after claim and emits LIVENESS_ATTESTATION_ACCREDITATION_COMPLETED', () => {
    ...
    expect(events.some((e) => e.event === 'LIVENESS_ATTESTATION_ACCREDITATION_COMPLETED')).toBe(true);
  });
  ...
  test('PqcBiometricVerificationGatingHub rejects un-attested biometric authority initializer', () => {
    ...
    const hub = new PqcBiometricVerificationGatingHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkBiometricClaimValidator rejects un-attested clearing committee', () => {
    ...
    const validator = new ZkBiometricClaimValidator({ policy: POLICY, hub, attestationClient });
    const clReq = baseClaimRequest(pool.poolId);
    clReq.clearingCommitteeAttestation = { authority: 'bad' };
    expect(() => validator.verifyBiometricClaim(clReq)).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq biometric gating configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqBiometricGating', {
    ...
    expect(() => engine.validate('t1', 'pqBiometricGating', { biometricAuthorityQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { templateExpirationSeconds: 99999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { livenessMetricDepth: 32 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { biometricAuthorityInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { clearingCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { banMalformedOrOutOfOrderBiometricClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqBiometricGating', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
