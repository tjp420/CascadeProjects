...
function setupInitAndClaim() {
  ...
  const claim = ctx.validator.verifyConservationClaim(
...
describe("Track 90 PQ wildlife conservation tracking gating", () => {
  test("PqcWildlifeConservationTrackingGatingHub initializes a pool and emits WILDLIFE_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "WILDLIFE_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkConservationClaimValidator verifies a conservation claim and emits ZK_CONSERVATION_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyConservationClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(
      events.some((e) => e.event === "ZK_CONSERVATION_CLAIM_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("PqcWildlifeConservationTrackingGatingHub completes accreditation after claim and emits BIODIVERSITY_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "BIODIVERSITY_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkConservationClaimValidator rejects un-attested biodiversity oversight committee", () => {
    ...
    expect(() => validator.verifyConservationClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("ZkConservationClaimValidator detects double-reporting via linkability tag and bans peer", () => {
    ...
    expect(() => validator.verifyConservationClaim(clReq2)).toThrow(
      HsmAdapterError,
    );
  ...
  test("CryptoPolicyEngine validates pq wildlife gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", { conservationQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
        monitoringWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", { telemetryChainDepth: 28 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
        conservationAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
        biodiversityOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
        banMalformedOrOutOfOrderConservationClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqWildlifeGating", {
