 ...
 */
const { PqcIdentityRevocationRegistry } = require('../pqc-identity-revocation-registry.cjs');
const { ZkRevocationProofVerifier } = require('../zk-revocation-proof-verifier.cjs');
...
describe('Track 61 PQ identity revocation', () => {
  test('PqcIdentityRevocationRegistry publishes a revocation and emits IDENTITY_REVOCATION_PUBLISHED', () => {
    ...
    expect(events.some((e) => e.event === 'IDENTITY_REVOCATION_PUBLISHED')).toBe(true);
  });
  ...
  test('ZkRevocationProofVerifier authenticates a valid non-membership proof and emits ZK_REVOCATION_PROOF_AUTHENTICATED', () => {
    ...
    const proof = verifier.verifyNonMembershipProof(baseProofRequest('blinded-hash-clean'));
    expect(proof.proofId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_REVOCATION_PROOF_AUTHENTICATED')).toBe(true);
  });
  ...
  test('ZkRevocationProofVerifier rejects un-attested verifier', () => {
    ...
    expect(() => verifier.verifyNonMembershipProof(proofReq)).toThrow(HsmAdapterError);
  });
  ...
  test('ZkRevocationProofVerifier bans peers broadcasting duplicate proofs', () => {
    ...
    expect(() => verifier.verifyNonMembershipProof(dupReq)).toThrow(HsmAdapterError);
    expect(verifier.isPeerBanned('peer-bad')).toBe(true);
  });

  test('CryptoPolicyEngine validates pq identity revocation configuration', () => {
    const engine = new CryptoPolicyEngine({ default: {} });
    expect(() => engine.validate('t1', 'pqIdentityRevocation', {
    ...
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { revocationCommitteeQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { revocationListCapacity: 200000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { proofExpirationSeconds: 7200 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { publisherAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { verifierAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { banMalformedNonMembershipProofs: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqIdentityRevocation', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
