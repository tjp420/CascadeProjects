...
describe("Track 56 encrypted search routing", () => {
  test("EncryptedSearchRouter routes an encrypted search query", () => {
    ...
    expect(events.some((e) => e.event === "ENCRYPTED_SEARCH_ROUTED")).toBe(
      true,
    );
  });
  ...
  test("MpcSearchMatchVerifier aggregates evaluations and verifies match", () => {
    ...
    expect(events.some((e) => e.event === "MPC_INDEX_MATCH_VERIFIED")).toBe(
      true,
    );
  });
  ...
  test("MpcSearchMatchVerifier rejects unauthorized node submission", () => {
    ...
    expect(() => verifier.submit("verify-2", "idx-unknown", "eval")).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates encrypted search routing configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", {
        keywordsPerQuery: 10,
    ...
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", { keywordsPerQuery: 64 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", {
        indexTraversalDepth: 32,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", {
        blindingCurve: "secp256k1",
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", {
    ...
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", {
        isolateLowQuorumIndexNodes: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "encryptedSearchRouting", {
