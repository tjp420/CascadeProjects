...
describe("Track 33 core gating hub", () => {
  ...
  test("FSM advances OPEN -> WITNESSES_COLLECTED -> PROOF_VALIDATED -> ACCREDITED", () => {
    ...
    expect(
      hsmMetrics.getMetrics().hsm_accumulatorgate_pool_initialized_total,
    ).toBe(1);
    expect(
      hsmMetrics.getMetrics().hsm_zk_accumulator_claim_verified_total,
    ).toBe(1);
    expect(
      hsmMetrics.getMetrics().hsm_accumulator_accreditation_completed_total,
    ).toBe(1);
  });
  ...
  test("DEFAULT_POLICY has exactly one accumulatorGating key (duplicate removed)", () => {
    ...
    const keys = Object.keys(DEFAULT_POLICY).filter(
      (k) => k === "accumulatorGating",
    );
  ...
  test("non-array witnesses to collectWitnesses throws ACCUMULATORGATE_INVALID_WITNESSES", () => {
    ...
    expect(() => hub.collectWitnesses("not-an-array")).toThrow(
      /ACCUMULATORGATE_INVALID_WITNESSES/,
    );
  });
