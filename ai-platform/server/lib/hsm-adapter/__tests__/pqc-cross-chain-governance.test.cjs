 ...
 */
const { PqcCrossChainGovernanceBridge } = require('../pqc-cross-chain-governance-bridge.cjs');
const { GovernanceProposalVotingMonitor } = require('../governance-proposal-voting-monitor.cjs');
...
describe('Track 59 PQC cross-chain governance', () => {
  test('PqcCrossChainGovernanceBridge broadcasts a proposal and emits CROSS_CHAIN_PROPOSAL_BROADCAST', () => {
    ...
    expect(events.some((e) => e.event === 'CROSS_CHAIN_PROPOSAL_BROADCAST')).toBe(true);
  });
  ...
  test('GovernanceProposalVotingMonitor records a valid vote and emits GOVERNANCE_VOTE_RECORDED', () => {
    ...
    const proposal = bridge.broadcastProposal(baseBroadcastRequest());
    const vote = monitor.recordVote(baseVoteRequest(proposal.proposalId, 'platform-a'));
    expect(vote.voteDecision).toBe('approve');
    expect(events.some((e) => e.event === 'GOVERNANCE_VOTE_RECORDED')).toBe(true);
  });
  ...
  test('GovernanceProposalVotingMonitor executes proposal after quorum and emits CROSS_CHAIN_PROPOSAL_EXECUTED', () => {
    ...
    expect(events.some((e) => e.event === 'CROSS_CHAIN_PROPOSAL_EXECUTED')).toBe(true);
  });
  ...
  test('CryptoPolicyEngine validates pqc cross-chain governance configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', {
    ...
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { platformVotingQuorum: 1 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { concurrentProposals: 32 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { proposalExecutionWindowSeconds: 200000 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { proposalBroadcasterAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { verifierRelayAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { banMalformedOrOutOfOrderVotes: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqcCrossChainGovernance', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
