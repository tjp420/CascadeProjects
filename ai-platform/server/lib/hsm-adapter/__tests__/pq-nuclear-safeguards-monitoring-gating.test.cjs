 ...
 */
const { PqcNuclearSafeguardsMonitoringGatingHub } = require('../pqc-nuclear-safeguards-monitoring-gating-hub.cjs');
const { ZkSafeguardsClaimValidator } = require('../zk-safeguards-claim-validator.cjs');
...
describe('Track 89 PQ nuclear safeguards monitoring gating', () => {
  test('PqcNuclearSafeguardsMonitoringGatingHub initializes a pool and emits NUCLEAR_GATING_POOL_INITIALIZED', () => {
    ...
    expect(events.some((e) => e.event === 'NUCLEAR_GATING_POOL_INITIALIZED')).toBe(true);
  });
  ...
  test('ZkSafeguardsClaimValidator verifies a safeguards claim and emits ZK_SAFEGUARDS_CLAIM_VERIFIED', () => {
    ...
    const claim = validator.verifySafeguardsClaim(baseClaimRequest(pool.poolId));
    expect(claim.claimId).toBeDefined();
    expect(events.some((e) => e.event === 'ZK_SAFEGUARDS_CLAIM_VERIFIED')).toBe(true);
  });
  ...
  test('PqcNuclearSafeguardsMonitoringGatingHub completes accreditation after claim and emits NUCLEAR_ACCREDITATION_COMPLETED', () => {
    ...
    expect(events.some((e) => e.event === 'NUCLEAR_ACCREDITATION_COMPLETED')).toBe(true);
  });
  ...
  test('PqcNuclearSafeguardsMonitoringGatingHub rejects un-attested safeguards authority initializer', () => {
    ...
    const hub = new PqcNuclearSafeguardsMonitoringGatingHub({ policy: POLICY, attestationClient });
    const request = baseInitRequest();
  ...
  test('ZkSafeguardsClaimValidator rejects un-attested nuclear oversight committee', () => {
    ...
    const validator = new ZkSafeguardsClaimValidator({ policy: POLICY, hub, attestationClient });
    const clReq = baseClaimRequest(pool.poolId);
    clReq.nuclearOversightCommitteeAttestation = { authority: 'bad' };
    expect(() => validator.verifySafeguardsClaim(clReq)).toThrow(HsmAdapterError);
  });
  ...
  test('CryptoPolicyEngine validates pq nuclear gating configuration', () => {
    ...
    expect(() => engine.validate('t1', 'pqNuclearGating', {
      safeguardsQuorum: 6,
      inspectionWindowSeconds: 3888000,
      telemetryChainDepth: 6,
      pqcSignatureScheme: 'ML-DSA-65',
    ...
    expect(() => engine.validate('t1', 'pqNuclearGating', { safeguardsQuorum: 3 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { inspectionWindowSeconds: 99999999 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { telemetryChainDepth: 24 })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { pqcSignatureScheme: 'RSA-2048' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { safeguardsAuthorityInitializerAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { nuclearOversightCommitteeAttestation: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { attestationAuthority: 'bad' })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { banMalformedOrOutOfOrderSafeguardsClaims: false })).toThrow(HsmAdapterError);
    expect(() => engine.validate('t1', 'pqNuclearGating', { canonicalPayloadLayout: false })).toThrow(HsmAdapterError);
  });
});
