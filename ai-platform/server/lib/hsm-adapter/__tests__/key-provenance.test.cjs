...
describe('ProvenanceTracker', () => {
  ...
  beforeEach(() => {
    tracker = new ProvenanceTracker({ buildHash: 'test-build-123', hardwareRootToken: 'hw-token-123' });
  });
  ...
  test('getRecord returns the stored record', () => {
    tracker.register('tenant-a', 'kek-1', { algorithm: 'aes-kw', kekBits: 256 });
  ...
  test('verify returns true for a valid record', () => {
    const record = tracker.register('tenant-a', 'kek-1', { algorithm: 'aes-kw', kekBits: 256 });
  ...
  test('validate passes for matching in-memory info', () => {
    tracker.register('tenant-a', 'kek-1', { algorithm: 'aes-kw', kekBits: 256, createdAt: 1_700_000_000_000 });
    expect(tracker.validate('kek-1', {
      tenantId: 'tenant-a',
      algorithm: 'aes-kw',
      kekBits: 256,
      createdAt: 1_700_000_000_000,
  ...
  test('validate throws for missing record', () => {
    expect(() => tracker.validate('missing', { tenantId: 't', algorithm: 'aes-kw', kekBits: 256 })).toThrow(HsmAdapterError);
  });

  test('records form a hash chain', () => {
    const r1 = tracker.register('t', 'kek-1', { algorithm: 'aes-kw', kekBits: 256 });
    const r2 = tracker.register('t', 'kek-2', { algorithm: 'aes-kw', kekBits: 256 });
...
describe('ProvenanceProof', () => {
  test('create returns a proof containing the record', () => {
    ...
    const record = tracker.register('t', 'kek-1', { algorithm: 'aes-kw', kekBits: 256 });
  ...
  test('verify throws for a tampered record', () => {
    ...
    record.algorithm = 'aes-kwp';
    const proof = ProvenanceProof.create(record);
    expect(() => ProvenanceProof.verify(proof, tracker.publicKey)).toThrow(HsmAdapterError);
  });
  ...
  test('verify throws with a wrong public key', () => {
    ...
    const record = tracker.register('t', 'kek-1', { algorithm: 'aes-kw', kekBits: 256 });
    const proof = ProvenanceProof.create(record);
    expect(() => ProvenanceProof.verify(proof, other.publicKey)).toThrow(HsmAdapterError);
  });
});
...
describe('Adapter provenance integration', () => {
  ...
  test('AsymmetricHsmAdapter creates and validates provenance on wrap/unwrap', async () => {
    const adapter = new AsymmetricHsmAdapter({ algorithm: 'ecdh', keySize: 256, buildHash: 'asym-build' });
    await adapter.initialize();
    const kekId = await adapter.createKEK('t1');
    const record = adapter._provenanceTracker.getRecord(kekId);
    expect(record).not.toBeNull();
  ...
  test('corrupted provenance throws KEY_PROVENANCE_CORRUPTED on wrap', async () => {
    ...
    const record = adapter._provenanceTracker.getRecord(kekId);
    record.algorithm = 'aes-kwp';
    await expect(adapter.wrap('t1', kekId, Buffer.alloc(16))).rejects.toMatchObject({
