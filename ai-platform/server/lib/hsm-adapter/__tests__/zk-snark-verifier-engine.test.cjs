...
describe('Track 57: ZkSnarkVerifierEngine', () => {
  ...
  function setupCircuit(engine, circuitId = 'mul-circuit', setupId = 'setup-1') {
  ...
  describe('compileCircuit', () => {
    ...
    test('rejects empty constraints', () => {
      expect(() => engine.compileCircuit({
        circuitId: 'c1', constraints: [],
      })).toThrow(HsmAdapterError);
    });

    test('rejects too many constraints', () => {
      const constraints = Array.from({ length: 1001 }, () => ({ type: 'mul', a: 0, b: 0 }));
      expect(() => engine.compileCircuit({
        circuitId: 'c1', constraints,
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects invalid constraint structure', () => {
      expect(() => engine.compileCircuit({
        circuitId: 'c1', constraints: [{ noType: true }],
      })).toThrow(HsmAdapterError);
    });
  });
  ...
  describe('generateTrustedSetup', () => {
    test('generates a trusted setup', () => {
      createSimpleCircuit(engine);
      const result = engine.generateTrustedSetup('mul-circuit', { setupId: 's1' });
    ...
    test('rejects unknown circuit', () => {
      expect(() => engine.generateTrustedSetup('unknown'))
        .toThrow(HsmAdapterError);
    });
    ...
    test('rejects duplicate setupId', () => {
      ...
      expect(() => engine.generateTrustedSetup('mul-circuit', { setupId: 's1' }))
        .toThrow(HsmAdapterError);
    });
  });
  ...
  describe('generateProof', () => {
    ...
    test('rejects duplicate proofId', () => {
      ...
      engine.generateProof({
        proofId: 'p1', circuitId: 'mul-circuit', setupId: 'setup-1',
        publicInputs: { x: 5 }, privateInputs: { y: 7 },
      });
      expect(() => engine.generateProof({
        proofId: 'p1', circuitId: 'mul-circuit', setupId: 'setup-1',
        publicInputs: { x: 5 }, privateInputs: { y: 7 },
    ...
    test('rejects unknown circuit', () => {
      expect(() => engine.generateProof({
        proofId: 'p1', circuitId: 'unknown', setupId: 's1',
    ...
    test('rejects unknown setup', () => {
      createSimpleCircuit(engine);
      expect(() => engine.generateProof({
        proofId: 'p1', circuitId: 'mul-circuit', setupId: 'unknown',
        publicInputs: { x: 5 }, privateInputs: { y: 7 },
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects setup-circuit mismatch', () => {
      ...
      expect(() => engine.generateProof({
        proofId: 'p1', circuitId: 'other-circuit', setupId: 'setup-1',
        publicInputs: { a: 5 }, privateInputs: { b: 7 },
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects missing public input', () => {
      ...
      expect(() => engine.generateProof({
        proofId: 'p1', circuitId: 'mul-circuit', setupId: 'setup-1',
        publicInputs: {}, privateInputs: { y: 7 },
      })).toThrow(HsmAdapterError);
    });
    ...
    test('rejects missing private input', () => {
      ...
      expect(() => engine.generateProof({
        proofId: 'p1', circuitId: 'mul-circuit', setupId: 'setup-1',
        publicInputs: { x: 5 }, privateInputs: {},
      })).toThrow(HsmAdapterError);
    });
  ...
  describe('aggregateProofs', () => {
    test('aggregates verified proofs', () => {
      ...
      for (let i = 0; i < 3; i++) {
        engine.generateProof({
          proofId: `p${i}`, circuitId: 'mul-circuit', setupId: 'setup-1',
          publicInputs: { x: i + 1 }, privateInputs: { y: i + 2 },
        });
    ...
    test('rejects insufficient proofs', () => {
      setupCircuit(engine);
      engine.generateProof({
        proofId: 'p0', circuitId: 'mul-circuit', setupId: 'setup-1',
        publicInputs: { x: 1 }, privateInputs: { y: 2 },
      });
    ...
    test('rejects unverified proof', () => {
      ...
      engine.generateProof({
        proofId: 'p1', circuitId: 'mul-circuit', setupId: 'setup-1',
        publicInputs: { x: 2 }, privateInputs: { y: 3 },
      });
      engine.verifyProof('p0');
      // p1 not verified
      expect(() => engine.aggregateProofs(['p0', 'p1'])).toThrow(HsmAdapterError);
    });

    test('rejects unknown proof', () => {
      expect(() => engine.aggregateProofs(['unknown1', 'unknown2']))
        .toThrow(HsmAdapterError);
    });
  });
  ...
  describe('verifyAggregatedProof', () => {
    ...
    test('rejects unknown aggregated proof', () => {
      expect(() => engine.verifyAggregatedProof('unknown'))
        .toThrow(HsmAdapterError);
    });
  });
  ...
  describe('full zk-SNARK flow', () => {
    test('complete compile -> setup -> prove -> verify -> aggregate flow', () => {
      ...
      engine.generateTrustedSetup('enclave-computation', { setupId: 'main-setup' });
