...
describe("Track 92 PQ global health epidemiological surveillance gating", () => {
  test("PqcGlobalHealthEpidemiologicalSurveillanceGatingHub initializes a pool and emits EPIDEMIOLOGY_GATING_POOL_INITIALIZED", () => {
    ...
    expect(
      events.some((e) => e.event === "EPIDEMIOLOGY_GATING_POOL_INITIALIZED"),
    ).toBe(true);
  });
  ...
  test("ZkEpidemiologicalClaimValidator verifies an epidemiological claim and emits ZK_EPIDEMIOLOGICAL_CLAIM_VERIFIED", () => {
    ...
    const claim = validator.verifyEpidemiologicalClaim(
      baseClaimRequest(pool.poolId),
    );
    expect(claim.claimId).toBeDefined();
    expect(
      events.some((e) => e.event === "ZK_EPIDEMIOLOGICAL_CLAIM_VERIFIED"),
    ).toBe(true);
  });
  ...
  test("PqcGlobalHealthEpidemiologicalSurveillanceGatingHub completes accreditation after claim and emits OUTBREAK_ACCREDITATION_COMPLETED", () => {
    ...
    expect(
      events.some((e) => e.event === "OUTBREAK_ACCREDITATION_COMPLETED"),
    ).toBe(true);
  });
  ...
  test("ZkEpidemiologicalClaimValidator rejects un-attested epidemiology oversight committee", () => {
    ...
    expect(() => validator.verifyEpidemiologicalClaim(clReq)).toThrow(
      HsmAdapterError,
    );
  });
  ...
  test("CryptoPolicyEngine validates pq epidemiology gating configuration", () => {
    ...
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", { epidemiologyQuorum: 2 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
        surveillanceWindowSeconds: 99999999,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", { genomicChainDepth: 32 }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
        whoAuthorityInitializerAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
        epidemiologyOversightCommitteeAttestation: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
    ...
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
        banMalformedOrOutOfOrderEpidemiologicalClaims: false,
      }),
    ).toThrow(HsmAdapterError);
    expect(() =>
      engine.validate("t1", "pqEpidemiologyGating", {
