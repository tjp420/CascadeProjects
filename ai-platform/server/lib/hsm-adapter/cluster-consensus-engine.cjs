...
class ClusterConsensusEngine {
  ...
  constructor(options = {}) {
    ...
    if (!options.clusterNodes.includes(options.nodeId)) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        `nodeId ${options.nodeId} not in clusterNodes`,
      );
    }
  ...
  receiveHeartbeat(heartbeat = {}) {
    if (typeof heartbeat.term !== "number" || !heartbeat.leaderId) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "heartbeat requires term and leaderId",
      );
    }
    if (!this.clusterNodes.has(heartbeat.leaderId)) {
      throw new HsmAdapterError(
        "UNKNOWN_LEADER",
        `leader ${heartbeat.leaderId} not in cluster`,
      );
    }
    ...
    if (
      !this.verifyRpcFrame(payload, heartbeat.leaderId, heartbeat.signature)
    ) {
      return {
        accepted: false,
    ...
    if (
      typeof heartbeat.leaderCommit === "number" &&
      heartbeat.leaderCommit > this._commitIndex
    ) {
  ...
  requestVote(request = {}) {
    if (typeof request.term !== "number" || !request.candidateId) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "vote request requires term and candidateId",
      );
    }
    if (!this.clusterNodes.has(request.candidateId)) {
      throw new HsmAdapterError(
        "UNKNOWN_CANDIDATE",
        `candidate ${request.candidateId} not in cluster`,
      );
    }
    ...
    const candidateLastLogTerm =
      typeof request.lastLogTerm === "number" ? request.lastLogTerm : 0;
    const candidateLastLogIndex =
      typeof request.lastLogIndex === "number" ? request.lastLogIndex : 0;
  ...
  async appendAndReplicate(command) {
    if (this._state !== NODE_STATE.LEADER) {
      throw new HsmAdapterError(
        "NOT_LEADER",
        `node ${this.nodeId} is not the leader (state: ${this._state})`,
      );
    }
  ...
  async addPeerKey(peerNodeId, publicKey) {
    ...
    if (!peerNodeId || typeof peerNodeId !== "string") {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "peerNodeId must be a non-empty string",
      );
    }
    if (!publicKey) {
  ...
  createSnapshot(stateOverride) {
    if (this._commitIndex === 0) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "cannot snapshot with commitIndex=0",
      );
    }
    ...
    const lastSnapshotTerm = lastEntry
      ? lastEntry.term
      : this._lastSnapshotTerm;
  ...
  installSnapshot(request = {}) {
    if (typeof request.term !== "number" || !request.leaderId) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "installSnapshot requires term and leaderId",
      );
    }
    if (!this.clusterNodes.has(request.leaderId)) {
      throw new HsmAdapterError(
        "UNKNOWN_LEADER",
        `leader ${request.leaderId} not in cluster`,
      );
    }
    if (
      typeof request.lastIncludedIndex !== "number" ||
      typeof request.lastIncludedTerm !== "number"
    ) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "installSnapshot requires lastIncludedIndex and lastIncludedTerm",
      );
    }
    ...
    if (!this.verifyRpcFrame(payload, request.leaderId, request.signature)) {
      ...
      return {
        success: false,
        term: this._currentTerm,
        matchIndex: this._lastSnapshotIndex,
        reason: "signature_invalid",
      };
    }
    ...
    this._lastAppliedIndex = Math.max(
      this._lastAppliedIndex,
      this._lastSnapshotIndex,
    );
  ...
  appendEntries(request = {}) {
    if (typeof request.term !== "number" || !request.leaderId) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "appendEntries requires term and leaderId",
      );
    }
    if (!this.clusterNodes.has(request.leaderId)) {
      throw new HsmAdapterError(
    ...
    if (!this.verifyRpcFrame(payload, request.leaderId, request.signature)) {
      return {
        success: false,
        term: this._currentTerm,
        matchIndex: this._matchIndexFor(this.nodeId),
        reason: "signature_invalid",
      };
    }
    ...
    if (
      typeof request.leaderCommit === "number" &&
      request.leaderCommit > this._commitIndex
    ) {
      this._commitIndex = Math.min(
        request.leaderCommit,
        this._log.length + this._lastSnapshotIndex,
      );
      this._applyCommittedEntries();
    }
  ...
  _becomeLeader() {
    ...
    for (const targetId of this.clusterNodes) {
      ...
      this._nextIndex.set(
        targetId,
        this._log.length + this._lastSnapshotIndex + 1,
      );
