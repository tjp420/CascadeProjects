...
class Musig2HsmOrchestrator {
  ...
  async createSession({
    ...
    if (q.length !== participantIds.length) {
      throw new HsmAdapterError(
        "INVALID_INPUT",
        "quorum length must match participantIds length",
      );
    }
    if (q.length < 2) {
      throw new HsmAdapterError(
    ...
    if (this._sessions.size >= this._maxSessions) {
      throw new HsmAdapterError(
        ...
        `maximum ${this._maxSessions} sessions reached`,
      );
    }
    ...
    this._emit(ORCHESTRATOR_EVENT.SESSION_CREATED, {
      sessionId,
      tenantId,
  ...
  async generateNonces(sessionId) {
    ...
    for (let i = 0; i < session.participantIds.length; i++) {
      const nonceResult = this._nonceGen.generateNoncePair(
        sessionId + "-p" + i,
      );
    ...
    this._emit(ORCHESTRATOR_EVENT.NONCES_GENERATED, {
      sessionId,
      count: nonces.length,
    });
  ...
  async aggregateKeys(sessionId) {
    ...
    const aggPublicKey = this._aggregator.aggregatePublicKeys(
      session.publicKeys,
      session.quorum,
    );
    ...
    this._emit(ORCHESTRATOR_EVENT.KEYS_AGGREGATED, {
      sessionId,
      aggPublicKey: "redacted",
    });
    return aggPublicKey;
  }
  ...
  async computeBindingFactor(sessionId) {
    ...
    const bindingFactor = this._aggregator.computeBindingFactor(
  ...
  async evaluateShares(sessionId) {
    ...
    for (let i = 0; i < session.wrappedShares.length; i++) {
      const share = await this.unwrapKeyShare(
        session.tenantId,
        session.wrappedShares[i],
      );
      keyShares.push(share);
    }

    // Compute challenge
    const publicNonces = session.nonces.map((n) =>
      BigInt("0x" + n.publicCommitment.h1),
    );
    const aggNonce = this._aggregator.aggregateNonces(
      publicNonces,
      session.bindingFactor,
    );
    session.aggNonce = aggNonce;

    const challenge = this._aggregator.computeChallenge(
      session.aggPublicKey,
      aggNonce,
      session.messageHash,
    );
    ...
    const partialShares = [];
    for (let i = 0; i < session.participantIds.length; i++) {
      const lagrangeWeight = this._aggregator.computeLagrangeWeight(
        session.quorum[i],
        session.quorum,
      );
    ...
    this._emit(ORCHESTRATOR_EVENT.SHARES_EVALUATED, {
      sessionId,
      count: partialShares.length,
    });
  ...
  async assembleSignature(sessionId) {
    ...
    const signature = this._aggregator.assembleSignature(
      session.aggNonce,
      session.partialShares,
    );
  ...
  async verifySignature(sessionId, signature) {
    ...
    for (let i = 0; i < keyShares.length; i++) {
      const lambda = this._aggregator.computeLagrangeWeight(
        session.quorum[i],
        session.quorum,
      );
    ...
    for (let i = 0; i < session.sealedNonces.length; i++) {
      const nonce = await this.unsealNonce(
        session.tenantId,
        session.sealedNonces[i],
      );
    ...
    for (let i = 0; i < unsealedNonces.length; i++) {
      ...
      const blended = this._aggregator.field.add(
        k1,
        this._aggregator.field.mul(session.bindingFactor, k2),
      );
    ...
    if (valid) {
      ...
      this._emit(ORCHESTRATOR_EVENT.SIGNATURE_VERIFICATION_FAILED, {
        sessionId,
      });
    }
  ...
  _getSession(sessionId) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "SESSION_TIMEOUT",
        `session ${sessionId} timed out after ${idleMs}ms idle`,
      );
    }
