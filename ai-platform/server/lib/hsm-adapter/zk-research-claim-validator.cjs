...
class ZkResearchClaimValidator {
  ...
  verifyResearchClaim(request) {
    ...
    if (pool.status !== 'open') {
      throw new HsmAdapterError('POLARCLAIM_POOL_NOT_OPEN', `pool ${request.poolId} is not open`);
    }
    if (this.policy.requirePolarResearchOversightCommitteeAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('POLARCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED', 'polar research oversight committee attestation invalid');
        }
      } catch (err) {
    ...
    if (this.policy.banMalformedOrOutOfOrderResearchClaims && request.peerId && this._bannedPeers.has(request.peerId)) {
    ...
    if (this.policy.banMalformedOrOutOfOrderResearchClaims && this._verifiedClaims.has(claimHash)) {
      if (request.peerId) this._bannedPeers.add(request.peerId);
      throw new HsmAdapterError('POLARCLAIM_DUPLICATE', `duplicate research claim for pool ${request.poolId}`);
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.blindedResearchDataCommitment || !request.blindedSensorTelemetryCommitment || !request.blindedInstitutionIdentityCommitment) {
    throw new HsmAdapterError('POLARCLAIM_FIELDS_MISSING', 'blindedResearchDataCommitment, blindedSensorTelemetryCommitment, and blindedInstitutionIdentityCommitment are required');
  }
  if (!request.zkResearchRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderResearchClaims && request.peerId) bannedPeers.add(request.peerId);
    throw new HsmAdapterError('POLARCLAIM_ZK_PROOF_MISSING', 'zkResearchRangeProofHash is required');
  }
  if (!request.vdfProofHash) {
    if (policy.banMalformedOrOutOfOrderResearchClaims && request.peerId) bannedPeers.add(request.peerId);
    throw new HsmAdapterError('POLARCLAIM_VDF_PROOF_MISSING', 'vdfProofHash is required');
  }
  if (policy.requirePolarResearchOversightCommitteeAttestation && !request.polarResearchOversightCommitteeAttestation) {
    throw new HsmAdapterError('POLARCLAIM_OVERSIGHT_ATTESTATION_MISSING', 'polar research oversight committee attestation is required');
  }
}
