...
class ThresholdIdentityIssuer {
  ...
  initiate(request) {
    if (!request.entityId || !request.identityHash) {
      throw new HsmAdapterError(
        "ISSUANCE_FIELDS_MISSING",
        "entityId and identityHash are required",
      );
    }
    if (this.policy.requireCommitteeAttestation && this._attestationClient) {
      ...
      if (!result.verified) {
        throw new HsmAdapterError(
          "ISSUANCE_HOST_UNATTESTED",
          `host attestation invalid for entity ${request.entityId}`,
        );
      }
    }
    const issuance = {
  ...
  sign(entityId, committeeMemberId, attestation, signature) {
    ...
    if (!issuance) {
      throw new HsmAdapterError(
        "ISSUANCE_NOT_FOUND",
        `no pending issuance for entity ${entityId}`,
      );
    }
    if (
      issuance.committeeSignatures.length >=
      (this.policy.maxCommitteeSize || 10)
    ) {
      throw new HsmAdapterError(
        "ISSUANCE_COMMITTEE_FULL",
        `committee size exceeds maximum ${this.policy.maxCommitteeSize}`,
      );
    }
    if (this.policy.requireCommitteeAttestation && this._attestationClient) {
    ...
    if (
      issuance.committeeSignatures.length >=
      (this.policy.minIssuanceQuorum || 3)
    ) {
