...
class PqcHolographicStorageContentAddressableGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireHolographicStorageAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.holographicStorageAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "HOLOGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "holographic storage authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "HOLOGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "HOLOGATE_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.phaseValidationWindowSeconds === "number" &&
      request.phaseValidationWindowSeconds >
        (this.policy.maxPhaseValidationWindowSeconds || 10)
    ) {
      throw new HsmAdapterError(
        "HOLOGATE_PHASE_VALIDATION_WINDOW_EXCEEDED",
        `phase validation window seconds ${request.phaseValidationWindowSeconds} exceeds maximum ${this.policy.maxPhaseValidationWindowSeconds}`,
      );
    }
    if (
      typeof request.volumetricChainDepth === "number" &&
      request.volumetricChainDepth > (this.policy.maxVolumetricChainDepth || 50)
    ) {
      throw new HsmAdapterError(
        "HOLOGATE_VOLUMETRIC_DEPTH_EXCEEDED",
        `volumetric chain depth ${request.volumetricChainDepth} exceeds maximum ${this.policy.maxVolumetricChainDepth}`,
      );
    }
    const poolId =
  ...
  completeAccreditation(request) {
    ...
    if (!pool.holographicClaimVerified) {
      throw new HsmAdapterError(
        "HOLOGATE_HOLOGRAPHIC_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} holographic claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "HOLOGATE_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minHolographicQuorum || 20)) {
      throw new HsmAdapterError(
        "HOLOGATE_QUORUM_INSUFFICIENT",
        `holographic quorum signatures ${signatures.length} below minimum ${this.policy.minHolographicQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  ...
  if (!request.sourceVolumetricSectorId || !request.targetVolumetricSectorId) {
    throw new HsmAdapterError(
      "HOLOGATE_FIELDS_MISSING",
      "sourceVolumetricSectorId and targetVolumetricSectorId are required",
    );
  }
  if (
    !request.blindedVolumetricSectorDigestCommitment ||
  ...
  if (typeof request.phaseValidationWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "HOLOGATE_FIELDS_MISSING",
      "phaseValidationWindowSeconds is required",
    );
  }
  if (typeof request.volumetricChainDepth !== "number") {
    throw new HsmAdapterError(
      "HOLOGATE_FIELDS_MISSING",
      "volumetricChainDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "HOLOGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "HOLOGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "HOLOGATE_OVERSIGHT_ATTESTATION_MISSING",
