 ...
 *
 * Succinct manifest verifier that processes non-interactive
 * zero-knowledge range and accreditation proofs, ensuring
 * that an entity's hidden manifest claim status strictly
 * satisfies policy-defined thresholds without disclosing
 * individual cargo or carrier attributes. Triggers defensive node
 * bans for malformed or out-of-order manifest claims.
 *
 * Extended with hardware-accelerated SNARK proof generation,
 * batch manifest claim verification, slashing window
...
const SLASH_REASON = {
  ...
  TRANSIT_WINDOW_OUT_OF_BOUNDS: 'transit_window_out_of_bounds',
...
class ZkManifestClaimValidator {
  ...
  verifyManifestClaim(request) {
    ...
    if (typeof request.transitWindowSeconds === 'number' && request.transitWindowSeconds > (this.policy.maxTransitWindowSeconds || 7776000)) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.TRANSIT_WINDOW_OUT_OF_BOUNDS);
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an manifest claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('LOGICLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.transitLogMetric !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('LOGICLAIM_HW_PROOF_FIELDS_MISSING',
        'transitLogMetric and claimValue numbers are required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('LOGICLAIM_HUB_MISSING', 'cross-border logistics gating hub is required');
    }
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.transitLogMetric}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
      zkManifestRangeProofHash: proofHash,
      poolId: request.poolId,
      transitLogMetric: request.transitLogMetric,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit('LOGICLAIM_HW_SNARK_PROOF_GENERATED', { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple manifest claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyManifestClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('LOGICLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('LOGICLAIM_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyManifestClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || 'LOGICLAIM_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit('LOGICLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('LOGICLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('LOGICLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('LOGICLAIM_HUB_MISSING', 'cross-border logistics gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('LOGICLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
    const maxWindow = this.policy.maxTransitWindowSeconds || 7776000;
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId) {
      throw new HsmAdapterError('LOGICLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('LOGICLAIM_AGG_NO_SIGNATURES', 'partialSignatures array is required');
    }
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('LOGICLAIM_PEER_BANNED',
    ...
    if (partialSignatures.length < (this.policy.minCustomsQuorum || 3)) {
      throw new HsmAdapterError('LOGICLAIM_AGG_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('LOGICLAIM_PARTIAL_SIGS_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit('LOGICLAIM_SLASHED', { poolId, peerId, reason });
    }
  }
}
