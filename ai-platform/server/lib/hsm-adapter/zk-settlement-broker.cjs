...
class ZkSettlementBroker {
  ...
  sign(settlementId, nodeId, attestation, signature) {
    ...
    if (this.policy.requireNodeAttestation && this._attestationClient) {
      ...
      if (!result.verified) {
        throw new HsmAdapterError(
          "SETTLEMENT_NODE_UNATTESTED",
          `node ${nodeId} attestation invalid`,
        );
      }
    }
  ...
  finalize(settlementId) {
    ...
    if (
      settlement.nodeSignatures.length <
      (this.policy.minClearingNodeQuorum || 3)
    ) {
      throw new HsmAdapterError(
        "SETTLEMENT_QUORUM_INSUFFICIENT",
        `signatures ${settlement.nodeSignatures.length} below minimum ${this.policy.minClearingNodeQuorum}`,
      );
    }
    if (this._prover) {
...
function _validateInitiate(policy, request) {
  if (!request.settlementId || !request.assetId) {
    throw new HsmAdapterError(
      "SETTLEMENT_FIELDS_MISSING",
      "settlement id and asset id are required",
    );
  }
  if (
    !Array.isArray(request.clearingNodes) ||
    request.clearingNodes.length < (policy.minClearingNodeQuorum || 3)
  ) {
    throw new HsmAdapterError(
      "SETTLEMENT_NODES_INSUFFICIENT",
      `clearing nodes ${(request.clearingNodes || []).length} below minimum ${policy.minClearingNodeQuorum}`,
    );
  }
  if (
    typeof request.incomingCommitment !== "bigint" &&
    typeof request.incomingCommitment !== "number"
  ) {
    throw new HsmAdapterError(
      "SETTLEMENT_INCOMING_INVALID",
      "incoming commitment is required",
    );
  }
  if (
    typeof request.outgoingCommitment !== "bigint" &&
    typeof request.outgoingCommitment !== "number"
  ) {
    throw new HsmAdapterError(
      "SETTLEMENT_OUTGOING_INVALID",
      "outgoing commitment is required",
    );
  }
  const bitWidth =
    _bitWidth(request.incomingCommitment) ||
    _bitWidth(request.outgoingCommitment);
  if (
    bitWidth < policy.minAssetBitWidth ||
    bitWidth > policy.maxAssetBitWidth
  ) {
    throw new HsmAdapterError(
      "SETTLEMENT_BIT_WIDTH_BLOCKED",
      `asset bit width ${bitWidth} outside allowed [${policy.minAssetBitWidth}, ${policy.maxAssetBitWidth}]`,
    );
  }
  if (
    typeof request.settlementTimeoutSeconds === "number" &&
    request.settlementTimeoutSeconds > policy.maxSettlementTimeoutSeconds
  ) {
    throw new HsmAdapterError(
      "SETTLEMENT_TIMEOUT_EXCEEDED",
      `timeout ${request.settlementTimeoutSeconds}s exceeds maximum ${policy.maxSettlementTimeoutSeconds}s`,
    );
  }
}
