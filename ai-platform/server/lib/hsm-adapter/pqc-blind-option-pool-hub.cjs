...
class PqcBlindOptionPoolHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('OPTPOOL_MAX_POOLS',
    ...
    if (vdfDifficulty < VDF_PARAMS.minDifficulty || vdfDifficulty > VDF_PARAMS.maxDifficulty) {
      throw new HsmAdapterError('OPTPOOL_VDF_DIFFICULTY_INVALID',
        `VDF difficulty ${vdfDifficulty} outside range [${VDF_PARAMS.minDifficulty}, ${VDF_PARAMS.maxDifficulty}]`);
    }
    const vdfLock = this._generateVdfLock(vdfDifficulty, request.expirationTimestamp, request.enforceVdfLock === true);
    const pool = {
      poolId,
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('OPTPOOL_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('OPTPOOL_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('OPTPOOL_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  executeContract(request) {
    ...
    if (pool.vdfLock && pool.vdfLock.enforced && now < pool.vdfLock.unlockTimestamp) {
      throw new HsmAdapterError('OPTPOOL_VDF_LOCKED',
  ...
  settleContract(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('OPTPOOL_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('OPTPOOL_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.EXECUTED) {
      throw new HsmAdapterError('OPTPOOL_NOT_EXECUTED',
        `pool ${request.poolId} status is ${pool.status}, expected executed`);
    }
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('OPTPOOL_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('OPTPOOL_SETTLE_CHAIN_MISMATCH',
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('OPTPOOL_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minExecutionSignatureQuorum || 3)) {
      throw new HsmAdapterError('OPTPOOL_EXEC_QUORUM_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('OPTPOOL_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.EXECUTED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('OPTPOOL_ALREADY_EXECUTED',
        `pool ${poolId} has been executed/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('OPTPOOL_ALREADY_CANCELLED',
  ...
  expirePool(poolId) {
    ...
    if (pool.status === POOL_STATUS.EXPIRED) {
      throw new HsmAdapterError('OPTPOOL_ALREADY_EXPIRED', `pool ${poolId} is already expired`);
    }
  ...
  _generateVdfLock(difficulty, expirationTimestamp, enforced) {
    ...
    const vdfProofHash = crypto.createHash('sha256')
