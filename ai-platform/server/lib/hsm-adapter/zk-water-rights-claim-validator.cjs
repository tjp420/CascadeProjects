 ...
 * that an entity's hidden water claim status strictly
 * satisfies policy-defined thresholds without disclosing
 * individual water or watershed attributes. Triggers defensive node
 * bans for malformed or out-of-order water claims.
 *
 * Extended with hardware-accelerated SNARK proof generation,
 * batch water claim verification, slashing window
...
class ZkWaterRightsClaimValidator {
  ...
  verifyWaterClaim(request) {
    ...
    if (!this._hub) {
      throw new HsmAdapterError('WATERCLAIM_HUB_MISSING', 'water rights allocation gating hub is required');
    }
    if (this.policy.requireWatershedOversightCommitteeAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('WATERCLAIM_COMMITTEE_UNATTESTED', 'watershed oversight committee attestation invalid');
        }
      } catch (err) {
    ...
    if (typeof request.attestationAuthority === 'string' && !this.policy.allowedAttestationAuthorities.includes(request.attestationAuthority)) {
      throw new HsmAdapterError('WATERCLAIM_AUTHORITY_BLOCKED', `attestation authority ${request.attestationAuthority} is not allowed; permitted: ${this.policy.allowedAttestationAuthorities.join(', ')}`);
    }
    if (typeof request.peerId === 'string' && this._bannedPeers.has(request.peerId)) {
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.BANNED_PEER);
      throw new HsmAdapterError('WATERCLAIM_PEER_BANNED', `peer ${request.peerId} is banned`);
    }
    if (!request.zkWaterRangeProofHash || typeof request.zkWaterRangeProofHash !== 'string') {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError('WATERCLAIM_ZK_PROOF_MISSING', 'zero-knowledge water range proof hash is required');
    }
    if (!request.mpcProof || typeof request.mpcProof !== 'string') {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.MALFORMED);
      throw new HsmAdapterError('WATERCLAIM_MPC_PROOF_MISSING', 'MPC proof is required');
    }
    const pool = this._hub.getPool(request.poolId);
    if (!pool) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.POOL_NOT_FOUND);
      throw new HsmAdapterError('WATERCLAIM_POOL_NOT_FOUND', `pool ${request.poolId} not found`);
    }
    if (typeof request.allocationWindowSeconds === 'number' && request.allocationWindowSeconds > (this.policy.maxAllocationWindowSeconds || 31536000)) {
      ...
      throw new HsmAdapterError('WATERCLAIM_ALLOCATION_WINDOW_OUT_OF_BOUNDS', `allocation window seconds ${request.allocationWindowSeconds} exceeds maximum ${this.policy.maxAllocationWindowSeconds}`);
    }
    ...
    if (this._verifiedClaims.has(claimKey)) {
      ...
      throw new HsmAdapterError('WATERCLAIM_DUPLICATE', `water claim for pool ${request.poolId} already verified`);
    }
    ...
    const claim = {
      ...
      blindedWatershedFlowCommitment: request.blindedWatershedFlowCommitment || 'unspecified',
      ...
      watershedOversightCommitteeAttestationHash: request.watershedOversightCommitteeAttestationHash || 'unspecified',
      verifiedAt: now,
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an water claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('WATERCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.watershedFlow !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('WATERCLAIM_HW_PROOF_FIELDS_MISSING',
        'watershedFlow and claimValue numbers are required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('WATERCLAIM_HUB_MISSING', 'water rights allocation gating hub is required');
    }
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.watershedFlow}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
      zkWaterRangeProofHash: proofHash,
      poolId: request.poolId,
      watershedFlow: request.watershedFlow,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit('WATERCLAIM_HW_SNARK_PROOF_GENERATED', { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple water claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyWaterClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('WATERCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('WATERCLAIM_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyWaterClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || 'WATERCLAIM_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit('WATERCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('WATERCLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('WATERCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('WATERCLAIM_HUB_MISSING', 'water rights allocation gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('WATERCLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
    const maxWindow = this.policy.maxAllocationWindowSeconds || 31536000;
   ...
   * @param {string} poolId
   * @param {object[]} mpcProofs - Array of {peerId, signature}
   * @returns {object}
   */
  aggregateMpcProofs(poolId, mpcProofs) {
    if (!poolId) {
      throw new HsmAdapterError('WATERCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(mpcProofs) || mpcProofs.length === 0) {
      throw new HsmAdapterError('WATERCLAIM_AGG_NO_SIGNATURES', 'mpcProofs array is required');
    }
    for (const sig of mpcProofs) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('WATERCLAIM_PEER_BANNED',
          `peer ${sig.peerId} is banned and cannot participate in aggregation`);
      }
    }
    if (mpcProofs.length < (this.policy.minWatershedQuorum || 4)) {
      throw new HsmAdapterError('WATERCLAIM_AGG_INSUFFICIENT',
        `${mpcProofs.length} signatures below minimum ${this.policy.minWatershedQuorum || 4}`);
    }
    const aggregatedSignature = crypto.createHash('sha256')
      .update(mpcProofs.map(s => s.signature).join(':'))
      .digest('hex');
    const result = {
      poolId,
      signatureCount: mpcProofs.length,
      aggregatedSignature,
      participantIds: mpcProofs.map(s => s.peerId || 'anonymous'),
      aggregatedAt: Math.floor(Date.now() / 1000),
    };
    if (this._audit) {
      this._audit('WATERCLAIM_MPC_PROOFS_AGGREGATED', { poolId, count: mpcProofs.length });
    }
    return result;
  }
  ...
  _banPeerIfPolicy(request) {
    if (this.policy.banMalformedOrOutOfOrderWaterClaims && typeof request.peerId === 'string') {
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit('WATERCLAIM_SLASHED', { poolId, peerId, reason });
    }
  }
}
...
function _validateClaimRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError('WATERCLAIM_FIELDS_MISSING', 'poolId is required');
  }
  if (policy.requireWatershedOversightCommitteeAttestation && !request.watershedOversightCommitteeAttestation) {
    throw new HsmAdapterError('WATERCLAIM_ATTESTATION_MISSING', 'watershed oversight committee attestation is required');
  }
}
