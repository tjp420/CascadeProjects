 ...
 *
 * Extended with batch pool initialization, claim scope
...
class PqcPatentVerificationGatingHub {
  ...
  /**
   * Batch initialize multiple patent verification gating pools.
  ...
  /**
   * Rebalance claim scope depth for a pool.
   * @param {object} request
   * @returns {object}
   */
  ...
  getPools() {
    return Array.from(this._pools.values()).map((p) => ({
      ...
      status: p.status,
      claimScopeDepth: p.claimScopeDepth,
      patentExpirationSeconds: p.patentExpirationSeconds,
      patentClaimVerified: p.patentClaimVerified,
    }));
  }
