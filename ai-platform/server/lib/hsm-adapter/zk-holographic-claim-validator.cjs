...
class ZkHolographicClaimValidator {
  ...
  verifyHolographicClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError('HOLOCCLAIM_POOL_NOT_FOUND', `pool ${request.poolId} not found`);
    }
    if (pool.status !== 'open') {
      throw new HsmAdapterError('HOLOCCLAIM_POOL_NOT_OPEN', `pool ${request.poolId} is not open`);
    }
    if (this.policy.requireHolographicEthicsOversightCommitteeAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('HOLOCCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED', 'holographic ethics oversight committee attestation invalid');
        }
      } catch (err) {
    ...
    if (this.policy.banMalformedOrOutOfOrderHolographicClaims && request.peerId && this._bannedPeers.has(request.peerId)) {
      throw new HsmAdapterError('HOLOCCLAIM_PEER_BANNED', `peer ${request.peerId} is banned`);
    }
    ...
    if (this.policy.banMalformedOrOutOfOrderHolographicClaims && this._verifiedClaims.has(claimHash)) {
      if (request.peerId) this._bannedPeers.add(request.peerId);
      throw new HsmAdapterError('HOLOCCLAIM_DUPLICATE', `duplicate holographic claim for pool ${request.poolId}`);
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  if (!request.poolId) {
    throw new HsmAdapterError('HOLOCCLAIM_FIELDS_MISSING', 'poolId is required');
  }
  if (!request.blindedVolumetricSectorDigestCommitment || !request.blindedHolographicStateCommitment || !request.blindedInterferencePatternPhaseCommitment) {
    throw new HsmAdapterError('HOLOCCLAIM_FIELDS_MISSING', 'blindedVolumetricSectorDigestCommitment, blindedHolographicStateCommitment, and blindedInterferencePatternPhaseCommitment are required');
  }
  if (!request.zkHolographicRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderHolographicClaims && request.peerId) bannedPeers.add(request.peerId);
    throw new HsmAdapterError('HOLOCCLAIM_ZK_PROOF_MISSING', 'zkHolographicRangeProofHash is required');
  }
  if (!request.holographicStateDigest) {
    if (policy.banMalformedOrOutOfOrderHolographicClaims && request.peerId) bannedPeers.add(request.peerId);
    throw new HsmAdapterError('HOLOCCLAIM_HOLOGRAPHIC_STATE_DIGEST_MISSING', 'holographicStateDigest is required');
  }
  if (policy.requireHolographicEthicsOversightCommitteeAttestation && !request.holographicEthicsOversightCommitteeAttestation) {
    throw new HsmAdapterError('HOLOCCLAIM_OVERSIGHT_ATTESTATION_MISSING', 'holographic ethics oversight committee attestation is required');
  }
}
