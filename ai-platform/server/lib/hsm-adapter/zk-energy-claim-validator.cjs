 ...
 * batch energy claim verification, slashing window
...
const SLASH_REASON = {
  ...
  CERTIFICATE_EXPIRATION_OUT_OF_BOUNDS: 'certificate_expiration_out_of_bounds',
...
class ZkEnergyClaimValidator {
   ...
   * @param {PqcEnergyCertificateGatingHub} options.hub
  ...
  verifyEnergyClaim(request) {
    ...
    if (!request.zkEnergyRangeProofHash || typeof request.zkEnergyRangeProofHash !== 'string') {
      ...
      throw new HsmAdapterError('ENERGYCLAIM_ZK_PROOF_MISSING', 'zero-knowledge academic range proof hash is required');
    }
    ...
    if (typeof request.certificateExpirationSeconds === 'number' && request.certificateExpirationSeconds > (this.policy.maxCertificateExpirationSeconds || 63072000)) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.CERTIFICATE_EXPIRATION_OUT_OF_BOUNDS);
      throw new HsmAdapterError('ENERGYCLAIM_CERTIFICATE_EXPIRATION_OUT_OF_BOUNDS', `transcript expiration seconds ${request.certificateExpirationSeconds} exceeds maximum ${this.policy.maxCertificateExpirationSeconds}`);
    }
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for an energy claim.
   * @param {object} request
   * @returns {object}
   */
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('ENERGYCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.gridMetric !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('ENERGYCLAIM_HW_PROOF_FIELDS_MISSING',
        'gridMetric and claimValue numbers are required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('ENERGYCLAIM_HUB_MISSING', 'energy certificate gating hub is required');
    }
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.gridMetric}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
      zkEnergyRangeProofHash: proofHash,
      poolId: request.poolId,
      gridMetric: request.gridMetric,
    ...
    this._hwProofCount++;
    if (this._audit) {
      this._audit('ENERGYCLAIM_HW_SNARK_PROOF_GENERATED', { ...proof });
    }
    return proof;
  }

  /**
   * Batch verify multiple energy claims.
   * @param {object[]} requests
   * @returns {object}
   */
  batchVerifyEnergyClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('ENERGYCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('ENERGYCLAIM_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyEnergyClaim(req);
        results.push({
          poolId: req.poolId,
        ...
        results.push({
          ...
          verified: false,
          error: err.code || 'ENERGYCLAIM_BATCH_ERROR',
        });
        failedCount++;
      }
    }
    ...
    if (this._audit) {
      this._audit('ENERGYCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('ENERGYCLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('ENERGYCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('ENERGYCLAIM_HUB_MISSING', 'energy certificate gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('ENERGYCLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
    const maxWindow = this.policy.maxCertificateExpirationSeconds || 63072000;
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId) {
      throw new HsmAdapterError('ENERGYCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('ENERGYCLAIM_AGG_NO_SIGNATURES', 'partialSignatures array is required');
    }
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('ENERGYCLAIM_PEER_BANNED',
    ...
    if (partialSignatures.length < (this.policy.minGridOperatorQuorum || 3)) {
      throw new HsmAdapterError('ENERGYCLAIM_AGG_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('ENERGYCLAIM_PARTIAL_SIGS_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  _recordSlash(poolId, peerId, reason) {
    ...
    if (this._audit) {
      this._audit('ENERGYCLAIM_SLASHED', { poolId, peerId, reason });
    }
  }
}
