    ...
    for (let i = 0; i < jobs; i++) {
      ...
      pool.submit(async () => {
        ...
        if (completed === jobs) {
          ...
          const p50 = latencies[Math.floor(jobs * 0.50) - 1] || latencies[latencies.length-1];
          const p95 = latencies[Math.floor(jobs * 0.95) - 1] || latencies[latencies.length-1];
          const p99 = latencies[Math.floor(jobs * 0.99) - 1] || latencies[latencies.length-1];
          ...
          console.log(JSON.stringify({
            ...
            p95,
            p99,
            worker_completed: metrics.hsm_track112_worker_task_done_total || 0,
