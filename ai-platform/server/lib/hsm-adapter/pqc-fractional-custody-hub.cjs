...
class PqcFractionalCustodyHub {
  ...
  initializeVault(request) {
    ...
    if (this._vaults.size >= this._maxVaults) {
      throw new HsmAdapterError('FRACVAULT_MAX_VAULTS',
  ...
  batchInitializeVaults(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('FRACVAULT_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('FRACVAULT_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('FRACVAULT_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  recordRelease(vaultId, fractionValue) {
    ...
    if (vault.status !== VAULT_STATUS.OPEN && vault.status !== VAULT_STATUS.ESCROWED) {
  ...
  lockEscrow(request) {
    if (!request || !request.vaultId) {
      throw new HsmAdapterError('FRACVAULT_ESCROW_FIELDS_MISSING', 'vaultId is required');
    }
    ...
    if (vault.status !== VAULT_STATUS.OPEN) {
      throw new HsmAdapterError('FRACVAULT_NOT_OPEN',
    ...
    if (!Object.values(ESCROW_LOCK_TYPES).includes(lockType)) {
      throw new HsmAdapterError('FRACVAULT_ESCROW_LOCK_TYPE_INVALID',
        `lock type ${lockType} is not valid; allowed: ${Object.values(ESCROW_LOCK_TYPES).join(', ')}`);
    }
    const now = Math.floor(Date.now() / 1000);
    const escrowId = request.escrowId || `escrow-${crypto.randomBytes(4).toString('hex')}`;
    const escrow = {
      escrowId,
  ...
  releaseEscrow(escrowId) {
    ...
    if (!escrow) {
      throw new HsmAdapterError('FRACVAULT_ESCROW_NOT_FOUND', `escrow ${escrowId} not found`);
    }
    const vault = this._vaults.get(escrow.vaultId);
    if (!vault) {
      throw new HsmAdapterError('FRACVAULT_NOT_FOUND', `vault ${escrow.vaultId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
    ...
    if (this._audit) {
      this._audit('FRACVAULT_ESCROW_RELEASED', { escrowId, vaultId: escrow.vaultId });
    }
  ...
  settleVault(request) {
    if (!request || !request.vaultId) {
      throw new HsmAdapterError('FRACVAULT_SETTLE_FIELDS_MISSING', 'vaultId is required');
    }
    const vault = this._vaults.get(request.vaultId);
    if (!vault) {
      throw new HsmAdapterError('FRACVAULT_NOT_FOUND', `vault ${request.vaultId} not found`);
    ...
    if (vault.status !== VAULT_STATUS.LIQUIDATED) {
      throw new HsmAdapterError('FRACVAULT_NOT_LIQUIDATED',
    ...
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('FRACVAULT_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== vault.targetChainId) {
      throw new HsmAdapterError('FRACVAULT_SETTLE_CHAIN_MISMATCH',
  ...
  aggregateCustodianSignatures(vaultId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('FRACVAULT_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minCustodianQuorum || 3)) {
      throw new HsmAdapterError('FRACVAULT_QUORUM_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('FRACVAULT_SIGNATURES_AGGREGATED', { vaultId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelVault(vaultId) {
    ...
    if (vault.status === VAULT_STATUS.LIQUIDATED || vault.status === VAULT_STATUS.SETTLED) {
      throw new HsmAdapterError('FRACVAULT_ALREADY_LIQUIDATED',
        `vault ${vaultId} has been liquidated/settled and cannot be cancelled`);
    }
    if (vault.status === VAULT_STATUS.CANCELLED) {
      throw new HsmAdapterError('FRACVAULT_ALREADY_CANCELLED',
  ...
  _parseLiquidityBridge(bridge) {
    if (!bridge || typeof bridge !== 'object') {
      throw new HsmAdapterError('FRACVAULT_BRIDGE_INVALID', 'liquidityBridge must be an object');
    }
    if (!bridge.sourceChainId || !bridge.targetChainId) {
      throw new HsmAdapterError('FRACVAULT_BRIDGE_CHAINS_MISSING',
