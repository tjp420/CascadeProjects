 ...
 * batch retirement proof verification, slashing window
...
class ZkCarbonRetirementValidator {
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for retirement.
   * @param {object} request
   * @returns {object}
   */
  ...
  /**
   * Batch verify multiple retirement proofs.
  ...
  batchVerifyRetirementProofs(requests) {
    ...
    for (const req of requests) {
      try {
        const retirement = this.verifyRetirementProof(req);
        results.push({
          poolId: req.poolId,
  ...
  /**
   * Validate that a retirement falls within the slashing window.
   * @param {string} poolId
  ...
  /**
   * Get all slashed retirements.
   * @returns {Array}
   */
  getSlashedRetirements() {
    return this._slashedRetirements.slice();
  }
  ...
  getSlashingStats() {
    const byReason = {};
    for (const s of this._slashedRetirements) {
      byReason[s.reason] = (byReason[s.reason] || 0) + 1;
    }
    return {
      totalSlashes: this._slashedRetirements.length,
  ...
  getStats() {
    return {
      totalVerified: this._verifiedRetirements.size,
      totalSlashed: this._slashedRetirements.length,
