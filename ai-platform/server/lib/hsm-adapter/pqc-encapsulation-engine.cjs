...
function _kdf(ikm, info, length) {
  return Buffer.from(
    crypto.hkdfSync("sha256", ikm, Buffer.alloc(0), info, length),
  );
}
...
class PqcEncapsulationEngine {
  ...
  constructor(kemLevel = 768) {
    if (!SUPPORTED_LEVELS.has(kemLevel)) {
      throw new HsmAdapterError(
        "PQC_NOT_SUPPORTED",
        `kemLevel ${kemLevel} is not supported`,
      );
    }
  ...
  generateKeypair() {
    ...
    const publicKey = {
      seed: _kdf(
        seed,
        `SimpleBeacon:Track20:PQC:pk:${this._kemLevel}`,
        32 * this._k,
      ),
  ...
  encapsulate(publicKey) {
    ...
    const sharedSecret = _kdf(
      ikm,
      `SimpleBeacon:Track20:PQC:ss:${this._kemLevel}`,
      32,
    );
    const c1 = _kdf(
      ikm,
      `SimpleBeacon:Track20:PQC:c1:${this._kemLevel}`,
      32 * this._k,
    );
  ...
  decapsulate(ciphertext, secretKey) {
    if (!ciphertext || !secretKey) {
      throw new HsmAdapterError(
        "PQC_KEY_INTEGRITY",
        "ciphertext and secret key are required",
      );
    }
    if (secretKey.kemLevel !== this._kemLevel) {
      throw new HsmAdapterError(
        "PQC_KEY_INTEGRITY",
        "secret key KEM level mismatch",
      );
    }
    if (
      !Buffer.isBuffer(ciphertext.c1) ||
      !Buffer.isBuffer(ciphertext.c2) ||
      !Buffer.isBuffer(ciphertext.r)
    ) {
      throw new HsmAdapterError(
        "PQC_KEY_INTEGRITY",
        "ciphertext fields are not buffers",
      );
    }

    const publicKeySeed = _kdf(
      secretKey.seed,
      `SimpleBeacon:Track20:PQC:pk:${this._kemLevel}`,
      32 * this._k,
    );
    const expectedC1 = _kdf(
      Buffer.concat([publicKeySeed, ciphertext.r]),
      `SimpleBeacon:Track20:PQC:c1:${this._kemLevel}`,
      32 * this._k,
    );
    const expectedC2 = _kdf(
      Buffer.concat([publicKeySeed, ciphertext.r]),
      `SimpleBeacon:Track20:PQC:c2:${this._kemLevel}`,
      32,
    );

    if (
      !ciphertext.c1.equals(expectedC1) ||
      !ciphertext.c2.equals(expectedC2)
    ) {
