...
class ZkCatchClaimValidator {
  ...
  verifyCatchClaim(request) {
    ...
    if (typeof request.catchTrackingWindowSeconds === 'number' && request.catchTrackingWindowSeconds > (this.policy.maxCatchTrackingWindowSeconds || 2592000)) {
      this._banPeerIfPolicy(request);
      this._recordSlash(request.poolId, request.peerId, SLASH_REASON.TRACKING_WINDOW_OUT_OF_BOUNDS);
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('FISHERIESCLAIM_HW_PROOF_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof request.quotaAllocation !== 'number' || typeof request.claimValue !== 'number') {
      throw new HsmAdapterError('FISHERIESCLAIM_HW_PROOF_FIELDS_MISSING',
    ...
    const proofHash = crypto.createHash('sha256')
      .update(`${request.poolId}:${request.quotaAllocation}:${request.claimValue}:${this._hwAccelType}`)
      .digest('hex');
    const proof = {
  ...
  batchVerifyCatchClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('FISHERIESCLAIM_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('FISHERIESCLAIM_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('FISHERIESCLAIM_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    if (!poolId) {
      throw new HsmAdapterError('FISHERIESCLAIM_WINDOW_FIELDS_MISSING', 'poolId is required');
    }
    if (typeof claimTimestamp !== 'number' || claimTimestamp <= 0) {
      throw new HsmAdapterError('FISHERIESCLAIM_WINDOW_FIELDS_MISSING', 'claimTimestamp must be a positive number');
    }
    if (!this._hub) {
      throw new HsmAdapterError('FISHERIESCLAIM_HUB_MISSING', 'ocean fisheries allocation gating hub is required');
    }
    ...
    if (!pool) {
      throw new HsmAdapterError('FISHERIESCLAIM_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  aggregateProxyReEncryptionKeyDigests(poolId, proxyReEncryptionKeyDigests) {
    if (!poolId) {
      throw new HsmAdapterError('FISHERIESCLAIM_AGG_FIELDS_MISSING', 'poolId is required');
    }
    if (!Array.isArray(proxyReEncryptionKeyDigests) || proxyReEncryptionKeyDigests.length === 0) {
      throw new HsmAdapterError('FISHERIESCLAIM_AGG_NO_SIGNATURES', 'proxyReEncryptionKeyDigests array is required');
    }
    for (const sig of proxyReEncryptionKeyDigests) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('FISHERIESCLAIM_PEER_BANNED',
          `peer ${sig.peerId} is banned and cannot participate in aggregation`);
      }
    }
    if (proxyReEncryptionKeyDigests.length < (this.policy.minMaritimeQuorum || 5)) {
      throw new HsmAdapterError('FISHERIESCLAIM_AGG_INSUFFICIENT',
    ...
    const result = {
      ...
      aggregatedSignature,
      participantIds: proxyReEncryptionKeyDigests.map(s => s.peerId || 'anonymous'),
      aggregatedAt: Math.floor(Date.now() / 1000),
    };
    if (this._audit) {
      this._audit('FISHERIESCLAIM_PRE_KEY_DIGESTS_AGGREGATED', { poolId, count: proxyReEncryptionKeyDigests.length });
    }
    return result;
  }
