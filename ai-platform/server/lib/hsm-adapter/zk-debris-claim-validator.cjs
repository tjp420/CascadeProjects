  ...
  verifyDebrisClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "ORBITCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (pool.status !== "open") {
      throw new HsmAdapterError(
        "ORBITCLAIM_POOL_NOT_OPEN",
    ...
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "ORBITCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (
      this.policy.banMalformedOrOutOfOrderDebrisClaims &&
      request.peerId &&
      this._bannedPeers.has(request.peerId)
    ) {
      throw new HsmAdapterError(
        "ORBITCLAIM_PEER_BANNED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "ORBITCLAIM_DUPLICATE",
        `duplicate debris claim for pool ${request.poolId}`,
      );
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.zkDebrisRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderDebrisClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "ORBITCLAIM_ZK_PROOF_MISSING",
      "zkDebrisRangeProofHash is required",
    );
  }
  if (!request.homomorphicHashCommitment) {
    if (policy.banMalformedOrOutOfOrderDebrisClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "ORBITCLAIM_HOMOMORPHIC_COMMITMENT_MISSING",
      "homomorphicHashCommitment is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "ORBITCLAIM_OVERSIGHT_ATTESTATION_MISSING",
