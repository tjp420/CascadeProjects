...
class ZkRiskExposureValidator {
  ...
  batchVerifyClaims(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('CLAIMELIG_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('CLAIMELIG_BATCH_TOO_LARGE',
    ...
    for (const req of requests) {
      try {
        ...
        results.push({ claimId: claim.claimId, poolId: claim.poolId, verified: true });
        verifiedCount++;
      } catch (err) {
        results.push({
    ...
    if (this._audit) {
      this._audit('CLAIMELIG_BATCH_VERIFIED', { verifiedCount, failedCount, batchSize: requests.length });
    }
  ...
  generateHwSnarkProof(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('CLAIMELIG_GEN_FIELDS_MISSING', 'poolId is required');
    }
    if (!this._hub) {
      throw new HsmAdapterError('CLAIMELIG_HUB_MISSING', 'insurance underwriting hub is required');
    }
    ...
    if (typeof request.reserveValue !== 'number' || typeof request.premiumValue !== 'number') {
      throw new HsmAdapterError('CLAIMELIG_GEN_VALUES_MISSING',
    ...
    const zkRiskExposureProofHash = crypto.createHash('sha256')
      .update(`snark:${proofSeed.toString('hex')}:${request.poolId}:${request.reserveValue}:${request.premiumValue}`)
      .digest('hex');
    const proof = {
      poolId: request.poolId,
  ...
  aggregatePartialSignatures(poolId, partialSignatures) {
    if (!poolId || typeof poolId !== 'string') {
      throw new HsmAdapterError('CLAIMELIG_POOL_ID_REQUIRED', 'poolId is required');
    }
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('CLAIMELIG_NO_PARTIAL_SIGS', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minClaimQuorum || 3)) {
      throw new HsmAdapterError('CLAIMELIG_QUORUM_INSUFFICIENT',
    ...
    for (const sig of partialSignatures) {
      if (sig.peerId && this._bannedPeers.has(sig.peerId)) {
        throw new HsmAdapterError('CLAIMELIG_PEER_BANNED',
    ...
    if (this._audit) {
      this._audit('CLAIMELIG_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return aggregated;
  }
  ...
  validateSlashingWindow(poolId, claimTimestamp) {
    ...
    if (!pool) {
      throw new HsmAdapterError('CLAIMELIG_POOL_NOT_FOUND', `pool ${poolId} not found`);
    }
    if (typeof claimTimestamp !== 'number') {
      throw new HsmAdapterError('CLAIMELIG_TIMESTAMP_INVALID', 'claimTimestamp must be a number');
    }
    const windowStart = pool.initializedAt;
    const windowEnd = Math.floor(Date.now() / 1000) + this._slashingWindowSeconds;
    const withinWindow = claimTimestamp >= windowStart && claimTimestamp <= windowEnd;
    const result = {
      poolId,
      claimTimestamp,
      windowStart,
    ...
    if (!withinWindow && this._audit) {
      this._audit('CLAIMELIG_SLASHING_WINDOW_VIOLATION',
        { poolId, claimTimestamp, windowStart, windowEnd });
    }
    return result;
  }
  ...
  _recordSlash(request, reason) {
    ...
    this._slashCount++;
    if (this._audit) {
      this._audit('CLAIMELIG_SLASHED', { poolId: request.poolId, peerId: request.peerId, reason });
    }
  }
}
