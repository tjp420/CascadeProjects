...
class ZkMeshClaimValidator {
  ...
  validateClaim(claim) {
    ...
    if (elapsedMs > maxWindowMs) {
      ...
      throw new HsmAdapterError('MESHCLAIM_EPOCH_FINALITY_WINDOW_EXCEEDED', `mesh state-reconciliation claim is ${elapsedMs}ms old; maximum window is ${maxWindowMs}ms`);
    }

    if (typeof claim.epochFinalityWindowSeconds === 'number' && this.policy.maxEpochFinalityWindowSeconds !== undefined && claim.epochFinalityWindowSeconds > this.policy.maxEpochFinalityWindowSeconds) {
      this._issueChallenge(claim.poolId, 'epoch_finality_window_exceeded');
      throw new HsmAdapterError('MESHCLAIM_EPOCH_FINALITY_WINDOW_EXCEEDED', `epoch finality window seconds ${claim.epochFinalityWindowSeconds} exceeds maximum ${this.policy.maxEpochFinalityWindowSeconds}`);
    }

    if (typeof claim.reconciliationChainDepth === 'number' && this.policy.maxReconciliationChainDepth !== undefined && claim.reconciliationChainDepth > this.policy.maxReconciliationChainDepth) {
      this._issueChallenge(claim.poolId, 'reconciliation_chain_depth_exceeded');
      throw new HsmAdapterError('MESHCLAIM_RECONCILIATION_CHAIN_DEPTH_EXCEEDED', `reconciliation chain depth ${claim.reconciliationChainDepth} exceeds maximum ${this.policy.maxReconciliationChainDepth}`);
    }

    if (typeof claim.meshQuorum === 'number' && this.policy.minMeshQuorum !== undefined && claim.meshQuorum < this.policy.minMeshQuorum) {
      this._issueChallenge(claim.poolId, 'mesh_quorum_insufficient');
      throw new HsmAdapterError('MESHCLAIM_QUORUM_INSUFFICIENT', `mesh quorum ${claim.meshQuorum} below minimum ${this.policy.minMeshQuorum}`);
    }
    ...
    if (typeof claim.pqcSignatureScheme === 'string' && this.policy.allowedPqcSignatureSchemes && !this.policy.allowedPqcSignatureSchemes.includes(claim.pqcSignatureScheme)) {
      ...
      throw new HsmAdapterError('MESHCLAIM_PQC_SCHEME_BLOCKED', `PQC signature scheme ${claim.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(', ')}`);
    }
    ...
    if (typeof claim.attestationAuthority === 'string' && this.policy.allowedAttestationAuthorities && !this.policy.allowedAttestationAuthorities.includes(claim.attestationAuthority)) {
      ...
      throw new HsmAdapterError('MESHCLAIM_ATTESTATION_AUTHORITY_BLOCKED', `attestation authority ${claim.attestationAuthority} is not allowed; permitted: ${this.policy.allowedAttestationAuthorities.join(', ')}`);
    }

    if (claim.proofValid === false) {
      this._issueChallenge(claim.poolId, 'proof_invalid');
      throw new HsmAdapterError('MESHCLAIM_PROOF_INVALID', `mesh state-reconciliation proof for pool ${claim.poolId} is invalid`);
    }
    ...
    if (this.policy.banMalformedOrOutOfOrderMeshStateReconciliationClaims) {
      ...
      if (this._verifiedClaims.has(claimHash)) {
        ...
        throw new HsmAdapterError('MESHCLAIM_DUPLICATE', `duplicate mesh reconciliation claim for pool ${claim.poolId}`);
      }
...
function _validateClaimShape(claim) {
  if (!claim || typeof claim !== 'object') {
    throw new HsmAdapterError('MESHCLAIM_CLAIM_SHAPE_INVALID', 'claim must be an object');
  }
  if (!claim.poolId) {
    throw new HsmAdapterError('MESHCLAIM_CLAIM_SHAPE_INVALID', 'poolId is required');
  }
  if (!claim.confidentialStateReconciliationDigest) {
    throw new HsmAdapterError('MESHCLAIM_CLAIM_SHAPE_INVALID', 'confidentialStateReconciliationDigest is required');
  }
  if (typeof claim.timestampMs !== 'number') {
    throw new HsmAdapterError('MESHCLAIM_CLAIM_SHAPE_INVALID', 'timestampMs must be a number');
  }
}
