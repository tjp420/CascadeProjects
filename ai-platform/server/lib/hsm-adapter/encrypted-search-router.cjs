  ...
  route(request) {
    ...
    if (this.policy.requireSubmitterAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('SEARCH_SUBMITTER_UNATTESTED', 'submitter attestation invalid');
        }
      } catch (err) {
    ...
    if (this.policy.requireIndexNodeAttestation && this._attestationClient) {
      for (const node of indexNodes) {
        if (this._isolatedNodes.has(node.nodeId)) {
          throw new HsmAdapterError('SEARCH_INDEX_NODE_ISOLATED', `index node ${node.nodeId} is isolated`);
        }
        try {
          const result = this._attestationClient.verify(node.attestation);
          if (!result.verified) {
            throw new HsmAdapterError('SEARCH_INDEX_NODE_UNATTESTED', `index node ${node.nodeId} attestation invalid`);
          }
        } catch (err) {
          if (err instanceof HsmAdapterError) throw err;
          throw new HsmAdapterError('SEARCH_INDEX_NODE_UNATTESTED', `index node ${node.nodeId} attestation invalid`);
    ...
    if (keywords.length > (this.policy.maxKeywordsPerQuery || 32)) {
      throw new HsmAdapterError('SEARCH_KEYWORDS_EXCEEDED', `keywords per query ${keywords.length} exceeds maximum ${this.policy.maxKeywordsPerQuery}`);
    }
    const traversalDepth = request.traversalDepth || 1;
    if (traversalDepth > (this.policy.maxIndexTraversalDepth || 16)) {
      throw new HsmAdapterError('SEARCH_TRAVERSAL_EXCEEDED', `index traversal depth ${traversalDepth} exceeds maximum ${this.policy.maxIndexTraversalDepth}`);
    }
    if (typeof request.blindingCurve === 'string' && !this.policy.allowedBlindingCurves.includes(request.blindingCurve)) {
      throw new HsmAdapterError('SEARCH_BLINDING_CURVE_BLOCKED', `blinding curve ${request.blindingCurve} is not permitted; allowed: ${this.policy.allowedBlindingCurves.join(', ')}`);
    }
    const now = Math.floor(Date.now() / 1000);
    const queryId = request.queryId || `query-${crypto.randomBytes(4).toString('hex')}`;
    const keywordTokenHashes = keywords.map((kw) => crypto.createHash('sha256').update(kw).digest('hex'));
    const matchProofHash = crypto.createHash('sha256').update(`${queryId}:${keywordTokenHashes.join(',')}`).digest('hex');
    const query = {
      queryId,
...
function _validateRequest(policy, request) {
  if (!request.sourceTenantId || !Array.isArray(request.indexNodes)) {
    throw new HsmAdapterError('SEARCH_FIELDS_MISSING', 'sourceTenantId and indexNodes are required');
  }
  if (policy.requireSubmitterAttestation && !request.submitterAttestation) {
    throw new HsmAdapterError('SEARCH_SUBMITTER_ATTESTATION_MISSING', 'submitter attestation is required');
  }
}
