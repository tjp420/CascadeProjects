  ...
  verifyAutonomousClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "AUTOCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (pool.status !== "open") {
      throw new HsmAdapterError(
        "AUTOCLAIM_POOL_NOT_OPEN",
    ...
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "AUTOCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (
      this.policy.banMalformedOrOutOfOrderAutonomousClaims &&
      request.peerId &&
      this._bannedPeers.has(request.peerId)
    ) {
      throw new HsmAdapterError(
        "AUTOCLAIM_PEER_BANNED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "AUTOCLAIM_DUPLICATE",
        `duplicate autonomous claim for pool ${request.poolId}`,
      );
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.zkAutonomousRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderAutonomousClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "AUTOCLAIM_ZK_PROOF_MISSING",
      "zkAutonomousRangeProofHash is required",
    );
  }
  if (!request.polynomialCommitmentDigest) {
    if (policy.banMalformedOrOutOfOrderAutonomousClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "AUTOCLAIM_POLYNOMIAL_DIGEST_MISSING",
      "polynomialCommitmentDigest is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "AUTOCLAIM_OVERSIGHT_ATTESTATION_MISSING",
