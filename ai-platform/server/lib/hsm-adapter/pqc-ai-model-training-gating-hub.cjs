...
class PqcAiModelTrainingGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireTrainingAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.trainingAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "TRAINGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "training authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "TRAINGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "TRAINGATE_TRAINING_WINDOW_EXCEEDED",
        `training window seconds ${request.trainingWindowSeconds} exceeds maximum ${this.policy.maxTrainingWindowSeconds}`,
      );
    }
    if (
      typeof request.provenanceDepth === "number" &&
      request.provenanceDepth > (this.policy.maxProvenanceDepth || 64)
    ) {
      throw new HsmAdapterError(
        "TRAINGATE_PROVENANCE_DEPTH_EXCEEDED",
        `provenance depth ${request.provenanceDepth} exceeds maximum ${this.policy.maxProvenanceDepth}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedDatasetProvenanceCommitment:
        request.blindedDatasetProvenanceCommitment,
  ...
  completeAccreditation(request) {
    ...
    if (!pool.trainingClaimVerified) {
      throw new HsmAdapterError(
        "TRAINGATE_TRAINING_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} training claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "TRAINGATE_MODEL_AUDIT_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minTrainingOversightQuorum || 3)) {
      throw new HsmAdapterError(
        "TRAINGATE_QUORUM_INSUFFICIENT",
        `training oversight signatures ${signatures.length} below minimum ${this.policy.minTrainingOversightQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "TRAINGATE_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedModelWeightCommitment ||
    !request.blindedDatasetProvenanceCommitment ||
    !request.blindedTrainingMetricCommitment
  ) {
    throw new HsmAdapterError(
      "TRAINGATE_FIELDS_MISSING",
      "blindedModelWeightCommitment, blindedDatasetProvenanceCommitment, and blindedTrainingMetricCommitment are required",
    );
  }
  if (typeof request.trainingWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "TRAINGATE_FIELDS_MISSING",
      "trainingWindowSeconds is required",
    );
  }
  if (typeof request.provenanceDepth !== "number") {
    throw new HsmAdapterError(
      "TRAINGATE_FIELDS_MISSING",
      "provenanceDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "TRAINGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "TRAINGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "TRAINGATE_AUDIT_ATTESTATION_MISSING",
