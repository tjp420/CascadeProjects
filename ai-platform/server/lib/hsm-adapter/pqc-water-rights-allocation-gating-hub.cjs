...
class PqcWaterRightsAllocationGatingHub {
  ...
  initializePool(request) {
    ...
    if (this.policy.requireWaterAuthorityInitializerAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('WATERGATE_AUTHORITY_INITIALIZER_UNATTESTED', 'water authority initializer attestation invalid');
        }
      } catch (err) {
  ...
  completeAccreditation(request) {
    ...
    if (signatures.length < (this.policy.minWatershedQuorum || 4)) {
      throw new HsmAdapterError('WATERGATE_QUORUM_INSUFFICIENT', `watershed signatures ${signatures.length} below minimum ${this.policy.minWatershedQuorum}`);
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  ...
  if (policy.requireWaterAuthorityInitializerAttestation && !request.waterAuthorityInitializerAttestation) {
    throw new HsmAdapterError('WATERGATE_AUTHORITY_ATTESTATION_MISSING', 'water authority initializer attestation is required');
  }
}
...
function _validateCompleteRequest(policy, request) {
  ...
  if (policy.requireWatershedOversightCommitteeAttestation && !request.watershedOversightCommitteeAttestation) {
    throw new HsmAdapterError('WATERGATE_WATERSHED_ATTESTATION_MISSING', 'watershed oversight committee attestation is required');
  }
}
