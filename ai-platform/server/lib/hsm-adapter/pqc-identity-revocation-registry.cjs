...
class PqcIdentityRevocationRegistry {
  ...
  publishRevocation(request) {
    ...
    if (this.policy.requirePublisherAttestation && this._attestationClient) {
      try {
        const result = this._attestationClient.verify(request.publisherAttestation);
        if (!result.verified) {
          throw new HsmAdapterError('REVOK_PUBLISHER_UNATTESTED', 'publisher attestation invalid');
        }
      } catch (err) {
    ...
    if (typeof request.attestationAuthority === 'string' && !this.policy.allowedAttestationAuthorities.includes(request.attestationAuthority)) {
      throw new HsmAdapterError('REVOK_ATTESTATION_AUTHORITY_BLOCKED', `attestation authority ${request.attestationAuthority} is not allowed; permitted: ${this.policy.allowedAttestationAuthorities.join(', ')}`);
    }
    if (typeof request.pqcSignatureScheme === 'string' && !this.policy.allowedPqcSignatureSchemes.includes(request.pqcSignatureScheme)) {
      throw new HsmAdapterError('REVOK_PQC_SCHEME_BLOCKED', `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(', ')}`);
    }
    if (this._revocations.size >= (this.policy.maxRevocationListCapacity || 100000)) {
      throw new HsmAdapterError('REVOK_LIST_CAPACITY_EXCEEDED', `revocation list capacity ${this._revocations.size} exceeds maximum ${this.policy.maxRevocationListCapacity}`);
    }
    const signatures = request.committeeSignatures || [];
    if (signatures.length < (this.policy.minRevocationCommitteeQuorum || 3)) {
      throw new HsmAdapterError('REVOK_QUORUM_INSUFFICIENT', `committee signatures ${signatures.length} below minimum ${this.policy.minRevocationCommitteeQuorum}`);
    }
    const revocationId = request.revocationId || `revok-${crypto.randomBytes(4).toString('hex')}`;
    if (this._revocations.has(revocationId)) {
      throw new HsmAdapterError('REVOK_DUPLICATE', `revocation ${revocationId} already exists`);
    }
    const blindedHash = request.blindedIdentityHash;
    if (this._blindedSet.has(blindedHash)) {
      throw new HsmAdapterError('REVOK_IDENTITY_DUPLICATE', `blinded identity ${blindedHash} already revoked`);
    }
    ...
    this._accumulatorRoot = crypto.createHash('sha256').update(this._accumulatorRoot + blindedHash).digest('hex');
...
function _validatePublishRequest(policy, request) {
  if (!request.sourceTenantId || !request.blindedIdentityHash) {
    throw new HsmAdapterError('REVOK_FIELDS_MISSING', 'sourceTenantId and blindedIdentityHash are required');
  }
  if (policy.requirePublisherAttestation && !request.publisherAttestation) {
    throw new HsmAdapterError('REVOK_PUBLISHER_ATTESTATION_MISSING', 'publisher attestation is required');
  }
}
