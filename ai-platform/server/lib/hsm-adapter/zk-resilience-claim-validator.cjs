  ...
  verifyResilienceClaim(request) {
    ...
    if (!pool) {
      throw new HsmAdapterError(
        "RESILIOCLAIM_POOL_NOT_FOUND",
        `pool ${request.poolId} not found`,
      );
    }
    if (pool.status !== "open") {
      throw new HsmAdapterError(
        "RESILIOCLAIM_POOL_NOT_OPEN",
    ...
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "RESILIOCLAIM_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (
      this.policy.banMalformedOrOutOfOrderResilienceClaims &&
      request.peerId &&
      this._bannedPeers.has(request.peerId)
    ) {
      throw new HsmAdapterError(
        "RESILIOCLAIM_PEER_BANNED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "RESILIOCLAIM_DUPLICATE",
        `duplicate resilience claim for pool ${request.poolId}`,
      );
    }
...
function _validateClaimRequest(policy, request, bannedPeers) {
  ...
  if (!request.zkResilienceRangeProofHash) {
    if (policy.banMalformedOrOutOfOrderResilienceClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "RESILIOCLAIM_ZK_PROOF_MISSING",
      "zkResilienceRangeProofHash is required",
    );
  }
  if (!request.verifiableSecretSharingDigest) {
    if (policy.banMalformedOrOutOfOrderResilienceClaims && request.peerId)
      bannedPeers.add(request.peerId);
    throw new HsmAdapterError(
      "RESILIOCLAIM_VSS_DIGEST_MISSING",
      "verifiableSecretSharingDigest is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "RESILIOCLAIM_OVERSIGHT_ATTESTATION_MISSING",
