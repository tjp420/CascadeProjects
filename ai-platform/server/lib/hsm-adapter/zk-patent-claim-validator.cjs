 ...
 * batch patent claim verification, slashing window
...
class ZkPatentClaimValidator {
  ...
  /**
   * Generate a hardware-accelerated SNARK proof for a patent claim.
   * @param {object} request
   * @returns {object}
   */
  ...
  /**
   * Batch verify multiple patent claims.
  ...
  batchVerifyPatentClaims(requests) {
    ...
    for (const req of requests) {
      try {
        const claim = this.verifyPatentClaim(req);
        results.push({
          poolId: req.poolId,
