...
class ClusterKeyringPrimitiveAuthorization {
  ...
  registerGate(trackType, hub, validator) {
    if (!trackType || typeof trackType !== 'string') {
      throw new HsmAdapterError('GATE_NOT_REGISTERED', 'trackType must be a non-empty string');
    }
    if (!hub || typeof hub.getPool !== 'function') {
      throw new HsmAdapterError('GATE_NOT_REGISTERED', `hub for trackType ${trackType} is invalid`);
    }
  ...
  authorizeAccreditedPool(trackType, poolId, request = {}) {
    ...
    if (!reg) {
      throw new HsmAdapterError('GATE_NOT_REGISTERED', `trackType ${trackType} has no registered gate`);
    }
    const pool = reg.hub.getPool(poolId);
    if (!pool) {
      throw new HsmAdapterError('POOL_NOT_FOUND', `pool ${poolId} not found in trackType ${trackType}`);
    }
    if (this.policy.requireAccreditedPoolStatus && pool.status !== 'accredited') {
      throw new HsmAdapterError('POOL_NOT_ACCREDITED', `pool ${poolId} status is ${pool.status}, not accredited`);
    }
    if (this._authorizedPools.has(poolId)) {
      throw new HsmAdapterError('DUPLICATE_AUTHORIZATION', `pool ${poolId} is already authorized`);
    }
    if (this.policy.requireNodeAttestation && this._attestationClient) {
      const nodeId = request.nodeId || pool.sourceOpticalNodeId || pool.sourceVolumetricSectorId || pool.sourceTenantId;
      let verified = false;
      ...
      if (!verified) {
        throw new HsmAdapterError('NODE_UNATTESTED', `node ${nodeId} has not been attested`);
      }
    }
    if (typeof request.pqcSignatureScheme === 'string' && !this.policy.allowedPqcSignatureSchemes.includes(request.pqcSignatureScheme)) {
      throw new HsmAdapterError('POLICY_VIOLATION_BLOCKED', `PQC signature scheme ${request.pqcSignatureScheme} is not permitted`);
    }
    if (typeof request.attestationAuthority === 'string' && !this.policy.allowedAttestationAuthorities.includes(request.attestationAuthority)) {
      throw new HsmAdapterError('POLICY_VIOLATION_BLOCKED', `attestation authority ${request.attestationAuthority} is not allowed`);
    }
    const signatures = request.committeeSignatures || [];
    if (signatures.length < (this.policy.minAuthorizationQuorum || 3)) {
      throw new HsmAdapterError('QUORUM_INSUFFICIENT', `authorization quorum ${signatures.length} below minimum ${this.policy.minAuthorizationQuorum}`);
    }
    if (typeof request.tenantId === 'string' && pool.sourceTenantId && request.tenantId !== pool.sourceTenantId) {
      throw new HsmAdapterError('CROSS_TENANT_VIOLATION', `request tenant ${request.tenantId} does not match pool tenant ${pool.sourceTenantId}`);
    }
    ...
    if (this._ratchet && typeof this._ratchet.evolveShare === 'function') {
      try {
        const token = { nodeIndex: 1, value: BigInt('0x' + crypto.randomBytes(8).toString('hex')), sequence: 0 };
        ratchetedShare = this._ratchet.evolveShare(token, `auth-${poolId}-${Date.now()}`);
      } catch (e) {
        if (this._audit) this._audit('AUTHORIZATION_RATCHET_FAILED', { poolId, err: String(e) });
      }
    }

    // Disperse to cluster nodes if disperser is available
    let dispersalResult = null;
    if (this._shardDisperser && typeof this._shardDisperser.disperse === 'function') {
      try {
        ...
        if (this._audit) this._audit('AUTHORIZATION_DISPERSE_FAILED', { poolId, err: String(e) });
      }
    }
    ...
    if (this._keyringSync && typeof this._keyringSync.recordTelemetry === 'function') {
      try {
  ...
  revokeAuthorization(poolId, reason = 'manual') {
    const rec = this._authorizedPools.get(poolId);
    if (!rec) {
      throw new HsmAdapterError('AUTHORIZATION_REVOKED', `pool ${poolId} has no authorization record`);
    }
    rec.status = 'revoked';
  ...
  syncAuthorizedPool(poolId, targetEnclaveId) {
    if (!this.isPoolAuthorized(poolId)) {
      throw new HsmAdapterError('AUTHORIZATION_REVOKED', `pool ${poolId} is not authorized or has expired`);
    }
    const rec = this._authorizedPools.get(poolId);
    const now = Math.floor(Date.now() / 1000);
    if (now - rec.authorizedAt > (this.policy.maxSyncWindowSeconds || 300)) {
      throw new HsmAdapterError('SYNC_WINDOW_EXCEEDED', `sync window exceeded for pool ${poolId}`);
    }
    let syncResult = null;
    if (this._stateSync && typeof this._stateSync.syncState === 'function') {
      try {
        ...
        if (this._audit) this._audit('PRIMITIVE_POOL_SYNC_FAILED', { poolId, err: String(e) });
      }
    }
    if (this._keyringSync && typeof this._keyringSync.recordTelemetry === 'function') {
      try {
  ...
  detectShareDivergence(poolId) {
    if (!this.isPoolAuthorized(poolId)) {
      throw new HsmAdapterError('AUTHORIZATION_REVOKED', `pool ${poolId} is not authorized`);
    }
    if (this._reconciler && typeof this._reconciler.detectDivergence === 'function') {
      try {
        return this._reconciler.detectDivergence(`primitive-pool-${poolId}`);
      } catch (e) {
        return { keyId: `primitive-pool-${poolId}`, severity: 'none', error: String(e) };
      }
    }
    return { keyId: `primitive-pool-${poolId}`, severity: 'none', divergentNodes: [], quorumEpoch: null };
  }
...
function _validateAuthorizeRequest(policy, trackType, poolId, request) {
  if (!trackType) throw new HsmAdapterError('GATE_NOT_REGISTERED', 'trackType is required');
  if (!poolId) throw new HsmAdapterError('POOL_NOT_FOUND', 'poolId is required');
  if (policy.banUnauthorizedShareDispersal === false) {
    throw new HsmAdapterError('POLICY_VIOLATION_BLOCKED', 'unauthorized share dispersal must remain banned');
  }
