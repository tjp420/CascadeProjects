...
class PqcOceanFisheriesAllocationGatingHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError(
        "FISHERIESGATE_MAX_POOLS",
        `maximum ${this._maxPools} pools reached`,
      );
    }
    if (
      this.policy.requireRfmoAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.rfmoAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "FISHERIESGATE_INSTITUTION_INITIALIZER_UNATTESTED",
            "RFMO authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "FISHERIESGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "FISHERIESGATE_PQC_SCHEME_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "FISHERIESGATE_TRACKING_WINDOW_EXCEEDED",
    ...
    if (
      typeof request.vesselTelemetryChainDepth === "number" &&
      request.vesselTelemetryChainDepth >
        (this.policy.maxVesselTelemetryChainDepth || 12)
    ) {
      throw new HsmAdapterError(
        "FISHERIESGATE_TELEMETRY_DEPTH_EXCEEDED",
        `vessel telemetry chain depth ${request.vesselTelemetryChainDepth} exceeds maximum ${this.policy.maxVesselTelemetryChainDepth}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedQuotaAllocationCommitment:
        request.blindedQuotaAllocationCommitment,
      blindedMaritimeAuthorityIdentityCommitment:
        request.blindedMaritimeAuthorityIdentityCommitment,
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError(
        "FISHERIESGATE_BATCH_EMPTY",
        "batch requests array is required",
      );
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError(
        "FISHERIESGATE_BATCH_TOO_LARGE",
    ...
    if (this._audit) {
      this._audit("FISHERIESGATE_BATCH_INITIALIZED", {
        successCount,
        failedCount,
  ...
  rebalanceVesselTelemetryChainDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "FISHERIESGATE_REBALANCE_FIELDS_MISSING",
        "poolId is required",
      );
    }
    ...
    if (
      ...
      throw new HsmAdapterError(
        "FISHERIESGATE_NOT_REBALANCEABLE",
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError(
        "FISHERIESGATE_REBALANCE_DIRECTION_INVALID",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "FISHERIESGATE_REBALANCE_AMOUNT_INVALID",
    ...
    const rebalance = {
      ...
      newVesselTelemetryChainDepth:
        request.newVesselTelemetryChainDepth !== undefined
          ? request.newVesselTelemetryChainDepth
          : pool.vesselTelemetryChainDepth,
  ...
  completeAccreditation(request) {
    ...
    if (!pool.catchClaimVerified) {
      throw new HsmAdapterError(
        "FISHERIESGATE_CATCH_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} catch claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "FISHERIESGATE_SANCTUARY_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minMaritimeQuorum || 5)) {
      throw new HsmAdapterError(
        "FISHERIESGATE_ACCREDITATION_QUORUM_INSUFFICIENT",
        `accreditation signatures ${signatures.length} below minimum ${this.policy.minMaritimeQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError(
        "FISHERIESGATE_SETTLE_FIELDS_MISSING",
        "poolId is required",
      );
    }
    const pool = this._pools.get(request.poolId);
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (this._audit) {
      this._audit("FISHERIESGATE_SIGNATURES_AGGREGATED", {
        poolId,
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "FISHERIESGATE_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedCatchTelemetryCommitment ||
    !request.blindedQuotaAllocationCommitment ||
    !request.blindedMaritimeAuthorityIdentityCommitment
  ) {
    throw new HsmAdapterError(
      "FISHERIESGATE_FIELDS_MISSING",
      "blindedCatchTelemetryCommitment, blindedQuotaAllocationCommitment, and blindedMaritimeAuthorityIdentityCommitment are required",
    );
  }
  if (typeof request.catchTrackingWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "FISHERIESGATE_FIELDS_MISSING",
      "catchTrackingWindowSeconds is required",
    );
  }
  if (typeof request.vesselTelemetryChainDepth !== "number") {
    throw new HsmAdapterError(
      "FISHERIESGATE_FIELDS_MISSING",
      "vesselTelemetryChainDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "FISHERIESGATE_INSTITUTION_INITIALIZER_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "FISHERIESGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "FISHERIESGATE_CLEARING_ATTESTATION_MISSING",
