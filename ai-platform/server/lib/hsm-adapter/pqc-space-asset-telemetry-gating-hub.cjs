...
class PqcSpaceAssetTelemetryGatingHub {
  ...
  initializePool(request) {
    ...
    if (this._pools.size >= this._maxPools) {
      throw new HsmAdapterError('SPACEGATE_MAX_POOLS',
    ...
    if (this.policy.requireSpaceAuthorityInitializerAttestation && this._attestationClient) {
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError('SPACEGATE_INSTITUTION_INITIALIZER_UNATTESTED', 'space authority initializer attestation invalid');
        }
      } catch (err) {
  ...
  batchInitializePools(requests) {
    if (!Array.isArray(requests) || requests.length === 0) {
      throw new HsmAdapterError('SPACEGATE_BATCH_EMPTY', 'batch requests array is required');
    }
    if (requests.length > this._maxBatchSize) {
      throw new HsmAdapterError('SPACEGATE_BATCH_TOO_LARGE',
    ...
    if (this._audit) {
      this._audit('SPACEGATE_BATCH_INITIALIZED', { successCount, failedCount, batchSize: requests.length });
    }
  ...
  rebalanceTelemetryChainDepth(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('SPACEGATE_REBALANCE_FIELDS_MISSING', 'poolId is required');
    }
    ...
    if (pool.status !== POOL_STATUS.OPEN && pool.status !== POOL_STATUS.REBALANCING) {
      throw new HsmAdapterError('SPACEGATE_NOT_REBALANCEABLE',
    ...
    if (!Object.values(REBALANCE_DIRECTION).includes(direction)) {
      throw new HsmAdapterError('SPACEGATE_REBALANCE_DIRECTION_INVALID',
    ...
    if (typeof request.rebalanceAmount !== 'number' || request.rebalanceAmount <= 0) {
      throw new HsmAdapterError('SPACEGATE_REBALANCE_AMOUNT_INVALID',
    ...
    if (request.newTelemetryChainDepth !== undefined) {
      if (request.newTelemetryChainDepth > (this.policy.maxTelemetryChainDepth || 16)) {
        throw new HsmAdapterError('SPACEGATE_TELEMETRY_DEPTH_EXCEEDED',
  ...
  completeAccreditation(request) {
    ...
    if (signatures.length < (this.policy.minOrbitalSlotQuorum || 5)) {
      throw new HsmAdapterError('SPACEGATE_ACCREDITATION_QUORUM_INSUFFICIENT', `accreditation signatures ${signatures.length} below minimum ${this.policy.minOrbitalSlotQuorum}`);
    }
    const now = Math.floor(Date.now() / 1000);
  ...
  settlePool(request) {
    if (!request || !request.poolId) {
      throw new HsmAdapterError('SPACEGATE_SETTLE_FIELDS_MISSING', 'poolId is required');
    }
    const pool = this._pools.get(request.poolId);
    if (!pool) {
      throw new HsmAdapterError('SPACEGATE_NOT_FOUND', `pool ${request.poolId} not found`);
    ...
    if (pool.status !== POOL_STATUS.ACCREDITED) {
      throw new HsmAdapterError('SPACEGATE_NOT_ACCREDITED',
    ...
    if (!request.targetChainId || typeof request.targetChainId !== 'string') {
      throw new HsmAdapterError('SPACEGATE_SETTLE_CHAIN_MISSING', 'targetChainId is required for settlement');
    }
    if (request.targetChainId !== pool.targetChainId) {
      throw new HsmAdapterError('SPACEGATE_SETTLE_CHAIN_MISMATCH',
  ...
  aggregateCommitteeSignatures(poolId, partialSignatures) {
    ...
    if (!Array.isArray(partialSignatures) || partialSignatures.length === 0) {
      throw new HsmAdapterError('SPACEGATE_NO_SIGNATURES', 'partialSignatures array is required');
    }
    if (partialSignatures.length < (this.policy.minOrbitalSlotQuorum || 5)) {
      throw new HsmAdapterError('SPACEGATE_ACCREDITATION_QUORUM_INSUFFICIENT',
    ...
    if (this._audit) {
      this._audit('SPACEGATE_SIGNATURES_AGGREGATED', { poolId, count: partialSignatures.length });
    }
    return result;
  }
  ...
  cancelPool(poolId) {
    ...
    if (pool.status === POOL_STATUS.ACCREDITED || pool.status === POOL_STATUS.SETTLED) {
      throw new HsmAdapterError('SPACEGATE_ALREADY_ACCREDITED',
        `pool ${poolId} has been accredited/settled and cannot be cancelled`);
    }
    if (pool.status === POOL_STATUS.CANCELLED) {
      throw new HsmAdapterError('SPACEGATE_ALREADY_CANCELLED',
...
function _validateInitRequest(policy, request) {
  ...
  if (policy.requireSpaceAuthorityInitializerAttestation && !request.spaceAuthorityInitializerAttestation) {
    throw new HsmAdapterError('SPACEGATE_INSTITUTION_INITIALIZER_ATTESTATION_MISSING', 'space authority initializer attestation is required');
  }
}
...
function _validateCompleteRequest(policy, request) {
  ...
  if (policy.requireOrbitalOversightCommitteeAttestation && !request.orbitalOversightCommitteeAttestation) {
    throw new HsmAdapterError('SPACEGATE_CLEARING_ATTESTATION_MISSING', 'orbital oversight committee attestation is required');
  }
}
