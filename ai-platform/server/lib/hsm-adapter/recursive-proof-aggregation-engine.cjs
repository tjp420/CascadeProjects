...
class RecursiveProofAggregationEngine {
  ...
  submitProof(config) {
    ...
    if (this._proofs.size >= this.maxProofs) {
      throw new HsmAdapterError('MAX_PROOFS_REACHED',
    ...
    if (typeof this._audit === 'function') {
      this._audit('PROOF_SUBMITTED', { proofId: config.proofId, sourceType: proof.sourceType });
    }
    return {
  ...
  foldProofs(proofId1, proofId2, foldedProofId) {
    ...
    if (!proof1) {
      throw new HsmAdapterError('PROOF_NOT_FOUND', `proof ${proofId1} not found`);
    }
    const proof2 = this._proofs.get(proofId2);
    if (!proof2) {
      throw new HsmAdapterError('PROOF_NOT_FOUND', `proof ${proofId2} not found`);
    }
    if (proof1.status !== PROOF_STATUS.VERIFIED && proof1.status !== PROOF_STATUS.FOLDED) {
      throw new HsmAdapterError('PROOF_NOT_VERIFIED',
        `proof ${proofId1} status is ${proof1.status}`);
    }
    if (proof2.status !== PROOF_STATUS.VERIFIED && proof2.status !== PROOF_STATUS.FOLDED) {
      throw new HsmAdapterError('PROOF_NOT_VERIFIED',
        `proof ${proofId2} status is ${proof2.status}`);
    }
    const foldId = foldedProofId || `fold-${Date.now()}-${crypto.randomInt(0, 1000000)}`;
    if (this._proofs.has(foldId)) {
      throw new HsmAdapterError('PROOF_ALREADY_EXISTS', `proof ${foldId} already exists`);
    }
    // Simulate recursive proof folding (in real systems, this uses
    // a recursion circuit that verifies both inner proofs)
    const foldedData = crypto.createHash('sha256')
    ...
    this._foldCount++;
    if (typeof this._audit === 'function') {
      this._audit('PROOFS_FOLDED', { foldId, innerProofs: [proofId1, proofId2] });
    }
    return {
      proofId: foldId,
  ...
  aggregateChain(proofIds, aggId) {
    if (!this.enableChainAggregation) {
      throw new HsmAdapterError('CHAIN_AGG_DISABLED', 'chain aggregation is disabled');
    }
    ...
    if (proofIds.length > this.maxChainLength) {
      throw new HsmAdapterError('CHAIN_TOO_LONG',
        `${proofIds.length} exceeds max chain length ${this.maxChainLength}`);
    }
    const resultId = aggId || `chain-${Date.now()}-${crypto.randomInt(0, 1000000)}`;
    if (this._aggregations.has(resultId)) {
      throw new HsmAdapterError('AGG_ALREADY_EXISTS', `aggregation ${resultId} already exists`);
    }
    ...
    if (typeof this._audit === 'function') {
      this._audit('CHAIN_AGGREGATED', { aggId: resultId, proofCount: proofIds.length });
    }
    return {
      aggId: resultId,
      type: AGG_TYPE.CHAIN,
  ...
  aggregateTree(proofIds, aggId) {
    if (!this.enableTreeAggregation) {
      throw new HsmAdapterError('TREE_AGG_DISABLED', 'tree aggregation is disabled');
    }
    if (!Array.isArray(proofIds) || proofIds.length < 2) {
      throw new HsmAdapterError('INSUFFICIENT_PROOFS',
    ...
    if (treeDepth > this.maxTreeDepth) {
      throw new HsmAdapterError('TREE_TOO_DEEP',
        `tree depth ${treeDepth} exceeds max ${this.maxTreeDepth}`);
    }
    const resultId = aggId || `tree-${Date.now()}-${crypto.randomInt(0, 1000000)}`;
    if (this._aggregations.has(resultId)) {
      throw new HsmAdapterError('AGG_ALREADY_EXISTS', `aggregation ${resultId} already exists`);
    }
    for (const id of proofIds) {
    ...
    this._recordCompletedAgg(aggregation);
    if (typeof this._audit === 'function') {
      this._audit('TREE_AGGREGATED', { aggId: resultId, proofCount: proofIds.length });
    }
    return {
      aggId: resultId,
      type: AGG_TYPE.TREE,
  ...
  aggregateVdfProofs(proofIds) {
    ...
    for (const id of proofIds) {
      ...
      if (proof.sourceType !== 'vdf') {
        throw new HsmAdapterError('NOT_VDF_PROOF',
    ...
    if (typeof this._audit === 'function') {
      this._audit('VDF_PROOFS_AGGREGATED', { aggId: resultId, proofCount: proofIds.length });
    }
    return {
      aggId: resultId,
      type: AGG_TYPE.VDF,
  ...
  compressMixnetState(proofIds) {
    ...
    for (const id of proofIds) {
      ...
      if (proof.sourceType !== 'mixnet') {
        throw new HsmAdapterError('NOT_MIXNET_PROOF',
    ...
    if (typeof this._audit === 'function') {
      this._audit('MIXNET_STATE_COMPRESSED', { aggId: resultId, hopCount: proofIds.length });
    }
    return {
      aggId: resultId,
  ...
  verifyAggregation(aggId) {
    ...
    if (!agg) {
      throw new HsmAdapterError('AGG_NOT_FOUND', `aggregation ${aggId} not found`);
    }
    if (agg.status !== AGG_STATUS.COMPLETED) {
      throw new HsmAdapterError('AGG_NOT_COMPLETED',
    ...
    if (finalProof.status !== PROOF_STATUS.FOLDED && finalProof.status !== PROOF_STATUS.VERIFIED) {
      return { aggId, verified: false, reason: `final proof status is ${finalProof.status}` };
    }
