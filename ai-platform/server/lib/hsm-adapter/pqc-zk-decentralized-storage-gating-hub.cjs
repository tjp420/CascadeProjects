...
class PqcZkDecentralizedStorageAttestationGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireStorageAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.storageAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "ZKSTORAGE_AUTHORITY_INITIALIZER_UNATTESTED",
            "storage authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "ZKSTORAGE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "ZKSTORAGE_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.proofOfSpaceTimeWindowSeconds === "number" &&
      request.proofOfSpaceTimeWindowSeconds >
        (this.policy.maxProofOfSpaceTimeWindowSeconds || 300)
    ) {
      throw new HsmAdapterError(
        "ZKSTORAGE_PROOF_OF_SPACE_TIME_WINDOW_EXCEEDED",
        `proof-of-space-time window seconds ${request.proofOfSpaceTimeWindowSeconds} exceeds maximum ${this.policy.maxProofOfSpaceTimeWindowSeconds}`,
      );
    }
    if (
      typeof request.storageAttestationChainDepth === "number" &&
      request.storageAttestationChainDepth >
        (this.policy.maxStorageAttestationChainDepth || 46)
    ) {
      throw new HsmAdapterError(
        "ZKSTORAGE_CHAIN_DEPTH_EXCEEDED",
        `storage attestation chain depth ${request.storageAttestationChainDepth} exceeds maximum ${this.policy.maxStorageAttestationChainDepth}`,
      );
    }
    if (
      typeof request.replicaDispersalDistance === "number" &&
      request.replicaDispersalDistance >
        (this.policy.maxReplicaDispersalDistance || 12)
    ) {
      throw new HsmAdapterError(
        "ZKSTORAGE_REPLICA_DISPERSAL_DISTANCE_EXCEEDED",
        `replica dispersal distance ${request.replicaDispersalDistance} exceeds maximum ${this.policy.maxReplicaDispersalDistance}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedStorageSectorDigestCommitment:
        request.blindedStorageSectorDigestCommitment,
      blindedProofOfSpaceTimeCommitment:
        request.blindedProofOfSpaceTimeCommitment,
      blindedReplicaDispersalCommitment:
        request.blindedReplicaDispersalCommitment,
  ...
  verifyStorageProof(request) {
    ...
    if (!request.proofValid) {
      ...
      throw new HsmAdapterError(
        "ZKSTORAGE_PROOF_INVALID",
        `storage proof for pool ${request.poolId} is invalid`,
      );
    }
    pool.storageProofVerified = true;
    hsmMetrics.incrementCounter("hsm_zk_storage_proof_verified_total", 1);
    if (this._audit) {
      this._audit("ZKSTORAGE_STORAGE_PROOF_VERIFIED", {
  ...
  confirmReplicaConsensus(request) {
    ...
    if (!pool.storageProofVerified) {
      throw new HsmAdapterError(
        "ZKSTORAGE_PROOF_NOT_VERIFIED",
        `pool ${request.poolId} storage proof not verified`,
      );
    }
    const signatures = request.replicaSignatures || [];
    if (signatures.length < (this.policy.minReplicationFactor || 3)) {
      throw new HsmAdapterError(
        "ZKSTORAGE_REPLICATION_FACTOR_INSUFFICIENT",
        `replica signatures ${signatures.length} below minimum ${this.policy.minReplicationFactor}`,
      );
    }
    ...
    if (this._audit) {
      this._audit("ZKSTORAGE_REPLICA_CONSENSUS_CONFIRMED", {
        poolId: request.poolId,
        replicaCount: signatures.length,
      });
    }
    return pool;
  }
  ...
  completeDispersal(request) {
    ...
    if (!pool.replicaConsensusConfirmed) {
      throw new HsmAdapterError(
        "ZKSTORAGE_REPLICA_CONSENSUS_NOT_CONFIRMED",
        `pool ${request.poolId} replica consensus not confirmed`,
      );
    }
  ...
  completeReplicationAccreditation(request) {
    ...
    if (
      this.policy.requireStorageEthicsOversightCommitteeAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.storageEthicsOversightCommitteeAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "ZKSTORAGE_OVERSIGHT_COMMITTEE_UNATTESTED",
            "storage ethics oversight committee attestation invalid",
          );
        }
      } catch (err) {
        if (err instanceof HsmAdapterError) throw err;
    ...
    if (signatures.length < (this.policy.minReplicationFactor || 3)) {
      throw new HsmAdapterError(
        "ZKSTORAGE_QUORUM_INSUFFICIENT",
        `replication quorum signatures ${signatures.length} below minimum ${this.policy.minReplicationFactor}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
    ...
    if (this._audit) {
      this._audit("ZKSTORAGE_REPLICATION_ACCREDITATION_COMPLETED", {
        ...completion,
      });
    }
    return completion;
  ...
  recordSlash(request) {
    ...
    if (this._audit) {
      this._audit("ZKSTORAGE_SLASH_RECORDED", {
        poolId: request.poolId,
  ...
  issueChallenge(request) {
    ...
    if (this._audit) {
      this._audit("ZKSTORAGE_CHALLENGE_ISSUED", {
        poolId: request.poolId,
...
function _validateInitRequest(policy, request) {
  ...
  if (!request.sourceStorageNodeId || !request.targetStorageNodeId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_FIELDS_MISSING",
      "sourceStorageNodeId and targetStorageNodeId are required",
    );
  }
  if (
    !request.blindedStorageSectorDigestCommitment ||
    ...
    throw new HsmAdapterError(
      "ZKSTORAGE_FIELDS_MISSING",
      "blindedStorageSectorDigestCommitment, blindedProofOfSpaceTimeCommitment, and blindedReplicaDispersalCommitment are required",
    );
  }
  if (typeof request.proofOfSpaceTimeWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "ZKSTORAGE_FIELDS_MISSING",
      "proofOfSpaceTimeWindowSeconds is required",
    );
  }
  if (typeof request.storageAttestationChainDepth !== "number") {
    throw new HsmAdapterError(
      "ZKSTORAGE_FIELDS_MISSING",
      "storageAttestationChainDepth is required",
    );
  }
  if (typeof request.replicaDispersalDistance !== "number") {
    throw new HsmAdapterError(
      "ZKSTORAGE_FIELDS_MISSING",
      "replicaDispersalDistance is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "ZKSTORAGE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateProofRequest(request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_PROOF_FIELDS_MISSING",
      "poolId is required",
    );
  }
...
function _validateReplicaConsensusRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_REPLICA_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "ZKSTORAGE_OVERSIGHT_ATTESTATION_MISSING",
...
function _validateDispersalRequest(request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_DISPERSAL_FIELDS_MISSING",
      "poolId is required",
    );
  }
}
...
function _validateAccreditationRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_ACCREDITATION_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
...
function _validateSlashRequest(request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_SLASH_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (!request.reason) {
...
function _validateChallengeRequest(request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "ZKSTORAGE_CHALLENGE_FIELDS_MISSING",
      "poolId is required",
    );
  }
