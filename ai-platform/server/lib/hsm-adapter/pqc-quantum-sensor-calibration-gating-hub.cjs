...
class PqcQuantumSensorCalibrationGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireQuantumMetrologyAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.quantumMetrologyAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "QUANTGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "quantum metrology authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "QUANTGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      throw new HsmAdapterError(
        "QUANTGATE_PQC_SCHEME_BLOCKED",
        `PQC signature scheme ${request.pqcSignatureScheme} is not permitted; allowed: ${this.policy.allowedPqcSignatureSchemes.join(", ")}`,
      );
    }
    if (
      typeof request.calibrationWindowSeconds === "number" &&
      request.calibrationWindowSeconds >
        (this.policy.maxCalibrationWindowSeconds || 7776000)
    ) {
      throw new HsmAdapterError(
        "QUANTGATE_CALIBRATION_WINDOW_EXCEEDED",
        `calibration window seconds ${request.calibrationWindowSeconds} exceeds maximum ${this.policy.maxCalibrationWindowSeconds}`,
      );
    }
    if (
      typeof request.calibrationChainDepth === "number" &&
      request.calibrationChainDepth >
        (this.policy.maxCalibrationChainDepth || 22)
    ) {
      throw new HsmAdapterError(
        "QUANTGATE_CALIBRATION_DEPTH_EXCEEDED",
        `calibration chain depth ${request.calibrationChainDepth} exceeds maximum ${this.policy.maxCalibrationChainDepth}`,
      );
    }
    const poolId =
  ...
  completeAccreditation(request) {
    ...
    if (!pool.quantumClaimVerified) {
      throw new HsmAdapterError(
        "QUANTGATE_QUANTUM_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} quantum claim not verified`,
      );
    }
    if (
      ...
      try {
        ...
        if (!result.verified) {
          throw new HsmAdapterError(
            "QUANTGATE_OVERSIGHT_COMMITTEE_UNATTESTED",
    ...
    if (signatures.length < (this.policy.minQuantumQuorum || 7)) {
      throw new HsmAdapterError(
        "QUANTGATE_QUORUM_INSUFFICIENT",
        `quantum quorum signatures ${signatures.length} below minimum ${this.policy.minQuantumQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  ...
  if (typeof request.calibrationWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "QUANTGATE_FIELDS_MISSING",
      "calibrationWindowSeconds is required",
    );
  }
  if (typeof request.calibrationChainDepth !== "number") {
    throw new HsmAdapterError(
      "QUANTGATE_FIELDS_MISSING",
      "calibrationChainDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "QUANTGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "QUANTGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "QUANTGATE_OVERSIGHT_ATTESTATION_MISSING",
