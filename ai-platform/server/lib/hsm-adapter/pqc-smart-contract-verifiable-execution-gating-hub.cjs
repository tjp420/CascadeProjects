...
class PqcSmartContractVerifiableExecutionGatingHub {
  ...
  initializePool(request) {
    ...
    if (
      this.policy.requireExecutionAuthorityInitializerAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.executionAuthorityInitializerAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "EXECGATE_AUTHORITY_INITIALIZER_UNATTESTED",
            "execution authority initializer attestation invalid",
          );
        }
      } catch (err) {
    ...
    if (
      ...
      throw new HsmAdapterError(
        "EXECGATE_ATTESTATION_AUTHORITY_BLOCKED",
    ...
    if (
      ...
      request.executionWindowSeconds >
        (this.policy.maxExecutionWindowSeconds || 172800)
    ) {
      throw new HsmAdapterError(
        "EXECGATE_EXECUTION_WINDOW_EXCEEDED",
        `execution window seconds ${request.executionWindowSeconds} exceeds maximum ${this.policy.maxExecutionWindowSeconds}`,
      );
    }
    if (
      typeof request.executionChainDepth === "number" &&
      request.executionChainDepth > (this.policy.maxExecutionChainDepth || 30)
    ) {
      throw new HsmAdapterError(
        "EXECGATE_EXECUTION_DEPTH_EXCEEDED",
        `execution chain depth ${request.executionChainDepth} exceeds maximum ${this.policy.maxExecutionChainDepth}`,
      );
    }
    const poolId =
    ...
    const pool = {
      ...
      blindedComputationTraceCommitment:
        request.blindedComputationTraceCommitment,
      blindedExecutionAuthorityIdentityCommitment:
        request.blindedExecutionAuthorityIdentityCommitment,
  ...
  completeAccreditation(request) {
    ...
    if (!pool.executionClaimVerified) {
      throw new HsmAdapterError(
        "EXECGATE_EXECUTION_CLAIM_NOT_VERIFIED",
        `pool ${request.poolId} execution claim not verified`,
      );
    }
    if (
      this.policy.requireExecutionEthicsOversightCommitteeAttestation &&
      this._attestationClient
    ) {
      try {
        const result = this._attestationClient.verify(
          request.executionEthicsOversightCommitteeAttestation,
        );
        if (!result.verified) {
          throw new HsmAdapterError(
            "EXECGATE_OVERSIGHT_COMMITTEE_UNATTESTED",
            "execution ethics oversight committee attestation invalid",
          );
        }
      } catch (err) {
        if (err instanceof HsmAdapterError) throw err;
    ...
    if (signatures.length < (this.policy.minExecutionQuorum || 10)) {
      throw new HsmAdapterError(
        "EXECGATE_QUORUM_INSUFFICIENT",
        `execution quorum signatures ${signatures.length} below minimum ${this.policy.minExecutionQuorum}`,
      );
    }
    const now = Math.floor(Date.now() / 1000);
...
function _validateInitRequest(policy, request) {
  if (!request.sourceTenantId || !request.targetChainId) {
    throw new HsmAdapterError(
      "EXECGATE_FIELDS_MISSING",
      "sourceTenantId and targetChainId are required",
    );
  }
  if (
    !request.blindedExecutionStateCommitment ||
    !request.blindedComputationTraceCommitment ||
    !request.blindedExecutionAuthorityIdentityCommitment
  ) {
    throw new HsmAdapterError(
      "EXECGATE_FIELDS_MISSING",
      "blindedExecutionStateCommitment, blindedComputationTraceCommitment, and blindedExecutionAuthorityIdentityCommitment are required",
    );
  }
  if (typeof request.executionWindowSeconds !== "number") {
    throw new HsmAdapterError(
      "EXECGATE_FIELDS_MISSING",
      "executionWindowSeconds is required",
    );
  }
  if (typeof request.executionChainDepth !== "number") {
    throw new HsmAdapterError(
      "EXECGATE_FIELDS_MISSING",
      "executionChainDepth is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "EXECGATE_AUTHORITY_ATTESTATION_MISSING",
...
function _validateCompleteRequest(policy, request) {
  if (!request.poolId) {
    throw new HsmAdapterError(
      "EXECGATE_COMPLETE_FIELDS_MISSING",
      "poolId is required",
    );
  }
  if (
    ...
    throw new HsmAdapterError(
      "EXECGATE_OVERSIGHT_ATTESTATION_MISSING",
