...
class HomomorphicKeyDeriver {
  ...
  derive(kemSharedSecret, proposalId, depth) {
    ...
    if (!ALLOWED_KEM.has(this.kemPrimitive)) {
      throw new HsmAdapterError('POLICY_VIOLATION_BLOCKED', `KEM primitive ${this.kemPrimitive} is not allowed`);
    }
    if (this.requirePqcBlinding && !ALLOWED_KEM.has(this.kemPrimitive)) {
      throw new HsmAdapterError('POLICY_VIOLATION_BLOCKED', 'PQC blinding requires an allowed KEM primitive');
    }
    if (!ALLOWED_CURVES.has(this.derivationCurve)) {
      throw new HsmAdapterError('POLICY_VIOLATION_BLOCKED', `derivation curve ${this.derivationCurve} is not allowed`);
    }
    if (typeof depth !== 'number' || depth < 0 || depth > this.maxChildDerivationDepth) {
      throw new HsmAdapterError('DERIVATION_DEPTH_EXCEEDED', `depth ${depth} exceeds max ${this.maxChildDerivationDepth}`);
    }
