    ...
    return new Promise((resolve, reject) => {
        ...
        const req = client.get(url, {
            ...
            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location && maxRedirects > 0) {
                return resolve(fetchUrl(new URL(res.headers.location, url).href, { ...opts, maxRedirects: maxRedirects - 1 }));
            }
...
async function scanWebsite(targetUrl, options = {}) {
    ...
    if (scanTypes.includes('ai-slop') || scanTypes.includes('all')) {
        ...
        findings.push(...scanPatterns(content, slopCatalog.RULE_CATALOG || [], { minConfidence: options.minConfidence || 0.5 }));
    }
    ...
    if (scanTypes.includes('pii') || scanTypes.includes('all')) {
        const piiPatterns = [
            { id: 'email-leak', regexSource: '\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b', type: 'pii', severity: 'high', message: 'Email address exposed', suggestion: 'Remove or obfuscate', confidence: 0.9 },
            { id: 'phone-leak', regexSource: '\\b\\d{3}[-.\\s]?\\d{3}[-.\\s]?\\d{4}\\b', type: 'pii', severity: 'medium', message: 'Phone number exposed', suggestion: 'Remove or obfuscate', confidence: 0.7 }
        ];
        findings.push(...scanPatterns(content, piiPatterns, { minConfidence: options.minConfidence || 0.5 }));
    }
