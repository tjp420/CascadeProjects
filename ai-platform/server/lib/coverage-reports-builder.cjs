...
function applyProjectCoverage(projects, files) {
  ...
  return (projects || []).map((project) => {
    ...
    return {
      ...
      statementCoverage,
      status:
        overallCoverage != null && overallCoverage >= 70
          ? "healthy"
          : "warning",
    };
  });
...
function buildCoverageReportsModel(baseDir, sample = {}, options = {}) {
  ...
  if (!istanbul.available || !istanbul.totals) {
    const passedTests =
      baseline.jestTestsPassing ?? sample.overview?.passedTests ?? null;
    const totalTests =
      baseline.jestTestsPassing ?? sample.overview?.totalTests ?? null;
    return {
      ...sample,
      ...
      overview: {
        ...
        testPassRate:
          passedTests != null && totalTests
            ? Math.round(
                (passedTests / totalTests) * constants.PERCENTAGE_MULTIPLIER,
              )
