...
function resolveGateResult(bundle = {}) {
  ...
  const sprintAt = parseTimestamp(
    assessment?.generatedAt || bundle.compliance?.evaluatedAt || bundle.sprintReport?.generatedAt
  );
...
function isLiveGatePreferred(bundle = {}) {
  ...
  return (
    embedded?.gatePass != null && embeddedAt != null && sprintAt != null && embeddedAt > sprintAt
  );
}
...
function reconcileComplianceForLiveGate(compliance, bundle = {}) {
  ...
  return {
    ...
    summary: {
      ...
      score,
      headline:
        buildChecklistHeadline(passed, failed, rules.length) ?? compliance.summary?.headline,
...
function buildEuAiActSummary(bundle = {}) {
  ...
  return {
    ...
    evaluatedAt:
      compliance?.evaluatedAt ?? assessment?.generatedAt ?? embedded?.generatedAt ?? null,
...
function syncAssessmentEuMetrics(assessment, summary) {
  ...
  return {
    ...
    euAiActSummary: {
      ...
      aiSystemIndicators:
        summary.aiSystemIndicators ?? assessment.euAiActSummary.aiSystemIndicators,
      highRiskIndicators:
        summary.highRiskIndicators ?? assessment.euAiActSummary.highRiskIndicators,
      transparencyGaps: summary.transparencyGaps ?? assessment.euAiActSummary.transparencyGaps,
      documentationArtifacts:
        summary.documentationArtifacts ?? assessment.euAiActSummary.documentationArtifacts,
    },
  };
}
...
function sanitizeLegalAttestationExport(attestation, _projectLabel) {
  ...
  return {
    ...
    approver_email: attestation.approver_email || null,
...
function buildSprintArtifactExportNotes(sprint = {}, options = {}) {
  ...
  const sprintRepo =
    sprint.report?.repositoryFilesTotal ?? sprint.report?.repositoryInventory?.totalFiles ?? null;
  ...
  const sprintFiction =
    sprint.report?.fictionJsonFilesScanned ?? sprint.report?.scanScope?.fictionJsonFilesScanned;
  if (
    sprintFiction != null &&
    gateFiction != null &&
    fictionSamples != null &&
...
function buildSprintArtifactHygieneSummary(sprint = {}, options = {}) {
  ...
  return {
    ...
    euAiActScanned:
      sprint.report?.euAiActScanned ?? sprint.report?.scanScope?.euAiActFilesScanned ?? null,
...
function reconcileEuAiActSprintReportLimitations(report) {
  ...
  const sourceFiction =
    report.sourceCodeFilesScanned ?? report.scanScope?.sourceCodeFilesScanned ?? 0;
...
module.exports = {
  ...
  resolveGateResult,
  resolveFreshEuAiActMetrics,
  buildFreshnessNote,
  isLiveGatePreferred,
  resolveSprintGateResult,
  reconcileExecutiveSummaryForLiveGate,
  dedupeFindingSummary,
  splitDocumentationPaths,
  applyExportReconciliation,
  reconcileComplianceForLiveGate,
  reconcileAssessmentForLiveGate,
  sanitizeComplianceExport,
  sanitizeAssessmentExport,
  sanitizeClassificationExport,
  sanitizeLegalAttestationExport,
  sanitizeEmbeddedMainReportExport,
  sanitizeSprintReportExport,
  buildEuAiActSummary,
  buildExportProvenance,
  buildEuAiActExportBundle,
  sanitizeEuAiActExport,
  sanitizeEuAiActSprintArtifactExport,
  buildEuAiActChecklistCsv,
  buildEuAiActDocumentationCsv,
  buildEuAiActSummaryCsv,
  buildEuAiActCsv,
