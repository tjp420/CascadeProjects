...
const PLACEHOLDER_PATTERN =
  /replace|changeme|demo|example|xxx|your_|YOUR_.*_HERE|todo|placeholder|dummy|sk_test_your|REPLACE_ME/i;
...
function assertAuthConfiguration() {
  ...
  if (!isConfiguredSecret(process.env.JWT_REFRESH_SECRET))
...
function applyLocalV1InternalDevProfile() {
  ...
  const jwtMissing =
    !isConfiguredSecret(process.env.JWT_SECRET) ||
...
function assertProductionAuthSafety() {
  ...
  if (requireAuth !== "true") {
    violations.push(
      `REQUIRE_AUTH=true (got ${JSON.stringify(process.env.REQUIRE_AUTH)})`,
    );
  }
  if (seedDemoUsers !== "false") {
    violations.push(
      `SEED_DEMO_USERS=false (got ${JSON.stringify(process.env.SEED_DEMO_USERS)})`,
    );
  }
  if (allowLegacyLogin === "true") {
    violations.push(
      `ALLOW_LEGACY_LOGIN must not be true (got ${JSON.stringify(process.env.ALLOW_LEGACY_LOGIN)})`,
    );
  }
