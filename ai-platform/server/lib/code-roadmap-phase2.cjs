...
function shouldIgnorePhase2Path(relativePath) {
  ...
  return normalized
    .split("/")
    .some((segment) => segment === "docs" || segment === "archive");
}
  ...
  const jsFiles = files
    .filter(
      (f) =>
        f.ext === ".js" &&
        f.size < 200000 &&
        !shouldIgnorePhase2Path(f.relativePath),
    )
  ...
  const dfs = (node, pathStack, depth) => {
    ...
    for (const neighbor of adjacency[node] || []) {
      if (!visited.has(neighbor)) {
        ...
        const cyclePath =
          start >= 0
            ? [...pathStack.slice(start), neighbor]
            : [...pathStack, neighbor];
        cycles.push({
          path: cyclePath,
...
function findFuzzySimilarPairs(files, _projectRoot) {
  const scoped = files
    .filter(
      (f) =>
        ...
        f.size < 80000 &&
        f.size > 100 &&
        (f.relativePath.startsWith("server/") ||
  ...
  for (let i = 0; i < loaded.length; i++) {
    for (let j = i + 1; j < loaded.length; j++) {
      const similarity = fuzzySimilarity(
        loaded[i].normalized,
        loaded[j].normalized,
      );
      if (
        similarity >= FUZZY_THRESHOLD &&
        loaded[i].normalized !== loaded[j].normalized
      ) {
        pairs.push({
          ...
          recommendation:
            "Review for merge/refactor — not identical duplicates",
        });
      }
    }
  }
