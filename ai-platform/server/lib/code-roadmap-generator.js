...
const { resolveSampleFilePath } = require('./sample-path-resolver');
const { buildPhase2Analysis } = require('./code-roadmap-phase2');
const { REPOSITORY_AUDIT_BASELINE } = require('./repository-audit-baseline');
const { loadJestCoverageSummary } = require('./jest-coverage-reader');
const { getCodeExtensions } = require('./universal-language-config');
...
function detectPlatformSignalsAt(projectRoot) {
    ...
    return {
        ...
        codeRoadmapGenerator: fs.existsSync(path.join(root, 'server/lib/code-roadmap-generator.js')),
        ...
        buildFromPath: fs.existsSync(path.join(root, 'src/api/build-from-path-route.js')),
        assessmentApi: fs.existsSync(path.join(root, 'server/api/assessment/index.js')),
