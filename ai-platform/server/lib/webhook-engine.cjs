  ...
  while (true) {
    try {
      ...
      try {
        ...
        logger.debug("webhook retry recordDelivery failed", {
          error: e.message,
        });
      }
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(timeoutMs, () => {
      try {
        ...
        logger.debug("req.abort failed during timeout", { error: e.message });
      }
      var latencyMs = Date.now() - startMs;
      try {
        signingStore.recordDelivery({
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        try {
          ...
          logger.debug("signingStore.recordDelivery failed", {
            error: e.message,
          });
        }
        try {
    ...
    req.on("error", (err) => {
      ...
      try {
        ...
        logger.debug("signingStore.recordDelivery failed on error", {
          error: e.message,
        });
      }
      reject(err);
