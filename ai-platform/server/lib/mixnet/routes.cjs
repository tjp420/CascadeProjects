...
const net = new Mixnet(3, {
  seed: "global-mixnet-seed",
  threshold: 5,
  epochMs: 3000,
});
...
router.post("/register-node", (req, res) => {
  ...
  if (!id || !pubkey)
    return res.status(400).json({ error: "id and pubkey required" });
...
router.post("/submit-packet", async (req, res) => {
  const pkt = req.body;
  if (!pkt || !pkt.id || pkt.payload === undefined)
    return res.status(400).json({ error: "packet missing id/payload" });
  // packet.payload is expected to be base64 of the outermost layer
  await net.submitPacket({
    id: pkt.id,
    payload: Buffer.from(pkt.payload, "base64"),
  });
