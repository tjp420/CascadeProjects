...
async function runCheck() {
  const m = new Mixnet(NODE_COUNT, { seed: 'ci-seed', threshold: THRESHOLD, epochMs: EPOCH_MS });
  ...
  for (const n of metrics) {
    ...
    console.log(`node ${n.id} samples: count=${samples.length} max=${localMax} avg=${(samples.reduce((a,b)=>a+b,0)/samples.length).toFixed(2)}`);
  }
...
runCheck().catch(err => { console.error(err); process.exit(1); });
