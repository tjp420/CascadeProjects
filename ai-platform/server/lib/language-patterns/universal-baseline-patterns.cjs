...
const UNIVERSAL_TECH_DEBT = [
  ...
  {
    id: "stub-marker",
    pattern: /\b(?:stub|not\s+implemented|unimplemented)\b/i,
    label: "Stub / not implemented marker",
  },
];
...
const UNIVERSAL_PLACEHOLDERS = [
  ...
  },
  {
    ...
    pattern: /\b(?:placeholder|changeme|replace-?me|your-?api-?key)\b/i,
    label: "Placeholder token",
  },
  {
    id: "example-domain",
    pattern: /\bexample\.(?:com|org|net)\b/i,
    label: "example.* domain placeholder",
  },
];
...
const FAMILY_DEBUG_PATTERNS = {
  web: [
    {
      id: "console-log",
      pattern: /\bconsole\.(?:log|debug|info|trace)\s*\(/g,
      label: "Console logging call",
    },
    {
  ...
  systems: [
    {
      id: "c-printf",
      pattern: /\bprintf\s*\(/g,
      label: "C printf debug output",
    },
    { id: "rust-dbg", pattern: /\bdb!?\s*\(/g, label: "Rust dbg!/debug macro" },
    {
      id: "rust-println",
      pattern: /\b(?:println!|eprintln!)\s*\(/g,
      label: "Rust println/eprintln",
    },
    {
      id: "go-fmt-print",
      pattern: /\bfmt\.Print(?:ln|f)?\s*\(/g,
      label: "Go fmt.Print* debug output",
  ...
  data: [
    ...
    {
      id: "python-pdb",
      pattern: /\bpdb\.set_trace\s*\(/g,
      label: "Python pdb breakpoint",
    },
  ...
  devops: [
    {
      id: "shell-echo-debug",
      pattern: /\becho\s+["']?(?:DEBUG|TRACE|TODO)/gi,
      label: "Shell debug echo",
    },
  ],
  scripting: [
    {
      id: "script-print",
      pattern: /\b(?:print|Write-Host|echo)\s*\(?/gi,
      label: "Script debug output",
    },
    {
      id: "ruby-puts",
      pattern: /\b(?:puts|p)\s+/g,
      label: "Ruby puts/p debug output",
    },
    {
      id: "php-var-dump",
      pattern: /\b(?:var_dump|print_r|dd)\s*\(/gi,
      label: "PHP dump helper",
    },
  ],
  domain: [
    {
      id: "domain-print",
      pattern: /\b(?:print|printf|display)\s*\(/gi,
      label: "Debug print call",
    },
  ],
};
  ...
  systems: [
    {
      ...
      label: "Rust unwrap() — prefer explicit error handling",
    },
  ...
  data: [
    {
      ...
      pattern: /except\s*:/g,
      label: "Python bare except clause",
    },
  ],
  devops: [
    {
      id: "terraform-count",
      pattern: /\bcount\s*=\s*0\b/g,
      label: "Terraform count=0 (disabled resource pattern)",
    },
  ],
};
...
const UNIVERSAL_DEBUG = [
  {
    id: "debug-comment",
    pattern: /\/\/\s*DEBUG\b|#\s*DEBUG\b|\/\*\s*DEBUG\b/i,
    label: "DEBUG comment marker",
  },
];
