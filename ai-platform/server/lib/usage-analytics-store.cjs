...
function recordScan(params) {
  ...
  const scanId =
    params.scanId || `scan-${crypto.randomBytes(6).toString("hex")}`;
...
function updateOrgAggregate(store, entry) {
  ...
  if (entry.repository) {
    if (!org.repositories[entry.repository]) {
      org.repositories[entry.repository] = {
        scans: 0,
        findings: 0,
        lastScanAt: null,
      };
...
function getOrgSummary(orgId) {
  ...
  const avgPosture =
    org.postureScores.length > 0
      ? Math.round(
          org.postureScores.reduce((a, b) => a + b, 0) /
            org.postureScores.length,
        )
      : 0;
  const latestPosture =
    org.postureScores.length > 0
...
function getGlobalStats(filters = {}) {
  ...
  if (filters.orgId)
    allScans = allScans.filter((s) => s.orgId === filters.orgId);
  if (filters.repository)
    allScans = allScans.filter((s) => s.repository === filters.repository);
  if (filters.branch)
    allScans = allScans.filter((s) => s.branch === filters.branch);
  if (filters.startDate)
    allScans = allScans.filter((s) => s.timestamp >= filters.startDate);
  if (filters.endDate)
    allScans = allScans.filter((s) => s.timestamp <= filters.endDate);
  ...
  const postureScores = allScans
    .map((s) => s.postureScore)
    .filter((v) => v != null);
  const avgPosture =
  ...
  for (const t of trend) {
    t.avgPosture =
      t.postureScores.length > 0
        ? Math.round(
            t.postureScores.reduce((a, b) => a + b, 0) / t.postureScores.length,
          )
        : 0;
...
function getViolationHeatmap(filters = {}) {
  ...
  for (const scan of scans) {
    for (const [category, count] of Object.entries(scan.categoryCounts || {})) {
      if (!heatmap[category])
        heatmap[category] = { category, totalFindings: 0, scanCount: 0 };
