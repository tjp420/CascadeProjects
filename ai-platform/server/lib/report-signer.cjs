...
function buildCoreFields(reportJson) {
  return {
    qualityScore: reportJson.qualityScore === undefined ? null : reportJson.qualityScore,
    projectName: reportJson.projectName || reportJson.projectRoot || reportJson.scanTargetRoot || null,
    gateStatus: reportJson.gate ? (reportJson.gate.pass ? 'pass' : 'fail') : null,
...
function canonicalizeForSigning(reportJson) {
  if (reportJson === null || typeof reportJson !== 'object') return stableStringify(reportJson);
  ...
  if (cloned && typeof cloned === 'object' && 'cryptoValidation' in cloned) {
    try { delete cloned.cryptoValidation; } catch (e) { /* ignore */ }
  }
