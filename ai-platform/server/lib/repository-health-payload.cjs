...
function computeRepositoryHealthScore(summary = {}) {
  let score = 100;
  const savingsRatio =
    (summary.potentialSavingsBytes || 0) / Math.max(summary.totalSizeBytes || 1, 1);
...
function buildRepositoryHealthSnapshot(report, label) {
  ...
  return {
    ...
    repositoryFilesTotal:
      summary.repositoryFilesTotal ?? report.repositoryInventory?.totalFiles ?? null,
    repositoryFoldersTotal:
      summary.repositoryFoldersTotal ?? report.repositoryInventory?.totalFolders ?? null,
...
function buildRepositoryHealthPayload(options = {}) {
  ...
  const monorepo =
    ...
    path.resolve(monorepoReport.projectRoot).toLowerCase() !==
      path.resolve(paths.platformRoot).toLowerCase()
