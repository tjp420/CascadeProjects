...
const ROLES = {
  ...
  auditor: {
    ...
    name: "Auditor",
    description:
      "Read-only access to analytics, audit logs, and compliance reports",
    permissions: [
      "read:all",
      "export:audit",
      "read:audit",
  ...
  operator: {
    ...
    name: "Operator",
    description:
      "Can run scans, manage tickets, trigger evaluations, and view analytics",
    permissions: [
      "read:all",
      "write:tickets",
      "write:scans",
...
const PERMISSIONS = {
  ...
  "read:siem_telemetry":
    "View SIEM cluster telemetry metrics and distributed sync state",
};
...
function setAssignment(userId, role, orgId, actorEmail) {
  ...
  if (!ROLES[role])
    throw new Error(
      `Invalid role: ${role}. Valid roles: ${ALL_ROLE_IDS.join(", ")}`,
    );
  ...
  // Prune
  const orgAssignments = Object.entries(store.assignments).filter(
    ([, v]) => v.orgId === orgId,
  );
  if (orgAssignments.length > MAX_ASSIGNMENTS_PER_ORG) {
    const sorted = orgAssignments.sort((a, b) =>
...
function hasPermission(userPermissions, requiredPermission) {
  ...
  if (
    requiredPermission.startsWith("read:") &&
  ...
  if (
    requiredPermission.startsWith("delete:") &&
    userPermissions.includes("delete:all")
  )
    return true;
