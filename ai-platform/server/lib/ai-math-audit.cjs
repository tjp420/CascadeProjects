    ...
    if (meanEntropy > entropyThreshold) {
      findings.push({
        ...
        metrics: {
          ...
          diagonal_dominance: diagDominance,
        },
      });
    }
...
function auditVectorDrift(embeddingLog, referenceDb, opts) {
  const cosineMin = opts.cosine_min ?? 0.7;
...
function runAudit(logSource, opts) {
  ...
  const findingsActivation = auditActivations({ activations: raw.activations || {} }, args);
  ...
  const findingsDrift = auditVectorDrift({ embeddings: raw.embeddings || {} }, refDb, args);
  const findingsTrace = auditExecutionTrace({ trace: raw.trace || [] }, args);
