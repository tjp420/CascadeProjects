...
describe("selectStrategy()", () => {
  it("routes debugger-statement to DELETE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("debugger-statement")),
      STRATEGIES.DELETE,
    );
  });
  ...
  it("routes console-log to DELETE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("console-log")),
      STRATEGIES.DELETE,
    );
  });
  ...
  it("routes todo-comment to DELETE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("todo-comment")),
      STRATEGIES.DELETE,
    );
  });
  ...
  it("routes fixme-comment to DELETE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("fixme-comment")),
      STRATEGIES.DELETE,
    );
  });
  ...
  it("routes eval-usage to REPLACE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("eval-usage")),
      STRATEGIES.REPLACE,
    );
  });
  ...
  it("routes hardcoded-secret to REPLACE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("hardcoded-secret")),
      STRATEGIES.REPLACE,
    );
  });
  ...
  it("routes unhandled-promise to WRAP", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("unhandled-promise")),
      STRATEGIES.WRAP,
    );
  });
  ...
  it("routes missing-strict-mode to WRAP", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("missing-strict-mode")),
      STRATEGIES.WRAP,
    );
  });
  ...
  it("routes missing-rate-limit to INSERT", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("missing-rate-limit")),
      STRATEGIES.INSERT,
    );
  });
  ...
  it("defaults unknown types to REPLACE", () => {
    assert.strictEqual(
      selectStrategy(makeFinding("unknown-type")),
      STRATEGIES.REPLACE,
    );
  });
});
...
describe("INSERT strategy", () => {
  it("inserts rate-limit import at line 1", () => {
    ...
    const result = applyPatch(FIXTURE.missingStrict, patch);
    assert.ok(
      result.startsWith("const rateLimit = require('express-rate-limit')"),
    );
  });
});
...
describe("Round-trip: patch → apply → diff", () => {
  it("produces a readable diff after applying a DELETE patch", () => {
    ...
    const diff = generateDiff(FIXTURE.consoleLog, patched, {
      filePath: "test.js",
    });
...
describe("Edge cases", () => {
  ...
  it("returns original content for unknown strategy in applyPatch", () => {
    const result = applyPatch("unchanged", {
      strategy: "unknown",
      line: 1,
      newText: "",
    });
