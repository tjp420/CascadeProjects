...
function createScheduler(options) {
        ...
        for (const recipient of cfg.recipients) {
          ...
          if (cfg.includeCertificate && report) {
            attachments.push({
              ...
              content: Buffer.from(JSON.stringify(report, null, 2)).toString(
                "base64",
              ),
            });
          }
      ...
      if (cfg.zeroRetention) {
        ...
        try {
          if (fs.existsSync(reportOut)) {
            ...
            logger.info(
              "[Schedule] Zero-retention: report file removed after delivery",
            );
          }
        } catch (unlinkErr) {
          logger.warn(
  ...
  function startScheduler() {
    ...
    readScheduleConfig()
      ...
      .catch((err) =>
        logger.error("[Schedule] Failed to start scheduler:", err.message),
      );
  }

  return {
    startScheduler,
    runScheduledScanAndDeliver,
    readScheduleConfig,
    writeScheduleConfig,
  };
}
