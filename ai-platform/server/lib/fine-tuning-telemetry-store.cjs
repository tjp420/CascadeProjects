...
const TOKEN_RE = /\b(?:sk-[a-zA-Z0-9_-]{20,}|ghp_[a-zA-Z0-9]{36}|bearer\s+[a-zA-Z0-9]{20,})\b/gi;
...
function scoreEntry(entry) {
  ...
  if (input.includes('?') || input.includes('how') || input.includes('explain') || input.includes('write')) score += 1;

  return Math.min(10, score);
}

function recordTelemetry({ orgId, userId, operation, model, input, output, metadata }) {
  ...
  const lastIdx = store.entries.findLastIndex(function (e) {
    return e.orgId === entry.orgId && e.userId === entry.userId && e.label !== 'exclude' &&
      (new Date(entry.timestamp).getTime() - new Date(e.timestamp).getTime()) <= windowMs;
  });
...
function listEntries(orgId, filters) {
  ...
  return store.entries
    ...
    .filter(function (e) { return (e.turns || 1) >= minTurns; })
    .filter(function (e) { return !label || e.label === label; })
    .filter(function (e) { return !operation || e.operation === operation; })
    .filter(function (e) {
...
function labelEntry(eventId, label) {
  const store = readStore();
  const entry = store.entries.find(function (e) { return e.eventId === eventId; });
...
function exportDataset(orgId, format, filters) {
  ...
  const lines = entries.map(function (e) { return formatRow(e, format); });
...
function listDatasets(orgId) {
  ...
  return fs.readdirSync(DATASET_DIR)
    .filter(function (f) { return f.startsWith(prefix); })
    .map(function (f) {
      const stat = fs.statSync(path.join(DATASET_DIR, f));
      return { filename: f, createdAt: stat.birthtime.toISOString(), sizeBytes: stat.size };
    })
