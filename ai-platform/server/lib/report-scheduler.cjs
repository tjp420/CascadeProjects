...
async function generateReport(schedule, orgId) {
  ...
  const REMEDIATION_GUIDANCE = {
    ...
    'outdated-dependencies': {
      ...
      steps: ['Run npm audit', 'Update vulnerable packages', 'Run full test suite'],
    },
    'missing-rate-limit': {
    ...
    'insecure-random': {
      ...
      priority: 'medium',
      description: 'Replace Math.random() with crypto for security contexts.',
      steps: [
    ...
    _default: {
      ...
      steps: ['Review the findings', 'Determine appropriate fix', 'Apply and test the fix'],
    },
  };
