...
async function generateReport(schedule, orgId) {
  ...
  const result = analyticsStoreRef.getScans({
    orgId,
    limit: 100000,
    offset: 0,
  });
  ...
  const REMEDIATION_GUIDANCE = {
    ...
    "missing-security-headers": {
      ...
      priority: "high",
      description: "Add standard security headers to all responses.",
      steps: [
    ...
    "hardcoded-secrets": {
      strategy: "Vault Migration",
      priority: "critical",
      description: "Move secrets to environment variables or vault.",
      steps: [
        "Identify all hardcoded credentials",
        "Move to env vars or secrets manager",
        "Rotate exposed credentials",
      ],
    },
    "debug-artifacts": {
      strategy: "Production Stripping",
      priority: "medium",
      description: "Remove debug code from production builds.",
      steps: [
        "Remove console.log statements",
        "Remove debugger statements",
        "Enable production build stripping",
      ],
    },
    ...
    "prototype-pollution": {
      ...
      steps: [
        ...
        "Sanitize recursive merge functions",
        "Add Object.freeze to prototypes",
      ],
    },
    "eval-usage": {
      ...
      priority: "high",
      description:
        "Replace dynamic code execution with safer, explicit parsing or structured alternatives.",
      steps: [
        "Identify dynamic-code execution sites",
        "Replace with explicit parsers (e.g., JSON.parse) or validated templates",
        "Add CSP restrictions where appropriate",
      ],
    },
    "insecure-random": {
      strategy: "CSPRNG Migration",
      priority: "medium",
      description:
        "Replace non-cryptographic RNG usage with a CSPRNG for security-sensitive contexts.",
      steps: [
        "Identify security-sensitive random usage",
        "Replace with crypto.randomBytes",
        "Test entropy quality",
      ],
    },
    "config-drift": {
      strategy: "Config Reconciliation",
      priority: "low",
      description: "Align configuration across environments.",
      steps: [
        "Audit environment configs",
        "Create config templates",
        "Deploy config validation CI check",
      ],
    },
    _default: {
      strategy: "Manual Review",
      priority: "medium",
      description: "Review and remediate findings manually.",
      steps: [
        "Review the findings",
  ...
  for (const scan of scans) {
    ...
    for (const [category, count] of Object.entries(cc)) {
      ...
      const status =
        ticketStatuses[
          ticketStatusStore.buildTicketKey(orgId, scan.scanId, category)
        ];
  ...
  if (filters.ticketStatus === "ticketed")
    violations = violations.filter((v) => v.ticketed);
  else if (filters.ticketStatus === "unticketed")
    violations = violations.filter((v) => !v.ticketed);
  if (filters.slaBreached === "true" || filters.slaBreached === true)
    violations = violations.filter((v) => v.slaBreached);
...
async function runSchedule(schedule, orgId) {
  ...
  try {
    logger.info(
      `[ReportScheduler] Running schedule "${schedule.name}" (${id})`,
    );
    ...
    reportScheduleStore.updateScheduleRunResult(
      id,
    ...
    logger.info(
      `[ReportScheduler] Schedule "${schedule.name}" completed: ${emailResult.method}`,
    );
    return { success: true, report, emailResult };
  } catch (err) {
    logger.error(
      `[ReportScheduler] Schedule "${schedule.name}" failed:`,
      err.message,
    );
    reportScheduleStore.updateScheduleRunResult(
      id,
...
function startScheduler() {
  ...
  logger.info(
    "[ReportScheduler] Background scheduler started (60s check interval)",
  );
