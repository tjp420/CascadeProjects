...
async function generateReport(schedule, orgId) {
  ...
  const REMEDIATION_GUIDANCE = {
    ...
    'eval-usage': { strategy: 'Eval Removal', priority: 'high', description: 'Replace eval() with safer alternatives.', steps: ['Identify all eval() calls', 'Replace with Function() or JSON.parse', 'Add CSP unsafe-eval restriction'] },
    'insecure-random': { strategy: 'CSPRNG Migration', priority: 'medium', description: 'Replace Math.random() with crypto for security contexts.', steps: ['Identify security-sensitive random usage', 'Replace with crypto.randomBytes', 'Test entropy quality'] },
