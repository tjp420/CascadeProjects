...
function createEventStore(options = {}) {
  ...
  const STORE =
    options.storePath ||
    process.env.STRIPE_EVENT_STORE ||
    path.join(projectRoot, '.simplebeacon', 'stripe-events.json');
  ...
  async function writeProcessedEvents(events) {
    ...
    async function doWrite(events) {
      ...
      const payload = JSON.stringify(
        { eventIds: ids, updatedAt: new Date().toISOString(), count: ids.length },
        null,
        2
      );
      ...
      try {
        ...
        try {
          ...
          try {
            logger.debug && logger.debug('[StripeEventStore] fsync failed', err);
          } catch (_) {}
        }
        fs.closeSync(fd);
      } catch (err) {
