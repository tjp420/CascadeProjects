...
function countEuPatternHits(report) {
  ...
  return issues.filter((item) => /eu ai act/i.test(String(item.type || "")))
    .length;
}
...
async function runEuAiActSprint(input = {}, options = {}) {
  const platformRoot = path.resolve(
    options.platformRoot || path.join(__dirname, "../.."),
  );
  ...
  let npmAudit = null;
  try {
    const { runNpmAuditAsync } = require(
      path.join(detectedRoot, "server/lib/npm-audit-runner"),
    );

    npmAudit = await runNpmAuditAsync(scanRoot, {
      force: input.forceNpmAudit === true,
    });
  } catch {
    npmAudit = null;
  }
  ...
  if (workspaceDir) {
    ...
    await fsp.copyFile(
      compliancePath,
      path.join(workspaceDir, "compliance.json"),
    );
    await fsp.copyFile(
      assessmentPath,
      path.join(workspaceDir, "assessment.json"),
    );
  }
   ...
   * @returns {any}
   */
  const failedRules = (complianceChecklist.rules || []).filter(
  ...
  return {
    ...
    gate: {
      ...
      blockingCount:
        gateResult.blockingCount ?? gateResult.blockingIssues?.length ?? null,
    ...
    relativeArtifacts: {
      ...
      compliance: path
        .relative(detectedRoot, compliancePath)
        .replace(/\\/g, "/"),
      assessment: path
        .relative(detectedRoot, assessmentPath)
        .replace(/\\/g, "/"),
    },
    disclaimer:
      "Static technical readiness review — not legal conformity certification.",
  };
}
