...
const DB_PATH =
  process.env.SIMPLEBEACON_TOKEN_DB_PATH ||
  path.join(__dirname, "../db/token-registry.json");
...
function rotateSessionToken(id, newTokenHash, newExpiresAt) {
  return withDbQueued((db) => {
    ...
    db.session_tokens[idx].token_sequence =
      (db.session_tokens[idx].token_sequence || 0) + 1;
...
function syncSessionToken(token) {
  return withDbQueued((db) => {
    ...
    const existing = db.session_tokens.find(
      (t) => t.token_hash === token.token_hash,
    );
    if (!existing) {
...
function revokeRefreshTokenBySession(sessionTokenId, reason) {
  return withDbQueued((db) => {
    const idx = db.refresh_tokens.findIndex(
      (t) => t.session_token_id === sessionTokenId && !t.revoked_at,
    );
    if (idx === -1) return null;
...
function getLicenseTokensByEmail(email) {
  ...
  const db = loadDb();
  return db.license_tokens.filter(
    (t) =>
      String(t.email || "")
        .trim()
