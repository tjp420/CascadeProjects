...
function resolveProjectPath(baseDir, rawPath, monorepoRoot) {
    ...
    if (!fs.existsSync(normalized)) {
      ...
      if (normalizedKey.includes("/ai-platform")) {
        ...
        if (serverPlatformKey.includes("/ai-platform")) {
          ...
          const existingPlatform = findExistingPlatformDir(
            baseDir,
            effectiveMonoRoot,
          );
          ...
          if (existingPlatform) {
            const existingPlatformKey = existingPlatform
              .replace(/\\/g, "/")
              .toLowerCase();
      ...
      if (platformIdx !== -1) {
        const afterPlatform = normalized.slice(
          platformIdx + "/ai-platform/".length,
        );
  ...
  const monoBasename = monorepoRoot
    ? path.basename(monorepoRoot).toLowerCase()
    : "";
  const baseBasename = path.basename(baseDir).toLowerCase();
  const trimmedLower = trimmedPath.toLowerCase();
  if (
    monoBasename &&
    trimmedLower === monoBasename &&
    fs.existsSync(monorepoRoot)
  ) {
    return monorepoRoot;
  }
  ...
  if (
    !fs.existsSync(fromBase) &&
    monorepoRoot &&
    fromBase.startsWith(monorepoRoot)
  ) {
...
function deriveSeverityCounts(findings) {
  ...
  for (const issue of findings) {
    ...
    const increment =
      typeof issue.count === "number" && issue.count > 0 ? issue.count : 1;
...
function resolveModelId(registry, aiProvider) {
  ...
  if (provider === "demo") {
    return (
      registry.models?.find((m) => m.provider === "demo")?.id ||
  ...
  if (provider === "ollama") {
    return (
      registry.models?.find((m) => m.provider === "ollama")?.id ||
      registry.activeModelId
    );
  }
  return registry.activeModelId;
...
function normalizeReportForSummary(report, reportType = "") {
  if (!report || typeof report !== "object")
    return {
      reportKind: reportType || "simplebeacon-report",
      detectedIssues: [],
    };
  ...
  if (type === "file-merger-reduction-report") {
    return {
      ...
      analysisOverview: {
        ...
        issuesDetected:
          (report.summary?.mergeCandidates || 0) +
  ...
  return {
    ...
    analysisOverview: {
      ...
      ruleScopedFilesAnalyzed:
        ...
        (report.filesAnalyzed !== report.repositoryFilesTotal
      ...
      for (const ent of entries) {
        if (ent.isDirectory()) {
          if (!skip.has(ent.name))
            queue.push({ dir: path.join(cur, ent.name), depth: depth + 1 });
        } else {
          count++;
        }
      }
...
function sendAnalyzeJson(res, payload, statusCode = 200, opts = {}) {
  if (
    ...
    throw new TypeError(
      "sendAnalyzeJson requires a valid Express response object",
    );
  }
  const code =
  ...
  const body =
    opts.publicGateEnabled &&
    typeof opts.applyPublicGateToAnalyzeResponse === "function"
...
function rejectPaidDeliverable(res, auditCheckoutUrl) {
  if (!res || typeof res.status !== "function") {
    throw new TypeError(
      "rejectPaidDeliverable requires a valid Express response object",
    );
  }
...
function resolveMockScanPaths(baseDir, projectPath, helpers = {}) {
  ...
  const projectKey = path
    ...
    .replace(/\\/g, "/")
    .toLowerCase();
  const platformKey = path
    .resolve(platformRoot)
    .replace(/\\/g, "/")
    .toLowerCase();
...
async function pathLooksLikeMockScan(targetPath) {
  try {
    ...
    const sourceCount = files.filter((n) =>
      /\.(js|ts|jsx|tsx|py|cjs|mjs)$/.test(n),
    ).length;
    if (jsonCount === 0) return false;
    if (sourceCount > 0) return false;
    if (files.length === 0) return false;
    const mockNamedCount = files.filter((n) =>
      /mock|sample|demo|fixture/i.test(n),
    ).length;
 ...
 */
async function resolveAnalysisType(
  requestedType,
  targetPath,
  ...
  return typeof pathLooksLikeMockScan === "function" &&
    (await pathLooksLikeMockScan(targetPath))
    ? "mock-scan"
    : "roadmap";
}
 ...
 * @returns {Object|null}
 */
function resolveOllamaSummaryProvider(
  registry,
...
function resolveSummaryProvider(
  ...
  if (aiProvider === "active") {
    ...
    return resolveOllamaSummaryProvider(
      registry,
      userCredentials,
      defaultOllamaUrl,
    );
...
async function fetchWebsiteToTemp(rawUrl) {
  ...
  const fetchTimeoutMs =
    Number(process.env.SIMPLEBEACON_WEBSITE_FETCH_TIMEOUT_MS) || 30000;
  await new Promise((resolve, reject) => {
    ...
    const fetchIndex = (currentUrl, redirects = 0) => {
          ...
          if (
            ...
            try {
              ...
              if (isPrivateHostname(redirectParsed.hostname)) {
                return reject(
                  new Error("Blocked redirect to private/internal address"),
                );
              }
            } catch {
              return reject(
                new Error(
                  `Invalid redirect location: ${location.slice(0, 120)}`,
                ),
              );
            }
      ...
      request.on("timeout", () => {
        ...
        reject(
          new Error(
            `Website fetch timeout after ${fetchTimeoutMs}ms for ${currentUrl.slice(0, 120)}`,
          ),
        );
      });
    };
  ...
  try {
    ...
    for (const match of assetMatches) {
      ...
      try {
        const client2 =
          new NodeURL(assetUrl).protocol === "https:" ? https : http;
