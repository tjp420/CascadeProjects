...
function buildSprintModel(signals, metrics, samples, scanReport = null) {
  ...
  for (const sprint of sprints) {
    ...
    if (sprint.id === 'sprint-3' && safeSamples.withSpecs > 0 && safeSamples.specTotal) {
      progress = Math.max(progress, safeSamples.withSpecs / safeSamples.specTotal);
    }
    ...
    phases.push({
      ...
      status,
      progress,
      description: sprintDeliverableDescription(sprint.id, safeSignals, safeSamples, safeMetrics),
      features: sprintFeatureList(sprint.id, safeSignals, safeSamples, safeMetrics),
...
function sprintMilestones(sprintId, signals, metrics) {
  ...
  if (sprintId === 'sprint-2')
    return ['dashboard-stub-api.js', metrics.jestTestsLabel || `${metrics.testFiles} tests`];
...
function extractDetectedFeatures(signals, metrics, samples) {
  ...
  if (signals.serverEntry)
    list.push({ name: 'Dashboard Server', category: 'Infrastructure', status: 'implemented' });
  if (signals.phase2Auth)
    list.push({ name: 'Phase 2 JWT Auth', category: 'Security', status: 'implemented' });
  if (signals.stubApi)
    list.push({ name: 'Dashboard Stub API', category: 'API', status: 'implemented' });
  if (signals.fixtureScanner)
    list.push({ name: 'Mock Data Scanner', category: 'Analysis', status: 'implemented' });
  if (signals.npmAudit)
    list.push({ name: 'npm Audit Runner', category: 'Security', status: 'implemented' });
  if (signals.fileMergerScanner)
    list.push({ name: 'File Merger Scanner', category: 'Analysis', status: 'implemented' });
  if (signals.codeRoadmapGenerator)
    list.push({ name: 'Code Roadmap Generator', category: 'Planning', status: 'implemented' });
  if (signals.assessmentApi)
    list.push({ name: 'Assessment API', category: 'API', status: 'implemented' });
