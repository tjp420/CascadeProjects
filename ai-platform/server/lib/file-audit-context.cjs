...
function resolveFileAuditContext(relativePath, hooks = {}) {
  ...
  const isMetaCatalogDoc =
    typeof hooks.isMetaCatalogDoc === "function"
...
function shouldIncludeFinding(finding, context) {
  ...
  if (context.isDocumentation && finding.category === "debug-artifact")
    return false;
  if (context.isExampleFile && finding.category === "meaningless-data")
    return false;
  if (
    String(finding.type || "").includes("production-leak") &&
  ...
  if (
    finding.type === "missing-proptypes" &&
    context.relativePath.endsWith(".cjs")
  )
    return false;
  // Config / route files don't need JSDoc on every helper
  if (
    finding.type === "missing-jsdoc" &&
    /server\/(config|api|lib)\//.test(context.relativePath)
  )
    return false;
  // Shell scripts use echo/Write-Host by design — not debug artifacts
  if (
    finding.category === "debug-artifact" &&
    /\.(sh|ps1|bash|zsh)$/.test(context.relativePath)
  )
    return false;
  // Markdown files contain code blocks by design — not fence leaks
  if (
    (finding.category === "markdown-fence-leak" ||
      finding.type === "markdown-fence-in-code") &&
  ...
  if (
    finding.category === "workspace-health" &&
    finding.type === "circular-import-risk"
  )
    return false;
  return true;
}
...
function adjustFindingSeverity(finding, context) {
  ...
  if (finding.category === "debug-artifact" && finding.severity === "medium") {
    return {
      ...
      metadata: {
        ...(finding.metadata || {}),
        contextAdjusted: "non-production-path",
      },
    };
  }
  if (
