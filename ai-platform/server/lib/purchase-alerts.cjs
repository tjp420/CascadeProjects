...
function buildSlackPayload(params) {
  var tierEmoji = params.tier === 'team_pro' ? ':rocket:' :
                  params.tier === 'developer' ? ':computer:' :
                  params.tier === 'enterprise' ? ':office:' : ':moneybag:';
  return {
    text: tierEmoji + ' New SimpleBeacon Purchase',
    attachments: [{
      color: params.tier === 'enterprise' ? '#7c3aed' :
             params.tier === 'team_pro' ? '#22c55e' : '#3b82f6',
      fields: [
        { title: 'Tier', value: params.tier, short: true },
        { title: 'Amount', value: params.amount ? '$' + (params.amount / 100).toFixed(2) : 'N/A', short: true },
        { title: 'Customer', value: maskEmail(params.email), short: true },
        { title: 'Price ID', value: params.priceId || 'N/A', short: true },
        { title: 'Stripe Customer', value: params.customerId || 'N/A', short: true },
...
function buildDiscordPayload(params) {
  var color = params.tier === 'enterprise' ? 8126975 :  // purple
              params.tier === 'team_pro' ? 2293760 :    // green
              3932160;                                   // blue
  var emoji = params.tier === 'team_pro' ? '🚀' :
  ...
  return {
    embeds: [{
      ...
      fields: [
        ...
        { name: 'Amount', value: params.amount ? '$' + (params.amount / 100).toFixed(2) : 'N/A', inline: true },
        { name: 'Customer', value: maskEmail(params.email), inline: true },
        { name: 'Price ID', value: params.priceId || 'N/A', inline: true },
        { name: 'Stripe Customer', value: params.customerId || 'N/A', inline: true },
...
async function sendPurchaseAlert(params) {
  ...
  var isDiscord = webhookUrl.indexOf('discord.com/api/webhooks') >= 0 ||
                  webhookUrl.indexOf('discordapp.com/api/webhooks') >= 0;
  var isSlack = webhookUrl.indexOf('hooks.slack.com') >= 0;

  if (!isDiscord && !isSlack) {
    logger.warn('[PurchaseAlert] Webhook URL does not match Slack or Discord pattern, skipping');
    return { sent: false, platform: null };
  }

  var platform = isDiscord ? 'discord' : 'slack';
  var payload = isDiscord ? buildDiscordPayload(params) : buildSlackPayload(params);
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn('[PurchaseAlert] ' + platform + ' returned HTTP ' + response.status);
      return { sent: false, platform: platform };
    }

    logger.info('[PurchaseAlert] Notification sent to ' + platform + ' for tier:', params.tier);
    return { sent: true, platform: platform };
  } catch (err) {
    logger.warn('[PurchaseAlert] Failed to send to ' + platform + ':', err.message);
    return { sent: false, platform: platform };
  }
}
