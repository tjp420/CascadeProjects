 ...
 * @returns {Array} ΓÇö decrypted rules
 */
...
// ΓöÇΓöÇ Webhook Secret Rotation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 ...
 * @param {string} ruleId
 * @param {string} orgId
 * @param {string} newSecret ΓÇö the new HMAC secret (plaintext, will be encrypted at rest)
...
/**
 * Get the rotation status for a rule ΓÇö whether a grace window is active,
 * when it expires, and whether the previous secret is still available.
 * @param {string} ruleId
 * @param {string} orgId
 * @param {number} graceWindowMs ΓÇö override the default grace window
 ...
 * @param {string} ruleId
 * @param {string} orgId
 * @param {boolean} force ΓÇö if true, clear even if grace window is still active
 ...
 * @param {number} bytes ΓÇö number of random bytes (default 32 = 256 bits)
 ...
 *
 * @param {number} graceWindowMs ΓÇö grace window in milliseconds (defaults to DEFAULT_GRACE_WINDOW_MS)
...
function purgeExpiredPreviousSecrets(graceWindowMs) {
  ...
  for (const [key, rule] of Object.entries(store.rules)) {
    ...
    if (!rotatedAt) {
      // No timestamp ΓÇö shouldn't happen, but purge to be safe
      delete rule.destination.previousSecret;
      delete rule.destination.secretRotatedAt;
      rule.updatedAt = new Date().toISOString();
      purged++;
