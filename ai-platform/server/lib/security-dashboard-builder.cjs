...
function buildOverviewFromVulnerabilities(raw) {
  ...
  const openEngineeringFindings = openFindings.filter(
    (item) => !isNpmAuditFinding(item),
  );
  const openNpmFindings = openFindings.filter(isNpmAuditFinding);
  const npmSummaryTotal = raw.npmAudit?.summary?.total;
  const npmAuditTotal =
    npmSummaryTotal != null ? npmSummaryTotal : openNpmFindings.length;
  const criticalVulns = openNpmFindings.filter(
  ...
  return {
    ...
    resolvedIncidents: incidents.filter((item) => item.status === "resolved")
      .length,
    npmAuditTotal,
  };
}
...
function mergeNpmAuditIntoSecurityModel(model, auditPayload) {
  if (!model || !auditPayload || auditPayload.error) {
    return {
      ...model,
      overview: buildOverviewFromVulnerabilities(model || {}),
    };
  }
   ...
   * @returns {any}
   */
  const checklist = (model.vulnerabilities || []).filter(
    (item) => item.source !== "npm-audit",
  );
