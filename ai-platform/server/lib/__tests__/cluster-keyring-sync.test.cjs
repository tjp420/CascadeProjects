...
describe("Cluster Keyring Sync — Unit Tests", () => {
  ...
  describe("Event Timeline (Sync.com-style)", () => {
    it("records events with unique event IDs and timestamps", () => {
      const ev1 = clusterSync._recordEvent("leader_elected", "node-A", {
        leader: "node-A",
      });
      const ev2 = clusterSync._recordEvent("key_commit", "node-A", {
        fingerprint: "abc123",
      });
    ...
    it("S-01: event details never contain raw key material", () => {
      ...
      assert.ok(
        !evStr.match(/[0-9a-f]{64}/),
        "Event contains 64-char hex (possible raw key)",
      );
  ...
  describe("getStatus()", () => {
    ...
    it("returns keyring fingerprints (not raw keys)", () => {
      ...
      assert.ok(
        !statusStr.match(/[0-9a-f]{64}/),
        "Status contains 64-char hex (possible raw key)",
      );
    });
  });
  ...
  describe("proposeRotate()", () => {
    ...
    it("L3-05: grace window is passed through rotation", () => {
      ...
      if (status.isLeader) {
        ...
        const newStatus = clusterSync.proposeRotate(
          crypto.randomBytes(32).toString("hex"),
          graceMs,
        );
        assert.ok(newStatus);
  ...
  describe("Idempotency & ordering (L3-03)", () => {
    ...
    it("rejects a duplicate KEY_COMMIT with the same rotatedAt (no re-apply)", () => {
      ...
      assert.strictEqual(
        afterSecond.activeFingerprint,
        afterFirst.activeFingerprint,
      );
    ...
    it("rejects a stale (out-of-order) KEY_COMMIT with older rotatedAt", () => {
      ...
      assert.strictEqual(
        afterOlder.activeFingerprint,
        afterNewer.activeFingerprint,
      );
    ...
    it("rejects a KEY_COMMIT with missing/invalid rotatedAt", () => {
      ...
      assert.ok(rejects.total >= 1);
      assert.strictEqual(
        rejects.events[0].details.reason,
    ...
    it("S-04: key_reject events do not contain raw key material", () => {
      ...
      assert.ok(
        !evStr.match(/[0-9a-f]{64}/),
        "key_reject event contains 64-char hex (possible raw key)",
      );
    });
  });
});
...
describe("Cluster Keyring Sync — API Routes", () => {
  ...
  describe("POST /api/audit/cluster/keyring/rotate", () => {
    ...
    it("L2-02: leader can rotate and event is recorded", async () => {
      const statusRes = await request(adminApp).get(
        "/api/audit/cluster/keyring",
      );
      if (statusRes.body.status.isLeader) {
        const res = await request(adminApp)
          .post("/api/audit/cluster/keyring/rotate")
          .send({
            newKeyRaw: crypto.randomBytes(32).toString("hex"),
            graceMs: 48 * 60 * 60 * 1000,
          });
        assert.strictEqual(res.status, 200);
        assert.strictEqual(res.body.success, true);
        ...
        const eventsRes = await request(adminApp).get(
          "/api/audit/cluster/events?eventType=key_commit",
        );
  ...
  describe("GET /api/audit/cluster/events", () => {
    ...
    it("filters by event type", async () => {
      const res = await request(adminApp).get(
        "/api/audit/cluster/events?eventType=leader_elected",
      );
      assert.strictEqual(res.status, 200);
    ...
    it("paginates with limit and offset", async () => {
      const res = await request(adminApp).get(
        "/api/audit/cluster/events?limit=5&offset=0",
      );
      assert.strictEqual(res.status, 200);
    ...
    it("S-01: events do not contain raw key material", async () => {
      ...
      assert.ok(
        !eventsStr.match(/[0-9a-f]{64}/),
        "Events contain 64-char hex (possible raw key)",
      );
    });
  });
});
