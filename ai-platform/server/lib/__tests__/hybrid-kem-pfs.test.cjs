...
describe("L2-02: deriveRekeyRoot HKDF ratchet", () => {
  it("produces a deterministic 32-byte root from prevRoot + fresh secrets", () => {
    ...
    assert.ok(
      root1.equals(root2),
      "same inputs must produce same root (deterministic)",
    );
  });
  ...
  it("different prevRoot produces different newRoot", () => {
    ...
    assert.ok(
      !root1.equals(root2),
      "different prevRoot must produce different newRoot",
    );
  });
  ...
  it("different ECDH secret produces different newRoot", () => {
    ...
    assert.ok(
      !root1.equals(root2),
      "different ECDH must produce different newRoot",
    );
  });
  ...
  it("different ML-KEM secret produces different newRoot", () => {
    ...
    assert.ok(
      !root1.equals(root2),
      "different ML-KEM must produce different newRoot",
    );
  });
});
...
describe("L2-04: Suspension state queues outbound data during REKEYING", () => {
  it("HybridSession.send returns false and queues data during REKEYING", () => {
    const { clientSocket, serverSocket } = createMockSocketPair();
    const session = new hk.HybridSession(clientSocket, {
      initiator: true,
      timeoutMs: 10000,
    });
    session.setKeys({
  ...
  it("HybridSession.send writes immediately when ACTIVE", () => {
    ...
    assert.ok(
      writtenData.equals(testData),
      "data should be written immediately",
    );
  });
});
...
describe("L3-01 & S-01: Replay of recorded REKEY_INIT is rejected", () => {
  it("replayed REKEY_INIT with stale ephemeral keys produces MAC mismatch", async () => {
    ...
    const { clientSocket: capClient, serverSocket: capServer } =
...
describe("L3-02 & S-02: Leaked single ephemeral does not compromise prevRoot", () => {
  it("knowing ecdhSecret alone does not reveal prevRoot or newRoot", () => {
    ...
    const fakeRoot1 = hk.deriveRekeyRoot(
      crypto.randomBytes(32),
      ecdh,
      crypto.randomBytes(32),
    );
    const fakeRoot2 = hk.deriveRekeyRoot(
      crypto.randomBytes(32),
      ecdh,
      crypto.randomBytes(32),
    );
    assert.ok(
      !fakeRoot1.equals(newRoot),
      "attacker cannot derive newRoot from ecdh alone",
    );
    assert.ok(
      !fakeRoot1.equals(fakeRoot2),
      "different unknowns produce different roots",
    );
  });
  ...
  it("knowing mlkemSecret alone does not reveal prevRoot or newRoot", () => {
    ...
    const fakeRoot = hk.deriveRekeyRoot(
      crypto.randomBytes(32),
      crypto.randomBytes(32),
      mlkem,
    );
    assert.ok(
      !fakeRoot.equals(newRoot),
      "attacker cannot derive newRoot from mlkem alone",
    );
  });
});
...
describe("L3-03: Break-in recovery from compromised root", () => {
  it("compromised prevRoot + fresh re-key = independent new root", () => {
    ...
    const newRoot = hk.deriveRekeyRoot(
      compromisedRoot,
      freshEcdh1,
      freshMlkem1,
    );
    ...
    assert.ok(
      !secondRoot.equals(compromisedRoot),
      "second root must not equal compromised root",
    );
    assert.ok(
      !secondRoot.equals(newRoot),
      "second root must not equal first newRoot",
    );
    ...
    const guessedFromCompromised = hk.deriveRekeyRoot(
      compromisedRoot,
      crypto.randomBytes(32),
      crypto.randomBytes(32),
    );
    assert.ok(
      !guessedFromCompromised.equals(newRoot),
      "cannot derive newRoot from compromised root alone",
    );
    assert.ok(
      !guessedFromCompromised.equals(secondRoot),
      "cannot derive secondRoot from compromised root alone",
    );
  });
});
...
describe("Queue bound: MAX_QUEUE_BYTES overflow protection", () => {
  it("send() throws QUEUE_FULL when writeQueue exceeds MAX_QUEUE_BYTES", () => {
    ...
    assert.strictEqual(
      session.state,
      hk.REKEY_STATES.IDLE,
      "state should reset to IDLE after queue overflow",
    );
  });
  ...
  it("MAX_QUEUE_BYTES is exported and is a positive number", () => {
    ...
    assert.ok(
      hk.MAX_QUEUE_BYTES >= 1024,
      "MAX_QUEUE_BYTES should be at least 1KB",
    );
  });
});
