...
describe('SiemSecurityBroker (unit)', () => {
  ...
  describe('rate limiting (token bucket)', () => {
    ...
    it('emits telemetry_dropped event when rate limited', () => {
      ...
      assert.strictEqual(droppedEvents.length, 5, 'expected 5 telemetry_dropped events');
  ...
  describe('transport strategies', () => {
    ...
    it('STREAMING emits transport_winston_stream for all severities', () => {
      ...
      broker.logEvent({ siemSeverity: 'CRITICAL', siemCategory: 'CRITICAL_EVENT' });
    ...
    it('STDOUT_ONLY does not emit transport events', () => {
      ...
      let emitted = false;
      broker.on('transport_batch_queue', () => { emitted = true; });
      broker.on('transport_winston_stream', () => { emitted = true; });

      broker.logEvent({ siemSeverity: 'LOW', siemCategory: 'LOW_EVENT' });

      assert.strictEqual(emitted, false, 'STDOUT_ONLY should not emit transport events');
    });
  });
  ...
  describe('event normalization', () => {
    it('produces immutable event with eventId and timestamp', () => {
      let captured;
      broker.on('transport_batch_queue', (event) => { captured = event; });
  ...
  describe('metrics', () => {
    ...
    it('includes currentTokens in metrics snapshot', () => {
      broker.logEvent({ siemSeverity: 'LOW', siemCategory: 'A' });
      const m = broker.getMetrics();
      assert.strictEqual(m.currentTokens, 4, 'should have 4 tokens remaining after 1 consumption');
    });
  });
