...
const MANIFEST = path.join(ROOT, 'coming-soon', 'public', 'models', 'manifest.json');
...
describe('Dockerfile.enterprise', () => {
  it('exists', () => {
    assert.ok(fs.existsSync(DOCKERFILE_ENGINE), 'Dockerfile.enterprise should exist');
  });

  it('uses multi-stage build', () => {
    const content = readFile(DOCKERFILE_ENGINE);
    const stages = content.match(/^FROM\s+\S+\s+AS\s+\S+/gm) || [];
    assert.ok(stages.length >= 3, `should have at least 3 stages, got ${stages.length}`);
  });
  ...
  it('builds dashboard assets', () => {
    const content = readFile(DOCKERFILE_ENGINE);
    assert.ok(content.includes('vite build'), 'should run vite build for dashboard assets');
  ...
  it('has health check', () => {
    const content = readFile(DOCKERFILE_ENGINE);
    assert.ok(content.includes('HEALTHCHECK'), 'should have a HEALTHCHECK directive');
  });

  it('uses tini for signal handling', () => {
    const content = readFile(DOCKERFILE_ENGINE);
    assert.ok(content.includes('tini'), 'should use tini for PID 1 signal handling');
  });
  ...
  it('starts simplebeacon-server.cjs', () => {
    const content = readFile(DOCKERFILE_ENGINE);
    assert.ok(content.includes('simplebeacon-server.cjs'), 'should start the SimpleBeacon server');
  });
...
describe('Dockerfile.ollama', () => {
  it('exists', () => {
    assert.ok(fs.existsSync(DOCKERFILE_OLLAMA), 'Dockerfile.ollama should exist');
  });

  it('uses ollama/ollama base image', () => {
    const content = readFile(DOCKERFILE_OLLAMA);
    assert.ok(content.includes('ollama/ollama'), 'should use ollama/ollama base image');
  });

  it('copies all Modelfiles', () => {
    const content = readFile(DOCKERFILE_OLLAMA);
    assert.ok(content.includes('Modelfile'), 'should copy Modelfile');
    assert.ok(content.includes('Modelfile.llama32'), 'should copy Modelfile.llama32');
    assert.ok(content.includes('Modelfile.mistral'), 'should copy Modelfile.mistral');
    assert.ok(content.includes('Modelfile.qwen25-coder'), 'should copy Modelfile.qwen25-coder');
  });
  ...
  it('has model initialization script', () => {
    const content = readFile(DOCKERFILE_OLLAMA);
    assert.ok(content.includes('init-models.sh'), 'should have model initialization script');
    assert.ok(content.includes('ollama create'), 'should create models from Modelfiles');
  });
...
describe('docker-compose.enterprise.yml', () => {
  it('exists and is valid YAML', () => {
    assert.ok(fs.existsSync(COMPOSE_FILE), 'docker-compose.enterprise.yml should exist');
  ...
  it('has three services', () => {
    ...
    assert.ok(services.includes('simplebeacon-engine'), 'should have simplebeacon-engine service');
    assert.ok(services.includes('simplebeacon-ollama'), 'should have simplebeacon-ollama service');
    assert.ok(services.includes('simplebeacon-db'), 'should have simplebeacon-db service');
  });
  ...
  it('engine depends on db and ollama', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const engine = compose.services['simplebeacon-engine'];
    assert.ok(engine.depends_on?.['simplebeacon-db'], 'engine should depend on db');
    assert.ok(engine.depends_on?.['simplebeacon-ollama'], 'engine should depend on ollama');
  });

  it('engine has correct port mapping', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const engine = compose.services['simplebeacon-engine'];
    assert.ok(engine.ports?.some(p => p.includes('3000')), 'engine should map port 3000');
  });

  it('ollama has correct port mapping', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const ollama = compose.services['simplebeacon-ollama'];
    assert.ok(ollama.ports?.some(p => p.includes('11434')), 'ollama should map port 11434');
  });

  it('db has correct port mapping', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const db = compose.services['simplebeacon-db'];
    assert.ok(db.ports?.some(p => p.includes('5432')), 'db should map port 5432');
  });

  it('has named volumes for persistence', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const volumes = Object.keys(compose.volumes || {});
    assert.ok(volumes.includes('engine-data'), 'should have engine-data volume');
    assert.ok(volumes.includes('ollama-models'), 'should have ollama-models volume');
    assert.ok(volumes.includes('db-data'), 'should have db-data volume');
  });

  it('has a bridge network', () => {
    const compose = loadYaml(COMPOSE_FILE);
    assert.ok(compose.networks?.['simplebeacon-net'], 'should have simplebeacon-net network');
  ...
  it('engine has OLLAMA_NUM_CTX env var', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const engine = compose.services['simplebeacon-engine'];
    assert.ok(engine.environment?.OLLAMA_NUM_CTX, 'engine should have OLLAMA_NUM_CTX env var');
  });

  it('engine has OLLAMA_BASE_URL pointing to ollama service', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const engine = compose.services['simplebeacon-engine'];
    assert.ok(engine.environment?.OLLAMA_BASE_URL?.includes('simplebeacon-ollama'),
  ...
  it('engine has DATABASE_URL (via env_file pointing to db service)', () => {
    const compose = loadYaml(COMPOSE_FILE);
    const engine = compose.services['simplebeacon-engine'];
    assert.ok(engine.env_file, 'engine should use env_file for DATABASE_URL and secrets');
    ...
    assert.ok(envTemplate.includes('simplebeacon-db'), '.env.enterprise.example should have DATABASE_URL pointing to simplebeacon-db');
  });
  ...
  it('all services use restart: unless-stopped', () => {
    ...
    for (const [name, service] of Object.entries(compose.services)) {
      assert.strictEqual(service.restart, 'unless-stopped', `${name} should have restart: unless-stopped`);
    }
  });
});
...
describe('docker-compose.enterprise.gpu.yml', () => {
  ...
  it('overrides ollama service with GPU resources', () => {
    ...
    assert.ok(ollama.deploy?.resources?.reservations?.devices, 'should have GPU device reservations');
    const devices = ollama.deploy.resources.reservations.devices;
    assert.ok(devices.some(d => d.driver === 'nvidia'), 'should specify NVIDIA driver');
    assert.ok(devices.some(d => d.capabilities?.includes('gpu')), 'should request GPU capability');
  });
});
...
describe('.env.enterprise.example', () => {
  it('exists', () => {
    assert.ok(fs.existsSync(ENV_TEMPLATE), '.env.enterprise.example should exist');
  });

  it('includes all required security vars', () => {
    const content = readFile(ENV_TEMPLATE);
    for (const key of ['JWT_SECRET', 'SIMPLEBEACON_LICENSE_SECRET', 'DASHBOARD_VAULT_PASSWORD', 'REPORT_SIGNING_KEY']) {
  ...
  it('includes OLLAMA vars', () => {
    ...
    assert.ok(content.includes('OLLAMA_NUM_CTX'), 'should include OLLAMA_NUM_CTX');
  });

  it('includes POSTGRES_PASSWORD', () => {
    const content = readFile(ENV_TEMPLATE);
    assert.ok(content.includes('POSTGRES_PASSWORD'), 'should include POSTGRES_PASSWORD');
  });
  ...
  it('includes email and Stripe sections', () => {
    const content = readFile(ENV_TEMPLATE);
    assert.ok(content.includes('RESEND_API_KEY'), 'should include email config');
    assert.ok(content.includes('STRIPE_SECRET_KEY'), 'should include Stripe config');
  });
  ...
  it('has empty values for secrets (not pre-filled)', () => {
    ...
    assert.strictEqual(jwtMatch[1].trim(), '', 'JWT_SECRET should be empty in the template');
  });
});
...
describe('hydrate-airgap.sh', () => {
  ...
  it('has deploy command', () => {
    ...
    assert.ok(content.includes('docker load'), 'should load images from archive');
    assert.ok(content.includes('docker compose'), 'should start the stack with docker compose');
  });
  ...
  it('checks prerequisites', () => {
    const content = readFile(HYDRATE_SCRIPT);
    assert.ok(content.includes('check_prerequisites'), 'should check for docker and docker compose');
  });
  ...
  it('pulls base models during packaging', () => {
    const content = readFile(HYDRATE_SCRIPT);
    assert.ok(content.includes('ollama pull'), 'should pull base models during packaging');
...
describe('model manifest', () => {
  it('manifest.json exists and is valid JSON', () => {
    ...
    assert.doesNotThrow(() => JSON.parse(readFile(MANIFEST)), 'should be valid JSON');
  });
  ...
  it('has all models referenced in Dockerfiles', () => {
    ...
    for (const modelId of Object.keys(manifest.models)) {
      ...
      assert.ok(ollamaDockerfile.includes(modelfileName),
