...
jest.mock("../../lib/hsm-vault.cjs", () => ({
  hsmHandshake: jest
    .fn()
    .mockResolvedValue({ status: "ok", provider: "mockhsm" }),
...
describe("E-04: hsm-vault-routes authBeforeThrottle ordering", () => {
  ...
  it("non-admin user gets 403 (auth rejection), NOT 429 (throttle)", async () => {
    ...
    assert.strictEqual(
      throttleCallCount,
      0,
      "adminThrottle was called even though auth failed — ordering bug!",
    );
  });
  ...
  it("admin user with throttle allowing traffic reaches the route handler", async () => {
    ...
    assert.ok(
      throttleCallCount >= 1,
      "adminThrottle should have been called for admin user",
    );
  });
  ...
  it("authorize runs before adminThrottle (call order verification)", async () => {
    ...
    assert.strictEqual(
      throttleCallCount,
      0,
      "throttle ran before auth rejected — D-02 regression!",
    );
  });
  ...
  it("non-admin user gets 403 on all vault routes (not 429)", async () => {
    ...
    for (const route of routes) {
      ...
      assert.strictEqual(
        res.status,
        403,
        `${route.method.toUpperCase()} ${route.path} should return 403 for non-admin`,
      );
      assert.strictEqual(
        throttleCallCount,
        0,
        `throttle was called for non-admin on ${route.path}`,
      );
    }
  });
});
