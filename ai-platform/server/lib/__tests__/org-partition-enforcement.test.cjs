...
const _tempSettingsPath = path.join(
  os.tmpdir(),
  "sb-partition-test-settings.json",
);
  ...
  afterEach(() => {
    ...
    try {
      delete require.cache[
        require.resolve("../../lib/security-monitor-settings-store.cjs")
      ];
...
describe("getPartitionStats", () => {
  ...
  it("should limit recentViolations to 10 entries", () => {
    ...
    const stats = getPartitionStats();
    assert.ok(
      stats.recentViolations.length <= 10,
      "should cap at 10 recent violations",
    );
  });
});
