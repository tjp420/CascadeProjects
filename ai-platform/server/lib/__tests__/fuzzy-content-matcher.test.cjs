...
describe('fuzzy-content-matcher', () => {
  ...
  test('tokenJaccardSimilarity returns 1 for identical strings', () => {
    expect(tokenJaccardSimilarity('hello world foo', 'hello world foo')).toBe(1);
  });
  ...
  test('tokenJaccardSimilarity returns partial for similar strings', () => {
    const score = tokenJaccardSimilarity('hello world foo bar', 'hello world baz bar');
  ...
  test('isCiLogFragmentPair returns true when both are CI logs', () => {
    expect(isCiLogFragmentPair('docs/1-stdout.txt', 'docs/2-stderr.txt')).toBe(true);
    expect(isCiLogFragmentPair('docs/1-stdout.txt', 'src/index.js')).toBe(false);
  });
});
