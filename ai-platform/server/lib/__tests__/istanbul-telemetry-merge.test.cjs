...
describe('istanbul-telemetry-merge', () => {
  ...
  test('isOpenIstanbulAlert returns true for open istanbul alert', () => {
    expect(isOpenIstanbulAlert({ title: 'Istanbul coverage', resolved: false })).toBe(true);
  });

  test('isOpenIstanbulAlert returns false for resolved istanbul alert', () => {
    expect(isOpenIstanbulAlert({ title: 'Istanbul coverage', resolved: true })).toBe(false);
    expect(isOpenIstanbulAlert({ title: 'Istanbul coverage', status: 'resolved' })).toBe(false);
  });
