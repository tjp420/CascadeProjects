...
describe("stripe-event-store", () => {
  ...
  it("persists events across cache clears", async () => {
    ...
    assert.strictEqual(
      result,
      false,
      "event should still be duplicate after cache clear",
    );
  });
  ...
  it("persists events to disk file", async () => {
    ...
    assert.ok(
      raw.eventIds.includes("evt_test_004"),
      "should contain evt_test_004",
    );
    assert.ok(
      raw.eventIds.includes("evt_test_005"),
      "should contain evt_test_005",
    );
