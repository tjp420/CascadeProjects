...
describe('audit-report/executive', () => {
  ...
  test('calculateAuditConfidence returns 100 for clean summary', () => {
    const score = calculateAuditConfidence({ gatePass: true, ruleScopedFiles: 100, codebaseHealth: 90, codeFilesAnalyzed: 50 });
    expect(score).toBeGreaterThan(0);
    expect(score).toBeLessThanOrEqual(100);
  });

  test('calculateAuditConfidence penalizes zero ruleScopedFiles', () => {
    const score = calculateAuditConfidence({ ruleScopedFiles: 0, gatePass: true });
    expect(score).toBeLessThan(100);
  });

  test('calculateAuditConfidence penalizes null gatePass', () => {
    const score = calculateAuditConfidence({ ruleScopedFiles: 100, gatePass: null });
    expect(score).toBeLessThan(100);
  });

  test('calculateAuditConfidence penalizes low codebaseHealth', () => {
    const score = calculateAuditConfidence({ ruleScopedFiles: 100, gatePass: true, codebaseHealth: 20 });
    expect(score).toBeLessThan(80);
  });

  test('buildExecutivePriorities returns array of strings', () => {
    const priorities = buildExecutivePriorities({ gatePass: false, severityCounts: { high: 1 }, productionFindings: 1 });
  ...
  test('buildExecutivePriorities handles gate pass with no findings', () => {
    const priorities = buildExecutivePriorities({ gatePass: true, productionFindings: 0, codeFilesAnalyzed: 100 });
    expect(Array.isArray(priorities)).toBe(true);
    expect(priorities[0]).toContain('Zero production-path');
  });

  test('buildLaunchReadiness returns blocked for gate fail', () => {
    const readiness = buildLaunchReadiness({ summary: { gatePass: false, severityCounts: { high: 2 } } });
  ...
  test('buildLaunchReadiness returns ready for gate pass', () => {
    const readiness = buildLaunchReadiness({ summary: { gatePass: true, productionFindings: 0, severityCounts: { high: 0 }, codebaseHealth: 90 } });
  ...
  test('buildLaunchReadiness returns conditional for production findings', () => {
    const readiness = buildLaunchReadiness({ summary: { gatePass: null, productionFindings: 5, severityCounts: { high: 0 }, codebaseHealth: 70 } });
    expect(readiness.tone).toBe('conditional');
  });

  test('buildLaunchReadiness returns review required for unknown state', () => {
    const readiness = buildLaunchReadiness({ summary: { severityCounts: { high: 0 }, productionFindings: 0 } });
    expect(readiness.tone).toBe('conditional');
  ...
  test('buildCodebaseActionPlan returns string for model with findings', () => {
    const plan = buildCodebaseActionPlan({
      summary: { productionFindings: 3, documentationFindings: 2, codebaseFindingsDeduped: 5 },
      priorityFindings: [
        { tier: 'production', severity: 'high', category: 'cred', filePath: 'server/lib/x.cjs', line: 10, recommendedAction: 'Fix it' }
      ]
    });
  ...
  test('buildCodebaseActionPlan returns no-backlog message for clean model', () => {
    const plan = buildCodebaseActionPlan({
      summary: { productionFindings: 0, documentationFindings: 0, codebaseFindingsDeduped: 0 },
      priorityFindings: []
    });
  ...
  test('buildDeterministicExecutive produces a model object', () => {
    const model = {
      summary: { gatePass: true, productionFindings: 0, codeFilesAnalyzed: 50, severityCounts: { high: 0, medium: 0, low: 5 }, codebaseHealth: 85, documentationFindings: 2 },
      priorityFindings: [],
      projectPath: '/test',
  ...
  test('buildCompleteAuditPrompt returns a string', () => {
    const model = {
      summary: { gatePass: true, productionFindings: 0, codeFilesAnalyzed: 50, severityCounts: { high: 0, medium: 0, low: 5 }, codeSeverity: { high: 0, medium: 0, low: 0 }, codebaseHealth: 85, documentationFindings: 2 },
      priorityFindings: [],
      projectPath: '/test',
      readiness: buildLaunchReadiness({ summary: { gatePass: true, productionFindings: 0, severityCounts: { high: 0 }, codebaseHealth: 85 } })
    };
  ...
  test('mergeExecutiveSummary merges AI and deterministic content', () => {
    const deterministic = { summary: 'Det summary', priorities: ['p1'], readiness: { label: 'Ready', tone: 'ready', score: 90 } };
