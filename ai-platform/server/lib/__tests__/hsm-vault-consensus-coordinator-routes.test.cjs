...
describe('Track 40: Consensus Coordinator Routes', () => {
  ...
  describe('POST /consensus/groups', () => {
    ...
    test('creates a group with topic and keyRange', async () => {
      ...
      const res = await request(app)
        .post('/api/vault/consensus/groups')
        .send({ groupId: 'g1', clusterNodes: ['node-A', 'node-B'], topic: 'east', keyRange: { start: 'a', end: 'm' } });
      expect(res.status).toBe(201);
    ...
    test('returns 400 for missing groupId', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/groups').send({ clusterNodes: ['node-A'] });
      expect(res.status).toBe(400);
    });

    test('returns 400 for missing clusterNodes', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/groups').send({ groupId: 'g1' });
      expect(res.status).toBe(400);
    });
    ...
    test('returns 409 for duplicate groupId', async () => {
      coordinator.createGroup({ groupId: 'g1', clusterNodes: ['node-A', 'node-B'] });
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/groups').send({ groupId: 'g1', clusterNodes: ['node-A', 'node-B'] });
      expect(res.status).toBe(409);
    });

    test('returns 403 for non-admin', async () => {
      const app = buildApp(VIEWER);
      const res = await request(app).post('/api/vault/consensus/groups').send({ groupId: 'g1', clusterNodes: ['node-A'] });
      expect(res.status).toBe(403);
    });
  });
  ...
  describe('POST /consensus/proposals', () => {
    beforeEach(() => {
      coordinator.createGroup({ groupId: 'g1', clusterNodes: ['node-A', 'node-B', 'node-C'], topic: 'east' });
    });

    test('routes by groupId and returns 200', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/proposals').send({ groupId: 'g1', command: { type: 'put' } });
      expect(res.status).toBe(200);
    ...
    test('routes by topic and returns 200', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/proposals').send({ topic: 'east', command: {} });
      expect(res.status).toBe(200);
      expect(res.body.accepted).toBe(true);
    });
    ...
    test('returns 400 when no routing key', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/proposals').send({ command: {} });
      expect(res.status).toBe(400);
    });

    test('returns 404 for non-existent group', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/proposals').send({ groupId: 'nonexistent', command: {} });
      expect(res.status).toBe(404);
    });

    test('returns 403 for non-admin', async () => {
      const app = buildApp(VIEWER);
      const res = await request(app).post('/api/vault/consensus/proposals').send({ groupId: 'g1', command: {} });
      expect(res.status).toBe(403);
    });
  });
  ...
  describe('POST /consensus/heartbeat', () => {
    ...
    test('records heartbeat and returns 200', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/heartbeat').send({ nodeId: 'node-B', groupId: 'g1', leaderId: 'node-A' });
      expect(res.status).toBe(200);
    ...
    test('returns 400 for missing params', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/heartbeat').send({ nodeId: 'node-B' });
      expect(res.status).toBe(400);
    });

    test('returns 404 for unknown group', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/heartbeat').send({ nodeId: 'node-B', groupId: 'nonexistent' });
      expect(res.status).toBe(404);
    });

    test('returns 403 for non-admin', async () => {
      const app = buildApp(VIEWER);
      const res = await request(app).post('/api/vault/consensus/heartbeat').send({ nodeId: 'node-B', groupId: 'g1' });
      expect(res.status).toBe(403);
    });
  });
  ...
  describe('POST /consensus/view-change', () => {
    ...
    test('initiates view change and returns 200', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/view-change').send({ groupId: 'g1', failedLeaderId: 'node-B', candidateId: 'node-A' });
      expect(res.status).toBe(200);
      expect(res.body.accepted).toBe(true);
    });
    ...
    test('returns 400 for missing params', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/view-change').send({ groupId: 'g1', failedLeaderId: 'node-B' });
      expect(res.status).toBe(400);
    });

    test('returns 404 for unknown group', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/view-change').send({ groupId: 'nonexistent', failedLeaderId: 'node-B', candidateId: 'node-A' });
      expect(res.status).toBe(404);
    });
    ...
    test('returns 409 when already in progress', async () => {
      ...
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/view-change').send({ groupId: 'g1', failedLeaderId: 'node-B', candidateId: 'node-C' });
      expect(res.status).toBe(409);
    });
  ...
  describe('POST /consensus/view-change/vote', () => {
    ...
    test('casts a vote and returns 200', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/view-change/vote').send({ groupId: 'g1', voterId: 'node-B', candidateId: 'node-A' });
      expect(res.status).toBe(200);
      expect(res.body.accepted).toBe(true);
    });
    ...
    test('completes view change when quorum reached', async () => {
      const app = buildApp(ADMIN);
      await request(app).post('/api/vault/consensus/view-change/vote').send({ groupId: 'g1', voterId: 'node-B', candidateId: 'node-A' });
      const res = await request(app).post('/api/vault/consensus/view-change/vote').send({ groupId: 'g1', voterId: 'node-C', candidateId: 'node-A' });
      expect(res.status).toBe(200);
    ...
    test('returns 400 for missing params', async () => {
      const app = buildApp(ADMIN);
      const res = await request(app).post('/api/vault/consensus/view-change/vote').send({ groupId: 'g1', voterId: 'node-B' });
      expect(res.status).toBe(400);
    });
