...
describe('Track 31 lookup-gating vault routes', () => {
  ...
  test('full round-trip: submit, validate, accredit', async () => {
    const create = await request(app).post('/api/vault/lookup-gating/pool').send({ policy: {} });
  ...
  test('GET /api/vault/lookup-gating/telemetry exposes counters', async () => {
    const res = await request(app)
      ...
      .expect(200);
    expect(res.body.telemetry).toHaveProperty('hsm_lookupgate_pool_initialized_total');
    expect(res.body.telemetry).toHaveProperty('hsm_zk_lookup_claim_verified_total');
    expect(res.body.telemetry).toHaveProperty('hsm_lookup_accreditation_completed_total');
  });
