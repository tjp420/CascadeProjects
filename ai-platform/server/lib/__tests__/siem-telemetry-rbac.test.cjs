...
describe('RBAC on SIEM Telemetry Endpoint', () => {
  ...
  const authorizePath = path.join(__dirname, '..', '..', 'middleware', 'authorize.cjs');
  const rbacStorePath = path.join(__dirname, '..', '..', 'lib', 'rbac-store.cjs');
  ...
  test('R1: admin role receives 200 with telemetry data', async () => {
    setupMocks('admin', ['read:all', 'write:all', 'delete:all', 'admin:all', 'read:siem_telemetry']);
  ...
  test('R2: auditor role receives 200 with telemetry data', async () => {
    setupMocks('auditor', ['read:all', 'export:audit', 'read:audit', 'read:siem_telemetry']);
    const express = require('express');
    const request = require('supertest');
    const router = require(path.join(__dirname, '..', '..', 'routes', 'analytics-routes.cjs'));
  ...
  test('R3: operator role receives 403 forbidden', async () => {
    setupMocks('operator', ['read:all', 'write:tickets', 'write:scans', 'write:evals', 'trigger:gates']);
    const express = require('express');
    const request = require('supertest');
    const router = require(path.join(__dirname, '..', '..', 'routes', 'analytics-routes.cjs'));
    ...
    const res = await request(app).get('/api/analytics/siem-telemetry').expect(403);
  ...
  test('R5: unauthenticated request receives 401', async () => {
    ...
    jest.mock(rbacStorePath, () => ({
      resolveUserRole: () => ({ role: 'unknown', permissions: [], source: 'default' }),
      hasPermission: (perms, p) => perms.includes(p),
    }));
    jest.resetModules();
    const express = require('express');
    ...
    const res = await request(app).get('/api/analytics/siem-telemetry').expect(401);
