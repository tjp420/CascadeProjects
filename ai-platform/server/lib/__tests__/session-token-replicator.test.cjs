...
describe('session-token-replicator', () => {
  ...
  it('rejects cross-tenant SESSION_TOKEN_ISSUE', () => {
    ...
    assert.rejects(async () => replicator.handleSessionTokenMessage(msg, socket), /CROSS_TENANT_SESSION_INJECTION_REJECTED/);
  });
  ...
  it('processes SESSION_STATE_RESPONSE batch and records rejected items', async () => {
    const socket = makeSocket('tenant-1');
    const tokens = [
      { token_hash: 'resp-1', account_id: 'acc-1', tenant_id: 'tenant-1', token_sequence: 1, epoch: 1, expires_at: new Date(Date.now() + 3600000).toISOString() },
      { token_hash: 'resp-2', account_id: 'acc-1', tenant_id: 'tenant-1', token_sequence: 0, epoch: 0 }, // missing expires_at but not required by sync
    ];
    const msg = {
