...
process.env.KEY_ROTATION_STORE_PATH = path.join(os.tmpdir(), 'sb-load-profile-key-state.json');
process.env.AUDIT_LOG_PATH = path.join(os.tmpdir(), 'sb-load-profile-audit.json');
...
describe('Cluster Keyring Sync Load Profile Simulation', () => {
  ...
  describe('Scenario 1: Concurrent DKG Session Initiation', () => {
    test('LOAD-01: Single session initiation succeeds under no contention', () => {
      const status = clusterSync.initDkgSession({ dkgEngine, nodeId: 'node-1' });
    ...
    test('LOAD-02: Concurrent initiation — second call rejects (single-session guard)', () => {
      ...
      expect(() => clusterSync.initDkgSession({ dkgEngine, nodeId: 'node-1' }))
  ...
  describe('Scenario 2: DKG Message Flood', () => {
    test('LOAD-04: 1000 valid DKG_COMMIT messages from unique peers', () => {
      ...
      const dupEvents = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
    ...
    test('LOAD-05: 1000 malformed DKG messages — all rejected, no crash', () => {
      ...
      const invalidEvents = clusterSync.queryEvents({ eventType: 'dkg_invalid_message' });
    ...
    test('LOAD-06: DKG_COMMIT flood from same peer (duplicate detection)', () => {
      ...
      clusterSync._handleDkgMessage(firstMsg, socketPool.create('127.0.0.1', 7001));
      const commitEvents1 = clusterSync.queryEvents({ eventType: 'dkg_commit_received' });
      ...
      const commitEventsFinal = clusterSync.queryEvents({ eventType: 'dkg_commit_received' });
  ...
  describe('Scenario 3: Event Timeline Saturation', () => {
    ...
    test('LOAD-10: getEventStats after saturation — verify counts sum to MAX_EVENTS', () => {
      ...
      const typeEvents = clusterSync.queryEvents({ eventType: 'uniform_event' });
  ...
  describe('Scenario 5: Peer Connection Churn', () => {
    ...
    test('LOAD-14: Heartbeat broadcast to 50 peers — verify all receive write', () => {
      ...
      const heartbeatMsg = JSON.stringify({ type: 'HEARTBEAT', from: 'node-1', ts: Date.now() });
