...
const RENDER_YAML = path.resolve(
  __dirname,
  "..",
  "..",
  "..",
...
describe("render.yaml structure", () => {
  ...
  it("has envVars defined on the web service", () => {
    ...
    assert.ok(
      yaml.services[0].envVars.length >= 30,
      "should have at least 30 env vars",
    );
  });
});
...
describe("LLM / Ollama env vars", () => {
  ...
  it("OLLAMA_BASE_URL is present", () => {
    assert.ok(
      keys.includes("OLLAMA_BASE_URL"),
      "OLLAMA_BASE_URL must be in render.yaml",
    );
  });

  it("OLLAMA_MODEL is present", () => {
    assert.ok(
      keys.includes("OLLAMA_MODEL"),
      "OLLAMA_MODEL must be in render.yaml",
    );
  });

  it("OLLAMA_NUM_CTX is present (critical fix)", () => {
    assert.ok(
      keys.includes("OLLAMA_NUM_CTX"),
      "OLLAMA_NUM_CTX must be in render.yaml — without it Ollama defaults to 2048 tokens",
    );
  });

  it("OLLAMA_NUM_CTX value is a valid context window size", () => {
    const yaml = loadRenderYaml();
    const envVar = yaml.services[0].envVars.find(
      (e) => e.key === "OLLAMA_NUM_CTX",
    );
    const val = parseInt(envVar?.value, 10);
    assert.ok(
      [2048, 4096, 8192, 16384, 32768, 65536].includes(val),
  ...
  it("OLLAMA_TIMEOUT_MS is present", () => {
    assert.ok(
      keys.includes("OLLAMA_TIMEOUT_MS"),
      "OLLAMA_TIMEOUT_MS should be in render.yaml",
    );
  });

  it("SIMPLEBEACON_OFFLINE is present", () => {
    assert.ok(
      keys.includes("SIMPLEBEACON_OFFLINE"),
      "SIMPLEBEACON_OFFLINE should be in render.yaml",
    );
  });
});
...
describe("Report signing env vars", () => {
  ...
  it("REPORT_SIGNING_KEY is present", () => {
    assert.ok(
      keys.includes("REPORT_SIGNING_KEY"),
      "REPORT_SIGNING_KEY must be in render.yaml for tamper-evident evidence packs",
    );
  });
  ...
  it("REPORT_SIGNING_KEY is sync:false (manually set secret)", () => {
    const yaml = loadRenderYaml();
    const envVar = yaml.services[0].envVars.find(
      (e) => e.key === "REPORT_SIGNING_KEY",
    );
    assert.strictEqual(
      envVar?.sync,
      false,
      "REPORT_SIGNING_KEY should be sync:false (set manually in Render dashboard)",
    );
  });

  it("REPORT_SIGNING_KEY_ID is present", () => {
    assert.ok(
      keys.includes("REPORT_SIGNING_KEY_ID"),
      "REPORT_SIGNING_KEY_ID should be in render.yaml",
    );
  });
});
...
describe("Dashboard vault env vars", () => {
  ...
  it("DASHBOARD_VAULT_PASSWORD is present", () => {
    assert.ok(
      keys.includes("DASHBOARD_VAULT_PASSWORD"),
      "DASHBOARD_VAULT_PASSWORD must be in render.yaml for internal dashboard access",
    );
  });
  ...
  it("DASHBOARD_VAULT_PASSWORD is sync:false (manually set secret)", () => {
    const yaml = loadRenderYaml();
    const envVar = yaml.services[0].envVars.find(
      (e) => e.key === "DASHBOARD_VAULT_PASSWORD",
    );
    assert.strictEqual(
      envVar?.sync,
      false,
      "DASHBOARD_VAULT_PASSWORD should be sync:false",
    );
  });
});
...
describe("Stripe Checkout Links", () => {
  ...
  it("STRIPE_LINK_INSTANT is present", () => {
    assert.ok(
      keys.includes("STRIPE_LINK_INSTANT"),
      "STRIPE_LINK_INSTANT should be in render.yaml",
    );
  });

  it("STRIPE_LINK_EXECUTIVE is present", () => {
    assert.ok(
      keys.includes("STRIPE_LINK_EXECUTIVE"),
      "STRIPE_LINK_EXECUTIVE should be in render.yaml",
    );
  });

  it("STRIPE_LINK_EU_SPRINT is present", () => {
    assert.ok(
      keys.includes("STRIPE_LINK_EU_SPRINT"),
      "STRIPE_LINK_EU_SPRINT should be in render.yaml",
    );
  });
});
...
describe("Security-critical env vars", () => {
  ...
  it("SEED_DEMO_USERS is false", () => {
    ...
    assert.strictEqual(
      v?.value,
      "false",
      "SEED_DEMO_USERS must be false in production",
    );
  });
  ...
  it("ALLOW_LEGACY_LOGIN is false", () => {
    ...
    assert.strictEqual(
      v?.value,
      "false",
      "ALLOW_LEGACY_LOGIN must be false in production",
    );
  });

  it("DEV_AUTH_BYPASS is NOT present", () => {
    assert.ok(
      !keys.includes("DEV_AUTH_BYPASS"),
      "DEV_AUTH_BYPASS must NOT be in render.yaml — it enables auth bypass",
    );
  });
  ...
  it("SIMPLEBEACON_ENABLE_DEMO_REPORT is false", () => {
    ...
    assert.strictEqual(
      v?.value,
      "false",
      "SIMPLEBEACON_ENABLE_DEMO_REPORT must be false in production",
    );
  });
  ...
  it("ENABLE_EXTERNAL_APIS is false", () => {
    ...
    assert.strictEqual(
      v?.value,
      "false",
      "ENABLE_EXTERNAL_APIS should be false in production",
    );
  });
});
...
describe("Core infrastructure env vars", () => {
  ...
  it("DATABASE_URL is present (from database)", () => {
    ...
    assert.ok(
      v?.fromDatabase,
      "DATABASE_URL should be sourced from the database resource",
    );
  });
  ...
  it("SIMPLEBEACON_LICENSE_SECRET is present (generated)", () => {
    const yaml = loadRenderYaml();
    const v = yaml.services[0].envVars.find(
      (e) => e.key === "SIMPLEBEACON_LICENSE_SECRET",
    );
    assert.strictEqual(
      v?.generateValue,
      true,
      "SIMPLEBEACON_LICENSE_SECRET should be auto-generated",
    );
  });
  ...
  it("STRIPE_SECRET_KEY is present (sync:false)", () => {
    ...
    assert.strictEqual(
      v?.sync,
      false,
      "STRIPE_SECRET_KEY should be sync:false",
    );
  });
  ...
  it("STRIPE_WEBHOOK_SECRET is present (sync:false)", () => {
    const yaml = loadRenderYaml();
    const v = yaml.services[0].envVars.find(
      (e) => e.key === "STRIPE_WEBHOOK_SECRET",
    );
    assert.strictEqual(
      v?.sync,
      false,
      "STRIPE_WEBHOOK_SECRET should be sync:false",
    );
  });
});
...
describe("PRODUCTION_ENV_VARS.md documentation", () => {
  ...
  it("documents REPORT_SIGNING_KEY", () => {
    const content = fs.readFileSync(ENV_SPEC, "utf8");
    assert.ok(
      content.includes("REPORT_SIGNING_KEY"),
      "should document REPORT_SIGNING_KEY",
    );
  });
  ...
  it("documents DASHBOARD_VAULT_PASSWORD", () => {
    const content = fs.readFileSync(ENV_SPEC, "utf8");
    assert.ok(
      content.includes("DASHBOARD_VAULT_PASSWORD"),
      "should document DASHBOARD_VAULT_PASSWORD",
    );
  });
  ...
  it("includes Render dashboard setup instructions", () => {
    const content = fs.readFileSync(ENV_SPEC, "utf8");
    assert.ok(
      content.includes("Render Dashboard Setup"),
  ...
  it("includes security checklist", () => {
    ...
    assert.ok(
      ...
      "should include security checklist",
    );
  });
  ...
  it("includes render.yaml patch section", () => {
    const content = fs.readFileSync(ENV_SPEC, "utf8");
    assert.ok(
      content.includes("render.yaml Patch"),
      "should include render.yaml patch section",
    );
  });
});
