...
const { zeroizeBuffer, zeroizeString, withZeroizedBuffer } = require('../crypto/zeroize.cjs');
...
describe('zeroize utility', () => {
  ...
  describe('zeroizeBuffer', () => {
    ...
    test('Z8: zeroizes hex-encoded cryptographic buffer', () => {
      const buf = Buffer.from('a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2', 'hex');
  ...
  describe('withZeroizedBuffer', () => {
    ...
    test('Z19: throws TypeError when fn is not a function', () => {
      expect(() => withZeroizedBuffer('test', 'not-a-function')).toThrow(TypeError);
    ...
    test('Z21: supports custom encoding for string conversion', () => {
      const result = withZeroizedBuffer('6b6579', (buf) => {
