...
describe("email-service smoke", () => {
  ...
  it("exports expected functions", () => {
    assert.strictEqual(
      typeof mod.sendEmail,
      "function",
      "sendEmail should be exported",
    );
  });
...
describe("email-service fallback chain", () => {
  ...
  it("creates queue directory if it does not exist", async () => {
    ...
    assert.ok(
      ...
      "queue dir should not exist before send",
    );
  ...
  it("queues multiple emails with unique IDs", async () => {
    ...
    assert.strictEqual(
      new Set(paths).size,
      3,
      "all queue paths should be unique",
    );
...
describe("email-service getResendConfig validation", () => {
  ...
  it("uses default from address when RESEND_FROM not set", () => {
    ...
    assert.ok(cfg);
    assert.ok(
      cfg.from.includes("simplebeacon.ai"),
      "default from should contain domain",
    );
    delete process.env.RESEND_API_KEY;
  });
});
