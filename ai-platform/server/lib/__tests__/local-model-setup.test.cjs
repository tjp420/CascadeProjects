...
const SETUP_SCRIPT = path.resolve(__dirname, '..', '..', '..', 'scripts', 'setup-local-model.cjs');
const AUTO_PROCESSOR = path.resolve(__dirname, '..', '..', '..', 'auto-processor.js');
const OLLAMA_CLIENT = path.resolve(__dirname, '..', '..', 'services', 'ollama-client.cjs');
const CHUNKER_PATH = path.resolve(__dirname, '..', 'context-window-chunker.cjs');
...
describe('Model manifest', () => {
  ...
  it('has expected top-level structure', () => {
    ...
    assert.ok(Object.keys(manifest.models).length >= 3, 'should have at least 3 models');
  });
  ...
  it('each model has required fields', () => {
    ...
    for (const [id, config] of Object.entries(manifest.models)) {
      ...
      assert.ok(typeof config.contextWindow === 'number', `model ${id} should have numeric contextWindow`);
      assert.ok(config.contextWindow > 0, `model ${id} contextWindow should be positive`);
      assert.ok(typeof config.numPredict === 'number', `model ${id} should have numeric numPredict`);
  ...
  it('default model exists in models map', () => {
    const manifest = JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf8'));
    assert.ok(manifest.models[manifest.defaultModel], 'default model should exist in models map');
  });

  it('includes envVars documentation', () => {
    const manifest = JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf8'));
    assert.ok(manifest.envVars, 'should have envVars section');
    assert.ok(manifest.envVars.OLLAMA_BASE_URL, 'should document OLLAMA_BASE_URL');
    assert.ok(manifest.envVars.OLLAMA_MODEL, 'should document OLLAMA_MODEL');
    assert.ok(manifest.envVars.OLLAMA_NUM_CTX, 'should document OLLAMA_NUM_CTX');
  });
});
...
describe('Modelfiles', () => {
  ...
  for (const [modelId, config] of Object.entries(manifest.models)) {
    ...
    it(`${modelId}: Modelfile exists at ${config.modelfile}`, () => {
      assert.ok(fs.existsSync(modelfilePath), `Modelfile should exist: ${modelfilePath}`);
    });

    it(`${modelId}: Modelfile contains PARAMETER num_ctx`, () => {
      const content = fs.readFileSync(modelfilePath, 'utf8');
      assert.ok(/PARAMETER\s+num_ctx\s+\d+/i.test(content), 'Modelfile must set PARAMETER num_ctx');
    });
    ...
    it(`${modelId}: Modelfile num_ctx matches manifest contextWindow`, () => {
      ...
      assert.strictEqual(modelfileCtx, config.contextWindow,
    ...
    it(`${modelId}: Modelfile contains PARAMETER num_predict`, () => {
      const content = fs.readFileSync(modelfilePath, 'utf8');
      assert.ok(/PARAMETER\s+num_predict\s+\d+/i.test(content), 'Modelfile must set PARAMETER num_predict');
    });

    it(`${modelId}: Modelfile contains PARAMETER temperature`, () => {
      const content = fs.readFileSync(modelfilePath, 'utf8');
      assert.ok(/PARAMETER\s+temperature\s+[\d.]+/i.test(content), 'Modelfile must set PARAMETER temperature');
    });

    it(`${modelId}: Modelfile contains SYSTEM prompt`, () => {
      const content = fs.readFileSync(modelfilePath, 'utf8');
      assert.ok(/^SYSTEM\s+/m.test(content), 'Modelfile must have a SYSTEM prompt');
    });

    it(`${modelId}: Modelfile contains FROM directive`, () => {
      const content = fs.readFileSync(modelfilePath, 'utf8');
      assert.ok(/^FROM\s+/m.test(content), 'Modelfile must have a FROM directive');
    });
  }
});
...
describe('Setup script', () => {
  ...
  it('setup script is valid JavaScript', () => {
    ...
    const result = spawnSync(process.execPath, ['--check', SETUP_SCRIPT], { stdio: 'pipe' });
    assert.strictEqual(result.status, 0, `setup script should be valid JS: ${result.stderr?.toString() || ''}`);
  });

  it('setup script references manifest.json', () => {
    const content = fs.readFileSync(SETUP_SCRIPT, 'utf8');
    assert.ok(content.includes('manifest.json'), 'setup script should reference manifest.json');
  });
...
describe('auto-processor.js num_ctx', () => {
  it('reads OLLAMA_NUM_CTX from environment', () => {
    ...
    assert.ok(content.includes('OLLAMA_NUM_CTX'), 'auto-processor should read OLLAMA_NUM_CTX env var');
  });

  it('sends num_ctx in Ollama options', () => {
    const content = fs.readFileSync(AUTO_PROCESSOR, 'utf8');
    assert.ok(/num_ctx\s*:/.test(content), 'auto-processor should send num_ctx in options');
  });

  it('defaults num_ctx to 8192 when env var not set', () => {
    const content = fs.readFileSync(AUTO_PROCESSOR, 'utf8');
    assert.ok(/OLLAMA_NUM_CTX.*8192/.test(content), 'should default to 8192 tokens');
  });

  it('sends repeat_penalty in Ollama options', () => {
    const content = fs.readFileSync(AUTO_PROCESSOR, 'utf8');
    assert.ok(/repeat_penalty\s*:/.test(content), 'should send repeat_penalty in options');
  });
});
...
describe('ollama-client.cjs num_ctx', () => {
  it('reads OLLAMA_NUM_CTX from environment', () => {
    ...
    assert.ok(content.includes('OLLAMA_NUM_CTX'), 'ollama-client should read OLLAMA_NUM_CTX env var');
  });

  it('sends num_ctx in ollamaGenerate options', () => {
    const content = fs.readFileSync(OLLAMA_CLIENT, 'utf8');
    assert.ok(/num_ctx\s*:/.test(content), 'ollama-client should send num_ctx in options');
  });

  it('defaults num_ctx to 8192 when env var not set', () => {
    const content = fs.readFileSync(OLLAMA_CLIENT, 'utf8');
    assert.ok(/DEFAULT_NUM_CTX.*8192/.test(content), 'should default to 8192 tokens');
  });
  ...
  it('allows numCtx override via options parameter', () => {
    const content = fs.readFileSync(OLLAMA_CLIENT, 'utf8');
    assert.ok(/options\.numCtx/.test(content), 'should allow numCtx override via options');
  });
});
...
describe('Context window alignment with chunker', () => {
  ...
  it('unbreakable-oracle context window matches chunker default', () => {
    ...
    const chunkerCtx = DEFAULT_CONTEXT_WINDOWS['unbreakable-oracle'] || DEFAULT_CONTEXT_WINDOWS['unbreakable-oracle:latest'];
    assert.strictEqual(manifestCtx, chunkerCtx,
  ...
  it('simplebeacon-qwen-coder context window matches chunker qwen2.5-coder default', () => {
    const manifestCtx = manifest.models['simplebeacon-qwen-coder'].contextWindow;
