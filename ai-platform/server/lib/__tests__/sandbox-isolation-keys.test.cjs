...
describe("sandbox directory isolation keys", () => {
  ...
  test("throws on missing or invalid orgId", () => {
    expect(() => cryptoUtils.deriveDirectoryKey("", "customers")).toThrow(
      TypeError,
    );
    expect(() => cryptoUtils.deriveDirectoryKey(null, "customers")).toThrow(
      TypeError,
    );
    expect(() => cryptoUtils.deriveDirectoryKey(123, "customers")).toThrow(
      TypeError,
    );
  });

  test("throws on missing or invalid directory", () => {
    expect(() => cryptoUtils.deriveDirectoryKey("org-a", "")).toThrow(
      TypeError,
    );
    expect(() => cryptoUtils.deriveDirectoryKey("org-a", null)).toThrow(
      TypeError,
    );
    expect(() => cryptoUtils.deriveDirectoryKey("org-a", 456)).toThrow(
      TypeError,
    );
  });
  ...
  test("encrypt and decrypt for directory round-trips", () => {
    ...
    const stored = cryptoUtils.encryptForDirectory(
      plaintext,
      "org-a",
      "customers",
    );
    ...
    const recovered = cryptoUtils.decryptForDirectory(
      stored,
      "org-a",
      "customers",
    );
