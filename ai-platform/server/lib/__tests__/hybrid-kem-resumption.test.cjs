...
describe("hybrid-kem-resumption", () => {
  ...
  test("createTicket/validateTicket works and prevents replay", async () => {
    ...
    const { ticket } = createTicket(
      { sessionId: "sess-1", nodeId: "nodeA", prevRoot },
      stekObj.stek,
      stekObj.stekId,
      60000,
    );
  ...
  test("expired ticket is rejected with EXPIRED", async () => {
    ...
    const stekObj = generateStek();
    const { ticket } = createTicket(
      { sessionId: "sess-exp", nodeId: "nodeX", prevRoot },
      stekObj.stek,
      stekObj.stekId,
      24 * 60 * 60 * 1000,
    );
    ...
    const newCiphertext = Buffer.concat([
      cipher.update(newPlain),
      cipher.final(),
    ]);
    const newTag = cipher.getAuthTag();
    const newTicket = Buffer.concat([
      ticket.slice(0, HEADER_LENGTH),
      newCiphertext,
      newTag,
    ]);
...
function createMockSocketPair() {
  ...
  client.write = (d) => {
    if (d) process.nextTick(() => server.emit("data", d));
    return true;
  };
  server.write = (d) => {
    if (d) process.nextTick(() => client.emit("data", d));
    return true;
  };
  client.destroy = () => {};
  server.destroy = () => {};
...
describe("hybrid-kem-resumption", () => {
  ...
  test("L1: PSK differs when prevRoot, nodeId, or sessionId change", () => {
    ...
    const changedRoot = resumption.deriveResumptionPsk(
      crypto.randomBytes(32),
      "node-A",
      "sid-1",
    );
    const changedNode = resumption.deriveResumptionPsk(
      prevRoot,
      "node-B",
      "sid-1",
    );
    const changedSession = resumption.deriveResumptionPsk(
      prevRoot,
      "node-A",
      "sid-2",
    );
  ...
  test("L3: tryResumption fail-closed when bloom filter errors", async () => {
    ...
    const server = handshake.tryResumption(
      serverSocket,
      stekById,
      failingBloom,
      5000,
