...
describe('Partition Enforcement E2E Integration', () => {
  ...
  describe('POST /api/audit/partition-violations/clear', () => {
    it('should clear violations and return cleared count for admin user', async () => {
      ...
      // Now clear
      const res = await request(adminApp).post('/api/audit/partition-violations/clear');
      assert.strictEqual(res.status, 200);
      assert.strictEqual(res.body.success, true);
    ...
    it('should reject non-admin user with 403', async () => {
      const res = await request(regularApp).post('/api/audit/partition-violations/clear');
      assert.strictEqual(res.status, 403);
      assert.strictEqual(res.body.success, false);
    });

    it('should reject unauthenticated request with 401', async () => {
      const res = await request(noAuthApp).post('/api/audit/partition-violations/clear');
      assert.strictEqual(res.status, 401);
    });
  ...
  describe('GET /api/audit/partition-retention/config', () => {
    it('should return retention config for admin user', async () => {
      const res = await request(adminApp).get('/api/audit/partition-retention/config');
      assert.strictEqual(res.status, 200);
      assert.strictEqual(res.body.success, true);
      assert.ok(res.body.config);
      assert.ok(typeof res.body.config.orgPartitionViolationTtlMs === 'number');
      assert.ok(typeof res.body.config.orgPartitionViolationMaxLog === 'number');
      assert.ok(typeof res.body.config.orgPartitionViolationCleanupIntervalMs === 'number');
      assert.ok(typeof res.body.config.orgPartitionViolationMemoryGuardMb === 'number');
    });
    ...
    it('should reject non-admin user with 403', async () => {
      const res = await request(regularApp).get('/api/audit/partition-retention/config');
      assert.strictEqual(res.status, 403);
    });

    it('should reject unauthenticated request with 401', async () => {
      const res = await request(noAuthApp).get('/api/audit/partition-retention/config');
      assert.strictEqual(res.status, 401);
    });
  });
  ...
  describe('PUT /api/audit/partition-retention/config', () => {
    it('should update retention config for admin user', async () => {
      ...
      assert.strictEqual(res.status, 200);
      assert.strictEqual(res.body.success, true);
      assert.strictEqual(res.body.config.orgPartitionViolationTtlMs, 48 * 60 * 60 * 1000);
    ...
    it('should update memory guard and cleanup interval', async () => {
      ...
      assert.strictEqual(res.status, 200);
      assert.strictEqual(res.body.config.orgPartitionViolationMemoryGuardMb, 100);
      assert.strictEqual(res.body.config.orgPartitionViolationCleanupIntervalMs, 10 * 60 * 1000);
    });
  ...
  describe('Full retention config update cycle', () => {
    it('should reflect retention config changes in GET endpoint', async () => {
      ...
      const get = await request(adminApp).get('/api/audit/partition-retention/config');
      assert.strictEqual(get.status, 200);
      assert.strictEqual(get.body.config.orgPartitionViolationTtlMs, 72 * 60 * 60 * 1000);
      assert.strictEqual(get.body.config.orgPartitionViolationMaxLog, 2000);
      assert.strictEqual(get.body.config.orgPartitionViolationCleanupIntervalMs, 15 * 60 * 1000);
      assert.strictEqual(get.body.config.orgPartitionViolationMemoryGuardMb, 75);
    });
