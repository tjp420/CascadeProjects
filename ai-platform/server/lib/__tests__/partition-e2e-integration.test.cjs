 ...
 *
 * Tests the full request path: authenticate → authorize → route handler → audit log
 */
...
describe('Partition Enforcement E2E Integration', () => {
  ...
  before(() => {
    // Require the settings store once — we'll use updateSettings() to reset
  ...
  beforeEach(() => {
    // Reset settings to defaults via updateSettings() — this updates both
  ...
  // ── Full request path: partition-status → config update → status reflects change ──

  describe('Full config update cycle', () => {
    it('should reflect config changes in partition-status endpoint', async () => {
      // 1. Get initial status — enforcement should be enabled (from beforeEach defaults)
      ...
      // 2. Update config via admin — disable enforcement
      ...
      // 3. Get updated status — should reflect the change
