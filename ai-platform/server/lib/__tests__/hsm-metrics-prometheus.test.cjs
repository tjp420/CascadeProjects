...
const STEK_GAUGES = ["hsm_stek_active_count", "hsm_stek_retired_count"];
...
describe("HSM Metrics Prometheus Exposition — Option F", () => {
  ...
  test("L2-12: Prometheus exposition format is valid for all new metrics", () => {
    ...
    expect(lines.some((l) => l === "# TYPE hsm_stek_active_count gauge")).toBe(
      true,
    );
    expect(
      lines.some((l) => l === "# TYPE hsm_dkg_round_duration_ms histogram"),
    ).toBe(true);
    expect(
      lines.some((l) => l.startsWith('hsm_dkg_round_duration_ms_bucket{le="')),
    ).toBe(true);
    expect(lines.some((l) => l === "hsm_dkg_round_duration_ms_sum 100")).toBe(
      true,
    );
    expect(lines.some((l) => l === "hsm_dkg_round_duration_ms_count 1")).toBe(
      true,
    );
  });
  ...
  describe("L3: Edge cases & validation", () => {
    ...
    test("L3-02: Observe histogram with unknown name — no crash", () => {
      expect(() =>
        hsmMetrics.observeHistogram("unknown_histogram", 100),
      ).not.toThrow();
    });
    ...
    test("L3-05: Alert rules YAML is valid Prometheus rule format", () => {
      ...
      const ruleNames = doc.groups.flatMap((g) =>
    ...
    test("L3-06: Grafana dashboard JSON has required fields", () => {
      ...
      const files = [
        "track115-mesh-reconciliation.json",
        "dkg-operations.json",
        "stek-lifecycle.json",
      ];
    ...
    test("L3-08: All 22 new counters + 2 gauges + 1 histogram are registered", () => {
      const m = hsmMetrics.getMetrics();
      for (const n of ALL_NEW_COUNTERS) {
        expect(m).toHaveProperty(n);
        expect(m[n]).toBe(0);
      }
      for (const n of ALL_NEW_GAUGES) {
        expect(m).toHaveProperty(n);
        expect(m[n]).toBe(0);
      }
  ...
  describe("Security", () => {
    test("S-01: No credentials or PII in dashboard JSON or alert rules", () => {
      ...
      const files = [
        ...
        fs.readFileSync(
          path.join(dir, "track115-mesh-reconciliation.json"),
          "utf8",
        ),
      ...
      for (const c of files) {
        for (const p of patterns) {
          ...
          if (matches) {
            ...
            if (lower.includes("private key") && p.source.includes("private"))
              continue;
