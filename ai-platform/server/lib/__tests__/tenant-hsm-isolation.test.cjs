...
describe('tenant HSM isolation', () => {
  ...
  test('L3-01: empty orgId throws missing_org_id', () => {
    expect(() => keyRotationStore.deriveTenantKey('')).toThrow(/missing_org_id|valid orgId/);
  });
  ...
  test('L2-03: HSM timeout fails closed with hsm_timeout', async () => {
    ...
    await expect(hsm.deriveOrgKeyViaHsm('org-a')).rejects.toMatchObject({ code: 'hsm_timeout' });
    const events = clusterSync.queryEvents({ eventType: clusterSync.EVENT_TYPES.HSM_TIMEOUT });
    expect(events.events.length).toBeGreaterThan(0);
  });

  test('L2-04: isolation violation is recorded when actor accesses wrong org key', async () => {
    await expect(hsm.deriveOrgKeyViaHsm('org-b', { actorOrgId: 'org-a' }))
  ...
  test('S-04: cluster event for violation does not include raw key material', async () => {
    await expect(hsm.deriveOrgKeyViaHsm('org-b', { actorOrgId: 'org-a' })).rejects.toBeDefined();
    const events = clusterSync.queryEvents({ eventType: clusterSync.EVENT_TYPES.ISOLATION_VIOLATION });
    expect(events.events.length).toBeGreaterThan(0);
