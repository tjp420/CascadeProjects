...
const KEY_ROTATION_PATH = path.resolve(process.cwd(), 'server', 'lib', 'key-rotation-store.cjs');
...
describe('Autonomous Background Ledger Re-Keying Worker', () => {
  ...
  before(() => {
    ...
    _tempQuarantineDir = path.join(process.cwd(), '.simplebeacon', 'quarantine');
  ...
  describe('when rotation is active', () => {
    ...
    beforeEach(() => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'original-master-key-1234567890';
      ...
      const currentKey = crypto.createHash('sha256').update('original-master-key-1234567890').digest();
    ...
    it('should re-key quarantine files from old key to active key', () => {
      ...
      assert.ok(rawBefore.startsWith('enc:sb:dir:'), 'Quarantine file should be encrypted');
      ...
      assert.ok(result.migrated >= 1, `Should migrate at least 1 file, got ${result.migrated}`);
      assert.strictEqual(result.failed, 0, 'Should have no failures');
      assert.strictEqual(result.rotationActive, true, 'Should report rotation active');
    });
    ...
    it('should purge previous key after successful migration', () => {
      ...
      if (result.migrated > 0 && result.failed === 0) {
        assert.strictEqual(result.purged, true, 'Should purge previous key after successful migration');
      }
    });
    ...
    it('should skip orgs with no quarantine files', () => {
      ...
      assert.ok(result.skipped >= 0, 'Should not fail for missing quarantine files');
      assert.strictEqual(result.failed, 0, 'Should have no failures');
    });
  ...
  describe('when grace window has expired', () => {
    ...
    beforeEach(() => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'grace-expired-key-123456789012';
      process.env.KEY_ROTATION_GRACE_MS = '1'; // 1ms grace
      const mods = reloadAllModules();
      keyRotationStore = mods.keyRotationStore;
      auditLogger = mods.auditLogger;

      const currentKey = crypto.createHash('sha256').update('grace-expired-key-123456789012').digest();
      keyRotationStore._reset(currentKey);
    });
    ...
    it('should skip migration and attempt purge when grace has expired', () => {
      ...
      assert.strictEqual(result.migrated, 0, 'Should not migrate when grace expired');
      assert.strictEqual(result.rotationActive, false, 'Should report rotation not active');
      // Purge should be attempted
      assert.ok(typeof result.purged === 'boolean', 'Should report purge status');
    });
  });
  ...
  describe('getReKeyStats', () => {
    ...
    it('should increment totalSweeps after each run', () => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'sweep-count-key-12345678901234';
      const auditLogger = reloadModule(AUDIT_LOGGER_PATH);
    ...
    it('should not expose raw key material in stats', () => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'no-leak-test-key-12345678901234';
      const auditLogger = reloadModule(AUDIT_LOGGER_PATH);
      auditLogger.runAutonomousReKeying();
      const stats = auditLogger.getReKeyStats();
      ...
      assert.ok(!statsJson.includes('no-leak-test-key-12345678901234'), 'Stats should not expose raw key');
      // Should not contain hex key material (64-char hex strings)
      assert.ok(!/"[a-f0-9]{64}"/.test(statsJson), 'Stats should not expose hex key material');
    });
  });
  ...
  describe('re-keyed file integrity', () => {
    it('should keep re-keyed quarantine file encrypted with sb-dir: prefix', () => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'integrity-test-key-12345678901';
      ...
      const currentKey = crypto.createHash('sha256').update('integrity-test-key-12345678901').digest();
      keyRotationStore._reset(currentKey);
      ...
      assert.ok(rawAfter.startsWith('enc:sb:dir:'), 'Re-keyed file should still be encrypted with sb-dir: prefix');
    });

    it('should keep re-keyed quarantine file readable with correct orgId', () => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'readable-test-key-123456789012';
      process.env.KEY_ROTATION_GRACE_MS = '172800000';
      ...
      const currentKey = crypto.createHash('sha256').update('readable-test-key-123456789012').digest();
      keyRotationStore._reset(currentKey);
      ...
      assert.ok(quarantine.entries.length > 0, 'Should have at least one quarantined entry');
      assert.strictEqual(quarantine.entries[0].orgId, 'org-readable', 'Entry should have correct orgId');
    });
  });
});
