...
describe('simplebeacon-proxy', () => {
  ...
  it('exposes consolidation helpers', () => {
    ...
    assert.strictEqual(typeof proxy.consolidationCandidateTouchesExcluded, 'function');
  ...
  it('compactDataCleanupReportForClient passes through objects', () => {
    const input = { baz: 1 };
    assert.deepStrictEqual(proxy.compactDataCleanupReportForClient(input), input);
  });

  it('compactDataCleanupReportForClient returns empty object for invalid input', () => {
    assert.deepStrictEqual(proxy.compactDataCleanupReportForClient(undefined), {});
    assert.deepStrictEqual(proxy.compactDataCleanupReportForClient(42), {});
  });

  it('syncMeasuredBaseline throws on invalid baseDir', () => {
    assert.throws(() => proxy.syncMeasuredBaseline(null), /baseDir must be a non-empty string/);
    assert.throws(() => proxy.syncMeasuredBaseline(''), /baseDir must be a non-empty string/);
    assert.throws(() => proxy.syncMeasuredBaseline(123), /baseDir must be a non-empty string/);
  });
});
