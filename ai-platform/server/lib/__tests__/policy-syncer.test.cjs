...
const {
  ...
  clearCache,
  POLICY_STORE_DIR,
} = require("../policy-syncer.cjs");
const {
  enforceCompliancePolicy,
} = require("../../middleware/compliance-guard.cjs");
...
describe("Multi-Tenant Compliance Policy Syncer Engine Suite", () => {
  ...
  const setupHook =
    typeof beforeEach === "function" ? beforeEach : (fn) => test.beforeEach(fn);
  ...
  it("should halt request execution lines if a mandatory DLP token condition is missing", async () => {
    ...
    await guardMiddleware(req, res, () => {
      assert.fail(
        "Should never invoke next() if an explicit DENY threshold breaks compliance",
      );
    });
