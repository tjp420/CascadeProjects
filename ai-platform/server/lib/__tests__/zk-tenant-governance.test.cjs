...
describe("Track 125: ZK Tenant Governance", () => {
  ...
  test("GOV-02b: resolvePolicy throws for invalid tenant ID", () => {
    const engine = new CryptoPolicyEngine();
    expect(() => resolvePolicy(engine, "../bad", "energyGating")).toThrow(
      "invalid tenant ID",
    );
    expect(
  ...
  test("GOV-02c: resolvePolicy throws for unknown domain", () => {
    const engine = new CryptoPolicyEngine();
    expect(() => resolvePolicy(engine, "default", "nonExistentGating")).toThrow(
      "unknown domain",
    );
  });
  ...
  test("GOV-03a: trackVerification increments verified counter", () => {
    ...
    expect(hsmMetrics.getMetrics().hsm_zk_tenant_context_validated_total).toBe(
      1,
    );
  });
  ...
  test("GOV-04a: wrapWithTenantGovernance adds validateTenant method", () => {
    ...
    const mockValidator = {
      ...
      verifyTestClaim: function (req) {
        return { verified: true, request: req };
      },
    };
    wrapWithTenantGovernance(
      mockValidator,
  ...
  test("GOV-04f: wrapped validator restores policy even on failure", () => {
    ...
    try {
      mockValidator.validateTenant("tenant-1", {});
    } catch (e) {}
    expect(mockValidator.policy).toBe(originalPolicy);
  });
  ...
  test("GOV-04g: wrapped validator rejects invalid tenant ID", () => {
    ...
    expect(() => mockValidator.validateTenant("../bad", {})).toThrow(
      "invalid tenant ID",
    );
    expect(
      hsmMetrics.getMetrics().hsm_zk_tenant_isolation_violation_total,
  ...
  test("GOV-05a: createGovernedClaimValidator wraps energy validator", () => {
    ...
    expect(() => governed.validateTenant("../bad", {})).toThrow(
      "zk_isolation_violation",
    );
  });
  ...
  describe("MET: hsm-metrics ZK counters", () => {
    test("MET-01: hsm_zk_energy_claim_verified_total exists", () => {
      ...
      expect(hsmMetrics.getMetrics().hsm_zk_energy_claim_verified_total).toBe(
        1,
      );
    });
    ...
    test("MET-02: hsm_zk_biometric_claim_verified_total exists", () => {
      ...
      expect(
        hsmMetrics.getMetrics().hsm_zk_biometric_claim_verified_total,
      ).toBe(1);
    });
    ...
    test("MET-04: hsm_zk_tenant_context_validated_total exists", () => {
      ...
      expect(
        hsmMetrics.getMetrics().hsm_zk_tenant_context_validated_total,
      ).toBe(1);
    });
    ...
    test("MET-05: all 16 new ZK counters exist", () => {
      const counters = [
        ...
        "hsm_zk_authentication_claim_verified_total",
  ...
  describe("POL: crypto-policy-engine new sections", () => {
    test("POL-01: energyGating exists with correct defaults", () => {
      ...
      expect(policy.energyGating.requireClearingCommitteeAttestation).toBe(
        true,
      );
      expect(policy.energyGating.allowedAttestationAuthorities).toEqual([
        "mock-authority",
      ]);
    });
    ...
    test("POL-02: biometricGating exists with correct defaults", () => {
      ...
      expect(policy.biometricGating.requireClearingCommitteeAttestation).toBe(
        true,
      );
    });
    ...
    test("POL-03: neuralGating exists with correct defaults", () => {
      ...
      expect(
        policy.neuralGating.requireNeuralEthicsOversightCommitteeAttestation,
      ).toBe(true);
    });
