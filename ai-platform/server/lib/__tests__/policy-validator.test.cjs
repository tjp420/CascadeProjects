...
const { validateCryptoPolicyEngine, PolicyEngineValidationError } = require('../policy-validator.cjs');
...
describe('Policy Validator - validateCryptoPolicyEngine', () => {
  ...
  test('threshold below BFT quorum throws', () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(PolicyEngineValidationError);
      expect(String(err.message)).toMatch(/BFT threshold|Cryptographic policy threshold drops below/);
    }
  });
  ...
  test('invalid sigma bound throws', () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(PolicyEngineValidationError);
      expect(String(err.message)).toMatch(/Critical post-quantum vulnerability/);
    }
  });
});
