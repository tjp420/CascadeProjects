...
const { BackupCoordinator, createMemoryStorage, BACKUP_VERSION } = require('../backup-coordinator.cjs');
...
describe('backup-coordinator', () => {
  ...
  function makeBundle(overrides = {}) {
    return {
      ...
      auditLog: Buffer.from(JSON.stringify([{ eventId: 1, type: 'KEY_COMMIT', node: 'node-A' }]), 'utf8'),
      resumptionTickets: [
        { sessionId: 'sid-1', nodeId: 'node-A', issuedAt: Date.now(), prevRootHash: 'abc123' },
      ],
      issuedAt: Date.now(),
      ...overrides,
    };
  ...
  describe('BC-01: Constructor', () => {
    ...
    test('L3-02: rejects short KEK (16 bytes)', () => {
      expect(() => new BackupCoordinator({ kek: crypto.randomBytes(16) })).toThrow(/INVALID_KEK/);
    });
    ...
    test('accepts custom retention and immutable flag', () => {
      const coord = new BackupCoordinator({ kek: VALID_KEK, retentionDays: 7, immutable: true });
  ...
  describe('BC-04: deriveArchiveKey', () => {
    ...
    test('different KEK produces different key', () => {
      const key1 = BackupCoordinator.deriveArchiveKey(crypto.randomBytes(32), 'archive-001');
      const key2 = BackupCoordinator.deriveArchiveKey(crypto.randomBytes(32), 'archive-001');
      expect(key1.equals(key2)).toBe(false);
    });
  });
  ...
  describe('BC-02/BC-03: backup()', () => {
    ...
    test('L2-06: rejects bundle missing keyringMaterial', async () => {
      const coord = new BackupCoordinator({ kek: VALID_KEK });
      await expect(coord.backup({ auditLog: Buffer.from('test'), issuedAt: Date.now() })).rejects.toThrow(/INVALID_BUNDLE/);
    });

    test('rejects bundle where keyringMaterial is not a Buffer', async () => {
      const coord = new BackupCoordinator({ kek: VALID_KEK });
      await expect(coord.backup({ keyringMaterial: 'not-a-buffer', auditLog: Buffer.from('test'), issuedAt: Date.now() })).rejects.toThrow(/INVALID_BUNDLE/);
    });

    test('rejects bundle missing auditLog', async () => {
      const coord = new BackupCoordinator({ kek: VALID_KEK });
      await expect(coord.backup({ keyringMaterial: crypto.randomBytes(32), issuedAt: Date.now() })).rejects.toThrow(/INVALID_BUNDLE/);
    });

    test('rejects bundle missing issuedAt', async () => {
      const coord = new BackupCoordinator({ kek: VALID_KEK });
      await expect(coord.backup({ keyringMaterial: crypto.randomBytes(32), auditLog: Buffer.from('test') })).rejects.toThrow(/INVALID_BUNDLE/);
    });
  });
  ...
  describe('BC-05/BC-06: restore() and verifyArchive()', () => {
    ...
    test('BACKUP_NOT_FOUND for unknown archiveId', async () => {
      const coord = new BackupCoordinator({ kek: VALID_KEK });
      await expect(coord.restore('nonexistent-id')).rejects.toThrow(/BACKUP_NOT_FOUND/);
    });
  ...
  describe('BC-07/BC-08: prune()', () => {
    test('L2-04: prune removes only old archives', async () => {
      const storage = createMemoryStorage();
      const coord = new BackupCoordinator({ kek: VALID_KEK, storage, retentionDays: 1 });
      ...
      await expect(coord.restore(old.archiveId)).rejects.toThrow(/BACKUP_NOT_FOUND/);
    ...
    test('L2-05: immutability prevents deletion', async () => {
      const storage = createMemoryStorage();
      const coord = new BackupCoordinator({ kek: VALID_KEK, storage, immutable: true, retentionDays: 1 });
    ...
    test('prune emits audit event for each deletion', async () => {
      ...
      const prunedEvents = coord.events.filter((e) => e.type === 'BACKUP_PRUNED');
  ...
  describe('S-03: pruning cannot delete immutable archives', () => {
    test('immutable archive survives prune with old timestamp', async () => {
      ...
      const { archiveId } = await coord.backup(makeBundle({ issuedAt: oldTime }));
