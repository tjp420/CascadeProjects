...
describe("Audit Compliance Report Exporter Suite", () => {
  ...
  it("should isolate cross-tenant profiles while keeping caller context first", () => {
    ...
    assert.strictEqual(
      report.orgs[0].orgId,
      mockOrgId,
      "Primary index must match caller org context",
    );
  ...
  it("should compile flat multi-sectional CSV envelopes deterministically", () => {
    ...
    assert.ok(
      csv.includes("SimpleBeacon Compliance Proof Bundle"),
      "Missing bundle header metadata",
    );
    assert.ok(
      csv.includes("SECTION 1: GLOBAL PLATFORM SECURITY CONTROLS"),
      "Missing control break markers",
    );
    assert.ok(
      csv.includes("SECTION 2: MULTI-TENANT CRYPTOGRAPHIC ATTESTATION MATRIX"),
      "Missing attestation headers",
    );
  });
});
