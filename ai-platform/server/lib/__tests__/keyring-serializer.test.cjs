...
describe("keyring-serializer", () => {
  ...
  describe("serialize", () => {
    ...
    test("rejects missing or non-Buffer KEK", () => {
      ...
      expect(() => serialize(keyring, "not-a-buffer")).toThrow(
        KeyringValidationError,
      );
    });
    ...
    test("rejects invalid KEK bit lengths", () => {
      const keyring = makeValidKeyring();
      expect(() => serialize(keyring, Buffer.alloc(15))).toThrow(
        /Invalid KEK length/,
      );
      expect(() => serialize(keyring, Buffer.alloc(17))).toThrow(
        /Invalid KEK length/,
      );
      expect(() => serialize(keyring, Buffer.alloc(33))).toThrow(
        /Invalid KEK length/,
      );
    });
  ...
  describe("deserialize", () => {
    ...
    test("rejects non-Buffer input", () => {
      const kek = Buffer.alloc(32);
      expect(() => deserialize("not-a-buffer", kek)).toThrow(
        KeyringValidationError,
      );
    ...
    test("rejects invalid KEK lengths for decryption", () => {
      ...
      expect(() => deserialize(blob, Buffer.alloc(15))).toThrow(
        /Invalid KEK length/,
      );
    });
    ...
    test("rejects a truncated buffer", () => {
      ...
      expect(() => deserialize(truncated, kek)).toThrow(
        /Header chunk length is under threshold/,
      );
    });
    ...
    test("rejects a buffer with the wrong magic", () => {
      ...
      tampered[0] = 0x00;
      expect(() => deserialize(tampered, kek)).toThrow(
        /Unrecognized signature magic/,
      );
    });
    ...
    test("rejects a corrupted ciphertext", () => {
      ...
      expect(() => deserialize(tampered, kek)).toThrow(
        /Cryptographic envelope unpacking failed/,
      );
    });
    ...
    test("rejects decryption with the wrong KEK", () => {
      ...
      wrongKek[0] = 0x01;
      const blob = serialize(makeValidKeyring(), kek);
      expect(() => deserialize(blob, wrongKek)).toThrow(
        /Cryptographic envelope unpacking failed/,
      );
    });
  });
