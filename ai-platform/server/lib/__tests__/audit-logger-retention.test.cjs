...
describe('Audit Logger Retention Engine', () => {
  ...
  describe('purgeOldEntries()', () => {
    ...
    it('should purge entries older than retention days', () => {
      auditPolicyStore._resetCache();
      writePolicyDirectly(tmpPolicyPath, 'test-org', { retentionDays: 1, maxEntries: 100 });
      auditPolicyStore._resetCache();
    ...
    it('should preserve at least maxEntries most recent entries', () => {
      auditPolicyStore._resetCache();
      writePolicyDirectly(tmpPolicyPath, 'test-org', { retentionDays: 1, maxEntries: 5 });
      auditPolicyStore._resetCache();

      // Add 10 old entries (all older than 1 day)
      for (let i = 0; i < 10; i++) {
        const oldDate = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000 - i * 1000);
        writeEntryDirectly(auditLogger, tmpLogPath, {
          orgId: 'test-org',
          timestamp: oldDate.toISOString(),
    ...
    it('should not cross org boundaries', () => {
      auditPolicyStore._resetCache();
      writePolicyDirectly(tmpPolicyPath, 'org-a', { retentionDays: 1, maxEntries: 100 });
      writePolicyDirectly(tmpPolicyPath, 'org-b', { retentionDays: 365, maxEntries: 100 });
      auditPolicyStore._resetCache();
    ...
    it('should archive entries when archive policy is enabled', () => {
      auditPolicyStore._resetCache();
      writePolicyDirectly(tmpPolicyPath, 'archive-org', { retentionDays: 1, maxEntries: 100, archive: true });
      auditPolicyStore._resetCache();
