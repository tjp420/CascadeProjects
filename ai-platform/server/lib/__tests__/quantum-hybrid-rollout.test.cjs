...
describe("quantum-hybrid-rollout", () => {
  const config = loadCanaryConfig(
    path.join(__dirname, "..", "..", "config", "quantum-hybrid-canary.json"),
  );
  ...
  test("L2-01: enrollmentScore is deterministic for the same NODE_ID", () => {
    const score1 = enrollmentScore(
      "node-canary-01",
      config.canary_parameters.salt,
    );
    const score2 = enrollmentScore(
      "node-canary-01",
      config.canary_parameters.salt,
    );
  ...
  test("L2-03: JSON config loads with expected threshold shape", () => {
    ...
    expect(
      config.rollback_thresholds.connection_drop_rate_spike_pct,
    ).toBeGreaterThan(0);
    expect(
      config.rollback_thresholds.handshake_failure_rate_pct,
    ).toBeGreaterThan(0);
  });
  ...
  test("L2-06: rollback can be recorded with quantum_hybrid_rollback event", () => {
    const { shouldRollback, reasons } = checkRollback(
      { handshakeFailureRatePct: 15.0 },
      config,
    );
