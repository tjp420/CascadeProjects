...
describe("SiemSecurityBroker distributed token bucket sync", () => {
  describe("enableDistributedSync", () => {
    ...
    it("caps local tokens to fair share on enable", () => {
      ...
      assert.strictEqual(
        broker.tokens,
        25,
        "tokens should be capped to fair share",
      );
      broker.close();
    });
    ...
    it("throws on missing nodeCount", () => {
      ...
      assert.throws(
        () => broker.enableDistributedSync({ nodeId: "n1" }),
        /nodeCount/,
      );
      broker.close();
    });
  ...
  describe("handleTokenGrant", () => {
    it("adds granted tokens to local bucket", () => {
      ...
      assert.strictEqual(
        broker.tokens,
        10,
        "should have 10 tokens after grant",
      );
      broker.close();
    });
    ...
    it("ignores grants for other nodes", () => {
      ...
      assert.strictEqual(
        broker.tokens,
        0,
        "should not accept grants for other nodes",
      );
      broker.close();
    });
  ...
  describe("cluster-wide rate limit convergence", () => {
    it("N=3 nodes: total events before exhaustion ≈ maxTokens, not 3×maxTokens", () => {
      ...
      assert.ok(
        totalProcessed <= 60,
        `total processed (${totalProcessed}) should not exceed maxTokens (60)`,
      );
      assert.ok(
        totalProcessed >= 20,
    ...
    it("N=1 node: distributed sync does not change behavior", () => {
      ...
      assert.strictEqual(
        processed,
        50,
        "single node should process maxTokens events",
      );
      broker.close();
    });
    ...
    it("CRITICAL/FATAL bypass distributed rate limiter", () => {
      ...
      assert.strictEqual(
        result,
        true,
        "CRITICAL must bypass distributed rate limiter",
      );
  ...
  describe("close cleanup", () => {
    it("clears distributed sync timer on close", () => {
      ...
      assert.ok(
        timer._destroyed || !broker._distSyncTimer,
        "timer should be destroyed or cleared",
      );
    });
  });
});
