...
const { createClientHandshaker, createServerHandshaker } = require('../hybrid-kem-handshake.cjs');
...
describe('hybrid-kem-mitm', () => {
  ...
  test('L2-06: forced downgrade strip is rejected and logged', async () => {
    ...
    const client = createClientHandshaker(clientSocket, { timeoutMs: DEFAULT_TIMEOUT });
    const server = createServerHandshaker(serverSocket, { timeoutMs: DEFAULT_TIMEOUT }).catch((err) => {
      _recordEvent(EVENT_TYPES.QUANTUM_DEGRADE_REJECTED, 'mitm', { attack: 'downgrade-strip', error: err.message });
      return err;
    });
    ...
    const result = queryEvents({ eventType: EVENT_TYPES.QUANTUM_DEGRADE_REJECTED });
