...
describe("analyze-export-bundle/engines", () => {
  ...
  test("resolveEnginesRun returns from options first", () => {
    expect(resolveEnginesRun({}, { enginesRun: ["a", "b"] })).toEqual([
      "a",
      "b",
    ]);
  });
  ...
  test("resolveEnginesRun returns from analysisConfig", () => {
    expect(
      resolveEnginesRun({ analysisConfig: { enginesRun: ["y"] } }),
    ).toEqual(["y"]);
  });

  test("resolveEnginesRun returns from steps", () => {
    expect(resolveEnginesRun({ steps: [{ id: "s1" }, { id: "s2" }] })).toEqual([
      "s1",
      "s2",
    ]);
  });
  ...
  test("resolveSelectedEnginesForExport returns from options", () => {
    expect(
      resolveSelectedEnginesForExport({}, { selectedEngines: ["a", "a", "b"] }),
    ).toEqual(["a", "b"]);
  });
  ...
  test("artifactAllowedForEngines returns true for complete-scan-bundle on complete", () => {
    expect(
      artifactAllowedForEngines("complete-scan-bundle", new Set(), {
        scanKind: "complete",
      }),
    ).toBe(true);
    expect(
  ...
  test("shouldIncludeEuAiActArtifacts respects explicit false", () => {
    expect(shouldIncludeEuAiActArtifacts({}, { includeEuAiAct: false })).toBe(
      false,
    );
  });

  test("shouldIncludeEuAiActArtifacts respects explicit true", () => {
    expect(shouldIncludeEuAiActArtifacts({}, { includeEuAiAct: true })).toBe(
      true,
    );
  });
  ...
  test("resolveSelectedEnginesForExport reads payload enginesRun", () => {
    expect(
      resolveSelectedEnginesForExport({
        enginesRun: ["simplebeacon", "roadmap"],
      }),
  ...
  test("filterCompleteScanForEngines filters enginesRun and steps", () => {
    ...
    const filtered = filterCompleteScanForEngines(scan, [
      "simplebeacon",
      "roadmap",
    ]);
    expect(filtered.enginesRun).toEqual(["simplebeacon", "roadmap"]);
    expect(filtered.steps.map((s) => s.id)).toEqual([
      "simplebeacon",
      "roadmap",
    ]);
    expect(filtered.analysisConfig.selectedEngines).toEqual([
      "simplebeacon",
      "roadmap",
    ]);
  });
  ...
  test("artifactAllowedForEngines gates EU AI Act artifacts", () => {
    ...
    expect(
      artifactAllowedForEngines("eu-ai-act-sprint", engines, {
        includeEuAiAct: false,
      }),
    ).toBe(false);
    expect(
      artifactAllowedForEngines("eu-ai-act-sprint", engines, {
  ...
  test("artifactAllowedForEngines requires mapped engine for known artifacts", () => {
    expect(
      artifactAllowedForEngines("roadmap", new Set(["simplebeacon"])),
    ).toBe(false);
    expect(artifactAllowedForEngines("roadmap", new Set(["roadmap"]))).toBe(
      true,
    );
  });
  ...
  test("shouldIncludeEuAiActArtifacts detects sprint results and eu-ai-act kind", () => {
    expect(
      shouldIncludeEuAiActArtifacts({ type: "simplebeacon-eu-ai-act-sprint" }),
    ).toBe(true);
    expect(
      shouldIncludeEuAiActArtifacts({ results: { sprint: { ok: true } } }),
    ).toBe(true);
    expect(shouldIncludeEuAiActArtifacts({ enginesRun: ["eu-ai-act"] })).toBe(
      true,
    );
  });
});
