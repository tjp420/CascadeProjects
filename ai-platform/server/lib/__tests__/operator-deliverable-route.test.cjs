...
const { listProducts, vaultUrls } = require('../operator-deliverable-service.cjs');
...
describe('operator-deliverable-route', () => {
  ...
  test('registerOperatorDeliverableRoute registers GET and POST routes', () => {
    ...
    expect(app.get).toHaveBeenCalledWith('/api/operator/products', expect.any(Function));
    expect(app.get).toHaveBeenCalledWith('/api/operator/bootstrap', expect.any(Function));
    expect(app.post).toHaveBeenCalledWith('/api/operator/deliverable', expect.any(Function));
  });
