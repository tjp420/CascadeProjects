  ...
  KW_VECTORS.forEach((vector, index) => {
    ...
    describe(`Vector ${index + 1}: ${name}`, () => {
      it("should correctly wrap plaintext into valid NIST ciphertext", () => {
        ...
        expect(result.toString("hex").toUpperCase()).toBe(
      ...
      it("should correctly unwrap NIST ciphertext into valid plaintext", () => {
        ...
        expect(result.toString("hex").toUpperCase()).toBe(
          plaintext.toString("hex").toUpperCase(),
        );
      });
    });
  });
  ...
  describe("Error Handling and Integrity Safeguards", () => {
    ...
    it("should reject malformed KEK bit-lengths", () => {
      ...
      invalidKeks.forEach((invalidKek) => {
        ...
        expect(() => unwrap(invalidKek, Buffer.alloc(24))).toThrow(
          /Invalid KEK length/,
        );
      });
    });
    ...
    it("should reject misaligned or undersized ciphertext blocks during unwrap", () => {
      ...
      expect(() => unwrap(validKek, Buffer.alloc(16))).toThrow(
        /at least 24 bytes/,
      );
...
describe("AES-KWP (RFC 5649) Padding Compliance Suite", () => {
  KWP_VECTORS.forEach((vector, index) => {
    it(`should pass RFC 5649 KWP validation vector ${index + 1} (${vector.name})`, () => {
      ...
      expect(wrapped.toString("hex").toUpperCase()).toBe(
        vector.ciphertext.toString("hex").toUpperCase(),
      );

      const unwrapped = unwrapPad(vector.kek, vector.ciphertext);
      expect(unwrapped.toString("hex").toUpperCase()).toBe(
        vector.plaintext.toString("hex").toUpperCase(),
      );
    });
  });
  ...
  it("should reject invalid padding values or modified data under KWP", () => {
    ...
    expect(() => unwrapPad(validKek, corrupted)).toThrow(
