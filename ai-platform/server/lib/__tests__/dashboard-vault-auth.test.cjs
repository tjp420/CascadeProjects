...
} = require("../dashboard-vault-auth.cjs");
  ...
  describe("setVaultSessionCookie", () => {
    ...
    it("sets SameSite=Strict attribute", () => {
      ...
      assert.ok(
        ...
        `Expected SameSite=Strict in: ${cookie}`,
      );
    });
    ...
    it("sets Max-Age=86400 (24 hours)", () => {
      ...
      assert.ok(
        ...
        `Expected Max-Age=86400 in: ${cookie}`,
      );
    });
    ...
    it("does NOT set Secure flag in development", () => {
      ...
      assert.ok(
        !cookie.includes("Secure"),
        `Expected no Secure in development: ${cookie}`,
      );
    });
    ...
    it("sets cookie name to sb_vault", () => {
      ...
      const cookie = res.getHeader("Set-Cookie");
      assert.ok(
        cookie.startsWith("sb_vault="),
        `Expected cookie to start with sb_vault=: ${cookie}`,
      );
    });
