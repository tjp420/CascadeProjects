...
describe('redis-rate-limit-store', () => {
  ...
  afterEach(async () => {
    ...
    try {
      ...
      if (cached && cached.exports && typeof cached.exports.resetRedisStore === 'function') {
  ...
  describe('RedisStore class', () => {
    ...
    it('uses custom prefix when provided', () => {
      const { RedisStore } = require(STORE_PATH);
      const store = new RedisStore({ client: createMockClient(), prefix: 'custom:' });
  ...
  describe('decrement', () => {
    it('calls DECR on the full key', async () => {
      ...
      let decrKey = null;
      const client = createMockClient({
        decr: async (key) => { decrKey = key; return 4; },
      });
      const store = new RedisStore({ client, prefix: 'test:' });
  ...
  describe('resetKey', () => {
    it('calls DEL on the full key', async () => {
      ...
      let delKey = null;
      const client = createMockClient({
        del: async (key) => { delKey = key; return 1; },
      });
      const store = new RedisStore({ client, prefix: 'test:' });
