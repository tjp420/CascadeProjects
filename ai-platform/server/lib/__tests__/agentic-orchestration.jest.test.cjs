...
jest.mock("../../../server/lib/agentic-orchestration-store.cjs", () => ({
  ...
  executeAgentLoop: async (id, orgId, input, inferenceFn, options) => {
    if (typeof inferenceFn === "function") {
      try {
        const inf = await inferenceFn(input || "test", {
          provider: "mock",
          model: "mock",
        });
        return {
...
jest.mock("../../../server/lib/crypto-utils.cjs", () => ({
  ...
  createReplayDetector: () => ({
    checkAndMark: () => ({ isReplay: false, firstSeen: null }),
    getStats: () => ({}),
  }),
}));
...
function buildApp() {
  ...
  const routerPath =
    require.resolve("../../../server/routes/agentic-orchestration-routes.cjs");
...
describe("agentic-orchestration routes (Jest mirror)", () => {
  ...
  test("enforces rate limit for repeated execute triggers", async () => {
    ...
    const lastCall =
      audit.logEvent.mock.calls[audit.logEvent.mock.calls.length - 1];
    expect(lastCall[1]).toEqual(
      expect.objectContaining({
        payloadHash: expect.any(String),
        sourceIp: expect.anything(),
