...
describe('token-budget-allocation-store', () => {
  test('creates and retrieves a budget', () => {
    const res = tokenBudget.createBudget('org-a', { limitUSD: 100, period: 'monthly' });
  ...
  test('records usage and updates spend', () => {
    const res = tokenBudget.recordUsage('org-a', { model: 'gpt-4', inputTokens: 1000, outputTokens: 1000, userId: 'u1' });
  ...
  test('fires custom alert intervals and dispatches webhook events', () => {
    tokenBudget.updateBudget('org-a', { config: { alertIntervals: [50, 80, 100], alertCooldownMinutes: 0, webhookAlertsEnabled: true } });

    // Cross the 50% custom interval
    const r1 = tokenBudget.recordUsage('org-a', { model: 'gpt-4', inputTokens: 2000000, outputTokens: 0, userId: 'u1' });
    ...
    const r2 = tokenBudget.recordUsage('org-a', { model: 'gpt-4', inputTokens: 1000000, outputTokens: 0, userId: 'u1' });
    expect(r2.thresholdCrossed.type).toBe('soft_cap');
    expect(r2.thresholdCrossed.crossed).toBe(80);

    // Cross the 100% hard-stop boundary
    const r3 = tokenBudget.recordUsage('org-a', { model: 'gpt-4', inputTokens: 1000000, outputTokens: 0, userId: 'u1' });
    ...
    const lastCall = webhookEngine.dispatchEvent.mock.calls[webhookEngine.dispatchEvent.mock.calls.length - 1];
  ...
  test('auto-resets a budget when periodEnd has passed', () => {
    const create = tokenBudget.createBudget('org-roll', { limitUSD: 50, period: 'monthly', config: { autoResetEnabled: true } });
    ...
    const res = tokenBudget.recordUsage('org-roll', { model: 'gpt-4', inputTokens: 1000, outputTokens: 0 });
    expect(res.recorded).toBe(true);
  ...
  test('does not record against a disabled budget', () => {
    ...
    const res = tokenBudget.recordUsage('org-disabled', { model: 'gpt-4', inputTokens: 1000, outputTokens: 0 });
