...
const hsmMetrics = require(path.join(__dirname, '..', 'hsm-adapter', 'hsm-metrics.cjs'));
const SiemSecurityBroker = require(path.join(__dirname, '..', 'siem', 'siem-broker.cjs'));

const ALERTS_YML = path.join(__dirname, '..', '..', '..', 'monitoring', 'siem-alerts.rules.yml');
...
describe('SIEM Broker Prometheus Metrics Integration', () => {
  ...
  describe('hsm-metrics.cjs SIEM counter registration', () => {
    it('registers all 5 SIEM counters', () => {
      assert.ok(hsmMetrics.counters.siem_events_processed_total !== undefined, 'siem_events_processed_total registered');
      assert.ok(hsmMetrics.counters.siem_events_dropped_total !== undefined, 'siem_events_dropped_total registered');
      assert.ok(hsmMetrics.counters.siem_events_bypassed_total !== undefined, 'siem_events_bypassed_total registered');
      assert.ok(hsmMetrics.counters.siem_tokens_consumed_total !== undefined, 'siem_tokens_consumed_total registered');
      assert.ok(hsmMetrics.counters.siem_token_bucket_current !== undefined, 'siem_token_bucket_current registered');
    });
    ...
    it('META has help and type for all SIEM counters', () => {
      ...
      assert.ok(rendered.includes('# HELP siem_events_processed_total'), 'processed has HELP');
      assert.ok(rendered.includes('# TYPE siem_events_processed_total counter'), 'processed has TYPE');
      assert.ok(rendered.includes('# HELP siem_events_dropped_total'), 'dropped has HELP');
      assert.ok(rendered.includes('# TYPE siem_events_dropped_total counter'), 'dropped has TYPE');
      assert.ok(rendered.includes('# HELP siem_events_bypassed_total'), 'bypassed has HELP');
      assert.ok(rendered.includes('# TYPE siem_events_bypassed_total counter'), 'bypassed has TYPE');
      assert.ok(rendered.includes('# HELP siem_tokens_consumed_total'), 'tokens_consumed has HELP');
      assert.ok(rendered.includes('# TYPE siem_tokens_consumed_total counter'), 'tokens_consumed has TYPE');
      assert.ok(rendered.includes('# HELP siem_token_bucket_current'), 'bucket_current has HELP');
      assert.ok(rendered.includes('# TYPE siem_token_bucket_current gauge'), 'bucket_current is gauge type');
    });
  });
  ...
  describe('updateSiemMetrics(broker)', () => {
    ...
    it('syncs broker processed count into registry', () => {
      ...
      assert.ok(hsmMetrics.counters.siem_events_processed_total >= 2, 'processed should be >= 2');
    });
    ...
    it('syncs broker dropped count into registry', () => {
      ...
      hsmMetrics.updateSiemMetrics(broker);
      assert.strictEqual(hsmMetrics.counters.siem_events_dropped_total, 1, '1 event dropped');
    });

    it('syncs broker bypassed count into registry', () => {
      broker.logEvent({ siemSeverity: 'CRITICAL', siemCategory: 'bypass_test' });

      hsmMetrics.updateSiemMetrics(broker);
      assert.strictEqual(hsmMetrics.counters.siem_events_bypassed_total, 1, '1 CRITICAL bypassed');
    });
    ...
    it('syncs current token bucket capacity (gauge)', () => {
      ...
      hsmMetrics.updateSiemMetrics(broker);
      assert.strictEqual(hsmMetrics.counters.siem_token_bucket_current, 7, '7 tokens remaining');
    });

    it('handles null broker gracefully', () => {
      // Should not throw
      hsmMetrics.updateSiemMetrics(null);
      assert.strictEqual(hsmMetrics.counters.siem_events_processed_total, 0, 'no change with null broker');
    });
    ...
    it('handles broker without getMetrics gracefully', () => {
      ...
      assert.strictEqual(hsmMetrics.counters.siem_events_processed_total, 0, 'no change with fake broker');
    });

    it('reflects updated values in renderPrometheus output', () => {
      broker.logEvent({ siemSeverity: 'LOW', siemCategory: 'render_test' });
      broker.logEvent({ siemSeverity: 'CRITICAL', siemCategory: 'render_critical' });

      hsmMetrics.updateSiemMetrics(broker);
      const rendered = hsmMetrics.renderPrometheus();

      assert.ok(rendered.match(/siem_events_processed_total\s+\d+/), 'processed in Prometheus output');
      assert.ok(rendered.match(/siem_events_bypassed_total\s+1/), 'bypassed=1 in output');
      assert.ok(rendered.match(/siem_token_bucket_current\s+\d+/), 'bucket gauge in output');
    });
  });
  ...
  describe('siem-alerts.rules.yml structural validity', () => {
    it('file exists at monitoring/siem-alerts.rules.yml', () => {
      assert.ok(fs.existsSync(ALERTS_YML), `Alert rules file should exist at ${ALERTS_YML}`);
    });
    ...
    it('SiemCriticalEventDetected rule is P1 with for: 0s', () => {
      ...
      const doc = yaml.load(fs.readFileSync(ALERTS_YML, 'utf8'));
      const group = doc.groups.find(g => g.name === 'siem_broker_alerts');
      const rule = group.rules.find(r => r.alert === 'SiemCriticalEventDetected');
      assert.ok(rule, 'SiemCriticalEventDetected exists');
      assert.strictEqual(rule.labels.severity, 'critical', 'severity is critical');
      assert.strictEqual(rule.labels.priority, 'P1', 'priority is P1');
      assert.strictEqual(rule.for, '0s', 'fires immediately');
      assert.ok(rule.expr.includes('siem_events_bypassed_total'), 'references bypassed counter');
    });
    ...
    it('SiemHighDropRate rule is P1 with 5m window', () => {
      ...
      assert.strictEqual(rule.labels.severity, 'critical', 'severity is critical');
      assert.strictEqual(rule.labels.priority, 'P1', 'priority is P1');
      assert.ok(rule.expr.includes('siem_events_dropped_total'), 'references dropped counter');
    ...
    it('SiemTokenBucketExhaustion rule references gauge', () => {
      ...
      const doc = yaml.load(fs.readFileSync(ALERTS_YML, 'utf8'));
      const group = doc.groups.find(g => g.name === 'siem_broker_alerts');
      const rule = group.rules.find(r => r.alert === 'SiemTokenBucketExhaustion');
      assert.ok(rule, 'SiemTokenBucketExhaustion exists');
      assert.ok(rule.expr.includes('siem_token_bucket_current'), 'references token bucket gauge');
      assert.strictEqual(rule.labels.severity, 'warning', 'severity is warning');
    });
    ...
    it('SiemPipelineStall rule references processed counter', () => {
      ...
      assert.ok(rule.expr.includes('siem_events_processed_total'), 'references processed counter');
  ...
  describe('end-to-end broker → metrics → Prometheus output', () => {
    ...
    it('full flow: broker events appear in Prometheus exposition format', () => {
      ...
      broker.logEvent({ siemSeverity: 'CRITICAL', siemCategory: 'e2e_critical' });
      ...
      const processedLine = lines.find(l => l.startsWith('siem_events_processed_total ') && !l.startsWith('#'));
      ...
      const bypassedLine = lines.find(l => l.startsWith('siem_events_bypassed_total ') && !l.startsWith('#'));
      ...
      // Find the gauge line
      const gaugeLine = lines.find(l => l.startsWith('siem_token_bucket_current ') && !l.startsWith('#'));
