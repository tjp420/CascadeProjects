...
describe("Audit Log Auto-Healing Worker", () => {
  ...
  describe("healChain", () => {
    ...
    it("should quarantine tampered entry and re-link remaining", () => {
      ...
      const e2 = auditLogger.log({
        orgId: "org-heal",
        actorId: "u2",
        actorEmail: "u2@org-heal.com",
        action: "UPDATE",
        entity: "test",
        entityId: "1",
      });
      const e3 = auditLogger.log({
        orgId: "org-heal",
        actorId: "u3",
        actorEmail: "u3@org-heal.com",
        action: "DELETE",
        entity: "test",
        entityId: "1",
      });
  ...
  describe("getQuarantine", () => {
    it("should return all quarantine entries when no orgId filter", () => {
      ...
      assert.ok(
        quarantineA.entries.length >= 1,
        "org-a should have quarantined entries",
      );
      assert.ok(
        quarantineB.entries.length >= 1,
        "org-b should have quarantined entries",
      );
    });
