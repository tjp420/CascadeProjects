...
const EXPORTER_PATH = path.resolve(process.cwd(), 'server', 'lib', 'siem-exporter.cjs');
const TRANSPORT_PATH = path.resolve(process.cwd(), 'server', 'middleware', 'transports', 'siem-transport.cjs');
...
describe('SIEM Broker Transport Connections', () => {
  ...
  describe('siem-exporter connectBroker', () => {
    ...
    it('does not enqueue CRITICAL events (they go to winston_stream)', () => {
      ...
      const queue = exporter._debug.getQueue();
      assert.strictEqual(queue.length, 0, 'CRITICAL events should not be batched');
    });
  ...
  describe('siem-transport connectBroker', () => {
    it('forwards transport_winston_stream events to Winston logger', () => {
      ...
      assert.strictEqual(logCalls[0].level, 'error', 'CRITICAL should map to error level');
    ...
    it('maps HIGH severity to warn level', () => {
      ...
      assert.strictEqual(logCalls[0].level, 'warn', 'HIGH should map to warn level');
    });
