...
describe('GET /api/audit/pii/sync-history', () => {
  ...
  before(() => { auditLogger = require('../../lib/audit-logger.cjs'); });
  ...
  beforeEach(() => {
    ...
    auditLogger.log({
      ...
      entity: 'pii_policies',
      entityId: 'org-source',
      metadata: { sourceOrgId: 'org-source', targetCount: 3, mode: 'merge', totalCloned: 6, totalSkipped: 2, totalRemoved: 0 },
    });
    auditLogger.log({
      orgId: 'admin@org-test.com',
      actorId: 'admin@org-test.com',
      ...
      entity: 'pii_policies',
      entityId: 'org-source',
      metadata: { sourceOrgId: 'org-source', targetCount: 2, mode: 'replace', totalCloned: 4, totalSkipped: 0, totalRemoved: 2 },
    });
    auditLogger.log({
      orgId: 'admin@org-test.com',
      ...
      entity: 'pii_policies',
      entityId: 'org-other',
      metadata: { sourceOrgId: 'org-other', targetCount: 1, mode: 'merge', totalCloned: 2, totalSkipped: 1, totalRemoved: 0 },
    });
  ...
  after(() => {
    try { delete process.env.AUDIT_LOG_PATH; delete process.env.PII_POLICY_PATH; delete process.env.AUDIT_LOG_SCRUB_PII; if (_tempDir && fs2.existsSync(_tempDir)) fs2.rmSync(_tempDir, { recursive: true, force: true }); } catch (e) {}
  });

  it('returns aggregated sync history data', async () => {
    const res = await request(adminApp).get('/api/audit/pii/sync-history?days=30');
  ...
  it('returns top actors list', async () => {
    ...
    const admin1 = res.body.actors.find((a) => a.actor === 'admin@org-test.com');
    assert.ok(admin1);
  ...
  it('returns top source orgs list', async () => {
    ...
    const srcOrg = res.body.sourceOrgs.find((s) => s.sourceOrg === 'org-source');
    assert.ok(srcOrg);
  ...
  it('returns recent sync events', async () => {
    const res = await request(adminApp).get('/api/audit/pii/sync-history?limit=2');
  ...
  it('returns empty data when no sync events exist', async () => {
    ...
    const res = await request(adminApp).get('/api/audit/pii/sync-history?days=7');
  ...
  it('respects days parameter for time filtering', async () => {
    ...
    const res = await request(adminApp).get('/api/audit/pii/sync-history?days=0');
