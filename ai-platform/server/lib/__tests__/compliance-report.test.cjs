...
describe('Compliance Report Exporter', () => {
  ...
  describe('generateComplianceReport()', () => {
    it('should return report with reportId, generatedAt, frameworks', () => {
      ...
      assert.ok(report.reportId.startsWith('rep_'), 'reportId should start with rep_');
      assert.ok(report.generatedAt, 'should have generatedAt');
      assert.ok(Array.isArray(report.frameworks), 'should have frameworks array');
    ...
    it('should include per-org chainIntegrity with valid, totalEntries, verifiedEntries', () => {
      ...
      assert.strictEqual(typeof orgProfile.chainIntegrity.totalEntries, 'number');
      assert.strictEqual(typeof orgProfile.chainIntegrity.verifiedEntries, 'number');
    });

    it('should include per-org retentionPolicy with retentionDays, maxEntries, archive', () => {
      writePolicyDirectly(tmpPolicyPath, 'test-org', { retentionDays: 60, maxEntries: 5000, archive: true });
      auditPolicyStore._resetCache();
      const report = auditLogger.generateComplianceReport('test-org');
      const orgProfile = report.orgs.find((o) => o.orgId === 'test-org');
    ...
    it('should include per-org retentionStats with total, purgeableCount', () => {
      ...
      assert.strictEqual(typeof orgProfile.retentionStats.purgeableCount, 'number');
    });
    ...
    it('should include global autoPurgeStats with totalSweeps, totalPurged, totalArchived', () => {
      ...
      assert.strictEqual(typeof report.global.autoPurgeStats.totalSweeps, 'number');
      assert.strictEqual(typeof report.global.autoPurgeStats.totalPurged, 'number');
      assert.strictEqual(typeof report.global.autoPurgeStats.totalArchived, 'number');
    });
    ...
    it('should include global healStats with totalRuns, totalQuarantined, totalRelinked', () => {
      ...
      assert.strictEqual(typeof report.global.healStats.totalQuarantined, 'number');
      assert.strictEqual(typeof report.global.healStats.totalRelinked, 'number');
    });
    ...
    it('should include global keyRotation (or error if unavailable)', () => {
      ...
      const hasFields = report.global.keyRotation.hasActive !== undefined ||
                        report.global.keyRotation.error !== undefined;
      assert.ok(hasFields, 'keyRotation should have either status fields or error');
    });
    ...
    it('should handle empty store (no orgs) with valid structure', () => {
      ...
      assert.ok(report.orgs.length >= 1, 'should include at least the caller org');
    });

    it('should handle missing key-rotation-store gracefully', () => {
      // The function should not crash if key-rotation-store is unavailable
      const report = auditLogger.generateComplianceReport('test-org');
      assert.ok(report.global.keyRotation, 'should still have keyRotation object');
    });
    ...
    it('should include reportId that is unique per generation', () => {
      ...
      assert.notStrictEqual(report1.reportId, report2.reportId, 'reportIds should be unique');
    });
    ...
    it('should audit-log the report generation with action compliance_report_generated', () => {
      ...
      assert.ok(logEntries.length >= 1, 'should have audit-logged the report generation');
      assert.strictEqual(logEntries[0].entity, 'compliance_report');
      assert.ok(logEntries[0].entityId.startsWith('rep_'), 'entityId should be the reportId');
    });
    ...
    it('should not include raw PII in the report (uses scrubbed data)', () => {
      ...
      assert.ok(!reportStr.includes('user@example.com'), 'should not contain raw PII');
    });

    it('should place caller org first in the orgs array', () => {
      const now = new Date();
      writeChainedEntries(auditLogger, tmpLogPath, 'org-a', [now.toISOString()]);
      writeChainedEntries(auditLogger, tmpLogPath, 'org-b', [now.toISOString()]);
      writeChainedEntries(auditLogger, tmpLogPath, 'caller-org', [now.toISOString()]);
      const report = auditLogger.generateComplianceReport('caller-org');
      assert.strictEqual(report.orgs[0].orgId, 'caller-org', 'caller org should be first');
    });
  });
  ...
  describe('complianceReportToCsv()', () => {
    it('should produce CSV with report ID and generated timestamp in header', () => {
      ...
      assert.ok(csv.includes(report.generatedAt), 'CSV should contain generatedAt');
    });
    ...
    it('should include SECTION 1 and SECTION 2 headers', () => {
      ...
      assert.ok(csv.includes('GLOBAL PLATFORM SECURITY CONTROLS'), 'should have global controls section');
      assert.ok(csv.includes('CRYPTOGRAPHIC ATTESTATION MATRIX'), 'should have attestation matrix section');
    });
    ...
    it('should include PII scrubbing status in CSV', () => {
      ...
      assert.ok(csv.includes('ENABLED') || csv.includes('DISABLED'), 'should show PII status');
    });
  });
});
