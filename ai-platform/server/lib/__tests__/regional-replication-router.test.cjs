...
const TEST_ZONES = [
  {
    id: "us-east",
    endpoint: "https://us.example.com/api/replication",
    apiKeyEnv: "TEST_US_KEY",
  },
  {
    id: "eu-west",
    endpoint: "https://eu.example.com/api/replication",
    apiKeyEnv: "TEST_EU_KEY",
  },
  {
    id: "ap-southeast",
    endpoint: "https://ap.example.com/api/replication",
    apiKeyEnv: "TEST_AP_KEY",
  },
];
...
describe("regional-replication-router", () => {
  ...
  describe("validatePayload", () => {
    it("accepts a valid payload", () => {
      assert.doesNotThrow(() =>
        validatePayload({ type: "scan", data: { id: 1 } }),
      );
    });
  ...
  describe("sync", () => {
    it("syncs a payload to a single zone successfully", async () => {
      ...
      const result = await router.sync("us-east", {
        type: "scan",
        data: { id: "test-1" },
      });
    ...
    it("throws on invalid payload", async () => {
      ...
      await assert.rejects(router.sync("us-east", null), /non-null object/);
    });
    ...
    it("succeeds without endpoint (local dev mode)", async () => {
      ...
      const result = await router.sync("local", {
        type: "scan",
        data: { id: "local-1" },
      });
      assert.strictEqual(result.success, true);
  ...
  describe("syncAll", () => {
    it("syncs a payload to all zones", async () => {
      ...
      const results = await router.syncAll({
        type: "scan",
        data: { id: "all-1" },
      });
  ...
  describe("conflict detection and resolution", () => {
    ...
    it("resolves a conflict with latest-wins strategy", () => {
      ...
      const result = router.resolveConflict(
        conflictId,
        CONFLICT_STRATEGIES.LATEST_WINS,
      );
