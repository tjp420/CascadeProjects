...
const { resolveCorsOptions, isAllowedOrigin, resolveAllowedOrigins, parseOriginList } = require('../cors-config.cjs');

test('parseOriginList splits comma-separated origins', () => {
    assert.deepStrictEqual(parseOriginList('https://a.com,https://b.com'), ['https://a.com', 'https://b.com']);
...
test('isAllowedOrigin enforces explicit list in production', () => {
    ...
    process.env.ALLOWED_ORIGIN = 'https://simplebeacon.ai,https://www.simplebeacon.ai';
...
test('isAllowedOrigin allows pages.dev preview origins in production', () => {
    const origEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = 'production';
    assert.strictEqual(isAllowedOrigin('https://abc123.simplebeacon.pages.dev'), true);
...
test('isAllowedOrigin allows onrender.com origins in production', () => {
    const origEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = 'production';
    assert.strictEqual(isAllowedOrigin('https://simplebeacon.onrender.com'), true);
    process.env.NODE_ENV = origEnv;
});
