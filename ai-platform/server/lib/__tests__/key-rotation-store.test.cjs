...
const STORE_PATH = path.resolve(process.cwd(), 'server', 'lib', 'key-rotation-store.cjs');
...
describe('Zero-Downtime Master Key Rotation Store', () => {
  ...
  describe('key ring initialization', () => {
    ...
    it('should not overwrite active key if already set', () => {
      ...
      assert.deepStrictEqual(store.getActiveKeyBuffer(), key1, 'Should not overwrite');
    });
  });
  ...
  describe('rotateKey', () => {
    ...
    it('should set a new active key that differs from the old one', () => {
      ...
      assert.notDeepStrictEqual(newKey, oldKey, 'New active key must differ from old');
    });

    it('should throw TypeError for empty key', () => {
      assert.throws(
        () => store.rotateKey(''),
        { name: 'TypeError', message: /high-entropy/ }
      );
    });

    it('should throw TypeError for null key', () => {
      assert.throws(
        () => store.rotateKey(null),
        { name: 'TypeError', message: /high-entropy/ }
      );
    });
    ...
    it('should throw TypeError for short string key (<32 chars)', () => {
      assert.throws(
        () => store.rotateKey('short-key'),
        { name: 'TypeError', message: /32 characters/ }
      );
    });

    it('should throw TypeError for short Buffer key (<32 bytes)', () => {
      assert.throws(
        () => store.rotateKey(crypto.randomBytes(16)),
        { name: 'TypeError', message: /32 bytes/ }
      );
    });
    ...
    it('should accept a Buffer key', () => {
      ...
      store.rotateKey(buf);
      assert.ok(store.getActiveKeyBuffer(), 'Should have active key after Buffer rotation');
      assert.strictEqual(store.getActiveKeyBuffer().length, 32);
    });
  ...
  describe('getDecryptionKeys', () => {
    ...
    it('should return only active key after grace window expires', () => {
      ...
      const keys = store.getDecryptionKeys();
      assert.strictEqual(keys.length, 1, 'Should have only active key after grace expiry');
    });
  });
  ...
  describe('purgeExpiredKeys', () => {
    ...
    it('should purge previous key after grace expires', () => {
      ...
      const status = store.getRotationStatus();
      assert.strictEqual(status.hasPrevious, false, 'Previous key should be purged');
    });
    ...
    it('should return false when there is no previous key', () => {
      const purged = store.purgeExpiredKeys();
      assert.strictEqual(purged, false, 'Should return false when no previous key');
    });
  });
  ...
  describe('getRotationStatus', () => {
    ...
    it('should return fingerprints (not raw keys) after rotation', () => {
      ...
      assert.notStrictEqual(status.activeFingerprint, status.previousFingerprint);
    ...
    it('should report graceExpired correctly', () => {
      ...
      const status = store.getRotationStatus();
      assert.strictEqual(status.graceExpired, true, 'Should report grace expired');
      delete process.env.KEY_ROTATION_GRACE_MS;
    });
  });
  ...
  describe('reKeyValue / reKeyStore', () => {
    ...
    beforeEach(() => {
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'test-encryption-key-for-rotation';
      store = reloadModule(STORE_PATH);
    ...
    it('should re-encrypt a value from old key to new key', () => {
      ...
      const result = store.reKeyValue(encrypted, cryptoUtils.decrypt, cryptoUtils.encrypt);
      assert.strictEqual(result.migrated, true);
      assert.ok(result.newValue, 'Should produce a new encrypted value');
      assert.notStrictEqual(result.newValue, encrypted, 'Re-encryption should produce different ciphertext (random IV)');

      // Verify the re-encrypted value decrypts correctly
      const decrypted = cryptoUtils.decrypt(result.newValue);
      assert.strictEqual(decrypted, plaintext, 'Re-encrypted value should decrypt to original');
    });

    it('should return migrated=false for null input', () => {
      const result = store.reKeyValue(null, cryptoUtils.decrypt, cryptoUtils.encrypt);
      assert.strictEqual(result.migrated, false);
      assert.strictEqual(result.newValue, null);
    });

    it('should return migrated=false for non-string input', () => {
      const result = store.reKeyValue(123, cryptoUtils.decrypt, cryptoUtils.encrypt);
      assert.strictEqual(result.migrated, false);
    });
    ...
    it('should re-key an entire store object', () => {
      ...
      const result = store.reKeyStore(
        ...
        cryptoUtils.encrypt,
        (entry) => entry.data,
        (entry, newVal) => { entry.data = newVal; }
      );
    ...
    it('should handle empty store', () => {
      const result = store.reKeyStore({}, cryptoUtils.decrypt, cryptoUtils.encrypt);
      assert.strictEqual(result.migrated, 0);
      assert.strictEqual(result.skipped, 0);
    });

    it('should handle null store', () => {
      const result = store.reKeyStore(null, cryptoUtils.decrypt, cryptoUtils.encrypt);
      assert.strictEqual(result.migrated, 0);
    });
  });
  ...
  describe('integration with crypto-utils.cjs', () => {
    ...
    it('should decrypt data encrypted with previous key after rotation (continuous decryption)', () => {
      ...
      const oldKey = crypto.createHash('sha256').update('integration-test-key-123456').digest();
      store._reset(oldKey);
      ...
      process.env.SIMPLEBEACON_ENCRYPTION_KEY = 'new-master-key-after-rotation-1234567890';
      cryptoUtils = reloadModule(CRYPTO_UTILS_PATH);
      ...
      const decKeys = store.getDecryptionKeys();
      assert.ok(decKeys.length >= 2, 'Should have active + previous keys for fallback');
    });
    ...
    it('should support encrypt/decrypt round-trip after rotation via re-keying', () => {
      ...
      // Verify round-trip
      const decrypted = cryptoUtils.decrypt(result.newValue);
      assert.strictEqual(decrypted, plaintext, 'Re-keyed value should decrypt correctly');
    });
    ...
    it('should expose key fingerprints (not raw keys) in rotation status', () => {
      ...
      assert.notStrictEqual(status.activeFingerprint, store.getActiveKeyBuffer().toString('hex'));
      assert.ok(status.activeFingerprint.length < 64, 'Fingerprint should be truncated, not full hash');
    });
  });
});
