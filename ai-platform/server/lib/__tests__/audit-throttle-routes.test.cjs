...
describe("D-01: adminThrottle exclusion on non-admin audit routes", () => {
  ...
  for (const route of NON_ADMIN_ROUTES) {
    it(`does NOT throttle ${route.method.toUpperCase()} ${route.path}`, async () => {
      ...
      assert.strictEqual(
        throttleCallCount,
        0,
        `adminThrottle was called for non-admin route ${route.path}`,
      );
    });
  }
  ...
  it("DOES throttle admin route GET /api/audit/telemetry", async () => {
    ...
    assert.strictEqual(
      throttleCallCount,
      1,
      "adminThrottle was NOT called for admin route /api/audit/telemetry",
    );
  });
  ...
  it("DOES throttle admin route GET /api/audit/key/status", async () => {
    ...
    assert.strictEqual(
      throttleCallCount,
      1,
      "adminThrottle was NOT called for admin route /api/audit/key/status",
    );
  });
  ...
  it("does NOT return 429 for non-admin routes even when throttle would block", async () => {
    ...
    assert.strictEqual(
      throttleCallCount,
      0,
      "adminThrottle was called for non-admin /log even though it should be excluded",
    );
  });
});
