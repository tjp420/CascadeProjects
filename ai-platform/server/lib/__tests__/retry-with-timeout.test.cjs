...
const { retryWithTimeout } = require(path.resolve(__dirname, '..', 'retry-with-timeout.cjs'));

describe('retryWithTimeout', () => {
  test('resolves immediately when operation succeeds', async () => {
    const result = await retryWithTimeout(() => Promise.resolve(42), { retries: 0, timeoutMs: 100 });
    expect(result).toBe(42);
  });

  test('rejects with timeout error after retries exhausted for hanging operation', async () => {
    const hang = () => new Promise(() => {});
    await expect(retryWithTimeout(hang, { retries: 1, timeoutMs: 20, baseDelayMs: 5, maxDelayMs: 10, jitter: 0 }))
  ...
  test('retries on rejection and eventually succeeds', async () => {
    ...
    const result = await retryWithTimeout(flaky, { retries: 2, timeoutMs: 100 });
  ...
  test('times out on first attempt then succeeds on second attempt', async () => {
    ...
    const result = await retryWithTimeout(op, { retries: 2, timeoutMs: 30, baseDelayMs: 5, maxDelayMs: 10, jitter: 0 });
