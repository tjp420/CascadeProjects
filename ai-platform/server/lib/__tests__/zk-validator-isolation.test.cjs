...
const {
  CryptoPolicyEngine,
} = require("../hsm-adapter/crypto-policy-engine.cjs");
...
describe("Track 125: ZK Validator Isolation Integration", () => {
  ...
  describe("Validator wrapping integration", () => {
    const validators = [
      {
        file: "zk-energy-claim-validator.cjs",
      ...
      {
        file: "zk-biometric-claim-validator.cjs",
      ...
      {
        file: "zk-neural-claim-validator.cjs",
      ...
      {
        file: "zk-lookup-claim-validator.cjs",
      ...
      {
        file: "zk-storage-claim-validator.cjs",
      ...
      {
        file: "zk-authentication-claim-validator.cjs",
      ...
      {
        file: "zk-drone-claim-validator.cjs",
      ...
      {
        file: "zk-genomic-claim-validator.cjs",
      ...
      {
        file: "zk-insurance-claim-validator.cjs",
      ...
      {
        file: "zk-quantum-claim-validator.cjs",
  ...
  describe("Energy validator detailed integration", () => {
    test("MIG-01: energy validator with tenant governance resolves per-tenant policy", () => {
      ...
      } = require("../hsm-adapter/zk-energy-claim-validator.cjs");
      const engine = new CryptoPolicyEngine();

      const mockHub = {
        getPool: () => ({ id: "p1" }),
        markEnergyClaimVerified: () => {},
      };
      ...
      expect(() =>
        validator.validateTenant("../bad", { poolId: "p1" }),
      ).toThrow("invalid tenant ID");
      expect(
        hsmMetrics.getMetrics().hsm_zk_tenant_isolation_violation_total,
      ).toBe(1);
    });
    ...
    test("MIG-01b: energy validator tracks metrics on verification failure", () => {
      ...
      expect(() =>
        validator.validateTenant("tenant-1", { poolId: "p1" }),
      ).toThrow();
  ...
  describe("Biometric validator detailed integration", () => {
    test("MIG-02: biometric validator can be wrapped", () => {
      ...
      } = require("../hsm-adapter/zk-biometric-claim-validator.cjs");
      const engine = new CryptoPolicyEngine();
  ...
  describe("Neural validator detailed integration", () => {
    test("MIG-03: neural validator can be wrapped", () => {
      const {
        ZkNeuralClaimValidator,
      } = require("../hsm-adapter/zk-neural-claim-validator.cjs");
      const engine = new CryptoPolicyEngine();
  ...
  describe("Policy engine integration", () => {
    ...
    test("POL: per-tenant policy isolation works", () => {
      ...
      expect(
        customPolicy.energyGating.requireClearingCommitteeAttestation,
      ).toBe(defaultPolicy.energyGating.requireClearingCommitteeAttestation);
    });
  });
  ...
  describe("Production hub wiring", () => {
    test("WIRE-01: lookup hub uses governed validateTenant by default", () => {
      ...
      } = require("../hsm-adapter/pqc-homomorphic-lookup-gating-hub.cjs");
      ...
      ).toThrow("zk_isolation_violation");
      expect(
        hsmMetrics.getMetrics().hsm_zk_tenant_isolation_violation_total,
      ).toBe(1);
    });
  ...
  describe("Backward compatibility", () => {
    test("BC-01: validators still work without tenant governance (original method)", () => {
      ...
      const validator = new ZkEnergyClaimValidator({
        policy: { requireClearingCommitteeAttestation: false },
      });
    ...
    test("BC-02: existing governed validators (ring, accumulator) still work", () => {
      const {
        ZkRingClaimValidator,
      } = require("../hsm-adapter/zk-ring-claim-validator.cjs");
      const {
        ZkAccumulatorClaimValidator,
      } = require("../hsm-adapter/zk-accumulator-claim-validator.cjs");
      const engine = new CryptoPolicyEngine();
