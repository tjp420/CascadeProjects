...
jest.mock('../../lib/admin-throttle.cjs', () => ({ middleware: (_req, _res, next) => next() }));
...
describe('mounted audit program routes', () => {
  test('export-tier requires authentication and classifies a scan', async () => {
    ...
    const unauthenticated = await request(app).post('/api/audit/export-tier').send({});
  ...
  test('export-pdf refuses unsigned output when no signing key is configured', async () => {
    ...
    const response = await request(app)
      ...
      .set('Authorization', 'Bearer test')
      .send({ results: { simplebeacon: { gate: { pass: true }, issueCount: 0 }, codebase: { summary: { codeFilesAnalyzed: 1 } } } });
  ...
  test('booking listing uses the configured authentication middleware', async () => {
    ...
    const response = await request(app).get('/api/audit-bookings').set('Authorization', 'Bearer test');
    expect(response.status).toBe(200);
