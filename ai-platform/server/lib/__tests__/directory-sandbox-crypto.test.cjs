...
describe("Directory Sandbox Crypto Suite", () => {
  ...
  before(() => {
    ...
    process.env.SIMPLEBEACON_ENCRYPTION_KEY =
      "test-master-key-for-directory-crypto";
    jest.resetModules();
  ...
  describe("deriveDirectoryKey", () => {
    ...
    it("should be deterministic for same orgId + directory", () => {
      ...
      assert.deepStrictEqual(
        key1,
        key2,
        "Same org+dir must yield identical keys",
      );
    });
    ...
    it("should produce unique keys for different directories within same org", () => {
      ...
      assert.notDeepStrictEqual(
        keyA,
        keyB,
        "Different directories must yield unique keys",
      );
    });
    ...
    it("should produce unique keys for same directory across different orgs", () => {
      ...
      assert.notDeepStrictEqual(
        keyOrgA,
        keyOrgB,
        "Different orgs must yield unique keys",
      );
    });

    it("should throw TypeError for empty orgId", () => {
      assert.throws(() => cryptoUtils.deriveDirectoryKey("", DIR_A), {
        name: "TypeError",
        message: /orgId/,
      });
    });
    ...
    it("should throw TypeError for non-string orgId", () => {
      assert.throws(() => cryptoUtils.deriveDirectoryKey(null, DIR_A), {
        name: "TypeError",
        message: /orgId/,
      });
    });
    ...
    it("should throw TypeError for empty directory", () => {
      assert.throws(() => cryptoUtils.deriveDirectoryKey(ORG_A, ""), {
        name: "TypeError",
        message: /directory/,
      });
    });
    ...
    it("should throw TypeError for non-string directory", () => {
      assert.throws(() => cryptoUtils.deriveDirectoryKey(ORG_A, null), {
        name: "TypeError",
        message: /directory/,
      });
    });
  ...
  describe("directoryKeyFingerprint", () => {
    ...
    it("should be deterministic for same orgId + directory", () => {
      ...
      assert.strictEqual(
        fp1,
        fp2,
        "Same org+dir must yield identical fingerprints",
      );
    });
    ...
    it("should produce unique fingerprints for different directories", () => {
      ...
      assert.notStrictEqual(
        fpA,
        fpB,
        "Different directories must yield unique fingerprints",
      );
    });
    ...
    it("should produce unique fingerprints for different orgs", () => {
      ...
      assert.notStrictEqual(
        fpOrgA,
        fpOrgB,
        "Different orgs must yield unique fingerprints",
      );
    });
    ...
    it("should match the SHA-256 hash of the derived key", () => {
      ...
      assert.strictEqual(
        actualFp,
        expectedFp,
        "Fingerprint should be SHA-256 of derived key",
      );
    });
  });
  ...
  describe("encryptForDirectory / decryptForDirectory", () => {
    it("should encrypt and decrypt successfully with correct org + directory", () => {
      ...
      const ciphertext = cryptoUtils.encryptForDirectory(
        plaintext,
        ORG_A,
        DIR_A,
      );
      ...
      const decrypted = cryptoUtils.decryptForDirectory(
        ciphertext,
        ORG_A,
        DIR_A,
      );
    ...
    it("should produce ciphertext with the directory prefix", () => {
      ...
      assert.ok(
        ciphertext.startsWith("enc:sb:dir:"),
        "Ciphertext must have enc:sb:dir: prefix",
      );
    });
    ...
    it("should produce different ciphertexts for same plaintext (random IV)", () => {
      ...
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ct1, ORG_A, DIR_A),
        plaintext,
      );
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ct2, ORG_A, DIR_A),
        plaintext,
      );
    });
    ...
    it("should fail to decrypt with wrong directory (cross-directory isolation)", () => {
      ...
      assert.strictEqual(
        decrypted,
        "",
        "Cross-directory decryption must return empty string",
      );
    });
    ...
    it("should fail to decrypt with wrong org (cross-tenant isolation)", () => {
      ...
      assert.strictEqual(
        decrypted,
        "",
        "Cross-tenant decryption must return empty string",
      );
    });
    ...
    it("should handle non-string plaintext by converting to string", () => {
      ...
      assert.ok(
        ciphertext.startsWith("enc:sb:dir:"),
        "Numeric plaintext should be converted and encrypted",
      );
      const decrypted = cryptoUtils.decryptForDirectory(
        ciphertext,
        ORG_A,
      ...
      assert.strictEqual(
        decrypted,
        "12345",
        "Decrypted text should be string representation of number",
      );
    });
    ...
    it("should return empty string for null stored value", () => {
      ...
      assert.strictEqual(
        result,
        "",
        "Null stored value should return empty string",
      );
    });

    it("should return empty string for non-directory-encrypted value", () => {
      const result = cryptoUtils.decryptForDirectory(
        "plain text",
        ORG_A,
        DIR_A,
      );
      assert.strictEqual(
        result,
        "",
        "Non-encrypted value should return empty string",
      );
    });
    ...
    it("should return empty string for malformed ciphertext (wrong number of parts)", () => {
      ...
      assert.strictEqual(
        result,
        "",
        "Malformed ciphertext should return empty string",
      );
    });
    ...
    it("should return empty string for tampered ciphertext", () => {
      ...
      assert.strictEqual(
        result,
        "",
        "Tampered ciphertext should fail AES-GCM auth and return empty string",
      );
    });
    ...
    it("should handle unicode plaintext correctly", () => {
      ...
      assert.strictEqual(
        decrypted,
        plaintext,
        "Unicode plaintext should round-trip correctly",
      );
    });
    ...
    it("should handle long plaintext correctly", () => {
      ...
      assert.strictEqual(
        decrypted,
        plaintext,
        "Long plaintext should round-trip correctly",
      );
    });
  });
  ...
  describe("isDirectoryEncrypted", () => {
    ...
    it("should return false for plain text string", () => {
      assert.strictEqual(
        cryptoUtils.isDirectoryEncrypted("plain text string"),
        false,
      );
    });
    ...
    it("should NOT match standard encrypted values (enc: prefix)", () => {
      ...
      assert.strictEqual(
        cryptoUtils.isDirectoryEncrypted(standardEncrypted),
        false,
      );
    });
  });
  ...
  describe("cross-function integration", () => {
    it("should maintain isolation across multiple encrypt/decrypt cycles", () => {
      ...
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ctA, ORG_A, DIR_A),
        dataA,
      );
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ctB, ORG_A, DIR_B),
        dataB,
      );
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ctA, ORG_A, DIR_B),
        "",
      );
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ctB, ORG_A, DIR_A),
        "",
      );
    });
    ...
    it("should produce fingerprints that match derived keys used for encryption", () => {
      ...
      assert.strictEqual(
        cryptoUtils.decryptForDirectory(ciphertext, ORG_A, DIR_A),
        plaintext,
      );
    });
  });
