...
const AUTHORIZE_PATH = path.resolve(
  process.cwd(),
  "server",
  "middleware",
  "authorize.cjs",
...
describe("Real-Time Log Stream Interdiction Engine", () => {
  ...
  describe("recordStreamFailure()", () => {
    it("should record a stream failure and return count", () => {
      const result = authorize.recordStreamFailure(
        "test-key-1",
        "chain_verification",
        "broken link",
      );
    ...
    it("should track different keys independently", () => {
      ...
      const resultB = authorize.recordStreamFailure(
        "key-B",
        "chain_verification",
        "fail 1",
      );
      ...
      assert.strictEqual(
        authorize.checkInterdiction("key-A").interdicted,
        false,
      );
      assert.strictEqual(
        authorize.checkInterdiction("key-B").interdicted,
        false,
      );
    });
  });
