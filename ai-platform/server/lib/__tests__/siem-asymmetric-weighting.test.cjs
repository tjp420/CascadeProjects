...
describe("SiemSecurityBroker asymmetric node weighting", () => {
  describe("enableDistributedSync with weight", () => {
    ...
    it("W5: rejects non-numeric weight (defaults to 1)", () => {
      ...
      assert.strictEqual(
        broker._nodeWeight,
        1,
        "string weight should default to 1",
      );
      broker.close();
    });
    ...
    it("W6: initial fair share is still maxTokens/nodeCount before peer sync", () => {
      ...
      assert.strictEqual(
        broker._fairShare,
        25,
        "initial fair share should be 100/4",
      );
      broker.close();
    });
  });
  ...
  describe("SIEM_BUCKET_SYNC weight broadcast", () => {
    it("W7: broadcast message includes weight property", () => {
      ...
      assert.strictEqual(
        sentMsg.weight,
        4,
        "broadcast should include weight=4",
      );
      broker.close();
    });
    ...
    it("W8: broadcast weight defaults to 1 when not specified", () => {
      ...
      assert.strictEqual(
        sentMsg.weight,
        1,
        "default broadcast weight should be 1",
      );
      broker.close();
    });
  });
  ...
  describe("handlePeerSync with weight", () => {
    it("W9: stores peer weight from sync message", () => {
      ...
      assert.strictEqual(
        state.peers["n2"].weight,
        5,
        "peer weight should be stored",
      );
      broker.close();
    });
    ...
    it("W10: defaults peer weight to 1 when not in message", () => {
      ...
      assert.strictEqual(
        state.peers["n2"].weight,
        1,
        "missing weight should default to 1",
      );
      broker.close();
    });
  });
  ...
  describe("weighted fair share recalculation", () => {
    it("W11: high-weight node gets proportionally larger fair share", () => {
      ...
      assert.strictEqual(
        broker._fairShare,
        66,
        "high-weight core should get 66 tokens",
      );
      broker.close();
    });
    ...
    it("W12: low-weight edge node gets proportionally smaller fair share", () => {
      ...
      assert.strictEqual(
        broker._fairShare,
        16,
        "low-weight edge should get 16 tokens",
      );
      broker.close();
    });
    ...
    it("W13: homogeneous weights (all 1) produce same result as maxTokens/N", () => {
      ...
      assert.strictEqual(
        broker._fairShare,
        25,
        "homogeneous weights should produce 25",
      );
      broker.close();
    });
    ...
    it("W14: reserve floor scales with weighted fair share", () => {
      ...
      assert.strictEqual(
        broker._reserveFloor,
        13,
        "reserve floor should be 20% of 66",
      );
      broker.close();
    });
    ...
    it("W15: tokens are capped when fair share decreases on weight update", () => {
      ...
      assert.strictEqual(
        broker.tokens,
        33,
        "tokens should not change when fair share increases",
      );
      broker.close();
    });
  });
  ...
  describe("weighted borrowing preference", () => {
    it("W16: _borrowFromPeers sorts peers by weight descending", () => {
      ...
      if (core1Idx >= 0 && core2Idx >= 0) {
        assert.ok(
          core1Idx < core2Idx,
          "core-1 (weight 4) should be requested before core-2 (weight 3)",
        );
      }
      if (core2Idx >= 0 && edge2Idx >= 0) {
        assert.ok(
          core2Idx < edge2Idx,
          "core-2 (weight 3) should be requested before edge-2 (weight 1)",
        );
      }
      broker.close();
    });
  ...
  describe("getDistributedState with weight info", () => {
    it("W17: state includes nodeWeight and clusterWeight", () => {
      ...
      assert.strictEqual(
        state.nodeWeight,
        5,
        "state should include local node weight",
      );
      assert.strictEqual(
        state.clusterWeight,
        8,
        "cluster weight should be 5+3",
      );
      broker.close();
    });
  ...
  describe("weighted cluster convergence", () => {
    it("W20: 3-node weighted cluster: core(4) + edge(1) + edge(1), total capacity = maxTokens", () => {
      ...
      assert.strictEqual(
        coreState.fairShare,
        40,
        "core should have fair share of 40",
      );
      // Edge: 60 * (1/6) = 10
      assert.strictEqual(
        edge1State.fairShare,
        10,
        "edge-1 should have fair share of 10",
      );
      assert.strictEqual(
        edge2State.fairShare,
        10,
        "edge-2 should have fair share of 10",
      );
    ...
    it("W21: CRITICAL/FATAL bypass still works with weighted cluster", () => {
      ...
      // Exhaust all tokens
      for (let i = 0; i < 100; i++) {
        brokers[i % 3].logEvent({
          siemSeverity: "LOW",
          siemCategory: `EXHAUST_${i}`,
        });
      }
      ...
      assert.strictEqual(
        result,
        true,
        "CRITICAL must bypass weighted rate limiter",
      );
      for (const b of brokers) b.close();
    });
    ...
    it("W22: partition fallback — node alone uses its own weight-scaled share", () => {
      ...
      assert.strictEqual(
        broker._fairShare,
        33,
        "partitioned node should use initial fair share",
      );
      ...
      assert.ok(
        processed <= 33,
        `processed (${processed}) should not exceed fair share (33)`,
      );
