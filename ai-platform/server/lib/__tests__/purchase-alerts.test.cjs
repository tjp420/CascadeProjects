...
describe('purchase-alerts', () => {
  ...
  describe('maskEmail', () => {
    it('masks the local part of a standard email', () => {
      assert.strictEqual(maskEmail('jane.developer@company.com'), 'j***r@company.com');
    });
  ...
  describe('buildSlackPayload', () => {
    it('includes tier and masked email in fields', () => {
      ...
      var tierField = fields.find(function (f) { return f.title === 'Tier'; });
      assert.strictEqual(tierField.value, 'developer');
      var customerField = fields.find(function (f) { return f.title === 'Customer'; });
      assert.strictEqual(customerField.value, 'j***e@example.com');
    });

    it('formats amount as dollars', () => {
      var payload = buildSlackPayload({ tier: 'team_pro', email: 'a@b.com', amount: 14900 });
      var amountField = payload.attachments[0].fields.find(function (f) { return f.title === 'Amount'; });
  ...
  describe('buildDiscordPayload', () => {
    it('includes embeds with tier and masked email', () => {
      ...
      var tierField = embed.fields.find(function (f) { return f.name === 'Tier'; });
      assert.strictEqual(tierField.value, 'team_pro');
      var customerField = embed.fields.find(function (f) { return f.name === 'Customer'; });
  ...
  describe('sendPurchaseAlert', () => {
    it('returns sent:false when PURCHASE_ALERT_WEBHOOK not set', async () => {
      delete process.env.PURCHASE_ALERT_WEBHOOK;
      var result = await sendPurchaseAlert({ tier: 'developer', email: 'a@b.com' });
    ...
    it('attempts to send to Slack URL (may fail without real endpoint)', async () => {
      process.env.PURCHASE_ALERT_WEBHOOK = 'https://hooks.slack.com/services/T000/B000/XXX';
      var result = await sendPurchaseAlert({ tier: 'developer', email: 'dev@example.com', amount: 4900 });
    ...
    it('attempts to send to Discord URL (may fail without real endpoint)', async () => {
      process.env.PURCHASE_ALERT_WEBHOOK = 'https://discord.com/api/webhooks/000/xxx';
      var result = await sendPurchaseAlert({ tier: 'team_pro', email: 'team@example.com', amount: 14900 });
