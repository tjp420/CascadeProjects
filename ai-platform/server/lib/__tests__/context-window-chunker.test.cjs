...
describe('Token estimation', () => {
  ...
  it('estimateTokens scales with content length', () => {
    const short = 'hello';
    const long = 'hello world this is a much longer string with many more tokens';
  ...
  it('estimateTotalTokens sums system prompt + content', () => {
    ...
    assert.strictEqual(total, estimateTokens(systemPrompt) + estimateTokens(content));
  });
});
...
describe('Context window resolution', () => {
  it('returns known model context window', () => {
    assert.strictEqual(resolveContextWindow('llama3.2'), DEFAULT_CONTEXT_WINDOWS['llama3.2']);
    assert.strictEqual(resolveContextWindow('mistral'), DEFAULT_CONTEXT_WINDOWS['mistral']);
    assert.strictEqual(resolveContextWindow('qwen2.5-coder'), DEFAULT_CONTEXT_WINDOWS['qwen2.5-coder']);
  });

  it('returns default for unknown model', () => {
    assert.strictEqual(resolveContextWindow('unknown-model'), DEFAULT_CONTEXT_WINDOWS.default);
  });
  ...
  it('matches model variants with prefix (e.g. llama3.2:latest)', () => {
    assert.strictEqual(resolveContextWindow('llama3.2:latest'), DEFAULT_CONTEXT_WINDOWS['llama3.2']);
    assert.strictEqual(resolveContextWindow('mistral:7b'), DEFAULT_CONTEXT_WINDOWS['mistral']);
  });
...
describe('chunkContent', () => {
  ...
  it('splits content on line boundaries when exceeding budget', () => {
    ...
    for (const chunk of chunks) {
      assert.ok(chunk.length <= maxChars, `chunk length ${chunk.length} should be <= ${maxChars}`);
    }
  });
  ...
  it('handles single line exceeding chunk size by splitting at char boundary', () => {
    ...
    const maxChars = tokensToChars(100);
    for (const chunk of chunks) {
      assert.ok(chunk.length <= maxChars, 'each chunk should be within char budget');
    }
  });
  ...
  it('preserves content integrity — concatenating chunks restores original (approximately)', () => {
    ...
    assert.ok(reconstructed.length >= content.length * 0.95, 'reconstructed content should be close to original');
  });
...
describe('planMultiFileSweep', () => {
  ...
  it('groups small files into a single batch', () => {
    ...
    assert.strictEqual(batches.length, 1, 'small files should fit in one batch');
    assert.strictEqual(batches[0].length, 3, 'batch should contain all 3 files');
  });
  ...
  it('skips files with no content', () => {
    ...
    assert.strictEqual(allChunks.length, 1, 'only the non-empty file should be included');
...
describe('createResultAggregator', () => {
  ...
  it('tracks summary statistics', () => {
    ...
    agg.add({ path: 'b.js', chunkCount: 2, estimatedTokens: 300, error: 'failed' });
...
describe('buildChunkPrompt', () => {
  it('includes file path in prompt', () => {
    const prompt = buildChunkPrompt('const x = 1;', { filePath: 'src/index.js' });
  ...
  it('includes chunk position for multi-chunk files', () => {
    ...
    assert.ok(prompt.includes('chunk 3 of 5'), 'should show 1-based chunk position');
  });
  ...
  it('omits chunk position for single-chunk files', () => {
    ...
    assert.ok(!prompt.includes('chunk 1 of 1'), 'should not show chunk position for single chunk');
  });

  it('includes analysis type-specific instructions', () => {
    const securityPrompt = buildChunkPrompt('const x = 1;', { analysisType: 'security' });
    assert.ok(securityPrompt.includes('Vulnerabilities'));

    const perfPrompt = buildChunkPrompt('const x = 1;', { analysisType: 'performance' });
    assert.ok(perfPrompt.includes('Performance bottlenecks'));

    const archPrompt = buildChunkPrompt('const x = 1;', { analysisType: 'architecture' });
...
describe('executeMultiFileSweep', () => {
  ...
  it('returns empty result for non-function analyzer', async () => {
    const result = await executeMultiFileSweep([{ path: 'a.js', content: 'x' }], null);
    assert.strictEqual(result.summary.totalProcessed, 0);
  });
  ...
  it('calls analyzeFn for each chunk and collects results', async () => {
    ...
    const result = await executeMultiFileSweep(files, analyzeFn, { model: 'llama3.2' });
  ...
  it('handles analyzer errors gracefully', async () => {
    const files = [{ path: 'a.js', content: 'const a = 1;' }];
    const analyzeFn = async () => { throw new Error('LLM unavailable'); };
...
describe('Large codebase simulation (enterprise workload)', () => {
  it('handles a single very large file (100K lines)', () => {
    ...
    const chunks = chunkContent(content, { model: 'llama3.2' });
    assert.ok(chunks.length > 1, '100K-line file should produce multiple chunks');

    // Each chunk should be within the context window budget
    const contextWindow = resolveContextWindow('llama3.2');
    const available = availableContentTokens(contextWindow, DEFAULT_SYSTEM_PROMPT_TOKENS, DEFAULT_RESPONSE_TOKENS);
    const maxChars = tokensToChars(available);
    for (const chunk of chunks) {
      assert.ok(chunk.length <= maxChars, `chunk should fit within context window (${chunk.length} <= ${maxChars})`);
    }
  });
  ...
  it('executeMultiFileSweep completes within reasonable time for 100 files', async () => {
    ...
    const result = await executeMultiFileSweep(files, async () => ({ ok: true }), {
      model: 'llama3.2',
    });
...
describe('Edge cases', () => {
  ...
  it('resolveContextWindow handles empty/null model', () => {
    assert.strictEqual(resolveContextWindow(''), DEFAULT_CONTEXT_WINDOWS.default);
    assert.strictEqual(resolveContextWindow(null), DEFAULT_CONTEXT_WINDOWS.default);
    assert.strictEqual(resolveContextWindow(undefined), DEFAULT_CONTEXT_WINDOWS.default);
  });
