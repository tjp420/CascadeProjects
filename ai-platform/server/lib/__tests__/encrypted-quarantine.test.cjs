...
function tamperEntry(entryId, field, newValue) {
  ...
  const key = Object.keys(store.entries).find(
    (k) => store.entries[k].id === entryId,
  );
  if (key) {
...
describe("Per-Tenant Encrypted Quarantine Storage", () => {
  ...
  describe("getTenantQuarantinePath", () => {
    it("should return a path containing the orgId", () => {
      ...
      assert.ok(
        p.includes("tenant-org-123"),
        "Path should contain tenant-org-123",
      );
      assert.ok(
        p.endsWith("audit-quarantine.json"),
        "Path should end with audit-quarantine.json",
      );
    });
    ...
    it("should use default for null orgId", () => {
      ...
      assert.ok(
        p.includes("tenant-default"),
        "Path should contain tenant-default",
      );
    });
    ...
    it("should sanitize unsafe characters in orgId", () => {
      ...
      assert.ok(
        p.includes("tenant-org_") && p.includes("_etc"),
  ...
  describe("writeTenantQuarantineStore / readTenantQuarantineStore", () => {
    ...
    it("should write encrypted content (sb-dir: prefix) to disk", () => {
      ...
      assert.ok(
        cryptoUtils.isDirectoryEncrypted(rawContent),
        "File content must be directory-encrypted (sb-dir: prefix)",
      );
      assert.ok(
        !rawContent.includes("secret-entry"),
        "Raw file must not contain plaintext entry IDs",
      );
      assert.ok(
        !rawContent.includes("DELETE"),
        "Raw file must not contain plaintext actions",
      );
    });
    ...
    it("should fail to read with wrong orgId (cross-tenant isolation)", () => {
      ...
      assert.strictEqual(
        crossRead.entries.length,
        0,
        "Cross-tenant read must return empty store",
      );
    });
  ...
  describe("healChain integration with encrypted quarantine", () => {
    it("should quarantine tampered entry to encrypted per-tenant file", () => {
      ...
      assert.ok(
        fs.existsSync(tenantPath),
        "Per-tenant quarantine file should exist",
      );

      const rawContent = fs.readFileSync(tenantPath, "utf8");
      assert.ok(
        cryptoUtils.isDirectoryEncrypted(rawContent),
        "Quarantine file must be encrypted",
      );
      ...
      assert.ok(
        ...
        "Should have quarantined entries",
      );
      assert.strictEqual(quarantine.entries[0].id, e1.id);
      assert.strictEqual(
        quarantine.entries[0].quarantineReason,
        "content_tampered",
      );
    });
    ...
    it("should not leak quarantined data across tenants on disk", () => {
      ...
      const rawA = fs.readFileSync(
        auditLogger.getTenantQuarantinePath("org-iso-a"),
        "utf8",
      );
      const rawB = fs.readFileSync(
        auditLogger.getTenantQuarantinePath("org-iso-b"),
        "utf8",
      );
      assert.ok(
        !rawA.includes(eB.id),
        "org-a quarantine must not contain org-b entry IDs",
      );
      assert.ok(
        !rawB.includes(eA.id),
        "org-b quarantine must not contain org-a entry IDs",
      );
    });
  });
  ...
  describe("backward compatibility", () => {
    it("should read from legacy global quarantine file when no per-tenant file exists", () => {
      ...
      assert.ok(
        ...
        "Should read from legacy quarantine file",
      );
    ...
    it("should handle getQuarantine without orgId (legacy global read)", () => {
      ...
      assert.ok(
        ...
        "Legacy global quarantine should be readable",
      );
    });
  });
  ...
  describe("file system isolation", () => {
    it("should create per-tenant directories on write", () => {
      ...
      assert.ok(
        fs.existsSync(path.dirname(tenantPath)),
        "Tenant directory should exist",
      );
    });
