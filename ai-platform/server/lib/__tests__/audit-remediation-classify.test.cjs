...
describe("audit-remediation-recipes/classify", () => {
  ...
  test("classifyRowKind detects credentials", () => {
    ...
    expect(classifyRowKind({ rule: "credential", snippet: credSnippet })).toBe(
      "credentials",
    );
  });

  test("classifyRowKind detects env-secret", () => {
    expect(classifyRowKind({ rule: "env-secret", snippet: "" })).toBe(
      "env-secret",
    );
  });

  test("classifyRowKind detects config-sprawl", () => {
    expect(classifyRowKind({ rule: "config-sprawl", snippet: "" })).toBe(
      "config-sprawl",
    );
  });

  test("classifyRowKind detects orphaned-data and dev-dependency", () => {
    expect(classifyRowKind({ rule: "orphaned-data", snippet: "" })).toBe(
      "orphaned-data",
    );
    expect(classifyRowKind({ rule: "", snippet: "import pytest" })).toBe(
      "dev-dependency",
    );
  });

  test("classifyRowKind detects eslint and syntax kinds", () => {
    expect(classifyRowKind({ rule: "eslint", snippet: "no-unused-vars" })).toBe(
      "eslint",
    );
    expect(classifyRowKind({ rule: "", snippet: "parse error syntax" })).toBe(
      "syntax",
    );
  });
  ...
  test("classifyRowKind detects pii", () => {
    expect(
      classifyRowKind({ rule: "data-privacy", snippet: "pii pattern" }),
    ).toBe("pii");
  });

  test("classifyRowKind detects file-reduction", () => {
    expect(classifyRowKind({ rule: "file-reduction", snippet: "" })).toBe(
      "file-reduction",
    );
  });

  test("classifyRowKind detects production-leak", () => {
    expect(classifyRowKind({ rule: "", snippet: "mock/sample.json" })).toBe(
      "production-leak",
    );
  });

  test("classifyRowKind detects llm-slop", () => {
    expect(classifyRowKind({ rule: "", snippet: "```markdown fence```" })).toBe(
      "llm-slop",
    );
  });
  ...
  test("classifyRowKind detects fiction-kpi", () => {
    expect(
      classifyRowKind({ rule: "", snippet: "completion_rate 98.5%" }),
    ).toBe("fiction-kpi");
  });

  test("classifyRowKind detects tech-debt", () => {
    expect(classifyRowKind({ rule: "", snippet: "TODO: fix this" })).toBe(
      "tech-debt",
    );
  });

  test("classifyRowKind returns general for unknown", () => {
    expect(classifyRowKind({ rule: "", snippet: "some random text" })).toBe(
      "general",
    );
  });
