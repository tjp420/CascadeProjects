...
} = require("./register-generic-fallback-plugins.cjs");
const {
  extractGenericStructureHints,
} = require("./generic-structure-hints.cjs");
 ...
 * @returns {any}
 */
function registerDedicatedPatternPlugin(
  manager,
  languageId,
  patterns,
  options = {},
  ...
  manager.register(
    createPatternPlugin("decorate", {
      bestPractices: [
        {
          id: "actor-block",
          pattern: /\bActor\s+\w+/gi,
          label: "DECORATE actor definition",
        },
      ],
    }),
  );
      ...
      debug: [
        {
          id: "lua-print",
          pattern: /\bprint\s*\(/gi,
          label: "Lua print statement",
        },
      ],
    }),
  );

  registerDedicatedPatternPlugin(manager, "python", pythonPatterns, {
    version: "1.0.0",
  });
  registerDedicatedPatternPlugin(manager, "rust", rustPatterns, {
    version: "1.0.0",
  });
  registerDedicatedPatternPlugin(manager, "go", goPatterns, {
    version: "1.0.0",
  });
  registerDedicatedPatternPlugin(manager, "sql", sqlPatterns, {
    version: "1.0.0",
  });
