...
const REQUIRED_MODULES = [
  {
    file: "./language-plugin.cjs",
    exports: ["LanguagePlugin", "EMPTY_PATTERNS"],
  },
  {
    file: "./plugin-manager.cjs",
    exports: ["PluginManager", "getBuiltinPluginManager"],
  },
  { file: "./builtin-plugins.cjs", exports: ["registerBuiltinPlugins"] },
  {
    file: "./register-generic-fallback-plugins.cjs",
    exports: ["registerGenericFallbackPlugins"],
  },
];
...
const { PluginManager, getBuiltinPluginManager } =
  loaded["./plugin-manager.cjs"];
const { registerBuiltinPlugins } = loaded["./builtin-plugins.cjs"];
const { registerGenericFallbackPlugins } =
  loaded["./register-generic-fallback-plugins.cjs"];
...
class PluginSystem {
  ...
  registerPlugin(plugin) {
    if (!this._manager) {
      throw new Error(
        "[PluginSystem] Not initialized. Call initialize() first.",
      );
    }
    ...
    if (!plugin.language) {
      throw new Error(
        '[PluginSystem] Plugin must define a "language" property.',
      );
    }
    if (!plugin.extensions || !Array.isArray(plugin.extensions)) {
      throw new Error(
        '[PluginSystem] Plugin must define an "extensions" array.',
      );
    }
  ...
  discoverPlugins(dir) {
    ...
    if (!fs.existsSync(absDir)) {
      this._errors.push(
        `discoverPlugins: directory does not exist "${absDir}"`,
      );
      return 0;
    }
    let discovered = 0;
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        this._errors.push(
          `discoverPlugins: failed to load "${entry.name}": ${err.message}`,
        );
      }
    }
    return discovered;
