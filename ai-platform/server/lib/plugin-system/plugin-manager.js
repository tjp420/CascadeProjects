...
const { resolveLanguageFromPath } = require('../universal-language-registry');
...
class PluginManager {
    ...
    getByExtension(extension) {
        const matches = this.extensionIndex.get(String(extension || '').toLowerCase()) || [];
    ...
    shouldUsePlugin(extension) {
        const legacyHandled = new Set(['.js', '.mjs', '.cjs', '.ts', '.tsx', '.jsx', '.json']);
