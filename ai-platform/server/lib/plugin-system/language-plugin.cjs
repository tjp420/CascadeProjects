...
class LanguagePlugin {
  constructor(config = {}) {
    ...
    this.extensions = new Set(
      (config.extensions || []).map((ext) => String(ext).toLowerCase()),
    );
    this.basenames = new Set(
      (config.basenames || []).map((name) => String(name).toLowerCase()),
    );
  ...
  scoreContent(content) {
    ...
    for (const indicator of this.contentIndicators) {
      ...
      if (matches)
        score += (indicator.weight || 1) * Math.min(matches.length, 4);
    }
    return score;
  }
  ...
  async analyze(content, relativePath, helpers = {}) {
    ...
    if (skipPatternCatalogDoc) {
      return {
        ...
        parser: this.parser,
        findings: [],
        structure: this.structureParser
          ? this.structureParser(content, rel)
          : null,
      };
    }
    ...
    if (!isCliToolingPath?.(rel)) {
      ...
      const debugPatterns = (this.patterns.debug || []).filter((pattern) => {
        if (pattern.id === "r-print" && this.language === "python")
          return false;
        if (pattern.id === "python-print" && this.language === "r")
          return false;
        return true;
      });
      findings.push(
    ...
    if (this.patterns.productionLeak?.length) {
      const productionOnly =
        typeof isProductionRelevantPath === "function"
    ...
    const skipPlaceholders =
      ...
      isPlaceholderCatalogOrMetaDoc?.(rel);

    if (this.useGenericPlaceholders && !skipPlaceholders) {
      if (detectPlaceholderAndFictionalData) {
        findings.push(
          ...(await detectPlaceholderAndFictionalData(content, rel)),
        );
      }
      findings.push(
        ...(await scanContentPatterns(
