  ...
  async analyze(content, relativePath, helpers = {}) {
    ...
    if (this.useGenericTechDebt && !skipTechDebt) {
      if (TECH_DEBT_PATTERNS) {
        findings.push(
          ...(await scanContentPatterns(content, rel, TECH_DEBT_PATTERNS, 'tech-debt', 'medium'))
        );
      }
      findings.push(
        ...(await scanContentPatterns(content, rel, this.patterns.techDebt, 'tech-debt', 'medium'))
      );
    }
    ...
    if (!isCliToolingPath?.(rel)) {
      ...
      findings.push(
        ...(await scanContentPatterns(content, rel, debugPatterns, 'debug-artifact', 'medium'))
      );
    }

    if (this.patterns.productionLeak?.length) {
      const productionOnly =
        typeof isProductionRelevantPath === 'function' ? isProductionRelevantPath(rel) : false;
      if (productionOnly) {
        findings.push(
    ...
    const skipPlaceholders =
      isNonProductionAuditContentPath?.(rel) || isPlaceholderCatalogOrMetaDoc?.(rel);
    ...
    findings.push(
      ...(await scanContentPatterns(content, rel, this.patterns.bestPractices, 'tech-debt', 'low'))
    );
