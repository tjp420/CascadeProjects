...
function formatScanDuration(durationMs) {
  ...
  if (elapsedMs < 1000)
    return `${(elapsedMs / constants.MS_PER_SECOND).toFixed(2)} seconds`;
...
function truncateForDisplay(text, maxLen = 96) {
  ...
  const cut =
    wordBreak > Math.floor(maxLen * 0.55) ? slice.slice(0, wordBreak) : slice;
  return `${cut.trim()}…`;
}
...
function redactSnippet(text) {
  ...
  const redacted = String(text)
    .replace(
      /(?:sk|pk|rk)_(?:live|test)_[A-Za-z0-9]{10,}/gi,
      "sk_****REDACTED****",
    )
    .replace(/AKIA[0-9A-Z]{16}/g, "AKIA****REDACTED****")
    .replace(
      /(?:api[_-]?key|secret|token|password)\s*[:=]\s*['"][^'"]{4,}['"]/gi,
      (match) => match.replace(/(['"])[^'"]+(['"])/, "$1****REDACTED****$2"),
    )
    .replace(
