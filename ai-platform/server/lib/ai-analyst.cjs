...
async function generateAutomatedVerdict(scanJson, options = {}) {
  const provider =
    options.provider || process.env.AI_ANALYST_PROVIDER || "openai";
  ...
  try {
    ...
    return {
      ...
      remediationSteps: [
        "Re-run with a configured AI provider for structured output.",
      ],
    };
  } catch (err) {
    logger.warn(
      "[AI Analyst] LLM verdict failed, using deterministic fallback:",
      err.message,
    );
    return buildDeterministicVerdict(scanJson);
  }
...
async function emailAutomatedVerdict(options = {}) {
  ...
  return sendEmail({
    ...
    subject:
      options.subject ||
      `Your Automated Compliance Report — ${reportData.project || "Project"}`,
