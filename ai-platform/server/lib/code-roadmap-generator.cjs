...
const {
    normalizeRelativePath, toPosixPath, getBasename, getDirname, ensureExt, hasExt, truncate, slugify,
    shouldIgnoreRoadmapPath, shouldSkipWalkDirectory, filterRoadmapAnalysisFiles, filterByExtension,
    filterBySize, sortBySize, sortByName, walkProject, readJsonSafe,
    SKIP_DIRS, ROADMAP_NOISE_DIR_NAMES, ROADMAP_SKIP_RELATIVE_PREFIXES, CODE_EXTENSIONS, API_ROUTE_SOURCE_PREFIXES
...
function buildRecommendations(signals, sprintModel, baseline, v1Internal = {}, scanReport = null) {
    ...
    const safeSprintModel = sprintModel && typeof sprintModel === 'object' ? sprintModel : {};
    const immediate = [];
    const shortTerm = [];
    ...
    if (!safeSignals.phase2SmokeInCi) immediate.push('Add docker-compose.phase2.yml smoke test to CI');
    if (!safeSignals.istanbulInCi) immediate.push('Enable Istanbul coverage in CI (npm run test:coverage)');
    if (safeSprintModel.plannedFeatures > 0) shortTerm.push('Complete remaining sprint deliverables');
