...
function pickCanonicalFile(files, keepFile) {
  if (keepFile) {
    return (
      files.find((file) => file.path === keepFile || file.name === keepFile) ||
      files[0]
    );
  }
  return [...files].sort(
    (a, b) => String(a.path || "").length - String(b.path || "").length,
  )[0];
}
...
async function readFileSnapshot(absPath) {
  if (!fs.existsSync(absPath)) {
    // simplebeacon-ignore sync-io — existence check before async stat/read
...
function assessMergeRisk(candidate, snapshots, conflicts) {
  ...
  if (candidate?.mergeType === "exact-duplicate") {
    ...
    factors.push(
      `Fuzzy similarity ${Math.round((candidate.similarity || 0) * 100)}% — not byte-identical`,
    );
  ...
  const invalidJson = snapshots.filter(
    (snap) => snap.exists && snap.validJson === false,
  ).length;
  if (invalidJson) {
...
async function buildMergePreview(options = {}) {
  ...
  const strategy =
    options.strategy || candidate?.mergeStrategy || "keep-one-delete-others";
  ...
  const redundant = files.filter(
    (file) => file.path !== canonical.path && file.name !== canonical.name,
  );

  if (candidate?.mergeType === "exact-duplicate" && snapshots.length > 1) {
    const hashes = new Set(
      snapshots.filter((s) => s.exists).map((s) => s.contentHash),
    );
    if (hashes.size > 1) {
      conflicts.push({
        reason: "Files are not byte-identical — exact-duplicate merge blocked",
      });
    }
  ...
  const preview = {
    ...
    savingsBytes:
      ...
      redundant.reduce((sum, file) => sum + (file.sizeBytes || 0), 0),
