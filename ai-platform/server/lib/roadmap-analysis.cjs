...
function countTestFiles(files) {
  ...
  return files.filter((file) => /\.(test|spec)\.(js|ts|jsx|tsx)$/.test(file.name)).length;
}
...
function extractJsDependencies(files, _projectRoot) {
  if (!Array.isArray(files))
    return { internalCount: 0, externalCount: 0, externalPackages: [], sampleInternal: [] };
  ...
  for (const file of jsFiles) {
    ...
    const patterns = [/require\(['"]([^'"]+)['"]\)/g, /import\s+.*?\s+from\s+['"]([^'"]+)['"]/g];
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (dep.startsWith('.') || dep.startsWith('/')) {
          ...
          external.add(
            dep.startsWith('@') ? dep.split('/').slice(0, 2).join('/') : dep.split('/')[0]
          );
        }
      }
    }
  ...
  try {
    ...
    return {
      ...
      stripePriceIdTeamsMonthly:
        get('STRIPE_PRICE_ID_TEAMS_MONTHLY') || get('SIMPLEBEACON_PRO_PRICE_ID'),
      stripePriceIdTeamsAnnual:
        get('STRIPE_PRICE_ID_TEAMS_ANNUAL') || get('SIMPLEBEACON_ANNUAL_PROMOTION_ID'),
      stripePriceIdEnterpriseSetup:
        get('STRIPE_PRICE_ID_ENTERPRISE_SETUP') || get('SIMPLEBEACON_ENTERPRISE_SETUP_ID'),
      stripePriceIdEnterpriseRetainer:
        get('STRIPE_PRICE_ID_ENTERPRISE_RETAINER') || get('SIMPLEBEACON_ENTERPRISE_RETAINER_ID'),
...
function detectV1InternalReadinessAt(projectRoot) {
  ...
  const productionArtifactsReady =
    artifacts.productionExample && artifacts.deployScript && artifacts.cloudflaredConfig;
...
function detectPlatformSignalsAt(projectRoot) {
  ...
  return {
    ...
    phase2Auth:
      fs.existsSync(path.join(root, 'server/bootstrap/phase2-integration.cjs')) ||
    ...
    fileMergerScanner:
      fs.existsSync(path.join(root, 'server/lib/file-merger-reduction-scanner.cjs')) ||
      fs.existsSync(path.join(root, 'server/lib/file-merger-reduction-scanner.js')),
    npmAudit:
...
function scopeFilesToPlatform(files, scanRoot, platformRoot) {
  ...
  return files
    ...
    .map((file) => ({
      ...file,
      relativePath: file.relativePath === prefix ? '' : file.relativePath.slice(prefix.length + 1),
    }))
