...
async function run() {
  ...
  ratchet.createSessionRecord({
    sessionId: sid + "-A",
    tenantId: tenant,
    localKeyPair: aKeys,
    remotePublicKeyObj: bKeys.publicKeyObj,
    initialShared: sharedA,
  });
  ratchet.createSessionRecord({
    sessionId: sid + "-B",
    tenantId: tenant,
    localKeyPair: bKeys,
    remotePublicKeyObj: aKeys.publicKeyObj,
    initialShared: sharedB,
  });
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const decrypted = ratchet.decryptWithChainKey(
      sResponder,
  ...
  let exists = false;
  try {
    exists = fs.existsSync(base) && fs.readdirSync(base).length > 0;
  } catch (e) {
    exists = false;
  }
