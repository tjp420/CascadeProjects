...
function scanFileContent(content, filename, opts = {}) {
  ...
  if (m) {
    findings.push({ reason: 'TRACK113_KEK assignment', value: m[1].slice(0, 64), filename });
  }
  ...
  for (const c of candidates) {
    ...
    if (s.length >= 32 && isHexString(s)) {
      findings.push({ reason: 'long-hex-string', value: s.slice(0, 64), filename });
      continue;
    }
    ...
    if (s.length >= lenThreshold) {
      ...
      if (/^\.?\.?\/.*\.(cjs|js|json|ts|mjs|md|html|htm|css|yml|yaml|svg|png|jpg|gif|woff2?|ttf|eot)$/i.test(s) || /^[a-z][-a-z0-9]*\/[a-z][-a-z0-9/]*\.(cjs|js|json|ts|mjs|md|html|htm|css|yml|yaml|svg|png|jpg|gif|woff2?|ttf|eot)$/i.test(s)) continue;
      const ent = shannonEntropy(s);
      const perChar = ent; // entropy computed per-symbol already
      if (perChar >= perCharEntropyThreshold) {
        findings.push({ reason: 'high-entropy-string', entropy: perChar, length: s.length, value: s.slice(0, 64), filename });
      }
    }
  }
...
function scanFiles(filePaths, opts = {}) {
  ...
  for (const p of filePaths) {
    ...
    // Skip built frontend bundles/maps — minified charset alphabets (e.g. nanoid) trip entropy checks
      if (/\.md$/i.test(p) || p.startsWith('ai-platform/docs') || p.startsWith('.github/') || /__tests__\/.*\.cjs$/.test(p) || /\.test\.cjs$/.test(p) || /(^|\/)tests\/.*\.(js|cjs)$/.test(p) || /(^|\/)package-lock\.json$/.test(p) || /(^|\/)npm-shrinkwrap\.json$/.test(p) || /\.js\.map$/i.test(p) || /(^|\/)(ai-platform\/web\/simplebeacon-dashboard\/assets|coming-soon\/public\/(?:app|dashboard)\/assets|simplebeacon-vscode-merged\/dashboard-web\/assets)\//i.test(p) || (new RegExp('tests' + '\\/' + 'fixtures' + '\\/' + 'true-positives' + '\\/', 'i')).test(p) || /(^|\/)vendor\/.*\.js$/i.test(p) || /(^|\/)js-es2018\/dashboard\/main\.js$/i.test(p)) continue;
    let full = p;
