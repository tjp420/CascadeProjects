...
describe('Compatibility Shim (Track 113)', () => {
  ...
  it('completes classical handshake end-to-end', async () => {
    ...
    const telemetry = [];
    const init = await initiateHandshake(alice, bob.publicKey, { telemetry: (ev) => telemetry.push(ev) });
  ...
  it('rejects handshake past deprecation deadline', async () => {
    ...
    await assert.rejects(
      () => initiateHandshake(alice, bob.publicKey, { deprecationDeadline: past, now: Date.now() }),
      (err) => err instanceof CompatibilityError && err.code === 'CLASSICAL_DEPRECATION_DEADLINE',
    );
  });
