...
const bootstrap = require(path.resolve(process.cwd(), 'server', 'lib', 'crypto', 'ratchet', 'hybrid-bootstrap.cjs'));
const { initiateHandshake, acceptHandshake, detectMode, CompatibilityError } = require(path.resolve(process.cwd(), 'server', 'lib', 'crypto', 'ratchet', 'compatibility-shim.cjs'));

describe('Track 113 — Handshake Fuzzing', () => {
  describe('deserializePublicKey fuzzing', () => {
    it('rejects empty buffer', () => {
      assert.throws(() => bootstrap.deserializePublicKey(Buffer.alloc(0)), bootstrap.HybridBootstrapError);
    });

    it('rejects single-byte buffer', () => {
      assert.throws(() => bootstrap.deserializePublicKey(Buffer.from([0x01])), bootstrap.HybridBootstrapError);
    });
    ...
    it('rejects truncated component header', () => {
      ...
      assert.throws(
        ...
        (err) => err instanceof bootstrap.HybridBootstrapError && err.code === 'INVALID_HYBRID_KEY_LAYOUT',
      );
    });
    ...
    it('returns empty components for zero-component key (parser tolerates, encapsulate rejects)', () => {
      ...
      const components = bootstrap.deserializePublicKey(Buffer.from([0x01, 0x00]));
  ...
  describe('encapsulate/decapsulate fuzzing', () => {
    ...
    it('rejects cipherText with corrupted ML-KEM component', async () => {
      ...
      try {
        ...
        assert.notStrictEqual(enc.chainKey, dec.chainKey, 'corrupted cipherText should not produce same key');
      } catch (e) {
  ...
  describe('signature fuzzing', () => {
    it('rejects corrupted signature', async () => {
      ...
      assert.strictEqual(bob.verifyHandshake(corrupted, transcript, alice.publicKey), false);
    });
    ...
    it('rejects signature with wrong transcript', async () => {
      ...
      assert.strictEqual(bob.verifyHandshake(signature, Buffer.from('handshake-v2'), alice.publicKey), false);
    });
    ...
    it('rejects signature with wrong public key', async () => {
      ...
      const transcript = Buffer.from('handshake-v1');
      const signature = alice.signHandshake(transcript);
      assert.strictEqual(bob.verifyHandshake(signature, transcript, carol.publicKey), false);
    });
  });
  ...
  describe('compatibility shim fuzzing', () => {
    it('rejects handshake with invalid version byte', async () => {
      ...
      const fakeHandshake = Buffer.concat([Buffer.from([0xff]), crypto.randomBytes(100)]);
      await assert.rejects(
...
describe('Track 113 — Interop Tests', () => {
  ...
  it('rotation produces matching keys on both sides', async () => {
    ...
    assert.strictEqual(aliceResult.chainKey, bobResult.chainKey, 'Rotated chain keys must match');
    assert.strictEqual(aliceResult.rotationEpoch, bobResult.rotationEpoch, 'Rotation epochs must match');
