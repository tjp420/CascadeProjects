...
const { HybridBootstrapError, deserializePublicKey } = require(
  path.resolve(
    process.cwd(),
    "server",
    "lib",
...
describe("IdentityRatchet (Track 113)", () => {
  it("generates a hybrid keypair with versioned public key", async () => {
    const ratchet = await new IdentityRatchet({
      deviceId: "test-1",
    }).generate();
  ...
  it("signs and verifies handshake transcripts", async () => {
    ...
    assert.ok(signature);
    assert.strictEqual(
      bob.verifyHandshake(signature, transcript, alice.publicKey),
      true,
    );
    assert.strictEqual(
      bob.verifyHandshake(signature, transcript, bob.publicKey),
      false,
    );
  });
  ...
  it("steps the chain and produces distinct message keys", async () => {
    ...
    await bob.decapsulateFrom(
      (await alice.encapsulateFor(bob.publicKey)).cipherText,
    );
  ...
  it("emits QUANTUM_ROTATE_PENDING and rotates at message threshold", async () => {
    const events = [];
    const audit = (event, info) => events.push({ event, info });
    const alice = await new IdentityRatchet({
      deviceId: "alice",
      audit,
      rotation: { maxMessages: 10, warningRatio: 0.8 },
    }).generate();
    const bob = await new IdentityRatchet({
      deviceId: "bob",
      audit,
      rotation: { maxMessages: 10, warningRatio: 0.8 },
    }).generate();
    ...
    assert.ok(
      events.some((e) => e.event === "QUANTUM_ROTATE_PENDING"),
      "pending warning emitted",
    );
    assert.ok(
      events.some((e) => e.event === "IDENTITY_RATCHET_ROTATED"),
      "rotation completed",
    );
  ...
  it("throws on unknown public key version", () => {
    assert.throws(
      () => deserializePublicKey(Buffer.from([0xff])),
