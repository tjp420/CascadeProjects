...
const DURATION_BUCKETS_MS = [1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000];
...
class RatchetMetrics {
  ...
  snapshot() {
    ...
    for (const [mode, h] of this._durations) {
      durations[mode] = { sum: h.sum, count: h.count, buckets: Object.fromEntries(h.buckets) };
    }
    const failures = Object.fromEntries(this._failures);
    return { identity_handshake_duration_ms: durations, identity_handshake_failed_total: failures };
  }
}
...
function reasonFromError(err) {
  if (err && err.code) {
    switch (err.code) {
      case 'SIGNATURE_INVALID': return 'signature_invalid';
      case 'CLASSICAL_DEPRECATION_DEADLINE': return 'expired_deadline';
      ...
      case 'PQC_BOOTSTRAP_FAILED':
        return 'pqc_error';
      default: return err.code.toLowerCase();
    }
  }
  return 'unknown';
}
