...
function encapsulate(recipientPublicKeyObj) {
  ...
  const { publicKey: ephPubObj, privateKey: ephPrivObj } =
    crypto.generateKeyPairSync("x25519");
  const ephPubDer = ephPubObj.export({ type: "spki", format: "der" });

  const shared = crypto.diffieHellman({
    privateKey: ephPrivObj,
    publicKey: recipientPublicKeyObj,
  });
  const sharedSecret = hkdfSha256(
    shared,
    undefined,
    Buffer.from("ml-kem-768"),
    32,
  );
...
function decapsulate(ciphertextDer, recipientPrivateKeyObj) {
  if (!Buffer.isBuffer(ciphertextDer))
    ciphertextDer = Buffer.from(ciphertextDer);
  if (!recipientPrivateKeyObj || typeof recipientPrivateKeyObj !== "object") {
    throw new Error("recipientPrivateKeyObj must be a KeyObject");
  }
  const ephPubObj = crypto.createPublicKey({
    key: ciphertextDer,
    format: "der",
    type: "spki",
  });
  const shared = crypto.diffieHellman({
    privateKey: recipientPrivateKeyObj,
    publicKey: ephPubObj,
  });
  const sharedSecret = hkdfSha256(
    shared,
