...
function readStore() {
  ...
  try {
    ...
    try {
      logger.warn(
        "[AuditLogger] readStore failed, returning empty store:",
        e && e.message ? e.message : e,
      );
    } catch (_) {
      /* logger may itself be unavailable — defensive no-op */
    }
    return { entries: {} };
  }
}
...
function _getPiiPolicyStore() {
  if (_piiPolicyStore === null) {
    try {
      ...
      try {
        logger.info(
          "[AuditLogger] pii-policy-store unavailable:",
          e && e.message ? e.message : e,
        );
      } catch (_) {
        /* logger may itself be unavailable — defensive no-op */
...
function log(params) {
  ...
  try {
    ...
    logger.debug("SIEM enqueue failed for audit event", {
      error: e.message,
      action: entry.action,
    });
  }
