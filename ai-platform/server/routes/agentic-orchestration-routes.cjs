...
function asyncHandler(fn) {
  return function (req, res, next) {
    Promise.resolve(fn(req, res, next)).catch((err) => {
      try {
        logger.error(
          "[AgenticOrchestration] Unexpected handler error: " + err.message,
        );
      } catch {}
...
function replayDetectionMiddleware(req, res, next) {
  ...
  try {
    ...
    if (result.isReplay) {
      logger.warn(
        `[AgenticOrchestration] Replay detected for org ${orgId}: ${fingerprint}`,
      );
...
async function checkAndReserveActiveSlot(orgId) {
  ...
  try {
    if (
      rateLimiter &&
      typeof rateLimiter.incrementActiveExecutions === "function"
    ) {
      ...
      if (cnt > QUOTA.MAX_ACTIVE_EXECUTIONS_PER_ORG) {
        ...
        try {
          await rateLimiter.decrementActiveExecutions(orgId);
        } catch (e) {
          logger.debug("decrementActiveExecutions failed during quota check", {
            error: e.message,
            orgId,
          });
        }
    ...
    logger.debug(
      "Redis-backed quota check failed; falling back to store-backed check",
      { error: e.message, orgId },
    );
  }
  ...
  try {
    ...
    if (active.length >= QUOTA.MAX_ACTIVE_EXECUTIONS_PER_ORG)
      return { allowed: false, current: active.length };
    ...
    if (typeof agenticStore.reserveExecution === "function") {
      try {
        agenticStore.reserveExecution(orgId);
      } catch (e) {
        logger.debug("reserveExecution failed", { error: e.message, orgId });
      }
    }
...
router.post(
  ...
  asyncHandler(async function (req, res) {
    ...
    if (!agentId) {
      ...
      return sendError(res, 409, "agent_create_failed", {
    ...
    logger.info(
      "[AgenticOrchestration] Agent created: " +
        agentId +
        " by " +
...
router.put("/agents/:id", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[AgenticOrchestration] Agent updated: " +
        req.params.id +
        " by " +
        ((req.user && req.user.email) || "admin"),
    );
...
router.delete("/agents/:id", authorize("admin:all"), function (req, res) {
  try {
    ...
    if (!result.success) return sendError(res, 404, "agent_not_found");
    logger.info(
      "[AgenticOrchestration] Agent deleted: " +
        req.params.id +
        " by " +
        ((req.user && req.user.email) || "admin"),
    );
  ...
  asyncHandler(async function (req, res) {
    ...
    if (req.canonicalFingerprint) {
      try {
        auditLogger.logEvent &&
          auditLogger.logEvent("AGENTIC_EXECUTE_REQUEST", {
            orgId,
            agentId: req.params.id,
            fingerprint: req.canonicalFingerprint,
            user: req.user && req.user.id,
          });
      } catch (e) {
        logger.debug("audit log AGENTIC_EXECUTE_REQUEST failed", {
          error: e.message,
        });
      }
    }
    ...
    try {
      ...
      if (!slot.allowed) {
        try {
          ...
          logger.debug("audit log AGENTIC_QUOTA_EXHAUSTED failed", {
            error: e.message,
          });
        }
        return sendError(res, 429, "rate_limited", {
          message: "max_active_executions_reached",
        });
      }
      reserved = true;
    } catch (e) {
      ...
      logger.debug(
        "quota enforcement failed; failing safe to allow execution",
        { error: e.message, orgId },
      );
    }
    ...
    if (!rl.allowed) {
      ...
      try {
        ...
        logger.debug("audit log AGENTIC_RATE_LIMIT_TRIPPED failed", {
          error: e.message,
        });
      }
      return sendError(res, 429, "rate_limited", {
    ...
    try {
      ...
      return sendError(res, 500, "inference_service_unavailable", {
        message: err.message,
      });
    }
    ...
    var inferenceFn = async function (prompt, opts) {
      var result = await inferenceService.generateWithProvider(
        ...
        prompt,
        {
          model: opts.model || agent.config.model,
          temperature: opts.temperature || agent.config.temperature,
        },
      );
      return {
        text:
    ...
    try {
      var result = await agenticStore.executeAgentLoop(
        req.params.id,
        orgId,
        input,
        inferenceFn,
      ...
      if (!result || result.success === false)
        return sendError(res, 500, "agent_execute_failed", {
      ...
      if (reserved) {
        try {
          ...
          logger.debug("decrementActiveExecutions failed in finally", {
            error: e.message,
            orgId,
          });
        }
...
router.get("/executions", function (req, res) {
  try {
    ...
    var history = agenticStore.getExecutionHistory(
      orgId,
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      if (!result.success)
        return sendError(res, 400, "pause_failed", { message: result.error });
      res.json(result);
    } catch (err) {
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      if (!result.success)
        return sendError(res, 400, "resume_failed", { message: result.error });
      res.json(result);
    } catch (err) {
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      if (!result.success)
        return sendError(res, 400, "abort_failed", { message: result.error });
...
router.post("/tools", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[AgenticOrchestration] Tool registered: " +
        toolId +
        " by " +
        ((req.user && req.user.email) || "admin"),
    );
...
router.get("/metrics", authorize("admin:all"), function (req, res) {
  try {
    ...
    if (
      siemExporter &&
      siemExporter._debug &&
      typeof siemExporter._debug.getMetrics === "function"
    ) {
    ...
    const lines = [];
    const meta = {
      siem_delivery_retries_total:
        "Total count of SIEM log delivery retry attempts.",
      siem_delivery_dropped_total:
        "Total number of SIEM events dropped due to queue trimming.",
    };
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "canonical_fingerprint_failed", {
        message: err.message,
      });
    }
  },
