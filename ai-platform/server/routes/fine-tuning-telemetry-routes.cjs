...
function normalizeFilters(query) {
  const filters = {};
  if (query.minRating !== undefined) filters.minRating = Number(query.minRating);
...
router.post('/export', authorize('admin:all'), function (req, res) {
  try {
    ...
    const filters = normalizeFilters(req.body && req.body.filters ? req.body.filters : {});
...
router.post('/label', authorize('admin:all'), function (req, res) {
  try {
    ...
    if (!eventId || !label) return sendError(res, 400, 'missing_event_or_label');
