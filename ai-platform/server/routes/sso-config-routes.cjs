  ...
  try {
    ...
    if (method === "saml" && (!saml?.entryPoint || !saml?.cert)) {
      return sendError(
        res,
        400,
        "saml.entryPoint and saml.cert are required for SAML",
      );
    }
    if (
      method === "oidc" &&
      (!oidc?.clientId || !oidc?.clientSecret || !oidc?.issuer)
    ) {
      return sendError(
        res,
        400,
    ...
    logger.info(
      `[SSO] Config created for org ${orgId}: ${config.providerId} (${method})`,
    );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updated = ssoConfigStore.updateConfig(
    ...
    try {
      ...
      if (config.method === "saml") {
        ...
        results.checks.push({
          ...
          detail: config.saml?.cert
            ? "PEM certificate configured"
            : "Missing certificate",
        });
        results.checks.push({
        ...
        results.checks.push({
          ...
          detail: config.oidc?._decryptedSecret
            ? "Decryptable"
            : "Missing or corrupt",
        });
        results.checks.push({
      ...
      results.checks.push({
        ...
        detail:
          config.domain ||
          "No domain specified — manual provider selection required",
      });
