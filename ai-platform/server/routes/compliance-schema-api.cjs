...
function buildSchemaPayload() {
  return {
    ...
    integration: {
      ...
      privacyNote:
        "No source code ever uploaded. Scan runs locally; only abstract tokens leave the machine.",
...
function registerComplianceSchemaRoute(app, options = {}) {
  ...
  app.get(`${basePath}/schema/anonymized-v1`, (_req, res) => {
    ...
    res.json({
      ...
      fields: {
        repoFingerprint:
          "SHA-256 hash of project root (salted) — identifies repo without revealing path",
        rulesFingerprint:
          "SHA-256 hash of active ruleset names — proves which rules were evaluated",
        gate: {
          pass: "boolean",
        ...
        metrics:
          "Aggregated counts only — no file paths, no code snippets, no descriptions",
        issues:
          "Array of abstract tokens: { i, t (type code), c (category), s (severity), n (count) }",
