...
router.get("/stats", function (req, res) {
  try {
    res.json({ success: true, stats: signingStore.getStats() });
  } catch (err) {
    sendError(res, 500, "signing_stats_failed", { message: err.message });
  }
});

router.get("/config", function (req, res) {
  try {
    res.json({ success: true, config: signingStore.getConfig() });
  } catch (err) {
    sendError(res, 500, "signing_config_failed", { message: err.message });
  }
});
...
router.put("/config", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[WebhookSigning] Config updated by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json(result);
  } catch (err) {
    sendError(res, 400, "signing_config_update_failed", {
      message: err.message,
    });
  }
});
...
router.post("/config/reset", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[WebhookSigning] Config reset by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json(result);
  } catch (err) {
    sendError(res, 500, "signing_config_reset_failed", {
      message: err.message,
    });
  }
});
...
router.get("/keys", function (req, res) {
  try {
    ...
    sendError(res, 500, "keys_list_failed", { message: err.message });
  }
});
...
router.post("/keys/generate", authorize("admin:all"), function (req, res) {
  try {
    var keyId =
      req.body.keyId || "key-" + crypto.randomBytes(6).toString("hex");
    ...
    sendError(res, 400, "key_generation_failed", { message: err.message });
  }
});
...
router.delete("/keys/:keyId", authorize("admin:all"), function (req, res) {
  try {
    ...
    sendError(res, 500, "key_delete_failed", { message: err.message });
  }
});
...
router.get("/deliveries", function (req, res) {
  try {
    ...
    res.json({
      success: true,
      deliveries: signingStore.getDeliveryHistory(limit),
    });
  } catch (err) {
    sendError(res, 500, "deliveries_list_failed", { message: err.message });
  }
});
...
router.post("/deliveries/clear", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[WebhookSigning] Delivery history cleared by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json(result);
  } catch (err) {
    sendError(res, 500, "deliveries_clear_failed", { message: err.message });
  }
});
...
router.post("/test-sign", authorize("admin:all"), function (req, res) {
  try {
    var payload =
      req.body.payload || JSON.stringify({ test: true, timestamp: Date.now() });
    var keyId = req.body.keyId || null;
    var result = signingStore.testSign(payload, keyId);
    res.json({ success: true, result: result });
  } catch (err) {
    sendError(res, 500, "test_sign_failed", { message: err.message });
  }
});
...
router.post("/verify", function (req, res) {
  try {
    ...
    if (!payload || !signature || !keyId)
      return sendError(res, 400, "payload_signature_and_keyId_required");
    var result = signingStore.verifySignature(payload, signature, keyId);
    res.json({ success: true, result: result });
  } catch (err) {
    sendError(res, 500, "verify_failed", { message: err.message });
  }
});
