...
router.get("/providers", function (req, res) {
  try {
    res.json({
      success: true,
      providers: failoverStore.getAllProviderStatuses(),
    });
  } catch (err) {
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      logger.info(
        "[ProviderFailover] Circuit reset for " +
          req.params.id +
          " by " +
      ...
      res.json({
        success: true,
        provider: failoverStore.getProviderStatus(req.params.id),
      });
    } catch (err) {
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      logger.info(
        "[ProviderFailover] All circuits reset by " +
          ((req.user && req.user.email) || "admin"),
      );
      res.json({
        success: true,
...
router.put(
  ...
  function (req, res) {
    try {
      ...
      res.json({
        success: true,
        provider: failoverStore.getProviderStatus(providerId),
...
router.put("/config", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[ProviderFailover] Config updated by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json({ success: true, config: result.config });
  } catch (err) {
    sendError(res, 500, "failover_config_update_failed", {
      message: err.message,
    });
  }
});
...
router.post("/config/reset", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[ProviderFailover] Config reset by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json({ success: true, config: result.config });
  } catch (err) {
    sendError(res, 500, "failover_config_reset_failed", {
      message: err.message,
    });
  }
});
...
router.post("/stats/reset", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[ProviderFailover] Stats reset by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json({ success: true });
  } catch (err) {
