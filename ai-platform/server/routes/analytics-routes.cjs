  ...
  try {
    ...
    for (const scan of result.scans) {
      ...
      for (const [category, count] of Object.entries(cats)) {
        ...
        if (ticketTargetFilter && (!ticketEntry || ticketEntry.ticketTarget !== ticketTargetFilter))
          continue;
  ...
  try {
    ...
    const jiraPayload = {
      fields: {
        ...
        issuetype: {
          name: guidance.priority === 'critical' || guidance.priority === 'high' ? 'Bug' : 'Task',
        },
        priority: {
          name:
...
// POST /api/analytics/violations/mark-ticketed — mark a violation as ticketed
router.post('/violations/mark-ticketed', authorize('write:tickets'), (req, res) => {
  try {
...
// POST /api/analytics/violations/unmark-ticketed — remove ticket status from a violation
router.post('/violations/unmark-ticketed', authorize('write:tickets'), (req, res) => {
  try {
...
// POST /api/analytics/violations/bulk-mark-ticketed — mark multiple violations as ticketed
router.post('/violations/bulk-mark-ticketed', authorize('write:tickets'), (req, res) => {
  try {
...
// POST /api/analytics/violations/bulk-unmark-ticketed — remove ticket status from multiple violations
router.post('/violations/bulk-unmark-ticketed', authorize('write:tickets'), (req, res) => {
  try {
...
router.get('/violations/summary', (req, res) => {
  try {
    ...
    const coverage =
      totalViolations > 0 ? Math.round((ticketedViolations / totalViolations) * 100) : 0;
...
router.post('/violations/bulk-dispatch-ticket', async (req, res) => {
  try {
      ...
      if (ticketTarget === 'jira') {
        ...
        body = {
          fields: {
            ...
            issuetype: {
              name:
                guidance.priority === 'critical' || guidance.priority === 'high' ? 'Bug' : 'Task',
            },
            priority: {
              name:
                guidance.priority === 'critical'
    ...
    for (let i = 0; i < violations.length; i += MAX_CONCURRENT) {
      ...
      const batchResults = await Promise.all(batch.map((v) => dispatchOne(v.scanId, v.category)));
...
router.post('/webhook/configs', authorize('write:all'), (req, res) => {
  try {
...
// DELETE /api/analytics/webhook/configs/:target — delete a webhook configuration
router.delete('/webhook/configs/:target', authorize('delete:all'), (req, res) => {
  try {
    const orgId = getOrgId(req);
...
// POST /api/analytics/report/schedules — create or update a report schedule
router.post('/report/schedules', authorize('write:all'), (req, res) => {
  try {
    const {
      id,
      name,
...
// DELETE /api/analytics/report/schedules/:id — delete a report schedule
router.delete('/report/schedules/:id', authorize('delete:all'), (req, res) => {
  try {
    const orgId = getOrgId(req);
    const { id } = req.params;
    const oldSchedule = reportScheduleStore.getSchedule(id, orgId);
...
router.post('/record', authorize('write:scans'), (req, res) => {
  try {
