...
const {
  ...
  anonymizeProjectId,
} = require("../lib/dashboard-analytics.cjs");
...
const REMEDIATION_GUIDANCE = {
  "EU AI Act — Prohibited Practices": {
    ...
    priority: "critical",
    description:
      "Remove the prohibited AI practice entirely. EU AI Act Article 5 bans certain uses (social scoring, manipulative AI, real-time biometric ID in public spaces).",
    steps: [
      "Identify the prohibited use case in the codebase",
      "Remove or replace the functionality with a compliant alternative",
      "Document the decision in the AI conformity assessment",
    ],
  },
  "EU AI Act — High-Risk Obligations": {
    ...
    priority: "critical",
    description:
      "High-risk AI systems require risk management, data governance, transparency, and human oversight under EU AI Act Articles 8-15.",
    steps: [
      "Implement a risk management system",
      "Ensure training data quality and bias mitigation",
      "Add human oversight mechanisms",
      "Create technical documentation and logging",
    ],
  },
  "California SB 1047 — Critical Harm": {
    ...
    priority: "critical",
    description:
      "SB 1047 requires safety evaluations for frontier models capable of causing critical harm. Implement safety shutdown protocols and red-team testing.",
    steps: [
      "Conduct safety evaluation before deployment",
      "Implement emergency shutdown capability",
      "Document critical harm risk assessment",
  ...
  "GDPR — Data Subject Rights": {
    ...
    priority: "high",
    description:
      "Ensure data subject rights are implementable: right to access, rectification, erasure, and portability under GDPR Articles 15-20.",
    steps: [
      "Add data export endpoints for portability",
      "Implement deletion workflows for erasure requests",
  ...
  "HIPAA — PHI Exposure": {
    ...
    priority: "critical",
    description:
      "Protected Health Information must be encrypted at rest and in transit. Implement access controls and audit logging per HIPAA Security Rule.",
    steps: [
      "Encrypt all PHI fields at rest (AES-256)",
      "Enforce TLS 1.2+ for all PHI transit",
  ...
  "OWASP — Injection": {
    ...
    priority: "high",
    description:
      "Use parameterized queries and input validation to prevent SQL, NoSQL, and command injection attacks.",
    steps: [
      "Replace string concatenation with parameterized queries",
      "Add input validation and sanitization",
      "Use ORM query builders where possible",
      "Implement allowlists for user input",
    ],
  },
  "OWASP — Broken Access Control": {
    ...
    priority: "high",
    description:
      "Enforce proper authorization checks on every protected resource. Implement deny-by-default and principle of least privilege.",
    steps: [
      ...
      "Add ownership checks on resource access",
    ],
  },
  "OWASP — Security Misconfiguration": {
    ...
    priority: "medium",
    description:
      "Apply security hardening to all components: disable default credentials, remove unused features, set secure headers.",
    steps: [
      "Remove default credentials and change default ports",
  ...
  "OWASP — Vulnerable Dependencies": {
    ...
    priority: "high",
    description:
      "Update or replace dependencies with known CVEs. Use dependency scanning in CI to catch vulnerabilities early.",
    steps: [
      "Run npm audit / pip audit / snyk scan",
      "Upgrade vulnerable packages to safe versions",
  ...
  "Hardcoded Secrets": {
    ...
    priority: "critical",
    description:
      "Move all secrets to environment variables or a secrets manager. Never hardcode API keys, passwords, or tokens in source code.",
    steps: [
  ...
  "Unsafe Deserialization": {
    ...
    priority: "high",
    description:
      "Replace unsafe deserialization (pickle, eval, JSON.parse with reviver) with schema-validated parsing.",
    steps: [
  ...
  "Missing Input Validation": {
    ...
    priority: "medium",
    description:
      "Add input validation on all API endpoints and user-facing forms. Use schema validation libraries.",
    steps: [
      "Define input schemas for all endpoints",
      "Add validation middleware (zod, joi, pydantic)",
  ...
  "Improper Error Handling": {
    ...
    priority: "low",
    description:
      "Wrap operations in try/catch blocks with meaningful error messages. Avoid exposing stack traces to users.",
    steps: [
  ...
  "Non-deterministic Output": {
    ...
    priority: "medium",
    description:
      "AI model outputs must be reproducible for auditing. Set random seeds and log model parameters.",
    steps: [
      "Set fixed random seeds for reproducibility",
      "Log model version and parameters with each output",
      "Implement output caching for identical inputs",
  ...
  "Missing Model Card": {
    ...
    priority: "medium",
    description:
      "Create a model card documenting intended use, training data, performance metrics, and known limitations.",
    steps: [
      ...
      "Publish model card with the model artifact",
    ],
  },
  "Bias Detection Gap": {
    strategy: "evaluate",
    priority: "high",
    description:
      "Implement bias evaluation across protected demographics. Add fairness metrics to the model evaluation pipeline.",
    steps: [
      "Define fairness metrics (demographic parity, equalized odds)",
      "Test model outputs across demographic groups",
      "Add bias detection to CI evaluation",
      "Document mitigation strategies for detected biases",
    ],
  },
  _default: {
    strategy: "review",
    priority: "medium",
    description:
      "Review the finding and apply appropriate remediation based on severity and context.",
    steps: [
      "Analyze the finding in context of the codebase",
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      if (!summary)
        return sendError(res, 404, "org_not_found", {
          message: "No scans recorded for this organization",
        });
...
router.get("/export", (req, res) => {
  try {
    ...
    if (format === "json") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="analytics-export-${new Date().toISOString().slice(0, 10)}.json"`,
      );
      res.json({
    ...
    rows.push(
      `# Total Files,${stats.totalFilesAnalyzed ?? stats.totalFiles ?? ""}`,
    );
...
router.get("/violations", (req, res) => {
  try {
    ...
    const slaBreachedFilter =
      (req.query.slaBreached || "").toLowerCase() === "true";
    const now = Date.now();
    ...
    for (const scan of result.scans) {
      ...
      for (const [category, count] of Object.entries(cats)) {
        ...
        if (
          ticketTargetFilter &&
          (!ticketEntry || ticketEntry.ticketTarget !== ticketTargetFilter)
        )
          continue;

        const guidance =
          REMEDIATION_GUIDANCE[category] || REMEDIATION_GUIDANCE._default;
        const slaLimit =
          SLA_THRESHOLDS[guidance.priority] || SLA_THRESHOLDS.medium;
...
router.post("/violations/ticket-payload", (req, res) => {
  try {
    ...
    if (!scan)
      return sendError(res, 404, "scan_not_found", {
        message: "Scan not found",
      });

    const count = (scan.categoryCounts || {})[category] || 0;
    if (count === 0)
      return sendError(res, 404, "category_not_found", {
        message: "Category not found in this scan",
      });
    ...
    const markdownDescription = descriptionLines
      .join("\n")
      .replace(/h2\.\s/g, "## ")
      .replace(/\*/g, "**");
    ...
    const jiraPayload = {
      fields: {
        ...
        issuetype: {
          name:
            guidance.priority === "critical" || guidance.priority === "high"
              ? "Bug"
              : "Task",
        },
        priority: {
...
router.post("/violations/mark-ticketed", (req, res) => {
  try {
    ...
    const orgId = getOrgId(req);
    const entry = ticketStatusStore.markTicketed(
      scanId,
      category,
      ticketRef,
      ticketTarget,
...
router.post("/violations/bulk-mark-ticketed", (req, res) => {
  try {
    ...
    if (!Array.isArray(violations) || violations.length === 0)
      return sendError(res, 400, "violations array is required");
    if (!ticketRef) return sendError(res, 400, "ticketRef is required");
    ...
    for (const v of violations) {
      ...
      try {
        ...
        auditLogger.log({
          ...
          action: "CREATE",
          entity: "ticket_status",
          entityId: `${v.scanId}::${v.category}`,
          newValue: entry,
          metadata: { route: req.originalUrl, bulk: true, ticketRef },
        });
        results.push(entry);
        succeeded++;
      } catch {
...
router.post("/violations/bulk-unmark-ticketed", (req, res) => {
  try {
    ...
    for (const v of violations) {
      ...
      try {
        const oldVal = ticketStatusStore.getTicketStatus(
          v.scanId,
          v.category,
          orgId,
        );
        ...
        auditLogger.log({
          ...
          entityId: `${v.scanId}::${v.category}`,
          oldValue: oldVal,
          metadata: { route: req.originalUrl, bulk: true },
        });
        succeeded++;
      } catch {
        failed++;
...
router.get("/violations/summary", (req, res) => {
  try {
    ...
    const coverage =
      totalViolations > 0
        ? Math.round((ticketedViolations / totalViolations) * 100)
        : 0;
...
router.get("/violations/export", (req, res) => {
  try {
    ...
    if (format === "json") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="compliance-ledger-${new Date().toISOString().slice(0, 10)}.json"`,
      );
      res.json({
        exportedAt: new Date().toISOString(),
        ...
        filters: {
          ...
          slaBreachedOnly: slaBreachedFilter,
        },
        violations,
      });
      return;
    ...
    const rows = violations.map((v) =>
      [
        ...
        v.gateStatus,
        v.priority,
        v.remediationStrategy,
        v.remediationDescription,
        v.remediationSteps,
        v.ticketed,
        v.ticketRef,
        v.ticketTarget,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="compliance-ledger-${new Date().toISOString().slice(0, 10)}.csv"`,
    );
    res.send(csv);
  } catch (err) {
...
router.post("/violations/dispatch-ticket", async (req, res) => {
  try {
    ...
    if (!config || !config.apiUrl) {
      return sendError(res, 400, "webhook_not_configured", {
        message: `No webhook configuration found for ${ticketTarget}. Configure it first.`,
      });
    }
    ...
    if (ticketTarget === "jira") {
      ...
      return sendError(res, 400, "invalid_target", {
        message: `Unknown target: ${ticketTarget}`,
      });
    }
    ...
    if (!apiResp.ok) {
      logger.error(
        `[Analytics] Dispatch to ${ticketTarget} failed: ${apiResp.status}`,
    ...
    if (ticketTarget === "jira" && respData.key) {
      ticketRef = respData.self
        ? `${config.apiUrl.split("/rest/")[0]}/browse/${respData.key}`
        : respData.key;
    ...
    if (ticketRef) {
      ticketStatusStore.markTicketed(
        scanId,
        category,
        ticketRef,
        ticketTarget,
...
router.post("/violations/bulk-dispatch-ticket", async (req, res) => {
  try {
    ...
    if (!config || !config.apiUrl) {
      return sendError(res, 400, "webhook_not_configured", {
        message: `No webhook configuration found for ${ticketTarget}.`,
      });
    }
    ...
    async function dispatchOne(scanId, category) {
      // Generate payload
      const result = analyticsStore.getScans({
        orgId,
        limit: 100000,
        offset: 0,
      });
      const scan = result.scans.find((s) => s.scanId === scanId);
      if (!scan)
        return { scanId, category, success: false, error: "scan_not_found" };
      const count = (scan.categoryCounts || {})[category] || 0;
      if (count === 0)
        return {
          scanId,
        ...
        try {
          ...
          if (apiResp.status === 429 || apiResp.status >= 500) {
            ...
            return {
              ...
              category,
              success: false,
              error: `rate_limited_${apiResp.status}`,
          ...
          if (!apiResp.ok) {
            return {
              ...
              category,
              success: false,
              error: `api_${apiResp.status}`,
              response: respData,
            };
          }
          let ticketRef = "";
          if (ticketTarget === "jira" && respData.key)
            ticketRef = `${config.apiUrl.split("/rest/")[0]}/browse/${respData.key}`;
          else if (ticketTarget === "linear" && respData.data?.id)
            ticketRef = respData.data.url || respData.data.id;
          else if (ticketTarget === "github" && respData.html_url)
            ticketRef = respData.html_url;
...
router.post("/webhook/configs", (req, res) => {
  try {
    ...
    const config = webhookConfigStore.setConfig(
      target,
      { apiUrl, authToken, projectKey, teamId, repoOwner, repoName },
      orgId,
    );
    auditLogger.log({
      orgId,
...
router.delete("/webhook/configs/:target", (req, res) => {
  try {
    ...
    sendError(res, 500, "webhook_config_delete_failed", {
      message: err.message,
    });
  }
});
...
router.post("/report/schedules", (req, res) => {
  try {
    ...
    if (!Array.isArray(recipients) || recipients.length === 0)
      return sendError(res, 400, "recipients array is required");
    const validFreq = ["daily", "weekly", "monthly"];
    if (frequency && !validFreq.includes(frequency))
      return sendError(res, 400, "frequency must be daily, weekly, or monthly");
    const orgId = getOrgId(req);
    const oldSchedule = reportScheduleStore.getSchedule(id, orgId);
    const schedule = reportScheduleStore.setSchedule(
      id,
      {
        name,
        enabled,
    ...
    auditLogger.log({
      ...
      actorId: req.user?.id,
      actorEmail: req.user?.email,
      action: oldSchedule ? "UPDATE" : "CREATE",
    ...
    sendError(res, 500, "report_schedule_save_failed", {
      message: err.message,
    });
  }
});
...
router.delete("/report/schedules/:id", (req, res) => {
  try {
    ...
    sendError(res, 500, "report_schedule_delete_failed", {
      message: err.message,
    });
  }
});
...
router.post("/report/schedules/:id/run", async (req, res) => {
  try {
    ...
    auditLogger.log({
      ...
      entity: "report_schedule",
      entityId: id,
      newValue: { success: result.success, error: result.error || null },
      metadata: { route: req.originalUrl },
    });
    if (result.success) {
      res.json({
        success: true,
        schedule: reportScheduleStore.getSchedule(id, orgId),
        result: result,
      });
    } else {
      sendError(res, 500, "run_failed", {
...
router.post("/record", (req, res) => {
  try {
    ...
    if (!summary || typeof summary !== "object")
      return sendError(res, 400, "summary object is required");
...
router.get("/summary", authenticate, async (req, res) => {
  try {
    ...
    const tenantSalt =
      process.env.TELEMETRY_SALT || "simplebeacon_production_fallback";
    ...
    logger.info(
      "[Analytics] Summary served for project signature: " + projectHash,
    );
