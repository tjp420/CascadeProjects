...
describe('GET /api/admin/improvement-report', () => {
  ...
  beforeEach(() => {
    ...
    delete require.cache[require.resolve('../../cron/internal-improvement-report.cjs')];
  });
  ...
  it('returns 200 with structured JSON for admin users', async () => {
    const { recordCiTelemetryEvent, summarizeAllTelemetry, resolveOrgKey } = require('../../lib/ci-telemetry-store.cjs');
    const { generateImprovementReportMarkdown } = require('../../cron/internal-improvement-report.cjs');
  ...
  it('response contains no PII fields', async () => {
    const { recordCiTelemetryEvent, summarizeAllTelemetry } = require('../../lib/ci-telemetry-store.cjs');
    const { generateImprovementReportMarkdown } = require('../../cron/internal-improvement-report.cjs');
    ...
    assert.ok(!fullOutput.includes('user@example.com'), 'No emails in response');
    assert.ok(!fullOutput.includes('/secret/'), 'No file paths in response');
    assert.ok(!fullOutput.includes('secret description'), 'No issue descriptions in response');
  });
