...
describe("Health Diagnostics — Integration Tests", () => {
  ...
  describe("GET / (endpoint)", () => {
    it("AC1: returns 200 with UP status when all checks pass", async () => {
      ...
      try {
        ...
        assert.ok(
          res.body.checks.encryption,
          "should include encryption check",
        );
    ...
    it("AC2: returns 503 when encryption key is missing (DOWN)", async () => {
      ...
      try {
        ...
        assert.strictEqual(
          res.body.checks.encryption.detail.reason,
  ...
  describe("checkDatastoreIntegrity", () => {
    it("AC5: reports INITIALIZED_EMPTY for missing data files", () => {
      ...
      const oldWebhook = hadWebhook
        ? fs.readFileSync(webhookPath, "utf8")
        : null;
      ...
      try {
        ...
        assert.strictEqual(result.status, "UP");
        const webhookResult = result.files.find(
          (f) => f.file === "webhook-configs.json",
        );
        const aiKeysResult = result.files.find(
          (f) => f.file === "user-ai-keys.json",
        );
    ...
    it("AC6: returns UP with entry count for valid JSON data files", () => {
      ...
      fs.writeFileSync(
        aiKeysPath,
        JSON.stringify({ user1: { provider: "openai" } }),
      );
  ...
  describe("checkMemoryGauge", () => {
    it("AC8: returns UP with memory metrics", () => {
      ...
      assert.ok(
        result.detail.criticalThresholdMB,
        "should include critical threshold",
      );
    });
  });
  ...
  describe("runHealthChecks (aggregate)", () => {
    it("AC9: returns aggregate result with timestamp and all sub-checks", () => {
      ...
      assert.ok(
        ["UP", "DEGRADED", "DOWN"].includes(result.status),
