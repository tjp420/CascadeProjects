...
const { loadAgencyBranding, saveAgencyBranding } = require('../lib/agency-branding-store.cjs');
...
function setupFlexibleAnalyzeAPI(app, options = {}) {
  ...
  const closedVaultMode =
    opts.closedVaultMode === true || process.env.SIMPLEBEACON_CLOSED_VAULT === 'true';
      ...
      res.json({
        ...
        analysisTypes: [
          ...
          },
          {
            ...
            description: 'Filesystem sprint scan — optional strategic insights layer',
          },
          {
            id: 'codebase',
            label: 'Codebase analysis',
            description: 'Technical debt, broken files, debug artifacts, placeholders',
          },
          {
            id: 'file-reduction',
            label: 'File reduction',
            description: 'Build artifacts, duplicate assets, unused files (dry-run)',
          },
          {
            id: 'data-quality',
            label: 'Data quality',
            description: 'Config sprawl, env keys, stale data, privacy, lineage, shape drift',
          },
        ],
        roadmapInsightsModes: [
          {
            ...
            description: 'Pure code-roadmap-generator — no strategic insights block',
          },
          {
            id: 'deterministic',
            label: 'Deterministic insights',
            description: 'Rule-based executive summary and risk scoring — no LLM',
          },
          {
            id: 'llm',
            label: 'LLM strategic layer',
            description: 'LLM interprets aggregated metrics only — data stays deterministic',
          },
        ],
        understandingModes: [
          {
            id: 'off',
            label: 'Static only',
            description: 'Pattern-based static analysis without semantic/context layers',
          },
          {
            id: 'deterministic',
            label: 'Semantic + context',
            description: 'Business logic heuristics, intent, git/doc context — no LLM',
          },
          {
            id: 'llm',
          ...
          },
          {
            ...
            description: 'Machine learning-inspired pattern detection with intelligent fallback',
          },
        ],
        analysisProfiles: [
          {
        ...
        scanProfiles: [
          ...
          },
          {
            id: 'game-dev',
            label: 'Game dev',
            description: 'Web + game modding extensions (ZScript, ACS, DECORATE, GLSL, Lua)',
          },
          {
            id: 'universal',
            label: 'Universal',
            description: 'All registered language families — used by dashboard scans',
          },
        ],
  ...
  app.put('/api/simplebeacon/agency/branding', authenticate, requireDashboardWrite, (req, res) => {
    try {
      const body = req.body || {};
      const orgId =
        String(body.org_id || body.orgId || req.query.org_id || 'default').trim() || 'default';
  ...
  app.post('/api/analyze/flexible', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const debug =
          process.env.DEBUG_CLIENT_ERRORS === '1' || process.env.DEBUG_CLIENT_ERRORS === 'true';
      ...
      const analysisType = isWebsite
        ? 'mock-scan'
        : await resolveAnalysisType(body.analysisType, projectPath, pathLooksLikeMockScan);
      const registry = await ensureRegistry(baseDir);
      ...
      if (analysisType === 'complete') {
        ...
        const simplebeaconFindings =
          results.simplebeacon?.findings || results.simplebeacon?.rawIssues || null;
        ...
        const [fileReductionResult, removableFilesResult, npmAuditResult, dataCleanupResult] =
        ...
        const summary = {
          ...
          simplebeaconIssues:
            simplebeaconReport?.gate?.blockingCount ?? simplebeaconReport?.issueCount ?? null,
      ...
      if (analysisType === 'compliance') {
        ...
        const dataCleanup =
          dataCleanupResult.status === 'fulfilled' ? dataCleanupResult.value : null;
      ...
      (async () => {
        ...
        try {
          ...
          const summary = {
            ...
            simplebeaconIssues:
              results.simplebeacon?.gate?.blockingCount ?? results.simplebeacon?.issueCount ?? null,
          ...
          const reportJson = {
            ...
            integrityScore:
              results.simplebeacon?.gate?.score ?? results.codebase?.summary?.healthScore ?? 100,
            qualityScore:
              results.simplebeacon?.gate?.score ?? results.codebase?.summary?.healthScore ?? 100,
            generatedAt: new Date().toISOString(),
          };
        ...
        logger.error('[Async Scan] unhandled background error:', safeErrorMessage(err))
      );
  ...
  const uploadMulter = multer({
    ...
    limits: {
      files: 100000,
      fileSize: 5 * constants.BYTES_PER_KB * constants.BYTES_PER_KB * constants.BYTES_PER_KB,
  ...
  async function validateLicenseToken(token) {
    ...
    const record = Object.values(store.subscriptions || {}).find((s) => s.licenseToken === token);
    const VALID_TIERS = [
      'community',
      'clearance499',
      'agency999',
    ...
    const secret =
      ...
      (process.env.NODE_ENV === 'production' ? null : 'simplebeacon-dev-insecure');
      ...
      try {
        ...
        (async () => {
          ...
          const tierAllowed = (analyzer) =>
            ...
            (!instantTierLimited || ['simplebeacon', 'mock-scan', 'codebase'].includes(analyzer));
          ...
          if ((analysisType === 'codebase' || runComplete) && tierAllowed('codebase')) {
            results.codebase = await runAnalyzer('codebase', () =>
              getAnalyzeCodebase()(projectDir, { context: 'dashboard', scanProfile: 'default' })
            );
          }
          ...
          if ((analysisType === 'file-reduction' || runComplete) && tierAllowed('file-reduction')) {
            results.fileReduction = await runAnalyzer('file-reduction', () =>
              scanFileMergerReduction(projectDir, { includeRepositoryInventory: true })
            );
          }
          if (
          ...
          if ((analysisType === 'roadmap' || runComplete) && tierAllowed('roadmap')) {
            results.roadmap = await runAnalyzer('roadmap', () =>
              generateCodeRoadmap(projectDir, {}, { scanReport: report, includeFiles: true })
            );
          }
          ...
          if (
            ...
            if (specific && !specific.error) {
              ...
              const analysisKey =
                analysisKeyMap[analysisType] || `_${analysisType.replace(/-/g, '')}Analysis`;
          ...
          (async () => {
            try {
              const aiVerdict = await generateAutomatedVerdict(sanitizeReportForAi(report), {
            ...
            logger.warn('[Upload Directory] AI Analyst unhandled error:', safeErrorMessage(err))
          );

          // --- EU AI Act Article 14 Human Oversight Evaluator (fire-and-forget) ---
          (async () => {
            try {
              const euCompliance = await evaluateHumanOversightCompliance(report, {
                projectPath: projectDir,
              });
              const jobMeta = scanJobs.get(scanId) || {};
            ...
            logger.warn('[Upload Directory] EU evaluator unhandled error:', safeErrorMessage(err))
          );
        })();
  ...
  app.get('/api/analyze/codebase', async (req, res) => {
    try {
      ...
      const scanProfile = resolveScanProfile({ scanProfile: req.query.scanProfile }, 'dashboard');
      const scanContext = String(
      ...
      const analyzeOptions = {
        ...
        includeBrowserAnalyzers:
          req.query.includeBrowserAnalyzers === '1' || req.query.includeBrowserAnalyzers === 'true',
      ...
      return sendAnalyzeJson(
        res,
        { success: true, data: report, scanProfile, scanContext, understandingMode },
        200,
        sendAnalyzeJsonOpts
      );
    } catch (error) {
  ...
  app.get('/api/analyze/inventory', async (req, res) => {
    try {
      ...
      const fullTree =
        req.query.fullDirectoryScan === 'true' || req.query.fullDirectoryScan === '1';
      if (fullTree) {
  ...
  app.post('/api/analyze/export-bundle', async (req, res) => {
    ...
    try {
      ...
      const internalDashboard =
        process.env.SIMPLEBEACON_INTERNAL_DASHBOARD === 'true' || body.internalDashboard === true;
  ...
  app.post('/api/simplebeacon/export/certificate', async (req, res) => {
    try {
      ...
      const clientName =
        String(body.client_name || body.clientName || storedProfile?.clientName || '').trim() ||
        'Client';
      const projectName =
        String(body.project_name || body.projectName || storedProfile?.projectName || '').trim() ||
        'Project';
      ...
      if (format === 'zip') {
        ...
        const completeScan =
          normalizeCompleteScanInput(body.completeScan) || body.completeScan || report;
        const zipResult = await buildAnalyzeExportZipStream(completeScan, {
          ...
          hasAuditDeliverableAccess:
            (!publicGateEnabled && !closedVaultMode) || hasAdminDeliverableAccess(req),
          milestone,
          client: clientName,
          projectName,
  ...
  app.get('/api/analyze/list-directories', async (req, res) => {
    try {
      ...
      const isRoot =
        targetPath === parent ||
        (!isPathWithinRoots(parent, allowedRoots) && !isPathAncestorOfRoots(parent, allowedRoots));
      res.json({
        success: true,
  ...
  app.post('/api/analyze/summary', async (req, res) => {
    try {
      ...
      const enhanced = await summarizeScanWithProvider(providerOpts.providerId, payload, {
        projectPath,
        userCredentials,
        registry,
  ...
  async function downloadGithubZipball(repoUrl, destPath) {
    ...
    await new Promise((resolve, reject) => {
      ...
      https
        .get(zipUrl, { headers: { 'User-Agent': 'SimpleBeacon/1.0' } }, handleResponse)
        .on('error', reject);
    });
      ...
      try {
        ...
        if (!fs.existsSync(cliBin)) {
          ...
          const scanConfig = {
            ...
            fullDirectoryScanSkipDirs: [
              ...new Set([...defaultSkipDirs, ...(baseConfig.fullDirectoryScanSkipDirs || [])]),
            ],
        ...
        if (analysisType === 'codebase' || runComplete) {
          results.codebase = await runAnalyzer('codebase', () =>
            getAnalyzeCodebase()(tmpDir, { context: 'dashboard', scanProfile: 'universal' })
          );
        }
        ...
        if (analysisType === 'file-reduction' || runComplete) {
          results.fileReduction = await runAnalyzer('file-reduction', () =>
            scanFileMergerReduction(tmpDir, { includeRepositoryInventory: true })
          );
        }
        ...
        if (analysisType === 'roadmap' || runComplete) {
          results.roadmap = await runAnalyzer('roadmap', () =>
            generateCodeRoadmap(tmpDir, {}, { scanReport: report, includeFiles: true })
          );
        }
          ...
          if (specific && !specific.error) {
            if (analysisType === 'codebase') {
              ...
              reportJson = {
                ...
                issueCount:
                  specific.brief?.tiers?.safeNow?.directories?.length ?? report.issueCount,
              };
