...
} = require("../lib/file-merger-reduction-scanner.cjs");
...
if (_scanJobCleanupInterval && _scanJobCleanupInterval.unref)
  _scanJobCleanupInterval.unref();
const {
  loadAgencyBranding,
  saveAgencyBranding,
...
} = require("../lib/analyze-export-bundle.cjs");
const {
  generateAutomatedVerdict,
  emailAutomatedVerdict,
} = require("../lib/ai-analyst.cjs");
const { DEFAULT_OLLAMA_URL } = require("../services/ollama-client.cjs");
const {
  evaluateHumanOversightCompliance,
} = require("../lib/compliance-rules.cjs");
const {
  progressiveAnalysis,
  ANALYSIS_PROFILES,
  StreamingAnalyzer,
...
const {
  detectMLPatterns,
} = require("../lib/code-understanding/ml-pattern-detector.cjs");
...
function resolveAnalyzeAllowedRoots(baseDir, options = {}) {
  ...
  const opts =
    options && typeof options === "object" && !Array.isArray(options)
      ...
      : {};
  const monorepoRoot =
    opts.monorepoRoot || path.resolve(path.join(baseDir, ".."));
...
function setupFlexibleAnalyzeAPI(app, options = {}) {
  if (!app || typeof app.use !== "function") {
    throw new TypeError(
      "setupFlexibleAnalyzeAPI requires a valid Express app instance",
    );
  }
  const opts =
    options && typeof options === "object" && !Array.isArray(options)
  ...
  const publicGateEnabled =
    opts.publicGateEnabled === true ||
    (opts.publicGateEnabled !== false &&
      process.env.SIMPLEBEACON_PUBLIC_GATE === "true");
  const closedVaultMode =
    opts.closedVaultMode === true ||
    process.env.SIMPLEBEACON_CLOSED_VAULT === "true";
  const auditCheckoutUrl =
    opts.auditCheckoutUrl ||
    process.env.SIMPLEBEACON_AUDIT_CHECKOUT_URL ||
    "mailto:admin@simplebeacon.ai?subject=Unlock%20Pre-Launch%20Audit%20Report";

  const sendAnalyzeJsonOpts = {
    publicGateEnabled,
    applyPublicGateToAnalyzeResponse,
  };
  ...
  function hasAdminDeliverableAccess(req) {
    ...
    if (
      ...
      return true;
    if (user.trustLevel === "platinum" || user.trustLevel === "operator")
      return true;
    return false;
  }
  ...
  app.get(
    ...
    (req, res) => {
      ...
      const debugClientErrors =
        process.env.DEBUG_CLIENT_ERRORS === "1" || req.query.debug === "1";
      const response = {
        success: true,
        publicGateLocked: locked,
        closedVaultMode,
        hasAuditDeliverableAccess:
          (!publicGateEnabled && !closedVaultMode) || adminAccess,
        auditCheckoutUrl,
        auditPriceLabel: "$499",
      };
  ...
  if (closedVaultMode) {
    app.use("/api/analyze", (req, res) => {
      res.status(403).json({
        ...
        error:
          "Public scanning is disabled. Book a private pre-launch audit at simplebeacon.ai.",
        checkoutUrl: auditCheckoutUrl,
        auditPriceLabel: "$499",
      });
    });
    logger.info(
      "[Flexible Analyze API] Closed vault — /api/analyze disabled on public deploy",
    );
    return;
  }
  ...
  if (process.env.REQUIRE_AUTH === "true") {
    ...
    const publicAnalyzePaths = new Set([
      "/upload-directory",
      "/progress",
      "/providers",
      "/test-sources",
        ...
        analysisTypes: [
          {
            id: "auto",
            label: "Auto-detect",
            description: "Mock scan if JSON found, else roadmap",
          },
          {
            id: "mock-scan",
            label: "Mock data scan",
            description: "Run mock-data-scanner on the directory",
          },
          {
            id: "roadmap",
            label: "Project roadmap",
            description:
              "Filesystem sprint scan — optional strategic insights layer",
          },
          {
            id: "codebase",
            label: "Codebase analysis",
            description:
              "Technical debt, broken files, debug artifacts, placeholders",
          },
          {
            id: "file-reduction",
            label: "File reduction",
            description:
              "Build artifacts, duplicate assets, unused files (dry-run)",
          },
          {
            id: "data-quality",
            label: "Data quality",
            description:
              "Config sprawl, env keys, stale data, privacy, lineage, shape drift",
          },
        ],
        roadmapInsightsModes: [
          {
            ...
            description:
              "Pure code-roadmap-generator — no strategic insights block",
          },
          {
            id: "deterministic",
            label: "Deterministic insights",
            description:
              "Rule-based executive summary and risk scoring — no LLM",
          },
          {
            id: "llm",
            label: "LLM strategic layer",
            description:
              "LLM interprets aggregated metrics only — data stays deterministic",
          },
        ],
        understandingModes: [
          {
            ...
            label: "Static only",
            description:
              "Pattern-based static analysis without semantic/context layers",
          },
          {
            id: "deterministic",
            label: "Semantic + context",
            description:
              "Business logic heuristics, intent, git/doc context — no LLM",
          },
          {
            id: "llm",
            label: "AI-enhanced understanding",
            description: "Adds AI narrative when a provider is configured",
          },
          {
            id: "enhanced",
            label: "Enhanced ML patterns",
            description:
              "Machine learning-inspired pattern detection with intelligent fallback",
          },
        ],
        analysisProfiles: [
          {
            ...
            description: "Fast, lightweight analysis for quick feedback",
          },
          {
            id: "balanced",
            label: "Balanced analysis",
            description: "Comprehensive analysis for general use",
          },
          {
            id: "comprehensive",
            label: "Comprehensive analysis",
            description: "Deep analysis with expert reviews",
          },
          {
            id: "realtime",
            label: "Real-time streaming",
            description: "Incremental analysis for live updates",
          },
        ],
        zscriptReport: {
          ...
          description:
            "Structure, CVAR map, function flow, and intensity diagnostics for GZDoom mods",
        },
        scanProfiles: [
          {
            id: "default",
            label: "Web + ZScript",
            description: "Default CLI profile — web stack plus .zs files",
          },
          {
            id: "game-dev",
            label: "Game dev",
            description:
              "Web + game modding extensions (ZScript, ACS, DECORATE, GLSL, Lua)",
          },
          {
            id: "universal",
            label: "Universal",
            description:
              "All registered language families — used by dashboard scans",
          },
        ],
  ...
  app.get("/api/simplebeacon/agency/branding", (req, res) => {
    try {
      const orgId =
        String(req.query.org_id || req.query.orgId || "default").trim() ||
        "default";
      ...
      sendError(
        res,
        500,
        toClientError(error, "Failed to load agency branding"),
      );
    }
  });

  app.put(
    "/api/simplebeacon/agency/branding",
    authenticate,
    requireDashboardWrite,
    (req, res) => {
      try {
        ...
        const orgId =
          String(
            body.org_id || body.orgId || req.query.org_id || "default",
          ).trim() || "default";
        const branding = saveAgencyBranding(
          baseDir,
          orgId,
          body.branding || body,
        );
        ...
        sendError(
          res,
          500,
          toClientError(error, "Failed to save agency branding"),
        );
      }
    },
  );
    ...
    if (/^https?:\/\/github\.com\//i.test(trimmed)) {
      ...
      if (!cacheExists) {
        ...
        try {
          ...
          logger.warn(
            "[Analyze/Flexible] git clone failed for GitHub URL, trying zipball fallback:",
    ...
    try {
      ...
      try {
        ...
        const debug =
          ...
          process.env.DEBUG_CLIENT_ERRORS === "true";
      ...
      analysisType = isWebsite
        ? "mock-scan"
        : await resolveAnalysisType(
            body.analysisType,
            projectPath,
            pathLooksLikeMockScan,
          );
      ...
      if (analysisType === "roadmap") {
        ...
        return res.json({
          ...
          aiProvider,
          roadmapInsightsMode: String(
            body.roadmapInsightsMode || "off",
          ).toLowerCase(),
      ...
      if (analysisType === "codebase") {
        const understandingMode = String(
          body.understandingMode || "off",
        ).toLowerCase();
        const scanProfile = resolveScanProfile(body, "dashboard");
        const scanContext = String(
          body.context || body.scanContext || body.scanMode || "dashboard",
        ).toLowerCase();
        const explicitMaxDeep = Number(body.maxDeepAnalyze);
        const maxDeepAnalyze =
          Number.isFinite(explicitMaxDeep) && explicitMaxDeep > 0
        ...
        (async () => {
          ...
          try {
            ...
            if (understandingMode !== "off") {
              ...
              report = await attachUnderstandingToCodebaseReport(
                report,
                projectPath,
                {
            ...
            logger.info(
              `[Async Codebase] ${asyncScanId} completed in ${Date.now() - startedAt}ms`,
            );
          } catch (err) {
            logger.error(
              "[Async Codebase] fatal error:",
        ...
        return res.status(202).json({
          ...
          total: fileCount,
          message:
            "Codebase scan started. Poll /api/analyze/progress?scanId=... for results.",
        });
      }
      ...
      if (analysisType === "workspace-health") {
        const report = await withTimeout(
          getAnalyzeCodebase()(projectPath, {
            includeEslint: false,
            context: "dashboard",
            scanProfile: "universal",
          }),
          120_000,
          "flexible workspace-health analysis",
        );
        const workspaceFindings = (report.findings || []).filter(
          (f) => f.category === "workspace-health",
        );
        ...
        if (simplebeaconFindings) {
          results.codebase = {
            ...
            summary: {
              ...
              severityCounts: results.simplebeacon.summary?.severityCounts ?? {
                high: 0,
                medium: 0,
                low: 0,
              },
      ...
      if (analysisType === "compliance") {
        const [scanResult, npmAuditResult, dataCleanupResult] =
          await Promise.allSettled([
            withTimeout(
              analyzeWithModel(baseDir, resolveModelId(registry, aiProvider), {
              ...
              120_000,
              "flexible compliance scan",
            ),
            withTimeout(
              runNpmAuditAsync(projectPath, { force: false }),
              120_000,
              "flexible compliance npm-audit",
            ),
            withTimeout(
              runDataCleanupScan(projectPath, { profile: "all" }),
              180_000,
              "flexible compliance data-cleanup",
            ),
          ]);
        const baseReport =
          scanResult.status === "fulfilled" ? scanResult.value.report : {};
        const npmAudit =
          npmAuditResult.status === "fulfilled" ? npmAuditResult.value : null;
        const dataCleanup =
          dataCleanupResult.status === "fulfilled"
            ? dataCleanupResult.value
            : null;
      ...
      if (
        ["openai", "anthropic", "ollama"].includes(aiProvider) &&
        providerConfigured(aiProvider, registry, userCredentials)
      ) {
        try {
          const providerOpts = resolveSummaryProvider(
            aiProvider,
            registry,
            userCredentials,
            DEFAULT_OLLAMA_URL,
          ...
          if (enhanced.enhanced && enhanced.summary) {
            ...
            scanResult.report.ggufAIInsights =
              scanResult.report.ggufAIInsights || {};
          ...
          if (enhanced.timingBuckets) {
            scanResult.report.inferenceMeta =
              scanResult.report.inferenceMeta || {};
      ...
      logger.error("[Analyze/Flexible] Analysis request failed:", {
        ...
        pathType: /^https?:\/\//i.test(rawPath || "")
          ? /^https?:\/\/github\.com\//i.test(rawPath || "")
            ? "github-url"
            : "website-url"
          : "local-path",
  ...
  app.post("/api/analyze/website", async (req, res) => {
    try {
      ...
      if (
        !isPaidTier(userTier) &&
        !getTierCapability(userTier, "websiteScans")
      ) {
        return sendError(
          res,
          403,
          "Website scanning requires a Pro or higher tier.",
        );
      }
    ...
    try {
      ...
      (async () => {
        ...
        try {
          ...
          updateProgress(
            "simplebeacon",
            "Walking files & running rule engines…",
          );
          try {
            const {
              runSimplebeaconScan,
            ...
            logger.warn(
              "[Async Scan] simplebeacon failed:",
              safeErrorMessage(err),
            );
          }
          ...
          updateProgress(
            "file-reduction",
            "Detecting mergeable & duplicate files…",
          );
          try {
            results.fileReduction = await scanFileMergerReduction(projectPath, {
              includeRepositoryInventory: true,
            });
            engines.push("file-reduction");
          } catch (err) {
            logger.warn(
              "[Async Scan] file-reduction failed:",
              safeErrorMessage(err),
            );
          }
          ...
          updateProgress(
            "removable-files",
            "Finding unused & removable assets…",
          );
          try {
            ...
            logger.warn(
              "[Async Scan] removable-files failed:",
              safeErrorMessage(err),
            );
          }
          ...
          try {
            results.npmAudit = await runNpmAuditAsync(projectPath, {
              force: false,
            });
            engines.push("npm-audit");
          } catch (err) {
            logger.warn(
              "[Async Scan] npm-audit failed:",
              safeErrorMessage(err),
            );
          }
          ...
          try {
            results.dataCleanup = await runDataCleanupScan(projectPath, {
              profile: "all",
            });
            engines.push("data-cleanup");
          } catch (err) {
            logger.warn(
              "[Async Scan] data-cleanup failed:",
              safeErrorMessage(err),
            );
          }
          try {
          ...
          const summary = {
            ...
            simplebeaconIssues:
              results.simplebeacon?.gate?.blockingCount ??
            ...
            dataCleanupFindings:
              results.dataCleanup?.summary?.totalFindings ?? null,
            compliancePassed: results.compliance?.summary?.passed ?? null,
            complianceFailed: results.compliance?.summary?.failed ?? null,
            npmVulnerabilities:
            ...
            handoffEligible:
              ...
              (results.codebase?.summary?.healthScore || 100) >= 80,
            tier: req.user?.tier || body.tier || "starter",
          };
          ...
          const reportJson = {
            ...
            issues:
              ...
              results.codebase?.findings ||
              [],
            severityCounts: deriveSeverityCounts(
              results.simplebeacon?.findings ||
                results.simplebeacon?.rawIssues ||
                results.codebase?.findings ||
                [],
            ...
            integrityScore:
              results.simplebeacon?.gate?.score ??
              results.codebase?.summary?.healthScore ??
              100,
            qualityScore:
              results.simplebeacon?.gate?.score ??
              results.codebase?.summary?.healthScore ??
          ...
          logger.info(
            `[Async Scan] ${scanId} completed in ${Date.now() - startedAt}ms, engines=[${engines.join(",")}]`,
          );
        } catch (err) {
        ...
        logger.error(
          "[Async Scan] unhandled background error:",
          safeErrorMessage(err),
        ),
      );
  ...
  const uploadMulter = multer({
    ...
    limits: {
      ...
      fileSize:
        ...
        constants.BYTES_PER_KB *
        constants.BYTES_PER_KB,
      fieldSize: 50 * constants.BYTES_PER_KB * constants.BYTES_PER_KB,
    },
  });
  ...
  app.post(
    ...
    uploadMulter.array("files", 100000),
    async (req, res) => {
      let projectDir = null;
      let multerFiles = [];
      try {
        ...
        const bearerToken = authHeader.startsWith("Bearer ")
          ...
          : "";
        const licenseToken =
          bearerToken || String(req.body?.licenseToken || "").trim();
        const analysisType = String(
          req.body?.analysisType || "simplebeacon",
        ).toLowerCase();
        logger.info(
          `[Upload Directory] Received analysisType="${analysisType}" from req.body keys=[${Object.keys(req.body || {}).join(",")}]`,
        );
        ...
        for (let i = 0; i < req.files.length; i++) {
          const relPath =
            filePaths[i] || req.files[i].originalname || req.files[i].fieldname;
        ...
        if (
          ...
          logger.info(
            `[Upload Directory] Detected ZIP archive. Streaming extract to ${projectDir}...`,
          );
          try {
            ...
            try {
              const safeRel = sanitizeUploadPath(
                filePaths[0] || req.files[0].originalname,
              );
              const copiedZip = path.join(projectDir, safeRel);
              if (fs.existsSync(copiedZip)) fs.unlinkSync(copiedZip);
            } catch (cleanupErr) {
              logger.warn(
                `[Upload Directory] ZIP cleanup skipped: ${safeErrorMessage(cleanupErr)}`,
              );
            }
          } catch (zipErr) {
            logger.warn(
              `[Upload Directory] ZIP extraction failed: ${safeErrorMessage(zipErr)}. Proceeding with raw upload.`,
            );
          }
        }
        ...
        (async () => {
          ...
          logger.info(
            `[Upload Directory] Starting scan ${scanId} for ${analysisType}, ${req.files.length} files`,
          );
          ...
          scanTimer = setTimeout(() => {
            ...
            if (job && job.status === "scanning") {
              logger.error(
                `[Upload Directory] Scan ${scanId} timed out after ${scanTimeoutMs}ms`,
              );
              scanJobs.set(scanId, {
                ...job,
                status: "error",
                error: `Scan timed out after ${scanTimeoutMs / constants.MS_PER_SECOND}s`,
              });
              try {
          ...
          if (!fs.existsSync(cliPath)) {
            logger.warn(
              `[Upload Directory] CLI binary not found at ${cliPath} — falling back to programmatic analysis for ${scanId}`,
            );
            cliFailed = true;
            try {
              ...
              report = {
                ...
                gate: {
                  pass: (analysis.summary?.healthScore || 100) >= 80,
                  score: analysis.summary?.healthScore || 100,
                },
              };
              logger.info(
                `[Upload Directory] Programmatic scan completed for ${scanId}, health=${report.gate.score}`,
              );
            } catch (progErr) {
              clearScanTimer();
              logger.error(
                "[Upload Directory] Programmatic fallback failed:",
                safeErrorMessage(progErr),
              );
              scanJobs.set(scanId, {
                ...scanJobs.get(scanId),
                status: "error",
                error: safeErrorMessage(progErr) || "Analysis failed",
              });
              return;
            }
          } else {
            try {
              ...
              logger.info(
                `[Upload Directory] Running CLI scan for ${scanId}...`,
              );
              ...
              logger.info(
                `[Upload Directory] CLI scan completed for ${scanId} in ${(Date.now() - scanStart) / constants.MS_PER_SECOND}s`,
              );
              try {
                report = JSON.parse(stdout);
              } catch (parseErr) {
                logger.error(
                  "[Upload Directory] Failed to parse scan output:",
                  safeErrorMessage(parseErr),
                );
                scanJobs.set(scanId, {
                  ...scanJobs.get(scanId),
                  status: "error",
                  error: "Scan completed but output parsing failed",
                });
                return;
              }
            } catch (err) {
              cliFailed = true;
              logger.warn(
                `[Upload Directory] CLI scan exited non-zero for ${scanId}:`,
                safeErrorMessage(err),
              );
              if (err.stdout) {
                try {
                  ...
                  logger.info(
                    `[Upload Directory] Parsed gate-fail report for ${scanId}, issues=${report.issueCount || report.gate?.blockingCount || "n/a"}`,
                  );
                } catch (parseErr) {
                  logger.error(
                    "[Upload Directory] Failed to parse gate-fail output:",
                    safeErrorMessage(parseErr),
                  );
                }
              }
           ...
           * @returns {any}
           */
          const tierAllowed = (analyzer) =>
            typeof analyzer === "string" &&
            (!instantTierLimited ||
              ["simplebeacon", "mock-scan", "codebase"].includes(analyzer));
          ...
          const runAnalyzer = async (
            label,
            fn,
            timeoutMs = ANALYZER_TIMEOUT,
          ) => {
            if (typeof fn !== "function") {
              throw new TypeError(
                `Analyzer ${label} requires a function, received ${typeof fn}`,
              );
            }
            const t0 = Date.now();
            logger.info(
              `[Upload Directory] Starting analyzer: ${label} (timeout ${timeoutMs}ms)`,
            );
            try {
              const result = await withTimeout(fn(), timeoutMs, label);
              logger.info(
                `[Upload Directory] Finished analyzer: ${label} in ${Date.now() - t0}ms`,
              );
              return result;
            } catch (e) {
              logger.warn(
                `[Upload Directory] Analyzer ${label} failed after ${Date.now() - t0}ms:`,
                e.message,
              );
          ...
          if (
            (analysisType === "codebase" || runComplete) &&
            tierAllowed("codebase")
          ) {
            results.codebase = await runAnalyzer("codebase", () =>
              getAnalyzeCodebase()(projectDir, {
                context: "dashboard",
                scanProfile: "default",
              }),
            );
          ...
          if (
            (analysisType === "npm-audit" || runComplete) &&
            tierAllowed("npm-audit")
          ) {
            results.npmAudit = await runAnalyzer("npm-audit", () =>
              runNpmAuditAsync(projectDir, { force: false }),
            );
          }
          if (
            (analysisType === "data-cleanup" || runComplete) &&
            tierAllowed("data-cleanup")
          ) {
            results.dataCleanup = await runAnalyzer("data-cleanup", () =>
              runDataCleanupScan(projectDir, { profile: "all" }),
            );
          }
          if (
            (analysisType === "file-reduction" || runComplete) &&
            tierAllowed("file-reduction")
          ) {
            results.fileReduction = await runAnalyzer("file-reduction", () =>
              scanFileMergerReduction(projectDir, {
                includeRepositoryInventory: true,
              }),
            );
          }
          if (
            (analysisType === "removable-files" || runComplete) &&
            tierAllowed("removable-files")
          ) {
            results.removableFiles = await runAnalyzer("removable-files", () =>
              scanRemovableFiles(projectDir),
            );
          }
          if (
            (analysisType === "roadmap" || runComplete) &&
            tierAllowed("roadmap")
          ) {
            results.roadmap = await runAnalyzer("roadmap", () =>
              generateCodeRoadmap(
                projectDir,
                {},
                { scanReport: report, includeFiles: true },
              ),
            );
          }
          if (
            (analysisType === "data-quality" || runComplete) &&
            tierAllowed("data-quality")
          ) {
            const profile =
              analysisType === "data-quality" ? "data-quality" : "all";
            results.dataQuality = await runAnalyzer("data-quality", () =>
              runDataCleanupScan(projectDir, { profile }),
            );
          }
          if (
            (analysisType === "cleanup-assistant" || runComplete) &&
            tierAllowed("cleanup-assistant")
          ) {
            results.cleanupAssistant = await runAnalyzer(
              "cleanup-assistant",
              async () => {
                const fileReduction = await runDataCleanupScan(projectDir, {
                  profile: "file-reduction",
                });
                const dataQuality = await runDataCleanupScan(projectDir, {
                  profile: "data-quality",
                });
                const inventory =
                  fileReduction?.inventory || dataQuality?.inventory || null;
                const plan =
                ...
                const brief = {
                  ...
                  tiers: {
                    ...
                    reviewFirst: {
                      files: reviewDirs.length,
                      bytes: 0,
                      items: reviewDirs,
                    },
                    protected: { files: 0, bytes: 0, directories: [] },
                    investigate: {
                      files: plan?.unusedFiles?.candidates ?? 0,
                      note: plan?.unusedFiles?.note || null,
                    },
                  },
                  analysis: {
          ...
          if (
            (analysisType === "compliance" || runComplete) &&
            tierAllowed("compliance")
          ) {
            const dataCleanupForCompliance =
              results.dataCleanup ||
              results.cleanupAssistant?.fileReduction ||
              results.fileReduction ||
              null;
            results.compliance = await runAnalyzer("compliance", () =>
              Promise.resolve(
                evaluateComplianceChecklist(report, {
          ...
          if (
            (analysisType === "consolidation" || runComplete) &&
            tierAllowed("consolidation")
          ) {
            results.consolidation = await runAnalyzer("consolidation", () =>
              buildCompleteAuditModel({ results }),
            );
          }
          if (
            (analysisType === "mock-scan" || runComplete) &&
            tierAllowed("mock-scan")
          ) {
            results.mockScan = await runAnalyzer("mock-scan", async () => {
              ...
              return {
                ...
                dataQualityScore:
                  mockFiles.length > 0
                    ? `${(((mockFiles.length - issues.length) / mockFiles.length) * 100).toFixed(1)}%`
                    : "0%",
          ...
          if (
            ...
            if (specific && !specific.error) {
              ...
              const analysisKey =
                analysisKeyMap[analysisType] ||
                `_${analysisType.replace(/-/g, "")}Analysis`;
          ...
          if (reportJson && typeof reportJson === "object") {
            ...
            if (reportJson.scanTargetRoot)
              reportJson.scanTargetRoot = projectDir;
            reportJson.projectName = originalDirName;
          }

          clearScanTimer();
          logger.info(
            `[Upload Directory] Scan ${scanId} completed successfully in ${(Date.now() - scanStart) / constants.MS_PER_SECOND}s`,
          );
          scanJobs.set(scanId, {
            ...scanJobs.get(scanId),
            status: "complete",
          ...
          (async () => {
            try {
              const aiVerdict = await generateAutomatedVerdict(
                sanitizeReportForAi(report),
              ...
              const recipient =
                req.projectContext?.clientEmail ||
                req.projectContext?.email ||
              ...
              logger.warn(
                "[Upload Directory] AI Analyst autopilot error:",
                safeErrorMessage(aiErr),
              );
            }
          })().catch((err) =>
            logger.warn(
              "[Upload Directory] AI Analyst unhandled error:",
              safeErrorMessage(err),
            ),
          );
          ...
          (async () => {
            try {
              const euCompliance = await evaluateHumanOversightCompliance(
                report,
                { projectPath: projectDir },
              );
              const jobMeta = scanJobs.get(scanId) || {};
              ...
              logger.warn(
                "[Upload Directory] EU Article 14 evaluator error:",
                safeErrorMessage(euErr),
              );
            }
          })().catch((err) =>
            logger.warn(
              "[Upload Directory] EU evaluator unhandled error:",
              safeErrorMessage(err),
            ),
          );
        })();
        ...
        try {
          if (projectDir) {
            ...
            logger.info(
              "[Privacy Guard] Purged repository assets from server memory",
            );
          }
  ...
  app.get("/api/analyze/codebase", async (req, res) => {
    try {
      ...
      const scanContext = String(
        req.query.context || req.query.scanMode || "dashboard",
      ).toLowerCase();
      const understandingMode = String(
        req.query.understandingMode || "deterministic",
      ).toLowerCase();
      const startedAt = Date.now();
      logger.info(
        `[analyze] codebase start path=${projectPath} profile=${scanProfile} context=${scanContext}`,
      );
      ...
      const analyzeOptions = {
        ...
        scanProfile,
        context: scanContext,
        includeBrowserAnalyzers:
          req.query.includeBrowserAnalyzers === "1" ||
          req.query.includeBrowserAnalyzers === "true",
        includeAllFiles:
          req.query.includeAllFiles === "1" ||
          req.query.includeAllFiles === "true",
      };
      if (maxDeepAnalyze != null) {
      ...
      logger.info(
        `[analyze] codebase done path=${projectPath} context=${scanContext} ms=${Date.now() - startedAt} analyzed=${report.summary?.codeFilesAnalyzed ?? "—"}/${report.summary?.codeFilesDiscovered ?? "—"}`,
      );
      if (understandingMode !== "off") {
        const registry = await ensureRegistry(baseDir);
        const userCredentials = await loadUserCredentials(
      ...
      logger.warn("[Analyze/Codebase] Analysis failed:", {
      ...
      const errorPayload = {
        success: false,
        error: toClientError(error, "Codebase analysis failed"),
      };
      if (debug) {
        errorPayload._debug = {
          projectPath,
  ...
  app.get("/api/analyze/inventory", async (req, res) => {
    try {
      ...
      const fullTree =
        req.query.fullDirectoryScan === "true" ||
        req.query.fullDirectoryScan === "1";
      if (fullTree) {
      ...
      const inventory = await countRepositoryInventory(
        projectPath,
        inventoryOptions,
      );
  ...
  app.post("/api/analyze/understand", async (req, res) => {
    try {
      ...
      const understandingMode = String(
        body.understandingMode || "deterministic",
      ).toLowerCase();
      ...
      if (body.code) {
        report = await understandCodeSnippet(
          String(body.code),
          {
        ...
        return sendError(
          res,
          400,
          "Provide code (string) or filePath + projectPath",
        );
      }
      ...
      return sendError(
        res,
        400,
        toClientError(error, "Code understanding failed"),
      );
    }
  });
  ...
  app.get("/api/analyze/zscript-report", async (req, res) => {
    try {
      let projectPath;
      try {
        projectPath = resolveSafeProjectPath(
          req.query.projectPath || req.query.path,
        );
      } catch (error) {
        return sendError(res, 400, toClientError(error, "Invalid projectPath"));
      }
  ...
  app.post("/api/analyze/expert-review", async (req, res) => {
    try {
      ...
      return sendError(
        res,
        400,
        toClientError(error, "Expert review save failed"),
      );
    }
  });
    ...
    try {
      ...
      const reviews = await loadExpertReviews(baseDir, {
        ...
        filePath: req.query.filePath
          ? String(req.query.filePath).replace(/\\/g, "/")
          : undefined,
      ...
      return sendError(
        res,
        400,
        toClientError(error, "Expert review load failed"),
      );
    }
  });
    ...
    try {
      ...
      if (!completeScan || typeof completeScan !== "object") {
        ...
        return sendError(
          res,
          400,
          "completeScan payload is required — run Complete scan first, then export ZIP.",
        );
      }
      if (!completeScanHasExportableResults(completeScan)) {
        ...
        return sendError(
          res,
          400,
          "Export bundle requires at least one completed scan step with results.",
        );
      }
      ...
      try {
        zipResult = await buildAnalyzeExportZipStream(completeScan, {
          ...
          internalDashboard,
          publicGateLocked:
            publicGateEnabled && !internalDashboard && !hasAdmin,
          hasAuditDeliverableAccess:
            (!publicGateEnabled && !closedVaultMode) || hasAdmin,
          ...
          setHeaders: () => {
            ...
            res.set(
              "Content-Disposition",
              'attachment; filename="simplebeacon-export.zip"',
            );
          },
        });
        logger.debug(
          `[export-bundle] ZIP streamed, filename=${zipResult.filename}`,
        );
      } catch (error) {
        logger.debug(
          `[export-bundle] error during ZIP generation: ${safeErrorMessage(error)}`,
        );
      ...
      logger.warn("[export-bundle] generation failed", {
        error: safeErrorMessage(error),
      });
      return sendError(
        res,
        400,
        toClientError(error, "Export bundle generation failed"),
      );
    }
  });
  ...
  app.post("/api/simplebeacon/export/certificate", async (req, res) => {
    try {
      ...
      const milestone = String(
        body.milestone || storedProfile?.milestone || "release",
      ).toLowerCase();
      const clientName =
        String(
          body.client_name ||
            body.clientName ||
            storedProfile?.clientName ||
            "",
        ).trim() || "Client";
      const projectName =
        String(
          body.project_name ||
            body.projectName ||
            storedProfile?.projectName ||
            "",
        ).trim() || "Project";
      const projectId = String(body.project_id || body.projectId || "").trim();
      const orgId = String(
        body.org_id || body.orgId || storedProfile?.orgId || "default",
      ).trim();
      ...
      if (!report || typeof report !== "object") {
        return sendError(
          res,
          400,
          "report is required — run a Simplebeacon gate scan first.",
        );
      }
      ...
      if (format === "zip") {
        ...
        const completeScan =
          normalizeCompleteScanInput(body.completeScan) ||
          body.completeScan ||
          report;
        const zipResult = await buildAnalyzeExportZipStream(completeScan, {
          ...
          internalDashboard:
            process.env.SIMPLEBEACON_INTERNAL_DASHBOARD === "true",
          publicGateLocked:
            publicGateEnabled && !hasAdminDeliverableAccess(req),
          hasAuditDeliverableAccess:
            (!publicGateEnabled && !closedVaultMode) ||
            hasAdminDeliverableAccess(req),
          milestone,
          client: clientName,
          projectName,
      ...
      if (format === "email") {
        ...
        if (!recipient) {
          return sendError(
            res,
            400,
            "email is required for format=email. Provide body.email or sign in.",
          );
        }
      ...
      return sendError(
        res,
        400,
        `Unsupported format: ${format}. Use html, zip, or email.`,
      );
    } catch (error) {
      logger.warn("[certificate-export] failed", {
        error: safeErrorMessage(error),
      });
      return sendError(
        res,
        400,
        toClientError(error, "Certificate export failed"),
      );
    }
  });
  ...
  app.post("/api/analyze/complete-audit-report", async (req, res) => {
    try {
      ...
      const reportModulePath =
        require.resolve("../lib/complete-scan-audit-report.cjs");
      if (process.env.SIMPLEBEACON_INTERNAL_DASHBOARD === "true") {
        delete require.cache[reportModulePath];
      }
      const {
      ...
      } = require("../lib/audit-export-tier.cjs");
      ...
      const projectPath =
        completeScan.projectPath || completeScan.projectRoot || "";
      ...
      const report = await buildCompleteAuditReport(completeScan, {
        ...
        summarizeFn: providerOpts
          ? async (providerId, payload, _opts) =>
              summarizeScanWithProvider(
                providerId,
      ...
      logger.warn("[complete-audit-report] generation failed", {
        error: safeErrorMessage(error),
      });
      return sendError(
        res,
        400,
        toClientError(error, "Audit report generation failed"),
      );
    }
  });
  ...
  app.get("/api/analyze/list-directories", async (req, res) => {
    try {
        ...
        if (process.platform === "win32") {
          ...
          for (let i = 65; i <= 90; i++) {
            ...
            try {
              ...
              console.error(
                "flexible-analyze-api.cjs error:",
                e,
      ...
      const isRoot =
        targetPath === parent ||
        (!isPathWithinRoots(parent, allowedRoots) &&
          !isPathAncestorOfRoots(parent, allowedRoots));
      res.json({
        success: true,
      ...
      logger.warn("[list-directories] failed", {
        error: safeErrorMessage(error),
      });
      return sendError(
        res,
        400,
        toClientError(error, "Directory listing failed"),
      );
    }
  });
  ...
  app.post("/api/analyze/summary", async (req, res) => {
    try {
      ...
      const payload = normalizeReportForSummary(
        report,
        body.reportType || report.type,
      );
      const providerOpts = resolveSummaryProvider(
        aiProvider,
        registry,
      ...
      if (!providerOpts) {
        return res.json({
          ...
          aiProvider,
          message:
            "Filesystem scan only — select a configured provider for an AI narrative summary.",
        });
      }
  ...
  app.post("/api/analyze/compliance-checklist", async (req, res) => {
    try {
      ...
      if (!npmAudit) {
        try {
          npmAudit = await withTimeout(
            runNpmAuditAsync(resolvedRoot, {
              force: body.forceNpmAudit === true,
            }),
            20000,
          );
        } catch {
      ...
      return sendError(
        res,
        400,
        toClientError(error, "Compliance checklist failed"),
      );
    }
  });
    ...
    await new Promise((resolve, reject) => {
      ...
      const handleResponse = (response) => {
        ...
        if (response.statusCode !== 200) {
          reject(
            new Error(`GitHub zipball download failed: ${response.statusCode}`),
          );
          return;
        }
        response.pipe(file);
      ...
      https
        .get(
          zipUrl,
          { headers: { "User-Agent": "SimpleBeacon/1.0" } },
          handleResponse,
        )
        .on("error", reject);
    });
  ...
  app.post("/api/analyze/github-clone", async (req, res) => {
    ...
    try {
      ...
      return sendError(
        res,
        500,
        safeErrorMessage(err) || "GitHub clone failed",
      );
    }
  });
  ...
  const {
    ...
    runDataCleanupScan,
  } = require("../lib/data-cleanup-scan.cjs");
  ...
  app.use("/api/analyze/upload-directory", (err, req, res, next) => {
    if (err && err instanceof multer.MulterError) {
      logger.warn(
        `[Upload Directory] Multer error: ${err.code} - ${err.message}`,
      );
      if (err.code === "LIMIT_UNEXPECTED_FILE") {
        return sendError(
          res,
          413,
          "Too many files. Maximum 100,000 files allowed per upload.",
        );
      }
      if (err.code === "LIMIT_FILE_SIZE") {
        return sendError(
          res,
          413,
          "One or more files exceed the 5 GB size limit.",
        );
      }
      return sendError(
        res,
  ...
  app.post(
    "/api/analyze/upload-directory",
    uploadDirLimiter,
    uploadDirMulter.array("files", 100000),
    async (req, res) => {
      ...
      try {
        ...
        if (
          ...
          logger.info(
            `[Upload Directory] Detected ZIP archive. Streaming extract to ${tmpDir}...`,
          );
          try {
            await new Promise((resolve, reject) => {
              fs.createReadStream(zipPath)
        ...
        logger.info(
          `[Upload Directory] Received ${req.files.length} files, wrote to ${tmpDir}`,
        );
        ...
        if (!fs.existsSync(cliBin)) {
          logger.warn(
            `[Upload Directory] CLI binary not found at ${cliBin} — falling back to programmatic analysis`,
          );
          try {
            const analysis = await withTimeout(
              getAnalyzeCodebase()(tmpDir, {
                includeEslint: false,
                maxDeepAnalyze: 3000,
                context: "dashboard",
                scanProfile: "universal",
            ...
            logger.info(
              `[Upload Directory] Programmatic scan completed, health=${report.gate.score}`,
            );
          } catch (progErr) {
            logger.error(
              "[Upload Directory] Programmatic fallback failed:",
          ...
          for (const candidate of configCandidates) {
            try {
              ...
              if (raw) {
                try {
                  baseConfig = JSON.parse(redactTextSecrets(raw));
                } catch {
                  baseConfig = {};
                }
                break;
              ...
              logger.warn(
                "[Flexible Analyze] Could not read config candidate:",
                candidate,
                readErr.message,
              );
            }
          ...
          const scanConfig = {
            ...
            fullDirectoryScanSkipDirs: [
              ...new Set([
                ...defaultSkipDirs,
                ...(baseConfig.fullDirectoryScanSkipDirs || []),
              ]),
            ],
            ignore: [
              ...new Set([...defaultIgnore, ...(baseConfig.ignore || [])]),
            ],
          };
          ...
          logger.info(
            `[Upload Directory] Scan found: totalFiles=${report.totalFiles || report.repositoryFilesTotal || "n/a"}, scanned=${report.ruleScopedFilesAnalyzed || "n/a"}, issues=${report.issueCount || report.gate?.blockingCount || "n/a"}`,
          );
        }
        const results = { simplebeacon: report };
        ...
        if (analysisType === "npm-audit" || runComplete) {
          results.npmAudit = await runAnalyzer("npm-audit", () =>
            runNpmAuditAsync(tmpDir, { force: false }),
          );
        }
        if (analysisType === "data-cleanup" || runComplete) {
          results.dataCleanup = await runAnalyzer("data-cleanup", () =>
            runDataCleanupScan(tmpDir, { profile: "all" }),
          );
        }
        ...
        if (analysisType === "file-reduction" || runComplete) {
          results.fileReduction = await runAnalyzer("file-reduction", () =>
            scanFileMergerReduction(tmpDir, {
              includeRepositoryInventory: true,
            }),
          );
        }
        ...
        if (analysisType === "data-quality" || runComplete) {
          const profile =
            analysisType === "data-quality" ? "data-quality" : "all";
          results.dataQuality = await runAnalyzer("data-quality", () =>
            runDataCleanupScan(tmpDir, { profile }),
          );
        }
        if (analysisType === "cleanup-assistant" || runComplete) {
          results.cleanupAssistant = await runAnalyzer(
            "cleanup-assistant",
            async () => {
              const fileReduction = await runDataCleanupScan(tmpDir, {
                profile: "file-reduction",
              });
              const dataQuality = await runDataCleanupScan(tmpDir, {
                profile: "data-quality",
              });
              const inventory =
                fileReduction?.inventory || dataQuality?.inventory || null;
              const plan =
                fileReduction?.fileReductionPlan || fileReduction?.plan || null;
              const safeDirs = plan?.safeToDelete?.topDirectories || [];
              const reviewDirs = plan?.reviewBeforeDelete?.logs || [];
              const brief = {
                projectPath: tmpDir,
          ...
          if (specific && !specific.error) {
            if (analysisType === "codebase") {
              reportJson = {
                ...
                qualityScore:
                  specific.summary?.healthScore ?? report.qualityScore,
                totalFiles:
                  specific.summary?.codeFilesAnalyzed ?? report.totalFiles,
                issueCount:
                  specific.summary?.findingsTotal ?? report.issueCount,
              };
            } else if (
              analysisType === "data-quality" ||
              analysisType === "data-cleanup"
              ...
              reportJson = {
                ...
                issueCount:
                  specific.summary?.totalFindings ?? report.issueCount,
              };
              ...
              reportJson = {
                ...
                totalFiles:
                  specific.brief?.inventory?.totalFiles ?? report.totalFiles,
                issueCount:
                  specific.brief?.tiers?.safeNow?.directories?.length ??
                  report.issueCount,
              };
              ...
              reportJson = {
                ...
                qualityScore: specific.dataQualityScore
                  ? parseFloat(specific.dataQualityScore)
                  : report.qualityScore,
              ...
              reportJson = {
                ...
                qualityScore:
                  specific.executiveSummary?.completionRate ??
                  report.qualityScore,
              ...
              reportJson = {
                ...
                totalFiles: report.totalFiles,
                issueCount:
                  specific.vulnerabilities?.length ?? report.issueCount,
              };
              ...
              reportJson = {
                ...
                qualityScore:
                  specific.summary?.readinessScore ?? report.qualityScore,
                totalFiles: report.totalFiles,
                issueCount: specific.summary?.failed ?? report.issueCount,
              };
            } else {
        ...
        res.json({
          ...
          privacy: {
            ...
            description:
              "Scan ran entirely on this machine. No files or data were transmitted to external services.",
          },
        });
      } catch (error) {
        logger.error(
          "[Upload Directory] Scan failed:",
        ...
        setTimeout(() => {
          try {
            ...
            logger.warn(
              "[Upload Directory] Cleanup failed:",
              safeErrorMessage(e),
            );
          }
