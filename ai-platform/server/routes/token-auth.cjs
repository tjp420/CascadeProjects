...
function getRedis() {
    ...
    if (!redisClient) {
        ...
        redisClient.connect().catch((err) => logger.warn('Redis connection failed:', err.message));
    }
    return redisClient;
}
...
async function signAccessToken(payload) {
    if (!SignJWT) {
        throw new Error('Token authentication requires the "jose" package. Run: npm install jose');
    }
...
function auditEvent(accountId, eventType, actor, metadata, req) {
    ...
    db.insertAuditLog(entry).catch((err) => logger.warn('Audit log failed:', err.message));
}
...
router.post('/device-verify', async (req, res) => {
    try {
        ...
        let isValid = false;
        try {
            const verifyPayload = Buffer.from(challengeData.challenge + device_key_id, 'utf8');
        ...
        // Update last seen
        await db.updateDeviceKey(device_key_id, { last_seen_at: new Date().toISOString() });
        ...
        const now = new Date();
        const accessTokenExpires = new Date(now.getTime() + ACCESS_TOKEN_TTL_SECONDS * 1000);
        ...
        const refreshExpires = new Date(now.getTime() + REFRESH_TOKEN_TTL_SECONDS * 1000);
        ...
        auditEvent(account.id, 'session_created', device_key_id, { session_id: sessionTokenId, access_jti: accessTokenJti }, req);
...
router.post('/recover-init', async (req, res) => {
    try {
        ...
        res.json({
            ...
            message: recovery_factor_type === 'email_otp' ? 'OTP sent to registered email' : 'Use your authenticator app'
        });
    } catch (err) {
...
router.post('/enroll-device', optionalAuthenticate, async (req, res) => {
    try {
        ...
        if (activeDevices.length >= account.max_devices) {
            return res.status(403).json({ success: false, error: `Device limit reached (${account.max_devices})` });
        }
        ...
        auditEvent(accountId, 'device_enrolled', deviceKeyId, { key_type, trust_level: trustLevel }, req);
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        auditEvent(account.id, 'session_refreshed', 'system', { session_id: newSessionId, access_jti: newAccessTokenJti }, req);
...
router.post('/logout', optionalAuthenticate, async (req, res) => {
    try {
        ...
        auditEvent(session.account_id, 'session_revoked', session.id, { reason: 'logout' }, req);
...
router.post('/register', async (req, res) => {
    try {
        ...
        auditEvent(accountId, 'account_created', 'system', { email, account_type: account.account_type }, req);

        res.json({
            success: true,
            account_id: accountId,
            message: 'Account created. Enroll a device key to authenticate.',
            next_steps: ['POST /auth/enroll-device (with enrollment_ticket from recovery)']
        });
    } catch (err) {
