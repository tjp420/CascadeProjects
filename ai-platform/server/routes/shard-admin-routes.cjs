...
function tenantGuard(req, res, next) {
  ...
  if (!req.headers || !req.headers['x-tenant-id']) return res.status(403).json({ error: 'tenant-required' });
...
router.post('/api/admin/shard/reconcile', tenantGuard, async (req, res) => {
  try {
    ...
    if (!Array.isArray(shardIds)) return res.status(400).json({ error: 'shardIds-array-required' });
    const result = await reconciler.triggerSync(shardIds, { tenantId: req.tenantId, requestedBy: req.user && req.user.id });
