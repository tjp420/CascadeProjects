...
describe("stripe-webhook subscription.updated handler", () => {
  ...
  beforeEach(() => {
    ...
    process.env.STRIPE_WEBHOOK_SECRET = WEBHOOK_SECRET;
    process.env.EMAIL_QUEUE_DIR = tempStoreDir;
    process.env.STRIPE_EVENT_STORE = path.join(
      tempStoreDir,
      "stripe-events.json",
    );
    delete process.env.CF_API_TOKEN;
  ...
  it("handleSubscriptionUpdated updates the subscription store when customer exists", async () => {
    ...
    assert.ok(
      router,
      "router should be loadable with findSubscriptionByCustomerId",
    );
    ...
    for (const email of Object.keys(store.subscriptions)) {
      if (
        store.subscriptions[email].stripeCustomerId === "cus_test_update_001"
      ) {
    ...
    assert.ok(
      foundRecord,
      "findSubscriptionByCustomerId logic should find the record",
    );
  ...
  it("handleSubscriptionUpdated returns null for unknown customer", async () => {
    ...
    for (const email of Object.keys(store.subscriptions)) {
      if (
        store.subscriptions[email].stripeCustomerId === "cus_nonexistent_999"
      ) {
        foundRecord = store.subscriptions[email];
        break;
      }
    ...
    assert.strictEqual(
      foundRecord,
      null,
      "should not find non-existent customer",
    );
  });
});
...
describe("stripe-webhook subscription.deleted handler", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SIMPLEBEACON_SUBSCRIPTION_STORE = path.join(
      tempStoreDir,
      "subscriptions.json",
    );
    process.env.STRIPE_WEBHOOK_SECRET = WEBHOOK_SECRET;
  ...
  it("handleSubscriptionDeleted deactivates subscription and sends cancellation email", async () => {
    ...
    assert.strictEqual(
      deactivated.subscriptionActive,
      false,
      "should be deactivated after deletion",
    );
  });
});
...
describe("subscriptions-billing tier detection by exact price match", () => {
  ...
  it("old >= logic would have misclassified developer annual as team_pro", () => {
    ...
    const oldLogic =
      unitAmount >= PRICE_TEAM_PRO_MONTHLY ? "team_pro" : "developer";
    assert.strictEqual(
      oldLogic,
      "team_pro",
