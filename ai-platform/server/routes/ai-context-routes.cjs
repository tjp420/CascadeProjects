...
router.post('/ai-context', express.json({ limit: '10mb' }), async (req, res) => {
  try {
    ...
    const safePath = path.isAbsolute(projectPath)
      ? path.resolve(projectPath)
      : path.resolve(platformRoot, projectPath);
    ...
    if (Array.isArray(issues) && issues.length > 0) {
      ...
      for (const issue of issues.slice(0, 50)) {
        ...
        const desc =
          issue.description || issue.message || issue.title || JSON.stringify(issue).slice(0, 200);
    ...
    res.json({
      ...
      path: contextPath,
      content: md,
      message: 'AI context saved. Mention @.simplebeacon/ai-context.md in chat.',
    });
  } catch (err) {
    ...
    try {
      ...
      if (!raw) {
        return sendError(
          res,
          404,
          'No AI context file found. Upload a report and click "Send to AI Agent" first.'
        );
      }
