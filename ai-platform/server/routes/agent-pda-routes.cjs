 ...
 */
const WORKSPACES_FILE = require("path").join(
  process.cwd(),
  ".simplebeacon",
  "agent-pda",
  "workspaces.json",
  ...
  if (explicitRoot) {
    ...
    if (allowedRoots.length > 0 && !allowedRoots.includes(explicitRoot)) {
      return {
        ...
        message: `Project root "${explicitRoot}" is not in the allowed list`,
      };
    }
...
router.post("/agent-pda/workspaces", authenticate, (req, res) => {
  ...
  const id =
    "ws_" +
    crypto.createHash("sha256").update(root).digest("hex").substring(0, 12);
...
router.delete(
  "/agent-pda/workspaces/:workspaceId",
  authenticate,
  (req, res) => {
    const ws = loadWorkspaces();
...
router.post(
  "/agent-pda/register",
  authenticate,
  withWorkspace((req, res, engine, root) => {
...
router.post(
  "/agent-pda/agents/:agentId/memory",
  authenticate,
  withWorkspace((req, res, engine, root) => {
...
router.post(
  "/agent-pda/agents/:agentId/tasks",
  authenticate,
  withWorkspace((req, res, engine, root) => {
    const { title, description, priority, parentId, approvalRequired } =
      req.body || {};
    if (!title) return sendError(res, 400, "title is required");
    const task = engine.createTask(root, req.params.agentId, title, {
      description,
      priority,
      parentId,
      approvalRequired,
...
router.patch(
  ...
  authenticate,
  withWorkspace((req, res, engine, root) => {
    const { title, description, status, priority, blockReason } =
      req.body || {};
    const task = engine.updateTask(root, req.params.taskId, {
      title,
      description,
      status,
      priority,
...
router.post(
  "/agent-pda/tasks/:taskId/complete",
  authenticate,
  withWorkspace((req, res, engine, root) => {
...
router.post(
  "/agent-pda/tasks/:taskId/approve",
  authenticate,
  withWorkspace((req, res, engine, root) => {
    const task = engine.approveTask(
      root,
      req.params.taskId,
      req.user?.email || "remote",
    );
    if (!task) return sendError(res, 404, "Task not found");
    sendSuccess(res, { task });
  }),
...
router.get(
  "/agent-pda/tasks/approvals/pending",
  authenticate,
  withWorkspace((req, res, engine, root) => {
...
router.post(
  ...
  withWorkspace((req, res, engine, root) => {
    ...
    if (!type || !action)
      return sendError(res, 400, "type and action are required");
    try {
      const policy = engine.addPolicy(root, {
        type,
        action,
        description,
        severity,
...
router.post(
  ...
  withWorkspace((req, res, engine, root) => {
    ...
    if (!Array.isArray(changes))
      return sendError(res, 400, "changes array is required");
