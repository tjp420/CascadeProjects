...
router.get("/stats", function (req, res) {
  try {
    res.json({ success: true, stats: schemaStore.getStats() });
  } catch (err) {
    sendError(res, 500, "schema_stats_failed", { message: err.message });
  }
});
...
router.get("/", function (req, res) {
  try {
    ...
    sendError(res, 500, "schemas_list_failed", { message: err.message });
  }
});
...
router.get("/:toolId", function (req, res) {
  try {
    ...
    sendError(res, 500, "schema_get_failed", { message: err.message });
  }
});
...
router.post("/:toolId", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[ToolSchemaValidation] Schema registered for tool: " +
        req.params.toolId +
        " by " +
    ...
    sendError(res, 400, "schema_register_failed", { message: err.message });
  }
});
...
router.put("/:toolId", authorize("admin:all"), function (req, res) {
  try {
    ...
    sendError(res, 400, "schema_update_failed", { message: err.message });
  }
});
...
router.delete("/:toolId", authorize("admin:all"), function (req, res) {
  try {
    ...
    sendError(res, 500, "schema_delete_failed", { message: err.message });
  }
});
...
router.post("/:toolId/validate", function (req, res) {
  try {
    ...
    sendError(res, 500, "validate_failed", { message: err.message });
  }
});
...
router.post("/infer", function (req, res) {
  try {
    ...
    sendError(res, 500, "infer_failed", { message: err.message });
  }
});
...
router.post("/violations/clear", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[ToolSchemaValidation] Violations cleared by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json(result);
  } catch (err) {
...
router.put("/config", authorize("admin:all"), function (req, res) {
  try {
    ...
    logger.info(
      "[ToolSchemaValidation] Config updated by " +
        ((req.user && req.user.email) || "admin"),
    );
    res.json(result);
  } catch (err) {
