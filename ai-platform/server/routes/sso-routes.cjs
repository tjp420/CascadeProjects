...
router.get(
  "/login/:provider",
  validateParam("provider", VALIDATION_PATTERNS.provider),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (!org || !org.ssoEnabled) {
        logger.warn(
          `[SSO] Domain not configured for SAML SSO: ${maskEmail(assertion.email)}`,
        );
      ...
      logger.info(
        `[SSO] SAML success for ${maskEmail(user.email)} via ${provider}`,
      );
      res.json({
        success: true,
        provider,
...
router.get("/oidc/callback", async (req, res) => {
  try {
    ...
    if (!org || !org.ssoEnabled) {
      logger.warn(
        `[SSO] Domain not configured for OIDC SSO: ${maskEmail(idpProfile.email)}`,
      );
      return sendError(res, 403, "Organization not enabled for SSO");
    }
    ...
    logger.info(
      `[SSO] OIDC success for ${maskEmail(user.email)} via ${provider}`,
    );
    res.json({
      success: true,
      provider,
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      const publicBaseUrl =
        process.env.PUBLIC_BASE_URL || "https://simplebeacon.ai";
