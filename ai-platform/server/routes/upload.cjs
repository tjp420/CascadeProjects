...
const uploadConfig = {
  ...
  allowedTypes: (
    process.env.UPLOAD_ALLOWED_TYPES || "js,ts,jsx,tsx,py,html,json,md,txt"
  ).split(","),
...
const fileFilter = (req, file, cb) => {
  ...
  if (uploadConfig.allowedTypes.includes(ext)) {
    ...
    cb(
      new Error(
        `File type .${ext} not allowed. Allowed types: ${uploadConfig.allowedTypes.join(", ")}`,
      ),
      false,
    );
  }
...
router.post("/git", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      (typeof accessToken !== "string" || accessToken.length > 500)
    ) {
...
router.post("/api/:provider", async (req, res) => {
  try {
    ...
    switch (provider.toLowerCase()) {
      case "github":
        analysis = await handleGitHubIntegration(
          repositoryUrl,
          accessToken,
          req.user,
        );
        break;
      case "gitlab":
        analysis = await handleGitLabIntegration(
          repositoryUrl,
          accessToken,
          req.user,
        );
        break;
      case "bitbucket":
        analysis = await handleBitbucketIntegration(
          repositoryUrl,
          accessToken,
          req.user,
        );
...
async function analyzeGitRepository(tempDir, repoInfo, _user) {
  ...
  analysis.issues = [
    ...
    {
      type: "warning",
      message: "Large repository size detected",
      size: analysis.size,
    },
  ];
...
async function analyzeFile(filePath, _user) {
  try {
    ...
    return {
      ...
      issues: [{ type: "info", message: "File analyzed successfully" }],
    };
  } catch (error) {
    return {
      file: filePath,
      error: error.message,
      issues: [{ type: "error", message: "Failed to analyze file" }],
    };
  }
}
...
function shouldWatchFile(filePath) {
  return constants.hasAnyExtension(filePath, [
    "CODE",
    "CONFIG",
    "MARKUP",
    "DOCUMENT",
...
async function getDirectorySize(dirPath) {
  ...
  try {
    ...
    logger.warn(
      `[Upload] Failed to calculate directory size for ${dirPath}:`,
      error.message,
    );
  }
