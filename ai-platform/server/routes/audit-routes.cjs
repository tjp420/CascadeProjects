...
if (process.env.CLUSTER_NODES && process.env.NODE_ENV !== "test") {
  process.nextTick(() => {
    try {
      ...
      logger.warn("[Audit] cluster-keyring-sync init failed:", err.message);
    }
  });
}
...
function getAgenticRoutes() {
  if (!_agenticRoutes) {
    try {
      ...
      logger.warn(
        "[Audit] Could not load agentic-orchestration-routes for telemetry:",
        e.message,
      );
    }
  }
...
const NON_ADMIN_AUDIT_PATHS = new Set([
  "/log",
  "/stats",
  "/export",
  "/partition-status",
...
router.get("/partition-config", authorize("admin:all"), (req, res) => {
  try {
    ...
    res.json({
      success: true,
      config: {
        orgPartitionEnforcementEnabled:
          settings.orgPartitionEnforcementEnabled !== false,
        orgPartitionAlertOnViolation:
          settings.orgPartitionAlertOnViolation !== false,
        orgPartitionViolationAlertThreshold:
          settings.orgPartitionViolationAlertThreshold || 5,
        orgPartitionViolationTtlMs:
          settings.orgPartitionViolationTtlMs || 24 * 60 * 60 * 1000,
        orgPartitionViolationMaxLog:
          settings.orgPartitionViolationMaxLog || 1000,
        orgPartitionViolationCleanupIntervalMs:
          settings.orgPartitionViolationCleanupIntervalMs || 5 * 60 * 1000,
        orgPartitionViolationMemoryGuardMb:
          settings.orgPartitionViolationMemoryGuardMb || 50,
      },
      updatedAt: settings.updatedAt,
    });
  } catch (err) {
    logger.warn("[Audit] partition_config_get_failed:", err.message);
    sendError(res, 500, "partition_config_get_failed", {
      message: err.message,
    });
  }
});
...
router.put("/partition-config", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (req.body.orgPartitionEnforcementEnabled !== undefined) {
      updates.orgPartitionEnforcementEnabled =
        !!req.body.orgPartitionEnforcementEnabled;
    }
    if (req.body.orgPartitionAlertOnViolation !== undefined) {
      updates.orgPartitionAlertOnViolation =
        !!req.body.orgPartitionAlertOnViolation;
    }
    if (req.body.orgPartitionViolationAlertThreshold !== undefined) {
      updates.orgPartitionViolationAlertThreshold = parseInt(
        req.body.orgPartitionViolationAlertThreshold,
        10,
      );
    }
    ...
    if (!result.success) {
      return sendError(res, 400, "partition_config_update_failed", {
        message: result.error,
      });
    }

    logger.info(
      `[Audit] Partition config updated by ${req.user?.email || "admin"}`,
    );
    ...
    try {
      ...
      logger.warn(
        "[Audit] Failed to audit-log partition config update:",
        logErr.message,
      );
    }
    ...
    res.json({
      ...
      config: {
        orgPartitionEnforcementEnabled:
          result.settings.orgPartitionEnforcementEnabled !== false,
        orgPartitionAlertOnViolation:
          result.settings.orgPartitionAlertOnViolation !== false,
        orgPartitionViolationAlertThreshold:
          result.settings.orgPartitionViolationAlertThreshold || 5,
      },
      updatedAt: result.settings.updatedAt,
    });
  } catch (err) {
    logger.warn("[Audit] partition_config_update_failed:", err.message);
    sendError(res, 500, "partition_config_update_failed", {
      message: err.message,
    });
  }
});
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      if (format === "csv") {
        const header =
          "timestamp,callerOrgId,clientOrgId,userId,method,path,ip\n";
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="partition-violations-${Date.now()}.csv"`,
        );
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="partition-violations-${Date.now()}.json"`,
        );
        res.json({
          success: true,
      ...
      sendError(res, 500, "partition_violations_export_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/partition-violations/clear",
  authorize("admin:all"),
  (req, res) => {
    try {
      const cleared = clearViolations();
      logger.info(
        `[Audit] Partition violations cleared by ${req.user?.email || "admin"} (${cleared} removed)`,
      );
      ...
      try {
        ...
        logger.warn(
          "[Audit] Failed to audit-log partition violations clear:",
          logErr.message,
        );
      }
      ...
      sendError(res, 500, "partition_violations_clear_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      logger.warn(
        "[Audit] partition_retention_config_get_failed:",
        err.message,
      );
      sendError(res, 500, "partition_retention_config_get_failed", {
        message: err.message,
      });
    }
  },
  ...
  (req, res) => {
    try {
      ...
      if (req.body.orgPartitionViolationTtlMs !== undefined) {
        updates.orgPartitionViolationTtlMs = parseInt(
          req.body.orgPartitionViolationTtlMs,
          10,
        );
      }
      if (req.body.orgPartitionViolationMaxLog !== undefined) {
        updates.orgPartitionViolationMaxLog = parseInt(
          req.body.orgPartitionViolationMaxLog,
          10,
        );
      }
      if (req.body.orgPartitionViolationCleanupIntervalMs !== undefined) {
        updates.orgPartitionViolationCleanupIntervalMs = parseInt(
          req.body.orgPartitionViolationCleanupIntervalMs,
          10,
        );
      }
      if (req.body.orgPartitionViolationMemoryGuardMb !== undefined) {
        updates.orgPartitionViolationMemoryGuardMb = parseInt(
          req.body.orgPartitionViolationMemoryGuardMb,
          10,
        );
      }
      ...
      const result = settingsStore.updateSettings(updates);
      if (!result.success) {
        return sendError(res, 400, "partition_retention_config_update_failed", {
          message: result.error,
        });
      }

      logger.info(
        `[Audit] Partition retention config updated by ${req.user?.email || "admin"}`,
      );
      ...
      try {
        ...
        logger.warn(
          "[Audit] Failed to audit-log partition retention config update:",
          logErr.message,
        );
      }
      ...
      res.json({
        ...
        config: {
          orgPartitionViolationTtlMs:
            result.settings.orgPartitionViolationTtlMs,
          orgPartitionViolationMaxLog:
            result.settings.orgPartitionViolationMaxLog,
          orgPartitionViolationCleanupIntervalMs:
            result.settings.orgPartitionViolationCleanupIntervalMs,
          orgPartitionViolationMemoryGuardMb:
            result.settings.orgPartitionViolationMemoryGuardMb,
        },
        updatedAt: result.settings.updatedAt,
      });
    } catch (err) {
      logger.warn(
        "[Audit] partition_retention_config_update_failed:",
        err.message,
      );
      sendError(res, 500, "partition_retention_config_update_failed", {
        message: err.message,
      });
    }
  },
  ...
  try {
    ...
    if (!result.valid) {
      ...
      recordStreamFailure(
        apiKey,
        "chain_verification",
        `brokenLinks=${result.brokenLinks.length}, tampered=${result.tamperedEntries.length}`,
      );
    }
...
router.post("/heal-chain", authorize("admin:all"), (req, res) => {
  try {
    ...
    logger.info(
      `[Audit] Chain heal triggered by ${req.user?.email || "admin"} for org ${orgId}: ${result.quarantined.length} quarantined, ${result.relinked} relinked`,
    );
...
router.post("/quarantine/verify-entry", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (!entryId) {
      sendError(res, 400, "missing_entry_id", {
    ...
    if (!result.found) {
      sendError(res, 404, "entry_not_found", {
        message: `Quarantine entry ${entryId} not found for org ${orgId}`,
      });
      return;
    }
    res.json({ success: true, ...result });
...
router.get("/scrubber-stats", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (expired > 0) {
      logger.info(
        `[Audit] Scrubber registry: expired ${expired} idle scrubbers`,
      );
    }
  ...
  try {
    ...
    try {
      ...
      const quarantine = auditLogger.getQuarantine
        ? auditLogger.getQuarantine(orgId)
        : [];
      auditStats = {
...
router.get("/key/status", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (!store) {
      sendError(res, 503, "key_rotation_unavailable", {
        message: "Key rotation store not loaded",
      });
      return;
    }
...
router.post("/key/rotate", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (typeof newKeyRaw === "string" && newKeyRaw.length < 32) {
      sendError(res, 400, "invalid_key", {
        message: "newKeyRaw must be at least 32 characters",
      });
      return;
    }
    if (Buffer.isBuffer(newKeyRaw) && newKeyRaw.length < 32) {
      sendError(res, 400, "invalid_key", {
        message: "newKeyRaw must be at least 32 bytes",
      });
      return;
    }
    store.rotateKey(newKeyRaw, graceMs);
    logger.info(
      "[Audit] key_rotation_triggered by user:",
      getActor(req).actorEmail,
    );
    res.json({
      success: true,
      message: "Global master key rotation initialized successfully.",
    });
  } catch (err) {
...
router.post("/key/rekey-now", authorize("admin:all"), (req, res) => {
  try {
    ...
    logger.info(
      "[Audit] manual_rekey_triggered by user:",
      getActor(req).actorEmail,
      "result:",
...
router.post("/key/purge", authorize("admin:all"), (req, res) => {
  try {
    ...
    logger.info(
      "[Audit] key_purge_triggered by user:",
      getActor(req).actorEmail,
      "purged:",
      purged,
      "force:",
...
router.post("/interdiction/block", authorize("admin:all"), (req, res) => {
  try {
    ...
    logger.info(
      "[Audit] manual_interdiction by user:",
      getActor(req).actorEmail,
      "key:",
      apiKey.slice(0, 4) + "…",
    );
...
router.post("/interdiction/release", authorize("admin:all"), (req, res) => {
  try {
    ...
    logger.info(
      "[Audit] interdiction_released by user:",
      getActor(req).actorEmail,
      "wasBlocked:",
      result.wasBlocked,
    );
    ...
    sendError(res, 500, "interdiction_release_failed", {
      message: err.message,
    });
  }
});
...
router.get(
  ...
  (req, res) => {
    try {
      ...
      sendError(res, 500, "stream_interdiction_status_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/interdiction/stream/clear",
  authorize("admin:all"),
  (req, res) => {
    try {
      const cleared = clearStreamFailures();
      logger.info(
        "[Audit] stream_failures_cleared by user:",
        getActor(req).actorEmail,
        "count:",
        cleared,
      );
      ...
      sendError(res, 500, "stream_interdiction_clear_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/interdiction/stream/record",
  authorize("admin:all"),
  (req, res) => {
    try {
      const { apiKey, failureType, detail } = req.body || {};
      if (!apiKey) {
        sendError(res, 400, "missing_api_key", {
          message: "apiKey is required",
        });
        return;
      }
      if (!failureType) {
        sendError(res, 400, "missing_failure_type", {
          message: "failureType is required",
        });
        return;
      }
      ...
      sendError(res, 500, "stream_interdiction_record_failed", {
        message: err.message,
      });
    }
  },
...
router.get("/retention/config", authorize("admin:all"), (req, res) => {
  try {
    ...
    sendError(res, 500, "retention_config_get_failed", {
      message: err.message,
    });
  }
});
...
router.put("/retention/config", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (!result.success) {
      return sendError(res, 400, "retention_config_update_failed", {
        message: result.error,
      });
    }
    logger.info(
      `[Audit] Retention policy updated by ${req.user?.email || "admin"} for org ${orgId}`,
    );
    try {
      auditLogger.log({
        orgId,
      ...
      logger.warn(
        "[Audit] Failed to audit-log retention policy update:",
        logErr.message,
      );
    }
    ...
    sendError(res, 500, "retention_config_update_failed", {
      message: err.message,
    });
  }
});
...
router.post("/retention/purge", authorize("admin:all"), (req, res) => {
  try {
    ...
    logger.info(
      `[Audit] Retention purge triggered by ${req.user?.email || "admin"} for org ${orgId}: ${result.purged} purged, ${result.remaining} remaining`,
    );
    try {
      auditLogger.log({
        orgId,
      ...
      logger.warn(
        "[Audit] Failed to audit-log retention purge:",
        logErr.message,
      );
    }
...
router.get("/cluster/keyring", authorize("admin:all"), (req, res) => {
  try {
    ...
    sendError(res, 500, "cluster_keyring_status_failed", {
      message: err.message,
    });
  }
});
...
router.post("/cluster/keyring/rotate", authorize("admin:all"), (req, res) => {
  try {
    const { newKeyRaw, graceMs } = req.body || {};
    if (!newKeyRaw) {
      return sendError(res, 400, "missing_new_key", {
        message: "newKeyRaw is required",
      });
    }
    ...
    sendError(res, 500, "cluster_keyring_rotate_failed", {
      message: err.message,
    });
  }
});
...
router.get("/archive/search", authorize("admin:all"), async (req, res) => {
  try {
    const { startDate, endDate, action, orgId, limit, offset } =
      req.query || {};
    const result = await coldArchiveSearch.search({
      startDate,
      endDate,
      action,
      orgId,
...
router.get("/compliance/report", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (format === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="compliance_proof_${report.reportId}.csv"`,
      );
...
router.get("/pii/frameworks", authorize("admin:all"), (req, res) => {
  try {
    res.json({
      success: true,
      frameworks: piiPolicyStore.COMPLIANCE_FRAMEWORKS,
    });
  } catch (err) {
    logger.warn("[Audit] pii_frameworks_failed:", err.message);
    sendError(res, 500, "pii_frameworks_failed", { message: err.message });
  }
});
  ...
  try {
    ...
    const orgDetails = orgIds.map((orgId) => {
      ...
      return {
        ...
        enabledPolicies: stats.enabledPolicies || 0,
        disabledPolicies:
          (stats.totalPolicies || 0) - (stats.enabledPolicies || 0),
    ...
    logger.warn("[Audit] pii_orgs_list_failed:", err.message);
...
router.get("/pii/policies/:orgId", authorize("admin:all"), (req, res) => {
  try {
    ...
    if (!orgId)
      return sendError(res, 400, "invalid_org_id", {
        message: "orgId is required",
      });
    const policies = piiPolicyStore.getPolicies(orgId);
    res.json({ success: true, orgId, policies, total: policies.length });
  } catch (err) {
    logger.warn("[Audit] pii_policies_get_failed:", err.message);
    sendError(res, 500, "pii_policies_get_failed", { message: err.message });
  }
});
router.post("/pii/sync-preview", authorize("admin:all"), (req, res) => {
  try {
    const { sourceOrgId, targetOrgIds, mode, compliance, severity, isDefault } =
      req.body || {};
    if (!sourceOrgId)
      return sendError(res, 400, "invalid_request", {
        message: "sourceOrgId is required",
      });
    ...
    if (cf)
      sourcePolicies = sourcePolicies.filter(
        (p) =>
          ...
          p.compliance.some((c) => cf.includes(c)),
      );
    if (sf)
      sourcePolicies = sourcePolicies.filter((p) => sf.includes(p.severity));
    if (df !== null)
      sourcePolicies = sourcePolicies.filter(
        (p) => Boolean(p.isDefault) === df,
      );
    const allOrgs = piiPolicyStore.getAllOrgIds();
    const targets =
      Array.isArray(targetOrgIds) && targetOrgIds.length > 0
        ? targetOrgIds.filter((o) => o !== sourceOrgId)
        : allOrgs.filter((o) => o !== sourceOrgId);
    ...
    const previews = targets.map((tid) => {
      ...
      for (const src of sourcePolicies) {
        const k = src.name + "::" + src.pattern;
        if (syncMode === "merge" && ek.has(k)) ws++;
        else wc++;
      }
      return {
        orgId: tid,
    ...
    logger.warn("[Audit] pii_sync_preview_failed:", err.message);
    sendError(res, 500, "pii_sync_preview_failed", { message: err.message });
  }
});
  ...
  try {
    ...
    const targets =
      Array.isArray(targetOrgIds) && targetOrgIds.length > 0
        ...
        : allOrgs.filter((o) => o !== sourceOrgId);
    if (targets.length === 0)
      return sendError(res, 400, "no_targets", {
        message: "No target orgs available to sync to",
      });
    const options = {};
    ...
    const result = piiPolicyStore.syncPoliciesToOrgs(
      sourceOrgId,
      targets,
      options,
    );
    try {
      ...
      logger.warn(
        "[Audit] Failed to audit-log PII policy sync:",
        logErr.message,
      );
    }
    logger.info(
      "[Audit] pii_policy_sync source:",
      sourceOrgId,
      "targets:",
      targets.length,
      "cloned:",
    ...
    logger.warn("[Audit] pii_sync_failed:", err.message);
...
router.get("/pii/sync-history", authorize("admin:all"), (req, res) => {
  try {
    ...
    for (const entry of entries) {
      ...
      if (!bySourceOrg[srcOrg])
        bySourceOrg[srcOrg] = { sourceOrg: srcOrg, syncs: 0, cloned: 0 };
    ...
    const actors = Object.values(byActor)
      ...
      .slice(0, 10);
    const sourceOrgs = Object.values(bySourceOrg)
      .sort((a, b) => b.syncs - a.syncs)
      .slice(0, 10);
