...
router.put('/config', authorize('admin:all'), function (req, res) {
  try {
    ...
    logger.info('[SemanticCache] Config updated by ' + (req.user && req.user.email || 'admin'));
    res.json(result);
  } catch (err) {
...
router.post('/config/reset', authorize('admin:all'), function (req, res) {
  try {
    ...
    logger.info('[SemanticCache] Config reset by ' + (req.user && req.user.email || 'admin'));
    res.json(result);
  } catch (err) {
...
router.post('/clear', authorize('admin:all'), function (req, res) {
  try {
    ...
    logger.info('[SemanticCache] Cache cleared by ' + (req.user && req.user.email || 'admin') + ' (' + result.cleared + ' entries)');
    res.json(result);
  } catch (err) {
...
router.post('/invalidate/provider', authorize('admin:all'), function (req, res) {
  try {
    var provider = req.body.provider;
    if (!provider) return sendError(res, 400, 'provider_required');
    var result = semanticCache.invalidateByProvider(provider);
    logger.info('[SemanticCache] Invalidated provider=' + provider + ' (' + result.invalidated + ' entries) by ' + (req.user && req.user.email || 'admin'));
    res.json(result);
  } catch (err) {
    sendError(res, 500, 'invalidate_provider_failed', { message: err.message });
  }
});
...
router.post('/invalidate/pattern', authorize('admin:all'), function (req, res) {
  try {
    ...
    logger.info('[SemanticCache] Invalidated pattern=' + pattern + ' (' + result.invalidated + ' entries) by ' + (req.user && req.user.email || 'admin'));
    res.json(result);
  } catch (err) {
...
router.post('/test-similarity', function (req, res) {
  try {
    ...
    if (!textA || !textB) return sendError(res, 400, 'textA_and_textB_required');
