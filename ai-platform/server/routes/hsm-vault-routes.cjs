...
} = require("../lib/hsm-adapter/recursive-proof-aggregation-engine.cjs");
const hsmMetrics = require("../lib/hsm-adapter/hsm-metrics.cjs");
const baseAdapter = require("../lib/hsm-adapter/base-adapter.cjs");
const {
  PqcHomomorphicDatabaseLookupGatingHub,
} = require("../lib/hsm-adapter/pqc-homomorphic-lookup-gating-hub.cjs");
const {
  PqcBlindedRingSignatureGatingHub,
} = require("../lib/hsm-adapter/pqc-blinded-ring-signature-gating-hub.cjs");
const {
  PqcDirectAccumulatorMembershipGatingHub,
} = require("../lib/hsm-adapter/pqc-direct-accumulator-membership-gating-hub.cjs");
const {
  PqcLatticeVssGatingHub,
} = require("../lib/hsm-adapter/pqc-lattice-vss-gating-hub.cjs");
const {
  PqcLatticeVfhssGatingHub,
} = require("../lib/hsm-adapter/pqc-lattice-vfhss-gating-hub.cjs");
const {
  CryptoPolicyEngine,
} = require("../lib/hsm-adapter/crypto-policy-engine.cjs");
const SessionStore = require("../lib/crypto/ratchet/session-store.cjs");
const {
  encryptEnvelope,
} = require("../lib/crypto/ratchet/envelope-crypto.cjs");
...
function resolveOrgId(req) {
  return String(
    req.orgId || req.query.orgId || (req.body && req.body.orgId) || "default",
  );
}
...
router.post(
  "/handshake",
  validateInput("vaultHandshake"),
  authorize("admin:all"),
  runAsync(async (req, res) => {
    const provider =
      (req.body && req.body.provider) || process.env.HSM_PROVIDER || "mockhsm";
    const keyId =
      (req.body && req.body.keyId) || process.env.HSM_KEY_ID || null;
    const region =
      (req.body && req.body.region) || process.env.HSM_REGION || "us-east-1";
    const handshake = await hsm.hsmHandshake(provider, keyId, region);
...
router.get("/consensus/status", authorize("admin:all"), function (req, res) {
  try {
    ...
    const baseAdapter = require("../lib/hsm-adapter/base-adapter.cjs");
    if (
      baseAdapter.getConsensusEngine &&
      typeof baseAdapter.getConsensusEngine === "function"
    ) {
...
function getCoordinator() {
  const baseAdapter = require("../lib/hsm-adapter/base-adapter.cjs");
  if (
    baseAdapter.getConsensusCoordinator &&
    typeof baseAdapter.getConsensusCoordinator === "function"
  ) {
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      for (const [key, value] of Object.entries(allMetrics)) {
        if (
          key.startsWith("hsm_consensus_coord_") &&
          typeof value === "number"
        ) {
      ...
      sendError(res, 500, "coordinator_status_failed", {
        message: err.message,
      });
    }
  },
...
router.get("/consensus/groups", authorize("admin:all"), function (req, res) {
  try {
    ...
    sendError(res, 500, "consensus_groups_list_failed", {
      message: err.message,
    });
  }
});
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      if (!group) {
        return sendError(res, 404, "group_not_found", {
      ...
      sendError(res, 500, "consensus_group_get_failed", {
        message: err.message,
      });
    }
  },
...
router.post("/consensus/groups", authorize("admin:all"), function (req, res) {
  try {
    ...
    if (!groupId) {
      return sendError(res, 400, "missing_group_id", {
        message: "groupId is required",
      });
    }
    if (!Array.isArray(clusterNodes) || clusterNodes.length === 0) {
      return sendError(res, 400, "missing_cluster_nodes", {
        message: "clusterNodes must be a non-empty array",
      });
    }
    ...
    res
      .status(201)
      .json({ success: true, timestamp: Date.now(), group: result });
  } catch (err) {
    if (err.code === "GROUP_EXISTS") {
      return sendError(res, 409, "group_already_exists", {
        message: err.message,
      });
    }
    if (err.code === "MAX_GROUPS_EXCEEDED") {
      return sendError(res, 507, "max_groups_exceeded", {
        message: err.message,
      });
    }
    ...
    sendError(res, 500, "consensus_group_create_failed", {
      message: err.message,
    });
  }
});
...
router.delete(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "consensus_group_destroy_failed", {
        message: err.message,
      });
    }
  },
  ...
  function (req, res) {
    try {
      ...
      if (!proposal.groupId && !proposal.topic && !proposal.key) {
        return sendError(res, 400, "no_routing_key", {
          message: "Proposal must specify groupId, topic, or key",
        });
      }
      ...
      if (result.accepted) {
        ...
        res
          .status(status)
          .json({ success: false, timestamp: Date.now(), ...result });
      }
    } catch (err) {
      sendError(res, 500, "consensus_proposal_route_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  ...
  function (req, res) {
    try {
      ...
      if (!nodeId || !groupId) {
        return sendError(res, 400, "missing_params", {
          message: "nodeId and groupId are required",
        });
      }
      ...
      if (err.code === "GROUP_NOT_FOUND" || err.code === "NODE_NOT_IN_GROUP") {
        return sendError(res, 404, err.code.toLowerCase(), {
          message: err.message,
        });
      }
      sendError(res, 500, "consensus_heartbeat_failed", {
        message: err.message,
      });
    }
  },
  ...
  function (req, res) {
    try {
      ...
      if (!groupId || !failedLeaderId || !candidateId) {
        return sendError(res, 400, "missing_params", {
          message: "groupId, failedLeaderId, and candidateId are required",
        });
      }

      const result = coordinator.initiateViewChange(
        groupId,
        failedLeaderId,
        candidateId,
      );
      ...
      sendError(res, 500, "consensus_view_change_failed", {
        message: err.message,
      });
    }
  },
  ...
  function (req, res) {
    try {
      ...
      if (!groupId || !voterId || !candidateId) {
        return sendError(res, 400, "missing_params", {
          message: "groupId, voterId, and candidateId are required",
        });
      }

      const result = coordinator.castViewChangeVote(
        groupId,
        voterId,
        candidateId,
      );
      ...
      sendError(res, 500, "consensus_view_change_vote_failed", {
        message: err.message,
      });
    }
  },
...
function getEnclaveAdapter() {
  const baseAdapter = require("../lib/hsm-adapter/base-adapter.cjs");
  if (
    baseAdapter.getHardwareEnclaveAdapter &&
    typeof baseAdapter.getHardwareEnclaveAdapter === "function"
  ) {
  ...
  async function (req, res) {
    try {
      ...
      if (!attestationDocument) {
        return sendError(res, 400, "missing_params", {
          message: "attestationDocument is required",
        });
      }
      ...
      if (
        err.code === "ENCLAVE_ATTESTATION_REQUIRED" ||
        err.code === "ATTESTATION_UNTRUSTED_AUTHORITY" ||
        err.code === "ATTESTATION_UNTRUSTED_MEASUREMENT" ||
        err.code === "ATTESTATION_EXPIRED" ||
        err.code === "ATTESTATION_SIGNATURE_INVALID" ||
        err.code === "ATTESTATION_INVALID_DOCUMENT"
      ) {
        return sendError(res, 403, err.code.toLowerCase(), {
          message: err.message,
        });
      }
      sendError(res, 500, "enclave_initialize_failed", {
        message: err.message,
      });
    }
  },
...
router.post("/enclave/seal", authorize("admin:all"), async function (req, res) {
  try {
    ...
    if (err.code === "ENCLAVE_NOT_INITIALIZED") {
      return sendError(res, 409, err.code.toLowerCase(), {
        message: err.message,
      });
    }
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      if (!keyMaterial) {
        return sendError(res, 400, "missing_params", {
          message: "keyMaterial is required",
        });
      }
      ...
      sendError(res, 500, "enclave_provision_key_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      if (!measurement) {
        return sendError(res, 400, "missing_params", {
          message: "measurement query param is required",
        });
      }
      ...
      sendError(res, 500, "enclave_attestation_verify_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/enclave/attestation/clear-cache",
  authorize("admin:all"),
  function (req, res) {
    try {
      const adapter = requireEnclaveAdapter(res);
      ...
      sendError(res, 500, "enclave_attestation_clear_cache_failed", {
        message: err.message,
      });
    }
  },
...
function requireRecursiveProofEngine(res) {
  ...
  if (!engine) {
    sendError(res, 503, "recursive_proof_engine_unavailable", {
      message:
        "No RecursiveProofAggregationEngine is registered with the HSM adapter.",
    });
    return null;
  }
  return engine;
...
if (!baseAdapter.getRecursiveProofAggregationEngine()) {
  baseAdapter.registerRecursiveProofAggregationEngine(
    new RecursiveProofAggregationEngine({
      audit: (event) => {
        switch (event) {
...
function handleHsmError(res, err) {
  ...
  sendError(
    res,
    err.code ? err.statusCode || 400 : 500,
    err.code || "recursive_aggregation_error",
    { message: err.message },
  );
}
...
router.post(
  "/recursive-aggregation/aggregate/chain",
  authorize("admin:all"),
  function (req, res) {
    try {
      const engine = requireRecursiveProofEngine(res);
...
router.post(
  "/recursive-aggregation/aggregate/tree",
  authorize("admin:all"),
  function (req, res) {
    try {
      const engine = requireRecursiveProofEngine(res);
...
router.post(
  "/recursive-aggregation/aggregate/vdf",
  authorize("admin:all"),
  function (req, res) {
    try {
      const engine = requireRecursiveProofEngine(res);
...
router.post(
  "/recursive-aggregation/compress/mixnet",
  authorize("admin:all"),
  function (req, res) {
    try {
      const engine = requireRecursiveProofEngine(res);
...
router.get(
  "/recursive-aggregation/aggregations/:aggId",
  authorize("admin:all"),
  function (req, res) {
    try {
      const engine = requireRecursiveProofEngine(res);
      ...
      if (!result)
        return sendError(res, 404, "aggregation_not_found", {
          message: `aggregation ${req.params.aggId} not found`,
        });
      res.json({ success: true, ...result });
    } catch (err) {
      handleHsmError(res, err);
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "decentralized_identity_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/decentralized-identity/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "decentralized_identity_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/decentralized-identity/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const hsmMetrics = require("../lib/hsm-adapter/hsm-metrics.cjs");
      ...
      const telemetry = {
        hsm_didgate_pool_initialized_total:
          allMetrics.hsm_didgate_pool_initialized_total || 0,
        hsm_zk_identity_claim_verified_total:
          allMetrics.hsm_zk_identity_claim_verified_total || 0,
        hsm_revocation_accreditation_completed_total:
          allMetrics.hsm_revocation_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "decentralized_identity_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "laser_mesh_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/space-based-laser-mesh/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "laser_mesh_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/space-based-laser-mesh/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const hsmMetrics = require("../lib/hsm-adapter/hsm-metrics.cjs");
      ...
      const telemetry = {
        hsm_lasergate_pool_initialized_total:
          allMetrics.hsm_lasergate_pool_initialized_total || 0,
        hsm_zk_laser_mesh_claim_verified_total:
          allMetrics.hsm_zk_laser_mesh_claim_verified_total || 0,
        hsm_handoff_accreditation_completed_total:
          allMetrics.hsm_handoff_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "laser_mesh_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "qkd_link_switch_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/qkd-link-switch/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "qkd_link_switch_policy_validate_failed", {
        message: err.message,
      });
    }
  },
    ...
    try {
      ...
      const telemetry = {
        hsm_qkdswitchgate_pool_initialized_total:
          allMetrics.hsm_qkdswitchgate_pool_initialized_total || 0,
        hsm_zk_qkd_link_claim_verified_total:
          allMetrics.hsm_zk_qkd_link_claim_verified_total || 0,
        hsm_entanglement_accreditation_completed_total:
          allMetrics.hsm_entanglement_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "qkd_link_switch_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "holographic_storage_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/holographic-storage/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "holographic_storage_policy_validate_failed", {
        message: err.message,
      });
    }
  },
    ...
    try {
      ...
      const telemetry = {
        hsm_hologate_pool_initialized_total:
          allMetrics.hsm_hologate_pool_initialized_total || 0,
        hsm_zk_holographic_claim_verified_total:
          allMetrics.hsm_zk_holographic_claim_verified_total || 0,
        hsm_phase_accreditation_completed_total:
          allMetrics.hsm_phase_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "holographic_storage_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "supply_chain_provenance_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/supply-chain-provenance/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "supply_chain_provenance_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/supply-chain-provenance/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const hsmMetrics = require("../lib/hsm-adapter/hsm-metrics.cjs");
      ...
      const telemetry = {
        hsm_supplygate_pool_initialized_total:
          allMetrics.hsm_supplygate_pool_initialized_total || 0,
        hsm_zk_provenance_claim_verified_total:
          allMetrics.hsm_zk_provenance_claim_verified_total || 0,
        hsm_lineage_accreditation_completed_total:
          allMetrics.hsm_lineage_accreditation_completed_total || 0,
        hsm_supplygate_settled_total:
          allMetrics.hsm_supplygate_settled_total || 0,
        hsm_supplygate_rebalanced_total:
          allMetrics.hsm_supplygate_rebalanced_total || 0,
        hsm_supplygate_slash_recorded_total:
          allMetrics.hsm_supplygate_slash_recorded_total || 0,
        hsm_provenance_batch_verified_total:
          allMetrics.hsm_provenance_batch_verified_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "supply_chain_provenance_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
function requireZkDecentralizedStorageGating(res) {
  if (!ZK_DECENTRALIZED_STORAGE_GATING_ENABLED) {
    sendError(res, 503, "zk_decentralized_storage_gating_unavailable", {
      message:
        "Track 111 Zero-Knowledge Decentralized Storage Attestation Gating is not yet available.",
    });
    return false;
  }
  return true;
...
router.get(
  "/zk-decentralized-storage/policy",
  authorize("admin:all"),
  function (req, res) {
    ...
    try {
      ...
      sendError(res, 500, "zk_decentralized_storage_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/zk-decentralized-storage/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    if (!requireZkDecentralizedStorageGating(res)) return;
    try {
      ...
      sendError(res, 500, "zk_decentralized_storage_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
function requireBioDigitalNeuralTelemetryGating(res) {
  if (!BIO_DIGITAL_NEURAL_TELEMETRY_GATING_ENABLED) {
    sendError(res, 503, "bio_digital_neural_telemetry_gating_unavailable", {
      message:
        "Track 112 Bio-Digital Interface Neural-Telemetry Gating is not yet available.",
    });
    return false;
  }
  return true;
...
router.get(
  "/bio-digital-neural-telemetry/policy",
  authorize("admin:all"),
  function (req, res) {
    ...
    try {
      ...
      sendError(res, 500, "bio_digital_neural_telemetry_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/bio-digital-neural-telemetry/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    if (!requireBioDigitalNeuralTelemetryGating(res)) return;
    try {
      ...
      sendError(
        res,
        500,
        "bio_digital_neural_telemetry_policy_validate_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/bio-digital-neural-telemetry/telemetry",
  authorize("admin:all"),
  function (req, res) {
    if (!requireBioDigitalNeuralTelemetryGating(res)) return;
    try {
      ...
      const telemetry = {
        hsm_neurogate_pool_initialized_total:
          allMetrics.hsm_neurogate_pool_initialized_total || 0,
        hsm_zk_neural_telemetry_verified_total:
          allMetrics.hsm_zk_neural_telemetry_verified_total || 0,
        hsm_synapse_accreditation_completed_total:
          allMetrics.hsm_synapse_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(
        res,
        500,
        "bio_digital_neural_telemetry_telemetry_fetch_failed",
        { message: err.message },
      );
    }
  },
...
function requireAutonomousDroneSwarmMeshRoutingGating(res) {
  if (!DRONE_SWARM_MESH_ROUTING_GATING_ENABLED) {
    sendError(
      res,
      503,
      "autonomous_drone_swarm_mesh_routing_gating_unavailable",
      {
        message:
          "Track 113 Autonomous Drone Swarm Mesh-Routing Gating is not yet available.",
      },
    );
    return false;
  }
...
router.get(
  "/autonomous-drone-swarm-mesh-routing/policy",
  authorize("admin:all"),
  function (req, res) {
    ...
    try {
      ...
      sendError(
        res,
        500,
        "autonomous_drone_swarm_mesh_routing_policy_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.post(
  "/autonomous-drone-swarm-mesh-routing/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    if (!requireAutonomousDroneSwarmMeshRoutingGating(res)) return;
    try {
      ...
      sendError(
        res,
        500,
        "autonomous_drone_swarm_mesh_routing_policy_validate_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/autonomous-drone-swarm-mesh-routing/telemetry",
  authorize("admin:all"),
  function (req, res) {
    if (!requireAutonomousDroneSwarmMeshRoutingGating(res)) return;
    try {
      ...
      const telemetry = {
        hsm_dronegate_pool_initialized_total:
          allMetrics.hsm_dronegate_pool_initialized_total || 0,
        hsm_zk_swarm_routing_verified_total:
          allMetrics.hsm_zk_swarm_routing_verified_total || 0,
        hsm_topology_accreditation_completed_total:
          allMetrics.hsm_topology_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(
        res,
        500,
        "autonomous_drone_swarm_mesh_routing_telemetry_fetch_failed",
        { message: err.message },
      );
    }
  },
...
function requireSwarmRoboticsKineticAssemblyGating(res) {
  if (!SWARM_ROBOTICS_KINETIC_ASSEMBLY_GATING_ENABLED) {
    sendError(res, 503, "swarm_robotics_kinetic_assembly_gating_unavailable", {
      message:
        "Track 114 Swarm Robotics Kinetic Assembly Gating is not yet available.",
    });
    return false;
  }
  return true;
...
router.get(
  "/swarm-robotics-kinetic-assembly/policy",
  authorize("admin:all"),
  function (req, res) {
    ...
    try {
      ...
      sendError(
        res,
        500,
        "swarm_robotics_kinetic_assembly_policy_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.post(
  "/swarm-robotics-kinetic-assembly/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    if (!requireSwarmRoboticsKineticAssemblyGating(res)) return;
    try {
      ...
      sendError(
        res,
        500,
        "swarm_robotics_kinetic_assembly_policy_validate_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/swarm-robotics-kinetic-assembly/telemetry",
  authorize("admin:all"),
  function (req, res) {
    if (!requireSwarmRoboticsKineticAssemblyGating(res)) return;
    try {
      ...
      const telemetry = {
        hsm_kineticgate_pool_initialized_total:
          allMetrics.hsm_kineticgate_pool_initialized_total || 0,
        hsm_zk_kinetic_posture_verified_total:
          allMetrics.hsm_zk_kinetic_posture_verified_total || 0,
        hsm_assembly_accreditation_completed_total:
          allMetrics.hsm_assembly_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(
        res,
        500,
        "swarm_robotics_kinetic_assembly_telemetry_fetch_failed",
        { message: err.message },
      );
    }
  },
...
function requireMultiEnclaveConfidentialMeshStateReconciliationGating(res) {
  if (!MULTI_ENCLAVE_CONFIDENTIAL_MESH_STATE_RECONCILIATION_GATING_ENABLED) {
    sendError(
      res,
      503,
      "multi_enclave_confidential_mesh_state_reconciliation_gating_unavailable",
      {
        message:
          "Track 115 Multi-Enclave Confidential Mesh State-Reconciliation Gating is not yet available.",
      },
    );
    return false;
  }
...
router.get(
  "/multi-enclave-confidential-mesh-state-reconciliation/policy",
  authorize("admin:all"),
  function (req, res) {
    if (!requireMultiEnclaveConfidentialMeshStateReconciliationGating(res))
      return;
    try {
      const {
        DEFAULT_POLICY,
      ...
      sendError(
        res,
        500,
        "multi_enclave_confidential_mesh_state_reconciliation_policy_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.post(
  "/multi-enclave-confidential-mesh-state-reconciliation/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    if (!requireMultiEnclaveConfidentialMeshStateReconciliationGating(res))
      return;
    try {
      ...
      sendError(
        res,
        500,
        "multi_enclave_confidential_mesh_state_reconciliation_policy_validate_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/multi-enclave-confidential-mesh-state-reconciliation/telemetry",
  authorize("admin:all"),
  function (req, res) {
    if (!requireMultiEnclaveConfidentialMeshStateReconciliationGating(res))
      return;
    try {
      ...
      const telemetry = {
        hsm_meshgate_pool_initialized_total:
          allMetrics.hsm_meshgate_pool_initialized_total || 0,
        hsm_zk_mesh_state_reconciled_total:
          allMetrics.hsm_zk_mesh_state_reconciled_total || 0,
        hsm_epoch_finality_completed_total:
          allMetrics.hsm_epoch_finality_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(
        res,
        500,
        "multi_enclave_confidential_mesh_state_reconciliation_telemetry_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/zk-decentralized-storage/telemetry",
  authorize("admin:all"),
  function (req, res) {
    if (!requireZkDecentralizedStorageGating(res)) return;
    try {
      ...
      const telemetry = {
        hsm_zkstorage_pool_initialized_total:
          allMetrics.hsm_zkstorage_pool_initialized_total || 0,
        hsm_zk_storage_proof_verified_total:
          allMetrics.hsm_zk_storage_proof_verified_total || 0,
        hsm_zkstorage_replication_accreditation_completed_total:
          allMetrics.hsm_zkstorage_replication_accreditation_completed_total ||
          0,
        hsm_zkstorage_dispersal_completed_total:
          allMetrics.hsm_zkstorage_dispersal_completed_total || 0,
        hsm_zkstorage_slash_recorded_total:
          allMetrics.hsm_zkstorage_slash_recorded_total || 0,
        hsm_zkstorage_challenge_issued_total:
          allMetrics.hsm_zkstorage_challenge_issued_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "zk_decentralized_storage_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
    ...
    try {
      ...
      const telemetry = {
        hsm_lookupgate_pool_initialized_total:
          allMetrics.hsm_lookupgate_pool_initialized_total || 0,
        hsm_zk_lookup_claim_verified_total:
          allMetrics.hsm_zk_lookup_claim_verified_total || 0,
        hsm_lookup_accreditation_completed_total:
          allMetrics.hsm_lookup_accreditation_completed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "lookup_gating_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    try {
      ...
      sendError(res, 400, err.code || "LOOKUPGATE_INVALID_INPUT", {
        message: err.message,
      });
    }
  }),
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    try {
      ...
      sendError(res, 400, err.code || "LOOKUPCLAIM_INVALID_INPUT", {
        message: err.message,
      });
    }
  }),
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    try {
      ...
      sendError(res, 400, err.code || "LOOKUPGATE_INVALID_STATE", {
        message: err.message,
      });
    }
  }),
...
function handshakeProofIsValid(record, clientProof) {
  ...
  const expected = crypto
    .createHmac("sha256", record.sessionId)
    .update("track113-proof-challenge")
    .digest("hex");
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    const lifecycleTimeout =
      req.body && typeof req.body.lifecycleTimeout === "number"
        ? req.body.lifecycleTimeout
        : 3600000;
    if (!clientId || !handshakeDigest)
      return sendError(res, 400, "HANDSHAKE_MISSING_PARAMETERS");

    const sessionId = crypto.randomBytes(16).toString("hex");
    const tenantId = resolveOrgId(req);
    const now = Date.now();
    const handshakeDigestEncrypted = encryptEnvelope(
      handshakeDigest,
      process.env.TRACK113_KEK,
    );
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    const expectedStateDigest =
      (req.body && req.body.expectedStateDigest) || "";
    if (!sessionId || !clientProof || !expectedStateDigest)
      return sendError(res, 400, "HANDSHAKE_MISSING_PARAMETERS");

    const record = SessionStore.get(sessionId);
    if (!record) return sendError(res, 404, "HANDSHAKE_SESSION_NOT_FOUND");
    if (record.status !== "INITIALIZED")
      return sendError(res, 400, "HANDSHAKE_INVALID_STATE");

    // verify client proof; expectedStateDigest is accepted as a caller-supplied structural check
    if (!handshakeProofIsValid(record, clientProof))
      return sendError(res, 400, "HANDSHAKE_INVALID_PROOF");
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    if (!Array.isArray(anonymitySet))
      return sendError(res, 400, "RINGGATE_INVALID_KEYS");
    try {
      ...
      sendError(res, 400, err.code || "RINGGATE_INVALID_TRANSITION", {
        message: err.message,
      });
    }
  }),
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    if (!entry) return sendError(res, 404, "ring_pool_not_found");
    const claim = (req.body && req.body.claim) || {};
    const linkabilityToken =
      (req.body && req.body.linkabilityToken) || undefined;
    const blindedLinkabilityAttestation =
      (req.body && req.body.blindedLinkabilityAttestation) || undefined;
    try {
      ...
      const code =
        err.message &&
        err.message.includes("RINGCLAIM_INVALID_ANONYMITY_SET_SIZE")
          ? "RINGCLAIM_INVALID_ANONYMITY_SET_SIZE"
          : err.message &&
              err.message.includes("RINGCLAIM_UNATTESTED_LINKABILITY")
    ...
    try {
      ...
      const telemetry = {
        hsm_ringgate_pool_initialized_total:
          allMetrics.hsm_ringgate_pool_initialized_total || 0,
        hsm_zk_ring_claim_verified_total:
          allMetrics.hsm_zk_ring_claim_verified_total || 0,
        hsm_ring_accreditation_completed_total:
          allMetrics.hsm_ring_accreditation_completed_total || 0,
      ...
      sendError(res, 500, "ring_gating_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/accumulator-gating/:poolId/witnesses",
  authorize("admin:all"),
  runAsync(async (req, res) => {
    const orgId = resolveOrgId(req);
    ...
    if (!Array.isArray(witnesses))
      return sendError(res, 400, "ACCUMULATORGATE_INVALID_WITNESSES");
    try {
      ...
      sendError(res, 400, err.code || "ACCUMULATORGATE_INVALID_TRANSITION", {
        message: err.message,
      });
    }
  }),
...
router.post(
  "/accumulator-gating/:poolId/validate",
  authorize("admin:all"),
  runAsync(async (req, res) => {
    const orgId = resolveOrgId(req);
    const entry = resolveAccumulatorGatingPool(req.params.poolId);
...
router.post(
  "/accumulator-gating/:poolId/accredit",
  authorize("admin:all"),
  runAsync(async (req, res) => {
    const orgId = resolveOrgId(req);
    const entry = resolveAccumulatorGatingPool(req.params.poolId);
    ...
    try {
      ...
      const telemetry = {
        hsm_accumulatorgate_pool_initialized_total:
          allMetrics.hsm_accumulatorgate_pool_initialized_total || 0,
        hsm_zk_accumulator_claim_verified_total:
          allMetrics.hsm_zk_accumulator_claim_verified_total || 0,
        hsm_accumulator_accreditation_completed_total:
          allMetrics.hsm_accumulator_accreditation_completed_total || 0,
      ...
      sendError(res, 500, "accumulator_gating_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    if (!Array.isArray(shares))
      return sendError(res, 400, "VSSGATE_INVALID_SHARES");
    try {
      ...
      sendError(res, 400, err.code || "VSSGATE_INVALID_TRANSITION", {
        message: err.message,
      });
    }
  }),
    ...
    try {
      ...
      const telemetry = {
        hsm_vssgate_pool_initialized_total:
          allMetrics.hsm_vssgate_pool_initialized_total || 0,
        hsm_zk_vss_claim_verified_total:
          allMetrics.hsm_zk_vss_claim_verified_total || 0,
        hsm_vss_accreditation_completed_total:
          allMetrics.hsm_vss_accreditation_completed_total || 0,
      ...
      sendError(res, 500, "lattice_vss_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  ...
  runAsync(async (req, res) => {
    const orgId = resolveOrgId(req);
    const hub = new PqcLatticeVfhssGatingHub(
      orgId,
      latticeVfhssGatingPolicyEngine,
    );
    const poolId = crypto.randomBytes(16).toString("hex");
...
router.post(
  ...
  runAsync(async (req, res) => {
    ...
    const shares = (req.body && req.body.shares) || [];
    if (!Array.isArray(shares))
      return sendError(res, 400, "VFHSSGATE_INVALID_SHARES");
    try {
      const status = entry.hub.collectShares(shares);
      res.json({
        success: true,
      ...
      sendError(res, 400, err.code || "VFHSSGATE_INVALID_TRANSITION", {
        message: err.message,
      });
    }
  }),
  ...
  runAsync(async (req, res) => {
    ...
    try {
      const status = entry.hub.validateProof({
        ...claim,
        homomorphicDepth:
          manifest.homomorphicDepth !== undefined
            ? manifest.homomorphicDepth
            : claim.homomorphicDepth,
      });
      res.json({
        success: true,
        orgId,
    ...
    try {
      ...
      const telemetry = {
        hsm_vfhssgate_pool_initialized_total:
          allMetrics.hsm_vfhssgate_pool_initialized_total || 0,
        hsm_zk_vfhss_claim_verified_total:
          allMetrics.hsm_zk_vfhss_claim_verified_total || 0,
        hsm_vfhss_accreditation_completed_total:
          allMetrics.hsm_vfhss_accreditation_completed_total || 0,
      ...
      sendError(res, 500, "lattice_vfhss_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "cluster_isolation_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/cluster-isolation/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "cluster_isolation_policy_validate_failed", {
        message: err.message,
      });
    }
  },
    ...
    try {
      ...
      const telemetry = {
        hsm_isolation_violation_total:
          allMetrics.hsm_isolation_violation_total || 0,
      ...
      sendError(res, 500, "cluster_isolation_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "bft_shard_sync_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/bft-shard-sync/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "bft_shard_sync_policy_validate_failed", {
        message: err.message,
      });
    }
  },
    ...
    try {
      ...
      const telemetry = {
        ...
        hsm_shard_catchup_batch_total:
          allMetrics.hsm_shard_catchup_batch_total || 0,
        hsm_shard_byzantine_detected_total:
          allMetrics.hsm_shard_byzantine_detected_total || 0,
        hsm_shard_limit_exceeded_total:
          allMetrics.hsm_shard_limit_exceeded_total || 0,
      ...
      sendError(res, 500, "bft_shard_sync_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
function getMusig2Orchestrator() {
  if (
    baseAdapter.getMusig2Orchestrator &&
    typeof baseAdapter.getMusig2Orchestrator === "function"
  ) {
 ...
 */
router.post(
  "/musig2/session/create",
  authorize("admin:all"),
  async function (req, res) {
    try {
      const orchestrator = getMusig2Orchestrator();
      if (!orchestrator) {
        return sendError(res, 503, "musig2_not_configured", {
          message: "MuSig2 HSM orchestrator is not registered",
        });
      }
      const { tenantId, participantIds, quorum, messageHash, keyShares } =
        req.body;
      ...
      sendError(res, 400, "musig2_session_create_failed", {
        message: err.message,
      });
    }
  },
 ...
 */
router.get(
  "/musig2/session/:sessionId/status",
  authorize("admin:all"),
  function (req, res) {
    try {
      const orchestrator = getMusig2Orchestrator();
...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      res.json({
        success: true,
        signature: { R: signature.R.toString(), s: signature.s.toString() },
...
router.get(
  "/distributed-consensus-coordinator/policy",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(
        res,
        500,
        "distributed_consensus_coordinator_policy_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.post(
  "/distributed-consensus-coordinator/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(
        res,
        500,
        "distributed_consensus_coordinator_policy_validate_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/distributed-consensus-coordinator/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const allMetrics = hsmMetrics.getMetrics();
      const telemetry = {
        hsm_consensus_coord_groups_created_total:
          allMetrics.hsm_consensus_coord_groups_created_total || 0,
        hsm_consensus_coord_groups_destroyed_total:
          allMetrics.hsm_consensus_coord_groups_destroyed_total || 0,
        hsm_consensus_coord_proposals_routed_total:
          allMetrics.hsm_consensus_coord_proposals_routed_total || 0,
        hsm_consensus_coord_proposals_rejected_total:
          allMetrics.hsm_consensus_coord_proposals_rejected_total || 0,
        hsm_consensus_coord_faults_detected_total:
          allMetrics.hsm_consensus_coord_faults_detected_total || 0,
        hsm_consensus_coord_view_change_started_total:
          allMetrics.hsm_consensus_coord_view_change_started_total || 0,
        hsm_consensus_coord_view_change_completed_total:
          allMetrics.hsm_consensus_coord_view_change_completed_total || 0,
        hsm_consensus_coord_view_change_aborted_total:
          allMetrics.hsm_consensus_coord_view_change_aborted_total || 0,
        hsm_consensus_coord_quorum_verified_total:
          allMetrics.hsm_consensus_coord_quorum_verified_total || 0,
        hsm_consensus_coord_quorum_denied_total:
          allMetrics.hsm_consensus_coord_quorum_denied_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(
        res,
        500,
        "distributed_consensus_coordinator_telemetry_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/cross-cluster-migration/policy",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "cross_cluster_migration_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/cross-cluster-migration/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      if (sanitized.allowedAttestationAuthorities !== undefined) {
        ...
        sanitized.allowedAttestationAuthorities =
          sanitized.allowedAttestationAuthorities.filter(
            (a) => typeof a === "string",
          );
      }
      ...
      sendError(res, 500, "cross_cluster_migration_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/cross-cluster-migration/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const allMetrics = hsmMetrics.getMetrics();
      const telemetry = {
        hsm_migration_initiated_total:
          allMetrics.hsm_migration_initiated_total || 0,
        hsm_migration_attested_total:
          allMetrics.hsm_migration_attested_total || 0,
        hsm_migration_committed_total:
          allMetrics.hsm_migration_committed_total || 0,
        hsm_migration_rolled_back_total:
          allMetrics.hsm_migration_rolled_back_total || 0,
        hsm_migration_ack_total: allMetrics.hsm_migration_ack_total || 0,
        hsm_migration_verification_failed_total:
          allMetrics.hsm_migration_verification_failed_total || 0,
      ...
      sendError(res, 500, "cross_cluster_migration_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/cluster-key-reconciliation/policy",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "cluster_key_reconciliation_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/cluster-key-reconciliation/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "cluster_key_reconciliation_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/cluster-key-reconciliation/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const allMetrics = hsmMetrics.getMetrics();
      const telemetry = {
        hsm_reconciliation_scans_total:
          allMetrics.hsm_reconciliation_scans_total || 0,
        hsm_reconciliation_divergence_detected_total:
          allMetrics.hsm_reconciliation_divergence_detected_total || 0,
        hsm_reconciliation_promoted_total:
          allMetrics.hsm_reconciliation_promoted_total || 0,
        hsm_reconciliation_quarantined_total:
          allMetrics.hsm_reconciliation_quarantined_total || 0,
        hsm_reconciliation_rollback_blocked_total:
          allMetrics.hsm_reconciliation_rollback_blocked_total || 0,
        hsm_reconciliation_promotion_votes_total:
          allMetrics.hsm_reconciliation_promotion_votes_total || 0,
        hsm_reconciliation_divergent_keys:
          allMetrics.hsm_reconciliation_divergent_keys || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "cluster_key_reconciliation_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  ...
  function (req, res) {
    try {
      ...
      sendError(res, 500, "multiparty_rekeying_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/multiparty-re-keying/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "multiparty_rekeying_policy_validate_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/multiparty-re-keying/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const allMetrics = hsmMetrics.getMetrics();
      const telemetry = {
        ...
        hsm_rekey_resharing_submitted_total:
          allMetrics.hsm_rekey_resharing_submitted_total || 0,
        ...
        hsm_rekey_rollback_blocked_total:
          allMetrics.hsm_rekey_rollback_blocked_total || 0,
      ...
      sendError(res, 500, "multiparty_rekeying_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.get(
  "/replication-tenant-isolation/policy",
  authorize("admin:all"),
  function (req, res) {
    try {
      res.json({
      ...
      sendError(res, 500, "replication_tenant_isolation_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/replication-tenant-isolation/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
        validateTenantContext,
      } = require("../lib/replication-tenant-context.cjs");
      const message = req.body || {};
      const result = validateTenantContext(message);
      if (!result.valid) {
        return sendError(res, 400, "REPLICATION_TENANT_ISOLATION_VIOLATION", {
          message: result.reason || "invalid tenant context",
        });
      }
...
router.get(
  "/replication-tenant-isolation/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const allMetrics = hsmMetrics.getMetrics();
      const telemetry = {
        hsm_replication_tenant_isolation_violation_total:
          allMetrics.hsm_replication_tenant_isolation_violation_total || 0,
        hsm_replication_tenant_context_validated_total:
          allMetrics.hsm_replication_tenant_context_validated_total || 0,
        hsm_replication_cross_tenant_rejected_total:
          allMetrics.hsm_replication_cross_tenant_rejected_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(
        res,
        500,
        "replication_tenant_isolation_telemetry_fetch_failed",
        { message: err.message },
      );
    }
  },
...
router.get(
  "/zk-verification-isolation/policy",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
      ...
      sendError(res, 500, "zk_verification_isolation_policy_fetch_failed", {
        message: err.message,
      });
    }
  },
...
router.post(
  "/zk-verification-isolation/policy/validate",
  authorize("admin:all"),
  function (req, res) {
    try {
      const {
        validateTenantId,
      } = require("../lib/hsm-adapter/zk-tenant-governance.cjs");
      const tenantId = req.body && req.body.tenantId;
      if (!validateTenantId(tenantId)) {
        return sendError(res, 400, "ZK_TENANT_ISOLATION_VIOLATION", {
...
router.get(
  "/zk-verification-isolation/telemetry",
  authorize("admin:all"),
  function (req, res) {
    try {
      const allMetrics = hsmMetrics.getMetrics();
      const telemetry = {
        hsm_zk_tenant_isolation_violation_total:
          allMetrics.hsm_zk_tenant_isolation_violation_total || 0,
        hsm_zk_tenant_context_validated_total:
          allMetrics.hsm_zk_tenant_context_validated_total || 0,
        hsm_zk_energy_claim_verified_total:
          allMetrics.hsm_zk_energy_claim_verified_total || 0,
        hsm_zk_energy_claim_failed_total:
          allMetrics.hsm_zk_energy_claim_failed_total || 0,
        hsm_zk_biometric_claim_verified_total:
          allMetrics.hsm_zk_biometric_claim_verified_total || 0,
        hsm_zk_biometric_claim_failed_total:
          allMetrics.hsm_zk_biometric_claim_failed_total || 0,
        hsm_zk_neural_claim_verified_total:
          allMetrics.hsm_zk_neural_claim_verified_total || 0,
        hsm_zk_neural_claim_failed_total:
          allMetrics.hsm_zk_neural_claim_failed_total || 0,
        hsm_zk_quantum_claim_verified_total:
          allMetrics.hsm_zk_quantum_claim_verified_total || 0,
        hsm_zk_quantum_claim_failed_total:
          allMetrics.hsm_zk_quantum_claim_failed_total || 0,
      };
      res.json({
        success: true,
        orgId: resolveOrgId(req),
      ...
      sendError(res, 500, "zk_verification_isolation_telemetry_fetch_failed", {
        message: err.message,
      });
    }
  },
