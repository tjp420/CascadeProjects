...
const {
  ...
  registerUser,
  toAuthUser,
} = require("../services/user-service.cjs");
...
function resolveRpId(req) {
  const forwarded = String(req.headers["x-forwarded-host"] || "")
    .split(",")[0]
    .trim();
  const host = (forwarded || req.headers.host || "localhost")
    .split(":")[0]
    .toLowerCase();
...
function setupWebAuthnAPI(app) {
  ...
  app.post("/api/webauthn/challenge", (req, res) => {
    pruneChallenges();
    const purpose =
      req.body?.purpose === "register" ? "register" : "authenticate";
    ...
    try {
      ...
      if (!stored) {
        return sendError(
          res,
          401,
          "Unknown security key — register it in Profile first",
        );
      }
      pendingChallenges.delete(challengeId);
