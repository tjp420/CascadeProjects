...
router.get('/stats', authorize('admin:all'), (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch webhook event stats' });
  }
});
...
router.get('/', authorize('admin:all'), (req, res) => {
  try {
    ...
    if (req.query.limit) opts.limit = Math.min(200, Math.max(1, parseInt(req.query.limit, 10) || 50));
