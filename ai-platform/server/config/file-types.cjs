...
function _getLowerExt(filePath) {
  ...
  const result =
    idx < 0 || idx === base.length - 1 ? "" : base.slice(idx).toLowerCase();
...
function hasCategory(category) {
  return (
    typeof category === "string" && category !== "" && category in EXTENSIONS
  );
}
