...
const {
  readJsonFileCached,
  readTextFileCached,
} = require("../lib/json-file-cache.cjs");
...
} = require("../lib/snapshot-seeds.cjs");
const {
  PUBLIC_API_PATHS,
  isPublicApiRoute,
  isPublicApiRequest,
} = require("./public-api-routes.cjs");
...
async function ensurePhase2Schema(db) {
  ...
  for (const file of [
    "schema-phase2.sql",
    "schema-subscription.sql",
    "migrations/001_init_licensing.sql",
    "migrations/002-audit-bookings.sql",
  ]) {
...
async function seedDashboardSnapshotsFromSamples(db, webRoot, redis = null) {
  for (const seed of SNAPSHOT_SEEDS) {
    try {
      ...
      logger.warn(
        `[Phase2] Snapshot seed skipped for ${seed.key}:`,
        error.message,
      );
    }
  }
  if (redis) {
    ...
    if (cleared > 0 && shouldLogRuntimeInfo()) {
      logger.info(
        `[Phase2] Cleared ${cleared} stale Redis snapshot cache entries`,
      );
    }
  }
}
...
async function createDatabaseConnection() {
  ...
  try {
    ...
    if (process.env.LOG_QUERIES === "true" && shouldLogRuntimeInfo()) {
      logger.info(
        "[Phase2] Database query logging enabled (diagnostic env flag active)",
      );
    }
...
function setupAuthRoutes(app, authRateLimit, refreshRateLimit) {
  ...
  app.post("/api/auth/register", authRateLimit, async (req, res, next) => {
    try {
      const {
        handleRegister,
      } = require("../lib/auth/registration-service.cjs");
...
async function setupPhase2Integration(app, options = {}) {
  ...
  if (shouldLogRuntimeInfo()) {
    logger.info(
      `[Phase2] Integration ready - database: ${dbStatus}, redis: ${redisStatus}`,
    );
  }
