...
const { assertAuthConfiguration, assertProductionAuthSafety } = require('../lib/secret-config.cjs');
...
function createRateLimiters() {
  const rawAuthWindowMs = Number(
    process.env.AUTH_LOGIN_RATE_LIMIT_WINDOW_MS || constants.RATE_LIMIT_WINDOW_MS
  );
  const authWindowMs =
  ...
  const authMax =
    Number.isFinite(rawAuthMax) && rawAuthMax > 0 ? rawAuthMax : constants.AUTH_RATE_LIMIT;
