...
describe("public-api-routes", () => {
  ...
  it("allows free token routes without auth", () => {
    ...
    assert.strictEqual(
      mod.isPublicApiRoute("simplebeacon/billing/resend-token", "POST"),
      true,
    );
  });
  ...
  it("allows webauthn challenge and authenticate without auth gate", () => {
    assert.strictEqual(
      mod.isPublicApiRoute("webauthn/challenge", "POST"),
      true,
    );
    assert.strictEqual(
      mod.isPublicApiRoute("webauthn/authenticate", "POST"),
      true,
    );
    assert.strictEqual(
      mod.isPublicApiRoute("webauthn/register", "POST"),
      false,
    );
    assert.strictEqual(
      mod.isPublicApiRoute("webauthn/credentials", "GET"),
      false,
    );
  });
  ...
  it("allows read-only dashboard stub endpoints without auth", () => {
    ...
    assert.strictEqual(
      mod.isPublicApiRoute("dev-tools/workflows", "GET"),
      true,
    );
    ...
    assert.strictEqual(
      mod.isPublicApiRoute("coverage-reports/overview", "GET"),
      true,
    );
    assert.strictEqual(mod.isPublicApiRoute("help", "GET"), true);
    assert.strictEqual(mod.isPublicApiRoute("analytics/overview", "GET"), true);
    assert.strictEqual(
      mod.isPublicApiRoute("merger-tool/reduction-scan", "GET"),
      false,
    );
    assert.strictEqual(
      mod.isPublicApiRoute("security/npm-audit", "GET"),
      false,
    );
  });
