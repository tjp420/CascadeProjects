...
function isPublicOptimizationRoute(relativePath, method) {
  ...
  if (
    method === 'POST' &&
    (relativePath === 'optimization/analyze' || relativePath === 'optimization/merge-preview')
  ) {
    return true;
  }
  return false;
...
function isPublicAssessmentRoute(relativePath, method) {
  ...
  if (
    method === 'GET' &&
    /^assessment\/report\/assessment_\d+(?:\/download\/[\w-]+)?$/.test(relativePath)
  )
    return true;
  return false;
}
...
function isPublicWebAuthnRoute(relativePath, method) {
  const pathKey = String(relativePath || '').replace(/^\/+/, '');
  return (
    method === 'POST' && (pathKey === 'webauthn/challenge' || pathKey === 'webauthn/authenticate')
  );
}
