...
class DLPDashboard {
  constructor(config = {}) {
    ...
    this.violationLogPath =
      config.violationLogPath ||
      process.env.VIOLATION_LOG_PATH ||
      "./ai-violations.log";
    this.server = null;
  }
  ...
  calculateRiskScore(violations) {
    ...
    const score = violations.reduce(
      (total, v) => total + (severityWeights[v.severity] || 0),
      0,
    );
  ...
  assessRegulatoryExposure(violations) {
    ...
    for (const violation of violations) {
      for (const pattern of violation.patterns || []) {
        ...
        if (
          pattern.includes("medical") ||
          pattern.includes("patient") ||
          pattern.includes("diagnosis")
        ) {
          regulations.HIPAA += 1;
        }
        if (
          pattern.includes("credit-card") ||
          pattern.includes("bank") ||
  ...
  generateRecommendations(stats) {
    ...
    if (stats.bySeverity.critical > 0) {
      recommendations.push({
        priority: "CRITICAL",
        action:
          "Review all critical violations and implement additional security measures",
        timeline: "24 hours",
      });
    }

    if (
      (stats.byPattern["openai-key"] || 0) +
        (stats.byPattern["aws-access-key"] || 0) >
      0
    ) {
      recommendations.push({
        priority: "HIGH",
    ...
    if ((stats.byPattern["email-address"] || 0) > 10) {
      recommendations.push({
        priority: "MEDIUM",
        action:
          "Review email handling and implement redaction policies for AI prompts",
        timeline: "2 weeks",
      });
    }
