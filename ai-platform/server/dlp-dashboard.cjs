...
class DLPDashboard {
  constructor(config = {}) {
    ...
    this.violationLogPath =
      config.violationLogPath || process.env.VIOLATION_LOG_PATH || './ai-violations.log';
    this.server = null;
  }
  ...
  calculateComplianceScore(stats) {
    const deductions =
      stats.bySeverity.critical * 10 + stats.bySeverity.high * 5 + stats.bySeverity.medium * 2;
