...
class ModelHealthTracker {
  ...
  updateCircuitBreaker(modelId) {
    ...
    if (
      circuit.state === CIRCUIT_STATES.OPEN &&
      Date.now() - circuit.lastFailureTime > 60000
    ) {
      // 1 minute timeout
      circuit.state = CIRCUIT_STATES.HALF_OPEN;
      circuit.lastTestTime = Date.now();
      logger.info(
        `[Model Manager] Circuit half-open for model ${modelId}, testing...`,
      );
    }
    ...
    if (
      circuit.state === CIRCUIT_STATES.HALF_OPEN &&
      recent[recent.length - 1]?.success
    ) {
      circuit.state = CIRCUIT_STATES.CLOSED;
      circuit.failures = 0;
      logger.info(
        `[Model Manager] Circuit closed for model ${modelId}, recovery successful`,
      );
    }
  ...
  getHealthSummary(modelId) {
    ...
    return {
      ...
      avgResponseTime:
        recent.length > 0
          ? recent.reduce((sum, r) => sum + (r.responseTime || 0), 0) /
            recent.length
          : 0,
...
class EnhancedModelManager {
  ...
  async getOptimalModel(requirements = {}) {
    const { preferredProvider, analysisType, complexity, maxResponseTime } =
      requirements;
    ...
    const availableModels = models.filter((model) =>
      this.healthTracker.isModelAvailable(model.id),
    );

    if (availableModels.length === 0) {
      logger.warn(
        "[Model Manager] All models have open circuits, forcing fallback",
      );
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt === 0) {
          ...
          logger.info(
            `[Model Manager] Retry ${attempt + 1} with model: ${model.id}`,
          );
        }
        ...
        logger.warn(
          `[Model Manager] Analysis attempt ${attempt + 1} failed:`,
          error.message,
        );
        ...
        if (attempt === maxRetries) {
          throw new Error(
            `Analysis failed after ${maxRetries + 1} attempts: ${error.message}`,
          );
        }
  ...
  async getAvailableModels() {
    ...
    if (
      now - this.lastCacheUpdate < this.cacheTimeoutMs &&
