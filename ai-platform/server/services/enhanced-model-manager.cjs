...
class ModelHealthTracker {
    ...
    updateCircuitBreaker(modelId) {
        ...
        const avgResponseTime = recent.reduce((sum, r) => sum + (r.responseTime || 0), 0) / recent.length;
        ...
        if (failureRate > 0.5 && circuit.state === CIRCUIT_STATES.CLOSED) {
            ...
            logger.warn(`[Model Manager] Circuit opened for model ${modelId} due to high failure rate: ${failureRate}`);
        }
        
        // Test circuit after timeout
        if (circuit.state === CIRCUIT_STATES.OPEN && 
            Date.now() - circuit.lastFailureTime > 60000) { // 1 minute timeout
    ...
    getHealthSummary(modelId) {
        ...
        return {
            ...
            avgResponseTime: recent.length > 0 ? 
                recent.reduce((sum, r) => sum + (r.responseTime || 0), 0) / recent.length : 0,
...
class EnhancedModelManager {
    ...
    async getOptimalModel(requirements = {}) {
        ...
        logger.debug(`[Model Manager] Selected optimal model: ${bestModel.id} (score: ${bestModel.score})`);
    ...
    async executeCloudAnalysis(model, prompt, options) {
        ...
        const result = await summarizeScanWithProvider(model.provider, { prompt }, {
            customPrompt: prompt,
