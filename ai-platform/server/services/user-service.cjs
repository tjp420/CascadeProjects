...
async function authenticateWithDemoFile(identifier, password) {
  ...
  const match = demoUsers.find(
    (user) =>
      user.email.toLowerCase() === lower || String(user.username || '').toLowerCase() === lower
  );
  ...
  return {
    ...
    createdAt:
      match.createdAt ||
      new Date(Date.now() - 30 * 24 * 60 * constants.ONE_MINUTE_MS).toISOString(),
...
async function authenticateUser(db, email, password) {
  ...
  const emergencyEmail = String(process.env.SIMPLEBEACON_EMERGENCY_EMAIL || 'admin@simplebeacon.ai')
    .trim()
    .toLowerCase();
