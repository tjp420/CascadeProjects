...
function ensureSqliteDemoUsers() {
  ...
  for (const u of demoUsers) {
    ...
    const passwordHash = crypto
      .scryptSync(u.password, salt, 64)
      .toString("hex");
    try {
      sqlite.createUser(u.email, passwordHash, salt, u.tier);
    } catch (err) {
      logger.warn(
        "[UserService] SQLite demo seed failed for",
        u.email,
        err.message,
      );
    }
...
function sqliteUserToAuthUser(user) {
  ...
  return {
    ...
    trustLevel:
      tier === "admin" ? "gold" : tier === "silver" ? "silver" : "bronze",
...
function registerUserSqlite(email, password, name, options = {}) {
  ...
  if (
    username &&
    typeof sqlite.getUserByUsername === "function" &&
    sqlite.getUserByUsername(username)
  ) {
  ...
  const passwordHash = crypto
    ...
    .toString("hex");
  const user =
    typeof sqlite.createUser === "function"
...
function toAuthUser(row) {
  return {
    ...
    communityContributions:
      row.community_contributions ?? row.communityContributions ?? 0,
    verificationStatus:
      row.verification_status || row.verificationStatus || "email",
  };
}
...
async function authenticateWithDemoFile(identifier, password) {
  ...
  return {
    ...
    createdAt:
      match.createdAt ||
      new Date(
        Date.now() - 30 * 24 * 60 * constants.ONE_MINUTE_MS,
      ).toISOString(),
...
async function authenticateUser(db, email, password) {
  ...
  if (db) {
    try {
      ...
      logger.warn(
        "[UserService] Database auth failed, falling back to demo file:",
        err.message,
      );
    }
  }
...
async function registerUserDatabase(db, email, password, name, options = {}) {
  ...
  try {
    ...
    logger.warn(
      "[UserService] PostgreSQL registration failed, falling back:",
      err.message,
    );
    return null;
  }
...
async function registerUser(email, password, name, options = {}) {
  ...
  if (db) {
    const pgResult = await registerUserDatabase(
      db,
      email,
      password,
      name,
  ...
  if (
    username &&
    demoUsers.some((u) => String(u.username || "").toLowerCase() === username)
  ) {
    return { error: "That username is already taken" };
  }
