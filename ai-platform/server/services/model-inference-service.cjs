...
const {
  ensureRegistry,
  getActiveModelInfo,
} = require("./local-model-service.cjs");
const {
  scanMockDataDirectories,
  _formatBytes,
} = require("../lib/mock-data-scanner.cjs");
...
const SAMPLE_PATH =
  process.env.MOCK_ANALYSIS_SAMPLE_PATH ||
  path.join("data", "templates", "analysis-template.json");
...
function buildQualityMetrics(scan) {
  ...
  return {
    ...
    dataIntegrity: Math.max(
      55,
      Math.round(overall - invalidPenalty - emptyPenalty),
    ),
    schemaCompliance: schema,
    consistencyScore:
      scan.consistencyScore ??
      Math.max(55, Math.round(overall - duplicatePenalty)),
...
async function tryLlamaCppInference(modelPath, prompt) {
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      prompt,
      "-n",
      String(process.env.LLAMA_CPP_MAX_TOKENS || 512),
      "--temp",
      "0.2",
    ];
    ...
    child.on("close", (code) => {
      ...
      resolve(
        extractJsonObject(stdout) || {
          rawText: stdout.slice(0, constants.MAX_RATE_LIMIT),
  ...
  return {
    report: {
      ...
      ggufAIInsights: {
        ...
        aiSuggestedIssues: parsed.detectedIssues?.length
          ? parsed.detectedIssues
          : undefined,
      },
      inferenceMeta: {
        ...baseReport.inferenceMeta,
        issueSource: scanIssueCount
          ? "scan"
          : parsed.detectedIssues?.length
            ? "ollama-suggested"
            : "none",
        aiIssuesSupplementary: Boolean(
          scanIssueCount && parsed.detectedIssues?.length,
        ),
        timingBuckets,
      },
    },
...
async function analyzeWithModel(baseDir, modelId, options = {}) {
  ...
  let report = {
    ...
    generatedBy:
      baseline?.generatedBy || "mock-data-scanner (repository-audit)",
    ...
    privacyAndSecurity:
      baseline?.privacyAndSecurity || template.privacyAndSecurity,
    performanceMetrics: {
      analysisDuration: `${((Date.now() - started) / constants.MS_PER_SECOND).toFixed(1)} seconds`,
      filesProcessedPerSecond: Math.max(
        1,
        Math.round(
          scan.totalFiles /
            Math.max((Date.now() - started) / constants.MS_PER_SECOND, 0.5),
        ),
      ),
  ...
  if (model.provider === "demo") {
    ...
    report.rejectedFiction = {
      warning:
        "Prior demo scans used legacy oracle branding and unverified confidence scores — not model output",
      claims: [
        "Legacy demo oracle active model",
        "98.5% aiConfidence (rejected fiction baseline)",
        "545/545 or 558/558 jest baseline false positives",
      ],
    };
    ...
    try {
      ...
      report.inferenceMeta.mode = ollamaUsed
        ? "ollama+json"
        : "filesystem-fallback";
      if (!ollamaUsed) {
        report.inferenceMeta.ollamaError =
          "Could not parse Ollama JSON response";
      }
    } catch (error) {
    ...
    if (llamaResult && !llamaResult.rawText) {
      ...
      report = {
        ...
        ggufAIInsights: {
          ...
          aiSuggestedIssues: llamaResult.detectedIssues?.length
            ? llamaResult.detectedIssues
            : undefined,
        },
        inferenceMeta: {
          ...report.inferenceMeta,
          issueSource: scanIssues.length
            ? "scan"
            : llamaResult.detectedIssues?.length
              ? "llama-suggested"
              : "none",
          aiIssuesSupplementary: Boolean(
            scanIssues.length && llamaResult.detectedIssues?.length,
          ),
        },
      };
      ...
      report.inferenceMeta.llamaCppAvailable = Boolean(
