...
const SAMPLE_PATH =
  process.env.MOCK_ANALYSIS_SAMPLE_PATH || path.join('data', 'templates', 'analysis-template.json');
...
async function enhanceWithOllama(registry, model, scan, baseReport) {
  ...
  const generated = await ollamaGenerate(registry.ollamaBaseUrl, model.ollamaModel, prompt, {
    format: 'json',
    includeMeta: true,
  });
...
async function analyzeWithModel(baseDir, modelId, options = {}) {
  ...
  let report = {
    ...
    performanceMetrics: {
      ...
      filesProcessedPerSecond: Math.max(
        1,
        Math.round(
          scan.totalFiles / Math.max((Date.now() - started) / constants.MS_PER_SECOND, 0.5)
        )
      ),
  ...
  if (model.provider === 'demo') {
    ...
    const llamaResult = await tryLlamaCppInference(model.path, buildAnalysisPrompt(scan, model));
