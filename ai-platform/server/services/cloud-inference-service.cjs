...
function checkCircuitBreaker(providerId) {
  ...
  if (state.isOpen) {
    ...
    if (timeSinceFailure > CIRCUIT_BREAKER_TIMEOUT) {
      ...
      logger.info(
        `[Circuit Breaker] ${providerId} circuit half-open — allowing probe request`,
      );
      return false;
    }
...
function recordFailure(providerId) {
  ...
  if (state.failures >= CIRCUIT_BREAKER_THRESHOLD) {
    state.isOpen = true;
    logger.warn(
      `[Circuit Breaker] ${providerId} circuit opened after ${state.failures} failures`,
    );
  }
}
...
async function fetchWithTimeout(
  url,
  options = {},
  timeoutMs = constants.TIMEOUT_30S,
) {
...
async function retryWithBackoff(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warn(
        `[Retry] Attempt ${attempt + 1}/${maxRetries + 1} failed, retrying in ${delayMs}ms: ${error.message}`,
      );
 ...
 * @returns {string|null}
 */
function resolveOllamaModelSync(
  registry = null,
  ...
  return (
    ...
    registry?.models?.find(
      (m) => m.id === registry.activeModelId && m.provider === "ollama",
    )?.ollamaModel ||
    registry?.models?.find((m) => m.provider === "ollama" && m.ollamaModel)
      ?.ollamaModel ||
    null
  );
}
...
async function resolveOllamaModel(
  ...
  if (!available.length) {
    throw new Error(
      "ollama has no models installed — run `ollama pull llama3.2` or set Ollama model in Settings → AI providers",
    );
  }
...
function listAvailableProviders(registry = null, userCredentials = null) {
  const activeModel = registry?.models?.find(
    (m) => m.id === registry.activeModelId,
  );
  const _ollamaRegistry = registry?.models?.some(
    (m) => m.provider === "ollama",
  );
  ...
  const entries = [
    ...
    },
    {
      ...
      available: Boolean(
        resolveCredential(userCredentials, "openai", "OPENAI_API_KEY"),
      ),
    ...
    },
    {
      ...
      available: Boolean(
        resolveCredential(userCredentials, "anthropic", "ANTHROPIC_API_KEY"),
      ),
  ...
  return entries.map((entry) => ({
    ...entry,
    configured:
      entry.id === "ollama" ? Boolean(entry.available) : entry.available,
  }));
}
...
function providerConfigured(
  ...
  const match = listAvailableProviders(registry, userCredentials).find(
    (p) => p.id === providerId,
  );
 ...
 */
async function summarizeScanWithProvider(
  providerId,
  scanPayload,
  options = {},
) {
  ...
  if (
    !providerConfigured(providerId, options.registry || null, userCredentials)
  ) {
    throw new Error(
      `${providerId} is not configured — ${providerConfigHint(providerId)}`,
    );
  }
...
const CLOUD_SUMMARY_SYSTEM_PROMPT =
  "You write concise technical audit summaries for engineering teams. Use plain bullet points only. Do not roleplay, greet the reader, invent metrics, or mention topics outside the user message. Never include full absolute file paths — use the project folder label provided.";
...
function buildScanPrompt(
  ...
  const gateLabel =
    gatePass === true ? "PASS" : gatePass === false ? "FAIL" : "—";
  ...
  if (type === "file-merger-reduction-report") {
    ...
    const paths =
      (scanPayload.scanPaths || [])
        .map((p) => redactPathForSummary(p))
        .join(", ") || "—";
    const repoFiles =
      merger.repositoryFilesTotal ??
      scanPayload.repositoryInventory?.totalFiles ??
      "—";
    const repoFolders =
      merger.repositoryFoldersTotal ??
      scanPayload.repositoryInventory?.totalFolders ??
      "—";
...
Important: if duplicate groups is 0, say "0 duplicate groups" — never claim the JSON scan count equals duplicates. Lead with repository file count — sample JSON path count is a subset used for structure similarity.`;
  ...
  if (type === "codebase-analyzer-report") {
    ...
    const categories = (scanPayload.detectedIssues || []).reduce(
      (acc, item) => {
...
Important: this scan flags technical debt markers, broken JSON/syntax, debug artifacts, and placeholder text — not full dead-code elimination.`;
  ...
  if (type === "data-cleanup-report") {
    ...
    const reclaimableLabel =
      reclaimable >= constants.BYTES_PER_MB
...
function sanitizeSummaryText(text, providerId = "") {
  ...
  if (providerId !== "ollama") {
    ...
    out = out
      .replace(/^(?:i shall (?:decipher|decode)[^.\n]*[.\n]+)/i, "")
      .trim();
  }
  out = out.replace(/[A-Za-z]:[\\/][^\s,)]+/g, (match) =>
    redactPathForSummary(match),
  );
  out = out.replace(
    /(?:\(|\s)([A-Za-z]:[\\/][^\s,)]+)/g,
    (_, path) => ` (${redactPathForSummary(path)})`,
  );
  return out;
}
...
async function callProvider(providerId, prompt, options = {}) {
  ...
  try {
    ...
    if (cacheResult) {
      logger.info(
        "[SemanticCache] Hit for provider=" +
          providerId +
          " (similarity=" +
          (cacheResult.similarity || 1.0).toFixed(4) +
          ", saved=" +
          cacheResult.savedLatencyMs +
          "ms)",
      );
      return {
    ...
    logger.warn(
      "[SemanticCache] Lookup failed, proceeding to provider:",
      cacheErr.message,
    );
  }
  ...
  if (checkCircuitBreaker(providerId)) {
    throw new Error(
      `503: The circuit breaker for provider '${providerId}' is currently open due to high error frequencies. Try again later.`,
    );
  }
  ...
  try {
    ...
    if (hardStopResult.blocked) {
      throw new Error(
        `403: Budget hard stop exceeded for org=${orgId} (${hardStopResult.pct.toFixed(1)}% of $${hardStopResult.limitUSD})`,
      );
    }
  } catch (budgetErr) {
    if (
      budgetErr &&
      budgetErr.message &&
      budgetErr.message.indexOf("Budget hard stop") !== -1
    )
      throw budgetErr;
  ...
  try {
    ...
    try {
      ...
      if (result && result.usage) {
        tokenCount =
          result.usage.total_tokens || result.usage.completion_tokens || 0;
      }
      semanticCache.store(
        providerId,
        options.model,
    ...
    try {
      var inputTokens =
        (result && result.usage && result.usage.prompt_tokens) || 0;
      var outputTokens =
        (result && result.usage && result.usage.completion_tokens) ||
        tokenCount ||
        0;
    ...
    try {
      ...
      if (budgetResult.blocked) {
        logger.warn(
          "[TokenBudget] Hard stop exceeded for org=" +
            (orgId || "default") +
            ": " +
...
async function callOpenAI(prompt, options = {}) {
  ...
  const requestId =
    options.requestId ||
    `sb-${startedAt}-${Math.random().toString(36).slice(2, 8)}`;
...
async function callOllama(prompt, options = {}) {
  ...
  if (!model) {
    throw new Error(
      "Ollama is not configured — start Ollama with `ollama serve`, then pull a model (e.g. `ollama pull llama3.2`)",
    );
  }
  const messages = normalizeMessages(prompt, options);
...
const CODE_UNDERSTANDING_SYSTEM =
  "You explain code purpose for engineering audits. Use 4-6 bullet points. Ground every claim in the provided facts. Do not invent metrics, file paths, or business requirements not present in the input.";
