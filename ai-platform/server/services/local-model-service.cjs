    ...
    models: [
      {
        ...
        status: "active",
        description:
          "Filesystem mock-data scan — not GGUF inference. Upload phi-2 or set Ollama for live models.",
        path: null,
        ollamaModel: null,
        size: null,
        confidence: null,
...
async function ensureRegistry(baseDir) {
  ...
  if (!fs.existsSync(registryPath)) {
    // simplebeacon-ignore sync-io — existence check before async write
 ...
 * @returns {any}
 */
async function findUploadFileByHash(
  uploadsDir,
  fileHash,
  fileSize,
  { excludePath } = {},
...
function getActiveModel(registry) {
  ...
  return (
    active ||
    registry.models.find((m) => m.isDefault) ||
    registry.models[0] ||
    null
  );
}
...
async function registerUploadedModel(baseDir, file, meta = {}) {
  const registry = await ensureRegistry(baseDir);
  const name = String(
    meta.name || path.parse(file.originalname || file.filename).name,
  ).trim();
  ...
  const duplicatePath = await findUploadFileByHash(
    uploadsDir,
    fileHash,
    stat.size,
    {
  ...
  if (duplicatePath) {
    ...
    const existingByPath = registry.models.find(
      (m) =>
        m.path && normalizePathKey(m.path) === normalizePathKey(storedPath),
    );
    if (existingByPath) {
...
async function registerOrphanedUpload(baseDir, filename) {
  ...
  const result = await registerUploadedModel(
    baseDir,
    {
      path: storedPath,
...
async function removeModel(baseDir, modelId) {
  ...
  if (registry.activeModelId === modelId) {
    const fallback =
      registry.models.find((m) => m.isDefault) || registry.models[0];
...
async function testOllamaConnection(baseUrl, modelName) {
  ...
  try {
    ...
    if (
      modelName &&
      !names.some((n) => n === modelName || n.startsWith(`${modelName}:`))
    ) {
      return {
        ok: false,
...
async function testModel(baseDir, modelId) {
  ...
  if (model.provider === "demo") {
    result = {
      ok: true,
      message: "Demo model — metadata only (no inference backend)",
    };
    ...
    result = {
      ok: false,
      message: "Model has no path or Ollama configuration",
    };
  }
