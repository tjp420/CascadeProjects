...
process.env.ROADMAP_MEMORY_LIMIT_MB =
  process.env.ROADMAP_MEMORY_LIMIT_MB || "1024";
...
} = require("../routes/lib/flexible-analyze-roadmap.cjs");
...
describe("server/routes/lib/flexible-analyze-roadmap", () => {
  ...
  test("buildRoadmapFromPath throws for nonexistent path", async () => {
    await expect(buildRoadmapFromPath("/nonexistent/path/xyz")).rejects.toThrow(
      "does not exist",
    );
  });
  ...
  test("buildRoadmapFromPath throws for file path (not directory)", async () => {
    ...
    try {
      await expect(buildRoadmapFromPath(tmpFile)).rejects.toThrow(
