...
class UploadSecurityMiddleware {
    constructor(options = {}) {
        this.options = {
            ...
            allowedTypes: options.allowedTypes || ['js', 'ts', 'jsx', 'tsx', 'py', 'html', 'json', 'md', 'txt'],
            ...
            suspiciousPatterns: options.suspiciousPatterns || this.getDefaultSuspiciousPatterns(),
            ...options
        };
    ...
    checkSuspiciousContent(content) {
        ...
        for (const [name, pattern] of Object.entries(this.options.suspiciousPatterns)) {
    ...
    detectContentType(content) {
        ...
        if (content.includes('#') || content.includes('import ') || content.includes('function ')) {
            return 'code';
        }
    ...
    isValidFilename(filename) {
        ...
        const invalidChars = /[<>:"/\\|?*]/;
        const hasControlChars = [...String(filename || '')].some((ch) => ch.charCodeAt(0) < 32);
    ...
    async removeFile(filePath) {
        try {
            ...
            logger.warn(`[Upload Security] Failed to remove file ${filePath}:`, error.message);
        }
    }
    ...
    logUpload(req, success) {
        ...
        logger.debug(`[Upload Security] ${success ? 'SUCCESS' : 'BLOCKED'}:`, logEntry);
    ...
    getStats() {
        return {
            ...
            successRate: this.uploadStats.totalUploads > 0 
                ? ((this.uploadStats.totalUploads - this.uploadStats.blockedUploads) / this.uploadStats.totalUploads * 100).toFixed(2)
                : 0,
            timestamp: new Date().toISOString()
        };
    }
