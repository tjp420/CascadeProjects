...
const auditConfig = {
  ...
  complianceFrameworks: (
    process.env.AUDIT_FRAMEWORKS || "SOC2,ISO27001,GDPR"
  ).split(","),
...
const cleanupOldLogs = async () => {
  try {
    ...
    const cutoffDate = new Date(
      Date.now() -
        auditConfig.retentionDays * 24 * 60 * constants.ONE_MINUTE_MS,
    );
...
const auditLog = (level, eventType, message, metadata = {}) => {
  ...
  if (
    auditConfig.enableConsole &&
    logLevels[level] >= logLevels[auditConfig.logLevel]
  ) {
...
const auditAIOperation = createAuditMiddleware(
...
const auditDataAccess = createAuditMiddleware(
...
const auditSecurity = createAuditMiddleware(
  eventTypes.SECURITY,
  (req, phase) => {
...
const logComplianceEvent = (complianceType, details, user = null) => {
  auditLog(
    ...
    `Compliance event: ${complianceType}`,
    {
      userId: user?.id,
      trustLevel: user?.trustLevel,
      complianceType,
  ...
  try {
    ...
    if (filters.level) {
      entries = entries.filter(
        (entry) => entry.level === filters.level.toUpperCase(),
      );
    }

    if (filters.eventType) {
      entries = entries.filter(
        (entry) => entry.eventType === filters.eventType,
      );
    }

    if (filters.userId) {
      entries = entries.filter(
        (entry) => entry.metadata.userId === filters.userId,
      );
    }
