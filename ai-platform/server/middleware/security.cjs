  ...
  const config = {
    ...
    message: {
      ...
      retryAfter: Math.ceil(
        securityConfig.rateLimitWindowMs / constants.MS_PER_SECOND,
      ),
    },
    ...
    skipSuccessfulRequests:
      ...
      securityConfig.rateLimitSkipSuccessfulRequests,
    skipFailedRequests:
      options.skipFailedRequests || securityConfig.rateLimitSkipFailedRequests,
  };
...
const buildConnectSrc = () => {
  ...
  const dashUrl =
    process.env.OPERATOR_DASHBOARD_BASE_URL || process.env.DASHBOARD_BASE_URL;
  ...
  base.push(
    ...
    "http://localhost:*",
    "https://localhost:*",
  ); // simplebeacon-ignore hardcoded-url — Render/local origins for dashboard API and bridge probes
  if (!isProd) {
    ...
    ); // simplebeacon-ignore hardcoded-url — dev-only CSP connect-src
  }
  return base;
};
...
const requestLogger = (req, res, next) => {
  ...
  // Log request details
  logger.debug(
    `[${new Date().toISOString()}] ${req.method} ${req.originalUrl} - Request ID: ${requestId} - IP: ${req.ip}`,
  );
  ...
  res.end = function (chunk, encoding) {
    ...
    logger.debug(
      `[${new Date().toISOString()}] ${req.method} ${req.originalUrl} - Request ID: ${requestId} - Status: ${res.statusCode} - Duration: ${duration}ms`,
    );

    // Log security events
    if (res.statusCode >= 400) {
      logger.warn(
        `[SECURITY] Request ID: ${requestId} - Error response: ${res.statusCode} - IP: ${req.ip} - User-Agent: ${req.headers["user-agent"]}`,
      );
    }
...
function shouldSkipBodySuspiciousScan(req) {
  ...
  return BODY_SUSPICIOUS_SCAN_SKIP_PREFIXES.some((prefix) =>
...
const ipProtection = (req, res, next) => {
      ...
      if (typeof obj[key] === "string") {
        for (const pattern of suspiciousPatterns) {
          if (pattern.test(obj[key])) {
            logger.warn(
              `[SECURITY] Suspicious content detected - IP: ${clientIP} - Field: ${key} - Pattern: ${pattern}`,
            );
            return true;
          }
        }
  ...
  if (
    !shouldSkipBodySuspiciousScan(req) &&
    req.body &&
