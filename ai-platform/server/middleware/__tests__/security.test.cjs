...
describe('security middleware — vault validation schemas', () => {
  ...
  describe('validationSchemas', () => {
    it('exports vaultHandshake schema', () => {
      assert.ok(validationSchemas.vaultHandshake, 'vaultHandshake schema should exist');
    });

    it('exports vaultDecrypt schema', () => {
      assert.ok(validationSchemas.vaultDecrypt, 'vaultDecrypt schema should exist');
    });
  ...
  describe('vaultHandshake schema', () => {
    ...
    it('rejects non-string provider', () => {
      const { error } = validationSchemas.vaultHandshake.validate({ provider: 123 });
  ...
  describe('vaultDecrypt schema', () => {
    ...
    it('rejects missing ciphertext (required field)', () => {
      ...
      assert.ok(error.details[0].message.includes('required'), `Expected "required" in: ${error.details[0].message}`);
    });
    ...
    it('rejects non-string ciphertext', () => {
      const { error } = validationSchemas.vaultDecrypt.validate({ ciphertext: 42 });
  ...
  describe('vaultRekey schema', () => {
    ...
    it('rejects non-string newKeyId', () => {
      const { error } = validationSchemas.vaultRekey.validate({ newKeyId: false });
  ...
  describe('validateInput middleware', () => {
    ...
    it('calls next() for valid vaultHandshake input', () => {
      ...
      const req = mockReq({ provider: 'mockhsm', keyId: 'k1', region: 'us-east-1' });
      const res = mockRes();
      let called = false;
    ...
    it('strips unknown fields from vaultHandshake input', () => {
      ...
      const res = mockRes();
      middleware(req, res, () => {});
      assert.strictEqual(req.body.extraField, undefined, 'unknown fields should be stripped');
