...
class EnterpriseDLP {
  constructor(config = {}) {
    ...
    this.config = {
      proxyPort:
        Number(config.proxyPort || env.PROXY_PORT) || constants.AI_PROXY_PORT,
      dashboardPort: Number(config.dashboardPort || env.DASHBOARD_PORT) || 3000,
      violationLogPath:
        config.violationLogPath ||
        env.VIOLATION_LOG_PATH ||
        "./ai-violations.log",
      blockOnMatch:
        config.blockOnMatch !== false && env.BLOCK_ON_MATCH !== "false",
  ...
  start() {
    ...
    logger.info(
      `Block mode: ${this.config.blockOnMatch ? "ENABLED" : "MONITOR ONLY"}`,
    );
