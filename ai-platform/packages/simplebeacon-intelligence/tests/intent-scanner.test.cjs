...
test("scanIntent detects hollow Python function (SB-INTENT-001)", () => {
  ...
  assert.ok(
    result.findings.some((f) => f.id === INTENT_RULE_IDS.HOLLOW_FUNCTION),
  );
});
...
test("scanIntent detects credential mock stub (SB-INTENT-002)", () => {
  ...
  assert.ok(
    result.findings.some((f) => f.id === INTENT_RULE_IDS.CREDENTIAL_STUB),
  );
...
def flaky_handler():
    ...
    return {"ok": True}
`;
  const findings = scanStructuralIntent(code, {
    filePath: "handler.py",
    language: "python",
  });
...
test("fingerprint matching adds SB-INTENT-003 when profile aligns", () => {
  ...
  const fp = result.findings.filter(
    (f) => f.id === INTENT_RULE_IDS.FINGERPRINT_MATCH,
  );
  assert.ok(
    fp.length >= 1,
    "expected at least one fingerprint match for hollow Python stub",
  );
});
