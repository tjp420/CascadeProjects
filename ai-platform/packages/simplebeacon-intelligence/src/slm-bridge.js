...
function canRunSlm(options = {}) {
  ...
  const modelPath =
    options.modelPath ||
    options.slm?.modelPath ||
    process.env.SIMPLEBEACON_SLM_MODEL;
...
function runSlmReview(content, options = {}) {
  ...
  try {
    ...
    const validation = parsed
      ? validateSlmResultDetailed(parsed)
      : { valid: false };
    ...
    return {
      ...
      reason: validation.valid
        ? validation.reason
        : parsed?.reason || stdout.slice(0, 500),
    ...
    if (options.signal) {
      const onAbort = () => {
        ...
        finish({
          enabled: true,
          reviewed: false,
          error: "SLM review aborted by signal",
        });
      };
