...
This document categorizes production-leak findings for the **cascade** profile (`.samplebeacon/config.json`).

**Current gate status (2026-05-24):** `productionLeakFindings: 0` with cascade config — **Gate PASS**.

The historical **52 medium findings** in `.samplebeacon/test-report.json` are documented below for outreach demos and allowlist decisions.
...
3. **Re-run after server changes:** `npm run samplebeacon:report`  
4. **Use fresh report** — ignore stale `.samplebeacon/test-report.json` (52 leaks)
