...
const FILE_IO_LINE_REGEX = /readFileSync\s*\(|fs\.readFile\s*\(|fs\.promises\.readFile\s*\(|createReadStream\s*\(/;
const JSON_STRINGIFY_LINE_REGEX = /JSON\.stringify\s*\(/;
const CHUNKING_HELPER_REGEX = /RecursiveCharacterTextSplitter|textSplitter|\.chunk\s*\(|splitDocuments|CharacterTextSplitter/;
const MESSAGES_CONTEXT_LINE_REGEX = /\bmessages\s*:|content\s*:|role\s*:\s*['"]user['"]/i;
...
const RULE_CATALOG = [
    {
        ...
        type: 'Token Bleed',
        severity: 'medium',
        description: 'File read (sync/stream) within 10 lines of an LLM API call — risk of unchunked context'
    },
    {
        id: 'SB-TB-002',
        category: 'token-bleed',
        type: 'Token Bleed',
        severity: 'medium',
        description: 'JSON.stringify near LLM API call — may serialize a large object into the prompt'
    },
    {
        id: 'SB-TB-003',
        category: 'token-bleed',
        type: 'Token Bleed',
        severity: 'medium',
        description: 'Very long string literal (>2000 chars) in messages/content — likely uncapped prompt payload'
    },
    {
        id: 'SB-TB-004',
        category: 'token-bleed',
        type: 'Token Bleed',
        severity: 'medium',
        description: 'File I/O without chunking/splitting helpers — read whole files before LLM calls'
    },
    {
        id: 'SB-TB-005',
        category: 'token-bleed',
        type: 'Token Bleed',
        severity: 'medium',
        description: 'LLM API call without max_tokens, max_completion_tokens, or maxOutputTokens limit'
    }
];
...
function scanProximityBleed(relativePath, content, ext, options = {}) {
    ...
    if (options.productionPathsOnly && !isUnderProductionPaths(relativePath, options.productionPaths)) {
        return findings;
    }
...
function scanLongMessageLiterals(relativePath, content, ext, options = {}) {
    ...
    if (!MESSAGES_CONTEXT_LINE_REGEX.test(content) && !API_LINE_REGEX.test(content)) {
        return findings;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!MESSAGES_CONTEXT_LINE_REGEX.test(line) && !API_LINE_REGEX.test(line)) continue;
...
function scanMissingChunking(relativePath, content, ext, options = {}) {
    ...
    if (!FILE_IO_LINE_REGEX.test(content) || !API_LINE_REGEX.test(content)) return findings;
