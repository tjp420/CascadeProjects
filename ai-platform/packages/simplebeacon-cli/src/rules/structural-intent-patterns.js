...
const SCANNABLE_EXTENSIONS = new Set(['.js', '.mjs', '.cjs', '.ts', '.tsx', '.jsx', '.py', '.pyw', '.go']);
const SKIP_DIRS = new Set([
    'node_modules', '.git', 'coverage', 'dist', 'build', 'archive',
    '.simplebeacon', 'tests', 'test', '__tests__', 'fixtures', 'docs', 'deliverables',
    'github-cache', '.github-sync'
]);
const SCANNER_IMPL_RE = /(?:^|\/)packages\/simplebeacon-(?:cli|intelligence)\//i;
const MAX_SCAN_BYTES = 512000;

const RECOMMENDED_ACTIONS = {
    'SB-INTENT-001': 'Replace AI boilerplate with real logic, error routing, and domain-specific naming',
    'SB-INTENT-002': 'Load secrets from vault/env; never commit placeholder credentials',
    'SB-INTENT-003': 'Review for unchecked AI generation; consider intelligence.slm for local verification',
...
function toScanIssues(findings, relativePath) {
    return (findings || []).map((finding, index) => {
        ...
        return {
            ...
            recommendedAction: RECOMMENDED_ACTIONS[pattern] || finding.recommendedAction || 'Review and remediate',
...
async function scanStructuralIntentPatterns(baseDir, options = {}) {
    const intelligence = options.intelligence || getIntelligenceOptions({ intelligence: options.intelligenceConfig });
    if (!intelligence?.enabled) {
        return { scanned: 0, findings: 0, issues: [], available: isIntelligenceAvailable(), enabled: false };
    }
    ...
    const sourcePaths = intelligence.sourcePaths || options.sourcePaths || options.productionPaths || DEFAULT_SOURCE_PATHS;
    ...
    const slmEnabled = intelligence.slm?.enabled === true || process.env.SIMPLEBEACON_SLM_ENABLED === '1';
    ...
    for (const file of files) {
        ...
        const needsSlm = slmEnabled && fileIssues.some((i) => i.severity === 'high' || i.severity === 'critical');
        if (needsSlm) {
            ...
            if (slm.reviewed) {
                ...
                slmReviews.push({ file: file.relativePath, risk: 'skipped', reason: slm.note || slm.error });
            }
        }
