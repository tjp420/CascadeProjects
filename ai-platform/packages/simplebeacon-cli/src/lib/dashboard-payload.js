...
function overlayAuditPageSamples(samples = {}, report = {}, baseline = {}) {
    const qualityMetrics = samples.qualityMetrics || {};
    const baselineComparison = samples.baselineComparison || {};
    const qualityScore = report.gate?.pass && report.schemaCompliance != null
        ? report.schemaCompliance
        : (report.qualityScore ?? qualityMetrics.currentScore ?? qualityMetrics.overview?.qualityScore ?? 0);
    const pageSamplesLabel = report.pageSampleSchemaChecked != null
        ? `${report.pageSampleSchemaPassed ?? 0}/${report.pageSampleSchemaChecked}`
        : (baseline.pageSamplesLabel ?? baselineComparison.overview?.pageSamplesLabel ?? null);

    return {
        ...samples,
        qualityMetrics: {
            ...qualityMetrics,
            generatedAt: report.generatedAt || qualityMetrics.generatedAt,
            currentScore: qualityScore,
            overview: {
                ...(qualityMetrics.overview || {}),
                qualityScore,
                schemaCompliance: report.schemaCompliance ?? qualityMetrics.overview?.schemaCompliance ?? null,
                consistencyScore: report.consistencyScore ?? qualityMetrics.overview?.consistencyScore ?? null
            },
            metrics: (qualityMetrics.metrics || []).map((metric) => {
                if (metric.name === 'quality_score') {
                    return {
                        ...metric,
                        value: qualityScore,
                        context: {
                            ...(metric.context || {}),
                            issueCount: (report.gate?.blockingCount ?? 0) + (report.gate?.warningCount ?? 0),
                            gatePass: report.gate?.pass ?? metric.context?.gatePass ?? false
                        }
                    };
                }
                if (metric.name === 'schema_compliance') {
                    return {
                        ...metric,
                        value: report.schemaCompliance ?? metric.value,
                        context: {
                            ...(metric.context || {}),
                            checked: report.schemaChecked ?? metric.context?.checked,
                            passed: report.schemaPassed ?? metric.context?.passed
                        }
                    };
                }
                if (metric.name === 'consistency_score') {
                    return {
                        ...metric,
                        value: report.consistencyScore ?? metric.value,
                        context: {
                            ...(metric.context || {}),
                            samplesScanned: report.consistencyChecked ?? metric.context?.samplesScanned,
                            fictionHits: Math.max(0, (report.consistencyChecked ?? 0) - (report.consistencyPassed ?? 0))
                        }
                    };
                }
                if (metric.name === 'credential_findings') {
                    return {
                        ...metric,
                        value: report.credentialFindings ?? metric.value,
                        context: { ...(metric.context || {}), scanned: report.credentialScanned ?? metric.context?.scanned }
                    };
                }
                if (metric.name === 'production_leak_findings') {
                    return {
                        ...metric,
                        value: report.productionLeakFindings ?? metric.value,
                        context: { ...(metric.context || {}), scanned: report.productionLeakScanned ?? metric.context?.scanned }
                    };
                }
                return metric;
            })
        ...
        baselineComparison: {
            ...baselineComparison,
            generatedAt: report.generatedAt || baselineComparison.generatedAt,
            currentValue: report.consistencyScore ?? report.schemaCompliance ?? baselineComparison.currentValue,
            overview: {
                ...(baselineComparison.overview || {}),
                compliance: `${report.consistencyScore ?? report.schemaCompliance ?? 100}%`,
                jestTestsLabel: baseline.jestTestsLabel ?? baselineComparison.overview?.jestTestsLabel ?? null,
                pageSamplesLabel,
                gatePass: report.gate?.pass ?? baselineComparison.overview?.gatePass ?? false
            },
            comparisons: (baselineComparison.comparisons || []).map((row) => {
                if (row.metric === 'schema_compliance') {
                    return {
                        ...row,
                        currentValue: report.schemaCompliance ?? row.currentValue,
                        variance: Math.max(0, (row.baselineValue ?? 100) - (report.schemaCompliance ?? row.currentValue ?? 0))
                    };
                }
                if (row.metric === 'consistency_score') {
                    return {
                        ...row,
                        currentValue: report.consistencyScore ?? row.currentValue,
                        variance: Math.max(0, (row.baselineValue ?? 100) - (report.consistencyScore ?? row.currentValue ?? 0))
                    };
                }
                if (row.metric === 'page_samples') {
                    const current = report.pageSampleSchemaPassed ?? row.currentValue;
                    return {
                        ...row,
                        currentValue: current,
                        variance: Math.max(0, (row.baselineValue ?? current) - (current ?? 0))
                    };
                }
                return row;
            })
...
function buildDashboardPayload({ report, baseline, history, fictionCatalog }) {
    ...
    return {
        ...
        scanStatus: {
            lastScan: report.generatedAt || historyEntry.date || null,
            lastScanRelative: formatRelativeTime(report.generatedAt || historyEntry.date),
            ...
            qualityScore: report.qualityScore ?? historyEntry.qualityScore ?? 0,
            ...
            gatePass: report.gate?.pass ?? historyEntry.gatePass ?? false,
            issueCount: report.gate
                ? (report.gate.blockingCount ?? 0) + (report.gate.warningCount ?? 0)
                : (report.issueCount ?? historyEntry.issueCount ?? 0),
            warningCount: report.gate?.warningCount ?? historyEntry.warningCount ?? 0,
            blockingCount: report.gate?.blockingCount ?? historyEntry.blockingCount ?? 0,
            totalFilesScanned: resolveFilesAnalyzed(report) || historyEntry.totalFilesScanned || 0
        },
        trends: {
...
function buildScanResults(report, historyEntry = {}, baseline = {}) {
    ...
    return {
        ...
        results: {
            ...
            issueCount: historyEntry.issueCount
                ?? ((report.gate?.blockingCount ?? 0) + (report.gate?.warningCount ?? 0)),
            warningCount: historyEntry.warningCount ?? report.gate?.warningCount ?? 0,
            blockingCount: historyEntry.blockingCount ?? report.gate?.blockingCount ?? 0,
...
function resolveJestBaselineStatus(report = {}, baseline = {}) {
    if (!baseline.jestTestsLabel && baseline.jestTestsPassing == null) {
        return { status: 'skipped', label: null, summary: null };
    }
    const summary = report.jestSummary;
    const expected = baseline.jestTestsPassing;
    const label = baseline.jestTestsLabel ?? (expected != null ? `${expected}/${expected}` : null);
    if (!summary) {
        return {
            status: report.jestBaselinePassed === false ? 'fail' : 'warn',
            label,
            summary: null
        };
    }
    const matches = expected == null || summary.testsPassed === expected;
    const suitesMatch = baseline.jestSuites == null
        || summary.suitesPassed == null
        || summary.suitesPassed === baseline.jestSuites;
    const status = summary.testsFailed > 0 || !matches || !suitesMatch
        ? 'fail'
        : 'pass';
...
function buildAuditLayers(report = {}, baseline = {}) {
    ...
    return {
        ...
        jestBaseline: {
            enabled: Boolean(baseline.jestTestsLabel || baseline.jestTestsPassing != null),
            label: baseline.jestTestsLabel ?? null,
            suites: baseline.jestSuites ?? null,
            summary: report.jestSummary ?? null,
            severity: 'high',
...
function buildAuditPayload(context, extras = {}) {
    ...
    return {
        ...
        pageSamples: overlayAuditPageSamples(extras.pageSamples || {}, report, baseline),
