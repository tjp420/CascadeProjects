...
const { getSubscriptionByEmail } = require('../../server/lib/simplebeacon-subscription-store.cjs');
...
async function resolveAdminLicense(req) {
  ...
  try {
    ...
    logger.warn('[LicenseSeats] Failed to verify admin license token:', err.message);
    return null;
  }
}

function resolveLicenseSecret() {
  const secret = String(process.env.SIMPLEBEACON_LICENSE_SECRET || process.env.SIMPLEBEACON_SIGNING_PRIVATE_KEY || '').trim();
...
router.get('/seats', authorize('admin:all'), async (req, res) => {
  try {
    ...
    if (!admin) {
      return res.status(403).json({ error: 'no_license', message: 'No active license found for this account' });
    }
    ...
    if (!seatData) {
      ...
      seatData = await initSeatStore(env, admin.licenseKey, admin.tier, admin.email);
    }

    const seatsUsed = seatData.seats.filter(s => s.status === 'active').length;
    ...
    res.json({
      ...
      seatsUsed,
      seatsRemaining: seatData.maxSeats === Infinity ? Infinity : seatData.maxSeats - seatsUsed,
      tier: seatData.tier,
...
router.post('/seats/invite', authorize('admin:all'), async (req, res) => {
  try {
    ...
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return res.status(400).json({ error: 'valid_email_required', message: 'A valid email is required' });
    }
    ...
    const existing = seatData.seats.find(s => s.email.toLowerCase() === email.toLowerCase());
    if (existing) {
      return res.status(409).json({
    ...
    logger.info(`[LicenseSeats] Invite created: ${email} for ${admin.email}'s license`);
    ...
    res.status(201).json({
      ...
      seatsRemaining: seatData.maxSeats === Infinity ? Infinity : seatData.maxSeats - seatsUsed - 1
    });
  } catch (err) {
...
router.delete('/seats/revoke/:seatId', authorize('admin:all'), async (req, res) => {
  try {
    const { seatId } = req.params;
    if (!seatId || !/^seat_[a-f0-9]{12}$/.test(seatId)) {
      return res.status(400).json({ error: 'invalid_seat_id', message: 'Invalid seat ID format' });
    }
    ...
    if (!seatData) {
      return res.status(404).json({ error: 'seat_not_found', message: 'No seat data found for this license' });
    }

    const seatIdx = seatData.seats.findIndex(s => s.seatId === seatId);
    if (seatIdx === -1) {
      return res.status(404).json({ error: 'seat_not_found', message: `Seat ${seatId} not found in this license` });
    }

    // Prevent revoking your own admin seat
    if (seatData.seats[seatIdx].email === admin.email) {
      return res.status(400).json({ error: 'cannot_revoke_self', message: 'You cannot revoke your own admin seat' });
    }
    ...
    try {
      const { setSubscriptionActive } = require('../../server/lib/simplebeacon-subscription-store.cjs');
      await setSubscriptionActive(revokedSeat.email, false, { certOrgId: null });
    } catch (err) {
      logger.warn(`[LicenseSeats] Failed to deactivate subscription for ${revokedSeat.email}:`, err.message);
    }

    const seatsUsed = seatData.seats.filter(s => s.status === 'active').length;

    logger.info(`[LicenseSeats] Seat revoked: ${revokedSeat.email} (${seatId}) by ${admin.email}`);
