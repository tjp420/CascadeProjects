...
const {
  executeSafeMerge,
  rollbackMerge,
} = require("../../server/lib/safe-merge-guard.cjs");
const {
  buildDevSecOpsCompliancePayload,
  buildComplianceHtml,
} = require("../../server/lib/devsecops-compliance-payload.cjs");
const {
...
function loadMergeCandidate(projectRoot, candidateId) {
  const report = readJsonIfExists(
    path.join(projectRoot, ".simplebeacon", "consolidation-report.json"),
  );
...
function setupOptimizationAPI(app, options = {}) {
  ...
  const defaultMonorepoRoot =
    options.monorepoRoot || path.join(defaultPlatformRoot, "..");
  ...
  app.get("/api/optimization/compliance", (req, res) => {
    try {
      ...
      if (wantsHtmlResponse(req)) {
        ...
        return res.send(
          `<!DOCTYPE html><html><body><h1>Compliance unavailable</h1><p>${error.message}</p></body></html>`,
        );
      }
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  app.get("/api/optimization/candidates", (req, res) => {
    try {
      ...
      res.json({
        ...
        candidates,
        exclusionsNote:
          (sanitized?.summary?.monorepoAliasPairsExcluded ?? 0) > 0
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const scope =
          String(
            req.body?.scope || req.query?.scope || "repository",
          ).toLowerCase() === "sample-data-only"
            ? "sample-data-only"
            : "repository";
  ...
  app.post(
    "/api/optimization/merge-preview",
    optimizationRateLimit,
    async (req, res) => {
      try {
        const projectRoot = resolveProjectPath(
  ...
  logger.debug(
    "✅ Repository optimization API at /api/optimization/* (health, compliance, merge-preview)",
  );
}
