...
} = require('./roadmap-analysis-history');
...
const { buildHistoryEntryFromRoadmap } = require('../../server/lib/roadmap-history-metrics');
...
function setupBuildFromPathRoute(app) {
    ...
    app.get('/api/code-roadmap/analyze', async (req, res) => {
        try {
            ...
            const roadmap = await generateCodeRoadmap(resolvedPath, {}, {
                includePaths: normalizeStringList(String(req.query.includePaths || '').split(',')),
                excludePatterns: normalizeStringList(String(req.query.excludePatterns || '').split(','))
            });
    ...
    app.post('/api/dynamic-roadmap/build-from-path', async (req, res) => {
        try {
            const { projectPath, title, description, includePaths, excludePatterns } = req.body || {};
