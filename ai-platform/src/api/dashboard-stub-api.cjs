...
async function loadDashboardHomeSample(webRoot) {
  try {
    ...
    if (!dashboardHomeRawSample || dashboardHomeRawSampleMtimeMs !== stat.mtimeMs) {
      const content = await fs.readFile(filePath, 'utf8');
...
async function loadQualityDashboardSample(webRoot) {
  ...
  try {
    ...
    const content = await fs.readFile(filePath, 'utf8');
    qualityDashboardSample = mergeIstanbulTelemetry(JSON.parse(content), path.join(webRoot, '..'));
  } catch {
...
function setupDashboardStubAPIs(app, webRoot, options = {}) {
  ...
  if (typeof authMiddleware === 'function') {
    ...
    dashboardRouter.use((req, res) =>
      res.status(500).json({ success: false, error: 'Authentication layer misconfiguration.' })
    );
  }
  ...
  dashboardRouter.get('/api/dashboard-home', async (_req, res) => {
    ...
    const data = {
      ...
      overview: {
        ...sample.overview,
        totalFiles:
          report?.repositoryFilesTotal ?? report?.totalFiles ?? sample.overview?.totalFiles,
  ...
  dashboardRouter.get('/api/merger-tool/reduction-scan', async (req, res) => {
    try {
      ...
      if (rawPath) {
        try {
          baseDir = assertSafeProjectPath(
            path.isAbsolute(rawPath) ? path.normalize(rawPath) : path.join(defaultDir, rawPath),
            allowedRoots
          );
        } catch (error) {
      ...
      const scope =
        String(req.query?.scope || 'repository').toLowerCase() === 'sample-data-only'
          ? 'sample-data-only'
          : 'repository';
