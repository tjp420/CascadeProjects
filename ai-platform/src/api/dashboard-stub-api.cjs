...
} = require("../../server/lib/file-merger-reduction-scanner.cjs");
...
} = require("../../server/lib/coverage-reports-builder.cjs");
const {
  buildAnalyticsModel,
} = require("../../server/lib/analytics-builder.cjs");
const {
  mergeIstanbulTelemetry,
} = require("../../server/lib/istanbul-telemetry-merge.cjs");
...
} = require("../../server/lib/security-dashboard-builder.cjs");
const {
  buildDashboardHomeModel,
} = require("../../server/lib/dashboard-home-builder.cjs");
...
const dashboardLimiter = require("express-rate-limit")({
  ...
  message: {
    success: false,
    error: "Too many requests from this IP. Access throttled.",
  },
});
...
async function loadDevToolsSample(webRoot) {
  ...
  try {
    const filePath = path.join(
      webRoot,
      "data",
      ["dev-tools", FIXTURE_SUFFIX].join(""),
    ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
...
async function loadMergerToolSample(webRoot) {
  ...
  try {
    const filePath = path.join(
      webRoot,
      "data",
      ["merger-tool", FIXTURE_SUFFIX].join(""),
    ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
    const content = await fs.readFile(filePath, "utf8");
...
async function loadCoverageReportsSample(webRoot) {
  if (!coverageReportsRawSample) {
    try {
      const filePath = path.join(
        webRoot,
        "data",
        ["coverage-reports", FIXTURE_SUFFIX].join(""),
      ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
      const content = await fs.readFile(filePath, "utf8");
  ...
  return buildCoverageReportsModel(
    path.join(webRoot, ".."),
    coverageReportsRawSample,
  );
}
...
async function loadAnalyticsSample(webRoot) {
  ...
  try {
    const filePath = path.join(
      webRoot,
      "data",
      ["analytics", FIXTURE_SUFFIX].join(""),
    ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
    const content = await fs.readFile(filePath, "utf8");
    analyticsSample = buildAnalyticsModel(
      path.join(webRoot, ".."),
      JSON.parse(content),
    );
  } catch {
...
async function loadDashboardHomeSample(webRoot) {
  try {
    const filePath = path.join(
      webRoot,
      "data",
      ["dashboard-home", FIXTURE_SUFFIX].join(""),
    ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
...
async function loadQualityDashboardSample(webRoot) {
  ...
  try {
    const filePath = path.join(
      webRoot,
      "data",
      ["quality-dashboard", FIXTURE_SUFFIX].join(""),
    ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
    const content = await fs.readFile(filePath, "utf8");
...
async function loadSecurityDashboardSample(webRoot) {
  if (!securityDashboardRawSample) {
    try {
      const filePath = path.join(
        webRoot,
        "data",
        ["security-dashboard", FIXTURE_SUFFIX].join(""),
      ); // simplebeacon:production-leak-intent: stub-data - Dashboard page sample loader
      const content = await fs.readFile(filePath, "utf8");
  ...
  return await buildSecurityDashboardModel(
    path.join(webRoot, ".."),
    securityDashboardRawSample,
  );
}
...
async function loadSupportDashboardSample(webRoot) {
  ...
  try {
    const filePath = path.join(
      webRoot,
      "data",
      ["support-dashboard", FIXTURE_SUFFIX].join(""),
    );
    const content = await fs.readFile(filePath, "utf8");
...
function setupDashboardStubAPIs(app, webRoot, options = {}) {
  ...
  if (isProd && !allowStubsInProd) {
    logger.warn(
      "[Security Guard] Blocked attempts to mount development dashboard stubs in production environment.",
    );
    return;
  }
  ...
  } = require("../../server/lib/snapshot-resolver.cjs");
  const {
    resolvePlatformRoot,
  ...
  if (typeof authMiddleware === "function") {
    ...
    logger.error(
      "[Security Failure] Critical: Dashboard Stub API loaded without a valid authMiddleware gate!",
    );
   ...
   * @returns {any}
   */
  const snapshotSend = (res, key, fallback) =>
    sendSnapshotOrSample(res, db, key, fallback, redis);
  /**
   * Snapshot get.
   * @param {any} route
   * @param {any} key
  ...
  const snapshotGet = (route, key, fallback) => {
    dashboardRouter.get(route, async (req, res) =>
      snapshotSend(res, key, fallback),
    );
  };
  ...
  snapshotGet("/api/analytics/errors", "analytics-errors", async () => {
    ...
    const total = errorPoints.reduce(
    ...
    const data = {
      ...
      overview: {
        ...
        totalFiles:
          ...
          report?.totalFiles ??
          sample.overview?.totalFiles,
        codeQuality: report?.qualityScore ?? sample.overview?.codeQuality,
        schemaPassRate:
          report?.schemaCompliance ?? sample.overview?.schemaPassRate,
        scannerIssues: report?.issueCount ?? sample.overview?.scannerIssues,
        securityScore: gate.pass
          ? "100/100"
          : `${100 - (gate.blockingCount || 0) * 10}/100`,
  ...
  dashboardRouter.get("/api/status", async (req, res) => {
    await snapshotSend(res, "api-status-summary", async () => {
      ...
      return {
        ...
        activeAPIs:
          o.activeAPIs ??
          (sample.apis || []).filter((a) => a.status === "active").length,
  ...
  dashboardRouter.get("/api/merger-tool/reduction-scan", async (req, res) => {
    try {
      ...
      const scope =
        String(req.query?.scope || "repository").toLowerCase() ===
        "sample-data-only"
          ? "sample-data-only"
          : "repository";
  ...
  dashboardRouter.post("/api/security/npm-audit", async (_req, res) => {
    try {
      const result = await runNpmAuditAsync(path.join(webRoot, ".."), {
        force: true,
      });
      res.json(result);
    } catch (error) {
      res
        .status(500)
        .json({ error: "npm audit failed", message: error.message });
    }
  });
