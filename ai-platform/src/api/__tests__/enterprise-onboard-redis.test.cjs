...
test("redis-rate-limiter: blocks after configured quota", async () => {
  const key =
    "test-org-" + Date.now() + "-" + Math.floor(Math.random() * 10000);

  const usingRedis = !!(
    redisLimiter &&
    redisLimiter._debug &&
    typeof redisLimiter._debug.usingRedis === "function" &&
    redisLimiter._debug.usingRedis()
  );
  if (!process.env.REDIS_URL && !usingRedis) {
    console.warn(
      "Skipping redis integration test: no REDIS_URL and adapter not using Redis",
    );
    return;
  }
  ...
  assert.strictEqual(
    last.allowed,
    false,
    "expected blocked after quota exceeded",
  );
  if (last.retryAfterMs !== undefined) {
    assert.ok(
      Number(last.retryAfterMs) > 0,
      "retryAfterMs should be positive when provided",
    );
  }
});
