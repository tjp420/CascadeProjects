...
function setupEnterpriseOnboardingRoutes(app) {
    ...
    try {
      ...
      for (const email of seatEmails) {
        ...
        try {
          ...
          logger.warn(
            `[Enterprise] Failed to provision seat for ${email}: ${err.message}`,
          );
        }
      }
      ...
      auditStore.appendEntry({
        ...
        metadata: {
          contractPeriodMonths: org.contractPeriodMonths,
      ...
      res
        .status(500)
        .json({ error: "onboarding_failed", message: err.message });
    }
  });
  ...
  app.post(
    "/api/enterprise/organizations/:orgId/seats",
    enterpriseRateLimit,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "seat_provisioning_failed", message: err.message });
      }
    },
  );
  ...
  app.delete(
    "/api/enterprise/organizations/:orgId/seats/:email",
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "seat_removal_failed", message: err.message });
      }
    },
  );
  ...
  app.post("/api/enterprise/trial", enterpriseRateLimit, async (req, res) => {
    try {
      ...
      if (!companyName || !adminEmail) {
        return res
          .status(400)
          .json({ error: "companyName and adminEmail are required" });
      }
  ...
  app.get("/api/enterprise/trial/:orgId", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "trial_lookup_failed", message: err.message });
    }
  });

  // ── POST /api/enterprise/organizations/:orgId/azure-devops ──
  app.post(
    "/api/enterprise/organizations/:orgId/azure-devops",
    enterpriseRateLimit,
    async (req, res) => {
      try {
        const store = readEnterpriseStore();
        ...
        const pipelineYaml = buildAzureDevOpsPipeline(
          org.orgId,
          org.apiKey,
          projectPath,
        );
  ...
  app.get("/api/enterprise/audit", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "audit_query_failed", message: err.message });
    }
  });
  ...
  app.get("/api/enterprise/audit/stats", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "audit_stats_failed", message: err.message });
    }
  });
  ...
  app.get("/api/enterprise/audit/verify", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "audit_verify_failed", message: err.message });
    }
  });
  ...
  app.get("/api/enterprise/audit/export", async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="enterprise-audit-export.json"',
      );
      res.send(JSON.stringify(exportData, null, 2));
    } catch (err) {
      res
        .status(500)
        .json({ error: "audit_export_failed", message: err.message });
    }
  });
}
