...
function buildScopeTransparency(payload) {
  const primary =
    payload?.headlineSource === "monorepo"
      ? payload?.monorepo
      : payload?.platform;
  const repositoryFilesTotal =
    primary?.repositoryFilesTotal ??
    payload?.headline?.repositoryFilesTotal ??
    null;
  const ruleScopedFilesAnalyzed =
    primary?.ruleScopedFilesAnalyzed ??
    payload?.headline?.ruleScopedFilesAnalyzed ??
    null;
  const coveragePercent =
    repositoryFilesTotal && ruleScopedFilesAnalyzed
      ? Number(
          (
            (ruleScopedFilesAnalyzed / Math.max(repositoryFilesTotal, 1)) *
            100
          ).toFixed(2),
        )
      : null;
  ...
  return {
    ...
    fictionJsonFilesScanned:
      fiction?.fictionJsonFilesScanned ??
      primary?.fictionJsonFilesScanned ??
      null,
    fictionSampleFilesScanned:
      fiction?.fictionSampleFilesScanned ??
      primary?.fictionSampleFilesScanned ??
      null,
...
function setupTrustAPI(app, options = {}) {
  ...
  app.get("/api/trust/verification", (req, res) => {
    try {
      ...
      const includePublished = String(
        req.query.includePublished || "",
      ).toLowerCase();
      if (
        includePublished === "1" ||
        includePublished === "true" ||
        includePublished === "yes"
      ) {
        return res.json({
          success: true,
          live,
  ...
  app.get("/api/trust/badge", (req, res) => {
    try {
      const payload = buildTrustVerificationPayload({
        platformRoot,
        monorepoRoot,
      });
      const origin = `${req.protocol}://${req.get("host")}`;
