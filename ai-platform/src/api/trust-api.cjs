...
function wantsRawSvg(req) {
  const raw = String(req.query.raw || '')
    .toLowerCase()
    .trim();
...
function buildScopeTransparency(payload) {
  ...
  const repositoryFilesTotal =
    primary?.repositoryFilesTotal ?? payload?.headline?.repositoryFilesTotal ?? null;
  const ruleScopedFilesAnalyzed =
    primary?.ruleScopedFilesAnalyzed ?? payload?.headline?.ruleScopedFilesAnalyzed ?? null;
  ...
  return {
    ...
    fictionJsonFilesScanned:
      fiction?.fictionJsonFilesScanned ?? primary?.fictionJsonFilesScanned ?? null,
    fictionSampleFilesScanned:
      fiction?.fictionSampleFilesScanned ?? primary?.fictionSampleFilesScanned ?? null,
    fictionWalkRoot: fiction?.walkRoot ?? null,
    limitations: primary?.scopeNote ? [primary.scopeNote] : payload?.disclaimers || [],
  };
}
...
function setupTrustAPI(app, options = {}) {
  ...
  app.get('/api/trust/badge.svg', (req, res) => {
    ...
    try {
      ...
      if (wantsHtml) {
        ...
        return res.send(
          `<!DOCTYPE html><html><body><h1>Badge unavailable</h1><p>${error.message}</p></body></html>`
        );
      }
  ...
  app.get('/api/trust/verify', (req, res) => {
    try {
      ...
      if (wantsHtmlResponse(req)) {
        res.status(500).set('Content-Type', 'text/html; charset=utf-8');
        return res.send(
          `<!DOCTYPE html><html><body><h1>Trust verify unavailable</h1><p>${error.message}</p></body></html>`
        );
      }
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  app.get('/api/trust/history', (req, res) => {
    try {
      ...
      const entries = hasLimit ? history.entries.slice(0, Math.min(limit, 365)) : history.entries;
      const trend = buildTrustTrend(
        entries,
        Number.parseInt(String(req.query.window || ''), 10) || 30
      );
  ...
  app.get('/trust-verification.json', (req, res) => {
    try {
      if (!fs.existsSync(PUBLIC_TRUST_PATH)) {
        return res
          .status(404)
          .json({ success: false, error: 'No published trust verification found' });
      }
      res.set('Content-Type', 'application/json; charset=utf-8');
      res.set('Cache-Control', 'public, max-age=300');
      return res.sendFile(PUBLIC_TRUST_PATH);
    } catch (error) {
      return res.status(500).json({ success: false, error: error.message });
    }
  });
