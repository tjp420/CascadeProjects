...
const { buildAnalyticsModel } = require('../../server/lib/analytics-builder');
const { mergeIstanbulTelemetry } = require('../../server/lib/istanbul-telemetry-merge');
const { buildSecurityDashboardModel } = require('../../server/lib/security-dashboard-builder');
...
async function loadAiRoadmapSample(webRoot) {
    ...
    try {
        ...
        const filePath = resolveSampleFilePath(platformRoot, 'ai-roadmap-sample.json');
        const content = await fs.readFile(filePath, 'utf8');
...
function setupDashboardStubAPIs(app, webRoot, options = {}) {
    ...
    const { sendSnapshotOrSample } = require('../../server/lib/snapshot-resolver');
