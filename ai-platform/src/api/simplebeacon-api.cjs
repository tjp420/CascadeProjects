...
async function runSimplebeaconScan(projectPath, opts = {}) {
  ...
  if (!fs.existsSync(cliBin)) {
    ...
    try {
      ...
      const analyzedCount =
        analysis.summary?.codeFilesAnalyzed ?? analysis.summary?.codeFilesDiscovered ?? 0;
...
function setupSimplebeaconAPI(app, options = {}) {
  ...
  const requirePaidReadOnly =
    options.requirePaidReadOnly || createRequireSubscription({ allowFree: true });
  const requirePaid = options.requirePaid || createRequireSubscription();
  const requirePaidWithQuota =
    options.requirePaidWithQuota || createRequireSubscription({ consumeQuota: true });
  ...
  app.get('/api/simplebeacon/report', requirePaidReadOnly, async (req, res) => {
    ...
    try {
      ...
      const isAdmin =
        req.user?.role === 'admin' ||
        (Array.isArray(req.user?.permissions) && req.user.permissions.includes('admin:all'));
      ...
      if (uploadSecret) {
        ...
        if (!remote.includes('127.0.0.1') && !remote.includes('::1')) {
          return res
            ...
            .json({
              success: false,
              error: 'Uploads restricted. Set SIMPLEBEACON_UPLOAD_SECRET to enable remote uploads.',
            });
        }
      }
      ...
      if (projectPath) {
        try {
          ...
          const usesDefaultScanPaths =
            ...
            JSON.stringify(config.scanPaths) === JSON.stringify(DEFAULT_MOCK_SCAN_RELATIVE_PATHS);
  ...
  app.put('/api/simplebeacon/config', requirePaid, async (req, res) => {
    ...
    if (!incoming || typeof incoming !== 'object' || Array.isArray(incoming)) {
      return res
        .status(400)
        .json({ error: 'Invalid config', message: 'Request body must be a JSON object' });
    }
    ...
    async (req, res) => {
      try {
        ...
        const toWrite = {
          ...
          apiAccess:
            body.apiAccess ||
            (body.plan === 'pro' ? 'Full' : req.user?.tier === 'pro' ? 'Full' : 'Limited'),
        };
        await fs.promises.mkdir(SIMPLEBEACON_DIR, { recursive: true });
  ...
  app.post('/api/simplebeacon/tools/baseline-sync', requirePaidWithQuota, async (_req, res) => {
    try {
      ...
      const clientError =
        /Jest is not configured|Could not parse Jest summary|Jest reported failures/i.test(message);
  ...
  app.post('/api/simplebeacon/schedule', requirePaid, async (req, res) => {
    try {
      ...
      const updated = {
        ...
        zeroRetention:
          typeof body.zeroRetention === 'boolean' ? body.zeroRetention : current.zeroRetention,
      };
