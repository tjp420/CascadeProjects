...
const {
  patchRemediationPhases,
  normalizeDashboardReport,
} = require("../../server/lib/scan-report-patch.cjs");
const {
  createRequireSubscription,
} = require("../../server/middleware/simplebeacon-subscription.cjs");
const {
...
function getAnalyzeAllowedRoots() {
  return resolveDefaultAllowedRoots(PROJECT_ROOT, {
 ...
 */
const {
  setupSimplebeaconDemoAPI,
} = require("../../server/routes/demo-simplebeacon-api.cjs");
...
function resolveSafeAnalyzePath(rawPath) {
  ...
  if (path.isAbsolute(trimmed)) {
    ...
    if (!fs.existsSync(trimmed) && fs.existsSync(PROJECT_ROOT)) {
      ...
      const isPlatformStyleFallback =
        normalized.includes("/ai-platform/") && !isUnderPlatformRoot;
...
function resolveReportFilePath(projectPath) {
  ...
  if (
    projectResolved !== platformResolved &&
...
async function appendHistory(report) {
  ...
  if (
    ...
    last.issueCount === entry.issueCount
  ) {
    return history;
  }
...
async function loadAssessment(report) {
  ...
  if (saved) {
    const sourceGenerated =
      saved.sourceReport?.generatedAt || saved.generatedAt;
    if (sourceGenerated) {
...
async function runSimplebeaconScan(projectPath, opts = {}) {
  ...
  if (!fs.existsSync(cliBin)) {
    logger.warn(
      "[runSimplebeaconScan] CLI binary not found at",
      cliBin,
      "— falling back to programmatic analyzeCodebase",
    );
    try {
      const {
        analyzeCodebase,
      ...
      const analyzedCount =
        ...
        analysis.summary?.codeFilesDiscovered ??
        0;
      ...
      const report = normalizeDashboardReport(
        {
          ...
          gate: {
            pass: analyzedCount > 0 && healthScore >= 80,
            score: analyzedCount > 0 ? healthScore : 0,
          },
        },
        scanRoot,
      );
      ...
      await fs.promises.writeFile(
        reportOut,
        JSON.stringify(patchedReport, null, 2),
      );
      ...
      isScanRunning = false;
      logger.error(
        "[runSimplebeaconScan] Programmatic fallback failed:",
        fallbackErr.message,
      );
      throw fallbackErr;
    }
  ...
  try {
    ...
    logger.warn(
      "[runSimplebeaconScan] License generation failed:",
      tokenErr.message,
    );
  }
  ...
  try {
    try {
      ...
      if (!reportExists) {
        logger.error(
          "[runSimplebeaconScan] CLI failed — report missing:",
          err.message,
          "stderr:",
          stderr.slice(0, 500),
        );
    ...
    if (fs.existsSync(reportFilePath)) {
      ...
      logger.warn(
        "[runSimplebeaconScan] Report file missing — attempting stdout fallback",
      );
      report = tryExtractJsonFromStdout(stdout);
      if (!report) {
        logger.warn(
          "[runSimplebeaconScan] No JSON in stdout — creating minimal fallback report",
        );
        report = {
          type: "simplebeacon-report",
          version: "1.0.0",
      ...
      await fs.promises.mkdir(path.dirname(reportFilePath), {
        recursive: true,
      });
      await fs.promises.writeFile(
        reportFilePath,
    ...
    report = normalizeDashboardReport(
      report,
      projectPath || report.projectRoot || report.projectPath || PROJECT_ROOT,
    );
...
function setupSimplebeaconAPI(app, options = {}) {
  logger.warn(
    "[simplebeacon-api] default report path:",
    REPORT_PATH.replace(/\\/g, "/"),
    "project root:",
    PROJECT_ROOT.replace(/\\/g, "/"),
  );
  const requirePaidReadOnly =
    options.requirePaidReadOnly ||
    createRequireSubscription({ allowFree: true });
  const requirePaid = options.requirePaid || createRequireSubscription();
  const requirePaidWithQuota =
    options.requirePaidWithQuota ||
    createRequireSubscription({ consumeQuota: true });
  ...
  function requireUserAccount(req, res, next) {
    if (!req.user?.email) {
      ...
      if (debug) {
        logger.warn(
          `[simplebeacon-api] requireUserAccount 401 - ${req.method} ${req.originalUrl}: ${debug}`,
        );
      }
    ...
    try {
      if (rawProjectPath) {
        try {
          ...
          logger.info(
            "[simplebeacon-api] report path outside allowed roots:",
            rawProjectPath,
          ...
          findings: [
            {
              ...
              severity: "high",
              message:
                "No scan report found for the requested path. Run a scan or verify the project path.",
            },
          ],
          modules: [],
          gate: {
  ...
  app.post("/api/simplebeacon/report/upload", async (req, res) => {
    try {
      ...
      const normalized = normalizeDashboardReport(
        report,
        report.projectPath || PROJECT_ROOT,
      );
  ...
  app.post(
    "/api/simplebeacon/report/browser-error",
    optionalAuthenticate,
    requireUserAccount,
    async (req, res) => {
      try {
        ...
        try {
          ...
          logger.warn(
            "[simplebeacon-api] failed to write audit log for browser-error",
  ...
  app.get(
    "/api/simplebeacon/report/browser-errors",
    requirePaidReadOnly,
    async (req, res) => {
      try {
        const errorsPath = path.join(
          path.dirname(resolveReportFilePath(null)),
          "browser-errors.ndjson",
        );
  ...
  app.get(
    "/api/simplebeacon/report/ai-agent",
    requirePaidReadOnly,
    async (_req, res) => {
      try {
  ...
  app.get("/api/simplebeacon/baseline", async (_req, res) => {
    try {
      ...
      if (!enriched.jestTestsLabel && report?.jestSummary?.testsTotal != null) {
        ...
        enriched.jestSuites =
          report.jestSummary.suitesPassed ?? enriched.jestSuites;
      }
      res.json(enriched);
    } catch (err) {
      console.error("simplebeacon-api.cjs error:", err);
  ...
  app.get("/api/simplebeacon/config/presets", async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to load presets", message: err.message });
    }
  });
    ...
    try {
      ...
      if (projectPath) {
        try {
          ...
          const usesDefaultScanPaths =
            !config.scanPaths ||
            config.scanPaths.length === 0 ||
            JSON.stringify(config.scanPaths) ===
              JSON.stringify(DEFAULT_MOCK_SCAN_RELATIVE_PATHS);
          const usesDefaultProductionPaths =
            !config.productionPaths ||
            config.productionPaths.length === 0 ||
            JSON.stringify(config.productionPaths) ===
              JSON.stringify(DEFAULT_CONFIG.productionPaths);
  ...
  app.put("/api/simplebeacon/config", requirePaid, async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to write config", message: err.message });
    }
  });

  app.get(
    "/api/simplebeacon/user/ai-keys",
    optionalAuthenticate,
    async (req, res) => {
      try {
    ...
    async (req, res) => {
      try {
        ...
        if (!email) {
          ...
          logger.warn(
            "[PUT /api/simplebeacon/user/ai-keys] Auth failed: missing req.user.email",
            {
          ...
          return res.status(401).json({
            ...
            debug: verifyError
              ? `Token verification failed: ${verifyError}`
              : token
                ? "Token verified but req.user was not set"
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const [aiKeys, report, history, baseline, config, assessment] =
          await Promise.all([
            getUserAiKeysPublic(email).catch(() => ({
              providers: {},
              ollamaBaseUrl: "",
              ollamaModel: "",
              updatedAt: null,
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        let sub = null;
        try {
          const raw = await readTextFileWithLimit(
            SUBSCRIPTION_PATH,
            64 * 1024,
          ).catch(() => null);
        ...
        if (!sub) {
          sub = {
            ...
            scansRemaining:
              req.user?.plan === "free" ? "Unlimited" : "Unlimited",
            apiAccess:
              req.user?.tier === "pro" || req.user?.plan === "pro"
                ? "Full"
                : "Limited",
          };
        }
    ...
    async (req, res) => {
      try {
        ...
        const toWrite = {
          ...
          scansRemaining:
            typeof body.scansRemaining !== "undefined"
              ? body.scansRemaining
              : body.plan === "free"
                ? "Unlimited"
                : "Unlimited",
  ...
  app.get("/api/simplebeacon/ollama/models", async (req, res) => {
    try {
      let rawBaseUrl = String(req.query.baseUrl || "http://127.0.0.1:11434")
        .trim()
        .replace(/^["']+|["']+$/g, "")
        .replace(/\/$/, "");
      ...
      try {
        ...
        try {
          ...
          return res
            .status(400)
            .json({ success: false, error: "Invalid Ollama base URL" });
        }
      }
      const baseUrl = `${parsed.protocol}//${parsed.host}`;
      if (!isAllowedOllamaHost(parsed.hostname)) {
        logger.warn(
          "[GET /api/simplebeacon/ollama/models] Rejected baseUrl:",
          rawBaseUrl,
          "hostname:",
          parsed.hostname,
        );
  ...
  app.post("/api/simplebeacon/ollama/chat", async (req, res) => {
    try {
      ...
      if (!isAllowedOllamaHost(parsed.hostname)) {
        logger.warn(
          "[POST /api/simplebeacon/ollama/chat] Rejected baseUrl:",
          rawBaseUrl,
          "hostname:",
          parsed.hostname,
        );
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(404)
          .json({ error: "Dashboard not found", message: err.message });
      }
    },
  );

  app.get(
    "/api/simplebeacon/results/:scanId",
    requirePaidReadOnly,
    async (req, res) => {
      try {
        const context = await loadDashboardContext();
        ...
        if (!entry) {
          return res
            .status(404)
            .json({ error: "Scan not found", scanId: req.params.scanId });
        }
        res.json(buildScanResults(context.report, entry, context.baseline));
      } catch (err) {
        res
          .status(404)
          .json({ error: "Results not found", message: err.message });
      }
    },
  );

  app.get(
    "/api/simplebeacon/assessment",
    requirePaidReadOnly,
    async (_req, res) => {
      try {
        const context = await loadDashboardContext();
        ...
        res
          .status(404)
          .json({ error: "Assessment not found", message: err.message });
      }
    },
  );
  ...
  app.post(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Assessment failed", message: err.message });
      }
    },
  );
  ...
  app.get("/api/simplebeacon/audit", requirePaidReadOnly, async (req, res) => {
    try {
      const includeNpm =
        req.query.npmAudit === "1" || req.query.npmAudit === "true";
      ...
      logger.error(
        "[/api/simplebeacon/audit] Failed to load audit context:",
        err?.message || err,
      );
  ...
  app.post(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "npm audit failed", message: err.message });
      }
    },
  );
  ...
  app.post("/api/simplebeacon/scan", requirePaidWithQuota, async (req, res) => {
    ...
    try {
      ...
      logger.error(
        "[POST /api/simplebeacon/scan] Scan error:",
        err.message,
        "code:",
        err.code,
        "killed:",
      ...
      if (reportExists) {
        try {
          ...
          return res.json({
            ...
            gateFailed: true,
            message:
              "Simplebeacon scan complete — gate review (recovered from CLI exit)",
            report,
            history,
            projectPath: projectPath || PROJECT_ROOT,
      ...
      res.status(err.killed ? 504 : 500).json({
        ...
        stdout: err.stdout?.slice(-constants.MAX_RATE_LIMIT),
        stderr: err.stderr?.slice(-500),
        partialReport: reportExists
          ? await readJson(reportOut).catch(() => null)
          : null,
        _debug: {
          stack: err.stack,
          code: err.code,
  ...
  app.get(
    "/api/ai-validation/results/:scanId",
    requirePaidReadOnly,
    async (req, res) => {
      try {
        const context = await loadDashboardContext();
  ...
  app.post(
    "/api/simplebeacon/tools/baseline-sync",
    requirePaidWithQuota,
    async (_req, res) => {
      try {
        ...
        res.json({
          ...
          pageSamplesLabel:
            result.pageSamplesLabel ??
            result.baseline?.pageSamplesLabel ??
            null,
        ...
        const clientError =
          /Jest is not configured|Could not parse Jest summary|Jest reported failures/i.test(
            message,
          );
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        ...
        res
          .status(404)
          .json({ error: "Summary not found", message: err.message });
      }
    },
  );

  app.post(
    "/api/simplebeacon/cloud-scan",
    requirePaidWithQuota,
    async (req, res) => {
  ...
  app.post("/api/simplebeacon/schedule", requirePaid, async (req, res) => {
    try {
      ...
      const updated = {
        ...
        intervalMinutes:
          Number(body.intervalMinutes) || current.intervalMinutes || 60,
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        logger.info(
          `[WebhookScan] ${webhookResult.success ? "delivered" : "failed"} to ${webhookUrl}`,
        );
        ...
        if (req.body?.zeroRetention) {
          try {
            ...
            if (fs.existsSync(reportOut)) {
              ...
              logger.info(
                "[WebhookScan] Zero-retention: report file removed after delivery",
              );
            }
          } catch (unlinkErr) {
            logger.warn(
              "[WebhookScan] Zero-retention: failed to remove report file:",
              unlinkErr.message,
            );
          }
        }
    ...
    req.on("end", async () => {
      try {
        ...
        if (!folderName) {
          res
            .status(400)
            .json({ success: false, error: "folderName is required" });
          return;
        }
...
function findFolderByName(folderName, allowedRoots) {
  ...
  for (const root of allowedRoots) {
    ...
    if (
      ...
      continue;
    if (path.basename(resolvedRoot).toLowerCase() === targetLower)
      return resolvedRoot;
