...
} = require("./billing/validate-project-token.cjs");
const {
  verifyLicenseToken,
} = require("../../server/lib/simplebeacon-proxy.cjs");
...
function setupSimplebeaconBillingWebhook(app) {
  app.post(
    ...
    async (req, res) => {
      ...
      const webhookSecretPrimary =
        String(process.env.STRIPE_WEBHOOK_SECRET || "").trim() || null;
      const webhookSecretSecondary =
        String(
      ...
      const secrets = [webhookSecretPrimary, webhookSecretSecondary].filter(
        Boolean,
      );
      let lastErr = null;
      ...
      try {
        switch (event.type) {
          case "checkout.session.completed": {
            ...
            try {
              ...
              logger.error(
                "[Simplebeacon billing] Failed to fetch line items:",
                err.message,
              );
            }
            ...
            if (isOneTimeProduct || isPaymentMode) {
              ...
              const expiresInMinutes =
                PRODUCT_EXPIRY_MINUTES_MAP[product] || 60;

              // For custom plans, derive features from metadata.scans
              let customFeatures = PRODUCT_FEATURES_MAP[product] || [
                "pdf-generation",
              ];
              ...
              if (emailPayload) {
                const cfg =
                  TIER_EMAIL_CONFIG[product] ||
                  TIER_EMAIL_CONFIG.executive_clearance;
                sendEmail({
                  to: email,
                  ...
                  logger.error(
                    "[Simplebeacon billing] Failed to send welcome email",
                  );
                });
              } else {
                ...
                sendEmail({
                  ...
                  logger.error(
                    "[Simplebeacon billing] Failed to send fallback email",
                  );
                });
              }
              ...
              const isTeamPro =
                product === "team_pro_tier" ||
                product === "team_pro_monthly" ||
              ...
              const record = await setSubscriptionActive(email, true, {
                ...
                licenseToken,
                licenseTier: subTier,
                complianceCertLimit: isContinuousShield
                  ? 3
                  : isRuntimeShield
                    ? 5
                    : 0,
              ...
              if (emailPayload) {
                ...
                sendEmail({
                  ...
                  logger.error(
                    "[Simplebeacon billing] Failed to send subscription welcome email",
                  );
                });
              }
        ...
        logger.error(
          "[Simplebeacon billing] Webhook handler error:",
          err.message,
        );
...
function setupSimplebeaconBillingRoutes(app) {
  ...
  app.post("/api/quota/check", billingRateLimit, async (req, res) => {
    ...
    const allowed =
      status.scansRemaining === "unlimited" || status.scansRemaining > 0;
    res.json({
      allowed,
  ...
  const PRODUCT_PRICE_FALLBACK = {
    ...
    one_time_certificate: {
      amount: 14900,
      interval: null,
      name: "Board-Ready Audit Certificate",
      desc: "One-time board-ready audit certificate",
    },
    executive_clearance: {
      amount: 49900,
      interval: null,
      name: "Executive Risk Certificate",
      desc: "Executive risk clearance certificate",
    },
    eu_ai_act_sprint: {
      amount: 249900,
      interval: null,
      name: "EU AI Act Sprint",
      desc: "EU AI Act compliance sprint — 30-day engagement",
    },
  };
  ...
  app.post(
    "/api/simplebeacon/billing/checkout",
    billingRateLimit,
    async (req, res) => {
      ...
      try {
        const projectName = String(
          req.body?.projectName || req.body?.certProjectName || "",
        ).trim();
        ...
        const extraSeats =
          product.startsWith("team_pro") && req.body?.extraSeats
            ? Math.max(0, Math.min(50, parseInt(req.body.extraSeats, 10) || 0))
            : 0;
        if (extraSeats > 0) {
          const isAnnual = product.endsWith("_annual");
          const seatAmount = isAnnual
            ? PRICE_EXTRA_SEAT_ANNUAL
            : PRICE_EXTRA_SEAT_MONTHLY;
          ...
          lineItems.push({
            price_data: {
              ...
              product_data: {
                ...
                description:
                  "Additional seat beyond the 5 included in Team Pro",
              },
        ...
        const sessionParams = {
          ...
          metadata: {
            ...
            ...(extraSeats > 0 ? { extraSeats: String(extraSeats) } : {}),
          },
        };
    ...
    try {
      ...
      if (email && session.payment_status === "paid" && !record?.licenseToken) {
        ...
        if (teamProducts.has(product) || session.mode === "subscription") {
          const isGrowth = /growth|team_annual|team_monthly|teams/.test(
            product,
          );
      ...
      res
        .status(500)
        .json({ error: "session_lookup_failed", message: err.message });
    }
  });
  ...
  app.post(
    "/api/simplebeacon/billing/portal",
    billingRateLimit,
    async (req, res) => {
      if (!isMonetizationEnabled()) {
        return billingDisabledResponse(res);
      ...
      if (!record?.stripeCustomerId) {
        return res
          .status(404)
          .json({ error: "No Stripe customer for this email" });
      }
  ...
  app.post("/api/reports/upload", billingRateLimit, async (req, res) => {
    try {
      ...
      if (!reportJson || typeof reportJson !== "object") {
        return res
          .status(400)
          .json({ success: false, error: "reportJson is required" });
      }
      ...
      try {
        console.error(
          "[DIAG] /api/reports/upload reportJson before build:",
          JSON.stringify(reportJson),
        );
      } catch (e) {
        console.error(
          "simplebeacon-billing-api.cjs error:",
          e,
        ); /* best-effort */
      }
      ...
      try {
        ...
        const reportFile = path.join(
          REPORT_STORE_DIR,
          `${bundle.deliveryId}.json`,
        );
        if (fs.existsSync(reportFile)) {
          const snap = fs.readFileSync(reportFile, "utf8");
          try {
            logger.info("[DIAG] post-bundle persisted file exists", {
          ...
          try {
            console.error("[DIAG] post-bundle persisted snapshot (start)");
          } catch (e) {
            console.error(
              "simplebeacon-billing-api.cjs error:",
              e,
          ...
          try {
            console.error(snap.slice(0, 4000));
          } catch (e) {
            console.error(
              "simplebeacon-billing-api.cjs error:",
              e,
          ...
          try {
            console.error("[DIAG] post-bundle persisted snapshot (end)");
          } catch (e) {
            console.error(
              "simplebeacon-billing-api.cjs error:",
              e,
          ...
          try {
            logger.info("[DIAG] post-bundle persisted file missing", {
              deliveryId: bundle.deliveryId,
            });
          } catch (e) {
            console.error(
        ...
        try {
          console.error("[DIAG] post-bundle inspect failed:", e && e.message);
        } catch (e2) {
          console.error(
            "simplebeacon-billing-api.cjs error:",
            e2,
  ...
  app.post("/api/reports/download", billingRateLimit, async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${bundle.zipFilename}"`,
      );
  ...
  app.get("/api/reports/status/:licenseToken", async (req, res) => {
    try {
      ...
      if (!record) {
        return res
          .status(404)
          .json({ success: false, error: "License token not found" });
      }
      res.json({
        success: true,
        email: record.email,
  ...
  app.post("/api/reports/verify", async (req, res) => {
    try {
      ...
      if (!reportJson || typeof reportJson !== "object") {
        return res
          .status(400)
          .json({ success: false, error: "reportJson is required in body" });
      }
      const {
        verifyReportSignature,
      } = require("../../server/lib/report-signer.cjs");
      ...
      res
        .status(500)
        .json({ success: false, error: err.message || "verification_failed" });
    }
  });
   ...
   */
  app.post(
    "/api/simplebeacon/billing/resend-token",
    billingRateLimit,
    async (req, res) => {
      try {
        const email = normalizeEmail(req.body?.email);
        ...
        const certUploadUrl = `${getAppBaseUrl()}/coming-soon/certificate-upload.html`;
        const licenseTier =
          record.licenseTier || PRODUCT_TIER_MAP[product] || "executive";

        const { subject, body } = buildResendEmail(
          product,
          record,
          certUploadUrl,
        );
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to resend token" });
      }
    },
  );
  ...
  function hasTeamComplianceLicense(subscription) {
    const tier = String(
      subscription?.licenseTier || subscription?.tier || "community",
    ).toLowerCase();
  ...
  app.get("/api/simplebeacon/ci/telemetry/summary", async (req, res) => {
    ...
    if (subscription && hasTeamComplianceLicense(subscription)) {
      ...
      return res.json({
        ...
        ...(teamSummary.workspace_breakdown
          ? { workspace_breakdown: teamSummary.workspace_breakdown }
          : {}),
      });
    }
