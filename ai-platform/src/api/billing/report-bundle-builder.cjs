...
async function buildReportBundle(licenseToken, reportJson) {
  ...
  if (!record) {
    ...
    if (payload) {
      record.certClientName =
        record.certClientName || payload.clientName || record.clientName || 'Client';
      record.certProjectName =
        record.certProjectName || payload.projectName || record.projectName || 'Project';
    }
  }
  if (
    !['executive', 'agency', 'universal', 'euai', 'instant', 'community'].includes(
      record.licenseTier
    )
  ) {
  ...
  try {
    ...
    auditReportHtml =
      typeof auditResult === 'string' ? auditResult : (auditResult && auditResult.html) || null;
  ...
  const browserMockScan = reportJson.mockDataCategories?.length
    ? {
        ...
        total: reportJson.mockSampleFiles || reportJson.mockDataCategories.length,
      }
    : null;
  const browserDataQuality =
    reportJson.dataQuality?.emptyJsonCount !== undefined ? reportJson.dataQuality : null;
  const browserCleanup =
    reportJson.cleanup?.debugArtifactCount !== undefined ? reportJson.cleanup : null;
  const browserNpmAudit =
    reportJson.npmAudit?.packageJsonCount !== undefined ? reportJson.npmAudit : null;
  const browserCompliance =
    reportJson.compliance?.licenseCount !== undefined ? reportJson.compliance : null;
  const browserEuAiAct =
    reportJson.euAiActSummary?.highRiskIndicators !== undefined ? reportJson.euAiActSummary : null;

  const analysisResults = {
    simplebeacon: embeddedResults.simplebeacon || simplebeaconReport || reportJson,
    codebase:
      embeddedResults.codebase || reportJson._codebaseAnalysis || reportJson.codebase || null,
    ...
    compliance:
      embeddedResults.compliance || reportJson._complianceAnalysis || browserCompliance || null,
    euAiAct:
      embeddedResults.euAiAct ||
      reportJson._euAiActAnalysis ||
      browserEuAiAct ||
      (reportJson.type === 'simplebeacon-eu-ai-act-sprint' ? reportJson : null) ||
      null,
  };
