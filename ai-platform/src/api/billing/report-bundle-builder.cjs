...
} = require("../../../server/lib/complete-scan-audit-report.cjs");
const {
  buildAnalyzeExportZipStream,
} = require("../../../server/lib/analyze-export-bundle.cjs");
const {
  loadAgencyBranding,
} = require("../../../server/lib/agency-branding-store.cjs");
const {
  safeStringify,
  streamToBuffer,
  ensureReportDir,
...
async function buildReportBundle(licenseToken, reportJson) {
  ...
  if (!record) {
    ...
    payload = verifyLicenseToken(licenseToken, secret);
    if (payload) {
      record.certClientName =
        record.certClientName ||
        payload.clientName ||
        record.clientName ||
        "Client";
      record.certProjectName =
        record.certProjectName ||
        payload.projectName ||
        record.projectName ||
        "Project";
  ...
  function buildSignedPayload(srcReport) {
    const signingKey =
      String(process.env.REPORT_SIGNING_KEY || "").trim() || null;
    const signingKeyId =
      String(process.env.REPORT_SIGNING_KEY_ID || "").trim() || null;
    let cloned;
    try {
      ...
      try {
        cloned = JSON.parse(safeStringify(srcReport, 2));
      } catch (e2) {
        cloned = srcReport;
      }
    }
    if (!signingKey) {
      try {
        console.error(
          "[DIAG] REPORT_SIGNING_KEY not configured — building unsigned snapshot",
        );
      } catch (e) {
        console.error("Failed to log diagnostic info:", e);
      }
      return cloned;
    }
    try {
      try {
        console.error(
          "[DIAG] buildSignedPayload signingKey present, keyId=",
          signingKeyId || null,
        );
      } catch (e) {
        console.error("Failed to log diagnostic info:", e);
      ...
      if (envelope) {
        ...
        try {
          console.error(
            "[DIAG] buildSignedPayload attached signature length=",
      ...
      logger.warn(
        "[Reports] buildSignedPayload signing failed, continuing without signature:",
  ...
  try {
    try {
      logger.info("[Reports] about to build signed payload and write file", {
        deliveryId,
        reportPath,
      });
    } catch (e) {
      console.error("Failed to log report write info:", e);
    }
    try {
      console.error(
        "[DIAG] about to build signed payload for deliveryId=",
        deliveryId,
        "reportPath=",
        reportPath,
      );
    ...
    try {
      logger.info(
        "[Reports] pre-write signed hasCryptoValidation=" +
          !!signedPayload.cryptoValidation,
        { deliveryId },
      );
    } catch (e) {
      console.error("Failed to log pre-write info:", e);
    }
    try {
      console.error(
        "[DIAG] pre-write signed hasCryptoValidation=",
        !!signedPayload.cryptoValidation,
        "toWriteLen=",
        toWrite.length,
      );
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to fsync file descriptor:", e);
      }
    } finally {
      try {
        fs.closeSync(fd);
      } catch (e) {
        console.error("Failed to close file descriptor:", e);
      }
    }
    ...
    try {
      ...
      try {
        console.error("[DIAG] post-rename file snapshot (start)");
      } catch (e) {
        console.error("Failed to log diagnostic info:", e);
      }
      try {
        console.error(after.slice(0, 4000));
      } catch (e) {
        console.error("Failed to log file snapshot:", e);
      }
      try {
        console.error("[DIAG] post-rename file snapshot (end)");
      } catch (e) {
        console.error("Failed to log diagnostic info:", e);
      }
    } catch (e) {
      try {
        console.error("[DIAG] post-rename read failed:", e && e.message);
      } catch (e2) {
        console.error("Failed to log post-rename read failure:", e2);
      }
    }
    ...
    try {
      const watcher = fs.watch(reportPath, (eventType) => {
        try {
          ...
          logger.info("[Reports] watcher detected modification", {
            deliveryId,
            eventType,
          });
          try {
            console.error("[DIAG] watcher snapshot (start)");
          } catch (e) {
            console.error("Failed to log diagnostic info:", e);
          }
          try {
            console.error(now.slice(0, 4000));
          } catch (e) {
            console.error("Failed to log watcher snapshot:", e);
          }
          try {
            console.error("[DIAG] watcher snapshot (end)");
          } catch (e) {
            console.error("Failed to log diagnostic info:", e);
          }
        } catch (e) {
          try {
            console.error("[DIAG] watcher read failed:", e && e.message);
          } catch (e2) {
            console.error("Failed to log watcher read failure:", e2);
          }
        }
      });
      setTimeout(() => {
        try {
          ...
          console.error("Failed to close file watcher:", e);
        }
      }, 2000);
    } catch (e) {
      try {
        console.error("[DIAG] watcher install failed:", e && e.message);
      } catch (e2) {
        console.error("Failed to log watcher install failure:", e2);
      }
    }
  } catch (writeErr) {
    try {
      ...
      console.error("Failed to clean up temp file:", _);
    }
    throw writeErr;
  }
  ...
  const totalScanned =
    reportJson.ruleScopedFilesAnalyzed ||
  ...
  try {
    ...
    auditReportHtml =
      typeof auditResult === "string"
        ? auditResult
        : (auditResult && auditResult.html) || null;
    auditReportFilename =
      auditResult && auditResult.filename ? auditResult.filename : null;
  } catch (auditErr) {
  ...
  const tierLabel =
    TIER_TO_DELIVERABLE_SKU[record.licenseTier] ||
  ...
  const simplebeaconReport = (() => {
    ...
    const embeddedKeys = [
      ...
      "_npmAuditAnalysis",
      "_complianceAnalysis",
      "_dataCleanupAnalysis",
      "_dataQualityAnalysis",
      "_cleanupAssistantAnalysis",
      "_fileReductionAnalysis",
      "_roadmapAnalysis",
      "_consolidationAnalysis",
      "_mockScanAnalysis",
      "_euAiActAnalysis",
      "_analysisError",
    ];
  ...
  const browserMockScan = reportJson.mockDataCategories?.length
    ? {
        categories: reportJson.mockDataCategories,
        total:
          reportJson.mockSampleFiles || reportJson.mockDataCategories.length,
      }
    : null;
  const browserDataQuality =
    reportJson.dataQuality?.emptyJsonCount !== undefined
      ? reportJson.dataQuality
      : null;
  const browserCleanup =
    reportJson.cleanup?.debugArtifactCount !== undefined
      ? reportJson.cleanup
      : null;
  const browserNpmAudit =
    reportJson.npmAudit?.packageJsonCount !== undefined
      ? reportJson.npmAudit
      : null;
  const browserCompliance =
    reportJson.compliance?.licenseCount !== undefined
      ? reportJson.compliance
      : null;
  const browserEuAiAct =
    reportJson.euAiActSummary?.highRiskIndicators !== undefined
      ? reportJson.euAiActSummary
      : null;

  const analysisResults = {
    simplebeacon:
      embeddedResults.simplebeacon || simplebeaconReport || reportJson,
    codebase:
      embeddedResults.codebase ||
      reportJson._codebaseAnalysis ||
      reportJson.codebase ||
      null,
    mockScan:
      embeddedResults.mockScan ||
      reportJson._mockScanAnalysis ||
      browserMockScan ||
      null,
    roadmap:
      embeddedResults.roadmap ||
      reportJson._roadmapAnalysis ||
      reportJson.roadmap ||
      null,
    consolidation:
      embeddedResults.consolidation ||
      reportJson._consolidationAnalysis ||
      reportJson.consolidation ||
      null,
    fileReduction:
      embeddedResults.fileReduction ||
      reportJson._fileReductionAnalysis ||
      reportJson.fileReduction ||
      null,
    dataQuality:
      embeddedResults.dataQuality ||
      embeddedResults.dataCleanup ||
      reportJson._dataQualityAnalysis ||
      browserDataQuality ||
      null,
    cleanupAssistant:
      embeddedResults.cleanupAssistant ||
      reportJson._cleanupAssistantAnalysis ||
      browserCleanup ||
      null,
    npmAudit:
      embeddedResults.npmAudit ||
      reportJson._npmAuditAnalysis ||
      browserNpmAudit ||
      null,
    compliance:
      embeddedResults.compliance ||
      reportJson._complianceAnalysis ||
      browserCompliance ||
      null,
    euAiAct:
      embeddedResults.euAiAct ||
      reportJson._euAiActAnalysis ||
      browserEuAiAct ||
      (reportJson.type === "simplebeacon-eu-ai-act-sprint"
        ? reportJson
        : null) ||
      null,
  };
  ...
  logger.info(
    "[buildReportBundle] results keys with data:",
    Object.keys(analysisResults).filter((k) => !!analysisResults[k]),
  );
  ...
  let zipFilename;
  try {
    const { stream, filename } = await buildAnalyzeExportZipStream(
      completeScanPayload,
      {
        ...
        includeEuAiAct:
          tierLabel === "euai2499" ||
          record.licenseTier === "euai2499" ||
          record.licenseTier === "euai" ||
          record.licenseTier === "universal",
      },
    );
    ...
    logger.warn(
      "[Reports] Full export ZIP failed, falling back to minimal:",
      zipErr.message,
      zipErr.code || "",
    );
    ...
    const slug = String(
      reportJson.projectRoot || reportJson.scanTargetRoot || "project",
    )
      .replace(/[\\/]/g, "-")
      .replace(/[^a-zA-Z0-9_-]/g, "-")
      .slice(0, 40);
    const root = `simplebeacon-export-${tierLabel}-${slug}-${date}`;
    archive.append(certificateHtml, {
      name: `${root}/reports/agency-certificate.html`,
    });
    if (auditReportHtml)
      archive.append(auditReportHtml, {
        name: `${root}/reports/executive-audit.html`,
      });
    if (analysisResults.euAiAct?.html) {
      archive.append(analysisResults.euAiAct.html, {
        name: `${root}/reports/eu-ai-act-audit.html`,
      });
    } else if (analysisResults.euAiAct) {
      try {
        const {
          buildEuAiActAuditReport,
        } = require("../../../server/lib/eu-ai-act-audit-report.cjs");
        const gateReport = analysisResults.simplebeacon || reportJson;
        const eu = await buildEuAiActAuditReport({
          projectPath:
            reportJson.projectRoot || reportJson.scanTargetRoot || "",
        ...
        archive.append(eu.html, {
          name: `${root}/reports/eu-ai-act-audit.html`,
        });
      } catch (euErr) {
        logger.warn(
          "[Reports] EU AI Act audit HTML generation skipped in fallback:",
          euErr.message,
        );
      }
    }
    ...
    let primaryText;
    try {
      primaryText = JSON.stringify(primaryReport, null, 2);
    } catch (e) {
      primaryText = safeStringify(primaryReport, 2);
    }
    ...
    for (const [key, meta] of Object.entries(analysisMap)) {
      if (reportJson[key]) {
        let text;
        try {
          text = JSON.stringify(reportJson[key], null, 2);
        } catch (e) {
          text = safeStringify(reportJson[key], 2);
        }
    ...
    const manifest = {
      ...
      artifactCount:
        1 + Object.entries(analysisMap).filter(([k]) => reportJson[k]).length,
    };
    let manifestText;
    try {
      manifestText = JSON.stringify(manifest, null, 2);
    } catch (e) {
      manifestText = safeStringify(manifest, 2);
    }
    archive.append(manifestText, { name: `${root}/manifest.json` });

    archive.append(
      "SimpleBeacon fallback export — analysis artifacts included.\n",
