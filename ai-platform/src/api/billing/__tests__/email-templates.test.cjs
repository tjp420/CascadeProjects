...
describe('email-templates', () => {
  describe('TIER_EMAIL_CONFIG', () => {
    test('has instant_report config', () => {
      expect(TIER_EMAIL_CONFIG.instant_report.headline).toBe('Your Report is Ready');
      expect(TIER_EMAIL_CONFIG.instant_report.tokenVisible).toBe(false);
    });
    test('has executive_clearance config', () => {
      expect(TIER_EMAIL_CONFIG.executive_clearance.headline).toBe('Payment Confirmed');
  ...
  describe('buildTierEmail', () => {
    test('returns null when no template loaded', () => {
      ...
      const result = buildTierEmail('instant_report', 'tok123', 'https://upload', 'sess_abc');
