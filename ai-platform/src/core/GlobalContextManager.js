...
class GlobalContextManager {
    ...
    async _initializeOnce(options = {}) {
        ...
        try {
            ...
            const watchEnabled =
                options.watch === true ||
                (options.watch !== false && process.env.SIMPLEBEACON_CONTEXT_WATCH === 'true');
            if (watchEnabled) {
    ...
    extractExports(content) {
        const exportRegex = /export\s+(?:(?:default\s+(?:class|function|const|let|var)\s+(\w+))|(?:\{([^}]+)\})|(?:class|function|const|let|var)\s+(\w+))/g;
        const exports = [];
        let match;
    ...
    analyzeJsonStructure(jsonData) {
        const analyze = (obj, depth = 0) => {
            ...
            if (Array.isArray(obj)) {
                ...
                return keys.length > 0 ? analyze(obj[keys[0]], depth + 1) : 'empty-object';
            } else {
                return typeof obj;
            }
        };
    ...
    async handleFileChange(filePath, changeType) {
        ...
        try {
            ...
            console.warn(`⚠️ Failed to handle file change: ${relativePath}`, error.message);
        }
    }
    ...
    search(query, options = {}) {
        ...
        for (const [relativePath, fileData] of this.context) {
            ...
            // Search in content
            if (fileData.content && fileData.content.toLowerCase().includes(searchTerm)) {
                results.push({
                    path: relativePath,
                    type: fileData.type,
            ...
            if (fileData.analysis) {
                const analysisMatches = this.searchInAnalysis(fileData.analysis, searchTerm);
                if (analysisMatches.length > 0) {
                    results.push({
                        ...
                        score: this.calculateRelevanceScore(JSON.stringify(fileData.analysis), searchTerm)
                    });
                }
            }
        }
    ...
    calculateRelevanceScore(content, searchTerm) {
        ...
        // Count occurrences
        const occurrences = (contentLower.match(new RegExp(searchLower, 'g')) || []).length;
    ...
    getStatistics() {
        ...
        for (const fileData of this.context.values()) {
            stats.categories[fileData.category] = (stats.categories[fileData.category] || 0) + 1;
