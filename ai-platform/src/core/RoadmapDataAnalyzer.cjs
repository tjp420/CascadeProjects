...
class RoadmapDataAnalyzer {
  ...
  async analyzeProjectForRoadmap() {
    ...
    logger.debug(
      "🔍 Analyzing project structure for dynamic roadmap generation...",
    );
  ...
  async analyzeProjectStructure() {
    try {
      ...
      for (const entry of entries) {
        ...
        if (
          this.includePaths.length &&
          !this.includePaths.includes(entry.name)
        ) {
          continue;
        }
        ...
        try {
          ...
          structure.totalDirectories +=
            1 + (dirAnalysis.subdirectoryCount || 0);
          structure.totalFiles += dirAnalysis.fileCount || 0;
        } catch (error) {
          structure.mainCategories[entry.name] = {
            exists: false,
            error: error.message,
          };
        }
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (stats.isDirectory()) {
          ...
          if (
            this.isKeyFile(item, ext) &&
            analysis.keyFiles.length < this.maxKeyFilesPerDir
          ) {
    ...
    try {
      ...
      for (const file of filesToProcess) {
        ...
        if (this.isCodeFile(ext)) {
          ...
          metrics.complexity[language] =
            (metrics.complexity[language] || 0) + complexity;
        }
      ...
      const totalFiles = Object.values(metrics.languages).reduce(
        (sum, count) => sum + count,
        0,
      );
      if (totalFiles > 0) {
        metrics.documentation.coverage =
          (metrics.documentation.totalDocs / totalFiles) * 100;
      }
      ...
      if (this.globalContextManager) {
        ...
        if (context.files) {
          Object.keys(context.files).forEach((filePath) => {
            ...
            if (
              dbIndicators.some((indicator) => lowerPath.includes(indicator))
            ) {
              dbAnalysis.hasDatabase = true;

              // Detect database type
              if (
                lowerPath.includes("mongodb") ||
            ...
            if (lowerPath.includes("auth"))
              dbAnalysis.features.authentication = true;
            ...
            if (lowerPath.includes("backup"))
              dbAnalysis.features.backups = true;
          });
        }
      }
  ...
  async analyzeAIIntegration() {
    try {
          ...
          Object.keys(context.files).forEach((filePath) => {
            ...
            if (
              ...
              if (content.includes("analysis"))
                integration.features.analysis = true;
              if (content.includes("generate"))
                integration.features.generation = true;
              if (content.includes("process"))
                integration.features.processing = true;
              if (content.includes("automat"))
                integration.features.automation = true;

              // Extract model names
              const modelMatches = content.match(
                /(gpt|assistant|llama|gguf|bert)/gi,
              );
              if (modelMatches) {
      ...
      integration.notes =
        "Route list omitted — regex over all files produced false positives from docs and comments";
  ...
  async generateRoadmapData(analysis) {
    const measured = await generateCodeRoadmap(
      this.projectRoot,
      {
        ...
        developmentProgress: {
          metrics: {
            documentationCoverage:
              analysis.developmentProgress?.metrics?.documentationCoverage,
    ...
    return {
      ...
      aiIntegration: {
        ...
        apis: measuredApis,
        apiRouteCount:
          measured.progressMetrics?.metrics?.apiRouteCount ??
          measuredApis.length,
        confidence: null,
        notes:
          measured.codeAnalysis?.aiIntegration?.notes ||
      ...
      recommendations: {
        ...
        longTerm:
          measured.recommendations?.longTerm ||
          analysis.recommendations?.longTerm ||
          [],
        priorities:
          measured.recommendations?.priorities ||
  ...
  async calculateDocumentationScore() {
    if (this.globalContextManager) {
      ...
      const docFiles = Object.keys(files).filter(
        (path) =>
  ...
  async calculateCategoryProgress(features) {
    return {
      "AI Tools": features.ai
        ? this.calculateFeatureScore({ ai: features.ai })
        : 85,
      Analytics: features.data
        ? this.calculateFeatureScore({ data: features.data })
        : 72,
      "Development Tools": features.devops
        ? this.calculateFeatureScore({ devops: features.devops })
        : 90,
      Infrastructure: 45,
    };
  }
  ...
  calculateCompletedFeatures(analysis) {
    const pct = this.normalizeProgressPercent(
      analysis.developmentProgress.overall,
    );
  ...
  static truncate(str, maxLen = 80, suffix = "…") {
    const s = String(str ?? "");
    const limit =
      Number.isFinite(maxLen) && maxLen > 0 ? Math.floor(maxLen) : 80;
    ...
    return [
      {
        ...
        description:
          "Core platform architecture and basic AI processing capabilities",
        features: [
          "AI Platform Setup",
          "Basic Processing",
          "Core Architecture",
