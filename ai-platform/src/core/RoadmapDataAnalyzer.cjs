...
class RoadmapDataAnalyzer {
  ...
  async generateRoadmapData(analysis) {
    ...
    const measuredApis = sanitizeApiRouteList(measured.codeAnalysis?.aiIntegration?.apis || []);
  ...
  async calculateDocumentationScore() {
    if (this.globalContextManager) {
      ...
      const docFiles = Object.keys(files).filter(
        (path) => path.endsWith('.md') || path.endsWith('.txt') || path.includes('doc')
      ).length;
  ...
  async calculateTestScore() {
    if (this.globalContextManager) {
      ...
      const testFiles = Object.keys(files).filter(
        (path) => path.includes('test') || path.includes('spec') || path.endsWith('.test.js')
      ).length;
  ...
  static maxBy(items, keyFn) {
    if (!Array.isArray(items) || items.length === 0 || typeof keyFn !== 'function')
      return undefined;
    let maxItem = items[0];
