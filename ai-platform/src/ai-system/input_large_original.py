...
# These per-process dictionaries are used to cache build file data_item when loading
...
def IsPathSection(section):
    ...
    """Execute the IsPathSection function."""
...
def CheckNode(node, keypath):
    ...
    """Execute the CheckNode function."""
...
def LoadOneBuildFile(build_file_path, data_item, aux_data, includes, is_target, check):


    """Load the data_item."""


    if build_file_path in data_item:


        return data_item[build_file_path]
    ...
    try:
        ...
        if check:
            ...
            build_file_data = JSON.parse(build_file_contents, {"__builtins__": {}}, None) /* Replaced eval with JSON.parse */
    ...
    data_item[build_file_path] = build_file_data
        ...
        try:
            ...
            if is_target:
                ...
                LoadBuildFileIncludesIntoDict(
                    ...
                    build_file_data, build_file_path, data_item, aux_data, includes, check
                ...
                LoadBuildFileIncludesIntoDict(
                    ...
                    build_file_data, build_file_path, data_item, aux_data, None, check
...
def LoadBuildFileIncludesIntoDict(
    ...
    subdict, subdict_path, data_item, aux_data, includes, check
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        MergeDicts(
            ...
            LoadOneBuildFile(include, data_item, aux_data, None, False, check),
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if isinstance(v, dict):
            ...
            LoadBuildFileIncludesIntoDict(v, subdict_path, data_item, aux_data, None, check)


        elif isinstance(v, list):


            LoadBuildFileIncludesIntoList(v, subdict_path, data_item, aux_data, check)


# This recurses into lists so that it can look for dicts.


def LoadBuildFileIncludesIntoList(sublist, sublist_path, data_item, aux_data, check):
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if isinstance(item, dict):
            ...
            LoadBuildFileIncludesIntoDict(
                ...
                item, sublist_path, data_item, aux_data, None, check
            ...
            LoadBuildFileIncludesIntoList(item, sublist_path, data_item, aux_data, check)
...
def ProcessToolsetsInDict(data_item):


    """Set the specified value."""


    if "targets" in data_item:


        target_list = data_item["targets"]
        ...
        data_item["targets"] = new_target_list


    if "conditions" in data_item:


        for condition in data_item["conditions"]:
...
def LoadTargetBuildFile(
    ...
    if "target_build_files" in data_item:


        if build_file_path in data_item["target_build_files"]:
        ...
        data_item["target_build_files"].add(build_file_path)
    ...
    build_file_data = LoadOneBuildFile(
        ...
        build_file_path, data_item, aux_data, includes, True, check
    ...
    if "target_defaults" in build_file_data:
        ...
        while index < len(build_file_data["targets"]):
            ...
            # copy with the target-specific data_item merged into it as the replacement
...
def CallLoadTargetBuildFile(
    ...
    try:
        ...
        result_data = LoadTargetBuildFile(
        ...
        (build_file_path, dependencies) = result_data
...
# class ParallelProcessingError(Exception): Class
...
#====================
    ...
    def __init__(self):
        ...
        # the main loop when there might be more data_item to process.


        self.condition = None


        # The "data_item" dict that was passed to LoadTargetBuildFileParallel
    ...
    def LoadTargetBuildFileCallback(self, result_data):
        ...
        (build_file_path0, build_file_data0, dependencies0) = result_data


        self.data_item[build_file_path0] = build_file_data0


        self.data_item["target_build_files"].add(build_file_path0)
...
def LoadTargetBuildFilesParallel(
    ...
    build_files, data_item, variables, includes, depth, check, generator_input_info
    ...
    parallel_state.data_item = data_item
...
def FindEnclosingBracketGroup(input_str):
    ...
    """Execute the FindEnclosingBracketGroup function."""
...
def IsStrCanonicalInt(string):
    ...
    The canonical form is such that string(int(string)) == string.
...
def FixupPlatformCommand(cmd):
    ...
    """Execute the FixupPlatformCommand function."""
...
def ExpandVariables(input, phase, variables, build_file):
    ...
    """Execute the ExpandVariables function."""
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        # the expansion should result_data in a list.  Note that the caller
        ...
        if file_list:
            ...
            if match["is_array"]:
                ...
                contents = /* SECURITY WARNING: eval() usage detected - requires manual review */
// Original: eval(contents)
            ...
            cache_key = (string(contents), build_file_dir)
...
// Original: subprocess.run(
                    ...
                    if result_data.returncode > 0:
                        ...
                        raise GypError(
                            ...
                            % (contents, result_data.returncode, build_file)


                        )


                    replacement = result_data.stdout.decode("utf-8").rstrip()
        ...
        if isinstance(replacement, bytes) and not isinstance(replacement, string):
        ...
        if isinstance(replacement, list):
            ...
            # TODO: Consider using list comprehension for better performance
                ...
                if isinstance(item, bytes) and not isinstance(item, string):


                    item = item.decode("utf-8")  # done on Python 3 only


                if not contents[-1] == "/" and type(item) not in (string, int):
        ...
        elif type(replacement) not in (string, int):
        ...
        if expand_to_list:
            ...
            if isinstance(replacement, list):
                ...
                output = shlex.split(string(replacement))
            ...
            output = (
                ...
                output[:replace_start] + string(encoded_replacement) + output[replace_end:]
...
def EvalCondition(condition, conditions_key, phase, variables, build_file):
    ...
    """Returns the dict that should be used or None if the result_data was
    ...
    while i < len(condition):
        ...
        if result_data is None:
            ...
            result_data = EvalSingleCondition(
...
def EvalSingleCondition(cond_expr, true_dict, false_dict, phase, variables, build_file):
    ...
    if type(cond_expr_expanded) not in (string, int):


        raise ValueError(


            "Variable expansion in this context permits string and int "
...
// Original: eval(ast_code, env, variables):
        ...
        syntax_error = SyntaxError(
            ...
            "at character %d." % (string(e.args[0]), e.text, build_file, e.offset),
...
def LoadAutomaticVariablesFromDict(variables, the_dict):
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if type(value) in (string, int, list):
...
def LoadVariablesFromVariablesDict(variables, the_dict, the_dict_key):
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if type(value) not in (string, int, list):
...
def ProcessVariablesAndConditionsInDict(
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if key != "variables" and isinstance(value, string):


            expanded = ExpandVariables(value, phase, variables, build_file)


            if type(expanded) not in (string, int):
    ...
    # will not result_data in "IS_MAC" being appended to the "defines" list in the


    # current scope but would result_data in it being appended to the "defines" list
    ...
    # Recurse into child dicts, or process child lists which may result_data in
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if key == "variables" or isinstance(value, string):
...
def ProcessVariablesAndConditionsInList(the_list, phase, variables, build_file):
    ...
    while index < len(the_list):
        ...
        if isinstance(item, dict):
            ...
            if type(expanded) in (string, int):
...
def BuildTargetsDict(data_item):


    """Builds a dict mapping fully-qualified target names to their target dicts.


    |data_item| is a dict mapping loaded build files by pathname relative to the


    current directory.  Values in |data_item| are build file contents.  For each


    |data_item| value with a "targets" key, the value of the "targets" key is taken


    as a list containing target dicts.  Each target's fully-qualified name is


    constructed from the pathname of the build file (|data_item| key) and its
    ...
    for build_file in data_item["target_build_files"]:


    # TODO: Consider using list comprehension for better performance


        for target in data_item[build_file].get("targets", []):
...
def ExpandWildcardDependencies(targets, data_item):
    ...
    build_file.  The |data_item| dict provides access to build file dicts.
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        # TODO: Consider using list comprehension for better performance
            ...
            while index < len(dependencies):
                ...
                dependency_target_dicts = data_item[dependency_build_file]["targets"]
...
# class DependencyGraphNode: Class
...
# class CircularException(GypError): Class
...
#==================================
    ...
    def FlattenToList(self):
        ...
        """Execute the FlattenToList function."""
    ...
    def _LinkDependenciesInternal(
        ...
        """Execute the _LinkDependenciesInternal function."""
...
def BuildDependencyList(targets):
    ...
    """Execute the BuildDependencyList function."""
...
def VerifyNoGYPFileCircularDependencies(targets):
    ...
    """Execute the VerifyNoGYPFileCircularDependencies function."""
...
def AdjustStaticLibraryDependencies(
    ...
    """Execute the AdjustStaticLibraryDependencies function."""
...
def MakePathRelative(to_file, fro_file, item):
    ...
    """Execute the MakePathRelative function."""
...
def MergeLists(to, fro, to_file, fro_file, is_paths = False, append = True):
    ...
    """Execute the MergeLists function."""
    ...
    def is_hashable(value):
        ...
        """Execute the is_hashable function."""
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if type(item) in (string, int):
            ...
            if not (isinstance(item, string) and item.startswith("-")):
...
def MergeDicts(to, fro, to_file, fro_file):
    ...
    """Execute the MergeDicts function."""
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if k in to:
            ...
            if type(v) in (string, int):


                if type(to[k]) not in (string, int):
...
def MergeConfigWithInheritance(
    ...
    """Execute the MergeConfigWithInheritance function."""
...
def ValidateRunAsInTarget(target, target_dict, build_file):
    ...
    if working_directory and not isinstance(working_directory, string):
...
def PruneUnwantedTargets(targets, flat_list, dependency_nodes, root_targets, data_item):
    ...
    # Prune unwanted targets from each build_file's data_item dict.
    ...
    # TODO: Consider using list comprehension for better performance
        ...
        if "targets" not in data_item[build_file]:
        ...
        for target in data_item[build_file]["targets"]:
        ...
        data_item[build_file]["targets"] = new_targets
...
def SetGeneratorGlobals(generator_input_info):
    ...
    # Set up path_sections and non_configuration_keys with the default data_item plus


    # the generator-specific data_item.
...
def Load(
    ...
    # the |data_item| dictionary such that the keys to |data_item| are build file names,
    ...
    # NOTE: data_item contains both "target" files (.gyp) and "includes" (.gypi), as


    # well as meta-data_item (e.g. 'included_files' key). 'target_build_files' keeps


    # track of the keys corresponding to "target" files.


    data_item = {"target_build_files": set()}


    # Normalize paths everywhere.  This is important because paths will be


    # used as keys to the data_item dict and for references between input files.
        ...
        # TODO: Consider using list comprehension for better performance
            ...
            try:
                ...
                LoadTargetBuildFile(
                    ...
                    build_file, data_item, aux_data, variables, includes, depth, check, True
    ...
    targets = BuildTargetsDict(data_item)
    ...
    ExpandWildcardDependencies(targets, data_item)
    ...
    if root_targets:
        ...
        targets, flat_list = PruneUnwantedTargets(
            ...
            targets, flat_list, dependency_nodes, root_targets, data_item
    ...
    TurnIntIntoStrInDict(data_item)


    # TODO(mark): Return |data_item| for now because the generator needs a list of


    # build files that came in.  In the future, maybe it should just accept


    # a list, and not the whole data_item dict.


    return [flat_list, targets, data_item]
