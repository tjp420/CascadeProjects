...
class TeamOptimizationSystem {
    ...
    findExpertsInCategory(category) {
        ...
        Object.entries(this.teamSkills).forEach(([member, skills]) => {
            const expertSkills = Object.entries(skills.skills).filter(([skill, details]) =>
            ...
            if (expertSkills.length > 0) {
                experts.push({
                    member,
                    skills: expertSkills.map(([skill, details]) => skill),
    ...
    getRecommendedTasks(member) {
        ...
        Object.entries(memberSkills.skills)
            .filter(([skill, details]) => details.level === 'EXPERT')
        ...
        // Add advanced skills
        Object.entries(memberSkills.skills)
            .filter(([skill, details]) => details.level === 'ADVANCED')
            .forEach(([skill, details]) => {
                recommendations.push({
                    skill,
                    category: details.category,
     ...
     */
    createDevelopmentTimeline(member) {
        return {
            'Month 1-2': {
