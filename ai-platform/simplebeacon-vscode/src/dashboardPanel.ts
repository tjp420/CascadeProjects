...
export class DashboardPanel implements vscode.WebviewViewProvider {
    ...
    public resolveWebviewView(
        ...
        // Start polling for scan results from any source (web dashboard, CLI, scheduled)
    ...
    public updateStats(scanResult?: { issues: number; critical: number; high: number; medium: number; low: number; score: number; timestamp?: string }) {
        if (this._view) {
    ...
    private _startReportPolling() {
        ...
        const apiUrl = config.get<string>('apiUrl', 'http://127.0.0.1:3001');
        ...
        const poll = async () => {
            ...
            this._view?.webview.postMessage({ command: 'serverStatus', online: true, hasData });
        };
...
<body>
    ...
    <script nonce="${nonce}">
        ...
        async function checkServer() {
            ...
            try {
                await fetch(apiBase + '/api/health', { method: 'GET', mode: 'no-cors' });
                dot.className = 'status-dot';
                text.textContent = 'Server online';
            } catch {
                dot.className = 'status-dot offline';
                text.textContent = 'Server offline — start with: node simplebeacon-server.cjs';
            }
        }
        checkServer();
            ...
            if (msg.command === 'serverStatus') {
                if (dot && text) {
                    if (msg.online) {
                        ...
                        dot.className = 'status-dot offline';
                        text.textContent = 'Server offline — start with: node server/index.cjs';
                    }
                }
