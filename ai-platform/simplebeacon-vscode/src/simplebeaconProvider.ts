...
class IssueItem extends vscode.TreeItem {
    constructor(
        ...
        this.description = issue.filePath ? `${issue.filePath.split(/[\\/]/).pop()}${issue.line ? `:${issue.line}` : ''}` : '';
...
export class SimpleBeaconProvider implements vscode.TreeDataProvider<IssueItem | CategoryItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<IssueItem | CategoryItem | undefined | null | void> = new vscode.EventEmitter<IssueItem | CategoryItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<IssueItem | CategoryItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    getChildren(element?: IssueItem | CategoryItem): Thenable<(IssueItem | CategoryItem)[]> {
        ...
        if (element instanceof CategoryItem) {
            ...
            return Promise.resolve(
                filtered.map(issue => new IssueItem(issue, vscode.TreeItemCollapsibleState.None))
            );
        }
