...
export class RealtimeMonitor {
    ...
    private readonly ollamaUrl: string = 'http://127.0.0.1:11434';
    ...
    private constructor() {
        ...
        this.statusBarItem = vscode.window.createStatusBarItem('simplebeacon-realtime-status',
    ...
    private initAISlopPatterns(): void {
        this.aiSlopPatterns = [
            {
                regex: /\b(Here is the implementation|In summary|As an AI|As a language model|I cannot|I apologise|I apologize|As requested|As per your request|Below is the|Please let me know if you need|Feel free to ask|I hope this helps|Let me know if you have any questions)\b/gi,
                severity: 'warning',
                type: 'ai-boilerplate',
            ...
            {
                regex: /\b(const|let|var)\s+(temp|tmp|result|res|data|dat|item|itm|value|val|obj|object|arr|array|num|number|str|string|bool|boolean|func|function|fn)\d*\s*[:=]/gi,
                severity: 'info',
            ...
            {
                regex: /(\/\/\s*(This function|This method|This class|This module|This component|This variable|This is used to|This will))\b/gi,
                severity: 'info',
            ...
            {
                regex: /\b(helper|util|utility|manager|handler|service|factory|provider|controller|middleware)\d*\s*(=|:|\(|<)/gi,
                severity: 'info',
                type: 'generic-naming',
            ...
            {
                regex: /\b(processData|handleRequest|manageState|updateUI|renderComponent|fetchData|sendRequest|getData|setData|createItem|deleteItem|updateItem)\s*\(/gi,
                severity: 'info',
                type: 'crud-generic',
    ...
    private setupEventListeners(): void {
        const textDocumentChangeDisposable = vscode.workspace.onDidChangeTextDocument((event) => {
            if (this.isMonitoring) { this.handleTextDocumentChange(event); }
        });
        const activeEditorChangeDisposable = vscode.window.onDidChangeActiveTextEditor((editor) => {
            if (this.isMonitoring && editor) { this.handleActiveEditorChange(editor); }
        });
        ...
        const checkClipboard = () => {
            ...
            Promise.resolve(vscode.env.clipboard.readText()).then((text) => {
                if (text && text !== lastText && text.length > 200) {
                    ...
                    if (pasteScore > 0.6) {
                        this.outputChannel.appendLine(`📋 Large paste detected (AI score: ${Math.round(pasteScore * 100)}%) — ${text.length} chars`);
                        vscode.window.showWarningMessage('Large AI-like paste detected. Review before committing.', 'Review', 'Ignore').then((choice) => {
                            if (choice === 'Review') {
    ...
    private scorePasteForAI(text: string): number {
        let score = 0;
        const checks = [
            { pattern: /\b(Here is the|In summary|As an AI|As a language model|I cannot|I apologize|As requested|Below is the|Please let me know if you need|Feel free to ask|I hope this helps)\b/gi, weight: 0.3 },
            { pattern: /\b(processData|handleRequest|manageState|updateUI|renderComponent|fetchData|sendRequest|getData|setData|createItem|deleteItem|updateItem)\b/gi, weight: 0.2 },
            { pattern: /\b(helper|util|utility|manager|handler|service|factory|provider|controller|middleware)\d*\b/gi, weight: 0.15 },
            { pattern: /\n\n\n+/g, weight: 0.1 },
            { pattern: /^(\s{4}|\t)\1{2,}\S/gm, weight: 0.1 },
            { pattern: /\/\/\s*(This function|This method|This class|This module|This component|This variable|This is used to|This will)/gi, weight: 0.15 }
        ];
    ...
    private handleActiveEditorChange(editor: vscode.TextEditor): void {
        if (editor.document) { this.debounceFileAnalysis(editor.document.uri.fsPath); }
    }
    ...
    private async analyzeFile(filePath: string): Promise<void> {
        try {
            ...
            if (issues.length >= 3) {
                ...
                this.queryOllamaForAI(filePath, snippet).then((ollamaIssues) => {
    ...
    private getTypeSpecificPatterns(fileExtension: string): Array<{ regex: string; severity: 'error' | 'warning' | 'info'; type: string; message: string; suggestion: string }> {
        switch (fileExtension) {
            case 'js': case 'ts': case 'jsx': case 'tsx':
                return [
                    { regex: 'var\\s+\\w+\\s*=', severity: 'info' as const, type: 'var-declaration', message: 'var declaration found', suggestion: 'Use let or const instead of var' },
                    { regex: '==\\s*["\'][^"\']+["\']|["\'][^"\']+["\']\\s*==', severity: 'warning' as const, type: 'equality-comparison', message: 'String comparison with == detected', suggestion: 'Use === for strict equality comparison' },
                    { regex: 'function\\s+\\w+\\s*\\([^)]*\\)\\s*\\{[^}]*}\\s*\\(', severity: 'info' as const, type: 'immediately-invoked-function', message: 'Immediately invoked function expression', suggestion: 'Consider using arrow functions or named functions' }
                ];
            case 'json':
                return [
                    { regex: ',\\s*[,\\s*]', severity: 'error' as const, type: 'json-trailing-comma', message: 'Trailing comma in JSON', suggestion: 'Remove trailing comma' },
                    { regex: '"[^"]*"\\s*:\\s*"[^"]*"\\s*[,\\s*]', severity: 'warning' as const, type: 'json-key-quotes', message: 'JSON key should be quoted', suggestion: 'Ensure all JSON keys are properly quoted' }
                ];
            case 'py':
                return [
                    { regex: 'print\\(', severity: 'warning' as const, type: 'print-statement', message: 'print statement found', suggestion: 'Use logging module instead of print' },
                    { regex: 'except:', severity: 'warning' as const, type: 'bare-except', message: 'Bare except clause', suggestion: 'Specify exception types to catch' }
                ];
            default:
                return [];
        }
    ...
    private async detectFileLevelIssues(filePath: string, content: string, fileExtension: string): Promise<RealtimeIssue[]> {
        const issues: RealtimeIssue[] = [];
        if (content.length > 1000000) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'warning', type: 'large-file', message: 'Large file detected', suggestion: 'Consider splitting large files for better maintainability', timestamp: new Date() });
        }
        if (content.trim().length === 0) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'empty-file', message: 'Empty file detected', suggestion: 'Add content or remove the file', timestamp: new Date() });
        }
        if (content.includes('\uFFFD')) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'warning', type: 'encoding-issue', message: 'Character encoding issue detected', suggestion: 'Check file encoding and ensure UTF-8', timestamp: new Date() });
        }
        return issues;
    }
        ...
        for (const pattern of this.aiSlopPatterns) {
            ...
            for (const match of matches) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (charCount > matchIndex) { lineNumber = i + 1; break; }
                }
                const column = match.index ? (match.index - content.lastIndexOf('\n', match.index - 1) - 1) : 1;
                issues.push({
                    file: filePath, line: lineNumber, column: column > 0 ? column : 1,
                    severity: pattern.severity, type: pattern.type,
                    message: pattern.message, suggestion: pattern.suggestion,
                    timestamp: new Date()
                });
            }
        }
    ...
    private displayIssues(issues: RealtimeIssue[]): void {
        ...
        for (const issue of issues) {
            ...
            if (issue.suggestion) { this.outputChannel.appendLine(`   💡 ${issue.suggestion}`); }

            const diagnosticSeverity = issue.severity === 'error' ? vscode.DiagnosticSeverity.Error
                : issue.severity === 'warning' ? vscode.DiagnosticSeverity.Warning
                : vscode.DiagnosticSeverity.Information;
            const range = new vscode.Range(new vscode.Position(issue.line - 1, issue.column - 1), new vscode.Position(issue.line - 1, issue.column + 20));
    ...
    private detectEntropyAnomalies(filePath: string, content: string): RealtimeIssue[] {
        ...
        if (indents.length > 10) {
            ...
            if (variance === 0) {
                issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'uniform-indent-entropy',
                    message: 'Zero indentation variance — suspiciously uniform structure',
                    suggestion: 'Check for AI-generated boilerplate or copy-pasted code', timestamp: new Date() });
            }
        }
        ...
        if (density > 0.5) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'high-comment-density',
                message: `High comment density (${Math.round(density * 100)}%) — possible AI verbosity`,
                suggestion: 'Trim redundant comments; code should be self-documenting', timestamp: new Date() });
        } else if (density === 0 && lines.length > 50) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'zero-comment-density',
                message: 'Zero comments in a large file — possible AI under-documentation',
                suggestion: 'Add strategic comments for complex logic', timestamp: new Date() });
        }
        ...
        if (lengths.length > 20) {
            ...
            if (cv < 0.15) {
                issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'uniform-line-length',
                    message: 'Suspiciously uniform line lengths — possible AI formatting',
                    suggestion: 'Review for boilerplate or auto-generated code', timestamp: new Date() });
            }
        }
    ...
    private detectASTPatterns(filePath: string, content: string, fileExtension: string): RealtimeIssue[] {
        ...
        if (tryCatchCount > 5) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'warning', type: 'excessive-try-catch',
                message: `${tryCatchCount} try-catch blocks — possible AI defensive coding pattern`,
                suggestion: 'Consolidate error handling or use higher-level error boundaries', timestamp: new Date() });
        }
        ...
        if (totalFunctions > 0) {
            ...
            if (ratio > 0.9 && totalFunctions > 10) {
                issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'arrow-function-monoculture',
                    message: `${Math.round(ratio * 100)}% arrow functions — possible AI style homogenization`,
                    suggestion: 'Mix function styles where appropriate for readability', timestamp: new Date() });
            }
        }

        if (repetitiveBlocks > 8) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'warning', type: 'repetitive-block-structure',
                message: 'Repetitive block structures detected — possible AI copy-paste pattern',
                suggestion: 'Extract common patterns into reusable functions or utilities', timestamp: new Date() });
        }
        ...
        if (chainMatches && chainMatches.length > 15) {
            issues.push({ file: filePath, line: 1, column: 1, severity: 'info', type: 'excessive-method-chaining',
                message: 'Excessive method chaining — possible AI pipeline pattern',
                suggestion: 'Break long chains into intermediate variables for debugging', timestamp: new Date() });
        }
    ...
    private async queryOllamaForAI(filePath: string, snippet: string): Promise<RealtimeIssue[]> {
        ...
        try {
            ...
            if (text.includes('YES')) {
                issues.push({ file: filePath, line: 1, column: 1, severity: 'warning', type: 'ollama-ai-suspicion',
                    message: 'Local LLM flagged this code as likely AI-generated',
                    suggestion: 'Review for AI artifacts, boilerplate, or impersonal style', timestamp: new Date() });
            }
        } catch {
    ...
    private updateStatusBar(issueCount: number): void {
        if (issueCount === 0) { this.updateStatus('🟢 Clean', 'No issues detected'); }
        else if (issueCount <= 5) { this.updateStatus('🟡 Minor', `${issueCount} issues detected`); }
        else if (issueCount <= 10) { this.updateStatus('🟠 Moderate', `${issueCount} issues detected`); }
        else { this.updateStatus('🔴 Critical', `${issueCount} issues detected`); }
    }
    ...
    private startPeriodicMonitoring(): void {
        this.monitorInterval = setInterval(() => {
            if (this.isMonitoring) { this.cleanupOldMonitors(); }
        }, 30000);
    }
    ...
    private cleanupOldMonitors(): void {
        ...
        for (const [filePath, monitor] of this.fileMonitors.entries()) {
            if (now.getTime() - monitor.lastCheck.getTime() > threshold) { this.fileMonitors.delete(filePath); }
        }
        if (this.activeIssues.size > 500) {
            ...
            for (const [key] of oldest) { this.activeIssues.delete(key); }
        }
    }
    ...
    public getActiveIssues(): RealtimeIssue[] {
        ...
        for (const issues of this.activeIssues.values()) { allIssues.push(...issues); }
