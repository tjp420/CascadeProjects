...
const PORTS = process.env.PORTS
  ? process.env.PORTS.split(",").map(Number)
  : [3001, 54358];
    ...
    try {
      ...
      reqDebug(
        ...
        req.headers && {
          ...
          "x-sb-debug": req.headers["x-sb-debug"],
        },
      );
      // Handle preflight
      ...
      reqDebug(perReqDebug, "[proxy] forward to target", {
      ...
      const proxyReq = http.request(opts, (targetRes) => {
        reqDebug(perReqDebug, "[proxy] upstream response", {
...
for (const p of PORTS) {
  ...
  server.listen(p, "127.0.0.1", () => {
    console.log(
      `proxy_local_bridge listening on http://127.0.0.1:${p} -> ${TARGET}`,
    );
  });
  server.on("error", (e) =>
    console.error(`proxy error on ${p}:`, e && e.message),
  );
}
