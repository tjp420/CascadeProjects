...
function check() {
  ...
  for (const v of required) {
    ...
    if (v.name === 'STRIPE_SECRET_KEY' && !val.startsWith('sk_live_')) invalid.push(`${v.name} (expected sk_live_)`);
    if (v.name === 'STRIPE_WEBHOOK_SECRET' && !val.startsWith('whsec_')) invalid.push(`${v.name} (expected whsec_)`);
    if (v.name === 'RESEND_API_KEY' && !val.startsWith('re_')) invalid.push(`${v.name} (expected re_)`);
  }
