...
function safeNodeId(nodeId) {
  if (!/^[a-zA-Z0-9_-]+$/.test(nodeId)) {
    throw new Error(`Invalid nodeId "${nodeId}" — only a-z, A-Z, 0-9, _, - are allowed`);
  }
}
...
function genCA() {
  ...
  if (fileExists(cert) && fileExists(key)) {
    console.log('Staging CA already exists; remove staging/mtls to regenerate.');
    return;
  }
...
function genNode(nodeId) {
  ...
  execFileSync('openssl', [
    ...
    '-out', serverCsr,
    '-nodes',
    '-subj', `/O=SimpleBeacon Staging/OU=Cluster Keyring/CN=${nodeId}`,
    '-config', CONF,
    '-reqexts', 'server_cert',
  ], { stdio: 'inherit', cwd: STAGING_DIR });
  ...
  execFileSync('openssl', [
    ...
    '-out', clientCsr,
    '-nodes',
    '-subj', `/O=SimpleBeacon Staging/OU=Cluster Keyring/CN=${nodeId}-client`,
    '-config', CONF,
    '-reqexts', 'client_cert',
  ], { stdio: 'inherit', cwd: STAGING_DIR });
  ...
  execFileSync('openssl', [
    ...
    '-inkey', serverKey,
    '-in', serverCert,
    '-certfile', path.join(CA_DIR, 'ca.crt'),
    '-out', bundle,
    '-nodes',
    '-passout', 'pass:',
  ], { stdio: 'inherit', cwd: STAGING_DIR });
...
function main() {
  ...
  if (!nodes.length) {
    ...
    console.error('Usage: node scripts/gen-staging-mtls.cjs <nodeId1> [nodeId2 ...]');
    process.exit(1);
  }
  ...
  // eslint-disable-next-line no-console
  console.log('Done. Assets written to ai-platform/staging/mtls/ (ignored by git).');
}
