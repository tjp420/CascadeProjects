...
const {
  serialize,
  deserialize,
} = require("../server/lib/keyring-serializer.cjs");
const {
  wrap,
  unwrap,
  wrapPad,
...
[16, 24, 32].forEach((kekLen) => {
  ...
  assert(
    Buffer.isBuffer(blob) && blob.length > 12,
    `KEK len ${kekLen}: serialize did not produce a valid blob`,
  );
  ...
  assert.deepStrictEqual(
    restored,
    keyring,
    `KEK len ${kekLen}: round-trip mismatch`,
  );
...
assert.throws(
  () => deserialize(magicTampered, kek),
  "Tampered magic should fail",
);
...
assert.throws(
  () => deserialize(cipherTampered, kek),
