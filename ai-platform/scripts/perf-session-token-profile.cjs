...
process.env.KEY_ROTATION_STORE_PATH = path.join(os.tmpdir(), 'sb-session-token-key-state.json');
process.env.AUDIT_LOG_PATH = path.join(os.tmpdir(), 'sb-session-token-audit.json');
...
const REVOKE_BURST_COUNT = parseInt(process.env.REVOKE_BURST_COUNT || '1000', 10);
...
function ms(start, end) { return Number((end - start) / 1000000n); }
function percentile(sorted, p) {
  const idx = Math.floor((p / 100) * (sorted.length - 1));
  return sorted[idx];
}
function snapshotMemory() { return process.memoryUsage(); }
function snapshotHeap() { return v8.getHeapStatistics(); }
function tryGc() { if (typeof global.gc === 'function') { global.gc(); return true; } return false; }
...
async function phase2PartitionedRevoke() {
  ...
  for (let i = 0; i < REVOKE_BURST_COUNT; i++) {
    const t0 = now();
    await replicator.revokeToken({ tokenHash: tokenHashes[i], tenantId: TENANT }).catch(() => {});
    const t1 = now();
    latencies.push(ms(t0, t1));
  }
...
async function phase3DeltaRecovery() {
  ...
  replicator.setBroadcast((msg) => { capturedRequest = msg; });
...
function phase4Manifest(p1, p2, p3) {
  ...
  console.log('#'.repeat(60));
  console.log(`  Login burst throughput:      ${p1.throughput.toFixed(2)} ops/s`);
  console.log(`  Revoke storm throughput:     ${p2.throughput.toFixed(2)} ops/s`);
  ...
  console.log(`    heapTotal:    ${(heap.total_heap_size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`    heapLimit:    ${(heap.heap_size_limit / 1024 / 1024).toFixed(2)} MB`);

  console.log('\n  Deployment sizing recommendations:');
  console.log(`    - Memory ceiling observed: ~${(mem.rss / 1024 / 1024).toFixed(2)} MB`);
  console.log(`    - Token replication is file-DB queued; scale vertically for write throughput`);
  console.log(`    - GC pressure: ${heap.used_heap_size / heap.heap_size_limit < 0.5 ? 'LOW' : 'MODERATE'}`);
  console.log('    - For >1,000 logins/s, replace token-db JSON queue with a real DB or in-memory cache');
}
...
async function main() {
  ...
  console.log(`PID: ${process.pid} | Node: ${process.version} | Platform: ${process.platform}`);
  console.log(`LOGIN_BURST_COUNT=${LOGIN_BURST_COUNT} REVOKE_BURST_COUNT=${REVOKE_BURST_COUNT} DELTA_TOKEN_COUNT=${DELTA_TOKEN_COUNT}`);
