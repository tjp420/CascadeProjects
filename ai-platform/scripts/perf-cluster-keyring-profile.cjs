...
process.env.KEY_ROTATION_STORE_PATH = path.join(os.tmpdir(), 'sb-perf-key-state.json');
...
const { SchnorrThresholdAggregator } = require('../server/lib/mpc/schnorr/protocol.cjs');
const { SchnorrShareEvaluator } = require('../server/lib/mpc/schnorr/signature_share.cjs');
...
function phase1DkgSaturation() {
  ...
  report('Phase 1: DKG Message Saturation', {
    ...
    heapTotalDeltaMb: ((memAfter.heapTotal - memBefore.heapTotal) / 1024 / 1024).toFixed(2),
  });
...
function phase3ThresholdSignature() {
  ...
  const binding = 3n;
  const nodeCount = 6;
  const keyShares = Array.from({ length: nodeCount }, (_, i) => BigInt(i + 1) % q);
...
function phase4Manifest(p1, p2, p3) {
  ...
  console.log('#'.repeat(60));
  console.log(`  DKG saturation throughput:        ${p1.throughput.toFixed(2)} ops/s`);
  console.log(`  Epoch drift throughput:           ${p2.throughput.toFixed(2)} ops/s`);
  console.log(`  Threshold signature throughput:   ${p3.throughput.toFixed(2)} ops/s`);
  ...
  console.log(`    - Thread/worker allocation: proportional to DKG throughput target`);
  console.log(`    - GC pressure: ${heap.used_heap_size / heap.heap_size_limit < 0.5 ? 'LOW' : 'MODERATE'}`);
  console.log('    - Threshold signing is CPU-bound by PrimeField.exp; consider worker pool for >1 kops/s');
}
