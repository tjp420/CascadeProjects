@echo off
echo 🧙‍♂️ THE UNBREAKABLE ORACLE'S ULTIMATE DESKTOP LAUNCHER
echo.
...
docker run -d --name oracle-desktop-launcher -p 32795:80 agent0ai/agent-zero:latest
if "%ERRORLEVEL%"=="0" (
...
docker exec oracle-desktop-launcher sed -i '502s/.*/# _completion = await acompletion(/' /a0/models.py
if "%ERRORLEVEL%"=="0" echo ✅ Step 1 completed successfully!

echo.
echo Step 2: Adding Ollama implementation...
docker cp enlightened_fix.py oracle-desktop-launcher:/tmp/enlightened_fix.py
docker exec oracle-desktop-launcher sed -i '502r /tmp/enlightened_fix.py' /a0/models.py
if "%ERRORLEVEL%"=="0" echo ✅ Step 2 completed successfully!

echo.
echo Step 3: Restarting container to apply changes...
docker restart oracle-desktop-launcher
...
docker ps | grep oracle-desktop-launcher
if "%ERRORLEVEL%"=="0" (
    ...
    docker logs oracle-desktop-launcher --tail 10
    echo.
...
echo.
echo 📱 Agent Zero URL: http://localhost:32795
echo.
...
set "ARGUMENTS=--new-window http://localhost:32795"
...
echo Set-WSManInstanceTimeout -Seconds 3000 > "%TEMP%\create_shortcut.ps1"
...
echo $Shortcut.TargetPath = "%TARGET_PATH%" >> "%TEMP%\create_shortcut.ps1"
echo $Shortcut.Arguments = "%ARGUMENTS%" >> "%TEMP%\create_shortcut.ps1"
echo $Shortcut.WorkingDirectory = "%WORKING_DIR%" >> "%TEMP%\create_shortcut.ps1"
echo $Shortcut.Description = "Agent Zero with Local Ollama - Ultimate AI Freedom" >> "%TEMP%\create_shortcut.ps1"
...
if "%ERRORLEVEL%"=="0" (
    ...
    echo 💡 You can manually create a shortcut to: http://localhost:32795
)
...
echo 2. Check container logs: docker logs oracle-desktop-launcher
...
echo 3. Or manually: docker restart oracle-desktop-launcher
echo.
echo 🌟 The Oracle has spoken - enjoy your ultimate AI freedom! 🌟
echo.
pause
