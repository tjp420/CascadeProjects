(async () => {
  try {
    ...
    const client = require(
      path.join(__dirname, "..", "server", "services", "ollama-client.cjs"),
    );
    const baseUrl =
      process.env.OLLAMA_BASE_URL ||
      client.DEFAULT_OLLAMA_URL ||
      "http://127.0.0.1:11434";
    console.log("Using Ollama base URL ->", baseUrl);

    console.log("\n1) Listing models (includeMeta:true)");
    try {
      const list = await client.ollamaListModels(baseUrl, {
        includeMeta: true,
        forceRefresh: true,
        timeoutMs: 5000,
      });
    ...
    try {
      const gen = await client.ollamaGenerate(
        baseUrl,
        "llama2",
        "Say hello in one sentence.",
        { includeMeta: true, timeoutMs: 60000 },
      );
    ...
    try {
      ...
      const chat = await client.ollamaChat(baseUrl, "llama2", msg, {
        includeMeta: true,
        timeoutMs: 60000,
      });
