...
const {
  sha256HexFromObject,
  canonicalize,
} = require("../server/lib/nizk/utils.cjs");
...
function verifyObject(obj) {
  ...
  const policyId =
    (obj.publicInputs && obj.publicInputs.policyId) || obj.policyId || null;

  const expected = sha256HexFromObject({
    policyId,
    proof_bundle: obj.proof_bundle,
    meta: metaWithoutProv,
  });
...
function main() {
  ...
  if (args.length < 1) {
    console.error(
      "Usage: verify-release-artifact.cjs <extracted-artifact-dir>",
    );
    process.exit(2);
  }
  const dir = args[0];
  ...
  for (const f of files) {
    ...
    for (const obj of objs) {
      if (!obj || !obj.proof_bundle || !obj.meta || !obj.meta.provenanceHash)
        continue;
      const out = verifyObject(obj);
      if (!out.ok) {
        failures++;
        console.error(
          `FAIL: ${f} expected=${out.expected} actual=${out.actual}`,
        );
      } else {
        console.log(`OK: ${f}`);
      }
