...
let siem = null;
try {
  siem = require("../server/lib/siem-exporter.cjs");
} catch (e) {
  siem = null;
}
...
const snapshotsArg = args.snapshots
  ? String(args.snapshots)
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
  : [];
const snapshotDirs = snapshotsArg.length
  ? snapshotsArg.map((p) => path.resolve(p))
  : defaultSnapshotCandidates;

const dryRun =
  typeof args["dry-run"] === "undefined"
    ? true
    : String(args["dry-run"]).toLowerCase() !== "false";
const bitLimit = args["bit-limit"] ? parseInt(args["bit-limit"], 10) : 4096;
const backupExt = args["backup-ext"] ? String(args["backup-ext"]) : ".bak";
const siemEnabled = args["siem"]
  ? String(args["siem"]).toLowerCase() !== "false"
  : false;
const quarantineDir = args["quarantine-dir"]
  ? path.resolve(args["quarantine-dir"])
  : path.join(cwd, "quarantine-legacy-transcripts");
...
function findNumericOversize(obj, pathStack = []) {
  ...
  if (Array.isArray(obj)) {
    for (let i = 0; i < obj.length; i++) {
      findings.push(
        ...findNumericOversize(obj[i], pathStack.concat([`[${i}]`])),
      );
    }
    return findings;
  }
...
function enqueueSiemIfConfigured(event) {
  ...
  try {
    ...
    log("siem.enqueue failed", e && e.message);
  }
}
...
function run() {
  ...
  for (const sdir of snapshotDirs) {
    ...
    const candidates = findFilesUnder(
      sdir,
      /snapshot|transcript|dkg|contribution|transcript|wal|commitment|transcripts|transcript/i,
    );
  ...
  if (fs.existsSync(fallbackRoot)) {
    const candidates = findFilesUnder(
      fallbackRoot,
      /snapshot|transcript|dkg|contribution|commitment/i,
    );
  ...
  // Summarize
  const summary = {
    scannedWal: fs.existsSync(walPath),
  ...
  const outPath = path.join(
    outDir,
    `legacy-transcript-report-${Date.now()}.json`,
  );
  fs.writeFileSync(
    outPath,
  ...
  if (dryRun) {
    log(
      "Dry-run mode; no files modified. To apply changes re-run with --dry-run=false",
    );
  ...
  for (const it of report) {
    if (it.source === "wal" && it.walPath) {
      ...
      const qFile = path.join(
        quarantineDir,
        `wal-${path.basename(it.walPath)}-line-${it.line}-${Date.now()}.json`,
      );
      try {
        fs.writeFileSync(qFile, lines[it.line - 1] || "", "utf8");
        log("Quarantined WAL line to", qFile);
      } catch (e) {
        log("Failed to quarantine WAL line", e && e.message);
      }
      ...
      const qFile = path.join(
        quarantineDir,
        `snapshot-${path.basename(it.file)}-${Date.now()}.json`,
      );
      try {
        fs.copyFileSync(it.file, qFile);
        log("Quarantined snapshot copy to", qFile);
      } catch (e) {
        log("Failed to quarantine snapshot", e && e.message);
      }
    }
    // emit SIEM now for applied quarantine
    const evt = {
      eventType: "NUMERIC_OVERSIZE_HISTORICAL_QUARANTINED",
  ...
  log(
    "Quarantine complete. Manual review required to decide on rewrite vs removal.",
  );
  return { summary, report, reportPath: outPath };
}
