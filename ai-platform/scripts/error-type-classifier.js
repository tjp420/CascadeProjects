...
/* SECURITY WARNING: Command execution - use subprocess.run with shell=False and validate inputs */
// Original: os.system(user_input)`,
            secure: `# Python - Safe
import subprocess
user_input = getUserInput()
/* SECURITY WARNING: Command execution - use subprocess.run with shell=False and validate inputs */
// Original: subprocess.run(['ls', user_input], shell=False)
...
if not re.match(r'^[a-zA-Z0-9_]+$', user_input):
    ...
    getXSSExamples() {
        return {
            vulnerable: `// Dangerous
element.textContent = userInput /* Replaced innerHTML with textContent for safety */
...
element.textContent = DOMPurify.sanitize(userInput) /* Replaced innerHTML with textContent for safety */`
        };
    }
...
// Original: execute("SELECT * FROM users WHERE id = %s" % user_input)`,
            secure: `# Python - Safe
cursor./* SECURITY WARNING: Potential SQL injection - use parameterized queries */
// Original: execute("SELECT * FROM users WHERE name = %s", (user_input,))
