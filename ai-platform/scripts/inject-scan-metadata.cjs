...
if (!data.scanId) {
  if (data.scan_summary && data.scan_summary.scan_id) data.scanId = data.scan_summary.scan_id;
  else data.scanId = `scan-${now}`;
}

if (!data.project) {
  if (data.projectRoot) {
    const parts = data.projectRoot.replace(/\\/g, '/').split('/').filter(Boolean);
...
if (!data.timestamp) {
  data.timestamp = data.generatedAt || (data.scan_summary && data.scan_summary.timestamp) || now;
}
...
if (typeof data.syntheticTodoMarkers === 'undefined') data.syntheticTodoMarkers = 0;
...
const wrapper = {
  ...
  profile: (data.repositoryInventory && data.repositoryInventory.profile) || data.profile || 'all',
  todoCount: typeof data.todoCount === 'number' ? data.todoCount : (typeof data.issueCount === 'number' ? data.issueCount : 0),
