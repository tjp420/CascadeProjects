...
const SiemSecurityBroker = require(
  path.join("..", "server", "lib", "siem", "siem-broker.cjs"),
);
...
const keyringSync = require(
  path.join("..", "server", "lib", "cluster-keyring-sync.cjs"),
);
const siemExporter = require(
  path.join("..", "server", "lib", "siem-exporter.cjs"),
);
const SIEMTransport = require(
  path.join("..", "server", "middleware", "transports", "siem-transport.cjs"),
);
...
process.stdout.write = function (chunk, ...args) {
  if (_suppressStdout && typeof chunk === "string" && chunk.startsWith("{"))
    return true;
...
check("broker constructs with valid config", () => {
  assert.ok(
    broker instanceof SiemSecurityBroker,
    "should be SiemSecurityBroker instance",
  );
});
...
check(
  "CRITICAL event routes to transport_winston_stream (bypass batch)",
  () => {
    batchQueue = [];
    winstonStream = [];
    broker.logEvent({
    ...
    assert.strictEqual(
      winstonStream.length,
      1,
      "winston stream should have 1 event",
    );
    assert.strictEqual(
      batchQueue.length,
      0,
      "CRITICAL should not go to batch queue",
    );
...
check("CRITICAL/FATAL bypass rate limiter even when tokens exhausted", () => {
  ...
  assert.strictEqual(
    winstonStream.length,
    1,
    "CRITICAL should bypass rate limiter",
  );
});
...
check("broker metrics track processed/dropped/bypassed", () => {
  ...
  assert.strictEqual(
    m.siem_events_processed_total,
    11,
    "processed should be 11",
  );
...
check("keyring state corruption flows to broker as CRITICAL", () => {
  ...
  const event = winstonStream.find(
...
check(
  "exporter does not receive CRITICAL events (winston_stream instead)",
  () => {
    setupBroker("HYBRID");
    siemExporter._debug.resetQueue();
    siemExporter.connectBroker(broker);
    ...
    const found = queue.find(
      (e) => e.siemCategory === "exporter_critical_test",
    );
    assert.strictEqual(
      found,
      undefined,
      "CRITICAL should not be in exporter queue",
    );
  },
);
...
check(
  "attestation HIGH event flows producer ΓåÆ broker ΓåÆ exporter queue",
  () => {
    setupBroker("HYBRID");
    siemExporter._debug.resetQueue();
    siemExporter.connectBroker(broker);
    ...
    const found = queue.find(
      (e) =>
        e.siemCategory === "attestation_nonce_mismatch" &&
        e.metadata.sandboxId === sandboxId,
    );
    assert.ok(
      found,
      "full flow: attestation event should reach exporter queue",
    );
...
check(
  "keyring CRITICAL event flows producer ΓåÆ broker ΓåÆ winston stream",
  () => {
    setupBroker("HYBRID");
    keyringSync._resetEpochState();
    keyringSync.setBroker(broker);
    ...
    assert.ok(
      found,
      "full flow: CRITICAL keyring event should reach Winston logger",
    );
...
check("Distributed: handleTokenRequest grants from surplus", () => {
  ...
  const granted = b.handleTokenRequest({
...
check("Distributed: N=3 cluster total events ≤ maxTokens", () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    b._dispatch = function (e) {
      this.emit("test_evt", e);
    };
    brokers.push(b);
  }
  ...
  for (let i = 0; i < 200; i++)
    brokers[i % 3].logEvent({ siemSeverity: "LOW", siemCategory: `E${i}` });
  assert.ok(
    total <= maxTokens,
    `total (${total}) should not exceed maxTokens (${maxTokens})`,
  );
...
check(
  "Distributed: partition fallback — node processes fair share only",
  () => {
    const b = new SiemSecurityBroker({
    ...
    for (let i = 0; i < 100; i++)
      b.logEvent({ siemSeverity: "LOW", siemCategory: `E${i}` });
    assert.strictEqual(
      processed,
      20,
      "partitioned node should process exactly fair share (20)",
    );
    b.close();
  },
);
...
const { parseSevSnpReport: _parseSevSnp, parseSgxQuote: _parseSgx } = require(
  path.join(
    "..",
    "server",
    "lib",
...
check("HW: SEV-SNP wrong MEASUREMENT rejected", () => {
  ...
  try {
    ...
    assert.ok(
      e.code === "ATTESTATION_UNTRUSTED_MEASUREMENT" ||
        e.message.includes("measurement"),
      e.message,
    );
  }
});
