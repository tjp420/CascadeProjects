...
function openPageAndNotify(pid,title,icon){
  if(window.parent && window.parent !== window && !window._inVsCodePanel){
    ...
    if(window.vscode){
      try{window.vscode.postMessage({command:pid,page:pid});}catch(e){/* ignore postMessage failures */}
    }
    return;
  }
...
function createPage(pid,title,icon,urlOverride){
  ...
  var apiBase=(window.__SB_API_URL__||window.location.origin).replace(/\/$/,'').replace(':55444',':'+DEFAULT_API_PORT);
  var knownPages={dashboard:1,analyze:1,report:1,settings:1,roadmap:1,aiContext:1,upload:1,preview:1,audit:1,security:1,trust:1,compliance:1,repositoryHealth:1,quality:1,assessments:1,platform:1,profile:1,analytics:1,certificate:1,codeMap:1,team:1};
  // Pages that should load from the local relay server instead of the external dashboard API
  var localPages={dashboard:1,analyze:1,report:1,settings:1,certificate:1,codeMap:1,roadmap:1,aiContext:1,audit:1,security:1,trust:1,compliance:1,quality:1,assessments:1,platform:1,profile:1,analytics:1,team:1,upload:1,repositoryHealth:1};
  ...
  var loadingOverlay = '<div class="sb-loading" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#1e1e1e;z-index:10;">' +
  ...
  function showError(targetUrl) {
    ...
    wrap.style.cssText = 'display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;color:#888;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;';
    ...
    hint.textContent = 'Start your dashboard server or change SimpleBeacon \u203A API Server URL in VS Code: settings to a running server.';
  ...
  function loadIframe(url) {
    ...
    lo.style.cssText = 'position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#1e1e1e;z-index:10;';
    var spin = document.createElement('div');
    spin.style.cssText = 'width:40px;height:40px;border:3px solid #333;border-top-color:#007acc;border-radius:50%;animation:sb-spin 1s linear infinite;margin-bottom:16px;';
    var txt = document.createElement('div');
    txt.style.cssText = 'color:#888;font-size:13px;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;';
  ...
  if(pid==='preview'){
    if(window.vscode){
      try{window.vscode.postMessage({command:'preview',openUrl:relayBase+'/'});}catch(e){/* ignore postMessage failures */}
    }
    if(window.parent && window.parent !== window){
      try{window.parent.postMessage({command:'preview',openUrl:relayBase+'/'},'*');}catch(e){/* ignore postMessage failures */}
    }
  ...
  if(window._inVsCodePanel && window.vscode){
    try{window.vscode.postMessage({command:'openSimpleBrowser',openUrl:targetUrl});}catch(e){/* ignore postMessage failures */}
    // Show placeholder indicating page opened in Simple Browser
    var ph = document.createElement('div');
    ph.style.cssText = 'display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;color:#888;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;';
  ...
  checkReq.onerror = function() {
    if(checked) return;
    checked = true;
    if(knownPages[pid]){ loadIframe(relayBase + '/' + pid); } else { loadIframe('about:blank'); }
  };
...
function bindCommandBtn(btn){
  function activate(){
    ...
    if (window.vscode) {
      try { window.vscode.postMessage({command: cmd}); } catch(e) { /* ignore postMessage failures */ }
    } else {
      const rp = window._relayPort || DEFAULT_RELAY_PORT;
      if (cmd === 'scan' || cmd === 'scanWorkspace') {
        fetch(window.location.origin + '/api/scan', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({command: cmd, source: 'browser'})}).catch(function(err){ /* ignore relay fetch failures */ });
      } else {
        fetch(window.location.origin + '/api/command', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({command: cmd, source: 'browser'})}).catch(function(err){ /* ignore relay fetch failures */ });
      }
    }
  }
  btn.addEventListener('click', activate);
...
(function(){
  ...
  if(showWelcome && typeof openPage === 'function'){
    try{openPage('welcome','Welcome','\u1F3E0');}catch(e){/* ignore openPage errors */}
  }
})();
  ...
  if(m&&m.command){
    ...
    if(m.command==='getServerUrl'){
      ...
      if(!window._inVsCodePanel){
        const rp=window._relayPort||DEFAULT_RELAY_PORT;
        fetch(window.location.origin+'/api/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:m.command,source:'browser'})}).catch(function(err){/* ignore relay fetch failures */});
      }
      return;
    }
    if(m.command==='requestData'){
      ...
      fetch(window.location.origin+'/api/data').then(function(r){return r.json();}).then(function(data){
        if(e.source)e.source.postMessage({command:'updateData',data:buildCodeMapData(data||{})},'*');
      }).catch(function(err){
        if(window.__vscodeApi) window.__vscodeApi.postMessage({command:'logError',message:'CodeMap relay fetch failed: '+(err&&err.message)||String(err)});
      });
      return;
    }
    if(m.command==='openTab'&&m.page){
      if(m.source==='extension'){
        openPage(m.page,m.title||m.page,m.icon||'',m.url);return;
      }
      if(m.url){
        openPage(m.page,m.title||m.page,m.icon||'',m.url);return;
      }
      ...
      var sidebarHidden=sidebarEl&&(getComputedStyle(sidebarEl).display==='none'||sidebarEl.offsetParent===null);
    ...
    if(pid){
      ...
      if(pid==='preview'){
        if(window.vscode){
          var rb=getRelayBase();
          try{window.vscode.postMessage({command:'preview',openUrl:rb+'/'});}catch(e){/* ignore postMessage failures */}
        }
        if(window.parent && window.parent !== window){
          try{window.parent.postMessage({command:'preview',openUrl:rb+'/'},'*');}catch(e){/* ignore postMessage failures */}
        }
      ...
      var sidebarHidden2=sidebarEl2&&(getComputedStyle(sidebarEl2).display==='none'||sidebarEl2.offsetParent===null);
      ...
      const t={dashboard:['Dashboard','&#x1F4CA;'],analyze:['Analyze','&#x1F50D;'],report:['Report','&#x1F4CB;'],settings:['Settings','&#x2699;'],certificate:['Certificate','&#x1F3C6;'],codeMap:['Code Map','&#x1F5FA;'],audit:['Audit','&#x1F4CB;'],security:['Security','&#x1F512;'],trust:['Trust','&#x2705;'],quality:['Quality','&#x1F3C6;'],assessments:['Assessments','&#x1F4DD;'],roadmap:['Roadmap','&#x1F6E4;'],platform:['Platform','&#x1F4C8;'],profile:['Profile','&#x1F464;'],upload:['Upload','&#x1F4E4;'],repositoryHealth:['Repo Health','&#x1F4E6;'],aiContext:['AI Context','&#x1F916;'],analytics:['Analytics','&#x1F4C8;'],team:['Team','&#x1F465;'],welcome:['Welcome','&#x1F3E0;']};const x=t[pid];if(x)openPage(pid,x[0],x[1]);
    }
    const navigationCommands = {dashboard:1, report:1, settings:1, certificate:1, codeMap:1, roadmap:1, aiContext:1, upload:1, welcome:1, navDashboard:1, navResults:1, navRepoHealth:1, navAudit:1, navSecurity:1, navQuality:1, navTrust:1, navAssessments:1, navRoadmap:1, navPlatform:1, navProfile:1, navCodeMap:1, navSettings:1, navCertificate:1, navAiContext:1, generateCertificate:1, showReport:1, showCodeMap:1, openInIde:1, openUpload:1, openDashboard:1, cert:1, openSidebarDebug:1, sendSidebarToAi:1, openTab:1, enhanced:1, realtime:1, pattern:1, health:1, toggleRealtime:1, sendToAi:1};
    ...
    if(!window._inVsCodePanel){
      ...
      if (scanCmd === 'scan' || scanCmd === 'scanWorkspace') {
        fetch(window.location.origin+'/api/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command: scanCmd, source:'browser'})}).catch(function(err){/* ignore relay fetch failures */});
      } else {
        var relayCmd = pid || m.command;
        fetch(window.location.origin+'/api/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command: relayCmd, source:'browser'})}).catch(function(err){/* ignore relay fetch failures */});
      }
    }
  }
});
