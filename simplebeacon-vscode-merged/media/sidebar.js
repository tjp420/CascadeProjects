...
if (window.vscode) {
  try {
    window.vscode.postMessage({ command: 'getServerUrl' });
  } catch (e) {
    /* ignore postMessage failures in non-webview contexts */
  }
}
// When loaded in browser context, read injected initial data immediately
if (window.__SB_INITIAL_DATA__) {
  try {
    if (window.showResults) window.showResults(window.__SB_INITIAL_DATA__);
  } catch (e) {
...
if (window.__SB_INITIAL_STATUS__) {
  try {
    if (window.setStatus) window.setStatus(window.__SB_INITIAL_STATUS__.status, window.__SB_INITIAL_STATUS__.text);
  } catch (e) {
    /* ignore initialization errors */
  }
}
if (window.__SB_API_URL__) {
  try {
    ...
    if (el) el.textContent = window.__SB_API_URL__;
    let el2 = document.getElementById('serverUrlText2');
    if (el2) el2.textContent = window.__SB_API_URL__;
  } catch (e) {
...
function switchTab(tabId) {
  ...
  if (tabId === 'codeMap' && window.vscode) {
    try {
      window.vscode.postMessage({ command: 'getCodeMap' });
    } catch (e) {
      sbLog('error', '[SB Sidebar] getCodeMap postMessage failed:', e);
    }
  }
...
function logClick(btnId, cmd) {
  try {
    ...
    if (!relayUrl) {
      return;
    } // Skip logging if no relay configured
    let xhr = new XMLHttpRequest();
    xhr.open('POST', relayUrl + '/api/command', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(
      JSON.stringify({ command: 'click', source: 'sidebar-debug', btnId: btnId, cmd: cmd, timestamp: Date.now() })
    );
  } catch (e) {
    /* simplebeacon-ignore error-swallowing — relay logging is best-effort; failures are non-critical */
  }
}
function routeCommand(cmd, payload) {
  ...
  if (window.vscode) {
    try {
      ...
      sbLog('error', '[SB Sidebar] postMessage failed for', cmd, ':', e);
    }
    return;
  }
  ...
  if (window.parent && window.parent !== window) {
    try {
      ...
      sbLog('error', '[SB Sidebar] parent postMessage failed for', cmd, ':', e);
    }
    return;
  }
...
function bindBtn(id, cmd, before) {
  ...
  if (!el) {
    sbLog('warn', '[SB Sidebar] Button not found:', id);
    return;
  }
  el.addEventListener('click', function () {
    // simplebeacon-ignore memory-leak — static webview buttons, never unmounted
    logClick(id, cmd);
    try {
      if (before) before();
    } catch (e) {
      sbLog('error', '[SB Sidebar] before callback error:', e);
    }
    routeCommand(cmd);
  });
}
...
function openPageAndNotify(pid, title, icon) {
  ...
  if (window.vscode) {
    try {
      ...
      /* VS Code: API unavailable — ignore */
    }
    return;
  }
  if (window.parent && window.parent !== window) {
    try {
      window.parent.postMessage({ command: pid, page: pid }, '*');
    } catch (e) {
      /* Parent postMessage failed — ignore */
    }
    return;
  }
  // Standalone browser fallback: try switchTab first, then urlMap
...
bindBtn('scanBtn', 'scan', function () {
  setStatus('scanning', 'Scanning workspace...');
});
bindBtn('clearBtn', 'clear', function () {
...
if (sidebarExportBtn) {
  sidebarExportBtn.addEventListener('click', exportSidebarDashboard);
}
let codeMapSidebarExportBtn = document.getElementById('codeMapSidebarExportBtn');
if (codeMapSidebarExportBtn) {
  codeMapSidebarExportBtn.addEventListener('click', exportCodeMapSidebar);
}
        ...
        if (Array.isArray(items) && items.length > 0) {
          ...
          items.forEach(function (item, i) {
            txt +=
              ...
              '] ' +
              (item.description || item.message || item.type || '') +
              '\n';
...
function exportCodeMapSidebar() {
  ...
  let sortedExts = Object.keys(extCounts).sort(function (a, b) {
  ...
  let payload = {
    ...
    extensions: sortedExts.map(function (ext) {
      return { extension: ext, count: extCounts[ext] };
    }),
  ...
  if (fmt === 'json') {
    ...
    txt += '\n--- Extensions ---\n';
    payload.extensions.forEach(function (e) {
      txt += e.extension + ': ' + e.count + '\n';
    });
    txt += '\n--- Files ---\n' + filePaths.join('\n');
    downloadFile(txt, fname + '.txt', 'text/plain');
  } else {
    ...
    if (payload.summary.gatePass != null)
      csv += 'summary,gatePass,' + (payload.summary.gatePass ? 'PASS' : 'FAIL') + '\n';
    ...
    payload.extensions.forEach(function (e) {
      csv += 'extension,"' + String(e.extension).replace(/"/g, '""') + '",' + e.count + '\n';
    });
    filePaths.forEach(function (f) {
      csv += 'file,"' + String(f).replace(/"/g, '""') + '",1\n';
    });
    downloadFile(csv, fname + '.csv', 'text/csv');
  }
}
...
if (allIssuesExportBtn) {
  allIssuesExportBtn.addEventListener('click', exportAllIssues);
}
...
function exportAllIssues() {
  ...
  if (fmt === 'json') {
    downloadFile(
      JSON.stringify({ exportDate: new Date().toISOString(), lines: lines }, null, 2),
      fname + '.json',
      'application/json'
    );
  } else if (fmt === 'csv') {
...
if (serverBadge) {
  serverBadge.addEventListener('click', function () {
    routeCommand('setServerUrl', { command: 'setServerUrl' });
  });
}
...
if (activateTeamBtn) {
  activateTeamBtn.addEventListener('click', function () {
    ...
    if (domainNav) {
      ...
      s1.textContent = '\u{1F465}';
      let s2 = document.createElement('span');
      s2.style.marginLeft = '6px';
      s2.textContent = isHidden ? 'Close Team Dashboard' : 'Open Team Dashboard';
      activateTeamBtn.appendChild(s1);
      activateTeamBtn.appendChild(s2);
    }
  });
}
  ...
  btn.addEventListener('click', function (e) {
    ...
    if (pageCmdMap[cmd] && !_isRealVsCode && window.parent !== window) {
      try {
        ...
        /* Ignore cross-origin postMessage failures — parent may be a different origin */
      }
      return;
    }
    routeCommand(cmd);
  ...
  try {
    ...
    if (statScore)
      statScore.textContent =
        report.qualityScore !== null && report.qualityScore !== undefined ? report.qualityScore : '?';
...
function populatePages(report) {
  ...
  for (let _si = 0; _si < severities.length; _si++) {
    ...
    let items = allFindings.filter(function (f) {
      return f.sev === s;
    });
      ...
      itemDiv.addEventListener('click', function () {
        try {
          window.vscode.postMessage({
            ...
            line: parseInt(this.getAttribute('data-line'), 10),
          });
        } catch (e) {
          sbLog('error', e);
        }
  ...
  if (listScore) {
    ...
    scoreItem.className = 'page-item';
    scoreItem.textContent = 'Quality Score: ' + score + '/100';
    listScore.appendChild(scoreItem);
    let gateItem = document.createElement('div');
    gateItem.className = 'page-item';
    gateItem.textContent = 'Gate Status: ' + gate;
    listScore.appendChild(gateItem);
    let findingsItem = document.createElement('div');
    findingsItem.className = 'page-item';
    findingsItem.textContent = 'Total Findings: ' + (report.issueCount || 0);
    listScore.appendChild(findingsItem);
    let filesItem = document.createElement('div');
    filesItem.className = 'page-item';
    filesItem.textContent = 'Files Analyzed: ' + (report.ruleScopedFilesAnalyzed || report.filesAnalyzed || 0);
    listScore.appendChild(filesItem);
  }
...
function populateCodeMap(report) {
  ...
  let files =
    ...
    report.fileList ||
    report.sampleFiles ||
    (report.summary && report.summary.fileList) ||
    report.scannedFiles ||
    [];
  let filePaths = Array.isArray(files) ? files : [];
  ...
  let sortedExts = extKeys.sort(function (a, b) {
    return (extCounts[b] || 0) - (extCounts[a] || 0);
  });
    ...
    if (filePaths.length === 0) {
      ...
      if (total > 0) {
        emptyDiv.textContent =
          'Scanned ' + total.toLocaleString() + ' files. Directory tree requires a full scan with --full.';
      } else {
...
window.addEventListener('message', function (e) {
  try {
    ...
    if (msg.command === 'relayCommand' && msg.relayCommand) {
      ...
      if (statusText) {
        statusText.textContent = 'Browser: ' + msg.relayCommand;
      }
    }
  } catch (err) {
...
function renderExplorer() {
  ...
  downloadedFiles.forEach(function (f, idx) {
    ...
    btnOpen.title = 'Open';
    btnOpen.textContent = '\u{1F4C2}';
    let btnReveal = document.createElement('button');
    btnReveal.className = 'file-btn';
    btnReveal.title = 'Reveal in Explorer';
    btnReveal.textContent = '\u{1F4C1}';
    let btnCopy = document.createElement('button');
    btnCopy.className = 'file-btn';
    btnCopy.title = 'Copy Path';
    btnCopy.textContent = '\u{1F4CB}';
    let btnRemove = document.createElement('button');
    btnRemove.className = 'file-btn';
    btnRemove.title = 'Remove';
    btnRemove.textContent = '\u{1F5D1}';
        ...
        if (bi === 0) {
          if (!f.path) {
            try {
              window.vscode.postMessage({
                command: 'showInfo',
                message: 'Browser downloads cannot be opened from the sidebar. Use your OS file manager.',
              });
            } catch (e) {
          ...
          try {
            window.vscode.postMessage({ command: 'openFolder', file: f.path });
          } catch (e) {
            /* ignore postMessage failures */
          }
        } else if (bi === 2) {
          ...
          try {
            window.vscode.postMessage({ command: 'copyPath', path: copyValue });
          } catch (e) {
            /* ignore postMessage failures */
          }
        } else if (bi === 3) {
...
(function initArchInspector() {
  ...
  let details = {
    web: {
      ...
      status: 'Operational',
      uptime: '99.99%',
      desc: 'Primary user-facing dashboard. Served via CDN with edge caching.',
    },
    mobile: {
      name: 'Mobile App',
      type: 'React Native',
      status: 'Operational',
      uptime: '99.95%',
      desc: 'Cross-platform mobile client. Supports iOS and Android.',
    },
    vscode: {
      name: 'VS Code: Extension',
      type: 'TypeScript / VS Code: API',
      status: 'Operational',
      uptime: '99.9%',
      desc: 'Sidebar provider and command palette integration.',
    },
    api: {
      name: 'API Gateway',
      ...
      status: 'Degraded',
      uptime: '99.5%',
      desc: 'Rate-limited reverse proxy. Currently experiencing elevated latency.',
    },
    auth: {
      name: 'Auth Service',
      type: 'OAuth2 / JWT',
      status: 'Operational',
      uptime: '99.99%',
      desc: 'Token issuance, validation, and session management.',
    },
    scan: {
      ...
      status: 'Operational',
      uptime: '99.9%',
      desc: 'Asynchronous file analysis engine. Queue-based processing.',
    },
    db: {
      name: 'PostgreSQL',
      type: 'RDS / Primary',
      status: 'Operational',
      uptime: '99.99%',
      desc: 'Relational datastore for reports, users, and settings.',
    },
    cache: {
      name: 'Redis',
      type: 'ElastiCache',
      status: 'Operational',
      uptime: '99.99%',
      desc: 'In-memory cache for scan results and session tokens.',
    },
    storage: {
      ...
      status: 'Operational',
      uptime: '99.99%',
      desc: 'Blob storage for exported reports and certificates.',
    },
  };
  ...
  catBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      groups.forEach(function (g) {
        g.classList.toggle('active', g.id === 'cat-' + cat);
      });
    });
  });
})();
...
(function initAiContext() {
  ...
  if (catBtns.length) {
    catBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        groups.forEach(function (g) {
          g.classList.toggle('active', g.id === 'ai-ctx-' + cat);
        });
      });
    });
  }
  ...
  if (tempSlider && tempVal) {
    tempSlider.addEventListener('input', function () {
      tempVal.textContent = (parseInt(this.value, 10) / 10).toFixed(1);
    });
  }
  ...
  if (topPSlider && topPVal) {
    topPSlider.addEventListener('input', function () {
      topPVal.textContent = (parseInt(this.value, 10) / 10).toFixed(1);
    });
  }
  ...
  if (chunkSlider && chunkVal) {
    ...
    chunkSlider.addEventListener('input', function () {
      chunkVal.textContent = chunkMap[Math.min(parseInt(this.value, 10) - 1, 9)];
    });
  }
})();
...
(function initSecurityTabs() {
  ...
  catBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      groups.forEach(function (g) {
        g.classList.toggle('active', g.id === 'sec-' + cat);
      });
    });
  });
})();
