...
(function () {
  ...
  const allNodes = GRAPH.nodes.map((n, i) => ({
    ...
    vx: 0,
    vy: 0,
    radius: Math.max(4, Math.min(14, Math.sqrt(n.size || 1) * 0.6)),
  ...
  function draw() {
    ...
    if (is3D) {
      ...
      for (const e of visEdges) {
        const p1 = project3D(e.source),
          p2 = project3D(e.target);
        const avgZ = (p1.z2 + p2.z2) / 2;
        const edgeScale = Math.max(0.2, Math.min(1.5, 800 / (800 + avgZ)));
        ctx.beginPath();
        ctx.moveTo(p1.x, p1.y);
        ctx.lineTo(p2.x, p2.y);
        ctx.strokeStyle = 'rgba(148,163,184,' + (0.05 + 0.12 * edgeScale) + ')';
        ctx.lineWidth = Math.max(0.3, 0.5 * edgeScale);
        ctx.stroke();
      }
      for (const { n, p } of projected) {
        ...
        ctx.globalAlpha = Math.max(0.4, Math.min(1, p.depthScale));
        ctx.fill();
        ctx.globalAlpha = 1;
