...
(function(){
  ...
  const allNodes = GRAPH.nodes.map((n,i) => ({id:n.id,label:n.label,group:n.group,x:W()/2+Math.cos(i*2.4)*initRadius,y:H()/2+Math.sin(i*2.4)*initRadius,vx:0,vy:0,radius:Math.max(3,Math.min(14,3+Math.log10((n.size||1)+1))),color:colors[n.group]||'#64748b',visible:true,highlighted:false,connCount:0}));
  ...
  document.getElementById('closeControlsModal')?.addEventListener('click', () => { const cm = getControlsModal(); if (cm) cm.classList.add('hidden'); stopCapture(); });
  document.getElementById('resetControlsBtn')?.addEventListener('click', () => { keyBindings = {}; for (const def of CONTROL_DEFS) keyBindings[def.id] = def.default; buildControlsGrid(); });
  document.getElementById('saveControlsBtn')?.addEventListener('click', () => { try { localStorage.setItem('codemapKeyBindings', JSON.stringify(keyBindings)); } catch (e) {} const cm = getControlsModal(); if (cm) cm.classList.add('hidden'); stopCapture(); });
  document.addEventListener('click', e => { const cm = getControlsModal(); if (cm && e.target === cm) { cm.classList.add('hidden'); stopCapture(); } });
  ...
  function draw() {
    ...
    if (is3D) {
      ...
      for (const { n, p } of projected) {
        ...
        const showLabel = labelsVisible || n === hoverNode || n === selectedNode || (p.depthScale > labelThreshold && n.connCount >= 3);
        if (showLabel && projectedR > 2) {
          const isDimmed = focusMode && connectedIds && !connectedIds.has(n.id);
          ctx.fillStyle = isDimmed ? 'rgba(226,232,240,0.2)' : '#e2e8f0';
          ctx.font = (n === selectedNode ? 'bold ' : '') + Math.max(9, Math.min(12, 9 + projectedR * 0.4)) + 'px sans-serif';
      ...
      for (const n of visNodes) {
        ...
        if (showLabels || n === hoverNode || n === selectedNode || n === crosshairNode || (scale > 0.8 && n.connCount >= 5)) {
          ctx.fillStyle = isDimmed ? 'rgba(226,232,240,0.3)' : '#e2e8f0';
          ctx.font = (n === selectedNode ? 'bold ' : '') + Math.max(9, Math.min(12, 9 + n.radius * 0.3)) + 'px sans-serif';
          ctx.textAlign = 'center';
