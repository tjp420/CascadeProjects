...
function severityForIssues(issues, defaultSev = "low") {
  ...
  if (issues.some((i) => i.severity === "high" || i.severityBand === "high"))
    return "high";
  if (
    issues.some((i) => i.severity === "medium" || i.severityBand === "medium")
  )
    return "medium";
  return defaultSev;
}
...
function decorateIssueCategories(categories = []) {
  return categories.map((cat) => ({
    ...cat,
    ...(ISSUE_CATEGORY_ICONS[cat.id]
      ? { icon: ISSUE_CATEGORY_ICONS[cat.id] }
      : {}),
  }));
}
/**
...
function buildJestNotRunNote(report, scanConclusion) {
  ...
  if (
    ...
    ((_c = report === null || report === void 0 ? void 0 : report.scanScope) ===
      null || _c === void 0
      ? void 0
    ...
    return (
      ((_a =
        entry === null || entry === void 0
          ? void 0
          : entry.totalFilesScanned) !== null && _a !== void 0
        ? _a
        : 0) >
      repositoryFiles * 2
    );
...
function buildHistoryInventoryNote(history, repositoryFiles) {
  ...
  const counts = history
    .map((entry) =>
      entry === null || entry === void 0 ? void 0 : entry.totalFilesScanned,
    )
...
function splitDocumentationPaths(docs = []) {
  ...
  const operatorDocumentationFound = all.filter(
    (doc) => !String(doc).startsWith(".simplebeacon/"),
  );
...
export function buildDashboardInsightsSummary(report, baseline, dashboardHome) {
  ...
  const healthLabel =
    totalIssues === 0 ? "Healthy" : totalIssues <= 5 ? "Review" : "Attention";
  return {
 ...
 * @returns {any}
 */
function sanitizeReportForDashboardExport(
  report,
    ...
    ? {
        euAiActSummary: {
          ...
          ...splitDocumentationPaths(
            sanitized.euAiActSummary.documentationFound,
          ),
        },
      }
    : {};
...
function sanitizeBaselineExport(baseline, context = {}) {
  ...
  const documentedExceptions = Array.isArray(clean.documentedExceptions)
    ? clean.documentedExceptions.map((item) => ({
        ...item,
        reason: item.reason
          ? normalizeSimpleBeaconBranding(item.reason)
          : item.reason,
        scope: item.scope
          ? normalizeSimpleBeaconBranding(item.scope)
          : item.scope,
      }))
...
export function buildDashboardSummary({
  ...
  const insights = buildDashboardInsightsSummary(
    report,
    baseline,
    dashboardHome,
  );
  ...
  const historyScopeNote = buildHistoryScopeNote(
    history,
    metrics.repositoryFiles,
    benchmarkScan,
  );
  const historyInventoryNote = buildHistoryInventoryNote(
    history,
    metrics.repositoryFiles,
  );
  return {
    ...
    gateBlockingCount:
      (_b = gate.blockingCount) !== null && _b !== void 0 ? _b : null,
    gateWarningCount:
      (_c = gate.warningCount) !== null && _c !== void 0 ? _c : null,
    gateFailureNote:
      gate.pass === false
    ...
    jestBaselineLabel:
      (_f = insights.jestBaselineLabel) !== null && _f !== void 0 ? _f : null,
    jestScanNote:
      (_h =
        (_g = insights.jestScanNote) !== null && _g !== void 0
          ? _g
          : jestNotRunNote) !== null && _h !== void 0
        ? _h
        : null,
    pageSpecsLabel,
    ...
    schemaCompliance:
      (_j =
        report === null || report === void 0
          ? void 0
          : report.schemaCompliance) !== null && _j !== void 0
        ? _j
        : null,
    credentialFindings:
      (_k =
        report === null || report === void 0
          ? void 0
          : report.credentialFindings) !== null && _k !== void 0
        ? _k
        : null,
    productionLeakFindings:
      (_l =
        report === null || report === void 0
          ? void 0
          : report.productionLeakFindings) !== null && _l !== void 0
        ? _l
        : null,
    qualityScore:
      (_m =
...
export function buildDashboardExportBundle({
  ...
  const repositoryFilesTotal =
    (_c =
      ...
      ? _c
      : null;
  const sanitizedReport = sanitizeReportForDashboardExport(
    report,
    projectLabel,
    { exportFilename },
  );
  const categories = decorateIssueCategories(
    buildIssueCategories(sanitizedReport || report),
  );
  ...
  return {
    ...
    baseline: sanitizeBaselineExport(baseline, {
...
export function buildVulnerabilityTrendCsv(report, history = []) {
  ...
  const lastHistoryDate = history.length
