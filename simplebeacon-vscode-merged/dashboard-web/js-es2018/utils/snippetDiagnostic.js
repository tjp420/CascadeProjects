/**
 * Client-side snippet scanner — same pattern set as /audit.html homepage diagnostic.
...
export function isScannerMetaFileName(name) {
    ...
    if (SCANNER_META_FILENAMES.has(base))
        return true;
    if (base.startsWith('cleanup-export-') && base.endsWith('.json'))
        return true;
    if (base.startsWith('fiction-digest-') && base.endsWith('.json'))
        return true;
...
function isInventoryPathLine(line) {
    ...
    if (/"path"\s*:\s*"[^"]+"/.test(trimmed))
        return true;
    if (/^"[^"]+[\\/][^"]+\.(?:js|mjs|cjs|ts|tsx|json)"\s*,?\s*$/.test(trimmed))
        return true;
    return false;
}
/**
...
function looksLikeAuditReportHtml(text, fileName) {
    ...
    if (/^SB-AUD-\d{8}-[A-Z0-9]+.*\.html$/i.test(base))
        return true;
    const sample = String(text || '').slice(0, 6000);
    if (!/<html[\s>]/i.test(sample))
        return false;
...
function shouldSkipMockPathMatch(text, match, options = {}) {
    ...
    if (isSampleSuffixDocumentationMatch(matchText))
        return true;
    if (looksLikeMarkdownContent(text, options.fileName))
        return true;
    if (isTestConfigFileName(options.fileName))
        return true;
    if (isLockfileName(options.fileName))
        return true;
    if (isMarkdownFileName(options.fileName))
        return true;
    if (isNpmBinEntryLine(line))
        return true;
    if (isPathRegistryLine(line))
        return true;
    if (isFindingRegistryLine(line))
        return true;
    if (isInventoryPathLine(line))
        return true;
    if (isSampleCatalogLine(line))
        return true;
    if (isPackageManifestPathLine(line))
        return true;
    ...
    if (/__tests__[/\\]|(?:^|[/\\'"])tests[/\\]|\/temp\/|\\temp\\|\/\.simplebeacon\/|github-cache[/\\]/i.test(lower))
        return true;
    if (/['"]tests[/\\]fixtures[/\\]/i.test(lower))
        return true;
    if (/\/scripts\/.*mock/i.test(lower))
        return true;
    return false;
}
/**
...
export function scanSnippetText(text, options = {}) {
    ...
    PATTERNS.forEach((pattern) => {
        ...
        while ((match = pattern.regex.exec(text)) !== null) {
            if (findings.length >= 12)
                break;
            if (isAllowlisted(text, match))
                continue;
            if (pattern.id === 'mock-path' && shouldSkipMockPathMatch(text, match, options))
                continue;
