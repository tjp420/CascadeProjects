...
function isEphemeralConsolidationPath(filePath) {
    ...
    if (/^\.tmp[-.]/i.test(base))
        return true;
    if (base === '.tmp-vault-cookies.txt')
        return true;
    if (base === 'cookies.txt') {
        if (!rel.includes('/'))
            return true;
        if (/\/\.?tmp/i.test(rel) || /\/vault\//i.test(rel))
            return true;
        const dir = rel.includes('/') ? rel.slice(0, rel.lastIndexOf('/')) : '';
        if (dir.split('/').some((seg) => /^\.tmp/i.test(seg)))
            return true;
    }
    return false;
}
...
function isBrowserBuildMirrorPair(pathA, pathB) {
    ...
    if (!browserRe.test(a) && !browserRe.test(b))
        return false;
    /**
     * To source.
     * @param {any} p
...
function isConsolidationExcludedPair(pathA, pathB) {
    if (isEphemeralConsolidationPath(pathA) || isEphemeralConsolidationPath(pathB))
        return true;
    if (isMonorepoPlatformAliasPair(pathA, pathB))
        return true;
    if (isBrowserBuildMirrorPair(pathA, pathB))
        return true;
    if (isIntentionalMcpExamplePair(pathA, pathB))
        return true;
    return false;
}
/**
...
function countExcludedFuzzyPairs(pairs = []) {
    ...
    for (const pair of pairs) {
        ...
        if (!pathA || !pathB)
            continue;
        if (!isConsolidationExcludedPair(pathA, pathB))
            continue;
...
function inferConsolidationScanTargetFromHints(scan, options = {}) {
    ...
    const sourceRoot = String(options.projectPath || scan.projectRoot || ((_a = scan.repositoryInventory) === null || _a === void 0 ? void 0 : _a.projectRoot) || '').replace(/\\/g, '/');
...
function resolveIntentionalPairsExcludedCount(summaryBase, pairExclusions, rawMergeCount, mergeCandidatesLength) {
    var _a, _b, _c;
    const fromPairs = (_a = pairExclusions === null || pairExclusions === void 0 ? void 0 : pairExclusions.intentionalPairsExcluded) !== null && _a !== void 0 ? _a : 0;
    const fromDiff = rawMergeCount > mergeCandidatesLength ? rawMergeCount - mergeCandidatesLength : 0;
    const fromSummary = (_b = summaryBase.intentionalPairsExcluded) !== null && _b !== void 0 ? _b : 0;
...
function reconcileLegacyConsolidationCounts(summary = {}) {
    var _a, _b, _c;
    let benchmarkCacheCandidatesExcluded = (_a = summary.benchmarkCacheCandidatesExcluded) !== null && _a !== void 0 ? _a : 0;
...
function refreshProductConsolidationScopeLimitations(scanScope, summary = {}) {
    ...
    const extra = [];
    if (benchmarkCacheCandidatesExcluded > 0) {
        extra.push(`${benchmarkCacheCandidatesExcluded} duplicate group(s) under github-cache/ or deliverables/ excluded from platform consolidation scores.`);
    }
    if (fuzzyPairsExcluded > 0) {
        extra.push(`${fuzzyPairsExcluded} near-duplicate pair(s) excluded (MCP examples, session temps, monorepo mirrors).`);
    }
...
function rewriteProductScopedText(text, benchmarkScan) {
    if (!benchmarkScan || text == null)
        return text;
    return String(text)
        .replace(/trim or archive for dashboard load/gi, 'trim or archive if no longer needed in this OSS clone')
        .replace(/Restart the dashboard server and re-run consolidation for platform-scoped counts \(~2,200 files\)\./gi, 'Re-run consolidation on ai-platform root for Simplebeacon product sample-path metrics.')
...
function consolidationCandidateTouchesExcludedExport(candidate) {
     ...
     * @returns {any}
     */
    const paths = ((candidate === null || candidate === void 0 ? void 0 : candidate.files) || []).map((file) => file.path || file.relativePath || file.name).filter(Boolean);
    if (consolidationCandidateTouchesExcluded(candidate))
        return true;
    if (paths.length === 2 && isConsolidationExcludedPair(paths[0], paths[1]))
        return true;
...
function filterConsolidationRecommendations(recommendations, benchmarkScan) {
    return (recommendations || []).filter((rec) => {
        ...
        if (files.length === 2 && isConsolidationExcludedPair(files[0], files[1]))
            return false;
        if (files.some(isEphemeralConsolidationPath))
            return false;
        if (benchmarkScan)
            return true;
...
function resolveBenchmarkConsolidationHealth(summary) {
    var _a, _b;
    const mergeN = (_a = summary === null || summary === void 0 ? void 0 : summary.mergeCandidates) !== null && _a !== void 0 ? _a : 0;
    const savings = (_b = summary === null || summary === void 0 ? void 0 : summary.potentialSavingsBytes) !== null && _b !== void 0 ? _b : 0;
    if (mergeN === 0 && savings === 0)
        return 'benchmark-hygiene-clean';
    if (mergeN > 0)
        return 'benchmark-review-merge-candidates';
...
function buildBenchmarkConsolidationExportNotes(summary, pairExclusions) {
    ...
    const notes = [];
    const intentional = Math.max((_a = pairExclusions === null || pairExclusions === void 0 ? void 0 : pairExclusions.intentionalPairsExcluded) !== null && _a !== void 0 ? _a : 0, (_b = summary === null || summary === void 0 ? void 0 : summary.intentionalPairsExcluded) !== null && _b !== void 0 ? _b : 0, ((_c = summary === null || summary === void 0 ? void 0 : summary.browserMirrorPairsExcluded) !== null && _c !== void 0 ? _c : 0)
        + ((_d = summary === null || summary === void 0 ? void 0 : summary.mcpExamplePairsExcluded) !== null && _d !== void 0 ? _d : 0));
    if (intentional > 0) {
        notes.push(`${intentional} near-duplicate pair(s) excluded as intentional CJS/browser mirrors or MCP example configs — do not merge.`);
    }
    if (((_e = summary === null || summary === void 0 ? void 0 : summary.mergeCandidates) !== null && _e !== void 0 ? _e : 0) === 0) {
        notes.push('No actionable merge candidates on this OSS clone — consolidation is inventory hygiene only.');
    }
    if (((_f = summary === null || summary === void 0 ? void 0 : summary.potentialSavingsBytes) !== null && _f !== void 0 ? _f : 0) === 0) {
...
function assembleBenchmarkConsolidationExportNotes(existingNotes = [], summary, pairExclusions) {
    ...
    const filtered = dedupeExportNotes(existingNotes).filter((note) => {
        ...
        if (/Restart the dashboard|~2,200 files|includes github-cache\/\)/i.test(text))
            return false;
...
function filterAdvancedAnalysis(analysis, benchmarkScan) {
    ...
    if (!analysis)
        return analysis;
    const fuzzyPairs = filterFuzzyPairs(((_a = analysis.fuzzyNearDuplicates) === null || _a === void 0 ? void 0 : _a.pairs) || [])
        .filter((pair) => benchmarkScan
        || (!consolidationPathTouchesExcluded(pair.fileA) && !consolidationPathTouchesExcluded(pair.fileB)));
    const patternGroups = (((_b = analysis.patternConsolidation) === null || _b === void 0 ? void 0 : _b.recommendations) || [])
...
function resolveProductConsolidationHealth(summary) {
    ...
    const savings = (_b = summary === null || summary === void 0 ? void 0 : summary.potentialSavingsBytes) !== null && _b !== void 0 ? _b : 0;
    if (mergeN === 0 && savings === 0)
        return 'clean-no-merge-candidates';
    if (mergeN > 0)
        return 'review-merge-candidates';
...
function resolveConsolidationGateContext(scan, options = {}) {
    ...
    const repositoryFilesTotal = (_h = (_f = (_d = (_b = (_a = options.repositoryFilesTotal) !== null && _a !== void 0 ? _a : gateReport.repositoryFilesTotal) !== null && _b !== void 0 ? _b : (_c = gateReport.repositoryInventory) === null || _c === void 0 ? void 0 : _c.totalFiles) !== null && _d !== void 0 ? _d : (_e = scan.hygieneSummary) === null || _e === void 0 ? void 0 : _e.gateRepositoryFilesTotal) !== null && _f !== void 0 ? _f : (_g = scan.scanScope) === null || _g === void 0 ? void 0 : _g.gateRepositoryFilesTotal) !== null && _h !== void 0 ? _h : null;
    const credentialScanned = (_p = (_m = (_k = (_j = gateReport.credentialScanned) !== null && _j !== void 0 ? _j : gateReport.productionLeakScanned) !== null && _k !== void 0 ? _k : (_l = gateReport.scanScope) === null || _l === void 0 ? void 0 : _l.productionDirsScanned) !== null && _m !== void 0 ? _m : (_o = scan.hygieneSummary) === null || _o === void 0 ? void 0 : _o.credentialScanned) !== null && _p !== void 0 ? _p : null;
    const contentScanned = (_y = (_w = (_v = (_s = (_r = (_q = gateReport.scanScope) === null || _q === void 0 ? void 0 : _q.fullDirectoryStats) === null || _r === void 0 ? void 0 : _r.contentScanned) !== null && _s !== void 0 ? _s : (_u = (_t = gateReport.scanScope) === null || _t === void 0 ? void 0 : _t.fullDirectoryStats) === null || _u === void 0 ? void 0 : _u.filesContentScanned) !== null && _v !== void 0 ? _v : gateReport.credentialScanned) !== null && _w !== void 0 ? _w : (_x = scan.hygieneSummary) === null || _x === void 0 ? void 0 : _x.contentFilesScanned) !== null && _y !== void 0 ? _y : null;
    const gateProfile = (_4 = (_2 = (_0 = (_z = gateReport.scanScope) === null || _z === void 0 ? void 0 : _z.profile) !== null && _0 !== void 0 ? _0 : (_1 = scan.scanScope) === null || _1 === void 0 ? void 0 : _1.gateRuleBundleProfile) !== null && _2 !== void 0 ? _2 : (_3 = scan.hygieneSummary) === null || _3 === void 0 ? void 0 : _3.gateRuleBundleProfile) !== null && _4 !== void 0 ? _4 : null;
    return {
        gateReport,
        ...
        contentScanned,
        gateProfile,
        fictionJsonFilesScanned: (_9 = (_7 = (_5 = gateReport.fictionJsonFilesScanned) !== null && _5 !== void 0 ? _5 : (_6 = gateReport.scanScope) === null || _6 === void 0 ? void 0 : _6.fictionJsonFilesScanned) !== null && _7 !== void 0 ? _7 : (_8 = scan.hygieneSummary) === null || _8 === void 0 ? void 0 : _8.fictionJsonFilesScanned) !== null && _9 !== void 0 ? _9 : null,
        fictionSampleFilesScanned: (_15 = (_13 = (_11 = (_10 = gateReport.fictionSampleFilesScanned) !== null && _10 !== void 0 ? _10 : gateReport.mockSampleFiles) !== null && _11 !== void 0 ? _11 : (_12 = gateReport.scanScope) === null || _12 === void 0 ? void 0 : _12.fictionSampleFilesScanned) !== null && _13 !== void 0 ? _13 : (_14 = scan.hygieneSummary) === null || _14 === void 0 ? void 0 : _14.fictionSampleFilesScanned) !== null && _15 !== void 0 ? _15 : null
    };
}
/**
...
function buildProductConsolidationHygieneSummary(summaryBase, scan, options = {}) {
    ...
    const repoTotal = (_c = (_a = summaryBase.repositoryFilesTotal) !== null && _a !== void 0 ? _a : (_b = scan.repositoryInventory) === null || _b === void 0 ? void 0 : _b.totalFiles) !== null && _c !== void 0 ? _c : null;
    const repoAudited = (_f = (_d = summaryBase.repositoryFilesAudited) !== null && _d !== void 0 ? _d : (_e = scan.scanScope) === null || _e === void 0 ? void 0 : _e.repositoryFilesAudited) !== null && _f !== void 0 ? _f : null;
    return {
        ...
        potentialSavingsBytes: (_h = summaryBase.potentialSavingsBytes) !== null && _h !== void 0 ? _h : 0,
        exactDuplicateGroups: (_j = summaryBase.exactDuplicateGroups) !== null && _j !== void 0 ? _j : 0,
        jsonFilesAnalyzed: (_m = (_k = summaryBase.jsonFilesAnalyzed) !== null && _k !== void 0 ? _k : (_l = scan.scanScope) === null || _l === void 0 ? void 0 : _l.jsonFilesAnalyzed) !== null && _m !== void 0 ? _m : null,
        sampleDataFilesAnalyzed: (_q = (_o = summaryBase.sampleDataFilesAnalyzed) !== null && _o !== void 0 ? _o : (_p = scan.scanScope) === null || _p === void 0 ? void 0 : _p.sampleDataFilesAnalyzed) !== null && _q !== void 0 ? _q : null,
        ...
        intentionalPairsExcluded: (_s = (_r = summaryBase.intentionalPairsExcluded) !== null && _r !== void 0 ? _r : summaryBase.fuzzyPairsExcluded) !== null && _s !== void 0 ? _s : 0,
        ...(gateReport.jestBaselineChecked === false || ((_t = scan.hygieneSummary) === null || _t === void 0 ? void 0 : _t.jestBaselineChecked) === false
...
function enrichProductConsolidationScanScope(scanScope, scan, options = {}) {
    ...
    return {
        ...
        ...(gateTotal != null ? { gateRepositoryFilesTotal: gateTotal } : {}),
        ...(gateProfile ? { gateRuleBundleProfile: gateProfile } : {}),
        mergeWalkFiles: (_b = (_a = summary.repositoryFilesAudited) !== null && _a !== void 0 ? _a : scanScope === null || scanScope === void 0 ? void 0 : scanScope.repositoryFilesAudited) !== null && _b !== void 0 ? _b : null,
        jsonFilesHashed: (_d = (_c = summary.jsonFilesAnalyzed) !== null && _c !== void 0 ? _c : scanScope === null || scanScope === void 0 ? void 0 : scanScope.jsonFilesAnalyzed) !== null && _d !== void 0 ? _d : null,
        sampleDataFilesAnalyzed: (_f = (_e = summary.sampleDataFilesAnalyzed) !== null && _e !== void 0 ? _e : scanScope === null || scanScope === void 0 ? void 0 : scanScope.sampleDataFilesAnalyzed) !== null && _f !== void 0 ? _f : null,
...
function buildProductConsolidationExportNotes(scan, ephemeralExcluded, context = {}) {
    ...
    const repoTotal = (_b = (_a = scan.summary) === null || _a === void 0 ? void 0 : _a.repositoryFilesTotal) !== null && _b !== void 0 ? _b : (_c = scan.repositoryInventory) === null || _c === void 0 ? void 0 : _c.totalFiles;
    const repoAudited = (_e = (_d = scan.summary) === null || _d === void 0 ? void 0 : _d.repositoryFilesAudited) !== null && _e !== void 0 ? _e : scope.repositoryFilesAudited;
    if (repoTotal != null && repoAudited != null && repoTotal !== repoAudited) {
        notes.push(`Inventory: ${Number(repoTotal).toLocaleString()} files in audit profile; merge walks used ${Number(repoAudited).toLocaleString()} audit-scoped paths.`);
    }
    ...
    const profile = ((_f = scan.repositoryInventory) === null || _f === void 0 ? void 0 : _f.profile) || scope.repositoryInventoryProfile || 'audit';
    if (gateTotal != null && repoTotal != null && gateTotal !== repoTotal) {
        notes.push(`repositoryFilesTotal (${Number(repoTotal).toLocaleString()}, ${profile} profile) — gate full-tree inventory is ${Number(gateTotal).toLocaleString()} paths.`);
    }
    ...
    const jsonN = (_h = (_g = scan.summary) === null || _g === void 0 ? void 0 : _g.jsonFilesAnalyzed) !== null && _h !== void 0 ? _h : (_j = scan.scanScope) === null || _j === void 0 ? void 0 : _j.jsonFilesAnalyzed;
    const sampleN = (_l = (_k = scan.summary) === null || _k === void 0 ? void 0 : _k.sampleDataFilesAnalyzed) !== null && _l !== void 0 ? _l : (_m = scan.scanScope) === null || _m === void 0 ? void 0 : _m.sampleDataFilesAnalyzed;
    if (jsonN != null && sampleN != null) {
        notes.push(`${Number(jsonN).toLocaleString()} JSON file(s) hashed for exact duplicates — ${Number(sampleN).toLocaleString()} under sample paths (web/data, data/roadmap).`);
    }
    if (ephemeralExcluded > 0) {
        notes.push(`${ephemeralExcluded} near-duplicate pair(s) involving vault/session cookie temps (.tmp-*, cookies.txt) excluded — not merge candidates.`);
    }
    const aliasExcluded = ((_p = (_o = scan.summary) === null || _o === void 0 ? void 0 : _o.monorepoAliasPairsExcluded) !== null && _p !== void 0 ? _p : 0)
        + ((_r = (_q = scan.summary) === null || _q === void 0 ? void 0 : _q.browserMirrorPairsExcluded) !== null && _r !== void 0 ? _r : 0)
        + ((_t = (_s = scan.summary) === null || _s === void 0 ? void 0 : _s.mcpExamplePairsExcluded) !== null && _t !== void 0 ? _t : 0);
    if (aliasExcluded > 0) {
        notes.push(`${aliasExcluded} pair(s) excluded as monorepo path aliases, browser build mirrors, or intentional MCP example configs.`);
    }
    if (((_v = (_u = scan.summary) === null || _u === void 0 ? void 0 : _u.exactDuplicateGroups) !== null && _v !== void 0 ? _v : 0) === 0 && ((_x = (_w = scan.summary) === null || _w === void 0 ? void 0 : _w.mergeCandidates) !== null && _x !== void 0 ? _x : 0) === 0) {
        notes.push('No exact duplicate groups or actionable merge candidates in this export.');
    }
    if (((_z = (_y = scan.summary) === null || _y === void 0 ? void 0 : _y.potentialSavingsBytes) !== null && _z !== void 0 ? _z : 0) === 0) {
    ...
    if (gateReport.jestBaselineChecked === false || ((_0 = scan.hygieneSummary) === null || _0 === void 0 ? void 0 : _0.jestBaselineChecked) === false) {
...
function buildProductConsolidationAiSummary(scan) {
    ...
    const repoTotal = (_a = s.repositoryFilesTotal) !== null && _a !== void 0 ? _a : (_b = scan.repositoryInventory) === null || _b === void 0 ? void 0 : _b.totalFiles;
    const repoAudited = (_c = s.repositoryFilesAudited) !== null && _c !== void 0 ? _c : (_d = scan.scanScope) === null || _d === void 0 ? void 0 : _d.repositoryFilesAudited;
    const parts = [
...
function buildBenchmarkConsolidationConclusion(scan) {
    var _a, _b, _c, _d;
    const s = scan.summary || {};
    const repoFiles = (_b = (_a = s.repositoryFilesTotal) !== null && _a !== void 0 ? _a : s.repositoryFilesAudited) !== null && _b !== void 0 ? _b : (_c = scan.repositoryInventory) === null || _c === void 0 ? void 0 : _c.totalFiles;
...
export function sanitizeConsolidationExport(scan, options = {}) {
    ...
    if (!scan || scan.type !== 'file-merger-reduction-report' || !scan.summary)
        return scan;
    ...
    const repoRaw = (_c = (_a = scan.summary.repositoryFilesTotal) !== null && _a !== void 0 ? _a : (_b = scan.repositoryInventory) === null || _b === void 0 ? void 0 : _b.totalFiles) !== null && _c !== void 0 ? _c : null;
    ...
    let reductionOpportunities = (scan.reductionOpportunities || []).filter((o) => !shouldExcludeConsolidationCandidate(o, benchmarkScan));
    ...
    const rawFuzzyPairExclusions = !benchmarkScan
        ? countExcludedFuzzyPairs(((_d = rawAdvanced === null || rawAdvanced === void 0 ? void 0 : rawAdvanced.fuzzyNearDuplicates) === null || _d === void 0 ? void 0 : _d.pairs) || [])
    ...
    const intentionalPairsExcluded = resolveIntentionalPairsExcludedCount({
        ...
        fuzzyPairsExcluded
    }, pairExclusions, rawMergeList.length, mergeCandidates.length);
    const summaryBase = {
        ...scan.summary,
        mergeCandidates: mergeCandidates.length,
        reductionOpportunities: reductionOpportunities.length,
        fuzzyNearDuplicatePairs: (_f = (_e = advancedAnalysis === null || advancedAnalysis === void 0 ? void 0 : advancedAnalysis.fuzzyNearDuplicates) === null || _e === void 0 ? void 0 : _e.pairsFound) !== null && _f !== void 0 ? _f : scan.summary.fuzzyNearDuplicatePairs,
        benchmarkCacheCandidatesExcluded,
        fuzzyPairsExcluded,
        ...(intentionalPairsExcluded > 0
            ? {
                intentionalPairsExcluded,
                browserMirrorPairsExcluded: ((_g = scan.summary.browserMirrorPairsExcluded) !== null && _g !== void 0 ? _g : 0)
                    + pairExclusions.browserMirrorPairsExcluded
                    + rawFuzzyPairExclusions.browserMirrorPairsExcluded,
                mcpExamplePairsExcluded: ((_h = scan.summary.mcpExamplePairsExcluded) !== null && _h !== void 0 ? _h : 0)
    ...
    const exportNotes = benchmarkScan
        ...
        }).concat(buildProductConsolidationExportNotes({ ...scan, summary: summaryBase, advancedAnalysis }, ephemeralFuzzyExcluded, {
            repositoryFilesTotal: (_j = options.repositoryFilesTotal) !== null && _j !== void 0 ? _j : null,
    ...
    const productScanPathsOnBenchmark = benchmarkScan
        ...
        && ((_l = (_k = scan.summary) === null || _k === void 0 ? void 0 : _k.sampleDataFilesAnalyzed) !== null && _l !== void 0 ? _l : 0) === 0;
    const result = {
        ...scan,
        ...
        ...(scan.repositoryInventory
            ? {
                repositoryInventory: {
                    ...scan.repositoryInventory,
                    projectRoot: String(scan.repositoryInventory.projectRoot || scan.projectRoot || projectPath || '').replace(/\\/g, '/')
                }
            }
            : {}),
        advancedAnalysis: benchmarkScan && (advancedAnalysis === null || advancedAnalysis === void 0 ? void 0 : advancedAnalysis.semanticHints)
            ? {
                ...advancedAnalysis,
                semanticHints: {
            ...
            ...(benchmarkScan
                ...
                : staleProductInventory
                    ? {
                        ...
                        repositoryFilesTotal: (_o = (_m = scan.scanScope) === null || _m === void 0 ? void 0 : _m.platformRepositoryFilesTotal) !== null && _o !== void 0 ? _o : null,
        ...
        aiSummary: benchmarkScan
            ...
            : (mergeCandidates.length === 0 && ((_q = summaryBase.potentialSavingsBytes) !== null && _q !== void 0 ? _q : 0) === 0
        ...
        ...(benchmarkScan
            ? {
                ...
                hygieneSummary: {
                    mergeCandidates: mergeCandidates.length,
                    potentialSavingsBytes: (_r = summaryBase.potentialSavingsBytes) !== null && _r !== void 0 ? _r : 0,
                    intentionalPairsExcluded,
                    repositoryFilesTotal: (_s = repoRaw !== null && repoRaw !== void 0 ? repoRaw : summaryBase.repositoryFilesTotal) !== null && _s !== void 0 ? _s : null,
                    jsonFilesAnalyzed: (_t = summaryBase.jsonFilesAnalyzed) !== null && _t !== void 0 ? _t : null,
            ...
            : {
                ...
                hygieneSummary: buildProductConsolidationHygieneSummary(summaryBase, scan, {
                    repositoryFilesTotal: (_u = options.repositoryFilesTotal) !== null && _u !== void 0 ? _u : null,
                    gateReport: options.gateReport || null
                })
            }),
    ...
    if (productScanPathsOnBenchmark) {
        ...
        if (result.scanScope) {
            result.scanScope = {
                ...result.scanScope,
                sampleDataPaths: [],
                sampleDataPathsOmitted: scan.scanPaths || ((_v = result.scanScope) === null || _v === void 0 ? void 0 : _v.sampleDataPaths) || []
            };
        }
    }
    if (benchmarkScan && ((_w = result.implementationPhases) === null || _w === void 0 ? void 0 : _w.length)) {
