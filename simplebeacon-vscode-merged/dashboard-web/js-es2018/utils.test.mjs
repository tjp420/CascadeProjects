...
/**
 * Unit tests for js-es2018/utils.js inline utility functions.
...
test("reverseInline copies and reverses", () => {
  assert.deepStrictEqual(reverseInline([1, 2, 3]), [3, 2, 1]);
  assert.strictEqual(reverseInline("abc"), "cba");
});

test("sort copies and sorts", () => {
...
test("flattenInline flattens to depth", () => {
  assert.deepStrictEqual(flattenInline(1, [1, [2, [3]]]), [1, 2, [3]]);
  assert.deepStrictEqual(flattenInline(2, [1, [2, [3]]]), [1, 2, 3]);
});
...
test("evolve transforms keys", () => {
  assert.deepStrictEqual(evolve({ a: (x) => x + 1 }, { a: 1, b: 2 }), {
    a: 2,
    b: 2,
  });
});
...
test("mergeDeepLeft a wins on conflicts", () => {
  assert.deepStrictEqual(mergeDeepLeft({ a: { b: 1 } }, { a: { b: 2 } }), {
    a: { b: 1 },
  });
});

test("mergeDeepRight b wins on conflicts", () => {
  assert.deepStrictEqual(mergeDeepRight({ a: { b: 1 } }, { a: { b: 2 } }), {
    a: { b: 2 },
  });
});
...
test("isElement", () => {
  // In Node.js DOM elements don't exist, so only false cases are testable
...
test("isPromise", () => {
  ...
  assert.strictEqual(isPromise({ then() {} }), false); // thenable but not Promise
...
test("isArrayBuffer", () => {
  assert.strictEqual(isArrayBuffer(new ArrayBuffer(8)), true);
  assert.strictEqual(isArrayBuffer(null), false);
});

test("isSharedArrayBuffer", () => {
  if (typeof SharedArrayBuffer !== "undefined") {
    assert.strictEqual(isSharedArrayBuffer(new SharedArrayBuffer(8)), true);
  }
  assert.strictEqual(isSharedArrayBuffer(null), false);
});

test("isDataView", () => {
  assert.strictEqual(isDataView(new DataView(new ArrayBuffer(8))), true);
  assert.strictEqual(isDataView(null), false);
});

test("isTypedArray", () => {
  assert.strictEqual(isTypedArray(new Uint8Array(8)), true);
...
// ── String helpers ─────────────────────────────────────────────
test("match returns array or null", () => {
  const result = match(/a/, "abc");
  assert.ok(Array.isArray(result));
  assert.strictEqual(result[0], "a");
  assert.strictEqual(match(/z/, "abc"), null);
});

test("replace safe", () => {
  assert.strictEqual(replace("a", "b", "abc"), "bbc");
  assert.strictEqual(replace("a", "b", null), "");
});

test("trim", () => {
  assert.strictEqual(trim("  hello  "), "hello");
  assert.strictEqual(trim(42), "");
});
...
test("startsWith", () => {
  assert.strictEqual(startsWith("he", "hello"), true);
  assert.strictEqual(startsWith("lo", "hello"), false);
});

test("endsWith", () => {
  assert.strictEqual(endsWith("lo", "hello"), true);
  assert.strictEqual(endsWith("he", "hello"), false);
});

test("includes", () => {
  assert.strictEqual(includes("ell", "hello"), true);
  assert.strictEqual(includes("xyz", "hello"), false);
});
...
test("freezeNamespace deep freezes", () => {
  const ns = freezeNamespace({ a: { b: 1 } });
  assert.strictEqual(Object.isFrozen(ns), true);
  assert.strictEqual(Object.isFrozen(ns.a), true);
});
