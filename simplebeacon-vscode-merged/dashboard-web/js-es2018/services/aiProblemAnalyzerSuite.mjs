  ...
  try {
    import('./extendedAnalyzers.mjs')
      ...
      .catch((e) => {
        ...
        try {
          if (typeof window !== 'undefined' && window.__SIMPLEBEACON_DEBUG__ && typeof console !== 'undefined')
    ...
    return input.claims
      .map((claim, index) => {
        ...
        return {
          ...
          confidence: Number(
            (_b = (_a = claim.confidence) !== null && _a !== void 0 ? _a : input.responseConfidence) !== null &&
              _b !== void 0
              ? _b
              : 0.5
          ),
    ...
    return input.roleTasks.map((row) => {
      ...
      return {
        ...
        exposureWeight: Number(
          (_b = (_a = row.exposureWeight) !== null && _a !== void 0 ? _a : row.exposure) !== null && _b !== void 0
            ? _b
            : 25
        ),
      };
  ...
  return {
    ...
    findings: [
      ...
      ...(overrideAvailabilityRate < 50
        ? [{ level: 'warn', message: 'Human override controls are not clearly documented.', code: 'MISSING_OVERRIDE' }]
        : []),
    ...
    return input.accessSegments.map((row) => {
      ...
      return {
        ...
        accessRate: clampScore(
          Number(
            (_b = (_a = row.accessRate) !== null && _a !== void 0 ? _a : row.favorableRate) !== null && _b !== void 0
              ? _b
              : 0
          )
        ),
        affordabilityScore: clampScore(Number((_c = row.affordabilityScore) !== null && _c !== void 0 ? _c : 50)),
      };
    });
  }
  return [];
  ...
  if (Array.isArray(input.marketShares) && input.marketShares.length) {
    return input.marketShares.map((row) => {
      ...
      return {
        ...
        share: clampScore(
          Number(
            (_b = (_a = row.share) !== null && _a !== void 0 ? _a : row.marketShare) !== null && _b !== void 0 ? _b : 0
          )
        ),
      };
    });
...
function runEnvironmentalImpactAnalyzer(definition, issueId, input = {}) {
  ...
  const scanDurationMs = Number(
    (_d = (_c = metrics.scanDurationMs) !== null && _c !== void 0 ? _c : metrics.dataQualityScanDurationMs) !== null &&
      _d !== void 0
      ? _d
      : 0
  );
...
function runBiasDetectionAnalyzer(definition, issueId, input = {}) {
  ...
  return {
    ...
    findings: [
      ...(risk.evidenceStatus === 'insufficient_data'
        ? [{ level: 'info', message: 'No subgroup outcomes or scan parity input supplied.', code: 'INSUFFICIENT_DATA' }]
        : []),
      ...(disparityGap > 20
  ...
  return {
    ...
    findings: [
      ...(risk.evidenceStatus === 'insufficient_data'
        ...
        : []),
      ...(promptHits
        ? [{ level: 'warn', message: `Detected ${promptHits} prompt-injection marker(s).`, code: 'INJECTION_MARKERS' }]
        : []),
      ...(responseLeakHits
        ? [
            {
  ...
  return source.map((row) => {
    ...
    return {
      claimedConfidence: normalizeConfidenceValue(
        (_b = (_a = row.claimedConfidence) !== null && _a !== void 0 ? _a : row.confidence) !== null && _b !== void 0
          ? _b
          : row.score
      ),
      correct:
...
function collectContextCheckpoints(input = {}) {
  if (Array.isArray(input.contextCheckpoints) && input.contextCheckpoints.length) {
    return input.contextCheckpoints.map((row, index) => {
      ...
      return {
        turn: Number(
          (_b = (_a = row.turn) !== null && _a !== void 0 ? _a : row.step) !== null && _b !== void 0 ? _b : index + 1
        ),
  ...
  return buildHigherBetterResult(
    ...
    [
      ...(overconfidenceRate >= 25
        ? [{ level: 'warn', message: `${overconfident} overconfidence case(s) detected.`, code: 'OVERCONFIDENCE' }]
        : []),
    ],
    [
      'Log claimed confidence alongside verification outcomes.',
  ...
  return buildHigherBetterResult(
    ...
    [
      ...
      ...(reasoningAccuracy < 60
        ? [{ level: 'warn', message: 'Reasoning benchmark accuracy is below target.', code: 'LOW_REASONING_ACCURACY' }]
        : []),
    ],
    [
      'Use fixed benchmark tasks with expected answers.',
    ...
    return input.latencySamples
      .map((row) => {
        ...
        return {
          latencyMs: Number(
            (_c =
              (_b = (_a = row.latencyMs) !== null && _a !== void 0 ? _a : row.durationMs) !== null && _b !== void 0
                ? _b
...
function runResponseLatencyAnalyzer(definition, issueId, input = {}) {
  ...
  if (isScanReportContext(input)) {
    ...
    const scanDurationMs = Number(
      (_d =
        (_b = (_a = metrics.scanDurationMs) !== null && _a !== void 0 ? _a : metrics.dataQualityScanDurationMs) !==
          null && _b !== void 0
          ? _b
...
function collectClassificationPairs(input = {}) {
  if (Array.isArray(input.classificationLabels) && input.classificationLabels.length) {
    return input.classificationLabels.map((row) => ({
      predictedPositive:
        row.predictedPositive === true || row.predicted === true || row.predicted === 'positive' || row.predicted === 1,
      actualPositive:
  ...
  if (Array.isArray(input.predictions) && Array.isArray(input.groundTruth) && input.predictions.length) {
    return input.predictions.map((pred, index) => ({
      ...
      actualPositive:
        input.groundTruth[index] === true || input.groundTruth[index] === 'positive' || input.groundTruth[index] === 1,
    }));
  }
  return [];
}
...
function runAiOutputReliabilityAnalyzer(definition, issueId, input = {}) {
  ...
  return {
    ...
    findings: [
      ...(risk.evidenceStatus === 'insufficient_data'
        ? [{ level: 'info', message: 'No AI output text supplied for reliability scan.', code: 'INSUFFICIENT_DATA' }]
        : []),
      ...flags.map((flag) => ({
  ...
  return {
    ...
    findings: [
      ...
      ...(traceCompleteness < 50
        ? [{ level: 'warn', message: 'Decision traces lack required completeness fields.', code: 'INCOMPLETE_TRACES' }]
        : []),
...
function runScalabilityAnalyzer(definition, issueId, input = {}) {
  ...
  const latencyClaim = /\bp(50|95|99)\b/i.test(text) || /\b\d+\s*ms\b/i.test(text) || Number(metrics.p95LatencyMs) > 0;
  const throughputEvidence =
    [/\bbenchmark(ed)?\b/i, /\bload test(ed)?\b/i, /\bthroughput (measured|observed)\b/i, /\b\d[\d,]*\s*rps\b/i].some(
  ...
  return {
    ...
    findings: [
      ...
      ...(throughputClaim && !throughputEvidence
        ? [{ level: 'warn', message: 'Throughput claims lack benchmark evidence.', code: 'UNVERIFIED_THROUGHPUT' }]
        : []),
...
function runGeneralizationAnalyzer(definition, issueId, input = {}) {
  ...
  const idScore = Number(
    (_b = (_a = benchmarks.inDistributionAccuracy) !== null && _a !== void 0 ? _a : benchmarks.idAccuracy) !== null &&
      _b !== void 0
      ? _b
      : NaN
  );
  ...
  const benchmarkHits = [/\bbenchmark\b/i, /\beval (set|suite)\b/i, /\bholdout\b/i, /\bcross[- ]validation\b/i].filter(
    (pattern) => pattern.test(text)
  ).length;
  let oodRetention = 50;
  ...
  return {
    ...
    findings: [
      ...
      ...(oodClaims && !benchmarkHits
        ? [{ level: 'warn', message: 'OOD claims present without benchmark references.', code: 'UNBENCHMARKED_OOD' }]
        : []),
      ...(degradationHits
        ? [
            {
  ...
  return {
    ...
    findings: [
      ...
      ...(risk.evidenceStatus === 'limited_data'
        ...
        : []),
      ...(clarityScore < 70
        ? [{ level: 'warn', message: 'Error messages are frequently non-actionable.', code: 'LOW_ERROR_CLARITY' }]
        : []),
      ...(recoveryScore < 70
...
function deriveClaimsFromContext(context = {}) {
  ...
  return unique.slice(0, 20).map((text, index) => {
    ...
    return {
      text,
      hasEvidence: /\[[^\]]+\]|\b(source|citation|evidence|according to|scan|eslint|test|verified|benchmark)\b/i.test(
        text
      ),
...
function deriveTracesFromContext(context = {}) {
  ...
  for (const item of insights.slice(0, 8)) {
    const reason =
      ...
      ((_e =
        (_d = (_c = item.understanding) === null || _c === void 0 ? void 0 : _c.layers) === null || _d === void 0
          ? void 0
...
export function collectAnalyzerInputs(context = {}) {
  ...
  const snippets = {
    ...
    logs: String(enriched.logs || ((_a = enriched.context) === null || _a === void 0 ? void 0 : _a.logs) || '').trim(),
...
export const IMPLEMENTED_ANALYZER_ISSUE_IDS = AI_SYSTEM_ISSUES.filter((issue) => {
  var _a;
  return ((_a = ANALYZER_BY_ID.get(issue.analyzerId)) === null || _a === void 0 ? void 0 : _a.status) === 'implemented';
}).map((issue) => issue.id);
/**
 * Run all analyzers.
 * @param {string} context
...
export function enrichAnalyzerContextFromCodeInsights(context = {}) {
  ...
  const report = ((_a = context.zscriptReport) === null || _a === void 0 ? void 0 : _a.report) || context.zscriptReport;
  if (
    ...
    const lines = understanding.fileInsights
      .map((item) => {
        ...
        const summary =
          ...
          ((_d =
            (_c = (_b = item.understanding) === null || _b === void 0 ? void 0 : _b.layers) === null || _c === void 0
              ? void 0
  ...
  if (report === null || report === void 0 ? void 0 : report.problem_diagnosis) {
    ...
    next.responseText = [next.responseText, diagnosis.problem, ...(diagnosis.suspected_root_causes || []).slice(0, 3)]
      .filter(Boolean)
      .join('. ');
  }
  return next;
