...
export class ChatbotView {
  ...
  async _refreshProvidersInner() {
    ...
    if (resolved.available.length === 0 && isHostedDashboard() && !isIdeDashboardSurface()) {
      ...
      const panel =
        document.getElementById('chatbot-ollama-setup') || document.getElementById('chatbot-bridge-connect-bar');
  ...
  async tryConnectLocalOllama() {
    ...
    if (statusEl) {
      ...
      if (probe.error) {
        ...
        const siteOrigin = getSiteOrigin();
        const corsLikely = probe.corsBlocked || probe.status === 403 || (isHostedDashboard() && !canUseBrowserOllama());
        if (corsLikely) {
  ...
  async saveChatbotSettings(personalitySelect, removeFiltersCheckbox, settingsPanel) {
    this.personality =
      (personalitySelect === null || personalitySelect === void 0 ? void 0 : personalitySelect.value) || 'helpful';
    this.removeFilters =
      (removeFiltersCheckbox === null || removeFiltersCheckbox === void 0 ? void 0 : removeFiltersCheckbox.checked) ||
      false;
    const modelSelect = document.getElementById('chatbot-ollama-model');
    if (modelSelect?.value) {
      this.ollamaModel = modelSelect.value.trim();
