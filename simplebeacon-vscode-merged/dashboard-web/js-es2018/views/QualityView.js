...
function extractTopUntestedFiles(coverage, limit = 5) {
  ...
  for (const [file, data] of Object.entries(coverage)) {
    if (typeof data !== 'object' || !data.s) continue;
...
function saveQualityHistory(snapshot) {
  ...
  hist.lineCoverage.push(snapshot.lineCoverage ?? null);
  hist.securityScore.push(snapshot.securityScore ?? null);
  hist.qualityScore.push(snapshot.qualityScore ?? null);
  ...
  try { localStorage.setItem('simplebeacon_quality_history', JSON.stringify(hist)); } catch { /* ignore */ }
}
...
export class QualityView {
  ...
  render() {
    ...
    const lineCov = coverage.overallCoverage ?? coverage.lineCoverage ?? null;
    const secScore = security.securityScore ?? null;
    const qualScore = quality.overallScore ?? quality.qualityScore ?? null;
    ...
    el.innerHTML = `
      ...
      <div class="qu-v3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px;">
        <div class="qu-v3-kpi kpi-cov">
          ...
          <div class="qu-v3-kpi-value" style="color:${covColor};">${lineCov ?? '—'}${lineCov != null ? '%' : ''}</div>
        ...
        <div class="qu-v3-kpi kpi-sec">
          ...
          <div class="qu-v3-kpi-value" style="color:${secColor};">${secScore ?? '—'}${secScore != null ? '/100' : ''}</div>
        ...
        <div class="qu-v3-kpi kpi-qual">
          ...
          <div class="qu-v3-kpi-value">${qualScore ?? '—'}</div>
          ...
          <div class="qu-v3-card-bd">
            <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:12px;">
              <div class="qu-v3-metric" style="margin-bottom:0;">
                ...
                <div><div class="qu-v3-metric-val">${coverage.branchCoverage ?? '—'}${coverage.branchCoverage != null ? '%' : ''}</div><div class="qu-v3-metric-label">Branch</div></div>
              </div>
              <div class="qu-v3-metric" style="margin-bottom:0;">
                <div class="qu-v3-metric-icon" style="background:rgba(99,102,241,0.15);color:#a78bfa;">⚙️</div>
                <div><div class="qu-v3-metric-val">${coverage.functionCoverage ?? '—'}${coverage.functionCoverage != null ? '%' : ''}</div><div class="qu-v3-metric-label">Function</div></div>
              </div>
              <div class="qu-v3-metric" style="margin-bottom:0;">
                <div class="qu-v3-metric-icon" style="background:rgba(6,182,212,0.15);color:#67e8f9;">📄</div>
                <div><div class="qu-v3-metric-val">${coverage.statementCoverage ?? '—'}${coverage.statementCoverage != null ? '%' : ''}</div><div class="qu-v3-metric-label">Statement</div></div>
              </div>
              <div class="qu-v3-metric" style="margin-bottom:0;">
                <div class="qu-v3-metric-icon" style="background:rgba(34,197,94,0.15);color:#4ade80;">🧪</div>
                <div><div class="qu-v3-metric-val">${coverage.passedTests ?? '—'}/${coverage.totalTests ?? '—'}</div><div class="qu-v3-metric-label">Tests</div></div>
              </div>
            </div>
        ...
        <div class="qu-v3-card">
          ...
          <div class="qu-v3-card-bd">
            <div style="display:grid;grid-template-columns:1fr;gap:10px;">
              ...
              <div class="qu-v3-metric" style="margin-bottom:0;">
                <div class="qu-v3-metric-icon" style="background:rgba(34,197,94,0.15);color:#4ade80;">📋</div>
                <div><div class="qu-v3-metric-val">${security.complianceRate ?? '—'}${security.complianceRate != null ? '%' : ''}</div><div class="qu-v3-metric-label">Compliance</div></div>
              </div>
            </div>
          </div>
        </div>
    ...
    el.querySelectorAll('.qu-v3-draft-test').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        const vscode = this._getVscodeApi();
        if (vscode) {
          try { vscode.postMessage({ command: 'sendToAI', data: payload }); showToast('Draft test request sent to AI agent', 'success'); }
          catch (err) { showToast('VS Code: API error', 'error'); }
        } else {
          try {
            await navigator.clipboard.writeText(JSON.stringify(payload, null, 2));
            showToast('Test draft payload copied — paste into AI agent', 'success');
          } catch { showToast('Failed to copy payload', 'error'); }
        }
      });
    });
