...
export class SettingsView {
    constructor(app) {
        ...
        this._safetyValve = { target: null, countdown: 0, timer: null };
        this._pathErrors = { scan: [], production: [] };
        this._profileHover = '';
    }
    loadTokenVault() {
        try {
    ...
    render() {
      ...
      <style>
        .settings-redesign .settings-hero { text-align: center; margin: var(--space-6) 0 var(--space-5); }
        .settings-redesign .settings-hero h1 { font-size: 1.75rem; font-weight: 800; margin-bottom: var(--space-2); letter-spacing: -0.02em; }
        .settings-redesign .settings-hero p { color: var(--text-muted); font-size: 0.95rem; max-width: 560px; margin: 0 auto; }
        .settings-redesign .settings-intro { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: var(--space-2); margin-bottom: var(--space-4); font-size: var(--font-size-sm); color: var(--text-muted); }
        .settings-redesign .settings-nav { position: sticky; top: 0; z-index: 10; background: var(--surface-elevated); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: var(--space-2) var(--space-3); margin-bottom: var(--space-4); display: flex; align-items: center; gap: var(--space-1); flex-wrap: wrap; }
        .settings-redesign .settings-nav-label { font-weight: 700; font-size: var(--font-size-sm); margin-right: var(--space-2); color: var(--text-secondary); }
        .settings-redesign .settings-nav-link { padding: 4px 10px; border-radius: 999px; font-size: var(--font-size-xs); text-decoration: none; color: var(--text-secondary); background: var(--surface); border: 1px solid var(--border); transition: all var(--transition); white-space: nowrap; cursor: pointer; }
        .settings-redesign .settings-nav-link:hover { background: var(--surface-hover); border-color: var(--primary); color: var(--primary); }
        .settings-redesign .settings-nav-actions { margin-left: auto; display: flex; gap: var(--space-2); align-items: center; }
        .settings-redesign .settings-dirty-hint { color: var(--warning); font-size: var(--font-size-xs); font-weight: 600; margin-bottom: var(--space-4); }
        .settings-redesign .card.settings-grid { background: var(--surface-elevated); border-radius: var(--radius-xl); border: 1px solid var(--border); }
        .settings-redesign .card-title { font-size: var(--font-size-base); font-weight: 700; color: var(--text-primary); }
        .settings-profile-chip:hover { border-color: var(--accent) !important; background: rgba(99,102,241,0.08) !important; }
        .settings-path-invalid { border-color: #f59e0b !important; box-shadow: 0 0 0 3px rgba(245,158,11,0.15) !important; }
        .settings-path-error { display:flex; flex-direction:column; gap:4px; margin-top:6px; font-size:0.72rem; color:#fbbf24; }
        .settings-destruct-btn.confirming { background: rgba(239,68,68,0.15) !important; border-color: #ef4444 !important; color: #f87171 !important; animation: destructPulse 1s ease-in-out infinite; }
        @keyframes destructPulse { 0%,100% { opacity:1; } 50% { opacity:0.7; } }
      </style>

      <div class="settings-hero">
        <h1>Settings</h1>
        <p>Scan paths, rule profiles, and AI provider keys.</p>
      </div>
      ${this.renderSettingsNav(dirty)}

      <div class="settings-intro">
        <span>Edits write to <code>.simplebeacon/config.json</code> on the server. Save before running a scan.</span>
        ${dirty ? '<span class="settings-dirty-hint">You have unsaved changes.</span>' : ''}
      </div>
      ...
      <div class="card settings-grid mb-6" id="settings-section-scan">
        ...
        <div class="settings-field">
          ...
          <div class="settings-profile-matrix" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;max-width:420px;margin-bottom:8px;">
            ${PROFILES.map((p) => {
            var _a;
            const isActive = config.profile === p;
            const preset = (_a = this.presets) === null || _a === void 0 ? void 0 : _a[p];
            const ruleNames = (preset === null || preset === void 0 ? void 0 : preset.rules) ? Object.keys(preset.rules) : [];
            return `
                <button type="button" class="settings-profile-chip ${isActive ? 'active' : ''}" data-profile="${escapeHtml(p)}"
                  onmouseenter="this.closest('.settings-redesign').querySelectorAll('[data-rule-toggle]').forEach((b)=>{const n=b.dataset.ruleToggle;const on=${JSON.stringify(ruleNames)}.includes(n);b.style.outline=on?'2px solid var(--accent)':'none';b.style.outlineOffset=on?'2px':'0';})"
                  onmouseleave="this.closest('.settings-redesign').querySelectorAll('[data-rule-toggle]').forEach((b)=>{b.style.outline='none';b.style.outlineOffset='0';})"
                  style="padding:10px 14px;border-radius:12px;border:1px solid ${isActive ? 'var(--accent)' : 'var(--border)'};background:${isActive ? 'rgba(99,102,241,0.12)' : 'var(--surface)'};color:${isActive ? 'var(--accent)' : 'var(--text-secondary)'};font-weight:700;font-size:0.82rem;cursor:pointer;text-align:center;transition:all .15s;">
                  ${escapeHtml(p.charAt(0).toUpperCase() + p.slice(1))}
                </button>
              `;
        }).join('')}
          </div>
          <span class="text-muted" style="font-size:var(--font-size-xs);">Hover a profile to preview which rules will activate. Click to apply.</span>
        </div>
        <div class="settings-field settings-field-stack">
          ...
          <textarea class="settings-textarea ${this._pathErrors.scan.length ? 'settings-path-invalid' : ''}" id="settings-scan-paths" rows="4" placeholder="One path per line">${escapeHtml(pathsToText(config.scanPaths))}</textarea>
          ${this._pathErrors.scan.length ? `<div class="settings-path-error">${this._pathErrors.scan.map((e) => `<span>⚠ ${escapeHtml(e)}</span>`).join('')}</div>` : ''}
        </div>
        <div class="settings-field settings-field-stack">
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:var(--space-2);">
          ...
          <textarea class="settings-textarea ${this._pathErrors.production.length ? 'settings-path-invalid' : ''}" id="settings-production-paths" rows="3" placeholder="One path per line">${escapeHtml(pathsToText(config.productionPaths))}</textarea>
          ${this._pathErrors.production.length ? `<div class="settings-path-error">${this._pathErrors.production.map((e) => `<span>⚠ ${escapeHtml(e)}</span>`).join('')}</div>` : ''}
    ...
    renderSettingsNav(dirty) {
      ...
      <nav class="settings-nav">
        <span class="settings-nav-label">Jump to:</span>
        ${sections.map((s) => `
          <a href="#${s.id}" class="settings-nav-link" data-scroll-to="${s.id}">
            ${escapeHtml(s.label)}
          </a>
        `).join('')}
    ...
    renderTokenSection() {
            ...
            try {
                ...
                if (parts.length === 3) {
                    ...
                    if (pl.exp) {
                        ...
                        const now = Date.now();
                        if (expMs < now)
                            healthPill = `<span class="badge badge-danger" style="margin-left:var(--space-2);font-size:0.68rem;">Expired</span>`;
                        else if (expMs - now < 24 * 60 * 60 * 1000)
                            healthPill = `<span class="badge badge-warning" style="margin-left:var(--space-2);font-size:0.68rem;">Expires Soon</span>`;
                        else
                            healthPill = `<span class="badge badge-info" style="margin-left:var(--space-2);font-size:0.68rem;">Valid</span>`;
                    }
                }
            }
            catch (_c) {
                healthPill = `<span class="badge badge-warning" style="margin-left:var(--space-2);font-size:0.68rem;">Unreadable</span>`;
            }
        ...
        <div class="settings-row" style="align-items:center;gap:var(--space-2);">
          <span class="settings-value" style="flex:1;min-width:0;">
            ...
            ${activeLabel}${healthPill}${timeLabel}
          </span>
          ${returnBtn}
      ...
      <div class="card settings-grid mb-6" id="settings-token-card">
        ...
        <div class="settings-field-actions">
          ...
          ${total > 0 ? `<button type="button" class="btn btn-ghost btn-sm settings-destruct-btn" id="settings-token-clear-vault" style="color:var(--danger);" data-confirm-target="token-clear">Clear vault (${total})</button>` : ''}
        </div>
        ${total > 0 ? `
    ...
    renderAiKeysSection() {
      ...
      <div class="card settings-grid mb-6" id="settings-ai-keys-card">
        ...
        <div class="settings-field-actions">
          ...
          <button type="button" class="btn btn-secondary btn-sm settings-destruct-btn" id="settings-ai-clear" ${this.aiKeysBusy ? 'disabled' : ''} data-confirm-target="ai-clear">
            Clear all keys
          </button>
        </div>
    ...
    bindEvents(root) {
        ...
        // Profile matrix chips
        root.querySelectorAll('[data-profile]').forEach((chip) => {
            chip.addEventListener('click', () => {
                this.applyProfilePreset(chip.dataset.profile, rerender);
            });
        });
        // Destructive action safety valves
        root.querySelectorAll('.settings-destruct-btn').forEach((btn) => {
            btn.addEventListener('click', () => {
                const target = btn.dataset.confirmTarget;
                if (this._safetyValve.target === target && this._safetyValve.countdown <= 0) {
                    // Confirmed — execute
                    this._clearSafetyValve();
                    if (target === 'ai-clear')
                        this.clearAllAiKeys(rerender);
                    if (target === 'token-clear')
                        this.confirmAndClearVault(rerender);
                    return;
                }
                // Start countdown
                this._clearSafetyValve();
                this._safetyValve = { target, countdown: 3, timer: null };
                btn.classList.add('confirming');
                btn.textContent = `Confirm? (${this._safetyValve.countdown}s…)`;
                this._safetyValve.timer = setInterval(() => {
                    this._safetyValve.countdown--;
                    if (this._safetyValve.countdown > 0) {
                        btn.textContent = `Confirm? (${this._safetyValve.countdown}s…)`;
                    }
                    else {
                        clearInterval(this._safetyValve.timer);
        ...
        const validatePaths = (id, key) => {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (!line)
                    continue;
                if (/[<>"|?*]/.test(line))
                    errors.push(`Line ${i + 1}: Invalid characters in path`);
                if (/\\{2,}/.test(line))
                    errors.push(`Line ${i + 1}: Double backslashes detected`);
                if (/^\/[A-Za-z]:/.test(line))
                    errors.push(`Line ${i + 1}: Mixed path separators`);
            }
            this._pathErrors[key] = errors;
            rerender();
        };
        let scanTimer, prodTimer;
        (_c = root.querySelector('#settings-scan-paths')) === null || _c === void 0 ? void 0 : _c.addEventListener('input', () => {
            clearTimeout(scanTimer);
            scanTimer = setTimeout(() => validatePaths('#settings-scan-paths', 'scan'), 600);
        });
        (_d = root.querySelector('#settings-production-paths')) === null || _d === void 0 ? void 0 : _d.addEventListener('input', () => {
            clearTimeout(prodTimer);
            prodTimer = setTimeout(() => validatePaths('#settings-production-paths', 'production'), 600);
        });
        (_e = root.querySelector('#settings-sample-dir')) === null || _e === void 0 ? void 0 : _e.addEventListener('input', onFieldChange);
        (_f = root.querySelector('#settings-scan-paths')) === null || _f === void 0 ? void 0 : _f.addEventListener('input', onFieldChange);
        (_g = root.querySelector('#settings-production-paths')) === null || _g === void 0 ? void 0 : _g.addEventListener('input', onFieldChange);
        (_h = root.querySelector('#settings-full-directory-scan')) === null || _h === void 0 ? void 0 : _h.addEventListener('change', onFieldChange);
        ...
        (_j = root.querySelector('#settings-reset')) === null || _j === void 0 ? void 0 : _j.addEventListener('click', () => {
            this.resetDraft();
            rerender();
            showToast('Changes discarded', 'info');
        });
        (_k = root.querySelector('#settings-export')) === null || _k === void 0 ? void 0 : _k.addEventListener('click', () => {
            const config = this.buildConfigFromDom(this.draft, root);
            downloadJson(config, 'simplebeacon-config.json');
            showToast('Config exported', 'success');
        });
        (_l = root.querySelector('#settings-save')) === null || _l === void 0 ? void 0 : _l.addEventListener('click', () => this.save(rerender));
        (_m = root.querySelector('#settings-discover-paths')) === null || _m === void 0 ? void 0 : _m.addEventListener('click', () => this.discoverPathsFromProject(rerender));
        (_o = root.querySelector('#settings-sync-paths')) === null || _o === void 0 ? void 0 : _o.addEventListener('click', () => this.syncPathsFromProject(rerender));
        (_p = root.querySelector('#settings-baseline-sync')) === null || _p === void 0 ? void 0 : _p.addEventListener('click', () => this.syncBaseline(rerender));
        (_q = root.querySelector('#settings-run-scan')) === null || _q === void 0 ? void 0 : _q.addEventListener('click', () => this.runScan(rerender));
        (_r = root.querySelector('#settings-ai-save')) === null || _r === void 0 ? void 0 : _r.addEventListener('click', () => this.saveAiKeys(root, rerender));
        (_s = root.querySelector('#settings-ai-clear')) === null || _s === void 0 ? void 0 : _s.addEventListener('click', () => this.clearAllAiKeys(rerender));
        (_t = root.querySelector('#settings-ai-test-ollama')) === null || _t === void 0 ? void 0 : _t.addEventListener('click', () => this.testOllamaConnection(root, rerender));
        (_u = root.querySelector('#settings-download-save')) === null || _u === void 0 ? void 0 : _u.addEventListener('click', () => {
        ...
        (_v = root.querySelector('#settings-token-update')) === null || _v === void 0 ? void 0 : _v.addEventListener('click', () => {
        ...
        (_w = root.querySelector('#settings-token-get')) === null || _w === void 0 ? void 0 : _w.addEventListener('click', () => {
            const discountCode = 'SB-FRIEND-25';
            const salesUrl = `https://simplebeacon.ai/checkout/tokens?code=${encodeURIComponent(discountCode)}&ref=dashboard`;
            window.open(salesUrl, '_blank', 'noopener,noreferrer');
        });
