        ...
        <div class="settings-field">
          ...
          <div class="settings-profile-matrix" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;max-width:420px;margin-bottom:8px;">
            ${PROFILES.map((p) => {
              ...
              const ruleNames = preset?.rules ? Object.keys(preset.rules) : [];
              return `
      ...
      try {
        ...
        if (parts.length === 3) {
          ...
          if (pl.exp) {
            ...
            const now = Date.now();
            if (expMs < now) healthPill = `<span class="badge badge-danger" style="margin-left:var(--space-2);font-size:0.68rem;">Expired</span>`;
            else if (expMs - now < 24 * 60 * 60 * 1000) healthPill = `<span class="badge badge-warning" style="margin-left:var(--space-2);font-size:0.68rem;">Expires Soon</span>`;
            else healthPill = `<span class="badge badge-info" style="margin-left:var(--space-2);font-size:0.68rem;">Valid</span>`;
          }
        }
      } catch { healthPill = `<span class="badge badge-warning" style="margin-left:var(--space-2);font-size:0.68rem;">Unreadable</span>`; }
 ...
 */
    ...
    root.querySelectorAll('.settings-destruct-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (this._safetyValve.target === target && this._safetyValve.countdown <= 0) {
          ...
          if (target === 'ai-clear') this.clearAllAiKeys(rerender);
          if (target === 'token-clear') this.confirmAndClearVault(rerender);
          return;
        }
        // Start countdown
        this._clearSafetyValve();
    ...
    const validatePaths = (id, key) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (!line) continue;
        if (/[<>"|?*]/.test(line)) errors.push(`Line ${i + 1}: Invalid characters in path`);
        if (/\\{2,}/.test(line)) errors.push(`Line ${i + 1}: Double backslashes detected`);
        if (/^\/[A-Za-z]:/.test(line)) errors.push(`Line ${i + 1}: Mixed path separators`);
      }
      this._pathErrors[key] = errors;
      rerender();
    };
    let scanTimer, prodTimer;
    root.querySelector('#settings-scan-paths')?.addEventListener('input', () => {
      clearTimeout(scanTimer);
      scanTimer = setTimeout(() => validatePaths('#settings-scan-paths', 'scan'), 600);
    });
    root.querySelector('#settings-production-paths')?.addEventListener('input', () => {
