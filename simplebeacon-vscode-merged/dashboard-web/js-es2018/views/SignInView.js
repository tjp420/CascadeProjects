...
export class SignInView {
  ...
  async handleEmailSubmit(e) {
    ...
    if (this._emailMode === 'register' ? !password || password.length < 8 : !password) {
      if (errorEl) {
        errorEl.textContent =
          this._emailMode === 'register' ? 'Password must be at least 8 characters.' : 'Please enter your password.';
        errorEl.hidden = false;
      }
      return;
    }
    ...
    try {
      if (this._emailMode === 'register') {
        ...
        if (result.pending) {
          const card =
            (_c = this._mountContainer) === null || _c === void 0 ? void 0 : _c.querySelector('.signin-card');
          if (card) {
            setHtml(
              card,
              `
