...
import { AnalyzeResultRenderer } from './AnalyzeResultRenderer.js?v=20260629resultsummary2';
...
export class AnalyzeView {
  ...
  render() {
    const hasResult = Boolean(this.lastResult || this.app.state.analyzeResult || this.app.state.report);
    const el = document.createElement('div');
    el.className = 'fade-in';
    el.innerHTML = `
      <style>
        .analyze-page { max-width: 1200px; margin: 0 auto; }
        .analyze-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; flex-wrap: wrap; margin-bottom: 24px; }
        ...
        .analyze-header-actions { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
        .analyze-header-export-btn { display: inline-flex; align-items: center; gap: 6px; }
      ...
      <div class="analyze-page">
        <div class="analyze-header">
          ...
          <div class="analyze-header-actions">
            <button type="button" class="btn btn-secondary btn-sm analyze-header-export-btn" id="analyze-header-export-btn" ${hasResult ? '' : 'disabled'} title="Export current analysis">
        ...
        <div class="analyze-grid">
          <div class="analyze-left">
            ${this._targetConfig.render(this.app.state.defaultProjectPath || '', this.app.state.lastProjectPath || this.app.state.defaultProjectPath || '')}
          </div>
  ...
  renderResultsContent() {
    ...
    if (report) {
      return this._resultRenderer.render({ kind: 'simplebeacon-report', report });
    }
  ...
  bindEvents(el) {
    ...
    el.querySelector('#analyze-header-export-btn')?.addEventListener('click', () => this.exportLastResult());
    el.querySelector('#analyze-view-results-btn')?.addEventListener('click', () => this.app.navigate('results'));
  }

  bindResultEvents(container) {
    container.querySelector('#analyze-view-results-btn')?.addEventListener('click', () => this.app.navigate('results'));
  }
  ...
  async runPathAnalysis(path) {
    ...
    try {
      ...
      const correctedPath = report.projectRoot || report.projectPath || report.scanPaths?.[0] || projectPath;
      this.app.state.lastProjectPath = correctedPath;
      const pathInput = document.getElementById('project-path-input');
      if (pathInput) { pathInput.value = correctedPath; }
      this.lastResult = {
        kind: 'simplebeacon-report',
        report,
        projectPath: correctedPath,
        label: 'Scan: ' + correctedPath
      };
  ...
  exportLastResult() {
    const result = this.lastResult || this.app.state.analyzeResult || this.app.state.report;
    if (!result) {
    ...
    try {
      ...
      const projectPath = result.projectPath || result.fileName || result.projectRoot || 'analyze';
      const slug = String(projectPath).replace(/[^a-zA-Z0-9_-]/g, '-').replace(/-+/g, '-').substring(0, 40);
      const date = new Date().toISOString().slice(0, 10);
      const filename = `analyze-${slug}-${date}.json`;
      downloadJson(payload, filename);
      showToast('Analysis exported', 'success');
    } catch (err) {
      console.error('[AnalyzeView export] failed:', err);
