...
function renderSuccess(banner, { token, email }, onActivate, onStockpile) {
  ...
  banner
    .querySelector("#sb-activate-token-btn")
    ?.addEventListener("click", () => {
      onActivate?.(token, email);
    });
  banner
    .querySelector("#sb-stockpile-token-btn")
    ?.addEventListener("click", () => {
...
export async function mountCheckoutSuccessBanner(container, options = {}) {
  ...
  const zone =
    container.querySelector("#settings-notification-zone") || container;
  ...
  const activate = (token, email) => {
    ...
    authService.setSession(token, {
      email: email || "team@simplebeacon.ai",
      tier: "team",
    });
  ...
  const attemptLoad = async (sessionId) => {
    for (let i = 0; i < MAX_ATTEMPTS; i += 1) {
      try {
        ...
        renderError(
          banner,
          err.message || "Please refresh or check your email.",
        );
        return false;
      }
    }
