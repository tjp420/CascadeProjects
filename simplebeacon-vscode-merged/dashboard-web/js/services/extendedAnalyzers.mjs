...
export function createExtendedAnalyzers(deps) {
  ...
  function runCatastrophicForgettingAnalyzer(definition, issueId, input = {}) {
    ...
    const score = clampScore(
      retentionRate * 0.55 + (100 - knowledgeDecaySpeed) * 0.25 + (100 - interferenceScore) * 0.2
    );
    ...
    return buildHigherBetterResult(
      ...
      retentionRate < 80
        ? [
            {
              level: 'warn',
              message: 'Knowledge retention dropped below target across sequential tasks.',
              code: 'LOW_RETENTION',
            },
          ]
        : [],
  ...
  function runPromptEngineeringDifficultyAnalyzer(definition, issueId, input = {}) {
    ...
    const attemptsToSuccess = rows.length
      ? rows.reduce((sum, row) => sum + Math.max(1, Number(row.attempts ?? 1)), 0) / rows.length
      : 0;
    ...
    const expertAvg = expertRows.length
      ? expertRows.reduce((sum, row) => sum + Math.max(1, Number(row.attempts ?? 1)), 0) / expertRows.length
      : attemptsToSuccess;
    const noviceAvg = noviceRows.length
      ? noviceRows.reduce((sum, row) => sum + Math.max(1, Number(row.attempts ?? 1)), 0) / noviceRows.length
      : attemptsToSuccess;
    ...
    return buildLowerBetterResult(
      ...
      attemptsToSuccess >= 4
        ? [
            {
              level: 'warn',
              message: 'High attempts-to-success indicates prompt engineering friction.',
  ...
  function runCostBarrierAnalyzer(definition, issueId, input = {}) {
    ...
    const costs = tiers
      .map((tier) => Number(tier.monthlyCost ?? tier.cost ?? 0))
    ...
    return buildHigherBetterResult(
      ...
      barrierIndex >= 50
        ? [
            {
              level: 'warn',
              message: 'Cost barrier index exceeds preferred affordability threshold.',
  ...
  function runUsageLimitAnalyzer(definition, issueId, input = {}) {
    ...
    const limitHitRate = clampScore(
      events.length ? Math.min(100, events.length * 25) : /\b429\b/i.test(text) ? 40 : 15
    );
    ...
    return buildHigherBetterResult(
      ...
      workflowDisruptionIndex >= 40
        ? [{ level: 'warn', message: 'Quota or rate limits are disrupting workflows.', code: 'LIMIT_DISRUPTION' }]
        : [],
      [
        'Track rate-limit and quota hit frequency.',
        'Measure workflow interruption severity.',
    ...
    return buildHigherBetterResult(
      ...
      dependencyRatio >= 50
        ? [
            {
              level: 'warn',
              message: 'Provider dependency ratio indicates platform lock-in risk.',
  ...
  function runLanguageLimitationAnalyzer(definition, issueId, input = {}) {
    ...
    const coverageScore = clampScore(
      (evaluations.filter((row) => row.passed !== false).length / evaluations.length) * 100
    );
    ...
    return buildHigherBetterResult(
      ...
      languageParityScore < 75
        ? [{ level: 'warn', message: 'Locale quality parity gap exceeds target.', code: 'LOCALE_PARITY_GAP' }]
        : [],
      [
        'Run multilingual benchmark sets per locale.',
        'Track parity gaps across locales.',
  ...
  function runDomainKnowledgeAnalyzer(definition, issueId, input = {}) {
    ...
    return buildHigherBetterResult(
      ...
      domainAccuracy < 70
        ? [{ level: 'warn', message: 'Domain benchmark accuracy is below target.', code: 'LOW_DOMAIN_ACCURACY' }]
        : [],
      [
        'Maintain domain Q&A benchmarks with expert labels.',
        'Validate terminology usage in outputs.',
  ...
  function runOutputConsistencyAnalyzer(definition, issueId, input = {}) {
    ...
    return buildHigherBetterResult(
      ...
      schemaComplianceRate < 90
        ? [{ level: 'warn', message: 'Structured output schema compliance is below target.', code: 'SCHEMA_DRIFT' }]
        : [],
      [
        'Validate outputs against expected schema.',
        'Track parser success across runs.',
  ...
  function runSessionManagementAnalyzer(definition, issueId, input = {}) {
    ...
    return buildHigherBetterResult(
      ...
      stateIntegrityRate < 80
        ? [{ level: 'warn', message: 'Session state integrity falls below target.', code: 'SESSION_INTEGRITY_GAP' }]
        : [],
      [
        'Log session state transitions with integrity checks.',
        'Measure recovery after interruptions.',
  ...
  function runPrivacyConcernAnalyzer(definition, issueId, input = {}) {
    ...
    return buildHigherBetterResult(
      ...
      policyClarityScore < 60
        ? [{ level: 'warn', message: 'Privacy policy clarity signals are incomplete.', code: 'LOW_POLICY_CLARITY' }]
        : [],
      [
        'Publish clear privacy policy coverage.',
        'Align runtime behavior with policy statements.',
  ...
  function runContentFilteringAnalyzer(definition, issueId, input = {}) {
    ...
    const score = clampScore(
      moderationConsistencyScore * 0.5 + (100 - leakageRate) * 0.3 + (100 - wrongfulBlockRate) * 0.2
    );
    const risk = finalizeRiskAssessment(score, definition.scoringDirection, {
    ...
    return buildHigherBetterResult(
      ...
      transparencyScore < 60
        ? [
            {
              level: 'info',
              message: 'Transparency disclosures are partial; expand rationale coverage.',
    ...
    return buildLowerBetterResult(
      ...
      dependenceRiskScore >= 45
        ? [
            {
              level: 'warn',
              message: 'Overreliance markers exceed preferred dependence threshold.',
  ...
  function runApiComplexityAnalyzer(definition, issueId, input = {}) {
    ...
    const docsHits =
      (/\b(openapi|sdk|readme|docs)\b/i.test(text) ? 2 : 0) + (/\b(openapi|sdk|readme|docs)\b/i.test(codeText) ? 1 : 0);
    ...
    return buildHigherBetterResult(
      ...
      complexityPenalty >= 40
        ? [
            {
              level: 'info',
              message: 'API surface complexity is elevated; simplify integration paths.',
  ...
  function runCustomizationLimitAnalyzer(definition, issueId, input = {}) {
    ...
    const customizationCoverageScore = clampScore(
      options.length ? (successCount / options.length) * 100 : /\bcustom\b/i.test(text) ? 70 : 0
    );
    ...
    return buildHigherBetterResult(
      ...
      customizationCoverageScore < 70
        ? [{ level: 'info', message: 'Customization pathway coverage is limited.', code: 'LOW_CUSTOMIZATION_COVERAGE' }]
        : [],
      [
        'Document customization pathways.',
        'Track adaptation success by use case.',
