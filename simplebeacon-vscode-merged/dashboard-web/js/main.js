...
import {
  renderSiemTelemetryDashboard,
  cleanupSiemTelemetryDashboard,
} from "./components/SiemTelemetryDashboard.js?v=20260805siem1";
...
function vaultUnlockUrl(returnPath = "/app") {
  ...
  if (isLocalDevHost()) {
    ...
    if (!vaultPassword) {
      console["warn"](
        "Vault password not configured. Set VAULT_PASSWORD environment variable on the server.",
      );
...
class SimplebeaconDashboard {
  constructor() {
    ...
    this.views = {
      ...
      "siem-telemetry": {
        mount: (container) => {
          container.appendChild(renderSiemTelemetryDashboard());
        },
        unmount: () => {
          cleanupSiemTelemetryDashboard();
        },
  ...
  resetMainScroll(main) {
    ...
    try {
      for (
        let cur = root instanceof Element ? root : document.body;
        cur;
  ...
  showTokenPrompt() {
    ...
    tokenForm.addEventListener("submit", async (e) => {
      ...
      const password =
        overlay.querySelector("#token-prompt-password")?.value || "";
  ...
  showLockScreen(view) {
    ...
    if (tokenForm) {
      tokenForm.addEventListener("submit", async (e) => {
        ...
        const password =
          main.querySelector("#lock-token-password")?.value || "";
  ...
  updateNavVisibility(authed) {
    ...
    document.querySelectorAll(".nav-group-toggle").forEach((toggle) => {
      ...
      const itemsContainer = toggle
        .closest(".nav-group")
        ?.querySelector(".nav-group-items");
  ...
  updateAuthUi() {
    ...
    const showAdmin =
      authed && Boolean(authService.isAdmin && authService.isAdmin());
    if (adminLink) {
  ...
  setupShell() {
    ...
    try {
      if (
        ...
        !document.querySelector('.nav-link[data-view="compliance"]')
      ) {
  ...
  async loadData() {
    ...
    const badPathPattern =
      /ai-platform\/CascadeProjects$|google-earthenterprise/i;
  ...
  startBackgroundScanWatcher() {
    this.stopBackgroundScanWatcher();
    const currentScanId =
      this.state.report?.scanId || this.state.report?.generatedAt || null;
