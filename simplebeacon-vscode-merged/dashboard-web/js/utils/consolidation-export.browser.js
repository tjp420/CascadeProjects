...
function reconcileLegacyConsolidationCounts(summary = {}) {
  ...
  if (benchmarkCacheCandidatesExcluded > 0 && (summary.exactDuplicateGroups ?? 0) === 0 && fuzzyPairsExcluded === 0) {
...
function consolidationCandidateTouchesExcludedExport(candidate) {
   ...
   * @returns {any}
   */
  const paths = (candidate?.files || []).map((file) => file.path || file.relativePath || file.name).filter(Boolean);
...
function buildProductConsolidationHygieneSummary(summaryBase, scan, options = {}) {
  ...
  return {
    ...
    intentionalPairsExcluded: summaryBase.intentionalPairsExcluded ?? summaryBase.fuzzyPairsExcluded ?? 0,
...
export function sanitizeConsolidationExport(scan, options = {}) {
  ...
  const summaryBase = {
    ...
    fuzzyNearDuplicatePairs: advancedAnalysis?.fuzzyNearDuplicates?.pairsFound ?? scan.summary.fuzzyNearDuplicatePairs,
    benchmarkCacheCandidatesExcluded,
    fuzzyPairsExcluded,
    ...(intentionalPairsExcluded > 0
      ? {
