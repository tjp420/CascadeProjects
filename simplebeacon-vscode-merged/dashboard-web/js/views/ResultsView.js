...
export class ResultsView {
  ...
  renderScanSummary(report) {
    ...
    const gateClass = report.gate?.pass ? 'success' : 'warning';
      ...
      <div class="results-summary-v2">
        <div class="results-summary-meta">
          ...
          <div class="results-summary-info">
            ...
            <div class="results-summary-files"><i data-lucide="files" class="icon-14"></i> ${formatNumber(metrics.repositoryFiles ?? metrics.mockSampleFiles ?? report.totalFiles ?? 0)} files scanned</div>
          </div>
        </div>
  ...
  render() {
    ...
    : `Showing ${totalIssues} issue group(s) from the latest scan.`}
      ...
      ${this.app.state.routeParams?.q ? `<p class="text-muted" style="margin-bottom:var(--space-4)">Search: "${escapeHtml(this.app.state.routeParams.q)}"</p>` : ''}

      <div class="results-search-modern">
        <input type="text" class="results-search-input" id="results-search-input" placeholder="Search issues by type, description, or file path…" value="${escapeHtml(this.app.state.routeParams?.q || '')}" />
        ...
        ${renderEmptyState({
          ...
          actions: filtersActive && totalIssues > 0
            ? [
                ...
                ...(this.filterCategory !== 'all' ? [{ id: 'clear-category-filter', label: `Clear ${escapeHtml(activeCategory?.title || 'filter')}`, className: 'btn-secondary btn-sm' }] : [])
              ]
            : filtersActive
              ? [{ id: 'clear-results-filters', label: 'Clear filters', className: 'btn-secondary btn-sm' }]
              : (totalIssues === 0 && report.gate?.pass ? [] : [{ id: 'run-scan-empty', label: 'Run Scan', className: 'btn-primary btn-sm' }])
        })}
      ` : `
    ...
    el.querySelector('#clear-category-filter')?.addEventListener('click', () => {
      this.filterCategory = 'all';
    ...
    if (list) {
      ...
      issues.forEach((issue) => {
        ...
        const category = categories.find((c) => c.filter?.(issue)) || { icon: '📁', title: issue.type || 'Finding' };
  ...
  bindExport(el, issues) {
    ...
    el.querySelector('#send-ai-btn')?.addEventListener('click', () => {
      ...
      if (aiPanel?.style.display === 'block' && aiNotesInput) {
        aiNotesInput.focus();
      }
    });
    ...
    if (searchInput) {
      searchInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
          ...
          if (!q) delete this.app.state.routeParams.q;
  ...
  showDetail(container, issue) {
    ...
    slot.innerHTML = `
      <div class="panel-modern detail-panel-modern">
        ...
        ${issue.affectedFiles?.length ? `<div class="detail-meta-row"><span class="detail-meta-label">Affected</span><span class="detail-meta-value">${issue.affectedFiles.map(escapeHtml).join(', ')}</span></div>` : ''}
