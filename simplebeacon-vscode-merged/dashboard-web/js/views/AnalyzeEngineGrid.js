...
export function isEngineTierLocked(engineId) {
  if (authService.isPaidTier()) return false;
...
export function normalizeSelectedEngines(raw, { allowEmpty = false } = {}) {
  ...
  if (!allowEmpty && !selected.length) return defaultSelectedEngines().filter((id) => !isEngineTierLocked(id));
  const set = new Set(selected);
  for (const [engineId, deps] of Object.entries(ENGINE_DEPENDENCIES)) {
    if (!set.has(engineId)) continue;
    for (const dep of deps) {
      if (!isEngineTierLocked(dep)) set.add(dep);
    }
  }
...
export function applyEngineSelectionChange(selectedSet, engineId, checked) {
  if (checked) {
    ...
    if (deps) deps.forEach((d) => selectedSet.add(d));
  } else {
...
export class AnalyzeEngineGrid {
  ...
  renderQueuePanel() {
    ...
    const queueItems = runOrder.map((engineId, index) => {
      ...
      const label = step?.label || opt?.label || engineId;
      const hint = opt?.hint || step?.desc || '';
