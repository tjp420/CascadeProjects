    ...
    return `
      <div class="an-res-v3">
        <div class="an-res-v3-hd">
          ...
          <span class="db-v3-panel-badge ${report.gate?.pass ? 'success' : 'danger'}">${report.gate?.pass ? 'PASS' : 'FAIL'}</span>
        </div>
        ...
        <div class="an-res-v3-body">
          ${report.issues?.length ? renderIssueList(report.issues) : '<p class="text-muted">No issues found.</p>'}
        </div>
      </div>
    `;
  }
