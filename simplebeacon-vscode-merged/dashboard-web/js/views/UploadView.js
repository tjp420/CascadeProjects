...
import { loadReportFromText } from '../services/analyzeService.js';
...
export class UploadView {
      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <div class="drop-zone" id="upload-drop-zone" style="border:2px dashed var(--border);border-radius:12px;padding:40px 24px;text-align:center;cursor:pointer;transition:border-color 0.2s,background 0.2s;background:var(--bg-input);">
            ...
            <input type="file" id="upload-file-input" accept=".json,.html,application/json,text/html" style="display:none;" aria-label="Upload scan report file">
          </div>
  ...
  handleFile(file, fileNameEl, scanPreviewEl, scanMetaEl, previewContentEl, updateSubmit) {
    ...
    reader.onload = (e) => {
      ...
      try {
        // Use resilient loader that accepts JSON or HTML-wrapped JSON
        const parsed = loadReportFromText(text);
        ...
        this.showStatus(status, err.message || 'Invalid report file', 'error');
        this.reportData = null;
        updateSubmit();
      }
    };
  ...
  renderPreview(data, scanMetaEl, previewContentEl) {
    ...
    const project =
      data.projectRoot || data.scanTargetRoot || (Array.isArray(data.scanPaths) ? data.scanPaths[0] : null) || '—';
