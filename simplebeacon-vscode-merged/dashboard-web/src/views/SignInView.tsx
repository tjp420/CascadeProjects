...
export function SignInView() {
  ...
  useEffect(() => {
    ...
    if (hashQueryIndex !== -1) {
      new URLSearchParams(hash.slice(hashQueryIndex + 1)).forEach((v, k) =>
        allParams.set(k, v),
      );
    }
  ...
  const checkSsoDomain = useCallback(async (emailValue: string) => {
    ...
    try {
      ...
      const resp = await fetch(
        apiUrl(`/sso/resolve?email=${encodeURIComponent(emailValue)}`),
      );
      if (resp.ok) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        try {
          ...
          friendlyMsg =
            errData.message || errData.error || errData.detail || friendlyMsg;
        } catch {
  ...
  const handleLicenseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.valid && data.registered) {
        ...
        toast.error(
          "Invalid license key. Please check the key from your confirmation email.",
        );
      }
    } catch (err: any) {
              ...
              {ssoProvider && ssoProvider.found && (
                <div className="rounded-md border border-primary/30 bg-primary/5 p-3 space-y-2">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    ...
                    <span>
                      {ssoProvider.displayName || "Enterprise SSO"} detected
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Your organization uses{" "}
                    {ssoProvider.method?.toUpperCase() || "SSO"} for
                    authentication.
                  </p>
                  <Button
                    type="button"
