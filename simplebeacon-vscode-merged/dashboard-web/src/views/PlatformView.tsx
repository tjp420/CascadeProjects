...
export function PlatformView() {
  ...
  const [consensusData, setConsensusData] = useState<ConsensusStatus | null>(
    null,
  );
    ...
    try {
      const fetchPromises: Promise<Response | null>[] = [
        ...
        consensusErrorRef.current
          ...
          : fetch(apiUrl("/vault/consensus/status"), {
              headers: authHeaders(),
            }),
      ];

      const [statusResp, healthResp, consensusResp] =
      ...
      if (consensusResp.status === "fulfilled" && consensusResp.value) {
        if (consensusResp.value.ok) {
          ...
          consensusResp.value.status === 401 ||
          consensusResp.value.status === 403
        ) {
      ...
      <div className="mx-auto max-w-5xl p-6 space-y-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-foreground-muted">
            Platform analytics and deployment metrics
          </p>
        </div>
        <Card>
    ...
    <div className="mx-auto max-w-5xl p-6 space-y-6">
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-1 py-4">
            ...
            <span className="text-lg font-bold">
              {platformData?.platform || "Simplebeacon"}
            </span>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            HSM adapter stack — 16 tracks (10-25), all production-ready
          </CardDescription>
        </CardHeader>
            ...
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div>
                <div className="text-sm font-medium">
                  Cross-Tenant Key Escrow
                </div>
                <div className="text-xs text-foreground-muted">
                  Dual-consent declassification with replay guarding
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div>
                <div className="text-sm font-medium">
                  Blind Signatures & PIR
                </div>
                <div className="text-xs text-foreground-muted">
                  Chaum RSA blind issuers, homomorphic query gating
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div>
                ...
                <div className="text-xs text-foreground-muted">
                  NIST known-answer vectors, EU AI Act telemetry
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div>
                <div className="text-sm font-medium">
                  ZKP Identity Federation
                </div>
                <div className="text-xs text-foreground-muted">
                  Zero-knowledge proof tokens, temporal drift anchors
                </div>
              </div>
            </div>
          </div>
      ...
      {consensusData && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Distributed Byzantine consensus engine — Track 34 (7 phases)
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {consensusData.engine ? (
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="flex flex-col items-center gap-1 rounded-lg border p-3">
                  ...
                  <span className="text-base font-bold">
                    Term {consensusData.engine.term ?? "—"}
                  </span>
                  <span className="text-xs text-foreground-muted">
                    Current Term
                  </span>
                ...
                <div className="flex flex-col items-center gap-1 rounded-lg border p-3">
                  ...
                  <span className="text-base font-bold">
                    {consensusData.engine.commitIndex ?? "—"}
                  </span>
                  <span className="text-xs text-foreground-muted">
                    Commit Index
                  </span>
                ...
                <div className="flex flex-col items-center gap-1 rounded-lg border p-3">
                  ...
                  <span className="text-base font-bold">
                    {consensusData.engine.logLength ?? "—"}
                  </span>
                  <span className="text-xs text-foreground-muted">
                    Log Length
                  </span>
                ...
                <div className="flex flex-col items-center gap-1 rounded-lg border p-3">
                  ...
                  <span className="text-base font-bold">
                    {consensusData.engine.clusterSize ?? "—"}
                  </span>
                  <span className="text-xs text-foreground-muted">
                    Cluster Size
                  </span>
            ...
            {consensusData.engine?.hasSnapshot && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">
                  Index {consensusData.engine.lastSnapshotIndex} (term{" "}
                  {consensusData.engine.lastSnapshotTerm})
                </span>
              </div>
            )}

            {/* Metrics Counters */}
            {consensusData.counters &&
              Object.keys(consensusData.counters).length > 0 && (
