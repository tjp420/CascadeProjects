...
export function getApiBase(): string {
  ...
  try {
    ...
    if (explicit && !isForeignPagesPreviewBase(explicit)) {
      ...
      // IDE website-mode sign-in sends sb_api_base=http://127.0.0.1:port/api from HTTPS.
      // That loopback is the extension data server — use it instead of simplebeacon.ai.
      if (ideBridge || !(hostedHttps && loopback)) return base;
    }
    // Browser must call /api on this origin unless the IDE explicitly bridged localhost.
