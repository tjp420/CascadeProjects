...
(function () {
  ...
  const W = () => canvas.width,
    H = () => canvas.height;
  const initRadius = Math.min(Math.min(W(), H()) / 2.5, Math.max(80, Math.sqrt(GRAPH.nodes.length) * 10));
  const allNodes = GRAPH.nodes.map((n, i) => ({
    id: n.id,
    label: n.label,
    group: n.group,
    x: W() / 2 + Math.cos(i * 2.4) * initRadius,
    y: H() / 2 + Math.sin(i * 2.4) * initRadius,
    vx: 0,
    vy: 0,
  ...
  function step() {
    ...
    for (let i = 0; i < visNodes.length; i++) {
      for (let j = i + 1; j < visNodes.length; j++) {
        ...
        if (!is3D && dist > 180) continue; // cutoff prevents islands from drifting apart in 2D
  ...
  function draw() {
    ...
    if (is3D) {
      ...
      // Zoom-dependent labels: show based on projected size and scale
      const labelThreshold = 0.35;
      for (const { n, p } of projected) {
        const projectedR = Math.max(1, n.radius * p.depthScale);
        const showLabel =
          labelsVisible || n === hoverNode || n === selectedNode || (p.depthScale > labelThreshold && n.connCount >= 3);
        if (showLabel && projectedR > 2) {
          const isDimmed = focusMode && connectedIds && !connectedIds.has(n.id);
          ctx.fillStyle = isDimmed ? 'rgba(226,232,240,0.2)' : '#e2e8f0';
          ctx.font =
            (n === selectedNode ? 'bold ' : '') + Math.max(9, Math.min(12, 9 + projectedR * 0.4)) + 'px sans-serif';
      ...
      const showLabels = labelsVisible || scale > 1.5;
      for (const n of visNodes) {
        const isDimmed = focusMode && connectedIds && !connectedIds.has(n.id);
        if (isDimmed && n !== selectedNode && n !== hoverNode && n !== crosshairNode) continue;
        if (
          showLabels ||
          n === hoverNode ||
          n === selectedNode ||
          n === crosshairNode ||
          (scale > 0.8 && n.connCount >= 5)
        ) {
          ctx.fillStyle = isDimmed ? 'rgba(226,232,240,0.3)' : '#e2e8f0';
          ctx.font =
            (n === selectedNode ? 'bold ' : '') + Math.max(9, Math.min(12, 9 + n.radius * 0.3)) + 'px sans-serif';
          ctx.textAlign = 'center';
