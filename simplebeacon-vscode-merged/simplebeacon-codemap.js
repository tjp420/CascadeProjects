...
function buildGraphFromTree(items, parentPath) {
  ...
  function addEdge(source, target) {
    ...
    if (source && target && source !== target && !seenE.has(k)) {
      seenE.add(k);
...
let ANALYSIS = {};
try {
  ANALYSIS = JSON.parse(document.getElementById('analysisData')?.textContent || '{}');
} catch (e) {}
const nodeSeverity = {};
...
if (statsEl) {
  ...
  const sections = [
    ...
    { title: 'Leaf Modules', count: LEAVES.length, items: LEAVES.slice(0, 8) },
    {
      ...
      items: CONNECTED.slice(0, 8).map((c) => c.name + ' (' + c.count + ' conn)'),
    },
  ];
...
(function () {
  ...
  try {
    ...
    function makeRing(val, label, cls) {
      ...
      return (
        '<div class="score-ring"><svg viewBox="0 0 72 72"><circle cx="36" cy="36" r="' +
        r +
        '" stroke="#1e293b" stroke-width="6" fill="none"/><circle cx="36" cy="36" r="' +
        r +
        '" class="' +
        cls +
        '" stroke-width="6" fill="none" stroke-dasharray="' +
        c +
        '" stroke-dashoffset="' +
        off +
        '" stroke-linecap="round" transform="rotate(-90 36 36)"/></svg><span class="score-val">' +
        val +
        '</span><span class="score-label">' +
        label +
        '</span></div>'
      );
    }
    ...
    if (scoreBoard && ANALYSIS.summary) {
      ...
      const cls =
        s.score >= 80 ? 'score-excellent' : s.score >= 60 ? 'score-good' : s.score >= 40 ? 'score-fair' : 'score-poor';
      scoreBoard.textContent = '';
      scoreBoard.insertAdjacentHTML(
        'beforeend',
        makeRing(s.score, s.scoreLabel || 'Score', cls) +
          makeRing(
            s.architectureScore || 0,
            'Arch',
            s.architectureScore >= 80
              ? 'score-excellent'
              : s.architectureScore >= 60
                ? 'score-good'
                : s.architectureScore >= 40
                  ? 'score-fair'
                  : 'score-poor'
          ) +
          makeRing(
          ...
          makeRing(
            s.complexityScore || 0,
            'Complexity',
            s.complexityScore >= 80
              ? 'score-excellent'
              : s.complexityScore >= 60
                ? 'score-good'
                : s.complexityScore >= 40
                  ? 'score-fair'
                  : 'score-poor'
          ) +
          makeRing(
    ...
    if (metricsBoard && ANALYSIS.metrics) {
      ...
      let html =
        '<h4 style="margin:8px 0 4px;font-size:13px;color:#94a3b8">Metrics</h4><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px">';
      html +=
        '<div class="analysis-card"><div style="color:#94a3b8;font-size:11px">Avg Lines / File</div><div style="font-size:18px;font-weight:700">' +
        (m.avgFileLines || 0) +
        '</div><div class="metric-bar"><div style="width:' +
        Math.min(m.avgFileLines || 0, 100) +
        '%"></div></div></div>';
      html +=
        '<div class="analysis-card"><div style="color:#94a3b8;font-size:11px">Max Lines</div><div style="font-size:18px;font-weight:700">' +
        (m.maxFileLines || 0) +
        '</div></div>';
      html +=
        '<div class="analysis-card"><div style="color:#94a3b8;font-size:11px">Test Ratio</div><div style="font-size:18px;font-weight:700">' +
        (m.testRatio || 0) +
        '%</div><div class="metric-bar"><div style="width:' +
        Math.min((m.testRatio || 0) * 2, 100) +
        '%"></div></div></div>';
      html +=
        '<div class="analysis-card"><div style="color:#94a3b8;font-size:11px">Dependency Density</div><div style="font-size:18px;font-weight:700">' +
        (m.dependencyDensity || 0) +
        '%</div><div class="metric-bar"><div style="width:' +
        Math.min((m.dependencyDensity || 0) * 3, 100) +
        '%"></div></div></div>';
      html +=
        '<div class="analysis-card"><div style="color:#94a3b8;font-size:11px">Orphan Files</div><div style="font-size:18px;font-weight:700">' +
        (m.orphanCount || 0) +
        '</div></div>';
      html +=
        '<div class="analysis-card"><div style="color:#94a3b8;font-size:11px">Source / Test Files</div><div style="font-size:18px;font-weight:700">' +
        (m.sourceFileCount || 0) +
        ' / ' +
        (m.testFileCount || 0) +
        '</div></div>';
      if (m.languageDistribution && m.languageDistribution.length) {
        html +=
          '<div class="analysis-card" style="grid-column:1 / -1"><div style="color:#94a3b8;font-size:11px;margin-bottom:6px">Language Distribution</div><div style="display:flex;flex-wrap:wrap;gap:8px">';
        m.languageDistribution.forEach((l) => {
          html +=
            '<div class="lang-donut"><span class="dot" style="background:' +
            (window.EXT_COLORS && window.EXT_COLORS[l.language] ? window.EXT_COLORS[l.language] : '#64748b') +
            '"></span><span>' +
            l.language +
            ' ' +
            l.percentage +
    ...
    if (archBoard && ANALYSIS.architecture) {
      ...
      if (a.bidirectionalDeps && a.bidirectionalDeps.length) {
        html +=
          '<div style="margin-top:8px;font-size:11px;color:#f59e0b">Bidirectional pairs: ' +
          a.bidirectionalDeps
            .slice(0, 5)
            .map((b) => (b.source.split('/').pop() || b.source) + ' &harr; ' + (b.target.split('/').pop() || b.target))
            .join(', ') +
          (a.bidirectionalDeps.length > 5 ? ' +' + (a.bidirectionalDeps.length - 5) + ' more' : '') +
          '</div>';
      }
    ...
    if (recBoard && ANALYSIS.recommendations && ANALYSIS.recommendations.length) {
      ...
      top3.forEach((r, i) => {
        const refactor = r.title.includes('circular')
          ? 'Extract shared interfaces into a new shared-utils module'
          : r.title.includes('large')
            ? 'Split by feature/domain — one exported class/function per file'
            : r.title.includes('coupl')
              ? 'Introduce event bus or dependency injection to decouple hubs'
              : r.title.includes('bidirectional')
                ? 'Move shared types to a common-types file both import'
                : r.title.includes('chain')
                  ? 'Inline small intermediates or use facade pattern'
                  : r.title.includes('test')
                    ? 'Add unit tests for uncovered public APIs'
                    : 'Review and refactor affected files';
        html +=
          '<div class="recommendation-card" style="border-left:3px solid #ef4444"><div class="rec-header"><span class="rec-priority" style="background:#ef4444">#' +
          (i + 1) +
          '</span><b style="font-size:13px">' +
          r.title +
          '</b><span class="rec-effort">' +
          r.effort +
          '</span><span class="rec-impact">' +
          r.impact +
          '</span></div><div style="font-size:12px;color:#94a3b8">' +
          (r.description || '') +
          '</div><div style="font-size:11px;color:#22c55e;margin-top:4px">Suggested: ' +
          refactor +
          '</div></div>';
      });
      ...
      ANALYSIS.recommendations.forEach((r) => {
        html +=
          '<div class="recommendation-card"><div class="rec-header"><span class="rec-priority">P' +
          (r.priority + 1) +
          '</span><b style="font-size:13px">' +
          r.title +
          '</b><span class="rec-effort">' +
...
(function () {
  ...
  function updateResizer() {
    if (resizer) {
      resizer.style.display = sidebar && sidebar.classList.contains('hidden') ? 'none' : '';
    }
  }
  if (btn && sidebar && main) {
...
(function () {
  ...
  if (GRAPH.nodes.length === 0) {
    ...
    msg.style.cssText =
      'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:24px;text-align:center;color:#94a3b8;font-size:13px;line-height:1.5';
  ...
  if (GRAPH_MODE === 'folder') {
    ...
    hint.style.cssText =
      'position:absolute;top:48px;left:50%;transform:translateX(-50%);background:rgba(15,23,42,0.92);border:1px solid #334155;border-radius:8px;padding:8px 14px;font-size:11px;color:#94a3b8;z-index:12;pointer-events:none';
  ...
  const allNodes = GRAPH.nodes.map((n, i) => {
    ...
    return {
      ...
      vx: 0,
      vy: 0,
      radius: Math.max(3, Math.min(14, 3 + Math.log10((n.size || 1) + 1))),
      color: colors[n.group] || '#64748b',
      visible: true,
      highlighted: false,
      connCount: 0,
    };
  ...
  edges.forEach((e) => {
    if (e.source) e.source.connCount++;
    if (e.target) e.target.connCount++;
  });
  allNodes.forEach((n) => {
    n.radius = Math.max(4, Math.min(20, 4 + Math.sqrt(n.connCount) * 2.5));
  });
  ...
  let middlePanning = false,
    middlePanStart = { x: 0, y: 0, px: 0, py: 0 };
  ...
  function getFilteredNodes() {
    return allNodes.filter((n) => n.visible);
  }
  function getFilteredEdges() {
    return edges.filter((e) => e.source.visible && e.target.visible);
  }
  ...
  function clampCameraDistance() {
    const dist = Math.sqrt(
      cameraOffset.x * cameraOffset.x + cameraOffset.y * cameraOffset.y + cameraOffset.z * cameraOffset.z
    );
    if (dist < 0.001) {
      cameraOffset = { x: 0, y: 0, z: 700 };
      return;
    }
    const minDist = 50,
      maxDist = 200000;
  ...
  function nodeAt3D(cp) {
    ...
    for (const n of vis) {
      ...
      if (dist < Math.max(r + 10, 18) && dist < bestDist) {
        best = n;
        bestDist = dist;
      }
    }
  ...
  function classifyLayer(fp) {
    ...
    if (
      /(?:^|\/)(test|tests|__tests__|__mocks__|spec|e2e|cypress|playwright)(?:\/|$)/.test(p) ||
      /\.(test|spec)\.(js|ts|jsx|tsx|py|go|rs)$/.test(name)
    )
      return 'tests';
    if (/^(index|main|app|server|cli|entry|bootstrap|start)\.(js|ts|cjs|mjs|py|go|rs|java)$/.test(name)) return 'entry';
    if (
      /(?:^|\/)(components?|ui|pages?|views?|templates?|widgets|screens|layouts?)(?:\/|$)/.test(p) ||
      /\.(tsx|jsx|vue|svelte|html|css|scss|less)$/.test(name)
    )
      return 'ui';
    if (
      /(?:^|\/)(services?|controllers?|business|logic|api|routes?|handlers?|middleware|actions?)(?:\/|$)/.test(p) ||
      /(service|controller|route|handler|middleware|action)\.(js|ts|cjs|mjs)$/.test(name)
    )
      return 'business';
    if (
      /(?:^|\/)(db|database|models?|repositories?|stores?|schemas?|migrations?|configs?|settings?|infra|docker|k8s|helm)(?:\/|$)/.test(
        p
      ) ||
      /(config|model|schema|repository|store|migration|docker|dockerfile|docker-compose|k8s|helm)\.(js|ts|json|yaml|yml|env)$/.test(
        name
      )
    )
      return 'data';
    if (
      /(?:^|\/)(utils?|helpers?|lib|common|shared|tools?|scripts?|packages?)(?:\/|$)/.test(p) ||
      /(util|helper|common|shared|tool|lib)\.(js|ts|cjs|mjs)$/.test(name)
    )
      return 'utils';
    return 'other';
  }
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    if (is3D) {
      hoverNode = nodeAt3D(cp);
      canvas.style.cursor = hoverNode ? 'pointer' : 'move';
      return;
    }
    const wp = worldPosFromClient(cp);
    hoverNode = nodeAt(wp);
    if (dragging) {
      dragging.x = wp.x - offset.x;
      dragging.y = wp.y - offset.y;
      canvas.style.cursor = 'grabbing';
      dragMoved = true;
    } else {
  ...
  function showContextMenu(cx, cy) {
    ...
    menu.insertAdjacentHTML(
      'beforeend',
      items.map((it) => '<div class="context-menu-item">' + it.label + '</div>').join('')
    );
  ...
  function showNodeDetails(n) {
    ...
    html +=
      '<div class="detail-row"><span class="detail-label">Incoming</span><span>' + incoming.length + '</span></div>';
    html +=
      '<div class="detail-row"><span class="detail-label">Outgoing</span><span>' + outgoing.length + '</span></div>';
    html += '<div class="detail-row"><span class="detail-label">Cycles</span><span>' + cycles + '</span></div>';
    html +=
      '<div style="margin-top:8px;"><button id="openInEditorBtn" style="background:#06b6d4;border:none;border-radius:6px;color:#0f172a;padding:6px 12px;font-size:12px;cursor:pointer;font-weight:600;width:100%">Open in Editor</button></div>';
    if (incoming.length) {
      html += '<div style="margin-top:6px;color:#64748b;font-size:11px;">Imported by:</div>';
      html +=
        '<div style="max-height:60px;overflow-y:auto;font-size:11px;">' + incoming.slice(0, 5).join('<br>') + '</div>';
    }
    if (outgoing.length) {
      html += '<div style="margin-top:6px;color:#64748b;font-size:11px;">Imports:</div>';
      html +=
        '<div style="max-height:60px;overflow-y:auto;font-size:11px;">' + outgoing.slice(0, 5).join('<br>') + '</div>';
    }
  ...
  function applyFilters() {
    ...
    allNodes.forEach((n) => {
      ...
      if (n.visible && searchQuery) {
        n.visible = n.label.toLowerCase().includes(searchQuery) || n.id.toLowerCase().includes(searchQuery);
      }
    });
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    const n = allNodes.find(
      (n) => n.id === path || n.label === path || n.label.endsWith('/' + path) || path.endsWith('/' + n.label)
    );
    if (n) {
      selectedNode = n;
      showNodeDetails(n);
  ...
  function applyLayout(name) {
    ...
    if (name === 'radial') {
      ...
      const tree = {};
      vis.forEach((n) => {
        tree[n.id] = { n, children: [], depth: 0 };
      });
      const nodeEdgeMap = {};
      edges.forEach((e) => {
        if (tree[e.source.id]) tree[e.source.id].children.push(e.target.id);
      });
  ...
  document.getElementById('fitScreenBtn')?.addEventListener('click', () => {
    ...
    const minX = Math.min(...vis.map((n) => n.x - n.radius)),
      maxX = Math.max(...vis.map((n) => n.x + n.radius));
    const minY = Math.min(...vis.map((n) => n.y - n.radius)),
      maxY = Math.max(...vis.map((n) => n.y + n.radius));
    const pad = 40;
    const bw = maxX - minX + pad * 2,
      bh = maxY - minY + pad * 2;
    scale = Math.min(W() / bw, H() / bh, 20);
    pan = {
      x: (W() - bw * scale) / 2 - minX * scale + pad * scale,
      y: (H() - bh * scale) / 2 - minY * scale + pad * scale,
    };
    updateZoomDisplay();
  });
  ...
  if (pauseBtn)
    pauseBtn.addEventListener('click', () => {
      ...
      pauseBtn.textContent = physicsPaused ? '▶' : '⏸';
      pauseBtn.title = physicsPaused ? 'Resume Physics' : 'Pause Physics';
      pauseBtn.classList.toggle('active', physicsPaused);
    });
  ...
  document.getElementById('exportGraphBtn')?.addEventListener('click', () => {
    ...
    const payload = {
      ...
      graph: {
        nodes: allNodes.map((n) => ({ id: n.id, label: n.label, group: n.group, x: n.x, y: n.y, radius: n.radius })),
    ...
    a.href = url;
    const proj = (document.querySelector('.subtitle')?.textContent || 'project')
      .replace(/[^a-z0-9]/gi, '-')
      .toLowerCase();
  ...
  function applyTheme(val) {
    ...
    try {
      localStorage.setItem('codemapTheme', val);
    } catch (e) {}
  ...
  function loadBindings() {
    try {
      const saved = localStorage.getItem('codemapKeyBindings');
      if (saved) keyBindings = JSON.parse(saved);
    } catch (e) {}
    for (const def of CONTROL_DEFS) {
      if (!keyBindings[def.id]) keyBindings[def.id] = def.default;
    }
  }
  loadBindings();
  function getBoundKey(action) {
    return (keyBindings[action] || '').toLowerCase();
  }

  // Controls modal — elements are defined AFTER this script in HTML, query lazily
  function getControlsModal() {
    return document.getElementById('controlsModal');
  }
  ...
  function buildControlsGrid() {
    ...
    for (const def of CONTROL_DEFS) {
      ...
      box.addEventListener('click', () => {
        capturingFor = { action: def.id, box };
        box.classList.add('active');
        box.textContent = 'Press key...';
      });
  ...
  document.getElementById('controlsBtn')?.addEventListener('click', () => {
    buildControlsGrid();
    const cm = getControlsModal();
    if (cm) cm.classList.remove('hidden');
  });
  // Modal buttons are rendered AFTER the script tag; use event delegation
  document.addEventListener('click', (e) => {
    const t = e.target;
    if (t.id === 'closeControlsModal') {
      const cm = getControlsModal();
      if (cm) cm.classList.add('hidden');
      stopCapture();
    } else if (t.id === 'resetControlsBtn') {
      keyBindings = {};
      for (const def of CONTROL_DEFS) keyBindings[def.id] = def.default;
      buildControlsGrid();
    } else if (t.id === 'saveControlsBtn') {
      try {
        localStorage.setItem('codemapKeyBindings', JSON.stringify(keyBindings));
      } catch (e) {}
      const cm = getControlsModal();
      if (cm) cm.classList.add('hidden');
      stopCapture();
  ...
  function onKeyDown(e) {
    ...
    if (checkBinding(e, 'zoomIn')) {
      e.preventDefault();
      zoomAtCenter(zoomFactor);
    } else if (checkBinding(e, 'zoomOut')) {
      e.preventDefault();
      zoomAtCenter(1 / zoomFactor);
    } else if (checkBinding(e, 'resetView')) {
      e.preventDefault();
      scale = 1;
      pan = { x: 0, y: 0 };
      updateZoomDisplay();
    } else if (checkBinding(e, 'fitScreen')) {
      e.preventDefault();
      if (is3D) {
        scale = 1.0;
        cameraOffset.z = 700;
      ...
      [
        ...
        'rotateLeft',
        'rotateRight',
      ].some((id) => checkBinding(e, id))
    ) {
      e.preventDefault();
    } else if (checkBinding(e, 'toggle3D')) {
      e.preventDefault();
      document.getElementById('toggle3DBtn')?.click();
    } else if (checkBinding(e, 'pausePhysics')) {
      e.preventDefault();
      physicsPaused = !physicsPaused;
      const pb = document.getElementById('pausePhysicsBtn');
      if (pb) {
        pb.textContent = physicsPaused ? '▶' : '⏸';
        pb.title = physicsPaused ? 'Resume Physics' : 'Pause Physics';
        pb.classList.toggle('active', physicsPaused);
      }
    } else if (checkBinding(e, 'searchFocus')) {
      e.preventDefault();
      document.getElementById('graphSearch')?.focus();
    } else if (checkBinding(e, 'saveNode')) {
      e.preventDefault();
      saveCurrentNode();
    } else if (checkBinding(e, 'toggleMouseLock')) {
      e.preventDefault();
      toggleMouseLock();
    } else if (e.key === 'Escape' && manualMouseLook) {
      e.preventDefault();
      toggleMouseLock();
  ...
  function loadSavedNodes() {
    try {
      const saved = localStorage.getItem('codemapSavedNodes');
      if (saved) savedNodes = new Set(JSON.parse(saved));
    } catch (e) {}
  }
  loadSavedNodes();
  allNodes.forEach((n) => {
    n.saved = savedNodes.has(n.id);
  });
  function saveCurrentNode() {
    ...
    try {
      localStorage.setItem('codemapSavedNodes', JSON.stringify([...savedNodes]));
    } catch (e) {}
  }
  ...
  function step() {
    ...
    for (const e of visEdges) {
      ...
      e.source.vz = (e.source.vz || 0) + dz * force;
      e.target.vx -= dx * force;
      e.target.vy -= dy * force;
      e.target.vz = (e.target.vz || 0) - dz * force;
    }
    const layerZ = { entry: -2500, ui: -1200, business: 0, data: 1200, utils: -800, tests: 2500, other: 800 };
    ...
    for (const n of visNodes) {
      if (is3D) {
        ...
        n.z = (n.z || 0) + (n.vz || 0);
        if (n.z > zBoundary) {
          n.vz -= (n.z - zBoundary) * 0.001;
        }
        if (n.z < -zBoundary) {
          n.vz += (-zBoundary - n.z) * 0.001;
        }
        n.vx += (W() / 2 - n.x) * 0.0003;
        n.vy += (H() / 2 - n.y) * 0.0003;
      } else {
  ...
  function draw() {
    ...
    if (is3D) {
      ...
      const projected = visNodes
        .map((n) => ({ n, p: project3D(n) }))
        .filter((item) => item.p !== null)
        .sort((a, b) => b.p.z2 - a.p.z2);
      const projMap = new Map();
      projected.forEach((item) => {
        if (item.p) projMap.set(item.n.id, item.p);
      });
      // Draw edges back-to-front with depth fog
      const edgeProjections = [];
      for (const e of visEdges) {
        const p1 = projMap.get(e.source.id),
          p2 = projMap.get(e.target.id);
        if (!p1 || !p2) continue;
      ...
      for (const { p1, p2, avgZ, edge } of edgeProjections) {
        ...
        const isDimmed =
          focusMode && connectedIds && !connectedIds.has(edge.source.id) && !connectedIds.has(edge.target.id);
        ctx.beginPath();
        ctx.moveTo(p1.x, p1.y);
        ctx.lineTo(p2.x, p2.y);
        ctx.strokeStyle = isDimmed ? 'rgba(148,163,184,0.03)' : 'rgba(148,163,184,' + fog * 0.22 + ')';
        ctx.lineWidth = Math.max(0.4, fog * 1.5);
        ctx.stroke();
      }
      ...
      for (const { n, p } of projected) {
        ...
        ctx.beginPath();
        ctx.arc(p.x + shadowOff, p.y + shadowOff, r, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(0,0,0,' + 0.35 * depthAlpha * (isDimmed ? 0.2 : 1) + ')';
        ctx.fill();
        ctx.beginPath();
        ctx.arc(p.x, p.y, r + Math.max(2, 5 * p.depthScale), 0, Math.PI * 2);
        ctx.fillStyle = n.color;
        ctx.globalAlpha = 0.85 * depthAlpha * (isDimmed ? 0.15 : 1);
        ctx.fill();
        ctx.globalAlpha = 1;
        ...
        if (sev && !isDimmed) {
          ...
          ctx.strokeStyle = severityColor[sev] || '#64748b';
          ctx.lineWidth = 2;
        ...
        ctx.fillStyle = n.color;
        ctx.globalAlpha = depthAlpha * (isDimmed ? 0.2 : 1);
        ctx.fill();
        ctx.globalAlpha = 1;
      ...
      for (const n of visNodes) {
        ...
        if (n === hoverNode || n === selectedNode) {
          ctx.strokeStyle = n === selectedNode ? '#06b6d4' : '#fff';
          ctx.lineWidth = n === selectedNode ? 3 : 2;
          ctx.stroke();
        }
        if (n.highlighted && !isDimmed) {
          ctx.beginPath();
    ...
    if (minimapVisible) {
      ...
      if (mmCanvas) {
        ...
        function mmX(x) {
          return (x - minX + pad) * mmScale + offX;
        }
        function mmY(y) {
          return (y - minY + pad) * mmScale + offY;
        }
        // Draw faint edges
        mm.strokeStyle = 'rgba(148,163,184,0.08)';
        mm.lineWidth = 0.5;
        for (const e of GRAPH.edges) {
          const sa = allNodes.find((n) => n.id === e.source),
            ta = allNodes.find((n) => n.id === e.target);
          if (sa && ta) {
            mm.beginPath();
            mm.moveTo(mmX(sa.x), mmY(sa.y));
            mm.lineTo(mmX(ta.x), mmY(ta.y));
            mm.stroke();
          }
        }
        // Draw nodes
        for (const n of allNodes) {
          ...
          mm.beginPath();
          mm.arc(mmX(n.x), mmY(n.y), r, 0, Math.PI * 2);
        ...
        mm.beginPath();
        mm.arc(camX + vW / 2, camY + vH / 2, 4, 0, Math.PI * 2);
        mm.fillStyle = '#06b6d4';
        mm.fill();
        mm.beginPath();
        mm.arc(camX + vW / 2, camY + vH / 2, 8, 0, Math.PI * 2);
        mm.fillStyle = 'rgba(6,182,212,0.3)';
        mm.fill();
        // Compass labels
        mm.fillStyle = '#64748b';
        mm.font = '9px sans-serif';
        mm.textAlign = 'left';
  ...
  function loop() {
    ...
    if (is3D) {
      const fwd = getBoundKey('moveForward'),
        bwd = getBoundKey('moveBackward');
      const rightKey = getBoundKey('strafeRight'),
        leftKey = getBoundKey('strafeLeft');
      const upKey = getBoundKey('moveUp'),
        downKey = getBoundKey('moveDown');
      const rotLeftKey = getBoundKey('rotateLeft'),
        rotRightKey = getBoundKey('rotateRight');
      ...
      const fwd = getBoundKey('moveForward'),
        bwd = getBoundKey('moveBackward');
      const leftKey = getBoundKey('strafeLeft'),
        rightKey = getBoundKey('strafeRight');
      const rotLeftKey = getBoundKey('rotateLeft'),
        rotRightKey = getBoundKey('rotateRight');
