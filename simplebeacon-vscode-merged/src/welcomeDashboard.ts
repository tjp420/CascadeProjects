...
export class WelcomeDashboard {
  ...
  static createOrShow(extUri: vscode.Uri, force = false) {
    const displayMode = vscode.workspace.getConfiguration('simplebeacon').get<string>('displayMode', 'sidebar');
  ...
  static updateReportPaneIfOpen(data: { files?: string; gate?: string; issues?: string; score?: string; severity?: any; findings?: any; filesList?: any }) {
  ...
  public updateSettingsPane(data?: { severity?: any; qualityScore?: string; issues?: string; gate?: string }) {
    ...
    this.queueOrPostMessage({
      ...
      apiUrl: config.get('apiServerUrl', 'http://localhost:55000') || 'http://localhost:55000',
  ...
  public updateReportPane(data: { files?: string; gate?: string; issues?: string; score?: string; severity?: any; findings?: any; filesList?: any }) {
    ...
    this.panel.webview.onDidReceiveMessage(async (msg) => {
      ...
      switch (msg.command) {
        ...
        case 'openPreviewInBrowser': {
          const { ModernSidebarProvider } = require('./modernSidebarProvider');
          if (ModernSidebarProvider && typeof ModernSidebarProvider.openSidebarInBrowserStatic === 'function') {
            ModernSidebarProvider.openSidebarInBrowserStatic('/');
          } else {
            vscode.window.showInformationMessage('Sidebar not available. Open the SimpleBeacon sidebar first.');
          }
          break;
        }
        case 'openAnalyze':
        ...
        case 'inviteTeamMember':
          vscode.window.showInformationMessage('Invite sent to ' + (msg.email || '') + ' as ' + (msg.role || 'viewer'));
          break;
        case 'openScan':
          this.showScanPane();
          break;
        ...
        case 'saveAnalysis':
          vscode.window.showInformationMessage('Save analysis configuration — feature coming soon');
          break;
        case 'exportAnalysis':
        ...
        case 'drillDownAnalysis':
          vscode.window.showInformationMessage(`Drill-down: ${msg.dimension} — feature coming soon`);
          break;
        case 'sortAnalysisTable':
          this.panel.webview.postMessage({ command: 'sortAnalysisTable', key: msg.key });
          break;
        case 'toggleAnalysisCompare':
          vscode.window.showInformationMessage(`Compare ${msg.enabled ? 'enabled' : 'disabled'} — feature coming soon`);
          break;
        case 'refreshReport':
          vscode.window.showInformationMessage('Refreshing report data...');
          vscode.commands.executeCommand('simplebeacon.scanWorkspace');
          break;
        case 'exportReport':
        ...
        case 'exportCodeMap':
          ...
          break;
        case 'refreshCodeMap':
          vscode.window.showInformationMessage('Refreshing code map data...');
          vscode.commands.executeCommand('simplebeacon.generateCodeMap');
          break;
        case 'openFullRoadmap':
          ...
          break;
        case 'exportRoadmap':
          vscode.window.showInformationMessage('Roadmap export is handled in the dashboard.');
          break;
        ...
        case 'openFullUpload':
          ...
          break;
        case 'showInfo':
          vscode.window.showInformationMessage(msg.text || 'Info');
          break;
        case 'validateUpload':
          vscode.window.showInformationMessage(msg.fileCount ? `Validated ${msg.fileCount} uploaded file(s)` : 'Validating uploaded files...');
          break;
        ...
        case 'openFullTrust':
          ...
          break;
        case 'verifyTrust':
          vscode.window.showInformationMessage('Running trust verification...');
          break;
        ...
        case 'refreshPlatform':
          vscode.window.showInformationMessage('Refreshing platform data...');
          break;
        ...
        case 'refreshAnalytics':
          vscode.window.showInformationMessage('Refreshing analytics...');
          break;
        ...
        case 'openFullTeam':
          ...
          break;
        case 'inviteTeamMember':
          vscode.window.showInformationMessage('Team invitation feature coming soon');
          break;
        ...
        case 'testApiConnection':
          {
            const url = (msg.value || config.get('apiServerUrl', 'http://localhost:55000') as string).replace(/\/$/, '') + '/health';
            try {
        ...
        case 'resetSettings':
          ...
          await config.update('apiServerUrl', 'http://localhost:55000', true);
  ...
  <div class="db-container">
    <div class="analyze-hero">
      <div class="analyze-hero-main">
        <h2>Analyze</h2>
        <span class="analyze-build-badge" id="analyzeBuildBadge">v${this.version}</span>
      </div>
        ...
        <div class="map-section" id="mapGraphWrap">
          ...
          <div class="map-graph-frame">
            ...
            <div class="graph-controls" id="graphControls" style="display:none;">
              <button id="graphResetBtn" title="Reset view">&#8634;</button>
...
function bindBtn(id, paneId, command) {
  ...
  if (el) {
    el.addEventListener('click', () => {
      try { addTab(paneId.replace('Pane',''), paneId); } catch (e) { /* ignore addTab errors */ }
      try { activateTab(paneId); } catch (e) { /* ignore activateTab errors */ }
      if (command) {
        try { vscode.postMessage({ command }); } catch (e) { /* ignore postMessage errors */ }
      }
    });
  }
}
...
function renderCodeMapGraph(canvas, graph, style = 'force') {
  ...
  const colors = {'.js':'#f7df1e','.ts':'#3178c6','.tsx':'#3178c6','.jsx':'#61dafb','.cjs':'#f0db4f','.mjs':'#f0db4f','.py':'#3776ab','.json':'#f8fafc','.html':'#e34c26','.css':'#38bdf8','.md':'#8b5cf6','.yml':'#ef4444','.yaml':'#ef4444'};
  const W = () => canvas.width, H = () => canvas.height;
  const nodeMap = {};
  const nodes = graph.nodes.map((n, i) => {
    const node = { id: n.id, label: n.label || n.id, group: n.group || '', size: n.size || 1, x: W() / 2 + Math.cos(i * 2.4) * 150, y: H() / 2 + Math.sin(i * 2.4) * 150, vx: 0, vy: 0, radius: Math.max(4, Math.min(14, Math.sqrt(n.size || 1) * 0.6)) };
    nodeMap[n.id] = node;
    return node;
  });
  ...
  canvas.addEventListener('wheel', e => { e.preventDefault(); const rect = canvas.getBoundingClientRect(); const w = screenToWorld(e.clientX - rect.left, e.clientY - rect.top); const factor = e.deltaY > 0 ? 0.9 : 1.1; scale = Math.max(0.2, Math.min(4, scale * factor)); pan.x = (e.clientX - rect.left) - w.x * scale; pan.y = (e.clientY - rect.top) - w.y * scale; }, { passive: false });
  let t = 0;
  function step() {
    ...
    for (const n of nodes) {
      ...
      if (showLabels && (n.radius >= 6 || n === hoverNode)) {
  ...
  return {
    ...
    zoomOut() { scale = Math.max(0.2, scale / 1.2); },
...
if (aiScanBtn) aiScanBtn.addEventListener('click', () => vscode.postMessage({ command: 'openFullAiContext' }));
...
vscode.postMessage({ command: 'ready' });
  ...
  public static buildBrowserHtml(report?: Record<string, unknown>): string {
    ...
    if (WelcomeDashboard.currentPanel) {
      ...
      // Build a browser-safe version without a real webview panel
      const tempPanel = {
        webview: {
          cspSource: '*',
          asWebviewUri: (uri: any) => uri && uri.toString ? uri.toString() : String(uri),
        },
      } as any;
      const instance = Object.create(WelcomeDashboard.prototype);
      instance.panel = tempPanel;
      html = WelcomeDashboard.prototype.buildHtml.call(instance);
    }
    // Browser mode: remove VS Code: CSP and nonce so the page runs standalone
    html = html.replace(/<meta http-equiv="Content-Security-Policy"[^>]*>/gi, '');
    html = html.replace(/<script nonce="[^"]*">/g, '<script>');
    // Inject a browser bridge so tab switching works without the VS Code: API
    const bridge = `<script>
(function(){
  if (typeof window !== 'undefined') {
    window.__SB_BROWSER_MODE__ = true;
    window.__SB_DISPATCH__ = function(msg) {
      window.dispatchEvent(new MessageEvent('message', { data: msg, origin: window.location.origin }));
    };
    window.__SB_LOAD_REPORT__ = function() {
      fetch('/api/data')
        .then(function(r){ return r.json(); })
        .then(function(state){
          var report = state && state.currentReport;
          if (!report) return;
          var summary = report.summary || {};
          var sev = report.severityCounts || summary.severityCounts || {};
          var findings = report.findings || report.rawIssues || report.detectedIssues || [];
          var issueCount = report.issueCount || findings.length || summary.totalFindings || 0;
          var score = report.qualityScore != null ? String(report.qualityScore) : (report.score != null ? String(report.score) : '--');
          var gatePass = report.gate && report.gate.pass;
          var gate = gatePass ? 'Pass' : 'Fail';
          var critical = sev.critical || 0;
          var high = sev.high || 0;
          var medium = sev.medium || 0;
          var low = sev.low || 0;
          var files = String(report.totalFiles || report.filesAnalyzed || report.fileCount || '--');
          var mappedFindings = findings.slice(0, 20).map(function(f){
            return { severity: f.severity || 'low', type: f.type || 'Finding', text: f.message || f.description || f.title || '', file: f.file || f.path || '--' };
          });
          window.__SB_DISPATCH__({ command: 'updateDashboard', score: score, gate: gate, issues: String(issueCount), files: files, severity: sev, findings: mappedFindings });
          var vulnTypes = ['innerHTML XSS Risk','Sensitive Data Exposure','Production Leak','Configuration Drift','Token Bleed','DB Anti-Pattern','Eval Danger','Debug Artifact'];
          var vulnCount = findings.filter(function(f){ return vulnTypes.indexOf(f.type) >= 0; }).length;
          var secretCount = findings.filter(function(f){ return f.type === 'Sensitive Data Exposure' || f.patternId === 'sensitiveData' || f.patternId === 'tokenBleed' || f.patternId === 'credentialLeak'; }).length;
          var codeSmellCount = findings.filter(function(f){ return ['Lint Error','Style Violation','Dead Code','Duplicate Code','Complexity'].indexOf(f.type) >= 0; }).length;
          var complianceCount = findings.filter(function(f){ return ['EU AI Act','GDPR','SOC2','ISO27001'].indexOf(f.type) >= 0; }).length;
          var recommendations = [];
          ...
          var auditFindings = findings.slice(0, 20).map(function(f){
            return { severity: f.severity || 'low', type: f.type || 'Finding', text: f.title || f.message || f.description || '', file: f.file || '--' };
          });
          window.__SB_DISPATCH__({ command: 'updateAuditPane', vulnerabilities: String(vulnCount), secrets: String(secretCount), passed: String(Math.max(0, 100 - (vulnCount + secretCount))), score: score === '--' ? '--' : String(Math.max(0, 100 - (vulnCount * 10 + secretCount * 5))), status: gatePass ? 'Pass' : 'Fail', critical: String(critical), high: String(high), medium: String(medium), low: String(low), catSecrets: String(secretCount), catVulns: String(vulnCount), catSmells: String(codeSmellCount), catCompliance: String(complianceCount), findings: auditFindings, recommendations: recommendations, gate: gate });
          var reportFindings = findings.slice(0, 50).map(function(f){
            return { title: f.title || f.type || f.message || 'Finding', severity: f.severity || 'medium', type: f.type || '', file: f.file || f.path || '', line: f.line || 1, category: f.category || f.type || 'General', description: f.description || f.detail || f.message || '' };
          });
          window.__SB_DISPATCH__({ command: 'updateReportPane', score: score, gate: gate, issues: String(issueCount), files: files, severity: sev, findings: reportFindings, filesList: (report.files || []).slice(0, 30) });
        })
        .catch(function(e){ /* simplebeacon-ignore debug-artifact — browser bridge error handler */ });
    };
    window.vscode = {
      postMessage: function(msg) {
        if (msg && msg.command === 'ready') { return; }
        // Navigate to combined IDE page when preview is requested in browser
        if (msg && msg.command === 'openPreviewInBrowser') {
          window.location.href = '/';
          return;
        }
        // Local tab switching commands can be handled in the browser
        if (msg && (msg.command === 'openDashboard' || msg.command === 'openAnalyze' || msg.command === 'openReport' || msg.command === 'openCertificate' || msg.command === 'openCodeMap' || msg.command === 'openRoadmap' || msg.command === 'openAiContext' || msg.command === 'openUpload' || msg.command === 'openAudit' || msg.command === 'openSecurity' || msg.command === 'openTrust' || msg.command === 'openQuality' || msg.command === 'openAssessments' || msg.command === 'openPlatform' || msg.command === 'openProfile' || msg.command === 'openCompliance' || msg.command === 'openRepoHealth' || msg.command === 'openAnalytics' || msg.command === 'openTeam' || msg.command === 'openScan')) {
          if (typeof activateTab === 'function') {
            const map = { openDashboard:'dashboardPane', openAnalyze:'analyzePane', openReport:'reportPane', openCertificate:'certificatePane', openCodeMap:'codeMapPane', openRoadmap:'roadmapPane', openAiContext:'aiContextPane', openUpload:'uploadPane', openAudit:'auditPane', openSecurity:'securityPane', openTrust:'trustPane', openQuality:'qualityPane', openAssessments:'assessmentsPane', openPlatform:'platformPane', openProfile:'profilePane', openCompliance:'compliancePane', openRepoHealth:'repoHealthPane', openAnalytics:'analyticsPane', openTeam:'teamPane', openScan:'scanPane' };
            try { addTab(map[msg.command].replace('Pane',''), map[msg.command]); } catch (e) {}
            activateTab(map[msg.command] || msg.command.replace(/^open/, '').toLowerCase() + 'Pane');
          }
          return;
        }
        // Data messages are handled by the internal dashboard message handler
        if (msg && msg.command && msg.command.indexOf('update') === 0) {
          window.__SB_DISPATCH__(msg);
          return;
        }
        // Other commands are logged but require the VS Code: extension to run
        /* simplebeacon-ignore debug-artifact — browser bridge command stub */
      }
    };
  }
})();
</script>`;
    // Load report data once the dashboard has initialized
    const dataScript = `<script>setTimeout(function(){ if (typeof window.__SB_LOAD_REPORT__ === 'function') window.__SB_LOAD_REPORT__(); }, 150);</script>`;
