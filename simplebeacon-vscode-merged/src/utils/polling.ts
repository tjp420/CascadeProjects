// simplebeacon-ignore memory-leak — polling utility functions
...
export function createPoller(
  ...
  opts?: {
    immediate?: boolean;
    onError?: (err: Error, count: number) => void;
    maxRetries?: number;
  }
  ...
  let running = false;
  let errorCount = 0;
  const maxRetries = Number.isFinite(opts?.maxRetries) ? Math.max(0, opts?.maxRetries ?? 0) : 0;
