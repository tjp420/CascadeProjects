// simplebeacon-ignore memory-leak — report generation with short-lived iterations
...
/**
 * Team dashboard for tracking collaborative security review sessions.
 */
export class TeamDashboard {
  ...
  async getTeamMetrics(): Promise<TeamMetrics> {
    const activeMembers = this.teamMembers.filter(
      (m) => Date.now() - m.lastActive.getTime() < 7 * 24 * 60 * 60 * 1000 // Active in last 7 days
    );

    const averageQualityScore =
      this.teamMembers.reduce((sum, m) => sum + m.contributions.qualityScore, 0) / this.teamMembers.length || 0;
    ...
    const teamProductivity =
      activeMembers.length > 0 ? (totalIssuesFixed + totalCodeReviews) / activeMembers.length : 0;
  ...
  private async generateTeamInsights(): Promise<void> {
    ...
    if (metrics.collaborationScore > 80) {
      this.insights.push({
        ...
        impact: 'high',
        actionable: false,
        suggestedActions: ['Share best practices with other teams', 'Document collaboration processes'],
        affectedMembers: this.teamMembers.map((m) => m.name),
        timeframe: 'Ongoing',
      });
    }
