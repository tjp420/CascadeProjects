...
/**
 * Webview panel for displaying detailed scan report findings.
 */
  ...
  private normalizeReport(r: unknown): unknown {
    ...
    if (report.summary) {
      ...
      return {
        ...
        qualityScore:
          high + critical === 0 ? 100 : Math.max(0, 100 - (high + critical) * 5 - severityCounts.medium * 2),
        gate: {
          pass: high + critical === 0,
          failOn: ['high'],
          warnOn: ['medium', 'low'],
          blockingCount: high + critical,
          warningCount: report.summary.totalFindings || 0,
        },
...
<div class="container">
  ...
  <div class="toolbar">
    <input class="search" id="search" placeholder="Search findings..." aria-label="Search findings" />
...
setTimeout(()=>{
  ...
  private buildCategoryChart(categories: { label: string; count: number; severity: string }[]): string {
    if (!categories.length)
      return '<div style="color:var(--muted);font-size:.85rem;text-align:center;padding:20px;">No findings</div>';
    ...
    return categories
      ...
      .map((c, i) => {
        ...
        return `<div class="bar-row" style="animation:fadeInUp .4s ease ${i * 0.06}s both;"><div class="bar-label">${c.label}</div><div class="bar-track"><div class="bar-fill" style="width:0%;background:${color};" data-count="${c.count}" data-target="${pct}"></div></div><div class="bar-val">${c.count}</div></div>`;
      })
      .join('');
  }
  ...
  private buildSeverityChart(findings: { cat: string; sev: string }[]): string {
    ...
    return `<div class="sev-breakdown">${items.map((it, i) => `<div class="sev-item" style="animation:fadeInUp .4s ease ${i * 0.15}s both;"><div class="sev-ring" style="--color:${it.color};--pct:${Math.round((it.count / total) * 100)}"><span style="color:${it.color}">${it.count}</span></div><div class="sev-name">${it.name}</div></div>`).join('')}</div>`;
  }
}
