...
export interface DashboardDeps {
  ...
  pushRoadmapToSidebar?: (report: unknown) => void;
}

export function safeUpdateUIs(report: unknown, deps: DashboardDeps, statusMessage?: string): void {
  const { outputChannel, context, modernSidebarProvider, summaryProvider, scanCountRef, pushRoadmapToSidebar } = deps;
  ...
  try {
    if (pushRoadmapToSidebar) {
      pushRoadmapToSidebar(report);
    }
  } catch (e) {
    outputChannel.appendLine(`[SimpleBeacon] Roadmap push failed: ${e}`);
  }
  try {
  ...
  try {
    ...
    const gateRaw =
      (r?.gate as any)?.pass ?? r?.gateStatus ?? summary?.gatePass ?? summary?.gate ?? scanSummary?.status ?? 'Pending';
    const gateText = typeof gateRaw === 'string' ? gateRaw.toUpperCase() : gateRaw ? 'PASS' : 'FAIL';
    const gate =
      gateText === 'PASS' ||
      gateText === 'PASSED' ||
      gateText === 'SUCCESS' ||
      ...
      gateText === 'OK'
        ? 'PASS'
        : gateText === 'FAIL' || gateText === 'FAILED' || gateText === 'ERROR'
          ? 'FAIL'
          : String(gateRaw);
    ...
    const dashboardRawSevCounts =
      (r?.severityCounts as Record<string, number>) || (summary?.severityCounts as Record<string, number>) || {};
    const scanSevCounts = {
    ...
    const dashboardRawScore =
      (r?.qualityScore as number | string) ??
      (r?.score as number | string) ??
      summary?.qualityScore ??
      summary?.score ??
      scanSummary?.qualityScore ??
      null;
    const numericScore =
      dashboardRawScore === null ||
      dashboardRawScore === undefined ||
      String(dashboardRawScore).toLowerCase().includes('hidden') ||
      isNaN(Number(dashboardRawScore))
        ? null
        : Number(dashboardRawScore);
    const score = numericScore !== null ? String(numericScore) : '--';
    const sev = dashboardSevCounts;

    const dashboardFindings = ((r?.findings as any[]) || (r?.rawIssues as any[]) || (r?.detectedIssues as any[]) || [])
      .slice(0, 20)
      .map((f: any) => ({
    ...
    modernSidebarProvider.updateReport({
      gate,
      issues,
      score,
      qualityScore: score,
    ...
    WelcomeDashboard.updateReportPaneIfOpen({ files, gate, issues, score, severity: sev });
    outputChannel.appendLine(
      `[SimpleBeacon] Dashboard update: gate=${gate}, issues=${issues}, score=${score}, files=${files}, severity=${JSON.stringify(sev)}, findings=${dashboardFindings.length}`
    );
    ...
    const certModules = String(
      (r?.files as any[])?.length || (r?.fileCount as number) || summary?.filesAnalyzed || '0'
    );
    ...
    try {
      modernSidebarProvider.updateReport({ ...(r || {}), certificate: panes.certificate });
    } catch (e) {
      /* ignore */
    }
    try {
      WelcomeDashboard.updateCertificatePaneIfOpen(panes.certificate as any);
    } catch (e) {
      /* ignore */
    }
    ...
    panes.roadmap = {
      open: String(openVulns),
      risk,
      done: '0',
      target,
      status: gate === 'PASS' ? 'Active' : 'Blocked',
      severity: sev,
    ...
    const aiFindings = findings
      .filter(
        (f) =>
          ['AI Indicator', 'AI Residue', 'LLM Slop', 'Fiction KPI', 'Hallucinated Import', 'Placeholder Code'].includes(
            f?.type
          ) ||
    ...
    const aiModels = [
      {
        ...
        meta: 'Common patterns detected',
        status: aiFindings.length > 0 ? 'Detected' : 'Monitoring',
      },
      {
        name: 'Code Generator',
        meta: 'Stub / boilerplate patterns',
        status: findings.some((f) => (f.type || '').includes('Stub')) ? 'Detected' : 'Monitoring',
      },
      {
        name: 'Documentation Bot',
        meta: 'Inline comment patterns',
        status: findings.some((f) => (f.type || '').includes('Comment')) ? 'Detected' : 'Monitoring',
      },
    ];
    panes.aiContext = {
      files,
      ...
      score,
      severity: sev,
      status: gate === 'PASS' ? 'Clear' : 'Issues Found',
      models: String(aiFindings.length > 0 ? aiFindings.length : '0'),
      aiFindings,
      aiModels,
    };
    ...
    const secretCount = findings.filter(
      (f) =>
        f?.type === 'Sensitive Data Exposure' ||
        f?.patternId === 'sensitiveData' ||
        f?.patternId === 'tokenBleed' ||
        f?.patternId === 'credentialLeak'
    ).length;
    const codeSmellCount = findings.filter((f) =>
      ['Lint Error', 'Style Violation', 'Dead Code', 'Duplicate Code', 'Complexity'].includes(f?.type)
    ).length;
    ...
    const sevCounts =
      Object.keys(rawSevCounts).length > 0
        ? rawSevCounts
        : (() => {
    ...
    const trustScore = String(
      (r?.summary as any)?.qualityScore ??
        (r?.summary as any)?.score ??
        (r?.qualityScore as any) ??
        (r?.score as any) ??
        '--'
    );
    ...
    const trustPayload = {
      trustScore,
      verified: gate === 'PASS' ? 'Yes' : 'No',
      warnings: String((sev.medium || 0) + (sev.low || 0)),
    ...
    if (rawScore != null) {
      ...
      const penalty = Math.min(
        80,
        (sevCounts.critical || 0) * 10 +
          (sevCounts.high || 0) * 5 +
          (sevCounts.medium || 0) * 2 +
          (sevCounts.low || 0) * 0.5
      );
    ...
    panes.quality = {
      ...
      coverage: String((r?.summary as any)?.coverage ?? '--'),
      files,
      status: gate === 'PASS' ? 'Pass' : 'Fail',
    ...
    const asstCompleted =
      gate === 'PASS'
        ? '5'
        : String(Math.max(0, 5 - (critical > 0 ? 1 : 0) - ((sev.high || 0) > 0 ? 1 : 0) - (secretCount > 0 ? 1 : 0)));
    ...
    const asstChecklist = [
      { text: 'Code quality gate passed', checked: gate === 'PASS', status: gate === 'PASS' ? 'Pass' : 'Fail' },
      {
        ...
        status: findings.length > 0 ? 'Complete' : 'Pending',
      },
    ...
    panes.platform = {
      ...
      ext: getExtensionVersion(context),
    ...
    panes.profile = {
      ...
      activity: [
        {
          icon: '\u{1F50D}',
          text: 'Scan completed — ' + findings.length + ' issues found',
    ...
    const compRules = [
      {
        id: 'sensitive-logs',
        text: 'No sensitive data in logs',
        severity: 'critical',
        pass: !findings.some((f) => f.type === 'Sensitive Data Exposure' && (f.file || '').includes('log')),
      },
      {
        id: 'license',
        text: 'Dependency license compliance',
        severity: 'high',
        pass: !findings.some((f) => f.type === 'License Issue'),
      },
      { id: 'conduct', text: 'Code of conduct present', severity: 'medium', pass: true },
      {
        id: 'security-policy',
        text: 'Security policy defined',
        severity: 'medium',
        pass: !findings.some((f) => f.type === 'Security Policy Missing'),
      },
    ...
    const rhFindings = findings.slice(0, 10).map((f) => ({
      severity: f.severity || 'low',
      text: f.message || f.type || 'Finding',
      file: f.file || 'unknown',
    }));
    const rhRecs = [] as any[];
    ...
    if (report && statusMessage && /complete/i.test(statusMessage)) {
      scanCountRef.value += 1;
    }
    const scans = String(Math.max(1, scanCountRef.value));
    const generatedAt = (r as any)?.generatedAt;
    const lastScan = generatedAt
      ? isNaN(new Date(generatedAt).getTime())
    ...
    panes.team = {
      ...
      score: qualityScore,
      status: 'Active',
      membersList: [{ name: 'Admin', role: 'Project Owner', status: 'Active' }],
      severity: sev,
      qualityScore,
      issues: qualityIssues,
      gate,
    ...
    const scResults = findings.slice(0, 10).map((f) => ({
      severity: f.severity || 'low',
      type: f.type || 'Finding',
      text: f.message || f.type || 'Finding',
      file: f.file || 'unknown',
      line: f.line != null ? f.line : undefined,
    }));
    const scHistory = [
      { text: 'Workspace scan completed', time: new Date().toLocaleTimeString(), score: qualityScore },
    ];
    panes.scan = {
      total: scans,
      issues,
    ...
    try {
      modernSidebarProvider.updateAnalyticsPane(panes.analytics);
    } catch {
      /* sidebar may not support analytics pane */
    }
  } catch (e) {
