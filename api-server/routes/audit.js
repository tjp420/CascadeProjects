...
router.get(
  ...
  async (req, res) => {
    ...
    if (from && !parseIsoDate(from))
      return res
        .status(400)
        .json({ error: "Invalid from date; use ISO format" });
    if (to && !parseIsoDate(to))
      return res.status(400).json({ error: "Invalid to date; use ISO format" });
    ...
    try {
      ...
      res.setHeader(
        "X-Prev-Offset",
        String(Math.max(pageOffset - pageLimit, 0)),
      );
      ...
      res
        .status(500)
        .json({ error: "Audit query failed", detail: err.message });
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    ...
    if (!["json", "csv"].includes(format))
      return res
        .status(400)
        .json({ error: "Invalid format; supported: json, csv" });
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Audit export failed", detail: err.message });
    }
  },
);
