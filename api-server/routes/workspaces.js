...
const { requirePermission, requireWorkspaceMembership, setWorkspaceRlsContext } = require('../lib/rbac.js');
...
router.post('/api/v2/workspaces', requirePermission('workspace.admin'), async (req, res) => {
    ...
    try {
        const existing = await db.get('SELECT id FROM workspaces WHERE slug = $1', [slug]);
        if (existing) {
        ...
        if (!targetOrgId) {
            ...
            if (!org) {
                return res.status(400).json({ error: 'No organization found — create one first or provide orgId' });
            }
            targetOrgId = org.id;
        }
        ...
        const workspace = await db.get(
            ...
            [name, slug, description || null, req.auth.userId, req.auth.email, targetOrgId]
        );
...
router.patch('/api/v2/workspaces/:workspaceId',
    ...
    async (req, res) => {
        const { name, description, gatePolicy, ignorePatterns, scanScheduleCron, maxMembers } = req.body || {};
        const updates = [];
        const params = [];
        let idx = 1;
        ...
        if (gatePolicy) { updates.push(`gate_policy = $${idx++}`); params.push(JSON.stringify(gatePolicy)); }
        if (ignorePatterns) { updates.push(`ignore_patterns = $${idx++}`); params.push(ignorePatterns); }
        if (scanScheduleCron) { updates.push(`scan_schedule_cron = $${idx++}`); params.push(scanScheduleCron); }
        if (maxMembers) { updates.push(`max_members = $${idx++}`); params.push(maxMembers); }
...
router.post('/api/v2/workspaces/:workspaceId/invitations',
    ...
    async (req, res) => {
        ...
        try {
            const roleRow = await db.get('SELECT id FROM roles WHERE name = $1', [role]);
            if (!roleRow) {
            ...
            const targetUser = await db.get('SELECT id FROM users WHERE email = $1', [email.toLowerCase()]);

            await db.query(
                `INSERT INTO workspace_members
                 (workspace_id, user_id, role_id, invite_email, invite_token_hash, invite_expires_at, invitation_accepted, invited_by)
                 VALUES ($1, $2, $3, $4, $5, $6, false, $7)`,
                [req.params.workspaceId, targetUser?.id || null, roleRow.id, email.toLowerCase(), tokenHash, expiresAt, req.auth.userId]
            );
 ...
 */
router.post('/api/v2/workspaces/:workspaceId/invitations/:token/accept', async (req, res) => {
    ...
    try {
        ...
        res.json({ message: 'Invitation accepted', workspaceId: invite.workspace_id });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
