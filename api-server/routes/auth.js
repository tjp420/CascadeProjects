...
router.post('/api/v2/auth/login', async (req, res) => {
  ...
  try {
    ...
    // Update last login
    await db.query(`UPDATE users SET last_login_at = now(), last_login_ip = $1 WHERE id = $2`, [
      req.ip || null,
      user.id,
    ]);
...
router.post('/api/v2/auth/mfa/verify', requireAuth, async (req, res) => {
  ...
  const user = await db.get(`SELECT mfa_secret FROM users WHERE id = $1`, [req.auth.userId]);
  if (!user?.mfa_secret) {
  ...
  await db.query(`UPDATE users SET mfa_enabled = true WHERE id = $1`, [req.auth.userId]);
