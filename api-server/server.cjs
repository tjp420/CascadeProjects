...
if (!process.env.SIMPLEBEACON_LICENSE_SECRET) {
  logger.error(
    "[Env] FATAL: SIMPLEBEACON_LICENSE_SECRET not set. Server requires a secure secret.",
  );
  logger.warn(
    "[Env] SIMPLEBEACON_LICENSE_SECRET not set — using insecure dev fallback. DO NOT USE IN PRODUCTION.",
  );
...
function cleanupExpiredRateLimiters() {
  ...
  for (const [ip, entry] of freeTokenLog) {
    if (now - entry.createdAt >= FREE_TOKEN_COOLDOWN_MS)
      freeTokenLog.delete(ip);
  }
...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "Content-Type, Authorization, X-Token-Password, X-SimpleBeacon-Bridge-Token",
  );
...
try {
  ...
  } = require("../ai-platform/src/api/simplebeacon-billing-api.cjs");
  ...
  try {
    const {
      setupSubscriptionWebhook,
    } = require("../coming-soon/routes/subscriptions-billing.cjs");
...
try {
  ...
  } = require("../ai-platform/server/routes/flexible-analyze-api.cjs");
...
app.get("/", (req, res) => {
  res
    .status(200)
    .json({ status: "ok", service: "simplebeacon-api", version: "1.3.0" });
});
...
try {
  const {
    runSimplebeaconScan,
  } = require("../ai-platform/src/api/simplebeacon-api.cjs");
  app.post(
...
app.post("/api/notify", express.json({ limit: "100kb" }), (req, res) => {
  try {
    ...
    const type =
      entry.type || (entry.payload && entry.payload.type) || "unknown";
...
app.get("/api/analyze/wiring", async (_req, res) => {
  ...
  try {
    ...
    checks.jwt.detail = checks.jwt.ok
  ...
  try {
    const routes = app._router.stack
      ...
      .map((s) =>
        s.route ? s.route.path : s.regexp ? s.regexp.toString() : s.name,
      );
    checks.aiPlatform.ok = routes.some(
      (r) => r && r.includes && r.includes("analyze"),
    );
    checks.aiPlatform.detail = checks.aiPlatform.ok
      ? "flexible-analyze-api mounted"
      : "not detected in stack";
  } catch (e) {
...
app.post("/api/scan-directory", express.json({ limit: "1mb" }), (req, res) => {
  try {
    ...
    function walk(rootDir) {
      ...
      while (stack.length > 0) {
        ...
        for (const name of names) {
          ...
          if (
            /^tmp-[^/]*\\.(js|txt)$|^patch-main\\d*\\.js$|^repair\\.py$/.test(
              rel,
            )
          ) {
            skippedDir++;
          ...
          try {
            ...
            statFail++;
            if (entryCount <= 50)
              logger.info(`[Scan Directory] STAT_FAIL ${rel}: ${e.message}`);
            continue;
          }
          if (stat.isDirectory()) {
            dirEntry++;
    ...
    const PATTERNS = {
      aiSdk:
        /openai|anthropic|claude|google-generative-ai|langchain|llamaindex|chromadb|gpt-4|gpt-3\.5|stable-diffusion|dall-e|whisper|transformers\.pipeline/i,
      credential:
        /password\s*[:=]\s*['"][^'"]{8,}|api[_-]?key\s*[:=]\s*['"][^'"]{8,}|secret\s*[:=]\s*['"][^'"]{8,}|token\s*[:=]\s*['"][^'"]{8,}|aws_access_key_id|private[_-]?key/i,
      debugArtifact:
        /console\.(log|warn|error|info|debug)\s*\(|debugger\s*;?|alert\s*\(|confirm\s*\(/i,
    ...
    for (const { full, rel } of allFiles) {
      ...
      if (text.length < 5 * 1024 * 1024) {
        for (const [key, regex] of Object.entries(PATTERNS)) {
          ...
          if (
            key === "largeComment" &&
            /tmp-js-check\.|repair\.|deploy-auto\./.test(rel)
          )
            continue;
          if (
            key === "debugArtifact" &&
            /\/vendor\/|\.min\.js$|\.bundle\.min\.js$/.test(rel)
          )
            continue;
          if (
            key === "perf" &&
    ...
    const qualityScore = Math.max(
      0,
      Math.round(100 - Math.min(issuesPerK * 3, 60)),
    );
    ...
    const report = {
      ...
      cleanup: {
        ...
        debugArtifacts: [
          ...new Set(findings.debugArtifact.map((i) => i.file)),
        ].slice(0, 10),
      ...
      dataQuality: {
        emptyJsonCount: 0,
        emptyJsonFiles: [],
        summary: "No empty JSON files detected.",
      },
    };
    ...
    res
      .status(500)
      .json({ error: "Scan failed. Check server logs for details." });
  }
});
...
function buildAiPrompt(report) {
  ...
  for (const issue of issues.slice(0, 10)) {
    lines.push(
      `- [${md(issue.severity)}] ${md(issue.type)}: ${md(issue.humanReadable || issue.impact)}`,
    );
  ...
  (req, res) => {
    try {
      ...
      if (!projectPath || !fsSync.existsSync(projectPath)) {
        return res
          .status(400)
          .json({ error: "Invalid or missing projectPath" });
      }

      const SKIP_DIRS_ANALYZE = SHARED_SKIP_DIRS;
      const BINARY_EXTS_ANALYZE = SHARED_BINARY_EXTS;
      const COPY_FILE =
        / copy( \d+)?\.(xml|txt|tfvars|py|js|ts|cjs|mjs|json|md|html|css|scss|sass|less|yml|yaml)$/i;
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        chainResult.error !== "Token not registered in chain registry."
      ) {
        return null;
      }
    } catch {
...
app.get("/api/dashboard/stats", (_req, res) => {
  try {
    ...
    const subs = db
      .getDb()
      .prepare(
        "SELECT COUNT(*) as count FROM paid_subscriptions WHERE status = ?",
      )
      .get("active");
    const totalCerts = db
      .getDb()
      ...
      for (const name of names.slice(0, 50)) {
        try {
          const data = JSON.parse(
            fsSync.readFileSync(path.join(sbDir, name), "utf8"),
          );
          files.push({
            name,
            title:
              data.title || data.scanProfileLabel || name.replace(".json", ""),
            generatedAt: data.generatedAt || null,
            qualityScore:
              data.qualityScore || data.summary?.qualityScore || null,
...
async function handleLicenseStatus(req, res) {
  try {
    ...
    try {
      customer = await db.get("SELECT * FROM customers WHERE email = $1", [
        payload.email,
      ]);
      ...
      logger.warn(
        "[TokenStatus] Subscription DB unavailable, failing open for valid token:",
        dbErr.message,
      );
    }
    const registered = customer ? true : null;
    const hasActiveSubscription = hasSubscription
      ? Number(hasSubscription.count) > 0
      : null;
    const active =
      hasActiveSubscription !== null
        ? registered && hasActiveSubscription
        : true;
    const upgradeUrl =
...
app.get("/api/platform/status", (_req, res) =>
  res.json({ online: true, status: "ok", version: "3.2.0" }),
);
app.get("/api/dashboard-home", (_req, res) =>
  res.json({ sections: [], widgets: [], user: null }),
);
app.get("/api/dev-tools/tools", (_req, res) => res.json({ tools: [] }));
app.get("/api/dev-tools/workflows", (_req, res) => res.json({ workflows: [] }));
app.get("/api/security/overview", (_req, res) =>
  res.json({ score: 100, issues: 0, status: "ok" }),
);
app.get("/api/coverage-reports/overview", (_req, res) =>
  res.json({ coverage: 0, reports: [] }),
);
app.get("/api/help", (_req, res) => res.json({ topics: [], faqs: [] }));
app.get("/api/quality/overview", (_req, res) =>
  res.json({ score: 100, metrics: {}, status: "ok" }),
);
app.get("/api/optimization/health", (_req, res) =>
  res.json({ healthy: true, checks: [] }),
);
app.get("/api/health", (_req, res) =>
  res.json({ status: "ok", service: "simplebeacon-api" }),
);
...
const server = app.listen(PORT, async () => {
  ...
  if (
    process.env.ENABLE_DB_AUTO_MIGRATE === "true" &&
