    ...
    for (const { full, rel } of allFiles) {
      ...
      if (text.length < 5 * 1024 * 1024) {
        for (const [key, regex] of Object.entries(PATTERNS)) {
          ...
          if (key === 'debugArtifact' && /\/vendor\/|\.min\.js$|\.bundle\.min\.js$/.test(rel))
            continue;
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (isTextLike && size < 5 * 1024 * 1024) {
            try {
              totalLines += fsSync.readFileSync(full, 'utf8').split('\n').length;
            } catch (_) {
              readErrors++;
            }
          }
    ...
    try {
      const names = fsSync
        ...
        .filter(
          (f) =>
            f.endsWith('.json') &&
            (f.includes('report') || f.includes('scan') || f.includes('assessment'))
        );
...
app.get('/api/platform/status', (_req, res) =>
  res.json({ online: true, status: 'ok', version: '1.3.0' })
);
