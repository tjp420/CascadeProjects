...
const JWT_SECRET = process.env.SIMPLEBEACON_JWT_SECRET || process.env.SIMPLEBEACON_LICENSE_SECRET;
if (!JWT_SECRET || JWT_SECRET.length < 32) {
    throw new Error('SIMPLEBEACON_JWT_SECRET (or SIMPLEBEACON_LICENSE_SECRET) must be set and >= 32 chars');
}
...
function issueAccessToken(payload) {
    return jwt.sign(
        ...
        JWT_SECRET,
        { expiresIn: ACCESS_TOKEN_EXPIRY, issuer: 'simplebeacon', audience: 'simplebeacon-api' }
    );
}
...
function issueRefreshToken(payload) {
    return jwt.sign(
        ...
        JWT_SECRET,
        { expiresIn: REFRESH_TOKEN_EXPIRY, issuer: 'simplebeacon', audience: 'simplebeacon-api' }
    );
}
...
function verifyToken(token) {
    return jwt.verify(token, JWT_SECRET, { issuer: 'simplebeacon', audience: 'simplebeacon-api' });
}
