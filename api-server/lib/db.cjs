...
const DATABASE_URL =
  process.env.DATABASE_URL || "postgresql://localhost:5432/simplebeacon";

// Hardened, environment-driven pool defaults
const POOL_MAX = Number.parseInt(process.env.DB_POOL_MAX || "", 10) || 15;
const IDLE_TIMEOUT_MS =
  Number.parseInt(process.env.DB_IDLE_TIMEOUT_MS || "", 10) || 45000;
const CONNECT_TIMEOUT_MS =
  Number.parseInt(process.env.DB_CONNECT_TIMEOUT_MS || "", 10) || 5000;
...
try {
  const projectedInstances = Math.max(
    1,
    Number.parseInt(process.env.PROJECTED_INSTANCE_COUNT || "1", 10),
  );
  const dbServerMax = Number.parseInt(
    process.env.DB_SERVER_MAX_CONNECTIONS || "0",
    10,
  );
  if (dbServerMax > 0) {
    ...
    if (projectedConnections > dbServerMax) {
      const msg =
        `DB pool sizing risk: projected connections ${projectedConnections} exceed DB_SERVER_MAX_CONNECTIONS ${dbServerMax}. ` +
  ...
  console.warn(
    "DB sizing validator encountered an error:",
