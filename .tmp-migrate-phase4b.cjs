...
for (const file of filesToMigrate) {
    ...
    const pattern2 = /res\.status\((\d+)\)\.json\(\{ error: ('[^']*'|`[^`]*`), message: ([^}]+) \}\)/g;
    const matches2 = content.match(pattern2) || [];
    content = content.replace(pattern2, (match, status, error, message) => {
        replacements++;
        return 'sendError(res, ' + status + ', ' + error.trim() + ', { message: ' + message.trim() + ' })';
    });
    ...
    // res.status(N).json({\n  success: false,\n  error: '...'\n})
    const pattern4 = /res\.status\((\d+)\)\.json\(\{\s*\n\s*success: false,\s*\n\s*error: ([^}\n]+)\s*\n\s*\}\)/g;
    ...
    const pattern5 = /res\.status\((\d+)\)\.json\(\{\s*\n\s*error: ('[^']*'|`[^`]*`),\s*\n\s*message: ([^}\n]+)\s*\n\s*\}\)/g;
...
for (const r of results) {
    ...
    console.log(r.file.padEnd(35) + ' | ' + r.status.padEnd(12) + ' | ' + r.replacements + ' replacements' + importNote);
}
