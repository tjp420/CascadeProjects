...
module.exports = {
  activate: function activate(context, vscode) {
    ...
    try {
      ...
      console.log('SimpleBeacon plugin installed. Registered commands:', result.registeredCommands);
    } catch (err) {
    ...
    const scanCmd = vscode.commands.registerCommand('simplebeacon.runQuickScan', async () => {
      const projectPath = resolveSafeProjectPath(vscode);
      if (!projectPath) {
        vscode.window.showErrorMessage('SimpleBeacon: No workspace folder open. Open a folder and try again.');
        return;
      }
        ...
        async () => {
          try {
            ...
            vscode.window.showInformationMessage(
              ok ? 'SimpleBeacon: Gate PASSED' : 'SimpleBeacon: Gate reported findings — see Output'
            );
          } catch (err) {
    ...
    // --- Gate Status command ---
    const gateCmd = vscode.commands.registerCommand('simplebeacon.gateStatus', async () => {
      const projectPath = resolveSafeProjectPath(vscode);
      if (!projectPath) {
        vscode.window.showErrorMessage('SimpleBeacon: No workspace folder open.');
        return;
      }
      try {
        ...
        if (!gate) {
          vscode.window.showWarningMessage('SimpleBeacon: No gate data — run a scan first.');
          return;
        }
        out.show(true);
        ...
        vscode.window.showErrorMessage('SimpleBeacon gate status failed: ' + err.message);
      }
    });
