...
describe("getPlanningPrompt", () => {
  ...
  it("mentions available operations", () => {
    ...
    assert.ok(
      ...
      "Should mention read_file operation",
    );
    assert.ok(
      prompt.includes("patch_file"),
      "Should mention patch_file operation",
    );
    assert.ok(
      prompt.includes("run_tests"),
      "Should mention run_tests operation",
    );
  });
  ...
  it("insists on JSON array output only", () => {
    ...
    assert.ok(
      ...
      "Should require JSON array output",
    );
    assert.ok(
      prompt.includes("Output ONLY"),
      "Should restrict output to JSON only",
    );
  });
...
describe("getVerificationPrompt", () => {
  it("returns a string containing the terminal output", () => {
    ...
    assert.ok(
      ...
      "Prompt should include the terminal output",
    );
  });
