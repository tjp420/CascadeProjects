...
describe("orchestrator", () => {
  it("callLocalModel throws when Ollama is unreachable", async () => {
    ...
    try {
      await assert.rejects(callLocalModel("test"), /Ollama Connection Error/);
    } finally {
...
describe("parsePlan", () => {
  ...
  it("throws for non-array JSON", () => {
    const { parsePlan } = require("../orchestrator.cjs");
    assert.throws(
      () => parsePlan('{"op":"read_file"}'),
      /Plan is not a JSON array/,
    );
  });
});
...
describe("executePatchFile", () => {
  ...
  it("applies a valid patch", () => {
    ...
    const result = executePatchFile(
      {
        op: "patch_file",
        path: "tmp-patch-test.js",
        search: "hello world",
