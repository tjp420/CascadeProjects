...
function buildPrompt(report) {
  // simplebeacon-ignore debug-artifact — legitimate function name, not a debug stub
  ...
  const topIssues = issues
    ...
    .map(
      (i) =>
        `- [${i.severity?.toUpperCase() || "MEDIUM"}] ${i.type || i.rule || "Finding"}: ${i.description || i.message || ""} (${i.file || i.filePath || ""}${i.line ? ":" + i.line : ""})`,
    )
    .join("\n");
...
async function main() {
  ...
  if (!reportPath) {
    process.stderr.write(
      ["Usage: node report-analyzer.cjs <path-to-report.json>"].join(" ") +
        "\n",
