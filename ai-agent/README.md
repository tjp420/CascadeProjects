...
try {
  const { handlers, registeredCommands } = adapter.install(context, vscode, { pluginPath: './ai-agent/plugins/simplebeacon-plugin.cjs' });
  ...
  console.error('Failed to install SimpleBeacon plugin into VS Code agent:', err);
}
```
