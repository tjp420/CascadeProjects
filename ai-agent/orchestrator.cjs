...
function debugLog(...args) {
  if (DEBUG) process.stdout.write([...args].join(" ") + "\n");
} // simplebeacon-ignore debug-artifact — gated by AGENT_DEBUG env var
function debugError(...args) {
  if (DEBUG) process.stderr.write([...args].join(" ") + "\n");
} // simplebeacon-ignore debug-artifact — gated by AGENT_DEBUG env var
...
async function callLocalModel(prompt) {
  ...
  try {
    ...
    throw new Error(
      `[Ollama Connection Error] Ensure Ollama is running local model ${modelName}: ${err.message}`,
    );
  }
}
...
function validateStep(step, stepNum) {
  if (!step || typeof step.op !== "string") {
    debugError(
      `❌ [Step ${stepNum}] Invalid step structure: missing "op" field`,
    );
  ...
  if (step.path) {
    if (looksLikeHallucinatedPath(step.path)) {
      debugError(
        `🛡️ [Safety Block] Step ${stepNum} tried to reference hallucinated path: ${step.path}`,
      );
      return {
        valid: false,
        error: `Ghost file safety violation: ${step.path}`,
      };
    }
    const fullPath = path.resolve(process.cwd(), step.path);
    if (!fs.existsSync(fullPath)) {
      debugError(
        `🛡️ [Safety Block] Step ${stepNum} tried to reference ghost file: ${step.path}`,
      );
      return {
        valid: false,
        error: `Ghost file safety violation: ${step.path}`,
...
function executePatchFile(step, stepNum) {
  ...
  if (!step.search || !step.replace) {
    debugError(
      `❌ [Step ${stepNum}] patch_file requires "search" and "replace" fields`,
    );
  ...
  return {
    ok: true,
    log: { step: stepNum, op: "patch_file", path: step.path, status: "ok" },
  };
}

function runTests() {
  try {
    const logOutput = execSync("npm test", {
      encoding: "utf8",
      stdio: "pipe",
      cwd: process.cwd(),
    });
...
async function verifyTests(logOutput, stepNum) {
  debugLog(
    `🔍 [Step ${stepNum}] Sending test output to ${MODEL_NAME} for verification...`,
  );
  let assessment;
  try {
    ...
    debugError(
      `❌ [Step ${stepNum}] Cannot reach Ollama for verification: ${err.message}`,
    );
...
async function executeRunTests(stepNum) {
  ...
  return {
    ok: true,
    log: { step: stepNum, op: "run_tests", exitCode: testExitCode },
  };
}

const stepExecutors = {
  read_file: (step, stepNum) => ({
    ok: true,
    log: executeReadFile(step, stepNum),
  }),
  patch_file: (step, stepNum) => {
    const result = executePatchFile(step, stepNum);
    return result.ok
      ? { ok: true, log: result.log }
      : { ok: false, error: result.error, step: stepNum };
  },
...
async function runLocalAgent(userGoal) {
  ...
  try {
    ...
    debugError(
      "❌ [Agent Failure] The 2B model failed to return a valid JSON array structure.",
    );
    debugError("   Raw response:", planResponse.slice(0, 500));
    return {
      success: false,
      error: "Malformed plan structure",
      raw: planResponse,
    };
  }
...
async function runOptimizedAgent(userGoal, opts = {}) {
  ...
  const { prompt, estimatedTokens, breakdown } =
    await tokenOptimizer.preparePromptForEdit(
      projectRoot,
      changedPaths,
      userGoal,
  ...
  debugLog(
    `   Summaries: ${breakdown.summaryTokens} tokens, Retrieval: ${breakdown.retrievalTokens} tokens`,
  );

  if (!breakdown.withinBudget) {
    debugLog(
      `⚠️ [Warning] Prompt exceeds budget (${estimatedTokens} > ${maxTokens})`,
    );
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (!patch) {
      debugLog(
        `⚠️ [Attempt ${attempt}] Could not parse patch from model response`,
      );
...
async function escalateToRemote(params) {
  ...
  return {
    ...
    message:
      "Local model could not resolve the task. Escalation prompt built for remote model.",
  };
}
