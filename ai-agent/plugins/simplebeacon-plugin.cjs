...
module.exports = {
  ...
  register: function register(agent) {
    ...
    const registerHandler = (typeof agent.registerHandler === 'function')
      ...
      : (typeof agent.on === 'function') ? agent.on.bind(agent) : null;
    ...
    function attachHelpers() {
      ...
      agent.simplebeacon.scanProject = async (opts = {}) => handlerScanProject(opts);
      agent.simplebeacon.getGateStatus = async (opts = {}) => handlerGateStatus(opts);
    }

    async function runSimplebeaconCli(args, cwd) {
      const npx = process.platform === 'win32' ? 'npx.cmd' : 'npx';
      try {
        const { stdout } = await execFileP(npx, ['simplebeacon', ...args], { cwd: cwd || process.cwd(), maxBuffer: 20 * 1024 * 1024 });
        // CLI prints JSON when --format json and --output - are used
        try {
          return { success: true, stdout: stdout, json: JSON.parse(stdout) };
        } catch (e) {
          return { success: true, stdout: stdout, json: null, parseError: e.message };
        }
      } catch (err) {
        return { success: false, error: err.message, code: err.code, stderr: err.stderr && err.stderr.toString() };
      }
    }
