...
async function scanAndSummarize(projectRoot, changedPaths) {
  ...
  for (const relPath of changedPaths) {
    ...
    if (![".js", ".cjs", ".mjs", ".ts", ".tsx", ".py", ".jsx"].includes(ext))
      continue;
...
function buildSummaryText(filePath, imports, functions, classes, exports) {
  ...
  if (imports.length > 0) {
    ...
    imports
      .slice(0, 10)
      .forEach((i) => parts.push(`- L${i.line}: ${i.module}`));
  }
  ...
  function walk(dir, depth) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        results.push(
          path.relative(root, path.join(dir, entry.name)).replace(/\\/g, "/"),
        );
      }
    }
  }
 ...
 */
async function preparePromptForEdit(
  projectRoot,
  changedPaths,
  intent,
  opts = {},
  ...
  const { summaries, totalTokens: summaryTokens } = await scanAndSummarize(
    projectRoot,
    changedPaths,
  );
  ...
  if (index && index.length > 0) {
    ...
    retrievalTokens = retrievalResults.reduce(
  ...
  if (retrievalResults.length > 0) {
    ...
    for (const r of retrievalResults) {
      parts.push(
        `## ${r.filePath} (L${r.startLine}-${r.endLine}, score: ${r.score.toFixed(3)})`,
      );
      if (includeSnippets) {
        parts.push("```");
  ...
  parts.push(
    "Do not modify unrelated code. Include the file path and line numbers in any patch.",
  );
...
module.exports = {
  ...
  version: "1.0.0",
  description:
    "Token optimization module — summarizer, embeddings, prompt builder, usage logging",
