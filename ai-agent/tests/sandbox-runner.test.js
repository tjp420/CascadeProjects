...
test('createSandbox copies files into a temp directory', () => {
    ...
    assert.equal(fs.readFileSync(path.join(sandbox, 'app.js'), 'utf8'), 'function app() { return 42; }');
...
test('applyPatch replaces text in sandbox file', () => {
    ...
    const result = applyPatch(sandbox, 'target.js', 'const x = 1;', 'const x = 42;');
    assert.ok(result.ok);
    assert.ok(result.diff.includes('-const x = 1;'));
    assert.ok(result.diff.includes('+const x = 42;'));
    assert.ok(fs.readFileSync(path.join(sandbox, 'target.js'), 'utf8').includes('const x = 42;'));
...
test('applyPatch fails when search string not found', () => {
    ...
    const sandbox = createSandbox(tmp, ['target.js']);
    const result = applyPatch(sandbox, 'target.js', 'nonexistent code', 'replacement');
    assert.ok(!result.ok);
