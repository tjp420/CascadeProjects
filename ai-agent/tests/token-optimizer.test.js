...
test('buildEmbeddingsAndIndex creates chunks with embeddings', async () => {
    ...
    const result = await buildEmbeddingsAndIndex(tmp, { maxFiles: 10, chunkSize: 40 });
...
test('topKContext returns ranked results', async () => {
    ...
    fs.writeFileSync(path.join(tmp, 'auth.js'), 'function authenticate(user, password) { return true; }');
    fs.writeFileSync(path.join(tmp, 'billing.js'), 'function processPayment(amount, currency) { return true; }');
    fs.writeFileSync(path.join(tmp, 'utils.js'), 'function formatDate(date) { return date.toString(); }');
    ...
    assert.ok(results[0].filePath.includes('auth'), `Expected auth.js, got ${results[0].filePath}`);
    fs.rmSync(tmp, { recursive: true, force: true });
});
...
test('preparePromptForEdit uses index for retrieval when provided', async () => {
    ...
    fs.writeFileSync(path.join(tmp, 'target.js'), 'function target() { return 42; }');
    fs.writeFileSync(path.join(tmp, 'related.js'), 'function relatedUtil() { return "helper"; }');
...
test('logTokenUsage stores entries', () => {
    ...
    logTokenUsage({ event: 'edit', promptTokens: 500, completionTokens: 200, model: 'gpt-4', savedTokens: 5000 });
...
test('getTokenUsageSummary returns aggregated stats', () => {
    logTokenUsage({ event: 'edit', promptTokens: 100, completionTokens: 50, model: 'gpt-4', savedTokens: 1000 });
    logTokenUsage({ event: 'edit', promptTokens: 200, completionTokens: 100, model: 'gpt-4', savedTokens: 2000 });
...
test('register attaches handlers to agent context', () => {
    const registered = [];
    const agent = {
        registerHandler: (name, fn) => { registered.push(name); },
        simplebeacon: {},
    };
