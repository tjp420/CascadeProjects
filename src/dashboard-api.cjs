...
async function computeKpis() {
    const result = await db.query('SELECT COUNT(*) as total FROM features WHERE active = true');
    return {
...
router.get('/dashboard', async (req, res) => {
    try {
        const metrics = await db.query('SELECT * FROM metrics ORDER BY created_at DESC LIMIT 30');
        const analytics = await db.query('SELECT * FROM user_analytics ORDER BY created_at DESC LIMIT 30');
        ...
        res.status(500).json({ error: 'Dashboard data fetch failed', detail: err.message });
    }
});
