...
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || 'http://127.0.0.1,http://localhost')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
app.use(
  '/api/',
  rateLimit({ windowMs: 60 * 1000, max: parseInt(process.env.RATE_LIMIT_MAX || '100', 10) })
);
