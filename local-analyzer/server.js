...
const ALLOWED_PATHS = (process.env.ALLOWED_PATHS || process.cwd())
  .split(";")
  ...
  try {
    ...
    if (req.query.ext) {
      ...
      filtered = filtered.filter(
        (i) => i.name && i.name.toLowerCase().endsWith(ext),
      );
    }
    if (req.query.minMB) {
      const min = parseFloat(req.query.minMB) || 0;
      filtered = filtered.filter(
        (i) => (i.sizeBytes || 0) / (1024 * 1024) >= min,
      );
    }
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === "size") v = (a.sizeBytes || 0) - (b.sizeBytes || 0);
        ...
        v = new Date(a.lastModified) - new Date(b.lastModified);
    ...
    const perPage = Math.min(
      1000,
      Math.max(1, parseInt(req.query.perPage || "200", 10)),
    );
...
app.get("/", (req, res) =>
  res.json({ ok: true, message: "Local Analyzer bridge" }),
);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.debug &&
      console.debug(
        `⚡ HDD Analyzer bridge running at http://localhost:${PORT}`,
      );
