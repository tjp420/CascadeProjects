...
const ACCOUNT_ID =
  process.env.R2_ACCOUNT_ID || "8815946f6d5847844ea60cf05e43f871";
...
async function main() {
  ...
  if (!accessKeyId || !secretAccessKey) {
    console.error(
      "ERROR: R2_ACCESS_KEY_ID and R2_SECRET_ACCESS_KEY environment variables required.",
    );
    console.error("");
  ...
  console.log(
    `Size:   ${(totalSize / 1024 / 1024 / 1024).toFixed(2)} GB (${totalSize} bytes)`,
  );
  ...
  try {
    ...
    console.error(
      "Make sure your R2 API token has Object Read & Write permission for this bucket.",
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < totalParts; i++) {
    ...
    console.log(
      `  Uploading part ${partNumber}/${totalParts} (${(chunkSize / 1024 / 1024).toFixed(1)}MB)...`,
    );

    const uploadResp = await s3.send(
    ...
    console.log(
      `    ✓ Done in ${partDuration}s (${overallProgress}% complete)`,
    );
  }
  ...
  console.log("");
  console.log(
    "Next: Create a public bucket URL or Worker to serve this file to users.",
  );
}
