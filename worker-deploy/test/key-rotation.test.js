...
const ROTATE_SCRIPT = path.resolve(import.meta.dirname, '..', 'scripts', 'rotate-edge-keys.cjs');
const GENERATE_SCRIPT = path.resolve(import.meta.dirname, '..', 'scripts', 'generate-signing-keys.cjs');
...
test('rotate-edge-keys.cjs archives old key and writes new key', async () => {
    ...
    try {
        ...
        assert.notEqual(oldKeyId, newKeyId, 'new key should be different from old key');
    } finally {
        // Clean up
        if (fs.existsSync(keysDir)) {
            fs.rmSync(keysDir, { recursive: true, force: true });
...
test('certificate signed with old key fails verification against new public key', async () => {
    ...
    const oldPrivateJwk = await crypto.subtle.exportKey('jwk', oldKeypair.privateKey);
    ...
    assert.equal(isValid, false, 'old signature should NOT verify against new public key');
});
...
test('certificate signed with new key verifies against new public key', async () => {
    ...
    const newPrivateJwk = await crypto.subtle.exportKey('jwk', newKeypair.privateKey);
    const newPublicJwk = await crypto.subtle.exportKey('jwk', newKeypair.publicKey);
    ...
    const isValid = await verifyWithKey(newPublicJwk, signature, payload);
    assert.equal(isValid, true, 'new signature should verify against new public key');
});
...
test('JWK thumbprint differs for different keys', async () => {
    ...
    assert.notEqual(thumb1, thumb2, 'different keys should have different thumbprints');
});
