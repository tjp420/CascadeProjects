...
import { handleCertifyRequest, handlePublicKeyRequest } from '../src/certify.js';
...
test('signature can be verified with the public key', async () => {
    ...
    const sigBytes = new Uint8Array(body.signature.match(/.{2}/g).map(h => parseInt(h, 16)));
...
test('CORS origin is applied to successful responses', async () => {
    ...
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://simplebeacon.ai');
});
...
test('internal error details are not leaked to client', async () => {
    ...
    const req = mockRequest('POST', payload);
    const env = mockEnv({ SIGNING_PRIVATE_KEY: JSON.stringify({ kty: 'EC', crv: 'P-256', d: 'invalid-base64url' }) });
    const res = await handleCertifyRequest(req, env, 'https://simplebeacon.ai');
...
test('GET /api/v1/certify/public-key returns the JWK', async () => {
    ...
    assert.match(body.keyId, /^sb-edge-[a-f0-9]{16}$/, 'keyId should be sb-edge- + 16 hex chars');
