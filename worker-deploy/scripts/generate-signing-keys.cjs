...
function parseArgs(argv) {
    ...
    for (let i = 2; i < argv.length; i++) {
        ...
        if (arg === '--worker') {
            ...
            console.log('Usage: node generate-signing-keys.js [--worker <name>] [--dry-run]');
            console.log('');
            console.log('Generates an ECDSA P-256 keypair for compliance certificate signing.');
            console.log('');
            console.log('Options:');
...
async function main() {
    ...
    console.log('Generating ECDSA P-256 keypair for edge compliance signing...\n');
    ...
    console.log('── Cloudflare Secrets Setup Commands ──────────────────────────');
    console.log('');
    console.log('Run these commands to inject the keys into your Cloudflare Worker:');
    console.log('');
    console.log(`# Set the private signing key (used by /api/v1/certify)`);
    console.log(`echo '${privateKeyJson}' | npx wrangler secret put SIGNING_PRIVATE_KEY --name ${args.worker}`);
    console.log('');
    console.log(`# Set the public verification key (used by /api/v1/certify/public-key)`);
    console.log(`echo '${publicKeyJson}' | npx wrangler secret put SIGNING_PUBLIC_KEY --name ${args.worker}`);
    console.log('');
    console.log('── Key Details ───────────────────────────────────────────────');
    console.log(`  Algorithm:  ECDSA P-256`);
    // Compute the JWK thumbprint for the key ID
    const thumbprintCanonical = JSON.stringify({ crv: publicKeyJwk.crv, kty: publicKeyJwk.kty, x: publicKeyJwk.x, y: publicKeyJwk.y });
    const thumbprintHash = crypto.createHash('sha256').update(thumbprintCanonical, 'utf8').digest('hex');
    console.log(`  Key ID:     sb-edge-${thumbprintHash.slice(0, 16)}`);
    console.log(`  Private key: ${privateKeyJwk.d.slice(0, 8)}...${privateKeyJwk.d.slice(-8)} (${privateKeyJwk.d.length} chars)`);
    console.log(`  Public key:  x=${publicKeyJwk.x.slice(0, 8)}... y=${publicKeyJwk.y.slice(0, 8)}...`);
    console.log('');
    console.log('⚠  IMPORTANT: Never commit the private key to your repository.');
    console.log('   The .dev-keys/ directory is gitignored. Use wrangler secret put');
    console.log('   to inject keys directly into the Worker runtime environment.');
}
