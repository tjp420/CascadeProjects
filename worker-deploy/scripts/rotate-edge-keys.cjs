...
function parseArgs(argv) {
  const args = {
    worker: "simplebeacon-dashboard-v2",
    dryRun: false,
    execute: false,
  };
  for (let i = 2; i < argv.length; i++) {
    ...
    if (arg === "--worker") {
      ...
      console.log(
        "Usage: node rotate-edge-keys.cjs [--worker <name>] [--execute] [--dry-run]",
      );
      console.log("");
      console.log(
        "Rotates the ECDSA P-256 signing keypair for compliance certificates.",
      );
      console.log("");
      console.log("Options:");
      console.log(
      ...
      console.log(
        "  --execute        Run wrangler secret put commands automatically",
      );
      console.log(
      ...
      console.log("");
      console.log(
        "Without --execute, the script prints the wrangler commands for you to run manually.",
      );
      console.log(
        "With --execute, it runs them directly (requires wrangler auth).",
      );
      process.exit(0);
    }
  }
...
async function main() {
  ...
  console.log(
    "Rotating ECDSA P-256 signing keypair for edge compliance certificates...\n",
  );
  ...
  if (fs.existsSync(currentPubKeyPath) && !args.dryRun) {
    ...
    const currentPubKey = JSON.parse(
      fs.readFileSync(currentPubKeyPath, "utf8"),
    );
    const oldKeyId = await computeJwkThumbprint(currentPubKey);
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const archivePath = path.join(
      archiveDir,
      `public-key-${oldKeyId.slice(0, 16)}-${timestamp}.json`,
    );
    fs.writeFileSync(
      archivePath,
      JSON.stringify(currentPubKey, null, 2),
      "utf8",
    );
    console.log(
      `✓ Archived current public key to ${path.relative(process.cwd(), archivePath)}`,
    );
    ...
    const oldKeyId = await computeJwkThumbprint(currentPubKey);
    console.log(
      `[dry-run] Would archive current public key (key ID: sb-edge-${oldKeyId.slice(0, 16)})\n`,
    );
  } else {
  ...
  if (!args.dryRun) {
    ...
    console.log(
      `✓ New keys written to ${path.relative(process.cwd(), keysDir)}/`,
    );
  }
  ...
  if (args.execute && !args.dryRun) {
    console.log(
      "── Executing Wrangler Secret Updates ──────────────────────────\n",
    );
    ...
    console.log(
      "── Wrangler Secret Update Commands ────────────────────────────",
    );
    console.log("");
  ...
  console.log(
    `  Archived:   ${args.dryRun ? "(dry-run)" : fs.existsSync(archiveDir) ? "Yes — in .dev-keys/archive/" : "No previous key"}`,
  );
  console.log("");
  console.log("⚠  IMPORTANT:");
  console.log(
    "   • After updating secrets, deploy the Worker to pick up the new keys:",
  );
  console.log(`     npx wrangler deploy --config worker-deploy/wrangler.jsonc`);
  console.log(
    "   • Old certificates signed with the previous key will fail verification",
  );
  console.log(
    "     against the new public key. Archive old public keys for auditors who",
  );
  console.log("     need to verify historical certificates.");
  console.log(
    "   • The .dev-keys/ directory is gitignored — never commit these keys.",
  );
}
