...
function checkExpiringLicenses(activeLicenses, lookaheadDays = 30) {
  if (!Array.isArray(activeLicenses)) {
    console.warn(
      "[SimpleBeacon] checkExpiringLicenses expected an array, received:",
      typeof activeLicenses,
    );
    return [];
  }
  const days =
    Number.isFinite(lookaheadDays) && lookaheadDays > 0
      ...
      : 30;
  console.log(
    `[SimpleBeacon] Running License Renewal Audit (Lookahead: ${days} Days)...`,
  );
  ...
  for (const license of activeLicenses) {
    if (
      ...
      console.warn(
        "[SimpleBeacon] Skipping license with missing/invalid expiresAt:",
        license?.companyId,
      );
      continue;
    }
    ...
    if (Number.isNaN(expTime)) {
      console.warn(
        `[SimpleBeacon] Skipping license for ${license.companyId}: invalid date "${license.expiresAt}"`,
      );
      continue;
    }
    ...
    if (expTime > now.getTime() && expTime <= targetThresholdTime) {
      const daysRemaining = Math.ceil(
        (expTime - now.getTime()) / (1000 * 60 * 60 * 24),
      );
