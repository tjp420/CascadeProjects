...
function rotateLicenseToken(oldToken, oldPublicKeyPem, newPrivateKeyPem) {
  ...
  if (typeof oldPublicKeyPem !== "string" || !oldPublicKeyPem) {
    return {
      success: false,
      error: "oldPublicKeyPem must be a non-empty string",
    };
  }
  if (typeof newPrivateKeyPem !== "string" || !newPrivateKeyPem) {
    return {
      success: false,
      error: "newPrivateKeyPem must be a non-empty string",
    };
  }
  ...
  try {
    ...
    if (!payloadBase64 || !signatureBase64) {
      return {
        success: false,
        error: "Invalid token format: missing payload or signature",
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: `Token verification error: ${err?.message || String(err)}`,
      };
    }
    ...
    if (!isValid) {
      return {
        success: false,
        error: "Token verification failed against the provided old public key.",
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to decode license payload: ${err?.message || String(err)}`,
      };
    }
    ...
    if (!meta || typeof meta !== "object") {
      return {
        success: false,
        error: "Decoded license payload is not a valid object",
      };
    }
    if (typeof meta.companyId !== "string" || !meta.companyId) {
      return {
        success: false,
        error: "Decoded payload missing valid companyId",
      };
    }
...
if (require.main === module) {
  console.log(
    "[SimpleBeacon] Initializing Key Rotation Validation Subsystem...",
  );
  ...
  console.log(
    rotationResult.success
      ? "ROTATION CRYPTO MATRIX PASSED"
      : "ROTATION CRYPTO MATRIX FAILED",
  );
}
