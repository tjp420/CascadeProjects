...
test("dashboard app mounts without JavaScript fallback", async ({
  page,
  baseURL,
}) => {
  ...
  const rootUrl =
    baseURL || process.env.DASHBOARD_BASE_URL || "http://127.0.0.1:3003";
  ...
  expect(
    pageErrors,
    `Page errors found on ${targetUrl}: ${pageErrors.join("; ")}`,
  ).toEqual([]);
});
