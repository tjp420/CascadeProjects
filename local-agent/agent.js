...
const ALLOWED_ORIGINS = (
  ...
  "https://cascadeprojects-yzzd.onrender.com,http://localhost:3000,http://127.0.0.1:3000,http://localhost:4000,http://127.0.0.1:4000,https://simplebeacon.pages.dev"
).split(",");
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      try {
        if (String(origin).indexOf("simplebeacon.pages.dev") !== -1)
          return callback(null, true);
        const u = new URL(origin);
        if (u.hostname && u.hostname.endsWith(".simplebeacon.pages.dev"))
          return callback(null, true);
      } catch (e) {
        // fallthrough
      }
...
app.post("/api/analyze", analyzeLimiter, async (req, res) => {
  ...
  const targetPath = path.normalize(
    req.body && req.body.path ? String(req.body.path).trim() : "",
  );
  ...
  try {
    ...
    const rules = [
      {
        ...
        severity: "HIGH",
        regex: new RegExp(
          "(sk_live_" + "[a-zA-Z0-9]{24,}" + "|" + "AKIA[0-9A-Z]{16})",
          "g",
        ),
        ...
        try {
          ...
          if (stat.isFile()) {
            ...
            for (const rule of rules) {
              ...
              if (matchCount > 0) {
                if (rule.severity === "HIGH") highRiskCount += matchCount;
                if (rule.severity === "MEDIUM") mediumRiskCount += matchCount;
            ...
            fileReport.push({
              ...
              status:
                fileIssues.length > 0
                  ? `Issues Flagged: ${fileIssues.join(", ")}`
                  : "Clean",
            });
    ...
    if (score < 0) score = 0;
    if (highRiskCount > 0) score = Math.min(score, 55); // Immediate F override for exposed live keys
...
app.listen(PORT, () => {
  if (process.env.SB_DEBUG === "1") {
    process.stdout.write(
      [
        `SimpleBeacon Native Scanning Agent active on http://localhost:${PORT}`,
      ].join(" ") + "\n",
    );
  }
});
