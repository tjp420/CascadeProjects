...
const sampleSource = fs.readFileSync(
    path.join(__dirname, '..', 'ai-platform', 'web', 'simplebeacon-dashboard', 'js-es2018', 'utils', 'sample-report.js'),
    'utf8'
);
const cjsSample = sampleSource
...
const cjsAdapter = adapterSource
    .replace(/export function/g, 'function')
    .replace(/^export \{[^}]+\};?$/m, '')
...
describe('generateSampleReport severity distribution', () => {
    ...
    test('total issues = 12', () => {
        const report = generateSampleReport();
        const total = report.severityCounts.critical + report.severityCounts.high
...
describe('generateSampleReport dashboard widget compatibility', () => {
    ...
    test('sample report is detected by upload zone validation', () => {
        ...
        const isSbReport = report.type?.includes('simplebeacon')
