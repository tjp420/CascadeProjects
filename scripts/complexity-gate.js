...
function findLongFunctions(filePath, maxLines) {
  ...
  while ((match = funcRegex.exec(content)) !== null) {
    ...
    const { lines: funcLines, endIndex } = measureFunctionLines(content, startIndex);
...
function main() {
  ...
  console.log(`❌ Found ${allFindings.length} function(s) exceeding ${args.maxLines} lines:\n`);
  console.log(`${'File'.padEnd(60)} ${'Line'.padEnd(8)} ${'Lines'.padEnd(8)} ${'Function'}`);
