...
const REGEX_PATTERNS = [
  { id: "AWS_ACCESS_KEY_ID", re: /AKIA[0-9A-Z]{16}/g },
  {
    ...
    re: /aws(.{0,20})?(secret|secret_access_key).{0,20}["'=: ]([A-Za-z0-9\/+=]{20,40})/i,
  },
  ...
  {
    id: "JWT_LIKE",
    re: /[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}/g,
  },
];
  ...
  try {
    ...
    for (const p of REGEX_PATTERNS) {
      ...
      if (m && m.length)
        findings.push({
          id: p.id,
          matches: Array.from(new Set(m)).slice(0, 5),
        });
    }
    return findings;
  } catch (err) {
...
function runGitleaksOnFiles(files) {
  ...
  if (hasCommand("gitleaks"))
    tryCmds.push({ cmd: "gitleaks", args: ["detect", "--source"] });
...
function main() {
  ...
  const filtered = changed.filter(
    (f) => !isSkipped(f) && fs.existsSync(f) && fs.statSync(f).isFile(),
  );
