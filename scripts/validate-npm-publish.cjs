...
const EXPECTED_VERSION =
  require("../packages/simplebeacon-cli/package.json").version;
...
if (viewResult.status === 0 && viewResult.stdout.trim() === EXPECTED_VERSION) {
  ...
  console.error(
    `FAIL: Expected ${EXPECTED_VERSION}, got ${viewResult.stdout?.trim() || "none"}`,
  );
  ok = false;
}
...
ok =
  run(
    `cd ${tmpDir} && npm init -y && npm install ${PACKAGE_NAME}@${EXPECTED_VERSION}`,
  ) && ok;
...
if (scanResult.status === 0 && scanResult.stdout.includes("Usage")) {
  ...
  console.error(
    "FAIL: simplebeacon scan --help did not return expected output",
  );
  ok = false;
}
...
console.log(
  "\n" +
    (ok
      ? "All npm publish validation checks PASSED"
      : "Some npm publish validation checks FAILED"),
);
process.exit(ok ? 0 : 1);
