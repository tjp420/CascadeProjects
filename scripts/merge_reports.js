...
function sumField(field) {
  merged[field] = reports.reduce((s, r) => s + (Number(r[field]) || 0), 0);
}
...
numericFields.forEach(
  (f) => (merged[f] = reports.reduce((s, r) => s + (Number(r[f]) || 0), 0)),
);
// severityCounts
merged.severityCounts = { critical: 0, high: 0, medium: 0, low: 0 };
reports.forEach((r) => {
  if (r.severityCounts) {
    merged.severityCounts.critical += r.severityCounts.critical || 0;
    merged.severityCounts.high += r.severityCounts.high || 0;
    merged.severityCounts.medium += r.severityCounts.medium || 0;
    merged.severityCounts.low += r.severityCounts.low || 0;
  }
});
// Merge arrays and de-duplicate simple lists
function concatUnique(key) {
  const acc = [];
  reports.forEach((r) => {
...
merged.detectedIssues = [].concat(
  ...reports.map((r) => r.detectedIssues || []),
);
merged.rawIssues = [].concat(...reports.map((r) => r.rawIssues || []));
// gate
merged.gate = merged.gate || {
  pass: true,
  blockingCount: 0,
  warningCount: 0,
  blockingIssues: [],
...
merged.gate.blockingCount = reports.reduce(
  (s, r) => s + ((r.gate && r.gate.blockingCount) || 0),
  0,
);
merged.gate.warningCount = reports.reduce(
  (s, r) => s + ((r.gate && r.gate.warningCount) || 0),
  0,
);
merged.gate.blockingIssues = [].concat(
  ...reports.map((r) => (r.gate && r.gate.blockingIssues) || []),
);
merged.gate.warningIssues = [].concat(
  ...reports.map((r) => (r.gate && r.gate.warningIssues) || []),
);
merged.gate.pass = merged.gate.blockingCount === 0;
// scan_summary counts
merged.scan_summary = merged.scan_summary || {};
merged.scan_summary.total_risks_found = reports.reduce(
  (s, r) => s + ((r.scan_summary && r.scan_summary.total_risks_found) || 0),
  0,
);
...
const latest = reports.reduce((a, r) => {
  const t = new Date(
    r.generatedAt || (r.scan_summary && r.scan_summary.timestamp) || 0,
  );
  return t > a ? t : a;
}, new Date(0));
merged.generatedAt = new Date(
  Math.max(...reports.map((r) => new Date(r.generatedAt || 0))),
).toISOString();
