...
const suppressRegexes = (rule.suppressPatterns || [])
  .map((sp) => {
    try {
      return new RegExp(sp, "i");
    } catch {
      return null;
    }
  })
...
while ((match = regex.exec(content)) !== null) {
  ...
  const suppressed = suppressRegexes.some(
    (srx) => srx.test(contextWindow) || srx.test(match[0]),
  );
