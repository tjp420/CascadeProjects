...
function markTaskDone(phaseId, descSubstring) {
  ...
  if (!task.done) {
    ...
    phase.taskSummary.todo = Math.max(
      0,
      phase.taskSummary.total - phase.taskSummary.done,
    );
...
markTaskDone(
  "optimization",
  "Address TODO in ai-platform/docs/eu-ai-act-compliance.md",
);
markTaskDone(
  "optimization",
  "Address TODO in ai-platform/docs/risk-assessment.md",
);
markTaskDone(
  "optimization",
  "Address TODO in coming-soon/content/social-posts.md",
);
markTaskDone(
  "optimization",
...
const totalDone = data.phases.reduce(
  (sum, p) => sum + (p.taskSummary?.done || 0),
  0,
);
...
const inProgressPhases = data.phases.filter(
