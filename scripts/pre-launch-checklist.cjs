...
function main() {
  ...
  if (fs.existsSync(cliPackage)) {
    ...
    check(
      "CLI has publishConfig.access = public",
  ...
  check(
    "EULA exists",
    fs.existsSync(path.join(ROOT, "sales", "legal", "EULA.md")),
  );
  check(
  ...
  check(
    "User guide exists",
    fs.existsSync(path.join(ROOT, "sales", "docs", "user-guide.md")),
  );
  check(
    "LLM modes doc exists",
    fs.existsSync(
      path.join(ROOT, "packages", "simplebeacon-cli", "docs", "LLM-MODES.md"),
    ),
  );
  ...
  check(
    "action.yml exists",
    fs.existsSync(path.join(ROOT, "github-action", "action.yml")),
  );
