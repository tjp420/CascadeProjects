...
const {
  TOOL_DEFINITIONS,
} = require("../packages/simplebeacon-cli/src/mcp/tools");
      ...
      inputSchema: {
        ...
        properties: {
          query: {
            type: "string",
            description:
              "Natural language search query (e.g. 'scan for credentials', 'compliance checklist', 'agent memory', 'gate status')",
          },
        },
        required: ["query"],
      },
      ...
      inputSchema: {
        ...
        properties: {
          tool_name: {
            type: "string",
            description:
              "Exact tool name returned by search_available_tools (e.g. 'scan_snippet', 'gate_status', 'agent_remember')",
          },
        },
        required: ["tool_name"],
      },
  ...
  if (mode === "standard") {
    ...
    for (let t = 1; t <= turns; t++) {
      if (t === 1) {
        ...
        breakdown.push({
          turn: t,
          inputTokens: baseTokens + searchTokens + inspectTokens,
          outputTokens: 300,
        });
      } else {
...
console.log(
  `${colors.magenta}       SIMPLEBEACON MCP TOKEN BENCHMARK               ${colors.reset}`,
);
console.log(
  `${colors.cyan}====================================================${colors.reset}\n`,
);
...
console.log(
  `  Pricing:  $${pricing.inputPerM}/M input, $${pricing.outputPerM}/M output\n`,
);
...
console.log(
  `  Standard mode (all 34 tools):     ${colors.red}${stdSingleTurn.toLocaleString()}${colors.reset} tokens/turn`,
);
console.log(
  `  Progressive mode (first turn):    ${colors.yellow}${progFirstTurn.toLocaleString()}${colors.reset} tokens (meta-tools + search + inspect)`,
);
console.log(
  `  Progressive mode (subsequent):    ${colors.green}${progSubsequent.toLocaleString()}${colors.reset} tokens/turn`,
);
console.log(
  `  ${colors.dim}Breakdown: base=${progBase}, search=${progSearch}, inspect=${progInspect}${colors.reset}\n`,
);
...
const stdInputCost =
  (stdSession.totalInputTokens / 1_000_000) * pricing.inputPerM;
const stdOutputCost =
  (stdSession.totalOutputTokens / 1_000_000) * pricing.outputPerM;
const stdTotalCost = stdInputCost + stdOutputCost;

const progInputCost =
  (progSession.totalInputTokens / 1_000_000) * pricing.inputPerM;
const progOutputCost =
  (progSession.totalOutputTokens / 1_000_000) * pricing.outputPerM;
const progTotalCost = progInputCost + progOutputCost;

const savings = stdTotalCost - progTotalCost;
const savingsPct = (
  ((stdTotalCost - progTotalCost) / stdTotalCost) *
  100
).toFixed(1);
const tokenSavingsPct = (
  ((stdSession.totalInputTokens - progSession.totalInputTokens) /
    stdSession.totalInputTokens) *
  100
).toFixed(1);

console.log(
  `${colors.bold}── ${turns}-Turn Session Simulation ──${colors.reset}\n`,
);

console.log(`  ${colors.red}STANDARD MODE${colors.reset}:`);
console.log(
  `    Input tokens:  ${stdSession.totalInputTokens.toLocaleString()}`,
);
console.log(
  `    Output tokens: ${stdSession.totalOutputTokens.toLocaleString()}`,
);
console.log(`    Input cost:    ${formatCost(stdInputCost)}`);
console.log(`    Output cost:   ${formatCost(stdOutputCost)}`);
console.log(
  `    ${colors.bold}Total cost:     ${colors.red}${formatCost(stdTotalCost)}${colors.reset}\n`,
);

console.log(`  ${colors.green}PROGRESSIVE MODE${colors.reset}:`);
console.log(
  `    Input tokens:  ${progSession.totalInputTokens.toLocaleString()}`,
);
console.log(
  `    Output tokens: ${progSession.totalOutputTokens.toLocaleString()}`,
);
console.log(`    Input cost:    ${formatCost(progInputCost)}`);
console.log(`    Output cost:   ${formatCost(progOutputCost)}`);
console.log(
  `    ${colors.bold}Total cost:     ${colors.green}${formatCost(progTotalCost)}${colors.reset}\n`,
);

console.log(`${colors.bold}── Savings ──${colors.reset}\n`);
console.log(
  `  Token reduction:  ${colors.green}${tokenSavingsPct}%${colors.reset} (${(stdSession.totalInputTokens - progSession.totalInputTokens).toLocaleString()} tokens saved)`,
);
console.log(
  `  Cost reduction:   ${colors.green}${savingsPct}%${colors.reset} (${formatCost(savings)} saved per session)\n`,
);

// ── Monthly projections ──────────────────────────────────────────────────────
console.log(
  `${colors.bold}── Monthly Projections (30 days) ──${colors.reset}\n`,
);
...
console.log(
  `  ${"Scenario".padEnd(35)} ${"Standard".padStart(12)} ${"Progressive".padStart(12)} ${"Savings".padStart(12)} ${colors.dim}%'.padStart(6)}${colors.reset}`,
);
console.log(
  `  ${"─".repeat(35)} ${"─".repeat(12)} ${"─".repeat(12)} ${"─".repeat(12)} ${"─".repeat(6)}`,
);
...
for (const s of scenarios) {
  ...
  console.log(
    `  ${s.name.padEnd(35)} ${formatCost(stdMonthly).padStart(12)} ${formatCost(progMonthly).padStart(12)} ${formatCost(monthlySavings).padStart(12)} ${colors.green}${pct.padStart(5)}%${colors.reset}`,
  );
}
...
for (const tw of toolWeights.slice(0, 10)) {
  console.log(
    `  ${tw.name.padEnd(30)} ${colors.yellow}${String(tw.tokens).padStart(5)}${colors.reset} tokens  ${colors.dim}(${tw.propCount} props, ${tw.descLen}ch desc)${colors.reset}`,
  );
}
...
console.log(
  `  Standard per-turn:     ${stdSingleTurn.toLocaleString()} tokens`,
);
console.log(
  `  Progressive per-turn:  ${progSubsequent.toLocaleString()} tokens (after first turn)`,
);
console.log(
  `  ${colors.bold}Compression ratio:      ${colors.green}${ratio}x${colors.reset} reduction after first turn\n`,
);
...
console.log(
  `${colors.green}[BENCHMARK COMPLETE] Progressive discovery saves ${tokenSavingsPct}% tokens (${savingsPct}% cost) per ${turns}-turn session.${colors.reset}`,
);
console.log(
  `${colors.cyan}====================================================${colors.reset}\n`,
);
...
if (parseFloat(savingsPct) > 80) {
  console.log(
    `${colors.green}[VERDICT] Enterprise-ready. Token compression exceeds 80% threshold.${colors.reset}`,
  );
  process.exit(0);
} else if (parseFloat(savingsPct) > 50) {
  console.log(
    `${colors.yellow}[VERDICT] Good savings. Consider additional TOON compression for further gains.${colors.reset}`,
  );
  process.exit(0);
} else {
  console.log(
    `${colors.yellow}[VERDICT] Moderate savings. Review tool schema sizes for further optimization.${colors.reset}`,
  );
  process.exit(0);
}
