...
const SECURE =
  (getArg("secure") || process.env.SMTP_SECURE || "true") !== "false";
...
if (!PASS || PASS === "your Zoho app password") {
  ...
  console.error(
    "  3. Navigate to App Passwords / Application-specific Passwords",
  );
...
async function verifyConnection() {
  ...
  try {
    ...
    console.log(
      "    - Wrong app password (must be Zoho app-specific, not account password)",
    );
    console.log("    - 2FA not enabled on the Zoho account");
    console.log(
      "    - Wrong host (should be smtp.zohocloud.ca for custom domains)",
    );
  ...
  console.log(
    "Step 2: Sending test emails to " + RECIPIENTS.length + " mailboxes...",
  );
  let delivered = 0;
  let failed = 0;
  ...
  if (failed === 0) {
    console.log(
      "All test emails delivered successfully. Zoho SMTP is ready for production.",
    );
    console.log("");
