...
function createMockFile(relativePath, content, size) {
  const text =
    content || "// mock file content for " + relativePath + "\n".repeat(10);
  return {
...
function generateMockFiles(count) {
  ...
  for (let i = 0; i < count; i++) {
    const dir =
      i % 10 === 0 ? "src/components" : i % 5 === 0 ? "src/utils" : "src";
...
function createMockIndexedDB() {
  ...
  function createStore() {
    ...
    return {
      put: (record) => {
        records.set(record.path || record.scanId, record);
      },
  ...
  async _processMessage(data) {
    ...
    if (data.type === "scan-batch") {
      ...
      for (const fileEntry of batch) {
        ...
        const filePath =
          fileEntry.path || file.webkitRelativePath || file.name || "";
        ...
        const hash =
          "h" + (filePath.length * 31 + state.processed).toString(36);
        ...
        if (pendingHashBatch.length >= HASH_BATCH_SIZE) {
          this._send({
            type: "file-hash-batch",
            scanId: state.scanId,
            hashes: pendingHashBatch,
          });
          pendingHashBatch = [];
        }
      ...
      const newIssues = state.issues.slice(
        -(state.issues.length - (state._lastIssueCount || 0)),
      );
      this._send({
        type: "batch-complete",
        scanId: data.scanId,
        ...
        if (filePath.endsWith(".js") || filePath.endsWith(".ts")) {
          ...
          for (let fi = 0; fi < findingCount; fi++) {
            issues.push({
              ...
              count: 1,
              matches: [
                { line: fi + 1, snippet: "console.log(...)", context: [] },
              ],
            });
          }
        }
...
function createMockAbortSignal() {
  const signal = {
    ...
    addEventListener: function (event, cb) {
      if (event === "abort") this._listeners.push(cb);
    },
...
function setupBrowserGlobals() {
  ...
  globalThis.indexedDB = {
    open: () => {
      throw new Error("IndexedDB not available in test");
    },
  };
  ...
  console.warn = function (...args) {
    if (
      ...
      args[0].includes("[AuditScanService] IndexedDB unavailable")
    )
      return;
...
async function runTests() {
  ...
  console.log("\n--- Test 1: Basic scan with " + FILE_COUNT + " files ---");
  {
    ...
    assert(
      result.issueCount > 0,
      "Should have findings (mock files have console.log)",
    );
    assertEqual(
      result.findingsCapped,
      false,
      "Findings should not be capped for " + FILE_COUNT + " files",
    );
    assert(progressCount > 0, "Progress callbacks should fire");
    assert(
      progressCount < FILE_COUNT / 10,
      "Progress should be throttled (< " +
        Math.floor(FILE_COUNT / 10) +
        " callbacks for " +
        FILE_COUNT +
        " files, got " +
        progressCount +
    ...
    assertEqual(
      MockWorker.instances.filter((w) => !w.terminated).length,
      0,
      "All workers should be terminated after scan",
    );
    console.log(
      "  Processed: " +
        result.processed +
  ...
  console.log("\n--- Test 2: Memory cap enforcement (high finding rate) ---");
  {
    ...
    assert(
      result.issueCount > 5000,
      "Should have > 5000 findings (high rate, got " + result.issueCount + ")",
    );
    assert(
      result.findingsCapped === true,
      "Findings should be capped in memory (issueCount=" +
        result.issueCount +
        ")",
    );
    assert(
      result.findings.length <= 5000,
      "In-memory findings array should be capped at 5000 (got " +
  ...
  console.log("\n--- Test 3: Abort mid-scan ---");
  {
    ...
    assertEqual(
      MockWorker.instances.filter((w) => !w.terminated).length,
      0,
      "All workers terminated after abort",
    );
    console.log(
      "  Workers after abort: " +
        MockWorker.instances.filter((w) => !w.terminated).length +
  ...
  console.log("\n--- Test 4: Empty file list ---");
  {
    ...
    assertEqual(
      ...
      0,
      "No workers spawned for empty file list",
    );
  ...
  console.log("\n--- Test 5: Batch hash message handling ---");
  {
    ...
    assert(
      totalHashesReceived > 0,
      "Should receive file hashes via batch messages",
    );
    assert(
      totalHashesReceived <= 200,
      "Should not receive more hashes than files (got " +
        totalHashesReceived +
        ")",
    );
  ...
  console.log("\n--- Test 6: Progress throttle coalescing ---");
  {
    ...
    assert(
      progressCount < 100,
      "Progress should be heavily throttled (< 100 for 500 files, got " +
        progressCount +
        ")",
    );
  ...
  console.log("\n--- Test 7: Worker count respects hardware concurrency ---");
  {
    ...
    const expectedWorkers = Math.max(
      1,
      Math.min(4, Math.floor(MOCK_CORES / 2)),
    );
    assertEqual(
      MockWorker.instances.length,
      expectedWorkers,
  ...
  console.log("\n--- Test 8: Ignore patterns filter node_modules ---");
  {
    ...
    assertEqual(
      result.totalFiles,
      2,
      "Should only have 2 files after ignore filter (node_modules and .git excluded)",
    );
  ...
  console.log("\n--- Test 10: Stress test with 5000 files ---");
  {
    ...
    assert(
      elapsed < 30000,
      "5000-file scan should complete in < 30s (took " + elapsed + "ms)",
    );
    assertEqual(
      MockWorker.instances.filter((w) => !w.terminated).length,
      0,
      "All workers cleaned up after stress test",
    );
    console.log(
      "  5000 files in " + elapsed + "ms, findings: " + result.issueCount,
    );
  }
  ...
  console.log("\n--- Test 11: Security rule pattern validation ---");
  {
    ...
    const wrappedCode =
      workerCode.replace("self.onmessage", "sandboxSelf.onmessage") +
    ...
    const {
      ...
      SEVERITY_MAP,
      detectFileLanguage,
      getAnalyzersForLanguage,
      runAnalyzer,
    } = factory(sandbox, sandboxSelf);
    ...
    const securityTests = [
      ...
      [
        "azureKey",
        "AccountKey=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=",
        true,
      ],
      ...
      [
        "bearerToken",
        'fetch(url, { headers: { Authorization: "Bearer dGhpcyBpcyBhIHRlc3QgdG9rZW4" } });',
        true,
      ],
      ...
      [
        "jwtHardcoded",
        'const token = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";',
        true,
      ],
      // OAuth — token must have 10+ chars after the prefix
      [
        "oauthTokenInSource",
        'access_token = "ya29.a0ARrdaMabcdefghijklmnopqrstuvwxyz1234"',
        true,
      ],
    ...
    for (const [ruleName, code, shouldDetect] of securityTests) {
      ...
      if (!reg) {
        assert(
          false,
          'Rule "' + ruleName + '" should exist in PATTERN_REGISTRY',
        );
        ruleTestsFailed++;
        continue;
      }
      ...
      assert(
        !!severity,
        'Rule "' + ruleName + '" should have a severity in SEVERITY_MAP',
      );
      ...
      if (ruleName === "pinnedVersionMissing")
    ...
    for (const rule of newRules) {
      assert(
        !!PATTERN_REGISTRY[rule],
        'Rule "' + rule + '" exists in PATTERN_REGISTRY',
      );
      assert(
        !!SEVERITY_MAP[rule],
        'Rule "' + rule + '" has severity in SEVERITY_MAP',
      );
    }
    console.log(
      "  All " + newRules.length + " new rules registered with severity",
    );
  }
  ...
  console.log("\n--- Test 12: Hash cache skip on re-scan ---");
  {
    ...
    assertEqual(
      result1.filesSkippedByHashCache,
      0,
      "First scan: 0 cache hits (no cache yet)",
    );
    console.log(
      "  First scan: " +
        result1.processed +
    ...
    assert(
      result2.filesSkippedByHashCache > 0,
      "Second scan should have cache hits (got " +
        result2.filesSkippedByHashCache +
        ")",
    );
    assert(
      result2.issueCount === 0 || result2.issueCount < result1.issueCount / 100,
      "Cached scan should have fewer findings (skipped analysis)",
    );
    console.log(
      "  Second scan: " +
        result2.processed +
