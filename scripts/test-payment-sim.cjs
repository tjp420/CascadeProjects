...
Module.prototype.require = function (id) {
    ...
    if (id.includes('license-utils')) {
        return { generateLicenseToken: () => 'test-token-123', escapeHtml: (s) => String(s || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') };
    }
    // Stub email service
...
const server = app.listen(0, () => {
    ...
    const testCases = [
        ...
        { tier: 'team_pro',  mode: 'monthly',  expectedCents: 14900,  label: 'Team Pro monthly + negative extraSeats (clamped to 0)',
          extraSeats: -5, expectNoSeatItem: true },
        { tier: 'team_pro',  mode: 'monthly',  expectedCents: 14900,  label: 'Team Pro monthly + string extraSeats "3" (parsed to 3)',
          extraSeats: '3', expectedSeatCents: 1500, expectedSeatQty: 3 },
        { tier: 'team_pro',  mode: 'monthly',  expectedCents: 14900,  label: 'Team Pro monthly + over-max extraSeats 100 (clamped to 50)',
          extraSeats: 100, expectedSeatCents: 1500, expectedSeatQty: 50 },
    ];
    ...
    function runOneTimeTests(idx) {
        ...
        const req = http.request({
            ...
            res.on('end', () => {
                ...
                console.log('\n[' + (pass ? 'PASS' : 'FAIL') + '] ' + tc.label);
                console.log('  Payload: product=' + tc.product + ', scans=[' + tc.scans.join(',') + ']');
                console.log('  Expected: ' + tc.expectedCents + ' cents, one-time (no recurring)');
                console.log('  Actual:   ' + actualCents + ' cents, one-time=' + isOneTime + ', name="' + actualName + '"');
                console.log('  HTTP ' + res.statusCode + ': ' + (responseOk ? 'success=true' : data.substring(0, 200)));
