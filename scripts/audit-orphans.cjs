...
const JSON_PATH =
  process.argv[2] ||
  "j:/Downloads/cascadeprojects-codemap-analysis-2026-06-30.json";
...
function main() {
  ...
  const orphans =
    raw.analysis?.improvements?.find(
  ...
  for (const rel of orphans) {
    ...
    if (lastCommit === null) {
      ...
      recent.push({
        file: rel,
        lastCommit: new Date(lastCommit).toISOString(),
      });
    }
  }
  ...
  console.log(
    `-- Stale (> 6 months) — Candidates for deletion review: ${stale.length} --`,
  );
  stale
    .slice(0, 20)
    .forEach((o) =>
      console.log(`  [STALE] ${o.file}  (last: ${o.lastCommit})`),
    );
  if (stale.length > 20) console.log(`  ... and ${stale.length - 20} more`);
  console.log();

  console.log(
    `-- Recent (< 6 months) — Likely entry points / scripts: ${recent.length} --`,
  );
  recent
    .slice(0, 20)
    .forEach((o) =>
      console.log(`  [RECENT] ${o.file}  (last: ${o.lastCommit})`),
    );
