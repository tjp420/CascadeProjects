...
if (!fs.existsSync(toolsPath)) {
  console.log(
    `${colors.red}[FATAL] tools.js not found at ${toolsPath}${colors.reset}`,
  );
  process.exit(1);
}
...
console.log(
  `${colors.magenta}          MCP TOOL SCHEMA AUDITOR                    ${colors.reset}`,
);
console.log(
  `${colors.cyan}====================================================${colors.reset}\n`,
);
...
console.log(
  `Found ${colors.bold}${TOOL_DEFINITIONS.length}${colors.reset} tool definitions\n`,
);
...
for (const tool of TOOL_DEFINITIONS) {
  ...
  if (
    !desc.includes("no upload") &&
    !desc.includes("No upload") &&
    !desc.includes("local") &&
    !desc.includes("Local") &&
    !desc.includes("DO NOT") &&
    !desc.includes("runs locally")
  ) {
    warnings.push(
      `${toolName}: description lacks offline/local guardrail statement`,
    );
  }
  ...
  for (const [paramName, paramSchema] of Object.entries(props)) {
    ...
    if (
      ["type", "config", "data", "value", "options"].includes(paramName) &&
      paramName !== "value"
    ) {
      warnings.push(
        `${toolName}.${paramName}: generic parameter name "${paramName}" — consider more specific name`,
      );
    }
    ...
    if (
      ...
      KNOWN_ENUMS[paramName] &&
      !paramSchema.enum
    ) {
      issues.push(
        `${toolName}.${paramName}: string type with known values but no enum — should be enum: [${KNOWN_ENUMS[paramName].join(", ")}]`,
      );
    }
    ...
    if (
      paramSchema.type === "string" &&
      !paramSchema.enum &&
      !paramSchema.description?.includes("default")
    ) {
      // Check if description mentions options like "json | markdown"
      const descMatch = paramSchema.description?.match(/(\w+)\s*\|\s*(\w+)/);
      if (descMatch) {
        warnings.push(
          `${toolName}.${paramName}: description mentions pipe-separated options — consider using enum instead`,
        );
      }
    }
    ...
    if (paramName === "projectRoot" && !paramSchema.description) {
      issues.push(
        `${toolName}.${paramName}: projectRoot has no description — agents won't know the default behavior`,
      );
    }
...
// ── Output ──────────────────────────────────────────────────────────────────
console.log(
  `${colors.bold}Issues (must fix):${colors.reset} ${issues.length === 0 ? colors.green + "0" : colors.red + issues.length}${colors.reset}`,
);
console.log(
  `${colors.bold}Warnings (should fix):${colors.reset} ${warnings.length === 0 ? colors.green + "0" : colors.yellow + warnings.length}${colors.reset}\n`,
);
...
for (const tool of TOOL_DEFINITIONS) {
  ...
  const hasGuardrail =
    (tool.description || "").includes("no upload") ||
  ...
  console.log(
    `  ${status}[${guardIcon}]${colors.reset} ${tool.name.padEnd(28)} ${colors.dim}props: ${props.length}, required: ${required.length}, desc: ${descLen}ch${colors.reset}`,
  );
}
...
if (issues.length === 0 && warnings.length === 0) {
  console.log(
    `${colors.green}[SUCCESS] All tool schemas are LLM-optimized.${colors.reset}`,
  );
  process.exit(0);
} else {
  console.log(
    `${colors.yellow}[ACTION NEEDED] ${issues.length} issues, ${warnings.length} warnings to address.${colors.reset}`,
  );
