...
process.env.SIMPLEBEACON_LICENSE_SECRET =
  process.env.SIMPLEBEACON_LICENSE_SECRET ||
  "sim_license_secret_key_enterprise";
  ...
  updateCustomerSubscription: (email, status, tier) => {
    ...
    if (c) {
      ...
      c.tier =
        typeof tier === "string"
          ? tier
          : PRICE_TO_TIER[tier?.monthly] || "community";
    }
  },
  ...
  getDb: () => ({
    prepare: (sql) => ({
      ...
      all: (...args) => {
        ...
        if (
          sql.includes("paid_subscriptions") &&
...
Module.prototype.require = function (id) {
  ...
  if (id.includes("license-utils")) {
    return {
      generateLicenseToken: () => "sim-enterprise-license-token-" + Date.now(),
  ...
  if (id.includes("services/email")) {
    ...
    realEmail.sendEmail = async (opts) => {
      console.log(
        "  [EMAIL] Sending real email to " + opts.to + " via Resend API...",
      );
  ...
  if (id.includes("billing-email-templates")) {
    return {
      renderProrationNotice: () => ({
        subject: "Proration Notice",
        text: "Proration",
        html: "<html>Proration</html>",
      }),
    };
  }
...
if (routerModule.setupSubscriptionWebhook)
  routerModule.setupSubscriptionWebhook(app);
if (checkoutRouterModule.setupCheckoutWebhook)
  checkoutRouterModule.setupCheckoutWebhook(app);
...
const server = app.listen(0, async () => {
  ...
  console.log(
    "  [" +
      (centsOk ? "PASS" : "FAIL") +
      "] Price matches $4,990/yr (499000 cents)",
  );
  console.log(
    "  [" + (intervalOk ? "PASS" : "FAIL") + "] Interval is annual (year)",
  );
  console.log(
    "  [" +
      (responseOk ? "PASS" : "FAIL") +
  ...
  console.log(
    "Step 2b: Simulating Stripe webhook (customer.subscription.updated)...",
  );
  ...
  const step2b = await sendWebhook(
    port,
    subUpdatedEvent,
    "customer.subscription.updated",
  );
  const webhook2Ok =
    step2b.statusCode === 200 && step2b.body.includes('"received"');
  ...
  console.log(
    "  [" +
      (webhook2Ok ? "PASS" : "FAIL") +
      "] customer.subscription.updated processed",
  );
  console.log(
    "  [" +
      (emailSent ? "PASS" : "FAIL") +
      "] License token email sent to customer",
  );
  console.log("");
  ...
  console.log(
    "  Status:         Active (payment_status=paid, subscription.status=active)",
  );
  console.log(
    "  Features:       continuous_shield, team_dashboard, ci_integration,",
  );
  console.log(
    "                  compliance_certificate, eu_ai_act, analyst_support",
  );
  console.log("");
  ...
  if (emailLog.length > 0) {
    ...
    emailLog.forEach((e, i) => {
      console.log(
        "  " + (i + 1) + ". To: " + e.to + " | Subject: " + e.subject,
      );
      const tokenMatch = (e.text || "").match(/License Token: (.+)/);
      if (tokenMatch) console.log("     License Token: " + tokenMatch[1]);
      console.log(
        "     Provider: " +
          (e.result?.provider || "N/A") +
          " | MsgID: " +
          (e.result?.providerMessageId || "N/A"),
      );
      console.log(
        "     Delivered: " +
          (e.result?.sent ? "YES" : "NO (queued: " + e.result?.queued + ")"),
      );
    });
  } else {
  ...
  if (allPassed) {
    ...
    console.log(
      "  1. Stripe checkout session created for $4,990/yr Enterprise annual",
    );
    console.log(
      "  2. checkout.session.completed webhook → customer activated, subscription recorded",
    );
    console.log(
      "  3. customer.subscription.updated webhook → license token generated, email sent",
    );
    console.log(
      "  4. " +
        CUSTOMER_EMAIL +
        " would receive license token + API key via email",
    );
