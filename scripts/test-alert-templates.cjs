...
describe("ALERT_TEMPLATES structure", () => {
  test("has all 22 expected rule IDs", () => {
    ...
    assert.equal(
      ruleIds.length,
      22,
      `Expected 22 templates, got ${ruleIds.length}`,
    );
    for (const expectedId of Object.keys(EXPECTED_RULES)) {
      assert.ok(
        ALERT_TEMPLATES[expectedId],
        `Missing template for ${expectedId}`,
      );
    }
  });
  ...
  test("each template has correct severity", () => {
    for (const [ruleId, expectedSeverity] of Object.entries(EXPECTED_RULES)) {
      assert.equal(
        ALERT_TEMPLATES[ruleId].severity,
        expectedSeverity,
  ...
  test("remediationSteps is a non-empty array for each template", () => {
    for (const [ruleId, template] of Object.entries(ALERT_TEMPLATES)) {
      assert.ok(
        Array.isArray(template.remediationSteps),
        `${ruleId}: remediationSteps must be an array`,
      );
      assert.ok(
        template.remediationSteps.length > 0,
  ...
  test("rotationRequired is boolean for each template", () => {
    for (const [ruleId, template] of Object.entries(ALERT_TEMPLATES)) {
      assert.equal(
        typeof template.rotationRequired,
        "boolean",
  ...
  test("cwe field is present for security rules", () => {
    for (const [ruleId, template] of Object.entries(ALERT_TEMPLATES)) {
      if (ruleId !== "SB-SEC-020") {
        // Health check has no CWE
        assert.ok(template.cwe, `${ruleId}: should have a CWE identifier`);
      }
    }
  });
});
...
describe("enrichFindingsWithAlerts", () => {
  test("enriches an array of findings", () => {
    ...
    assert.ok(
      enriched[0].alertTemplate,
      "First finding should have alert template",
    );
    assert.ok(
      enriched[1].alertTemplate,
      "Second finding should have alert template",
    );
    assert.equal(
      enriched[2].alertTemplate,
      undefined,
      "Third finding should not have alert template",
    );
  });
...
describe("Integration with CLI report findings", () => {
  test("enriches realistic CLI findings with alert templates", () => {
    ...
    assert.ok(
      enriched[0].alertTemplate,
      "GCP finding should have alert template",
    );
    assert.ok(
      enriched[1].alertTemplate,
      "Docker finding should have alert template",
    );
    assert.ok(
      enriched[2].alertTemplate,
      "Unpinned dep finding should have alert template",
    );

    // Verify specific template content
    assert.equal(
      enriched[0].alertTemplate.title,
      "GCP Service Account Key Exposed",
    );
    assert.equal(enriched[0].alertTemplate.rotationRequired, true);
    assert.equal(
      enriched[1].alertTemplate.title,
      "Docker Container Running in Privileged Mode",
    );
    assert.equal(enriched[1].alertTemplate.rotationRequired, false);
    assert.equal(
      enriched[2].alertTemplate.title,
