...
async function checkSourceGuard() {
  ...
  if (findings > 0) {
    fail(
      "Source guard: " +
        findings +
        " files matched suspicious upload/telemetry patterns",
    );
    return;
  }

  success(
    "Source guard: zero suspicious upload/telemetry patterns in CLI source",
  );
}
...
async function checkPackageDependencies() {
  ...
  success(
    "Package dependencies contain no cloud-upload or telemetry libraries",
  );
}
