...
async function createCheckoutSession(params) {
  ...
  for (let i = 0; i < params.lineItems.length; i++) {
    ...
    if (item.recurring) {
      ...
      body.append(
        `line_items[${i}][price_data][recurring][interval_count]`,
        "1",
      );
    }
  ...
  if (!res.ok) {
    throw new Error(
      `Stripe API error: ${json.error?.message || JSON.stringify(json)}`,
    );
  }
  return json;
}
...
async function main() {
  ...
  if (oneTime === "certificate") {
    ...
    mode = "payment";
    successUrl =
      "http://localhost:3000/certificate-upload.html?session_id={CHECKOUT_SESSION_ID}";
  ...
  console.log(
    `Price: $${lineItems[0].unitAmount / 100}${mode === "subscription" ? "/" + (annual ? "yr" : "mo") : " one-time"}`,
  );
  console.log();
