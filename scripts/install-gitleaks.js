...
async function installWindows() {
  ...
  if (hasCmd("winget")) {
    ...
    console.warn(
      "winget invocation failed or package id not found. Falling back to PowerShell downloader.",
    );
  }
...
async function installLinux() {
  ...
  console.warn(
    "No single-package installer implemented for Linux in this helper.",
  );
  console.log("Suggested steps:");
  console.log(
    "1) Download the latest release from https://github.com/gitleaks/gitleaks/releases/latest",
  );
  console.log(
    "2) Extract the binary and place it on your PATH (e.g. /usr/local/bin)",
  );
  process.exit(1);
}
