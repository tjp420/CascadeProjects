...
function recordResult(testName, category, passed, details) {
  ...
  console.log(
    `${status} ${colors.yellow}${category}${colors.reset} -> ${testName}`,
  );
...
if (!fs.existsSync(mcpServerPath)) {
  console.log(
    `${colors.red}[FATAL] MCP server not found at ${mcpServerPath}${colors.reset}`,
  );
  process.exit(1);
}
...
async function runTests() {
  ...
  console.log(
    `${colors.magenta}        SIMPLEBEACON MCP FUZZER & INJECTION HARNESS   ${colors.reset}`,
  );
  console.log(
    `${colors.cyan}====================================================${colors.reset}\n`,
  );
  ...
  try {
    console.log(
      `${colors.cyan}[*] Testing MCP tool discovery and schema validation...${colors.reset}`,
    );
    ...
    await sendMcpMessage(child, {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "fuzzer", version: "1.0" },
      },
    });
    child.stdin.write(
    ...
    if (!listResponse?.result?.tools) {
      ...
      if (missing.length > 0) {
        ...
        recordResult(
          ...
          "Discovery",
          true,
          `All ${ALL_TOOLS.length} tools discovered`,
        );
      }
      ...
      for (const tool of listResponse.result.tools) {
        if (!tool.name)
          schemaErrors.push(`${tool.name || "unknown"}: missing name`);
        if (!tool.description)
          schemaErrors.push(`${tool.name}: missing description`);
        if (!tool.inputSchema)
          schemaErrors.push(`${tool.name}: missing inputSchema`);
  ...
  try {
    ...
    if (response?.result?.isError) {
      recordResult(
        ...
        "Injection",
        true,
        "Server rejects unknown tools with isError",
      );
    } else {
      recordResult(
        "Unknown Tool Rejection",
        "Injection",
        false,
        "Server did not flag unknown tool as error",
      );
    }
  } catch (err) {
  ...
  try {
    ...
    const malformedPayloads = [
      ...
      {
        name: "scan_snippet",
        arguments: { content: "A".repeat(100000), filePath: "test.js" },
      },
      {
        name: "scan_snippet",
        arguments: {
      ...
      try {
        ...
        if (response?.result?.isError || response?.error) {
          ...
          if (text.includes("polluted") || text.includes("__proto__")) {
            injectionFailed++;
            recordResult(
              `Prototype Pollution: ${payload.name}`,
              "Injection",
              false,
              "Prototype pollution detected in response",
            );
          } else {
            injectionPassed++;
          }
    ...
    if (injectionFailed === 0) {
      recordResult(
        "Malformed Argument Injection",
        "Injection",
        true,
        `${injectionPassed}/${malformedPayloads.length} payloads handled safely`,
      );
    } else {
      recordResult(
        "Malformed Argument Injection",
        "Injection",
        false,
        `${injectionFailed}/${malformedPayloads.length} payloads caused issues`,
      );
    }
  } catch (err) {
    recordResult(
  ...
  try {
    ...
    for (const traversal of traversalPaths) {
      ...
      const hasProjectRootError =
        text.includes("project root") || text.includes("within project");
      const hasRealFileContents =
        (text.includes("root:") && text.includes(":/bin")) ||
    ...
    if (traversalBlocked === traversalPaths.length) {
      recordResult(
        ...
        "Injection",
        true,
        `${traversalBlocked}/${traversalPaths.length} traversal attempts blocked`,
      );
    } else {
      recordResult(
        "Path Traversal Protection",
        "Injection",
        false,
        `${traversalPaths.length - traversalBlocked} traversal attempts succeeded`,
      );
    }
  } catch (err) {
  ...
  try {
    ...
    const serverSource = fs.readFileSync(
      path.resolve("packages/simplebeacon-cli/src/mcp/stdio-server.js"),
      "utf8",
    );
    ...
    const networkPatterns = [
      {
        ...
        label: "fetch() to external URL",
      },
      {
        pattern: /axios\.(get|post|put|delete|patch)\s*\(/g,
        label: "axios HTTP call",
      },
    ...
    const hasNetworkGuard =
      combined.includes("networkGuard") ||
      combined.includes("assertOfflineClean");

    if (foundLeaks.length > 0) {
      recordResult(
        "MCP Zero-Upload Isolation",
        "Compliance",
        false,
        `Network calls found: ${foundLeaks.join(", ")}`,
      );
      ...
      recordResult(
        ...
        "Compliance",
        false,
        "No networkGuard detected — offline enforcement missing",
      );
    } else {
      recordResult(
        "MCP Zero-Upload Isolation",
        "Compliance",
        true,
        "MCP layer enforces offline mode via networkGuard",
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (preInitResponse?.error?.code === -32002) {
      ...
      recordResult(
        ...
        "Protocol",
        false,
        "Server allowed request before initialize",
      );
    }
    ...
    if (unknownMethodResponse?.error?.code === -32601) {
      ...
      recordResult(
        ...
        "Protocol",
        false,
        `Expected -32601, got: ${JSON.stringify(unknownMethodResponse?.error)}`,
      );
    }
    ...
    if (pingResponse?.result && !pingResponse.error) {
      ...
      recordResult(
        ...
        "Protocol",
        false,
        "Server did not respond to ping correctly",
      );
    }
  ...
  try {
    ...
    const hugeContent =
      'function foo() { return "' + "A".repeat(500000) + '"; }';
    ...
    const response = await sendMcpMessage(child, {
      ...
      params: {
        name: "scan_snippet",
        arguments: { content: hugeContent, filePath: "huge.js" },
      },
    });
    ...
    if (response?.result && !response?.error) {
      recordResult(
        ...
        "DAST",
        true,
        `Handled 500KB payload in ${elapsed}ms`,
      );
      ...
      recordResult(
        ...
        "DAST",
        true,
        `Server safely rejected 500KB payload in ${elapsed}ms`,
      );
    } else {
      recordResult(
        "Large Payload Resilience",
        "DAST",
        false,
        `Server failed on 500KB payload: ${response?.error?.message || "unknown"}`,
      );
    }
  } catch (err) {
  ...
  console.log(
    `${colors.magenta}              MCP FUZZING COMPLETE                  ${colors.reset}`,
  );
  console.log(
    `${colors.cyan}====================================================${colors.reset}\n`,
  );
  ...
  console.log(
    `Passed: ${colors.green}${passedTests}/${totalTests}${colors.reset}`,
  );
  ...
  if (failedTests.length > 0) {
    ...
    for (const t of failedTests) {
      console.log(
        `  ${colors.red}\u2717${colors.reset} ${t.category} -> ${t.testName}`,
      );
