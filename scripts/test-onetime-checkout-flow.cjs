...
describe('One-time checkout: price consistency', () => {
    for (const [tier, expectedPrice] of Object.entries(FRONTEND_ONE_TIME_PRODUCTS)) {
        test(`frontend price for ${tier} matches backend SCAN_OPTION_MAP`, () => {
            ...
            assert.equal(opt.price * 100, expectedPrice * 100, 'Stripe unit_amount in cents');
        });
    }
});
    ...
    for (const [scanId, expected] of Object.entries(SCAN_OPTION_MAP)) {
        test(`line item for ${scanId} has correct name and unit_amount`, () => {
            const lineItem = {
                price_data: {
                    currency: 'usd',
                    product_data: { name: expected.name, description: 'SimpleBeacon ' + expected.name },
...
describe('One-time checkout: email template rendering', () => {
    const fs = require('fs');
    const templatePath = path.join(REPO_ROOT, 'coming-soon/email-template-universal.html');

    // Skip if template doesn't exist
    const templateExists = fs.existsSync(templatePath);

    for (const [product, config] of Object.entries(WEBHOOK_TIER_MAP)) {
        test(`email template renders for ${product} without throwing`, { skip: !templateExists }, () => {
            let template = fs.readFileSync(templatePath, 'utf8');
            const token = 'test-token-eyJhbGciOiJIUzI1NiJ9.test';
            const certUrl = 'https://simplebeacon.ai/certificate-upload.html?token=test';
            const scans = [product];
            ...
            const rendered = template
                ...
                .replace(/\{\{STEPS_LIST\}\}/g, safe('<li>Click the button above</li>'))
                .replace(/\{\{FEATURES_STYLE\}\}/g, safe('style="display:block;"'))
                .replace(/\{\{FEATURES_LIST\}\}/g, safe(scans.map(s => '<li>' + (SCAN_OPTION_MAP[s]?.name || s) + '</li>').join('')))
                .replace(/\{\{DELIVERY_STYLE\}\}/g, safe('style="display:none;"'))
                .replace(/\{\{DELIVERY_HEADLINE\}\}/g, '')
                .replace(/\{\{DELIVERY_DETAIL\}\}/g, '')
                .replace(/\{\{PRIVACY_TEXT\}\}/g, safe('Your source code never leaves your machine.'))
                .replace(/\{\{SUPPORT_TEXT\}\}/g, safe("Didn't see your token? Check your spam folder."));

            assert.ok(rendered.includes(config.label), 'rendered email should contain product name');
            assert.ok(rendered.includes(token), 'rendered email should contain license token');
...
describe('One-time checkout: production endpoint health', () => {
    test('POST /api/create-checkout-session returns 400 on missing fields (not 404)', async () => {
        const r = await fetch('https://simplebeacon.ai/api/create-checkout-session', {
            method: 'POST',
