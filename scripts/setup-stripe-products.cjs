...
async function createProductAndPrice(opt) {
    ...
    return { productId: product.id, priceId: price.id, scanId: opt.id, price: opt.price };
}

(async () => {
    console.log('Creating Stripe products for SimpleBeacon Custom Plan scans...\n');
    const results = [];
    ...
    for (const opt of SCAN_OPTIONS) {
        try {
            ...
            results.push(r);
            console.log(`${r.scanId.padEnd(14)}  $${String(r.price).padStart(3)}  ${r.priceId}`);
        } catch (err) {
    ...
    for (const r of results) {
        console.log(`STRIPE_PRICE_${r.scanId.replace(/-/g, '_').toUpperCase()}=${r.priceId}`);
    }
