...
class MockJsPDF {
  ...
  rect(x, y, w, h, style) {
    this.rects.push({ x, y, w, h, style, fill: [...this.fillColor] });
    return this;
  }
  roundedRect(x, y, w, h, rx, ry, style) {
    this.rects.push({ x, y, w, h, rx, ry, style, fill: [...this.fillColor] });
    return this;
  }
  circle(x, y, r, style) {
    this.rects.push({
...
test("Clean scan — no findings produces valid PDF", async () => {
  ...
  assert(
    result.reportId.startsWith("SB-RPT-"),
    "Report ID should start with SB-RPT-",
  );
});
...
test("Scan with critical findings — gate fails", async () => {
  const findings = [
    ...
    createMockFinding(
      ...
      "deploy/prod.pem",
      "privateKeyBlock",
      "CRITICAL: Private key block found",
    ),
    createMockFinding(
      "high",
      "src/api/auth.ts",
      "jwtHardcoded",
      "HIGH: Hardcoded JWT token",
    ),
  ];
  ...
  assert(result.blob.size > 0, "Blob should have content");
  assertEqual(
    result.filename.includes("compliance-report"),
    true,
    "Filename contains compliance-report",
  );
});
