...
const AUTH_PATTERN = /<script\s+src=["']\/js-es2018\/auth\.js["'][^>]*>\s*<\/script>/gi;
const SITE_CONFIG_PATTERN = /<script\s+src=["']site-config\.js(\?[^"']*)?["'][^>]*>\s*<\/script>/gi;
const APP_LINKS_PATTERN = /<script\s+src=["']app-links\.js(\?[^"']*)?["'][^>]*>\s*<\/script>/gi;
