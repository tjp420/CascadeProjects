...
(async () => {
  ...
  try {
    ...
    for (const t of runs) {
      ...
      try {
        require("fs").accessSync(t.cwd);
      } catch (e) {
        console.warn("Skipping missing path", t.cwd);
        continue;
      }
      ...
      try {
        ...
        if (
          fs.existsSync(dashboardAssetsSrc) &&
          !fs.existsSync(dashboardAssetsDst)
        ) {
    ...
    try {
      ...
      const summary = {
        generatedAt: new Date().toISOString(),
        totalMs: results.reduce((s, x) => s + (x.durationMs || 0), 0),
        targets: results,
      };
      require("fs").writeFileSync(outFile, JSON.stringify(summary, null, 2));
    } catch (e) {
