...
const SCAN_REPORT =
  "j:\\Downloads\\analyze-c-Users-Trevor-CascadeProjects-2026-07-01(10).json";
const PREV_REPORT =
...
if (fs.existsSync(PREV_REPORT)) {
  ...
  const prevKey = (f) =>
    `${f.id || "unknown"}|${f.filePath || f.file || "unknown"}|${f.line || 0}`;
...
console.log(
  "[analyze-scan] Distribution analysis → .simplebeacon/scan-distribution-analysis.json",
);
console.log(
  "[analyze-scan] Comparison report → .simplebeacon/scan-comparison-report.json",
);
console.log(
  "[analyze-scan] Summary report → .simplebeacon/scan-summary-report.md",
);
console.log("\n=== Findings Summary ===");
