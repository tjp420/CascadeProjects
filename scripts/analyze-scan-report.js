...
const PREV_REPORT = 'ai-platform\\.simplebeacon\\report.json.simplebeacon-backup.2026-07-01T04-38-17-887Z';
...
fs.writeFileSync(path.join(OUTDIR, 'scan-distribution-analysis.json'), JSON.stringify(distribution, null, 2), 'utf8');
...
if (fs.existsSync(PREV_REPORT)) {
    ...
    comparison = {
        ...
        resolvedDetails: resolved.slice(0, 20).map((f) => ({ id: f.id, type: f.type, file: f.filePath || f.file, line: f.line })),
        addedDetails: added.slice(0, 20).map((f) => ({ id: f.id, type: f.type, file: f.filePath || f.file, line: f.line }))
    };

    fs.writeFileSync(path.join(OUTDIR, 'scan-comparison-report.json'), JSON.stringify(comparison, null, 2), 'utf8');
}

// ── 3. Cleaned Markdown Summary ─────────────────────────────

const mediumPlus = findings.filter((f) => (f.severity === 'medium' || f.severity === 'high' || f.severity === 'critical'));
...
${comparison ? `| Previous Scan | ${comparison.previousTotal} findings |
...
| Category | Count |
|----------|-------|
${Object.entries(byType).sort((a, b) => b[1] - a[1]).map(([type, count]) => `| ${type} | ${count} |`).join('\n')}
...
2. **Focus on top affected files:** ${topFiles.slice(0, 3).map(([f]) => path.basename(f)).join(', ')} contain the highest concentration of issues.
