...
(async () => {
  ...
  console.log(
    ...
    "matches:",
    searchResult.tools.map((t) => t.name).join(", "),
  );
  ...
  const inspect = await send({
    ...
    params: {
      ...
      arguments: { tool_name: "scan_snippet" },
    },
  });
  const inspected = JSON.parse(inspect.result.content[0].text);
  console.log(
    "Inspect scan_snippet: props =",
    Object.keys(inspected.inputSchema.properties).join(", "),
  );
  ...
  const all = await send({
    ...
    id: id++,
    method: "tools/call",
    params: { name: "list_all_tools", arguments: {} },
  });
  ...
  const scan = await send({
    ...
    params: {
      name: "scan_snippet",
      arguments: { content: "const x = 1;", filePath: "test.js" },
    },
  });
  console.log(
    "scan_snippet still works:",
    scan.result?.isError ? "ERROR" : "OK",
  );
