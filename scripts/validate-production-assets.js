...
test('SSL certificate is valid and not expiring soon', true, async () => {
    return new Promise((resolve, reject) => {
        const req = https.get({ hostname: CONFIG.domain, port: 443, method: 'HEAD', timeout: CONFIG.timeoutMs }, (res) => {
...
test('Sensitive backend files are not exposed via static routes', true, async () => {
    const blockedPaths = ['/.env', '/server.cjs', '/package.json', '/subscriptions.json', '/.simplebeacon/config.json'];
...
test('Maintenance fallback page is present', false, async () => {
    ...
    const maintenancePath = path.resolve(__dirname, '..', 'coming-soon', 'public', 'maintenance.html');
