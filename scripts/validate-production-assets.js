...
async function httpGet(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = client.get(
      url,
      { timeout: CONFIG.timeoutMs, ...options },
      (res) => {
        let data = "";
        res.on("data", (chunk) => {
          data += chunk;
...
test("API health endpoint returns OK", true, async () => {
  ...
  try {
    ...
    success(
      `Health: ${body.status}` + (body.version ? ` (v${body.version})` : ""),
    );
  } catch (err) {
    ...
    const req = https.get(
      ...
      (res) => {
        ...
        if (daysUntilExpiry < 14) {
          throw new Error(
            `Certificate expires in ${daysUntilExpiry} days (${cert.valid_to})`,
          );
        }
        success(
          `Certificate valid until ${cert.valid_to} (${daysUntilExpiry} days)`,
        );
        resolve();
      },
    );
...
test("npm package is published and installable", true, async () => {
  const res = await httpGet(
    `https://registry.npmjs.org/${CONFIG.npmPackage}/latest`,
  );
  if (res.status !== 200)
    throw new Error(`HTTP ${res.status} from npm registry`);
  const body = JSON.parse(res.body);
...
test("VS Code: extension is published", false, async () => {
  ...
  success(
    `Marketplace listing reachable: ${CONFIG.vscePublisher}.${CONFIG.vsceExtension}`,
  );
});
...
test("GitHub Action is accessible", false, async () => {
  ...
  const res = await httpGet(url);
  if (res.status === 404)
    throw new Error("GitHub Action not found on marketplace");
  if (res.status !== 200) throw new Error(`HTTP ${res.status}`);
...
test("Stripe checkout endpoint responds", false, async () => {
  ...
  const res = await httpGet(
    `${CONFIG.apiBaseUrl}/simplebeacon/billing/checkout`,
    {
...
test(
  "Sensitive backend files are not exposed via static routes",
  true,
  async () => {
    const blockedPaths = [
      "/.env",
...
test("Scan endpoint accepts dry-run request", false, async () => {
  ...
  try {
    ...
    if (err.message.includes("ECONNREFUSED"))
      throw new Error("Scan endpoint unreachable");
    throw err;
  }
});
...
test("Local repo file count is reasonable", false, async () => {
  ...
  if (total < CONFIG.minExpectedFilesInRepo) {
    throw new Error(
      `Only ${total} files found (expected >${CONFIG.minExpectedFilesInRepo})`,
    );
  }
