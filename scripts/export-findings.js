...
const OUTPUT_FILE =
  process.argv[2] ||
  path.join(PROJECT_ROOT, ".simplebeacon", "findings-export.json");
...
function parseFindings(stdout) {
  ...
  if (issuesSection) {
    ...
    const issueRegex =
      /\[(\w+)\]\s+(.+?):\s+(.+?\.[a-z]+:\d+)\s+[—\-]\s+(.+?)(?=\[\w+\]|$)/g; // simplebeacon-ignore redos — parses internal scan output, not untrusted user input
    let m;
...
async function main() {
  ...
  console.log(
    `[export-findings] Saved ${report.findings.length} findings to ${OUTPUT_FILE}`,
  );
