    ...
    constructor() {
        this.replacements = {
            ...
            user_input: {
                ...
                replacement: '// SECURITY WARNING: eval() with user input - use input validation and safe parsers instead of: eval($1)',
...
const result = eval(JSON.stringify(obj));
...
const parsed = eval(req.body.data);
...
const calculated = eval(mathExpression);
...
// Conditional logic
// DANGEROUS
const condition = eval('value > threshold');
...
1. Replace all \`eval(JSON.stringify())\` with \`JSON.parse(JSON.stringify())\`
...
\`\`\`bash
#!/bin/bash
if git diff --cached --name-only | xargs grep -l 'eval('; then
