...
function regexScanFile(filePath) {
  try {
    ...
    for (const p of REGEX_PATTERNS) {
      ...
      if (m && m.length) findings.push({ id: p.id, matches: Array.from(new Set(m)).slice(0,3) });
    }
    return findings;
  } catch (e) { return []; }
}
...
function runGitleaks(files) {
  const attempts = [];
  if (hasCmd('gitleaks')) attempts.push({ cmd: 'gitleaks', args: ['detect', '--source'] });
  ...
  for (const at of attempts) {
    ...
    for (const f of files) {
      ...
      if (r.error && r.error.code === 'ENOENT') { failed = true; break; }
