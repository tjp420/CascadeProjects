...
for (const file of ENTRY_FILES) {
  ...
  for (const pattern of FORBIDDEN_PATTERNS) {
    ...
    if (match) {
      console.error(
        `FAIL: ${file} contains external script reference: ${match[0]}`,
      );
      failures++;
    }
  }
...
if (failures > 0) {
  ...
  console.error(
    "Inline site-config.js, app-links.js, auth.js, and referral-capture.js directly in the HTML instead.",
  );
  process.exit(1);
}

console.log(
  "All entry files are self-contained (no external site-config.js, app-links.js, auth.js, or referral-capture.js references).",
);
process.exit(0);
