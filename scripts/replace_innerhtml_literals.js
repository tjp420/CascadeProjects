...
for (const root of roots) {
  ...
  for (const file of all) {
    ...
    if (s.includes("setSafeHTML(") || s.includes("window.setSafeHTML"))
      continue;
    let newS = s;
    // Quick replacement: empty string assignments -> setSafeHTML(el, '')
    newS = newS.replace(
      /([\w\$\.\)\]\-\_]+)\.innerHTML\s*=\s*''\s*;/g,
      (m, left) => {
    ...
    while (true) {
      ...
      newS =
        ...
        newS
          .slice(leftSelectorStart, dotIdx)
          .replace(leftExprTrim, leftExprTrim) +
