...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/api/enterprise/onboard') {
    try {
      ...
      return jsonResponse(res, 400, { error: 'invalid_json', message: err.message });
    }
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/enterprise/trial') {
    try {
      ...
      const resp = {
        ...
        seatsUsed: body.seatCount || 10,
        seatCount: body.seatCount || 10,
        adminLicenseToken: 'trial_lic_' + Math.random().toString(36).slice(2,10),
  ...
  if (req.method === 'POST' && url.pathname.match(/^\/api\/enterprise\/organizations\/[^/]+\/azure-devops$/)) {
    try {
      const parts = url.pathname.split('/');
      const orgId = parts[4];
      const pipelineYaml = `# Azure pipeline for ${orgId}\ntrigger: none\npool: default\nsteps:\n  - script: echo \"Hello from mock\"`;
      const response = { pipelineYaml, instructions: [ 'Add pipeline to your Azure DevOps project', 'Replace secrets as needed' ] };
