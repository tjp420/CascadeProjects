...
function mockReportWithIssues() {
  ...
  report.rawIssues = [
    ...
    {
      id: "SB-FICTION-008-src/utils.js",
      severity: "low",
      ...
      file: "src/utils.js",
      line: 1,
      description: "Boilerplate comment",
    },
  ];
    ...
    const reports = [
      {
        ...
        issueCount: 5,
        severityCounts: { critical: 1, high: 1, medium: 1, low: 2 },
        gate: { pass: false },
      },
      {
...
describe("CliMetricsWidget integration", () => {
  test("adapter output has all fields required by dashboard widgets", () => {
    ...
    assert.ok(
      typeof adapted.gate.blockingCount === "number",
      "gate.blockingCount",
    );
    assert.ok(adapted.severityCounts, "severityCounts");
    assert.ok(
      typeof adapted.severityCounts.critical === "number",
      "severityCounts.critical",
    );
    assert.ok(
      typeof adapted.severityCounts.high === "number",
      "severityCounts.high",
    );
    assert.ok(
      typeof adapted.severityCounts.medium === "number",
      "severityCounts.medium",
    );
    assert.ok(typeof adapted.qualityScore === "number", "qualityScore");
    assert.ok(
      typeof adapted.ruleScopedFilesAnalyzed === "number",
      "ruleScopedFilesAnalyzed",
    );
    assert.ok(
      typeof adapted.repositoryFilesTotal === "number",
