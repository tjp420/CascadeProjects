...
async function run() {
  ...
  if (!r1.ok) {
    warn(
      `/api/simplebeacon/report returned status ${r1.status} (${r1.error || "no response"}) — endpoint may not be available in local CI dashboard`,
    );
    warningsOnly = true;
  } else {
    if (r1.json && r1.json.success === true) {
      if (
        r1.json.message &&
        r1.json.message.toLowerCase().includes("no report")
      ) {
        warn(
          "/api/simplebeacon/report reachable but serving fallback: no report available",
        );
  ...
  if (!r2.ok) {
    warn(
      `/api/trust/verification returned status ${r2.status} (${r2.error || "no response"}) — endpoint may not be available in local CI dashboard`,
    );
    warningsOnly = true;
  } else {
    const j = r2.json;
    if (j && (j.live || j.success)) {
      ...
      warn(
        "/api/trust/verification returned 200 but payload appears incomplete",
      );
      warningsOnly = true;
    }
  }
  ...
  if (!r3.ok) {
    warn(
      `/api/theme returned status ${r3.status} (${r3.error || "no response"}) — endpoint may not be available in local CI dashboard`,
    );
    warningsOnly = true;
  ...
  if (!r4.ok) {
    if (r4.status && r4.status >= 500) {
      ...
      warn(
        `API proxy target returned non-OK status ${r4.status} — backend not running in CI is expected`,
      );
      warningsOnly = true;
    }
  } else {
  ...
  if (!r5.ok) {
    if (r5.status && r5.status >= 500) {
      ...
      warn(
        `/api/ proxied path returned ${r5.status} (expected path-specific responses)`,
      );
    }
  } else {
