...
const { validateLicenseToken } = require(
  path.join(
    __dirname,
    "..",
    "packages",
...
const tokenNode = d
  .prepare("SELECT * FROM token_nodes ORDER BY rowid DESC LIMIT 1")
  .get();
...
console.log(
  "Token (first 50):",
  String(tokenNode.token_hash || "").substring(0, 50),
);
...
const emails = d
  .prepare("SELECT * FROM email_queue ORDER BY rowid DESC LIMIT 3")
  .all();
console.log("\n=== Recent emails ===");
emails.forEach((e) => {
  ...
  console.log(
    "Body (first 500):",
    String(e.body || e.html_body || "").substring(0, 500),
  );
  console.log("---");
});
