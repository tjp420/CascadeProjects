...
describe("bumpFromCommits", () => {
  ...
  test("detects BREAKING CHANGE as major", () => {
    assert.equal(
      bumpFromCommits(["feat: add thing\n\nBREAKING CHANGE: API removed"]),
      "major",
    );
  });
  ...
  test("returns null for only chore commits", () => {
    assert.equal(
      bumpFromCommits(["chore: update deps", "docs: update readme"]),
      null,
    );
  });
  ...
  test("breaking takes priority over feat", () => {
    assert.equal(
      bumpFromCommits(["feat: add thing", "breaking: change API"]),
      "major",
    );
  });
  ...
  test("feat takes priority over fix", () => {
    assert.equal(
      bumpFromCommits(["fix: fix bug", "feat: add feature"]),
      "minor",
    );
  });
...
describe("scopeFromLabels", () => {
  ...
  test("cli-only takes priority over vscode-only", () => {
    ...
    assert.equal(
      scopeFromLabels(["release:cli-only", "release:vscode-only"]),
      "cli",
    );
  });
});
...
describe("CLI entry point", () => {
  ...
  test("handles scope labels", () => {
    const output = runCli([
      "--labels",
      '["release:patch","release:cli-only"]',
      "--commits",
      "",
    ]);
    const result = JSON.parse(output);
