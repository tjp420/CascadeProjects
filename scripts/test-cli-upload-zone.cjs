...
function validateFile(file) {
    ...
    if (file.size > MAX_FILE_SIZE) {
        return { valid: false, error: `File too large (${file.size} bytes). Maximum size is ${MAX_FILE_SIZE} bytes.` };
    }
    const ext = getFileExtension(file.name);
    if (!ACCEPTED_EXTENSIONS.includes(ext)) {
        return { valid: false, error: `Unsupported file type: ${ext}. Please upload a .json file.` };
    }
    return { valid: true };
}
...
function detectSimpleBeaconReport(raw) {
    ...
    return raw.type?.includes('simplebeacon')
...
function processReport(rawReport) {
    if (!rawReport || typeof rawReport !== 'object') {
        return { error: 'File does not contain a valid JSON object', adapted: null };
    }
    if (!detectSimpleBeaconReport(rawReport)) {
        return {
            error: 'File does not appear to be a SimpleBeacon CLI report. Expected fields: type, reportVersion, gate, severityCounts, or rawIssues.',
            adapted: null
        };
    }
...
function mockCliReport() {
    return {
        ...
        rawIssues: [
            { severity: 'high', type: 'test', pattern: 'SB-SEC-017', filePath: 'Dockerfile', description: 'Privileged mode' },
        ],
    };
}
...
describe('processReport', () => {
    ...
    test('processes report with only rawIssues field', () => {
        const { error, adapted } = processReport({
            rawIssues: [{ severity: 'high', type: 'test', filePath: 'a.js', description: 'test' }]
        });
        assert.equal(error, null);
        assert.ok(adapted);
...
describe('Integration with real CLI report shape', () => {
    ...
    test('processes a report with critical findings', () => {
        const raw = {
            ...
            rawIssues: [
                { severity: 'critical', type: 'gcp', pattern: 'SB-SEC-014', filePath: 'gcp.json', description: 'GCP key' },
                { severity: 'critical', type: 'azure', pattern: 'SB-SEC-015', filePath: 'azure.env', description: 'Azure key' },
            ],
        };
