...
if(!input || !output){
  console.error(JSON.stringify({status:'error', message:'--input and --output required'}));
  process.exit(1);
}
...
const entries = obj.entries || obj.entries === undefined && obj.entries === undefined ? (obj.entries || {}) : obj.entries;
...
function redactObject(o){
  const out = {};
  const structuralKeys = new Set(['id','timestamp','prevHash','hash','orgId','entityId']);
...
if(Array.isArray(obj.entries)){
  ...
  fs.writeFileSync(output, JSON.stringify({entries: redacted}, null, 2), 'utf8');
}else{
  // assume object map
  const map = {};
...
const entriesCount = Array.isArray(obj.entries) ? obj.entries.length : Object.keys(obj.entries||{}).length;
fs.writeFileSync(outSummary, JSON.stringify({input, output, entriesCount, redactedEntries: entriesCount}, null, 2), 'utf8');
