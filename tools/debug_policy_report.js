...
(function(){
  ...
  delete require.cache[require.resolve('../ai-platform/server/lib/audit-policy-store.cjs')];
  ...
  let store = {};
  try { store = JSON.parse(fs.readFileSync(tmpPolicyPath, 'utf8')); } catch {}
  store['test-org'] = policy;
  fs.writeFileSync(tmpPolicyPath, JSON.stringify(store, null, 2));

  // reset cache on policy store
  if (auditPolicyStore && typeof auditPolicyStore._resetCache === 'function') auditPolicyStore._resetCache();

  const report = auditLogger.generateComplianceReport('test-org');
  const orgProfile = report.orgs.find(o => o.orgId === 'test-org');
  console.log('Reported retentionPolicy for test-org:', orgProfile.retentionPolicy);
