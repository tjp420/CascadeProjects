...
try {
  if (process.platform === "win32") {
    try {
      execSync(
        `powershell -Command "Get-NetTCPConnection -LocalPort ${PROXY_PORT} -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force }"`,
        { stdio: "ignore" },
      );
    ...
    try {
      execSync(`lsof -t -i:${PROXY_PORT} | xargs -r kill -9`, {
        stdio: "ignore",
      });
      console.log(`🧹 Cleared legacy bindings on proxy port ${PROXY_PORT}.`);
    } catch (_) {
