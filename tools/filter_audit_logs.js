...
const inputPath =
  argv.input ||
  path.join(process.cwd(), "ai-platform", ".simplebeacon", "audit-log.json");
...
const filtered = entries.filter((e) => {
  ...
  if (actionFilter) {
    if (e.action !== actionFilter) return false;
  }
...
const fileName =
  `filtered-audit-${orgFilter || "any"}${actionFilter ? "-" + actionFilter : ""}-${iso(start)}_to_${iso(end)}.json`.replace(
    /[:]/g,
    "-",
  );
...
fs.writeFileSync(
  summaryPath,
  JSON.stringify(
    { exportPath, totalFiltered: filtered.length, byAction },
    null,
    2,
  ),
  "utf8",
