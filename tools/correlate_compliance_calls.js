...
if (!date) {
  console.error('Usage: node tools/correlate_compliance_calls.js --date=YYYY-MM-DD [--orgId=org]');
  process.exit(1);
}
...
(async function main(){
  try {
      ...
      if (input) {
        ...
        entries = entries.filter(en => en.action === 'compliance_report_generated' && en.timestamp && en.timestamp.startsWith(date));
      } else {
        const start = `${date}T00:00:00.000Z`;
        const end = `${date}T23:59:59.999Z`;
        const gen = audit.query({ orgId, action: 'compliance_report_generated', startDate: start, endDate: end, limit: 1000 });
        entries = gen.entries || [];
      }
    if (entries.length === 0) {
    ...
    for (const e of entries) {
      ...
      const nearbyRes = audit.query({ orgId, startDate: t0, endDate: t1, limit: 200 });
      const nearby = (nearbyRes && nearbyRes.entries) ? nearbyRes.entries : (Array.isArray(nearbyRes) ? nearbyRes : []);
      console.log('---');
      ...
      for (const n of nearby) {
        ...
        console.log(`  ${n.timestamp}  ${n.action}  id=${n.id}${meta.length? '  - ' + meta.join(' | '): ''}`);
      }
    }
  } catch (err) {
