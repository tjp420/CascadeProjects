...
(async function () {
  try {
    ...
    const res = await db.updateLicenseToken(token, {
      revoked_at: new Date().toISOString(),
      revoked_reason: "manual_revoke",
    });
    fs.unlinkSync(p);
    console.log(
      JSON.stringify({ status: "revoked", dbUpdated: !!res, removedFile: p }),
    );
  } catch (err) {
    console.error(
      JSON.stringify({ status: "error", message: err && err.message }),
    );
    process.exit(2);
  }
})();
