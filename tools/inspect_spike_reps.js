...
const csvPath =
  argv.input ||
  ".simplebeacon/filtered-audit-compliance-attestation-2026-08-15_to_2026-08-22-fixed-entries.csv";
const date = argv.date; // YYYY-MM-DD
if (!fs.existsSync(csvPath)) {
  console.error(
    JSON.stringify({ status: "error", message: "csv not found", csvPath }),
  );
  process.exit(1);
}
...
const rows = lines.map((l) => {
  ...
  return {
    id: parts[idIdx],
    timestamp: parts[tsIdx],
    action: parts[actionIdx],
    entityId: parts[entityIdIdx],
    raw: l,
  };
});
const filtered = date
  ? rows.filter((r) => r.timestamp && r.timestamp.startsWith(date))
  : rows;
const counts = {};
