...
console.log(
  `🚀 Initializing Experimental Worker Pool across ${WORKER_COUNT} background threads...`,
);
...
const sampleFiles = collectSampleFilePaths(
  repoRoot,
  Number(process.env.BENCH_LIMIT || 5000),
);

if (!sampleFiles.length) {
  console.error(
    "No sample files found to benchmark. Try running from repository root or increase BENCH_LIMIT.",
  );
  process.exit(1);
}
...
runBenchmark(sampleFiles).then((res) => {
  ...
  console.log(
    `🧠 Memory Overhead: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB`,
  );
});
