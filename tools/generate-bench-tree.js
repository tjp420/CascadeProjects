...
const TARGET = path.resolve(
  argv.target ||
    process.env.BENCH_TARGET ||
    path.join(process.cwd(), "bench-tree"),
);
...
console.log(
  `Generating ${COUNT} files into ${TARGET} (approx ${FILE_SIZE_KB} KB each)`,
);
...
for (let d = 0; d < DIR_COUNT && created < COUNT; d++) {
  ...
  for (let i = 0; i < makeCount; i++) {
    const fileName = path.join(
      dirName,
      `file-${String(created + 1).padStart(6, "0")}.txt`,
    );
    try {
