...
async function listQueueFiles() {
  try {
    const files = readdirSync(EMAIL_QUEUE_DIR).filter((f) => f.startsWith('email_') && f.endsWith('.json'));
    files.sort((a,b) => {
      const aStat = fs.statSync ? fs.statSync(path.join(EMAIL_QUEUE_DIR,a)) : null;
      const bStat = fs.statSync ? fs.statSync(path.join(EMAIL_QUEUE_DIR,b)) : null;
...
async function flush() {
  ...
  try {
    sendEmail = require(path.join(__dirname, '..', 'ai-platform', 'server', 'lib', 'email-service.cjs')).sendEmail;
  } catch (e) {
  ...
  if (!dryRun && !(argv.yes || argv.y)) {
    console.log('This run will perform LIVE sends. Re-run with --yes to proceed, or use --dry-run to preview.');
    return;
  }
  ...
  console.log(`Found ${files.length} queued emails; processing ${toProcess.length}`);
      ...
      try {
        ...
        if (res && res.sent) {
          ...
          try {
            ...
            if (res.queuePath) {
              try { await fs.unlink(res.queuePath); } catch (e) { /* ignore */ console.warn('[flush_email_queue] swallowed error:', e); }
            }
            try { await fs.unlink(sendingPath); } catch (e) { /* ignore */ console.warn('[flush_email_queue] swallowed error:', e); }
...
flush().catch((e) => { console.error('Flush failed:', e && e.stack || e); process.exit(2); });
