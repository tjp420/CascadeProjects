...
if(!input || !output){ console.error(JSON.stringify({status:'error',message:'--input and --output required'})); process.exit(1); }
if(!fs.existsSync(input)){ console.error(JSON.stringify({status:'error',message:'input not found', input})); process.exit(1); }
...
function redactObject(o){
  ...
  for(const k of Object.keys(o)){
    ...
    if(/^(name|firstname|lastname|fullname|displayname|username)$/.test(lower)){
      out[k] = '[REDACTED_NAME]';
      continue;
    }
    const v = o[k];
...
if(Array.isArray(obj.entries)){
  ...
  console.log(JSON.stringify({status:'ok', output, count: Object.keys(map).length}));
}
