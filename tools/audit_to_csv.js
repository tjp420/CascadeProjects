...
const outPrefix =
  argv.output || argv.o || ".simplebeacon/filtered-audit-summary";
...
const entriesArr = Array.isArray(parsed.entries)
  ? parsed.entries
...
for (const e of entriesArr) {
  ...
  const row = [id, ts, action, entity, entityId, actorId, actorEmail]
    .map((v) => {
...
console.log(
  JSON.stringify({
    status: "ok",
    entriesCsv: entriesCsvPath,
    aggCsv: aggCsvPath,
    total: entriesArr.length,
  }),
);
