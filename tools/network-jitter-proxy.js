...
console.log(
  `🌀 Starting Network Jitter Proxy on port ${PROXY_PORT} -> Target API: ${TARGET_PORT}`,
);
console.log(
  `⏱️  Config: Base Latency = ${BASE_LATENCY}ms, Jitter = ±${JITTER_RANGE}ms, Fault Injection = ${ERROR_RATE * 100}%`,
);
...
http
  .createServer((req, res) => {
    ...
    if (Math.random() < ERROR_RATE) {
      console.warn(
        `💥 [Proxy] Injecting synthetic 429 Rate Limit Fault for: ${req.url}`,
      );
      res.writeHead(429, {
      ...
      res.end(
        JSON.stringify({
          ...
          message: "Synthetic jitter fault injected.",
        }),
      );
      return;
    }
  ...
  .listen(PROXY_PORT, () => {
    console.log(
      `✅ Network Jitter Proxy listening on http://127.0.0.1:${PROXY_PORT}`,
    );
  });
