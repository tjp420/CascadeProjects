...
console.log("⚡ Initializing Sprint 5 Zero-Copy Bit-Packing Performance Validator...");
...
function packFinding(severity, category, lineNumber) {
  return (severity & 0xF) | ((category & 0xFF) << 4) | ((lineNumber & 0xFFFFF) << 12);
}
...
console.log(`🧠 Compressing and extracting ${ITERATIONS.toLocaleString()} data vectors...`);
...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  if (unpacked.severity !== severity || unpacked.category !== category || unpacked.lineNumber !== line) {
    console.error('Pack/unpack mismatch at', i, { severity, category, line, unpacked });
    process.exit(2);
  }
}
...
console.log(`📦 Operations per Millisecond: ${(ITERATIONS / durationMs).toFixed(2)} ops/ms`);
console.log('🧠 Memory footprint per node: 4 bytes (Fixed Int32) vs ~120 bytes (Standard JS object)');
// log accumulator to avoid optimizer dropping loop
console.log('Accumulator (to avoid optimization):', accumulator.toString().slice(-12));
