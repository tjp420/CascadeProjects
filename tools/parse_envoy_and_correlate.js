...
if (!envoyLog || !input) {
  console.error(
    "Usage: node tools/parse_envoy_and_correlate.js --envoy-log=LOGFILE --input=AUDIT_JSON [--window=60] [--date=YYYY-MM-DD]",
  );
  process.exit(1);
}
...
async function loadReps() {
  ...
  if (dateFilter) {
    return reps.filter(
      (r) => r.timestamp && r.timestamp.startsWith(dateFilter),
    );
  }
  return reps;
}
...
(async function main() {
  ...
  console.error(
    `Loaded ${normalizedReps.length} rep entries. Window: ±${windowSec}s`,
  );
  ...
  if (envoyLog === "-") {
    ...
    if (!fs.existsSync(envoyLog)) {
      ...
      console.error(
        " - Provide the correct path to the Envoy access log (JSON-per-line).",
      );
      console.error(" - Or stream logs into stdin and pass --envoy-log=-");
      console.error(
        'Example (docker): docker logs --since "2026-08-15T00:00:00" --until "2026-08-22T23:59:59" <container> 2>&1 | node tools/parse_envoy_and_correlate.js --envoy-log=- --input=...',
      );
      process.exit(3);
    }
    rl = readline.createInterface({
      input: fs.createReadStream(envoyLog),
      crlfDelay: Infinity,
    });
  }
  ...
  for await (const line of rl) {
    ...
    const path = tryExtractField(obj, [
      "path",
      "request_path",
      "request.path",
      "req.path",
    ...
    const tsRaw = tryExtractField(obj, [
      "timestamp",
      "start_time",
      "time",
      "@timestamp",
    ...
    const ts =
      parseTime(tsRaw) ||
      parseTime(tryExtractField(obj, ["start_time_local", "time_local"]));
    const request_id = tryExtractField(obj, [
      "request_id",
      "req_id",
      "request_id.uuid",
      "request_id_value",
    ]);
    const upstream = tryExtractField(obj, [
      "upstream_host",
      "upstream",
      "upstream_host",
    ]);
    const remote = tryExtractField(obj, [
    ...
    const linked = normalizedReps.filter((r) => {
      const d = Math.abs(
        new Date(r.timestamp).getTime() - new Date(ts).getTime(),
      );
  ...
  if (matches.length === 0) {
    console.log(
      "No Envoy access log entries for /api/audit/compliance/report found in",
      envoyLog,
    );
  } else {
    console.log(
      `Found ${matches.length} access-log hits for /api/audit/compliance/report:`,
    );
    for (const m of matches) {
      const e = m.entry;
      console.log("---");
      console.log(
        `time: ${e.ts || "(unknown)"}  request_id: ${e.request_id || "(none)"}  remote: ${e.remote || "(unknown)"}  ua: ${e.ua || "(unknown)"}  upstream: ${e.upstream || "(unknown)"}  line:${e.line}`,
      );
      if (m.linked.length === 0) {
        ...
        for (const r of m.linked) {
          console.log(
            `  -> matches rep ${r.rep} (audit id=${r.id}) @ ${r.timestamp}`,
          );
        }
      }
    }
  ...
  console.log(
    `Matched via envoy logs: ${normalizedReps.length - unmatched.length}`,
  );
  ...
  if (unmatched.length > 0) {
    ...
    unmatched
      .slice(0, 20)
      .forEach((u) => console.log(` - ${u.rep} @ ${u.timestamp} id=${u.id}`));
  }
})();
