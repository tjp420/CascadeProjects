...
process.env.STRIPE_SECRET_KEY = process.env.STRIPE_SECRET_KEY || 'sk_test_simulated_local';
// Provide a valid mapping for team_pro_monthly from ai-platform/server/config/stripe.cjs
process.env.STRIPE_PRICE_ID_TEAM_PRO_MONTHLY = process.env.STRIPE_PRICE_ID_TEAM_PRO_MONTHLY || 'price_1U2fn7AQ0e20kzI8lXYh295F';
process.env.STRIPE_PRICE_ID_TEAM_PRO_ANNUAL = process.env.STRIPE_PRICE_ID_TEAM_PRO_ANNUAL || 'price_1U2fnYAQ0e20kzI8EI2LjRQC';
process.env.SIMPLEBEACON_APP_URL = process.env.SIMPLEBEACON_APP_URL || 'http://localhost:54355';

const args = process.argv.slice(2);
const product = (args[0] || 'team_pro_monthly').trim();
const email = (args[1] || 'you@example.com').trim();

const { resolvePriceId, checkoutModeForProduct, getAppBaseUrl } = require('../ai-platform/src/api/billing/license-utils.cjs');

console.log('Simulating checkout for product:', product, 'email:', email);

const priceId = resolvePriceId(product);
const stripeClientAvailable = !!require('../ai-platform/src/api/billing/license-utils.cjs').getStripeClient();
...
const successPath = ['/dashboard/settings?checkout=success&session_id={CHECKOUT_SESSION_ID}', '/certificate-upload.html?session_id={CHECKOUT_SESSION_ID}'];
...
console.log('If `priceId` is null or `Stripe client available` is NO, set the corresponding env vars in your .env and restart the server.');
