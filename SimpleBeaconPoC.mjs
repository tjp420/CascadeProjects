...
const minimatch = (_minimatch_mod && typeof _minimatch_mod === 'function') ? _minimatch_mod : (_minimatch_mod && _minimatch_mod.minimatch) ? _minimatch_mod.minimatch : null;
...
class ScanIgnoreMatcher {
    ...
    addIgnoreFile(filePath) {
        ...
        content.split(/\r?\n/).forEach(line => {
            ...
            const formattedPattern = pattern.startsWith('/') ? pattern.slice(1) : `**/${pattern}`;
...
class EuAiActEvaluator {
        ...
        this.schemaRules = [
            {
                ...
                description: 'Verifies if an AI pipeline code component lacks transparency logging metadata blocks.',
                severity: 'HIGH',
                confidence: 'HIGH',
                autoFixable: true,
                suggestedPatch: '// TODO: Provide User Notice - Content generated by AI system under EU AI Act Art 52.\n'
            }
        ];
        this.findings = [];
    }
    ...
    evaluateFileAST(filePath, ast, sourceCode) {
        ...
        if (sourceCode.includes('openai') || sourceCode.includes('llmPipeline')) {
            if (!sourceCode.includes('Article 52') && !sourceCode.includes('AI-transparency')) {
                ...
                this.findings.push({
                    ...
                    filePath: filePath,
                    detectedPattern: 'Raw AI module imports found without statutory transparency headers.',
...
export async function runSimpleBeaconAudit(targetWorkspace) {
    ...
    ignoreMatcher.addIgnoreFile(path.join(targetWorkspace, '.simplebeaconignore'));
