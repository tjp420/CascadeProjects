...
```javascript
// Before (broken)
const command = button.getAttribute('onclick').match(/vscode\.postMessage\({command:'([^']+)'/);
```
...
```html
<!-- After (fixed) -->
<button data-command="scanWorkspace"><span>&#128256;</span> Scan Workspace</button>
```
...
this.panel.webview.onDidReceiveMessage((msg) => {
  ...
  if (msg.command === 'scanWorkspace') {
    ...
    console.log('[SimpleBeacon Dashboard] Executing generateCertificate command');
