...
function log(status, msg, detail) {
    ...
    const reset = '\x1b[0m';
    console.log(`${colors[status]}${icons[status]}${reset} ${msg}${detail ? ' — ' + detail : ''}`);
...
function fetch(url, options = {}) {
    return new Promise((resolve, reject) => {
        const req = https.get(url, { timeout: 15000, ...options, headers: { 'User-Agent': 'simplebeacon-prelaunch-check', ...options.headers } }, (res) => {
            let body = '';
            res.on('data', (chunk) => body += chunk);
            res.on('end', () => resolve({ status: res.statusCode, ok: res.statusCode >= 200 && res.statusCode < 400, headers: res.headers, body }));
        });
        req.on('error', reject);
...
async function checkGitHubRepo() {
    console.log('\n── GitHub Repository ──');
    try {
        const r = await fetch('https://api.github.com/repos/tjp420/CascadeProjects');
        if (r.status === 404) {
            log('FAIL', 'GitHub repo is PRIVATE or does not exist', 'github.com/tjp420/CascadeProjects returns 404 — make it public before launching');
            return;
        }
        if (!r.ok) {
        ...
        if (j.private) {
            log('FAIL', 'GitHub repo is PRIVATE', 'Set to public in repo settings before launching');
        } else {
            log('PASS', 'GitHub repo is PUBLIC', `${j.full_name} | ${j.stargazers_count} stars`);
        }
        if (!j.description) {
...
async function checkNpmPackage() {
    ...
    try {
        ...
        if (!r.ok) {
            log('FAIL', 'npm package "simplebeacon" not found', 'Publish to npm before launching');
            return;
        }
        const j = JSON.parse(r.body);
        const latest = j['dist-tags']?.latest;
        if (!latest) {
            log('FAIL', 'No latest version on npm', 'Publish a version before launching');
            return;
        }
        ...
        if (latestVersion?.bin?.simplebeacon) {
            ...
            log('FAIL', 'No "simplebeacon" bin in latest version', 'npx simplebeacon will fail');
        }
        ...
        try {
            ...
            if (page.checkBody) {
                ...
                if (body.includes('52 engine') || body.includes('52 heuristic')) {
                    log('FAIL', `${page.name} contains "52 engine"`, 'Stale claim still live');
                }
                if (body.includes('38 analy')) {
                    log('FAIL', `${page.name} contains "38 analy"`, 'Stale claim still live');
                }
                ...
                if (body.includes('14,000 files')) {
                    log('FAIL', `${page.name} contains "14,000 files"`, 'Stale claim still live');
                }
...
async function checkApiEndpoints() {
    ...
    const endpoints = [
        ...
        { url: 'https://simplebeacon.ai/api/simplebeacon/billing/status?email=test@example.com', name: 'Billing status' },
    ];
...
async function checkStripeCheckout() {
    ...
    const endpoints = [
        { url: 'https://simplebeacon.ai/api/simplebeacon/billing/status?email=test@example.com', name: 'Billing status API' },
        { url: 'https://simplebeacon.ai/api/health/email', name: 'Email health API' },
    ];
    for (const ep of endpoints) {
        try {
            const r = await fetch(ep.url);
    ...
    try {
        ...
        const body = r.body;
        if (body.includes('checkout') || body.includes('subscribe') || body.includes('stripe')) {
    ...
    try {
        ...
        for (const p of vsixPaths) {
            ...
            if (fs.existsSync(full)) {
                ...
                log('PASS', `VSIX exists: ${p}`, `${(stat.size / 1024 / 1024).toFixed(1)} MB`);
                found = true;
                break;
            }
        }
        if (!found) {
            log('WARN', 'No VSIX file found locally', 'Check sales/marketplace/ for the extension package');
        }
        ...
        log('INFO', 'Verify marketplace listing', 'Open https://marketplace.visualstudio.com/search?term=simplebeacon in browser');
    } catch (e) {
...
async function checkCliLocally() {
    ...
    try {
        const output = execSync('npx simplebeacon scan --gate --offline --version 2>&1 || npx simplebeacon --version 2>&1', { encoding: 'utf8', timeout: 30000 });
    ...
    try {
        const output = execSync('npx simplebeacon scan --gate --offline 2>&1', { encoding: 'utf8', timeout: 120000 });
        if (output.includes('52 deterministic') || output.includes('52 engine')) {
            log('FAIL', 'CLI output contains "52 deterministic engines"', 'Stale claim in CLI banner');
        } else if (output.includes('48 analy')) {
            log('PASS', 'CLI banner shows "48 analyzers"');
        }
        if (output.includes('Quality score: 0/100')) {
            log('WARN', 'CLI shows "Quality score: 0/100"', 'Should show "(upgrade to view)" on free tier');
        }
    ...
    for (const { pattern, label } of patterns) {
        try {
            ...
            if (files.length > 0) {
                ...
                const realFiles = files.filter(f => !f.includes('node_modules') && !f.includes('pages-publish') && !f.includes('public/app'));
                if (realFiles.length > 0) {
                    log('FAIL', `Stale claim ${label} found in source`, `${realFiles.length} files: ${realFiles.slice(0, 3).join(', ')}${realFiles.length > 3 ? '...' : ''}`);
                } else {
                    log('INFO', `Stale claim ${label} only in build artifacts`, 'Will be regenerated');
                }
            } else {
                log('PASS', `No stale ${label} in source`);
            }
        } catch (e) {
            log('INFO', `Stale claim ${label} check skipped`, e.message?.slice(0, 80));
        }
    }
}
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║  SimpleBeacon Pre-Launch Verification                        ║');
    console.log('║  Run before posting to Hacker News or Product Hunt          ║');
    console.log('╚══════════════════════════════════════════════════════════════╝');
