    ...
    emails: [
      {
        ...
        dayOffset: 0,
        subject:
          "AI-generated code audit trail — can you produce one in 24 hours?",
      ...
      },
      {
        ...
        dayOffset: 10,
        subject:
          "How a fintech compliance team cut audit prep from 6 weeks to 2 days",
    ...
    emails: [
      {
        ...
        dayOffset: 0,
        subject:
          "Quantifying AI-generated code risk in your production systems",
...
const TEMPLATE_VARIABLES = {
  ...
  "{{calendly_url}}": () =>
    process.env.CALENDLY_URL || "https://calendly.com/simplebeacon/30min",
};
...
function getNextEmail(prospect, prospectState) {
  const sequenceKey =
    prospectState.sequence || getSequenceForPersona(prospect.persona);
  ...
  if (currentStep >= sequence.emails.length) {
    ...
    if (reactivationStep < SEQUENCES.REACTIVATION.emails.length) {
      ...
      const lastEmailDate =
        prospectState.lastEmailDate || prospectState.sequenceCompleteDate;
      if (!lastEmailDate) return null;
      const daysSinceLast = Math.floor(
        (Date.now() - new Date(lastEmailDate).getTime()) /
          (24 * 60 * 60 * 1000),
      );
...
function processProspect(prospect, campaignState, options = {}) {
  ...
  if (prospectState.replied || prospectState.closed) {
    return {
      action: "skip",
      reason: "already_replied_or_closed",
      prospectState,
    };
  }
...
async function sendEmail(emailPayload) {
  ...
  try {
    ...
    console.log(
      "[outreach-pipeline] nodemailer not installed — email would be sent:",
    );
...
function getCampaignAnalytics(campaignState) {
  ...
  const meetingRate =
    contacted > 0 ? ((meetings / contacted) * 100).toFixed(1) : "0.0";
  const pilotRate =
    meetings > 0 ? ((pilots / meetings) * 100).toFixed(1) : "0.0";
...
async function main() {
  ...
  if (args.status) {
    ...
    console.log(
      `Replied: ${analytics.replied} (${analytics.replyRate} reply rate)`,
    );
    console.log(
      `Meetings: ${analytics.meetings} (${analytics.meetingRate} meeting rate)`,
    );
    console.log(
      `Pilots: ${analytics.pilots} (${analytics.pilotRate} pilot conversion)`,
    );
    console.log(
      `Closed: ${analytics.closed} (${analytics.closeRate} close rate)`,
    );
    console.log(
      `Total contract value: $${analytics.totalContractValue.toLocaleString()}`,
    );
    console.log("\nBy sequence:");
...
`);
  ...
  for (const prospect of prospects) {
    ...
    const result = processProspect(prospect, campaignState, {
      dryRun: !shouldSend,
    });
    ...
    if (result.action === "send" || result.action === "dry-run") {
      if (!shouldSend) {
        ...
        try {
          ...
          console.error(
            `[error] Failed to send to ${prospect.email}: ${err.message}`,
          );
        }
      }
