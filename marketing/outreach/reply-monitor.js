...
const INTERVAL =
  (INTERVAL_ARG !== -1 ? parseInt(process.argv[INTERVAL_ARG + 1], 10) : null) ||
...
function loadProspectEmails() {
  try {
    ...
    const arr = Array.isArray(prospects)
      ? prospects
      : prospects.prospects || prospects.leads || [];
    return new Set(
      arr.map((p) => (p.email || "").toLowerCase()).filter(Boolean),
    );
  } catch {
    return new Set();
  }
...
async function checkInbox() {
  ...
  if (!host || !user || !pass) {
    console.error(
      "[reply-monitor] Missing IMAP env vars (IMAP_HOST, IMAP_USER, IMAP_PASS)",
    );
    return [];
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const sinceHours = parseInt(
          process.env.REPLY_MONITOR_SINCE || "24h".replace("h", ""),
          10,
        );
    ...
    if (err.code === "MODULE_NOT_FOUND") {
      console.error(
        "[reply-monitor] imapflow not installed. Install with: npm install -g imapflow",
      );
      console.error(
        "[reply-monitor] Falling back to manual mode — check your inbox manually.",
      );
      return [];
    }
    throw err;
...
function sendWebhook(message) {
  ...
  if (!webhookUrl) {
    console.log(
      "[reply-monitor] No WEBHOOK_URL set — printing notification only:",
    );
    console.log(message);
    return;
  }
      ...
      (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            console.error(
              `[reply-monitor] Webhook failed: ${res.statusCode} ${body}`,
            );
          }
          resolve();
        });
...
function classifyReply(subject, from, prospectEmails) {
  ...
  const isReply =
    subj.startsWith("re:") || subj.includes("reply") || isProspect;
  ...
  if (
    subj.match(/demo|call|meeting|schedule|book/i) ||
    subj.match(/interested|yes|sure|let.s do it/i)
  ) {
    intent = "demo-request";
    priority = "high";
...
async function checkOnce() {
  ...
  console.log(
    `[reply-monitor] Checking inbox... (${new Date().toISOString()})`,
  );
  ...
  const newMessages = messages.filter(
    (m) => !state.seenMessageIds.includes(m.messageId),
  );
  ...
  for (const msg of newMessages) {
    ...
    console.log(
      `[reply-monitor] Reply detected: ${msg.from} — ${msg.subject} [${classification.intent}]`,
    );
...
async function main() {
  ...
  console.log(
    "║  SimpleBeacon Reply Monitor                                  ║",
  );
  console.log(
    `║  Mode: ${WATCH ? "WATCH (polling every " + INTERVAL + "s)" : "ONE-TIME CHECK"}${" ".repeat(Math.max(0, 28 - (WATCH ? "WATCH (polling every " + INTERVAL + "s)" : "ONE-TIME CHECK").length))}║`,
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════╝",
  );
  ...
  if (!process.env.WEBHOOK_URL) {
    console.log(
      "[reply-monitor] No WEBHOOK_URL set — notifications will be printed only.",
    );
    console.log(
      "[reply-monitor] Set WEBHOOK_URL to enable Slack/Discord alerts.",
    );
  }
