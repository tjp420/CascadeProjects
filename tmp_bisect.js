...
(function(){
  ...
  function showNodeDetails(n) {
    ...
    if (openBtn) openBtn.addEventListener('click', () => {
      if (typeof acquireVsCodeApi !== 'undefined') {
        ...
        const root = document.querySelector('.subtitle')?.textContent || '';
        const fullPath = root ? root.split('\').join('/') + '/' + n.id : n.id;
        try { window.open('vscode://file/' + encodeURIComponent(fullPath), '_blank'); } catch (e) {}
      }
    });
  }
  ...
  function scrollTreeToNode(path) {
    ...
    for (const tn of treeNodes) {
      const nameEl = tn.querySelector('.node-name');
      if (!nameEl) continue;
      const nodePath = nameEl.getAttribute('title') || nameEl.textContent;
