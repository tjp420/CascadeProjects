...
function _resolveRoot(options) {
  return options && options.projectRoot
    ...
    execFile(
      ...
      (err, _stdout, stderr) => {
        if (err) {
          ...
          resolve({
            ok: true,
            message: `Syntax check passed for ${relativeFilePath}`,
          });
        }
      },
    );
...
function computeDiff(relativeFilePath, targetText, replacementText, options) {
  ...
  if (!fs.existsSync(fullPath)) {
    throw new Error(
      `[AI Safety] Target path does not exist on disk: ${relativeFilePath}`,
    );
  }
  ...
  if (Number.isFinite(count)) {
    ...
    while (idx !== -1 && occurrences < count) {
      after =
        after.slice(0, idx) +
        replacementText +
        after.slice(idx + targetText.length);
      occurrences += 1;
 ...
 * @returns {Promise<{ok:boolean, message?:string, error?:string, stderr?:string}>}
 */
async function proposeInlineFixAsync(
  relativeFilePath,
  targetText,
  replacementText,
  options,
...
function proposeBatchFix(relativeFilePath, replacements, options) {
  ...
  for (const { target, replacement } of replacements) {
    if (typeof target !== "string" || typeof replacement !== "string") {
      return {
        ok: false,
        error: "Each replacement must have {target, replacement} strings.",
      };
    }
    if (!updated.includes(target)) {
      return {
        ok: false,
        error: `Batch fix failed: target "${target}" not found.`,
      };
    }
...
function createBackup(relativeFilePath, options) {
  ...
  if (!fs.existsSync(fullPath)) {
    throw new Error(
      `[AI Safety] Cannot back up non-existent file: ${relativeFilePath}`,
    );
  }
