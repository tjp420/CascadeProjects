...
describe("ai-tools safety utilities", () => {
  ...
  describe("isBinaryFile", () => {
    it("returns false for text files", () => {
      ...
      assert.strictEqual(
        isBinaryFile(path.join(originalCwd, "index.js")),
        false,
      );
    });
  ...
  describe("proposeInlineFix", () => {
    ...
    it("fails gracefully when target text is not found", () => {
      ...
      try {
        ...
        assert.strictEqual(result.ok, false);
        assert.ok(
          result.error.includes("Target string to replace was not found"),
        );
      } finally {
        fs.unlinkSync(tmpFile);
      }
    ...
    it("respects replaceCount option", () => {
      ...
      try {
        const result = proposeInlineFix(tmpFile, "aaa", "bbb", {
          replaceCount: 2,
        });
        assert.strictEqual(result.ok, true);
        const content = fs.readFileSync(tmpFile, "utf8");
  ...
  describe("proposeInlineFixAsync", () => {
    it("applies a valid replacement async", async () => {
      ...
      fs.writeFileSync(tmpFile, "const x = 1;");
      try {
        const result = await proposeInlineFixAsync(
          tmpFile,
          "const x = 1",
          "const x = 2",
        );
